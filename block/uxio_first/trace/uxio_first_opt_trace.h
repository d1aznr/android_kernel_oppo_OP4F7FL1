...
TRACE_EVENT(
	block_uxio_first_peek_req,
	TP_PROTO(struct task_struct *task, long req_addr, char *group,
		 unsigned int ux, unsigned int fg, unsigned int bg),

	TP_ARGS(task, req_addr, group, ux, fg, bg),

	TP_STRUCT__entry(__array(char, comm, TASK_COMM_LEN) __field(pid_t, pid)
				 __field(long, req_addr) __array(char, group, 3)
					 __field(unsigned int,
						 ux) __field(unsigned int, fg)
						 __field(unsigned int, bg)),

	TP_fast_assign(memcpy(__entry->comm, task->comm, TASK_COMM_LEN);
		       __entry->pid = task->pid; __entry->req_addr = req_addr;
		       memcpy(__entry->group, group, 3); __entry->ux = ux;
		       __entry->fg = fg; __entry->bg = bg;),
