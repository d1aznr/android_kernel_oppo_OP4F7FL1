...
static inline bool should_get_req_from_ux_or_fg(struct request_queue *q)
{
	...
	if (((q->in_flight[BLK_RW_UX] + q->in_flight[BLK_RW_FG]) <
	     uxfg_max_depth) ||
...
{
	int ux_max_depth =
		q->queue_tags->max_depth - BLK_MIN_BG_DEPTH - BLK_MIN_FG_DEPTH;
...
{
	int fg_max_depth =
		q->queue_tags->max_depth - BLK_MIN_BG_DEPTH - BLK_MIN_UX_DEPTH;
...
static struct request *get_request_from_queue(struct request_queue *q,
...
static bool blk_queue_depth_check(struct request_queue *q)
{
	...
	if (bqt && bqt->bg_max_depth > 0 && bqt->max_depth >= BLK_MIN_DEPTH_ON)
		return true;
...
static struct request *get_req_by_inflight(struct request_queue *q)
{
	...
	if (should_get_req_from_ux_or_fg(q)) {
		if (should_get_req_from_ux(q)) {
			...
			if (rq)
				trace_block_uxio_first_peek_req(
					current, (long)rq, "UX\0",
...
void queue_throtl_add_request(struct request_queue *q, struct request *rq,
			      bool front)
{
...
void ohm_ioqueue_add_inflight(struct request_queue *q, struct request *rq)
{
	if (rq->cmd_flags & REQ_UX)
		q->in_flight[BLK_RW_UX]++;
	else if (rq->cmd_flags & REQ_FG)
...
void ohm_ioqueue_dec_inflight(struct request_queue *q, struct request *rq)
{
	if (rq->cmd_flags & REQ_UX)
		q->in_flight[BLK_RW_UX]--;
	else if (rq->cmd_flags & REQ_FG)
