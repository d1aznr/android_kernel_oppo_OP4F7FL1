...
extern void __wlan_subsystem_send_uevent(struct device *dev, char *reason,
					 const char *name);
extern void wlan_subsystem_send_uevent(struct subsys_device *dev, char *reason,
				       const char *name);
...
static inline void subsystem_send_uevent(struct subsys_device *dev,
					 char *reason)
{
	return;
}
