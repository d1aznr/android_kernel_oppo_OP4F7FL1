...
static inline void dump_mm_info(unsigned long len, unsigned long flags,
				int dump_vma)
{
}
#endif /* CONFIG_DUMP_MM_INFO */

#define GET_UNMMAPED_AREA_FIRST_TIME(info)                                     \
	do {                                                                   \
		if (mm->va_feature & RESERVE_AREA)                             \
			info.low_limit =                                       \
				max_t(unsigned long,                           \
				      GET_UNMMAPED_AREA_FIRST_LOW_LIMIT,       \
				      info.low_limit);                         \
	} while (0);

#define GET_UNMMAPED_AREA_THIRD_TIME(info, mm, addr)                           \
	do {                                                                   \
		if ((mm->va_feature & RESERVE_AREA) && offset_in_page(addr))   \
			addr = get_unmmaped_area_from_reserved(mm, &info);     \
	} while (0);

#define GET_UNMMPAED_AREA_FROME_ANTI_FRAGMENT(info, mm)                        \
	do {                                                                   \
		if ((mm->va_feature & ANTI_FRAGMENT_AREA) &&                   \
		    (info)->high_limit == mm->mmap_base) {                     \
			unsigned long addr =                                   \
				get_unmmaped_area_from_anti_fragment(mm,       \
								     info);    \
