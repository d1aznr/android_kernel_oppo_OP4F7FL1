...
/* TODO should handle this part after RATP phased in */
	...
	if (is_ratp_enable() && is_allowmost_enable()) {
		return task->im_flag &
		       (IM_MAIN | IM_ENQUEUE | IM_SURFACEFLINGER | IM_GL |
			IM_VK | IM_RENDER | IM_HWC | IM_HWBINDER | IM_BINDER |
			IM_BMT | IM_CRENDER);
	}
#endif

	return task->im_flag &
	       (IM_MAIN | IM_ENQUEUE | IM_SURFACEFLINGER | IM_GL | IM_VK |
		IM_HWC | IM_RENDER | IM_BMT | IM_CRENDER);
}
