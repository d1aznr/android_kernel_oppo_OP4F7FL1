...
frame_info_fbg_load(struct frame_info *frame_info)
{
...
void set_frame_rate(unsigned int frame_rate)
{
	...
	frame_info->max_vload_time =
		(frame_info->frame_interval / NSEC_PER_MSEC) +
...
int set_frame_min_util(int min_util, bool isBoost)
{
	...
	if (isBoost) {
		...
		sched_set_group_normalized_util(frame_info->frame_util,
						FRAME_FORCE_UPDATE);
	} else {
...
static unsigned long calc_prev_frame_load_util(struct frame_info *frame_info,
					       bool fake)
{
	...
	if (fake) {
		prev_frame_interval =
			max_t(unsigned long, frame_info->prev_frame_interval,
			      prev_frame_interval);
	}

	if (prev_frame_interval > 0) {
		frame_util =
			div64_u64((prev_frame_scale << SCHED_CAPACITY_SHIFT),
				  prev_frame_interval);
	}
...
static inline void update_frame_prev_load(struct frame_info *frame_info,
					  bool fake)
{
	frame_info->prev_frame_exec =
		frame_info_fbg_load(frame_info)->prev_window_exec;
	frame_info->prev_frame_scale =
		frame_info_fbg_load(frame_info)->prev_window_scale;
	frame_info->prev_frame_interval = frame_info->fbg->prev_window_size;

	frame_info->prev_frame_load_util =
		calc_prev_frame_load_util(frame_info, fake);
}
...
static inline void set_frame_start(struct frame_info *frame_info)
{
	...
	if (fake) {
		util = max_t(unsigned long, prev_real_util,
			     frame_info->frame_min_util);
	} else {
		util = max_t(unsigned long, frame_info->prev_frame_load_util,
			     frame_info->frame_min_util);
	}

	frame_info->frame_util =
		min_t(unsigned long, frame_info->frame_max_util, util);
}
...
static inline void set_frame_end(struct frame_info *frame_info, bool fake)
{
	...
	frame_info->frame_util =
		min_t(unsigned long, frame_info->frame_max_util,
		      frame_info->prev_frame_load_util);
}
...
unsigned long calc_frame_load(struct frame_info *frame_info, bool vload)
{
	...
	if (frame_info->frame_interval > 0) {
		if (vload) {
			...
			load = div_u64((frame_info_fbg_load(frame_info)
						->curr_window_exec
...
static inline bool check_frame_util_invalid(struct frame_info *frame_info,
					    u64 timeline)
{
	return ((frame_info->fbg)->util_invalid_interval <= timeline);
}

void update_frame_info_tick(struct frame_boost_group *grp, u64 wallclock,
			    struct task_struct *p)
{
	...
	case FRAME_END:
		if (timeline >= frame_info->frame_interval) {
			...
			frame_info->frame_vload =
				calc_frame_load(frame_info, true);
	...
	case FRAME_START:
		...
		if (!check_frame_util_invalid(frame_info, timeline)) {
			...
			if (!use_vload && exec >= max_exec_time &&
			    timeline >= max_exec_time) {
				use_vload = true;
			...
			if (frame_info->max_vload_time >= interval_time) {
				...
				critical_time =
					max_exec_time + (max_exec_time >> 1);
				critical_load = (vload >> 1) + (vload >> 2);

				/* add for discontinuous execution*/
				if (!use_vload && timeline >= critical_time &&
				    curr_load >= 358) {
					use_vload = true;
				}

				if ((curr_load <= critical_load) &&
				    !use_vload) {
					frame_info->frame_vload = curr_load;
				}

				frame_info->frame_vload =
					max(curr_load, frame_info->frame_vload);
			}
			frame_info->frame_util = calc_frame_util(frame_info);
		} else {
			frame_info->status = FRAME_INVALID;
	...
	sched_set_group_normalized_util(frame_info->frame_util,
					FRAME_NORMAL_UPDATE);
	trace_frame_info_tick(p, timeline, exec, curr_load, vload, use_vload,
			      frame_info->frame_qos, frame_info->frame_util);
}
