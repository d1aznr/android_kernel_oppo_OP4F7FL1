...
#define ofb_debug(fmt, ...)                                                    \
	printk_deferred(KERN_INFO "[frame_boost][%s]" fmt, __func__,           \
			##__VA_ARGS__)
...
static void ctrl_set_fbg(int pid, int tid, int hwtid1, int hwtid2)
{
	...
	if (DEBUG) {
		...
		list_for_each_safe (pos, n, &grp->tasks) {
			...
			ofb_debug("grp_thread pid:%d comm:%s depth:%d", p->pid,
				  p->comm, p->fbg_depth);
		}
...
#ifdef CONFIG_OPLUS_FEATURE_IM
	if (im_sf(current) ||
	    strnstr(current->comm, sf, strlen(current->comm))) {
		is_sf = true;
	}
#endif
	leader = current->group_leader;
	if (pid == atomic_read(&fbg_pid) && leader &&
	    strnstr(leader->comm, lc, strlen(leader->comm))) {
		stune_boost = 30;
	}
...
static long sys_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CMD_ID_BOOST_HIT:
		if (data.stage == BOOST_MOVE_FG) {
			ctrl_set_fbg(data.pid, data.tid, data.hwtid1,
				     data.hwtid2);
		}
...
#ifdef CONFIG_COMPAT
static long ofb_ctrl_compat_ioctl(struct file *file, unsigned int cmd,
				  unsigned long arg)
{
	return ofb_ioctl(file, cmd, (unsigned long)(compat_ptr(arg)));
}

static long sys_ctrl_compat_ioctl(struct file *file, unsigned int cmd,
				  unsigned long arg)
{
...
static int __init frame_boost_init(void)
{
	...
	pentry = proc_create("ctrl", S_IRWXUGO, frame_boost_proc,
			     &ofb_ctrl_fops);
	if (!pentry) {
		goto ERROR_INIT_VERSION;
	}

	pentry = proc_create("sys_ctrl", (S_IRWXU | S_IRWXG), frame_boost_proc,
			     &sys_ctrl_fops);
	if (!pentry) {
		goto ERROR_INIT_VERSION;
	}
