...
#ifdef CONFIG_OPLUS_FEATURE_UID_PERF
static inline bool get_uid_perf_enable(void)
{
	return false;
}
static inline int get_cpuset_cgrp_idx_by_name(const char *name)
{
	return -1;
}
static inline void cpuset_add_cg(int cgid, char *name)
{
	return;
}
static inline int cpuset_get_cgrp_idx(struct task_struct *task)
{
	return -1;
}
static inline int cpuset_get_cgrp_idx_locked(struct task_struct *task)
{
	return -1;
}
#endif
