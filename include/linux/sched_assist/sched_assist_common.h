...
#define ux_err(fmt, ...)                                                       \
	printk_deferred(KERN_ERR "[SCHED_ASSIST_ERR][%s]" fmt, __func__,       \
			##__VA_ARGS__)
#define ux_warn(fmt, ...)                                                      \
	printk_deferred(KERN_WARNING "[SCHED_ASSIST_WARN][%s]" fmt, __func__,  \
			##__VA_ARGS__)
#define ux_debug(fmt, ...)                                                     \
	printk_deferred(KERN_INFO "[SCHED_ASSIST_INFO][%s]" fmt, __func__,     \
			##__VA_ARGS__)
...
extern void pick_ux_thread(struct rq *rq, struct task_struct **p,
			   struct sched_entity **se);

extern void inherit_ux_dequeue(struct task_struct *task, int type);
extern void inherit_ux_dequeue_refs(struct task_struct *task, int type,
				    int value);
...
extern void set_inherit_ux(struct task_struct *task, int type, int depth,
			   int inherit_val);
extern void reset_inherit_ux(struct task_struct *inherit_task,
			     struct task_struct *ux_task, int reset_type);
extern void unset_inherit_ux(struct task_struct *task, int type);
extern void unset_inherit_ux_value(struct task_struct *task, int type,
				   int value);
...
static inline void find_slide_boost_task_cpu(struct task_struct *task,
					     int *target_cpu)
{
}
...
#define sched_assist_systrace(...)                                             \
	sched_assist_systrace_pid(SA_SYSTRACE_MAGIC, __VA_ARGS__)

extern void place_entity_adjust_ux_task(struct cfs_rq *cfs_rq,
					struct sched_entity *se, int initial);
extern bool should_ux_task_skip_further_check(struct sched_entity *se);
extern bool should_ux_preempt_wakeup(struct task_struct *wake_task,
				     struct task_struct *curr_task);
extern bool should_ux_task_skip_cpu(struct task_struct *task, unsigned int cpu);
extern void set_ux_task_to_prefer_cpu(struct task_struct *task,
				      int *orig_target_cpu);
extern int set_ux_task_cpu_common_by_prio(struct task_struct *task,
					  int *target_cpu, bool boost,
					  bool prefer_idle, unsigned int type);
extern bool ux_skip_sync_wakeup(struct task_struct *task, int *sync);
extern void set_ux_task_to_prefer_cpu_v1(struct task_struct *task,
...
extern unsigned long reweight_cgroup_task(u64 slice, struct sched_entity *se,
					  unsigned long task_weight,
					  struct load_weight *lw);
#if !defined(CONFIG_OPLUS_SYSTEM_KERNEL_QCOM) ||                               \
	(LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0))
extern void sched_assist_spread_tasks(struct task_struct *p,
				      cpumask_t new_allowed_cpus, int start_cpu,
				      int skip_cpu, cpumask_t *cpus,
				      bool strict);
#else
extern void sched_assist_spread_tasks(struct task_struct *p,
...
extern u64 sa_calc_delta(struct sched_entity *se, u64 delta_exec,
...
extern void update_sa_task_stats(struct task_struct *tsk, u64 delta_ns,
				 int stats_type);
extern void sched_assist_im_systrace_c(struct task_struct *tsk, int tst_type);
extern void sched_assist_update_record(struct task_struct *p, u64 delta_ns,
				       int stats_type);
...
extern bool sched_assist_task_misfit(struct task_struct *task, int cpu,
				     int flag);
#else
static inline bool sched_assist_task_misfit(struct task_struct *task, int cpu,
					    int flag)
{
	return false;
}
...
{
	return (sysctl_sched_assist_enabled >= 2) &&
...
{
	return (task->prio == ANDROID_PRIORITY_URGENT_AUDIO) ||
	       (task->prio == ANDROID_PRIORITY_AUDIO);
}
