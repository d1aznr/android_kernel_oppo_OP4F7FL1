...
struct task_struct *get_futex_owner_by_pid_v2(u32 owner_tid, int *pid,
					      int *tgid)
{
	struct task_struct *futex_owner = NULL;
...
void futex_set_inherit_ux(struct task_struct *owner, struct task_struct *task)
{
	...
	if (is_ux && owner && !test_task_ux(owner)) {
		set_inherit_ux(owner, INHERIT_UX_FUTEX, task->ux_depth,
			       task->ux_state);
	}
}
...
void futex_set_inherit_ux_refs(struct task_struct *owner,
			       struct task_struct *task)
{
...
int futex_set_inherit_ux_refs_v2(struct task_struct *owner,
				 struct task_struct *task)
{
	bool is_ux = test_set_inherit_ux(task);
