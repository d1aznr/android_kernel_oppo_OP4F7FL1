...
extern int sched_frame_rate_handler(struct ctl_table *table, int write,
				    void __user *buffer, size_t *lenp,
				    loff_t *ppos);
extern int sysctl_sched_slide_boost_handler(struct ctl_table *table, int write,
					    void __user *buffer, size_t *lenp,
					    loff_t *ppos);
...
{
	return slide_scene() && is_heavy_ux_task(p) &&
	       oplus_task_misfit(p, cpu);
}

static void inline slide_set_start_cpu(struct root_domain *rd,
				       struct task_struct *p, int *start_cpu)
{
...
static void inline slide_set_task_cpu(struct task_struct *p,
				      int *best_energy_cpu)
{
