...
extern int kswapd_threads_sysctl_handler(struct ctl_table *, int, void __user *,
					 size_t *, loff_t *);
...
static inline int kswapd_affinity_check(struct task_struct *p,
					const struct cpumask *mask)
{
	if (kswapd_unbind_cpu == -1)
		return 0;
	return (p->flags & PF_KSWAPD) &&
	       cpumask_test_cpu(kswapd_unbind_cpu, mask);
}
#endif
