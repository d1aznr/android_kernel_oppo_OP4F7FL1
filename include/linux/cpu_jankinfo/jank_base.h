...
#define jank_dbg(fmt, args...)                                                 \
	pr_info("[BYHP:%s:%d] " fmt, __func__, __LINE__, ##args)
#else
#define jank_dbg(fmt, args...)
#endif
...
#define REGISTER_TRACE_VH(vender_hook, handler)                                \
	{                                                                      \
		...
		if (ret) {                                                     \
			pr_err("failed to register_trace_" #vender_hook        \
			       ", ret=%d\n",                                   \
			       ret);                                           \
			return ret;                                            \
		}                                                              \
...
#define JANK_WIN_SIZE_SHIFT_IN_NS (JANK_WIN_SIZE_SHIFT + JANK_NS_PER_MS_SHIFT)
#define JANK_WIN_SIZE_IN_NS (1 << JANK_WIN_SIZE_SHIFT_IN_NS)
#define JANK_WIN_SIZE_IN_NS_MASK ((1 << JANK_WIN_SIZE_SHIFT_IN_NS) - 1)
...
#define time2winidx(timestamp) ((time2idx(timestamp)) & JANK_WIN_CNT_MASK)
#define time2wincnt(now, last)                                                 \
	(min_t(u32, (time2idx(now) - time2idx(last)), JANK_WIN_CNT))

#define winidx_add(idx, n) ((idx + n) & JANK_WIN_CNT_MASK)
#define winidx_sub(idx, n) (((idx) + JANK_WIN_CNT - (n)) & JANK_WIN_CNT_MASK)
...
void jank_systrace_print_idx(u32 pid, char *tag, u32 idx, u64 val);
#else
...
u32 split_window(u64 now, u64 delta, u64 *win_a, u64 *win_cnt, u64 *win_b);
