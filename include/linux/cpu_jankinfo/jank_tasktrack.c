...
void update_tasktrack_time_win(void)
{
	...
	jank_systrace_print_idx(JANK_SYSTRACE_TASKTRACK, "debug0_tick", 0,
				dbg_tick);
	dbg_tick = !dbg_tick;
#endif
	...
	for (idx = 0; idx < TASK_TRACK_NUM; idx++) {
		...
		jank_systrace_print_idx(JANK_SYSTRACE_TASKTRACK,
					"debug5_unprocessed_delta", idx,
					unprocessed_delta / 1000 / 1000);
		jank_systrace_print_idx(JANK_SYSTRACE_TASKTRACK,
		...
		for (type = TRACE_RUNNING; type < TRACE_CNT; type++) {
			...
			if (unprocessed_delta > JANK_WIN_SIZE_IN_NS) {
				if (is_current_type(type, nowtype)) {
					...
					jank_dbg(
						"m1: type=%d, dbg_borrowed_time=%llu\n",
					...
					jank_dbg(
						"m2: type=%d, dbg_borrowed_time=%llu\n",
						type, ti->borrowed_time[type]);
				}
			} else {
				delta = delta_snap[type] -
					ti->delta_lastwin[type];
				...
				if ((ti->delta_lastwin[type]) == 0) {
					 ...
					 *     window after the beginning of the tracking
					 ****************************************************/
					delta = min_t(u64, delta,
						      JANK_WIN_SIZE_IN_NS_MASK);
				} else {
					if (delta < ti->borrowed_time[type]) {
						delta = 0;
						jank_dbg(
							"WARNING2: an unexpected thing happened!!!\n");
					} else
						delta -=
							ti->borrowed_time[type];
				}
				...
				jank_dbg(
					"m3: type=%d, dbg_borrowed_time=%llu\n",
					type, ti->borrowed_time[type]);
#endif
				...
				if (is_current_type(type, nowtype)) {
					...
					ti->borrowed_time[type] +=
						unprocessed_delta;
					jank_dbg(
						"m4: type=%d, dbg_borrowed_time=%llu\n",
						type, ti->borrowed_time[type]);
				}
			}
			...
			jank_dbg(
				...
				"\n",
				winidx, type, nowtype, dbg_unprocessed_delta,
				dbg_unprocessed_delta / 1000 / 1000, delta,
				delta / 1000 / 1000, dbg_delta1,
				dbg_task_info_delta, dbg_task_info_delta_bak,
...
static enum hrtimer_restart tasktrack_timer_handler(struct hrtimer *timer)
{
	...
	if (type == TRACE_SLEEPING) {
		if (is_inbinder(nowtype))
			tmptype = (BIT(TRACE_SLEEPING) |
				   BIT(TRACE_SLEEPING_INBINDER));
		else if (is_infutex(nowtype))
			tmptype = (BIT(TRACE_SLEEPING) |
				   BIT(TRACE_SLEEPING_INFUTEX));
		else
			tmptype = BIT(TRACE_SLEEPING);
	}
...
void sched_stat_runtime_handler(void *unused, struct task_struct *tsk,
				u64 runtime, u64 vruntime)
{
	jankinfo_tasktrack_update_time(tsk, TRACE_RUNNING, runtime);
}

void sched_stat_wait_handler(void *unused, struct task_struct *tsk, u64 delta)
{
	jankinfo_tasktrack_update_time(tsk, TRACE_RUNNABLE, delta);
}

void sched_stat_sleep_handler(void *unused, struct task_struct *tsk, u64 delta)
{
	jankinfo_tasktrack_update_time(tsk, TRACE_SLEEPING, delta);
}

void sched_stat_blocked_handler(void *unused, struct task_struct *tsk,
				u64 delta)
{
	jankinfo_tasktrack_update_time(tsk, TRACE_DISKSLEEP, delta);
}

void sched_stat_iowait_handler(void *unused, struct task_struct *tsk, u64 delta)
{
	jankinfo_tasktrack_update_time(tsk, TRACE_DISKSLEEP_INIOWAIT, delta);
}

void android_vh_futex_sleep_start_handelr(void *unused, struct task_struct *p)
{
	u32 idx;
...
void android_vh_binder_wait_for_work_hanlder(void *unused, bool do_proc_work,
...
static void tasktrack_timer_start(void)
{
	...
	hrtimer_start(&tasktrack_timer, ns_to_ktime(JANK_WIN_SIZE_IN_NS),
		      HRTIMER_MODE_REL);
}
...
static int proc_task_track_read(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < TASK_TRACK_NUM; i++) {
		...
		jank_dbg(
			"i=%d, now=%d, last_update_time=%d, unprocessed_delta=%d\n",
			i, now >> 20, last_update_time >> 20,
			unprocessed_delta >> 20);
...
#ifdef JANK_DEBUG
				if (k == TRACE_RUNNING || k == TRACE_RUNNABLE ||
				    k == TRACE_SLEEPING || k == TRACE_DISKSLEEP)
					dbg_total += time;
#endif
...
static ssize_t proc_task_track_write(struct file *file, const char __user *buf,
				     size_t count, loff_t *offset)
{
	...
	for (i = 0; i < pid_count; i++) {
		...
		jank_systrace_print_idx(JANK_SYSTRACE_TASKTRACK, "debug2_write",
					i, pid[i]);
		...
		if (pid[i] < 0) {
			...
			for (j = 0; j < TASK_TRACK_NUM; j++) {
				...
				if (tmp_pid + pid[i] == 0) {
					memset(&trace_info->task_info[j], 0,
					       sizeof(struct jank_task_info));
					trace_info->task_info[j].pid =
						INVALID_PID;
...
static int proc_task_track_open(struct inode *inode, struct file *file)
{
					    ...
					    size_t count, loff_t *offset)
{
	...
	if (tasktrack_enable) {
		...
		tasktrack_timer_stop();
		memset(trace_info, 0,
		       ALIGN(sizeof(struct task_track_info),
			     cache_line_size()));
...
static int proc_task_track_enable_open(struct inode *inode, struct file *file)
{
...
static int dump_callstack(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < TASK_TRACK_NUM; i++) {
		...
		for (j = 0; j < JANK_WIN_CNT; j++) {
			...
			seq_printf(m, "[%llu.%lu] [%d] ", (u64)ts.tv_sec,
				   ts.tv_nsec, pid);
...
static int proc_callstack_open(struct inode *inode, struct file *file)
{
...
struct proc_dir_entry *jank_tasktrack_proc_init(struct proc_dir_entry *pde)
{
	...
	entry = proc_create("task_track", S_IRUGO | S_IWUGO, pde,
			    &proc_task_track_operations);
	if (!entry) {
		pr_err("create task_track fail\n");
		goto err_task_track;
	}

	entry = proc_create("task_track_enable", S_IRUGO | S_IWUGO, pde,
			    &proc_task_track_enable_operations);
	if (!entry) {
		pr_err("create task_track_neable fail\n");
		goto err_task_track_enable;
	}

	entry = proc_create("callstack", S_IRUGO, pde,
			    &proc_callstack_operations);
	if (!entry) {
