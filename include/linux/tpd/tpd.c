...
struct monitor_gp {
	...
	unsigned int miss_list_tgid
		[MAX_MISS_LIST]; /* used to check if need re-tag or not */
	char miss_list[MAX_MISS_LIST]
		      [TASK_COMM_LEN]; /* used to check if need re-tag or not */
	int not_yet;
	int cur_idx;
};
...
static atomic64_t tpd_ctl =
	ATOMIC64_INIT(0); /* used to ignore fg task checking */
		...
		if (mgp[j].not_yet > 0) {
			for (i = 0; i < MAX_MISS_LIST; ++i) {
				if (strlen(mgp[j].miss_list[i]) > 0) {
					cnt += snprintf(
						buf + cnt, PAGE_SIZE - cnt,
						"%s %d\n", mgp[j].miss_list[i],
						mgp[j].miss_list_tgid[i]);
				}
			}
		}
...
static inline void tagging_by_name(struct task_struct *tsk, char *name,
				   int decision, int *cnt)
{
	size_t tlen = 0, len = 0;
...
static void tag_from_tgid(unsigned int tgid, int decision, char *thread_name,
			  int *cnt)
{
...
static inline void dy_tagging_by_name(struct task_struct *tsk, const char *name,
				      int decision, int dtpd, int *cnt)
{
	size_t tlen = 0, len = 0;
...
static void dy_tag_from_tgid(unsigned int tgid, int decision,
			     const char *thread_name, int dtpd, int *cnt)
{
	struct task_struct *p, *t;
...
static int tpd_cmd_store(const char *buf, const struct kernel_param *kp)
{
	unsigned int tgid = 0;
	int tp_decision = -1;
	char threads[MAX_THREAD_INPUT][TASK_COMM_LEN] = { { 0 }, { 0 }, { 0 },
							  { 0 }, { 0 }, { 0 } };
	int ret, i, cnt = 0;
	...
	tpd_logi("tpd params: %u %d %s %s %s %s %s %s, from %s %d, total=%d\n",
		 tgid, tp_decision, threads[0], threads[1], threads[2],
		 threads[3], threads[4], threads[5], current->comm,
		 current->pid, ret);
...
static int tpd_store(const char *buf, const struct kernel_param *kp)
{
	...
	tpd_logi(
		"tpd param pid:%u tid:%u, tpd_enable:%d decision:%d from %s %d\n",
...
static int tpd_process_trigger_store(const char *buf,
				     const struct kernel_param *kp)
{
	...
	ret = sscanf(buf, "%d,%u,%d,%d\n", &tpd_group, &tgid, &tpdenable,
		     &tp_decision);

	tpd_logi(
		"tpd param group:%d pid:%u tpd_enable:%d decision:%d from %s %d\n",
		tpd_group, tgid, tpdenable, tp_decision, current->comm,
		current->pid);
	...
	if (tpd_group == TPD_GROUP_MEDIAPROVIDER) {
		for (i = 0; i < MONITOR_THREAD_NUM; i++) {
			cnt = 0;
			dy_tag_from_tgid(tgid, tpdenable ? tp_decision : 0,
					 threads[i], tpdenable, &cnt);
			/* can't find thread to tag/un-tag */
			if (cnt == 0) {
				spin_lock(&group->miss_list_lock);
				if (tpdenable) {
					/* need re-tag, add thread name into miss_list */
					strncpy(group->miss_list[group->cur_idx],
						threads[i], strlen(threads[i]));
					group->miss_list_tgid[group->cur_idx] =
						tgid;
					group->cur_idx = (group->cur_idx + 1) %
							 MAX_MISS_LIST;
					group->not_yet++;
				} else {
					/* dynmic tpd disabled when miss_list still have thread need to tag, we clear the miss list */
					for (j = 0; j < MAX_MISS_LIST; ++j) {
						if (group->miss_list_tgid[j] ==
						    tgid) {
							strcpy(group->miss_list
								       [j],
							       "");
							group->miss_list_tgid[j] =
								0;
							group->not_yet--;
						}
					}
...
int tpd_suggested(struct task_struct *tsk, int request_cluster)
{
	...
	case TPD_TYPE_P:
		...
		break;
	default:
		tpd_loge("suggest unexpected case: tpd = %d, tpd_st = %d",
			 tsk->tpd, tsk->tpd_st);
		break;
	}
out:
	tpd_logi(
		"pid = %d: comm = %s, tpd = %d, suggest_cpu = %d, task is fg? %d, tpd_ctl = %lld, tpd_st = %d\n",
		tsk->pid, tsk->comm, tsk->tpd, suggest_cluster, task_is_fg(tsk),
		atomic64_read(&tpd_ctl), tsk->tpd_st);
...
out:
	tpd_logi(
		"pid = %d: comm = %s, tpd = %d, suggest_cpu = %d, task is fg? %d, tpd_ctl = %lld, tpd_st = %d\n",
		tsk->pid, tsk->comm, tsk->tpd, suggest_cpu, task_is_fg(tsk),
		atomic64_read(&tpd_ctl), tsk->tpd_st);
	return suggest_cpu;
}
...
void tpd_mask(struct task_struct *tsk, cpumask_t *request)
{
	...
	if (!(task_is_fg(tsk) || atomic64_read(&tpd_ctl) ||
	      (tsk->tpd_st > 0))) {
		if (!task_is_fg(tsk))
...
out:
	tpd_logi(
		"pid = %d: comm = %s, tpd = %d, task is fg? %d, tpd_ctl = %lld, tpd_st = %d\n",
		tsk->pid, tsk->comm, tsk->tpd, task_is_fg(tsk),
		atomic64_read(&tpd_ctl), tsk->tpd_st);
}
...
bool tpd_check(struct task_struct *tsk, int dest_cpu)
{
	...
	case TPD_TYPE_GS:
		...
		if (cluster_total == 3 &&
		    cpumask_test_cpu(dest_cpu, &clusters[2].related_cpus))
			mismatch = true;
		break;
	case TPD_TYPE_P:
		if (cluster_total == 3 &&
	...
	case TPD_TYPE_PG:
		...
		break;
	default:
		tpd_loge("check unexpected case: tpd = %d, tpd_st = %d",
			 tsk->tpd, tsk->tpd_st);
		break;
	}
...
out:
	tpd_logi(
		"task:%d comm:%s dst: %d should migrate = %d, task is fg? %d, tpd = %d, tpd_st = %d, tpd_ctl = %lld\n",
		tsk->pid, tsk->comm, dest_cpu, !mismatch, task_is_fg(tsk),
		tsk->tpd, tsk->tpd_st, atomic64_read(&tpd_ctl));
