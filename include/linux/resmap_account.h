...
static inline int is_backed_addr(struct mm_struct *mm, unsigned long start,
				 unsigned long end)
{
	return (mm && mm->reserve_vma && start >= mm->reserve_vma->vm_start &&
...
static inline int check_general_addr(struct mm_struct *mm, unsigned long start,
				     unsigned long end)
{
...
vm_mmap_pgoff_with_check(struct file *file, unsigned long addr,
			 unsigned long len, unsigned long prot,
			 unsigned long flags, unsigned long pgoff);
extern void exit_reserved_mmap(struct mm_struct *mm);
extern int dup_reserved_mmap(struct mm_struct *mm, struct mm_struct *oldmm,
			     struct kmem_cache *vm_area_cachep);
extern int reserved_area_checking(struct mm_struct *mm, unsigned long *vm_flags,
