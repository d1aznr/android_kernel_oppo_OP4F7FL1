...
DECLARE_EVENT_CLASS(
	...
	TP_PROTO(size_t len, unsigned int mask, unsigned int flags,
		 char *dbg_name),

	TP_ARGS(len, mask, flags, dbg_name),

	TP_STRUCT__entry(__field(size_t, len) __field(unsigned int, mask)
				 __field(unsigned int, flags)
					 __array(char, dbg_name, 8)),

	TP_fast_assign(__entry->len = len; __entry->mask = mask;
		       __entry->flags = flags;
		       strlcpy(__entry->dbg_name, dbg_name, 8);),

	TP_printk("len=%zu mask=0x%x flags=0x%x dbg_name=%s", __entry->len,
		  __entry->mask, __entry->flags, __entry->dbg_name));
