...
TRACE_EVENT(
	...
	TP_STRUCT__entry(__field(unsigned long, target_id) __field(unsigned int,
								   nr_regions)
				 __field(unsigned long,
					 start) __field(unsigned long, end)
					 __field(unsigned int, nr_accesses)
						 __field(unsigned int, age)),

	TP_fast_assign(__entry->target_id = target_id;
		       __entry->nr_regions = nr_regions;
		       __entry->start = r->ar.start; __entry->end = r->ar.end;
...
TRACE_EVENT(
	...
	TP_ARGS(nr_reclaim_tried_regions, bytes_reclaim_tried_regions,
	...
	TP_printk(
		"nr_reclaim_tried_regions=%lu bytes_reclaim_tried_regions=%lu nr_reclaimed_regions=%lu, bytes_reclaimed_regions=%lu, nr_quota_exceeds=%lu",
		__entry->nr_reclaim_tried_regions,
		__entry->bytes_reclaim_tried_regions,
		__entry->nr_reclaimed_regions, __entry->bytes_reclaimed_regions,
