...
TRACE_EVENT(sugov_next_freq_tl,
	    TP_PROTO(unsigned int cpu, unsigned long util, unsigned long max,
		     unsigned int freq, unsigned int laf,
		     unsigned int prev_freq),
	    TP_ARGS(cpu, util, max, freq, laf, prev_freq),
	    TP_STRUCT__entry(__field(unsigned int, cpu) __field(
		    unsigned long, util) __field(unsigned long, max)
				     __field(unsigned int,
					     freq) __field(unsigned int, laf)
					     __field(unsigned int, prev_freq)),
	    TP_fast_assign(__entry->cpu = cpu; __entry->util = util;
			   __entry->max = max; __entry->freq = freq;
			   __entry->laf = laf; __entry->prev_freq = prev_freq;),
	    TP_printk("cpu=%u util=%lu max=%lu freq=%u laf=%u prev_freq=%u",
		      __entry->cpu, __entry->util, __entry->max, __entry->freq,
		      __entry->laf, __entry->prev_freq));
...
TRACE_EVENT(
	...
	TP_STRUCT__entry(__field(unsigned int, freq) __field(unsigned int,
							     prevfreq)
				 __field(unsigned int, freqmax)
					 __field(unsigned int, freqmin)
						 __field(unsigned int, tl)
							 __field(int, index)),
	TP_fast_assign(__entry->freq = freq; __entry->prevfreq = prevfreq;
		       __entry->freqmax = freqmax; __entry->freqmin = freqmin;
		       __entry->tl = tl; __entry->index = index;),
	TP_printk("freq=%u prevfreq=%u freqmax=%u freqmin=%u tl=%u index=%d",
		  __entry->freq, __entry->prevfreq, __entry->freqmax,
		  __entry->freqmin, __entry->tl, __entry->index));

TRACE_EVENT(sugov_time_limit,
	    TP_PROTO(unsigned int cpu, const char *reason,
		     unsigned long long delta, unsigned int curr,
		     unsigned int target),
	    TP_ARGS(cpu, reason, delta, curr, target),
	    TP_STRUCT__entry(__field(unsigned int, cpu) __string(reason, reason)
				     __field(unsigned long long,
					     delta) __field(unsigned int, curr)
					     __field(unsigned int, target)),
	    TP_fast_assign(__entry->cpu = cpu; __assign_str(reason, reason);
			   __entry->delta = delta; __entry->curr = curr;
			   __entry->target = target;),
	    TP_printk("cpu=%u reason=%s delta_ns=%llu curr=%u target=%u",
		      __entry->cpu, __get_str(reason), __entry->delta,
