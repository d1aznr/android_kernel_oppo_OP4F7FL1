...
static int check_msg_in_range(struct sock *nl_sock, int msg_type)
{
	...
	if (msg_type >= OPLUS_COMMON_MSG_BASE &&
	    msg_type < OPLUS_SYNC_MSG_BASE) {
		return 1;
	}

	/*not in common part*/
	if (nl_sock == oplus_sync_nl_sock) {
		if (msg_type >= OPLUS_SYNC_MSG_BASE &&
		    msg_type < OPLUS_ASYNC_MSG_BASE) {
			return 1;
		} else {
			return 0;
		}
	} else if (nl_sock == oplus_async_nl_sock) {
		if (msg_type >= OPLUS_ASYNC_MSG_BASE &&
		    msg_type < OPLUS_WIFI_CAP_CENTER_MAX) {
			return 1;
		} else {
			return 0;
		}
...
/* send to user space */
static int oplus_wcc_send_to_user(struct sock *oplus_sock, u32 oplus_pid,
				  int msg_type, char *payload, int payload_len)
{
	int ret = 0;
	...
	/* send data */
	if (oplus_pid) {
		ret = netlink_unicast(oplus_sock, skbuff, oplus_pid,
				      MSG_DONTWAIT);
	} else {
		printk(KERN_ERR
		       "oplus_wcc_netlink: can not unicast skbuff, oplus_pid=0\n");
		kfree_skb(skbuff);
	}
	if (ret < 0) {
		printk(KERN_ERR
		       "oplus_wcc_netlink: can not unicast skbuff,ret = %d\n",
		       ret);
		return 1;
	}
...
static void oplus_wcc_sample_resp(struct sock *oplus_sock, u32 oplus_pid,
				  int msg_type)
{
	int payload[4];
	payload[0] = 5;
	...
	oplus_wcc_send_to_user(oplus_sock, oplus_pid, msg_type, (char *)payload,
			       sizeof(payload));
	if (oplus_wcc_debug) {
...
static int oplus_wcc_sample_sync_get(struct nlmsghdr *nlh)
{
	...
	debug("sample_sync_get: %u%u%u%u\n", data[0], data[1], data[2],
	      data[3]);
	oplus_wcc_sample_resp(oplus_sync_nl_sock, oplus_sync_nl_pid,
...
static void get_supported_channels_stub(int band, int *len, int *freqs,
					int max_num)
{
...
void register_oplus_wfd_wlan_ops(struct oplus_wfd_wlan_ops_t *ops)
{
	...
	if (ops->remove_he_ie_from_probe_request)
		oplus_wfd_wlan_ops.remove_he_ie_from_probe_request =
			ops->remove_he_ie_from_probe_request;
	...
	if (ops->get_supported_channels)
		oplus_wfd_wlan_ops.get_supported_channels =
			ops->get_supported_channels;
...
static void oplus_wcc_get_dbs_capacity(struct nlmsghdr *nlh)
{
	...
	oplus_wcc_send_to_user(oplus_sync_nl_sock, oplus_sync_nl_pid,
			       OPLUS_SYNC_DBS_CAPACITY_GET, (char *)&cap,
			       sizeof(cap));
}
static void oplus_wcc_get_phy_capacity(struct nlmsghdr *nlh)
{
	...
	oplus_wcc_send_to_user(oplus_sync_nl_sock, oplus_sync_nl_pid,
			       OPLUS_SYNC_PHY_CAPACITY_GET, (char *)&cap,
			       sizeof(cap));
}
...
static void oplus_wcc_get_supported_channels(struct nlmsghdr *nlh)
{
	...
	if (nlmsg_len(nlh) > 0) {
		...
		if (band < oplus_wfd_band_max) {
			oplus_wfd_wlan_ops.get_supported_channels(
				band, &len, s_oplus_wfd_freqs + 1,
				OPLUS_WFD_FREQS_NUM_MAX);
...
static void oplus_wcc_get_avoid_channels(struct nlmsghdr *nlh)
{
	...
	oplus_wfd_wlan_ops.get_avoid_channels(&len, s_oplus_wfd_freqs + 1,
					      OPLUS_WFD_FREQS_NUM_MAX);
	s_oplus_wfd_freqs[0] = len;
...
static void oplus_wcc_timer_function(void)
{
	if (async_msg_type == OPLUS_SAMPLE_ASYNC_GET) {
		oplus_wcc_sample_resp(oplus_async_nl_sock, oplus_async_nl_pid,
...
static int oplus_wcc_sync_nl_rcv_msg(struct sk_buff *skb, struct nlmsghdr *nlh,
...
static int oplus_wcc_async_nl_rcv_msg(struct sk_buff *skb, struct nlmsghdr *nlh,
				      struct netlink_ext_ack *extack)
{
	u32 portid = NETLINK_CB(skb).portid;
...
static int oplus_wcc_netlink_init(void)
{
	...
	oplus_sync_nl_sock = netlink_kernel_create(
		&init_net, NETLINK_OPLUS_WIFI_CAP_CENTER_SYNC, &cfg1);
	oplus_async_nl_sock = netlink_kernel_create(
		&init_net, NETLINK_OPLUS_WIFI_CAP_CENTER_ASYNC, &cfg2);
...
{
	oplus_table_hrd = register_net_sysctl(&init_net, "net/oplus_wcc",
					      oplus_wcc_sysctl_table);
