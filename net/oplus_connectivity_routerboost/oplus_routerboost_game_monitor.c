...
static void detect_game_stream_info(struct sk_buff *skb)
{
	...
	if (iph->protocol == IPPROTO_UDP /*|| iph->protocol == IPPROTO_TCP*/) {
		...
		if (ctinfo == IP_CT_NEW) {
			...
			routerboost_genl_msg_send_to_user(
				ROUTERBOOST_MSG_REPOET_GAME_STREEM_INFO,
			...
			routerboost_genl_msg_send_to_user(
				ROUTERBOOST_MSG_REPOET_GAME_PKT_INFO,
				(char *)send_msg, sizeof(send_msg));
			++detect_playing_pkt_cnt;

			if (detect_playing_pkt_cnt == DETECT_PKT_THRED) {
				routerboost_genl_msg_send_to_user(
					ROUTERBOOST_MSG_REPOET_GAME_PKT_INFO_FIN,
					(char *)send_msg, sizeof(send_msg));
			}
		}
	}
...
static unsigned int
routerboost_nf_output_hook_func(void *priv, struct sk_buff *skb,
