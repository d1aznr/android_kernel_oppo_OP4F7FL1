	...
	if (flag) {
		for (i = 0; i < SCORE_WINDOW; i++) {
			if (uplink_score_info[link].uplink_score_save[i] > 0) {
				score_sum += uplink_score_info[link]
						     .uplink_score_save[i];
				j++;
			}
		}
		...
		for (i = 0; i < SCORE_WINDOW; i++) {
			if (downlink_score_info[link].downlink_score_save[i] >
			    0) {
				score_sum += downlink_score_info[link]
						     .downlink_score_save[i];
				j++;
			}
		}
	}
...
static int oplus_update_score_count(int link, int score, int flag)
{
	...
	if (flag) {
		if (score > WORSE_BASE_SCORE) {
			...
			if (uplink_score_info[link].uplink_score_count >
			    SCORE_KEEP) {
				uplink_score_info[link].uplink_score_count =
					SCORE_KEEP;
			}
		...
		if (score > WORSE_BASE_SCORE) {
			...
			if (downlink_score_info[link].downlink_score_count >
			    SCORE_KEEP) {
				downlink_score_info[link].downlink_score_count =
					SCORE_KEEP;
			}
...
void oplus_score_calc_and_report(void)
{
	...
	for (i = 0; i < MAX_LINK_NUM; i++) {
		...
		memcpy((void *)ifname, (void *)uplink_score_info[i].ifname,
		       IFNAME_LEN);
		uplink_packets = uplink_score_info[i].uplink_packets;
		uplink_retrans_packets =
		...
		if (uplink_total_packets == 0) {
			...
			retrans_rate = 100 * uplink_retrans_packets /
				       uplink_total_packets;
			if (uplink_packets >
			    ((para_rate * PACKET_PER_SEC) >> 3)) {
				uplink_score =
					(s32)(GOOD_BASE_SCORE -
					      (retrans_rate * para_loss) / 8);
				...
				if (uplink_srtt > WORST_VIDEO_RTT_THRESH) {
					uplink_score = (s32)(WORST_BASE_SCORE -
							     (WORST_BASE_SCORE *
							      retrans_rate *
							      para_loss) /
								     800);
				} else if (uplink_srtt > WORST_RTT_THRESH) {
					downlink_rate = downlink_packets *
							1000 / uplink_srtt;
					if (downlink_rate == 0) {
						uplink_score = -1;
					} else if (downlink_rate >
						   VIDEO_GOOD_RATE) {
						uplink_score =
							(s32)(GOOD_BASE_SCORE -
							      (GOOD_BASE_SCORE *
					...
					uplink_score = (s32)(WORSE_BASE_SCORE -
							     (WORSE_BASE_SCORE *
							      retrans_rate *
							      para_loss) /
								     800);
					...
					uplink_score = (s32)(GOOD_BASE_SCORE -
							     (retrans_rate *
							      para_loss) /
								     8);
				}
		...
		uplink_report =
			oplus_update_score_count(i, uplink_score, OPLUS_UPLINK);
		if (uplink_report) {
			index = uplink_score_info[i].uplink_score_index++ %
				SCORE_WINDOW;
			uplink_score_info[i].uplink_score_save[index] =
				uplink_score;
		}
		...
		if (oplus_score_debug ||
		    (uplink_smooth_score > 0 &&
		     uplink_smooth_score <= WORST_BASE_SCORE)) {
			if (net_ratelimit())
				printk("[oplus_score]:up_score:link=%u,if=%s,up_pack=%u,up_retran=%u,up_rtt=%u,score=%d,s_score=%d,seq=%u,uid=%u,retrans_rate=%u,index=%u,nodata=%u\n",
				       uplink_index, ifname, uplink_packets,
				       uplink_retrans_packets, uplink_srtt,
				       uplink_score, uplink_smooth_score,
				       uplink_seq,
				       oplus_score_foreground_uid[0],
				       retrans_rate, index,
				       uplink_nodata_count);
		}
		...
		if (downlink_total_packets == 0) {
			...
			retrans_rate = 100 * downlink_retrans_packets /
				       downlink_total_packets;
			if (downlink_packets >
			    ((para_rate * PACKET_PER_SEC) >> 3)) {
				downlink_score =
					(s32)(GOOD_BASE_SCORE -
					      retrans_rate * para_loss / 4);
			} else {
		...
		if ((downlink_total_packets < 15) &&
		    (downlink_score > WORST_BASE_SCORE)) {
			downlink_score = -1;
		}

		spin_lock_bh(&downlink_score_lock);
		downlink_report = oplus_update_score_count(i, downlink_score,
							   OPLUS_DOWNLINK);
		if (downlink_report) {
			index = downlink_score_info[i].downlink_score_index++ %
				SCORE_WINDOW;
			downlink_score_info[i].downlink_score_save[index] =
				downlink_score;
		}
		downlink_smooth_score =
			oplus_get_smooth_score(i, OPLUS_DOWNLINK);
		...
		if (oplus_score_debug ||
		    (downlink_smooth_score > 0 &&
		     downlink_smooth_score <= WORST_BASE_SCORE)) {
			if (net_ratelimit())
				printk("[oplus_score]:down_score:link=%u,if=%s,down_pack=%u,down_retran=%u,rtt=%u,score=%d,s_score=%d,seq=%u,uid=%u,retrans_rate=%u,index=%u,nodata=%u\n",
				       downlink_index, ifname, downlink_packets,
				       downlink_retrans_packets, uplink_srtt,
				       downlink_score, downlink_smooth_score,
				       downlink_seq,
				       oplus_score_foreground_uid[0],
				       retrans_rate, index,
				       downlink_nodata_count);
		...
		if (uplink_report || downlink_report) {
			...
			oplus_score_send_netlink_msg(
				OPLUS_SCORE_MSG_REPORT_NETWORK_SCORE,
				(char *)&link_score_msg,
				sizeof(link_score_msg));
			if (oplus_score_debug ||
			    (downlink_smooth_score <= WORST_BASE_SCORE) ||
			    (uplink_smooth_score <= WORST_BASE_SCORE)) {
				if (net_ratelimit())
		...
		if (oplus_score_debug) {
			printk("[oplus_score]:report_score_all:link=%u,uplink_score=%d,us_score=%d,downlink_score=%d,ds_score=%d,uid=%u,ul_p=%d,dl_p=%d\n",
			       uplink_index, uplink_score, uplink_smooth_score,
			       downlink_score, downlink_smooth_score,
			       oplus_score_foreground_uid[0], uplink_report,
			       downlink_report);
		}
	}
...
static void oplus_score_report_timer_init(void)
{
	...
	timer_setup(&oplus_score_report_timer,
		    oplus_score_report_timer_function, 0);
}
...
static void oplus_score_uplink_stat(struct sk_buff *skb)
{
	...
	if (i < 0) {
		if (oplus_score_debug) {
			printk("[oplus_score]:upskb_linkerr,link=%d,if=%s,seq=%u,ack=%u,sport=%u,dport=%u,uid=%u\n",
			       link_index, skb->dev->name, ntohl(tcph->seq),
			       ntohl(tcph->ack_seq), ntohs(tcph->source),
	...
	if (oplus_score_debug) {
		printk("[oplus_score]:uplink=%d,if=%s,seq=%u,high_seq=%u,uplink_retran=%u,npacket=%u,uid=%u,sport=%u,dport=%u,state=%d,len=%u\n",
		       ...
		       uplink_score_info[i].uplink_retrans_packets,
		       uplink_score_info[i].uplink_packets,
		       (u32)(sk->sk_uid.val), ntohs(tcph->source),
		...
		if ((tp->srtt_us != 0) && (m > 50)) {
			if (oplus_score_debug) {
				printk("[oplus_score]:now=%u,lrcttime=%u,half_rtt=%d,m=%d,Hz=%u,rtt=%u,seq=%u\n",
				       now, icsk->icsk_ack.lrcvtime, half_rtt,
				       m, HZ, tp->srtt_us, skb_seq);
			}
			return OPLUS_TRUE;
		}
	}
...
static inline int skb_v6_check(struct sk_buff *skb)
{
	...
	/* skb is pure ack*/
	if ((ntohs(ipv6h->payload_len) == tcph->doff * 4) &&
	    (!tcph->syn || !tcph->fin))
		return OPLUS_FALSE;
...
static unsigned int
oplus_score_postrouting_hook4(void *priv, struct sk_buff *skb,
...
static unsigned int
oplus_score_postrouting_hook6(void *priv, struct sk_buff *skb,
			      const struct nf_hook_state *state)
{
	if (!oplus_score_enable_flag)
		return NF_ACCEPT;
...
static void oplus_score_enable(struct nlattr *nla)
{
	...
	printk("[oplus_score]:oplus_score_enable_flag = %u",
...
static void oplus_score_set_foreground_uid(struct nlattr *nla)
{
	...
	if (num <= 0 || num > FOREGROUND_UID_MAX_NUM) {
		printk("[oplus_score]: foreground uid num out of range, num = %d",
		       num);
		return;
	}

	memset(oplus_score_foreground_uid, 0,
	       sizeof(oplus_score_foreground_uid));
	for (i = 0; i < num; i++) {
...
static void oplus_score_request_score(struct nlattr *nla)
{
	...
	link_score_msg.uplink_score =
		oplus_get_smooth_score(link_index, OPLUS_UPLINK);
	spin_unlock_bh(&uplink_score_lock);
	...
	link_score_msg.downlink_score =
		oplus_get_smooth_score(link_index, OPLUS_DOWNLINK);
	spin_unlock_bh(&downlink_score_lock);
...
static void oplus_score_add_uplink(u32 link_index, struct net_device *dev)
{
	...
	for (i = 0; i < MAX_LINK_NUM; i++) {
		...
		memset(&uplink_score_info[i], 0,
		       sizeof(struct uplink_score_info_st));
		uplink_score_info[i].link_index = link_index;
		memcpy((void *)uplink_score_info[i].ifname, (void *)dev->name,
		       IFNAME_LEN);
...
static void oplus_score_add_downlink(u32 link_index, struct net_device *dev)
{
	...
	for (i = 0; i < MAX_LINK_NUM; i++) {
		...
		memset(&downlink_score_info[i], 0,
		       sizeof(struct downlink_score_info_st));
...
static void oplus_score_add_link(struct nlattr *nla)
{
	...
	printk("[oplus_score]:to add_link index=%u,dev_name=%s, uplink_num=%d,downlink_num=%d!\n",
	       link_index, dev->name, oplus_score_uplink_num,
	       oplus_score_downlink_num);
...
{
	oplus_score_table_hrd = register_net_sysctl(
		&init_net, "net/oplus_score", oplus_score_sysctl_table);
