...
static int ipv6_rto_netlink_rcv_msg(struct sk_buff *skb, struct nlmsghdr *nlh,
...
static int oplus_ipv6_rto_send_to_user(int msg_type, char *payload,
				       int payload_len)
{
	int ret = 0;
	...
	/* send data */
	ret = netlink_unicast(oplus_ipv6_rto_sock, skbuff, oplus_ipv6_rto_pid,
			      MSG_DONTWAIT);
	if (ret < 0) {
...
int ipv6_rto_encounter(kuid_t uid, struct in6_addr v6_saddr)
{
	...
	oplus_ipv6_rto_send_to_user(IPV6_RTO_UID_NOTIFY, (char *)(&rto_info),
				    sizeof(struct oplus_ipv6_rto_info));
...
int ipv4_rto_encounter(kuid_t uid, unsigned int v4_saddr)
{
	...
	oplus_ipv6_rto_send_to_user(IPV4_RTO_UID_NOTIFY, (char *)(&rto_info),
				    sizeof(struct oplus_ipv4_rto_info));
...
static int oplus_ipv6_rto_netlink_init(void)
{
	...
	oplus_ipv6_rto_sock =
		netlink_kernel_create(&init_net, NETLINK_OPLUS_IPV6_RTO, &cfg);
