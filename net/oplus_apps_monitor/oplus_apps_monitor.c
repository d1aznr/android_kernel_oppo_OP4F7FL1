...
static int find_skb_uid_index_in_apps_record_table(struct sock *sk)
{
	...
	for (index = 0; index < g_monitor_apps_num; ++index) {
		uid = make_kuid(&init_user_ns,
				g_monitor_apps_table[index].app_uid);
...
void statistics_monitor_apps_rtt_via_uid(int if_index, int rtt, struct sock *sk)
{
	...
	if (if_index == DUAL_STA_WLAN0_INDEX ||
	    if_index == DUAL_STA_WLAN1_INDEX) {
		if_index = WLAN_INDEX;
	} else {
		if_index = CELL_INDEX;
	}
...
static int apps_monitor_netlink_init(void)
{
	...
	apps_monitor_netlink_sock = netlink_kernel_create(
		&init_net, NETLINK_OPLUS_APPS_MONITOR, &cfg);
...
{
	timer_setup(&apps_monitor_timer, (void *)apps_monitor_timer_function,
		    0);
