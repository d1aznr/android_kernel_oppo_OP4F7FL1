...
{
	if (ct != NULL) {
		ct->mark = (ct->mark & ~CT_MARK_APP_TYPE_MASK) |
			   ((type << CT_MARK_APP_TYPE_OFFSET) &
	...
	for (i = 0; i < video_app_list.count; i++) {
		if (video_app_list.uid[i]) {
			...
			if (is_same_app(uid.val, app_uid.val)) {
				debug("oplus_sla video app, the uid = %d",
				      app_uid.val);
				return true;
			}
		}
			  ...
			  struct sk_buff *skb)
{
	...
	if (SLA_MODE_DUAL_WIFI == sla_work_mode && is_video_app(app_uid)) {
		...
		if (NULL != sk) {
			...
			if (oplus_sla_debug) {
				debug("skb->mark = %x ct->mark = %x get_ct_mark = %x\n",
				      skb->mark, ct->mark, get_ct_mark(ct));
			}
		}
...
static void handle_timestamp_error_case(struct sk_buff *skb,
					const struct nf_hook_state *state)
{
	struct sock *sk = NULL;
	struct iphdr *iph;
	...
	if ((iph = ip_hdr(skb)) != NULL && iph->protocol == IPPROTO_TCP) {
		...
		if (sk && (!sk_fullsock(sk) || sk->sk_state == TCP_TIME_WAIT ||
...
static int oplus_sla_set_video_app_uid(struct nlattr *nla)
{
	...
	if (video_app_list.count > 0 &&
	    video_app_list.count < DUAL_STA_APP_NUM) {
		int i;
...
static int oplus_sla_disable(struct nlattr *nla)
{
	...
	if (oplus_sla_enable_status && disable_type) {
		...
		dns_network = 0;
		video_app_network = 0;
		oplus_sla_send_to_user(OPLUS_SLA_DISABLED_EVENT,
				       (char *)&disable_type, sizeof(int));
	}
	sla_write_unlock();
	return 0;
}
