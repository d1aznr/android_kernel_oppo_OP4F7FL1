...
static struct iface_uid_stats *get_stats_from_map(char *iface, u32 uid, u64 key)
{
	...
	struct hlist_head *t = &(s_iface_uid_stats_map[hash_min(
		key, HASH_BITS(s_iface_uid_stats_map))]);
	hlist_for_each_safe (pos, next, t) {
		stats = container_of(pos, struct iface_uid_stats, node);
		if (strcmp(stats->value.iface, iface) == 0 &&
		    uid == stats->value.uid) {
			return stats;
		}
	}
...
static int send_netlink_data(int type, char *data, int len)
{
	...
	ret = genl_msg_prepare_usr_msg(OPLUS_STATS_CALC_CMD_UP, size,
				       s_user_pid, &skbuff);
	if (ret) {
		return ret;
	}
...
static int send_all_stats(struct nlattr *nla)
{
	...
	spin_lock_bh(&s_stats_calc_lock);
	total_len = sizeof(s_stats_count) +
		    sizeof(struct iface_uid_stats_value) * s_stats_count;
	...
	if (s_stats_count != 0) {
		...
		hash_for_each_safe (s_iface_uid_stats_map, pkt, next, pos,
				    node) {
			if (count < s_stats_count) {
				memcpy(pvalue, &pos->value,
				       sizeof(struct iface_uid_stats_value));
				pvalue++;
				count++;
			}
		}
...
static unsigned int
oplus_stats_calc_input_hook(void *priv, struct sk_buff *skb,
...
static unsigned int
oplus_stats_calc_output_hook(void *priv, struct sk_buff *skb,
			     const struct nf_hook_state *state)
{
	u32 uid = 0;
...
static int oplus_stats_calc_netlink_rcv_msg(struct sk_buff *skb,
...
{
	oplus_stats_calc_table_hdr = register_net_sysctl(
		&init_net, "net/oplus_stats_calc", oplus_net_hook_sysctl_table);
...
static int __init oplus_stats_calc_init(void)
{
	...
	ret = nf_register_net_hooks(&init_net, oplus_stats_calc_netfilter_ops,
				    ARRAY_SIZE(oplus_stats_calc_netfilter_ops));
	if (ret < 0) {
		LOGK(1,
		     "oplus_stats_calc_init netfilter register failed, ret=%d",
		     ret);
		oplus_stats_calc_netlink_exit();
		return ret;
	} else {
		LOGK(1,
		     "oplus_stats_calc_init netfilter register successfully.");
	}
...
static void __exit oplus_stats_calc_fini(void)
{
	...
	oplus_stats_calc_netlink_exit();
	nf_unregister_net_hooks(&init_net, oplus_stats_calc_netfilter_ops,
				ARRAY_SIZE(oplus_stats_calc_netfilter_ops));
