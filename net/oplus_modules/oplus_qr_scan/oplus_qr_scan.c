...
static int qr_monitor_netlink_nlmsg_handle(struct sk_buff *skb,
...
static inline int qr_genl_msg_prepare_usr_msg(u8 cmd, size_t size, pid_t pid,
					      struct sk_buff **skbp)
{
	struct sk_buff *skb;
...
static inline int qr_genl_msg_mk_usr_msg(struct sk_buff *skb, int type,
					 void *data, int len)
{
	int ret;
...
/* send to user space */
static int oplus_qr_hooks_send_to_user(int msg_type, char *payload,
				       int payload_len)
{
	int ret = 0;
	void *head;
	...
	ret = qr_genl_msg_prepare_usr_msg(NETLINK_OPLUS_QR_HOOKS, size,
					  oplus_qr_netlink_pid, &skbuff);
	if (ret) {
		return ret;
	}
	...
	if (ret < 0) {
		printk(KERN_ERR
		       "oplus_qr_monitor: qr_monitor_netlink_send_to_user, can not unicast skbuff, ret = %d,android_pid pid=%d\n",
		       ret, oplus_qr_netlink_pid);
		return -1;
	}

	printk(" qr_monitor_netlink_send_to_user, skb_len=%u,android_pid pid=%d",
	       skbuff->len, oplus_qr_netlink_pid);
...
static void oplus_record_qr_ipv4_skb_info(int source, int dst, int saddr,
					  int daddr)
{
	if ((qr_ipv4_infos.ipv4_source) || (qr_ipv4_infos.ipv4_dst) ||
	    (qr_ipv4_infos.ipv4_saddr) || (qr_ipv4_infos.ipv4_daddr)) {
		return;
	}
...
static void oplus_record_qr_ipv6_skb_info(int source, int dst,
					  ...
					  struct in6_addr daddr)
{
	...
	if ((!ipv6_addr_any(&(qr_ipv6_infos.ipv6_saddr))) &&
	    (!ipv6_addr_any(&(qr_ipv6_infos.ipv6_daddr)))) {
		return;
	}
...
static bool oplus_check_qr_ipv4_skb_info(int source, int dst, int saddr,
					 int daddr)
{
	if ((!qr_ipv4_infos.ipv4_source) || (!qr_ipv4_infos.ipv4_dst) ||
	    (!qr_ipv4_infos.ipv4_saddr) || (!qr_ipv4_infos.ipv4_daddr)) {
	...
	if ((source == qr_ipv4_infos.ipv4_source) &&
	    (dst == qr_ipv4_infos.ipv4_dst) &&
	    (saddr = qr_ipv4_infos.ipv4_saddr) &&
	    (daddr = qr_ipv4_infos.ipv4_daddr)) {
		return true;
	}
...
static bool oplus_check_qr_ipv6_skb_info(int source, int dst,
					 struct in6_addr saddr,
					 struct in6_addr daddr)
{
	if ((!qr_ipv6_infos.ipv6_source) || (!qr_ipv6_infos.ipv6_dst)) {
	...
	if ((source == qr_ipv6_infos.ipv6_source) &&
	    (dst == qr_ipv6_infos.ipv6_dst) &&
	    (ipv6_addr_equal(&(qr_ipv6_infos.ipv6_saddr), &saddr)) &&
	    (ipv6_addr_equal(&(qr_ipv6_infos.ipv6_daddr), &daddr))) {
		return true;
	}
...
static unsigned int oplus_filter_qr_v4_send_skb(void *p, struct sk_buff *skb,
						const struct nf_hook_state *s)
{
	...
	if (strstr(userdata, "POST /mmtls/") != NULL) {
		oplus_record_qr_ipv4_skb_info(ipv4_qr_source, ipv4_qr_dst,
					      ipv4_qr_saddr, ipv4_qr_daddr);
...
static unsigned int
oplus_filter_qr_v4_receive_skb(void *p, struct sk_buff *skb,
			       const struct nf_hook_state *s)
{
	struct iphdr *iph = NULL;
	struct tcphdr *tcph = NULL;
	...
	if (!oplus_check_qr_ipv4_skb_info(ipv4_qr_dst, ipv4_qr_source,
					  ipv4_qr_daddr, ipv4_qr_saddr)) {
		return NF_ACCEPT;
	}
...
static unsigned int oplus_filter_qr_v6_send_skb(void *p, struct sk_buff *skb,
						const struct nf_hook_state *s)
{
	...
	if (skb->protocol == htons(ETH_P_IPV6) &&
	    (ipv6h = ipv6_hdr(skb)) != NULL && ipv6h->nexthdr == NEXTHDR_TCP) {
		...
		userdata = (unsigned char *)(skb->data +
					     header_len); /*tcp payload buffer*/
	...
	if (strstr(userdata, "POST /mmtls/") != NULL) {
		oplus_qr_hooks_send_to_user(QR_SCAN_POST_SUCCESS, NULL, 0);
		oplus_record_qr_ipv6_skb_info(ipv6_qr_source, ipv6_qr_dst,
...
static unsigned int
oplus_filter_qr_v6_receive_skb(void *p, struct sk_buff *skb,
			       const struct nf_hook_state *s)
{
	struct ipv6hdr *ipv6h = NULL;
	struct tcphdr *tcph = NULL;
	...
	if (!oplus_check_qr_ipv6_skb_info(ipv6_qr_dst, ipv6_qr_source,
...
static int __init oplus_qr_hook_init(void)
{
	...
	ret = nf_register_net_hooks(&init_net, filter_qr_skb,
				    ARRAY_SIZE(filter_qr_skb));
	if (ret < 0) {
...
{
	oplus_qr_hooks_netlink_exit();
	nf_unregister_net_hooks(&init_net, filter_qr_skb,
				ARRAY_SIZE(filter_qr_skb));
	printk("nf_hook_fini.");
}
