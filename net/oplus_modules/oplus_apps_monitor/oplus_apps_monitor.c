...
extern void statistics_monitor_apps_rtt_via_uid(int if_index, int rtt,
						struct sock *sk);
...
extern void oplus_app_monitor_update_app_info(struct sock *sk,
					      const struct sk_buff *skb,
					      int send, int retrans);
...
static rtt_params_thred g_rtt_params_thred = { DEFAULT_RTT_EXCE_THRED,
					       DEFAULT_RTT_GOOD_THRED,
					       DEFAULT_RTT_FAIR_THRED,
					       DEFAULT_RTT_POOR_THRED };
				      ...
				      int payload_len)
{
	...
	ret = genl_msg_prepare_usr_msg(APPS_MONITOR_CMD_UPLINK, size,
				       apps_monitor_netlink_pid, &skbuff);
	if (ret) {
		return ret;
	}
...
void update_app_rtt_count(int if_index, int sk_uid_index, int rtt)
{
	...
	if (rtt <= g_rtt_params_thred.rtt_exce_thred) {
		++(g_monitor_apps_table[sk_uid_index]
			   .app_rtt[if_index]
			   .rtt_exce_count);
...
void statistics_monitor_apps_rtt_via_uid(int if_index, int rtt, struct sock *sk)
{
	...
	if (rtt <= 0 || sk == NULL || if_index < 0 ||
	    if_index >= IFACE_NUM_MAX) {
		return;
	}
...
{
	apps_monitor_table_hrd = register_net_sysctl(
		&init_net, "net/oplus_apps_monitor", apps_monitor_sysctl_table);
	...
	for (i = 0; i < g_monitor_apps_num; ++i) {
		for (j = 0; j < IFACE_NUM_MAX; ++j) {
			printk("oplus_apps_monitor: print_apps_rtt_record, the uid = %d, the if_index = %d, RTT = %llu:%llu:%llu:%llu:%llu:%llu\n",
			       g_monitor_apps_table[i].app_uid, j,
...
static void print_dev_rtt_record(void)
{
	...
	for (i = 0; i < IFACE_NUM_MAX; ++i) {
		printk("oplus_apps_monitor: print_dev_rtt_record, the if_index = %d, RTT = %llu:%llu:%llu:%llu:%llu:%llu\n",
		       i, g_monitor_dev_table[i].rtt_exce_count,
...
#define MAX_RTT_MSG_LEN (2048)
	...
	case WLAN_INDEX:
		for (index = 0; index < g_monitor_apps_num; ++index) {
			...
			memcpy(send_msg + step,
			       &g_monitor_apps_table[index].app_uid, int_size);
			memcpy(send_msg + step + int_size,
			       &(g_monitor_apps_table[index]
					 .app_rtt[WLAN_INDEX]),
			       rtt_params_size);
		}

		ret = apps_monitor_netlink_send_to_user(
			APPS_MONITOR_REPORT_APPS_WLAN_RTT, (char *)send_msg,
			(rtt_params_size + int_size) * g_monitor_apps_num);
		break;
	...
	case CELL_INDEX:
		...
		ret = apps_monitor_netlink_send_to_user(
			APPS_MONITOR_REPORT_APPS_CELL_RTT, (char *)send_msg,
			(rtt_params_size + int_size) * g_monitor_apps_num);
		break;

	case ALL_IF_INDEX:
		for (index = 0; index < g_monitor_apps_num; ++index) {
			step = rtt_params_size * IFACE_NUM_MAX * index +
			       index * int_size;
			memcpy(send_msg + step,
			       &g_monitor_apps_table[index].app_uid, int_size);
			memcpy(send_msg + step + int_size,
			       &(g_monitor_apps_table[index].app_rtt),
			       rtt_params_size * IFACE_NUM_MAX);
		}

		ret = apps_monitor_netlink_send_to_user(
			APPS_MONITOR_REPORT_APPS_ALL_RTT, (char *)send_msg,
...
#define MAX_DEV_RTT_MSG_LEN (256)
	...
	for (index = 0; index < IFACE_NUM_MAX; ++index) {
		...
		memcpy(send_msg + step, &g_monitor_dev_table[index],
		       rtt_params_size);
	}

	ret = apps_monitor_netlink_send_to_user(
		APPS_MONITOR_REPORT_DEV_RTT, (char *)send_msg,
		rtt_params_size * IFACE_NUM_MAX);
...
static unsigned int
oplus_apps_monitor_postrouting_hook4(void *priv, struct sk_buff *skb,
...
static unsigned int
oplus_apps_monitor_input_hook4(void *priv, struct sk_buff *skb,
			       const struct nf_hook_state *state)
{
	struct sock *sk;
	struct iphdr *iph = NULL;
...
static unsigned int
oplus_apps_monitor_postrouting_hook6(void *priv, struct sk_buff *skb,
				     const struct nf_hook_state *state)
{
	struct sock *sk;
	struct tcphdr *tcph = NULL;
...
static unsigned int
oplus_apps_monitor_input_hook6(void *priv, struct sk_buff *skb,
			       const struct nf_hook_state *state)
{
	struct sock *sk;
	struct tcphdr *tcph = NULL;
...
static int __init oplus_apps_monitor_init(void)
{
	...
	ret = nf_register_net_hooks(&init_net, apps_monitor_netfilter_ops,
				    ARRAY_SIZE(apps_monitor_netfilter_ops));
	if (ret < 0) {
...
static void __exit oplus_apps_monitor_fini(void)
{
	...
	nf_unregister_net_hooks(&init_net, apps_monitor_netfilter_ops,
				ARRAY_SIZE(apps_monitor_netfilter_ops));
