...
static void print_report_info(struct app_monitor_ul_report_msg_st *report_msg)
{
	...
	printk("[app_monitor]:sys_total_packet=%llu,sys_send_packet=%llu,sys_recv_packet=%llu,sys_retrans_total_packet=%llu, sys_retrans_syn_packet=%llu\n",
	       ...
	       report_msg->sys_retrans_syn_packets);
...
void oplus_app_monitor_fill_beat_app_info(
	struct app_monitor_ul_report_msg_st *report_msg)
{
	int i, j, k;
	...
	for (i = 0; i < app_num; i++) {
		...
		for (j = 0; j < TOP_APP_NUM; j++) {
			if (top_app_beat_info.read_flag[j] ||
			    top_app_beat_info.app_beat_info[j].beat_period ==
				    0) {
				continue;
			}

			if (!max_count ||
			    max_count < top_app_beat_info.app_beat_info[j]
						.beat_count) {
				max_count = top_app_beat_info.app_beat_info[j]
						    .beat_count;
				k = j;
...
void oplus_app_monitor_fill_push_app_info(
	struct app_monitor_ul_report_msg_st *report_msg)
{
	int i, j, k;
	...
	for (i = 0; i < app_num; i++) {
		...
		for (j = 0; j < TOP_APP_NUM; j++) {
			if (top_app_push_info.read_flag[j] ||
			    top_app_push_info.app_push_info[j].push_period ==
				    0) {
				continue;
			}

			if (!max_count ||
			    max_count < top_app_push_info.app_push_info[j]
						.push_count) {
				max_count = top_app_push_info.app_push_info[j]
						    .push_count;
				k = j;
...
void oplus_app_monitor_fill_traffic_app_info(
	struct app_monitor_ul_report_msg_st *report_msg)
{
	int i, j, k;
	...
	for (i = 0; i < app_num; i++) {
		...
		for (j = 0; j < TOP_APP_NUM; j++) {
			if (top_app_traffic_info.read_flag[j] ||
			    top_app_traffic_info.app_traffic_info[j]
					    .total_bytes == 0) {
				continue;
			}
		...
		if (k < TOP_APP_NUM) {
			...
			report_msg->app_traffic_info[i].total_bytes =
				top_app_traffic_info.app_traffic_info[k]
					.total_bytes;
			report_msg->app_traffic_info[i].total_packets =
				top_app_traffic_info.app_traffic_info[k]
					.total_packets;
...
void oplus_app_monitor_fill_retrans_app_info(
	struct app_monitor_ul_report_msg_st *report_msg)
{
	int i, j, k;
	...
	for (i = 0; i < app_num; i++) {
		...
		for (j = 0; j < TOP_APP_NUM; j++) {
			if (top_app_retrans_info.read_flag[j] ||
			    top_app_retrans_info.app_retrans_info[j]
					    .retrans_total_packets == 0) {
				continue;
			}
		...
		if (k < TOP_APP_NUM) {
			...
			report_msg->app_retrans_info[i]
				.app_retrans_total_packets =
				top_app_retrans_info.app_retrans_info[k]
					.retrans_total_packets;
			report_msg->app_retrans_info[i].app_retrans_syn_packets =
				top_app_retrans_info.app_retrans_info[k]
					.retrans_syn_packets;
...
void oplus_app_monitor_send_app_info_report(void)
{
	...
	char msg_buf[1024] = { 0 };
	struct app_monitor_ul_report_msg_st *report_msg =
		(struct app_monitor_ul_report_msg_st *)msg_buf;
...
static void report_timer_start(void)
{
	...
	apps_monitor_report_timer.function =
		(void *)apps_monitor_report_timer_function;
	apps_monitor_report_timer.expires = jiffies + REPORT_PERIOD * HZ;
	mod_timer(&apps_monitor_report_timer,
		  apps_monitor_report_timer.expires);
}
...
static inline u32 oplus_app_monitor_find_app_traffic_index(u32 uid_val)
{
	...
	for (index = 0; index < app_num; index++) {
		if (uid_val ==
		    top_app_traffic_info.app_traffic_info[index].uid_val) {
			return index;
		}
	}
...
static inline u32 oplus_app_monitor_find_app_retrans_index(u32 uid_val)
{
	...
	for (index = 0; index < app_num; index++) {
		if (uid_val ==
		    top_app_retrans_info.app_retrans_info[index].uid_val) {
			return index;
		}
	}
...
void oplus_app_monitor_update_top_trafffic_app(
	struct app_monitor_app_info_st *app_info)
{
	...
	if (top_app_traffic_info.real_app_num < TOP_APP_NUM) {
		top_app_traffic_info
			.app_traffic_info[top_app_traffic_info.real_app_num] =
			*app_info;
	...
	for (i = 0; i < TOP_APP_NUM; i++) {
		...
		} else if (top_app_traffic_info.app_traffic_info[min_index]
				   .total_bytes >
...
void oplus_app_monitor_update_top_retrans_app(
	struct app_monitor_app_info_st *app_info)
{
	...
	if (top_app_retrans_info.real_app_num < TOP_APP_NUM) {
		top_app_retrans_info
			.app_retrans_info[top_app_retrans_info.real_app_num] =
			*app_info;
	...
	if (app_info->retrans_total_packets >
	    top_app_retrans_info.app_retrans_info[min_index]
		    .retrans_total_packets) {
	...
	for (i = 0; i < TOP_APP_NUM; i++) {
		...
		} else if (top_app_retrans_info.app_retrans_info[min_index]
				   .retrans_total_packets >
			   top_app_retrans_info.app_retrans_info[i]
				   .retrans_total_packets) {
			min_index = i;
		}
	}
...
void oplus_app_monitor_update_top_beat_app(
	struct app_monitor_app_info_st *app_info)
{
	...
	if (top_app_beat_info.real_app_num < TOP_APP_NUM) {
		top_app_beat_info.app_beat_info[top_app_beat_info.real_app_num] =
			*app_info;
...
static void
oplus_app_monitor_update_top_push_app(struct app_monitor_app_info_st *app_info)
{
	...
	if (top_app_push_info.real_app_num < TOP_APP_NUM) {
		top_app_push_info.app_push_info[top_app_push_info.real_app_num] =
			*app_info;
...
oplus_app_monitor_get_app_info_by_uid(u32 uid)
{
	...
	list_for_each_entry (app_info_node,
			     &app_monitor_list[hash_index].m_listhead,
			     list_node) {
	...
	app_info_node = kzalloc(
		sizeof(struct app_monitor_app_info_list_node_st), GFP_ATOMIC);

	if (!app_info_node) {
		return NULL;
	} else {
		app_info_node->app_info.uid_val = uid;
		list_add(&app_info_node->list_node,
			 &app_monitor_list[hash_index].m_listhead);
	}
...
void oplus_app_monitor_update_flow_stat(
	struct app_monitor_app_info_list_node_st *app_info_node,
...
void oplus_app_monitor_update_retrans_stat(
	struct app_monitor_app_info_list_node_st *app_info_node,
...
void oplus_app_monitor_send_beat_alarm(u32 uid_val, u32 period)
{
	...
	beat_alarm_msg->msg_len =
		sizeof(struct app_monitor_ul_beat_alarm_msg_st);
	ret = apps_monitor_netlink_send_to_user(
		APPS_POWER_MONITOR_MSG_UL_BEAT_ALARM, (char *)msg_buf,
		beat_alarm_msg->msg_len);
...
void oplus_app_monitor_send_push_alarm(u32 uid_val, u32 period)
{
	...
	push_alarm_msg->msg_len =
		sizeof(struct app_monitor_ul_push_alarm_msg_st);
	ret = apps_monitor_netlink_send_to_user(
		APPS_POWER_MONITOR_MSG_UL_PUSH_ALARM, (char *)msg_buf,
		push_alarm_msg->msg_len);
...
void oplus_app_monitor_send_traffic_alarm(
	struct app_monitor_app_info_st *app_info)
{
	int ret;
	char msg_buf[1024] = { 0 };
	...
	traffic_alarm_msg->msg_len =
		sizeof(struct app_monitor_ul_traffic_alarm_msg_st);
	...
	ret = apps_monitor_netlink_send_to_user(
		APPS_POWER_MONITOR_MSG_UL_TRAFFIC_ALARM, (char *)msg_buf,
		traffic_alarm_msg->msg_len);
				       ...
				       int retrans)
{
	...
	if (retrans) {
		...
		oplus_app_monitor_update_top_retrans_app(
	...
	if (send) {
		...
		app_samp_time =
			now_sec - app_info_node->app_info.last_active_time;

		if (app_samp_time >= PERIOD_CHECK_TIME &&
		    app_samp_time < REPORT_INTERVAL) {
			app_info_node->app_info.beat_period =
				(app_info_node->app_info.beat_period *
					 app_info_node->app_info.beat_count +
				 app_samp_time) /
				(app_info_node->app_info.beat_count + 1);

			if (!app_info_node->app_info.beat_count) {
				app_info_node->app_info.beat_alarm_start_time =
					now_sec;
			}

			app_info_node->app_info.beat_count++;
			oplus_app_monitor_update_top_beat_app(
				&app_info_node->app_info);

			if (app_info_node->app_info.beat_count %
				    ALARM_DETECT_COUNT ==
			    0) {
				int time_gap = now_sec -
					       app_info_node->app_info
						       .beat_alarm_start_time;
				int period = time_gap / ALARM_DETECT_COUNT;

				if (app_info_node->app_info
					    .beat_alarm_start_time &&
				    time_gap < ALARM_DETECT_PERIOD) {
					oplus_app_monitor_send_beat_alarm(
						app_info_node->app_info.uid_val,
						period);
				}
				app_info_node->app_info.beat_alarm_start_time =
					now_sec;
		...
		if (app_samp_time > PERIOD_CHECK_TIME &&
		    app_samp_time < REPORT_INTERVAL) {
			app_info_node->app_info.push_period =
				(app_info_node->app_info.push_period *
					 app_info_node->app_info.push_count +
				 app_samp_time) /
				(app_info_node->app_info.push_count + 1);

			if (!app_info_node->app_info.push_count) {
				app_info_node->app_info.push_alarm_start_time =
					now_sec;
			}

			app_info_node->app_info.push_count++;
			oplus_app_monitor_update_top_push_app(
				&app_info_node->app_info);
...
{
	struct app_monitor_dl_ctl_msg_st *dl_ctl_msg =
		(struct app_monitor_dl_ctl_msg_st *)NLA_DATA(nla);
	...
	if (dl_ctl_msg->msg_len != sizeof(struct app_monitor_dl_ctl_msg_st)) {
		printk("[app_monitor]:msg_len err, msg_len=%u, expected_len=%lu\n",
		       dl_ctl_msg->msg_len,
		       sizeof(struct app_monitor_dl_ctl_msg_st));
		return -EINVAL;
	}
					  ...
					  loff_t *ppos)
{
	...
	if (write && ret == 0) {
		...
		oplus_app_monitor_send_beat_alarm(
			top_app_beat_info.app_beat_info[index].uid_val,
			(u32)app_beat_alarm_period);
		printk("[app_monitor]:alarm:uid=%u,beat_period=%d",
		       top_app_beat_info.app_beat_info[index].uid_val,
		       app_beat_alarm_period);
	}
					  ...
					  loff_t *ppos)
{
	...
	if (write && ret == 0) {
		...
		oplus_app_monitor_send_push_alarm(
			top_app_push_info.app_push_info[index].uid_val,
			(u32)app_push_alarm_period);
		printk("[app_monitor]:alarm:uid=%u,push_period=%d",
		       top_app_push_info.app_push_info[index].uid_val,
		       app_push_alarm_period);
	}
...
{
	app_power_monitor_table_hrd =
		register_net_sysctl(&init_net, "net/oplus_app_power_monitor",
				    app_power_monitor_sysctl_table);
...
void oplus_app_power_monitor_init(void)
{
	...
	memset(&top_app_beat_info, 0,
	       sizeof(struct app_monitor_top_app_beat_info_st));
	memset(&top_app_push_info, 0,
	       sizeof(struct app_monitor_top_app_push_info_st));
