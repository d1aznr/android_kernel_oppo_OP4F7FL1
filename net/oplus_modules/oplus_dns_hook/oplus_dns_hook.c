...
static int parse_querys(char *start, int *querys_len, query_info_t *queryList,
	...
	for (i = 0; i < query_size; i++) {
		...
		do {
			if (tmpLen > (urlLen - len)) {
				LOGK(1, "len invalid! %d-%lu-%d", tmpLen,
				     urlLen, len);
		...
		for (j = 0; j < MAX_DNS_HOOK_SIZE; j++) {
			if (s_hook_addrs[j].inuse) {
				if (strcmp(tmpurl, s_hook_addrs[j].url) == 0) {
					...
					LOGK(1,
					     "hook %d parse_querys match %d-%d-%s-%u",
					     hook, i, j, tmpurl,
					     queryList->addr);
					queryList += 1;
					matchSize++;
				   ...
				   query_info_t *query, int match_count)
{
	...
	for (i = 0; i < match_count; i++) {
		ans_item_hdr_t *hdr =
			(ans_item_hdr_t *)(ans_buf + IPV4_ANSWER_ITEM_LEN * i);
	...
	if (skb_rsp->protocol == htons(ETH_P_IP)) {
		...
		ipv6h->daddr = tmp;
		ipv6h->payload_len =
			htons(ntohs(ipv6h->payload_len) + answer_len);
	}
	// update udp
	...
	udph_rsp->check = 0;
	leftpayload = ntohs(udph_rsp->len) - sizeof(struct udphdr) -
		      sizeof(dnshdr_t) - query_len - answer_len;
	LOGK(0, "udplen: %d, leftpayload value %d, queryLen:%d",
	     ntohs(udph_rsp->len), leftpayload, query_len);
	// update answer
	dnshdr = (dnshdr_t *)(skb_transport_header(skb_rsp) +
	...
	if (leftpayload != 0) {
		...
		ret = skb_copy_bits(skb_rsp,
				    skb_transport_offset(skb_rsp) +
					    sizeof(struct udphdr) +
					    sizeof(dnshdr_t) + query_len,
				    left_buf, leftpayload);
		if (ret) {
		...
		ret = skb_store_bits(
			skb_rsp,
			skb_transport_offset(skb_rsp) + sizeof(struct udphdr) +
				sizeof(dnshdr_t) + query_len + answer_len,
			left_buf, leftpayload);
		if (ret) {
...
static int dns_hook_process_postrouting(struct sk_buff *skb, int hook,
					const struct nf_hook_state *state)
{
	...
	dnshdr =
		(dnshdr_t *)(skb_transport_header(skb) + sizeof(struct udphdr));
	...
	ret = parse_querys((char *)(dnshdr + 1), &query_len, query_list,
	...
	if (skb->protocol == htons(ETH_P_IP)) {
		...
		if (ip_route_me_harder(state->net, state->sk, rsp_skb,
				       RTN_UNSPEC)) {
#else
...
static unsigned int
oplus_dns_hook_output_hook(void *priv, struct sk_buff *skb,
			   const struct nf_hook_state *state)
{
	...
	for (i = 0; i < MAX_DNS_HOOK_SIZE; i++) {
		if (s_hook_addrs[i].inuse) {
			if (strncmp(data + 1, s_hook_addrs[i].url, len) == 0) {
				...
				LOGK(1, "update url %s %u", s_hook_addrs[i].url,
				     s_hook_addrs[i].addr);
				is_modify = 1;
				break;
			}
...
static int del_dns_hook_url(struct nlattr *nla)
{
	...
	for (i = 0; i < MAX_DNS_HOOK_SIZE; i++) {
		if (s_hook_addrs[i].inuse) {
			if (strncmp(data + 1, s_hook_addrs[i].url, len) == 0) {
				LOGK(1, "del url %s %u", s_hook_addrs[i].url,
				     s_hook_addrs[i].addr);
				memset(&s_hook_addrs[i], 0,
				       sizeof(dns_hook_data_t));
...
static int clear_dns_hook_url(struct nlattr *nla)
{
	...
	for (i = 0; i < MAX_DNS_HOOK_SIZE; i++) {
		if (s_hook_addrs[i].inuse) {
			LOGK(1, "clear url %s %u", s_hook_addrs[i].url,
			     s_hook_addrs[i].addr);
					  ...
					  struct genl_info *info)
{
	...
	LOGK(0, "set s_user_pid=%u type=%u len=%u.", nlhdr->nlmsg_pid,
	     nla->nla_type, nla->nla_len);
	if (s_user_pid == 0) {
		s_user_pid = nlhdr->nlmsg_pid;
	} else if (s_user_pid != nlhdr->nlmsg_pid) {
		LOGK(1, "user pid changed!! %u - %u", s_user_pid,
		     nlhdr->nlmsg_pid);
		s_user_pid = nlhdr->nlmsg_pid;
	}
...
static int send_netlink_data(int type, char *data, int len)
{
	...
	ret = genl_msg_prepare_usr_msg(OPLUS_DNS_HOOK_CMD_NOTIFY, size,
				       s_user_pid, &skbuff);
	if (ret) {
		return ret;
	}
...
{
	oplus_dns_hook_table_hdr = register_net_sysctl(
		&init_net, "net/oplus_dns_hook", oplus_net_hook_sysctl_table);
...
static int __init oplus_dns_hook_init(void)
{
	...
	if (ret < 0) {
		LOGK(1, "oplus_dns_hook_init netfilter register failed, ret=%d",
		     ret);
