...
struct connection_ids {
	__be32 src_ip;
	int32_t recently_ids
		[DEDUP_IDS_LEN]; /*use 32-bit and init with -1, to avoid 0-id ambiguity*/
		...
		for (i = 0; i < DEDUP_IDS_LEN; i++) {
			if (g_id_list[cid].recently_ids[i] == ip_hdr(skb)->id) {
				...
				debug("drop id = %u, drop = %u, recv = %u\n",
				      ntohs(ip_hdr(skb)->id),
				      g_dup_pkt_drop_cnt, g_dup_pkt_recv_cnt);
