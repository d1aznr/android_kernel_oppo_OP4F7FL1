...
#define MAC_MASK(addr) addr[0], addr[1], addr[4], addr[5]
...
static int oplus_dhcp_send_to_user(int msg_type, char *payload, int payload_len)
{
	...
	/* send data */
	if (oplus_dhcp_pid) {
		ret = netlink_unicast(oplus_dhcp_sock, skbuff, oplus_dhcp_pid,
				      MSG_DONTWAIT);
	} else {
		printk(KERN_ERR
	...
	if (ret < 0) {
		printk(KERN_ERR
		       "oplus_dhcp_hooks_netlink: can not unicast skbuff,ret = %d\n",
		       ret);
		return 1;
	}
						...
						char *server_mac)
{
	...
	if (wlan_iface != NULL) {
		if (0 == memcmp(wlan_iface, MAIN_WLAN_IFACE,
				strlen(MAIN_WLAN_IFACE))) {
	...
	oplus_dhcp_send_to_user(OPLUS_DHCP_NOTIFY_DUP_OFFER_EVENT,
...
static int get_iface_index(char *dev_name)
{
	...
	if (0 ==
	    memcmp(dev_name, SECOND_WLAN_IFACE, strlen(SECOND_WLAN_IFACE))) {
...
static bool blackwhite_list_match(struct net_device *dev, __be32 server_id)
{
	...
	if (oplus_dhcp_expect_list[wlan_index].wlan_index >= 0) {
		is_list_empty = false;
		if (oplus_dhcp_expect_list[wlan_index].server_addr ==
		    server_id) {
...
static int handle_dhcp_packet(struct sock *sk, struct sk_buff *skb,
			      struct net_device *dev, struct packet_type *pt)
{
	struct dhcp_pkt *b;
	struct iphdr *iph;
	...
	if (ext_len >= 4) {
		...
		case DHCPNAK:
		case DHCPACK:
			if (memcmp(dev->dev_addr, b->hw_addr, dev->addr_len) !=
			    0) {
			...
			if (oplus_dhcp_get_notify_state(dev)) {
				...
				memcpy(server_mac, eth_hdr(skb)->h_source,
				       ETH_ALEN);
				send_dhcp_offer_packet_to_userspace(
					dev->name, server_id, server_mac);
			}
...
static int oplus_dhcp_netlink_init(void)
{
	...
	oplus_dhcp_sock =
		netlink_kernel_create(&init_net, NETLINK_OPLUS_DHCP, &cfg);
...
{
	oplus_dhcp_hooks_table_hrd =
		register_net_sysctl(&init_net, "net/oplus_dhcp_hooks",
...
static int __init oplus_dhcp_init(void)
{
	...
	memset(oplus_dhcp_drop_list, 0,
	       sizeof(struct dhcp_server) * IFACE_COUNT);
	memset(oplus_dhcp_expect_list, 0,
	       sizeof(struct dhcp_server) * IFACE_COUNT);
