...
static void
tcp_output_tcpsynretrans_hook_work_callback(struct work_struct *work);
static void
tcp_input_tcpsynretrans_hook_work_callback(struct work_struct *work);
...
static atomic_t tcp_is_input =
	ATOMIC_INIT(0); //1=v4_input,2=v6_input,3=output,0=default
...
static void nwpower_unsl_blacklist_reject()
{
	if (record_blacklist_reject_index > 0) {
		...
		nwpower_send_to_user(NW_POWER_REPORT_BLACK_REJECT,
				     (char *)blacklist_reject_uid,
...
extern bool oplus_check_socket_in_blacklist(int is_input, struct socket *sock)
{
	...
	if (blacklist_len > 0 && sock && sock->sk &&
	    (sock->sk->sk_family == 2 || sock->sk->sk_family == 10)) {
		...
		for (i = 0; i < blacklist_len; i++) {
			if (blacklist_uid[i] == uid) {
				if (is_input != 1) {
					if (record_blacklist_reject_index >=
					    OPLUS_MAX_RECORD_BLACK_REJECT_LEN) {
						nwpower_unsl_blacklist_reject();
					}
					if (record_blacklist_reject_index <
					    OPLUS_MAX_RECORD_BLACK_REJECT_LEN) {
						blacklist_reject_uid
...
extern void oplus_match_qrtr_service_port(int type, int id, int port)
{
	if (type == QRTR_TYPE_NEW_SERVER) {
		...
		match_qrtr_new_service_port(id, port,
					    mdaci_service_wakeup_times);
...
{
	return scnprintf(desc, size > BTS_BUFFER_SIZE ? BTS_BUFFER_SIZE : size,
				      ...
				      u64 wakeup[OPLUS_NW_WAKEUP_SUM], bool prt)
{
	...
	if (atomic_read(&qrtr_wakeup_hook_boot) == 1 &&
		...
		for (i = 0; i < OPLUS_MAX_QRTR_SERVICE_LEN; ++i) {
			if (qrtr[i][0] == 1 && (qrtr[i][2] == src_port ||
						qrtr[i][2] == dst_port)) {
				if (repeat_index < 4)
		...
		if (repeat_index == 1) {
			...
			if (prt)
				printk("[oplus_nwpower] QrtrWakeup: ServiceID: %d, NodeID: %d, PortID: %d, Msg: [%08x %08x], Count: %d",
				       qrtr[repeat[0]][1], src_node,
				       qrtr[repeat[0]][2], arg1, arg2,
				       qrtr[repeat[0]][3]);
			...
			if (prt)
				printk("[oplus_nwpower] QrtrWakeup: ServiceID: [%d/%d/%d/%d], NodeID: %d, PortID: %d, Msg: [%08x %08x], Count: %d",
				       ...
				       repeat_index > 2 ? qrtr[repeat[2]][1] :
							  -1,
				       repeat_index > 3 ? qrtr[repeat[3]][1] :
							  -1,
				       src_node,
				       qrtr[repeat[repeat_index - 1]][2], arg1,
				       arg2, qrtr[repeat[repeat_index - 1]][3]);
		} else {
			if (prt)
				printk("[oplus_nwpower] QrtrWakeup: ServiceID: %d, NodeID: %d, PortID: %d, Msg: [%08x %08x], Count: %d",
			      ...
			      u64 wakeup[OPLUS_NW_WAKEUP_SUM], bool prt)
{
	...
	if (prt)
		printk("[oplus_nwpower] AllWakeups: Mpss: %d, Qrtr: %d, Modem: %d, WiFi: %d",
		       wakeup[OPLUS_NW_MPSS], wakeup[OPLUS_NW_QRTR],
		       wakeup[OPLUS_NW_MD], wakeup[OPLUS_NW_WIFI]);
}
			...
			if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) -
				...
				if (type == OPLUS_TCP_TYPE_V4) {
					...
					atomic_set(&tcp_is_input,
						   OPLUS_TCP_TYPE_V4);
					tcp_input_list.ipv4_addr =
						tmp_v4iph->saddr;
					tcp_input_list.is_ipv6 = false;
					mdaci_tcp_input_list.ipv4_addr =
						tcp_input_list.ipv4_addr;
					...
					tcp_input_list.ipv6_addr1 =
						(u64)ntohl(
							tmp_v6iph->saddr
								.s6_addr32[0])
							<< 32 |
						ntohl(tmp_v6iph->saddr
							      .s6_addr32[1]);
					tcp_input_list.ipv6_addr2 =
						(u64)ntohl(
							tmp_v6iph->saddr
								.s6_addr32[2])
							<< 32 |
					...
					mdaci_tcp_input_list.ipv6_addr1 =
						tcp_input_list.ipv6_addr1;
					mdaci_tcp_input_list.ipv6_addr2 =
						tcp_input_list.ipv6_addr2;
...
{
	if (atomic_read(&ipa_wakeup_hook_boot) == 1 &&
	    atomic_read(&tcp_is_input) == 1 && !tcp_input_sch_work) {
		schedule_work(&tcp_input_hook_work);
		tcp_input_sch_work = true;
	}
}
		...
		if (atomic_read(&tcp_is_input) == 0) {
			...
			if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) -
				...
				if (sk->sk_v6_daddr.s6_addr32[0] == 0 &&
					...
					tcp_output_list.ipv4_addr =
						sk->sk_daddr;
					tcp_output_list.is_ipv6 = false;
					mdaci_tcp_output_list.ipv4_addr =
						tcp_output_list.ipv4_addr;
					...
					tcp_output_list.ipv6_addr1 =
						(u64)ntohl(
							sk->sk_v6_daddr
								.s6_addr32[0])
							<< 32 |
						ntohl(sk->sk_v6_daddr
							      .s6_addr32[1]);
					tcp_output_list.ipv6_addr2 =
						(u64)ntohl(
							sk->sk_v6_daddr
								.s6_addr32[2])
							<< 32 |
					...
					mdaci_tcp_output_list.ipv6_addr1 =
						tcp_output_list.ipv6_addr1;
					mdaci_tcp_output_list.ipv6_addr2 =
						tcp_output_list.ipv6_addr2;
...
extern void oplus_match_tcp_input_retrans(struct sock *sk)
{
	...
	if (atomic_read(&tcpsynretrans_hook_boot) == 1) {
		now_ts = current_kernel_time();
		if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) -
		     sk->oplus_last_rcv_stamp[0]) >
		    OPLUS_TCP_RETRANSMISSION_INTERVAL) {
			if (sk->sk_v6_daddr.s6_addr32[0] == 0 &&
			    sk->sk_v6_daddr.s6_addr32[1] == 0) {
				tcp_input_retrans_list.ipv4_addr = sk->sk_daddr;
				tcp_input_retrans_list.is_ipv6 = false;
				mdaci_tcp_input_retrans_list.ipv4_addr =
					tcp_input_retrans_list.ipv4_addr;
				mdaci_tcp_input_retrans_list.is_ipv6 = false;
			} else {
				tcp_input_retrans_list.ipv6_addr1 =
					(u64)ntohl(sk->sk_v6_daddr.s6_addr32[0])
						<< 32 |
					ntohl(sk->sk_v6_daddr.s6_addr32[1]);
				tcp_input_retrans_list.ipv6_addr2 =
					(u64)ntohl(sk->sk_v6_daddr.s6_addr32[2])
						<< 32 |
					ntohl(sk->sk_v6_daddr.s6_addr32[3]);
				tcp_input_retrans_list.is_ipv6 = true;
				mdaci_tcp_input_retrans_list.ipv6_addr1 =
					tcp_input_retrans_list.ipv6_addr1;
				mdaci_tcp_input_retrans_list.ipv6_addr2 =
					tcp_input_retrans_list.ipv6_addr2;
			...
			mdaci_tcp_input_retrans_list.uid =
				tcp_input_retrans_list.uid;
			mdaci_tcp_input_retrans_list.pid =
				tcp_input_retrans_list.pid;
...
extern void oplus_match_tcp_output_retrans(struct sock *sk)
{
	...
	if (atomic_read(&tcpsynretrans_hook_boot) == 1) {
		now_ts = current_kernel_time();
		if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) -
		     sk->oplus_last_send_stamp[0]) >
		    OPLUS_TCP_RETRANSMISSION_INTERVAL) {
			if (sk->sk_v6_daddr.s6_addr32[0] == 0 &&
			    sk->sk_v6_daddr.s6_addr32[1] == 0) {
				tcp_output_retrans_list.ipv4_addr =
					sk->sk_daddr;
				tcp_output_retrans_list.is_ipv6 = false;
				mdaci_tcp_output_retrans_list.ipv4_addr =
					tcp_output_retrans_list.ipv4_addr;
				...
				tcp_output_retrans_list.ipv6_addr1 =
					(u64)ntohl(sk->sk_v6_daddr.s6_addr32[0])
						<< 32 |
					ntohl(sk->sk_v6_daddr.s6_addr32[1]);
				tcp_output_retrans_list.ipv6_addr2 =
					(u64)ntohl(sk->sk_v6_daddr.s6_addr32[2])
						<< 32 |
					ntohl(sk->sk_v6_daddr.s6_addr32[3]);
				tcp_output_retrans_list.is_ipv6 = true;
				mdaci_tcp_output_retrans_list.ipv6_addr1 =
					tcp_output_retrans_list.ipv6_addr1;
				mdaci_tcp_output_retrans_list.ipv6_addr2 =
					tcp_output_retrans_list.ipv6_addr2;
			...
			mdaci_tcp_output_retrans_list.uid =
				tcp_output_retrans_list.uid;
			mdaci_tcp_output_retrans_list.pid =
				tcp_output_retrans_list.pid;
...
static void tcp_hook_insert_sort(struct tcp_hook_struct *pval)
{
	...
	for (i = 1; i < OPLUS_MAX_RECORD_IP_LEN; ++i) {
		...
		//If IPv4
		if (temp_sort[0] == 0 && temp_sort[1] == 0 &&
		    temp_sort[2] != 0) {
	...
	for (i = 0; i < OPLUS_MAX_RECORD_IP_LEN; ++i) {
		if (pval->is_ipv6) {
			if (pval->ipv6_addr1 == pval->set[3 * i] &&
				...
				count = 1;
				pval->set[3 * i + 2] =
					(u64)(pval->uid) << 32 | (u32)(count);
				handle = true;
				break;
			}
		} else {
			if (pval->ipv4_addr ==
			    (pval->set[3 * i + 2] & 0xFFFFFFFF)) {
				count = (pval->set[3 * i + 2] &
					 0xFFFC000000000000) >>
					50;
				uid = (pval->set[3 * i + 2] &
				       0x3FFFF00000000) >>
...
static void nwpower_unsl_app_wakeup()
{
	nwpower_send_to_user(NW_POWER_REPORT_APP_WAKEUP,
			     (char *)app_wakeup_monitor_list.set,
			     sizeof(app_wakeup_monitor_list.set));
	app_wakeup_monitor_list.count = 0;
	memset(app_wakeup_monitor_list.set, 0x0,
	       sizeof(app_wakeup_monitor_list.set));
}
			       ...
			       bool is_block, bool is_input, int pid, int uid)
{
	...
	if (pval->count < OPLUS_MAX_RECORD_APP_WAKEUP_LEN) {
		...
		pval->set[1 + pval->count * 3 + 1] =
			((u64)block << 48) | ((u64)(input & 0xFFFF) << 32) |
			((u64)(whitelist & 0xFFFF) << 16) | (0xFFFF);
		pval->set[1 + pval->count * 3 + 2] =
			now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000;
		/*
...
static void tcp_output_hook_work_callback(struct work_struct *work)
{
	...
	app_wakeup_monitor(&app_wakeup_monitor_list, false, false,
	...
	if (tcp_output_list.is_ipv6) {
		printk("[oplus_nwpower] IPAOutputWakeup: [%ld,****], %d, %d, %d",
		       tcp_output_list.ipv6_addr1, tcp_output_list.pid,
		       tcp_output_list.uid,
		...
		printk("[oplus_nwpower] IPAOutputWakeup: %#X, %d, %d, %d",
		       tcp_output_list.ipv4_addr & 0xFFFFFF,
		       tcp_output_list.pid, tcp_output_list.uid,
		       (tcp_output_list.set[3 * i + 2] & 0xFFFC000000000000) >>
			       50);
		snprintf(bts_net_wakeup_buffer, BTS_BUFFER_SIZE, "ipa");
	}
...
static void tcp_input_hook_work_callback(struct work_struct *work)
{
	...
	app_wakeup_monitor(&app_wakeup_monitor_list, false, true,
	...
	if (tcp_input_list.is_ipv6) {
		printk("[oplus_nwpower] IPAInputWakeup: [%ld,****], %d, %d, %d",
		       tcp_input_list.ipv6_addr1, tcp_input_list.pid,
		       tcp_input_list.uid,
		       tcp_input_list.set[3 * i + 2] & 0xFFFFFFFF);
		snprintf(bts_net_wakeup_buffer, BTS_BUFFER_SIZE, "ipa");
	} else {
		printk("[oplus_nwpower] IPAInputWakeup: %#X, %d, %d, %d",
		       tcp_input_list.ipv4_addr & 0xFFFFFF, tcp_input_list.pid,
		       tcp_input_list.uid,
		       (tcp_input_list.set[3 * i + 2] & 0xFFFC000000000000) >>
			       50);
		snprintf(bts_net_wakeup_buffer, BTS_BUFFER_SIZE, "ipa");
	}
	atomic_set(&tcp_is_input, 0);
...
tcp_output_tcpsynretrans_hook_work_callback(struct work_struct *work)
{
	...
	if (tcp_output_retrans_list.is_ipv6) {
		...
		printk("[oplus_nwpower] TCPOutputRetrans: %#X, %d, %d, %d",
		       tcp_output_retrans_list.ipv4_addr & 0xFFFFFF,
		       tcp_output_retrans_list.pid, tcp_output_retrans_list.uid,
		       (tcp_output_retrans_list.set[3 * i + 2] &
			0xFFFC000000000000) >>
			       50);
	}
}
...
static void tcp_input_tcpsynretrans_hook_work_callback(struct work_struct *work)
{
	...
	if (tcp_input_retrans_list.is_ipv6) {
		...
		printk("[oplus_nwpower] TCPInputRetrans: %#X, %d, %d, %d",
		       tcp_input_retrans_list.ipv4_addr & 0xFFFFFF,
		       tcp_input_retrans_list.pid, tcp_input_retrans_list.uid,
		       (tcp_input_retrans_list.set[3 * i + 2] &
			0xFFFC000000000000) >>
			       50);
	}
}
			    ...
			    bool prt)
{
	...
	tcp_hook_insert_sort(pval);
	for (i = 0; i < 5; ++i) {
		if (pval->set[3 * i] == 0 && pval->set[3 * i + 1] == 0 &&
		    pval->set[3 * i + 2] != 0) {
			count = (pval->set[3 * i + 2] & 0xFFFC000000000000) >>
				50;
			if (prt && count > 0) {
...
static void print_ipa_wakeup(bool unsl, struct tcp_hook_struct *ptcp_in,
			     ...
			     struct tcp_hook_struct *ptcp_re_in,
			     struct tcp_hook_struct *ptcp_re_out,
			     u64 wakeup[OPLUS_NW_WAKEUP_SUM], bool prt)
{
	wakeup[OPLUS_NW_TCP_IN] = print_tcp_wakeup("Input", ptcp_in, prt);
	wakeup[OPLUS_NW_TCP_OUT] = print_tcp_wakeup("Output", ptcp_out, prt);
	wakeup[OPLUS_NW_TCP_RE_IN] =
		print_tcp_wakeup("InputRetrans", ptcp_re_in, prt);
	wakeup[OPLUS_NW_TCP_RE_OUT] =
		print_tcp_wakeup("OutputRetrans", ptcp_re_out, prt);
	if (prt)
		printk("[oplus_nwpower] IPAAllWakeups: TCPInput: %d, TCPOutput: %d, TCPInputRetrans: %d, TCPOutputRetrans: %d",
		       wakeup[OPLUS_NW_TCP_IN], wakeup[OPLUS_NW_TCP_OUT],
		       wakeup[OPLUS_NW_TCP_RE_IN], wakeup[OPLUS_NW_TCP_RE_OUT]);
	if (unsl) {
		wakeup_unsl_msg[6] =
			(wakeup[OPLUS_NW_TCP_IN] << 48) |
...
static void nwpower_hook_off(bool unsl)
{
	...
	print_ipa_wakeup(unsl, &tcp_input_list, &tcp_output_list,
			 &tcp_input_retrans_list, &tcp_output_retrans_list,
			 oplus_nw_wakeup, true);
	...
	memset(app_wakeup_monitor_list.set, 0x0,
	       sizeof(app_wakeup_monitor_list.set));
	if (unsl) {
		nwpower_send_to_user(NW_POWER_UNSL_MONITOR,
				     (char *)wakeup_unsl_msg,
...
{
	print_qrtr_wakeup(true, mdaci_service_wakeup_times,
			  oplus_mdaci_nw_wakeup, true);
	print_ipa_wakeup(true, &mdaci_tcp_input_list, &mdaci_tcp_output_list,
			 &mdaci_tcp_input_retrans_list,
			 &mdaci_tcp_output_retrans_list, oplus_mdaci_nw_wakeup,
			 true);
...
static int nwpower_send_to_user(int msg_type, char *msg_data, int msg_len)
{
	...
	ret = netlink_unicast(oplus_nwpower_sock, skb, oplus_nwpower_pid,
			      MSG_DONTWAIT);
	if (ret < 0) {
		printk(KERN_ERR
		       "[oplus_nwpower] netlink: netlink_unicast failed, ret = %d.\n",
		       ret);
		return -4;
	}
	return 0;
				   ...
				   struct netlink_ext_ack *extack)
{
	...
	case NW_POWER_ANDROID_PID:
		...
		printk("[oplus_nwpower] netlink: oplus_nwpower_pid = %d",
		       oplus_nwpower_pid);
		break;
...
static int nwpower_netlink_init(void)
{
	...
	oplus_nwpower_sock = netlink_kernel_create(
		&init_net, NETLINK_OPLUS_NWPOWERSTATE, &cfg);
