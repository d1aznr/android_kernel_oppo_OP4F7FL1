				       ...
				       int payload_len)
{
	...
	if (oplus_nf_hooks_pid) {
		ret = netlink_unicast(oplus_nf_hooks_sock, skbuff,
				      oplus_nf_hooks_pid, MSG_DONTWAIT);
	} else {
		printk(KERN_ERR
	...
	if (ret < 0) {
		printk(KERN_ERR
		       "oplus_nf_hooks_netlink: can not unicast skbuff,ret = %d\n",
		       ret);
		return 1;
	}
...
*/
static unsigned int oplus_nf_hooks_lm_detect(void *priv, struct sk_buff *skb,
		...
		if (is_wechat_skb(ct, skb)) {
			...
			for (i = 0; i < wechat_param_count; i++) {
				if (tot_len >= wechat_infos[i].min_tot_len &&
					...
					if (memcmp(payload + wechat_infos[i]
								     .offset,
						   wechat_infos[i].fixed_value,
						   wechat_infos[i].len) == 0) {
						printk("oplus_nf_hooks_lm:i=%d received hong bao...\n",
						       i);
						oplus_nf_hooks_send_to_user(
							NF_HOOKS_LM_DETECTED,
							NULL, 0);
						break;
					} else {
						if (nf_hooks_debug)
...
*/
static unsigned int
oplus_nf_hooks_v6_lm_detect(void *priv, struct sk_buff *skb,
			    const struct nf_hook_state *state)
{
	struct nf_conn *ct = NULL;
	enum ip_conntrack_info ctinfo;
	...
	if (skb->protocol == htons(ETH_P_IPV6) &&
		...
		if (is_wechat_skb(ct, skb)) {
			...
			ihl = ipv6_skip_exthdr(skb, sizeof(struct ipv6hdr),
					       &ip_proto,
...
{
	oplus_nf_hooks_table_hrd = register_net_sysctl(
		&init_net, "net/oplus_nf_hooks", oplus_nf_hooks_sysctl_table);
...
static int oplus_nf_hooks_set_wechat_param(struct nlmsghdr *nlh)
{
	...
	if (nlh->nlmsg_len ==
	    NLMSG_HDRLEN + 2 * sizeof(u32) +
		    wechat_param_count * sizeof(struct wechat_pattern_info)) {
		for (i = 0; i < wechat_param_count && i < MAX_WECHAT_PARAMS;
		     i++) {
			info = (struct wechat_pattern_info *)(data + 2) + i;
			memset(&(wechat_infos[i]), 0,
			       sizeof(struct wechat_pattern_info));
			...
			memcpy(wechat_infos[i].fixed_value, info->fixed_value,
			       info->len);
			if (nf_hooks_debug) {
				printk("oplus_nf_hooks_set_wechat_param i=%d uid=%d,max=%d,min=%d,offset=%d,len=%d,value=",
				       i, wechat_uid,
				       wechat_infos[i].max_tot_len,
				       wechat_infos[i].min_tot_len,
				       wechat_infos[i].offset,
				       wechat_infos[i].len);
				for (j = 0; j < wechat_infos[i].len; j++) {
					printk("%d -> %02x  ", j,
					       wechat_infos[i].fixed_value[j]);
				}
				printk("\n");
			}
		}
...
static int oplus_nf_hooks_netlink_init(void)
{
	...
	oplus_nf_hooks_sock =
		netlink_kernel_create(&init_net, NETLINK_OPLUS_NF_HOOKS, &cfg);
