...
static int oplus_smart_bw_set_params(struct nlmsghdr *nlh)
{
	...
	if (count == sizeof(struct oplus_smart_bw_rom_update_info)) {
		...
		if (smart_bw_rom_update_info.debug_level == 2 &&
		    oplus_sla_debug) {
		...
		printk("oplus_smart_bw_set_params:set params invalid param count:%d",
		       count);
	}
#endif
	return 0;
...
static int oplus_sla_send_to_user(int msg_type, char *payload, int payload_len)
{
	...
	/* send data */
	ret = netlink_unicast(oplus_sla_sock, skbuff, oplus_sla_pid,
			      MSG_DONTWAIT);
	if (ret < 0) {
		printk(KERN_ERR
		       "oplus_sla_netlink: can not unicast skbuff,ret = %d\n",
		       ret);
		return 1;
	}
...
static void calc_network_congestion(void)
{
	...
	if (oplus_sla_info[MAIN_WLAN].wlan_score <=
	    rom_update_info.wlan_bad_score) {
		rtt_num = 1;
	}
	...
	for (i = 0; i < IFACE_NUM; i++) {
		if (oplus_sla_info[i].if_up) {
			if (oplus_sla_info[i].sla_rtt_num >= rtt_num) {
				avg_rtt = 0;
				if (oplus_sla_info[i].download_flag <
				    DOWNLOAD_FLAG) {
					avg_rtt =
						oplus_sla_info[i].sla_sum_rtt /
				...
				if (oplus_sla_debug) {
					printk("oplus_sla_rtt: sla rtt ,num = %d,sum = %d,avg rtt[%d] = %d\n",
					       oplus_sla_info[i].sla_rtt_num,
					       oplus_sla_info[i].sla_sum_rtt, i,
					       avg_rtt);
				}
				...
				if (avg_rtt >= BACK_OFF_RTT_2) {
					oplus_sla_info[i].congestion_flag =
						CONGESTION_LEVEL_HIGH;
...
static void init_dual_wifi_weight(void)
{
	...
	printk("oplus_sla_weight:init_dual_wifi_weight [%d] [%d]",
	       ...
	       oplus_sla_info[SECOND_WLAN].weight);
}
...
static void init_wifi_cell_weight(void)
{
	...
	printk("oplus_sla_weight:init_wifi_cell_weight [%d] [%d]",
	       oplus_sla_info[MAIN_WLAN].weight,
	       oplus_sla_info[CELL_INDEX].weight);
}
...
static int find_dev_index_by_mark(__u32 mark)
{
	...
	for (i = 0; i < IFACE_NUM; i++) {
		if (oplus_sla_info[i].if_up && mark == oplus_sla_info[i].mark) {
			return i;
		}
	}
...
static bool dst_is_lan_ip(struct sk_buff *skb)
{
	...
	if (NULL != iph) {
		...
		if ((10 == dstip[0]) || (192 == dstip[0] && 168 == dstip[1]) ||
					     ...
					     struct sk_buff *skb)
{
	...
	if ((iph = ip_hdr(skb)) != NULL && iph->protocol == IPPROTO_TCP) {
		...
		if (NULL != th && th->syn && !th->rst && !th->ack && !th->fin) {
...
static int mark_download_app(struct sock *sk, kuid_t app_uid,
			     struct nf_conn *ct, struct sk_buff *skb)
{
	int choose_mark = 0;
	int ret = SLA_SKB_CONTINUE;

	if (SLA_MODE_DUAL_WIFI == sla_work_mode && is_download_app(app_uid)) {
		// for manual active
		if (INIT_ACTIVE_TYPE == dual_wifi_active_type) {
			if ((oplus_sla_info[MAIN_WLAN].max_speed <
				     CALC_WEIGHT_MIN_SPEED_3 &&
			     oplus_sla_info[SECOND_WLAN].max_speed <
				     CALC_WEIGHT_MIN_SPEED_3 &&
			     oplus_sla_info[MAIN_WLAN].sla_avg_rtt <
				     NORMAL_RTT) ||
			    (oplus_sla_info[MAIN_WLAN].download_flag <
				     DOWNLOAD_FLAG &&
			     oplus_sla_info[MAIN_WLAN].download_speed >=
				     DUAL_WLAN_MAX_DOWNLOAD_SPEED)) {
				choose_mark = MAIN_WLAN_MARK;

			} else if (100 == oplus_sla_info[MAIN_WLAN].weight &&
				   oplus_sla_info[MAIN_WLAN].download_flag >=
					   DOWNLOAD_FLAG &&
				   oplus_sla_info[SECOND_WLAN].download_flag <
					   DOWNLOAD_FLAG) {
			...
			if ((init_weight_delay_count > 0 &&
			    ...
			    (oplus_sla_info[MAIN_WLAN].download_flag <
			     ...
			     oplus_sla_info[MAIN_WLAN].download_speed >=
				     MAX_WLAN_SPEED &&
			     oplus_sla_info[SECOND_WLAN].download_speed >=
				     MAX_WLAN_SPEED)) {
...
static int mark_video_app(struct sock *sk, kuid_t app_uid, struct nf_conn *ct,
			  struct sk_buff *skb)
{
	int choose_mark = 0;
	int ret = SLA_SKB_CONTINUE;

	if (SLA_MODE_DUAL_WIFI == sla_work_mode && is_vedio_app(app_uid)) {
		if (oplus_sla_info[MAIN_WLAN].download_speed >=
		    MAX_WLAN_SPEED) {
			choose_mark = MAIN_WLAN_MARK;

		} else if (oplus_sla_info[SECOND_WLAN].download_speed >
				   MAX_WLAN_SPEED ||
			   (oplus_sla_info[MAIN_WLAN].download_speed <=
				    VIDEO_SPEED &&
			    ((init_weight_delay_count > 0 &&
			      oplus_sla_info[MAIN_WLAN].dl_little_speed <
				      DOWNLOAD_SPEED) ||
			     (oplus_sla_info[SECOND_WLAN].download_speed >
				      VIDEO_SPEED &&
			      oplus_sla_info[SECOND_WLAN].sla_avg_rtt <=
				      150)))) {
			choose_mark = SECOND_WLAN_MARK;
		}
	}
...
static u32 get_skb_mark_by_weight(void)
{
	...
	/*0x147AE15 = 0x7FFFFFFF /100 + 1; for we let the weight * 100 to void
	  ...
	  */
	for (i = 0; i < IFACE_NUM; i++) {
		if (oplus_sla_info[i].if_up && oplus_sla_info[i].weight) {
			if (sla_random <
			    (0x147AE15 * oplus_sla_info[i].weight)) {
...
static void reset_oplus_sla_calc_speed(struct timeval tv)
{
	...
	if (time_interval >= 60 && oplus_speed_info[MAIN_WLAN].speed_done) {
...
static int get_wlan_syn_retran(struct sk_buff *skb)
{
	...
	if (ctinfo == IP_CT_NEW && (iph = ip_hdr(skb)) != NULL &&
		...
		if (NULL != th && th->syn && !th->rst && !th->ack && !th->fin) {
			...
			*/
			...
			if (tmp_mark != MAIN_WLAN_MARK) {
				...
				if (dev &&
				    !memcmp(dev->name,
					    oplus_sla_info[MAIN_WLAN].dev_name,
					    strlen(dev->name))) {
					if (oplus_sla_debug) {
...
static void game_rtt_estimator(int game_type, u32 rtt)
{
	...
	if (game_type && game_type < GAME_NUM) {
...
static void game_app_switch_network(u32 game_type)
{
	...
	if (GAME_WZRY != game_type && GAME_CJZC != game_type) {
		return;
	}

	if (oplus_sla_info[MAIN_WLAN].wlan_score_bad_count >=
	    WLAN_SCORE_BAD_NUM) {
		wlan_bad = true;
	}
	...
	if (cell_quality_good && !game_cell_to_wifi &&
		...
		if (!game_uid[index].switch_time ||
			...
			oplus_sla_send_to_user(SLA_SWITCH_GAME_NETWORK,
					       (char *)game_bp_info,
					       sizeof(game_bp_info));
	...
	if (!wlan_bad && game_uid[index].mark == CELL_MARK && game_rx_bad) {
		if (!game_uid[index].switch_time ||
		    (time_now - game_uid[index].switch_time) > 60000) {
			game_uid[game_type].rtt = 0;
			game_uid[index].switch_time = time_now;
...
static bool is_game_rtt_skb(struct nf_conn *ct, struct sk_buff *skb, bool isTx)
{
	...
	if (isTx) {
		...
		if (udph->len >= (tx_offset + tx_len) &&
		    memcmp(payload + tx_offset,
			   game_params[gameType].tx_fixed_value, tx_len) == 0) {
			if (oplus_sla_debug) {
		...
		if (udph->len >= (rx_offset + rx_len) &&
		    memcmp(payload + rx_offset,
			   game_params[gameType].rx_fixed_value, rx_len) == 0) {
			if (oplus_sla_debug) {
			     ...
			     enum ip_conntrack_info ctinfo)
{
	...
	if (ct->oplus_app_type > 0 && ct->oplus_app_type < GAME_NUM) {
		...
		if (GAME_WZRY != game_index && GAME_CJZC != game_index) {
			return ret;
		}
		...
		if (iph && (IPPROTO_UDP == iph->protocol ||
			...
			if (GAME_CJZC == game_index &&
			    ...
			    ((XT_STATE_BIT(ctinfo) &
			     ...
			     (XT_STATE_BIT(ctinfo) &
			      XT_STATE_BIT(IP_CT_RELATED)))) {
				if (MAIN_WLAN_MARK == ct_mark &&
				    oplus_sla_info[MAIN_WLAN].wlan_score > 40) {
					return SLA_SKB_ACCEPT;
			...
			if (ct_mark && skb->mark && ct_mark != skb->mark) {
				printk("oplus_sla_game_rtt:reset game ct proto= %u,srcport = %d,"
				       ...
				       nf_ct_is_dying(ct),
				       nf_ct_is_confirmed(ct), game_index,
				       ct_mark, skb->mark);
			    ...
			    enum ip_conntrack_info ctinfo)
{
	...
	if (INIT_APP_TYPE == ct->oplus_app_type) {
		...
		for (i = 1; i < GAME_NUM; i++) {
			if (game_uid[i].uid) {
				if ((uid.val % UID_MASK) ==
				    (game_uid[i].uid % UID_MASK)) {
					ct->oplus_app_type = i;
					if (oplus_sla_enable &&
					    CELL_MARK == game_uid[i].mark) {
						game_uid[i].cell_bytes +=
							skb->len;
					}
	...
	} else if (ct->oplus_app_type > 0 && ct->oplus_app_type < GAME_NUM) {
		i = ct->oplus_app_type;
		if (oplus_sla_enable &&
		    (oplus_sla_def_net == MAIN_WLAN ||
		     oplus_sla_def_net == SECOND_WLAN) &&
		    CELL_MARK == game_uid[i].mark) {
...
static int detect_game_up_skb(struct sk_buff *skb)
{
	...
	if (iph && IPPROTO_UDP == iph->protocol) {
		...
		if (is_game_rtt_skb(ct, skb, true)) {
			...
			if (ct->oplus_game_timestamp &&
			    time_now - ct->oplus_game_timestamp > 200) {
				//Tx done and no Rx, we've lost a response pkt
				ct->oplus_game_lost_count++;
				sla_game_write_lock();
				game_rtt_estimator(ct->oplus_app_type,
						   MAX_GAME_RTT);
				sla_game_write_unlock();

				if (game_rtt_show_toast) {
					u32 game_rtt = MAX_GAME_RTT;
					oplus_sla_send_to_user(
						SLA_NOTIFY_GAME_RTT,
						(char *)&game_rtt,
						sizeof(game_rtt));
				}
...
static void record_sla_app_cell_bytes(struct nf_conn *ct, struct sk_buff *skb)
{
	...
	if (oplus_sla_enable && (oplus_sla_def_net == MAIN_WLAN ||
				 oplus_sla_def_net == SECOND_WLAN)) {
		if (ct->oplus_app_type > 0 && ct->oplus_app_type < GAME_NUM) {
				      ...
				      struct sk_buff *skb)
{
	...
	if ((oplus_sla_info[MAIN_WLAN].if_up ||
	     oplus_sla_info[MAIN_WLAN].need_up) &&
	    (iph = ip_hdr(skb)) != NULL && iph->protocol == IPPROTO_TCP) {
		th = tcp_hdr(skb);
		if (NF_INET_LOCAL_OUT == hook && NULL != th &&
		    (th->rst || th->fin)) {
		...
		if (NULL == ct || nf_ct_is_untracked(ct) || (0 == ct->mark)) {
			...
			if (dev && !memcmp(dev->name,
				...
				if (NF_INET_LOCAL_OUT == hook) {
				   ...
				   !memcmp(dev->name,
					   oplus_sla_info[SECOND_WLAN].dev_name,
					   strlen(dev->name))) {
				if (NF_INET_LOCAL_OUT == hook) {
					wlan1_tcp_tx++;
				} else if (NF_INET_LOCAL_IN == hook) {
...
static unsigned int oplus_sla_game_rtt_calc(struct sk_buff *skb)
{
	...
	if (is_game_rtt_skb(ct, skb, false)) {
		time_now = ktime_get_ns() / 1000000;
		if (ct->oplus_game_timestamp &&
		    time_now > ct->oplus_game_timestamp) {
			...
			if (sla_switch_enable && cell_quality_good) {
				if (oplus_sla_enable &&
				    sla_work_mode == SLA_MODE_DUAL_WIFI) {
					/* dualsta Mode, enable sla, just game rtt is bad */
					if (game_rx_bad ||
					    game_rtt >= (MAX_GAME_RTT - 80)) {
						send_enable_to_framework(
							SLA_MODE_WIFI_CELL,
							INIT_ACTIVE_TYPE);
			...
			if (game_rtt_show_toast) {
				oplus_sla_send_to_user(SLA_NOTIFY_GAME_RTT,
						       (char *)&game_rtt,
						       sizeof(game_rtt));
			}
			...
			if (oplus_sla_debug) {
				...
				printk("oplus_sla_game_rtt: game_rtt=%u, srtt=%u\n",
				       game_rtt,
				       game_uid[ct->oplus_app_type].rtt >>
					       shift);
			}
		}
	}
...
static bool is_need_change_dns_network(int index)
{
	...
	if (oplus_sla_info[index].weight &&
		...
		if (0 == oplus_sla_info[MAIN_WLAN].weight ||
		    WEIGHT_STATE_USELESS ==
			    oplus_sla_info[MAIN_WLAN].weight_state ||
		    WEIGHT_STATE_SCORE_INVALID ==
			    oplus_sla_info[MAIN_WLAN].weight_state ||
		    (oplus_sla_info[index].max_speed >= 100 &&
		     oplus_sla_info[MAIN_WLAN].wlan_score_bad_count >=
			     WLAN_SCORE_BAD_NUM)) {
			ret = true;
		}
	}
		...
		*app_uid = filp->f_cred->fsuid;
		...
		for (i = 0; i < dual_wifi_app_list.count; i++) {
			if (dual_wifi_app_list.uid[i]) {
				if ((uid.val % UID_MASK) ==
				    (dual_wifi_app_list.uid[i] % UID_MASK)) {
					ct->oplus_app_type =
						i + DUAL_STA_APP_BASE;
					return true;
				}
	...
	if (temp_len) {
		...
		if (start != NULL) {
			...
			if (NULL != end) {
				...
				oplus_speed_info[index].sum_bytes +=
					content_len;
				if (0 == oplus_speed_info[index].bytes_time) {
					oplus_speed_info[index].bytes_time =
						tmp_time;
				} else {
					if (oplus_speed_info[index].sum_bytes >=
						    20000 ||
					    (tmp_time - oplus_speed_info[index]
								.bytes_time) >
						    5000000000) {
						oplus_speed_info[index]
							.bytes_time = tmp_time;
						if (oplus_speed_info[index]
							    .sum_bytes >=
						    20000) {
							oplus_speed_info[index]
								.ms_speed_flag =
...
static unsigned int oplus_sla_speed_calc(struct sk_buff *skb)
{
	...
	if (!enable_cell_to_user && oplus_sla_calc_speed &&
			...
			if ((iph = ip_hdr(skb)) != NULL &&
				...
				if ((datalen - header_len) >=
					...
					if (oplus_speed_info[index]
						...
						if (0 == oplus_speed_info[index]
								    .last_time &&
						    0 == oplus_speed_info[index]
								    .first_time) {
							oplus_speed_info[index]
								.last_time =
								time_now;
...
*/
static unsigned int oplus_sla_output_hook(void *priv, struct sk_buff *skb,
					  const struct nf_hook_state *state)
{
	int ret = NF_ACCEPT;
...
static unsigned int oplus_sla_input_hook(void *priv, struct sk_buff *skb,
					 const struct nf_hook_state *state)
{
	//add for android Q statictis tcp tx and tx
	statistics_wlan_tcp_tx_rx(state, skb);
...
*/
static void
sla_mark_streams_for_iptables_reject(struct sk_buff *skb,
...
{
	if (sla_screen_on && !enable_cell_to_user && !oplus_sla_enable &&
	    !oplus_sla_calc_speed && !rate_limit_info.rate_limit_enable) {
		if (speed <= 100 && speed > CALC_WEIGHT_MIN_SPEED_1 &&
		    oplus_sla_info[MAIN_WLAN].minute_speed >
			    MINUTE_LITTE_RATE) {
			printk("oplus_sla_speed: detect speed is little\n");
			oplus_sla_calc_speed = 1;
			memset(&oplus_speed_info[MAIN_WLAN], 0x0,
			       sizeof(struct oplus_speed_calc));
		}
	}
}
...
static void auto_disable_sla_for_good_wlan(void)
{
	...
	if (oplus_sla_enable && time_interval >= 300 &&
		...
		oplus_sla_send_to_user(SLA_DISABLE, (char *)&work_mode,
				       sizeof(int));
		...
		if (oplus_sla_info[i].little_speed_num < ADJUST_SPEED_NUM) {
			...
			if (ADJUST_SPEED_NUM ==
				...
				oplus_sla_info[i].dl_little_speed =
					oplus_sla_info[i].tmp_little_speed;
			}
		}
	} else {
...
static void detect_network_download(int index, int speed)
{
	...
	if (speed > DOWNLOAD_SPEED) {
		...
		if (ADJUST_SPEED_NUM == oplus_sla_info[i].download_num) {
			if (oplus_sla_info[i].download_speed >
			    oplus_sla_info[i].dl_mx_speed) {
		...
		if (speed > (oplus_sla_info[i].download_speed / 2)) {
			if (speed < rom_update_info.dual_wlan_download_speed &&
			    ...
			    oplus_sla_info[i].dual_wifi_download <
				    (2 * DOWNLOAD_FLAG)) {
				oplus_sla_info[i].dual_wifi_download++;
			} else if (speed >= rom_update_info
						    .dual_wlan_download_speed ||
				   oplus_sla_info[i].download_speed >=
					   rom_update_info
						   .dual_wlan_download_speed) {
...
static void adjust_speed_with_romupdate_param(int index)
{
	...
	if (MAIN_WLAN == i || SECOND_WLAN == i) {
		if (oplus_sla_info[i].wlan_score <=
			    rom_update_info.wlan_bad_score &&
		    oplus_sla_info[i].max_speed >
			    rom_update_info.wlan_little_score_speed) {
			oplus_sla_info[i].max_speed =
				rom_update_info.wlan_little_score_speed;
		} else if (MAIN_WLAN == i &&
			   oplus_sla_info[i].max_speed >
				   rom_update_info.wlan_speed) {
			oplus_sla_info[i].max_speed =
				rom_update_info.wlan_speed;
		} else if (SECOND_WLAN == i &&
			   oplus_sla_info[i].max_speed >
				   rom_update_info.second_wlan_speed) {
			oplus_sla_info[i].max_speed =
				rom_update_info.second_wlan_speed;
		}
	}
		...
		if (oplus_sla_info[i].if_up || oplus_sla_info[i].need_up) {
			...
			if (dev) {
				if (dev_isalive(dev) &&
					...
					if (0 == oplus_sla_info[i].total_bytes ||
						...
						temp_speed = temp_bytes /
							     time_interval;
						temp_speed = temp_speed /
							     1000; //kB/s
						oplus_sla_info[i].current_speed =
							temp_speed;
						...
						calc_download_speed(
							i, total_bytes,
							download_time);
						reset_network_state_by_speed(
							i, temp_speed);
						adjust_speed_with_romupdate_param(
							i);
					}
				}
				dev_put(dev);
			...
			if (oplus_sla_debug /*&& net_ratelimit()*/) {
				printk("oplus_sla: dev_name = %s,if_up = %d,max_speed = %d,"
				       ...
				       oplus_sla_info[i].dev_name,
				       oplus_sla_info[i].if_up,
				       oplus_sla_info[i].max_speed,
				       oplus_sla_info[i].current_speed,
				       oplus_sla_info[i].sla_avg_rtt,
				       oplus_sla_info[i].congestion_flag,
				       oplus_sla_info[i].download_flag,
				       oplus_sla_info[i].syn_retran,
				       oplus_sla_info[i].minute_speed,
				       oplus_sla_info[i].weight_state,
				       oplus_sla_info[i].download_speed,
				       oplus_sla_info[i].dual_wifi_download,
				       oplus_sla_info[i].dl_little_speed);
	...
	if ((oplus_sla_info[index1].download_flag >= DOWNLOAD_FLAG ||
		...
		if (oplus_sla_info[index1].download_flag >= DOWNLOAD_FLAG) {
			if (is_wlan_speed_good(index2) &&
			    oplus_sla_info[index1].download_speed <=
				    MAX_WLAN_SPEED) {
				if (oplus_sla_debug) {
			...
			if (oplus_sla_info[index2].max_speed <=
			    CALC_WEIGHT_MIN_SPEED_3) {
				if (oplus_sla_info[index2].avg_rtt >=
				    rom_update_info.sla_rtt) {
					if (oplus_sla_debug) {
		...
		if (oplus_sla_info[index2].download_flag >= DOWNLOAD_FLAG) {
			if (is_wlan_speed_good(index1) &&
			    oplus_sla_info[index2].download_speed <=
				    MAX_WLAN_SPEED) {
				if (oplus_sla_debug) {
			...
			if (oplus_sla_info[index1].max_speed <=
			    CALC_WEIGHT_MIN_SPEED_3) {
				if (oplus_sla_info[index1].avg_rtt >=
				    rom_update_info.sla_rtt) {
					if (oplus_sla_debug) {
		...
		oplus_sla_info[index1].weight =
			calc_weight_with_speed(speed1, speed2);
		oplus_sla_info[index2].weight = 100;
		if (oplus_sla_debug) {
...
static int calc_weight_with_weight_state(int network_1, int network_2)
{
	...
	if ((WEIGHT_STATE_USELESS == oplus_sla_info[index1].weight_state ||
	     WEIGHT_STATE_SCORE_INVALID ==
		     oplus_sla_info[index1].weight_state) &&
...
static bool is_main_wlan_poor(void)
{
	...
	if (oplus_sla_info[index].max_speed <= rom_update_info.sla_speed &&
	    oplus_sla_info[index].sla_avg_rtt >=
		    rom_update_info.dual_wifi_rtt) {
		return true;
	}
...
static void recalc_two_network_weight(int network_1, int network_2)
{
	...
	if (oplus_sla_info[index1].if_up && oplus_sla_info[index2].if_up) {
		...
		if ((speed1 >= CALC_WEIGHT_MIN_SPEED_2 &&
		    ...
		    (CONGESTION_LEVEL_HIGH ==
			     oplus_sla_info[index1].congestion_flag ||
		     CONGESTION_LEVEL_HIGH ==
			     oplus_sla_info[index2].congestion_flag) ||
		    ((CONGESTION_LEVEL_MIDDLE ==
			      oplus_sla_info[index1].congestion_flag ||
		      CONGESTION_LEVEL_MIDDLE ==
			      oplus_sla_info[index2].congestion_flag) &&
		     (speed1 >= CALC_WEIGHT_MIN_SPEED_1 &&
		      speed2 >= CALC_WEIGHT_MIN_SPEED_1))) {
			oplus_sla_info[index1].weight =
				calc_weight_with_speed(speed1, speed2);
		} else {
			if (SLA_MODE_DUAL_WIFI == sla_work_mode &&
...
calc_weight_finish:
	...
	if (oplus_sla_debug) {
		printk("oplus_sla_weight: work_mode = %d,weight[%d] = %d,weight[%d] = %d\n",
		       sla_work_mode, index1, oplus_sla_info[index1].weight,
		       index2, oplus_sla_info[index2].weight);
	}
	return;
}
...
static void recalc_dual_wifi_cell_weight(void)
{
	...
	if (is_wlan_speed_good(MAIN_WLAN) || is_wlan_speed_good(SECOND_WLAN) ||
	...
	if ((speed1 >= CALC_WEIGHT_MIN_SPEED_2 &&
		...
		oplus_sla_info[1].weight =
			(100 * (speed1 + speed2)) / sum_speed;
...
finish_calc:
	if (oplus_sla_debug) {
		printk("oplus_sla_weight: work_mode = %d,weight[0] = %d,weight[1] = %d,weight[2] = %d\n",
		       sla_work_mode, oplus_sla_info[0].weight,
		       oplus_sla_info[1].weight, oplus_sla_info[2].weight);
	}
	return;
}
...
{
	if (enable_cell_to_user && (tv.tv_sec - last_enable_cell_tv.tv_sec) >=
					   ENABLE_TO_USER_TIMEOUT) {
		enable_cell_to_user = false;
	}

	if (enable_second_wifi_to_user &&
	    (tv.tv_sec - last_enable_second_wifi_tv.tv_sec) >=
		    ENABLE_TO_USER_TIMEOUT) {
		enable_second_wifi_to_user = false;
	}
...
static void send_speed_and_rtt_to_user(void)
{
	...
	if (oplus_sla_info[MAIN_WLAN].if_up ||
		...
		memcpy(total_payload + sizeof(payload), tcp_tx_rx,
		       sizeof(tcp_tx_rx));

		ret = oplus_sla_send_to_user(SLA_NOTIFY_SPEED_RTT,
					     (char *)total_payload,
		...
		if (oplus_sla_debug) {
			printk("oplus_sla:send_speed_and_rtt_to_user wlan0:max_speed=%d, avg_rtt=%d  "
			       ...
			       tcp_tx_rx[0], tcp_tx_rx[1], tcp_tx_rx[2],
			       tcp_tx_rx[3]);
		}
	}
	return;
...
static int wlan_can_enable_second_wifi(int index)
{
	...
	if (oplus_sla_info[index].if_up || oplus_sla_info[index].need_up) {
		...
		if (oplus_sla_info[index].max_speed <=
			    rom_update_info.sla_speed &&
		    oplus_sla_info[index].sla_avg_rtt >=
			    rom_update_info.dual_wifi_rtt) {
...
static void sla_show_dailog(void)
{
	...
	if (sla_screen_on && !sla_switch_enable && need_show_dailog &&
	    cell_quality_good && !send_show_dailog_msg &&
...
static void sla_detect_should_enable(void)
{
	...
	if (sla_screen_on && CELL_INDEX != oplus_sla_def_net) {
		if (dual_wifi_switch_enable &&
		...
		} else if (!enable_cell_to_user && sla_switch_enable &&
			   cell_quality_good &&
			   !rate_limit_info.rate_limit_enable &&
			...
			if (SLA_MODE_DUAL_WIFI == sla_work_mode) {
				if (dual_wlan_enable_cell()) {
				   ...
				   wlan_can_enable_cell(MAIN_WLAN)) {
				send_enable_to_framework(SLA_MODE_WIFI_CELL,
							 INIT_ACTIVE_TYPE);
			}
		}
	}
	...
	for (index = 0; index < IFACE_NUM; index++) {
		...
		if (oplus_sla_info[index].if_up ||
			...
			if (oplus_sla_info[index].rtt_index >= RTT_NUM) {
				avg_rtt = oplus_sla_info[index].sum_rtt /
					  oplus_sla_info[index].rtt_index;
				if (oplus_sla_info[index].download_flag >=
				    DOWNLOAD_FLAG) {
					avg_rtt = avg_rtt / 2;
				}
				...
				oplus_sla_info[index].avg_rtt =
					(7 * oplus_sla_info[index].avg_rtt +
					 avg_rtt) /
					8;
			...
			if (oplus_sla_debug) {
				printk("oplus_sla_rtt: index = %d,wlan_rtt = %d,"
				       ...
				       cell_quality_good, need_show_dailog,
				       sla_screen_on, enable_cell_to_user,
				       enable_second_wifi_to_user,
				       sla_switch_enable, oplus_sla_enable,
				       oplus_sla_def_net, avg_rtt,
				       sla_detect_mode, sla_work_mode,
				       game_cell_to_wifi);
			}
		}
	}
...
{
	if (!oplus_sla_vpn_connected && game_online_info.game_online) {
		...
		cur_info->udp_rx_packet[cur_info->window_index %
					UDP_RX_WIN_SIZE] = delta_udp_pkts;
		...
		if (udp_rx_show_toast) {
			oplus_sla_send_to_user(SLA_NOTIFY_GAME_RX_PKT,
					       (char *)&delta_udp_pkts,
		...
		if (tcp_rx_large_count == TCP_RX_WIN_SIZE) {
			...
			if (inGame) {
				inGame = false;
				game_cell_to_wifi = false;
				memset(cur_info->udp_rx_packet, 0x0,
				       sizeof(u32) * UDP_RX_WIN_SIZE);
			...
			for (i = 0; i < UDP_RX_WIN_SIZE; i++) {
				if (cur_info->udp_rx_packet[i] >=
				    cur_info->udp_rx_min) {
					trafficCount++;
					if (cur_info->udp_rx_packet[i] <=
					    cur_info->udp_rx_low_thr) {
						lowTrafficCount++;
					}
				}
			}
			if (trafficCount >= cur_info->in_game_true_thr) {
				if (!inGame) {
					...
					inGame = true;
					game_cell_to_wifi = false;
					for (index = 0; index < UDP_RX_WIN_SIZE;
					     index++) {
						cur_info->udp_rx_packet[index] =
							20;
					}
					lowTrafficCount = 0;
				}
			} else if (trafficCount <=
				   cur_info->in_game_false_thr) {
				if (inGame) {
					inGame = false;
					game_cell_to_wifi = false;
					memset(cur_info->udp_rx_packet, 0x0,
			...
			if (inGame && cur_info->game_in_front) {
				if (lowTrafficCount >= cur_info->rx_bad_thr &&
				    game_index > 0 &&
				    game_start_state
					    [game_index]) { //wzry has low traffic when game ends
					game_rx_bad = true;
				} else {
					game_rx_bad = false;
				}
			...
			if (oplus_sla_debug) {
				printk("oplus_sla_game_rx_update:delta_udp_pkts=%d, lowTrafficCount=%d, inGame=%d, "
				       ...
				       game_rx_bad, delta_tcp_bytes,
				       tcp_rx_large_count, trafficCount);
			}
		}
	}
}
...
static void game_online_time_out(struct timeval tv)
{
	if (game_online_info.game_online &&
	    (tv.tv_sec - game_online_info.last_game_skb_tv.tv_sec) >=
		    GAME_SKB_TIME_OUT) {
...
static void dns_statistic_timer(struct timeval tv)
{
	...
	for (i = 0; i < IFACE_NUM; i++) {
		if (dns_info[i].in_timer &&
			...
			sla_write_lock();
			if (SLA_MODE_DUAL_WIFI == sla_work_mode) {
				calc_weight_with_weight_state(MAIN_WLAN,
							      SECOND_WLAN);
			} else if (SLA_MODE_WIFI_CELL == sla_work_mode) {
				calc_weight_with_weight_state(MAIN_WLAN,
							      CELL_INDEX);
...
static void oplus_sla_timer_init(void)
{
	...
	sla_timer.expires =
		jiffies + SLA_TIMER_EXPIRES; /* timer expires in ~1s */
	add_timer(&sla_timer);
...
static void oplus_sla_send_white_list_app_traffic(void)
{
	...
	oplus_sla_send_to_user(SLA_SEND_WHITE_LIST_APP_TRAFFIC, send_msg,
			       sizeof(send_msg));
...
static int oplus_sla_set_default_network(struct nlmsghdr *nlh)
{
	...
	memset(wzry_traffic_info.udp_rx_packet, 0x0,
	       sizeof(u32) * UDP_RX_WIN_SIZE);
	wzry_traffic_info.window_index = 0;
	memset(cjzc_traffic_info.udp_rx_packet, 0x0,
	       sizeof(u32) * UDP_RX_WIN_SIZE);
...
{
	oplus_sla_send_to_user(SLA_SEND_GAME_APP_STATISTIC, (char *)game_uid,
			       GAME_NUM * sizeof(struct oplus_sla_game_info));
...
static void oplus_sla_send_syn_retran_info(void)
{
	oplus_sla_send_to_user(SLA_GET_SYN_RETRAN_INFO,
			       ...
			       sizeof(struct oplus_syn_retran_statistic));
...
static int disable_oplus_sla_module(struct nlmsghdr *nlh)
{
	...
	if (oplus_sla_enable && disable_type) {
		...
		oplus_sla_send_to_user(SLA_DISABLED, (char *)&disable_type,
				       sizeof(int));
	}
	sla_write_unlock();
	return 0;
	...
	if (index >= 0 && index < IFACE_NUM) {
		...
		if (up) {
			...
			if (index == MAIN_WLAN || index == SECOND_WLAN) {
				...
				oplus_sla_info[CELL_INDEX].wlan_score =
					WLAN_SCORE_GOOD;
			}
			if (p) {
			...
			if (index == MAIN_WLAN || index == SECOND_WLAN) {
				...
				if (index == MAIN_WLAN) {
					...
					memset(game_uid, 0x0,
					       GAME_NUM *
						       sizeof(struct oplus_sla_game_info));
				}
			}

			sla_write_lock();
			memset(&dns_info[index], 0x0,
			       sizeof(struct sla_dns_statistic));
			memset(&oplus_speed_info[index], 0x0,
			       sizeof(struct oplus_speed_calc));
			memset(&oplus_sla_info[index], 0x0,
			       sizeof(struct oplus_dev_info));
			sla_write_unlock();
		}
	}
...
static int oplus_sla_set_primary_wifi(struct nlmsghdr *nlh)
{
	...
	if (*data == WLAN0_INDEX || *data == WLAN1_INDEX) {
		MAIN_WLAN = *data;
		SECOND_WLAN =
			(WLAN0_INDEX == MAIN_WLAN) ? WLAN1_INDEX : WLAN0_INDEX;
...
static void set_weight_state_by_score(int index, int score)
{
	if (index >= 0 && index < IFACE_NUM) {
		if (WLAN_NETWORK_INVALID == score &&
		    WEIGHT_STATE_SCORE_INVALID !=
			    oplus_sla_info[index].weight_state) {
...
static int oplus_sla_set_wifi_score(struct nlmsghdr *nlh)
{
	...
	if (NULL != data && data[0] >= 0 && data[0] < IFACE_NUM) {
		int index = data[0];
		int score = data[1];
		...
		if (oplus_sla_info[index].wlan_score_bad_count >
...
static int oplus_sla_set_params(struct nlmsghdr *nlh)
{
	...
	if (count == 13) {
		...
		printk("oplus_sla_netlink:set params invalid param count:%d",
		       count);
	}
...
static int oplus_sla_set_game_rtt_params(struct nlmsghdr *nlh)
{
	...
	if (index > 0 && index < GAME_NUM) {
		memcpy(&game_params[index], data,
		       sizeof(struct oplus_sla_game_rtt_params));
		printk("oplus_sla_netlink:set game params index=%d tx_offset=%d tx_len=%d tx_fix=",
		       game_params[index].game_index,
		       game_params[index].tx_offset, game_params[index].tx_len);
		for (i = 0; i < game_params[index].tx_len; i++) {
			printk("%02x", game_params[index].tx_fixed_value[i]);
		}
		printk(" rx_offset=%d rx_len=%d rx_fix=",
		       game_params[index].rx_offset, game_params[index].rx_len);
	...
	if (dual_wifi_app_list.count > 0 &&
		...
		for (i = 0; i < dual_wifi_app_list.count; i++) {
			...
			printk("oplus_sla_netlink:set_dual_wifi_app_uid count=%d, uid[%d]=%d\n",
			       dual_wifi_app_list.count, i,
			       dual_wifi_app_list.uid[i]);
		}
	}
	return 0;
}
...
static int oplus_sla_set_vedio_app_uid(struct nlmsghdr *nlh)
{
	...
	if (vedio_app_list.count > 0 &&
	    vedio_app_list.count < DUAL_STA_APP_NUM) {
		int i;
		for (i = 0; i < vedio_app_list.count; i++) {
...
static void reset_main_wlan_info(void)
{
	...
	if (oplus_sla_info[MAIN_WLAN].if_up) {
		...
		oplus_sla_info[MAIN_WLAN].congestion_flag =
			CONGESTION_LEVEL_NORMAL;
	}
}
...
static int oplus_sla_netlink_init(void)
{
	...
	oplus_sla_sock =
		netlink_kernel_create(&init_net, NETLINK_OPLUS_SLA, &cfg);
...
static void __exit oplus_sla_fini(void)
{
	...
	nf_unregister_net_hooks(&init_net, oplus_sla_ops,
				ARRAY_SIZE(oplus_sla_ops));
}
