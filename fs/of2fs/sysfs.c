			     ...
			     char *buf)
{
	...
	if (f2fs_sb_has_encrypt(sbi))
		len += snprintf(buf, PAGE_SIZE - len, "%s", "encryption");
	...
	if (f2fs_sb_has_compression(sbi))
		len += snprintf(buf + len, PAGE_SIZE - len, "%s%s",
				len ? ", " : "", "compression");
	len += snprintf(buf + len, PAGE_SIZE - len, "%s%s", len ? ", " : "",
			"pin_file");
			   ...
			   const char *buf, size_t count)
{
	...
	if (!strcmp(a->attr.name, "dpolicy_expect")) {
		...
		if (t == DPOLICY_BG)
			...
			wake_up_discard_thread_aggressive(sbi, DPOLICY_BALANCE);
...
F2FS_RW_ATTR(GC_THREAD, f2fs_gc_kthread, gc_dirty_rate_threshold,
...
F2FS_RW_ATTR(F2FS_HOT_COLD_PARAMS, f2fs_hot_cold_params, hc_hot_data_waterline,
	     hot_data_waterline);
F2FS_RW_ATTR(F2FS_HOT_COLD_PARAMS, f2fs_hot_cold_params,
	     hc_warm_data_lower_limit, warm_data_lower_limit);
F2FS_RW_ATTR(F2FS_HOT_COLD_PARAMS, f2fs_hot_cold_params, hc_warm_data_waterline,
	     warm_data_waterline);
F2FS_RW_ATTR(F2FS_HOT_COLD_PARAMS, f2fs_hot_cold_params,
	     hc_hot_node_lower_limit, hot_node_lower_limit);
F2FS_RW_ATTR(F2FS_HOT_COLD_PARAMS, f2fs_hot_cold_params, hc_hot_node_waterline,
	     hot_node_waterline);
F2FS_RW_ATTR(F2FS_HOT_COLD_PARAMS, f2fs_hot_cold_params,
	     hc_warm_node_lower_limit, warm_node_lower_limit);
F2FS_RW_ATTR(F2FS_HOT_COLD_PARAMS, f2fs_hot_cold_params, hc_warm_node_waterline,
	     warm_node_waterline);
...
static int __maybe_unused frag_score_seq_show(struct seq_file *seq,
					      void *offset)
{
	struct super_block *sb = seq->private;
	struct f2fs_sb_info *sbi = F2FS_SB(sb);
	...
	for (i = 0; i < total_segs; i++) {
		total_blocks +=
			of2fs_seg_freefrag(sbi, i, blocks, ARRAY_SIZE(blocks));
		cond_resched();
	}
	...
	for (i = 0; i < ARRAY_SIZE(blocks); i++) {
		if (!blocks[i])
			...
			f2fs_info(sbi, "%dK...%dK-: %u", 4 << i, 4 << (i + 1),
				  blocks[i]);
		else
			f2fs_info(sbi, "%dM...%dM-: %u", 1 << (i - 8),
				  1 << (i - 7), blocks[i]);
	}

	score = total_blocks ? (blocks[0] + blocks[1]) * 100ULL / total_blocks :
			       0;
	seq_printf(seq, "%u\n", score);
	return 0;
}
...
static int __maybe_unused undiscard_score_seq_show(struct seq_file *seq,
						   void *offset)
{
	struct super_block *sb = seq->private;
	struct f2fs_sb_info *sbi = F2FS_SB(sb);
...
static ssize_t f2fs_dc_opt_enable_write(struct file *filp,
...
int f2fs_register_sysfs(struct f2fs_sb_info *sbi)
{
	...
	if (sbi->s_proc) {
		...
		proc_create_data("gc_opt_enable", S_IRUGO | S_IWUGO,
				 sbi->s_proc, &f2fs_seq_gc_opt_enable_fops, sb);
		/* add for oDiscard */
