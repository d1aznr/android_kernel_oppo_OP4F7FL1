...
static void f2fs_set_compressed_page(struct page *page, struct inode *inode,
				     pgoff_t index, void *data, refcount_t *r)
{
	SetPagePrivate(page);
...
int f2fs_init_compress_ctx(struct compress_ctx *cc)
{
	...
	cc->rpages = f2fs_kzalloc(
		sbi, sizeof(struct page *) << cc->log_cluster_size, GFP_NOFS);
...
static int f2fs_compress_pages(struct compress_ctx *cc)
{
	...
	cc->cpages = f2fs_kzalloc(sbi, sizeof(struct page *) * cc->nr_cpages,
				  GFP_NOFS);
	if (!cc->cpages) {
		ret = -ENOMEM;
...
struct decompress_io_ctx *f2fs_alloc_dic(struct compress_ctx *cc)
{
	...
	dic->cpages = f2fs_kzalloc(sbi, sizeof(struct page *) * dic->nr_cpages,
				   GFP_NOFS);
	if (!dic->cpages)
		goto out_free;
	...
	dic->tpages = f2fs_kzalloc(
		sbi, sizeof(struct page *) * dic->cluster_size, GFP_NOFS);
	if (!dic->tpages)
		goto out_free;
