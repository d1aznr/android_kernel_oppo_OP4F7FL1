...
static inline bool need_SSR_by_type(struct f2fs_sb_info *sbi, int type,
				    int contig_level)
{
	int node_secs = get_blocktype_secs(sbi, F2FS_DIRTY_NODES);
	int dent_secs = get_blocktype_secs(sbi, F2FS_DIRTY_DENTS);
	...
	if (free_sections(sbi) <=
	    (unsigned int)(dirty_sum + 2 * reserved_sections(sbi))) {
		return true;
	}
	...
	if (unlikely(0 == left_space / KBS_PER_SEGMENT))
		return false;
	return (free_segs - ovp_segments) * 100 /
		       (left_space / KBS_PER_SEGMENT) <=
	       waterline;
}
#endif
			   ...
			   block_t *blocks, unsigned int n)
{
	...
	while (pos0 < sbi->blocks_per_seg) {
		...
		pos = __find_rev_next_bit(cur_map, sbi->blocks_per_seg,
					  pos0 + 1);
		blks = pos - pos0;
		order = ilog2(blks);
		if (order < n)
			blocks[order] += blks;
		total_blocks += blks;
		pos0 = __find_rev_next_zero_bit(cur_map, sbi->blocks_per_seg,
						pos + 1);
	}
	return total_blocks;
}
...
static inline bool of2fs_need_balance_dirty(struct f2fs_sb_info *sbi)
{
	...
	bd_lock(sbi);
	node_cnt =
		(long)(bd->curr_node_alloc_count - bd->last_node_alloc_count);
	data_cnt =
		(long)(bd->curr_data_alloc_count - bd->last_data_alloc_count);
...
static inline void of2fs_balance_fs(struct f2fs_sb_info *sbi)
{
	...
	if (has_not_enough_free_secs(sbi, 0, 0)) {
		...
		if (NULL != gc_th) {
			DEFINE_WAIT(__wait);
			prepare_to_wait(&gc_th->fggc_wait_queue_head, &__wait,
	...
	if (!dc->bio_ref && dc->state == D_SUBMIT) {
		...
		if (dc->discard_time) {
			...
			if (discard_end_time > dc->discard_time)
				dc->discard_time =
					discard_end_time - dc->discard_time;
			else
				dc->discard_time = 0;
		}
#endif
...
static struct discard_cmd *
__insert_discard_tree(struct f2fs_sb_info *sbi, struct block_device *bdev,
		      block_t lstart, block_t start, block_t len,
...
static inline void check_dpolicy_expect(struct f2fs_sb_info *sbi,
					int *dpolicy_curr)
{
	if (!sbi->dc_opt_enable)
		return;
			  ...
			  unsigned long long old_mtime, bool del)
{
	...
	if (!se->mtime) {
		...
		old_valid_blocks =
			del ? (se->valid_blocks - 1) : se->valid_blocks;
		total_valid = old_valid_blocks + 1;
		se->mtime =
			(se->mtime * old_valid_blocks + mtime) / total_valid;
	}
 ...
 */
void get_new_segment(struct f2fs_sb_info *sbi, unsigned int *newseg,
...
static void __reset_curseg(struct f2fs_sb_info *sbi, struct curseg_info *curseg,
 ...
 */
static void __change_curseg(struct f2fs_sb_info *sbi,
...
static int __get_ssr_segment(struct f2fs_sb_info *sbi,
			     struct curseg_info *curseg, int type,
...
{
	return __get_ssr_segment(sbi, CURSEG_I(sbi, type), type, alloc_mode,
				 age);
}
...
find_other_zone:
	if (dir == ALLOC_RIGHT) {
		secno = find_next_zero_bit(free_i->free_secmap, end_sec,
					   start_sec);
		...
		while (left_start > start_sec || right_start < end_sec) {
			if (rightward) {
				if (right_start == end_sec)
					goto next;
				end = min_t(unsigned int, end_sec,
					    right_start +
...
static void new_curseg_subdivision(struct f2fs_sb_info *sbi, int type,
				   bool new_sec)
{
	__new_curseg_subdivision(sbi, CURSEG_I(sbi, type), type, new_sec);
}

static void allocate_segment_subdivision(struct f2fs_sb_info *sbi, int type,
					 bool force, int contig_level)
{
	struct curseg_info *curseg = CURSEG_I(sbi, type);
	if (force)
	...
	if (fio->type == DATA) {
		...
		if (is_cold_data(fio->page)) {
			if (fio->sbi && fio->sbi->atgc_enabled &&
				...
				return CURSEG_COLD_DATA;
		} else if (file_is_cold(inode) || f2fs_compressed_file(inode))
			return CURSEG_COLD_DATA;
	...
	*new_blkaddr = NEXT_FREE_BLKADDR(sbi, curseg);
	...
	if (!__has_curseg_space(sbi, type)) {
		if (from_gc && sbi->atgc_enabled) {
			if (__get_ssr_segment(sbi, curseg, se->type, ASSR,
					      se->mtime)) {
				f2fs_bug_on(sbi,
					    curseg->next_segno ==
				...
				sit_i->s_ops->__new_curseg(sbi, curseg,
							   se->type, false);
			}
			stat_inc_seg_type(sbi, curseg);
		} else {
			     ...
			     enum iostat_type io_type)
{
	...
	if (fio.new_blkaddr >= le32_to_cpu(F2FS_RAW_SUPER(sbi)->cp_blkaddr) &&
		...
		bd_inc_array_val(sbi, hotcold_count, HC_META_CP, 1);
		 ...
		 le32_to_cpu(F2FS_RAW_SUPER(sbi)->nat_blkaddr))
		bd_inc_array_val(sbi, hotcold_count, HC_META_SIT, 1);
	else if (fio.new_blkaddr <
		 le32_to_cpu(F2FS_RAW_SUPER(sbi)->ssa_blkaddr))
...
void init_virtual_curseg(struct f2fs_sb_info *sbi)
{
	...
	if (__get_ssr_segment(sbi, curseg, CURSEG_COLD_DATA, SSR, 0)) {
		...
		SIT_I(sbi)->s_ops->get_new_segment(sbi, &segno, false,
						   ALLOC_LEFT);
		curseg->next_segno = segno;
...
static int build_curseg(struct f2fs_sb_info *sbi)
{
	...
	array = f2fs_kzalloc(sbi,
			     array_size(NR_INMEM_CURSEG_TYPE, sizeof(*array)),
			     GFP_KERNEL);
	if (!array)
		return -ENOMEM;
...
int __init f2fs_create_segment_manager_caches(void)
{
	discard_entry_slab = f2fs_kmem_cache_create(
		"discard_entry", sizeof(struct discard_entry));
	...
	sit_entry_set_slab = f2fs_kmem_cache_create(
		"sit_entry_set", sizeof(struct sit_entry_set));
