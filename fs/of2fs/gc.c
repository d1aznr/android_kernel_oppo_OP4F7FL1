...
static inline bool is_frag_urgent(struct f2fs_sb_info *sbi)
{
	unsigned long next_check =
		sbi->last_frag_check + msecs_to_jiffies(GC_URGENT_CHECK_TIME);
...
static inline int gc_perf_ratio(struct f2fs_sb_info *sbi)
{
	...
	return reclaimable_user_blocks == 0 ?
		       100 :
			       ...
			       struct victim_sel_policy *p)
{
	...
	unsigned int dirty_threshold =
		max(gc_th->dirty_count_threshold,
		    gc_th->dirty_rate_threshold * gc_th->victim_count / 100);
	...
	accu = min_t(unsigned long long, ULLONG_MAX / total_time / 100,
...
bool check_rb_tree_consistence(struct f2fs_sb_info *sbi, struct rb_root *root)
{
...
next_step:
	...
	for (off = 0; off < sbi->blocks_per_seg; off++, entry++) {
		...
		if (gc_type == BG_GC && has_not_enough_free_secs(sbi, 0, 0)) {
			...
			bd_inc_array_val(sbi, hotcold_count, HC_GC_COLD_DATA,
					 gc_blks);
			bd_unlock(sbi);
			return submitted;
		}
			      ...
			      struct gc_inode_list *gc_list, int gc_type)
{
	...
	freed:
		...
		if (gc_type == BG_GC || get_valid_blocks(sbi, segno, 1) == 0) {
			if (type == SUM_TYPE_NODE)
				bd_inc_array_val(sbi, gc_node_segments, gc_type,
						 1);
			else
				bd_inc_array_val(sbi, gc_data_segments, gc_type,
						 1);
			bd_inc_array_val(sbi, hotcold_gc_segments, hc_type + 1,
					 1);
		}
		bd_inc_array_val(sbi, hotcold_gc_blocks, hc_type + 1,
				 (unsigned long)get_valid_blocks(sbi, segno,
								 1));
		bd_unlock(sbi);
#endif
