...
#define SSR_HN_SAPCE_LIMIT_128G                                                \
	(8 << 20) /* 8G default sapce limit for 128G devices */
#define SSR_HN_WATERLINE_128G                                                  \
	80 /* 80% default hot node waterline for 128G devices */
#define SSR_WN_SAPCE_LIMIT_128G                                                \
	(5 << 20) /* 5G default warm node sapce limit for 128G devices */
#define SSR_WN_WATERLINE_128G                                                  \
	70 /* 70% default warm node waterline for 128G devices */
#define SSR_HD_SAPCE_LIMIT_128G                                                \
	(8 << 20) /* 8G default hot data sapce limit for 128G devices */
#define SSR_HD_WATERLINE_128G                                                  \
	65 /* 65% default hot data waterline for 128G devices */
#define SSR_WD_SAPCE_LIMIT_128G                                                \
	(5 << 20) /* 5G default warm data sapce limit for 128G devices */
#define SSR_WD_WATERLINE_128G                                                  \
	60 /* 60% default warm data waterline for 128G devices */
#endif
...
static int parse_options(struct super_block *sb, char *options)
{
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		case Opt_alloc:
			...
			if (strlen(name) == 7 && !strncmp(name, "default", 7)) {
		...
		case Opt_compress_algorithm:
			...
			if (strlen(name) == 3 && !strcmp(name, "lzo")) {
				F2FS_OPTION(sbi).compress_algorithm =
					COMPRESS_LZO;
			} else if (strlen(name) == 3 && !strcmp(name, "lz4")) {
				F2FS_OPTION(sbi).compress_algorithm =
					COMPRESS_LZ4;
			} else {
				kfree(name);
...
int f2fs_sanity_check_ckpt(struct f2fs_sb_info *sbi)
{
	...
	if (unlikely(fsmeta < F2FS_MIN_SEGMENTS || ovp_segments == 0 ||
		     reserved_segments == 0)) {
...
static int init_blkz_info(struct f2fs_sb_info *sbi, int devi)
{
	...
	FDEV(devi).blkz_seq = f2fs_kzalloc(
...
static int f2fs_init_grading_ssr(struct f2fs_sb_info *sbi)
{
	...
	if (total_blocks > 64) { /* 64G */
		sbi->hot_cold_params.hot_data_lower_limit =
			SSR_HD_SAPCE_LIMIT_128G;
		sbi->hot_cold_params.hot_data_waterline = SSR_HD_WATERLINE_128G;
		sbi->hot_cold_params.warm_data_lower_limit =
			SSR_WD_SAPCE_LIMIT_128G;
		sbi->hot_cold_params.warm_data_waterline =
			SSR_WD_WATERLINE_128G;
		sbi->hot_cold_params.hot_node_lower_limit =
			SSR_HD_SAPCE_LIMIT_128G;
		sbi->hot_cold_params.hot_node_waterline = SSR_HN_WATERLINE_128G;
		sbi->hot_cold_params.warm_node_lower_limit =
			SSR_WN_SAPCE_LIMIT_128G;
		sbi->hot_cold_params.warm_node_waterline =
			SSR_WN_WATERLINE_128G;
		sbi->hot_cold_params.enable = GRADING_SSR_OFF;
	} else {
		sbi->hot_cold_params.hot_data_lower_limit =
			SSR_DEFALT_SPACE_LIMIT;
