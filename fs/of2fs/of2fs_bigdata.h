...
struct f2fs_bigdata_info {
	...
	u64 cp_time, max_cp_time, max_cp_submit_time, max_cp_flush_meta_time,
		max_cp_discard_time;

	/* Discard info
	 * avg_discard_time = discard_time / discard_count
	 */
	unsigned int discard_count, discard_blocks, undiscard_count,
		undiscard_blocks;
	 ...
	 *
	 */
	unsigned int gc_count[2], gc_fail_count[2], gc_data_count[2],
		gc_node_count[2];
	...
	u64 fsync_time, max_fsync_time, fsync_cp_time, max_fsync_cp_time,
		fsync_wr_file_time, max_fsync_wr_file_time,
		fsync_sync_node_time, max_fsync_sync_node_time,
		fsync_flush_time, max_fsync_flush_time;
...
#define bd_inc_val(sbi, member, val)                                           \
	do {                                                                   \
		struct f2fs_bigdata_info *bd = F2FS_BD_STAT(sbi);              \
		if (bd)                                                        \
			bd->member += (val);                                   \
	} while (0)
#define bd_inc_array_val(sbi, member, idx, val)                                \
	do {                                                                   \
		struct f2fs_bigdata_info *bd = F2FS_BD_STAT(sbi);              \
		if (bd)                                                        \
			bd->member[(idx)] += (val);                            \
...
#define bd_set_val(sbi, member, val)                                           \
	do {                                                                   \
		struct f2fs_bigdata_info *bd = F2FS_BD_STAT(sbi);              \
		if (bd)                                                        \
			bd->member = (val);                                    \
	} while (0)
#define bd_set_array_val(sbi, member, idx, val)                                \
	do {                                                                   \
		struct f2fs_bigdata_info *bd = F2FS_BD_STAT(sbi);              \
		if (bd)                                                        \
			bd->member[(idx)] = (val);                             \
...
#define bd_max_val(sbi, member, val)                                           \
	do {                                                                   \
		struct f2fs_bigdata_info *bd = F2FS_BD_STAT(sbi);              \
		if (bd) {                                                      \
			if (bd->member < (val))                                \
