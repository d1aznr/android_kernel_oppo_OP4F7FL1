...
#define clear_priv_opt(sbi, option)                                            \
	(sbi->mount_opt.priv_opt &= ~F2FS_MOUNT_PRIV_##option)
#define set_priv_opt(sbi, option)                                              \
	(sbi->mount_opt.priv_opt |= F2FS_MOUNT_PRIV_##option)
#define test_priv_opt(sbi, option)                                             \
	(sbi->mount_opt.priv_opt & F2FS_MOUNT_PRIV_##option)
...
struct discard_entry {
	...
	unsigned char
		discard_map[SIT_VBLOCK_MAP_SIZE]; /* segment discard bitmap */
};
...
struct discard_policy {
	...
	unsigned int
		io_aware_gran; /* minimum granularity discard not be aware of I/O */
...
struct f2fs_nm_info {
	...
	struct list_head
		free_nid_list; /* list for free nids excluding preallocated nids */
...
datablock_addr(struct inode *inode, struct page *node_page, unsigned int offset)
{
...
int f2fs_add_inline_entry(struct inode *dir, const struct qstr *new_name,
			  const struct fscrypt_name *fname, struct inode *inode,
			  nid_t ino, umode_t mode);
...
static inline bool f2fs_is_space_free(struct f2fs_sb_info *sbi)
{
	...
	block_t fs_threshold =
		(block_t)(SM_I(sbi)->main_segments * sbi->blocks_per_seg *
			  F2FS_FS_FREE_PERCENT) /
		100;
	block_t device_threshold =
		(block_t)(SM_I(sbi)->main_segments * sbi->blocks_per_seg *
			  F2FS_DEVICE_FREE_PERCENT) /
		100;
