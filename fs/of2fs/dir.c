...
static struct f2fs_dir_entry *
find_in_block(struct inode *dir, struct page *dentry_page,
	      struct fscrypt_name *fname, f2fs_hash_t namehash, int *max_slots,
					 ...
					 struct fscrypt_str *cf_name)
{
	...
	cf_name->len = utf8_casefold(dir->i_sb->s_encoding, iname,
				     cf_name->name, F2FS_NAME_LEN);
...
static inline bool f2fs_match_name(struct f2fs_dentry_ptr *d,
				   ...
				   unsigned long bit_pos, f2fs_hash_t namehash)
{
#ifdef CONFIG_UNICODE
...
add_dentry:
	...
	if (inode) {
		...
		page = f2fs_init_inode_metadata(inode, dir, new_name, orig_name,
						NULL);
		if (IS_ERR(page)) {
			err = PTR_ERR(page);
			goto fail;
		    ...
		    struct inode *inode, nid_t ino, umode_t mode)
{
	...
	if (f2fs_has_inline_dentry(dir))
		err = f2fs_add_inline_entry(dir, &new_name, fname, inode, ino,
					    mode);
