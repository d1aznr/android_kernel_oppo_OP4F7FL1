...
#define IS_CURSEC(sbi, secno)                                                  \
	(((secno) ==                                                           \
	 ...
	 ((secno) ==                                                           \
	  CURSEG_I(sbi, CURSEG_FRAGMENT_DATA)->segno / (sbi)->segs_per_sec))
#else
#define IS_CURSEC(sbi, secno)                                                  \
	(((secno) ==                                                           \
	  CURSEG_I(sbi, CURSEG_HOT_DATA)->segno / (sbi)->segs_per_sec) ||      \
...
#define SSR_CONTIG_DIRTY_NUMS                                                  \
	32 /*Dirty pages for LFS alloction in grading ssr . */
#define SSR_CONTIG_LARGE 256 /*Larege files */

enum { SEQ_NONE, SEQ_32BLKS, SEQ_256BLKS };
#endif
...
#ifdef CONFIG_F2FS_GRADING_SSR
enum { GRADING_SSR_OFF = 0, GRADING_SSR_ON };
#endif
...
struct segment_allocation {
	...
	void (*get_new_segment)(struct f2fs_sb_info *, unsigned int *, bool,
				int);
	void (*new_curseg)(struct f2fs_sb_info *, int, bool);
	void (*__new_curseg)(struct f2fs_sb_info *, struct curseg_info *, int,
			     bool);
#else
