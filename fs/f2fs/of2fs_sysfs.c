...
#define OF2FS_PROC_DEF(_name)                                                  \
	static int of2fs_##_name##_open(struct inode *inode,                   \
					struct file *file)                     \
	{                                                                      \
		return single_open(file, of2fs_##_name##_show,                 \
				   PDE_DATA(inode));                           \
	}                                                                      \
                                                                               \
...
static ssize_t of2fs_base_info_write(struct file *file, const char __user *buf,
...
static ssize_t of2fs_cp_info_write(struct file *file, const char __user *buf,
				   size_t length, loff_t *ppos)
{
	struct seq_file *seq = file->private_data;
	struct super_block *sb = seq->private;
...
static int of2fs_gc_info_show(struct seq_file *seq, void *p)
{
	...
	bd_lock(sbi);
	seq_printf(seq,
		   "%u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %u %llu\n",
...
static ssize_t of2fs_gc_info_write(struct file *file, const char __user *buf,
				   size_t length, loff_t *ppos)
{
	struct seq_file *seq = file->private_data;
	struct super_block *sb = seq->private;
...
static int of2fs_fsync_info_show(struct seq_file *seq, void *p)
{
	...
	bd_lock(sbi);
	seq_printf(seq,
		   "%u %u %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu\n",
		   bd->fsync_reg_file_count, bd->fsync_dir_count,
		   bd->fsync_time, bd->max_fsync_time, bd->fsync_wr_file_time,
...
static ssize_t of2fs_fsync_info_write(struct file *file, const char __user *buf,
				      size_t length, loff_t *ppos)
{
	struct seq_file *seq = file->private_data;
	struct super_block *sb = seq->private;
...
static int of2fs_hotcold_info_show(struct seq_file *seq, void *p)
{
	...
	seq_printf(
		...
		bd->hotcold_count[HC_WARM_NODE],
		bd->hotcold_count[HC_COLD_NODE], bd->hotcold_count[HC_META],
		bd->hotcold_count[HC_META_SB], bd->hotcold_count[HC_META_CP],
		bd->hotcold_count[HC_META_SIT], bd->hotcold_count[HC_META_NAT],
		bd->hotcold_count[HC_META_SSA], bd->hotcold_count[HC_DIRECT_IO],
		bd->hotcold_count[HC_GC_COLD_DATA],
