...
#define IM_TAG_DESC_LEN (128)
	...
	seq_printf(m, "%d %s (%d)", task->im_flag, desc, arg);
	return 0;
}
...
#ifdef CONFIG_OPLUS_FEATURE_TPD
static ssize_t tpd_write(struct file *file, const char __user *buf,
			 size_t count, loff_t *offset)
{
	struct task_struct *task;
	char buffer[PROC_NUMBUF];
...
static ssize_t tpd_read(struct file *file, char __user *buf, size_t count,
			loff_t *ppos)
{
	char buffer[PROC_NUMBUF];
...
static ssize_t st_tpd_write(struct file *file, const char __user *buf,
			    size_t count, loff_t *offset)
{
	struct task_struct *task;
	char buffer[PROC_NUMBUF];
...
static ssize_t st_tpd_read(struct file *file, char __user *buf, size_t count,
			   loff_t *ppos)
{
	char buffer[PROC_NUMBUF];
	struct task_struct *task = NULL;
...
#if defined(OPLUS_FEATURE_VIRTUAL_RESERVE_MEMORY) &&                           \
