...
static int pidstat_init(void)
{
	...
	for (i = 0; i < HIGH_LOAD_MAX_TYPE; i++) {
		...
		memset(g_pid_stat_mgt[i].head, 0,
		       g_pid_stat_mgt[i].max_count *
			       sizeof(*(g_pid_stat_mgt[i].head)));
...
static void pid_stat_reset(u32 type)
{
	...
	memset(g_pid_stat_mgt[type].head, 0,
	       g_pid_stat_mgt[type].index_curr *
		       sizeof(*(g_pid_stat_mgt[type].head)));
	memset(&g_pid_stat_mgt[type].data, 0,
...
static void pid_stat_search_insert(pid_t key_pid, pid_t pid, uid_t uid,
				   pid_t tgid, u32 type)
{
 ...
 */
static void get_cpu_load(u64 *total_time, u64 *busy_time, unsigned long cpumask)
{
	u32 i = 0;
...
bool high_load_tick_mask(unsigned long bits, u32 type, int threhold_up,
			 int threhold_down);
...
static void high_load_tickfn(struct work_struct *work)
{
	...
	if ((high_load_switch & (1 << LOAD_SWITCH_LBG)) != 0)
		high_load_tick_mask(MASK_LBG, CPUSET_LBG, highload_cpuset_lbg,
				    lowload_cpuset_lbg);

	/* GRP_BG: cpu4,5,6,7 */
	if ((high_load_switch & (1 << LOAD_SWITCH_BG)) != 0)
		high_load_tick_mask(MASK_BG, CPUSET_BG, highload_cpuset_bg,
				    lowload_cpuset_bg);

	/* GRP_HBG: cpu0,1,2,3,4 */
	if ((high_load_switch & (1 << LOAD_SWITCH_HBG)) != 0)
		high_load_tick_mask(MASK_HBG, CPUSET_HBG, highload_cpuset_hbg,
				    lowload_cpuset_hbg);

	/* GRP_HFG: cpu4,5,6,7 */
	if ((high_load_switch & (1 << LOAD_SWITCH_HFG)) != 0)
		high_load_tick_mask(MASK_HFG, CPUSET_HFG, highload_cpuset_hfg,
				    lowload_cpuset_hfg);
...
static struct task_id threhold_pid_list[HIGH_LOAD_MAX_TYPE][PID_LIST_MAX] = {
	0
};
...
static void cpus_proc_static_high(u32 type)
{
	...
	for (i = 0, curr = g_pid_stat_mgt[type].head;
	     i < g_pid_stat_mgt[type].index_curr; i++, curr++) {
		if (curr->count > get_threhold_bytype(type)) {
			...
			if (threhold_pid_size[type] < PID_LIST_MAX) {
				...
				threhold_pid_list[type][index].tgid =
					curr->tgid;
...
bool high_load_tick(void)
{
	...
	if (check_intervals >= CPU_LOAD_TIMER_RATE) {
		if (high_load_cnt[CPUSET_ALL] >= CPU_HIGH_LOAD_THRESHOLD &&
			...
			for (i = 0; i < CPU_NUMS; i++) {
				total_delta_time = total_time_per[i] -
						   last_total_time_per[i];
				if (!total_delta_time)
					usage_per[i] = 0;
				else
					usage_per[i] =
						((busy_time_per[i] -
						  last_busy_time_per[i]) *
						 HIGH_LOAD_PERCENT) /
						total_delta_time;
			}
		...
		} else if (high_load_cnt[CPUSET_ALL] ==
				   CPU_LOW_LOAD_THRESHOLD &&
...
static void high_freqs_load_tick(void)
{
...
#ifdef CONFIG_HISI_FREQ_STATS_COUNTING_IDLE
	ret_err = hisi_time_in_freq_get(CPU_NUMS - 1, freqs_time, freqs_len);
	if (ret_err)
		return;
	...
	for (i = 0; i < freqs_len; i++) {
		if (freqs_weight[i] < fg_freqs_threshold)
			continue;
		delta_freqs_time +=
			(freqs_time[i] - freqs_time_last[i]) * freqs_weight[i];
...
cpus_proc_static_tickfn_unused(struct work_struct *work)
{
	u32 type;
...
static ssize_t proc_clm_enable_write(struct file *file, const char __user *buf,
				     size_t count, loff_t *ppos)
{
	char buffer[PROC_NUMBUF];
	int err, enable;
...
static ssize_t proc_fg_freqs_threshold_read(struct file *file, char __user *buf,
					    size_t count, loff_t *ppos)
{
	char buffer[PROC_NUMBUF];
	size_t len = 0;
...
static ssize_t proc_clm_highload_all_read(struct file *file, char __user *buf,
					  size_t count, loff_t *ppos)
{
	char buffer[PROC_NUMBUF];
	size_t len = 0;
...
static ssize_t proc_clm_highload_grp_read(struct file *file, char __user *buf,
					  size_t count, loff_t *ppos)
{
	char buffer[PROC_NUMBUF];
	size_t len = 0;
...
static ssize_t proc_clm_lowload_grp_read(struct file *file, char __user *buf,
					 size_t count, loff_t *ppos)
{
	char buffer[PROC_NUMBUF];
	size_t len = 0;
...
struct proc_dir_entry *jank_calcload_proc_init(struct proc_dir_entry *pde)
{
	...
	entry = proc_create("clm_enable", S_IRUGO | S_IWUGO, pde,
			    &proc_clm_enable_operations);
	if (!entry) {
		pr_err("create clm_enable fail\n");
		goto err_clm_enable;
	}

	entry = proc_create("fg_freqs_threshold", S_IRUGO | S_IWUGO, pde,
			    &proc_fg_freqs_threshold_operations);
	if (!entry) {
		pr_err("create fg_freqs_threshold fail\n");
		goto err_fg_freqs_threshold;
	}

	entry = proc_create("clm_highload_all", S_IRUGO | S_IWUGO, pde,
			    &proc_clm_highload_all_operations);
	if (!entry) {
		pr_err("create clm_highload_all fail\n");
		goto err_clm_highload_all;
	}

	entry = proc_create("clm_highload_grp", S_IRUGO | S_IWUGO, pde,
			    &proc_clm_highload_grp_operations);
	if (!entry) {
		pr_err("create clm_highload_grp fail\n");
		goto err_clm_highload_grp;
	}

	entry = proc_create("clm_lowload_grp", S_IRUGO | S_IWUGO, pde,
			    &proc_clm_lowload_grp_operations);
	if (!entry) {
