...
#ifdef JANK_DEBUG
		jank_dbg(
			"[CPUSET] DEBUG BYHP: [S] silver_load=%d, gold_load=%d, dstate_time=%llu\n",
			silver_load, gold_load, dstate_time);
		jank_systrace_print(JANK_SYSTRACE_TASKTRACK, "cpuset",
				    BG_CPU_0_7);
		jank_systrace_print(JANK_SYSTRACE_TASKTRACK, "cpuset_pid", pid);
		jank_systrace_print(JANK_SYSTRACE_TASKTRACK, "cpuset_s1",
				    silver_load);
		jank_systrace_print(JANK_SYSTRACE_TASKTRACK, "cpuset_s2",
				    gold_load);
		jank_systrace_print(JANK_SYSTRACE_TASKTRACK, "cpuset_s3",
				    dstate_time);
#endif
...
#ifdef JANK_DEBUG
		jank_dbg(
			"[CPUSET] DEBUG BYHP: [C] silver_load=%d, gold_load=%d, dstate_time=%llu\n",
			silver_load, gold_load, dstate_time);
		jank_systrace_print(JANK_SYSTRACE_TASKTRACK, "cpuset",
				    BG_CPU_0_3);
		jank_systrace_print(JANK_SYSTRACE_TASKTRACK, "cpuset_pid", 0);
		jank_systrace_print(JANK_SYSTRACE_TASKTRACK, "cpuset_c1",
				    silver_load);
		jank_systrace_print(JANK_SYSTRACE_TASKTRACK, "cpuset_c2",
				    gold_load);
		jank_systrace_print(JANK_SYSTRACE_TASKTRACK, "cpuset_c3",
				    dstate_time);
#endif
	}
}
...
{
	seq_printf(m, "%d %d %d %d %d %d\n", s_dstate_threshold, s_silver_usage,
		   s_gold_usage, c_dstate_threshold, c_silver_usage,
		   c_gold_usage);
...
static int proc_cpuset_threshold_open(struct inode *inode, struct file *file)
{
...
jank_cpuset_threshold_proc_init(struct proc_dir_entry *pde)
{
	struct proc_dir_entry *entry = NULL;

	entry = proc_create("cpuset_threshold", S_IRUGO | S_IWUGO, pde,
			    &proc_cpuset_threshold_operations);
	if (!entry) {
