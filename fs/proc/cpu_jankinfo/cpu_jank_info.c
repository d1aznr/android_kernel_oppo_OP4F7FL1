...
void android_vh_irqtime_account_process_tick_handler(void *unused,
						     struct task_struct *p,
						     struct rq *rq,
						     int user_tick, int ticks)
{
	jankinfo_update_time_info(rq, p, ticks * TICK_NSEC);
}
#else
void android_vh_account_task_time_handler(void *unused, struct task_struct *p,
					  struct rq *rq, int user_tick)
{
...
void android_vh_cpufreq_fast_switch_handler(void *unused,
					    ...
					    unsigned int old_target_freq)
{
	jankinfo_update_freq_reach_limit_count(
		policy, old_target_freq, target_freq, DO_CLAMP | DO_INCREASE);
}
