...
static void update_load_info(struct cputime *cputime, struct task_struct *p,
			     u64 now)
{
	u32 i, idx, now_idx;
	u32 cpu;
	...
	for (i = 0; i < MONITOR_WIN_CNT; i++) {
		...
		span_time = (i == 0) ? (now & JANK_WIN_SIZE_IN_NS_MASK) :
				       JANK_WIN_SIZE_IN_NS;
		...
		if (time_ratio(running_time, span_time) >= HIGH_LOAD_TOTAL)
			score.total++;
		else
			score.total = 0;

		if (time_ratio(default_runtime, span_time) >= HIGH_LOAD_DEFAULT)
			score.def++;
		else
			score.def = 0;
		...
		if (time_ratio(topapp_runtime, span_time) >= HIGH_LOAD_TOPAPP)
			score.topapp++;
		else
			score.topapp = 0;
	}
...
static int proc_load_indicator_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%llu %llu %llu %llu %llu\n", recorder.total,
		   recorder.def, recorder.fg, recorder.background,
		   recorder.topapp);

	return 0;
}

static int proc_load_indicator_open(struct inode *inode, struct file *file)
{
...
struct proc_dir_entry *jank_load_indicator_proc_init(struct proc_dir_entry *pde)
{
	return proc_create("load_indicator", S_IRUGO, pde,
			   &proc_load_indicator_operations);
}
