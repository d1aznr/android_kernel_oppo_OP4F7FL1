...
static __maybe_unused void get_cgroup_delta(u64 now, u32 cpu, u32 grp,
					    struct cgropu_delta *d)
{
	u64 delta;
...
void jankinfo_update_time_info(struct rq *rq, struct task_struct *p, u64 time)
{
	...
	jank_dbg("q3: cpu=%d, time=%lld(%lld), calldelta=%lld(%lld)\n",
		 smp_processor_id(), time / NSEC_PER_MSEC, time,
	...
	runtime += kcs->cpustat[CPUTIME_NICE] - cpustat_bak[cpu][CPUTIME_NICE];
	runtime +=
		kcs->cpustat[CPUTIME_SYSTEM] - cpustat_bak[cpu][CPUTIME_SYSTEM];
	runtime += kcs->cpustat[CPUTIME_IRQ] - cpustat_bak[cpu][CPUTIME_IRQ];
	runtime += kcs->cpustat[CPUTIME_SOFTIRQ] -
		   cpustat_bak[cpu][CPUTIME_SOFTIRQ];
	runtime +=
		kcs->cpustat[CPUTIME_STEAL] - cpustat_bak[cpu][CPUTIME_STEAL];
	runtime +=
		kcs->cpustat[CPUTIME_GUEST] - cpustat_bak[cpu][CPUTIME_GUEST];
	...
	cpustat_bak[cpu][CPUTIME_GUEST_NICE] = kcs->cpustat[CPUTIME_GUEST_NICE];
	...
	split_window(now, runtime_scale, &runtime_scale_a, &runtime_scale_cnt,
		     &runtime_scale_b);
	...
	if (!delta_cnt && last_idx == now_idx) {
		...
		cputime[cpu].fg_running_time[now_idx] +=
			is_foreground(p) ?
				cgropu_times_delta[cpu][CGROUP_FOREGROUND]
					.delta_b :
				0;
		cputime[cpu].fg_running_time[now_idx] +=
			is_topapp(p) ?
		...
		cputime[cpu].default_runtime[now_idx] +=
			is_default(p) ?
				cgropu_times_delta[cpu][CGROUP_DEFAULT].delta_b :
				0;
		...
		cputime[cpu].background_runtime[now_idx] +=
			is_background(p) ?
				cgropu_times_delta[cpu][CGROUP_BACKGROUND]
					.delta_b :
				0;
		...
		cputime[cpu].fg_running_time[last_idx] +=
			is_topapp(p) ?
				cgropu_times_delta[cpu][CGROUP_TOP_APP].delta_a :
				0;
		...
		cputime[cpu].default_runtime[last_idx] +=
			is_default(p) ?
				cgropu_times_delta[cpu][CGROUP_DEFAULT].delta_a :
				0;
		...
		for (i = 0; i < min_t(u64, runtime_cnt, (JANK_WIN_CNT - 1));
		     i++) {
			idx = (now_idx - 1 - i + JANK_WIN_CNT) &
			      JANK_WIN_CNT_MASK;
		...
		if (runtime_cnt < JANK_WIN_CNT - 1 && runtime_a) {
			idx = (now_idx + JANK_WIN_CNT - 1 -
			       min_t(u64, runtime_cnt, JANK_WIN_CNT)) &
			      JANK_WIN_CNT_MASK;
			cputime[cpu].running_time[idx] += runtime_a;
			cputime[cpu].running_time32[idx] +=
				compat ? runtime_a : 0;
			cputime[cpu].running_time32_scale[idx] +=
				compat ? runtime_scale_a : 0;
			cputime[cpu].fg_running_time[idx] +=
				is_foreground(p) ?
					cgropu_times_delta[cpu]
							  [CGROUP_FOREGROUND]
								  .delta_a :
					0;
			cputime[cpu].fg_running_time[idx] +=
				is_topapp(p) ?
					cgropu_times_delta[cpu][CGROUP_TOP_APP]
						.delta_a :
					0;

			cputime[cpu].default_runtime[idx] +=
				is_default(p) ?
					cgropu_times_delta[cpu][CGROUP_DEFAULT]
						.delta_a :
					0;
			cputime[cpu].foreground_runtime[idx] +=
				is_foreground(p) ?
...
u32 get_cpu_load32_scale(u32 win_cnt, struct cpumask *mask)
{
	...
	if (win_cnt == 1) {
		...
		for_each_cpu (i, mask) {
			...
			last_runtime =
				cputime[i].running_time32_scale[last_index];
			...
			if (!delta_cnt && last_index == now_index) {
				...
				tmp_time += (last_runtime_r * now_a) >>
				   ...
				   ((last_index + 1) & JANK_WIN_CNT_MASK) ==
					   now_index) {
				tmp_time = increase_b;
				tmp_time +=
		...
		for (j = 0; j < win_cnt; j++) {
			for_each_cpu (i, mask) {
				...
				last_runtime =
					cputime[i]
						.running_time32_scale[last_index];
				...
				if (!delta_cnt && last_index == now_index) {
					...
					if (j <= delta_cnt) {
						...
						if (idx == last_index) {
							...
							run_time +=
								cputime[i].running_time32_scale
									[idx];
							total_time +=
								JANK_WIN_SIZE_IN_NS;
						}
	...
	if (win_cnt == 1) {
		...
		for_each_cpu (i, mask) {
			...
			last_index_r = (last_index + JANK_WIN_CNT - 1) &
				       JANK_WIN_CNT_MASK;
			last_runtime_r =
				cputime[i].running_time32[last_index_r];
		...
		for (j = 0; j < win_cnt; j++) {
			for_each_cpu (i, mask) {
				...
				last_index = cputime[i].last_index;
				last_runtime =
					cputime[i].running_time32[last_index];
	...
	if (win_cnt == 1) {
		...
		for_each_cpu (i, mask) {
			...
			jank_dbg("r1:cpu%d, tmp_time=%llu, true1=%d, "
				 ...
				 i, tmp_time >> 20,
				 tmp_time > JANK_WIN_SIZE_IN_NS, delta >> 20,
				 delta_cnt, last_index, now_index,
				 last_runtime >> 20,
				 last_runtime > JANK_WIN_SIZE_IN_NS,
				 last_runtime_r >> 20, increase >> 20,
				 increase_a >> 20, increase_cnt,
				 increase_b >> 20, now_a >> 20, now_b >> 20,
		...
		for (j = 0; j < win_cnt; j++) {
			for_each_cpu (i, mask) {
				...
				last_index = cputime[i].last_index;
				last_runtime =
					cputime[i].running_time[last_index];
...
void jank_group_ratio_show(struct seq_file *m, u32 win_idx, u64 now)
{
	...
	for (i = 0; i < CPU_NUMS; i++) {
		...
		last_fg_time_r =
			cputime_snap[i].foreground_runtime[last_index_r];
		last_bg_time_r =
			cputime_snap[i].background_runtime[last_index_r];
		last_topapp_time_r =
			cputime_snap[i].topapp_runtime[last_index_r];
			...
			if (win_idx == 0) {
				if (now_delta < JANK_WIN_SIZE_THRESHOLD) {
					...
					tmp_fgtime = last_fgtime +
						     ...
						     delta_fg.delta;
					tmp_df_time =
						last_df_time + delta_df.delta;
					tmp_fg_time =
						last_fg_time + delta_fg.delta;
					tmp_bg_time =
						last_bg_time + delta_bg.delta;
				...
				tmp_fgtime =
					cputime_snap[i].fg_running_time[idx];
				tmp_df_time =
					cputime_snap[i].default_runtime[idx];
				tmp_fg_time =
					cputime_snap[i].foreground_runtime[idx];
				tmp_bg_time =
					cputime_snap[i].background_runtime[idx];
				tmp_topapp_time =
					cputime_snap[i].topapp_runtime[idx];
				tmp_runtime = cputime_snap[i].running_time[idx];
			}
		} else if (!delta_cnt && ((last_index + 1) &
					  JANK_WIN_CNT_MASK) == now_index) {
			if (win_idx == 0) {
				if (now_delta < JANK_WIN_SIZE_THRESHOLD) {
					tmp_fgtime = last_fgtime;
					...
					tmp_fg_time =
						cputime_snap[i]
							.foreground_runtime[idx] +
						delta_fg.delta_b;
					tmp_bg_time =
						cputime_snap[i]
							.background_runtime[idx] +
						delta_bg.delta_b;
					tmp_topapp_time =
						cputime_snap[i]
							.topapp_runtime[idx] +
		...
		/* Prevents the error of dividing by zero */
		tmp_runtime =
			(tmp_runtime == 0) ? JANK_WIN_SIZE_IN_NS : tmp_runtime;
	...
	for (cluster_id = 0; cluster_id < cluster_num; cluster_id++) {
		...
		seq_printf(m, "%d %d %d %d ", rlt_df / cpu_nr, rlt_fg / cpu_nr,
			   rlt_bg / cpu_nr, rlt_top / cpu_nr);
	}
}
...
void jank_busy_ratio_show(struct seq_file *m, u32 win_idx, u64 now)
{
	...
	for (i = 0; i < CPU_NUMS; i++) {
		...
		last_index_r =
			(last_index - 1 + JANK_WIN_CNT) & JANK_WIN_CNT_MASK;
		last_runtime_r = cputime_snap[i].running_time[last_index_r];
		...
		/* Prevents the error of dividing by zero */
		last_win_delta = (last_win_delta == 0) ? JANK_WIN_SIZE_IN_NS :
							 last_win_delta;
		...
		if (!delta_cnt && last_index == now_index) {
			if (win_idx == 0) {
				if (now_delta < JANK_WIN_SIZE_THRESHOLD) {
					...
					tmp_runtime = last_runtime + increase;
					tmp_avg =
						(tmp_runtime * 100) / now_delta;
				}
			} else {
				tmp_avg = (cputime_snap[i].running_time[idx] *
					   100) >>
					  JANK_WIN_SIZE_SHIFT_IN_NS;
			}
		} else if (!delta_cnt && ((last_index + 1) &
...
static int proc_cpu_info_open(struct inode *inode, struct file *file)
{
...
static int proc_cpu_info_sig_open(struct inode *inode, struct file *file)
{
...
static int proc_cpu_load32_open(struct inode *inode, struct file *file)
{
...
static int proc_cpu_load32_scale_open(struct inode *inode, struct file *file)
{
...
struct proc_dir_entry *jank_cpuload_proc_init(struct proc_dir_entry *pde)
{
	struct proc_dir_entry *entry = NULL;

	entry = proc_create("cpu_info", S_IRUGO, pde,
			    &proc_cpu_info_operations);
	if (!entry) {
		pr_err("create cpu_info fail\n");
		goto err_cpu_info;
	}

	entry = proc_create("cpu_info_sig", S_IRUGO, pde,
			    &proc_cpu_info_sig_operations);
	if (!entry) {
