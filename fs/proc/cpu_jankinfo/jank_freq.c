...
void jank_currfreq_update_win(u64 now)
{
	...
	for (cpu = 0; cpu < CPU_NUMS; cpu++) {
		...
		for (i = 0; i < win_cnt; i++) {
			...
			jank_dbg(
				"e1: cpu=%d, now_idx=%d, last_idx=%d, win_cnt=%d, "
				"idx=%d, i=%d, freq=%d\n",
				cpu, now_idx, last_idx, win_cnt, idx, i, freq);
		}
		cur_freq[cpu].last_update_time = now;
	}
}
...
void jank_curr_freq_show(struct seq_file *m, u32 win_idx, u64 now)
{
	...
	for (cls = 0; cls < cluster_num; cls++) {
		...
		jank_dbg("e3: cls=%d, start_cpu=%d, "
			 ...
			 "freq=%d\n",
			 cls, start_cpu, now_index, win_idx, idx,
			 cur_freq[start_cpu].freq[idx]);
	}
}

/* target_freq > policy->max */
static void update_freq_count(struct cpufreq_policy *policy, u32 old_freq,
			      u32 new_freq, u32 flags)
{
	...
	if (policy->cpu == 0) {
		jank_systrace_print_idx(JANK_SYSTRACE_CPUFREQ, "0flags", 0,
					flags);
		jank_systrace_print_idx(JANK_SYSTRACE_CPUFREQ, "0increase", 0,
					freq_increase);
		jank_systrace_print_idx(JANK_SYSTRACE_CPUFREQ, "1clamp", 0,
					freq_clamp);
		jank_systrace_print_idx(JANK_SYSTRACE_CPUFREQ, "2cpu", 0,
					policy->cpu);
		jank_systrace_print_idx(JANK_SYSTRACE_CPUFREQ, "3cur", 0,
					policy->cur);
		jank_systrace_print_idx(JANK_SYSTRACE_CPUFREQ, "4min", 0,
					policy->min);
		jank_systrace_print_idx(JANK_SYSTRACE_CPUFREQ, "5max", 0,
					policy->max);
		jank_systrace_print_idx(JANK_SYSTRACE_CPUFREQ, "6target_freq",
					0, target_freq);
		jank_systrace_print_idx(JANK_SYSTRACE_CPUFREQ, "7new_freq", 0,
					new_freq);
		jank_systrace_print_idx(JANK_SYSTRACE_CPUFREQ, "8old_freq", 0,
					old_freq);
	}
#endif
	...
	jank_dbg("m1: cpu=%d, old_freq=%d, new_freq=%d "
		 ...
		 cpu, old_freq, new_freq, policy->cur, policy->min, policy->max,
		 freq_increase, freq_clamp, now_idx,
		 time2winidx(last_update_time), win_cnt);
	...
	for (i = 0; i < win_cnt; i++) {
		...
		if (freq_increase) {
			...
			if (policy->cpu == 0) {
				...
				jank_systrace_print_idx(JANK_SYSTRACE_CPUFREQ,
							"3idx", 0, idx);
				jank_systrace_print_idx(
					JANK_SYSTRACE_CPUFREQ, "4valid", 0,
					timestamp_is_valid(timestamp, now));
				jank_systrace_print_idx(
					JANK_SYSTRACE_CPUFREQ, "5cnt", 0,
			...
			if (timestamp_is_valid(timestamp, now)) {
				if (i == 0) {
					freq_cnt[cpu].increase[idx].cnt++;
					freq_cnt[cpu].increase[idx].timestamp =
						now;
				}
			} else {
				if (i == 0)
		...
		if (freq_clamp) {
			...
			if (timestamp_is_valid(timestamp, now)) {
				if (i == 0) {
					freq_cnt[cpu].clamp[idx].cnt++;
					freq_cnt[cpu].clamp[idx].timestamp =
						now;
				}
			} else {
				if (i == 0)
...
void jank_increase_freq_show(struct seq_file *m, u32 win_idx, u64 now)
{
	u32 tmp_cnt;
	u64 timestamp;
	u32 now_index, idx, i;
