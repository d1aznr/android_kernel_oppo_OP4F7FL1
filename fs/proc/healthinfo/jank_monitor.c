			    ...
			    unsigned int cpu, u64 delta)
{
	...
	if (!ltt_cpu_nr) {
		...
		ltt_cpu_nr = cpumask_weight(
			topology_core_cpumask(rd->wrd.min_cap_orig_cpu));
#else
		ltt_cpu_nr = 4;
#endif
#else /* KERNEL_VERSION */
		ltt_cpu_nr = cpumask_weight(
			topology_core_cpumask(rd->min_cap_orig_cpu));
...
static int proc_jank_trace_show(struct seq_file *m, void *v)
{
	...
	d_time = p->jank_info.d_state.iowait_ns +
		 p->jank_info.d_state.mutex_ns +
		 p->jank_info.d_state.downread_ns +
		 p->jank_info.d_state.downwrite_ns +
		 p->jank_info.d_state.other_ns;

	s_time = p->jank_info.s_state.binder_ns +
		 p->jank_info.s_state.futex_ns + p->jank_info.s_state.epoll_ns +
		 p->jank_info.s_state.other_ns;
	...
	seq_printf(
		...
		big_time / NSEC_PER_MSEC, ltt_time / NSEC_PER_MSEC,
		rn_time / NSEC_PER_MSEC, d_time / NSEC_PER_MSEC,
		iow_time / NSEC_PER_MSEC, s_time / NSEC_PER_MSEC,
		binder_time / NSEC_PER_MSEC, futex_time / NSEC_PER_MSEC);
	put_task_struct(p);
	return 0;
}
