...
u64 time_threshold[MAX_THRESHOLD_NUM] = { 4000000, 4000000, 4000000, 4000000,
					  4000000 };
...
void update_backtrace_info(struct task_struct *p, u64 start_time)
{
	...
	task_backtrace.sched_info_one =
		task_sched_info_backtrace | (p->pid << 8);
...
void update_task_sched_info(struct task_struct *p, u64 delay, int type, int cpu)
{
	...
	if (pid_num != MAX_PID_NUM) {
		...
		case task_sched_info_running:
			...
			sched_info.sched_info_one = sched_info.sched_info_one |
						    (cur_sched_clock << 8);
			sched_info.sched_info_two = delay | (p_tid << 24);
			sched_info.sched_info_two =
				sched_info.sched_info_two | (pid_num << 58);
			break;
		...
		case task_sched_info_runnable:
			...
			sched_info.sched_info_two = delay | (p_tid << 24);
			sched_info.sched_info_two =
				sched_info.sched_info_two | (wake_tid << 40);
			sched_info.sched_info_two =
				sched_info.sched_info_two | (pid_num << 58);
			break;
...
void update_freq_limit_info(struct cpufreq_policy *policy)
{
	...
	if (prev_freq_min[policy->cpu] == min &&
	...
	freq_limit_info.sched_info_one =
		task_sched_info_freq_limit | (cpu << 5);
	freq_limit_info.sched_info_one =
		freq_limit_info.sched_info_one | (clock << 8);
	freq_limit_info.sched_info_two = min | (max << 24);
	freq_limit_info.sched_info_two =
		freq_limit_info.sched_info_two | (cur_tid << 48);
...
void update_cpu_isolate_info(int cpu, u64 type)
{
	...
	isolate_info.sched_info_one =
		isolate_info.sched_info_one | (clock << 8);
...
static int sched_buffer_open(struct inode *inode, struct file *file)
{
	...
	data_size = (arr_write < arr_read) ?
			    (BUFFER_SIZE + arr_write - arr_read) :
			    arr_write - arr_read;
	cur_read = arr_read;
	cur_write = arr_write;
						 ...
						 size_t count, loff_t *ppos)
{
	...
	if (err) {
		sched_err(
			"enable/disable task_sched_info_enable fail. buffer_info: %s\n",
			buffer);
		return err;
	}
...
static int proc_sched_info_threshold_show(struct seq_file *m, void *v)
{
	...
	seq_printf(
		...
		time_threshold[0], time_threshold[1], time_threshold[2],
		time_threshold[3], time_threshold[4]);
	return 0;
}
...
static int __init sched_info_init(void)
{
	...
	proc_entry = proc_create("task_sched_info_enable", S_IRUGO | S_IWUGO,
				 sched_info,
				 &proc_task_sched_info_enable_operations);
	if (!proc_entry) {
	...
	proc_entry =
		proc_create("sched_info_threshold", S_IRUGO | S_IWUGO,
			    sched_info, &proc_sched_info_threshold_operations);
	if (!proc_entry) {
		sched_err("create sched_info_threshold fail\n");
		goto ERROR_INIT_VERSION;
	}

	proc_entry = proc_create("d_convert", S_IRUGO | S_IWUGO, sched_info,
				 &proc_d_convert_operations);
	if (!proc_entry) {
