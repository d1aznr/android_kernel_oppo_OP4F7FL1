...
static const struct seq_operations seq_ops = { .start = task_cpustats_start,
	...
	for_each_possible_cpu (i) {
		...
		for (j = 0; j < MAX_CTP_WINDOW; j++) {
			...
			if (ts->begin >= begin && ts->end <= end) {
				...
				if (as->pwr == 0)
					memcpy(as->comm, ts->comm,
					       TASK_COMM_LEN);
				as->pwr += get_power(i, ts->freq) *
					   jiffies_to_msecs(r_time);
				as->tgid = ts->tgid;
			}
		}
	}
