...
static ssize_t proc_target_process_write(struct file *file,
...
static int proc_normalize_running_time_open(struct inode *inode,
					    struct file *file)
{
...
static int proc_normalize_real_time_show(struct seq_file *m, void *v)
{
	...
	for_each_possible_cpu (i) {
		normalize_wall_time +=
			wall_time * (u64)topology_get_cpu_scale(i);
	}
...
static ssize_t proc_monitor_status_write(struct file *file,
					 const char __user *buf, size_t count,
					 loff_t *ppos)
{
	char buffer[PROC_NUMBUF];
...
static ssize_t proc_monitor_status_read(struct file *file, char __user *buf,
...
static int __init load_info_init(void)
{
	...
	proc_entry = proc_create("monitor_status", S_IRUGO | S_IWUGO,
				 task_runtime, &proc_monitor_status_operations);
	if (!proc_entry) {
		task_load_err("create monitor_status fail\n");
		goto ERROR_INIT_VERSION;
	}

	proc_entry = proc_create("running_time", S_IRUGO | S_IWUGO,
				 task_runtime, &proc_running_time_operations);
	if (!proc_entry) {
		task_load_err("create running_time fail\n");
		goto ERROR_INIT_VERSION;
	}

	proc_entry = proc_create("normalize_running_time", S_IRUGO | S_IWUGO,
				 task_runtime,
				 &proc_normalize_running_time_operations);
	if (!proc_entry) {
		task_load_err("create normalize_running_time fail\n");
		goto ERROR_INIT_VERSION;
	}

	proc_entry = proc_create("real_time", S_IRUGO | S_IWUGO, task_runtime,
				 &proc_real_time_operations);
	if (!proc_entry) {
	...
	proc_entry =
		proc_create("normalize_real_time", S_IRUGO | S_IWUGO,
			    task_runtime, &proc_normalize_real_time_operations);
	if (!proc_entry) {
		task_load_err("create normalize_real_time fail\n");
		goto ERROR_INIT_VERSION;
	}

	proc_entry = proc_create("target_process", S_IRUGO | S_IWUGO,
				 task_runtime, &proc_target_process_operations);
	if (!proc_entry) {
