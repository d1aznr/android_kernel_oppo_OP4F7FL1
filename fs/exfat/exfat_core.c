...
static u8 free_bit[] = {
	...
	4, 0, 1, 0, 2, /*   0 ~  19 */
	0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 5, 0, 1, 0,
	2, 0, 1, 0, 3, /*  20 ~  39 */
	0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0,
	3, 0, 1, 0, 2, /*  40 ~  59 */
	0, 1, 0, 6, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0,
	2, 0, 1, 0, 4, /*  60 ~  79 */
	0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0,
	5, 0, 1, 0, 2, /*  80 ~  99 */
	0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0,
	2, 0, 1, 0, 3, /* 100 ~ 119 */
	0, 1, 0, 2, 0, 1, 0, 7, 0, 1, 0, 2, 0, 1, 0,
	3, 0, 1, 0, 2, /* 120 ~ 139 */
	0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0,
	2, 0, 1, 0, 5, /* 140 ~ 159 */
	0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0,
	4, 0, 1, 0, 2, /* 160 ~ 179 */
	0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 6, 0, 1, 0,
	2, 0, 1, 0, 3, /* 180 ~ 199 */
	0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0,
	3, 0, 1, 0, 2, /* 200 ~ 219 */
	0, 1, 0, 5, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0,
	2, 0, 1, 0, 4, /* 220 ~ 239 */
...
static u8 used_bit[] = {
	...
	1, 2, 2, 3, /*   0 ~  19 */
	2, 3, 3, 4, 2, 3, 3, 4, 3, 4, 4, 5, 1, 2, 2, 3,
	2, 3, 3, 4, /*  20 ~  39 */
	2, 3, 3, 4, 3, 4, 4, 5, 2, 3, 3, 4, 3, 4, 4, 5,
	3, 4, 4, 5, /*  40 ~  59 */
	4, 5, 5, 6, 1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4,
	3, 4, 4, 5, /*  60 ~  79 */
	2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6,
	2, 3, 3, 4, /*  80 ~  99 */
	3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 3, 4, 4, 5,
	4, 5, 5, 6, /* 100 ~ 119 */
	4, 5, 5, 6, 5, 6, 6, 7, 1, 2, 2, 3, 2, 3, 3, 4,
	2, 3, 3, 4, /* 120 ~ 139 */
	3, 4, 4, 5, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5,
	4, 5, 5, 6, /* 140 ~ 159 */
	2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6,
	3, 4, 4, 5, /* 160 ~ 179 */
	4, 5, 5, 6, 4, 5, 5, 6, 5, 6, 6, 7, 2, 3, 3, 4,
	3, 4, 4, 5, /* 180 ~ 199 */
	3, 4, 4, 5, 4, 5, 5, 6, 3, 4, 4, 5, 4, 5, 5, 6,
	4, 5, 5, 6, /* 200 ~ 219 */
	5, 6, 6, 7, 3, 4, 4, 5, 4, 5, 5, 6, 4, 5, 5, 6,
	5, 6, 6, 7, /* 220 ~ 239 */
...
s32 ffsReadFile(struct inode *inode, FILE_ID_T *fid, void *buffer, u64 count,
		u64 *rcount)
{
	...
	while (count > 0) {
		...
		offset = (s32)(fid->rwoffset &
			       (p_fs->cluster_size -
				1)); /* byte offset in cluster   */
		sec_offset =
			offset >>
			p_bd->sector_size_bits; /* sector offset in cluster */
...
s32 ffsWriteFile(struct inode *inode, FILE_ID_T *fid, void *buffer, u64 count,
		 u64 *wcount)
{
	...
	if (fid->size == 0)
		...
		num_clusters =
			(s32)((fid->size - 1) >> p_fs->cluster_size_bits) + 1;
	...
	while (count > 0) {
		...
		if (clu == CLUSTER_32(~0)) {
			num_alloc =
				(s32)((count - 1) >> p_fs->cluster_size_bits) +
				1;
			...
			num_alloced = p_fs->fs_func->alloc_cluster(
				sb, num_alloc, &new_clu);
			if (num_alloced == 0)
				break;
...
s32 ffsTruncateFile(struct inode *inode, u64 old_size, u64 new_size)
{
	...
	if (new_size > 0) {
		num_clusters =
			(s32)((new_size - 1) >> p_fs->cluster_size_bits) + 1;
...
static void update_parent_info(FILE_ID_T *fid, struct inode *parent_inode)
{
	...
	if (unlikely((parent_fid->flags != fid->dir.flags) ||
		...
		fid->dir.size =
			((parent_fid->size + (p_fs->cluster_size - 1)) >>
			 p_fs->cluster_size_bits);
	}
}

/* ffsMoveFile : move(rename) a old file into a new file */
s32 ffsMoveFile(struct inode *old_parent_inode, FILE_ID_T *fid,
...
s32 ffsSetAttr(struct inode *inode, u32 attr)
{
	...
	if (is_dir) {
		if ((fid->dir.dir == p_fs->root_dir) && (fid->entry == -1)) {
			if (p_fs->dev_ejected)
				return FFS_MEDIAERR;
			return FFS_SUCCESS;
		}
...
s32 ffsGetStat(struct inode *inode, DIR_ENTRY_T *info)
{
	...
	*(uni_name.name) = 0x0;
	...
	if (*(uni_name.name) == 0x0 && p_fs->vol_type != EXFAT)
		get_uni_name_from_dos_entry(sb, (DOS_DENTRY_T *)ep, &uni_name,
					    0x1);
...
s32 ffsReadDir(struct inode *inode, DIR_ENTRY_T *dir_entry)
{
	...
	while (clu.dir != CLUSTER_32(~0)) {
		...
		for (; i < dentries_per_clu; i++, dentry++) {
			...
			*(uni_name.name) = 0x0;
			p_fs->fs_func->get_uni_name_from_ext_entry(
				sb, &dir, dentry, uni_name.name);
			if (*(uni_name.name) == 0x0 && p_fs->vol_type != EXFAT)
				get_uni_name_from_dos_entry(
					sb, (DOS_DENTRY_T *)ep, &uni_name, 0x1);
			...
			if (p_fs->vol_type == EXFAT) {
				...
				get_uni_name_from_dos_entry(
					sb, (DOS_DENTRY_T *)ep, &uni_name, 0x0);
...
void fs_set_vol_flags(struct super_block *sb, u32 new_flag)
{
	...
	if (p_fs->vol_type == EXFAT) {
		if (p_fs->pbr_bh == NULL) {
			if (sector_read(sb, p_fs->PBR_sector, &(p_fs->pbr_bh),
					1) != FFS_SUCCESS)
				return;
		}
...
s32 load_alloc_bitmap(struct super_block *sb)
{
	...
	while (clu.dir != CLUSTER_32(~0)) {
		...
		for (i = 0; i < p_fs->dentries_per_clu; i++) {
			ep = (BMAP_DENTRY_T *)get_entry_in_dir(sb, &clu, i,
							       NULL);
			if (!ep)
				return FFS_MEDIAERR;
			...
			if (ep->flags == 0x0) {
				...
				p_fs->vol_amap = (struct buffer_head **)kmalloc(
 ...
 */
s32 __load_upcase_table(struct super_block *sb, u32 sector, u32 num_sectors,
			u32 utbl_checksum)
{
	int i, ret = FFS_ERROR;
	u32 j;
	...
	upcase_table = p_fs->vol_utbl =
		(u16 **)kmalloc(UTBL_COL_COUNT * sizeof(u16 *), GFP_KERNEL);
		...
		for (i = 0; i < p_bd->sector_size && index <= 0xFFFF; i += 2) {
			...
			if (skip) {
				...
				if (upcase_table[col_index] == NULL) {
					DPRINTK("alloc = 0x%X\n", col_index);
					upcase_table[col_index] = (u16 *)kmalloc(
						UTBL_ROW_COUNT * sizeof(u16),
						GFP_KERNEL);
					if (upcase_table[col_index] == NULL) {
						ret = FFS_MEMORYERR;
						goto error;
...
s32 load_upcase_table(struct super_block *sb)
{
	...
	while (clu.dir != CLUSTER_32(~0)) {
		for (i = 0; i < p_fs->dentries_per_clu; i++) {
			ep = (CASE_DENTRY_T *)get_entry_in_dir(sb, &clu, i,
							       NULL);
			if (!ep)
				return FFS_MEDIAERR;
			...
			num_sectors =
				((tbl_size - 1) >> p_bd->sector_size_bits) + 1;
...
s32 fat_init_dir_entry(struct super_block *sb, CHAIN_T *p_dir, s32 entry,
...
s32 exfat_init_dir_entry(struct super_block *sb, CHAIN_T *p_dir, s32 entry,
			 u32 type, u32 start_clu, u64 size)
{
	u32 sector;
	u8 flags;
	...
	strm_ep = (STRM_DENTRY_T *)get_entry_in_dir(sb, p_dir, entry + 1,
						    &sector);
	if (!strm_ep)
		return FFS_MEDIAERR;
...
s32 fat_init_ext_entry(struct super_block *sb, CHAIN_T *p_dir, s32 entry,
		       s32 num_entries, UNI_NAME_T *p_uniname,
		       DOS_NAME_T *p_dosname)
{
	...
	if ((--num_entries) > 0) {
		...
		for (i = 1; i < num_entries; i++) {
			ext_ep = (EXT_DENTRY_T *)get_entry_in_dir(
				sb, p_dir, entry - i, &sector);
			if (!ext_ep)
				return FFS_MEDIAERR;
...
s32 exfat_init_ext_entry(struct super_block *sb, CHAIN_T *p_dir, s32 entry,
			 s32 num_entries, UNI_NAME_T *p_uniname,
			 DOS_NAME_T *p_dosname)
{
	int i;
...
void fat_delete_dir_entry(struct super_block *sb, CHAIN_T *p_dir, s32 entry,
			  s32 order, s32 num_entries)
{
	int i;
	u32 sector;
	DENTRY_T *ep;
...
void exfat_delete_dir_entry(struct super_block *sb, CHAIN_T *p_dir, s32 entry,
			    s32 order, s32 num_entries)
{
	int i;
	u32 sector;
...
static s32 _walk_fat_chain(struct super_block *sb, CHAIN_T *p_dir,
...
s32 find_location(struct super_block *sb, CHAIN_T *p_dir, s32 entry,
		  u32 *sector, s32 *offset)
{
	...
	if (p_dir->dir == CLUSTER_32(0)) { /* FAT16 root_dir */
		...
		*offset = off &
			  p_bd->sector_size_mask; /* byte offset in sector    */
		*sector = off >>
			  p_bd->sector_size_bits; /* sector offset in cluster */
		*sector += START_SECTOR(clu);
	}
...
DENTRY_T *get_entry_in_dir(struct super_block *sb, CHAIN_T *p_dir, s32 entry,
			   u32 *sector)
{
	s32 off;
	u32 sec;
...
ENTRY_SET_CACHE_T *get_entry_set_in_dir(struct super_block *sb, CHAIN_T *p_dir,
					s32 entry, u32 type, DENTRY_T **file_ep)
{
	...
	DPRINTK("trying to kmalloc %zx bytes for %d entries\n",
		...
		num_entries);
	es = kmalloc(offsetof(ENTRY_SET_CACHE_T, __buf) +
			     (num_entries) * sizeof(DENTRY_T),
		     GFP_KERNEL);
	if (es == NULL)
		goto err_out;
...
{
	return __write_partial_entries_in_entry_set(
		sb, es, es->sector, es->offset, es->num_entries);
}

/* write back some entries in entry set */
s32 write_partial_entries_in_entry_set(struct super_block *sb,
				       ENTRY_SET_CACHE_T *es, DENTRY_T *ep,
				       u32 count)
{
...
s32 search_deleted_or_unused_entry(struct super_block *sb, CHAIN_T *p_dir,
				   s32 num_entries)
{
...
s32 find_empty_entry(struct inode *inode, CHAIN_T *p_dir, s32 num_entries)
{
	...
	while ((dentry = search_deleted_or_unused_entry(sb, p_dir,
							num_entries)) < 0) {
		if (p_fs->dev_ejected)
			break;
...
s32 fat_find_dir_entry(struct super_block *sb, CHAIN_T *p_dir,
		       UNI_NAME_T *p_uniname, s32 num_entries,
		...
		for (i = 0; i < dentries_per_clu; i++, dentry++) {
			...
			if ((entry_type == TYPE_FILE) ||
				...
				if (is_feasible_entry) {
					...
					len = extract_uni_name_from_ext_entry(
						ext_ep, entry_uniname, order);

					unichar = *(uniname + len);
					*(uniname + len) = 0x0;

					if (nls_uniname_cmp(sb, uniname,
							    entry_uniname))
...
/* return values of exfat_find_dir_entry()
   ...
   -1 : (root dir, ".") it is the root dir itself
   -2 : entry with the name does not exist */
s32 exfat_find_dir_entry(struct super_block *sb, CHAIN_T *p_dir,
			 UNI_NAME_T *p_uniname, s32 num_entries,
			 DOS_NAME_T *p_dosname, u32 type)
{
		...
		for (i = 0; i < dentries_per_clu; i++, dentry++) {
			...
			if ((entry_type == TYPE_UNUSED) ||
				...
				if (p_fs->hint_uentry.entry == -1) {
					...
					if (num_empty == 1) {
						p_fs->hint_uentry.clu.dir =
							clu.dir;
						p_fs->hint_uentry.clu.size =
							clu.size;
						p_fs->hint_uentry.clu.flags =
							clu.flags;
					}
					if ((num_empty >= num_entries) ||
				...
				if ((entry_type == TYPE_FILE) ||
					...
					if (is_feasible_entry) {
						...
						len = extract_uni_name_from_name_entry(
...
/* returns -1 on error */
s32 fat_count_ext_entries(struct super_block *sb, CHAIN_T *p_dir, s32 entry,
			  DENTRY_T *p_entry)
{
	s32 count = 0;
	u8 chksum;
	...
	for (entry--; entry >= 0; entry--) {
		ext_ep = (EXT_DENTRY_T *)get_entry_in_dir(sb, p_dir, entry,
							  NULL);
		if (!ext_ep)
			return -1;
...
/* returns -1 on error */
s32 exfat_count_ext_entries(struct super_block *sb, CHAIN_T *p_dir, s32 entry,
			    DENTRY_T *p_entry)
{
	int i, count = 0;
	u32 type;
...
s32 get_num_entries_and_dos_name(struct super_block *sb, CHAIN_T *p_dir,
...
s32 fat_generate_dos_name(struct super_block *sb, CHAIN_T *p_dir,
			  DOS_NAME_T *p_dosname)
{
	...
	while (clu.dir != CLUSTER_32(~0)) {
		...
		for (i = 0; i < dentries_per_clu; i++) {
			ep = (DOS_DENTRY_T *)get_entry_in_dir(sb, &clu, i,
							      NULL);
			if (!ep)
				return FFS_MEDIAERR;
...
/* return values of resolve_path()
   ...
   < 0 : return error */
s32 resolve_path(struct inode *inode, char *path, CHAIN_T *p_dir,
...
s32 fat16_mount(struct super_block *sb, PBR_SECTOR_T *p_pbr)
{
	...
	num_root_sectors =
		((num_root_sectors - 1) >> p_bd->sector_size_bits) + 1;

	p_fs->sectors_per_clu = p_bpb->sectors_per_clu;
	p_fs->sectors_per_clu_bits = ilog2(p_bpb->sectors_per_clu);
	p_fs->cluster_size_bits =
		p_fs->sectors_per_clu_bits + p_bd->sector_size_bits;
	...
	if (p_bpb->num_fats == 1)
		...
		p_fs->FAT2_start_sector =
			p_fs->FAT1_start_sector + p_fs->num_FAT_sectors;

	p_fs->root_start_sector =
	...
	p_fs->dentries_per_clu =
		1 << (p_fs->cluster_size_bits - DENTRY_SIZE_BITS);
...
s32 create_dir(struct inode *inode, CHAIN_T *p_dir, UNI_NAME_T *p_uniname,
	       FILE_ID_T *fid)
{
	s32 ret, dentry, num_entries;
	u64 size;
	...
	if (p_fs->vol_type == EXFAT) {
		...
		if (p_dir->dir == p_fs->root_dir)
			ret = p_fs->fs_func->init_dir_entry(
				sb, &clu, 1, TYPE_DIR, CLUSTER_32(0), 0);
		else
			ret = p_fs->fs_func->init_dir_entry(
				sb, &clu, 1, TYPE_DIR, p_dir->dir, 0);
...
s32 create_file(struct inode *inode, CHAIN_T *p_dir, UNI_NAME_T *p_uniname,
		u8 mode, FILE_ID_T *fid)
{
	s32 ret, dentry, num_entries;
	DOS_NAME_T dos_name;
	struct super_block *sb = inode->i_sb;
...
s32 rename_file(struct inode *inode, CHAIN_T *p_dir, s32 oldentry,
		UNI_NAME_T *p_uniname, FILE_ID_T *fid)
{
	...
	/* buf_lock() before call count_ext_entries() */
	num_old_entries =
		p_fs->fs_func->count_ext_entries(sb, p_dir, oldentry, epold);
	...
	if (num_old_entries < num_new_entries) {
		...
		if (p_fs->fs_func->get_entry_type(epnew) == TYPE_FILE) {
			p_fs->fs_func->set_entry_attr(
				epnew, p_fs->fs_func->get_entry_attr(epnew) |
					       ATTR_ARCHIVE);
			fid->attr |= ATTR_ARCHIVE;
		}
		...
		if (p_fs->fs_func->get_entry_type(epold) == TYPE_FILE) {
			p_fs->fs_func->set_entry_attr(
				epold, p_fs->fs_func->get_entry_attr(epold) |
					       ATTR_ARCHIVE);
			fid->attr |= ATTR_ARCHIVE;
		}
		...
		p_fs->fs_func->delete_dir_entry(
			sb, p_dir, oldentry, num_new_entries, num_old_entries);
	}
	      ...
	      CHAIN_T *p_newdir, UNI_NAME_T *p_uniname, FILE_ID_T *fid)
{
	...
	/* buf_lock() before call count_ext_entries() */
	num_old_entries =
		p_fs->fs_func->count_ext_entries(sb, p_olddir, oldentry, epmov);
	if (num_old_entries < 0) {
	...
	ret = p_fs->fs_func->init_ext_entry(
		sb, p_newdir, newentry, num_new_entries, p_uniname, &dos_name);
	if (ret != FFS_SUCCESS)
		return ret;
...
s32 sector_read(struct super_block *sb, u32 sec, struct buffer_head **bh,
		s32 read)
{
...
s32 sector_write(struct super_block *sb, u32 sec, struct buffer_head *bh,
		 s32 sync)
{
	s32 ret = FFS_MEDIAERR;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
...
s32 multi_sector_read(struct super_block *sb, u32 sec, struct buffer_head **bh,
		      s32 num_secs, s32 read)
{
	s32 ret = FFS_MEDIAERR;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
...
s32 multi_sector_write(struct super_block *sb, u32 sec, struct buffer_head *bh,
		       s32 num_secs, s32 sync)
{
	s32 ret = FFS_MEDIAERR;
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
