...
s32 buf_init(struct super_block *sb)
{
	...
	/* LRU list */
	p_fs->FAT_cache_lru_list.next = p_fs->FAT_cache_lru_list.prev =
	...
	for (i = 0; i < FAT_CACHE_SIZE; i++) {
		...
		p_fs->FAT_cache_array[i].prev = p_fs->FAT_cache_array[i].next =
			NULL;
		push_to_mru(&(p_fs->FAT_cache_array[i]),
			    &p_fs->FAT_cache_lru_list);
	}

	p_fs->buf_cache_lru_list.next = p_fs->buf_cache_lru_list.prev =
	...
	for (i = 0; i < BUF_CACHE_SIZE; i++) {
		...
		p_fs->buf_cache_array[i].prev = p_fs->buf_cache_array[i].next =
			NULL;
