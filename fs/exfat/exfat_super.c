...
static struct inode *exfat_build_inode(struct super_block *sb, FILE_ID_T *fid,
				       loff_t i_pos);
...
{
	qstr->hash =
		full_name_hash(NULL, qstr->name, exfat_striptail_len(qstr));
	return 0;
}
...
static int exfat_cmp(const struct dentry *parent, const struct dentry *dentry,
	...
	static int exfat_cmp(const struct dentry *parent,
			     ...
			     const struct inode *inode, unsigned int len,
			     const char *str, const struct qstr *name)
#endif
{
	unsigned int alen, blen;
...
#else
			if (filldir(dirent, "..", cpos + 1, cpos, inum,
				    DT_DIR) < 0)
#endif
				goto out;
			cpos++;
...
get_new:
	...
	if (!memcmp(de.ShortName, DOS_CUR_DIR_NAME, DOS_NAME_LENGTH)) {
		...
		loff_t i_pos =
			((loff_t)EXFAT_I(inode)->fid.start_clu << 32) |
...
#endif
{
	...
	dentry->d_time =
		(u64)atomic64_read(&(dentry->d_parent->d_inode->i_version));
...
#endif
{
	...
	i_mode = inode->i_mode;
	if (S_ISLNK(i_mode)) {
		EXFAT_I(inode)->target =
			kmalloc(i_size_read(inode) + 1, GFP_KERNEL);
...
out:
	...
	dentry = d_splice_alias(inode, dentry);
	if (dentry)
		dentry->d_time = (u64)atomic64_read(
			...
			unsigned int flags)
{
	...
	err = FsMoveFile(old_dir, &(EXFAT_I(old_inode)->fid), new_dir,
			 new_dentry);
	if (err) {
		if (err == FFS_PERMISSIONERR)
			err = -EPERM;
...
static int exfat_cont_expand(struct inode *inode, loff_t size)
{
	...
	if (IS_SYNC(inode)) {
		...
		if (!err)
			...
			err = wait_on_page_writeback_range(
				mapping, start >> PAGE_CACHE_SHIFT,
...
static int exfat_setattr(struct dentry *dentry, struct iattr *attr)
{
	...
	if ((ia_valid & (ATTR_MTIME_SET | ATTR_ATIME_SET | ATTR_TIMES_SET)) &&
		...
		attr->ia_valid &=
			~(ATTR_MTIME_SET | ATTR_ATIME_SET | ATTR_TIMES_SET);
	}
...
#else
	static void *exfat_follow_link(struct dentry * dentry,
				       struct nameidata * nd)
	{
		struct exfat_inode_info *ei = EXFAT_I(dentry->d_inode);
		      ...
		      unsigned long *mapped_blocks, int *create)
{
	...
	*mapped_blocks = 0;
	...
	sec_offset = sector &
		     (p_fs->sectors_per_clu - 1); /* sector offset in cluster */
...
#endif
{
	...
	ret = blockdev_direct_IO(iocb, inode, iter, offset, exfat_get_block);
		...
		ret = blockdev_direct_IO(rw, iocb, inode, inode->i_sb->s_bdev,
					 iov, offset, nr_segs, exfat_get_block,
					 NULL);
#endif
...
static const match_table_t exfat_tokens = { { Opt_uid, "uid=%u" },
					    { Opt_gid, "gid=%u" },
					    { Opt_umask, "umask=%o" },
					    { Opt_dmask, "dmask=%o" },
					    { Opt_fmask, "fmask=%o" },
	...
	*debug = 0;
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		default:
			if (!silent)
				printk(KERN_ERR
				       "[EXFAT] Unrecognized mount option %s or missing value\n",
				       p);
			return -EINVAL;
		}
	}
...
{
	return get_sb_bdev(fs_type, flags, dev_name, data, exfat_fill_super,
			   mnt);
}
#else
