...
s32 bdev_open(struct super_block *sb)
{
	...
	p_bd->num_sectors =
		i_size_read(sb->s_bdev->bd_inode) >> p_bd->sector_size_bits;
...
s32 bdev_read(struct super_block *sb, u32 secno, struct buffer_head **bh,
	      u32 num_secs, s32 read)
{
	BD_INFO_T *p_bd = &(EXFAT_SB(sb)->bd_info);
...
s32 bdev_write(struct super_block *sb, u32 secno, struct buffer_head *bh,
	       u32 num_secs, s32 sync)
{
	s32 count;
