...
static int z_erofs_lz4_decompress(struct z_erofs_decompress_req *rq, u8 *out)
{
	...
	if (rq->inplace_io) {
		const uint oend =
			(rq->pageofs_out + rq->outputsize) & ~PAGE_MASK;
		const uint nr = PAGE_ALIGN(rq->pageofs_out + rq->outputsize) >>
				PAGE_SHIFT;
...
#ifdef CONFIG_OPLUS_FEATURE_EROFS
	ret = z_erofs_lz4_decompress_partial(src + inputmargin, out, inlen,
					     rq->outputsize,
					     test_opt(EROFS_SB(rq->sb), LZ4ASM),
					     rq->inplace_io);
#else
	ret = LZ4_decompress_safe_partial(src + inputmargin, out, inlen,
