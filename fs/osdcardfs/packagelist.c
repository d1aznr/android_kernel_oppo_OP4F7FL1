 ...
 */
int check_caller_access_to_name(struct inode *parent_node,
...
static struct hashtable_entry *alloc_hashtable_entry(const struct qstr *key,
						     appid_t value)
{
	struct hashtable_entry *ret =
		kmem_cache_alloc(hashtable_entry_cachep, GFP_KERNEL);
	if (!ret)
		return NULL;
...
static inline struct package_details *
to_package_details(struct config_item *item)
{
						     ...
						     char *page)
{
	...
	hash_for_each_possible_rcu (package_to_userid, hash_cur, hlist, hash) {
		if (qstr_case_eq(&package_details->name, &hash_cur->key))
			count +=
				scnprintf(page + count, PAGE_SIZE - count,
...
static inline struct extensions_value *
to_extensions_value(struct config_item *item)
{
...
static inline struct extension_details *
to_extension_details(struct config_item *item)
{
...
extension_details_make_item(struct config_group *group, const char *name)
{
...
static ssize_t packages_list_show(struct config_item *item, char *page)
{
	...
	hash_for_each_rcu (package_to_appid, i, hash_cur_app, hlist) {
		...
		hash_for_each_possible_rcu (package_to_userid, hash_cur_user,
					    hlist, hash) {
			if (qstr_case_eq(&hash_cur_app->key,
					 &hash_cur_user->key)) {
				written +=
					scnprintf(
						page + count + written - 1,
						PAGE_SIZE - sizeof(errormsg) -
							count - written + 1,
						" %d\n",
						atomic_read(
							&hash_cur_user->value)) -
...
static int configfs_sdcardfs_init(void)
{
	...
	for (i = 0; sd_default_groups[i]; i++) {
		...
		configfs_add_default_group(sd_default_groups[i],
					   &subsys->su_group);
	}
