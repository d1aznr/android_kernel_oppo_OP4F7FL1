...
void fixup_lower_ownership(struct dentry *dentry, const char *name)
{
	...
	if (d_inode(path.dentry)->i_gid.val != gid ||
	    d_inode(path.dentry)->i_uid.val != uid) {
	retry_deleg:
		...
		if (error)
			pr_debug(
				"sdcardfs: Failed to touch up lower fs gid/uid for %s\n",
				name);
	}
...
static void __fixup_perms_recursive(struct dentry *dentry,
				    struct limit_search *limit, int depth)
{
	struct dentry *child;
	struct sdcardfs_inode_info *info;
...
int setup_obb_dentry(struct dentry *dentry, struct path *lower_path)
{
	...
	err = kern_path(sbi->obbpath_s, LOOKUP_FOLLOW | LOOKUP_DIRECTORY,
			&obbpath);
