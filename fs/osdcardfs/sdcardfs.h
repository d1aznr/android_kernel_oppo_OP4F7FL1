...
#define fixup_tmp_permissions(x)                                               \
	do {                                                                   \
		(x)->i_uid =                                                   \
			make_kuid(&init_user_ns, SDCARDFS_I(x)->data->d_uid);  \
...
sdcardfs_lower_super(const struct super_block *sb)
{
...
#define SDCARDFS_DENT_FUNC(pname)                                              \
	...
	static inline void sdcardfs_put_reset_##pname(                         \
...
static inline struct sdcardfs_inode_data *
data_get(struct sdcardfs_inode_data *data)
{
	if (data)
...
static inline struct sdcardfs_inode_data *
top_data_get(struct sdcardfs_inode_info *info)
{
	struct sdcardfs_inode_data *top_data;
...
static inline int get_gid(struct vfsmount *mnt, struct super_block *sb,
			  struct sdcardfs_inode_data *data)
{
...
extern void get_derived_permission_new(struct dentry *parent,
				       struct dentry *dentry,
				       const struct qstr *name);
extern void fixup_perms_recursive(struct dentry *dentry,
...
{
	dest->i_mode = (src->i_mode & S_IFMT) | S_IRWXU | S_IRWXG | S_IROTH |
		       S_IXOTH; /* 0775 */
