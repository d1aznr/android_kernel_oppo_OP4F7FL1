			   ...
			   umode_t mode, bool want_excl)
{
	...
	saved_cred =
		override_fsids(SDCARDFS_SB(dir->i_sb), SDCARDFS_I(dir)->data);
	if (!saved_cred)
		return -ENOMEM;
...
{
	struct file *filp = filp_open(
		abs_path, O_RDWR | O_CREAT | O_EXCL | O_NOFOLLOW, mode);
			  ...
			  umode_t mode)
{
	...
	if (make_nomedia_in_obb || ((pd->perm == PERM_ANDROID) &&
				    (qstr_case_eq(&dentry->d_name, &q_data)))) {
		revert_fsids(saved_cred);
		saved_cred =
			override_fsids(sbi, SDCARDFS_I(d_inode(dentry))->data);
		if (!saved_cred) {
			pr_err("sdcardfs: failed to set up .nomedia in %s: %d\n",
			       lower_path.dentry->d_name.name, -ENOMEM);
			goto out;
		}
			   ...
			   unsigned int flags)
{
	...
	err = vfs_rename2(lower_mnt, d_inode(lower_old_dir_dentry),
			  lower_old_dentry, d_inode(lower_new_dir_dentry),
	...
	get_derived_permission_new(new_dentry->d_parent, old_dentry,
				   &new_dentry->d_name);
...
{
	WARN_RATELIMIT(
		1, "sdcardfs does not support permission. Use permission2.\n");
	return -EINVAL;
}
...
{
	WARN_RATELIMIT(1,
		       "sdcardfs does not support setattr. User setattr2.\n");
	return -EINVAL;
}
			    ...
			    struct iattr *ia)
{
	...
	err = notify_change2(lower_mnt, lower_dentry,
