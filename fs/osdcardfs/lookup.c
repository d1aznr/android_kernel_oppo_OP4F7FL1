...
int sdcardfs_init_dentry_cache(void)
{
	sdcardfs_dentry_cachep =
		kmem_cache_create("sdcardfs_dentry",
				  sizeof(struct sdcardfs_dentry_info), 0,
...
static int
sdcardfs_inode_test(struct inode *inode,
		    void *candidate_data /*void *candidate_lower_inode*/)
{
	struct inode *current_lower_inode = sdcardfs_lower_inode(inode);
	userid_t current_userid = SDCARDFS_I(inode)->data->userid;

	if (current_lower_inode ==
		    ((struct inode_data *)candidate_data)->lower_inode &&
...
static int sdcardfs_name_match(struct dir_context *ctx, const char *name,
			       ...
			       unsigned int d_type)
{
	struct sdcardfs_name_data *buf =
		container_of(ctx, struct sdcardfs_name_data, ctx);
					...
					userid_t id)
{
	...
	found:
		...
		ret_dentry = __sdcardfs_interpose(dentry, dentry->d_sb,
						  &lower_path, id);
	...
	lower_dentry = lookup_one_len_unlocked(name->name, lower_dir_dentry,
					       name->len);
