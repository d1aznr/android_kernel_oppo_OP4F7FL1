...
int ext4_dirent_csum_verify(struct inode *inode, struct ext4_dir_entry *dirent)
{
	...
	if (t->det_checksum !=
	    ext4_dirent_csum(inode, dirent, (void *)t - (void *)dirent))
		return 0;
...
int ext4_handle_dirty_dirent_node(handle_t *handle, struct inode *inode,
				  struct buffer_head *bh)
{
	...
	do {
		...
		if (!buffer_verified(bh) &&
		    ...
		    !ext4_dirent_csum_verify(
			    dir, (struct ext4_dir_entry *)bh->b_data)) {
					 ...
					 struct dx_hash_info *hinfo)
{
	...
	count = dx_make_map(dir, (struct ext4_dir_entry_2 *)data1, blocksize,
			    hinfo, map);
	map -= count;
 ...
 */
int ext4_generic_delete_entry(handle_t *handle, struct inode *dir,
			     ...
			     struct buffer_head *bh)
{
	...
	err = ext4_generic_delete_entry(handle, dir, de_del, bh, bh->b_data,
...
retry:
	inode = ext4_new_inode_start_handle(dir, mode, &dentry->d_name, 0, NULL,
					    EXT4_HT_DIR, credits);
...
static int ext4_mknod(struct inode *dir, struct dentry *dentry, umode_t mode,
		      dev_t rdev)
{
	handle_t *handle;
	struct inode *inode;
...
retry:
	inode = ext4_new_inode_start_handle(
		dir, mode, NULL, 0, NULL, EXT4_HT_DIR,
		EXT4_MAXQUOTAS_INIT_BLOCKS(dir->i_sb) + 4 +
					      ...
					      int dotdot_real_len)
{
	...
	de->name_len = 1;
	de->rec_len =
		ext4_rec_len_to_disk(EXT4_DIR_REC_LEN(de->name_len), blocksize);
	strcpy(de->name, ".");
	...
	if (!dotdot_real_len)
		de->rec_len = ext4_rec_len_to_disk(
			blocksize - (csum_size + EXT4_DIR_REC_LEN(1)),
			blocksize);
	else
		de->rec_len = ext4_rec_len_to_disk(
...
int ext4_orphan_add(handle_t *handle, struct inode *inode)
{
	...
	if (!NEXT_ORPHAN(inode) ||
	    NEXT_ORPHAN(inode) > (le32_to_cpu(sbi->s_es->s_inodes_count))) {
...
int ext4_orphan_del(handle_t *handle, struct inode *inode)
{
	...
	if (prev == &sbi->s_orphan) {
		...
		struct inode *i_prev =
			&list_entry(prev, struct ext4_inode_info, i_orphan)
				 ->vfs_inode;
			 ...
			 struct ext4_dir_entry_2 **parent_de, int *inlined)
{
	...
	if (!ext4_has_inline_data(inode)) {
		...
		*parent_de =
			ext4_next_entry((struct ext4_dir_entry_2 *)bh->b_data,
					inode->i_sb->s_blocksize);
		return bh;
	}
...
static int ext4_rename_dir_prepare(handle_t *handle, struct ext4_renament *ent)
{
	...
	ent->dir_bh =
		ext4_get_first_dir_block(handle, ent->inode, &retval,
					 &ent->parent_de, &ent->dir_inlined);
	if (!ent->dir_bh)
		return retval;
		       ...
		       unsigned ino, unsigned file_type)
{
	...
	ent->dir->i_ctime = ent->dir->i_mtime = current_time(ent->dir);
	ext4_mark_inode_dirty(handle, ent->dir);
	BUFFER_TRACE(ent->bh, "call ext4_handle_dirty_metadata");
	if (!ent->inlined) {
		retval = ext4_handle_dirty_dirent_node(handle, ent->dir,
						       ent->bh);
		if (unlikely(retval)) {
			ext4_std_error(ent->dir->i_sb, retval);
			return retval;
