					   ...
					   struct ext4_group_desc *gdp)
{
	...
	if (ext4_block_in_group(sb, ext4_block_bitmap(sb, gdp), block_group)) {
		block_cluster =
			EXT4_B2C(sbi, ext4_block_bitmap(sb, gdp) - start);
	...
	if (ext4_block_in_group(sb, ext4_inode_bitmap(sb, gdp), block_group)) {
		inode_cluster =
			EXT4_B2C(sbi, ext4_inode_bitmap(sb, gdp) - start);
					    ...
					    struct buffer_head **bh)
{
	...
	desc = (struct ext4_group_desc *)((__u8 *)bh_p->b_data +
					    ...
					    struct buffer_head *bh)
{
	...
	next_zero_bit = ext4_find_next_zero_bit(
		bh->b_data, EXT4_B2C(sbi, offset + sbi->s_itb_per_group),
		EXT4_B2C(sbi, offset));
				      ...
				      struct buffer_head *bh)
{
	...
	if (buffer_verified(bh))
		goto verified;
	if (unlikely(!ext4_block_bitmap_csum_verify(sb, block_group, desc,
						    bh))) {
				  ...
				  unsigned int flags)
{
	...
	if (uid_eq(sbi->s_resuid, current_fsuid()) ||
		...
		if (free_clusters >=
		    (nclusters + dirty_clusters + resv_clusters))
			return 1;
	}
...
{
	if (!ext4_has_free_clusters(EXT4_SB(sb), 1, 0) || (*retries)++ > 1 ||
				  ...
				  unsigned long *count, int *errp)
{
	...
	if (!(*errp) && (flags & EXT4_MB_DELALLOC_RESERVED)) {
		dquot_alloc_block_nofail(inode, EXT4_C2B(EXT4_SB(inode->i_sb),
							 ar.len));
	}
	return ret;
}
