...
static ssize_t trigger_test_error(struct ext4_sb_info *sbi, const char *buf,
				  size_t count)
{
	int len = count;
...
int ext4_get_free_frag_data(struct seq_file *seq, struct free_frag_data *ff)
{
	...
	for (group = 0; group < ngroups; group++)
		ext4_mballoc_query_range(sb, group, 0, -1, count_free_frag,
					 ffd);
	return 0;
}
int ext4_seq_frag_score_show(struct seq_file *seq, void *offset)
{
	...
	score = ffd.total_free ? (ffd.counters[0] + ffd.counters[1]) * 100 /
					 ffd.total_free :
				 0;
	seq_printf(seq, "%u\n", score);
	return 0;
...
EXT4_ATTR_OFFSET(inode_readahead_blks, 0644, inode_readahead, ext4_sb_info,
...
EXT4_RW_ATTR_SBI_UI(warning_ratelimit_interval_ms,
		    s_warning_ratelimit_state.interval);
...
static ssize_t ext4_attr_show(struct kobject *kobj, struct attribute *attr,
			      char *buf)
{
...
static ssize_t ext4_attr_store(struct kobject *kobj, struct attribute *attr,
...
#if defined(CONFIG_OPLUS_FEATURE_EXT4_ASYNC_DISCARD)
		//add for ext4 async discard suppot
		proc_create_single_data("discard_info", S_IRUGO, sbi->s_proc,
					ext4_seq_discard_info_show, sb);
#endif
#if defined(CONFIG_OPLUS_FEATURE_EXT4_FRAGMENT)
		//add for ext4 fragment
		proc_create_single_data("frag_score", S_IRUGO, sbi->s_proc,
					ext4_seq_frag_score_show, sb);
		proc_create_single_data("free_frag", S_IRUGO, sbi->s_proc,
					ext4_seq_free_frag_show, sb);
#endif
...
int __init ext4_init_sysfs(void)
{
	...
	ret = kobject_init_and_add(ext4_feat, &ext4_feat_ktype, ext4_root,
				   "features");
	if (ret)
		goto feat_err;
