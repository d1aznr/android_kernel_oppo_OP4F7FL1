				...
				struct ext4_fsmap *rec)
{
	...
	if (rec_fsblk > info->gfi_next_fsblk) {
		...
		ext4_get_group_no_and_offset(sb, info->gfi_next_fsblk, &agno,
					     &cno);
					  ...
					  struct list_head *meta_list)
{
	...
	if (!len)
		return 0;
	error = ext4_getfsmap_fill(meta_list, fsb, len, EXT4_FMR_OWN_GDT);
	if (error)
		return error;
	fsb += len;
...
static int ext4_getfsmap_compare(void *priv, struct list_head *a,
				 struct list_head *b)
{
					     ...
					     struct list_head *meta_list)
{
	...
	for (agno = 0; agno < EXT4_SB(sb)->s_groups_count; agno++) {
		...
		/* Inodes */
		error = ext4_getfsmap_fill(meta_list, ext4_inode_table(sb, gdp),
				 ...
				 struct ext4_getfsmap_info *info)
{
	...
	/* Query each bg */
	for (info->gfi_agno = start_ag; info->gfi_agno <= end_ag;
	     info->gfi_agno++) {
		/*
		...
		error = ext4_mballoc_query_range(
			sb, info->gfi_agno,
		  ...
		  ext4_fsmap_format_t formatter, void *arg)
{
	...
	info.gfi_next_fsblk =
		head->fmh_keys[0].fmr_physical + head->fmh_keys[0].fmr_length;
