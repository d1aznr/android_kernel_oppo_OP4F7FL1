...
void ext4_stop_discard_thread(struct ext4_sb_info *sbi)
{
	...
	if (dcc && dcc->ext4_issue_discard_thread) {
		struct task_struct *discard_thread =
			dcc->ext4_issue_discard_thread;
...
int create_discard_cmd_control(struct ext4_sb_info *sbi)
{
	...
	dcc->groups_block_bitmap =
		vmalloc((sbi->s_clusters_per_group / 8) *
			sbi->s_groups_count); //for backup groups block bitmap
	if (!dcc->groups_block_bitmap)
		goto free_dcc;
	memset(dcc->groups_block_bitmap, 0xFF,
	       (sbi->s_clusters_per_group / 8) * sbi->s_groups_count);
...
init_thread:
	dcc->ext4_issue_discard_thread =
		kthread_run(issue_discard_thread, sbi, "ext4_discard-%u:%u",
			    MAJOR(dev), MINOR(dev));
...
int ext4_seq_discard_info_show(struct seq_file *seq, void *v)
{
	...
	seq_printf(
		seq,
		"  current triming group:%d\n  triming group internal offset:%d\n  cnt_io_interrupted:%lld\n",
		sbi->dcc_info->group_start, sbi->dcc_info->blk_offset,
		sbi->dcc_info->cnt_io_interrupted);

	seq_printf(
		seq,
		"Dpolicy info:\n  discard type:%d\n  min_interval:%d\n  mid_interval:%d\n  max_interval:%d\n",
		sbi->dcc_info->dpolicy.type,
		sbi->dcc_info->dpolicy.min_interval,
		sbi->dcc_info->dpolicy.mid_interval,
		sbi->dcc_info->dpolicy.max_interval);
	seq_printf(
		seq,
		"  max_request per round:%d\n  io_aware:%s\n  io_aware_gran:%d\n  discard granularity:%d\n",
		sbi->dcc_info->dpolicy.max_requests,
		sbi->dcc_info->dpolicy.io_aware ? "TRUE" : "FALSE",
		sbi->dcc_info->dpolicy.io_aware_gran,
		sbi->dcc_info->dpolicy.granularity);
