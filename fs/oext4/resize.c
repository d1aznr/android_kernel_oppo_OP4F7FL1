			      ...
			      struct ext4_new_group_data *input)
{
	...
	if (test_opt(sb, DEBUG))
		printk(KERN_DEBUG "EXT4-fs: adding %s group %u: %u blocks "
		       ...
		       ext4_bg_has_super(sb, input->group) ? "normal" :
							     "no-super",
		       input->group, input->blocks_count, free_blocks_count,
	...
	if (offset != 0)
		...
		ext4_warning(sb, "Bad blocks count %u", input->blocks_count);
	else if (IS_ERR(bh = ext4_sb_bread(sb, end - 1, 0))) {
		err = PTR_ERR(bh);
		bh = NULL;
		ext4_warning(sb, "Cannot read last block (%llu)", end - 1);
		...
		ext4_warning(sb, "Block bitmap (%llu) in GDT table (%llu-%llu)",
			     (unsigned long long)input->block_bitmap, start,
			     metaend - 1);
	else if (inside(input->inode_bitmap, start, metaend))
		ext4_warning(sb, "Inode bitmap (%llu) in GDT table (%llu-%llu)",
			     (unsigned long long)input->inode_bitmap, start,
			     metaend - 1);
...
static struct ext4_new_flex_group_data *alloc_flex_gd(unsigned long flexbg_size)
{
	...
	flex_gd->bg_flags = kmalloc_array(flexbg_size, sizeof(__u16), GFP_NOFS);
...
next_group:
	...
	for (i = 0; i < flex_gd->count; i++) {
		group_data[i].free_clusters_count -=
			EXT4_NUM_B2C(EXT4_SB(sb), group_data[i].mdata_blocks);
	}
		...
		for (i = 0; i < flex_gd->count; i++) {
			ext4_debug(
				...
				ext4_bg_has_super(sb, group + i) ? "normal" :
								   "no-super",
				group + i, group_data[i].blocks_count,
				group_data[i].free_clusters_count,
				       ...
				       struct ext4_new_flex_group_data *flex_gd)
{
	...
	for (i = 0; i < flex_gd->count; i++, group++) {
		...
		for (j = 0; j < gdblocks; j++, block++) {
			...
			memcpy(gdb->b_data,
			       sbi_array_rcu_deref(sbi, s_group_desc, j)->b_data,
			       gdb->b_size);
	...
	for (j = 0; j < GROUP_TABLE_COUNT; j++) {
		...
		for (i = 1; i < flex_gd->count; i++) {
			...
			err = set_flexbg_block_bitmap(
				sb, handle, flex_gd, EXT4_B2C(sbi, start),
				EXT4_B2C(sbi, start + count - 1));
			if (err)
				goto out;
			count = group_table_count[j];
 ...
 */
static int verify_reserved_gdb(struct super_block *sb, ext4_group_t end,
	...
	while ((grp = ext4_list_backups(sb, &three, &five, &seven)) < end) {
		if (le32_to_cpu(*p++) !=
			...
			ext4_warning(
				...
				" missing grp %d (%llu)",
				blk, grp,
				grp * (ext4_fsblk_t)EXT4_BLOCKS_PER_GROUP(sb) +
					blk);
			return -EINVAL;
		}
		       ...
		       ext4_group_t group)
{
	...
	n_group_desc = ext4_kvmalloc(
		(gdb_num + 1) * sizeof(struct buffer_head *), GFP_NOFS);
	if (!n_group_desc) {
		err = -ENOMEM;
 ...
 */
static int add_new_gdb_meta_bg(struct super_block *sb, handle_t *handle,
			       ext4_group_t group)
{
			      ...
			      ext4_group_t group)
{
	...
	data = (__le32 *)dind->b_data +
	       (EXT4_SB(sb)->s_gdb_count % EXT4_ADDR_PER_BLOCK(sb));
			      ...
			      ext4_group_t count)
{
	...
	for (i = 0; i < count; i++, group++) {
		...
		if (gdb_off) {
			gdb_bh =
				sbi_array_rcu_deref(sbi, s_group_desc, gdb_num);
			BUFFER_TRACE(gdb_bh, "get_write_access");
			err = ext4_journal_get_write_access(handle, gdb_bh);
...
static int ext4_set_bitmap_checksums(struct super_block *sb, ext4_group_t group,
			      ...
			      struct ext4_new_flex_group_data *flex_gd)
{
	...
	le32_add_cpu(&es->s_free_inodes_count,
		     EXT4_INODES_PER_GROUP(sb) * flex_gd->count);
	...
	sbi->s_blockfile_groups =
		min_t(ext4_group_t, sbi->s_groups_count,
		      (EXT4_MAX_BLOCK_FILE_PHYS / EXT4_BLOCKS_PER_GROUP(sb)));

	/* Update the reserved block counts only once the new group is
	 * active. */
	ext4_r_blocks_count_set(es, ext4_r_blocks_count(es) + reserved_blocks);
	...
	if (test_opt(sb, DEBUG))
		printk(KERN_DEBUG "EXT4-fs: added group %u:"
		       ...
		       flex_gd->count, blocks_count, free_blocks,
		       reserved_blocks);
}
			       ...
			       struct ext4_new_flex_group_data *flex_gd)
{
	...
	err = ext4_add_new_descs(handle, sb, group, resize_inode,
				 flex_gd->count);
	if (err)
		goto exit_journal;
				   ...
				   unsigned long flexbg_size)
{
	...
	for (i = 0; i < flex_gd->count; i++) {
		...
		if (ext4_has_group_desc_csum(sb)) {
			flex_gd->bg_flags[i] =
				EXT4_BG_BLOCK_UNINIT | EXT4_BG_INODE_UNINIT;
	...
	if ((last_group == n_group) && (last != clusters_per_group - 1)) {
		...
		group_data[i - 1].free_clusters_count -=
			clusters_per_group - last - 1;
	}
...
static int ext4_convert_meta_bg(struct super_block *sb, struct inode *inode)
{
	...
	if (inode) {
		...
		if (inode->i_blocks !=
		    1 << (inode->i_blkbits - (9 - sbi->s_cluster_bits)))
...
retry:
	...
	if (ext4_has_feature_resize_inode(sb)) {
		...
		if (n_desc_blocks >
		    o_desc_blocks + le16_to_cpu(es->s_reserved_gdt_blocks)) {
