 ...
 *
 */
static long swap_inode_boot_loader(struct super_block *sb, struct inode *inode)
{
	handle_t *handle;
	int err;
...
static int ext4_ioctl_setflags(struct inode *inode, unsigned int flags)
{
	struct ext4_inode_info *ei = EXT4_I(inode);
...
static int ext4_ioctl_setproject(struct file *filp, __u32 projid)
{
	...
	if (!EXT4_FITS_IN_INODE(raw_inode, ei, i_projid)) {
		err = ext4_expand_extra_isize(
			inode, EXT4_SB(sb)->s_want_extra_isize, &iloc);
		if (err)
			return err;
	} else {
		brelse(iloc.bh);
...
#define EXT4_SUPPORTED_FS_XFLAGS                                               \
			     ...
			     struct fsmap_head __user *arg)
{
	...
	if (!aborted && info.gi_idx) {
		...
		if (copy_to_user(
			    &info.gi_data->fmh_recs[info.gi_idx - 1].fmr_flags,
			    &info.gi_last_flags, sizeof(info.gi_last_flags)))
			return -EFAULT;
	}
...
long ext4_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case EXT4_IOC_SETFLAGS: {
		...
		inode_lock(inode);
		err = ext4_ioctl_check_immutable(
			inode, from_kprojid(&init_user_ns, ei->i_projid),
			flags);
		if (!err)
	...
	case EXT4_IOC_MOVE_EXT: {
		...
		if (copy_from_user(&me, (struct move_extent __user *)arg,
				   sizeof(me)))
			return -EFAULT;
		me.moved_len = 0;
	...
	case EXT4_IOC_FSGETXATTR: {
		...
		if (ext4_has_feature_project(inode->i_sb)) {
			fa.fsx_projid = (__u32)from_kprojid(
				&init_user_ns, EXT4_I(inode)->i_projid);
		}
