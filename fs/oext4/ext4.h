...
static inline void ext4_decode_extra_time(struct timespec64 *time, __le32 extra)
{
	...
	time->tv_nsec =
		(le32_to_cpu(extra) & EXT4_NSEC_MASK) >> EXT4_EPOCH_BITS;
}
...
#define EXT4_EINODE_SET_XTIME(xtime, einode, raw_inode)                        \
	do {                                                                   \
		if (EXT4_FITS_IN_INODE(raw_inode, einode, xtime))              \
			(raw_inode)->xtime =                                   \
				cpu_to_le32((einode)->xtime.tv_sec);           \
...
struct discard_cmd_control {
	...
	ext4_grpblk_t
		blk_offset; /*current trim point, group internal block offset*/
...
#define ext4_new_inode_start_handle(dir, mode, qstr, goal, owner, type,        \
				    nblocks)                                   \
	__ext4_new_inode(NULL, (dir), (mode), (qstr), (goal), (owner), 0,      \
...
extern int ext4_trim_groups(struct super_block *sb,
...
extern int ext4_handle_dirty_dirent_node(handle_t *handle, struct inode *inode,
					 struct buffer_head *bh);
extern int ext4_ci_compare(const struct inode *parent, const struct qstr *fname,
