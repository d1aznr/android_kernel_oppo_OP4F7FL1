...
#define MXS_MMC_IRQ_BITS                                                       \
	(BM_SSP_CTRL1_SDIO_IRQ | BM_SSP_CTRL1_RESP_ERR_IRQ |                   \
	 BM_SSP_CTRL1_RESP_TIMEOUT_IRQ | BM_SSP_CTRL1_DATA_TIMEOUT_IRQ |       \
	 BM_SSP_CTRL1_DATA_CRC_IRQ | BM_SSP_CTRL1_FIFO_UNDERRUN_IRQ |          \
	 BM_SSP_CTRL1_RECV_TIMEOUT_IRQ | BM_SSP_CTRL1_FIFO_OVERRUN_IRQ)
...
static int mxs_mmc_reset(struct mxs_mmc_host *host)
{
	...
	ctrl1 = BF_SSP(0x3, CTRL1_SSP_MODE) | BF_SSP(0x7, CTRL1_WORD_LENGTH) |
		BM_SSP_CTRL1_DMA_ENABLE | BM_SSP_CTRL1_POLARITY |
		BM_SSP_CTRL1_RECV_TIMEOUT_IRQ_EN |
		BM_SSP_CTRL1_DATA_CRC_IRQ_EN |
		BM_SSP_CTRL1_DATA_TIMEOUT_IRQ_EN |
		BM_SSP_CTRL1_RESP_TIMEOUT_IRQ_EN | BM_SSP_CTRL1_RESP_ERR_IRQ_EN;
...
mxs_mmc_prep_dma(struct mxs_mmc_host *host, unsigned long flags)
{
	struct mxs_ssp *ssp = &host->ssp;
	...
	desc = dmaengine_prep_slave_sg(ssp->dmach, sgl, sg_len, ssp->slave_dirn,
				       flags);
	if (desc) {
...
out:
	dev_warn(mmc_dev(host->mmc), "%s: failed to prep dma\n", __func__);
}
...
static void mxs_mmc_ac(struct mxs_mmc_host *host)
{
	...
	get_resp = (mmc_resp_type(cmd) & MMC_RSP_PRESENT) ?
			   ...
			   0;
	long_resp =
		(mmc_resp_type(cmd) & MMC_RSP_136) ? BM_SSP_CTRL0_LONG_RESP : 0;
...
static void mxs_mmc_adtc(struct mxs_mmc_host *host)
{
	...
	ctrl0 = BF_SSP(host->bus_width, CTRL0_BUS_WIDTH) | ignore_crc |
		get_resp | long_resp | BM_SSP_CTRL0_DATA_XFER | read |
		BM_SSP_CTRL0_WAIT_FOR_IRQ | BM_SSP_CTRL0_ENABLE;
			      ...
			      struct mmc_command *cmd)
{
	...
	case MMC_CMD_ADTC:
		...
		break;
	default:
		dev_warn(mmc_dev(host->mmc), "%s: unknown MMC command\n",
			 __func__);
		break;
	}
}
...
static void mxs_mmc_enable_sdio_irq(struct mmc_host *mmc, int enable)
{
	...
	if (enable &&
	    readl(ssp->base + HW_SSP_STATUS(ssp)) & BM_SSP_STATUS_SDIO_IRQ)
		mmc_signal_sdio_irq(host->mmc);
}
...
static int mxs_mmc_probe(struct platform_device *pdev)
{
	...
	if (!ssp->dmach) {
		dev_err(mmc_dev(host->mmc), "%s: failed to request dma\n",
			__func__);
		ret = -ENODEV;
		goto out_clk_disable;
	}
