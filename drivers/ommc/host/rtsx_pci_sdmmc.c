...
{
	rtsx_pci_write_register(host->pcr, CARD_STOP, SD_STOP | SD_CLR_ERR,
				SD_STOP | SD_CLR_ERR);
}
				...
				struct mmc_command *cmd)
{
	...
	rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, CARD_DATA_SOURCE, 0x01,
			 PINGPONG_BUFFER);
	...
	if (err < 0) {
		...
		sd_clear_error(host);
		dev_dbg(sdmmc_dev(host), "rtsx_pci_send_cmd error (err = %d)\n",
			err);
		goto out;
	}
			...
			u16 byte_cnt, u8 *buf, int buf_len, int timeout)
{
	...
	rtsx_pci_add_cmd(pcr, CHECK_REG_CMD, SD_TRANSFER, SD_TRANSFER_END,
			 SD_TRANSFER_END);

	err = rtsx_pci_send_cmd(pcr, timeout);
	if (err < 0) {
		sd_print_debug_regs(host);
		dev_dbg(sdmmc_dev(host), "rtsx_pci_send_cmd fail (err = %d)\n",
			err);
		return err;
	}
...
static int sd_write_data(struct realtek_pci_sdmmc *host,
			 struct mmc_command *cmd, u16 byte_cnt, u8 *buf,
			     ...
			     struct mmc_request *mrq)
{
	...
	rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, IRQSTAT0, DMA_DONE_INT,
			 DMA_DONE_INT);
	rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, DMATC3, 0xFF,
			 (u8)(data_len >> 24));
	rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, DMATC2, 0xFF,
			 (u8)(data_len >> 16));
	rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, DMATC1, 0xFF, (u8)(data_len >> 8));
...
{
	rtsx_pci_write_register(host->pcr, SD_CFG1, SD_CLK_DIVIDE_MASK,
				SD_CLK_DIVIDE_128);
}
			   ...
			   bool rx)
{
	...
	dev_dbg(sdmmc_dev(host), "%s(%s): sample_point = %d\n", __func__,
		rx ? "RX" : "TX", sample_point);

	rtsx_pci_write_register(pcr, CLK_CTL, CHANGE_CLK, CHANGE_CLK);
	if (rx) {
		SD_VP_CTL = SD_VPRX_CTL;
		rtsx_pci_write_register(pcr, SD_VPRX_CTL, PHASE_SELECT_MASK,
					sample_point);
	} else {
		SD_VP_CTL = SD_VPTX_CTL;
		rtsx_pci_write_register(pcr, SD_VPTX_CTL, PHASE_SELECT_MASK,
					sample_point);
	}
...
static int sd_tuning_rx_cmd(struct realtek_pci_sdmmc *host, u8 opcode,
			    u8 sample_point)
{
	int err;
	struct mmc_command cmd = {};
...
static int sd_tuning_phase(struct realtek_pci_sdmmc *host, u8 opcode,
			   u32 *phase_map)
{
	int err, i;
	u32 raw_phase_map = 0;
...
static void sd_request(struct work_struct *work)
{
	struct realtek_pci_sdmmc *host =
		container_of(work, struct realtek_pci_sdmmc, work);
	struct rtsx_pcr *pcr = host->pcr;
			    ...
			    unsigned char bus_width)
{
	...
	if (bus_width <= MMC_BUS_WIDTH_8)
		err = rtsx_pci_write_register(host->pcr, SD_CFG1, 0x03,
					      width[bus_width]);
...
static int sd_power_on(struct realtek_pci_sdmmc *host)
{
	...
	rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, CARD_SHARE_MODE, CARD_SHARE_MASK,
			 CARD_SHARE_48_SD);
	rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, CARD_CLK_EN, SD_CLK_EN, SD_CLK_EN);
...
static int sd_set_timing(struct realtek_pci_sdmmc *host, unsigned char timing)
{
	...
	case MMC_TIMING_UHS_SDR50:
		...
		rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, CLK_CTL, CLK_LOW_FREQ,
				 CLK_LOW_FREQ);
	...
	case MMC_TIMING_SD_HS:
		rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, SD_CFG1, 0x0C, SD_20_MODE);
		rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, CLK_CTL, CLK_LOW_FREQ,
				 CLK_LOW_FREQ);
		rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, CARD_CLK_SOURCE, 0xFF,
	...
	default:
		...
		rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, CARD_CLK_SOURCE, 0xFF,
				 CRC_FIX_CLK | SD30_VAR_CLK0 | SAMPLE_VAR_CLK1);
		rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, CLK_CTL, CLK_LOW_FREQ, 0);
		rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, SD_PUSH_POINT_CTL, 0xFF,
				 0);
		rtsx_pci_add_cmd(pcr, WRITE_REG_CMD, SD_SAMPLE_POINT_CTL,
...
static int sd_wait_voltage_stable_2(struct realtek_pci_sdmmc *host)
{
	...
	mask = SD_CMD_STATUS | SD_DAT3_STATUS | SD_DAT2_STATUS |
	       SD_DAT1_STATUS | SD_DAT0_STATUS;
	val = SD_CMD_STATUS | SD_DAT3_STATUS | SD_DAT2_STATUS | SD_DAT1_STATUS |
	      SD_DAT0_STATUS;
	if ((stat & mask) != val) {
		dev_dbg(sdmmc_dev(host), "%s: SD_BUS_STAT = 0x%x\n", __func__,
			stat);
		rtsx_pci_write_register(pcr, SD_BUS_STAT,
					SD_CLK_TOGGLE_EN | SD_CLK_FORCE_STOP,
					0);
