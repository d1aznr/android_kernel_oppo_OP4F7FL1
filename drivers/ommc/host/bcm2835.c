			    ...
			    const char *label)
{
	...
	dev_dbg(dev,
		...
		(cmd == host->cmd) ? '>' : ' ', label, cmd->opcode, cmd->arg,
		cmd->flags, cmd->resp[0], cmd->resp[1], cmd->resp[2],
				...
				struct mmc_data *data)
{
	...
	(void)dmaengine_slave_config(dma_chan, (dir_data == DMA_FROM_DEVICE) ?
						       &host->dma_cfg_rx :
						       &host->dma_cfg_tx);
	...
	if (len > 0) {
		desc = dmaengine_prep_slave_sg(
			dma_chan, data->sg, len, dir_slave,
...
static void bcm2835_set_transfer_irqs(struct bcm2835_host *host)
{
	...
	if (host->dma_desc) {
		host->hcfg = (host->hcfg & ~all_irqs) | SDHCFG_BUSY_IRPT_EN;
	} else {
		host->hcfg = (host->hcfg & ~all_irqs) | SDHCFG_DATA_IRPT_EN |
			     SDHCFG_BUSY_IRPT_EN;
	}
...
static bool bcm2835_send_command(struct bcm2835_host *host,
				 struct mmc_command *cmd)
{
	struct device *dev = &host->pdev->dev;
	u32 sdcmd, sdhsts;
...
static irqreturn_t bcm2835_irq(int irq, void *dev_id)
{
	...
	writel(SDHSTS_BUSY_IRPT | SDHSTS_BLOCK_IRPT | SDHSTS_SDIO_IRPT |
		       SDHSTS_DATA_FLAG,
	       host->ioaddr + SDHSTS);
...
static int bcm2835_add_host(struct bcm2835_host *host)
{
	...
	ret = request_threaded_irq(host->irq, bcm2835_irq, bcm2835_threaded_irq,
