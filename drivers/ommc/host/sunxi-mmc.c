...
#define mmc_readl(host, reg) readl((host)->reg_base + SDXC_##reg)
				    ...
				    struct mmc_data *data)
{
	...
	for (i = 0; i < data->sg_len; i++) {
		pdes[i].config =
			cpu_to_le32(SDXC_IDMAC_DES0_CH | SDXC_IDMAC_DES0_OWN |
				    SDXC_IDMAC_DES0_DIC);
	...
	pdes[i - 1].config |=
		cpu_to_le32(SDXC_IDMAC_DES0_LD | SDXC_IDMAC_DES0_ER);
				       ...
				       struct mmc_request *req)
{
	...
	cmd_val = SDXC_START | SDXC_RESP_EXPIRE | SDXC_STOP_ABORT_CMD |
				  ...
				  struct mmc_ios *ios)
{
	...
	if (ios->timing == MMC_TIMING_MMC_DDR52 &&
	    (host->use_new_timings || ios->bus_width == MMC_BUS_WIDTH_8)) {
		div = 2;
		clock <<= 1;
	}
				 ...
				 struct mmc_ios *ios)
{
	...
	case MMC_POWER_UP:
		...
		if (!IS_ERR(mmc->supply.vmmc)) {
			host->ferror = mmc_regulator_set_ocr(
				mmc, mmc->supply.vmmc, ios->vdd);
			if (host->ferror)
				return;
		}
...
static const struct of_device_id sunxi_mmc_of_match[] = {
	...
	{ .compatible = "allwinner,sun8i-a83t-emmc",
	  .data = &sun8i_a83t_emmc_cfg },
	{ .compatible = "allwinner,sun9i-a80-mmc", .data = &sun9i_a80_cfg },
	{ .compatible = "allwinner,sun50i-a64-mmc", .data = &sun50i_a64_cfg },
	{ .compatible = "allwinner,sun50i-a64-emmc",
	  .data = &sun50i_a64_emmc_cfg },
	{ /* sentinel */ }
};
				      ...
				      struct platform_device *pdev)
{
	...
	host->reg_base = devm_ioremap_resource(
	...
	host->reset =
		devm_reset_control_get_optional_exclusive(&pdev->dev, "ahb");
...
static const struct dev_pm_ops sunxi_mmc_pm_ops = { SET_RUNTIME_PM_OPS(
	sunxi_mmc_runtime_suspend, sunxi_mmc_runtime_resume, NULL) };
