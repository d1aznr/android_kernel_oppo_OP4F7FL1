...
static void tmio_mmc_cmd_irq(struct tmio_mmc_host *host, unsigned int stat)
{
	...
	if (stat & TMIO_STAT_CMDTIMEOUT)
		cmd->error = -ETIMEDOUT;
		 ...
		 stat & TMIO_STAT_STOPBIT_ERR || stat & TMIO_STAT_CMD_IDX_ERR)
		cmd->error = -EILSEQ;
...
static bool __tmio_mmc_card_detect_irq(struct tmio_mmc_host *host, int ireg,
				       int status)
{
...
static void tmio_mmc_done_work(struct work_struct *work)
{
	struct tmio_mmc_host *host =
		container_of(work, struct tmio_mmc_host, done);
	...
	if (host->mrq) {
		if (IS_ERR(host->mrq)) {
			dev_dbg(dev,
				...
				current->comm, task_pid_nr(current), ios->clock,
				ios->power_mode);
...
static int tmio_multi_io_quirk(struct mmc_card *card, unsigned int direction,
			       int blk_size)
{
...
int tmio_mmc_host_probe(struct tmio_mmc_host *_host)
{
	...
	_host->native_hotplug =
		!(mmc_can_gpio_cd(mmc) || mmc->caps & MMC_CAP_NEEDS_POLL ||
	...
	_host->sdcard_irq_mask =
		sd_ctrl_read16_and_16_as_32(_host, CTL_IRQ_MASK);
	tmio_mmc_disable_mmc_irqs(_host, TMIO_MASK_ALL);

	if (_host->native_hotplug)
		tmio_mmc_enable_mmc_irqs(_host, TMIO_STAT_CARD_REMOVE |
