...
static int sdricoh_blockio(struct sdricoh_host *host, int read, u8 *buf,
			   int len)
{
	int size;
	u32 data = 0;
...
static void sdricoh_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (data && cmd->error == 0) {
		...
		for (i = 0; i < data->blocks; i++) {
			...
			result = sdricoh_blockio(
				host, data->flags & MMC_DATA_READ, buf, len);
			kunmap(page);
...
static int sdricoh_pcmcia_probe(struct pcmcia_device *pcmcia_dev)
{
	...
	while ((pci_dev =
			pci_get_device(PCI_VENDOR_ID_RICOH,
				       PCI_DEVICE_ID_RICOH_RL5C476, pci_dev))) {
