...
static int ushc_hw_reset(struct ushc_data *ushc)
{
	return usb_control_msg(ushc->usb_dev, usb_sndctrlpipe(ushc->usb_dev, 0),
			       USHC_RESET, USHC_RESET_TYPE, 0, 0, NULL, 0, 100);
}
...
static int ushc_hw_get_caps(struct ushc_data *ushc)
{
	...
	ret = usb_control_msg(ushc->usb_dev, usb_rcvctrlpipe(ushc->usb_dev, 0),
			      USHC_GET_CAPS, USHC_GET_CAPS_TYPE, 0, 0,
			      &ushc->caps, sizeof(ushc->caps), 100);
	if (ret < 0)
		return ret;
...
static int ushc_hw_set_host_ctrl(struct ushc_data *ushc, u16 mask, u16 val)
{
	...
	ret = usb_control_msg(ushc->usb_dev, usb_sndctrlpipe(ushc->usb_dev, 0),
			      USHC_HOST_CTRL, USHC_HOST_CTRL_TYPE, host_ctrl, 0,
			      NULL, 0, 100);
	if (ret < 0)
		return ret;
...
static void ushc_request(struct mmc_host *mmc, struct mmc_request *req)
{
	...
	if (req->data) {
		...
		usb_fill_bulk_urb(ushc->data_urb, ushc->usb_dev, pipe, NULL,
				  data->sg->length, data_callback, ushc);
...
static int ushc_set_power(struct ushc_data *ushc, unsigned char power_mode)
{
	...
	return usb_control_msg(ushc->usb_dev, usb_sndctrlpipe(ushc->usb_dev, 0),
			       USHC_PWR_CTRL, USHC_PWR_CTRL_TYPE, voltage, 0,
			       NULL, 0, 100);
}
...
static int ushc_set_bus_freq(struct ushc_data *ushc, int clk, bool enable_hs)
{
	...
	ret = usb_control_msg(ushc->usb_dev, usb_sndctrlpipe(ushc->usb_dev, 0),
			      USHC_CLK_FREQ, USHC_CLK_FREQ_TYPE, clk & 0xffff,
			      (clk >> 16) & 0xffff, NULL, 0, 100);
	if (ret < 0)
		return ret;
		      ...
		      const struct usb_device_id *id)
{
	...
	usb_fill_int_urb(
		ushc->int_urb, ushc->usb_dev,
		usb_rcvintpipe(usb_dev, intf->cur_altsetting->endpoint[0]
						.desc.bEndpointAddress),
		ushc->int_data, sizeof(struct ushc_int_data), int_callback,
		ushc, intf->cur_altsetting->endpoint[0].desc.bInterval);
	...
	usb_fill_bulk_urb(ushc->cbw_urb, ushc->usb_dev,
			  usb_sndbulkpipe(usb_dev, 2), ushc->cbw,
			  sizeof(struct ushc_cbw), cbw_callback, ushc);
	...
	usb_fill_bulk_urb(ushc->csw_urb, ushc->usb_dev,
			  usb_rcvbulkpipe(usb_dev, 6), ushc->csw,
			  sizeof(struct ushc_csw), csw_callback, ushc);
