...
static unsigned long dw_mci_hi6220_caps[] = { MMC_CAP_CMD23, MMC_CAP_CMD23, 0 };
...
static int dw_mci_hi6220_parse_dt(struct dw_mci *host)
{
	...
	priv->reg = syscon_regmap_lookup_by_phandle(
		host->dev->of_node, "hisilicon,peripheral-syscon");
	if (IS_ERR(priv->reg))
		priv->reg = NULL;
				 ...
				 int smpl_phase)
{
	...
	if (smpl_phase == -1)
		smpl_phase = (hs_timing_cfg[ctrl_id][timing].smpl_phase_max +
			      hs_timing_cfg[ctrl_id][timing].smpl_phase_min) /
			     2;
...
static int dw_mci_hi3660_init(struct dw_mci *host)
{
	mci_writel(host, CDTHRCTL,
		   SDMMC_SET_THLD(SDCARD_RD_THRESHOLD, SDMMC_CARD_RD_THR_EN));
