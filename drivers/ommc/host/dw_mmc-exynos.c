...
static void dw_mci_exynos_set_ios(struct dw_mci *host, struct mmc_ios *ios)
{
	...
	case MMC_TIMING_MMC_HS400:
		...
		clksel = SDMMC_CLKSEL_UP_SAMPLE(priv->hs400_timing,
						priv->tuned_sample);
		wanted <<= 1;
		break;
...
static int dw_mci_exynos_parse_dt(struct dw_mci *host)
{
	...
	ret = of_property_read_u32_array(np, "samsung,dw-mshc-sdr-timing",
					 timing, 2);
	if (ret)
		return ret;
	...
	ret = of_property_read_u32_array(np, "samsung,dw-mshc-ddr-timing",
					 timing, 2);
	if (ret)
		return ret;
	...
	ret = of_property_read_u32_array(np, "samsung,dw-mshc-hs400-timing",
					 timing, 2);
	if (!ret && of_property_read_u32(np, "samsung,read-strobe-delay",
					 &priv->dqs_delay))
		dev_dbg(host->dev,
