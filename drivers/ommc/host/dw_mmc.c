...
#define DW_MCI_CMD_ERROR_FLAGS                                                 \
	(SDMMC_INT_RTO | SDMMC_INT_RCRC | SDMMC_INT_RESP_ERR | SDMMC_INT_HLE)
...
#define IDMAC_INT_CLR                                                          \
	(SDMMC_IDMAC_INT_AI | SDMMC_IDMAC_INT_NI | SDMMC_IDMAC_INT_CES |       \
	 SDMMC_IDMAC_INT_DU | SDMMC_IDMAC_INT_FBE | SDMMC_IDMAC_INT_RI |       \
	 SDMMC_IDMAC_INT_TI)
	...
	if (mrq) {
		...
		if (cmd)
			seq_printf(
				...
				cmd->resp[1], cmd->resp[2], cmd->resp[2],
				cmd->error);
		if (data)
...
static bool dw_mci_ctrl_reset(struct dw_mci *host, u32 reset)
{
	...
	if (readl_poll_timeout_atomic(host->regs + SDMMC_CTRL, ctrl,
				      !(ctrl & reset), 1,
				      500 * USEC_PER_MSEC)) {
		dev_err(host->dev, "Timeout resetting block (ctrl reset %#x)\n",
			ctrl & reset);
		return false;
	}
...
static u32 dw_mci_prep_stop_abort(struct dw_mci *host, struct mmc_command *cmd)
{
	...
	if (cmdr == MMC_READ_SINGLE_BLOCK || cmdr == MMC_READ_MULTIPLE_BLOCK ||
	    cmdr == MMC_WRITE_BLOCK || cmdr == MMC_WRITE_MULTIPLE_BLOCK ||
...
static void dw_mci_start_command(struct dw_mci *host, struct mmc_command *cmd,
				 u32 cmd_flags)
{
	host->cmd = cmd;
...
static void dw_mci_dmac_complete_dma(void *arg)
{
	...
	if ((host->use_dma == TRANS_MODE_EDMAC) && data &&
	    (data->flags & MMC_DATA_READ))
		/* Invalidate cache after read */
		dma_sync_sg_for_cpu(mmc_dev(host->slot->mmc), data->sg,
...
static int dw_mci_idmac_init(struct dw_mci *host)
{
	...
	if (host->dma_64bit_address == 1) {
		...
		for (i = 0, p = host->sg_cpu; i < host->ring_size - 1;
			...
			p->des6 =
				(host->sg_dma +
				 (sizeof(struct idmac_desc_64addr) * (i + 1))) &
				0xffffffff;
	...
	if (host->dma_64bit_address == 1) {
		...
		mci_writel(host, IDINTEN64,
			   SDMMC_IDMAC_INT_NI | SDMMC_IDMAC_INT_RI |
				   SDMMC_IDMAC_INT_TI);
					...
					unsigned int sg_len)
{
	...
	for (i = 0; i < sg_len; i++) {
		...
		for (; length; desc++) {
			 ...
			 */
			desc->des0 =
				IDMAC_DES0_OWN | IDMAC_DES0_DIC | IDMAC_DES0_CH;
					...
					unsigned int sg_len)
{
	...
	for (i = 0; i < sg_len; i++) {
		...
		for (; length; desc++) {
			 ...
			 */
			desc->des0 =
				cpu_to_le32(IDMAC_DES0_OWN | IDMAC_DES0_DIC |
					    IDMAC_DES0_CH);
...
static int dw_mci_edmac_start_dma(struct dw_mci *host, unsigned int sg_len)
{
	...
	if (host->data->flags & MMC_DATA_WRITE)
		dma_sync_sg_for_device(mmc_dev(host->slot->mmc), sgl, sg_elems,
				       DMA_TO_DEVICE);
...
static void dw_mci_pre_req(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (dw_mci_pre_dma_transfer(slot->host, mrq->data, COOKIE_PRE_MAPPED) <
	    0)
		data->host_cookie = COOKIE_UNMAPPED;
}

static void dw_mci_post_req(struct mmc_host *mmc, struct mmc_request *mrq,
			    int err)
{
	struct dw_mci_slot *slot = mmc_priv(mmc);
	struct mmc_data *data = mrq->data;
	...
	if (data->host_cookie != COOKIE_UNMAPPED)
		dma_unmap_sg(slot->host->dev, data->sg, data->sg_len,
			     mmc_get_dma_dir(data));
	data->host_cookie = COOKIE_UNMAPPED;
}
...
static int dw_mci_submit_data_dma(struct dw_mci *host, struct mmc_data *data)
{
	...
	if (host->use_dma == TRANS_MODE_IDMAC)
		dev_vdbg(host->dev, "sd sg_cpu: %#lx sg_dma: %#lx sg_len: %d\n",
			 (unsigned long)host->sg_cpu,
			 (unsigned long)host->sg_dma, sg_len);
...
static void dw_mci_hw_reset(struct mmc_host *mmc)
{
	...
	if (!dw_mci_ctrl_reset(host,
			       SDMMC_CTRL_DMA_RESET | SDMMC_CTRL_FIFO_RESET))
		return;
...
static void dw_mci_init_card(struct mmc_host *mmc, struct mmc_card *card)
{
	...
	if (mmc->caps & MMC_CAP_SDIO_IRQ) {
		...
		if (clk_en_a != clk_en_a_old) {
			mci_writel(host, CLKENA, clk_en_a);
			mci_send_cmd(slot,
				     SDMMC_CMD_UPD_CLK | SDMMC_CMD_PRV_DAT_WAIT,
				     0);
		}
	}
}
	...
	__releases(&host->lock) __acquires(&host->lock)
{
	...
	if (!list_empty(&host->queue)) {
		slot = list_entry(host->queue.next, struct dw_mci_slot,
				  queue_node);
	...
	*/
	...
	if (host->use_dma == TRANS_MODE_IDMAC) {
		...
		*/
		...
		host->sg_cpu = dmam_alloc_coherent(host->dev, DESC_RING_BUF_SZ,
		...
		if ((device_property_read_string_array(dev, "dma-names", NULL,
						       0) < 0) ||
...
int dw_mci_probe(struct dw_mci *host)
{
	...
	if (!i) {
		...
		WARN((i != 1), "HCON reports a reserved host data width!\n"
	...
	mci_writel(host, INTMASK,
		   SDMMC_INT_CMD_DONE | SDMMC_INT_DATA_OVER | SDMMC_INT_TXDR |
...
int dw_mci_runtime_suspend(struct device *dev)
{
	...
	if (host->slot && (mmc_can_gpio_cd(host->slot->mmc) ||
