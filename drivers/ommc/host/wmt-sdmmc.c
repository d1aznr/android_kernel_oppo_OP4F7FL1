...
static void wmt_dma_init_descriptor(struct wmt_dma_descriptor *desc,
				    u16 req_count, u32 buffer_addr,
				    u32 branch_addr, int end)
{
...
static void wmt_dma_config(struct mmc_host *mmc, u32 descaddr, u8 dir)
{
	...
	if (dir == PDMA_WRITE) {
		...
		writel(reg_tmp & DMA_CCR_IF_TO_PERIPHERAL,
		       priv->sdmmc_base + SDDMA_CCR);
	} else {
		reg_tmp = readl(priv->sdmmc_base + SDDMA_CCR);
		writel(reg_tmp | DMA_CCR_PERIPHERAL_TO_IF,
		       priv->sdmmc_base + SDDMA_CCR);
	}
}
...
static int wmt_mci_probe(struct platform_device *pdev)
{
	...
	priv->dma_desc_buffer =
		dma_alloc_coherent(&pdev->dev, mmc->max_blk_count * 16,
				   &priv->dma_desc_device_addr, GFP_KERNEL);
