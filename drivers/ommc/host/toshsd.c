...
static irqreturn_t toshsd_irq(int irq, void *dev_id)
{
	...
	if (int_reg & SD_BUF_CMD_TIMEOUT) {
	...
	} else if (int_reg & (SD_BUF_ILLEGAL_ACCESS | SD_BUF_CMD_INDEX_ERR |
			      SD_BUF_STOP_BIT_END_ERR | SD_BUF_OVERFLOW |
			      SD_BUF_UNDERFLOW | SD_BUF_DATA_TIMEOUT)) {
		...
		dev_err(&host->pdev->dev,
			...
			detail & SD_ERR0_RESP_NON_CMD12_END_BIT_ERR ?
				"RESP_END_BIT " :
				"",
			detail & SD_ERR0_RESP_CMD12_END_BIT_ERR ?
				"RESP_END_BIT " :
				"",
			detail & SD_ERR0_READ_DATA_END_BIT_ERR ?
				"READ_DATA_END_BIT " :
				"",
			detail & SD_ERR0_WRITE_CRC_STATUS_END_BIT_ERR ?
				"WRITE_CMD_END_BIT " :
				"",
			...
			detail & SD_ERR1_TIMEOUT_READ_DATA ?
				"READ_DATA_TIMEOUT " :
				"",
			detail & SD_ERR1_TIMEOUT_CRS_STATUS ?
				"CRS_STATUS_TIMEOUT " :
				"",
			detail & SD_ERR1_TIMEOUT_CRC_BUSY ?
				"CRC_BUSY_TIMEOUT " :
				"");
		error = -EIO;
	}
	...
	if (error) {
		...
		if (error == -ETIMEDOUT) {
			iowrite32(int_status & ~(SD_BUF_CMD_TIMEOUT |
						 SD_CARD_RESP_END),
	...
	if (int_reg & (SD_CARD_CARD_INSERTED_0 | SD_CARD_CARD_REMOVED_0)) {
		iowrite32(int_status & ~(SD_CARD_CARD_REMOVED_0 |
...
static int toshsd_get_ro(struct mmc_host *mmc)
{
	...
	/* active low */
	return !(ioread16(host->ioaddr + SD_CARDSTATUS) &
...
static const struct dev_pm_ops toshsd_pm_ops = { SET_SYSTEM_SLEEP_PM_OPS(
	toshsd_pm_suspend, toshsd_pm_resume) };
