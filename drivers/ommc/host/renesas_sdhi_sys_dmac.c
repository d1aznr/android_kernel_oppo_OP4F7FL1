...
static const struct of_device_id renesas_sdhi_sys_dmac_of_match[] = {
	...
	},
	{
		...
		.data = &of_rz_compatible,
	},
	{
		.compatible = "renesas,sdhi-r8a7778",
		.data = &of_rcar_gen1_compatible,
	},
	{
	...
	},
	{
		...
		.data = &of_rcar_r8a7795_compatible,
	},
	{
	...
	{
		.compatible = "renesas,rcar-gen1-sdhi",
		.data = &of_rcar_gen1_compatible,
	},
	{
		.compatible = "renesas,rcar-gen2-sdhi",
		.data = &of_rcar_gen2_compatible,
	},
	{
		.compatible = "renesas,rcar-gen3-sdhi",
...
static void renesas_sdhi_sys_dmac_dma_callback(void *arg)
{
	...
	if (host->data->flags & MMC_DATA_READ)
		dma_unmap_sg(host->chan_rx->device->dev, host->sg_ptr,
			     host->sg_len, DMA_FROM_DEVICE);
	else
		dma_unmap_sg(host->chan_tx->device->dev, host->sg_ptr,
			     host->sg_len, DMA_TO_DEVICE);
					      ...
					      struct tmio_mmc_data *pdata)
{
	...
	if (!host->chan_tx && !host->chan_rx) {
		struct resource *res =
			platform_get_resource(host->pdev, IORESOURCE_MEM, 0);
		...
		host->chan_tx = dma_request_slave_channel_compat(
			mask, priv->dma_priv.filter, pdata->chan_priv_tx,
			&host->pdev->dev, "tx");
		...
		cfg.dst_addr =
			res->start + (CTL_SD_DATA_PORT << host->bus_shift);
		...
		host->chan_rx = dma_request_slave_channel_compat(
			mask, priv->dma_priv.filter, pdata->chan_priv_rx,
			&host->pdev->dev, "rx");
