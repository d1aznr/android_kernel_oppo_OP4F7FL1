...
static int sdhci_cdns_write_phy_reg(struct sdhci_cdns_priv *priv, u8 addr,
				    u8 data)
{
...
static int sdhci_cdns_set_tune_val(struct sdhci_host *host, unsigned int val)
{
	...
	for (i = 0; i < 2; i++) {
		...
		ret = readl_poll_timeout(
			reg, tmp, !(tmp & SDHCI_CDNS_HRS06_TUNE_UP), 0, 1);
		if (ret)
			return ret;
	}
					     ...
					     struct mmc_ios *ios)
{
	...
	if (mode == SDHCI_CDNS_HRS06_MODE_MMC_HS400ES && !ios->enhanced_strobe)
		sdhci_cdns_set_emmc_mode(priv, SDHCI_CDNS_HRS06_MODE_MMC_HS400);
}
...
static const struct dev_pm_ops sdhci_cdns_pm_ops = { SET_SYSTEM_SLEEP_PM_OPS(
	sdhci_pltfm_suspend, sdhci_cdns_resume) };
