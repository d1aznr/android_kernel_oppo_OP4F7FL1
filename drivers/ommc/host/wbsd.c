...
#define DBG(x...) pr_debug(DRIVER_NAME ": " x)
#define DBGF(f, x...) pr_debug(DRIVER_NAME " [%s()]: " f, __func__, ##x)
				       ...
				       struct mmc_command *cmd)
{
	...
	for (i = 0; i < 4; i++) {
		cmd->resp[i] = wbsd_read_index(host, WBSD_IDX_RESP1 + i * 4)
			       << 24;
		cmd->resp[i] |= wbsd_read_index(host, WBSD_IDX_RESP2 + i * 4)
				<< 16;
		cmd->resp[i] |= wbsd_read_index(host, WBSD_IDX_RESP3 + i * 4)
				<< 8;
		cmd->resp[i] |= wbsd_read_index(host, WBSD_IDX_RESP4 + i * 4)
				<< 0;
	}
}
...
static void wbsd_request_dma(struct wbsd_host *host, int dma)
{
	 ...
	 */
	host->dma_buffer = kmalloc(WBSD_DMA_SIZE, GFP_NOIO | GFP_DMA |
							  __GFP_RETRY_MAYFAIL |
							  __GFP_NOWARN);
	if (!host->dma_buffer)
		goto free;
...
unmap:
	...
	dma_unmap_single(mmc_dev(host->mmc), host->dma_addr, WBSD_DMA_SIZE,
			 DMA_BIDIRECTIONAL);
	host->dma_addr = 0;
...
static int wbsd_request_resources(struct wbsd_host *host, int base, int irq,
				  int dma)
{
	int ret;
...
static int wbsd_init(struct device *dev, int base, int irq, int dma, int pnp)
{
...
#endif
MODULE_PARM_DESC(
	io, "I/O base to allocate. Must be 8 byte aligned. (default 0x248)");
