			   ...
			   int stop)
{
	snprintf(host->dbgmsg_cmd, 300,
		 "#%u%s op:%i arg:0x%08x flags:0x08%x retries:%u", host->ccnt,
		 (stop ? " (STOP)" : ""), cmd->opcode, cmd->arg, cmd->flags,
		 cmd->retries);

	if (cmd->data) {
		snprintf(host->dbgmsg_dat, 300,
			 "#%u bsize:%u blocks:%u bytes:%u", host->dcnt,
...
static inline int get_data_buffer(struct s3cmci_host *host, u32 *bytes,
				  u32 **pointer)
{
...
static void do_pio_read(struct s3cmci_host *host)
{
	...
	while ((fifo = fifo_count(host))) {
		...
		dbg(host, dbg_pio,
		    ...
		    host->pio_bytes, readl(host->base + S3C2410_SDIDCNT));
...
static void s3cmci_set_clk(struct s3cmci_host *host, struct mmc_ios *ios)
{
	...
	for (mci_psc = 0; mci_psc < 255; mci_psc++) {
		host->real_rate =
			host->clk_rate / (host->clk_div * (mci_psc + 1));
				     ...
				     unsigned long val, void *data)
{
	...
	if ((val == CPUFREQ_PRECHANGE && newclk > host->clk_rate) ||
		...
		if (mmc->ios.power_mode != MMC_POWER_OFF && mmc->ios.clock != 0)
...
static const struct s3cmci_reg debug_regs[] = {
	DBG_REG(CON),	  DBG_REG(PRE),	  DBG_REG(CMDARG), DBG_REG(CMDCON),
	DBG_REG(CMDSTAT), DBG_REG(RSP0),  DBG_REG(RSP1),   DBG_REG(RSP2),
	DBG_REG(RSP3),	  DBG_REG(TIMER), DBG_REG(BSIZE),  DBG_REG(DCON),
	DBG_REG(DCNT),	  DBG_REG(DSTA),  DBG_REG(FSTA),   {}
};
...
static void s3cmci_debugfs_attach(struct s3cmci_host *host)
{
	...
	host->debug_state = debugfs_create_file("state", 0444, host->debug_root,
						host, &s3cmci_fops_state);

	if (IS_ERR(host->debug_state))
		dev_err(dev, "failed to create debug state file\n");

	host->debug_regs = debugfs_create_file("regs", 0444, host->debug_root,
					       host, &s3cmci_fops_regs);
...
static int s3cmci_probe(struct platform_device *pdev)
{
	...
	dbg(host, dbg_debug,
	    ...
	    (host->is2440 ? "2440" : ""), host->base, host->irq, host->irq_cd,
	    host->dma);
...
MODULE_AUTHOR(
	"Thomas Kleffel <tk@maintech.de>, Ben Dooks <ben-linux@fluff.org>");
