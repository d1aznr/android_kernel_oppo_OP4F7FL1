...
{
	slot->host->caps =
		((0x21 << SDHCI_TIMEOUT_CLK_SHIFT) & SDHCI_TIMEOUT_CLK_MASK) |

		((0x21 << SDHCI_CLOCK_BASE_SHIFT) & SDHCI_CLOCK_BASE_MASK) |

		SDHCI_TIMEOUT_CLK_UNIT | SDHCI_CAN_VDD_330 |
		SDHCI_CAN_DO_HISPD | SDHCI_CAN_DO_SDMA;
	return 0;
}
...
static const struct sdhci_pci_fixes sdhci_ricoh = {
	.probe = ricoh_probe,
	.quirks = SDHCI_QUIRK_32BIT_DMA_ADDR | SDHCI_QUIRK_FORCE_DMA |
...
#endif
	.quirks = SDHCI_QUIRK_32BIT_DMA_ADDR | SDHCI_QUIRK_CLOCK_BEFORE_RESET |
		  SDHCI_QUIRK_NO_CARD_NO_RESET | SDHCI_QUIRK_MISSING_CAPS
};

static const struct sdhci_pci_fixes sdhci_ene_712 = {
	.quirks = SDHCI_QUIRK_SINGLE_POWER_WRITE | SDHCI_QUIRK_BROKEN_DMA,
};

static const struct sdhci_pci_fixes sdhci_ene_714 = {
	.quirks = SDHCI_QUIRK_SINGLE_POWER_WRITE |
		  SDHCI_QUIRK_RESET_CMD_DATA_ON_IOS | SDHCI_QUIRK_BROKEN_DMA,
};

static const struct sdhci_pci_fixes sdhci_cafe = {
	.quirks = SDHCI_QUIRK_NO_SIMULT_VDD_AND_POWER |
		  SDHCI_QUIRK_NO_BUSY_IRQ | SDHCI_QUIRK_BROKEN_CARD_DETECTION |
...
static void sdhci_pci_add_own_cd(struct sdhci_pci_slot *slot)
{
	...
	err = request_irq(irq, sdhci_pci_sd_cd,
			  IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "sd_cd",
			  slot);
	if (err)
		goto out_free;
...
static const guid_t intel_dsm_guid =
	GUID_INIT(0xF6C13EA5, 0x65CD, 0x461F, 0xAB, 0x7A, 0x29, 0xF7, 0xE8,
		  0xD5, 0xBD, 0x61);
...
static void glk_rpm_retune_wa(struct sdhci_pci_chip *chip, bool susp)
{
	...
	mmc_retune_needed(host->mmc);
	pr_info("%s: Requiring re-tune after rpm resume",
...
static const struct sdhci_pci_fixes sdhci_intel_byt_sd = {
	...
	.quirks2 = SDHCI_QUIRK2_CARD_ON_NEEDS_BUS_ON |
		   SDHCI_QUIRK2_PRESET_VALUE_BROKEN | SDHCI_QUIRK2_STOP_WITH_TC,
	.allow_runtime_pm = true,
	.own_cd_for_runtime_pm = true,
...
#else
static inline void
intel_mrfld_mmc_fix_up_power_slot(struct sdhci_pci_slot *slot)
{
}
#endif
...
static int intel_mrfld_mmc_probe_slot(struct sdhci_pci_slot *slot)
{
	...
	case INTEL_MRFLD_EMMC_1:
		slot->host->mmc->caps |= MMC_CAP_NONREMOVABLE |
					 MMC_CAP_8_BIT_DATA | MMC_CAP_1_8V_DDR;
		break;
	case INTEL_MRFLD_SD:
...
static const struct sdhci_pci_fixes sdhci_intel_mrfld_mmc = {
	.quirks = SDHCI_QUIRK_NO_ENDATTR_IN_NOPDESC,
	.quirks2 = SDHCI_QUIRK2_BROKEN_HS200 | SDHCI_QUIRK2_PRESET_VALUE_BROKEN,
	.allow_runtime_pm = true,
...
static int jmicron_probe(struct sdhci_pci_chip *chip)
{
	...
	if (mmcdev) {
		...
		sd_dev = NULL;
		while ((sd_dev = pci_get_device(PCI_VENDOR_ID_JMICRON, mmcdev,
						sd_dev)) != NULL) {
...
static int syskt_probe_slot(struct sdhci_pci_slot *slot)
{
	...
	dev_info(&slot->chip->pdev->dev,
		 ...
		 board_rev >> 4, board_rev & 0xf, chip_rev >> 4,
		 chip_rev & 0xf);
	if (chip_rev >= 0x20)
...
static const struct sdhci_pci_fixes sdhci_rtsx = {
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN |
		   SDHCI_QUIRK2_BROKEN_64_BIT_DMA | SDHCI_QUIRK2_BROKEN_DDR50,
...
static struct sdhci_pci_slot *sdhci_pci_probe_slot(struct pci_dev *pdev,
						   struct sdhci_pci_chip *chip,
						   int first_bar, int slotno)
{
	struct sdhci_pci_slot *slot;
	struct sdhci_host *host;
	...
	host->hw_name = "PCI";
	host->ops = chip->fixes && chip->fixes->ops ? chip->fixes->ops :
						      &sdhci_pci_ops;
	...
	if (gpio_is_valid(slot->rst_n_gpio)) {
		if (!devm_gpio_request(&pdev->dev, slot->rst_n_gpio,
				       "eMMC_reset")) {
