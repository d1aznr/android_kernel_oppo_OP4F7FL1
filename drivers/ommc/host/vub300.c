...
static void
__add_offloaded_reg_to_fifo(struct vub300_mmc_host *vub300,
...
{ /* softirq */
	struct vub300_mmc_host *vub300 =
		from_timer(vub300, t, inactivity_timer);
	if (!vub300->interface) {
		kref_put(&vub300->kref, vub300_delete);
	} else if (vub300->cmd) {
		mod_timer(&vub300->inactivity_timer, jiffies + HZ);
...
{
	struct vub300_mmc_host *vub300 =
		from_timer(vub300, t, sg_transfer_timer);
	vub300->usb_timed_out = 1;
	usb_sg_cancel(&vub300->sg_request);
					  ...
					  const struct firmware *fw)
{
	...
	if (interrupt_size < size) {
		...
		if (xfer_buffer) {
			...
			retval = usb_control_msg(
				vub300->udev, usb_sndctrlpipe(vub300->udev, 0),
				SET_INTERRUPT_PSEUDOCODE,
				USB_DIR_OUT | USB_TYPE_VENDOR |
					USB_RECIP_DEVICE,
				0x0000, 0x0000, xfer_buffer, xfer_length, HZ);
			kfree(xfer_buffer);
			if (retval < 0)
	...
	if (ts < size) {
		...
		if (xfer_buffer) {
			...
			memset(xfer_buffer + ts, 0, xfer_length - ts);
			size -= ts;
			data += ts;
			retval = usb_control_msg(
				vub300->udev, usb_sndctrlpipe(vub300->udev, 0),
			       ...
			       struct mmc_command *cmd, struct mmc_data *data)
{
	...
	if ((padded_length == linear_length) || !pad_input_to_usb_pkt) {
		...
		result = usb_sg_init(&vub300->sg_request, vub300->udev, pipe, 0,
				     data->sg, data->sg_len, 0, GFP_KERNEL);
		if (result < 0) {
			usb_unlink_urb(vub300->command_out_urb);
			usb_unlink_urb(vub300->command_res_urb);
			cmd->error = result;
		...
		if (buf) {
			...
			unsigned pipe = usb_rcvbulkpipe(vub300->udev,
							vub300->data_inp_ep);
			int actual_length = 0;
			result = vub300_usb_bulk_msg(
				vub300, pipe, buf, padded_length,
				&actual_length, 2000 + (padded_length / 16384));
			if (result < 0) {
				cmd->error = result;
				data->bytes_xfered = 0;
				kfree(buf);
				...
				struct mmc_command *cmd, struct mmc_data *data)
{
	...
	if (linear_length < 64) {
		int result;
		int actual_length;
		sg_copy_to_buffer(data->sg, data->sg_len, vub300->padded_buffer,
				  sizeof(vub300->padded_buffer));
		memset(vub300->padded_buffer + linear_length, 0,
		       sizeof(vub300->padded_buffer) - linear_length);
		result = vub300_usb_bulk_msg(
			vub300, pipe, vub300->padded_buffer,
			sizeof(vub300->padded_buffer), &actual_length,
			2000 + (sizeof(vub300->padded_buffer) / 16384));
		if (result < 0) {
			cmd->error = result;
			data->bytes_xfered = 0;
		} else {
		   ...
		   (vub300->large_usb_packets &&
		    (64 >
		     modulo_512_length))) { /* don't you just love these work-rounds */
		...
		if (result < 0) {
			...
			vub300->sg_transfer_timer.expires =
				jiffies +
				msecs_to_jiffies(2000 + linear_length / 16384);
			add_timer(&vub300->sg_transfer_timer);
			usb_sg_wait(&vub300->sg_request);
			if (cmd->error) {
				data->bytes_xfered = 0;
				      ...
				      struct mmc_data *data, int data_length)
{
	...
	respretval = wait_for_completion_timeout(
		&vub300->command_complete, msecs_to_jiffies(msec_timeout));
	if (respretval == 0) { /* TIMED OUT */
		...
		int offloaded_data_length = vub300->resp.common.header_size -
				 ...
				 struct mmc_command *cmd, u8 Function)
{
	...
	if (cmd0 == rf->command_byte[0] && cmd1 == rf->command_byte[1] &&
	    cmd2 == rf->command_byte[2] && cmd3 == rf->command_byte[3]) {
		u8 checksum = 0x00;
			      ...
			      struct mmc_ios *ios)
{
	...
	retval = usb_control_msg(
		vub300->udev, usb_sndctrlpipe(vub300->udev, 0), SET_CLOCK_SPEED,
		USB_DIR_OUT | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x00, 0x00,
		buf, buf_array_size, HZ);
	if (retval != 8) {
		dev_err(&vub300->udev->dev,
			"SET_CLOCK_SPEED"
...
{ /* NOT irq */
	...
	dev_info(&udev->dev, "probing VID:PID(%04X:%04X) %s %s %s\n",
		 ...
		 le16_to_cpu(udev->descriptor.idProduct), manufacturer, product,
		 serial_number);
	...
	if (vub300->cmnd_res_ep && vub300->cmnd_out_ep && vub300->data_inp_ep &&
	    vub300->data_out_ep) {
		dev_info(&vub300->udev->dev,
			 "vub300 %s packets"
	...
	retval = usb_control_msg(
		vub300->udev, usb_rcvctrlpipe(vub300->udev, 0), GET_HC_INF0,
		USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0000, 0x0000,
		&vub300->hc_info, sizeof(vub300->hc_info), HZ);
	if (retval < 0)
		goto error5;
	retval = usb_control_msg(vub300->udev, usb_rcvctrlpipe(vub300->udev, 0),
				 SET_ROM_WAIT_STATES,
				 USB_DIR_OUT | USB_TYPE_VENDOR |
					 USB_RECIP_DEVICE,
	...
	kref_get(&vub300->kref);
	timer_setup(&vub300->inactivity_timer, vub300_inactivity_timer_expired,
		    0);
