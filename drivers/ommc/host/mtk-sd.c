...
static void msdc_cmd_next(struct msdc_host *host, struct mmc_request *mrq,
			  struct mmc_command *cmd);
...
static const u32 data_ints_mask =
	...
	MSDC_INTEN_DMA_BDCSERR | MSDC_INTEN_DMA_GPDCSERR |
...
static void msdc_set_timeout(struct msdc_host *host, u32 ns, u32 clks)
{
	...
	if (host->mmc->actual_clock == 0) {
		...
		if (host->dev_comp->clk_div_bits == 8)
			sdr_get_field(host->base + MSDC_CFG, MSDC_CFG_CKMOD,
				      &mode);
		else
...
static void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	if (timing == MMC_TIMING_MMC_HS400 && host->dev_comp->hs400_tune)
			  ...
			  struct mmc_request *mrq, struct mmc_command *cmd)
{
	...
	if (!sbc_error && !(events & (MSDC_INT_CMDRDY | MSDC_INT_RSPCRCERR |
				      MSDC_INT_CMDTMO)))
		return done;
...
static void msdc_start_command(struct msdc_host *host, struct mmc_request *mrq,
			       struct mmc_command *cmd)
{
	u32 rawcmd;
				...
				struct mmc_request *mrq, struct mmc_data *data)
{
	...
	unsigned int check_data =
		events & (MSDC_INT_XFER_COMPL | MSDC_INT_DATCRCERR |
			  MSDC_INT_DATTMO | MSDC_INT_DMA_BDCSERR |
	...
	if (check_data || (stop && stop->error)) {
		...
		if ((events & MSDC_INT_XFER_COMPL) && (!stop || !stop->error)) {
			...
			dev_dbg(host->dev, "%s: cmd=%d; blocks=%d", __func__,
...
static void msdc_request_timeout(struct work_struct *work)
{
	struct msdc_host *host =
		container_of(work, struct msdc_host, req_timeout.work);
...
skip_fall:
	...
	for (i = 0; i < PAD_DELAY_MAX; i++) {
		sdr_set_field(host->base + tune_reg, MSDC_PAD_TUNE_CMDRRDLY, i);
		mmc_send_tuning(mmc, opcode, &cmd_err);
		if (!cmd_err)
			internal_delay |= (1 << i);
	}
...
static int msdc_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	if (host->hs400_mode && host->dev_comp->hs400_tune)
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	host->dma.gpd =
		dma_alloc_coherent(&pdev->dev, 2 * sizeof(struct mt_gpdma_desc),
...
release_mem:
	if (host->dma.gpd)
		dma_free_coherent(&pdev->dev, 2 * sizeof(struct mt_gpdma_desc),
