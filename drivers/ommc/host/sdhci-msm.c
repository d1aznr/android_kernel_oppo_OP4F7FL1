...
void sdhci_msm_enter_dbg_mode(struct sdhci_host *host)
{
	...
	minor = IPCAT_MINOR_MASK(readl_relaxed(host->ioaddr + SDCC_IP_CATALOG));
					  ...
					  struct sdhci_msm_pltfm_data *pdata)
{
	...
	if (sdhci_msm_dt_parse_vreg_info(
		    dev, &pdata->vreg_data->vdd_io_bias_data, "vdd-io-bias")) {
...
static void sdhci_msm_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	if ((clock > 400000) && !curr_pwrsave &&
	    card /*&& mmc_host_may_gate_card(card)*/)
		writel_relaxed(
			readl_relaxed(host->ioaddr +
				      msm_host_offset->CORE_VENDOR_SPEC) |
				CORE_CLK_PWRSAVE,
