...
#define INT_ERR_STS                                                            \
	(INT_CMDVIO | INT_BUFVIO | INT_WDATERR | INT_RDATERR | INT_RIDXERR |   \
	 INT_RSPERR | INT_CCSTO | INT_CRCSTO | INT_WDATTO | INT_RDATTO |       \
	 INT_RBSYTO | INT_RSPTO)

#define INT_ALL                                                                \
	(INT_RBSYE | INT_CRSPE | INT_BUFREN | INT_BUFWEN | INT_CMD12DRE |      \
	 INT_BUFRE | INT_DTRANE | INT_CMD12RBE | INT_CMD12CRE)
...
#define MASK_START_CMD                                                         \
	(MASK_MCMDVIO | MASK_MBUFVIO | MASK_MWDATERR | MASK_MRDATERR |         \
	 MASK_MRIDXERR | MASK_MRSPERR | MASK_MCRCSTO | MASK_MWDATTO |          \
	 MASK_MRDATTO | MASK_MRBSYTO | MASK_MRSPTO)

#define MASK_CLEAN                                                             \
	(INT_ERR_STS | MASK_MRBSYE | MASK_MCRSPE | MASK_MBUFREN |              \
	 MASK_MBUFWEN | MASK_MCMD12DRE | MASK_MBUFRE | MASK_MDTRANE |          \
...
#define STS2_CRC_ERR                                                           \
	(STS2_CRCSTE | STS2_CRC16E | STS2_AC12CRCE | STS2_RSPCRC7E |           \
	 STS2_CRCSTEBE)
#define STS2_TIMEOUT_ERR                                                       \
	(STS2_CCSTO | STS2_RDATTO | STS2_DATBSYTO | STS2_CRCSTTO |             \
	 STS2_AC12BSYTO | STS2_RSPBSYTO | STS2_AC12RSPTO | STS2_RSPTO)
...
static inline void sh_mmcif_bitset(struct sh_mmcif_host *host, unsigned int reg,
				   u32 val)
{
	writel(val | readl(host->addr + reg), host->addr + reg);
}

static inline void sh_mmcif_bitclr(struct sh_mmcif_host *host, unsigned int reg,
				   u32 val)
{
	writel(~val & readl(host->addr + reg), host->addr + reg);
}
...
static void sh_mmcif_start_dma_rx(struct sh_mmcif_host *host)
{
	...
	ret = dma_map_sg(chan->device->dev, sg, data->sg_len, DMA_FROM_DEVICE);
	if (ret > 0) {
	...
	if (!desc) {
		...
		dev_warn(dev, "DMA failed: %d, falling back to PIO\n", ret);
...
static void sh_mmcif_start_dma_tx(struct sh_mmcif_host *host)
{
	...
	ret = dma_map_sg(chan->device->dev, sg, data->sg_len, DMA_TO_DEVICE);
	if (ret > 0) {
		host->dma_active = true;
...
static void sh_mmcif_request_dma(struct sh_mmcif_host *host)
{
	...
	if (IS_ENABLED(CONFIG_SUPERH) && dev->platform_data) {
		...
		host->chan_tx =
			sh_mmcif_request_dma_pdata(host, pdata->slave_id_tx);
		host->chan_rx =
			sh_mmcif_request_dma_pdata(host, pdata->slave_id_rx);
	} else {
...
static void sh_mmcif_clock_control(struct sh_mmcif_host *host, unsigned int clk)
{
	...
	if (host->clkdiv_map) {
		...
		dev_dbg(dev, "clk %u/%u (%u, 0x%x)\n",
			(best_freq / (1 << (clkdiv + 1))), clk, best_freq,
			clkdiv);
...
static void sh_mmcif_sync_reset(struct sh_mmcif_host *host)
{
	...
	sh_mmcif_bitset(host, MMCIF_CE_CLK_CTRL,
			tmp | SRSPTO_256 | SRBSYTO_29 | SRWDTO_29);
	/* byte swap on */
...
static u32 sh_mmcif_set_cmd(struct sh_mmcif_host *host, struct mmc_request *mrq)
{
	struct device *dev = sh_mmcif_host_to_dev(host);
	struct mmc_data *data = mrq->data;
	...
	if (opc == MMC_READ_MULTIPLE_BLOCK || opc == MMC_WRITE_MULTIPLE_BLOCK) {
		...
		sh_mmcif_bitset(host, MMCIF_CE_BLOCK_SET, data->blocks << 16);
	}
	...
	if (opc == MMC_ALL_SEND_CID || opc == MMC_SEND_CSD ||
	    opc == MMC_SEND_CID)
...
static void sh_mmcif_timeout_work(struct work_struct *work)
{
	...
	struct sh_mmcif_host *host =
		container_of(d, struct sh_mmcif_host, timeout_work);
	struct mmc_request *mrq = host->mrq;
	struct device *dev = sh_mmcif_host_to_dev(host);
	unsigned long flags;
...
static int sh_mmcif_probe(struct platform_device *pdev)
{
	...
	if (irq[1] >= 0) {
		ret = devm_request_threaded_irq(dev, irq[1], sh_mmcif_intr,
						sh_mmcif_irqt, 0, "sh_mmc:int",
						host);
		if (ret) {
...
static const struct dev_pm_ops sh_mmcif_dev_pm_ops = { SET_SYSTEM_SLEEP_PM_OPS(
	sh_mmcif_suspend, sh_mmcif_resume) };
