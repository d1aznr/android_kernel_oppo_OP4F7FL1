...
#define ATMCI_DMA_CHKSIZE(x)                                                   \
	((x) << 4) /* DMA Channel Read and Write Chunk Size */
...
#define ATMCI_DATA_ERROR_FLAGS                                                 \
	(ATMCI_DCRCE | ATMCI_DTOE | ATMCI_OVRE | ATMCI_UNRE)
	...
	if (mrq) {
		...
		if (cmd)
			seq_printf(
				...
				cmd->opcode, cmd->arg, cmd->flags, cmd->resp[0],
...
static void atmci_show_status_reg(struct seq_file *s, const char *regname,
				  u32 value)
{
	static const char *sr_bit[] = {
		[0] = "CMDRDY",	  [1] = "RXRDY",     [2] = "TXRDY",
		[3] = "BLKE",	  [4] = "DTIP",	     [5] = "NOTBUSY",
		[6] = "ENDRX",	  [7] = "ENDTX",     [8] = "SDIOIRQA",
		[9] = "SDIOIRQB", [12] = "SDIOWAIT", [14] = "RXBUFF",
		[15] = "TXBUFE",  [16] = "RINDE",    [17] = "RDIRE",
		[18] = "RCRCE",	  [19] = "RENDE",    [20] = "RTOE",
		[21] = "DCRCE",	  [22] = "DTOE",     [23] = "CSTOE",
		[24] = "BLKOVRE", [25] = "DMADONE",  [26] = "FIFOEMPTY",
		[27] = "XFRDONE", [30] = "OVRE",     [31] = "UNRE",
	};
	unsigned int i;
...
static int atmci_regs_show(struct seq_file *s, void *v)
{
	...
	seq_printf(s, "MR:\t0x%08x%s%s ", buf[ATMCI_MR / 4],
	...
	if (host->caps.has_cfg_reg) {
		...
		seq_printf(s, "CFG:\t0x%08x%s%s%s%s\n", val,
			   val & ATMCI_CFG_FIFOMODE_1DATA ?
				   " FIFOMODE_ONE_DATA" :
				   "",
			   val & ATMCI_CFG_FERRCTRL_COR ?
...
static const struct of_device_id atmci_dt_ids[] = { { .compatible =
							      "atmel,hsmci" },
						    { /* sentinel */ } };
...
static void atmci_set_timeout(struct atmel_mci *host,
			      ...
			      struct mmc_data *data)
{
	static unsigned dtomul_to_shift[] = { 0, 4, 7, 8, 10, 12, 16, 20 };
	unsigned timeout;
	unsigned dtocyc;
	unsigned dtomul;

	timeout =
		atmci_ns_to_clocks(host, data->timeout_ns) + data->timeout_clks;
	...
	atmci_writel(host, ATMCI_DTOR,
		     (ATMCI_DTOMUL(dtomul) | ATMCI_DTOCYC(dtocyc)));
}
...
static void atmci_send_command(struct atmel_mci *host, struct mmc_command *cmd,
			       u32 cmd_flags)
{
	WARN_ON(host->cmd);
	host->cmd = cmd;
	...
	if (host->data_size <= buf_size) {
		if (host->data_size & 0x3) {
			...
			atmci_writel(host, ATMCI_MR,
				     host->mode_reg | ATMCI_MR_PDCFBYTE);
		} else {
...
static void atmci_pdc_complete(struct atmel_mci *host)
{
	...
	if ((!host->caps.has_rwproof) && (host->data->flags & MMC_DATA_READ)) {
...
static void atmci_dma_cleanup(struct atmel_mci *host)
{
	...
	if (data)
		dma_unmap_sg(host->dma.chan->device->dev, data->sg,
...
static void atmci_dma_complete(void *arg)
{
	...
	if (host->caps.has_dma_conf_reg)
		...
		atmci_writel(host, ATMCI_DMA,
			     atmci_readl(host, ATMCI_DMA) & ~ATMCI_DMAEN);
...
static u32 atmci_prepare_data(struct atmel_mci *host, struct mmc_data *data)
{
	 ...
	 */
	if (data->blocks * data->blksz < 12 || (data->blocks * data->blksz) & 3)
 ...
 * PDC registers.
 */
static u32 atmci_prepare_data_pdc(struct atmel_mci *host, struct mmc_data *data)
{
	u32 iflags, tmp;
	int i;
	...
	if ((!host->caps.has_rwproof) && (host->data->flags & MMC_DATA_WRITE)) {
...
static u32 atmci_prepare_data_dma(struct atmel_mci *host, struct mmc_data *data)
{
	struct dma_chan *chan;
	...
	sglen = dma_map_sg(chan->device->dev, data->sg, data->sg_len,
			   mmc_get_dma_dir(data));

	dmaengine_slave_config(chan, &host->dma_conf);
	desc = dmaengine_prep_slave_sg(chan, data->sg, sglen, slave_dirn,
 ...
 */
static void atmci_submit_data_pdc(struct atmel_mci *host, struct mmc_data *data)
{
	if (data->flags & MMC_DATA_READ)
...
static void atmci_submit_data_dma(struct atmel_mci *host, struct mmc_data *data)
{
				...
				struct atmel_mci_slot *slot)
{
	...
	if (data) {
		...
		atmci_writel(host, ATMCI_BLKR,
			     ATMCI_BCNT(data->blocks) |
	...
	__releases(&host->lock) __acquires(&host->lock)
{
	...
	host->mrq = NULL;
	if (!list_empty(&host->queue)) {
		slot = list_entry(host->queue.next, struct atmel_mci_slot,
				  queue_node);
...
static void atmci_tasklet_func(unsigned long priv)
{
	...
	do {
		...
		case STATE_SENDING_CMD:
			...
			if (!atmci_test_and_clear_pending(host, EVENT_CMD_RDY))
				break;
		...
		case STATE_WAITING_NOTBUSY:
			...
			if (!atmci_test_and_clear_pending(host, EVENT_NOTBUSY))
				break;
...
static void atmci_read_data_pio(struct atmel_mci *host)
{
	...
	do {
		...
		if (likely(offset + 4 <= sg->length)) {
			sg_pcopy_from_buffer(sg, 1, &value, sizeof(u32),
					     offset);
...
static irqreturn_t atmci_interrupt(int irq, void *dev_id)
{
	...
	do {
		...
		if (pending & ATMCI_DATA_ERROR_FLAGS) {
			...
			atmci_writel(host, ATMCI_IDR,
				     ATMCI_DATA_ERROR_FLAGS | ATMCI_RXRDY |
					     ATMCI_TXRDY | ATMCI_ENDRX |
					     ATMCI_ENDTX | ATMCI_RXBUFF |
					     ATMCI_TXBUFE);
		...
		if (pending & ATMCI_TXBUFE) {
			...
			if (host->data_size) {
				atmci_pdc_set_single_buf(host, XFER_TRANSMIT,
							 PDC_SECOND_BUF);
				atmci_writel(host, ATMCI_IER, ATMCI_ENDTX);
			}
		}
		...
		if (pending & ATMCI_RXBUFF) {
			...
			if (host->data_size) {
				atmci_pdc_set_single_buf(host, XFER_RECEIVE,
							 PDC_SECOND_BUF);
				atmci_writel(host, ATMCI_IER, ATMCI_ENDRX);
			}
		}
...
static void atmci_cleanup_slot(struct atmel_mci_slot *slot, unsigned int id)
{
...
{
	host->dma.chan =
		dma_request_slave_channel_reason(&host->pdev->dev, "rxtx");
...
static void atmci_get_cap(struct atmel_mci *host)
{
	...
	dev_info(&host->pdev->dev, "version: 0x%x\n", version);
...
static int atmci_probe(struct platform_device *pdev)
{
	...
	if (pdata->slot[0].bus_width) {
		ret = atmci_init_slot(host, &pdata->slot[0], 0,
				      ATMCI_SDCSEL_SLOT_A, ATMCI_SDIOIRQA);
		if (!ret) {
			nr_slots++;
	...
	if (pdata->slot[1].bus_width) {
		ret = atmci_init_slot(host, &pdata->slot[1], 1,
				      ATMCI_SDCSEL_SLOT_B, ATMCI_SDIOIRQB);
		if (!ret) {
			nr_slots++;
	...
	if (!host->caps.has_rwproof) {
		host->buffer =
			dma_alloc_coherent(&pdev->dev, host->buf_size,
...
static int atmci_remove(struct platform_device *pdev)
{
	...
	if (host->buffer)
		dma_free_coherent(&pdev->dev, host->buf_size, host->buffer,
				  host->buf_phys_addr);
...
static const struct dev_pm_ops atmci_dev_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
		...
		SET_RUNTIME_PM_OPS(atmci_runtime_suspend, atmci_runtime_resume,
				   NULL)
};
