...
#define OMAP_MMC_READ(host, reg)                                               \
	__raw_readw((host)->virt_base + OMAP_MMC_REG(host, reg))
#define OMAP_MMC_WRITE(host, reg, val)                                         \
...
static void mmc_omap_fclk_enable(struct mmc_omap_host *host,
				 unsigned int enable)
{
	unsigned long flags;
...
{
	struct mmc_omap_host *host =
		container_of(work, struct mmc_omap_host, slot_release_work);
...
static inline int mmc_omap_cover_is_open(struct mmc_omap_slot *slot)
{
...
static ssize_t mmc_omap_show_cover_switch(struct device *dev,
...
static ssize_t mmc_omap_show_slot_name(struct device *dev,
...
static void mmc_omap_start_command(struct mmc_omap_host *host,
				   struct mmc_command *cmd)
{
	u32 cmdreg;
	u32 resptype;
...
static void mmc_omap_release_dma(struct mmc_omap_host *host,
				 struct mmc_data *data, int abort)
{
...
{
	struct mmc_omap_host *host =
		container_of(work, struct mmc_omap_host, send_stop_work);
...
static void mmc_omap_xfer_done(struct mmc_omap_host *host,
			       struct mmc_data *data)
{
	if (host->dma_in_use)
...
static void mmc_omap_send_abort(struct mmc_omap_host *host, int maxloops)
{
	struct mmc_omap_slot *slot = host->current_slot;
...
static void mmc_omap_abort_xfer(struct mmc_omap_host *host,
				struct mmc_data *data)
{
	if (host->dma_in_use)
...
static void mmc_omap_end_of_data(struct mmc_omap_host *host,
				 struct mmc_data *data)
{
	unsigned long flags;
	int done;
			      ...
			      struct mmc_command *cmd)
{
	...
	if (cmd->flags & MMC_RSP_PRESENT) {
		if (cmd->flags & MMC_RSP_136) {
			/* response type 2 */
			cmd->resp[3] = OMAP_MMC_READ(host, RSP0) |
				       (OMAP_MMC_READ(host, RSP1) << 16);
			cmd->resp[2] = OMAP_MMC_READ(host, RSP2) |
				       (OMAP_MMC_READ(host, RSP3) << 16);
			cmd->resp[1] = OMAP_MMC_READ(host, RSP4) |
				       (OMAP_MMC_READ(host, RSP5) << 16);
			cmd->resp[0] = OMAP_MMC_READ(host, RSP6) |
...
{
	struct mmc_omap_host *host =
		container_of(work, struct mmc_omap_host, cmd_abort_work);
	BUG_ON(!host->cmd);
...
static void mmc_omap_cmd_timer(struct timer_list *t)
{
...
static void mmc_omap_clk_timer(struct timer_list *t)
{
...
static irqreturn_t mmc_omap_irq(int irq, void *dev_id)
{
	...
	if (host->cmd == NULL && host->data == NULL) {
		...
		dev_info(mmc_dev(host->slots[0]->mmc), "Spurious IRQ 0x%04x\n",
			 status);
		if (status != 0) {
	...
	while ((status = OMAP_MMC_READ(host, STAT)) != 0) {
		...
		if (host->cmd != NULL)
			...
			cmd = -1;
		dev_dbg(mmc_dev(host->mmc), "MMC IRQ %04x (CMD %d): ", status,
			cmd);
