...
#define MASK_RSP                                                               \
	(RSP_TIMEOUT | RSP_CRC_FAIL | RSP_CRC_OK | CARD_DETECT | CMD_SENT)

#define MASK_DATA (DATA_CRC_OK | DATA_END | DATA_CRC_FAIL | DATA_TIMEOUT)
...
static int moxart_wait_for_status(struct moxart_host *host, u32 mask,
				  u32 *status)
{
	int ret = -ETIMEDOUT;
	u32 i;
				...
				struct mmc_command *cmd)
{
	...
	writel(RSP_TIMEOUT | RSP_CRC_OK | RSP_CRC_FAIL | CMD_SENT,
	       host->base + REG_CLEAR);
