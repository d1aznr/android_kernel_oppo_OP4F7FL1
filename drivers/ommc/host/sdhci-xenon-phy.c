...
#define XENON_EMMC_5_0_PHY_FUNC_CONTROL (XENON_EMMC_5_0_PHY_REG_BASE + 0x4)
...
#define XENON_EMMC_5_0_PHY_PAD_CONTROL (XENON_EMMC_5_0_PHY_REG_BASE + 0x8)
...
#define XENON_EMMC_5_0_PHY_PAD_CONTROL2 (XENON_EMMC_5_0_PHY_REG_BASE + 0xC)
...
#define XENON_EMMC_5_0_PHY_DLL_CONTROL (XENON_EMMC_5_0_PHY_REG_BASE + 0x10)
...
static const char *const phy_types[] = { "emmc 5.0 phy", "emmc 5.1 phy" };
 ...
 */
static void xenon_emmc_phy_set(struct sdhci_host *host, unsigned char timing)
{
	u32 reg;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct xenon_priv *priv = sdhci_pltfm_priv(pltfm_host);
...
static int get_dt_pad_ctrl_data(struct sdhci_host *host, struct device_node *np,
				struct xenon_emmc_phy_params *params)
{
	int ret = 0;
	const char *name;
	...
	params->pad_ctrl.reg =
		devm_ioremap_resource(mmc_dev(host->mmc), &iomem);
...
void xenon_soc_pad_ctrl(struct sdhci_host *host, unsigned char signal_voltage)
{
