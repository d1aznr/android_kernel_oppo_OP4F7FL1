...
static void mxcmci_set_power(struct mxcmci_host *host, unsigned int vdd)
{
	if (!IS_ERR(host->mmc->supply.vmmc)) {
		if (host->power_mode == MMC_POWER_UP)
			mmc_regulator_set_ocr(host->mmc, host->mmc->supply.vmmc,
					      vdd);
...
static int mxcmci_setup_data(struct mxcmci_host *host, struct mmc_data *data)
{
	...
	nents = dma_map_sg(host->dma->device->dev, data->sg, data->sg_len,
			   host->dma_dir);
	...
	mod_timer(&host->watchdog,
		  jiffies + msecs_to_jiffies(MXCMCI_TIMEOUT_MS));
...
static int mxcmci_finish_data(struct mxcmci_host *host, unsigned int stat)
{
	...
	if (stat & STATUS_ERR_MASK) {
		...
		if (stat & STATUS_CRC_READ_ERR) {
			...
			dev_err(mmc_dev(host->mmc), "%s: read -ETIMEDOUT\n",
				__func__);
			data->error = -ETIMEDOUT;
		} else {
...
static int mxcmci_pull(struct mxcmci_host *host, void *_buf, int bytes)
{
	...
	while (bytes > 3) {
		stat = mxcmci_poll_status(host, STATUS_BUF_READ_RDY |
							STATUS_READ_OP_DONE);
		if (stat)
			return stat;
...
static void mxcmci_datawork(struct work_struct *work)
{
	struct mxcmci_host *host =
		container_of(work, struct mxcmci_host, datawork);
...
static int mxcmci_probe(struct platform_device *pdev)
{
	...
	if (host->dma)
		mmc->max_seg_size =
			dma_get_max_seg_size(host->dma->device->dev);
	else
...
static struct platform_driver mxcmci_driver = { .probe = mxcmci_probe,
