...
static inline void sd_scc_write32(struct tmio_mmc_host *host,
				  struct renesas_sdhi *priv, int addr, u32 val)
{
...
static void renesas_sdhi_hs400_complete(struct tmio_mmc_host *host)
{
	...
	/* Set HS400 mode */
	sd_ctrl_write16(host, CTL_SDIF_MODE,
			0x0001 | sd_ctrl_read16(host, CTL_SDIF_MODE));
				       ...
				       unsigned int direction, int blk_size)
{
	 ...
	 */
	if ((direction == MMC_DATA_READ) && blk_size == 2)
		return 1;
		       ...
		       const struct tmio_mmc_dma_ops *dma_ops)
{
	...
	if (!IS_ERR(priv->pinctrl)) {
		priv->pins_default = pinctrl_lookup_state(
			priv->pinctrl, PINCTRL_STATE_DEFAULT);
		priv->pins_uhs =
			pinctrl_lookup_state(priv->pinctrl, "state_uhs");
	}
	...
	dev_info(&pdev->dev, "%s base at 0x%08lx max clock rate %u MHz\n",
		 ...
		 (unsigned long)(platform_get_resource(pdev, IORESOURCE_MEM, 0)
					 ->start),
