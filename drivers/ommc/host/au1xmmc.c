...
#ifdef DEBUG
#define DBG(fmt, idx, args...) pr_debug("au1xmmc(%d): DEBUG: " fmt, idx, ##args)
#else
...
#define AU1XMMC_INTERRUPTS                                                     \
	(SD_CONFIG_SC | SD_CONFIG_DT | SD_CONFIG_RAT | SD_CONFIG_CR |          \
	 SD_CONFIG_I)
...
static void au1xmmc_receive_pio(struct au1xmmc_host *host)
{
	...
	if (host->pio.index < host->dma.len) {
		...
		/* This is the space left inside the buffer */
		sg_len = sg_dma_len(&data->sg[host->pio.index]) -
			 host->pio.offset;
				...
				struct mmc_data *data)
{
	...
	host->dma.len = dma_map_sg(mmc_dev(host->mmc), data->sg, data->sg_len,
				   host->dma.dir);
		...
		for (i = 0; i < host->dma.len; i++) {
			...
			if (host->flags & HOST_F_XMIT) {
				ret = au1xxx_dbdma_put_source(
					channel, sg_phys(sg), len, flags);
			} else {
				ret = au1xxx_dbdma_put_dest(
					channel, sg_phys(sg), len, flags);
			}
...
static int au1xmmc_dbdma_init(struct au1xmmc_host *host)
{
	...
	host->tx_chan = au1xxx_dbdma_chan_alloc(
		memid, txid, au1xmmc_dbdma_callback, (void *)host);
	if (!host->tx_chan) {
		dev_err(&host->pdev->dev, "cannot allocate TX DMA\n");
		return -ENODEV;
	}

	host->rx_chan = au1xxx_dbdma_chan_alloc(
		rxid, memid, au1xmmc_dbdma_callback, (void *)host);
	if (!host->rx_chan) {
...
static int au1xmmc_probe(struct platform_device *pdev)
{
	...
	host->ioarea =
		request_mem_region(r->start, resource_size(r), pdev->name);
	if (!host->ioarea) {
