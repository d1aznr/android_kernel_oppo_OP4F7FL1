...
static inline void sd_clear_error(struct rtsx_usb_sdmmc *host)
{
	...
	rtsx_usb_ep0_write_register(ucr, CARD_STOP, SD_STOP | SD_CLR_ERR,
				    SD_STOP | SD_CLR_ERR);
			...
			u16 byte_cnt, u8 *buf, int buf_len, int timeout)
{
	...
	if (cmd != NULL) {
		...
		rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_CMD0, 0xFF,
				 (u8)(cmd->opcode) | 0x40);
		rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_CMD1, 0xFF,
				 (u8)(cmd->arg >> 24));
		rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_CMD2, 0xFF,
				 (u8)(cmd->arg >> 16));
		rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_CMD3, 0xFF,
				 (u8)(cmd->arg >> 8));
		rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_CMD4, 0xFF,
				 (u8)cmd->arg);
	} else {
	...
	if (trans_mode != SD_TM_AUTO_TUNING)
		rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, CARD_DATA_SOURCE, 0x01,
				 PINGPONG_BUFFER);

	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_TRANSFER, 0xFF,
			 trans_mode | SD_TRANSFER_START);
	rtsx_usb_add_cmd(ucr, CHECK_REG_CMD, SD_TRANSFER, SD_TRANSFER_END,
			 SD_TRANSFER_END);
			 ...
			 u16 byte_cnt, u8 *buf, int buf_len, int timeout)
{
	...
	if (err) {
		sd_print_debug_regs(host);
		dev_dbg(sdmmc_dev(host), "rtsx_usb_get_rsp failed (err = %d)\n",
			err);
		return err;
	}
				...
				struct mmc_command *cmd)
{
	...
	if (cmd->opcode == SD_SWITCH_VOLTAGE) {
		err = rtsx_usb_write_register(
			ucr, SD_BUS_STAT, SD_CLK_TOGGLE_EN | SD_CLK_FORCE_STOP,
			SD_CLK_TOGGLE_EN);
		if (err)
			goto out;
	}
	...
	if (err) {
		dev_dbg(sdmmc_dev(host), "rtsx_usb_send_cmd error (err = %d)\n",
			err);
		goto out;
	}
...
static int sd_rw_multi(struct rtsx_usb_sdmmc *host, struct mmc_request *mrq)
{
	...
	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_BLOCK_CNT_L, 0xFF,
			 (u8)data->blocks);
	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_BLOCK_CNT_H, 0xFF,
			 (u8)(data->blocks >> 8));

	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, CARD_DATA_SOURCE, 0x01,
			 RING_BUFFER);

	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, MC_DMA_TC3, 0xFF,
			 (u8)(data_len >> 24));
	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, MC_DMA_TC2, 0xFF,
			 (u8)(data_len >> 16));
	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, MC_DMA_TC1, 0xFF,
			 (u8)(data_len >> 8));
	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, MC_DMA_TC0, 0xFF, (u8)data_len);
	if (read) {
		flag = MODE_CDIR;
...
{
	rtsx_usb_write_register(host->ucr, SD_CFG1, SD_CLK_DIVIDE_MASK,
				SD_CLK_DIVIDE_128);
}
...
static void sd_normal_rw(struct rtsx_usb_sdmmc *host, struct mmc_request *mrq)
{
...
static int sd_change_phase(struct rtsx_usb_sdmmc *host, u8 sample_point, int tx)
{
	...
	if (tx)
		rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_VPCLK0_CTL, 0x0F,
				 sample_point);
	else
		rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_VPCLK1_CTL, 0x0F,
				 sample_point);

	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_VPCLK0_CTL, PHASE_NOT_RESET, 0);
	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_VPCLK0_CTL, PHASE_NOT_RESET,
			 PHASE_NOT_RESET);
...
static void sd_wait_data_idle(struct rtsx_usb_sdmmc *host)
{
	...
	for (i = 0; i < 100; i++) {
		err = rtsx_usb_ep0_read_register(host->ucr, SD_DATA_STATE,
						 &val);
		if (val & SD_DATA_IDLE)
			return;
...
static int sd_tuning_rx_cmd(struct rtsx_usb_sdmmc *host, u8 opcode,
			    u8 sample_point)
{
	int err;
	struct mmc_command cmd = {};
...
static void sd_tuning_phase(struct rtsx_usb_sdmmc *host, u8 opcode,
			    u16 *phase_map)
{
	int err, i;
	u16 raw_phase_map = 0;
			    ...
			    unsigned char bus_width)
{
	...
	if (bus_width <= MMC_BUS_WIDTH_8)
		err = rtsx_usb_write_register(host->ucr, SD_CFG1, 0x03,
					      width[bus_width]);
...
static int sd_power_on(struct rtsx_usb_sdmmc *host)
{
	...
	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, CARD_SHARE_MODE, CARD_SHARE_MASK,
			 CARD_SHARE_SD);
	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, CARD_CLK_EN, SD_CLK_EN, SD_CLK_EN);
	err = rtsx_usb_send_cmd(ucr, MODE_C, 100);
	if (err)
		return err;
	...
	err = rtsx_usb_write_register(ucr, CARD_PWR_CTL, POWER_MASK,
				      PARTIAL_POWER_ON);
	if (err)
		return err;
	...
	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, CARD_OE, SD_OUTPUT_EN,
			 SD_OUTPUT_EN);
...
static int sd_power_off(struct rtsx_usb_sdmmc *host)
{
	...
	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, CARD_PWR_CTL, POWER_MASK,
			 POWER_OFF);
	rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, CARD_PWR_CTL,
			 POWER_MASK | LDO3318_PWR_MASK,
			 POWER_OFF | LDO_SUSPEND);
...
static int sd_set_timing(struct rtsx_usb_sdmmc *host, unsigned char timing,
			 bool *ddr_mode)
{
	struct rtsx_ucr *ucr = host->ucr;
	int err;
	...
	*ddr_mode = false;
	...
	default:
		...
		rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, CARD_CLK_SOURCE, 0xFF,
				 CRC_FIX_CLK | SD30_VAR_CLK0 | SAMPLE_VAR_CLK1);
		rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_PUSH_POINT_CTL, 0xFF,
				 0);
		rtsx_usb_add_cmd(ucr, WRITE_REG_CMD, SD_SAMPLE_POINT_CTL,
				 ...
				 enum led_brightness brightness)
{
	struct rtsx_usb_sdmmc *host =
		container_of(led, struct rtsx_usb_sdmmc, led);
