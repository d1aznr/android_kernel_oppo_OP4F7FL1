...
#define INT_EN_MASK                                                            \
	(BADA_EN | CERR_EN | ACE_EN | DEB_EN | DCRC_EN | DTO_EN | CIE_EN |     \
...
static int omap_hsmmc_reg_get(struct omap_hsmmc_host *host)
{
	...
	if (IS_ERR(mmc->supply.vqmmc)) {
		mmc->supply.vqmmc =
			devm_regulator_get_optional(host->dev, "vmmc_aux");
		if (IS_ERR(mmc->supply.vqmmc)) {
...
static inline int omap_hsmmc_cover_is_closed(struct omap_hsmmc_host *host)
{
	int r = 1;
...
static ssize_t omap_hsmmc_show_cover_switch(struct device *dev,
				...
				struct mmc_command *cmd)
{
	if (host->mrq->sbc && (host->cmd == host->mrq->sbc) &&
		...
		omap_hsmmc_start_command(host, host->mrq->cmd, host->mrq->data);
		return;
	}
...
static void omap_hsmmc_dma_cleanup(struct omap_hsmmc_host *host, int errno)
{
	...
	if (host->use_dma && dma_ch != -1) {
		struct dma_chan *chan =
			omap_hsmmc_get_dma_chan(host, host->data);

		dmaengine_terminate_all(chan);
		dma_unmap_sg(chan->device->dev, host->data->sg,
			     host->data->sg_len, mmc_get_dma_dir(host->data));
						   ...
						   unsigned long bit)
{
	...
	while ((OMAP_HSMMC_READ(host->base, SYSCTL) & bit) && (i++ < limit))
		udelay(1);
	...
	if (status & ERR_EN) {
		...
		if (status & (CTO_EN | DTO_EN))
			hsmmc_command_incomplete(host, -ETIMEDOUT, end_cmd);
			 ...
			 (CCRC_EN | DCRC_EN | DEB_EN | CEB_EN | BADA_EN))
...
{
	if (mmc->debugfs_root)
		debugfs_create_file("regs", S_IRUSR, mmc->debugfs_root, mmc,
				    &mmc_regs_fops);
}
