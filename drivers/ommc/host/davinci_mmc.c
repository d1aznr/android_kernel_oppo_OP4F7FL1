					...
					struct mmc_data *data)
{
	...
	if (host->data_dir == DAVINCI_MMC_DATADIR_WRITE) {
		...
		desc = dmaengine_prep_slave_sg(
			host->dma_tx, data->sg, host->sg_len, DMA_MEM_TO_DEV,
		...
		desc = dmaengine_prep_slave_sg(
			host->dma_rx, data->sg, host->sg_len, DMA_DEV_TO_MEM,
			DMA_PREP_INTERRUPT | DMA_CTRL_ACK);
		if (!desc) {
			dev_dbg(mmc_dev(host->mmc),
				     ...
				     struct mmc_request *req)
{
	...
	dev_dbg(mmc_dev(host->mmc), "%s, %d blocks of %d bytes\n",
		(data->flags & MMC_DATA_WRITE) ? "write" : "read", data->blocks,
		data->blksz);
					    ...
					    unsigned int mmc_req_freq)
{
	...
	if (mmc_req_freq && mmc_pclk > (2 * mmc_req_freq))
		mmc_push_pull_divisor =
			((unsigned int)mmc_pclk / (2 * mmc_req_freq)) - 1;
	else
		mmc_push_pull_divisor = 0;

	mmc_freq = (unsigned int)mmc_pclk / (2 * (mmc_push_pull_divisor + 1));

	if (mmc_freq > mmc_req_freq)
		mmc_push_pull_divisor = mmc_push_pull_divisor + 1;
	/* Convert ns to clock cycles */
	if (mmc_req_freq <= 400000)
		host->ns_in_one_cycle =
			(1000000) /
			(((mmc_pclk / (2 * (mmc_push_pull_divisor + 1))) /
			  1000));
	else
		host->ns_in_one_cycle =
			(1000000) /
...
static void calculate_clk_divider(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->bus_mode == MMC_BUSMODE_OPENDRAIN) {
		 ...
		 */
		open_drain_freq =
			((unsigned int)mmc_pclk / (2 * MMCSD_INIT_CLOCK)) - 1;
...
static void mmc_davinci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	dev_dbg(mmc_dev(host->mmc),
		...
		ios->bus_mode, ios->power_mode, ios->vdd);
				  ...
				  struct mmc_data *data)
{
	...
	if (host->mmc->caps & MMC_CAP_SDIO_IRQ) {
		 ...
		 */
		if (host->sdio_int &&
		    !(readl(host->base + DAVINCI_SDIOST0) & SDIOST0_DAT1_HI)) {
...
static void davinci_abort_data(struct mmc_davinci_host *host,
			       struct mmc_data *data)
{
...
static irqreturn_t mmc_davinci_sdio_irq(int irq, void *dev_id)
{
	...
	if (status & SDIOIST_IOINT) {
		dev_dbg(mmc_dev(host->mmc), "SDIO interrupt status %x\n",
			status);
...
static irqreturn_t mmc_davinci_irq(int irq, void *dev_id)
{
	...
	if (host->cmd == NULL && host->data == NULL) {
		...
		dev_dbg(mmc_dev(host->mmc), "Spurious interrupt 0x%04x\n",
			status);
	...
	if (qstatus & MMCST0_TOUTRD) {
		...
		dev_dbg(mmc_dev(host->mmc), "read data timeout, status %x\n",
			qstatus);
