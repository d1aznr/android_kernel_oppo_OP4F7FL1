...
static struct esdhc_soc_data usdhc_imx6sl_data = {
	.flags = ESDHC_FLAG_USDHC | ESDHC_FLAG_STD_TUNING |
		 ESDHC_FLAG_HAVE_CAP1 | ESDHC_FLAG_ERR004536 | ESDHC_FLAG_HS200,
};
...
static const struct of_device_id imx_esdhc_dt_ids[] = {
	{
		...
		.data = &esdhc_imx25_data,
	},
	{
		.compatible = "fsl,imx35-esdhc",
		.data = &esdhc_imx35_data,
	},
	{
		.compatible = "fsl,imx51-esdhc",
		.data = &esdhc_imx51_data,
	},
	{
		.compatible = "fsl,imx53-esdhc",
		.data = &esdhc_imx53_data,
	},
	{
		.compatible = "fsl,imx6sx-usdhc",
		.data = &usdhc_imx6sx_data,
	},
	{
		.compatible = "fsl,imx6sl-usdhc",
		.data = &usdhc_imx6sl_data,
	},
	{
		.compatible = "fsl,imx6q-usdhc",
		.data = &usdhc_imx6q_data,
	},
	{
		.compatible = "fsl,imx7d-usdhc",
		.data = &usdhc_imx7d_data,
	},
	{ /* sentinel */ }
};
...
static void esdhc_writel_le(struct sdhci_host *host, u32 val, int reg)
{
	...
	if (unlikely((imx_data->socdata->flags & ESDHC_FLAG_MULTIBLK_NO_INT) &&
		     (reg == SDHCI_INT_STATUS) && (val & SDHCI_INT_DATA_END))) {
		u32 v;
		...
		if (imx_data->multiblock_status == MULTIBLK_IN_PROCESS) {
			...
			data = MMC_STOP_TRANSMISSION << 24 | SDHCI_CMD_ABORTCMD
								     << 16;
...
static void esdhc_writew_le(struct sdhci_host *host, u16 val, int reg)
{
	...
	case SDHCI_COMMAND:
		...
		if (esdhc_is_usdhc(imx_data))
			writel(val << 16, host->ioaddr + SDHCI_TRANSFER_MODE);
		else
...
static void esdhc_prepare_tuning(struct sdhci_host *host, u32 val)
{
	...
	dev_dbg(mmc_dev(host->mmc),
		...
		readl(host->ioaddr + ESDHC_TUNE_CTRL_STATUS));
}
...
static int esdhc_change_pinstate(struct sdhci_host *host, unsigned int uhs)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct pltfm_imx_data *imx_data = sdhci_pltfm_priv(pltfm_host);
	...
	if (IS_ERR(imx_data->pinctrl) || IS_ERR(imx_data->pins_default) ||
...
#ifdef CONFIG_OF
static int sdhci_esdhc_imx_probe_dt(struct platform_device *pdev,
				    ...
				    struct pltfm_imx_data *imx_data)
{
	...
	if (esdhc_is_usdhc(imx_data) && !IS_ERR(imx_data->pins_default)) {
		imx_data->pins_100mhz = pinctrl_lookup_state(
			imx_data->pinctrl, ESDHC_PINCTRL_STATE_100MHZ);
				       ...
				       struct pltfm_imx_data *imx_data)
{
	...
	imx_data->boarddata = *(
		(struct esdhc_platform_data *)host->mmc->parent->platform_data);
	/* write_protect */
...
static int sdhci_esdhc_imx_probe(struct platform_device *pdev)
{
	...
	imx_data->socdata =
		of_id ? of_id->data :
			(struct esdhc_soc_data *)pdev->id_entry->driver_data;
	...
	imx_data->pins_default =
		pinctrl_lookup_state(imx_data->pinctrl, PINCTRL_STATE_DEFAULT);
