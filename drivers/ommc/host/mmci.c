...
static struct variant_data variant_qcom = {
	...
	.fifohalfsize = 8 * 4,
	.clkreg = MCI_CLK_ENABLE,
	.clkreg_enable = MCI_QCOM_CLK_FLOWENA | MCI_QCOM_CLK_SELECT_IN_FBCLK,
 ...
 */
static int mmci_validate_data(struct mmci_host *host, struct mmc_data *data)
{
	if (!data)
		return 0;
				...
				struct dma_async_tx_descriptor **dma_desc)
{
	...
	desc = dmaengine_prep_slave_sg(chan, data->sg, nr_sg, conf.direction,
				       flags);
	if (!desc)
		goto unmap_exit;
			  ...
			  unsigned int status)
{
	...
	status_err =
		status & (host->variant->start_err | MCI_DATACRCFAIL |
			  ...
			  unsigned int remain, u32 status)
{
	...
	do {
		...
		maxcnt = status & MCI_TXFIFOEMPTY ? variant->fifosize :
						    variant->fifohalfsize;
...
static void mmci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (variant->signal_direction && ios->power_mode != MMC_POWER_OFF) {
		...
		if (ios->bus_width == MMC_BUS_WIDTH_4)
			...
			pwr &= (~MCI_ST_DATA74DIREN & ~MCI_ST_DATA31DIREN &
				~MCI_ST_DATA2DIREN);
	}
...
static int mmci_sig_volt_switch(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (!IS_ERR(mmc->supply.vqmmc)) {
		...
		case MMC_SIGNAL_VOLTAGE_180:
			ret = regulator_set_voltage(mmc->supply.vqmmc, 1700000,
						    1950000);
			break;
...
static int mmci_probe(struct amba_device *dev, const struct amba_id *id)
{
	...
	if (!variant->opendrain) {
		...
		host->pins_opendrain = pinctrl_lookup_state(
			host->pinctrl, MMCI_PINCTRL_STATE_OPENDRAIN);
	...
	dev_info(&dev->dev,
		 ...
		 amba_rev(dev), (unsigned long long)dev->res.start, dev->irq[0],
		 dev->irq[1]);
...
static const struct dev_pm_ops mmci_dev_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
		...
		SET_RUNTIME_PM_OPS(mmci_runtime_suspend, mmci_runtime_resume,
				   NULL)
};
