...
static int mvsd_setup_data(struct mvsd_host *host, struct mmc_data *data)
{
	...
	dev_dbg(host->dev,
		...
		(u32)sg_virt(data->sg), data->blocks, data->blksz, tmout,
		tmout_index);
...
static irqreturn_t mvsd_irq(int irq, void *dev)
{
	...
	if (host->pio_size && (intr_status & host->intr_en &
		...
		dev_dbg(host->dev, "pio %d intr 0x%04x hw_state 0x%04x\n", s,
			intr_status, mvsd_read(MVSD_HW_STATE));
		host->pio_ptr = p;
		host->pio_size = s;
		irq_handled = 1;
		...
		if (s < 4) {
			...
			if (s == 0) {
				host->intr_en &= ~(MVSD_NOR_TX_AVAIL |
						   MVSD_NOR_TX_FIFO_8W);
				mvsd_write(MVSD_NOR_INTR_EN, host->intr_en);
			}
		}
...
static void mvsd_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	dev_dbg(host->dev, "ctrl 0x%04x: %s %s %s\n", ctrl_reg,
		(ctrl_reg & MVSD_HOST_CTRL_PUSH_PULL_EN) ? "push-pull" :
							   "open-drain",
		(ctrl_reg & MVSD_HOST_CTRL_DATA_WIDTH_4_BITS) ? "4bit-width" :
								"1bit-width",
		(ctrl_reg & MVSD_HOST_CTRL_HI_SPEED_EN) ? "high-speed" : "");
...
static void mv_conf_mbus_windows(struct mvsd_host *host,
