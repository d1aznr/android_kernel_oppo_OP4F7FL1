...
#define USDHI6_SD_INFO1_CARD_INSERT                                            \
	(USDHI6_SD_INFO1_CARD_IN | USDHI6_SD_INFO1_D3_CARD_IN)
#define USDHI6_SD_INFO1_CARD_EJECT                                             \
	(USDHI6_SD_INFO1_CARD_OUT | USDHI6_SD_INFO1_D3_CARD_OUT)
#define USDHI6_SD_INFO1_CARD                                                   \
	(USDHI6_SD_INFO1_CARD_INSERT | USDHI6_SD_INFO1_CARD_EJECT)
#define USDHI6_SD_INFO1_CARD_CD                                                \
	(USDHI6_SD_INFO1_CARD_IN | USDHI6_SD_INFO1_CARD_OUT)

#define USDHI6_SD_INFO2_ERR                                                    \
	(USDHI6_SD_INFO2_CMD_ERR | USDHI6_SD_INFO2_CRC_ERR |                   \
	 USDHI6_SD_INFO2_END_ERR | USDHI6_SD_INFO2_TOUT |                      \
	 USDHI6_SD_INFO2_IWA_ERR | USDHI6_SD_INFO2_IRA_ERR |                   \
	 USDHI6_SD_INFO2_RSP_TOUT | USDHI6_SD_INFO2_ILA)

#define USDHI6_SD_INFO1_IRQ                                                    \
	(USDHI6_SD_INFO1_RSP_END | USDHI6_SD_INFO1_ACCESS_END |                \
	 USDHI6_SD_INFO1_CARD)

#define USDHI6_SD_INFO2_IRQ                                                    \
	(USDHI6_SD_INFO2_ERR | USDHI6_SD_INFO2_BRE | USDHI6_SD_INFO2_BWE |     \
	 0x0800 | USDHI6_SD_INFO2_ILA)
...
#define USDHI6_SDIO_INFO1_IRQ                                                  \
	(USDHI6_SDIO_INFO1_IOIRQ | 3 | USDHI6_SDIO_INFO1_EXPUB52 |             \
...
static void usdhi6_wait_for_brwe(struct usdhi6_host *host, bool read)
{
	usdhi6_irq_enable(host,
			  ...
			  USDHI6_SD_INFO2_ERR | (read ? USDHI6_SD_INFO2_BRE :
							USDHI6_SD_INFO2_BWE));
}
...
static int usdhi6_error_code(struct usdhi6_host *host)
{
	...
	if (err != USDHI6_SD_ERR_STS1_CRC_NO_ERROR)
		dev_warn(mmc_dev(host->mmc), "Err sts 0x%x, state %u, CMD%d\n",
			 err, host->wait,
			 host->mrq ? host->mrq->cmd->opcode : -1);
 ...
 */
static void usdhi6_blk_bounce(struct usdhi6_host *host, struct scatterlist *sg)
{
	struct mmc_data *data = host->mrq->data;
	size_t blk_head = host->head_len;

	dev_dbg(mmc_dev(host->mmc), "%s(): CMD%u of %u SG: %ux%u @ 0x%x\n",
		__func__, host->mrq->cmd->opcode, data->sg_len, data->blksz,
		data->blocks, sg->offset);
...
static void *usdhi6_sg_map(struct usdhi6_host *host)
{
	...
	dev_dbg(mmc_dev(host->mmc),
		...
		page_to_pfn(host->pg.page), host->pg.mapped, sg->offset,
...
static void usdhi6_dma_stop_unmap(struct usdhi6_host *host)
{
	...
	if (data->flags & MMC_DATA_READ)
		dma_unmap_sg(host->chan_rx->device->dev, data->sg, data->sg_len,
			     DMA_FROM_DEVICE);
	else
		dma_unmap_sg(host->chan_tx->device->dev, data->sg, data->sg_len,
			     DMA_TO_DEVICE);
}
...
static void usdhi6_dma_kill(struct usdhi6_host *host)
{
	...
	dev_dbg(mmc_dev(host->mmc), "%s(): SG of %u: %ux%u\n", __func__,
		data->sg_len, data->blocks, data->blksz);
	/* Abort DMA */
	if (data->flags & MMC_DATA_READ)
...
static void usdhi6_clk_set(struct usdhi6_host *host, struct mmc_ios *ios)
{
	...
	for (i = 1000; i; i--) {
		if (usdhi6_read(host, USDHI6_SD_INFO2) &
	...
	val = usdhi6_read(host, USDHI6_SD_CLK_CTRL) &
	...
	if (rate) {
		...
		if (host->imclk <= rate) {
			...
			unsigned long div = roundup_pow_of_two(
				DIV_ROUND_UP(host->imclk, rate));
			val |= div >> 2;
...
static void usdhi6_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	dev_dbg(mmc_dev(mmc),
		...
		ios->clock, ios->vdd, ios->power_mode, ios->bus_width,
		ios->timing);
...
static void usdhi6_timeout_set(struct usdhi6_host *host)
{
	...
	usdhi6_write(host, USDHI6_SD_OPTION,
		     (val << USDHI6_SD_OPTION_TIMEOUT_SHIFT) |
			     (usdhi6_read(host, USDHI6_SD_OPTION) &
...
static void usdhi6_request_done(struct usdhi6_host *host)
{
	...
	if (mrq->cmd->error || (data && data->error) ||
		...
		dev_dbg(mmc_dev(host->mmc), "%s(CMD%d: %ux%u): err %d %d %d\n",
			...
			data ? data->blksz : 0, mrq->cmd->error,
			data ? data->error : 1,
...
static int usdhi6_rq_start(struct usdhi6_host *host)
{
	...
	if (data) {
		...
		if (cmd->opcode == MMC_READ_MULTIPLE_BLOCK ||
		    cmd->opcode == MMC_WRITE_MULTIPLE_BLOCK ||
		    (cmd->opcode == SD_IO_RW_EXTENDED && data->blocks > 1))
			usdhi6_sg_prep(host);

		usdhi6_write(host, USDHI6_SD_SIZE, data->blksz);

		if ((data->blksz >= USDHI6_MIN_DMA || data->blocks > 1) &&
		    (data->blksz % 4 || data->sg->offset % 4))
			dev_dbg(mmc_dev(host->mmc),
				"Bad SG of %u: %ux%u @ %u\n", data->sg_len,
				data->blksz, data->blocks, data->sg->offset);

		/* Enable DMA for USDHI6_MIN_DMA bytes or more */
		use_dma = data->blksz >= USDHI6_MIN_DMA && !(data->blksz % 4) &&
			  usdhi6_dma_start(host) >= DMA_MIN_COOKIE;

		if (use_dma)
			usdhi6_write(host, USDHI6_CC_EXT_MODE,
...
static void usdhi6_enable_sdio_irq(struct mmc_host *mmc, int enable)
{
	...
	if (enable) {
		host->sdio_mask =
			USDHI6_SDIO_INFO1_IRQ & ~USDHI6_SDIO_INFO1_IOIRQ;
		...
		usdhi6_write(host, USDHI6_SDIO_INFO1_MASK,
...
static bool usdhi6_end_cmd(struct usdhi6_host *host)
{
	...
	if (host->dma_active) {
		...
		if (cmd->opcode == MMC_READ_MULTIPLE_BLOCK ||
		    (cmd->opcode == SD_IO_RW_EXTENDED && mrq->data->blocks > 1))
...
static irqreturn_t usdhi6_sd(int irq, void *dev_id)
{
	...
	dev_dbg(mmc_dev(host->mmc), "IRQ status = 0x%08x, status2 = 0x%08x\n",
		status, status2);
...
static irqreturn_t usdhi6_cd(int irq, void *dev_id)
{
	...
	if (!work_pending(&mmc->detect.work) &&
	    (((status & USDHI6_SD_INFO1_CARD_INSERT) && !mmc->card) ||
	     ((status & USDHI6_SD_INFO1_CARD_EJECT) && mmc->card)))
...
static void usdhi6_timeout_work(struct work_struct *work)
{
	...
	struct usdhi6_host *host =
		container_of(d, struct usdhi6_host, timeout_work);
	struct mmc_request *mrq = host->mrq;
	...
	dev_warn(
		...
		host->dma_active ? "DMA" : "PIO", host->wait,
		mrq ? mrq->cmd->opcode : -1, usdhi6_read(host, USDHI6_SD_INFO1),
...
static int usdhi6_probe(struct platform_device *pdev)
{
	...
	if (!IS_ERR(host->pins_uhs)) {
		host->pins_default = pinctrl_lookup_state(
			host->pinctrl, PINCTRL_STATE_DEFAULT);
	...
	dev_info(dev, "A USDHI6ROL0 SD host detected with %d ports\n",
		 usdhi6_read(host, USDHI6_SD_PORT_SEL) >>
			 USDHI6_SD_PORT_SEL_PORTS_SHIFT);

	usdhi6_mask_all(host);

	if (irq_cd >= 0) {
		ret = devm_request_irq(dev, irq_cd, usdhi6_cd, 0, dev_name(dev),
				       host);
		if (ret < 0)
			goto e_clk_off;
	} else {
	...
	ret = devm_request_irq(dev, irq_sdio, usdhi6_sdio, 0, dev_name(dev),
			       host);
	if (ret < 0)
		goto e_clk_off;
	...
	mmc->ops = &usdhi6_ops;
	mmc->caps |=
		MMC_CAP_SD_HIGHSPEED | MMC_CAP_MMC_HIGHSPEED | MMC_CAP_SDIO_IRQ;
