...
static void meson_mx_mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_UP:
		if (!IS_ERR(mmc->supply.vmmc)) {
			host->error = mmc_regulator_set_ocr(
				mmc, mmc->supply.vmmc, vdd);
			if (host->error)
				return;
		}
		break;
...
static int meson_mx_mmc_add_host(struct meson_mx_mmc_host *host)
{
	...
	mmc->max_blk_count = FIELD_GET(
		MESON_MX_SDIO_SEND_REPEAT_PACKAGE_TIMES_MASK, 0xffffffff);
	mmc->max_blk_size =
		FIELD_GET(MESON_MX_SDIO_EXT_DATA_RW_NUMBER_MASK, 0xffffffff);
...
static int meson_mx_mmc_register_clks(struct meson_mx_mmc_host *host)
{
	...
	host->fixed_factor_clk =
		devm_clk_register(host->controller_dev, &host->fixed_factor.hw);
	if (WARN_ON(IS_ERR(host->fixed_factor_clk)))
		return PTR_ERR(host->fixed_factor_clk);

	clk_div_parent = __clk_get_name(host->fixed_factor_clk);
	init.name = devm_kasprintf(host->controller_dev, GFP_KERNEL, "%s#div",
				   dev_name(host->controller_dev));
	if (!init.name)
		return -ENOMEM;
	...
	host->cfg_div_clk =
		devm_clk_register(host->controller_dev, &host->cfg_div.hw);
