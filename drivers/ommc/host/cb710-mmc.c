...
#define CB710_MAX_DIVIDER_IDX (ARRAY_SIZE(cb710_clock_divider_log2) - 1)

static const u8 cb710_src_freq_mhz[16] = { 33, 10, 20, 25, 30, 35, 40, 45,
...
static void cb710_mmc_select_clock_divider(struct mmc_host *mmc, int hz)
{
	...
	for (divider_idx = 0; divider_idx < CB710_MAX_DIVIDER_IDX;
	     ++divider_idx) {
	...
	dev_dbg(cb710_slot_dev(slot),
		...
		src_hz >> cb710_clock_divider_log2[divider_idx & 7], hz,
		src_freq_idx, divider_idx & 7, divider_idx & 8);
}
				   ...
				   unsigned short enable, unsigned short mask)
{
	...
	enable = (cb710_read_port_16(slot, CB710_MMC_IRQ_ENABLE_PORT) & ~mask) |
		 enable;
...
static void cb710_mmc_enable_irq(struct cb710_slot *slot, unsigned short enable,
				 unsigned short mask)
{
...
static void cb710_mmc_enable_4bit_data(struct cb710_slot *slot, int enable)
{
	if (enable)
		...
		cb710_modify_port_8(slot, CB710_MMC_CONFIG1_PORT, 0,
...
static void cb710_mmc_set_transfer_size(struct cb710_slot *slot, size_t count,
					size_t blocksize)
{
				 ...
				 struct sg_mapping_iter *miter, size_t dw_count)
{
	...
	cb710_sg_dwiter_write_from_io(miter, slot->iobase + CB710_MMC_DATA_PORT,
				      dw_count);
...
static int cb710_mmc_receive(struct cb710_slot *slot, struct mmc_data *data)
{
	...
	if (unlikely(data->blksz & 15 &&
		     (data->blocks != 1 || data->blksz != 8)))
		return -EINVAL;

	sg_miter_start(&miter, data->sg, data->sg_len, SG_MITER_TO_SG);

	cb710_modify_port_8(slot, CB710_MMC_CONFIG2_PORT, 15,
	...
	while (blocks-- > 0) {
		...
		cb710_modify_port_8(slot, CB710_MMC_CONFIG2_PORT, len - 1,
				    CB710_MMC_C2_READ_PIO_SIZE_MASK);
...
static int cb710_mmc_send(struct cb710_slot *slot, struct mmc_data *data)
{
	...
	cb710_modify_port_8(slot, CB710_MMC_CONFIG2_PORT, 0,
			    CB710_MMC_C2_READ_PIO_SIZE_MASK);
	...
	while (blocks-- > 0) {
		...
		do {
			if (!(cb710_read_port_8(slot, CB710_MMC_STATUS2_PORT) &
				...
				err = cb710_wait_for_event(
					slot, CB710_MMC_S1_PIO_TRANSFER_DONE);
				if (err)
					goto out;
			}
			cb710_sg_dwiter_read_to_io(
				&miter, slot->iobase + CB710_MMC_DATA_PORT, 4);
		} while (--len);
	}
out:
	sg_miter_stop(&miter);
				   ...
				   struct mmc_command *cmd)
{
	...
	if (cmd->flags & MMC_RSP_136) {
		...
		rsp_opcode =
			cb710_read_port_32(slot, CB710_MMC_RESPONSE1_PORT) &
			0x3F;
		cmd->resp[0] =
			cb710_read_port_32(slot, CB710_MMC_RESPONSE0_PORT);
	}
...
static int cb710_mmc_command(struct mmc_host *mmc, struct mmc_command *cmd)
{
	...
	cb710_wait_while_busy(slot,
			      CB710_MMC_S2_BUSY_20 | CB710_MMC_S2_BUSY_10);
