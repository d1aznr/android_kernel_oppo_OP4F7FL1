...
#define SDHCI_HOST_IRQ_STATUS (1 << 17) /* host->irq status */
	...
	wait_queue_head_t
		buf_ready_int; /* Waitqueue for Buffer Read Ready interrupt */
...
struct sdhci_ops {
	...
	void (*set_timeout)(struct sdhci_host *host, struct mmc_command *cmd);
...
#define REQ_IO_HIGH (1 << 3)
	...
	int (*config_auto_tuning_cmd)(struct sdhci_host *host, bool enable,
				      u32 type);
