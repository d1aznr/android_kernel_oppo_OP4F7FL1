...
static bool ice_cap_idx_valid(struct cqhci_host *host, unsigned int cap_idx)
{
...
static uint8_t get_data_unit_size_mask(unsigned int data_unit_size)
{
	if (data_unit_size < MINIMUM_DUN_SIZE ||
	    data_unit_size > MAXIMUM_DUN_SIZE || !is_power_of_2(data_unit_size))
		return 0;
					    ...
					    unsigned int slot)
{
	...
	err = crypto_qti_keyslot_program(host->crypto_vops->priv, key, slot,
					  ...
					  unsigned int slot)
{
	...
	if (!cqhci_is_crypto_enabled(host) || !cqhci_keyslot_valid(host, slot))
		return -EINVAL;
				    ...
				    const struct keyslot_mgmt_ll_ops *ksm_ops)
{
	...
	for (cap_idx = 0; cap_idx < host->crypto_capabilities.num_crypto_cap;
	     cap_idx++) {
		host->crypto_cap_array[cap_idx].reg_val = cpu_to_le32(
			cqhci_readl(host, CQHCI_CRYPTOCAP +
	...
	host->ksm =
		keyslot_manager_create(host->mmc->parent,
				 ...
				 const struct keyslot_mgmt_ll_ops *ksm_ops)
{
	...
	cqhci_ice_memres = platform_get_resource_byname(
		msm_host->pdev, IORESOURCE_MEM, "cqhci_ice");
	if (!cqhci_ice_memres) {
	...
	host->icemmio =
		devm_ioremap(&msm_host->pdev->dev, cqhci_ice_memres->start,
	...
	err = crypto_qti_init_crypto(&msm_host->pdev->dev, host->icemmio,
