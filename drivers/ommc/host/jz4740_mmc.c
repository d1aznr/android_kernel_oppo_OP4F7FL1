...
jz4740_mmc_get_dma_chan(struct jz4740_mmc_host *host, struct mmc_data *data)
{
				       ...
				       struct dma_chan *chan)
{
	...
	if (next || data->host_cookie != host->next_data.cookie) {
		sg_len = dma_map_sg(chan->device->dev, data->sg, data->sg_len,
				    dir);
					 ...
					 struct mmc_data *data)
{
	...
	desc = dmaengine_prep_slave_sg(chan, data->sg, host->sg_len,
				       conf.direction,
...
static irqreturn_t jz_mmc_irq_worker(int irq, void *devid)
{
	...
	case JZ4740_MMC_STATE_SEND_STOP:
		...
		if (mmc_resp_type(req->stop) & MMC_RSP_BUSY) {
			timeout =
				jz4740_mmc_poll_irq(host, JZ_MMC_IRQ_PRG_DONE);
			if (timeout) {
				host->state = JZ4740_MMC_STATE_DONE;
				break;
			}
