...
static ssize_t goldfish_mmc_show_cover_switch(struct device *dev,
					      ...
					      char *buf)
{
	...
	return sprintf(buf, "%s\n",
		       goldfish_mmc_cover_is_open(host) ? "open" : "closed");
}

static DEVICE_ATTR(cover_switch, S_IRUGO, goldfish_mmc_show_cover_switch, NULL);

static void goldfish_mmc_start_command(struct goldfish_mmc_host *host,
				       struct mmc_command *cmd)
{
	u32 cmdreg;
	u32 resptype;
				      ...
				      struct mmc_request *req)
{
	...
	host->sg_len =
		dma_map_sg(mmc_dev(host->mmc), data->sg, sg_len, dma_data_dir);
	host->dma_done = 0;
	host->dma_in_use = 1;
...
static int goldfish_mmc_probe(struct platform_device *pdev)
{
	...
	host->virt_base = dma_alloc_coherent(&pdev->dev, BUFFER_SIZE, &buf_addr,
					     GFP_KERNEL);
