...
static int mmc_spi_readbytes(struct mmc_spi_host *host, unsigned len)
{
	int status;
	...
	if (host->dma_dev)
		dma_sync_single_for_device(host->dma_dev, host->data_dma,
					   sizeof(*host->data),
					   DMA_FROM_DEVICE);
	...
	if (host->dma_dev)
		dma_sync_single_for_cpu(host->dma_dev, host->data_dma,
					sizeof(*host->data), DMA_FROM_DEVICE);
...
static inline int mmc_spi_wait_unbusy(struct mmc_spi_host *host,
				...
				struct mmc_command *cmd, int cs_on)
{
	...
	snprintf(tag, sizeof(tag), "  ... CMD%d response SPI_%s", cmd->opcode,
		 maptype(cmd));
 ...
 */
static int mmc_spi_command_send(struct mmc_spi_host *host,
 ...
 */
static void mmc_spi_setup_data_message(struct mmc_spi_host *host, int multiple,
 ...
 */
static int mmc_spi_writeblock(struct mmc_spi_host *host, struct spi_transfer *t,
			      unsigned long timeout)
{
	...
	if (host->dma_dev)
		dma_sync_single_for_device(host->dma_dev, host->data_dma,
					   sizeof(*scratch), DMA_BIDIRECTIONAL);
 ...
 */
static int mmc_spi_readblock(struct mmc_spi_host *host, struct spi_transfer *t,
			     unsigned long timeout)
{
	struct spi_device *spi = host->spi;
	int status;
	...
	if (host->dma_dev) {
		dma_sync_single_for_device(host->dma_dev, host->data_dma,
					   sizeof(*scratch), DMA_BIDIRECTIONAL);
		dma_sync_single_for_device(host->dma_dev, t->rx_dma, t->len,
					   DMA_FROM_DEVICE);
	}
	...
	if (host->dma_dev) {
		dma_sync_single_for_cpu(host->dma_dev, host->data_dma,
					sizeof(*scratch), DMA_BIDIRECTIONAL);
		dma_sync_single_for_cpu(host->dma_dev, t->rx_dma, t->len,
					DMA_FROM_DEVICE);
	}
 ...
 */
static void mmc_spi_data_do(struct mmc_spi_host *host, struct mmc_command *cmd,
			    struct mmc_data *data, u32 blk_size)
{
	...
	timeout = data->timeout_ns + data->timeout_clks * 1000000 / clock_rate;
	...
	if (direction == DMA_TO_DEVICE && multiple) {
		...
		list_add(&host->early_status.transfer_list, &host->m.transfers);
...
static void mmc_spi_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (host->power_mode != ios->power_mode) {
		...
		dev_dbg(&host->spi->dev, "mmc_spi: power %s (%d)%s\n",
			mmc_powerstring(ios->power_mode), ios->vdd,
			canpower ? ", can switch" : "");
	...
	if (host->spi->max_speed_hz != ios->clock && ios->clock != 0) {
		...
		dev_dbg(&host->spi->dev, "mmc_spi:  clock to %d Hz, %d\n",
...
static irqreturn_t mmc_spi_detect_irq(int irq, void *mmc)
{
	struct mmc_spi_host *host = mmc_priv(mmc);
...
static int mmc_spi_probe(struct spi_device *spi)
{
	...
	if (status < 0) {
		dev_dbg(&spi->dev, "needs SPI mode %02x, %d KHz; %d\n",
			spi->mode, spi->max_speed_hz / 1000, status);
		return status;
	}
	...
	if (spi->master->dev.parent->dma_mask) {
		...
		host->dma_dev = dev;
		host->ones_dma = dma_map_single(dev, ones, MMC_SPI_BLOCKSIZE,
						DMA_TO_DEVICE);
		if (dma_mapping_error(dev, host->ones_dma))
			goto fail_ones_dma;
		host->data_dma =
			dma_map_single(dev, host->data, sizeof(*host->data),
				       DMA_BIDIRECTIONAL);
	...
	dev_info(&spi->dev, "SD/MMC host %s%s%s%s%s\n",
		 dev_name(&mmc->class_dev), host->dma_dev ? "" : ", no DMA",
		 has_ro ? "" : ", no WP",
		 (host->pdata && host->pdata->setpower) ? "" : ", no poweroff",
		 (mmc->caps & MMC_CAP_NEEDS_POLL) ? ", cd polling" : "");
	return 0;
