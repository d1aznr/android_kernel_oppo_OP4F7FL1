...
static inline bool sdhci_acpi_no_fixup_child_power(const char *hid,
						   const char *uid)
{
	return sdhci_acpi_cht() && !strcmp(hid, "80860F14") &&
	       !strcmp(uid, "2") &&
...
static const struct sdhci_acpi_slot sdhci_acpi_slot_int_emmc = {
	...
	.caps = MMC_CAP_8_BIT_DATA | MMC_CAP_NONREMOVABLE | MMC_CAP_HW_RESET |
		MMC_CAP_1_8V_DDR | MMC_CAP_CMD_DURING_TFR |
...
static const struct sdhci_acpi_slot sdhci_acpi_slot_int_sd = {
	...
	.quirks = SDHCI_QUIRK_NO_ENDATTR_IN_NOPDESC,
	.quirks2 =
		SDHCI_QUIRK2_CARD_ON_NEEDS_BUS_ON | SDHCI_QUIRK2_STOP_WITH_TC,
...
static int sdhci_acpi_probe(struct platform_device *pdev)
{
	...
	host = sdhci_alloc_host(dev,
				sizeof(struct sdhci_acpi_host) + priv_size);
	if (IS_ERR(host))
		return PTR_ERR(host);
	...
	host->ioaddr =
		devm_ioremap_nocache(dev, iomem->start, resource_size(iomem));
	if (host->ioaddr == NULL) {
		err = -ENOMEM;
		goto err_free;
	}
