...
static u16 esdhc_readw_fixup(struct sdhci_host *host, int spec_reg, u32 value)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_esdhc *esdhc = sdhci_pltfm_priv(pltfm_host);
	u16 ret;
...
static u8 esdhc_readb_fixup(struct sdhci_host *host, int spec_reg, u32 value)
{
	u8 ret;
	u8 dma_bits;
 ...
 * Return a fixed up register value
 */
static u32 esdhc_writel_fixup(struct sdhci_host *host, int spec_reg, u32 value,
			      u32 old_value)
{
	u32 ret;
...
static u32 esdhc_writew_fixup(struct sdhci_host *host, int spec_reg, u16 value,
			      u32 old_value)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	int shift = (spec_reg & 0x2) * 8;
...
static u32 esdhc_writeb_fixup(struct sdhci_host *host, int spec_reg, u8 value,
			      u32 old_value)
{
	u32 ret;
	u32 dma_bits;
				       ...
				       struct mmc_ios *ios)
{
	...
	case MMC_SIGNAL_VOLTAGE_180:
		...
		if (scfg_base) {
			sdhciovselcr =
				SDHCIOVSELCR_TGLEN | SDHCIOVSELCR_VSELVAL;
			...
			sdhciovselcr =
				SDHCIOVSELCR_TGLEN | SDHCIOVSELCR_SDHC_VS;
			iowrite32be(sdhciovselcr,
				    scfg_base + SCFG_SDHCIOVSELCR);
			iounmap(scfg_base);
		} else {
...
static SIMPLE_DEV_PM_OPS(esdhc_of_dev_pm_ops, esdhc_of_suspend,
			 esdhc_of_resume);
...
static void esdhc_init(struct platform_device *pdev, struct sdhci_host *host)
{
	...
	esdhc->vendor_ver =
		(host_ver & SDHCI_VENDOR_VER_MASK) >> SDHCI_VENDOR_VER_SHIFT;
