...
#define mmc_has_26MHz() (cpu_is_pxa300() || cpu_is_pxa310() || cpu_is_pxa935())
...
static int pxamci_init_ocr(struct pxamci_host *host)
{
	...
	if (IS_ERR(mmc->supply.vmmc)) {
		...
		mmc->ocr_avail = host->pdata ? host->pdata->ocr_mask :
...
static int pxamci_data_done(struct pxamci_host *host, unsigned int stat)
{
	...
	if (data->flags & MMC_DATA_READ)
		...
		chan = host->dma_chan_tx;
	dma_unmap_sg(chan->device->dev, data->sg, data->sg_len, host->dma_dir);
...
static int pxamci_of_init(struct platform_device *pdev, struct mmc_host *mmc)
{
...
static int pxamci_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_irq(dev, irq, pxamci_irq, 0, DRIVER_NAME, host);
	if (ret)
		goto out;
	...
	if (host->pdata) {
		...
		if (gpio_is_valid(gpio_ro)) {
			...
			if (ret) {
				dev_err(dev, "Failed requesting gpio_ro %d\n",
					gpio_ro);
				goto out;
			} else {
