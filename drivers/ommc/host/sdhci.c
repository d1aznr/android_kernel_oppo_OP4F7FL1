...
static void sdhci_dump_state(struct sdhci_host *host)
{
	...
	pr_info("%s: clk: %d claimer: %s pwr: %d\n", mmc_hostname(mmc),
		host->clock, mmc->claimer->task->comm, host->pwr);
#endif
...
static void sdhci_set_card_detection(struct sdhci_host *host, bool enable)
{
	...
	if (enable) {
		...
		host->ier |=
			present ? SDHCI_INT_CARD_REMOVE : SDHCI_INT_CARD_INSERT;
	} else {
				  ...
				  struct mmc_data *data, int cookie)
{
	...
	if (host->bounce_buffer) {
		...
		dma_sync_single_for_device(host->mmc->parent, host->bounce_addr,
					   host->bounce_buffer_size,
...
static void sdhci_adma_table_pre(struct sdhci_host *host, struct mmc_data *data,
				 int sg_count)
{
				  ...
				  struct mmc_command *cmd)
{
	...
	if (data) {
		...
		host->data_timeout =
			data->blocks * target_timeout + transfer_time;
	} else {
				    ...
				    struct mmc_command *cmd)
{
	...
	mmc_log_string(host->mmc, "ARGUMENT2=0x%08x TRANSFER_MODE=0x%08x\n",
...
static void sdhci_finish_data(struct sdhci_host *host)
{
	 ...
	 */
	if (data->stop && (data->error || !data->mrq->sbc)) {
		/*
	...
	if (!present || host->flags & SDHCI_DEVICE_DEAD) {
		...
		if (host->ops->config_auto_tuning_cmd) {
			if (sdhci_check_auto_tuning(host, mrq->cmd))
				host->ops->config_auto_tuning_cmd(
					host, true, sdhci_get_tuning_cmd(host));
			else
				host->ops->config_auto_tuning_cmd(
					host, false,
...
void sdhci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	else if (!host->ops->enable_controller_clock &&
		 (ios->power_mode & (MMC_POWER_UP | MMC_POWER_ON)))
...
static void sdhci_cmd_irq(struct sdhci_host *host, u32 intmask, u32 *intmask_p)
{
	...
	if (intmask & (SDHCI_INT_TIMEOUT | SDHCI_INT_CRC | SDHCI_INT_END_BIT |
		       SDHCI_INT_INDEX | SDHCI_INT_AUTO_CMD_ERR)) {
		...
		if (intmask & SDHCI_INT_AUTO_CMD_ERR) {
			...
			pr_err_ratelimited("%s: %s: AUTO CMD err sts 0x%08x\n",
					   ...
					   auto_cmd_status);
			if (auto_cmd_status &
			    (SDHCI_AUTO_CMD12_NOT_EXEC | SDHCI_AUTO_CMD_INDEX |
...
static void sdhci_adma_show_error(struct sdhci_host *host)
{
	...
	while (true) {
		...
		if (host->flags & SDHCI_USE_64_BIT_DMA)
			...
			SDHCI_DUMP(
				"%08llx: DMA 0x%08x, LEN 0x%04x, Attr=0x%02x\n",
				(unsigned long long)dma,
				le32_to_cpu(dma_desc->addr_lo),
				le16_to_cpu(dma_desc->len),
				le16_to_cpu(dma_desc->cmd));
	...
	if (!host->data) {
		...
		if (data_cmd && (data_cmd->flags & MMC_RSP_BUSY)) {
			...
			if (host->quirks2 &
				...
				pr_err_ratelimited(
					"%s: %s: ignoring interrupt: 0x%08x due to DATATOUT_FOR_R1B quirk\n",
					mmc_hostname(host->mmc), __func__,
					intmask);
				mmc_log_string(host->mmc,
...
static irqreturn_t sdhci_irq(int irq, void *dev_id)
{
	...
	if (!host->clock && host->mmc->card && mmc_card_sdio(host->mmc->card)) {
		if (!mmc_card_and_host_support_async_int(host->mmc)) {
			spin_unlock(&host->lock);
			return IRQ_NONE;
		}
...
{
	u8 mask =
		SDHCI_WAKE_ON_INSERT | SDHCI_WAKE_ON_REMOVE | SDHCI_WAKE_ON_INT;
	u32 irq_val = 0;
	u8 wake_val = 0;
	u8 val;
...
void __sdhci_read_caps(struct sdhci_host *host, u16 *ver, u32 *caps, u32 *caps1)
{
	...
	of_property_read_u64(mmc_dev(host->mmc)->of_node, "sdhci-caps-mask",
			     &dt_caps_mask);
	of_property_read_u64(mmc_dev(host->mmc)->of_node, "sdhci-caps",
			     &dt_caps);
...
int sdhci_setup_host(struct sdhci_host *host)
{
	...
	if (!(host->quirks & SDHCI_QUIRK_DATA_TIMEOUT_USES_SDCLK)) {
		...
		if (host->timeout_clk == 0) {
			...
			host->timeout_clk = DIV_ROUND_UP(
				host->ops->get_timeout_clock(host), 1000);
		}
	...
	if ((host->quirks & SDHCI_QUIRK_BROKEN_CARD_DETECTION) &&
	    mmc_card_is_removable(mmc) && mmc_gpio_get_cd(host->mmc) < 0 &&
	...
	if (host->caps & SDHCI_CAN_VDD_330) {
		...
		mmc->max_current_330 =
			((max_current_caps & SDHCI_MAX_CURRENT_330_MASK) >>
	...
	if (host->caps & SDHCI_CAN_VDD_300) {
		...
		mmc->max_current_300 =
			((max_current_caps & SDHCI_MAX_CURRENT_300_MASK) >>
	...
	if (host->caps & SDHCI_CAN_VDD_180) {
		...
		mmc->max_current_180 =
			((max_current_caps & SDHCI_MAX_CURRENT_180_MASK) >>
	...
	if (host->flags & SDHCI_USE_ADMA) {
		...
		if (swiotlb_max_segment()) {
			unsigned int max_req_size =
				(1 << IO_TLB_SHIFT) * IO_TLB_SEGSIZE;
			mmc->max_req_size =
				min(mmc->max_req_size, max_req_size);
		}
	} else { /* PIO */
		mmc->max_segs = SDHCI_MAX_SEGS;
	}
...
int __sdhci_add_host(struct sdhci_host *host)
{
	...
	pr_info("%s: SDHCI controller on %s [%s] using %s in %s mode\n",
		...
		(host->flags & SDHCI_USE_ADMA) ?
			((host->flags & SDHCI_USE_64_BIT_DMA) ? "64-bit ADMA" :
								"32-bit ADMA") :
			((host->flags & SDHCI_USE_SDMA) ? "DMA" : "PIO"),
		((mmc->caps2 & MMC_CAP2_CQE) && !ret) ? "CMDQ" : "legacy");
