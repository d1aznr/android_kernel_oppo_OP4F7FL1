...
static void tifm_sd_card_event(struct tifm_dev *sock)
{
	...
	if (host->req) {
		...
		if (host->no_dma && cmd->data) {
			...
			if (host_status &
			    (TIFM_MMCSD_AE | TIFM_MMCSD_AF | TIFM_MMCSD_BRS)) {
				local_irq_save(flags);
...
static void tifm_sd_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (r_data) {
		...
		if ((r_data->flags & MMC_DATA_WRITE) && !mrq->stop)
			writel(TIFM_MMCSD_EOFB | readl(sock->addr +
						       SOCK_MMCSD_INT_ENABLE),
			       sock->addr + SOCK_MMCSD_INT_ENABLE);

		if (host->no_dma) {
			writel(TIFM_MMCSD_BUFINT | readl(sock->addr +
							 SOCK_MMCSD_INT_ENABLE),
			       sock->addr + SOCK_MMCSD_INT_ENABLE);
			...
			host->sg_len =
				tifm_map_sg(sock, r_data->sg, r_data->sg_len,
					    r_data->flags & MMC_DATA_WRITE ?
						    PCI_DMA_TODEVICE :
						    PCI_DMA_FROMDEVICE);
			if (host->sg_len < 1) {
...
static void tifm_sd_abort(struct timer_list *t)
{
	...
	pr_err("%s : card failed to respond for a long period of time "
	       "(%x, %x)\n",
	       dev_name(&host->dev->dev), host->req->cmd->opcode,
	       host->cmd_flags);
...
static const struct mmc_host_ops tifm_sd_ops = { .request = tifm_sd_request,
						 .set_ios = tifm_sd_ios,
						 .get_ro = tifm_sd_ro };
...
static struct tifm_device_id tifm_sd_id_tbl[] = { { TIFM_TYPE_SD }, {} };

static struct tifm_driver tifm_sd_driver = { .driver = { .name = DRIVER_NAME,
							 .owner = THIS_MODULE },
