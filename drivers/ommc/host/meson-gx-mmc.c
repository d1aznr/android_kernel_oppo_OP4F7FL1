...
static void meson_mmc_apply_phase_delay(struct meson_mmc_phase *mmc,
					unsigned int phase, unsigned int delay)
{
	u32 val;
...
static int meson_mmc_clk_set_phase(struct clk_hw *hw, int degrees)
{
	...
	if (!mmc->delay_mask) {
		...
		d = DIV_ROUND_CLOSEST(r * period_ps, 360 * mmc->delay_step_ps);
...
static void meson_mmc_find_next_region(unsigned long *map, unsigned long *start,
				       unsigned long *stop)
{
...
static void meson_mmc_start_cmd(struct mmc_host *mmc, struct mmc_command *cmd)
{
	...
	if (data) {
		...
		if (data->blocks > 1) {
			cmd_cfg |= CMD_CFG_BLOCK_MODE;
			cmd_cfg |=
				FIELD_PREP(CMD_CFG_LENGTH_MASK, data->blocks);
			meson_mmc_set_blksz(mmc, data->blksz);
		} else {
...
static void meson_mmc_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	bool needs_pre_post_req =
		mrq->data && !(mrq->data->host_cookie & SD_EMMC_PRE_REQ_DONE);
...
static irqreturn_t meson_mmc_irq_thread(int irq, void *dev_id)
{
	...
	if (meson_mmc_bounce_buf_read(data)) {
		xfer_bytes = data->blksz * data->blocks;
		WARN_ON(xfer_bytes > host->bounce_buf_size);
		sg_copy_from_buffer(data->sg, data->sg_len, host->bounce_buf,
				    xfer_bytes);
	}
...
static int meson_mmc_probe(struct platform_device *pdev)
{
	...
	host->data =
		(struct meson_mmc_data *)of_device_get_match_data(&pdev->dev);
	if (!host->data) {
		ret = -EINVAL;
		goto free_host;
	}
	...
	host->pins_clk_gate = pinctrl_lookup_state(host->pinctrl, "clk-gate");
...
err_bounce_buf:
	dma_free_coherent(host->dev, host->bounce_buf_size, host->bounce_buf,
			  host->bounce_dma_addr);
err_free_irq:
...
static int meson_mmc_remove(struct platform_device *pdev)
{
	...
	dma_free_coherent(host->dev, SD_EMMC_DESC_BUF_LEN, host->descs,
			  host->descs_dma_addr);
	dma_free_coherent(host->dev, host->bounce_buf_size, host->bounce_buf,
			  host->bounce_dma_addr);
