...
static void mmc_clk_scaling_stop_busy(struct mmc_host *host, bool lock_needed)
{
	...
	clk_scaling->total_busy_time_us +=
		ktime_to_us(ktime_sub(ktime_get(), clk_scaling->start_busy));
 ...
 */
void mmc_cqe_clk_scaling_start_busy(struct mmc_queue *mq, struct mmc_host *host,
				    bool lock_needed)
{
	unsigned long flags;
 ...
 */
void mmc_cqe_clk_scaling_stop_busy(struct mmc_host *host, bool lock_needed,
				   bool is_cqe_dcmd)
{
	...
	host->clk_scaling.total_busy_time_us += ktime_to_us(
		ktime_sub(ktime_get(), host->clk_scaling.start_busy));
				      ...
				      struct devfreq_dev_status *status)
{
	...
	status->total_time = ktime_to_us(
		ktime_sub(ktime_get(), clk_scaling->measure_interval_start));
...
int mmc_clk_update_freq_deferred(struct mmc_host *host, unsigned long freq,
				 enum mmc_load state)
{
	int err = 0;
...
int mmc_clk_update_freq(struct mmc_host *host, unsigned long freq,
			enum mmc_load state)
{
	int err = 0;
...
static int mmc_devfreq_set_target(struct device *dev, unsigned long *freq,
				  u32 devfreq_flags)
{
	struct mmc_host *host = container_of(dev, struct mmc_host, class_dev);
	struct mmc_devfeq_clk_scaling *clk_scaling;
	...
	clk_scaling->state =
		*freq < clk_scaling->curr_freq ? MMC_LOAD_LOW : MMC_LOAD_HIGH;
	spin_unlock_irqrestore(&clk_scaling->lock, flags);
...
void mmc_deferred_scaling(struct mmc_host *host)
{
	...
	pr_debug("%s: doing deferred frequency change (%lu) (%s)\n",
		 mmc_hostname(host), target_freq, current->comm);
...
static int mmc_devfreq_create_freq_table(struct mmc_host *host)
{
	...
	if (!clk_scaling->freq_table) {
		...
		clk_scaling->freq_table =
			kzalloc((clk_scaling->freq_table_sz *
		...
		if (clk_scaling->pltfm_freq_table) {
			...
			pr_debug(
				"%s: no frequency table defined -  setting default\n",
				mmc_hostname(host));
	...
	if (host->card->clk_scaling_lowest > clk_scaling->freq_table[0])
...
int mmc_init_clk_scaling(struct mmc_host *host)
{
	...
	if (!mmc_can_scale_clk(host) || !host->bus_ops->change_bus_speed) {
	...
	if (host->clk_scaling.devfreq) {
		pr_err("%s: dev is already registered for dev %pK\n",
		       mmc_hostname(host), mmc_dev(host));
		return -EPERM;
	}
	...
	pr_debug(
		"%s: adding devfreq with: upthreshold=%u downthreshold=%u polling=%u\n",
		mmc_hostname(host),
		host->clk_scaling.ondemand_gov_data.upthreshold,
		host->clk_scaling.ondemand_gov_data.downdifferential,
		host->clk_scaling.devfreq_profile.polling_ms);

	devfreq = devfreq_add_device(mmc_classdev(host),
	...
	pr_debug(
		"%s: clk scaling is enabled for device %s (%pK) with devfreq %pK (clock = %uHz)\n",
		mmc_hostname(host), dev_name(mmc_classdev(host)),
		mmc_classdev(host), host->clk_scaling.devfreq, host->ios.clock);
	...
	if (!err || !cmd->retries || mmc_card_removed(host->card)) {
		...
		if (mrq->sbc) {
			pr_debug(
				...
				mrq->sbc->error, mrq->sbc->resp[0],
				mrq->sbc->resp[1], mrq->sbc->resp[2],
				mrq->sbc->resp[3]);
		}

		pr_debug("%s: req done (CMD%u): %d: %08x %08x %08x %08x\n",
			 mmc_hostname(host), cmd->opcode, err, cmd->resp[0],
		...
		if (mrq->stop) {
			pr_debug("%s:     (CMD%u): %d: %08x %08x %08x %08x\n",
				 ...
				 mrq->stop->error, mrq->stop->resp[0],
				 mrq->stop->resp[1], mrq->stop->resp[2],
				 mrq->stop->resp[3]);
		}
	}
	/*
			     ...
			     bool cqe)
{
	if (mrq->sbc) {
		pr_debug("<%s: starting CMD%u arg %08x flags %08x>\n",
			 mmc_hostname(host), mrq->sbc->opcode, mrq->sbc->arg,
			 mrq->sbc->flags);
	}
	...
	if (mrq->data) {
		pr_debug(
			...
			mmc_hostname(host), mrq->data->blksz, mrq->data->blocks,
			mrq->data->flags, mrq->data->timeout_ns / 1000000,
			mrq->data->timeout_clks);
	}

	if (mrq->stop) {
		pr_debug("%s:     CMD%u arg %08x flags %08x\n",
			 mmc_hostname(host), mrq->stop->opcode, mrq->stop->arg,
			 mrq->stop->flags);
	}
}
...
static inline void mmc_set_ios(struct mmc_host *host)
{
	...
	pr_debug("%s: clock %uHz busmode %u powermode %u cs %u Vdd %u "
		 "width %u timing %u\n",
		 mmc_hostname(host), ios->clock, ios->bus_mode, ios->power_mode,
		 ios->chip_select, ios->vdd, 1 << ios->bus_width, ios->timing);
	...
	if (ios->old_rate != ios->clock) {
		if (likely(ios->clk_ts)) {
			...
			mmc_log_string(host, "freq_KHz %d --> %d | t = %d",
				       ios->old_rate / 1000, ios->clock / 1000,
				       jiffies_to_msecs((long)jiffies -
							(long)ios->clk_ts));
		}
...
int mmc_regulator_set_vqmmc(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_SIGNAL_VOLTAGE_330:
		 ...
		 */
		if (!mmc_regulator_set_voltage_if_supported(
			    mmc->supply.vqmmc, min_uV, volt, max_uV))
			return 0;
			      ...
			      int card_drv_type, int *drv_type)
{
	...
	*drv_type = 0;
	 ...
	 */
	return host->ops->select_drive_strength(card, max_dtr, host_drv_type,
					  ...
					  unsigned int arg, unsigned int qty)
{
	...
	if (arg == MMC_DISCARD_ARG ||
		...
		timeout_clks <<= 1;
		timeout_us +=
			(timeout_clks * 1000) / (card->host->ios.clock / 1000);
					 ...
					 unsigned int arg, unsigned int qty)
{
	...
	if (card->ssr.erase_timeout) {
		...
		erase_timeout =
			card->ssr.erase_timeout * qty + card->ssr.erase_offset;
	} else {
		/*
...
static unsigned int mmc_erase_timeout(struct mmc_card *card, unsigned int arg,
				      unsigned int qty)
{
	if (mmc_card_sd(card))
			...
			unsigned int to, unsigned int arg)
{
	...
	if (card->erase_shift)
		...
		qty += ((to / card->erase_size) - (from / card->erase_size)) +
		       1;
...
unsigned int mmc_calc_max_discard(struct mmc_card *card)
{
	...
	pr_debug("%s: calculated max. discard sectors %u for timeout %u ms\n",
		 ...
		 host->max_busy_timeout ? host->max_busy_timeout :
					  MMC_ERASE_TIMEOUT_MS);
	return max_discard;
}
			 ...
			 unsigned long mode, void *unused)
{
	...
	case PM_POST_RESTORE:
		...
		if (mmc_bus_manual_resume(host) &&
		    !host->ignore_bus_resume_flags && present) {
			spin_unlock_irqrestore(&host->lock, flags);
			break;
		}
		spin_unlock_irqrestore(&host->lock, flags);
