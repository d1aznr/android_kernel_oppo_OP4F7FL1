...
static void mmc_blk_issue_drv_op(struct mmc_queue *mq, struct request *req)
{
	...
	case MMC_DRV_OP_IOCTL:
		...
		pr_err("mmc __mmc_blk_ioctl_cmd enter cardState 0x%x ,cmdq(%d %d %d) , adma2len %d\n",
		       card->state, card->ext_csd.cmdq_en,
...
static void mmc_blk_read_single(struct mmc_queue *mq, struct request *req)
{
	...
	do {
		...
		if (!mmc_host_is_spi(host) && !mmc_blk_in_tran_state(status)) {
...
static int mmc_blk_add_debugfs(struct mmc_card *card, struct mmc_blk_data *md)
{
	...
	if (mmc_card_mmc(card)) {
		md->life_time_dentry =
			debugfs_create_file("life_time",
					    (S_IRUSR | S_IRGRP | S_IROTH), root,
					    card, &mmc_dbg_life_time_fops);
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	if ((!strcmp(mmc_card_id(card), "mmc0:0001")) &&
		...
		sprintf(temp_version, "0x%02x,0x%llx", card->cid.prv,
			*(unsigned long long *)card->ext_csd.fwrev);
		register_device_proc("emmc", mmc_card_name(card),
				     manufacturerid);
		register_device_proc("emmc_version", mmc_card_name(card),
				     temp_version);
	}
#endif
