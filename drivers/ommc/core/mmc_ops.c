 ...
 */
static int mmc_send_cxd_data(struct mmc_card *card, struct mmc_host *host,
...
static int mmc_read_bkops_status(struct mmc_card *card)
{
	...
	card->ext_csd.raw_bkops_status =
		ext_csd[EXT_CSD_BKOPS_STATUS] & MMC_BKOPS_URGENCY_MASK;
	card->ext_csd.raw_exception_status =
		ext_csd[EXT_CSD_EXP_EVENTS_STATUS] &
		(EXT_CSD_URGENT_BKOPS | EXT_CSD_DYNCAP_NEEDED |
		 EXT_CSD_SYSPOOL_EXHAUSTED | EXT_CSD_PACKED_FAILURE);
	kfree(ext_csd);
	return 0;
}
...
void mmc_start_bkops(struct mmc_card *card, bool from_exception)
{
	...
	err = __mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BKOPS_START, 1,
			   timeout, 0, use_busy_signal, true, false);
	if (err) {
