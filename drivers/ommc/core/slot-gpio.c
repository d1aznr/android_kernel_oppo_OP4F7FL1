...
int mmc_gpio_alloc(struct mmc_host *host)
{
	...
	struct mmc_gpio *ctx =
		devm_kzalloc(host->parent, sizeof(*ctx) + 2 * len, GFP_KERNEL);
...
int mmc_gpio_get_cd(struct mmc_host *host)
{
	...
	if (host->extcon) {
		...
		if (ret < 0)
			dev_err(mmc_dev(host),
				"%s: Extcon failed to check card state, ret=%d\n",
				__func__, ret);
		return ret;
	}
	if (!ctx || !ctx->cd_gpio)
	...
	return cansleep ? gpiod_get_value_cansleep(ctx->cd_gpio) :
...
void mmc_gpiod_request_cd_irq(struct mmc_host *host)
{
	...
	if (irq >= 0) {
		...
		ret = devm_request_threaded_irq(
			host->parent, irq, NULL, ctx->cd_gpio_isr,
	...
	if (ret)
		dev_err(mmc_dev(host),
			"%s: enabling cd irq wake failed ret=%d\n", __func__,
			ret);
}
				    ...
				    unsigned long event, void *ptr)
{
	struct mmc_host *host =
		container_of(nb, struct mmc_host, card_detect_nb);
...
void mmc_register_extcon(struct mmc_host *host)
{
	...
	if (err) {
		dev_err(mmc_dev(host),
			"%s: extcon_register_notifier() failed ret=%d\n",
			__func__, err);
		host->caps |= MMC_CAP_NEEDS_POLL;
	}
}
...
void mmc_unregister_extcon(struct mmc_host *host)
{
	...
	if (err)
		dev_err(mmc_dev(host),
			"%s: extcon_unregister_notifier() failed ret=%d\n",
			__func__, err);
}
...
int mmc_gpio_set_cd_wake(struct mmc_host *host, bool on)
{
	...
	if (!(host->caps & MMC_CAP_CD_WAKE) || host->slot.cd_irq < 0 ||
