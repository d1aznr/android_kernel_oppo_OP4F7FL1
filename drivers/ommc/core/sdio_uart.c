...
static void sdio_uart_transmit_chars(struct sdio_uart_port *port)
{
	...
	if (tty == NULL || !kfifo_len(xmit) || tty->stopped ||
	    tty->hw_stopped) {
...
static int sdio_uart_activate(struct tty_port *tport, struct tty_struct *tty)
{
	...
	port->ier =
		UART_IER_RLSI | UART_IER_RDI | UART_IER_RTOIE | UART_IER_UUE;
	...
	for (i = 0; i < UART_NR; i++) {
		...
		if (port) {
			...
			if (capable(CAP_SYS_ADMIN)) {
				seq_printf(m, " tx:%d rx:%d", port->icount.tx,
					   port->icount.rx);
			   ...
			   const struct sdio_device_id *id)
{
	...
	if (func->class == SDIO_CLASS_UART) {
		...
		port->regs_offset = (tpl->data[4] << 0) | (tpl->data[5] << 8) |
				    (tpl->data[6] << 16);
		...
		pr_debug("%s: clk %d baudcode %u 4800-div %u\n",
			 sdio_func_id(func), port->uartclk, tpl->data[7],
			 tpl->data[8] | (tpl->data[9] << 8));
	} else {
		kfree(port);
		return -EINVAL;
	}
