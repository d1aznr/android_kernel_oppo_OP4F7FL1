...
static int mmc_ios_show(struct seq_file *s, void *data)
{
	static const char *vdd_str[] = {
		...
		[12] = "2.4", [13] = "2.5", [14] = "2.6", [15] = "2.7",
		[16] = "2.8", [17] = "2.9", [18] = "3.0", [19] = "3.1",
		[20] = "3.2", [21] = "3.3", [22] = "3.4", [23] = "3.5",
		[24] = "3.6",
	};
	...
	else if (ios->vdd < (ARRAY_SIZE(vdd_str) - 1) && vdd_str[ios->vdd] &&
		 vdd_str[ios->vdd + 1])
		seq_printf(s, "(%s ~ %s V)\n", vdd_str[ios->vdd],
			   vdd_str[ios->vdd + 1]);
	else
...
static int mmc_scale_set(void *data, u64 val)
{
	...
	if (err == -EAGAIN)
		...
		pr_debug(
			"%s: clock change to %llu finished successfully (%s)\n",
...
DEFINE_SIMPLE_ATTRIBUTE(mmc_scale_fops, mmc_scale_get, mmc_scale_set, "%llu\n");
...
DEFINE_SIMPLE_ATTRIBUTE(mmc_err_state, mmc_err_state_get, mmc_err_state_clear,
			"%llu\n");
...
static int mmc_force_err_set(void *data, u64 val)
{
	...
	if (host && host->card && host->ops && host->ops->force_err_irq) {
		/*
...
void mmc_add_host_debugfs(struct mmc_host *host)
{
	...
	if (!debugfs_create_file("clock", 0600, root, host, &mmc_clock_fops))
		goto err_node;
	...
	if (!debugfs_create_file("scale", 0600, root, host, &mmc_scale_fops))
		goto err_node;

	if (!debugfs_create_bool("skip_clk_scale_freq_update", 0600, root,
				 &host->clk_scaling.skip_clk_scale_freq_update))
		goto err_node;

	if (!debugfs_create_file("err_state", 0600, root, host, &mmc_err_state))
		goto err_node;
	...
	if (IS_ERR(fault_create_debugfs_attr("fail_mmc_request", root,
	...
	if (!debugfs_create_bool("crash_on_err", 0600, root,
				 &host->crash_on_err))
		goto err_node;
