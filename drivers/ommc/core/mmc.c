...
static void mmc_select_card_type(struct mmc_card *card)
{
	...
	if ((caps2 & MMC_CAP2_HS400_ES) && card->ext_csd.strobe_support &&
...
static void mmc_manage_enhanced_area(struct mmc_card *card, u8 *ext_csd)
{
	...
	if ((ext_csd[EXT_CSD_PARTITION_SUPPORT] & 0x2) &&
		...
		if (card->ext_csd.partition_setting_completed) {
			hc_erase_grp_sz = ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE];
			hc_wp_grp_sz = ext_csd[EXT_CSD_HC_WP_GRP_SIZE];
...
static void mmc_manage_gp_partitions(struct mmc_card *card, u8 *ext_csd)
{
	...
	if (ext_csd[EXT_CSD_PARTITION_SUPPORT] & EXT_CSD_PART_SUPPORT_PART_EN) {
		...
		for (idx = 0; idx < MMC_NUM_GP_PARTITION; idx++) {
			...
			part_size = (ext_csd[EXT_CSD_GP_SIZE_MULT + idx * 3 + 2]
				     << 16) +
				    (ext_csd[EXT_CSD_GP_SIZE_MULT + idx * 3 + 1]
				     << 8) +
			...
			mmc_part_add(card, part_size << 19,
				     EXT_CSD_PART_CONFIG_ACC_GP0 + idx, "gp%d",
				     idx, false, MMC_BLK_DATA_AREA_GP);
		}
	}
}
...
void mmc_check_bkops_support(struct mmc_card *card, u8 *ext_csd)
{
	if (!mmc_card_broken_hpi(card) &&
	    (ext_csd[EXT_CSD_BKOPS_SUPPORT] & 0x1) && card->ext_csd.hpi) {
		card->ext_csd.bkops = 1;
		card->ext_csd.man_bkops_en =
			(ext_csd[EXT_CSD_BKOPS_EN] & EXT_CSD_MANUAL_BKOPS_MASK);
		card->ext_csd.raw_bkops_status = ext_csd[EXT_CSD_BKOPS_STATUS];
		if (card->ext_csd.man_bkops_en)
			pr_debug("%s: MAN_BKOPS_EN bit is set\n",
				 mmc_hostname(card->host));
		card->ext_csd.auto_bkops_en =
			(ext_csd[EXT_CSD_BKOPS_EN] & EXT_CSD_AUTO_BKOPS_MASK);
...
static int mmc_decode_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	if (card->ext_csd.rev >= 3) {
		...
		card->ext_csd.hc_erase_size = ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE]
					      << 10;
	...
	card->ext_csd.raw_hc_erase_gap_size = ext_csd[EXT_CSD_HC_WP_GRP_SIZE];
	card->ext_csd.raw_sec_trim_mult = ext_csd[EXT_CSD_SEC_TRIM_MULT];
	card->ext_csd.raw_sec_erase_mult = ext_csd[EXT_CSD_SEC_ERASE_MULT];
	card->ext_csd.raw_sec_feature_support =
		ext_csd[EXT_CSD_SEC_FEATURE_SUPPORT];
	card->ext_csd.raw_trim_mult = ext_csd[EXT_CSD_TRIM_MULT];
	...
	if (card->ext_csd.rev >= 5) {
		...
		/* check whether the eMMC card supports HPI */
		if (!mmc_card_broken_hpi(card) && !broken_hpi &&
		...
		if (ext_csd[EXT_CSD_RPMB_MULT] && mmc_host_cmd23(card->host)) {
			mmc_part_add(card, ext_csd[EXT_CSD_RPMB_MULT] << 17,
				     EXT_CSD_PART_CONFIG_ACC_RPMB, "rpmb", 0,
				     false, MMC_BLK_DATA_AREA_RPMB);
		}
	}
	...
	if (card->ext_csd.rev >= 6) {
		...
		card->ext_csd.power_off_longtime =
			10 * ext_csd[EXT_CSD_POWER_OFF_LONG_TIME];
	...
	if (card->ext_csd.rev >= 7) {
		...
		pr_info("%s: eMMC FW version: 0x%02x\n",
			mmc_hostname(card->host), card->ext_csd.fw_version);
	}

	/* eMMC v5.1 or later */
	if (card->ext_csd.rev >= 8) {
		card->ext_csd.cmdq_support =
			ext_csd[EXT_CSD_CMDQ_SUPPORT] & EXT_CSD_CMDQ_SUPPORTED;
...
static ssize_t mmc_fwrev_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
				   ...
				   unsigned int bus_width)
{
	...
	if (pwrclass_val > 0) {
		err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
				 EXT_CSD_POWER_CLASS, pwrclass_val,
				 card->ext_csd.generic_cmd6_time);
	}
...
static int mmc_select_hs(struct mmc_card *card)
{
	...
	err = __mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING,
			   ...
			   MMC_TIMING_MMC_HS, true, true, true);
	if (err)
...
static int mmc_select_hs_ddr(struct mmc_card *card)
{
	...
	err = __mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BUS_WIDTH,
			   ext_csd_bits, card->ext_csd.generic_cmd6_time,
			   MMC_TIMING_MMC_DDR52, true, true, true);
	if (err) {
...
static int mmc_select_hs400(struct mmc_card *card)
{
	...
	err = __mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING, val,
			   card->ext_csd.generic_cmd6_time, 0, true, false,
			   true);
	if (err) {
	...
	/* Switch card to DDR */
	err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BUS_WIDTH, val,
			 card->ext_csd.generic_cmd6_time);
	if (err) {
	...
	val = EXT_CSD_TIMING_HS400 | card->drive_strength
	...
	if (card->mmc_avail_type & EXT_CSD_CARD_TYPE_HS400ES &&
	    card->ext_csd.strobe_support && host->ops->enhanced_strobe) {
...
int mmc_hs400_to_hs200(struct mmc_card *card)
{
	...
	/* Switch HS to HS200 */
	val = EXT_CSD_TIMING_HS200 | card->drive_strength
					     << EXT_CSD_DRV_STR_SHIFT;
	err = __mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING, val,
			   card->ext_csd.generic_cmd6_time, 0, true, false,
			   true);
...
static void mmc_select_driver_type(struct mmc_card *card)
{
	...
	card_drv_type =
		card->ext_csd.raw_driver_strength | mmc_driver_type_mask(0);

	if (fixed_drv_type >= 0)
		drive_strength =
			card_drv_type & mmc_driver_type_mask(fixed_drv_type) ?
				fixed_drv_type :
				0;
	else
		drive_strength =
...
static int mmc_select_hs200(struct mmc_card *card)
{
	...
	if (err > 0) {
		...
		err = __mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
				   ...
				   card->ext_csd.generic_cmd6_time, 0, true,
				   false, true);
		if (err)
			goto err;
			     ...
			     unsigned long event, void *unused)
{
	struct mmc_card *card =
		container_of(notify_block, struct mmc_card, reboot_notify);
...
void mmc_init_setup_scaling(struct mmc_card *card, struct mmc_host *host)
{
			 ...
			 struct mmc_card *oldcard)
{
	 ...
	 */
	if (card->ext_csd.hpi) {
		err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HPI_MGMT,
...
static int mmc_poweroff_notify(struct mmc_card *card, unsigned int notify_type)
{
	...
	err = __mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
			   EXT_CSD_POWER_OFF_NOTIFICATION, notify_type, timeout,
			   0, true, false, false);
	if (err)
...
static int mmc_test_awake_ext_csd(struct mmc_host *host)
{
	...
	pr_debug(
		"%s: %s: type(cached:current) cmdq(%d:%d) cache_ctrl(%d:%d) bus_width (%d:%d) timing(%d:%d)\n",
		mmc_hostname(host), __func__, card->ext_csd.raw_ext_csd_cmdq,
...
static int mmc_partial_init(struct mmc_host *host)
{
	...
	if (mmc_card_hs400(card)) {
		if (card->ext_csd.strobe_support && host->ops->enhanced_strobe)
			...
			err = host->ops->execute_tuning(
				host, MMC_SEND_TUNING_BLOCK_HS200);
