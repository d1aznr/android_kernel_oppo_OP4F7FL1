...
struct menfinfo manufacturers[] = { { 0x41, "KINGSTONE" },
				    { 0x1b, "SAMSUNG" },
				    { 0x03, "SANDISK" },
				    { 0x02, "TOSHIBA" } };
#define MANFINFS_SIZE (sizeof(manufacturers) / sizeof(struct menfinfo))

const char *string_class[] = { "Class 0", "Class 2", "Class 4", "Class 6",
			       "Class 10" };
	...
	if (au) {
		if (au <= 9 || card->scr.sda_spec3) {
			...
			card->ssr.speed_class =
				UNSTUFF_BITS(card->raw_ssr, 440 - 384, 8);
#endif /* OPLUS_FEATURE_EMMC_SDCARD_OPTIMIZE */
			if (es && et) {
...
{
	return mmc_card_uhs(card) ? STR_SPEED_UHS :
				    (mmc_card_hs(card) ? STR_SPEED_HS : "");
}
...
MMC_DEV_ATTR(devinfo,
	     " manufacturer: %s\n size: %s\n type: %s\n speed: %s\n class: %s\n",
	     manfinfo_string(card), capacity_string(card), type_string(card),
	     uhs_string(card), speed_class_string(card));
#endif /* OPLUS_FEATURE_EMMC_SDCARD_OPTIMIZE */
...
static int _mmc_sd_resume(struct mmc_host *host)
{
	...
	if (err == -ENOENT) {
		pr_debug(
			"%s: %s: found a different card(%d), do detect change\n",
			mmc_hostname(host), __func__, err);
		mmc_card_set_removed(host->card);
