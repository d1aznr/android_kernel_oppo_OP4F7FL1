...
static enum blk_eh_timer_return mmc_cqe_timed_out(struct request *req)
{
	...
	case MMC_ISSUE_DCMD:
		...
		pr_info("%s: %s: Timeout even before req reaching LDD, completing the req. Active reqs: %d Req: %p Tag: %d\n",
			mmc_hostname(host), __func__, mmc_cqe_qcnt(mq), req,
			req->tag);
		mmc_log_string(
			host,
...
{
	struct mmc_queue *mq =
		container_of(work, struct mmc_queue, recovery_work);
			     ...
			     const struct blk_mq_ops *mq_ops, spinlock_t *lock)
{
	...
	mq->tag_set.flags =
		BLK_MQ_F_SHOULD_MERGE | BLK_MQ_F_SG_MERGE | BLK_MQ_F_BLOCKING;
