...
int cnss_stats_self_recovery(void)
{
	...
	memcpy(cnss_event->payload, cnss_event_payload,
	       cnss_event->payload_length);
...
static ssize_t icnss_show_fw_ready(struct device_driver *driver, char *buf)
{
	...
	if (!plat_env) {
		...
		firmware_ready =
			test_bit(CNSS_FW_READY, &plat_env->driver_state);
		regdbloadsuccess = test_bit(CNSS_LOAD_REGDB_SUCCESS,
					    &plat_env->loadRegdbState);
		bdfloadsuccess = test_bit(CNSS_LOAD_BDF_SUCCESS,
					  &plat_env->loadBdfState);
#ifdef OPLUS_FEATURE_WIFI_DCS_SWITCH
		//avoid wifi firmware ready check fail when idle shutdown
		cnss_pr_info("firmware_ready: %d; power_on: %d", firmware_ready,
			     plat_env->powered_on);
		if (!firmware_ready && !plat_env->powered_on &&
		    regdbloadsuccess && bdfloadsuccess) {
			firmware_ready = true;
		}
#endif /* OPLUS_FEATURE_WIFI_DCS_SWITCH */
	}
...
static void icnss_create_fw_state_kobj(void)
{
	if (driver_create_file(&(cnss_platform_driver.driver),
			       &fw_ready_attr)) {
