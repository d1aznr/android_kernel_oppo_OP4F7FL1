...
#define P61_DBG_MSG(msg...)                                                    \
	...
	case P61_FULL_DEBUG:                                                   \
		...
		break;                                                         \
	default:                                                               \
		printk(KERN_ERR "[NXP-P61] :  Wrong debug level %d",           \
		       debug_level);                                           \
		break;                                                         \
	}
	...
	if (type == READ_THROUGH_PUT) {
		...
		p61_through_put_t.total_through_put_rtime +=
			...
			((p61_through_put_t.rstop_tv.tv_sec -
			  p61_through_put_t.rstart_tv.tv_sec) *
			 1000000);
		...
		p61_through_put_t.total_through_put_wtime +=
			...
			((p61_through_put_t.wstop_tv.tv_sec -
			  p61_through_put_t.wstart_tv.tv_sec) *
			 1000000);
			  ...
			  unsigned long arg)
{
		...
		if (arg == 2) {
			...
			if (p61_regulator != NULL) {
				...
				P61_DBG_MSG(KERN_ALERT
					    " Enabling p61_regulator");
			} else {
				P61_ERR_MSG(
					KERN_ALERT
					" ERROR : p61_regulator is not enabled");
			}
#endif
	...
	case P61_SET_THROUGHPUT:
		...
		P61_DBG_MSG(KERN_INFO
			    "[NXP-P61] -  P61_SET_THROUGHPUT enable %d",
...
static int p61_probe(struct spi_device *spi)
{
	...
	struct p61_spi_platform_data platform_data1 = { .irq_gpio = 0,
							.rst_gpio = 0 };
	struct p61_dev *p61_dev = NULL;
#ifdef P61_IRQ_ENABLE
