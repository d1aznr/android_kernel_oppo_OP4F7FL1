...
static struct page *ipa3_alloc_page(gfp_t flag, u32 *page_order, bool try_lower,
				    bool is_tmp_alloc)
{
	...
	gfp_t flag, bool is_tmp_alloc)
{
	...
	rx_pkt->page_data.page =
		ipa3_alloc_page(flag, &rx_pkt->page_data.page_order,
				(is_tmp_alloc && rx_pkt->page_data.page_order ==
							 IPA_WAN_PAGE_ORDER),
				is_tmp_alloc);
		...
		*notify, bool update_truesize)
{
	...
	/* go over the list backward to save computations on updating length */
		list_for_each_entry_safe_reverse(rx_pkt, tmp, head, link) {
			...
			skb_add_rx_frag(rx_skb, skb_shinfo(rx_skb)->nr_frags,
					rx_page.page, 0, notify->bytes_xfered,
