	...
	for (i = find_first_bit((unsigned long *)pending, gic->irq_nr);
		...
		do {
			...
			if (platform_id == KONA) {
				...
				//#ifdef OPLUS_FEATURE_NWPOWER
				if (strncmp(name, "ipcc_1", strlen("ipcc_1")) ==
				    0) {
					is_first_ipcc_msg = 1;
					oplus_match_modem_wakeup();
				}
			...
			wakeup_reasons_statics(
				name, WS_CNT_MODEM | WS_CNT_WLAN | WS_CNT_ADSP |
					      WS_CNT_CDSP | WS_CNT_SLPI);
		} while (0);
#endif /* OPLUS_FEATURE_POWERINFO_STANDBY */
	}
}
