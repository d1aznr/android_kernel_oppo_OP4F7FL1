...
static void clk_state_subtree(struct clk_core *c)
{
	...
	trace_clk_state(c->name, c->prepare_count, c->enable_count, c->rate,
			vdd_level);
				 ...
				 int level)
{
	...
	seq_printf(s, "%*s%-*s %7d %8d %8d %11lu %10lu %5d %6d\n",
		   level * 3 + 1, "", 30 - level * 3, c->name, c->enable_count,
		   c->prepare_count, c->protect_count, clk_core_get_rate(c),
...
static ssize_t clock_enable_store(struct file *file, const char __user *buffer,
...
static int list_rates_show(struct seq_file *s, void *unused)
{
	 ...
	 */
	while (!IS_ERR_VALUE(
		rate = core->ops->list_rate(core->hw, i++, rate_max))) {
		if (rate <= 0)
			break;
		if (rate <= rate_max)
			seq_printf(s, "%lu\n", rate);
...
static int clk_debug_create_one(struct clk_core *core,
				struct proc_dir_entry *pdentry)
{
	...
	d = proc_create_data("clk_rate", 0444, core->dentry, &clock_rate_fops,
			     core);
	if (!d)
		goto err_out;

	if (core->ops->list_rate) {
		if (!proc_create_data("clk_list_rates", 0444, core->dentry,
				      &list_rates_fops, core))
			goto err_out;
	}

	if (core->vdd_class &&
	    !proc_create_data("clk_rate_max", 0444, core->dentry,
			      &rate_max_fops, core))
		goto err_out;

	d = proc_create_data("clk_accuracy", 0444, core->dentry,
			     &clk_accuracy_fops, core);
	if (!d)
		goto err_out;

	d = proc_create_data("clk_phase", 0444, core->dentry, &clk_phase_fops,
			     core);
	if (!d)
		goto err_out;

	d = proc_create_data("clk_prepare_count", 0444, core->dentry,
			     &clk_prepare_count_fops, core);
	if (!d)
		goto err_out;

	d = proc_create_data("clk_flags", 0444, core->dentry, &clk_flags_fops,
			     core);
	if (!d)
		goto err_out;

	d = proc_create_data("clk_enable_count", 0444, core->dentry,
			     &clock_enable_fops, core);
	if (!d)
		goto err_out;

	d = proc_create_data("clk_notifier_count", 0444, core->dentry,
			     &clk_notifier_count_fops, core);
	if (!d)
		goto err_out;

	if (core->num_parents > 1) {
		d = proc_create_data("clk_possible_parents", 0444, core->dentry,
				     &possible_parents_fops, core);
		if (!d)
			goto err_out;
	}

	d = proc_create_data("clk_parent", 0444, core->dentry,
			     &clock_parent_fops, core);
	if (!d)
		goto err_out;

	d = proc_create_data("clk_print_regs", 0444, core->dentry,
...
static ssize_t debug_suspend_store(struct file *file, const char __user *buffer,
				   size_t count, loff_t *ppos)
{
	char buff_temp[8];
	char *end;
...
static int __init oplus_clk_debug_init(void)
{
	...
	d = proc_create_data("clk_summary", 0444, rootdir, &clk_summary_fops,
			     &all_lists);
	if (!d)
		return -ENOMEM;

	d = proc_create_data("clk_dump", 0444, rootdir, &clk_dump_fops,
			     &all_lists);
	if (!d)
		return -ENOMEM;

	d = proc_create_data("clk_enabled_list", 0444, rootdir,
			     &clk_enabled_list_fops, &clk_debug_list);
	if (!d)
		return -ENOMEM;
	...
	d = proc_create_data("trace_clocks", 0444, rootdir, &clk_state_fops,
			     &all_lists);
	if (!d)
		return -ENOMEM;
...
static int __clk_core_init(struct clk_core *core)
{
	 ...
	 */
	if (core->ops->recalc_accuracy)
		core->accuracy = core->ops->recalc_accuracy(
			core->hw, __clk_get_accuracy(core->parent));
	else if (core->parent)
