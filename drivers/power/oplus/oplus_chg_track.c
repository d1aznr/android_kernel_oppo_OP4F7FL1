...
#endif
static int
oplus_chg_track_get_charger_type(struct oplus_chg_chip *chip,
				 struct oplus_chg_track_status *track_status,
				 int type);
...
static struct oplus_chg_track_vooc_type vooc_type_table[] = {
	...
	{ 0x6b, TRACK_INPUT_VOL_MV(11000), TRACK_INPUT_CURR_MA(11000),
	  "svooc" },
	{ 0x32, TRACK_INPUT_VOL_MV(11000), TRACK_INPUT_CURR_MA(11000),
	  "svooc" },
};
...
oplus_chg_wired_get_break_sub_crux_info(char *crux_info)
{
	return 0;
}
...
static int
oplus_chg_track_get_base_type_info(int charge_type,
				   struct oplus_chg_track_status *track_status)
{
	int i;
...
int oplus_chg_track_get_mos_err_reason(int err_type, char *err_reason, int len)
{
	...
	for (i = 0; i < ARRAY_SIZE(mos_err_reason_table); i++) {
		if (mos_err_reason_table[i].err_type == err_type) {
			strncpy(err_reason, mos_err_reason_table[i].err_name,
				len);
			charge_index = i;
			break;
		}
...
int oplus_chg_track_get_i2c_err_reason(int err_type, char *err_reason, int len)
{
	...
	for (i = 0; i < ARRAY_SIZE(i2c_err_reason_table); i++) {
		if (i2c_err_reason_table[i].err_type == err_type) {
			strncpy(err_reason, i2c_err_reason_table[i].err_name,
				len);
			charge_index = i;
			break;
		}
...
int oplus_chg_track_get_wls_trx_err_reason(int err_type, char *err_reason,
					   int len)
{
	int i;
	int charge_index = -EINVAL;
	...
	for (i = 0; i < ARRAY_SIZE(wls_trx_err_reason_table); i++) {
		if (wls_trx_err_reason_table[i].err_type == err_type) {
			strncpy(err_reason,
				wls_trx_err_reason_table[i].err_name, len);
			charge_index = i;
			break;
		}
	}
...
int oplus_chg_track_get_gpio_err_reason(int err_type, char *err_reason, int len)
{
	...
	for (i = 0; i < ARRAY_SIZE(gpio_err_reason_table); i++) {
		if (gpio_err_reason_table[i].err_type == err_type) {
			strncpy(err_reason, gpio_err_reason_table[i].err_name,
				len);
			charge_index = i;
			break;
		}
...
int oplus_chg_track_get_pmic_err_reason(int err_type, char *err_reason, int len)
{
	...
	for (i = 0; i < ARRAY_SIZE(pmic_err_reason_table); i++) {
		if (pmic_err_reason_table[i].err_type == err_type) {
			strncpy(err_reason, pmic_err_reason_table[i].err_name,
				len);
			charge_index = i;
			break;
		}
...
int oplus_chg_track_get_cp_err_reason(int err_type, char *err_reason, int len)
{
	...
	for (i = 0; i < ARRAY_SIZE(cp_err_reason_table); i++) {
		if (cp_err_reason_table[i].err_type == err_type) {
			strncpy(err_reason, cp_err_reason_table[i].err_name,
				len);
			charge_index = i;
			break;
		}
...
int oplus_chg_track_get_gague_err_reason(int err_type, char *err_reason,
					 int len)
{
	int i;
	int charge_index = -EINVAL;
	...
	for (i = 0; i < ARRAY_SIZE(gague_err_reason_table); i++) {
		if (gague_err_reason_table[i].err_type == err_type) {
			strncpy(err_reason, gague_err_reason_table[i].err_name,
				len);
			charge_index = i;
			break;
		}
...
static int
oplus_chg_track_record_general_info(struct oplus_chg_chip *chip,
				    struct oplus_chg_track_status *track_status,
				    ...
				    int index)
{
	...
	index += snprintf(
		...
		chip->charging_state, chip->total_time, chip->temperature,
		chip->batt_volt, chip->batt_volt_min, chip->icharging,
		chip->ibus, chip->soc, chip->smooth_soc, chip->ui_soc,
		chip->soc_load, chip->batt_rm, chip->batt_fcc, chip->vbatt_over,
		chip->chging_over_time, chip->vchg_status, chip->tbatt_status,
		chip->stop_voter, chip->notify_code, chip->sw_full,
		chip->hw_full_by_sw, chip->hw_full, chip->otg_switch,
		chip->mmi_chg, chip->boot_reason, chip->boot_mode,
...
static void
oplus_chg_track_record_charger_info(struct oplus_chg_chip *chip,
				    oplus_chg_track_trigger *p_trigger_data,
				    struct oplus_chg_track_status *track_status)
{
	int index = 0;
	...
	oplus_chg_track_record_general_info(chip, track_status, p_trigger_data,
					    index);
}
...
DEFINE_DEBUGFS_ATTRIBUTE(adsp_track_debug_ops, adsp_track_debug_get,
			 adsp_track_debug_set, "%lld\n");
...
static int oplus_chg_adsp_track_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		count = kfifo_out_spinlocked(&chip->adsp_fifo, adsp_data,
					     sizeof(adsp_track_trigger),
					     &adsp_fifo_lock);
		if (count != sizeof(adsp_track_trigger)) {
		...
		if (oplus_chg_track_adsp_to_ap_type_mapping(
			    adsp_data->adsp_type_reason,
			    &(ap_data->type_reason)) >= 0 &&
		    oplus_chg_track_adsp_to_ap_flag_mapping(
			    adsp_data->adsp_flag_reason,
			    &(ap_data->flag_reason)) >= 0) {
			index = 0;
			memset(ap_data->crux_info, 0,
			       sizeof(ap_data->crux_info));
			index += snprintf(&(ap_data->crux_info[index]),
					  OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
					  "%s", adsp_data->adsp_crux_info);
...
static int oplus_chg_adsp_track_thread_init(struct oplus_chg_track *track_dev)
{
	...
	chip->adsp_track_upload_kthread = kthread_run(
		oplus_chg_adsp_track_thread, chip, "adsp_track_upload_kthread");
	...
	struct oplus_chg_track_status *track_status, int type)
{
	...
	if (strlen(track_status->power_info.wired_info.adapter_type) &&
	    ...
	    strcmp(track_status->power_info.wired_info.adapter_type,
		   "unknow")) {
...
static int
oplus_chg_track_check_chg_abnormal(struct oplus_chg_chip *chip,
				   struct oplus_chg_track_status *track_status)
{
	int notify_code;
...
static int
oplus_chg_track_cal_chg_common_mesg(struct oplus_chg_chip *chip,
				    struct oplus_chg_track_status *track_status)
{
...
static int
oplus_chg_track_cal_cool_down_stats(struct oplus_chg_chip *chip,
				    struct oplus_chg_track_status *track_status)
{
	int cool_down_max;
	int curr_time;
...
static int
oplus_chg_track_cal_rechg_counts(struct oplus_chg_chip *chip,
				 struct oplus_chg_track_status *track_status)
...
static int
oplus_chg_track_cal_led_on_stats(struct oplus_chg_chip *chip,
				 struct oplus_chg_track_status *track_status)
{
	int curr_time;
	bool chg_start;
...
static bool
oplus_chg_track_wired_fastchg_good_exit_code(struct oplus_chg_track *track_chip)
{
	bool ret = true;
	int code;
	struct oplus_chg_track_status *track_status;
...
int oplus_chg_track_check_wired_charging_break(int vbus_rising)
{
	...
	if (vbus_rising && (pre_vbus_rising != vbus_rising)) {
		...
		track_status->chg_attach_time_ms =
			local_clock() / TRACK_LOCAL_T_NS_TO_MS_THD;
		fastchg_code_ok = oplus_chg_track_wired_fastchg_good_exit_code(
			track_chip);
...
void oplus_chg_track_record_cv_start_time(void)
{
	...
	track_chip = g_track_chip;
	track_chip->track_status.cv_start_time =
		oplus_chg_track_get_local_time_s();
}
...
static int
oplus_chg_track_cal_tbatt_status(struct oplus_chg_chip *chip,
				 struct oplus_chg_track_status *track_status)
{
...
static int
oplus_chg_track_cal_batt_full_time(struct oplus_chg_chip *chip,
				   struct oplus_chg_track_status *track_status)
{
...
static int oplus_chg_track_obtain_action_info(struct oplus_chg_chip *chip,
					      char *action_info, int len)
{
	int index = 0;
	bool tx_mode;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	...
	if (chip->charger_type &&
		...
		if (chip->otg_online)
			index += snprintf(&(action_info[index]), len - index,
					  "%s", TRACK_ACTION_WRX_OTG);
		else
			index += snprintf(&(action_info[index]), len - index,
					  "%s", TRACK_ACTION_WRX);
		...
		if (tx_mode)
			index += snprintf(&(action_info[index]), len - index,
					  "%s", TRACK_ACTION_WTX_WIRED);
		else
			index += snprintf(&(action_info[index]), len - index,
					  "%s", TRACK_ACTION_WIRED);
	} else if (tx_mode) {
		if (chip->otg_online)
			index += snprintf(&(action_info[index]), len - index,
					  "%s", TRACK_ACTION_WTX_OTG);
		else
			index += snprintf(&(action_info[index]), len - index,
					  "%s", TRACK_ACTION_WTX);
	} else if (chip->otg_online) {
		index += snprintf(&(action_info[index]), len - index, "%s",
				  TRACK_ACTION_OTG);
	} else {
		index += snprintf(&(action_info[index]), len - index, "%s",
				  TRACK_ACTION_OTHER);
...
int oplus_chg_track_obtain_power_info(char *power_info, int len)
{
	...
	oplus_chg_track_obtain_action_info(chip, action_info,
					   sizeof(action_info));
	...
	index += snprintf(&(power_info[index]),
			  OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "$$action@@%s",
			  action_info);
...
static int
oplus_chg_track_cal_chg_end_soc(struct oplus_chg_chip *chip,
				struct oplus_chg_track_status *track_status)
{
	if (!track_status || !chip)
		return -EFAULT;
...
static int oplus_chg_need_record(struct oplus_chg_chip *chip,
				 struct oplus_chg_track *track_chip)
{
...
static int oplus_chg_track_debugfs_init(struct oplus_chg_track *track_dev)
{
	...
	debugfs_adsp =
		debugfs_create_file("adsp_debug", S_IFREG | 0444, debugfs_root,
				    track_dev, &adsp_track_debug_ops);
	if (!debugfs_adsp) {
		ret = -ENOENT;
		return ret;
	}
...
static int oplus_chg_track_driver_probe(struct platform_device *pdev)
{
	...
	rc = kfifo_alloc(&(track_dev->adsp_fifo),
			 (ADSP_TRACK_FIFO_NUMS * sizeof(adsp_track_trigger)),
			 GFP_KERNEL);
	if (rc) {
