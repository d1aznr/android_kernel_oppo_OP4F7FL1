...
static int oplus_to_power_supply_type[] = {
	...
	POWER_SUPPLY_TYPE_USB_PD,  POWER_SUPPLY_TYPE_USB,
				...
				union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		rc = oplus_mms_get_item_data(
			chip->gauge_topic, GAUGE_ITEM_CURR, &data,
				...
				const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		...
		if (chip->wired_online &&
			...
			if (chip->led_on) {
				vote(chip->wired_icl_votable, HIDL_VOTER, false,
				     0, true);
				if (is_vooc_curr_votable_available(chip))
					vote(chip->vooc_curr_votable,
					     HIDL_VOTER, false, 0, false);
			} else {
				vote(chip->wired_icl_votable, HIDL_VOTER,
				     (val == 0) ? false : true, val, true);
				if (is_vooc_curr_votable_available(chip))
					vote(chip->vooc_curr_votable,
					     HIDL_VOTER,
...
static void oplus_gki_subscribe_gauge_topic(struct oplus_mms *topic,
					    void *prv_data)
{
	...
	chip->gauge_subs = oplus_mms_subscribe(
		chip->gauge_topic, chip, oplus_gki_gauge_subs_callback, "gki");
	...
	chip->batt_psy = devm_power_supply_register(&topic->dev, &batt_psy_desc,
						    &psy_cfg);
	...
	case MSG_TYPE_ITEM:
		...
		case WIRED_ITEM_ONLINE:
			...
			if (!chip->wired_online) {
				...
				usb_psy_desc.type = oplus_to_power_supply_type
					[chip->wired_type];
				if ((usb_psy_desc.type ==
...
static void oplus_gki_subscribe_wired_topic(struct oplus_mms *topic,
					    void *prv_data)
{
	struct oplus_gki_device *chip = prv_data;
	struct power_supply_config psy_cfg = {};
	...
	chip->wired_subs = oplus_mms_subscribe(
		chip->wired_topic, chip, oplus_gki_wired_subs_callback, "gki");
	...
	if (chip->wired_online) {
		usb_psy_desc.type =
			oplus_to_power_supply_type[chip->wired_type];
...
static void oplus_gki_subscribe_wls_topic(struct oplus_mms *topic,
					  void *prv_data)
{
	struct oplus_gki_device *chip = prv_data;
	struct power_supply_config psy_cfg = {};
	...
	chip->wls_subs = oplus_mms_subscribe(
		chip->wls_topic, chip, oplus_gki_wls_subs_callback, "gki");
	...
	psy_cfg.drv_data = chip;
	chip->wls_psy = devm_power_supply_register(&topic->dev, &batt_psy_desc,
						   &psy_cfg);
	...
	case MSG_TYPE_ITEM:
		...
		case COMM_ITEM_LED_ON:
			...
			if (chip->led_on &&
				...
				vote(chip->vooc_curr_votable, HIDL_VOTER, false,
				     0, false);
			break;
					   ...
					   void *prv_data)
{
	...
	chip->comm_subs = oplus_mms_subscribe(
		chip->comm_topic, chip, oplus_gki_comm_subs_callback, "gki");
...
static __init int oplus_chg_gki_init(void)
{
	...
	oplus_mms_wait_topic("wireless", oplus_gki_subscribe_wls_topic,
			     gki_dev);
