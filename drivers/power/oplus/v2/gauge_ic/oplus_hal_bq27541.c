...
static __inline__ void bq27541_push_i2c_err(struct chip_bq27541 *chip,
					    bool read)
{
...
oplus_vooc_get_fastchg_started_pfunc(int (*pfunc)(void));
...
int oplus_vooc_get_fastchg_started(void)
{
	...
	rc = oplus_mms_get_item_data(vooc_topic, VOOC_ITEM_VOOC_STARTED, &data,
				     true);
	if (!rc)
...
int bq27541_i2c_txsubcmd(int cmd, int writeData)
{
	...
	if (rc < 0) {
		while (retry > 0) {
			usleep_range(5000, 5000);
			rc = i2c_smbus_write_word_data(gauge_ic->client, cmd,
						       writeData);
			if (rc < 0) {
				retry--;
			} else {
	...
	if (rc < 0) {
		while (retry > 0) {
			...
			rc = i2c_smbus_write_i2c_block_data(
				gauge_ic->client, cmd, length, writeData);
			if (rc < 0) {
				retry--;
			} else {
				break;
	...
	if (rc < 0) {
		while (retry > 0) {
			...
			rc = i2c_smbus_read_i2c_block_data(
				gauge_ic->client, cmd, length, returnData);
			if (rc < 0) {
				retry--;
			} else {
				break;
	...
	*returnData = i2c_smbus_read_byte_data(gauge_ic->client, cmd);
	if (*returnData < 0) {
		while (retry > 0) {
			usleep_range(5000, 5000);
			*returnData =
				i2c_smbus_read_byte_data(gauge_ic->client, cmd);
			if (*returnData < 0) {
				retry--;
			} else {
				break;
...
static int bq27541_i2c_txsubcmd_onebyte(u8 cmd, u8 writeData)
{
	...
	if (rc < 0) {
		while (retry > 0) {
			usleep_range(5000, 5000);
			rc = i2c_smbus_write_byte_data(gauge_ic->client, cmd,
						       writeData);
			if (rc < 0) {
				retry--;
			} else {
...
static int zy0603_read_altmac_block(u16 reg, u8 *buf, u8 offset, u8 length)
{
...
static int oplus_detection_reset_condition(struct oplus_chg_ic_dev *ic_dev,
					   bool *need_reset)
{
	if (gauge_ic && gauge_ic->batt_zy0603 && gauge_ic->zy0603_soc_opt &&
...
static int zy0603_full_access_mode(void)
{
	...
	mutex_lock(&bq28z610_alt_manufacturer_access);
	do {
		bq27541_write_i2c_block(ZY0603_CTRL_REG, 2, full_access_cmd_1);
		pr_info("%s write {0xEF, 0xCD} --> 0x00\n", __func__);
		usleep_range(U_DELAY_1_MS, U_DELAY_1_MS);

		bq27541_write_i2c_block(ZY0603_CTRL_REG, 2, full_access_cmd_2);
...
static int zy0603_disable_ssdf_and_pf(struct chip_bq27541 *chip)
{
	...
	if (!zy0603_unseal()) { /* ok */
		...
		if (gauge_ic->zy0603_soc_opt) {
			/* modelratio */
			bq27541_write_i2c_block(BQ28Z610_REG_CNTL1, 4,
						w_modelratio);
		...
		/* disable PF_EN */
		do {
			bq27541_write_i2c_block(BQ28Z610_REG_CNTL1, 2,
						pf_en_cmd);
				 ...
				 u16 len, const u8 *afi_buf1, char *msg)
{
	...
	for (i = 0; i < len1; i++) {
		...
		do {
			bq27541_write_i2c_block(
				BQ28Z610_REG_CNTL1, 18,
				write_buf); /* addr + valid data */
			usleep_range(U_DELAY_1_MS, U_DELAY_1_MS);
			bq27541_write_i2c_block(
				BQ28Z610_REG_CNTL2, 2,
	...
	if (len2 > 0) { /* tail: len2 < 16 */
		...
		retry = 0;
		do {
			bq27541_write_i2c_block(BQ28Z610_REG_CNTL1, len2 + 2,
						write_buf);
			usleep_range(U_DELAY_1_MS, U_DELAY_1_MS);
			bq27541_write_i2c_block(BQ28Z610_REG_CNTL2, 2,
						write_buf + len2 + 2);
			usleep_range(U_DELAY_5_MS, U_DELAY_5_MS);
			bq27541_read_i2c_block(BQ28Z610_REG_CNTL1, len2 + 2,
					       read_buf);
			for (k = 0; k < len2 + 2; k++) {
				error_flag = 0;
				if (write_buf[k] != read_buf[k]) {
...
int oplus_vooc_get_vooc_by_normal_path(void)
{
	...
	rc = oplus_mms_get_item_data(vooc_topic, VOOC_ITEM_VOOC_BY_NORMAL_PATH,
				     &data, true);
	if (!rc)
...
static int zy0603_protect_check(struct oplus_chg_ic_dev *ic_dev)
{
	...
	if (!gauge_ic || (gauge_ic && !gauge_ic->batt_zy0603) ||
		...
		printk(KERN_ERR "[%s] return for %s\n", __func__,
		       !gauge_ic ? "guage is null" : "is not zy0603 gauge");
		return -1;
	}
	...
	printk(KERN_ERR "[%s] checksum_error[%d %d] qmaxfg_error[%d, %d]\n",
	       __func__, checksum_error_flag, checksum_error, qmaxfg_error_flag,
	       qmax_fgstatus_error);
			...
			do {
				...
				if (GUAGE_OK !=
					    zy0603_disable_ssdf_and_pf_check(
						    chip) ||
				    GUAGE_OK != zy0603_check_ssdf_pf_checksum(
							chip)) {
					retry_cnt++;
					error_flag = 1;
...
int oplus_vooc_check_bcc_max_curr(void)
{
	...
	rc = oplus_mms_get_item_data(vooc_topic, VOOC_ITEM_GET_BCC_MAX_CURR,
				     &data, true);
	if (!rc)
...
int oplus_vooc_check_bcc_min_curr(void)
{
	...
	rc = oplus_mms_get_item_data(vooc_topic, VOOC_ITEM_GET_BCC_MIN_CURR,
				     &data, true);
	if (!rc)
...
int oplus_vooc_get_bcc_exit_curr(void)
{
	...
	rc = oplus_mms_get_item_data(vooc_topic, VOOC_ITEM_GET_BCC_STOP_CURR,
				     &data, true);
	if (!rc)
...
int oplus_vooc_check_bcc_temp(void)
{
	...
	rc = oplus_mms_get_item_data(comm_topic, COMM_ITEM_SHELL_TEMP, &data,
				     true);
	if (!rc)
...
int oplus_vooc_check_bcc_temp_range(void)
{
	...
	rc = oplus_mms_get_item_data(vooc_topic, VOOC_ITEM_GET_BCC_TEMP_RANGE,
				     &data, true);
	if (!rc)
...
int oplus_vooc_get_fastchg_ing(void)
{
	...
	rc = oplus_mms_get_item_data(vooc_topic, VOOC_ITEM_VOOC_CHARGING, &data,
				     true);
	if (!rc)
...
int oplus_vooc_get_fast_chg_type(void)
{
	...
	rc = oplus_mms_get_item_data(vooc_topic, VOOC_ITEM_GET_BCC_SVOOC_TYPE,
				     &data, true);
	if (!rc)
...
static int bq28z610_get_battery_bcc_parameters(struct oplus_chg_ic_dev *ic_dev,
					       char *buf)
{
	...
	if (oplus_vooc_get_fastchg_ing() &&
	    oplus_vooc_get_fast_chg_type() == BCC_TYPE_IS_SVOOC) {
		buffer[15] = 1;
	} else {
		buffer[15] = 0;
	}
...
static int
bq28z610_get_fastchg_battery_bcc_parameters(struct oplus_chg_ic_dev *ic_dev,
					    char *buf)
{
	unsigned int buffer[BCC_PARMS_COUNT] = { 0 };
	struct oplus_mms *wired_topic;
...
static int
bq28z610_get_prev_battery_bcc_parameters(struct oplus_chg_ic_dev *ic_dev,
					 char *buf)
{
	int i = 0;
	int idx = 0;
	...
	wired_topic = oplus_mms_get_by_name("wired");
	gauge_ic->bcc_buf[16] =
		oplus_wired_get_bcc_curr_done_status(wired_topic);
...
static int bq28z610_set_bcc_debug_parameters(struct oplus_chg_ic_dev *ic_dev,
					     const char *buf)
{
	int ret = 0;
#ifdef BCC_SET_DEBUG_PARMS
