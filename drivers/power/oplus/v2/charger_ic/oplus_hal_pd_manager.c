...
static void tcpc_set_voltage_max_and_min(struct pd_manager_chip *chip, int max,
					 int min)
{
...
static void pd_sink_set_vol_and_cur(struct pd_manager_chip *chip, int mv,
				    int ma, uint8_t type)
{
	unsigned long sel = 0;
	int max, min;
...
static int tcpc_pd_state_change(struct pd_manager_chip *chip,
...
static int oplus_pdc_setup(struct pd_manager_chip *chip, int *vbus_mv,
			   int *ibus_ma)
{
	int ret = 0;
	int vbus_mv_t = 0;
...
static int oplus_pdo_select(struct pd_manager_chip *chip, int vbus_mv,
			    int ibus_ma)
{
	...
	if (chip->pd_type == OPLUS_CHG_USB_TYPE_PD_PPS) {
		while (1) {
			ret = tcpm_inquire_pd_source_apdo(
				chip->tcpc, TCPM_POWER_CAP_APDO_TYPE_PPS,
				&cap_i, &apdo_cap);
...
static int pd_manager_is_oplus_svid(struct oplus_chg_ic_dev *ic_dev,
				    bool *oplus_svid)
{
	struct pd_manager_chip *chip;
...
static void tcpc_subscribe_wired_topic(struct oplus_mms *topic, void *prv_data)
{
	...
	chip->wired_subs = oplus_mms_subscribe(
		chip->wired_topic, chip, tcpc_wired_subs_callback, "tcpc");
