...
int oplus_chg_show_vooc_logo_ornot(void)
{
	...
	rc = oplus_mms_get_item_data(vooc_topic, VOOC_ITEM_ONLINE, &data,
				     false);
	if (!rc)
		online = data.intval;

	rc = oplus_mms_get_item_data(vooc_topic, VOOC_ITEM_ONLINE_KEEP, &data,
				     false);
	if (!rc)
...
int oplus_get_prop_status(void)
{
	...
	rc = oplus_mms_get_item_data(comm_topic, COMM_ITEM_BATT_STATUS, &data,
				     true);
	if (!rc)
...
int oplus_chg_get_notify_flag(void)
{
	...
	rc = oplus_mms_get_item_data(comm_topic, COMM_ITEM_NOTIFY_FLAG, &data,
				     false);
	if (!rc)
...
int oplus_chg_get_ui_soc(void)
{
	...
	rc = oplus_mms_get_item_data(comm_topic, COMM_ITEM_UI_SOC, &data,
				     false);
	if (!rc) {
		ui_soc = data.intval;
		if (ui_soc < 0) {
...
static void mtk_charger_parse_dt(struct mtk_charger *info, struct device *dev)
{
	...
	info->support_ntc_01c_precision =
		of_property_read_bool(np, "oplus,support_ntc_01c_precision");
...
static void mtk_charger_start_timer(struct mtk_charger *info)
{
	...
	chr_err("alarm timer start:%d, %ld %ld\n", ret, info->endtime.tv_sec,
		info->endtime.tv_nsec);
				...
				void *data)
{
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case PD_CONNECT_NONE:
			pinfo->pd_type = MTK_PD_CONNECT_NONE;
			chr_err("PD Notify Detach\n");
			oplus_chg_ic_virq_trigger(
				pinfo->ic_dev, OPLUS_IC_VIRQ_CHG_TYPE_CHANGE);
			break;
		...
		case PD_CONNECT_TYPEC_ONLY_SNK:
			...
			oplus_chg_ic_virq_trigger(
				pinfo->ic_dev,
				OPLUS_IC_VIRQ_SVID); /* not support svid */
			oplus_chg_ic_virq_trigger(
				pinfo->ic_dev, OPLUS_IC_VIRQ_CHG_TYPE_CHANGE);
			break;
		}
	...
	case TCP_NOTIFY_WD0_STATE:
		...
		oplus_chg_ic_virq_trigger(pinfo->ic_dev,
...
static int mtk_chg_smt_test(struct oplus_chg_ic_dev *ic_dev, char buf[],
			    int len)
{
	return 0;
}
...
static int mtk_chg_input_is_suspend(struct oplus_chg_ic_dev *ic_dev,
				    bool *suspend)
{
	/* TODO */
	return 0;
...
static int mtk_chg_output_is_suspend(struct oplus_chg_ic_dev *ic_dev,
				     bool *suspend)
{
	/* TODO */
	return 0;
...
static int mtk_chg_set_otg_boost_vol(struct oplus_chg_ic_dev *ic_dev,
				     int vol_mv)
{
	/* TODO */
	return 0;
...
static int mtk_chg_set_otg_boost_curr_limit(struct oplus_chg_ic_dev *ic_dev,
					    int curr_ma)
{
	/* TODO */
	return 0;
...
static int mtk_chg_get_cc_orientation(struct oplus_chg_ic_dev *ic_dev,
				      int *orientation)
{
	struct mtk_charger *chip;
...
static bool oplus_pd_without_usb(void)
{
	...
	if (!tcpm_inquire_pd_connected(tcpc))
		return true;
	return (tcpm_inquire_dpm_flags(tcpc) & DPM_FLAGS_PARTNER_USB_COMM) ?
		       false :
		       true;
}
...
static int mtk_chg_get_charger_type(struct oplus_chg_ic_dev *ic_dev, int *type)
{
	...
	if (!chg_psy || IS_ERR(chg_psy)) {
		...
		chg_psy = devm_power_supply_get_by_phandle(&chip->pdev->dev,
							   "charger");
...
static int mtk_chg_set_qc_config(struct oplus_chg_ic_dev *ic_dev,
...
static int mtk_chg_get_vbus_collapse_status(struct oplus_chg_ic_dev *ic_dev,
					    bool *collapse)
{
	/* TODO */
	return -ENOTSUPP;
...
static int mtk_chg_set_otg_switch_status(struct oplus_chg_ic_dev *ic_dev,
					 bool en)
{
	/* TODO */
	struct mtk_charger *chip;
	...
	tcpm_typec_change_role_postpone(
		pinfo->tcpc, en ? TYPEC_ROLE_DRP : TYPEC_ROLE_SNK, true);

	return 0;
}

static int mtk_chg_get_otg_switch_status(struct oplus_chg_ic_dev *ic_dev,
					 bool *en)
{
	struct mtk_charger *chip;
...
static int mtk_chg_is_oplus_svid(struct oplus_chg_ic_dev *ic_dev,
				 bool *oplus_svid)
{
	struct mtk_charger *chip;
	if (ic_dev == NULL) {
...
static int mtk_chg_set_usbtemp_dischg_enable(struct oplus_chg_ic_dev *ic_dev,
					     bool en)
{
	struct mtk_charger *chip;
	chip = oplus_chg_ic_get_drvdata(ic_dev);
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	pinfo->chargeric_temp_chan =
		devm_iio_channel_get(&pdev->dev, "auxadc5-chargeric_temp");
