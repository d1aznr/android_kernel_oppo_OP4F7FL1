...
static void oplus_voocphy_enable_check_func(struct work_struct *work)
{
	...
	if (chip->mmi_chg == 0 || chip->charger_exist == false ||
		...
		schedule_delayed_work(
			&bcdev->voocphy_enable_check_work,
			round_jiffies_relative(msecs_to_jiffies(5000)));
		return;
	}
				...
				size_t len)
{
	...
	case BC_PD_SVOOC:
#ifdef OPLUS_CHG_UNDEF /* TODO */
		if ((get_oplus_chg_chip() &&
		     get_oplus_chg_chip()->wireless_support == false) ||
...
static unsigned int get_chg_ctl_param_info(struct battery_chg_dev *bcdev)
{
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "!!![voocphy] read USB_VOOC_CHG_PARAM_INFO fail\n");
		return 0;
	}
...
static void oplus_vooc_handle_voocphy_status(void)
{
	...
	if (!chip || !bcdev) {
		printk(KERN_ERR
		       "!!!chip or bcdev null, voocphy non handle status: [%d]\n",
		       intval);
		return;
	}
	...
	if ((intval & 0xFF) == FAST_NOTIFY_PRESENT) {
		...
		printk(KERN_ERR
		       "!!![voocphy] fastchg start: [%d], adapter version: [0x%0x]\n",
		       chip->voocphy.fastchg_start,
		       chip->voocphy.fast_chg_type);
		if (pst_batt->psy)
			power_supply_changed(pst_batt->psy);
		...
		chip->voocphy.fast_chg_type = ((intval >> 8) & 0x7F);
		printk(KERN_ERR
		       "!!![voocphy] fastchg dummy start: [%d], adapter version: [0x%0x]\n",
		       chip->voocphy.fastchg_dummy_start,
		       chip->voocphy.fast_chg_type);
		if (pst_batt->psy)
			power_supply_changed(pst_batt->psy);
		...
		if (chip->voocphy.fast_chg_type ==
			...
			if (rc < 0) {
				printk(KERN_ERR
				       "!!![voocphy] read fastchg type fail\n");
				return;
			}
			chip->voocphy.fast_chg_type =
				(pst->prop[USB_VOOC_FAST_CHG_TYPE]) & 0x7F;
		...
		if (chip->chg_ops->get_charging_enable() == true) {
		   ...
		   (intval & 0xFF) == FAST_NOTIFY_BAD_CONNECTED) {
		chip->voocphy.fastchg_start = false;
		chip->voocphy.fastchg_to_warm = false;
		chip->voocphy.fastchg_dummy_start = false;
		...
		printk(KERN_ERR
		       "!!![voocphy] fastchg switch temp range: [%d]\n",
		       intval);
		...
		oplus_chg_wake_update_work();
		printk(KERN_ERR
		       "!!![voocphy] fastchg hw vbatt || tbatt high: [%d]\n",
		       intval);
...
static int battery_chg_parse_dt(struct battery_chg_dev *bcdev)
{
	...
	of_property_read_string(node, "qcom,wireless-fw-name",
				&bcdev->wls_fw_name);
	bcdev->oem_lcm_check =
		of_property_read_bool(node, "oplus,oem-lcm-check");
	bcdev->voocphy_support =
		of_property_read_bool(node, "oplus,voocphy_support");
...
static void oplus_voocphy_err_work(struct work_struct *work)
{
	...
	if (!chip || !bcdev) {
		printk(KERN_ERR
		       "!!!chip or bcdev null, oplus_voocphy_err_work\n");
		return;
	}
...
static int oplus_chg_8350_smt_test(struct oplus_chg_ic_dev *ic_dev, char buf[],
				   int len)
{
	return 0;
}

static int oplus_chg_8350_input_present(struct oplus_chg_ic_dev *ic_dev,
					bool *present)
{
	struct battery_chg_dev *bcdev;
	int prop_id = 0;
...
static int oplus_chg_8350_input_suspend(struct oplus_chg_ic_dev *ic_dev,
					bool suspend)
{
	struct battery_chg_dev *bcdev;
	int prop_id = 0;
...
static int oplus_chg_8350_input_is_suspend(struct oplus_chg_ic_dev *ic_dev,
					   bool *suspend)
{
	return 0;
}

static int oplus_chg_8350_output_suspend(struct oplus_chg_ic_dev *ic_dev,
					 bool suspend)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
static int oplus_chg_8350_output_is_suspend(struct oplus_chg_ic_dev *ic_dev,
					    bool *suspend)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
static int oplus_chg_8350_set_iterm(struct oplus_chg_ic_dev *ic_dev,
				    int iterm_ma)
{
	return 0;
}

static int oplus_chg_8350_set_rechg_vol(struct oplus_chg_ic_dev *ic_dev,
					int vol_mv)
{
	return 0;
}

static int oplus_chg_8350_get_input_curr(struct oplus_chg_ic_dev *ic_dev,
					 int *curr_ma)
{
	struct battery_chg_dev *bcdev;
	int prop_id = 0;
...
static int oplus_chg_8350_get_input_vol(struct oplus_chg_ic_dev *ic_dev,
					int *vol_mv)
{
	struct battery_chg_dev *bcdev;
	int prop_id = 0;
...
static int oplus_chg_8350_otg_boost_enable(struct oplus_chg_ic_dev *ic_dev,
					   bool en)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
	...
	bcdev = oplus_chg_ic_get_drvdata(ic_dev);
	pst = &bcdev->psy_list[PSY_TYPE_USB];
	rc = write_property_id(bcdev, pst, USB_OTG_VBUS_REGULATOR_ENABLE,
			       en ? 1 : 0);
	if (rc) {
...
static int oplus_chg_8350_set_otg_boost_vol(struct oplus_chg_ic_dev *ic_dev,
					    int vol_mv)
{
	return 0;
}
...
static int
oplus_chg_8350_set_otg_boost_curr_limit(struct oplus_chg_ic_dev *ic_dev,
					int curr_ma)
{
	return 0;
}
...
static int oplus_chg_8350_get_cc_orientation(struct oplus_chg_ic_dev *ic_dev,
					     int *orientation)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
static int oplus_chg_8350_get_hw_detect(struct oplus_chg_ic_dev *ic_dev,
					int *detected)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
static int oplus_chg_8350_get_charger_type(struct oplus_chg_ic_dev *ic_dev,
					   int *type)
{
	struct battery_chg_dev *bcdev;
	int prop_id = 0;
...
static int oplus_chg_8350_qc_detect_enable(struct oplus_chg_ic_dev *ic_dev,
					   bool en)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
#define PWM_COUNT 5
static int oplus_chg_8350_shipmode_enable(struct oplus_chg_ic_dev *ic_dev,
					  bool en)
{
	struct battery_chg_dev *bcdev;
...
static int oplus_chg_8350_set_qc_config(struct oplus_chg_ic_dev *ic_dev,
...
static int oplus_chg_8350_set_pd_config(struct oplus_chg_ic_dev *ic_dev,
					u32 pdo)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
static int
oplus_chg_8350_get_props_from_adsp_by_buffer(struct oplus_chg_ic_dev *ic_dev)
{
	oplus_get_props_from_adsp_by_buffer();
	return 0;
}

static int oplus_chg_8350_gauge_is_suspend(struct oplus_chg_ic_dev *ic_dev,
					   bool *suspend)
{
	struct battery_chg_dev *bcdev;
...
static int oplus_chg_8350_voocphy_enable(struct oplus_chg_ic_dev *ic_dev,
					 bool en)
{
...
static int oplus_chg_8350_get_charger_cycle(struct oplus_chg_ic_dev *ic_dev,
					    int *cycle)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
static int oplus_chg_8350_wls_boost_enable(struct oplus_chg_ic_dev *ic_dev,
					   bool en)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
static int oplus_chg_8350_set_wls_boost_vol(struct oplus_chg_ic_dev *ic_dev,
					    int vol_mv)
{
	return 0;
}
...
static int
oplus_chg_8350_set_wls_boost_curr_limit(struct oplus_chg_ic_dev *ic_dev,
					int curr_ma)
{
	return 0;
}

static int oplus_chg_8350_get_shutdown_soc(struct oplus_chg_ic_dev *ic_dev,
					   int *soc)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
static int
oplus_chg_8350_get_vbus_collapse_status(struct oplus_chg_ic_dev *ic_dev,
					bool *collapse)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
static int
oplus_chg_8350_get_typec_mode(struct oplus_chg_ic_dev *ic_dev,
...
static int
oplus_chg_8350_set_typec_mode(struct oplus_chg_ic_dev *ic_dev,
			      enum oplus_chg_typec_port_role_type mode)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
static int oplus_chg_8350_set_curr_level(struct oplus_chg_ic_dev *ic_dev,
					 int cool_down)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
static int oplus_chg_8350_set_match_temp(struct oplus_chg_ic_dev *ic_dev,
					 int match_temp)
{
	struct battery_chg_dev *bcdev;
	struct psy_state *pst = NULL;
...
static int oplus_chg_8350_get_otg_enbale(struct oplus_chg_ic_dev *ic_dev,
					 bool *enable)
{
	struct battery_chg_dev *bcdev;
...
static int oplus_sm8350_smt_test(struct oplus_chg_ic_dev *ic_dev, char buf[],
				 int len)
{
	return 0;
}

static int oplus_sm8350_get_batt_vol(struct oplus_chg_ic_dev *ic_dev, int index,
				     int *vol_mv)
{
...
static int oplus_sm8350_ic_register(struct battery_chg_dev *bcdev)
{
	...
	for_each_child_of_node (bcdev->dev->of_node, child) {
		...
		case OPLUS_CHG_IC_BUCK:
			...
			ic_cfg.virq_num =
				ARRAY_SIZE(oplus_chg_8350_buck_virq_table);
			break;
		case OPLUS_CHG_IC_GAUGE:
			...
			ic_cfg.virq_num =
				ARRAY_SIZE(oplus_chg_8350_gauge_virq_table);
			break;
		default:
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	rc = of_property_read_u32(dev->of_node, "oplus,batt_num",
				  &bcdev->batt_num);
	if (rc < 0) {
