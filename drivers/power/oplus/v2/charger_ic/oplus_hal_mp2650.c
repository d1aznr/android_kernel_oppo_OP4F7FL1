...
int mp2650_mms_set_vindpm_vol(struct oplus_chg_ic_dev *ic_dev,
			      int vol_mv) /* default 4.5V */
{
	int rc;
	int tmp = 0;
	struct chip_mp2650 *chip = charger_ic;
...
int mp2650_set_aicl_point(struct oplus_chg_ic_dev *ic_dev, int vbatt)
{
	...
	if (chip->hw_aicl_point == HW_AICL_POINT_LOW &&
...
static int mp2650_input_current_limit_without_aicl(struct chip_mp2650 *chip,
						   int current_ma)
{
	int rc = 0;
	int reg_val = 0;
...
static int mp2650_gpio_init(struct chip_mp2650 *chip)
{
	...
	if (gpio_is_valid(chip->acok_gpio)) {
		...
		rc = devm_request_irq(
			chip->dev, chip->acok_irq, mp2650_acok_handler,
...
static int mp2650_set_icl(struct oplus_chg_ic_dev *ic_dev, bool vooc_mode,
	...
	if (pre_icl_index < 2) {
		...
		for (i = pre_icl_index - 1; i > 0; i--) {
			...
			case 1:
				rc = mp2650_config_interface(
					REG00_MP2650_ADDRESS,
					REG00_MP2650_1ST_CURRENT_LIMIT_900MA,
					REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
				rc = mp2650_config_interface(
					REG0F_MP2650_ADDRESS,
					REG00_MP2650_1ST_CURRENT_LIMIT_900MA,
