...
static void oplus_monitor_update_charge_info(struct oplus_monitor *chip)
{
	...
	if (is_wired_charge_suspend_votable_available(chip))
		chip->wired_suspend = get_effective_result(
			chip->wired_charge_suspend_votable);
	if (is_wired_charge_suspend_votable_available(chip))
		chip->wired_user_suspend = get_client_vote(
			chip->wired_charge_suspend_votable, USER_VOTER);
	...
	if (is_wls_charge_suspend_votable_available(chip))
		chip->wls_suspend =
			get_effective_result(chip->wls_charge_suspend_votable);
	if (is_wls_charge_suspend_votable_available(chip))
		chip->wls_user_suspend = get_client_vote(
			chip->wls_charge_suspend_votable, USER_VOTER);

	if (chip->wired_online) {
		if (is_wired_fcc_votable_available(chip))
			chip->fcc_ma =
				get_effective_result(chip->wired_fcc_votable);
		...
		if (is_wired_charging_disable_votable_available(chip))
			chip->chg_disable = get_effective_result(
				chip->wired_charging_disable_votable);
		if (is_wired_charging_disable_votable_available(chip))
			chip->chg_user_disable = get_client_vote(
				chip->wired_charging_disable_votable,
				USER_VOTER);
		...
		if (is_wls_fcc_votable_available(chip))
			chip->fcc_ma =
				get_effective_result(chip->wls_fcc_votable);
		if (is_fv_votable_available(chip))
			chip->fv_mv = get_effective_result(chip->fv_votable);
		...
		if (is_wls_charging_disable_votable_available(chip))
			chip->chg_user_disable = get_client_vote(
				chip->wls_charging_disable_votable, USER_VOTER);
...
static void oplus_monitor_charge_info_update_work(struct work_struct *work)
{
	...
	printk(KERN_INFO "OPLUS_CHG[oplus_charge_info]: "
	       ...
	       chip->batt_err_code, chip->fv_mv, chip->fcc_ma,
	       chip->chg_disable, chip->chg_user_disable, chip->wired_online,
	       chip->wired_ibus_ma, chip->wired_vbus_mv, chip->wired_icl_ma,
	       chip->wired_charge_type, chip->wired_err_code,
	       chip->wired_suspend, chip->wired_user_suspend, chip->cc_mode,
	       chip->cc_detect, chip->otg_enable, chip->wls_online,
	       chip->wls_ibus_ma, chip->wls_vbus_mv, chip->wls_icl_ma,
	       chip->wls_charge_type, chip->wls_err_code, chip->wls_suspend,
	       chip->wls_user_suspend, chip->vooc_online, chip->vooc_started,
	       chip->vooc_charging, chip->vooc_online_keep, chip->vooc_sid,
					      ...
					      enum mms_msg_type type, u32 id)
{
	...
	case MSG_TYPE_TIMER:
		...
		oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_SOH,
					&data, false);
	...
	case MSG_TYPE_ITEM:
		...
		case WIRED_ITEM_CHG_TYPE:
			...
			oplus_chg_track_handle_wired_type_info(
				chip, TRACK_CHG_GET_THTS_TIME_TYPE);
			break;
						...
						void *prv_data)
{
	...
	chip->wired_subs =
		oplus_mms_subscribe(chip->wired_topic, chip,
				    oplus_monitor_wired_subs_callback,
				    "monitor");
					       ...
					       void *prv_data)
{
	...
	chip->comm_subs = oplus_mms_subscribe(chip->comm_topic, chip,
					      oplus_monitor_comm_subs_callback,
					      "monitor");
					       ...
					       void *prv_data)
{
	...
	chip->vooc_subs = oplus_mms_subscribe(chip->vooc_topic, chip,
					      oplus_monitor_vooc_subs_callback,
					      "monitor");
...
static int oplus_monitor_topic_init(struct oplus_monitor *chip)
{
	...
	chip->err_topic = devm_oplus_mms_register(
		chip->dev, &oplus_monitor_desc, &mms_cfg);
...
static int oplus_monitor_probe(struct platform_device *pdev)
{
	...
	oplus_mms_wait_topic("wired", oplus_monitor_subscribe_wired_topic,
			     chip);
	oplus_mms_wait_topic("wireless", oplus_monitor_subscribe_wls_topic,
			     chip);
	oplus_mms_wait_topic("vooc", oplus_monitor_subscribe_vooc_topic, chip);
	oplus_mms_wait_topic("common", oplus_monitor_subscribe_comm_topic,
			     chip);
	oplus_mms_wait_topic("gauge", oplus_monitor_subscribe_gauge_topic,
			     chip);
