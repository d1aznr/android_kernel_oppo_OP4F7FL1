...
static int oplus_chg_va_init(struct oplus_chg_ic_dev *ic_dev)
{
	...
	for (i = 0; i < va->asic_num; i++) {
		...
		rc = oplus_chg_ic_func(va->asic_list[i].ic_dev,
				       OPLUS_IC_FUNC_INIT);
...
static int oplus_chg_va_smt_test(struct oplus_chg_ic_dev *ic_dev, char buf[],
				 int len)
{
	struct oplus_virtual_asic_ic *va;
	int rc = 0;
				      ...
				      bool *pass)
{
	...
	rc = oplus_chg_ic_func(va->asic, OPLUS_IC_FUNC_VOOC_CHECK_FW_STATUS,
			       pass);
	if (rc < 0) {
...
static void oplus_va_err_handler(struct oplus_chg_ic_dev *ic_dev,
				 void *virq_data)
{
...
static int oplus_va_virq_register(struct oplus_virtual_asic_ic *chip)
{
	...
	rc = oplus_chg_ic_virq_register(chip->asic, OPLUS_IC_VIRQ_ERR,
					oplus_va_err_handler, chip);
	if (rc < 0)
