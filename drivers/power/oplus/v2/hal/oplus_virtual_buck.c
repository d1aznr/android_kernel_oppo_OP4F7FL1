...
static int
oplus_chg_vb_set_typec_mode(struct oplus_chg_ic_dev *ic_dev,
...
static int oplus_vc_usbtemp_l_gpio_init(struct oplus_virtual_buck_ic *chip)
{
	...
	usbtemp_l_gpio_default = pinctrl_lookup_state(chip->misc_gpio.pinctrl,
	...
	rc = pinctrl_select_state(chip->misc_gpio.pinctrl,
				  usbtemp_l_gpio_default);
	if (rc < 0)
...
static int oplus_vc_usbtemp_r_gpio_init(struct oplus_virtual_buck_ic *chip)
{
	...
	usbtemp_r_gpio_default = pinctrl_lookup_state(chip->misc_gpio.pinctrl,
...
static bool
oplus_vc_usbtemp_check_is_support(struct oplus_virtual_buck_ic *chip)
{
	int i;
...
static int oplus_vc_dischg_gpio_init(struct oplus_virtual_buck_ic *chip)
{
	...
	chip->misc_gpio.dischg_enable =
		pinctrl_lookup_state(chip->misc_gpio.pinctrl, "dischg_enable");
	if (IS_ERR_OR_NULL(chip->misc_gpio.dischg_enable)) {
		chg_err("get dischg_enable fail\n");
		return -EINVAL;
	}

	chip->misc_gpio.dischg_disable =
		pinctrl_lookup_state(chip->misc_gpio.pinctrl, "dischg_disable");
...
static int oplus_vc_usbtemp_iio_init(struct oplus_virtual_buck_ic *chip)
{
	...
	if (rc >= 0) {
		chip->usbtemp_adc_l =
			iio_channel_get(chip->dev, "usb_temp_adc_l");
	...
	if (rc >= 0) {
		chip->usbtemp_adc_r =
			iio_channel_get(chip->dev, "usb_temp_adc_r");
...
static int oplus_vc_ship_gpio_init(struct oplus_virtual_buck_ic *chip)
{
	...
	chip->misc_gpio.ship_active =
		pinctrl_lookup_state(chip->misc_gpio.pinctrl, "ship_active");
	if (IS_ERR_OR_NULL(chip->misc_gpio.ship_active)) {
		chg_err("get ship_active fail\n");
		return -EINVAL;
	}

	chip->misc_gpio.ship_sleep =
		pinctrl_lookup_state(chip->misc_gpio.pinctrl, "ship_sleep");
...
static int oplus_vc_ccdetect_gpio_init(struct oplus_virtual_buck_ic *chip)
{
	...
	chip->misc_gpio.ccdetect_sleep =
		pinctrl_lookup_state(chip->misc_gpio.pinctrl, "ccdetect_sleep");
...
static int
oplus_vc_ccdetect_before_irq_register(struct oplus_virtual_buck_ic *chip)
{
	int level = 1;
...
static int oplus_vc_ccdetect_irq_register(struct oplus_virtual_buck_ic *chip)
{
	...
	rc = devm_request_threaded_irq(
		chip->dev, chip->ccdetect_irq, NULL,
		oplus_vc_ccdetect_change_handler,
...
static int oplus_vc_misc_init(struct oplus_virtual_buck_ic *chip)
{
	...
	chip->misc_gpio.ship_gpio =
		of_get_named_gpio(node, "oplus,ship-gpio", 0);
	...
	chip->misc_gpio.ccdetect_gpio =
		of_get_named_gpio(node, "oplus,ccdetect-gpio", 0);
	if (!gpio_is_valid(chip->misc_gpio.ccdetect_gpio)) {
		chg_err("Couldn't read oplus,ccdetect-gpio, rc=%d\n",
			chip->misc_gpio.ccdetect_gpio);
	} else {
		if (oplus_vc_ccdetect_gpio_support(chip)) {
			rc = gpio_request(chip->misc_gpio.ccdetect_gpio,
					  "ccdetect-gpio");
			if (rc) {
...
static int oplus_vc_child_funcs_init(struct oplus_virtual_buck_ic *chip,
				     int child_num)
{
	struct device_node *node = chip->dev->of_node;
	...
	for (i = 0; i < child_num; i++) {
		func_node =
			of_parse_phandle(node, "oplus,buck_ic_func_group", i);
		if (func_node == NULL) {
...
static int oplus_vc_child_virqs_init(struct oplus_virtual_buck_ic *chip,
				     int child_num)
{
	struct device_node *node = chip->dev->of_node;
...
static int oplus_vc_child_init(struct oplus_virtual_buck_ic *chip)
{
	...
	rc = of_property_read_u32(node, "oplus,main_charger",
				  &chip->main_charger);
	if (rc < 0) {
	...
	for (i = 0; i < chip->child_num; i++) {
		chip->child_list[i].ic_dev =
			of_get_oplus_chg_ic(node, "oplus,buck_ic", i);
...
static int oplus_chg_vb_smt_test(struct oplus_chg_ic_dev *ic_dev, char buf[],
				 int len)
{
	struct oplus_virtual_buck_ic *vb;
	int i, index;
...
static int oplus_chg_vb_input_present(struct oplus_chg_ic_dev *ic_dev,
				      bool *present)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
	...
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_INPUT_PRESENT);
...
static int oplus_chg_vb_input_suspend(struct oplus_chg_ic_dev *ic_dev,
				      bool suspend)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_INPUT_SUSPEND);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
		...
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			if (rc < 0) {
				chg_err("child ic[%d] input %s error, rc=%d\n",
					i,
					suspend_temp ? "suspend" : "unsuspend",
					rc);
				return rc;
			}
		} else if (vb->connect_type == OPLUS_CHG_IC_CONNECT_SERIAL) {
...
static int oplus_chg_vb_input_is_suspend(struct oplus_chg_ic_dev *ic_dev,
					 bool *suspend)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_INPUT_IS_SUSPEND);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_output_suspend(struct oplus_chg_ic_dev *ic_dev,
				       bool suspend)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_OUTPUT_SUSPEND);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	for (i = 0; i < vb->child_num; i++) {
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			rc = oplus_chg_ic_func(
				vb->child_list[i].ic_dev,
				OPLUS_IC_FUNC_BUCK_OUTPUT_SUSPEND, suspend);
			if (rc < 0) {
				chg_err("child ic[%d] output %s error, rc=%d\n",
					i, suspend ? "suspend" : "unsuspend",
					rc);
...
static int oplus_chg_vb_output_is_suspend(struct oplus_chg_ic_dev *ic_dev,
					  bool *suspend)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_OUTPUT_IS_SUSPEND);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
				...
				bool step, int icl_ma)
{
	...
	if (unlikely(data != NULL)) {
		...
		chg_err("overwrite icl_ma=%d, vooc_mode=%s, step=%s\n", icl_ma,
			vooc_mode ? "true" : "false", step ? "true" : "false");
	}
#endif
...
static int oplus_chg_vb_set_rechg_vol(struct oplus_chg_ic_dev *ic_dev,
				      int vol_mv)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_SET_RECHG_VOL);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_get_input_curr(struct oplus_chg_ic_dev *ic_dev,
				       int *curr_ma)
{
	struct oplus_virtual_buck_ic *vb;
	int curr_temp;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_GET_INPUT_CURR);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	*curr_ma = 0;
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		if (vb->connect_type == OPLUS_CHG_IC_CONNECT_PARALLEL) {
			...
			rc = oplus_chg_ic_func(
				vb->child_list[i].ic_dev,
				OPLUS_IC_FUNC_BUCK_GET_INPUT_CURR, &curr_temp);
			if (rc < 0) {
...
static int oplus_chg_vb_get_input_vol(struct oplus_chg_ic_dev *ic_dev,
				      int *vol_mv)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_GET_INPUT_VOL);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_otg_boost_enable(struct oplus_chg_ic_dev *ic_dev,
					 bool en)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
static int oplus_chg_vb_set_otg_boost_vol(struct oplus_chg_ic_dev *ic_dev,
					  int vol_mv)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
static int
oplus_chg_vb_set_otg_boost_curr_limit(struct oplus_chg_ic_dev *ic_dev,
				      int curr_ma)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_SET_OTG_BOOST_CURR_LIMIT);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_get_cc_orientation(struct oplus_chg_ic_dev *ic_dev,
					   int *orientation)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_GET_CC_ORIENTATION);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_get_hw_detect(struct oplus_chg_ic_dev *ic_dev,
				      int *detected)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_GET_HW_DETECT);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_get_charger_type(struct oplus_chg_ic_dev *ic_dev,
					 int *type)
{
	struct oplus_virtual_buck_ic *vb;
	int chg_type;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_GET_CHARGER_TYPE);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_qc_detect_enable(struct oplus_chg_ic_dev *ic_dev,
					 bool en)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_QC_DETECT_ENABLE);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev,
				       OPLUS_IC_FUNC_BUCK_QC_DETECT_ENABLE, en);
		if (rc < 0)
...
static int oplus_chg_vb_shipmod_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_SHIPMODE_ENABLE);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	if (gpio_is_valid(vb->misc_gpio.ship_gpio)) {
		...
		pinctrl_select_state(vb->misc_gpio.pinctrl,
				     vb->misc_gpio.ship_sleep);
		for (i = 0; i < PWM_COUNT; i++) {
			pinctrl_select_state(vb->misc_gpio.pinctrl,
					     vb->misc_gpio.ship_active);
			mdelay(3);
			pinctrl_select_state(vb->misc_gpio.pinctrl,
					     vb->misc_gpio.ship_sleep);
			mdelay(3);
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev,
				       OPLUS_IC_FUNC_BUCK_SHIPMODE_ENABLE, en);
		if (rc < 0)
...
static int oplus_chg_vb_set_qc_config(struct oplus_chg_ic_dev *ic_dev,
				      enum oplus_chg_qc_version version,
				      int vol_mv)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_SET_QC_CONFIG);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_set_pd_config(struct oplus_chg_ic_dev *ic_dev, u32 pdo)
{
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_SET_PD_CONFIG);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev,
				       OPLUS_IC_FUNC_BUCK_SET_PD_CONFIG, pdo);
		if (rc < 0)
...
static int oplus_chg_vb_wls_boost_enable(struct oplus_chg_ic_dev *ic_dev,
					 bool en)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
static int oplus_chg_vb_set_wls_boost_vol(struct oplus_chg_ic_dev *ic_dev,
					  int vol_mv)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
static int
oplus_chg_vb_set_wls_boost_curr_limit(struct oplus_chg_ic_dev *ic_dev,
				      int curr_ma)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_SET_WLS_BOOST_CURR_LIMIT);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_get_charger_cycle(struct oplus_chg_ic_dev *ic_dev,
					  int *cycle)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
static int oplus_common_get_shutdown_soc(struct oplus_virtual_buck_ic *chip,
					 int *get_soc)
{
	u8 *buf = NULL;
	size_t len;
...
static int oplus_common_backup_soc(struct oplus_virtual_buck_ic *chip, int soc)
{
	...
	if (chip->soc_backup_nvmem) {
		...
		rc = nvmem_cell_write(chip->soc_backup_nvmem, &back_soc,
				      sizeof(back_soc));
		if (rc < 0)
...
static int oplus_chg_vb_get_shutdown_soc(struct oplus_chg_ic_dev *ic_dev,
					 int *soc)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
static int
oplus_chg_vb_get_vbus_collapse_status(struct oplus_chg_ic_dev *ic_dev,
				      bool *collapse)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_GET_VBUS_COLLAPSE_STATUS);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_get_usb_temp_volt(struct oplus_chg_ic_dev *ic_dev,
					  int *vol_l, int *vol_r)
{
	struct oplus_virtual_buck_ic *vb;
...
static int
oplus_chg_vb_usb_temp_check_is_support(struct oplus_chg_ic_dev *ic_dev,
				       bool *support)
{
	struct oplus_virtual_buck_ic *vb;
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_USB_TEMP_CHECK_IS_SUPPORT);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int
oplus_chg_vb_get_typec_mode(struct oplus_chg_ic_dev *ic_dev,
			    enum oplus_chg_typec_port_role_type *mode)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev,
				       OPLUS_IC_FUNC_GET_TYPEC_MODE, mode);
		if (rc < 0 && rc != -ENOTSUPP)
				       ...
				       enum oplus_chg_typec_port_role_type mode)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev,
				       OPLUS_IC_FUNC_SET_TYPEC_MODE, mode);
		if (rc < 0)
...
static int oplus_chg_vb_set_usb_dischg_enable(struct oplus_chg_ic_dev *ic_dev,
					      bool en)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_SET_USB_DISCHG_ENABLE);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev,
				       OPLUS_IC_FUNC_SET_USB_DISCHG_ENABLE, en);
		if (rc < 0)
...
static int oplus_chg_vb_get_usb_dischg_status(struct oplus_chg_ic_dev *ic_dev,
					      bool *en)
{
	struct oplus_virtual_buck_ic *vb;
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GET_USB_DISCHG_STATUS);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_set_otg_switch_status(struct oplus_chg_ic_dev *ic_dev,
					      bool en)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_SET_OTG_SWITCH_STATUS);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev,
				       OPLUS_IC_FUNC_SET_OTG_SWITCH_STATUS, en);
		if (rc < 0)
...
static int oplus_chg_vb_get_otg_switch_status(struct oplus_chg_ic_dev *ic_dev,
					      bool *en)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GET_OTG_SWITCH_STATUS);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	for (i = 0; i < vb->child_num; i++) {
		...
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev,
				       OPLUS_IC_FUNC_GET_OTG_SWITCH_STATUS, en);
		if (rc < 0)
...
static int oplus_chg_vb_get_otg_online_status(struct oplus_chg_ic_dev *ic_dev,
					      int *status)
{
	struct oplus_virtual_buck_ic *vb;
	int online = 0;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GET_OTG_ONLINE_STATUS);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
	...
	if (oplus_vc_ccdetect_gpio_support(vb)) {
		...
		if (rc < 0) {
			...
			online = (hw_detect == 1) ? STANDARD_TYPEC_DEV_CONNECT :
						    DISCONNECT;
		}
	}
...
static int oplus_chg_vb_wdt_enable(struct oplus_chg_ic_dev *ic_dev, bool enable)
{
	...
	for (i = 0; i < vb->child_num; i++) {
		if (!func_is_support(&vb->child_list[i],
			...
			continue;
		rc = oplus_chg_ic_func(vb->child_list[i].ic_dev,
				       OPLUS_IC_FUNC_BUCK_WDT_ENABLE, enable);
		if (rc < 0) {
...
static int oplus_chg_vb_set_aicl_point(struct oplus_chg_ic_dev *ic_dev,
				       int vbatt_mv)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_BUCK_SET_AICL_POINT);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_set_curr_level(struct oplus_chg_ic_dev *ic_dev,
				       int cool_down)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_VOOCPHY_SET_CURR_LEVEL);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_set_match_temp(struct oplus_chg_ic_dev *ic_dev,
				       int match_temp)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_VOOCPHY_SET_MATCH_TEMP);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_get_otg_enable(struct oplus_chg_ic_dev *ic_dev,
				       bool *enable)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
static int oplus_chg_vb_get_charger_vol_max(struct oplus_chg_ic_dev *ic_dev,
					    int *vol)
{
	struct oplus_virtual_buck_ic *vb;
	int i, vol_tmp;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GET_CHARGER_VOL_MAX);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_get_charger_vol_min(struct oplus_chg_ic_dev *ic_dev,
					    int *vol)
{
	struct oplus_virtual_buck_ic *vb;
	int i, vol_tmp;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GET_CHARGER_VOL_MIN);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_get_charger_curr_max(struct oplus_chg_ic_dev *ic_dev,
					     int *curr)
{
	struct oplus_virtual_buck_ic *vb;
	int i, curr_tmp;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GET_CHARGER_CURR_MAX);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			return -EINVAL;
...
static int oplus_chg_vb_disable_vbus(struct oplus_chg_ic_dev *ic_dev, bool en,
				     bool delay)
{
	struct oplus_virtual_buck_ic *vb;
	int i;
...
static void *oplus_chg_vb_get_func(struct oplus_chg_ic_dev *ic_dev,
				   enum oplus_chg_ic_func func_id)
{
	void *func = NULL;
	...
	case OPLUS_IC_FUNC_GET_USB_TEMP_VOLT:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GET_USB_TEMP_VOLT,
					       oplus_chg_vb_get_usb_temp_volt);
		break;
				      ...
				      const void *buf, size_t buf_len)
{
	...
	case OPLUS_IC_FUNC_BUCK_INPUT_SUSPEND:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_input_suspend(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_BUCK_OUTPUT_SUSPEND:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_output_suspend(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_BUCK_SET_ICL:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_icl(ic_dev,
					  oplus_chg_ic_get_item_data(buf, 0),
					  oplus_chg_ic_get_item_data(buf, 1),
					  oplus_chg_ic_get_item_data(buf, 2));
		break;
	case OPLUS_IC_FUNC_BUCK_SET_FCC:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_fcc(ic_dev,
					  oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_BUCK_SET_FV:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_fv(ic_dev,
					 oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_BUCK_SET_ITERM:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_iterm(ic_dev,
					    oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_BUCK_SET_RECHG_VOL:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_rechg_vol(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_OTG_BOOST_ENABLE:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_otg_boost_enable(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_SET_OTG_BOOST_VOL:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_otg_boost_vol(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_SET_OTG_BOOST_CURR_LIMIT:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_otg_boost_curr_limit(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_BUCK_AICL_ENABLE:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_aicl_enable(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_BUCK_AICL_RERUN:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
	...
	case OPLUS_IC_FUNC_BUCK_QC_DETECT_ENABLE:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_qc_detect_enable(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_BUCK_SHIPMODE_ENABLE:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_shipmod_enable(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_BUCK_SET_QC_CONFIG:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_qc_config(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0),
			oplus_chg_ic_get_item_data(buf, 1));
		break;
	case OPLUS_IC_FUNC_BUCK_SET_PD_CONFIG:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_pd_config(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_WLS_BOOST_ENABLE:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_wls_boost_enable(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_SET_WLS_BOOST_VOL:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_wls_boost_vol(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_SET_WLS_BOOST_CURR_LIMIT:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_wls_boost_curr_limit(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_GAUGE_UPDATE:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
	...
	case OPLUS_IC_FUNC_VOOCPHY_ENABLE:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_voocphy_enable(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_VOOCPHY_RESET_AGAIN:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
	...
	case OPLUS_IC_FUNC_BACKUP_SOC:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_backup_soc(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_SET_TYPEC_MODE:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_typec_mode(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_SET_USB_DISCHG_ENABLE:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_usb_dischg_enable(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_SET_OTG_SWITCH_STATUS:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_otg_switch_status(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_CC_DETECT_HAPPENED:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
	...
	case OPLUS_IC_FUNC_BUCK_WDT_ENABLE:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_wdt_enable(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_VOOCPHY_SET_CURR_LEVEL:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_curr_level(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_VOOCPHY_SET_MATCH_TEMP:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_match_temp(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_BUCK_BC12_COMPLETED:
	...
	case OPLUS_IC_FUNC_BUCK_SET_AICL_POINT:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_aicl_point(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	case OPLUS_IC_FUNC_BUCK_SET_VINDPM:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vb_set_vindpm(
			ic_dev, oplus_chg_ic_get_item_data(buf, 0));
		break;
	default:
					  ...
					  void *buf)
{
	...
	case OPLUS_IC_FUNC_GET_USB_TEMP_VOLT:
		...
		rc = oplus_chg_vb_get_usb_temp_volt(
			ic_dev, oplus_chg_ic_get_item_data_addr(buf, 0),
	...
	case OPLUS_IC_FUNC_GET_TYPEC_MODE:
		oplus_chg_ic_debug_data_init(buf, 1);
		item_data = oplus_chg_ic_get_item_data_addr(buf, 0);
		rc = oplus_chg_vb_get_typec_mode(
			ic_dev,
			(enum oplus_chg_typec_port_role_type *)item_data);
		if (rc < 0)
			break;
		*item_data = cpu_to_le32(*item_data);
		rc = oplus_chg_ic_debug_data_size(1);
...
static void oplus_vb_err_handler(struct oplus_chg_ic_dev *ic_dev,
				 void *virq_data)
{
...
static void oplus_vb_cc_detect_handler(struct oplus_chg_ic_dev *ic_dev,
				       void *virq_data)
{
	struct oplus_virtual_buck_ic *chip = virq_data;
...
static void oplus_vb_plugin_handler(struct oplus_chg_ic_dev *ic_dev,
				    void *virq_data)
{
	struct oplus_virtual_buck_ic *chip = virq_data;
...
static void oplus_vb_cc_changed_handler(struct oplus_chg_ic_dev *ic_dev,
					void *virq_data)
{
	struct oplus_virtual_buck_ic *chip = virq_data;
...
static void oplus_vb_suspend_check_handler(struct oplus_chg_ic_dev *ic_dev,
					   void *virq_data)
{
	struct oplus_virtual_buck_ic *chip = virq_data;
	int i;
...
static void oplus_vb_chg_type_change_handler(struct oplus_chg_ic_dev *ic_dev,
					     void *virq_data)
{
	struct oplus_virtual_buck_ic *chip = virq_data;
...
static void oplus_vb_svid_handler(struct oplus_chg_ic_dev *ic_dev,
				  void *virq_data)
{
	struct oplus_virtual_buck_ic *chip = virq_data;
...
static void oplus_vb_otg_enable_handler(struct oplus_chg_ic_dev *ic_dev,
					void *virq_data)
{
	struct oplus_virtual_buck_ic *chip = virq_data;
...
static void oplus_vb_voltage_change_handler(struct oplus_chg_ic_dev *ic_dev,
					    void *virq_data)
{
	struct oplus_virtual_buck_ic *chip = virq_data;
...
static void oplus_vb_current_change_handler(struct oplus_chg_ic_dev *ic_dev,
					    void *virq_data)
{
	struct oplus_virtual_buck_ic *chip = virq_data;
...
static void oplus_vb_bc12_completed_handler(struct oplus_chg_ic_dev *ic_dev,
					    void *virq_data)
{
	struct oplus_virtual_buck_ic *chip = virq_data;
...
static int oplus_vb_virq_register(struct oplus_virtual_buck_ic *chip)
{
	...
	for (i = 0; i < chip->child_num; i++) {
		...
		if (virq_is_support(&chip->child_list[i],
			...
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_CC_DETECT error, rc=%d",
					rc);
		}
		if (virq_is_support(&chip->child_list[i],
				    OPLUS_IC_VIRQ_PLUGIN)) {
			rc = oplus_chg_ic_virq_register(
				chip->child_list[i].ic_dev,
				OPLUS_IC_VIRQ_PLUGIN, oplus_vb_plugin_handler,
				chip);
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_PLUGIN error, rc=%d",
					rc);
		}
		if (virq_is_support(&chip->child_list[i],
				    OPLUS_IC_VIRQ_CC_CHANGED)) {
			...
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_CC_CHANGED error, rc=%d",
					rc);
		}
		if (virq_is_support(&chip->child_list[i],
			...
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_SUSPEND_CHECK error, rc=%d",
					rc);
		}
		if (virq_is_support(&chip->child_list[i],
			...
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_CHG_TYPE_CHANGE error, rc=%d",
					rc);
		}
		if (virq_is_support(&chip->child_list[i], OPLUS_IC_VIRQ_SVID)) {
			rc = oplus_chg_ic_virq_register(
				chip->child_list[i].ic_dev, OPLUS_IC_VIRQ_SVID,
				oplus_vb_svid_handler, chip);
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_SVID error, rc=%d",
					rc);
		}
		if (virq_is_support(&chip->child_list[i],
			...
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_OTG_ENABLE error, rc=%d",
					rc);
		}
		if (virq_is_support(&chip->child_list[i],
			...
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_VOLTAGE_CHANGED error, rc=%d",
					rc);
		}
		if (virq_is_support(&chip->child_list[i],
			...
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_CURRENT_CHANGED error, rc=%d",
					rc);
		}
		if (virq_is_support(&chip->child_list[i],
			...
			if (rc < 0)
				chg_err("register OPLUS_IC_VIRQ_BC12_COMPLETED error, rc=%d",
					rc);
		}
		if (virq_is_support(&chip->child_list[i],
				    OPLUS_IC_VIRQ_DATA_ROLE_CHANGED)) {
...
static int oplus_virtual_buck_probe(struct platform_device *pdev)
{
	...
	if (of_find_property(node, "nvmem-cells", NULL)) {
		chip->soc_backup_nvmem =
			devm_nvmem_cell_get(chip->dev, "oplus_soc_backup");
