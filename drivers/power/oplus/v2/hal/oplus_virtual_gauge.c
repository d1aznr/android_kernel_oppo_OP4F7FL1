...
static int oplus_chg_vg_smt_test(struct oplus_chg_ic_dev *ic_dev, char buf[],
				 int len)
{
	struct oplus_virtual_gauge_ic *chip;
	int i, index;
				     ...
				     int *vol_mv)
{
	...
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_VOL);
...
skip_overwrite:
	...
	for (i = 0; i < chip->child_num; i++) {
		...
		rc = oplus_chg_ic_func(chip->child_list[i].ic_dev,
				       OPLUS_IC_FUNC_GAUGE_GET_BATT_VOL, index,
				       vol_mv);
		if (rc < 0)
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_MAX);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_MIN);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_CURR);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_TEMP);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_SOC);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_FCC);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_SOH);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_AUTH);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_HMAC);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
static int oplus_chg_vg_set_batt_full(struct oplus_chg_ic_dev *ic_dev,
				      bool full)
{
	struct oplus_virtual_gauge_ic *chip;
	int i;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_SET_BATT_FULL);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_RCU);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_RCF);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_FCU);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_FCF);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_SOU);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_DO0);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_DOE);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_TRM);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_CB_STATUS);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_PASSEDCHG);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_BATT_NUM);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_DEVICE_TYPE);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
	data = oplus_chg_ic_get_overwrite_data(
		ic_dev, OPLUS_IC_FUNC_GAUGE_GET_DEVICE_TYPE_FOR_VOOC);
	if (unlikely(data != NULL)) {
		buf = (const void *)data->buf;
		if (!oplus_chg_ic_debug_data_check(buf, data->size))
			goto skip_overwrite;
...
static int oplus_chg_vg_get_bcc_prams(struct oplus_chg_ic_dev *ic_dev,
				      char *buf)
{
	struct oplus_virtual_gauge_ic *chip;
	int i;
...
static int
oplus_chg_vg_fastchg_update_bcc_parameters(struct oplus_chg_ic_dev *ic_dev,
					   char *buf)
{
	struct oplus_virtual_gauge_ic *chip;
	int i;
...
static int oplus_chg_vg_get_prev_bcc_prams(struct oplus_chg_ic_dev *ic_dev,
					   char *buf)
{
	struct oplus_virtual_gauge_ic *chip;
	int i;
...
static int oplus_chg_vg_set_bcc_parms(struct oplus_chg_ic_dev *ic_dev,
				      const char *buf)
{
	struct oplus_virtual_gauge_ic *chip;
	int i;
...
static int oplus_chg_vg_afi_update_done(struct oplus_chg_ic_dev *ic_dev,
					bool *status)
{
	struct oplus_virtual_gauge_ic *chip;
	int i;
				   ...
				   enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_GAUGE_GET_BATT_CAP:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_GAUGE_GET_BATT_CAP,
				      ...
				      const void *buf, size_t buf_len)
{
	...
	case OPLUS_IC_FUNC_GAUGE_SET_LOCK:
		if (!oplus_chg_ic_debug_data_check(buf, buf_len))
			return -EINVAL;
		rc = oplus_chg_vg_set_lock(ic_dev,
