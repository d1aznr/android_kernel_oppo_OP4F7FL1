...
bool oplus_chg_ic_debug_data_check(const void *buf, size_t len)
{
	...
	for (i = 0; i < debug_data->item_num; i++) {
		...
		item_size += (item->size +
			      sizeof(struct oplus_chg_ic_func_date_item));
	}
...
static struct device_attribute *oplus_chg_ic_attributes[] = {
	&dev_attr_func_id,	   &dev_attr_data,  &dev_attr_overwrite,
	&dev_attr_clean_overwrite, &dev_attr_virq,  &dev_attr_name,
	&dev_attr_manu_name,	   &dev_attr_fw_id, NULL
};
	...
	for (i = 0; i < ic_dev->virq_num; i++) {
		...
		if (virq->virq_id == virq_id) {
			if (virq->virq_handler == NULL ||
			    virq->virq_data == NULL) {
				chg_info(
					"%s virq(=%d) handler or data is NULL \n",
					ic_dev->name, virq_id);
				return 0;
			}
			virq->virq_handler(ic_dev, virq->virq_data);
...
__printf(4, 5) int oplus_chg_ic_creat_err_msg(struct oplus_chg_ic_dev *ic_dev,
					      enum oplus_chg_ic_err err_type,
					      int sub_err_type,
					      const char *format, ...)
{
	va_list args;
