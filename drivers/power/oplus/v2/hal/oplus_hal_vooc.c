...
static enum oplus_chg_vooc_switch_mode g_dpdm_switch_mode =
...
oplus_vooc_get_gpio_ap_data(struct oplus_chg_ic_dev *vooc_ic)
{
	int val;
	int rc;
	...
	rc = oplus_chg_ic_func(vooc_ic, OPLUS_IC_FUNC_VOOC_GET_DATA_GPIO_VAL,
			       &val);
	if (rc < 0) {
...
bool oplus_vooc_asic_fw_status(struct oplus_chg_ic_dev *vooc_ic)
{
	...
	rc = oplus_chg_ic_func(vooc_ic, OPLUS_IC_FUNC_VOOC_CHECK_FW_STATUS,
			       &pass);
	if (rc == -ENOTSUPP) {
		return true;
	} else if (rc < 0) {
