		...
		case OPLUS_CHG_USB_TYPE_PD_PPS:
			if (chip->pd_boost_disable_votable &&
			    is_client_vote_enabled(
				    chip->pd_boost_disable_votable, SVID_VOTER))
				pd_use_default = true;
			if (chip->pd_svooc_votable &&
...
static struct device_attribute *oplus_usb_attributes[] = {
	&dev_attr_otg_online,	  &dev_attr_otg_switch,
					...
					char *buf)
{
	...
	if (is_chg_disable_votable_available(chip))
		val = !get_client_vote(chip->chg_disable_votable,
				       MMI_CHG_VOTER);
	else
		return -ENOTSUPP;
			      ...
			      char *buf)
{
	...
	if (chip->vooc_topic) {
		rc = oplus_mms_get_item_data(
			chip->vooc_topic, VOOC_ITEM_VOOC_STARTED, &data, true);
