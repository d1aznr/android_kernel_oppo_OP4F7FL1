...
is_wired_icl_votable_available(struct oplus_chg_comm *chip)
{
...
__maybe_unused static bool
is_wired_fcc_votable_available(struct oplus_chg_comm *chip)
{
...
__maybe_unused static bool
is_wls_icl_votable_available(struct oplus_chg_comm *chip)
{
...
__maybe_unused static bool
is_wls_fcc_votable_available(struct oplus_chg_comm *chip)
{
...
static void oplus_comm_check_sw_full(struct oplus_chg_comm *chip)
{
	...
	if (chip->vooc_by_normal_path) {
		if (chip->vooc_charging &&
		    sid_to_adapter_chg_type(chip->vooc_sid) !=
			    CHARGER_TYPE_VOOC)
			return;
	} else {
		if (chip->vooc_charging)
...
static void oplus_comm_charge_timeout_work(struct work_struct *work)
{
	...
	struct oplus_chg_comm *chip =
		container_of(dwork, struct oplus_chg_comm, charge_timeout_work);
...
static void oplus_comm_check_battery_status(struct oplus_chg_comm *chip)
{
	...
	if (chip->batt_full) {
		if ((chip->temp_region > TEMP_REGION_COLD) &&
		    (chip->temp_region < TEMP_REGION_WARM) && chip->hmac) {
			if (chip->ui_soc == 100) {
...
static int oplus_comm_push_vbat_too_low_msg(struct oplus_chg_comm *chip)
{
	...
	msg = oplus_mms_alloc_str_msg(
		...
		"$$soc@@%d$$uisoc@@%d$$vbatt_max@@%d$$vbatt_min@@%d", chip->soc,
		chip->ui_soc, chip->vbat_mv, chip->vbat_min_mv);
	if (msg == NULL) {
...
static void oplus_comm_ui_soc_update(struct oplus_chg_comm *chip)
{
	...
	if (chip->vbat_min_mv <
		...
		chg_err("[%d %d %d %d %d %d]\n", spec->vbat_charging_uv_thr_mv,
			spec->vbat_uv_thr_mv, charging, chip->vbat_min_mv,
			chip->vbat_mv, force_dec_interval);
...
check_update_time:
	...
	if (update_delay == 0)
		update_delay = msecs_to_jiffies(
			oplus_mms_update_interval(chip->gauge_topic));
...
static void oplus_comm_ui_soc_update_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_comm *chip =
		container_of(dwork, struct oplus_chg_comm, ui_soc_update_work);
	...
	if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT ||
		...
		if (!wired_online && vbus_mv < POWER_OFF_VBUS_CHECK) {
			...
			oplus_mms_get_item_data(chip->wired_topic,
						WIRED_ITEM_ONLINE, &data, true);
			wired_online = data.intval;
			if (!wired_online && vbus_mv < POWER_OFF_VBUS_CHECK) {
				if (!chip->vooc_online &&
				    !chip->vooc_online_keep) {
					chg_info(
...
void oplus_comm_ui_soc_decimal_init(struct oplus_chg_comm *chip)
{
	...
	if (chip->ui_soc == 100) {
		...
		if ((soc_decimal->ui_soc_integer +
		     soc_decimal->ui_soc_decimal) >
			    soc_decimal->last_decimal_ui_soc &&
		    soc_decimal->last_decimal_ui_soc != 0) {
			soc_decimal->ui_soc_decimal =
				((soc_decimal->last_decimal_ui_soc % 1000 -
				  50) > 0) ?
					(soc_decimal->last_decimal_ui_soc %
						 1000 -
					 50) :
	...
	soc_decimal->init_decimal_ui_soc =
	...
	chg_info("2VBUS ui_soc_decimal:%d",
		 soc_decimal->ui_soc_integer + soc_decimal->ui_soc_decimal);
...
static void oplus_comm_show_ui_soc_decimal(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_comm *chip =
		container_of(dwork, struct oplus_chg_comm, ui_soc_decimal_work);
	struct oplus_comm_config *config = &chip->config;
	struct ui_soc_decimal *soc_decimal = &chip->soc_decimal;
	union mms_msg_data data = { 0 };
	...
	if (icharging > 0) {
		speed = 100000 * icharging * UPDATE_TIME * batt_num /
			(chip->batt_fcc * 3600);
		chg_info("ui_soc_decimal: icharging=%d, batt_fcc=%d",
			 chip->ibat_ma, chip->batt_fcc);
	...
	chg_info(
		"ui_soc_decimal: (ui_soc_decimal+ui_soc)=%d, speed=%d, soc=%d\n",
		(soc_decimal->ui_soc_decimal + soc_decimal->ui_soc_integer),
		speed, ((chip->batt_rm * 10000) / chip->batt_fcc));
	if (soc_decimal->ui_soc_integer + soc_decimal->ui_soc_decimal >=
	    100000) {
		soc_decimal->ui_soc_integer = 100000;
		soc_decimal->ui_soc_decimal = 0;
	}
	...
	if (soc_decimal->calculate_decimal_time <= MAX_UI_DECIMAL_TIME) {
		...
		/* update ui_soc */
		oplus_comm_set_ui_soc(chip, (soc_decimal->ui_soc_integer +
					     soc_decimal->ui_soc_decimal) /
						    1000);
...
static void oplus_comm_ffc_start_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_comm *chip =
		container_of(dwork, struct oplus_chg_comm, ffc_start_work);
	struct oplus_comm_spec_config *spec = &chip->spec;
	enum oplus_ffc_temp_region ffc_temp_region;

	if (spec->full_pre_ffc_judge) {
		if (chip->vbat_mv >= spec->full_pre_ffc_mv &&
		    chip->soc == NORMAL_FULL_SOC) {
...
static void oplus_comm_check_ffc(struct oplus_chg_comm *chip)
{
	...
	case FFC_FAST:
		...
		if (chip->ffc_fv_count >= FFC_VOLT_COUNTS) {
			...
			chip->ffc_fv_count = 0;
			chg_info(
				"vbat_mv(=%d) > fv_max_mv(=%d), switch to next step(=%d)\n",
		...
		if (chip->ffc_fcc_count >= FFC_CURRENT_COUNTS) {
			...
			chip->ffc_fv_count = 0;
			chg_info(
				"ibat_ma(=%d) > cutoff_ma(=%d), switch to next step(=%d)\n",
...
static int oplus_comm_set_cool_down_level(struct oplus_chg_comm *chip,
					  int level)
{
	struct mms_msg *msg;
	int rc;
...
static void oplus_comm_check_shell_temp(struct oplus_chg_comm *chip,
					bool update)
{
	struct mms_msg *msg;
	struct oplus_comm_spec_config *spec = &chip->spec;
	...
	batt_temp_high =
		spec->batt_temp_thr[ARRAY_SIZE(spec->batt_temp_thr) - 1];
	...
	if (chip->shell_themal == NULL) {
		...
		if (oplus_comm_override_by_shell_temp(chip, chip->batt_temp)) {
			...
			if (abs(diff) >= 150 || chip->batt_temp < 320 ||
			    chip->batt_temp > batt_temp_high)
				shell_temp = chip->batt_temp;
		} else {
			shell_temp = chip->batt_temp;
		}
					     ...
					     void *prv_data)
{
	...
	if (get_eng_version() == HIGH_TEMP_AGING ||
		...
		chg_info(
			"HIGH_TEMP_AGING/AGING/FACTORY, disable chg timeout\n");
...
static void oplus_comm_subscribe_wired_topic(struct oplus_mms *topic,
					     void *prv_data)
{
	struct oplus_chg_comm *chip = prv_data;
	union mms_msg_data data = { 0 };
			...
			if (!chip->vooc_online &&
				...
				if (soc_decimal->decimal_control) {
					cancel_delayed_work_sync(
						&chip->ui_soc_decimal_work);
					soc_decimal->last_decimal_ui_soc =
						(soc_decimal->ui_soc_integer +
						 soc_decimal->ui_soc_decimal);
					oplus_comm_ui_soc_decimal_deinit(chip);
					chg_info(
						"ui_soc_decimal: cancel last_decimal_ui_soc=%d",
						soc_decimal
							->last_decimal_ui_soc);
				}
				soc_decimal->calculate_decimal_time = 0;
					    ...
					    void *prv_data)
{
	...
	chip->vooc_subs =
		oplus_mms_subscribe(chip->vooc_topic, chip,
				    oplus_comm_vooc_subs_callback, "chg_comm");
...
static void oplus_comm_subscribe_wls_topic(struct oplus_mms *topic,
					   void *prv_data)
{
	struct oplus_chg_comm *chip = prv_data;
...
int read_signed_data_from_node(struct device_node *node, const char *prop_str,
...
int read_unsigned_data_from_node(struct device_node *node, const char *prop_str,
				 u32 *addr, int len_max)
{
	int rc = 0, length;
...
static int oplus_comm_parse_dt(struct oplus_chg_comm *comm_dev)
{
	...
	rc = read_unsigned_data_from_node(
		node, "oplus_spec,wired-ffc-fv-cutoff-mv",
...
static int oplus_comm_vote_init(struct oplus_chg_comm *chip)
{
	...
	chip->fv_max_votable = create_votable(
		"FV_MAX", VOTE_MAX, oplus_comm_fv_max_vote_callback, chip);
	...
	chip->fv_min_votable = create_votable(
		"FV_MIN", VOTE_MIN, oplus_comm_fv_min_vote_callback, chip);
					 ...
					 loff_t *data)
{
	...
	chg_info("ui_soc_decimal: boot_completed=%d",
		 chip->soc_decimal.boot_completed);
	return len;
}
static ssize_t proc_ui_soc_decimal_read(struct file *file, char __user *buff,
					size_t count, loff_t *off)
{
	...
	if (config->vooc_show_ui_soc_decimal) {
		...
		if (!svooc_is_control_by_vooc &&
		    !soc_decimal->calculate_decimal_time && !chip->wls_online &&
		    chip->vooc_online) {
		...
		val = (soc_decimal->ui_soc_integer +
		       soc_decimal->ui_soc_decimal) /
		      10;
		if (!soc_decimal->decimal_control)
			val = 0;
	...
	sprintf(read_data, "%d, %d", soc_decimal->init_decimal_ui_soc / 10,
		val);
	chg_info("APK successful, %d,%d", soc_decimal->init_decimal_ui_soc / 10,
		 val);
...
static int oplus_comm_tbatt_power_off_kthread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_TEMP,
					&data,
					(chip->batt_temp >=
					 OPCHG_PWROFF_FORCE_UPDATE_BATT_TEMP));
		chip->batt_temp = data.intval;
...
static ssize_t proc_tbatt_pwroff_read(struct file *file, char __user *buff,
				      size_t count, loff_t *off)
{
	char page[256] = { 0 };
	char read_data[3] = { 0 };
...
static int oplus_comm_init_proc(struct oplus_chg_comm *chip)
{
	...
	if (eng_version != OEM_RELEASE) {
		pr_entry_tmp = proc_create_data(
			"charger_factorymode_test", 0666, pr_entry_da,
	...
	pr_entry_tmp = proc_create_data("hmac", 0444, pr_entry_da,
					&hmac_proc_fops, chip);
	if (pr_entry_tmp == NULL)
...
__maybe_unused static int oplus_comm_set_led_on(struct oplus_chg_comm *chip,
						bool on)
{
	struct mms_msg *msg;
	int rc;
...
#if IS_ENABLED(CONFIG_DRM_PANEL_NOTIFY)
static void oplus_comm_panel_notifier_callback(
...
static void oplus_fg_soft_reset_work(struct work_struct *work)
{
	...
	if (chip->soc < SOFT_REST_SOC_THRESHOLD ||
	    oplus_gauge_check_reset_condition() || fg_reset_test) {
...
static void oplus_wired_chg_check_work(struct work_struct *work)
{
	...
	if (is_wired_charging_disable_votable_available(chip)) {
		vote(chip->wired_charging_disable_votable, CHG_FULL_VOTER,
		     false, 0, false);
		vote(chip->wired_charging_disable_votable, FFC_VOTER, false, 0,
		     false);
	}
...
static int oplus_comm_driver_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&comm_dev->noplug_batt_volt_work,
		  oplus_comm_noplug_batt_volt_work);
	...
	INIT_DELAYED_WORK(&comm_dev->charge_timeout_work,
			  oplus_comm_charge_timeout_work);
	INIT_DELAYED_WORK(&comm_dev->ui_soc_update_work,
			  oplus_comm_ui_soc_update_work);
	INIT_DELAYED_WORK(&comm_dev->ui_soc_decimal_work,
			  oplus_comm_show_ui_soc_decimal);
	INIT_DELAYED_WORK(&comm_dev->lcd_notify_reg_work,
			  oplus_comm_lcd_notify_reg_work);
	INIT_DELAYED_WORK(&comm_dev->fg_soft_reset_work,
			  oplus_fg_soft_reset_work);

	spin_lock_init(&comm_dev->remuse_lock);

	oplus_comm_temp_thr_init(comm_dev, TEMP_REGION_NORMAL);

	oplus_mms_wait_topic("wired", oplus_comm_subscribe_wired_topic,
			     comm_dev);
	oplus_mms_wait_topic("vooc", oplus_comm_subscribe_vooc_topic, comm_dev);
	oplus_mms_wait_topic("wireless", oplus_comm_subscribe_wls_topic,
			     comm_dev);
...
int oplus_comm_set_config(struct oplus_mms *topic,
