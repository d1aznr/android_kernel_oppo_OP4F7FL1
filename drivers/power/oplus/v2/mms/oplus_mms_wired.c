...
int oplus_wired_get_charger_cycle(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_CHARGER_CYCLE,
			       &cycle);
	if (rc < 0) {
		if (rc != -ENOTSUPP)
...
int oplus_wired_get_vbus(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_INPUT_VOL,
			       &vbus_mv);
	if (rc < 0) {
...
int oplus_wired_set_icl(int icl_ma, bool step)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_ICL, false,
			       step, icl_ma);
	if (rc < 0)
...
int oplus_wired_set_icl_by_vooc(struct oplus_mms *topic, int icl_ma)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_ICL, true,
			       false, icl_ma);
	if (rc)
...
int oplus_wired_set_fcc(int fcc_ma)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_FCC,
			       fcc_ma);
	if (rc < 0)
...
bool oplus_wired_is_present(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_INPUT_PRESENT,
			       &present);
	if (rc < 0) {
...
int oplus_wired_input_enable(bool enable)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_INPUT_SUSPEND,
			       !enable);
	if (rc < 0)
...
bool oplus_wired_input_is_enable(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic,
			       OPLUS_IC_FUNC_BUCK_INPUT_IS_SUSPEND, &enable);
	if (rc < 0) {
...
int oplus_wired_output_enable(bool enable)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_OUTPUT_SUSPEND,
			       !enable);
	if (rc < 0)
...
bool oplus_wired_output_is_enable(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic,
			       OPLUS_IC_FUNC_BUCK_OUTPUT_IS_SUSPEND, &enable);
	if (rc < 0) {
...
int oplus_wired_get_icl(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_ICL,
			       &icl_ma);
	if (rc < 0) {
...
int oplus_wired_set_iterm(int iterm_ma)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_ITERM,
			       iterm_ma);
	if (rc < 0)
...
int oplus_wired_set_rechg_vol(int vol_mv)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_RECHG_VOL,
			       vol_mv);
	if (rc < 0)
...
int oplus_wired_get_ibus(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_INPUT_CURR,
			       &ibus_ma);
	if (rc < 0) {
...
int oplus_wired_otg_boost_enable(bool enable)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_OTG_BOOST_ENABLE,
			       enable);
	if (rc < 0)
...
int oplus_wired_set_otg_boost_vol(int vol_mv)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_SET_OTG_BOOST_VOL,
			       vol_mv);
	if (rc < 0)
...
int oplus_wired_set_otg_boost_curr_limit(int curr_ma)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic,
			       OPLUS_IC_FUNC_SET_OTG_BOOST_CURR_LIMIT, curr_ma);
	if (rc < 0)
...
int oplus_wired_aicl_enable(bool enable)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_AICL_ENABLE,
			       enable);
	if (rc < 0)
...
int oplus_wired_get_hw_detect(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_GET_HW_DETECT,
			       &hw_detect);
	if (rc < 0) {
...
int oplus_wired_qc_detect_enable(bool enable)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic,
			       OPLUS_IC_FUNC_BUCK_QC_DETECT_ENABLE, enable);
	if (rc < 0)
...
int oplus_wired_set_qc_config(enum oplus_chg_qc_version version, int vol_mv)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_QC_CONFIG,
			       version, vol_mv);
	if (rc < 0)
...
int oplus_wired_set_pd_config(u32 pdo)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_BUCK_SET_PD_CONFIG,
			       pdo);
	if (rc < 0)
...
int oplus_wired_get_usb_temp_volt(int *vol_l, int *vol_r)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_USB_TEMP_VOLT,
			       vol_l, vol_r);
	if (rc < 0)
...
enum oplus_chg_typec_port_role_type oplus_wired_get_typec_mode(void)
{
	...
	rc = oplus_chg_ic_func(chip->buck_ic, OPLUS_IC_FUNC_GET_TYPEC_MODE,
			       &mode);
	if (rc < 0) {
...
int oplus_wired_set_usb_status(struct oplus_mms_wired *chip,
			       unsigned int status)
{
	unsigned int usb_status;
	struct mms_msg *msg;
...
int oplus_wired_clear_usb_status(struct oplus_mms_wired *chip,
				 unsigned int status)
{
	unsigned int usb_status;
	struct mms_msg *msg;
...
static void oplus_usbtemp_recover_work(struct work_struct *work)
{
	...
	struct oplus_mms_wired *chip = container_of(
		dwork, struct oplus_mms_wired, usbtemp_recover_work);
...
static int oplus_usbtemp_dischg_action(struct oplus_mms_wired *chip)
{
	...
	if (get_eng_version() != HIGH_TEMP_AGING) {
		...
		if (is_chg_suspend_votable_available(chip))
			rc = vote(chip->chg_suspend_votable, USB_VOTER, true, 1,
				  false);
		else
			rc = -ENOTSUPP;
		if (rc < 0)
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (chip->batt_temp / 10 > USB_50C &&
		    (((chip->usb_temp_l >= chip->batt_temp / 10 + 7) &&
		      (chip->usb_temp_l < USB_100C)) ||
		     ((chip->usb_temp_r >= chip->batt_temp / 10 + 7) &&
		      (chip->usb_temp_r < USB_100C)))) {
			chg_err("in loop 1");
			for (i = 1; i <= retry_cnt; i++) {
				mdelay(RETRY_CNT_DELAY);
		...
		if ((((chip->usb_temp_l - chip->batt_temp / 10) >
		      chip->usbtemp_batttemp_gap) &&
		     (chip->usb_temp_l < USB_100C)) ||
		    (((chip->usb_temp_r - chip->batt_temp / 10) >
		      chip->usbtemp_batttemp_gap) &&
		     (chip->usb_temp_r < USB_100C))) {
			if (count == 0) {
				last_usb_temp_r = chip->usb_temp_r;
			...
			if (((current_temp_l - last_usb_temp_l) >= 3) ||
				...
				if ((count_l >= retry_cnt &&
				    ...
				    (count_r >= retry_cnt &&
				     ...
				     chip->usb_temp_r < USB_100C)) {
					if (oplus_usbtemp_check_is_support(
						    chip)) {
						chip->dischg_flag = true;
...
{
	struct oplus_mms_wired *chip =
		container_of(work, struct oplus_mms_wired, gauge_update_work);
	union mms_msg_data data = { 0 };
...
{
	struct oplus_mms_wired *chip = container_of(
		work, struct oplus_mms_wired, otg_enable_handler_work);
	struct mms_msg *msg;
	bool enable;
...
static void oplus_wired_data_role_changed_handler_work(struct work_struct *work)
{
	...
	if (role == TYPEC_DEVICE) {
		if (!IS_ERR(vooc_disable_votable))
			vote(vooc_disable_votable, TYPEC_VOTER, false, 0,
			     false);
		if (!enable) {
			rc = oplus_chg_ic_func(chip->buck_ic,
...
static void oplus_mms_wired_init_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_mms_wired *chip = container_of(
		dwork, struct oplus_mms_wired, mms_wired_init_work);
...
{
	struct oplus_mms_wired *chip =
		container_of(work, struct oplus_mms_wired, err_handler_work);
					  ...
					  void *virq_data)
{
	...
	schedule_delayed_work(&chip->ccdetect_work,
...
static int oplus_mms_wired_virq_register(struct oplus_mms_wired *chip)
{
	...
	rc = oplus_chg_ic_virq_register(chip->buck_ic,
					...
					oplus_wired_voltage_changed_handler,
					chip);
	if (rc < 0)
	...
	rc = oplus_chg_ic_virq_register(chip->buck_ic,
					...
					oplus_wired_current_changed_handler,
					chip);
	if (rc < 0)
	...
	rc = oplus_chg_ic_virq_register(chip->buck_ic,
					...
					oplus_wired_bc12_completed_handler,
					chip);
	if (rc < 0)
...
static int oplus_mms_wired_topic_init(struct oplus_mms_wired *chip)
{
	...
	chip->wired_topic = devm_oplus_mms_register(
		chip->dev, &oplus_mms_wired_desc, &mms_cfg);
...
static int oplus_mms_wired_probe(struct platform_device *pdev)
{
	...
	rc = of_property_read_string(chip->dev->of_node, "oplus,adc_info_name",
				     &name);
	if (rc < 0) {
	...
	INIT_DELAYED_WORK(&chip->usbtemp_recover_work,
			  oplus_usbtemp_recover_work);
	INIT_DELAYED_WORK(&chip->ccdetect_work, oplus_mms_wired_ccdetect_work);
	INIT_DELAYED_WORK(&chip->typec_state_change_work,
			  oplus_mms_wired_typec_state_change_work);
	INIT_DELAYED_WORK(&chip->svid_handler_work,
			  oplus_mms_wired_svid_handler_work);
	...
	INIT_WORK(&chip->otg_enable_handler_work,
		  oplus_wired_otg_enable_handler_work);
