...
int oplus_gauge_get_batt_mvolts(void)
{
	...
	rc = oplus_chg_ic_func(g_mms_gauge->gauge_ic,
			       OPLUS_IC_FUNC_GAUGE_GET_BATT_MAX, &vol_mv);
	if (rc < 0) {
...
int oplus_gauge_get_batt_mvolts_2cell_min(void)
{
	...
	rc = oplus_chg_ic_func(g_mms_gauge->gauge_ic,
			       OPLUS_IC_FUNC_GAUGE_GET_BATT_MIN, &vol_mv);
	if (rc < 0) {
...
int oplus_gauge_get_batt_current(void)
{
	...
	rc = oplus_chg_ic_func(g_mms_gauge->gauge_ic,
			       OPLUS_IC_FUNC_GAUGE_GET_BATT_CURR, &curr_ma);
	if (rc < 0) {
...
int oplus_gauge_get_battery_cb_status(void)
{
	...
	rc = oplus_chg_ic_func(g_mms_gauge->gauge_ic,
			       OPLUS_IC_FUNC_GAUGE_GET_CB_STATUS, &cb_status);
	if (rc < 0) {
		chg_err("update soc smooth parameter, rc=%d\n", rc);
		return 0;
	}
...
int oplus_gauge_get_passedchg(int *val)
{
	...
	rc = oplus_chg_ic_func(g_mms_gauge->gauge_ic,
			       OPLUS_IC_FUNC_GAUGE_GET_PASSEDCHG, val);
	if (rc < 0) {
...
int oplus_gauge_lock(void)
{
	...
	rc = oplus_chg_ic_func(g_mms_gauge->gauge_ic,
			       OPLUS_IC_FUNC_GAUGE_SET_LOCK, true);
	if (rc == -ENOTSUPP)
		rc = 0;
...
int oplus_gauge_unlock(void)
{
	...
	rc = oplus_chg_ic_func(g_mms_gauge->gauge_ic,
			       OPLUS_IC_FUNC_GAUGE_SET_LOCK, false);
	if (rc == -ENOTSUPP)
		rc = 0;
...
bool oplus_gauge_is_locked(void)
{
	...
	rc = oplus_chg_ic_func(g_mms_gauge->gauge_ic,
			       OPLUS_IC_FUNC_GAUGE_IS_LOCKED, &locked);
	if (rc == -ENOTSUPP)
		return false;
	else if (rc < 0)
		return true;
...
static void oplus_mms_gauge_init_work(struct work_struct *work)
{
	...
	struct oplus_mms_gauge *chip = container_of(
		dwork, struct oplus_mms_gauge, hal_gauge_init_work);
	...
	rc = oplus_chg_ic_func(chip->gauge_ic,
			       OPLUS_IC_FUNC_GAUGE_GET_BATT_HMAC, &chip->hmac);
	if (rc < 0) {
		chg_err("get battery hmac status error, rc=%d\n", rc);
		chip->hmac = 0;
	} else {
...
{
	struct oplus_mms_gauge *chip =
		container_of(work, struct oplus_mms_gauge, err_handler_work);
...
{
	struct oplus_mms_gauge *chip =
		container_of(work, struct oplus_mms_gauge, online_handler_work);
	struct mms_msg *msg;
	unsigned int err_code;
	int rc;
...
{
	struct oplus_mms_gauge *chip =
		container_of(work, struct oplus_mms_gauge, resume_handler_work);
	struct mms_msg *msg;
	int rc;
...
static int oplus_mms_gauge_update_soc(struct oplus_mms *mms,
				      union mms_msg_data *data)
{
	...
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_BATT_SOC,
			       &soc);
	if (rc < 0) {
		chg_err("get battery soc error, rc=%d\n", rc);
		soc = 50;
...
static int oplus_mms_gauge_update_vol_max(struct oplus_mms *mms,
					  union mms_msg_data *data)
{
	struct oplus_mms_gauge *chip;
	int vol;
	...
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_BATT_MAX,
			       &vol);
	if (rc < 0) {
		chg_err("get battery voltage max error, rc=%d\n", rc);
		vol = 0;
...
static int oplus_mms_gauge_update_vol_min(struct oplus_mms *mms,
					  union mms_msg_data *data)
{
	struct oplus_mms_gauge *chip;
	int vol;
	...
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_BATT_MIN,
			       &vol);
	if (rc < 0) {
		chg_err("get battery voltage min error, rc=%d\n", rc);
		vol = 0;
...
static int oplus_mms_gauge_update_curr(struct oplus_mms *mms,
				       union mms_msg_data *data)
{
	struct oplus_mms_gauge *chip;
	int curr;
...
static int oplus_mms_gauge_update_temp(struct oplus_mms *mms,
				       union mms_msg_data *data)
{
	struct oplus_mms_gauge *chip;
	int temp;
...
static int oplus_mms_gauge_update_fcc(struct oplus_mms *mms,
				      union mms_msg_data *data)
{
	struct oplus_mms_gauge *chip;
	int fcc;
	...
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_BATT_FCC,
			       &fcc);
	if (rc < 0) {
		chg_err("get battery fcc error, rc=%d\n", rc);
		fcc = 0;
...
static int oplus_mms_gauge_update_rm(struct oplus_mms *mms,
				     union mms_msg_data *data)
{
	struct oplus_mms_gauge *chip;
	int rm;
	...
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_BATT_RM,
			       &rm);
	if (rc < 0) {
		chg_err("get battery remaining capacity error, rc=%d\n", rc);
		rm = 0;
...
static int oplus_mms_gauge_update_cc(struct oplus_mms *mms,
				     union mms_msg_data *data)
{
	struct oplus_mms_gauge *chip;
	int cc;
	...
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_BATT_CC,
			       &cc);
	if (rc < 0) {
		chg_err("get battery cc error, rc=%d\n", rc);
		cc = 0;
...
static int oplus_mms_gauge_update_soh(struct oplus_mms *mms,
				      union mms_msg_data *data)
{
	struct oplus_mms_gauge *chip;
	int soh;
	...
	rc = oplus_chg_ic_func(chip->gauge_ic, OPLUS_IC_FUNC_GAUGE_GET_BATT_SOH,
			       &soh);
	if (rc < 0) {
		chg_err("get battery soh error, rc=%d\n", rc);
		soh = 0;
...
static const u32 oplus_mms_gauge_update_item[] = {
	GAUGE_ITEM_SOC,	 GAUGE_ITEM_VOL_MAX, GAUGE_ITEM_VOL_MIN,
	GAUGE_ITEM_CURR, GAUGE_ITEM_TEMP,    GAUGE_ITEM_FCC,
	GAUGE_ITEM_CC,	 GAUGE_ITEM_SOH,     GAUGE_ITEM_RM,
};
...
*/
	...
	if (chip->wired_online || chip->wls_online)
		vote(chip->gauge_update_votable, USER_VOTER, true,
		     CHARGE_UPDATE_INTERVAL, false);
	else
...
static int oplus_mms_gauge_check_batt_vol_diff(struct oplus_mms_gauge *chip)
{
	...
	rc = oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_VOL_MAX,
				     &data, false);
	if (rc < 0) {
	...
	rc = oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_VOL_MIN,
				     &data, false);
	if (rc < 0) {
	...
	if (abs(vbat_max - vbat_min) > ALLOW_DIFF_VALUE_MV) {
		...
		if (chip->check_batt_vol_count > BATT_VOL_ERR_COUNT_MAX) {
			...
			if (chg_disable_votable)
				vote(chg_disable_votable, VOL_DIFF_VOTER, true,
				     1, false);
			if (!chip->bat_volt_different)
...
static int oplus_mms_gauge_topic_init(struct oplus_mms_gauge *chip)
{
	...
	chip->gauge_topic = devm_oplus_mms_register(
		chip->dev, &oplus_mms_gauge_desc, &mms_cfg);
	if (IS_ERR(chip->gauge_topic)) {
		chg_err("Couldn't register gauge topic\n");
		rc = PTR_ERR(chip->gauge_topic);
		return rc;
	}
	vote(chip->gauge_update_votable, DEF_VOTER, true,
	     oplus_mms_gauge_desc.update_interval, false);

	oplus_mms_gauge_subscribe_gauge_topic(chip);
	oplus_mms_gauge_push_hmac(chip);
	oplus_mms_wait_topic("common", oplus_mms_gauge_subscribe_comm_topic,
			     chip);
	oplus_mms_wait_topic("wired", oplus_mms_gauge_subscribe_wired_topic,
			     chip);
...
static int oplus_mms_gauge_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&chip->hal_gauge_init_work,
			  oplus_mms_gauge_init_work);
	INIT_WORK(&chip->err_handler_work, oplus_mms_gauge_err_handler_work);
	INIT_WORK(&chip->online_handler_work,
		  oplus_mms_gauge_online_handler_work);
	INIT_WORK(&chip->offline_handler_work,
		  oplus_mms_gauge_offline_handler_work);
	INIT_WORK(&chip->resume_handler_work,
		  oplus_mms_gauge_resume_handler_work);
	INIT_WORK(&chip->update_change_work,
		  oplus_mms_gauge_update_change_work);
