...
bool oplus_mms_item_update(struct oplus_mms *mms, u32 item_id,
			   bool check_update)
{
	struct mms_item *item;
...
__printf(4, 5) struct mms_msg *oplus_mms_alloc_str_msg(enum mms_msg_type type,
				 ...
				 struct oplus_chg_ic_err_msg *err_msg)
{
	...
	topic_msg = oplus_mms_alloc_str_msg(MSG_TYPE_ITEM, MSG_PRIO_HIGH,
					    item_id, "[%s]-[%d]-[%d]:%s",
					    err_msg->ic->name, err_msg->type,
					    err_msg->sub_type, err_msg->msg);
	if (topic_msg == NULL) {
...
__printf(4, 5) struct mms_subscribe *oplus_mms_subscribe(
...
static struct device_attribute *oplus_mms_attributes[] = {
	&dev_attr_item_id, &dev_attr_data, &dev_attr_subscribe, NULL
};
...
static void debug_subs_callback(struct mms_subscribe *subs,
				enum mms_msg_type type, u32 id)
{
	...
	snprintf(item_env_buf + item_env_len, PAGE_SIZE - item_env_len, "%d",
		 id);
	item = oplus_mms_get_item(mms, id);
	if (item == NULL) {
...
{
	struct oplus_mms *mms =
		container_of(work, struct oplus_mms, update_work.work);

	mms->desc->update(mms, true);

	if (mms->normal_update_interval > 0)
		queue_delayed_work(
			mms_wq, &mms->update_work,
			msecs_to_jiffies(mms->normal_update_interval));
}
...
{
	struct oplus_mms *mms =
		container_of(work, struct oplus_mms, msg_work.work);
...
oplus_mms_register(struct device *parent, const struct oplus_mms_desc *desc,
		   const struct oplus_mms_config *cfg)
{
	return __oplus_mms_register(parent, desc, cfg, true);
}

struct oplus_mms *__must_check oplus_mms_register_no_ws(
	struct device *parent, const struct oplus_mms_desc *desc,
	const struct oplus_mms_config *cfg)
{
...
struct oplus_mms *__must_check devm_oplus_mms_register(
	struct device *parent, const struct oplus_mms_desc *desc,
	const struct oplus_mms_config *cfg)
{
...
struct oplus_mms *__must_check devm_oplus_mms_register_no_ws(
	struct device *parent, const struct oplus_mms_desc *desc,
	const struct oplus_mms_config *cfg)
{
	struct oplus_mms **ptr, *mms;
...
static __init int oplus_mms_class_init(void)
{
	...
	mms_wq = alloc_workqueue("mms_wq",
				 WQ_UNBOUND | WQ_FREEZABLE | WQ_HIGHPRI, 0);
	if (!mms_wq) {
