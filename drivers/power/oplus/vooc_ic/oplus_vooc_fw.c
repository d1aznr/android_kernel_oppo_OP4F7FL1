...
int oplus_mcu_bcc_stop_curr_dt(struct oplus_vooc_chip *chip)
{
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_0_to_50_little_cold_stop_cur",
				  &chip->svooc_0_to_50_little_cold_stop_cur);
	if (rc) {
		chip->svooc_0_to_50_little_cold_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_0_to_50_cold_stop_cur",
				  &chip->svooc_0_to_50_cold_stop_cur);
	if (rc) {
		chip->svooc_0_to_50_cold_stop_cur = 0;
	}

	rc = of_property_read_u32(node,
				  "qcom,svooc_0_to_50_little_cool_stop_cur",
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_0_to_50_normal_low_stop_cur",
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_0_to_50_normal_high_stop_cur",
				  &chip->svooc_0_to_50_normal_high_stop_cur);
	if (rc) {
		chip->svooc_0_to_50_normal_high_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_0_to_50_warm_stop_cur",
				  &chip->svooc_0_to_50_warm_stop_cur);
	if (rc) {
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_51_to_75_little_cold_stop_cur",
				  &chip->svooc_51_to_75_little_cold_stop_cur);
	if (rc) {
		chip->svooc_51_to_75_little_cold_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_51_to_75_cold_stop_cur",
				  &chip->svooc_51_to_75_cold_stop_cur);
	if (rc) {
		chip->svooc_51_to_75_cold_stop_cur = 0;
	}

	rc = of_property_read_u32(node,
				  "qcom,svooc_51_to_75_little_cool_stop_cur",
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_51_to_75_normal_low_stop_cur",
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_51_to_75_normal_high_stop_cur",
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_76_to_85_little_cold_stop_cur",
				  &chip->svooc_76_to_85_little_cold_stop_cur);
	if (rc) {
		chip->svooc_76_to_85_little_cold_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_76_to_85_cold_stop_cur",
				  &chip->svooc_76_to_85_cold_stop_cur);
	if (rc) {
		chip->svooc_76_to_85_cold_stop_cur = 0;
	}

	rc = of_property_read_u32(node,
				  "qcom,svooc_76_to_85_little_cool_stop_cur",
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_76_to_85_normal_low_stop_cur",
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_76_to_85_normal_high_stop_cur",
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_86_to_90_little_cold_stop_cur",
				  &chip->svooc_86_to_90_little_cold_stop_cur);
	if (rc) {
		chip->svooc_86_to_90_little_cold_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_86_to_90_cold_stop_cur",
				  &chip->svooc_86_to_90_cold_stop_cur);
	if (rc) {
		chip->svooc_86_to_90_cold_stop_cur = 0;
	}

	rc = of_property_read_u32(node,
				  "qcom,svooc_86_to_90_little_cool_stop_cur",
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_86_to_90_normal_low_stop_cur",
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_86_to_90_normal_high_stop_cur",
...
static int opchg_bq27541_gpio_pinctrl_init(struct oplus_vooc_chip *chip)
{
	...
	if (1) {
		chip->vooc_gpio.gpio_switch1_act_switch2_act =
			pinctrl_lookup_state(chip->vooc_gpio.pinctrl,
		...
		chip->vooc_gpio.gpio_switch1_ctr1_sleep = pinctrl_lookup_state(
			chip->vooc_gpio.pinctrl, "charging_switch1_ctr1_sleep");
	...
	chip->vooc_gpio.gpio_switch1_act_switch2_sleep = pinctrl_lookup_state(
		chip->vooc_gpio.pinctrl, "switch1_act_switch2_sleep");
	...
	chip->vooc_gpio.gpio_switch1_sleep_switch2_act = pinctrl_lookup_state(
		chip->vooc_gpio.pinctrl, "switch1_sleep_switch2_act");
	...
	chip->vooc_gpio.gpio_mcu_ctrl_cp_active = pinctrl_lookup_state(
		chip->vooc_gpio.pinctrl, "mcu_ctrl_active");
	if (IS_ERR_OR_NULL(chip->vooc_gpio.gpio_mcu_ctrl_cp_active)) {
		...
		pinctrl_select_state(
			chip->vooc_gpio.pinctrl,
			chip->vooc_gpio.gpio_mcu_ctrl_cp_active); /* PULL_down */
	}
...
void oplus_vooc_fw_type_dt(struct oplus_vooc_chip *chip)
{
	...
	chip->vooc_current_lvl_cnt = of_property_count_elems_of_size(
		node, "qcom,vooc_current_lvl", sizeof(*chip->vooc_current_lvl));
	if (chip->vooc_current_lvl_cnt > 0) {
		chg_err("vooc_current_lvl_cnt[%d]\n",
			chip->vooc_current_lvl_cnt);
		chip->vooc_current_lvl =
			devm_kcalloc(chip->dev, chip->vooc_current_lvl_cnt,
		...
		rc = of_property_read_u32_array(node, "qcom,vooc_current_lvl",
						chip->vooc_current_lvl,
						chip->vooc_current_lvl_cnt);
		if (rc) {
	...
	chip->support_vooc_by_normal_charger_path = of_property_read_bool(
		node, "qcom,support_vooc_by_normal_charger_path");

#ifdef OPLUS_CUSTOM_OP_DEF
	chip->hiz_gnd_cable =
		of_property_read_bool(node, "qcom,supported_hiz_gnd_cable");
#endif
	chip->support_high_watt_svooc =
		of_property_read_bool(node, "qcom,support_high_watt_svooc");
	rc = of_property_read_u32(node, "qcom,vooc-fw-type",
				  &chip->vooc_fw_type);
	if (rc) {
		chip->vooc_fw_type = VOOC_FW_TYPE_INVALID;
	}

	rc = of_property_read_u32(node, "qcom,vooc-dis-temp-soc",
				  &chip->vooc_dis_temp_soc);
	if (rc) {
		chip->vooc_dis_temp_soc = 0;
	}

	rc = of_property_read_u32(node, "qcom,vooc-dis-id-verify",
				  &chip->vooc_dis_id_verify);
	if (rc) {
		chip->vooc_dis_id_verify = 0;
	}
	chg_debug(
		"oplus_vooc_fw_type_dt batt_type_4400 is %d,vooc_fw_type = 0x%x\n",
		chip->batt_type_4400mv, chip->vooc_fw_type);
	chip->support_single_batt_svooc =
		of_property_read_bool(node, "qcom,support-single-batt-svooc");
	chg_debug("oplus_vooc_fw_type_dt support_single_batt_svooc is %d\n",
		  chip->support_single_batt_svooc);

	chip->support_old_svooc_1_0 = of_property_read_bool(
		node, "qcom,support-old-svooc-1_0"); /*20638 RT5125 50W*/
	chg_debug("oplus_vooc_fw_type_dt support-old-svooc-1_0 is %d\n",
		  chip->support_old_svooc_1_0);

	chip->vooc_is_platform_gauge =
		of_property_read_bool(node, "qcom,vooc_is_platform_gauge");
	chg_debug("oplus_vooc_fw_type_dt vooc_is_platform_gauge is %d\n",
		  chip->vooc_is_platform_gauge);

	chip->vooc_fw_update_newmethod =
		of_property_read_bool(node, "qcom,vooc_fw_update_newmethod");
	...
	rc = of_property_read_u32(node, "qcom,vooc_cool_bat_volt",
	...
	rc = of_property_read_u32(node, "qcom,vooc_little_cool_bat_volt",
	...
	rc = of_property_read_u32(node, "qcom,vooc_normal_bat_volt",
	...
	rc = of_property_read_u32(node, "qcom,vooc_warm_bat_volt",
	...
	rc = of_property_read_u32(node, "qcom,vooc_cool_bat_suspend_volt",
	...
	rc = of_property_read_u32(node,
				  "qcom,vooc_little_cool_bat_suspend_volt",
	...
	rc = of_property_read_u32(node, "qcom,vooc_normal_bat_suspend_volt",
	...
	rc = of_property_read_u32(node, "qcom,vooc_warm_bat_suspend_volt",
	...
	chip->vooc_multistep_adjust_current_support = of_property_read_bool(
		node, "qcom,vooc_multistep_adjust_current_support");
	...
	chip->vooc_low_temp_smart_charge =
		of_property_read_bool(node, "qcom,vooc_low_temp_smart_charge");
	...
	chip->parse_fw_from_dt =
		of_property_read_bool(node, "qcom,parse_fw_from_dt");
	chg_debug("qcom,parse_fw_from_dt is %d\n", chip->parse_fw_from_dt);

	chip->abnormal_adapter_current_cnt = of_property_count_elems_of_size(
		node, "qcom,abnormal_adapter_current",
	...
	if (chip->abnormal_adapter_current_cnt > 0) {
		...
		chip->abnormal_adapter_current = devm_kcalloc(
			chip->dev, chip->abnormal_adapter_current_cnt,
		...
		for (loop = 0; loop < chip->abnormal_adapter_current_cnt;
		     loop++) {
	...
	chip->vooc_switch_reset =
		of_property_read_bool(node, "qcom,vooc_switch_reset");
...
int oplus_vooc_asic_hwid_check(struct oplus_vooc_chip *chip)
{
	...
	chip->vooc_gpio.vooc_asic_id_gpio =
		of_get_named_gpio(node, "qcom,vooc_asic_id-gpio", 0);
	if (chip->vooc_gpio.vooc_asic_id_gpio < 0) {
		...
		if (gpio_is_valid(chip->vooc_gpio.vooc_asic_id_gpio)) {
			rc = gpio_request(chip->vooc_gpio.vooc_asic_id_gpio,
					  "vooc-asic-id-gpio");
			if (rc) {
	...
	chip->vooc_gpio.gpio_vooc_asic_id_sleep = pinctrl_lookup_state(
		chip->vooc_gpio.pinctrl, "vooc_asic_id_sleep");
	...
	chip->vooc_gpio.gpio_vooc_asic_id_active = pinctrl_lookup_state(
		chip->vooc_gpio.pinctrl, "vooc_asic_id_active");
	...
	if (gpio_get_value(chip->vooc_gpio.vooc_asic_id_gpio) == 1) {
		...
		rc = of_property_read_u32(node, "qcom,vooc_hw_id_no_pull_val",
					  &hw_id_temp);
		if (!rc) {
...
/*This is only for P60 P(17197 P)*/
	...
	chip->vooc_gpio.vooc_mcu_id_gpio =
		of_get_named_gpio(node, "qcom,vooc_mcu_id-gpio", 0);
	if (chip->vooc_gpio.vooc_mcu_id_gpio < 0) {
		old_project_disable_hwid_check = of_property_read_bool(
			node, "old_project_disable_hwid_check");
		...
		if (gpio_is_valid(chip->vooc_gpio.vooc_mcu_id_gpio)) {
			rc = gpio_request(chip->vooc_gpio.vooc_mcu_id_gpio,
					  "vooc-mcu-id-gpio");
			if (rc) {
				chg_err("unable to request gpio [%d]\n",
					chip->vooc_gpio.vooc_mcu_id_gpio);
	...
	chip->vooc_gpio.gpio_vooc_mcu_id_default = pinctrl_lookup_state(
		chip->vooc_gpio.pinctrl, "vooc_mcu_id_default");
...
int oplus_vooc_gpio_dt_init(struct oplus_vooc_chip *chip)
{
	...
	if (chip->vooc_gpio.switch1_ctr1_gpio < 0) {
		...
		if (gpio_is_valid(chip->vooc_gpio.switch1_ctr1_gpio)) {
			rc = gpio_request(chip->vooc_gpio.switch1_ctr1_gpio,
			...
			if (rc) {
				...
				gpio_direction_output(
					chip->vooc_gpio.switch1_ctr1_gpio, 0);
			}
		}
	...
	if (1) {
		chip->vooc_gpio.switch2_gpio = of_get_named_gpio(
	...
	if (chip->vooc_gpio.clock_gpio < 0) {
		...
		if (gpio_is_valid(chip->vooc_gpio.clock_gpio)) {
			rc = gpio_request(chip->vooc_gpio.clock_gpio,
					  "charging-clock-gpio");
			if (rc) {
	...
	if (chip->vooc_gpio.data_gpio < 0) {
		...
		if (gpio_is_valid(chip->vooc_gpio.data_gpio)) {
			rc = gpio_request(chip->vooc_gpio.data_gpio,
					  "charging-data-gpio");
			if (rc) {
				chg_err("unable to request gpio [%d]\n",
					chip->vooc_gpio.data_gpio);
	...
	chip->vooc_gpio.mcu_ctrl_cp_gpio =
		of_get_named_gpio(node, "qcom,mcu_ctrl_cp-gpio", 0);
	if (chip->vooc_gpio.mcu_ctrl_cp_gpio < 0) {
		...
		if (gpio_is_valid(chip->vooc_gpio.mcu_ctrl_cp_gpio)) {
			rc = gpio_request(chip->vooc_gpio.mcu_ctrl_cp_gpio,
					  "mcu-ctrl-cp-gpio");
			if (rc) {
				chg_err("unable to request gpio [%d]\n",
					chip->vooc_gpio.mcu_ctrl_cp_gpio);
...
void opchg_set_clock_active(struct oplus_vooc_chip *chip)
{
	...
	pinctrl_select_state(chip->vooc_gpio.pinctrl,
			     chip->vooc_gpio.gpio_clock_sleep); /* PULL_down */
...
void opchg_set_clock_sleep(struct oplus_vooc_chip *chip)
{
	...
	mutex_lock(&chip->pinctrl_mutex);
	pinctrl_select_state(chip->vooc_gpio.pinctrl,
			     chip->vooc_gpio.gpio_clock_active); /* PULL_up */
...
void opchg_set_data_active(struct oplus_vooc_chip *chip)
{
	...
	pinctrl_select_state(chip->vooc_gpio.pinctrl,
			     chip->vooc_gpio.gpio_data_active); /* no_PULL */
	mutex_unlock(&chip->pinctrl_mutex);
}
...
void opchg_set_data_sleep(struct oplus_vooc_chip *chip)
{
	mutex_lock(&chip->pinctrl_mutex);
	pinctrl_select_state(chip->vooc_gpio.pinctrl,
			     chip->vooc_gpio.gpio_data_sleep); /* PULL_down */
...
void opchg_set_reset_active_force(struct oplus_vooc_chip *chip)
{
	...
	if (!chip->vooc_switch_reset)
		gpio_direction_output(chip->vooc_gpio.switch1_gpio,
				      0); /* in 0*/
	gpio_direction_output(chip->vooc_gpio.reset_gpio,
			      active_level); /* out 1 */
...
static void delay_reset_mcu_work_func(struct work_struct *work)
{
	...
	struct oplus_vooc_chip *chip = container_of(
		dwork, struct oplus_vooc_chip, delay_reset_mcu_work);
...
static bool is_allow_fast_chg_dummy(struct oplus_vooc_chip *chip)
{
	...
	if ((oplus_vooc_get_fastchg_dummy_started() == true ||
		...
		chip->reset_adapter = false;
		chg_debug(
			" dummy_started or fastchg to warm true, allow_real false\n");
		return false;
	}
	...
	if (mcu_hwid_type == OPLUS_VOOC_ASIC_HWID_RK826 ||
		...
		if ((oplus_vooc_get_fastchg_dummy_started() == true ||
			...
			chg_debug(
				" dummy_started or fastchg to warm true, allow_real true, reset adapter\n");
			return false;
		}
	}
...
void switch_fast_chg(struct oplus_vooc_chip *chip)
{
	...
	if (is_allow_fast_chg_dummy(chip) == true) {
		if (!chip->vooc_switch_reset) {
			if (oplus_vooc_get_adapter_update_status() ==
				...
				if (oplus_vooc_get_fastchg_allow() == false &&
					...
					mcu_hwid_type = get_vooc_mcu_type(chip);
					if (mcu_hwid_type ==
						    OPLUS_VOOC_ASIC_HWID_RK826 ||
					    mcu_hwid_type ==
						    OPLUS_VOOC_ASIC_HWID_OP10 ||
					    mcu_hwid_type ==
						    OPLUS_VOOC_ASIC_HWID_RT5125) {
						oplus_vooc_reset_mcu();
					...
					if (opchg_get_mcu_update_state() ==
						    false &&
					    oplus_vooc_check_asic_fw_status() ==
						    0) {
			  ...
			  int device_type)
{
	...
	if (chip->w_soc_temp_to_mcu == false) {
		for (i = 1; i < reply_counts; i++) {
			gpio_set_value(chip->vooc_gpio.data_gpio,
...
void opchg_set_switch_fast_charger(struct oplus_vooc_chip *chip)
{
	...
	if (chip->vooc_gpio.switch1_ctr1_gpio > 0) { //asic rk826
		gpio_direction_output(chip->vooc_gpio.switch1_ctr1_gpio,
				      0); /* out 0*/
	}
}
	...
	if (node) {
		if (node_new) {
			chip->vooc_gpio.data_irq =
				gpio_to_irq(chip->vooc_gpio.data_gpio);
...
void oplus_vooc_eint_register(struct oplus_vooc_chip *chip)
{
	...
	if (node) {
		...
		ret = request_irq(chip->vooc_gpio.data_irq,
				  (irq_handler_t)irq_rx_handler,
				  IRQF_TRIGGER_RISING, "VOOC_AP_DATA-eint",
				  chip);
		if (ret < 0) {
	...
	int retval = 0;
	opchg_set_data_active(chip);
	retval = request_irq(chip->vooc_gpio.data_irq, irq_rx_handler,
...
void oplus_vooc_set_mcu_pps_mode(struct oplus_vooc_chip *chip, bool mode)
{
	...
	if (mode) {
		...
		retval = gpio_direction_output(chip->vooc_gpio.mcu_ctrl_cp_gpio,
					       1);
