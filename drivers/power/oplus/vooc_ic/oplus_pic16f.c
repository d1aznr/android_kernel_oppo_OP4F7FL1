...
#ifdef CONFIG_OPLUS_CHARGER_MTK
		if (addr == ((Pic16F_firmware_data[fw_line * 34 + 1] << 8) |
		...
		if (addr == ((Pic16F_firmware_data[fw_line * 34 + 1] << 8) |
			...
			if (data_buf[0] !=
				...
				chg_err(" addr = 0x%x,%x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x\n",
					...
					data_buf[8], data_buf[9], data_buf[10],
					data_buf[11], data_buf[12],
					data_buf[13], data_buf[14],
					data_buf[15], data_buf[16],
					data_buf[17], data_buf[18],
					data_buf[19], data_buf[20],
					data_buf[21], data_buf[22],
					data_buf[23], data_buf[24],
					data_buf[25], data_buf[26],
					data_buf[27], data_buf[28],
					data_buf[29], data_buf[30],
					data_buf[31]);
				return FW_CHECK_FAIL;
			}
			fw_line++;
...
static void pic16f_fw_data_recover(struct oplus_vooc_chip *chip,
				   unsigned char *data_buf, unsigned int offset,
				   unsigned int length)
{
...
static int pic16f_fw_write(struct oplus_vooc_chip *chip,
			   unsigned char *data_buf, unsigned int offset,
			   unsigned int length)
{
	unsigned int count = 0;
...
update_fw:
	...
	pic16f_fw_write(chip, Pic16F_firmware_data, 0,
	...
	pic16f_fw_data_recover(chip, Pic16F_firmware_data, 0,
			       sizeof(Pic16F_firmware_data) - 34);
	msleep(10);
	if (rc == FW_CHECK_FAIL) {
		download_again++;
	...
	rc = pic16f_fw_write(chip, Pic16F_firmware_data,
...
#endif
{
	...
	if (i2c_add_driver(&pic16f_i2c_driver) != 0) {
		...
		chg_debug(
			"[pic16f_init] Success to register pic16f i2c driver.\n");
	}
	return ret;
}
