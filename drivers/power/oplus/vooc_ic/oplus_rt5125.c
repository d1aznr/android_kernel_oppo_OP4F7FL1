...
static int oplus_i2c_dma_read(struct i2c_client *client, u8 addr, s32 len,
			      u8 *rxbuf)
{
	int ret;
	s32 retry = 0;
...
static int oplus_i2c_dma_write(struct i2c_client *client, u8 addr, s32 len,
			       u8 const *txbuf)
{
	int ret = 0;
	s32 retry = 0;
...
static int rt5125_i2c_block_read(struct oplus_vooc_chip *chip, u8 cmd, u8 *data,
				 s32 len)
{
	u8 crc;
	int retry = 0, ret;
...
static int rt5125_i2c_block_write(struct oplus_vooc_chip *chip, u8 cmd,
...
retry_write:
	...
	if (ret < 0)
		goto out_write;
	ret = oplus_i2c_dma_read(chip->client, RT5125_CMD_CRC8_INFO, 1,
				 data_buff);
	if (ret < 0)
		goto out_write;
...
static int rt5125_fw_update(struct oplus_vooc_chip *chip)
{
	...
	for (i = 0; i < len; i += DEFAULT_MAX_PAGELEN) {
		...
		elapsed = len - i;
		wr_len = (elapsed > DEFAULT_MAX_PAGELEN) ? DEFAULT_MAX_PAGELEN :
							   elapsed;
		memset(fwdata, 0xff, DEFAULT_MAX_PAGELEN);
		memcpy(fwdata, data + i, wr_len);
		/* page idx */
		rwdata = (u8)idx;
		ret = rt5125_i2c_block_write(chip, RT5125_PAGE_IDX, &rwdata,
					     sizeof(rwdata));
		if (ret < 0) {
		...
		/* page data */
		ret = rt5125_i2c_block_write(chip, RT5125_DATA_BUF, fwdata,
					     DEFAULT_MAX_PAGELEN);
		if (ret < 0) {
		...
		ret = rt5125_i2c_block_write(chip, RT5125_ACCESS_CTL, &rwdata,
					     sizeof(rwdata));
		if (ret < 0) {
	...
	busy_check:
		...
		ret = rt5125_i2c_block_read(chip, RT5125_STATUS, &rwdata,
					    sizeof(rwdata));
		if (ret < 0) {
...
keyword_retry:
	...
	/* page data */
	ret = rt5125_i2c_block_read(chip, RT5125_DATA_BUF, last_page,
				    DEFAULT_MAX_PAGELEN);
	if (ret < 0) {
...
crc_busy_retry:
	...
	idx = DEFAULT_MAX_PAGELEN + fw_info % DEFAULT_MAX_PAGELEN -
	      DEFAULT_VERINFO_LEN;
...
int rt5125_get_battery_mvolts_current(void)
{
	...
	current_bat = (read_buf[3] << 24) | (read_buf[2] << 16) |
...
static void rt5125_update_battery_temperature_soc(struct work_struct *work)
{
	...
	schedule_delayed_work(&rt5125_update_temp_soc,
...
{
	INIT_DELAYED_WORK(&rt5125_update_temp_soc,
			  rt5125_update_battery_temperature_soc);
...
static int rt5125_parse_fw_from_array(struct oplus_vooc_chip *chip)
{
	...
	if (chip->vooc_fw_type == VOOC_FW_TYPE_RT5125_4450_VOOC_FFC_15C) {
		...
		chip->fw_data_count = sizeof(rt5125_fw);
		chip->fw_data_version = rt5125_fw[chip->fw_data_count - 4];
	} else if (chip->vooc_fw_type ==
		   VOOC_FW_TYPE_RT5125_4450_VOOC_FFC_5V6A_7BIT) {
		chip->firmware_data = rt5125_fw_data_4450_vooc_ffc_5v6a_7bit;
		chip->fw_data_count =
			sizeof(rt5125_fw_data_4450_vooc_ffc_5v6a_7bit);
		chip->fw_data_version = rt5125_fw_data_4450_vooc_ffc_5v6a_7bit
		...
		chip->fw_data_count =
			sizeof(rt5125_fw_data_4450_vooc_ffc_10v5a_7bit);
		chip->fw_data_version = rt5125_fw_data_4450_vooc_ffc_10v5a_7bit
			[chip->fw_data_count - 4];
	} else if (chip->vooc_fw_type ==
		   VOOC_FW_TYPE_RT5125_4450_VOOC_FFC_10V6P5A_7BIT) {
		chip->firmware_data = rt5125_fw_data_4450_vooc_ffc_10v6p5a_7bit;
		chip->fw_data_count =
			sizeof(rt5125_fw_data_4450_vooc_ffc_10v6p5a_7bit);
		chip->fw_data_version =
			rt5125_fw_data_4450_vooc_ffc_10v6p5a_7bit
				[chip->fw_data_count - 4];
	} else if (chip->vooc_fw_type ==
		   VOOC_FW_TYPE_RT5125_4450_VOOC_FFC_10V6P5A_7BIT_JIN) {
		chip->firmware_data =
			rt5125_fw_data_4450_vooc_ffc_10v6p5a_7bit_jin;
		chip->fw_data_count =
			sizeof(rt5125_fw_data_4450_vooc_ffc_10v6p5a_7bit_jin);
		chip->fw_data_version =
			rt5125_fw_data_4450_vooc_ffc_10v6p5a_7bit_jin
				[chip->fw_data_count - 4];
	} else if (chip->vooc_fw_type ==
		...
		chip->firmware_data =
			rt5125_fw_data_4500_svooc_ffc_6500ma_7bit_20817;
		chip->fw_data_count =
			sizeof(rt5125_fw_data_4500_svooc_ffc_6500ma_7bit_20817);
		chip->fw_data_version =
			rt5125_fw_data_4500_svooc_ffc_6500ma_7bit_20817
				[chip->fw_data_count - 4];
	} else if (chip->vooc_fw_type ==
		...
		chip->firmware_data =
			rt5125_fw_data_4500_svooc_ffc_6500ma_7bit_walle;
		chip->fw_data_count =
			sizeof(rt5125_fw_data_4500_svooc_ffc_6500ma_7bit_walle);
		chip->fw_data_version =
			rt5125_fw_data_4500_svooc_ffc_6500ma_7bit_walle
				[chip->fw_data_count - 4];
	}
			       ...
			       const struct i2c_device_id *id)
{
	...
	rt5125_update_wake_lock =
		wakeup_source_register(NULL, "rt5125_update_wake_lock");
