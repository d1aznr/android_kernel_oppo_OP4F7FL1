		...
		for (j = 0; j < 16; j++) {
			if (data_buf[j] !=
				...
				chg_err("fail, data_buf[%d]:0x%x != n76e_firmware_data[%d]:0x%x\n",
					...
					chip->firmware_data[fw_line * 18 + 2 +
							    j]);
	...
	for (i = 0; i < 16; i++) {
		if (data_buf[i] !=
			...
			chg_err("fail, data_buf[%d]:0x%x != n76e_firmware_data[]:0x%x\n",
				i, data_buf[i],
				chip->firmware_data[chip->fw_data_count - 16 +
						    i]);
...
update_fw:
	...
	rc = n76e_fw_write(chip, chip->firmware_data, chip->fw_data_count - 18,
			   18);
...
static int n76e_fw_check_then_recover(struct oplus_vooc_chip *chip)
{
	...
	if (oplus_is_power_off_charging(chip) == true ||
		...
		chip->mcu_boot_by_gpio = false;
		opchg_set_clock_sleep(chip);
		if (n76e_fw_check_frontline(chip) == FW_CHECK_FAIL ||
		    n76e_fw_check_lastline(chip) == FW_CHECK_FAIL) {
			n76e_fw_update(chip);
		} else {
			chip->vooc_fw_check = true;
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (chip->batt_type_4400mv) {
		...
		chip->fw_data_version =
			n76e_fw_data_4400[chip->fw_data_count - 4];
	} else { //default
		chip->firmware_data = n76e_fw_data_4400;
		chip->fw_data_count = sizeof(n76e_fw_data_4400);
		chip->fw_data_version =
	...
	if (chip->vooc_fw_type ==
		...
		chip->fw_data_version =
			n76e_fw_data_4400_ntc61c[chip->fw_data_count - 4];
		...
		chip->fw_data_version =
			n76e_fw_data_4400_vooc_ffc_15c[chip->fw_data_count - 4];
		...
		chip->fw_data_count =
			sizeof(n76e_fw_data_4400_vooc_ffc_15c_fv4450);
		chip->fw_data_version = n76e_fw_data_4400_vooc_ffc_15c_fv4450
