			...
			for (j = 0; j < 32; j++) {
				if (data_buf[j] !=
					...
					chg_err("addr = 0x%x, %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x "
						...
						addr, data_buf[0], data_buf[1],
						data_buf[2], data_buf[3],
						data_buf[4], data_buf[5],
						data_buf[6], data_buf[7],
						data_buf[8], data_buf[9],
						data_buf[10], data_buf[11],
						data_buf[12], data_buf[13],
						data_buf[14], data_buf[15],
						data_buf[16], data_buf[17],
						data_buf[18], data_buf[19],
						data_buf[20], data_buf[21],
						data_buf[22], data_buf[23],
						data_buf[24], data_buf[25],
						data_buf[26], data_buf[27],
						data_buf[28], data_buf[29],
	...
	for (i = 0; i < 32; i++) {
		if (data_buf[i] !=
			...
			chg_err("  fail, data_buf[%d]:0x%x != Stm8s_firmware_data[]:0x%x\n",
				i, data_buf[i],
				chip->firmware_data[chip->fw_data_count - 32 +
						    i]);
			chg_err(" data %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x \
				...
				data_buf[9], data_buf[10], data_buf[11],
				data_buf[12], data_buf[13], data_buf[14],
				data_buf[15], data_buf[16], data_buf[17],
				data_buf[18], data_buf[19], data_buf[20],
				data_buf[21], data_buf[22], data_buf[23],
				data_buf[24], data_buf[25], data_buf[26],
				data_buf[27], data_buf[28], data_buf[29],
				data_buf[30], data_buf[31]);
...
static int stm8s_fw_write(struct oplus_vooc_chip *chip,
			  const unsigned char *data_buf, unsigned int offset,
			  unsigned int length)
{
	...
	while (count < (offset + length)) {
		...
		oplus_vooc_i2c_write(
			chip->client, 0x02, BYTES_TO_WRITE,
...
update_fw:
	...
	rc = stm8s_fw_write(chip, chip->firmware_data, chip->fw_data_count - 34,
			    34);
	if (rc < 0) {
		goto update_fw_err;
	}
...
static stm8s_parse_fw_from_array(struct oplus_vooc_chip *chip)
{
	...
	case VOOC_FW_TYPE_STM8S_4400_AVOID_FAKE_ADAPTER:
		...
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4400_avoid_fake_adapter);
		chip->fw_data_version = Stm8s_fw_data_4400_avoid_fake_adapter
			[chip->fw_data_count - FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_AVOID_FG_I2C_ERR:
		chip->firmware_data = Stm8s_fw_data_4400_avoid_fg_i2c_err;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4400_avoid_fg_i2c_err);
		chip->fw_data_version =
			Stm8s_fw_data_4400_avoid_fg_i2c_err[chip->fw_data_count -
							    FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_AVOID_OVER_TEMP:
		chip->firmware_data = Stm8s_fw_data_4400_avoid_over_temp;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4400_avoid_over_temp);
		chip->fw_data_version =
			Stm8s_fw_data_4400_avoid_over_temp[chip->fw_data_count -
							   FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_AVOID_OVER_TEMP_NTC61C:
		chip->firmware_data = Stm8s_fw_data_4400_avoid_over_temp_ntc61c;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4400_avoid_over_temp_ntc61c);
		chip->fw_data_version =
			Stm8s_fw_data_4400_avoid_over_temp_ntc61c
				[chip->fw_data_count - FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_VOOC_FFC_09C:
		...
		chip->fw_data_version =
			Stm8s_fw_data_4400_vooc_ffc_09c[chip->fw_data_count -
							FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_VOOC_FFC_15C:
		...
		chip->fw_data_version =
			Stm8s_fw_data_4400_vooc_ffc_15c[chip->fw_data_count -
							FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_VOOC_FFC_15C_FV4450:
		chip->firmware_data = Stm8s_fw_data_4400_vooc_ffc_15c_fv4450;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4400_vooc_ffc_15c_fv4450);
		chip->fw_data_version = Stm8s_fw_data_4400_vooc_ffc_15c_fv4450
			[chip->fw_data_count - FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4450_FFC:
		...
		chip->fw_data_version =
			Stm8s_fw_data_4450_ffc[chip->fw_data_count -
					       FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4450_FFC_5V6A:
		...
		chip->fw_data_version =
			Stm8s_fw_data_4450_ffc_5v6a[chip->fw_data_count -
						    FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_VOOC_FFC_15C_TI411:
		chip->firmware_data = Stm8s_fw_data_4400_vooc_ffc_15c_ti411;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4400_vooc_ffc_15c_ti411);
		chip->fw_data_version = Stm8s_fw_data_4400_vooc_ffc_15c_ti411
			[chip->fw_data_count - FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4450_FFC_5C_VOOC:
		...
		chip->fw_data_version =
			Stm8s_fw_data_4450_ffc_5c_vooc[chip->fw_data_count -
						       FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_FFC_5V5P9A:
		...
		chip->fw_data_version =
			Stm8s_fw_data_4400_ffc_5v5p9a[chip->fw_data_count -
						      FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4450_FFC_5V6A_VOOC_4052MA_3BIT:
		...
		chip->fw_data_version =
			Stm8s_fw_data_4450_ffc_5v6a_3bit[chip->fw_data_count -
							 FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4450_FFC_5V4A_VOOC_4052MA_3BIT:
		chip->firmware_data = Stm8s_fw_data_4450_vooc_ffc_5v4a_3bit;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4450_vooc_ffc_5v4a_3bit);
		chip->fw_data_version = Stm8s_fw_data_4450_vooc_ffc_5v4a_3bit
			[chip->fw_data_count - FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_VOOC_FFC_15C_18041:
		chip->firmware_data = Stm8s_fw_data_4400_vooc_ffc_15c_18041;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4400_vooc_ffc_15c_18041);
		chip->fw_data_version = Stm8s_fw_data_4400_vooc_ffc_15c_18041
			[chip->fw_data_count - FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4450_VOOC_FFC_5V6A_19365:
		chip->firmware_data = Stm8s_fw_data_4450_VOOC_FFC_5V6A_19365;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4450_VOOC_FFC_5V6A_19365);
		chip->fw_data_version = Stm8s_fw_data_4450_VOOC_FFC_5V6A_19365
			[chip->fw_data_count - FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4450_FFC_SHORT_RESET_WINDOW:
		chip->firmware_data = Stm8s_fw_data_4450_ffc_ShortResetWindow;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4450_ffc_ShortResetWindow);
		chip->fw_data_version = Stm8s_fw_data_4450_ffc_ShortResetWindow
			[chip->fw_data_count - FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_SVOOC_3500MA:
		...
		chip->fw_data_version =
			Stm8s_fw_data_4400_svooc_3500MA[chip->fw_data_count -
							FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_SVOOC_1000MA:
		...
		chip->fw_data_version =
			Stm8s_fw_data_4400_svooc_1000MA[chip->fw_data_count -
							FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_SVOOC_5000MA:
		...
		chip->fw_data_version =
			Stm8s_fw_data_4400_svooc_5000MA[chip->fw_data_count -
							FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_SVOOC_6500MA:
		...
		chip->fw_data_version =
			Stm8s_fw_data_4400_svooc_6500MA[chip->fw_data_count -
							FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4450_SVOOC_6500MA:
		...
		chip->fw_data_version =
			Stm8s_fw_data_4450_svooc_6500MA[chip->fw_data_count -
							FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4450_SVOOC_6500MA_FV4490:
		chip->firmware_data = Stm8s_fw_data_4450_svooc_6500MA_fv4490;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4450_svooc_6500MA_fv4490);
		chip->fw_data_version = Stm8s_fw_data_4450_svooc_6500MA_fv4490
			[chip->fw_data_count - FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4450_SVOOC_6500MA_disableI2C:
		chip->firmware_data =
			Stm8s_fw_data_4450_svooc_6500MA_disableI2C;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4450_svooc_6500MA_disableI2C);
		chip->fw_data_version =
			Stm8s_fw_data_4450_svooc_6500MA_disableI2C
				[chip->fw_data_count - FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_SVOOC_6500MA_8250:
		...
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4400_svooc_6500MA_8250);
		chip->fw_data_version =
			Stm8s_fw_data_4400_svooc_6500MA_8250[chip->fw_data_count -
							     FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4450_SVOOC_6500MA_8250:
		chip->firmware_data = Stm8s_fw_data_4450_svooc_6500MA_8250;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4450_svooc_6500MA_8250);
		chip->fw_data_version =
			Stm8s_fw_data_4450_svooc_6500MA_8250[chip->fw_data_count -
							     FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_SVOOC_6500MA_8250_LINK:
		chip->firmware_data = Stm8s_fw_data_4400_svooc_6500MA_8250_link;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4400_svooc_6500MA_8250_link);
		chip->fw_data_version =
			Stm8s_fw_data_4400_svooc_6500MA_8250_link
				[chip->fw_data_count - FW_VERSION_OFFSET];
		break;
	case VOOC_FW_TYPE_STM8S_4400_SVOOC_6500MA_8250_LINK_LITE:
		chip->firmware_data =
			Stm8s_fw_data_4400_svooc_6500MA_8250_link_lite;
		chip->fw_data_count =
			sizeof(Stm8s_fw_data_4400_svooc_6500MA_8250_link_lite);
		chip->fw_data_version =
			Stm8s_fw_data_4400_svooc_6500MA_8250_link_lite
				[chip->fw_data_count - FW_VERSION_OFFSET];
		break;
			      ...
			      const struct i2c_device_id *id)
{
	...
	gpDMABuf_va = (u8 *)dma_alloc_coherent(
		&client->dev GTP_DMA_MAX_TRANSACTION_LENGTH, &gpDMABuf_pa,
		GFP_KERNEL);
	if (!gpDMABuf_va) {
	...
	chg_debug("stm8s probe success,\
		...
		fw_version = 0x%x, \
		mcu_version = 0x%x\n",
		  chip->vooc_fw_type, chip->fw_data_version,
		  chip->fw_mcu_version);
	return 0;
}
