...
static bool rk826_fw_update_check(struct oplus_vooc_chip *chip)
{
	...
	if ((fw_size % 128))
		code_size = (fw_size / 128 + 1) * 128 + 128 +
			    64; //128 is page ,64 is extended space
	else
		code_size = (fw_size / 128) * 128 + 128 +
			    64; //128 is page ,64 is extended space
...
static int rk826_fw_write_00_code(struct oplus_vooc_chip *chip)
{
	...
	req.timeout = 0;
	req.fw_crc = js_hash(chip->firmware_data,
			     chip->fw_data_count); //for crc hash
...
static int rk826_parse_fw_from_array(struct oplus_vooc_chip *chip)
{
	if (chip->batt_type_4400mv) {
		...
		chip->fw_data_version =
			rk826_fw_data_4400_vooc_ffc_15c[chip->fw_data_count - 4];
	} else {
		chip->firmware_data = rk826_fw_data_4400_vooc_ffc_15c;
		chip->fw_data_count = sizeof(rk826_fw_data_4400_vooc_ffc_15c);
		chip->fw_data_version =
	...
	if (chip->vooc_fw_type == VOOC_FW_TYPE_RK826_4400_VOOC_FFC_15C) {
		...
		chip->fw_data_count =
			sizeof(rk826_fw_data_4450_vooc_ffc_5v6a_4bit);
		chip->fw_data_version = rk826_fw_data_4450_vooc_ffc_5v6a_4bit
		...
		chip->fw_data_count =
			sizeof(rk826_fw_data_4450_svooc_ffc_5v6a_chaka);
		chip->fw_data_version = rk826_fw_data_4450_svooc_ffc_5v6a_chaka
			[chip->fw_data_count - 4];
	} else if (chip->vooc_fw_type ==
		   VOOC_FW_TYPE_RK826_4450_SVOOC_FFC_6300MA_HIMA) {
		chip->firmware_data = rk826_fw_data_4450_svooc_ffc_6300mA_hima;
		chip->fw_data_count =
			sizeof(rk826_fw_data_4450_svooc_ffc_6300mA_hima);
		chip->fw_data_version = rk826_fw_data_4450_svooc_ffc_6300mA_hima
			[chip->fw_data_count - 4];
	} else if (chip->vooc_fw_type ==
		   VOOC_FW_TYPE_RK826_4450_SVOOC_FFC_6300MA_HOREE) {
		chip->firmware_data = rk826_fw_data_4450_svooc_ffc_6300mA_horee;
		chip->fw_data_count =
			sizeof(rk826_fw_data_4450_svooc_ffc_6300mA_horee);
		chip->fw_data_version =
			rk826_fw_data_4450_svooc_ffc_6300mA_horee
				[chip->fw_data_count - 4];
		...
		chip->fw_data_version =
			rk826_fw_data_4450_svooc_6300ma[chip->fw_data_count - 4];
	} else if (chip->vooc_fw_type == VOOC_FW_TYPE_RK826_4450_SVOOC_6300MA &&
		   get_hwpcb_version()) {
		chip->firmware_data = rk826_fw_data_4450_svooc_evt2_4500ma;
		chip->fw_data_count =
			sizeof(rk826_fw_data_4450_svooc_evt2_4500ma);
		chip->fw_data_version =
			rk826_fw_data_4450_svooc_evt2_4500ma[chip->fw_data_count -
							     4];
	} else if (chip->vooc_fw_type ==
		   VOOC_FW_TYPE_RK826_4450_SVOOC_FFC_5V6A_DALI) {
		chip->firmware_data = rk826_fw_data_4450_svooc_ffc_5v6a_dali;
		chip->fw_data_count =
			sizeof(rk826_fw_data_4450_svooc_ffc_5v6a_dali);
		chip->fw_data_version = rk826_fw_data_4450_svooc_ffc_5v6a_dali
			[chip->fw_data_count - 4];
	} else if (chip->vooc_fw_type ==
		   VOOC_FW_TYPE_RK826_4450_SVOOC_FFC_6300MA_LEMON) {
		chip->firmware_data = rk826_fw_data_4450_svooc_ffc_6300mA_lemon;
		chip->fw_data_count =
			sizeof(rk826_fw_data_4450_svooc_ffc_6300mA_lemon);
		chip->fw_data_version =
			rk826_fw_data_4450_svooc_ffc_6300mA_lemon
				[chip->fw_data_count - 4];
	} else if (chip->vooc_fw_type ==
		   VOOC_FW_TYPE_RK826_4450_SVOOC_FFC_5V6A_WALLE) {
		chip->firmware_data = rk826_fw_data_4450_svooc_ffc_5v6a_walle;
		chip->fw_data_count =
			sizeof(rk826_fw_data_4450_svooc_ffc_5v6a_walle);
		chip->fw_data_version = rk826_fw_data_4450_svooc_ffc_5v6a_walle
			[chip->fw_data_count - 4];
	}
...
static ssize_t rk826_current_read(struct file *filp, char __user *buff,
				  size_t count, loff_t *off)
{
	char page[256] = { 0 };
	int len = 0;
...
static void rk826_update_battery_temperature_soc(struct work_struct *work)
{
	...
	chg_err("kilody in! soc = %d,temp = %d,uv_bat = %d,current_bat = %d,chging = %d\n",
		soc, temp, the_bat.uv_bat, the_bat.current_bat,
		oplus_vooc_get_fastchg_ing());
	schedule_delayed_work(&rk826_update_temp_soc,
...
{
	INIT_DELAYED_WORK(&rk826_update_temp_soc,
			  rk826_update_battery_temperature_soc);
