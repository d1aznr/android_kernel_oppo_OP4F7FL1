...
static int op10_get_fw_old_version(struct oplus_vooc_chip *chip,
				   u8 version_info[]);
...
static int check_flash_idle(struct oplus_vooc_chip *chip, u32 try_count)
{
	...
	do {
		rx_buf = 0xff;
		rc = oplus_vooc_i2c_read(chip->client, CMD_DEV_STATUS, 1,
					 &rx_buf);
		if (rc < 0) {
...
int op10_pps_mos_ctrl(int on)
{
	...
	if (on == 1) {
		...
		ret = oplus_vooc_i2c_write(the_chip->client, REG_HOST, 4,
					   (u8 *)(&ovp_flag));
		if (ret < 0) {
		...
		if (ret < 0) {
			for (i = 0; i < 3; i++) {
				msleep(50);
				//ret = oplus_i2c_dma_write(the_chip->client, REG_HOST, 4, (u8 *)(&ovp_flag));
				ret = oplus_vooc_i2c_write(the_chip->client,
							   REG_HOST, 4,
							   (u8 *)(&ovp_flag));
				if (ret >= 0)
					break;
	...
	*((u32 *)rx_buf) = chip->fw_data_count;
	...
	if (*((u32 *)rx_buf) != new_fw_crc32) {
		...
		*((u32 *)rx_buf) = 0x4641494C; /*FAIL*/
		...
		if (check_flash_idle(chip, write_done_try_count) == -1) {
			chg_debug(
				"cannot wait flash write fail done, timeout!\n");
		}
		goto update_fw_err;
	}
				   ...
				   u8 version_info[])
{
	...
	memset(version_info, 0xFF,
	       FW_VERSION_LEN); //clear version info at first
	...
	if (*((u32 *)rx_buf) < fw_len_address) {
		...
		oplus_vooc_i2c_write(chip->client, CMD_SET_ADDR, 2, rx_buf);
		msleep(1);
		oplus_vooc_i2c_read(chip->client, CMD_XFER_R_DATA,
...
static int op10_parse_fw_from_array(struct oplus_vooc_chip *chip)
{
	if (chip->batt_type_4400mv) {
		...
		chip->fw_data_version =
			op10_fw_data_4400_vooc_ffc_15c[chip->fw_data_count - 4];
	} else {
		chip->firmware_data = op10_fw_data_4400_vooc_ffc_15c;
		chip->fw_data_count = sizeof(op10_fw_data_4400_vooc_ffc_15c);
		chip->fw_data_version =
	...
	case VOOC_FW_TYPE_OP10_4450_VOOC_FFC_5V4A_4BIT:
		...
		chip->fw_data_count =
			sizeof(op10_fw_data_4450_vooc_ffc_5v4a_4bit);
		chip->fw_data_version =
			op10_fw_data_4450_vooc_ffc_5v4a_4bit[chip->fw_data_count -
							     4];
		break;
	case VOOC_FW_TYPE_OP10_4250_VOOC_FFC_10V6A_4BIT:
		chip->firmware_data = op10_fw_data_4250_vooc_ffc_10v6a_4bit;
		chip->fw_data_count =
			sizeof(op10_fw_data_4250_vooc_ffc_10v6a_4bit);
		chip->fw_data_version = op10_fw_data_4250_vooc_ffc_10v6a_4bit
	...
	case VOOC_FW_TYPE_OP10_4450_VOOC_FFC_5V6A_4BIT:
		...
		chip->fw_data_count =
			sizeof(op10_fw_data_4450_vooc_ffc_5v6a_4bit);
		chip->fw_data_version =
			op10_fw_data_4450_vooc_ffc_5v6a_4bit[chip->fw_data_count -
							     4];
	default:
		break;
	}
