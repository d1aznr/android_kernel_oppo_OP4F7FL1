...
#define bq_err(fmt, ...)                                                       \
	do {                                                                   \
		if (bq->mode == BQ25970_ROLE_MASTER)                           \
			printk(KERN_ERR "[bq2597x-wl-MASTER]:%s:" fmt,         \
			       __func__, ##__VA_ARGS__);                       \
		else if (bq->mode == BQ25970_ROLE_SLAVE)                       \
			printk(KERN_ERR "[bq2597x-wl-SLAVE]:%s:" fmt,          \
			       __func__, ##__VA_ARGS__);                       \
		else                                                           \
...
#define bq_info(fmt, ...)                                                      \
	do {                                                                   \
		if (bq->mode == BQ25970_ROLE_MASTER)                           \
			printk(KERN_INFO "[bq2597x-wl-MASTER]:%s:" fmt,        \
			       __func__, ##__VA_ARGS__);                       \
		else if (bq->mode == BQ25970_ROLE_SLAVE)                       \
			printk(KERN_INFO "[bq2597x-wl-SLAVE]:%s:" fmt,         \
			       __func__, ##__VA_ARGS__);                       \
		else                                                           \
...
#define bq_dbg(fmt, ...)                                                       \
	do {                                                                   \
		if (bq->mode == BQ25970_ROLE_MASTER)                           \
			...
			printk(KERN_DEBUG "[bq2597x-wl-SLAVE]:%s:" fmt,        \
			       __func__, ##__VA_ARGS__);                       \
		else                                                           \
...
static int bq2597x_wl_update_bits(struct bq2597x_wl *bq, u8 reg, u8 mask,
				  u8 data)
{
	int ret;
	u8 tmp;
...
static int bq2597x_wl_enable_charge(struct bq2597x_wl *bq, bool enable)
{
	...
	ret = bq2597x_wl_update_bits(bq, BQ2597X_REG_0C, BQ2597X_CHG_EN_MASK,
				     val);
...
static int bq2597x_wl_enable_wdt(struct bq2597x_wl *bq, bool enable)
{
	...
	ret = bq2597x_wl_update_bits(bq, BQ2597X_REG_0B,
...
static int bq2597x_wl_enable_batovp(struct bq2597x_wl *bq, bool enable)
{
	...
	ret = bq2597x_wl_update_bits(bq, BQ2597X_REG_00,
...
static int bq2597x_wl_enable_batocp(struct bq2597x_wl *bq, bool enable)
{
	...
	ret = bq2597x_wl_update_bits(bq, BQ2597X_REG_02,
...
static int bq2597x_wl_enable_busocp(struct bq2597x_wl *bq, bool enable)
{
	...
	ret = bq2597x_wl_update_bits(bq, BQ2597X_REG_08,
...
static int bq2597x_wl_set_acovp_th(struct bq2597x_wl *bq, int threshold)
{
	...
	ret = bq2597x_wl_update_bits(bq, BQ2597X_REG_05, BQ2597X_AC_OVP_MASK,
				     val);
...
int bq2597x_wl_enable_adc(struct bq2597x_wl *bq, bool enable)
{
	...
	ret = bq2597x_wl_update_bits(bq, BQ2597X_REG_14, BQ2597X_ADC_EN_MASK,
				     val);
	if (ret == 0)
...
static int bq2597x_wl_set_adc_bits(struct bq2597x_wl *bq, int bits)
{
	...
	ret = bq2597x_wl_update_bits(bq, BQ2597X_REG_14,
...
static int bq2597x_wl_set_adc_scan(struct bq2597x_wl *bq, int channel,
				   bool enable)
{
	int ret;
	u8 reg;
					 ...
					 const char *buf, size_t count)
{
	...
	if (ret == 2 && reg <= 0x2B)
		bq2597x_wl_write_byte(bq, (unsigned char)reg,
				      (unsigned char)val);
...
static void bq2597x_wl_irq_work(struct work_struct *work)
{
	...
	struct bq2597x_wl *bq =
		container_of(dwork, struct bq2597x_wl, irq_int_work);
...
static void create_debugfs_entry(struct bq2597x_wl *bq)
{
	if (bq->mode == BQ25970_ROLE_MASTER)
		...
		bq->debug_root =
			debugfs_create_dir("bq2597x-wl-standalone", NULL);
