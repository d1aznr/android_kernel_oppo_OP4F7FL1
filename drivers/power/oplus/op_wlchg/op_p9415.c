...
static int p9415_set_idt_en_val(struct op_p9415_ic *chip,
				int value) // 0 active, 1 inactive
{
	if (chip->idt_en_gpio <= 0) {
		pr_err("idt_en_gpio not exist, return\n");
		return -EINVAL;
...
static int p9415_get_cep_val_skip_check_update(struct op_p9415_ic *chip,
					       int *val)
{
	int rc;
	char temp = 0;
...
static int p9415_send_msg(struct rx_chip_prop *prop, enum rx_msg_type msg_type,
			  unsigned char msg)
{
	struct op_p9415_ic *chip = prop->private_data;
...
static unsigned char p9415_calculate_checksum(const unsigned char *data,
					      int len)
{
...
static int p9415_MTP(struct op_p9415_ic *chip, unsigned char *fw_buf,
		     int fw_size)
{
	int rc;
	int i, j;
...
static int p9415_load_fw(struct op_p9415_ic *chip, unsigned char *fw_data,
			 int CodeLength)
{
...
int p9415_upgrade_firmware(struct op_p9415_ic *chip, unsigned char *fw_buf,
			   int fw_size)
{
	int rc = 0;
	...
	if (temp[0] & P9415_EVENT_MASK) {
		...
		if (rc) {
			...
			if ((val_buf[0] == 0x4F) && (val_buf[1] == temp[0]) &&
			    (val_buf[3] == temp[1])) {
				rc = wlchg_send_msg(WLCHG_MSG_CMD_RESULT,
						    val_buf[3], val_buf[1]);
				...
				if (rc < 0) {
					...
					msleep(2);
					wlchg_send_msg(WLCHG_MSG_CMD_RESULT,
						       val_buf[3], val_buf[1]);
				}
				...
				rc = wlchg_send_msg(WLCHG_MSG_CMD_RESULT, 0,
						    val_buf[0]);
				if (rc < 0) {
					pr_err("send cmd result err, try again\n");
					msleep(2);
					wlchg_send_msg(WLCHG_MSG_CMD_RESULT, 0,
						       val_buf[0]);
				}
			}
		}
...
static void p9415_check_ldo_on_func(struct work_struct *work)
{
	...
	chg_info("connected_ldo_on is : %s",
...
static void p9415_update_work_process(struct work_struct *work)
{
	...
	if (!chip->check_fw_update) {
		...
		if (rc) {
			...
			retrycount++;
			schedule_delayed_work(&chip->p9415_update_work,
					      p9415_UPDATE_INTERVAL);
	...
	case UPGRADE_START:
		...
		if (fw_head->magic[0] == 0x02 && fw_head->magic[1] == 0x00 &&
			...
			pr_err("<IDT UPDATE>image header verification succeeded, fw_size=%d\n",
			       fw_size);
			memcpy(fw_buf, buf + sizeof(struct idt_fw_head),
...
static DEVICE_ATTR(upgrade_firmware, S_IWUSR, NULL,
		   p9415_upgrade_firmware_store);

static struct attribute *p9415_sysfs_attrs[] = {
	&dev_attr_upgrade_firmware.attr, NULL
};
...
static int p9415_driver_probe(struct platform_device *pdev)
{
...
#ifdef OP_DEBUG
	ret = sysfs_create_group(&chip->dev->parent->kobj,
				 &p9415_attribute_group);
	if (ret)
	...
	chip->update_fw_wake_lock =
		wakeup_source_register(chip->dev, "p9415_update_fw_wake_lock");

	if (get_boot_mode() == MSM_BOOT_MODE__CHARGE) {
		schedule_delayed_work(&chip->idt_connect_int_work,
