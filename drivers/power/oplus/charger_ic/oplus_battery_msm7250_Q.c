...
static void get_usb_temp(struct oplus_chg_chip *chg)
{
	...
	if (usbtemp_error_r != 0) {
		...
		if (usbtemp_debug == 1) {
			printk(KERN_ERR
			       "[OPLUS_CHG][%s]: chg->usb_temp_l  is  error !\n",
			       __func__);
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		get_usb_temp(chip);
		if ((chip->usb_temp_r < USB_50C) &&
		    (chip->usb_temp_l < USB_50C)) //get vbus when usbtemp < 50C
			vbus_volt = qpnp_get_prop_charger_voltage_now();
		else
			vbus_volt = 0;

		if ((chip->usb_temp_r < USB_40C) &&
		    (chip->usb_temp_l < USB_40C)) {
		...
		if (((chip->usb_temp_r < USB_50C) &&
		     (chip->usb_temp_l < USB_50C)) &&
		    vbus_volt < VBUS_VOLT_THRESHOLD && otg_delay_flag == false)
			delay = VBUS_MONITOR_INTERVAL;
		if (oplus_usbtemp_condition() == false) {
			if (dischg_flag == true &&
			    oplus_ccdetect_check_is_gpio(chip) == true) {
				dischg_flag = false;
			...
			if ((USB_57C <= chip->usb_temp_r &&
			     chip->usb_temp_r < USB_100C) ||
			    (USB_57C <= chip->usb_temp_l &&
					...
					if (count_r >= retry_cnt ||
						...
						if (!IS_ERR_OR_NULL(
							...
							if (oplus_vooc_get_fastchg_started() ==
							    true) {
								oplus_chg_set_chargerid_switch_val(
									0);
				...
				count = 0;
			} else if (((chip->usb_temp_r -
				     chip->temperature / 10) > 20 &&
				    chip->usb_temp_r < USB_100C) ||
				   ((chip->usb_temp_l -
				     chip->temperature / 10) > 20 &&
				    chip->usb_temp_l < USB_100C)) {
