...
{
	if (g_switching != NULL) {
		if (get_switching_hw_enable() &&
		    atomic_read(&g_switching->suspended) == 0) {
...
static int s2asl01_update_reg(struct i2c_client *client, u8 reg, u8 val,
			      u8 mask)
{
	struct s2asl01_switching_data *switching = i2c_get_clientdata(client);
	int ret = 0;
...
static void s2asl01_test_read(struct i2c_client *client)
{
	...
	pr_info("%s [%s]: %s\n", __func__,
		current_limiter_type_str[switching->pdata->bat_type], str);
}

static void s2asl01_set_in_ok(struct s2asl01_switching_data *switching,
			      bool onoff)
{
	chg_err("[%s]: INOK = %d\n",
		current_limiter_type_str[switching->pdata->bat_type], onoff);
	if (onoff)
		s2asl01_update_reg(switching->client,
				   S2ASL01_SWITCHING_CORE_CTRL3,
				   1 << S2ASL01_CTRL3_INOK_SHIFT,
				   S2ASL01_CTRL3_INOK_MASK);
	else
		s2asl01_update_reg(switching->client,
				   S2ASL01_SWITCHING_CORE_CTRL3,
				   0 << S2ASL01_CTRL3_INOK_SHIFT,
				   S2ASL01_CTRL3_INOK_MASK);
}

static void
s2asl01_set_supllement_mode(struct s2asl01_switching_data *switching,
			    bool onoff)
{
	...
	if (onoff) {
		...
		s2asl01_update_reg(switching->client,
				   S2ASL01_SWITCHING_CORE_CTRL3,
				   0 << S2ASL01_CTRL3_SUPLLEMENTMODE_SHIFT,
				   S2ASL01_CTRL3_SUPLLEMENTMODE_MASK);
...
static void
s2asl01_set_recharging_start(struct s2asl01_switching_data *switching)
{
...
static void s2asl01_set_eoc_on(struct s2asl01_switching_data *switching)
{
	...
	s2asl01_update_reg(switching->client, S2ASL01_SWITCHING_CORE_CTRL1,
			   1 << S2ASL01_CTRL1_EOC_SHIFT,
			   S2ASL01_CTRL1_EOC_MASK);
}

static void s2asl01_set_dischg_mode(struct s2asl01_switching_data *switching,
				    int chg_mode)
{
	u8 val = 0;
	int ret = 0;
	...
	ret = s2asl01_update_reg(switching->client,
				 S2ASL01_SWITCHING_CORE_CTRL3,
				 val << S2ASL01_CTRL3_DIS_MODE_SHIFT,
...
static int s2asl01_get_dischg_mode(struct s2asl01_switching_data *switching)
{
	...
	ret = s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_CORE_CTRL3,
			       &val);
	chg_mode = (val & S2ASL01_CTRL3_DIS_MODE_MASK) >>
		   S2ASL01_CTRL3_DIS_MODE_SHIFT;
...
static void s2asl01_set_chg_mode(struct s2asl01_switching_data *switching,
				 int chg_mode)
{
	u8 val = 0;
	int ret = 0;
	...
	ret = s2asl01_update_reg(switching->client,
				 S2ASL01_SWITCHING_CORE_CTRL3,
				 val << S2ASL01_CTRL3_CHG_MODE_SHIFT,
...
static int s2asl01_get_chg_mode(struct s2asl01_switching_data *switching)
{
	...
	ret = s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_CORE_CTRL3,
			       &val);
	chg_mode = (val & S2ASL01_CTRL3_CHG_MODE_MASK) >>
		   S2ASL01_CTRL3_CHG_MODE_SHIFT;
...
static int s2asl01_get_vchg(struct s2asl01_switching_data *switching, int mode)
{
	...
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_VAL1_VCHG,
			 &data[0]);
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_VAL2_VCHG,
			 &data[1]);
	...
	pr_info("%s [%s]: vchg = %d %s\n", __func__,
		current_limiter_type_str[switching->pdata->bat_type], vchg,
...
static int s2asl01_get_vbat(struct s2asl01_switching_data *switching, int mode)
{
	...
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_VAL1_VBAT,
			 &data[0]);
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_VAL2_VBAT,
			 &data[1]);
	...
	pr_info("%s [%s]: vbat = %d %s\n", __func__,
		current_limiter_type_str[switching->pdata->bat_type], vbat,
		(mode == S2M_BATTERY_VOLTAGE_UV) ? "uV" : "mV");
...
static int s2asl01_get_ichg(struct s2asl01_switching_data *switching, int mode)
{
	...
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_VAL1_ICHG,
			 &data[0]);
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_VAL2_ICHG,
			 &data[1]);
	...
	pr_info("%s [%s]: Ichg = %d %s\n", __func__,
		current_limiter_type_str[switching->pdata->bat_type], ichg,
		(mode == S2M_BATTERY_CURRENT_UA) ? "uA" : "mA");

	return ichg;
}

static int s2asl01_get_idischg(struct s2asl01_switching_data *switching,
			       int mode)
{
	u8 data[2] = {
		0,
	...
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_VAL1_IDISCHG,
			 &data[0]);
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_VAL2_IDISCHG,
			 &data[1]);
	...
	pr_info("%s [%s]: IDISCHG = %d %s\n", __func__,
		current_limiter_type_str[switching->pdata->bat_type], idischg,
		(mode == S2M_BATTERY_CURRENT_UA) ? "uA" : "mA");
	...
	struct s2asl01_switching_data *switching)
{
	...
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_CORE_CTRL4,
			 &data);
	...
	struct s2asl01_switching_data *switching)
{
	...
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_CORE_CTRL5,
			 &data);
	...
	struct s2asl01_switching_data *switching)
{
	...
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_CORE_CTRL6,
			 &data);
...
static void
s2asl01_set_recharging_voltage(struct s2asl01_switching_data *switching,
			       int charging_voltage)
{
	...
	s2asl01_write_reg(switching->client, S2ASL01_SWITCHING_TOP_RECHG_CTRL1,
			  data);
}

static int
s2asl01_get_recharging_voltage(struct s2asl01_switching_data *switching)
{
	u8 data = 0;
	int charging_voltage = 0;

	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_TOP_RECHG_CTRL1,
			 &data);
...
static void s2asl01_set_eoc_voltage(struct s2asl01_switching_data *switching,
				    int charging_voltage)
{
	u8 data = 0;
	...
	s2asl01_write_reg(switching->client, S2ASL01_SWITCHING_TOP_EOC_CTRL1,
			  data);
}
...
static int s2asl01_get_eoc_voltage(struct s2asl01_switching_data *switching)
{
	...
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_TOP_EOC_CTRL1,
			 &data);
	charging_voltage = (data * 10) + 2560;
...
static void s2asl01_set_eoc_current(struct s2asl01_switching_data *switching,
				    int charging_current)
{
	...
	s2asl01_update_reg(switching->client, S2ASL01_SWITCHING_TOP_EOC_CTRL2,
			   data, 0x1F);
}
...
static int s2asl01_get_eoc_current(struct s2asl01_switching_data *switching)
{
	...
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_TOP_EOC_CTRL2,
			 &data);
	data &= 0x1F;
...
static void s2asl01_powermeter_onoff(struct s2asl01_switching_data *switching,
				     bool onoff)
{
	pr_info("%s [%s]: (%d)\n", __func__,
		current_limiter_type_str[switching->pdata->bat_type], onoff);
	...
	if (onoff) {
		s2asl01_update_reg(switching->client,
				   S2ASL01_SWITCHING_PM_ENABLE, 0xF0, 0xF0);
		switching->power_meter = true;
	} else {
		s2asl01_update_reg(switching->client,
				   S2ASL01_SWITCHING_PM_ENABLE, 0x00, 0xF0);
		switching->power_meter = false;
	}
}

static void s2asl01_tsd_onoff(struct s2asl01_switching_data *switching,
			      bool onoff)
{
	pr_info("%s(%d)\n", __func__, onoff);
	if (onoff) {
				...
				union power_supply_propval *val)
{
	...
	enum s2m_power_supply_property s2m_psp =
		(enum s2m_power_supply_property)psp;
	...
	case POWER_SUPPLY_S2M_PROP_MIN ... POWER_SUPPLY_S2M_PROP_MAX:
		...
		case POWER_SUPPLY_S2M_PROP_DISCHG_CURRENT:
			val->intval =
				s2asl01_get_idischg(switching, val->intval);
			break;
		case POWER_SUPPLY_S2M_PROP_FASTCHG_LIMIT_CURRENT:
			val->intval = s2asl01_get_fast_charging_current_limit(
				switching);
			break;
		case POWER_SUPPLY_S2M_PROP_TRICKLECHG_LIMIT_CURRENT:
			val->intval =
				s2asl01_get_trickle_charging_current_limit(
					switching);
			break;
		case POWER_SUPPLY_S2M_PROP_DISCHG_LIMIT_CURRENT:
			val->intval = s2asl01_get_dischg_charging_current_limit(
				switching);
			break;
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_S2M_PROP_MIN ... POWER_SUPPLY_S2M_PROP_MAX:
		...
		case POWER_SUPPLY_S2M_PROP_CHG_MODE:
			...
			break;
		case POWER_SUPPLY_S2M_PROP_FASTCHG_LIMIT_CURRENT:
			s2asl01_set_fast_charging_current_limit(switching,
								val->intval);
			break;
		case POWER_SUPPLY_S2M_PROP_TRICKLECHG_LIMIT_CURRENT:
			s2asl01_set_trickle_charging_current_limit(switching,
								   val->intval);
			break;
		case POWER_SUPPLY_S2M_PROP_DISCHG_LIMIT_CURRENT:
			s2asl01_set_dischg_charging_current_limit(switching,
								  val->intval);
			break;
		case POWER_SUPPLY_S2M_PROP_RECHG_VOLTAGE:
...
static int s2asl01_switching_parse_dt(struct device *dev,
				      struct s2asl01_platform_data *pdata)
{
	...
	if (np == NULL) {
		...
		if (IS_ERR_OR_NULL(pdata->pinctrl)) {
			...
			pdata->switch_active = pinctrl_lookup_state(
				pdata->pinctrl, "switch_active");
		...
		if (pdata->en_gpio < 0) {
			...
			if (gpio_is_valid(pdata->en_gpio)) {
				ret = gpio_request(pdata->en_gpio,
						   "switch_en_gpio");
				if (ret) {
		...
		if (pdata->bat_type & LIMITER_MAIN) {
			...
			np = of_find_node_by_name(NULL,
						  "s2asl01-switching-sub");
		}

		ret = of_property_read_string(
			np, "limiter,switching_name",
		...
		if (ret < 0) {
			...
			pdata->float_voltage =
				4350; /* for interrupt setting, not used */
		}
...
static int s2asl01_get_rev_id(struct s2asl01_switching_data *switching)
{
	...
	if ((val1 & 0xF0) == 0) {
		s2asl01_read_reg(switching->client,
				 S2ASL01_SWITCHING_CORE_CTRL3, &val2);
		if (val2 != 0x0C) {
...
static ssize_t powermeter_en_show(struct device *dev,
...
static ssize_t powermeter_en_store(struct device *dev,
...
static ssize_t v_referesh_time_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct s2asl01_switching_data *switching = g_switching;
	u8 temp;
	...
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_PM_V_OPTION,
			 &temp);
...
static ssize_t v_referesh_time_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct s2asl01_switching_data *switching = g_switching;
	...
	if (ret)
		...
		case 3:
			val = (temp << 4) & 0x30;
			s2asl01_update_reg(switching->client,
					   S2ASL01_SWITCHING_PM_V_OPTION, val,
					   0x30);
			break;
		default:
			pr_info("%s : wrong input\n", __func__);
...
static ssize_t v_avg1_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	struct s2asl01_switching_data *switching = g_switching;
	u8 temp;
...
static ssize_t v_avg2_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	struct s2asl01_switching_data *switching = g_switching;
	u8 temp;
...
static ssize_t i_referesh_time_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct s2asl01_switching_data *switching = g_switching;
	u8 temp;
	...
	s2asl01_read_reg(switching->client, S2ASL01_SWITCHING_PM_I_OPTION,
			 &temp);
	refresh_time = (temp & 0x30) >> 4;
...
static ssize_t i_referesh_time_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct s2asl01_switching_data *switching = g_switching;
	...
	if (ret)
		...
		case 3:
			val = (temp << 4) & 0x30;
			s2asl01_update_reg(switching->client,
					   S2ASL01_SWITCHING_PM_I_OPTION, val,
					   0x30);
			break;
		default:
			pr_info("%s : wrong input\n", __func__);
...
static ssize_t i_avg1_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	struct s2asl01_switching_data *switching = g_switching;
	u8 temp;
...
static ssize_t i_avg2_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	struct s2asl01_switching_data *switching = g_switching;
	u8 temp;
			  ...
			  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "0x%x\n", &x) == 1) {
		val = (u8)(x & 0xff);
		switching->data = val;
		s2asl01_write_reg(switching->client, switching->addr,
				  switching->data);
		pr_info("%s : addr, : 0x%x, data : 0x%x\n", __func__,
			switching->addr, switching->data);
	}

	return count;
}

static ssize_t fastchg_curr_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	int curr_ma;
...
static ssize_t fastchg_curr_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int curr_ma;
...
static ssize_t dischg_curr_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int curr_ma;
...
static ssize_t dischg_curr_store(struct device *dev,
...
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
#ifdef CONFIG_OPLUS_CHARGER_MTK
	if (s2asl01_boot_mode == ATE_FACTORY_BOOT ||
	    s2asl01_boot_mode == FACTORY_BOOT ||
...
int s2asl01_switching_set_discharge_current(int discharge_current_ma)
{
...
#endif
	{
		...
		s2asl01_set_dischg_charging_current_limit(g_switching,
							  discharge_current_ma);
...
static DEVICE_ATTR(powermeter_en, 0644, powermeter_en_show,
		   powermeter_en_store);
static DEVICE_ATTR(v_referesh_time, 0644, v_referesh_time_show,
		   v_referesh_time_store);
static DEVICE_ATTR(v_avg1, 0644, v_avg1_show, v_avg1_store);
static DEVICE_ATTR(v_avg2, 0644, v_avg2_show, v_avg2_store);
static DEVICE_ATTR(i_referesh_time, 0644, i_referesh_time_show,
		   i_referesh_time_store);
...
struct oplus_switch_operations s2asl01_switching_ops = {
	...
	.switching_set_fastcharge_current =
		s2asl01_switching_set_fastcharge_current,
	.switching_set_discharge_current =
		s2asl01_switching_set_discharge_current,
				   ...
				   const struct i2c_device_id *id)
{
	...
	if (batt_volt > 3400 && sub_batt_volt < 3100 &&
	    batt_volt - sub_batt_volt > 1000) {
	...
	g_switching->switching_chip = devm_kzalloc(
		&client->dev, sizeof(struct oplus_switch_chip), GFP_KERNEL);
