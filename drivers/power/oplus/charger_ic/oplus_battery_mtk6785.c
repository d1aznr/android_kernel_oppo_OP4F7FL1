...
static int oplus_mt6360_suspend_charger(void)
{
	...
	if (get_project() >= 20730 && get_project() <= 20733) {
		if (chip->dischg_flag == true) {
			...
			if (rc < 0)
				chg_debug(
					"suspend charger fail for short-c!\n");
			return 0;
		}
	}
...
int mt_get_chargerid_volt(void)
{
	...
	if (is_support_chargerid_check() == true && pinfo &&
	    pinfo->chargerid_vol_disable != true) {
...
static int oplus_chg_chargerid_parse_dt(struct oplus_chg_chip *chip)
{
	...
	pinfo->chargerid_disable =
		of_property_read_bool(node, "qcom,chargerid_disable");
	pinfo->chargerid_vol_disable =
		of_property_read_bool(node, "qcom,chargerid_vol_disable");
	...
	if (tcpc_dev != NULL) {
		if (tcpm_inquire_typec_attach_state(tcpc_dev) !=
			...
			typec_cc_orientation =
				(int)tcpm_inquire_cc_polarity(tcpc_dev) + 1;
		} else {
...
void oplus_chg_set_camera_on(bool val)
{
	if (!g_oplus_chip) {
		...
		if (g_oplus_chip->dual_charger_support ||
		    oplus_chg_get_voocphy_support() == 1) {
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	pinfo->usb_temp_v_l_chan =
		iio_channel_get(oplus_chip->dev, "thermistor-ch2_usb_ntc_l");
	...
	pinfo->usb_temp_v_r_chan =
		iio_channel_get(oplus_chip->dev, "thermistor-ch3_usb_ntc_r");
