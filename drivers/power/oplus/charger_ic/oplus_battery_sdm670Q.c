...
module_param_named(audio_headset_drp_wait_ms, __audio_headset_drp_wait_ms, int,
		   0600);
...
static void oplus_ccdetect_work(struct work_struct *work)
{
	...
	if (level != 1) {
		...
		oplus_set_dpdm_status(POWER_SUPPLY_TYPEC_PLUGIN);
		if (divider_in_auto_mode == false ||
		    (oplus_vooc_get_allow_reading() == false)) {
			cancel_delayed_work_sync(&chg->divider_set_work);
			schedule_delayed_work(&chg->divider_set_work,
...
static void oplus_divider_set_work(struct work_struct *work)
{
	...
	bool vbus_rising;
	struct smb_charger *chg =
		container_of(work, struct smb_charger, divider_set_work.work);
...
static void smblib_uusb_otg_work(struct work_struct *work)
{
	...
	if (otg_status ^ otg) {
		otg_status = otg;
		printk(KERN_ERR
		       "!!!!! smblib_handle_usb_typec_change_for_uusb: otg status [%d]\n",
		       otg_status);
	}
#endif
...
int smbchg_get_chargerid_volt(void)
{
	...
	if (chg->charger_id_num == 7) {
		rc = qpnp_vadc_read(chip->pmic_spmi.pm660_vadc_dev, P_MUX10_1_1,
				    &results);
	} else {
		rc = qpnp_vadc_read(chip->pmic_spmi.pm660_vadc_dev, P_MUX3_1_1,
				    &results);
	}
	if (rc) {
...
{
	bool value = false;
	if (oplus_ccdetect_support_check() ==
	    OPLUS_SUPPORT_CCDETECT_IN_FTM_MODE) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: ccdetect is in FTM mode, enable otg!\n",
		       __func__);
		value = true;
	}
	if (g_oplus_chip && g_oplus_chip->vbatt_num == 2) {
...
static int oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	rc = qpnp_vadc_read(chip->pmic_spmi.pm660_usbtemp_vadc_dev, P_MUX5_1_1,
			    &results);
	if (rc) {
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (oplus_chg_is_usb_present() == true ||
		    usbtemp_volt <= USB_VBUS_SHORT_DISABLE_VOLT) {
			if (usbtemp_volt <= USB_VBUS_SHORT_ENABLE_VOLT &&
			    dischg_flag == false) {
...
static int smb2_parse_dt(struct smb2 *chip)
{
	...
	chg->reddragon_ipc_wa =
		of_property_read_bool(node, "qcom,qcs605-ipc-wa");
	...
	rc = of_property_read_u32(node, "qcom,boost-threshold-ua",
				  &chip->dt.boost_threshold_ua);
	if (rc < 0)
		chip->dt.boost_threshold_ua = MICRO_P1A;

	rc = of_property_read_u32(node, "qcom,min-freq-khz",
				  &chip->dt.min_freq_khz);
	if (rc < 0)
		chip->dt.min_freq_khz = -EINVAL;

	rc = of_property_read_u32(node, "qcom,max-freq-khz",
	...
	chip->dt.auto_recharge_soc =
		of_property_read_bool(node, "qcom,auto-recharge-soc");
	...
	chg->disable_stat_sw_override =
		of_property_read_bool(node, "qcom,disable-stat-sw-override");
...
static int smb2_init_usb_psy(struct smb2 *chip)
{
	...
	chg->usb_psy =
		power_supply_register(chg->dev, &chg->usb_psy_desc, &usb_cfg);
...
static int smb2_init_usb_port_psy(struct smb2 *chip)
{
	...
	chg->usb_port_psy = power_supply_register(chg->dev, &usb_port_psy_desc,
						  &usb_port_cfg);
...
static int smb2_init_usb_main_psy(struct smb2 *chip)
{
	...
	chg->usb_main_psy = power_supply_register(chg->dev, &usb_main_psy_desc,
						  &usb_main_cfg);
...
static enum power_supply_property smb2_dc_props[] = {
	POWER_SUPPLY_PROP_INPUT_SUSPEND, POWER_SUPPLY_PROP_PRESENT,
...
static int smb2_init_batt_psy(struct smb2 *chip)
{
	...
	chg->batt_psy =
		power_supply_register(chg->dev, &batt_psy_desc, &batt_cfg);
...
static int smb2_disable_typec(struct smb_charger *chg)
{
	...
	msleep(200);
	/* configure micro USB mode */
	rc = smblib_masked_write(chg, TYPE_C_CFG_REG, TYPE_C_OR_U_USB_BIT,
				 TYPE_C_OR_U_USB_BIT);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't enable micro USB mode rc=%d\n", rc);
		return rc;
...
static int smb2_init_hw(struct smb2 *chip)
{
	...
	rc = smblib_masked_write(
		chg, USBIN_AICL_OPTIONS_CFG_REG,
		USBIN_AICL_START_AT_MAX_BIT | USBIN_AICL_ADC_EN_BIT, 0);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure AICL rc=%d\n", rc);
		return rc;
	}
...
static int smb2_request_interrupt(struct smb2 *chip, struct device_node *node,
				  const char *irq_name)
{
	struct smb_charger *chg = &chip->chg;
...
static void smbchg_usbin_collapse_irq_enable(bool enable)
{
	...
	if (enable & !collapse_en) {
		enable_irq(chip->pmic_spmi.smb2_chip->chg
				   .irq_info[USBIN_COLLAPSE_IRQ]
				   .irq);
	} else if (!enable && collapse_en) {
		disable_irq(chip->pmic_spmi.smb2_chip->chg
				    .irq_info[USBIN_COLLAPSE_IRQ]
				    .irq);
	}
	collapse_en = enable;
...
static int oplus_chg_set_input_current(int current_ma)
{
	...
	if (pre_current > current_ma) {
		...
		if (usb_icl[n] > 1200) {
			...
			if (usb_icl[n] >= 1200) {
				rc = vote(chip->pmic_spmi.smb2_chip->chg
						  .usb_icl_votable,
					  USB_PSY_VOTER, true,
					  usb_icl[n] * 1000);
				msleep(90);
...
aicl_return:
	...
	if (chip->pmic_spmi.smb2_chip->chg.typec_mode ==
		...
		if ((bool)(stat & ICL_OVERRIDE_AFTER_APSD_BIT)) {
			rc = smblib_write(&chip->pmic_spmi.smb2_chip->chg,
...
static int smbchg_float_voltage_set(int vfloat_mv)
{
	...
	rc = vote(chip->pmic_spmi.smb2_chip->chg.fv_votable,
...
bool oplus_chg_is_usb_present(void)
{
	...
	if ((chip->pmic_spmi.smb2_chip->chg.typec_mode ==
	     ...
	     chip->pmic_spmi.smb2_chip->chg.typec_mode ==
...
int pm660l_bob_regulator_get_mode(unsigned int *mode)
{
	...
	if (!g_oplus_chip) {
		printk(KERN_ERR
		       "pm660l_bob_regulator_get_mode: g_oplus_chip NULL\n");
		return -1;
	}
	chg = &g_oplus_chip->pmic_spmi.smb2_chip->chg;
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (of_find_property(oplus_chip->dev->of_node, "qcom,pm660chg-vadc",
			     NULL)) {
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (of_find_property(oplus_chip->dev->of_node, "qcom,pm660usbtemp-vadc",
			     NULL)) {
...
static void smb2_shutdown(struct platform_device *pdev)
{
	...
	/* force enable APSD */
	smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG, AUTO_SRC_DETECT_BIT,
			    AUTO_SRC_DETECT_BIT);
