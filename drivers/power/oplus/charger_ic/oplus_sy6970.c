...
#define chg_debug(fmt, ...)                                                    \
	if (sy6970_chg_dbg_enable & SY6970_DEBUG) {                            \
		printk(KERN_ERR "[OPLUS_CHG][%s]" fmt, __func__,               \
		       ##__VA_ARGS__);                                         \
	} else {                                                               \
		printk(KERN_NOTICE "[OPLUS_CHG][%s]" fmt, __func__,            \
		       ##__VA_ARGS__);                                         \
	}
#endif
...
static int sy6970_disable_otg(struct sy6970 *bq)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_03, SY6970_OTG_CONFIG_MASK,
				  val);
}
...
static int sy6970_disable_batfet_rst(struct sy6970 *bq)
{
	...
	ret = sy6970_update_bits(bq, SY6970_REG_09, SY6970_BATFET_RST_EN_MASK,
				 val);
	return ret;
}
...
static int sy6970_enable_charger(struct sy6970 *bq)
{
	...
	ret = sy6970_update_bits(bq, SY6970_REG_03, SY6970_CHG_CONFIG_MASK,
				 val);
...
int sy6970_adc_start(struct sy6970 *bq)
{
	...
	ret = sy6970_update_bits(bq, SY6970_REG_02, SY6970_CONV_RATE_MASK,
				 SY6970_ADC_CONTINUE_ENABLE
					 << SY6970_CONV_RATE_SHIFT);
	if (ret < 0) {
		dev_err(bq->dev,
			"%s enable continue adc convert failed, ret=%d\n",
			__func__, ret);
	}
...
int sy6970_adc_read_battery_volt(struct sy6970 *bq)
{
	...
	if (ret < 0) {
		...
		volt = SY6970_BATV_BASE +
		       ((val & SY6970_BATV_MASK) >> SY6970_BATV_SHIFT) *
			       SY6970_BATV_LSB;
		return volt;
	}
}
...
int sy6970_adc_read_sys_volt(struct sy6970 *bq)
{
	...
	if (ret < 0) {
		...
		volt = SY6970_SYSV_BASE +
		       ((val & SY6970_SYSV_MASK) >> SY6970_SYSV_SHIFT) *
			       SY6970_SYSV_LSB;
		return volt;
	}
}
...
static int _sy6970_adc_read_vbus_volt(struct sy6970 *sy)
{
	...
	if (ret < 0) {
		...
		volt = SY6970_VBUSV_BASE +
		       ((val & SY6970_VBUSV_MASK) >> SY6970_VBUSV_SHIFT) *
			       SY6970_VBUSV_LSB;
		return volt;
	}
}
...
int sy6970_adc_read_temperature(struct sy6970 *sy)
{
	...
	if (ret < 0) {
		...
		temp = SY6970_TSPCT_BASE +
		       ((val & SY6970_TSPCT_MASK) >> SY6970_TSPCT_SHIFT) *
			       SY6970_TSPCT_LSB;
		return temp;
	}
}
...
int sy6970_adc_read_charge_current(void)
{
	...
	if (ret < 0) {
		...
		volt = (int)(SY6970_ICHGR_BASE +
			     ((val & SY6970_ICHGR_MASK) >> SY6970_ICHGR_SHIFT) *
				     SY6970_ICHGR_LSB);
		return volt;
	}
}
...
int sy6970_set_chargecurrent(struct sy6970 *sy, int curr)
{
	...
	return sy6970_update_bits(sy, SY6970_REG_04, SY6970_ICHG_MASK,
				  ichg << SY6970_ICHG_SHIFT);
}
...
int sy6970_set_term_current(struct sy6970 *sy, int curr)
{
	...
	return sy6970_update_bits(sy, SY6970_REG_05, SY6970_ITERM_MASK,
				  iterm << SY6970_ITERM_SHIFT);
}
...
int sy6970_set_prechg_current(struct sy6970 *sy, int curr)
{
	...
	return sy6970_update_bits(sy, SY6970_REG_05, SY6970_IPRECHG_MASK,
				  iprechg << SY6970_IPRECHG_SHIFT);
}
...
int sy6970_set_chargevolt(struct sy6970 *sy, int volt)
{
	...
	return sy6970_update_bits(sy, SY6970_REG_06, SY6970_VREG_MASK,
				  val << SY6970_VREG_SHIFT);
}
...
int sy6970_set_input_volt_limit(struct sy6970 *sy, int volt)
{
	...
	sy6970_update_bits(sy, SY6970_REG_0D, SY6970_FORCE_VINDPM_MASK,
			   SY6970_FORCE_VINDPM_ENABLE
				   << SY6970_FORCE_VINDPM_SHIFT);

	return sy6970_update_bits(sy, SY6970_REG_0D, SY6970_VINDPM_MASK,
				  val << SY6970_VINDPM_SHIFT);
}
...
int sy6970_reset_watchdog_timer(struct sy6970 *sy)
{
	...
	return sy6970_update_bits(sy, SY6970_REG_03, SY6970_WDT_RESET_MASK,
				  val);
}
...
int sy6970_force_dpdm(struct sy6970 *sy, bool enable)
{
	...
	ret = sy6970_update_bits(sy, SY6970_REG_02, SY6970_FORCE_DPDM_MASK,
				 val);
...
int sy6970_enter_hiz_mode(struct sy6970 *sy)
{
	...
	if ((boot_mode == META_BOOT) || (g_oplus_chip->mmi_chg == 0)) {
		...
		result = sy6970_update_bits(sy, SY6970_REG_00,
...
int sy6970_set_boost_current(struct sy6970 *bq, int curr)
{
	...
	return sy6970_update_bits(bq, SY6970_REG_0A, SY6970_BOOST_LIM_MASK,
...
static int sy6970_enable_auto_dpdm(struct sy6970 *bq, bool enable)
{
	...
	ret = sy6970_update_bits(bq, SY6970_REG_02, SY6970_AUTO_DPDM_EN_MASK,
				 val);
...
int sy6970_set_boost_voltage(struct sy6970 *bq, int volt)
{
	...
	if (volt >
	    SY6970_BOOSTV_BASE + (SY6970_BOOSTV_MASK >> SY6970_BOOSTV_SHIFT) *
					 SY6970_BOOSTV_LSB)
		volt = SY6970_BOOSTV_BASE +
...
static int sy6970_read_idpm_limit(struct sy6970 *bq, int *icl)
{
	...
	if (ret < 0) {
		...
		*icl = SY6970_IDPM_LIM_BASE +
		       ((val & SY6970_IDPM_LIM_MASK) >> SY6970_IDPM_LIM_SHIFT) *
			       SY6970_IDPM_LIM_LSB;
		return 0;
	}
}
						    ...
						    struct sy6970 *bq)
{
	...
	ret = of_property_read_u32(np, "ti,sy6970,boost-voltage",
				   &pdata->boostv);
	if (ret) {
		pdata->boostv = 5000;
	...
	ret = of_property_read_u32(np, "ti,sy6970,boost-current",
				   &pdata->boosti);
	if (ret) {
		pdata->boosti = 1200;
...
static void oplus_keep_resume_awake_init(struct sy6970 *bq)
{
	...
	bq->keep_resume_ws =
		wakeup_source_register(NULL, "split_chg_keep_resume");
	return;
}
...
static irqreturn_t sy6970_irq_handler(int irq, void *data)
{
	...
	if (atomic_read(&bq->is_suspended) == 1) {
		...
		wait_event_interruptible_timeout(
			bq->wait, atomic_read(&bq->is_suspended) == 0,
	...
	chg_info(
		" prev_chg_type %d --> cur_chg_type %d, bq->oplus_chg_type %d, chg_det_enable %d, ret %d\n",
		prev_chg_type, cur_chg_type, bq->oplus_chg_type,
		bq->chg_det_enable, ret);
	...
	if ((prev_chg_type == POWER_SUPPLY_TYPE_USB_DCP) &&
		...
		chg_info(" keep cur_chg_type %d as prev_chg_type %d ",
	...
	if ((cur_chg_type == POWER_SUPPLY_TYPE_USB) ||
		...
		if (!bq->hvdcp_checked && !sy6970_is_hvdcp(bq)) {
			...
			schedule_delayed_work(&bq->sy6970_hvdcp_bc12_work,
			...
			chg_err("oplus_chg_type = %d, hvdcp_checked = %d",
				bq->oplus_chg_type, bq->hvdcp_checked);
		}
	} else {
		chg_err("oplus_chg_type = %d, vbus_type = %d",
			bq->oplus_chg_type, bq->vbus_type);
	}
...
static int oplus_chg_irq_gpio_init(struct sy6970 *sy)
{
	...
	pinctrl_select_state(sy->pinctrl,
			     sy->splitchg_inter_active); /* no_PULL */
...
static ssize_t sy6970_show_registers(struct device *dev,
...
static ssize_t sy6970_store_registers(struct device *dev,
...
void oplus_sy6970_set_mivr(int vbatt)
{
	...
	if (g_bq->hw_aicl_point == HW_AICL_POINT_VOL_5V_PHASE1 &&
	    vbatt > AICL_POINT_VOL_5V_HIGH) {
		g_bq->hw_aicl_point = HW_AICL_POINT_VOL_5V_PHASE2;
	} else if (g_bq->hw_aicl_point == HW_AICL_POINT_VOL_5V_PHASE2 &&
		   vbatt < AICL_POINT_VOL_5V_MID) {
		...
		if (chg_vol > AICL_POINT_VOL_9V) {
			...
			if (chip->batt_volt > AICL_POINT_VOL_5V_LOW)
				aicl_point_temp = aicl_point =
					SW_AICL_POINT_VOL_5V_PHASE2;
			else
				aicl_point_temp = aicl_point =
					SW_AICL_POINT_VOL_5V_PHASE1;
		}
	} else {
		if (chip->batt_volt > AICL_POINT_VOL_5V_LOW)
			aicl_point_temp = aicl_point =
...
aicl_pre_step:
	if ((chip->is_double_charger_support) &&
		...
		chg_debug("aicl: main_cur: %d,  slave_cur: %d", main_cur,
			  slave_cur);
		sy6970_set_input_current_limit(g_bq, main_cur);

		if (chip->sub_chg_ops &&
		    chip->sub_chg_ops->input_current_write_without_aicl) {
			chip->sub_chg_ops->input_current_write_without_aicl(
				slave_cur);
		}
...
int oplus_sy6970_get_charger_type(void)
{
	...
	if (g_bq->oplus_chg_type != g_oplus_chip->charger_type &&
	...
	if (oplus_chg_get_voocphy_support() == AP_DUAL_CP_VOOCPHY) {
		...
		if ((g_bq->before_unsuspend_icl == 0) ||
			...
			chg_err("ignore set icl [%d %d]\n",
				...
				g_bq->before_unsuspend_icl);
		} else {
			sy6970_set_input_current_limit(
				g_bq, g_bq->before_suspend_icl);
		}
		...
		if (g_oplus_chip) {
			if (oplus_vooc_get_fastchg_to_normal() == false &&
				...
				if (g_oplus_chip->authenticate &&
				    ...
				    (g_oplus_chip->charging_state !=
				     CHARGING_STATUS_FAIL) &&
...
void vol_convert_work(struct work_struct *work)
{
	...
	if (!g_bq->pdqc_setup_5v) {
		...
		if (oplus_sy6970_get_vbus() > SY6970_9V_THRES_MV) {
			...
			chg_info(
				"set_to_5v complete, set is_force_aicl as false.\n");
			g_bq->is_force_aicl = false;
			sy6970_disable_enlim(g_bq);
		} else {
...
int oplus_sy6970_set_qc_config(void)
{
	...
	chg_info(
		"temperature %d, tbatt_pdqc_to_5v_thr %d, charger_volt %d, batt_volt %d, ui_soc %d, \
		icharging %d, \
		vbatt_pdqc_to_5v_thr %d, \
		cool_down_force_5v %d\n",
		chip->temperature, chip->limits.tbatt_pdqc_to_5v_thr,
		chip->charger_volt, chip->batt_volt, chip->ui_soc,
		chip->icharging, chip->limits.vbatt_pdqc_to_5v_thr,
		...
		if (oplus_sy6970_get_vbus() > COOL_DOWN_5V_CHARGER_VOL) {
			...
			if (g_bq->is_bc12_end) {
				...
				schedule_delayed_work(
					&g_bq->sy6970_vol_convert_work, 0);
			} else {
				ret = -1;
			}
		} else {
...
#endif
	while (1) {
		wait_event_interruptible(oplus_chgtype_wq,
					 bq->chg_need_check == true);
		re_check_count = 0;
	...
	RECHECK:
		...
		case SY6970_VBUS_TYPE_SDP:
			...
			if (!bq->sdp_retry) {
				bq->sdp_retry = true;
				schedule_delayed_work(
					&g_bq->sy6970_retry_adapter_detection,
		...
		case SY6970_VBUS_TYPE_CDP:
			...
			if (!bq->cdp_retry) {
				bq->cdp_retry = true;
				schedule_delayed_work(
					&bq->sy6970_retry_adapter_detection,
...
static void charger_type_thread_init(void)
{
	...
	charger_type_kthread =
		kthread_run(oplus_charger_type_thread, g_bq, "chgtype_kthread");
...
static void sy6970_init_work_handler(struct work_struct *work)
{
	...
	if (oplus_sy6970_get_vbus() > VBUS_VALID_MV) {
...
#endif
		{
			...
			if (0 == ret) {
				g_bq->power_good =
					!!(reg_val & SY6970_PG_STAT_MASK);
			}

			chg_info("USB is inserted, power_good = %d !",
				 g_bq->power_good);
...
static void aicr_setting_work_callback(struct work_struct *work)
{
	...
	chg_debug("start.");
	if (g_bq->oplus_chg_type == POWER_SUPPLY_TYPE_USB_DCP &&
	    g_bq->is_force_aicl) {
	...
	chg_debug("begin aicr current = %d mA, wait %d mS", g_bq->aicr,
		  re_check_count * AICL_RECHECK_DELAY_MS);
...
int oplus_sy6970_set_ichg(int cur)
{
	...
	if (chip->is_double_charger_support) {
		if (chip->slave_charger_enable || chip->em_mode) {
			main_uA = uA * (PERCENTAGE_100 - chip->slave_pct) /
				  PERCENTAGE_100;
			ret = sy6970_set_chargecurrent(g_bq,
						       main_uA / UA_PER_MA);
			if (!ret) {
			...
			slave_uA = uA - main_uA;
			if (chip->sub_chg_ops) {
				chg_debug("set main_uA = %d, slave_uA = %d",
					  main_uA, slave_uA);
				...
				if (chip->sub_chg_ops
					    ->charging_current_write_fast) {
					ret = chip->sub_chg_ops
						      ->charging_current_write_fast(
							      slave_uA /
							      UA_PER_MA);
					if (ret) {
						chg_err("set slave_uA:%d failed",
							slave_uA);
					}
				}
			}
	...
	chg_info(
		" current = %d, slave_pct=%d uA = %d, main_uA = %d, slave_uA = %d, slave_charger_enable = %d\n",
		cur, chip->slave_pct, uA, main_uA, slave_uA,
		chip->slave_charger_enable);
	return 0;
}
				...
				const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&bq->sy6970_aicr_setting_work,
	...
	INIT_DELAYED_WORK(&bq->sy6970_hvdcp_bc12_work,
			  sy6970_hvdcp_bc12_work_handler);
	...
	bq->chg_dev =
		charger_device_register(bq->chg_dev_name, &client->dev, bq,
					&sy6970_chg_ops, &sy6970_chg_props);
