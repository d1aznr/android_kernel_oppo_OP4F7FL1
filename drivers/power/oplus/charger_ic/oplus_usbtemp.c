...
static void usbtemp_restart_work(struct work_struct *work)
{
	struct oplus_chg_chip *chip =
		container_of(work, struct oplus_chg_chip, usbtemp_restart_work);
#ifndef CONFIG_OPLUS_CHARGER_MTK
	...
	case OPLUS_USBTEMP_TIMER_STAGE0:
		...
		if (alarmtimer_get_rtcdev()) {
			if (usbtemp_recover_interval >
			    USBTEMP_CC_RECOVER_INTERVAL)
				...
				alarm_start_relative(&chip->usbtemp_alarm_timer,
						     ms_to_ktime(10000));
		} else {
...
enum alarmtimer_restart usbtemp_alarm_timer_func(struct alarm *alarm,
						 ktime_t now)
{
	struct oplus_chg_chip *chip =
		container_of(alarm, struct oplus_chg_chip, usbtemp_alarm_timer);
	;
...
static void oplus_usbtemp_set_recover(struct oplus_chg_chip *chip)
{
	if (alarmtimer_get_rtcdev()) {
		chip->usbtemp_timer_stage = OPLUS_USBTEMP_TIMER_STAGE0;
		alarm_start_relative(&chip->usbtemp_alarm_timer,
				     ms_to_ktime(USBTEMP_CC_RECOVER_INTERVAL));
	} else {
		chg_err("Failed to get soc alarm-timer");
	}
}
...
void oplus_update_usbtemp_current_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->new_usbtemp_cool_down_support) {
		if ((chip->icharging * -1) > 5000) {
			if (chip->usbtemp_cool_down_temp_gap ==
			    chip->usbtemp_cool_down_temp_first_gap) {
				limit_cur_cnt_r = 0;
				recover_cur_cnt_r = 0;
				limit_cur_cnt_l = 0;
				recover_cur_cnt_l = 0;
				chg_err("usbtemp_cool_down_temp_gap to %d",
	...
	if ((chip->usb_temp_r - chip->tbatt_temp / 10) >=
	    chip->usbtemp_cool_down_temp_gap) {
		limit_cur_cnt_r++;
		recover_cur_cnt_r = 0;
	} else if ((chip->usb_temp_r - chip->tbatt_temp / 10) <=
		   chip->usbtemp_cool_down_recovery_temp_gap) {
		recover_cur_cnt_r++;
		limit_cur_cnt_r = 0;
	}
	...
	if ((chip->usb_temp_l - chip->tbatt_temp / 10) >=
		...
		limit_cur_cnt_l++;
		recover_cur_cnt_l = 0;
	} else if ((chip->usb_temp_l - chip->tbatt_temp / 10) <=
		   chip->usbtemp_cool_down_recovery_temp_gap) {
		recover_cur_cnt_l++;
		limit_cur_cnt_l = 0;
	}
...
int oplus_usbtemp_monitor_common(void *data)
{
...
#endif
	if (alarmtimer_get_rtcdev()) {
		alarm_init(&chip->usbtemp_alarm_timer, ALARM_REALTIME,
			   usbtemp_alarm_timer_func);
	...
	pr_err("[%s]:run first chip->usbtemp_max_temp_thr[%d], chip->usbtemp_temp_up_time_thr[%d]!",
	       __func__, chip->usbtemp_max_temp_thr,
...
bool oplus_usbtemp_judge_temp_gap(struct oplus_chg_chip *chip, int current_temp,
				  int last_temp)
{
	if (!chip)
		return false;
	...
	int total_count)
{
	...
	if (oplus_usbtemp_temp_rise_fast_without_batt_temp(chip)) {
		...
		if (oplus_usbtemp_judge_temp_gap(chip, current_temp_l,
						 last_usb_temp_l) ||
		    oplus_usbtemp_judge_temp_gap(chip, current_temp_r,
						 last_usb_temp_r)) {
			for (i = 1; i <= retry_cnt; i++) {
				mdelay(RETRY_CNT_DELAY);
				get_usb_temp(chip);
