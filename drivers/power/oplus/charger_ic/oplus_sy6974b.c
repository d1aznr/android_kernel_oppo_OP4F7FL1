...
static int __sy6974b_read_block(struct chip_sy6974b *chip, u8 reg, u8 length,
				u8 *data)
{
	s32 ret = 0;
	...
	if (ret < 0) {
		while (retry > 0) {
			usleep_range(I2C_RETRY_DELAY_US, I2C_RETRY_DELAY_US);
			mutex_lock(&chip->i2c_lock);
			ret = i2c_smbus_read_i2c_block_data(chip->client, reg,
							    length, data);
			mutex_unlock(&chip->i2c_lock);
			if (ret < 0) {
				retry--;
...
static __maybe_unused int sy6974b_write_reg(struct chip_sy6974b *chip, int reg,
					    int data)
{
	int ret;
...
static __maybe_unused int sy6974b_read_block(struct chip_sy6974b *chip, u8 reg,
					     u8 length, u8 *data)
{
	int ret;
...
static __maybe_unused int sy6974b_config_interface(struct chip_sy6974b *chip,
						   int reg, int data, int mask)
{
	int ret;
	int tmp;
...
int sy6974b_set_vindpm_vol(int vol)
{
	...
	tmp = (vol - REG06_SY6974B_VINDPM_OFFSET) /
	      REG06_SY6974B_VINDPM_STEP_MV;
...
int oplus_sy6974b_enter_shipmode(bool en)
{
	...
	rc = sy6974b_config_interface(chip, REG07_SY6974B_ADDRESS, val,
				      REG07_SY6974B_BATFET_DIS_MASK);
...
int sy6974b_input_current_limit_without_aicl(int current_ma)
{
	...
	rc = sy6974b_config_interface(
		chip, REG00_SY6974B_ADDRESS,
...
int sy6974b_input_current_limit_write(int current_ma)
{
	...
	for (pre_icl_index = ARRAY_SIZE(sy6974b_usb_icl) - 1;
	...
	for (i = pre_icl_index; i > 1; i--) {
		rc = sy6974b_input_current_limit_without_aicl(
			sy6974b_usb_icl[i]);
		if (rc) {
	...
	if (buf != NULL && size != 0) {
		...
		if (size > 3) {
			...
			pr_err("[%s] write sy6974b reg 0x%02x with value 0x%02x !\n",
			       __func__, (unsigned int)chip->reg_access,
			       reg_value);
			ret = sy6974b_write_reg(chip, chip->reg_access,
						reg_value);
		} else {
			ret = sy6974b_read_reg(chip, chip->reg_access,
					       &reg_value);
...
static DEVICE_ATTR(sy6974b_access, 0660, sy6974b_access_show,
		   sy6974b_access_store);
...
void sy6974b_dump_registers(void)
{
	...
	ret = sy6974b_read_block(chip, SY6974B_FIRST_REG, SY6974B_REG_NUMBER,
				 val_buf);
	if (ret) {
		chg_err("Couldn't read 0x%02x length=%d ret = %d\n", addr,
			SY6974B_REG_NUMBER, ret);
	}
...
void sy6974b_dump_registers_debug(void)
{
	...
	if (g_oplus_chip->charger_exist || g_oplus_chip->ac_online ||
...
int sy6974b_set_wdt_timer(int reg)
{
	...
	rc = sy6974b_config_interface(chip, REG05_SY6974B_ADDRESS, reg,
...
int sy6974b_set_stat_dis(bool enable)
{
	...
	rc = sy6974b_config_interface(chip, REG00_SY6974B_ADDRESS,
				      enable ? REG00_SY6974B_STAT_DIS_ENABLE :
					       REG00_SY6974B_STAT_DIS_DISABLE,
...
int sy6974b_set_int_mask(int val)
{
	...
	rc = sy6974b_config_interface(chip, REG0A_SY6974B_ADDRESS, val,
				      REG0A_SY6974B_VINDPM_INT_MASK |
					      REG0A_SY6974B_IINDPM_INT_MASK);
	if (rc) {
...
bool sy6974b_get_bus_gd(void)
{
	...
	bus_gd = ((reg_val & REG0A_SY6974B_BUS_GD_MASK) ==
		  REG0A_SY6974B_BUS_GD_YES) ?
			 1 :
			 0;
	return bus_gd;
}
...
bool sy6974b_get_power_gd(void)
{
	...
	power_gd = ((reg_val & REG08_SY6974B_POWER_GOOD_STAT_MASK) ==
		    REG08_SY6974B_POWER_GOOD_STAT_GOOD) ?
			   1 :
			   0;
	return power_gd;
}
...
bool oplus_chg_is_usb_present(void)
{
	...
	if (chip && atomic_read(&chip->driver_suspended) == 1 && g_oplus_chip &&
...
int sy6974b_otg_ilim_set(int ilim)
{
	...
	rc = sy6974b_config_interface(chip, REG02_SY6974B_ADDRESS, ilim,
...
int sy6974b_check_charging_enable(void)
{
	...
	charging_enable = ((reg_val & REG01_SY6974B_CHARGING_MASK) ==
			   REG01_SY6974B_CHARGING_ENABLE) ?
				  1 :
				  0;
...
int sy6974b_unsuspend_charger(void)
{
	...
	if ((chip->before_unsuspend_icl == 0) ||
		...
		sy6974b_input_current_limit_without_aicl(
			chip->before_suspend_icl);
	}
...
void sy6974b_really_suspend_charger(bool en)
{
	...
	rc = sy6974b_config_interface(chip, REG00_SY6974B_ADDRESS,
				      en ? REG00_SY6974B_SUSPEND_MODE_ENABLE :
					   REG00_SY6974B_SUSPEND_MODE_DISABLE,
				      REG00_SY6974B_SUSPEND_MODE_MASK);
	if (rc < 0) {
...
int sy6974b_set_rechg_voltage(int recharge_mv)
{
	...
	rc = sy6974b_config_interface(chip, REG04_SY6974B_ADDRESS, recharge_mv,
...
int sy6974b_registers_read_full(void)
{
	...
	reg_full = ((reg_full & REG08_SY6974B_CHG_STAT_MASK) ==
		    REG08_SY6974B_CHG_STAT_CHG_TERMINATION) ?
			   1 :
			   0;
	if (reg_full) {
...
int sy6974b_set_ovp(int val)
{
	...
	rc = sy6974b_config_interface(chip, REG06_SY6974B_ADDRESS, val,
...
int sy6974b_get_iindet(void)
{
	...
	is_complete = ((reg_val & REG07_SY6974B_IINDET_EN_MASK) ==
		       REG07_SY6974B_IINDET_EN_DET_COMPLETE) ?
			      1 :
			      0;
	return is_complete;
}
...
bool sy6974b_get_otg_enable(void)
{
	...
	otg_enabled = ((reg_val & REG01_SY6974B_OTG_MASK) ==
		       REG01_SY6974B_OTG_ENABLE);
	return otg_enabled;
}
...
static int sy6974b_batfet_reset_disable(struct chip_sy6974b *chip, bool enable)
{
	...
	if (enable) {
		val = SY6974_BATFET_RST_DISABLE
		      << REG07_SY6974B_BATFET_RST_EN_SHIFT;
	} else {
		val = SY6974_BATFET_RST_ENABLE
		      << REG07_SY6974B_BATFET_RST_EN_SHIFT;
	}

	rc = sy6974b_config_interface(chip, REG07_SY6974B_ADDRESS, val,
				      REG07_SY6974B_BATFET_RST_EN_MASK);
...
int sy6974b_hardware_init(void)
{
	...
	sy6974b_set_int_mask(REG0A_SY6974B_VINDPM_INT_NOT_ALLOW |
...
struct oplus_chg_operations sy6974b_chg_ops = {
	...
	.input_current_ctrl_by_vooc_write =
		sy6974b_input_current_limit_ctrl_by_vooc_write,
...
static int sy6974b_parse_dt(struct chip_sy6974b *chip)
{
	...
	chip->real_suspend_6974b = of_property_read_bool(
		chip->client->dev.of_node, "qcom,use_real_suspend_6974b");

	chip->sy6974b_irq_gpio = of_get_named_gpio(chip->client->dev.of_node,
						   "sy6974b-irq-gpio", 0);
	...
	ret = devm_gpio_request(chip->dev, chip->sy6974b_irq_gpio,
				"sy6974b-irq-gpio");
	if (ret) {
	...
	chip->batfet_reset_disable = of_property_read_bool(
		chip->client->dev.of_node, "qcom,batfet_reset_disable");

	if (of_property_read_u32(chip->client->dev.of_node,
				 "normal-init-work-delay-ms",
				 &chip->normal_init_delay_ms))
		chip->normal_init_delay_ms = INIT_WORK_NORMAL_DELAY;

	if (of_property_read_u32(chip->client->dev.of_node,
				 "other-init-work-delay-ms",
...
int opchg_get_charger_type(void)
{
	...
	if (chip->oplus_charger_type != g_oplus_chip->charger_type &&
...
static void sy6974b_bc12_retry_work(struct work_struct *work)
{
	...
	struct chip_sy6974b *chip =
		container_of(dwork, struct chip_sy6974b, bc12_retry_work);
	...
	if (!chip->bc12_done) {
		...
		case REG08_SY6974B_VBUS_STAT_FLOAT:
			...
			if (oplus_pd_connected() &&
			    oplus_sm8150_get_pd_type() == PD_INACTIVE) {
...
static irqreturn_t sy6974b_irq_handler(int irq, void *data)
{
	...
	if (atomic_read(&chip->driver_suspended) == 1) {
		...
		wait_event_interruptible_timeout(
			chip->wait, atomic_read(&chip->driver_suspended) == 0,
	...
	chg_err("(%d,%d, %d, %d)\n", prev_pg, chip->power_good, curr_pg,
		bus_gd);
...
static int sy6974b_irq_register(struct chip_sy6974b *chip)
{
	...
	ret = devm_request_threaded_irq(chip->dev,
					gpio_to_irq(chip->sy6974b_irq_gpio),
					NULL, sy6974b_irq_handler,
				 ...
				 const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct chip_sy6974b),
			    GFP_KERNEL);
	if (!chip)
		return -ENOMEM;
	...
	if (oplus_daily_build() || get_eng_version() == HIGH_TEMP_AGING ||
...
static int sy6974b_pm_resume(struct device *dev)
{
	...
	if (client) {
		...
		if (chip) {
			...
			if ((resume_tm_sec > suspend_tm_sec) &&
			    (sleep_time > 60)) {
