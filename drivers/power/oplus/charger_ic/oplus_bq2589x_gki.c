...
int bq2589x_adc_read_vbus_volt(struct bq2589x *bq)
{
	...
	if (ret < 0) {
		...
		volt = ((val & BQ2589X_VBUSV_MASK) >> BQ2589X_VBUSV_SHIFT) *
		       BQ2589X_VBUSV_LSB;
		if (volt == 0) {
			volt = 0;
		} else {
...
static irqreturn_t bq2589x_irq_handler(int irq, void *data)
{
	...
	if ((oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC) ||
		...
		chg_err("fast_chg_type=%d, wait_ffc_flag=%d, prev_pg = %d, bq->power_good = %d, fastchg_started = %d\n",
			oplus_vooc_get_fast_chg_type(), chip->waiting_for_ffc,
			prev_pg, bq->power_good,
	...
	if ((cur_chg_type == POWER_SUPPLY_TYPE_USB) ||
		...
		if (!bq->hvdcp_checked && !bq2589x_is_hvdcp(bq)) {
			chg_info(" enable hvdcp.waiting_for_ffc=%d",
				 chip->waiting_for_ffc);
			...
			if (chip->waiting_for_ffc == true) {
				...
				if (g_oplus_chip->chgic_mtk.oplus_info
					    ->hvdcp_disabled) {
					bq->hvdcp_checked = true;
				} else {
					schedule_delayed_work(
						&bq->bq2589x_hvdcp_bc12_work,
...
int oplus_bq2589x_set_aicr(int current_ma)
{
	...
	chg_info(
		"usb input max current limit=%d, aicl_point_temp=%d, chip->stop_chg=%d, chip->mmi_chg=%d",
...
int oplus_bq2589x_get_charger_type(void)
{
	...
	if ((oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC) ||
		...
		chg_info(
			"%s,fastchg_to_normal = %d;fast_chg_type = %d;fastchg_to_warm = %d\n",
			__func__, oplus_vooc_get_fastchg_to_normal(),
...
static char *bq2589x_charger_supplied_to[] = { "battery",
					       "mtk-master-charger" };
