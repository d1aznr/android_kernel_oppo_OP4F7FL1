...
oplus_vooc_get_fastchg_started_pfunc(bool (*pfunc)(void))
{
	return;
}
...
void oplus_get_props_from_adsp_by_buffer(void)
{
	...
	if (!chip->external_gauge) {
		if (pre_full_vol != full_vol || pre_full_curr != full_curr) {
			...
			oplus_adsp_voocphy_set_full_para_qbg(
				full_para_vol_curr);
...
static int get_otg_scheme(struct oplus_chg_chip *chip)
{
	...
	if (otg_scheme == OTG_SCHEME_UNDEFINE) {
		if (oplus_ccdetect_check_is_gpio(chip)) /*for 20031 series*/
			otg_scheme = ((get_PCB_Version() > DVT1) ?
...
static void oplus_otg_init_status_func(struct work_struct *work)
{
	int count = 20;
	struct battery_chg_dev *bcdev =
		container_of(work, struct battery_chg_dev, otg_init_work.work);

	printk(KERN_ERR "!!!!oplus_otg_init_status_func\n");
	while (count--) {
		if (bcdev->is_chargepd_ready &&
		    (g_oplus_chip->wireless_support &&
		     !oplus_wpc_check_chip_is_null()))
			break;
...
static int oplus_oem_misc_ctl(void)
{
	...
	rc = write_property_id(bcdev, pst, USB_OEM_MISC_CTL,
...
static void oplus_oem_lcm_en_check_work(struct work_struct *work)
{
	...
	bcdev->oem_misc_ctl_data |=
		OEM_MISC_CTL_DATA_PAIR(OEM_MISC_CTL_CMD_LCM_EN, enable);
	bcdev->oem_misc_ctl_data |=
		OEM_MISC_CTL_DATA_PAIR(OEM_MISC_CTL_CMD_NCM_AUTO_MODE, enable);
	bcdev->oem_misc_ctl_data |= OEM_MISC_CTL_DATA_PAIR(
		OEM_MISC_CTL_CMD_VPH_TRACK_HIGH, vph_track_high);
	if (vph_track_high) {
		bcdev->oem_misc_ctl_data |=
			OEM_MISC_CTL_DATA_PAIR(OEM_MISC_CTL_CMD_LCM_25K, false);
		bcdev->oem_misc_ctl_data |= OEM_MISC_CTL_DATA_PAIR(
			OEM_MISC_CTL_CMD_NCM_AUTO_MODE, vph_track_high);
	}
	chg_err(" oem_misc_ctl_data = 0x%x.\n", bcdev->oem_misc_ctl_data);
...
static void oplus_switch_to_wired_charge(struct battery_chg_dev *bcdev)
{
	...
	if (is_ext_chg_ops()) {
		...
		if (oplus_wpc_get_otg_charging()) {
			...
			schedule_delayed_work(&bcdev->wait_wired_charge_on,
...
static void oplus_switch_from_wired_charge(struct battery_chg_dev *bcdev)
{
	if (is_ext_chg_ops()) {
		if (oplus_wpc_get_otg_charging()) {
			...
			schedule_delayed_work(&bcdev->wait_wired_charge_off,
					      msecs_to_jiffies(100));
		} else {
...
bool oplus_get_wired_chg_present(void)
{
	...
	if (oplus_vchg_trig_is_support() == true &&
	    oplus_get_vchg_trig_status() == 0)
		return true;
	if (oplus_vchg_trig_is_support() == false &&
	    oplus_get_usb_online() == true)
		return true;
	return false;
}
#endif /*OPLUS_FEATURE_CHG_BASIC*/
...
static void battery_chg_update_usb_type_work(struct work_struct *work)
{
	...
	if (g_oplus_chip && usb_psy_desc.type == POWER_SUPPLY_TYPE_UNKNOWN &&
		...
		printk(KERN_ERR
		       "!!! usb_psy_desc.type: [%d]to_warm[%d]dummy[%d]to_normal[%d]started[%d]\n",
...
void oplus_adsp_voocphy_set_full_para_qbg(int full_volt_curr)
{
	...
	rc = write_property_id(bcdev, pst, BATT_BAT_FULL_VOL_SET,
			       full_volt_curr);
	if (rc) {
	...
	if (prop_id == USB_ONLINE) {
		...
		if (oplus_chg_get_voocphy_support() == NO_VOOCPHY ||
			...
			if (pval->intval == 2 ||
			    ...
			    (oplus_vooc_get_fastchg_to_normal() == true) ||
			    (g_oplus_chip &&
			     g_oplus_chip->charger_volt > 3000 &&
			     g_oplus_chip->tbatt_temp > 700)) {
		...
		if (online ^ pval->intval) {
			...
			if (online == 1) {
				...
				if (g_oplus_chip &&
				    g_oplus_chip->usbtemp_wq_init_finished) {
		...
		if (adap_type ^ pval->intval) {
			...
			if (oplus_chg_get_voocphy_support() == NO_VOOCPHY ||
				...
				if (pval->intval != 0 &&
					...
					g_oplus_chip->prop_status =
						POWER_SUPPLY_STATUS_CHARGING;
				}
				printk(KERN_ERR
				       "!!! usb adap type: [%d]to_warm[%d]dummy[%d]to_normal[%d]started[%d]\n",
				       pval->intval,
				       oplus_vooc_get_fastchg_to_warm(),
				       oplus_vooc_get_fastchg_dummy_started(),
				       oplus_vooc_get_fastchg_to_normal(),
				       oplus_vooc_get_fastchg_started());
				if (pval->intval == 0) {
					if (oplus_vooc_get_fastchg_to_warm() ==
						    true ||
					    oplus_vooc_get_fastchg_dummy_started() ==
						    true ||
					    oplus_vooc_get_fastchg_to_normal() ==
...
int oplus_input_pg_gpio_init(struct oplus_chg_chip *chip)
{
	...
	input_pg_default =
		pinctrl_lookup_state(input_pg_pinctrl, "input_pg_default");
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	bcdev->pmic_is_pm7250b = of_property_read_bool(bcdev->dev->of_node,
...
static int oplus_usbtemp_iio_init(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_match_string(bcdev->dev->of_node, "io-channel-names",
				      "battcon_therm_adc");
	if (rc >= 0) {
		bcdev->iio.battcon_btb_chan =
			iio_channel_get(bcdev->dev, "battcon_therm_adc");
		if (IS_ERR(bcdev->iio.battcon_btb_chan)) {
			rc = PTR_ERR(bcdev->iio.battcon_btb_chan);
			if (rc != -EPROBE_DEFER)
				dev_err(bcdev->dev,
					"battcon_btb_chan  get error, %ld\n",
					rc);
...
int oplus_chg_get_battery_btb_temp_cal(void)
{
	...
	if (IS_ERR_OR_NULL(bcdev->iio.battcon_btb_chan)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: bcdev->iio.battcon_btb_chan  is  NULL !\n",
		       __func__);
		return temp;
	}
...
static void oplus_init_usbtemp_wakelock(void)
{
	...
	if (!is_awake_init) {
		...
		usbtemp_wakelock = wakeup_source_register(
...
static void oplus_usbtemp_recover_func(struct oplus_chg_chip *chip)
{
	...
	if (level == 1) {
		...
		do {
			...
			msleep(2000);
			count_time++;
			pr_err("[OPLUS_USBTEMP] oplus_usbtemp_recover_func count=%d",
			       count_time);
...
void oplus_chg_parse_usbtemp_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_count_elems_of_size(node, "temp_curr_monitor_table",
					     sizeof(u32));
	if (rc < 0 || rc > 3)
		pr_err("Count temp_curr_monitor_table failed, rc = %d\n", rc);
	else {
		length = rc;
		rc = of_property_read_u32_array(node, "temp_curr_monitor_table",
						(u32 *)temp_curr_monitor_table,
						length);
		if (rc)
	...
	while (!kthread_should_stop()) {
		...
		if (chip->tbatt_temp / 10 <= USB_50C &&
		    (((chip->usb_temp_l >= USB_57C) &&
		      (chip->usb_temp_l < USB_100C)) ||
		     ((chip->usb_temp_r >= usb_temp_r_default) &&
		      (chip->usb_temp_r < USB_100C)))) {
			pr_err("in loop 1");
			for (i = 1; i < retry_cnt; i++) {
				...
				oplus_get_usbtemp_volt(chip);
				get_usb_temp(chip);
				if (chip->usb_temp_r >= usb_temp_r_default &&
				    chip->usb_temp_r < USB_100C)
					count_r++;
	...
	dischg:
		...
		if (usbtemp_recover_test ||
			...
			if (bcdev->pmic_is_pm7250b == false) {
				oplus_set_usbtemp_wakelock(true);
				cancel_delayed_work(
					&bcdev->usbtemp_recover_work);
				schedule_delayed_work(
					&bcdev->usbtemp_recover_work,
					msecs_to_jiffies(
						usbtemp_recover_interval));
			}
...
static int smbchg_wls_set_boost_en(bool en)
{
	...
	if (en && chip->wls_set_boost_vol == 0) {
		rc = write_property_id(bcdev, pst, WLS_BOOST_VOLT,
...
static int oplus_chg_set_input_current(int current_ma)
{
	...
	if (chip->charger_type != POWER_SUPPLY_TYPE_USB) {
		...
		for (pre_icl_index = ARRAY_SIZE(usb_icl) - 1;
		...
		for (i = pre_icl_index; i > 1; i--) {
			rc = write_property_id(bcdev, pst, prop_id,
					       usb_icl[i] * 1000);
			if (rc) {
...
aicl_end:
	...
	chg_debug(
		"aicl_end: usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_end\n",
		chg_vol, i, usb_icl[i], aicl_point);
	goto aicl_return;
aicl_boost_back:
	rc = write_property_id(bcdev, pst, prop_id, usb_icl[i] * 1000);
...
static int fg_bq27541_get_battery_fcc(void)
{
	...
	if (chip->fg_info_package_read_support) {
		...
		if (chip->batt_capacity_mah > 0 &&
		    chip->batt_capacity_mah < fcc) {
...
static void usb_enum_check(struct work_struct *work)
{
	...
	case CHECK_CHARGER_EXIST:
		...
		if (g_oplus_chip->charger_exist) {
			...
			schedule_delayed_work(
				&g_oplus_chip->pmic_spmi.bcdev_chip
					 ->usb_enum_check_work,
...
static void start_usb_enum_check(void)
{
	...
	schedule_delayed_work(
		&g_oplus_chip->pmic_spmi.bcdev_chip->usb_enum_check_work,
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&bcdev->oem_lcm_en_check_work,
			  oplus_oem_lcm_en_check_work);
	...
	INIT_DELAYED_WORK(&bcdev->reset_turn_on_chg_work,
			  oplus_reset_turn_on_chg_work);
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
