...
static struct bq2589x_pep20_efficiency_table
	bq2589x_pe2_table[BQ2589X_PE2_TABLE_COUNT] = {
		{ 3400000, 8000000 }, { 3500000, 8000000 },
		{ 3600000, 8000000 }, { 3700000, 8500000 },
		{ 3800000, 8500000 }, { 3900000, 8500000 },
		{ 4000000, 9000000 }, { 4100000, 9000000 },
		{ 4200000, 9000000 }, { 4400000, 9000000 },
	};
...
int bq2589x_enter_hiz_mode(struct bq2589x *bq)
{
	...
	if ((boot_mode == META_BOOT) ||
		...
		result = bq2589x_update_bits(bq, BQ2589X_REG_00,
...
static int bq2589x_vmin_limit(struct bq2589x *bq)
{
	...
	return bq2589x_update_bits(bq, BQ2589X_REG_03, BQ2589X_SYS_MINV_MASK,
				   val);
}
...
static int bq2589x_switch_to_hvdcp(struct bq2589x *bq, enum hvdcp_type type)
{
	...
	case HVDCP_5V:
		if (bq->is_bq2589x) {
			...
			ret = bq2589x_update_bits(bq, BQ2589X_REG_01, mask,
						  val);
		} else {
...
static irqreturn_t bq2589x_irq_handler(int irq, void *data)
{
	...
	if ((cur_chg_type == POWER_SUPPLY_TYPE_USB) ||
	    (cur_chg_type == POWER_SUPPLY_TYPE_USB_CDP)) {
		if ((get_project() == 21251) || (get_project() == 21253) ||
		    (get_project() == 21254))
		...
		if (!bq->hvdcp_checked && !bq2589x_is_hvdcp(bq)) {
			...
			schedule_delayed_work(&bq->bq2589x_hvdcp_bc12_work,
			...
			schedule_delayed_work(&bq->bq2589x_aicr_setting_work,
					      0);
			oplus_chg_wake_update_work();
		} else {
...
static int bq2589x_init_device(struct bq2589x *bq)
{
	...
	if (bq->boot_mode == META_BOOT) {
		...
		INIT_DELAYED_WORK(&bq->enter_hz_work,
				  bq2589x_enter_hz_work_handler);
...
static int bq2589x_chgdet_en(struct bq2589x *bq, bool en)
{
	...
	if (en) {
		...
		if (bq->power_good &&
		    mt6357_get_vbus_voltage() < MT6357_VBUS_VOL_TH) {
					     ...
					     u32 chr_vol)
{
	...
	for (i = 4; i >= 0; i--) {
		...
		if (flag == 0) {
			bq2589x_set_input_current_limit(bq,
							BQ2589X_INP_CURR_700MA);
			msleep(PEOFFTIME);
			get_monotonic_boottime(&ptime[j]);
...
int oplus_bq2589x_set_aicr(int current_ma)
{
	...
	if (chip) {
		chg_debug("usb input max current limit=%d, em_mode = %d",
			  current_ma, chip->em_mode);
	}
	...
	msleep(120);
	chg_vol =
		mt6357_get_vbus_voltage(); /* bq2589x_adc_read_vbus_volt(g_bq); */
	if (chg_vol < aicl_point_temp) {
		i = i - 2; /* 1.2 */
		goto aicl_pre_step;
	}
...
void oplus_bq2589x_safe_calling_status_check()
{
	...
	if ((g_oplus_chip->charger_volt > CHG_VOL_7V5) &&
	    (g_oplus_chip->calling_on)) {
		...
		if ((g_oplus_chip->ui_soc < SOC_90) ||
		    (g_oplus_chip->batt_volt < BATT_VOL_4V25)) {
...
int oplus_bq2589x_hardware_init(void)
{
	...
	if (strcmp(g_bq->chg_dev_name, "primary_chg") == 0) {
		...
		if (ret < 0) {
			...
			bq2589x_set_input_current_limit(g_bq,
							BQ2589X_INP_CURR_500MA);
		}
	}
...
#ifdef CONFIG_TCPC_CLASS
	if (g_bq->tcpc) {
		if (tcpm_inquire_typec_attach_state(g_bq->tcpc) ==
		    TYPEC_ATTACHED_SRC) {
			chg_info(" OTG online.");
			return false;
		}
		...
		if (mt6357_get_vbus_voltage() > MT6357_VBUS_VOL_7V5) {
			...
			if (g_bq->is_bc12_end) {
				...
				schedule_delayed_work(
					&g_bq->bq2589x_vol_convert_work, 0);
			}
		}
	} else { /* 9v */
		if (mt6357_get_vbus_voltage() < MT6357_VBUS_VOL_7V5) {
	...
	RECHECK:
		...
		case BQ2589X_VBUS_TYPE_SDP:
			...
			if (!bq->sdp_retry) {
				bq->sdp_retry = true;
				schedule_delayed_work(
					&g_bq->bq2589x_retry_adapter_detection,
		...
		case BQ2589X_VBUS_TYPE_CDP:
			...
			if (!bq->cdp_retry) {
				bq->cdp_retry = true;
				schedule_delayed_work(
					&bq->bq2589x_retry_adapter_detection,
...
int oplus_bq2589x_pd_setup(void)
{
	...
	if (chip->limits.vbatt_pdqc_to_5v_thr > 0 &&
	    chip->charger_volt > CHG_VOL_7V5 &&
	    chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr &&
	    chip->ui_soc >= SOC_85 && chip->icharging > -1000) {
...
static void bq2589x_init_work_handler(struct work_struct *work)
{
	...
	if ((boot_mode != META_BOOT) && mt6357_get_vbus_status() &&
	...
	if (chip->is_double_charger_support) {
		if (chip->slave_charger_enable || chip->em_mode) {
			...
			if (chip->sub_chg_ops) {
				...
				if (temp > 32) {
					slave_ua =
						slave_ua + (64 - temp) * 1000;
				}

				ret = oplus_bq2560x_set_current(slave_ua /
								1000);
				if (ret) {
				 ...
				 const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&bq->bq2589x_hvdcp_bc12_work,
			  bq2589x_hvdcp_bc12_work_handler);
