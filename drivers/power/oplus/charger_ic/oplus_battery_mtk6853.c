...
static void get_chargeric_temp(struct charger_data *pdata)
{
	...
	if (pinfo && pinfo->support_ntc_01c_precision) {
		...
		step_vol =
			((con_volt_19165[i] - con_volt_19165[i + 1]) * 1000) /
			10;
		temp_offset =
			((con_volt_19165[i] - pdata->chargeric_temp_volt) *
			 1000) /
			step_vol;
	...
	chg_err("chargeric_temp:%d con_volt_19165[%d] = %d con_temp_19165[%d] = %d chargeric_temp_volt = %d step_vol = %d temp_offset = %d\n",
		pdata->chargeric_temp, i, con_volt_19165[i], i,
		con_temp_19165[i], pdata->chargeric_temp_volt, step_vol,
		temp_offset);
	return;
}
#endif
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop() && dischg_flag == false) {
		...
		if ((chip->usb_temp_r >= USB_57C &&
			...
			count_l = 0;
			count = 0;
		} else if (((chip->usb_temp_r - chip->temperature / 10) > 12 &&
			    chip->usb_temp_r >= USB_20C &&
			    chip->usb_temp_r < USB_100C) ||
			   ((chip->usb_temp_l - chip->temperature / 10) > 12 &&
			    chip->usb_temp_l >= USB_20C &&
			    chip->usb_temp_l < USB_100C)) {
			if (count <= time_count) {
				if (count == 0) {
				...
				if (((chip->usb_temp_r - last_usb_temp_r) >=
				     3) ||
				    ((chip->usb_temp_l - last_usb_temp_l) >=
				     3)) {
					for (i = 0; i < RETRY_COUNT; i++) {
						mdelay(RETRY_CNT_DELAY);
						oplus_get_usbtemp_volt(chip);
	...
	case POWER_SUPPLY_PROP_TYPEC_CC_ORIENTATION:
		if (pinfo != NULL && pinfo->tcpc != NULL) {
			if (tcpm_inquire_typec_attach_state(pinfo->tcpc) !=
			    TYPEC_UNATTACHED) {
				val->intval = (int)tcpm_inquire_cc_polarity(
						      pinfo->tcpc) +
					      1;
			} else {
				val->intval = 0;
	...
	if (pinfo->data.dual_charger_support) {
		...
		if (is_vooc_project()) {
			...
			case POWER_SUPPLY_PROP_EM_MODE:
				...
				break;
			default:
				rc = oplus_battery_property_is_writeable(psy,
									 psp);
				break;
			}
		} else {
	...
	if (pinfo->data.dual_charger_support) {
		...
		if (is_vooc_project()) {
			...
			case POWER_SUPPLY_PROP_FIXED_FREQ:
				...
				printk(KERN_ERR
				       "[OPLUS_CHG][%s]: intval [%d] charger_exist[%d] charge_pump_mode[%d]\n",
				       __func__, val->intval,
				...
				union power_supply_propval *val)
{
	...
	if (pinfo->data.dual_charger_support) {
		...
		if (is_vooc_project()) {
			switch (psp) {
			case POWER_SUPPLY_PROP_FIXED_FREQ:
				val->intval = OPLUS_DIVIDER_WORK_MODE_FIXED ==
							      charge_pump_mode ?
						      1 :
						      0;
				break;
...
bool oplus_chg_check_vbus_change(const unsigned int checkpoint,
				 const bool isVbusBoost)
{
