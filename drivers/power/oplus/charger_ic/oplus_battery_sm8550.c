...
void lcm_frequency_ctrl(void)
{
	...
	bcdev = chip->pmic_spmi.bcdev_chip;
	if ((oplus_chg_get_charger_voltage() > LCM_CHARGER_VOL_THR_MV) ||
...
static void oplus_chg_wls_status_keep_clean_work(struct work_struct *work)
{
	...
	if (chip == NULL) {
		...
		schedule_delayed_work(
			&bcdev->status_keep_delay_unlock_work,
...
static int oplus_wrx_otg_en_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	bcdev->oplus_custom_gpio.wrx_otg_en_pinctrl =
		devm_pinctrl_get(bcdev->dev);
	...
	bcdev->oplus_custom_gpio.wrx_otg_en_active = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.wrx_otg_en_pinctrl,
		"wrx_otg_en_active");
	...
	bcdev->oplus_custom_gpio.wrx_otg_en_sleep = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.wrx_otg_en_pinctrl,
		"wrx_otg_en_sleep");
...
void oplus_set_wrx_otg_en_val(int value)
{
	...
	if (value) {
		gpio_direction_output(bcdev->oplus_custom_gpio.wrx_otg_en_gpio,
				      1);
		pinctrl_select_state(
			bcdev->oplus_custom_gpio.wrx_otg_en_pinctrl,
...
int oplus_chg_set_qc_config(void)
{
	...
	if (chip->limits.vbatt_pdqc_to_5v_thr > 0 &&
		...
		if (oplus_chg_get_charger_voltage() < NON_STANDARD_QC_VOL_THR) {
			...
			rc = write_property_id(bcdev, pst, BATT_SET_QC,
					       PDQC_5V);
			if (rc)
