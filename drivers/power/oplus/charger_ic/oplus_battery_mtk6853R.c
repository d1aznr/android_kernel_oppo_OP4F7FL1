...
int oplus_force_get_subboard_temp(void)
{
	...
	if (!is_param_init) {
		...
		if (of_property_read_bool(g_oplus_chip->dev->of_node,
					  "qcom,sub_board_pull_up_r_support")) {
			if (of_property_read_u32(g_oplus_chip->dev->of_node,
						 "qcom,sub_board_pull_up_r",
						 &pull_up_r) >= 0) {
...
static ssize_t soc_ajust_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
	int val = 0;
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if (oplus_ccdetect_support_check() == true) {
		...
		if (level <= 0) {
			schedule_delayed_work(
				&ccdetect_work,
				msecs_to_jiffies(SCHEDULE_DELAY_MS));
		}
...
static int mtk_charger_remove(struct platform_device *dev)
{
	...
	if (g_oplus_chip->nightstandby_support) {
		oplus_battery_node_delete(
			oplus_mtk6853_cust_battery_attributes);
	}
#ifdef OPLUS_FEATURE_CHG_BASIC
