...
#define INT_DISABLE_UFP_ACCESSORY                                              \
	(0x01 << INT_DISABLE_UFP_ACCESSORY_SHIFT) /*RW*/
...
struct type_c_parameters {
	...
	enum accessory_attach_type
		accessory_attach; /*if an accessory is attached*/
	enum cable_attach_type
		active_cable_attach; /*if an active_cable is attached*/
...
#ifdef OPLUS_ARCH_EXTENDS
struct blocking_notifier_head cc_audio_notifier =
	BLOCKING_NOTIFIER_INIT(cc_audio_notifier);
...
void sgm7220_set_typec_sinkonly(void)
{
	...
	if (sgm7220_update_reg(gchip->i2c, REG_CTL, value, CTR_DISABLE_RD_RP)) {
		pr_err("%s: REG_CTL CTR_DISABLE_RD_RP_DEFAULT failed\n",
		       __func__);
		msleep(300);
		if (sgm7220_update_reg(gchip->i2c, REG_CTL, value,
				       CTR_DISABLE_RD_RP)) {
	...
	if (sgm7220_update_reg(gchip->i2c, REG_INT, value,
			       INT_DISABLE_UFP_ACCESSORY)) {
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	value = (value & MOD_CURRENT_MODE_ADVERTISE) >>
		MOD_CURRENT_MODE_ADVERTISE_SHIFT;
	return snprintf(buf, PAGE_SIZE, "%d\n", value);
}
				       ...
				       const char *buf, size_t size)
{
	...
	ret = sgm7220_update_reg(info->i2c, REG_MOD, value,
				 MOD_CURRENT_MODE_ADVERTISE);
	if (ret < 0) {
		pr_err("%s: update reg fail!\n", __func__);
	}
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	value = (value & MOD_CURRENT_MODE_DETECT) >>
		MOD_CURRENT_MODE_DETECT_SHIFT;
...
static ssize_t show_chip_info(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct sgm7220_info *info = dev_get_drvdata(dev);
	u8 reg_val;
	...
	for (i = 0, j = 0; i < ID_REG_LEN; i++) {
		reg_val = 0;
		ret = sgm7220_read_reg(
			info->i2c, (REG_BASE + (ID_REG_LEN - 1) - i), &reg_val);
		if (ret < 0) {
			pr_err("%s: read reg fail!\n", __func__);
...
#define TYPE_C_ATTR(field, format_string)                                      \
	...
	{                                                                      \
		struct sgm7220_info *info = dev_get_drvdata(dev);              \
		return snprintf(buf, PAGE_SIZE, format_string,                 \
				info->type_c_param.field);                     \
	}                                                                      \
	static DEVICE_ATTR(field, S_IRUGO, field##_show, NULL);

static DEVICE_ATTR(gpio_debug, S_IRUGO | S_IWUSR, gpio_debug_show,
		   gpio_debug_store);
...
static struct device_attribute *usb_typec_attributes[] = {
	&dev_attr_mode_select, &dev_attr_current_advertise,
...
static int oplus_register_extcon(struct sgm7220_info *info)
{
	...
	rc = extcon_set_property_capability(info->extcon, EXTCON_USB,
					    EXTCON_PROP_USB_TYPEC_POLARITY);
	rc |= extcon_set_property_capability(info->extcon, EXTCON_USB,
					     EXTCON_PROP_USB_SS);
	rc |= extcon_set_property_capability(info->extcon, EXTCON_USB_HOST,
					     EXTCON_PROP_USB_TYPEC_POLARITY);
	rc |= extcon_set_property_capability(info->extcon, EXTCON_USB_HOST,
					     EXTCON_PROP_USB_SS);
...
static void oplus_notify_extcon_props(struct sgm7220_info *info, int id)
{
	...
	extcon_set_property(info->extcon, id, EXTCON_PROP_USB_TYPEC_POLARITY,
			    val);
	val.intval = true;
...
static void process_mode_register(struct sgm7220_info *info)
{
	...
	val = ((tmp & MOD_CURRENT_MODE_DETECT) >>
	       MOD_CURRENT_MODE_DETECT_SHIFT);
	info->type_c_param.current_det = val;

	/* check accessory attch */
	tmp = status;
	val = ((tmp & MOD_ACCESSORY_CONNECTED) >>
	       MOD_ACCESSORY_CONNECTED_SHIFT);
...
static void process_interrupt_register(struct sgm7220_info *info)
{
	...
	if (pre_state_type == CABLE_STATE_NOT_ATTACHED &&
	...
	if (attach_changed) {
		if (cur_attach) {
			if (info->type_c_param.attach_state ==
			    CABLE_STATE_AS_DFP) { //host
				info->sink_src_mode = SRC_MODE;
				oplus_typec_sink_insertion();
			} else if (info->type_c_param.attach_state ==
				   CABLE_STATE_AS_UFP) { //device
	...
	if (info->probe) {
		if (info->type_c_param.attach_state != CABLE_STATE_AS_DFP) {
			oplus_sgm7220_set_mode(MODE_UFP);
			pr_debug(
				"%s not found host devices and set to ufp mode]\n",
				__func__);
		} else {
...
static irqreturn_t sgm7220_irq_thread(int irq, void *handle)
{
	...
	if (atomic_read(&info->suspended) == 1) {
		pr_notice(
			" sgm7220_irq_thread:suspended and wait_event_interruptible %d\n",
			OPLUS_WAIT_RESUME_TIM);
		wait_event_interruptible_timeout(
			info->wait, atomic_read(&info->suspended) == 0,
	...
	ret = sgm7220_update_reg(info->i2c, REG_INT,
...
static int sgm7220_initialization(struct sgm7220_info *info)
{
	...
	ret = sgm7220_update_reg(info->i2c, REG_MOD, reg_val,
				 MOD_CURRENT_MODE_ADVERTISE);
	if (ret < 0) {
...
static int sgm7220_parse_gpio_from_dts(struct device_node *np,
...
int oplus_sgm7220_set_mode(int mode)
{
	...
	pr_err("%s: mode[%d] readback REG_SET:0x%0x[0x%0x] done\n", __func__,
	       mode, REG_SET, i2c_smbus_read_byte_data(gchip->i2c, REG_SET));
	...
	pr_err("%s: mode[%d] readback REG_INT:0x%0x[0x%0x] done\n", __func__,
	       mode, REG_INT, i2c_smbus_read_byte_data(gchip->i2c, REG_INT));
	return 0;
}
			 ...
			 const struct i2c_device_id *id)
{
	...
	ret = of_property_read_u32(client->dev.of_node,
				   "oplus,headphone-det-support",
				   &headphone_det_support);
	...
	pinctrl_select_state(info->pinctrl,
			     info->typec_inter_active); /* no_PULL */
