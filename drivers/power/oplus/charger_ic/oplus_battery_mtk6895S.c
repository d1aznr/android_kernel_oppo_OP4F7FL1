...
static ssize_t shipmode_test_store(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t size)
{
	unsigned long val = 0;
...
void oplus_chg_choose_gauge_curve(int index_curve)
{
	...
	if (index_curve == CHARGER_SUBTYPE_QC ||
		...
		if (voocphy_chip->fast_chg_type !=
		    FASTCHG_CHARGER_TYPE_UNKOWN) {
			if (voocphy_chip->adapter_type == ADAPTER_SVOOC) {
				svooc_30w_adapter = oplus_chg_svooc_30w_adapter(
					voocphy_chip);
			}
			...
			} else if ((voocphy_chip->adapter_type ==
				    ADAPTER_SVOOC) &&
				   (svooc_30w_adapter == false)) {
				target_index_curve =
					CHARGER_FASTCHG_SVOOC_CURVE;
			} else if ((voocphy_chip->adapter_type ==
				    ADAPTER_SVOOC) &&
				   (svooc_30w_adapter == true)) {
				target_index_curve =
					CHARGER_FASTCHG_VOOC_AND_QCPD_CURVE;
			} else {
	...
	chg_err("target_index_curve=%d, last_curve_index=%d, index_curve=%d, svooc_30w_adapter is %s\n",
		target_index_curve, last_curve_index, index_curve,
		svooc_30w_adapter == true ? "true" : "false");
	if (target_index_curve != last_curve_index) {
		printk(KERN_ERR
		       "%s: index_curve() =%d	target_index_curve =%d last_curve_index =%d  pd svooc %s",
		       __func__, index_curve, target_index_curve,
		       last_curve_index,
		       chip->pd_svooc == true ? "true" : "false");

		printk(KERN_ERR
		       "%s svooc_30w_adapter %s set_charge_power_sel  %d",
		       __func__, svooc_30w_adapter == true ? "true" : "false",
		       target_index_curve);
...
static void tea_encrypt(uint32_t *v, uint32_t *k)
{
	...
	for (i = 0; i < 32; i++) {
		sum += delta;
		temp_l = ((v[1] << 4) + k[0]) ^ (v[1] + sum) ^
			 ((v[1] >> 5) + k[1]);
		temp_h = ((v[0] << 4) + k[2]) ^ (v[0] + sum) ^
			 ((v[0] >> 5) + k[3]);
		v[0] += temp_l;
		v[1] += temp_h;
	}
...
extern int tcpc_dual_role_set_prop_dr(struct typec_port *port,
				      enum typec_data_role data);

static int oplus_pps_tcpc_set_dr(struct oplus_chg_chip *chip,
				 enum typec_data_role data)
{
	struct tcpc_device *tcpc_dev = tcpc_dev_get_by_name("type_c_port0");
...
static void init_pps_key(void)
{
	...
	if (np) {
		for (i = 0; i < PPS_KEY_COUNT; i++) {
			...
			ret = of_property_read_u32(np, oplus_pps_offset,
						   &(publick_key[i]));
			if (ret) {
				chg_err(" publick_key[%d]: %s %d fail", i,
					oplus_pps_offset, publick_key[i]);
			}
		}
		...
		for (i = 0; i < PPS_KEY_COUNT; i++) {
			...
			ret = of_property_read_u32(np, oplus_pps_offset,
						   &(private_key1[i]));
			if (ret) {
				chg_err(" private_key1[%d]: %s %d fail", i,
					oplus_pps_offset, private_key1[i]);
			}
		}
		...
		for (i = 0; i < PPS_KEY_COUNT; i++) {
			...
			ret = of_property_read_u32(np, oplus_pps_offset,
						   &(private_key2[i]));
			if (ret) {
				chg_err(" private_key2[%d]: %s %d fail", i,
					oplus_pps_offset, private_key2[i]);
			}
		}
...
int oplus_pps_pd_exit(void)
{
	...
	ret = tcpm_set_pd_charging_policy(tcpc, DPM_CHARGING_POLICY_VSAFE5V,
					  NULL);
...
int oplus_get_chargeric_temp(void)
{
	...
	if (!is_param_init) {
		...
		ntc_param.i_table_size = (sizeof(charger_ic_temp_table) /
					  sizeof(struct temp_param));
		is_param_init = true;
		chg_err("ntc_type:%d,critical_low:%d,pull_up_r=%d,pull_up_voltage=%d,tap_min=%d,tap_max=%d,table_size=%d\n",
			ntc_param.e_ntc_type, ntc_param.i_tap_over_critical_low,
...
void oplus_set_otg_switch_status(bool value)
{
	...
	if (pinfo != NULL && pinfo->tcpc != NULL) {
		...
		if (oplus_ccdetect_check_is_wd0(g_oplus_chip)) {
			printk(KERN_ERR
			       "[OPLUS_CHG][oplus_set_otg_switch_status]: wdo_detect = %d\n",
			       pinfo->wd0_detect);
			pinfo->wd0_detect = tcpci_get_wd0_status();
			if (pinfo->wd0_detect) {
				printk(KERN_ERR
		...
		if (0 == tcpm_typec_change_role(pinfo->tcpc,
						value ? TYPEC_ROLE_TRY_SNK :
							TYPEC_ROLE_SNK)) {
			printk(KERN_ERR
			       "[OPLUS_CHG][%s]: otg switch[%d] success\n",
			       __func__, value);
		} else {
			printk(KERN_ERR
			       "[OPLUS_CHG][%s]: otg switch[%d] failed.\n",
			       __func__, value);
		}
	} else {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: pinfo or  pinfo->tcpc is NULL, otg switch failed.\n",
		       __func__);
	}
}
...
int oplus_chg_set_qc_config_forvoocphy(void)
{
	...
	chg_err(" qc9v svooc [%d %d %d %d]", chip->limits.vbatt_pdqc_to_9v_thr,
	...
	if (!chip->calling_on && chip->charger_volt < 6500 && chip->soc < 90 &&
		...
		schedule_delayed_work(&hvdcp_detect_work,
		...
		printk(KERN_ERR "%s: qc9v svooc  batt_volt: %d", __func__,
		       chip->batt_volt);
	}
...
#ifdef CONFIG_OPLUS_HVDCP_SUPPORT
	if ((charg_subtype != CHARGER_SUBTYPE_PD) &&
	    (charg_subtype != CHARGER_SUBTYPE_PPS)) {
...
struct oplus_chg_operations mtk6375_chg_ops = {
	...
	.enable_qc_detect =
		mt6375_enable_hvdcp_detect, //for qc 9v2a //bringup 6895
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	pinfo->chargeric_temp_chan =
		devm_iio_channel_get(oplus_chip->dev, "auxadc5-chargeric_temp");
