...
static int bq24196_input_current_limit_write(int value)
{
	...
	j = 2; /* 500 */
	rc = bq24196_config_interface(
		REG00_BQ24196_ADDRESS,
		j << REG00_BQ24196_INPUT_CURRENT_LIMIT_SHIFT,
...
aicl_pre_step:
	if ((j >= 2) &&
	    (j <= ARRAY_SIZE(bq24196_usbin_input_current_limit) - 1))
...
static int bq24196_input_current_limit_write(void, int value)
{
	...
	for (i = ARRAY_SIZE(bq24196_usbin_input_current_limit) - 1; i >= 0;
	     i--) {
	...
	for (j = 2; j <= i; j++) {
		...
		if (chg_vol < chip->sw_aicl_point) {
			...
			if ((j >= 2) &&
			    (j <=
			     ARRAY_SIZE(bq24196_usbin_input_current_limit) - 1))
				aicl_result =
...
static int bq24196_charging_current_write_fast(int chg_cur)
{
	...
	if (chg_cur < BQ24196_MIN_FAST_CURRENT_MA_ALLOWED) {
		...
		chg_cur = chg_cur * 5;
		value = (chg_cur - BQ24196_MIN_FAST_CURRENT_MA) /
			BQ24196_FAST_CURRENT_STEP_MA;
		value <<= REG02_BQ24196_FAST_CHARGING_CURRENT_LIMIT_SHIFT;
		value = value |
			REG02_BQ24196_FAST_CHARGING_CURRENT_FORCE20PCT_ENABLE;
	} else {
		value = (chg_cur - BQ24196_MIN_FAST_CURRENT_MA) /
			BQ24196_FAST_CURRENT_STEP_MA;
		value <<= REG02_BQ24196_FAST_CHARGING_CURRENT_LIMIT_SHIFT;
...
static int bq24196_set_vindpm_vol(int vol)
{
	...
	value = (vol - REG00_BQ24196_VINDPM_OFFSET) /
		REG00_BQ24196_VINDPM_STEP_MV;
	value <<= REG00_BQ24196_VINDPM_SHIFT;

	rc = bq24196_config_interface(REG00_BQ24196_ADDRESS, value,
				      REG00_BQ24196_VINDPM_MASK);
	return rc;
}
...
static int bq24196_set_complete_charge_timeout(int val)
{
	...
	if (val == OVERTIME_AC) {
		val = REG05_BQ24196_CHARGING_SAFETY_TIME_ENABLE |
		      REG05_BQ24196_FAST_CHARGING_TIMEOUT_8H;
	} else if (val == OVERTIME_USB) {
		val = REG05_BQ24196_CHARGING_SAFETY_TIME_ENABLE |
		      REG05_BQ24196_FAST_CHARGING_TIMEOUT_12H;
	} else {
		val = REG05_BQ24196_CHARGING_SAFETY_TIME_DISABLE |
		      REG05_BQ24196_FAST_CHARGING_TIMEOUT_8H;
	}

	rc = bq24196_config_interface(
		REG05_BQ24196_ADDRESS, val,
...
int bq24196_float_voltage_write(int vfloat_mv)
{
	...
	rc = bq24196_config_interface(REG04_BQ24196_ADDRESS, value,
				      REG04_BQ24196_CHARGING_VOL_LIMIT_MASK);
	return rc;
}
...
int bq24196_set_prechg_current(int ipre_mA)
{
	...
	value = (ipre_mA - BQ24196_MIN_PRE_CURRENT_MA) /
		BQ24196_PRE_CURRENT_STEP_MA;
	value <<= REG03_BQ24196_PRE_CHARGING_CURRENT_LIMIT_SHIFT;

	return bq24196_config_interface(
		REG03_BQ24196_ADDRESS, value,
		REG03_BQ24196_PRE_CHARGING_CURRENT_LIMIT_MASK);
}
...
static int bq24196_set_termchg_current(int term_curr)
{
	...
	value = (term_curr - BQ24196_MIN_TERM_CURRENT_MA) /
		BQ24196_TERM_CURRENT_STEP_MA;
	value <<= REG03_BQ24196_TERM_CHARGING_CURRENT_LIMIT_SHIFT;
	chg_debug(" value=%d\n", value);

	bq24196_config_interface(
		REG03_BQ24196_ADDRESS, value,
		REG03_BQ24196_TERM_CHARGING_CURRENT_LIMIT_MASK);
	return rc;
}
...
int bq24196_set_rechg_voltage(int recharge_mv)
{
	...
	rc = bq24196_config_interface(
		REG04_BQ24196_ADDRESS, reg,
		REG04_BQ24196_RECHARGING_THRESHOLD_VOL_MASK);
	if (rc) {
...
int bq24196_set_wdt_timer(int reg)
{
	...
	rc = bq24196_config_interface(REG05_BQ24196_ADDRESS, reg,
				      REG05_BQ24196_I2C_WATCHDOG_TIME_MASK);
...
static int bq24196_set_chging_term_disable(void)
{
	...
	rc = bq24196_config_interface(REG05_BQ24196_ADDRESS,
				      REG05_BQ24196_TERMINATION_DISABLE,
...
int bq24196_disable_charging()
{
	...
	rc = bq24196_config_interface(REG01_BQ24196_ADDRESS,
				      REG01_BQ24196_CHARGING_DISABLE,
				      REG01_BQ24196_CHARGING_MASK);
	if (rc < 0) {
...
static bool bq24196_check_charger_suspend_enable(void)
{
	...
	charger_suspend_enable = ((reg_val & REG00_BQ24196_SUSPEND_MODE_MASK) ==
				  REG00_BQ24196_SUSPEND_MODE_ENABLE) ?
					 true :
					 false;
...
static int bq24196_check_charging_enable(void)
{
	...
	charging_enable = ((reg_val & REG01_BQ24196_CHARGING_MASK) ==
			   REG01_BQ24196_CHARGING_ENABLE) ?
				  1 :
				  0;
...
int bq24196_registers_read_full(void)
{
	...
	reg_ovp = ((reg_ovp & REG09_BQ24196_BATTERY_VOLATGE_MASK) ==
		   REG09_BQ24196_BATTERY_VOLATGE_HIGH_ERROR) ?
			  1 :
			  0;
...
int bq24196_otg_enable(void)
{
	...
	bq24196_config_interface(REG00_BQ24196_ADDRESS,
				 REG00_BQ24196_SUSPEND_MODE_DISABLE,
				 REG00_BQ24196_SUSPEND_MODE_MASK);

	rc = bq24196_config_interface(REG01_BQ24196_ADDRESS,
				      REG01_BQ24196_OTG_ENABLE,
				      REG01_BQ24196_OTG_MASK);
	if (rc) {
				...
				const struct i2c_device_id *id)
{
	...
	chg_ic = devm_kzalloc(&client->dev, sizeof(struct chip_bq24196),
			      GFP_KERNEL);
