	...
	if (info != NULL) {
		...
		if (idx == MAIN_CHARGER) {
			if (pinfo->chargeric_temp_chan) {
				...
				pdata->junction_temp_min =
					pdata->chargeric_temp;
				pdata->junction_temp_max =
					pdata->chargeric_temp;
...
void charger_ftm_init(void)
{
	...
	class_dev = (struct class_device *)device_create(
		charger_class, NULL, charger_devno, NULL, CHARGER_DEVNAME);
	if (IS_ERR(class_dev)) {
...
void mtk_charger_int_handler(void)
{
	...
	if (is_vooc_project() == false) {
		...
		if (mt_get_charger_type() != CHARGER_UNKNOWN) {
			oplus_wake_up_usbtemp_thread();
			oplus_set_divider_work_mode(
				OPLUS_DIVIDER_WORK_MODE_FIXED);
			    ...
			    unsigned long pm_event, void *unused)
{
	...
	case PM_POST_SUSPEND:
		...
		if (timespec_compare(&now, &pinfo->endtime) >= 0 &&
		    pinfo->endtime.tv_sec != 0 && pinfo->endtime.tv_nsec != 0) {
	...
	while (1) {
		...
		if (is_disable_charger() == false) {
			if (is_charger_on == true) {
				...
				wakeup_sc_algo_cmd(&pinfo->sc.data,
						   SC_EVENT_CHARGING, 0);
			} else
				wakeup_sc_algo_cmd(&pinfo->sc.data,
				...
				struct device *dev)
{
	...
	/* PE 4.0 single */
	if (of_property_read_u32(np, "pe40_single_charger_input_current",
				 &val) >= 0) {
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	pr_debug("%s: detected = %d, pe20_connect = %d, pe_connect = %d\n",
		 __func__, is_ta_detected, mtk_pe20_get_is_connect(pinfo),
...
static int oplus_chg_shipmode_parse_dt(struct oplus_chg_chip *chip)
{
	...
	chip->disable_ship_mode =
		of_property_read_bool(node, "qcom,disable-ship-mode");
	chip->normalchg_gpio.ship_gpio =
...
static void get_chargeric_temp(struct charger_data *pdata)
{
	...
	if (pinfo && pinfo->support_ntc_01c_precision) {
		if ((0 == i) || (ARRAY_SIZE(con_volt_20131) - 1 ==
				 i)) { /* avoid array out of bounds */
			pdata->chargeric_temp = pdata->chargeric_temp * 10;
		} else {
			pdata->chargeric_temp = pdata->chargeric_temp * 10;
			step_vol =
				((con_volt_20131[i] - con_volt_20131[i + 1]) *
				 1000) /
				10;
			temp_offset = ((con_volt_20131[i] -
					pdata->chargeric_temp_volt) *
				       1000) /
				      step_vol;
...
int oplus_mt6360_pd_setup_forsvooc(void)
{
	...
	if (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY ||
		...
		if (!chip->calling_on && !chip->camera_on &&
			...
			printk(KERN_ERR "%s: pd9v svooc  default[%d %d]",
			       __func__, em_mode, chip->batt_volt);
		}
	} else {
		if (!chip->calling_on && chip->charger_volt < 6500 &&
		    chip->soc < 90 && chip->temperature <= 530 &&
		    chip->cool_down_force_5v == false &&
			...
			ret = oplus_pdc_setup(&vbus_mv, &ibus_ma);
			printk(KERN_ERR
			       "%s: PD request: vbus[%d], ibus[%d], ret[%d]\n",
			...
			if (chip->charger_volt > 7500 &&
				...
				printk(KERN_ERR
				       ...
				       __func__, vbus_mv, ibus_ma, ret);
				msleep(300);
				printk(KERN_ERR "%s: charger voltage=%d",
...
int oplus_chg_set_qc_config_forsvooc(void)
{
	...
	if (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY ||
		...
		if (!chip->calling_on && !chip->camera_on &&
			...
			printk(KERN_ERR "%s: qc9v svooc  default[%d %d]",
			       __func__, em_mode, chip->batt_volt);
		}
	} else {
		if (!chip->calling_on && chip->charger_volt < 6500 &&
...
void scd_ctrl_cmd_from_user(void *nl_data, struct sc_nl_msg_t *ret_msg)
{
	...
	case SC_DAEMON_CMD_SETTING: {
		...
		chr_debug(
			"rcv data:%d %d %d %d %d %d %d %d %d %d %d %d %d %d Ans:%d\n",
			data.data[0], data.data[1], data.data[2], data.data[3],
			data.data[4], data.data[5], data.data[6], data.data[7],
			data.data[8], data.data[9], data.data[10],
				...
				struct charger_data *pdata)
{
	chr_err("sck: en:%d pid:%d %d %d %d %d %d\n", info->sc.enable,
		info->sc.g_scd_pid, info->sc.pre_ibat, info->sc.sc_ibat,
...
void sc_update(struct charger_manager *pinfo)
{
	...
	charger_dev_get_charging_current(pinfo->chg1_dev,
					 &pinfo->sc.data.data[SC_IBAT_SETTING]);
	pinfo->sc.data.data[SC_IBAT_SETTING] =
		pinfo->sc.data.data[SC_IBAT_SETTING] / 1000;
...
static ssize_t show_sc_en(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
...
static ssize_t store_sc_en(struct device *dev, struct device_attribute *attr,
			   const char *buf, size_t size)
{
	unsigned long val = 0;
	int ret;
...
static ssize_t show_sc_stime(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
...
static ssize_t store_sc_stime(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t size)
{
	unsigned long val = 0;
	int ret;
...
static ssize_t show_sc_etime(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
...
static ssize_t store_sc_etime(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t size)
{
	unsigned long val = 0;
	int ret;
...
static ssize_t show_sc_tuisoc(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
...
static ssize_t store_sc_tuisoc(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t size)
{
	unsigned long val = 0;
...
static ssize_t show_sc_ibat_limit(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
...
static ssize_t store_sc_ibat_limit(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t size)
{
	unsigned long val = 0;
...
static DEVICE_ATTR(sc_ibat_limit, 0664, show_sc_ibat_limit,
		   store_sc_ibat_limit);

static ssize_t show_typec_sbu_voltage(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
...
static ssize_t store_typec_sbu_voltage(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t size)
{
	return -1;
}
static DEVICE_ATTR(typec_sbu_voltage, 0664, show_typec_sbu_voltage,
		   store_typec_sbu_voltage);
...
/* add for charger_wakelock */
	...
	pinfo->usbcon_temp_chan =
		iio_channel_get(oplus_chip->dev, "usbcon_temp");
	...
	pinfo->batcon_temp_chan =
		iio_channel_get(oplus_chip->dev, "batcon_temp");
	...
	ret_device_file =
		device_create_file(&(pdev->dev), &dev_attr_sc_ibat_limit);
	if (IS_ERR(oplus_chip->usb_psy) == 0) {
		ret_device_file = device_create_file(
			&oplus_chip->usb_psy->dev, &dev_attr_typec_sbu_voltage);
	}
	info->chg1_consumer =
