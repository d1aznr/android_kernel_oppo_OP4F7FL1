...
bool oplus_get_wired_chg_present(void)
{
	...
	if (psy == NULL || IS_ERR(psy)) {
		...
		psy = devm_power_supply_get_by_phandle(&pinfo->pdev->dev,
						       "charger");
		pinfo->chg_psy = psy;
	}
...
int get_uisoc(struct mtk_charger *info)
{
	...
	if (bat_psy == NULL || IS_ERR(bat_psy)) {
		...
		ret = power_supply_get_property(
			bat_psy, POWER_SUPPLY_PROP_CAPACITY, &prop);
		ret = prop.intval;
	}
...
int get_battery_voltage(struct mtk_charger *info)
{
	...
	if (bat_psy == NULL || IS_ERR(bat_psy)) {
		...
		ret = power_supply_get_property(
			bat_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &prop);
...
int get_battery_temperature(struct mtk_charger *info)
{
	...
	if (bat_psy == NULL || IS_ERR(bat_psy)) {
		...
		tmp_ret = power_supply_get_property(
			bat_psy, POWER_SUPPLY_PROP_TEMP, &prop);
		ret = prop.intval / 10;
	}
...
int get_battery_current(struct mtk_charger *info)
{
	...
	if (bat_psy == NULL || IS_ERR(bat_psy)) {
		...
		tmp_ret = power_supply_get_property(
			bat_psy, POWER_SUPPLY_PROP_CURRENT_NOW, &prop);
		ret = prop.intval / 1000;
	}
...
static int get_pmic_vbus(struct mtk_charger *info, int *vchr)
{
	...
	if (chg_psy == NULL || IS_ERR(chg_psy)) {
		...
		ret = power_supply_get_property(
			chg_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &prop);
	}
	*vchr = prop.intval;
...
bool is_battery_exist(struct mtk_charger *info)
{
	...
	if (bat_psy == NULL || IS_ERR(bat_psy)) {
		...
		tmp_ret = power_supply_get_property(
			bat_psy, POWER_SUPPLY_PROP_PRESENT, &prop);
		ret = prop.intval;
	}
...
int get_charger_type(struct mtk_charger *info)
{
	...
	if (chg_psy == NULL || IS_ERR(chg_psy)) {
		...
		ret = power_supply_get_property(chg_psy, POWER_SUPPLY_PROP_TYPE,
						&prop2);

		ret = power_supply_get_property(
			chg_psy, POWER_SUPPLY_PROP_USB_TYPE, &prop3);
...
static void pmic_set_register_value(struct regmap *map, unsigned int addr,
...
unsigned int pmic_get_register_value(struct regmap *map, unsigned int addr,
				     unsigned int mask, unsigned int shift)
{
...
int disable_hw_ovp(struct mtk_charger *info, int en)
{
	...
	pmic_set_register_value(regmap, PMIC_RG_VCDT_HV_EN_ADDR,
				PMIC_RG_VCDT_HV_EN_SHIFT,
				PMIC_RG_VCDT_HV_EN_MASK, en);
...
static void mtk_charger_external_power_changed(struct power_supply *psy)
{
	...
	if (oplus_vooc_get_fastchg_started() == true &&
		...
		printk(KERN_ERR
		       "[OPLUS_CHG] %s oplus_vooc_get_fastchg_started = true!\n",
		       __func__);
		if (prop.intval) {
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (!prop.intval) {
		oplus_chg_global_event(chip->chgic_mtk.oplus_info->usb_ocm,
	...
	if (is_wls_ocm_available(chip)) {
		oplus_chg_mod_get_property(
			chip->wls_ocm, OPLUS_CHG_PROP_TRX_ONLINE, &temp_val);
		if (!!temp_val.intval) {
	...
	if (info->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
		while (1) {
			ret = tcpm_inquire_pd_source_apdo(
				info->tcpc, TCPM_POWER_CAP_APDO_TYPE_PPS,
				&cap_i, &apdo_cap);
			...
			if (info->srccap.pdp > 0 &&
			    !info->srccap.pwr_limit[idx])
			...
			info->srccap.maxwatt[idx] =
				apdo_cap.max_mv * apdo_cap.ma;
			info->srccap.minwatt[idx] =
		...
		if (noti->vbus_state.mv) {
			if (g_oplus_chip->chgic_mtk.oplus_info->usb_ocm) {
				oplus_chg_global_event(
					g_oplus_chip->chgic_mtk.oplus_info
						->usb_ocm,
			...
			if (primary_charger && !otg_flag) {
				charger_dev_set_boost_current_limit(
					primary_charger,
...
static __s16 oplus_ts_ch_volt_to_temp(__u32 dwvolt)
{
	...
	dwvcrich =
		((__u64)g_tap_over_critical_low * (__u64)g_rap_pull_up_voltage);
...
int oplus_get_chargeric_temp(void)
{
	...
	if (pinfo && pinfo->chargeric_temp_chan) {
		ret = iio_read_channel_processed(pinfo->chargeric_temp_chan,
						 &val);
		if (ret < 0) {
...
static struct ntc_temp *init_ntc_param(char *str)
{
	...
	if (length < 0) {
		...
		ntc_param = devm_kzalloc(chip->dev, sizeof(struct ntc_temp),
					 GFP_KERNEL);
		rc = of_property_read_u32_array(node, str, (u32 *)ntc_param,
						length);
		if (rc < 0) {
			...
			chg_err("%s: %d %d %d %d %d %d %d\n", str,
				ntc_param->i_tap_over_critical_low,
			...
			ntc_param->i_table_size = (sizeof(battcon_temp_table) /
...
static __s16 oplus_res_to_temp(struct ntc_temp *ntc_param)
{
	...
	if (ntc_param->ui_dwvolt > ((__u32)dwvcrich)) {
		...
		if (ntc_param->i_rap_pull_up_voltage != ntc_param->ui_dwvolt) {
			tres = (ntc_param->i_rap_pull_up_r *
				ntc_param->ui_dwvolt) /
			       (ntc_param->i_rap_pull_up_voltage -
				ntc_param->ui_dwvolt);
		} else {
...
static int oplus_get_temp_volt(struct iio_channel *ntc_temp_chan,
...
static int oplus_get_ntc_temp(struct iio_channel *ntc_temp_chan,
			      struct ntc_temp *ntc_param)
{
	...
	if (!is_param_init) {
		...
		ntc_param_default.i_table_size = (sizeof(battcon_temp_table) /
						  sizeof(struct temp_param));
		is_param_init = true;
		chg_err("ntc_param_default critical_low:%d,pull_up_r=%d,pull_up_voltage=%d,tap_min=%d,tap_max=%d,table_size=%d\n",
			ntc_param_default.i_tap_over_critical_low,
			ntc_param_default.i_rap_pull_up_r,
			ntc_param_default.i_rap_pull_up_voltage,
			ntc_param_default.i_tap_min,
			ntc_param_default.i_tap_max,
			ntc_param_default.i_table_size);
	}
	if (!ntc_param) {
		ntc_param_default.ui_dwvolt =
			oplus_get_temp_volt(ntc_temp_chan, &ntc_param_default);
		ntc_param_default.ui_dwvolt = ntc_param_default.ui_dwvolt +
					      ntc_param_default.volt_offset;
		chargeric_temp = oplus_res_to_temp(&ntc_param_default);
	} else {
		ntc_param->ui_dwvolt =
			oplus_get_temp_volt(ntc_temp_chan, ntc_param);
		ntc_param->ui_dwvolt =
			ntc_param->ui_dwvolt + ntc_param->volt_offset;
...
static int oplus_chg_get_main_battery_btb(void)
{
	...
	if (!pinfo->batcon_temp_chan) {
		pinfo->batcon_temp_chan =
			iio_channel_get(g_oplus_chip->dev, "batcon_temp");
	...
	if (pinfo->batcon_temp_chan) {
		temp = oplus_get_ntc_temp(pinfo->batcon_temp_chan,
					  pinfo->batt_ntc_param);
		return temp;
	}
...
static int oplus_chg_get_sub_battery_btb(void)
{
	...
	if (!pinfo->sub_batcon_temp_chan) {
		pinfo->sub_batcon_temp_chan =
			iio_channel_get(g_oplus_chip->dev, "sub_batcon_temp");
	...
	if (pinfo->sub_batcon_temp_chan) {
		sub_temp = oplus_get_ntc_temp(pinfo->sub_batcon_temp_chan,
					      pinfo->sub_batt_ntc_param);
		return sub_temp;
	}
...
int oplus_chg_get_usb_btb_temp_cal(void)
{
	...
	if (!pinfo->usbcon_temp_chan) {
		pinfo->usbcon_temp_chan =
			iio_channel_get(g_oplus_chip->dev, "usbcon_temp");
	...
	if (pinfo->usbcon_temp_chan) {
		temp = oplus_get_ntc_temp(pinfo->usbcon_temp_chan,
					  pinfo->usb_ntc_param);
		temp = temp / 10;
...
int oplus_get_otg_online_status(void)
{
	...
	if (oplus_ccdetect_check_is_gpio(chip) == true) {
		...
		if (level !=
		    gpio_get_value(chip->chgic_mtk.oplus_info->ccdetect_gpio)) {
			printk(KERN_ERR
	...
	if ((pre_level ^ level) || (pre_typec_otg ^ typec_otg)) {
		...
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: gpio[%s], c-otg[%d], otg_online[%d]\n",
...
void oplus_set_otg_switch_status(bool value)
{
	if (pinfo != NULL && pinfo->tcpc != NULL) {
		if (oplus_ccdetect_check_is_gpio(g_oplus_chip) == true) {
			if (gpio_get_value(g_oplus_chip->chgic_mtk.oplus_info
						   ->ccdetect_gpio) == 0) {
				printk(KERN_ERR
...
void oplus_ccdetect_irq_register(struct oplus_chg_chip *chip)
{
	...
	ret = devm_request_threaded_irq(
		chip->dev, chip->chgic_mtk.oplus_info->ccdetect_irq, NULL,
...
void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	if (IS_ERR_OR_NULL(pinfo->usb_temp_v_l_chan)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: pinfo->usb_temp_v_l_chan  is  NULL !\n",
		       __func__);
...
usbtemp_next:
	...
	if (IS_ERR_OR_NULL(pinfo->usb_temp_v_r_chan)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: pinfo->usb_temp_v_r_chan  is  NULL !\n",
		       __func__);
...
static void oplus_usbtemp_thread_init(void)
{
	if (g_oplus_chip->support_usbtemp_protect_v2)
		...
		oplus_usbtemp_kthread =
			kthread_run(oplus_usbtemp_monitor_common, g_oplus_chip,
				    "usbtemp_kthread");
		...
		if (g_oplus_chip->usbtemp_check) {
			if (g_oplus_chip->support_usbtemp_protect_v2)
				wake_up_interruptible(
					&g_oplus_chip
						 ->oplus_usbtemp_wq_new_method);
			else
				wake_up_interruptible(
					&g_oplus_chip->oplus_usbtemp_wq);
		}
	}
}
...
static int oplus_wrx_otg_en_gpio_init(struct oplus_chg_chip *chip)
{
	...
	info->wrx_otg_en_active = pinctrl_lookup_state(info->wrx_otg_en_pinctrl,
						       "wrx_otg_en_active");
	...
	info->wrx_otg_en_sleep = pinctrl_lookup_state(info->wrx_otg_en_pinctrl,
						      "wrx_otg_en_sleep");
...
void oplus_set_wrx_otg_en_val(int value)
{
	...
	if (value) {
		...
		pinctrl_select_state(info->wrx_otg_en_pinctrl,
...
int oplus_boost_en_gpio_init(struct oplus_chg_chip *chip)
{
	...
	info->boost_en_active =
		pinctrl_lookup_state(info->boost_en_pinctrl, "boost_en_active");
	...
	info->boost_en_sleep =
		pinctrl_lookup_state(info->boost_en_pinctrl, "boost_en_sleep");
...
void oplus_set_boost_en_val(int value)
{
	...
	if (value) {
		...
		pinctrl_select_state(info->boost_en_pinctrl,
				     info->boost_en_active);
	} else {
		gpio_direction_output(info->boost_en_gpio, 0);
		pinctrl_select_state(info->boost_en_pinctrl,
...
int oplus_ext1_otg_en_gpio_init(struct oplus_chg_chip *chip)
{
	...
	info->ext1_otg_en_active = pinctrl_lookup_state(
		info->ext1_otg_en_pinctrl, "ext1_otg_en_active");
	if (IS_ERR_OR_NULL(info->ext1_otg_en_active)) {
		chg_err("get ext1_otg_en_active fail\n");
		return -EINVAL;
	}

	info->ext1_otg_en_sleep = pinctrl_lookup_state(
		info->ext1_otg_en_pinctrl, "ext1_otg_en_sleep");
	...
	pinctrl_select_state(info->ext1_otg_en_pinctrl,
			     info->ext1_otg_en_sleep);
	return 0;
}
...
int oplus_ext2_otg_en_gpio_init(struct oplus_chg_chip *chip)
{
	...
	info->ext2_otg_en_active = pinctrl_lookup_state(
		info->ext2_otg_en_pinctrl, "ext2_otg_en_active");
	if (IS_ERR_OR_NULL(info->ext2_otg_en_active)) {
		chg_err("get ext2_otg_en_active fail\n");
		return -EINVAL;
	}

	info->ext2_otg_en_sleep = pinctrl_lookup_state(
		info->ext2_otg_en_pinctrl, "ext2_otg_en_sleep");
	...
	pinctrl_select_state(info->ext2_otg_en_pinctrl,
			     info->ext2_otg_en_sleep);
	return 0;
}
...
static int oplus_mtk_parse_wls_dt(struct oplus_chg_chip *chip)
{
	...
	if (info->wrx_otg_en_gpio <= 0) {
		...
		if (gpio_is_valid(info->wrx_otg_en_gpio)) {
			rc = gpio_request(info->wrx_otg_en_gpio,
					  "wrx_otg_en-gpio");
			if (rc) {
	...
	info->ext1_otg_en_gpio =
		of_get_named_gpio(node, "qcom,ext1_otg_en-gpio", 0);
	if (info->ext1_otg_en_gpio <= 0) {
		...
		if (gpio_is_valid(info->ext1_otg_en_gpio)) {
			rc = gpio_request(info->ext1_otg_en_gpio,
					  "ext1_otg_en-gpio");
			if (rc) {
	...
	info->ext2_otg_en_gpio =
		of_get_named_gpio(node, "qcom,ext2_otg_en-gpio", 0);
	if (info->ext2_otg_en_gpio <= 0) {
		...
		if (gpio_is_valid(info->ext2_otg_en_gpio)) {
			rc = gpio_request(info->ext2_otg_en_gpio,
					  "ext2_otg_en-gpio");
			if (rc) {
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	if (rc) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: oplus_mtk_parse_wls_dt fail!\n",
		       __func__);
		return -EINVAL;
	}
...
static enum power_supply_property wls_props[] = {
	...
	POWER_SUPPLY_PROP_CURRENT_MAX, POWER_SUPPLY_PROP_PRESENT,
};
...
static int oplus_power_supply_init(struct oplus_chg_chip *chip)
{
	...
	mt_chg->wls_psy =
		power_supply_register(mt_chg->dev, &mt_chg->wls_psd, NULL);
	if (IS_ERR(mt_chg->wls_psy)) {
		dev_err(mt_chg->dev,
			"Failed to register power supply wirless: %ld\n",
...
int mt_power_supply_type_check(void)
{
	...
	chg_psy =
		devm_power_supply_get_by_phandle(&pinfo->pdev->dev, "charger");
	if (IS_ERR_OR_NULL(chg_psy)) {
		chr_err("%s Couldn't get chg_psy\n", __func__);
	} else {
...
int oplus_chg_set_qc_config_forsvooc(void)
{
	...
	if (!chip->calling_on && chip->charger_volt < CHARGER_VOLT_NORMAL &&
	    chip->soc < HIGH_SOC && chip->temperature <= HIGH_TEMP &&
		...
		if (chip->charger_volt > CHARGER_VOLT_HV &&
			...
			msleep(400);
			printk(KERN_ERR "%s: charger voltage=%d", __func__,
			       chip->charger_volt);
			oplus_chg_unsuspend_charger();
			ret = 0;
		}
		printk(KERN_ERR "%s: qc9v svooc  default[%d]", __func__,
		       chip->batt_volt);
	}
...
bool oplus_mt_get_vbus_status(void)
{
	...
	if (g_oplus_chip->vbatt_num == 2 && !oplus_chg_is_wls_present() &&
	    tcpm_inquire_typec_attach_state(pinfo->tcpc) ==
		    TYPEC_ATTACHED_SRC) {
		return false;
	}
...
int oplus_chg_set_pd_config(void)
{
	...
	if (!chip->calling_on && chip->charger_volt < 6500 && chip->soc < 90 &&
		...
		if (ret != TCPM_SUCCESS) {
			printk(KERN_ERR
			       "%s: inquire current vbus_mv and ibus_ma fail\n",
			       __func__);
			return -EINVAL;
		}
		...
		if (chip->charger_volt > 7500 &&
			...
			if (ret != TCPM_SUCCESS) {
				printk(KERN_ERR
				       "%s: tcpm_dpm_pd_request fail\n",
				       __func__);
				return -EINVAL;
			}

			ret = tcpm_inquire_pd_contract(tcpc, &vbus_mv_t,
						       &ibus_ma_t);
			if (ret != TCPM_SUCCESS) {
				printk(KERN_ERR
				       "%s: inquire current vbus_mv and ibus_ma fail\n",
			...
			oplus_chg_unsuspend_charger();
			printk(KERN_ERR
			       "%s: PD Default vbus_mv[%d], ibus_ma[%d]\n",
			       __func__, vbus_mv_t, ibus_ma_t);
		}
	}
...
int oplus_chg_set_pps_config(int vbus_mv, int ibus_ma)
{
	...
	ret = tcpm_set_apdo_charging_policy(tcpc, DPM_CHARGING_POLICY_PPS,
					    vbus_mv, ibus_ma, NULL);
...
u32 oplus_chg_get_pps_status(void)
{
	...
	return (PD_PPS_SET_OUTPUT_MV(pps_status.output_mv) |
...
int oplus_chg_pps_get_max_cur(int vbus_mv)
{
	...
	if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
		for (i = 0; i < pinfo->srccap.nr; i++) {
			if (pinfo->srccap.min_mv[i] <= vbus_mv &&
					     ...
					     unsigned long val, void *v)
{
	...
	case OPLUS_CHG_EVENT_OFFLINE:
		...
		if (!strcmp(owner_ocm->desc->name, "wireless")) {
			pr_info("%s wls %s\n", __func__,
				val == OPLUS_CHG_EVENT_ONLINE ? "online" :
								"offline");
			oplus_chg_wake_update_work();
		}
		break;
...
static int oplus_chg_usb_init_mod(struct oplus_chg_chip *chip)
{
	...
	chip->chgic_mtk.oplus_info->usb_ocm = oplus_chg_mod_register(
		chip->dev, &oplus_chg_usb_mod_desc, &ocm_cfg);
	...
	rc = oplus_chg_reg_event_notifier(
		&chip->chgic_mtk.oplus_info->usb_event_nb);
	if (rc) {
...
static int oplus_chg_wls_get_input_curr(struct oplus_chg_ic_dev *ic_dev,
					int *curr)
{
	int rc = 0;
...
static int oplus_chg_wls_get_input_vol(struct oplus_chg_ic_dev *ic_dev,
				       int *vol)
{
	int rc = 0;
...
static int oplus_chg_wls_set_boost_vol(struct oplus_chg_ic_dev *ic_dev,
				       int vol_mv)
{
	int rc = 0;
...
static int oplus_chg_wls_set_aicl_enable(struct oplus_chg_ic_dev *ic_dev,
					 bool en)
{
	int rc = 0;
...
static int oplus_chg_wls_set_vindpm(struct oplus_chg_ic_dev *ic_dev,
				    int vindpm_mv)
{
	int rc = 0;
	int vol = 0;
...
static int oplus_mt6375_hardware_init(void)
{
	...
	if (get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT) {
		...
		oplus_mt6375_float_voltage_write(
			DEFAULT_POWER_OFF_FLOAT_VOLTAGE);
		msleep(100);
	}
...
int oplus_chg_set_pd_config_voocphy(void)
{
	...
	if (!chip->calling_on && !chip->camera_on &&
	    chip->charger_volt < CHARGER_VOLT_NORMAL && chip->soc < HIGH_SOC &&
	    chip->temperature <= HIGH_TEMP &&
	    chip->cool_down_force_5v == false &&
	    (chip->batt_volt < chip->limits.vbatt_pdqc_to_9v_thr)) {
		oplus_voocphy_set_pdqc_config();
...
struct oplus_chg_operations mtk6375_chg_ops = {
	...
	.enable_qc_detect =
		oplus_chg_enable_qc_detect, //for qc 9v2a //bringup 6895
...
static int cp_thermal_read_temp(struct thermal_zone_device *tzd, int *temp)
{
	...
	cp_temp = oplus_chg_get_main_battery_btb() *
		  100; /* thermal zone need 0.001 */
	*temp = cp_temp;
...
static int register_tz_thermal(struct mtk_charger *info)
{
	...
	info->cp_temp_tzd = thermal_zone_device_register(
		"charge_pump_temp", 0, 0, NULL, &cp_temp_tzd_ops, NULL, 0, 0);
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	rc = of_property_read_u32(oplus_chip->dev->of_node, "oplus,ic_type",
				  &ic_type);
	if (rc < 0) {
		pr_err("can't get ic type, rc=%d\n", rc);
	}

	rc = of_property_read_u32(oplus_chip->dev->of_node, "oplus,ic_index",
				  &ic_index);
	if (rc < 0) {
		pr_err("can't get ic index, rc=%d\n", rc);
	}

	info->ic_dev = devm_oplus_chg_ic_register(
		oplus_chip->dev, oplus_chip->dev->of_node->name, ic_index);
	if (!info->ic_dev) {
		rc = -ENODEV;
	...
	pinfo->chargeric_temp_chan =
		devm_iio_channel_get(oplus_chip->dev, "auxadc3-chargeric_temp");
	...
	pinfo->charger_id_chan =
		devm_iio_channel_get(oplus_chip->dev, "auxadc3-charger_id");
	...
	pinfo->usb_temp_v_l_chan =
		devm_iio_channel_get(oplus_chip->dev, "usb_temp_v_l");
	...
	pinfo->usb_temp_v_r_chan =
		devm_iio_channel_get(oplus_chip->dev, "usb_temp_v_r");
	...
	pinfo->usbcon_temp_chan =
		devm_iio_channel_get(oplus_chip->dev, "usbcon_temp");
	if (IS_ERR(pinfo->usbcon_temp_chan)) {
		chg_err("Couldn't get usbcon_temp_chan...\n");
		pinfo->usbcon_temp_chan = NULL;
	}
	...
	pinfo->batcon_temp_chan =
		devm_iio_channel_get(oplus_chip->dev, "batcon_temp");
	if (IS_ERR(pinfo->batcon_temp_chan)) {
		chg_err("Couldn't get batcon_temp_chan...\n");
		pinfo->batcon_temp_chan = NULL;
	}
	...
	pinfo->sub_batcon_temp_chan =
		devm_iio_channel_get(oplus_chip->dev, "sub_batcon_temp");
	if (IS_ERR(pinfo->sub_batcon_temp_chan)) {
		chg_err("Couldn't get sub_batcon_temp_chan...\n");
		pinfo->sub_batcon_temp_chan = NULL;
	}
	...
	if (oplus_ccdetect_support_check() == true) {
		...
		INIT_DELAYED_WORK(&usbtemp_recover_work,
				  oplus_usbtemp_recover_work);
		oplus_ccdetect_irq_register(oplus_chip);
		level = gpio_get_value(
			oplus_chip->chgic_mtk.oplus_info->ccdetect_gpio);
	...
	INIT_DELAYED_WORK(&pinfo->status_keep_clean_work,
			  oplus_chg_wls_status_keep_clean_work);
	pinfo->status_wake_lock =
		wakeup_source_register(&pinfo->pdev->dev, "status_wake_lock");
	...
	if (wd0_get_status_work.work.func) {
		schedule_delayed_work(
			&wd0_get_status_work,
			msecs_to_jiffies(WD0_GET_STATUS_DELAY_MS));
	}

	if (pinfo->tcpc &&
	    tcpm_inquire_typec_attach_state(pinfo->tcpc) == TYPEC_UNATTACHED) {
		if (oplus_chip->chgic_mtk.oplus_info->usb_ocm)
			oplus_chg_global_event(
				oplus_chip->chgic_mtk.oplus_info->usb_ocm,
	...
	oplus_mt6375_wired_charging_break(
		&oplus_chg_track_check_wired_charging_break);
