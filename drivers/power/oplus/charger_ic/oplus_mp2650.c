...
int mp2650_input_current_limit_write(int current_ma)
{
	...
	chg_debug("hw_sw_vol[%d %d] chg_vol [%d] pdqc_9v_voltage_adaptive %s\n",
		  ...
		  oplus_chip->pdqc_9v_voltage_adaptive == true ? "true" :
								 "false");
...
void mp2650_set_aicl_point(int vbatt)
{
	...
	if ((oplus_chip->pdqc_9v_voltage_adaptive) &&
		...
		chg_debug(
			"pdqc_hw_volpont_vol [%d] chg_vol %d hw_sw_vbat [%d %d %d]\n",
		...
		} else if (chip->hw_aicl_point ==
				   MP2762HW_AICL_POINT_5V_PHASE2 &&
...
int mp2650_registers_read_full(void)
{
	...
	reg_full = ((reg_full & REG13_MP2650_CHARGING_STATUS_MASK) ==
		    REG13_MP2650_CHARGING_STATUS_CHARGE_TERMINATION) ?
			   1 :
			   0;
	if (reg_full) {
...
int mp2650_set_mps_otg_current(void)
{
	...
	rc = mp2650_config_interface(REG07_MP2650_ADDRESS,
				     REG07_MP2650_OTG_CURRENT_LIMIT_1500MA,
				     REG07_MP2650_OTG_CURRENT_LIMIT_MASK);
	return 0;
}
...
struct oplus_chg_operations mp2650_chg_ops = {
	...
	.input_current_ctrl_by_vooc_write =
		mp2650_input_current_limit_ctrl_by_vooc_write,
...
static int mp2650_gpio_init(struct chip_mp2650 *chip)
{
	...
	chip->mps_otg_en_gpio =
		of_get_named_gpio(node, "qcom,mps_otg_en-gpio", 0);
