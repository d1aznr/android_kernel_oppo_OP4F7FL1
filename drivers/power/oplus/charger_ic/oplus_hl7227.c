...
__maybe_unused static int hl7227_read_byte_mask(struct oplus_hl7227 *chip,
...
__maybe_unused static int hl7227_write_byte_mask(struct oplus_hl7227 *chip,
						 u8 addr, u8 mask, u8 data)
{
	u8 temp;
	int rc;
...
static int hl7227_hardware_init(struct oplus_hl7227 *chip)
{
	...
	if (version == 0xa0) {
		...
		rc = hl7227_write_byte(chip, 0x0F,
				       0xC0); //#set thermal alarm to 135d
		if (rc < 0) {
...
static int hl7227_set_cp_en_gpio_enable(struct oplus_hl7227 *chip, bool en)
{
	...
	if (rc < 0)
		...
		pr_err("set value:%d, gpio_val:%d\n", en,
		       gpio_get_value(chip->cp_en_gpio));
...
static int hl7227_gpio_init(struct oplus_hl7227 *chip)
{
	...
	chip->cp_int_default =
		pinctrl_lookup_state(chip->pinctrl, "cp_int_default");
			       ...
			       const struct i2c_device_id *id)
{
	...
	pr_err("call !\n");
	chip = devm_kzalloc(&client->dev, sizeof(struct oplus_hl7227),
			    GFP_KERNEL);
	if (!chip) {
