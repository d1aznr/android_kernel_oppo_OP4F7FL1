...
enum cc_modes_type { MODE_DEFAULT = 0, MODE_UFP, MODE_DFP, MODE_DRP };
...
#define SMB5_CHAN_VOLT(_dname, chan)                                           \
	[PARAM(chan)] = SMB5_CHAN(_dname, PARAM(chan), IIO_VOLTAGE,            \
				  BIT(IIO_CHAN_INFO_PROCESSED))

#define SMB5_CHAN_CUR(_dname, chan)                                            \
	[PARAM(chan)] = SMB5_CHAN(_dname, PARAM(chan), IIO_CURRENT,            \
				  BIT(IIO_CHAN_INFO_PROCESSED))

#define SMB5_CHAN_RES(_dname, chan)                                            \
	[PARAM(chan)] = SMB5_CHAN(_dname, PARAM(chan), IIO_RESISTANCE,         \
				  BIT(IIO_CHAN_INFO_PROCESSED))

#define SMB5_CHAN_TEMP(_dname, chan)                                           \
	[PARAM(chan)] = SMB5_CHAN(_dname, PARAM(chan), IIO_TEMP,               \
				  BIT(IIO_CHAN_INFO_PROCESSED))

#define SMB5_CHAN_POWER(_dname, chan)                                          \
	[PARAM(chan)] = SMB5_CHAN(_dname, PARAM(chan), IIO_POWER,              \
				  BIT(IIO_CHAN_INFO_PROCESSED))

#define SMB5_CHAN_CAP(_dname, chan)                                            \
	[PARAM(chan)] = SMB5_CHAN(_dname, PARAM(chan), IIO_CAPACITANCE,        \
				  BIT(IIO_CHAN_INFO_PROCESSED))

#define SMB5_CHAN_COUNT(_dname, chan)                                          \
	[PARAM(chan)] = SMB5_CHAN(_dname, PARAM(chan), IIO_COUNT,              \
				  BIT(IIO_CHAN_INFO_PROCESSED))

#define SMB5_CHAN_INDEX(_dname, chan)                                          \
	[PARAM(chan)] = SMB5_CHAN(_dname, PARAM(chan), IIO_INDEX,              \
				  BIT(IIO_CHAN_INFO_PROCESSED))

#define SMB5_CHAN_ACTIVITY(_dname, chan)                                       \
	[PARAM(chan)] = SMB5_CHAN(_dname, PARAM(chan), IIO_ACTIVITY,           \
				  BIT(IIO_CHAN_INFO_PROCESSED))
...
static const struct smb5_iio_prop_channels oplus_discrete_chans_pmic[] = {
	SMB5_CHAN_CUR("usb_pd_current_max", PD_CURRENT_MAX) SMB5_CHAN_INDEX("usb_typec_mode", TYPEC_MODE) SMB5_CHAN_INDEX(
		...
		TYPEC_POWER_ROLE) SMB5_CHAN_INDEX("usb_typec_cc_orientation",
						  TYPEC_CC_ORIENTATION) SMB5_CHAN_INDEX("usb_pd_active",
											PD_ACTIVE) SMB5_CHAN_CUR("usb_input_current_settled",
														 USB_INPUT_CURRENT_SETTLED)
		SMB5_CHAN_ACTIVITY("usb_pe_start", PE_START) SMB5_CHAN_CUR("usb_ctm_current_max", CTM_CURRENT_MAX) SMB5_CHAN_CUR("usb_hw_current_max", HW_CURRENT_MAX) SMB5_CHAN_INDEX(
			"usb_real_type",
			USB_REAL_TYPE) SMB5_CHAN_VOLT("usb_pd_voltage_max", PD_VOLTAGE_MAX) SMB5_CHAN_VOLT("usb_pd_voltage_min", PD_VOLTAGE_MIN) SMB5_CHAN_INDEX("usb_connector_type", CONNECTOR_TYPE)
			SMB5_CHAN_INDEX("usb_connector_health", CONNECTOR_HEALTH) SMB5_CHAN_VOLT("usb_voltage_max_limit", VOLTAGE_MAX_LIMIT) SMB5_CHAN_INDEX("usb_smb_en_mode", SMB_EN_MODE) SMB5_CHAN_INDEX(
				"usb_smb_en_reason", SMB_EN_REASON) SMB5_CHAN_INDEX("usb_adapter_cc_mode",
										    ADAPTER_CC_MODE) SMB5_CHAN_INDEX("usb_moisture_detected",
														     MOISTURE_DETECTED) SMB5_CHAN_INDEX("usb_moisture_detection_en",
																			MOISTURE_DETECTION_EN)
				SMB5_CHAN_INDEX("usb_hvdcp_opti_allowed", HVDCP_OPTI_ALLOWED) SMB5_CHAN_ACTIVITY("usb_qc_opti_disable", QC_OPTI_DISABLE) SMB5_CHAN_VOLT(
					"usb_voltage_vph",
					VOLTAGE_VPH) SMB5_CHAN_CUR("usb_therm_icl_limit",
								   THERM_ICL_LIMIT) SMB5_CHAN_INDEX("usb_skin_health",
												    SKIN_HEALTH)
					SMB5_CHAN_ACTIVITY("usb_apsd_rerun", APSD_RERUN) SMB5_CHAN_COUNT("usb_apsd_timeout", APSD_TIMEOUT) SMB5_CHAN_INDEX(
						"usb_charger_status",
						CHARGER_STATUS) SMB5_CHAN_VOLT("usb_input_voltage_settled", USB_INPUT_VOLTAGE_SETTLED)
						SMB5_CHAN_ACTIVITY("usb_typec_src_rp", TYPEC_SRC_RP) SMB5_CHAN_ACTIVITY("usb_pd_in_hard_reset", PD_IN_HARD_RESET) SMB5_CHAN_INDEX(
								...
								SMB5_CHAN_ACTIVITY("main_toggle_stat", TOGGLE_STAT) SMB5_CHAN_CUR(
									...
									MAIN_FCC_MAX) SMB5_CHAN_INDEX("main_irq_status",
												      IRQ_STATUS) SMB5_CHAN_ACTIVITY("main_force_main_fcc",
																     FORCE_MAIN_FCC) SMB5_CHAN_ACTIVITY("main_force_main_icl", FORCE_MAIN_ICL)
									SMB5_CHAN_INDEX("main_comp_clamp_level", COMP_CLAMP_LEVEL) SMB5_CHAN_TEMP(
										...
										SMB5_CHAN_CUR("main_current_max", CURRENT_MAX) SMB5_CHAN_INDEX(
											"main_health",
											HEALTH) SMB5_CHAN_VOLT("dc_input_voltage_regulation", INPUT_VOLTAGE_REGULATION)
											SMB5_CHAN_INDEX("dc_real_type", DC_REAL_TYPE) SMB5_CHAN_ACTIVITY(
												"dc_reset",
												DC_RESET) SMB5_CHAN_ACTIVITY("dc_aicl_done",
															     AICL_DONE) SMB5_CHAN_TEMP("battery_charger_temp", CHARGER_TEMP)
												SMB5_CHAN_TEMP("battery_charger_temp_max", CHARGER_TEMP_MAX) SMB5_CHAN_CUR(
													"battery_input_current_limited",
													INPUT_CURRENT_LIMITED) SMB5_CHAN_ACTIVITY("battery_sw_jeita_enabled",
																		  SW_JEITA_ENABLED)
													SMB5_CHAN_ACTIVITY("battery_charge_done", CHARGE_DONE) SMB5_CHAN_ACTIVITY(
														"battery_parallel_disable",
														PARALLEL_DISABLE) SMB5_CHAN_ACTIVITY("battery_set_ship_mode",
																		     SET_SHIP_MODE)
														SMB5_CHAN_INDEX("battery_die_health", DIE_HEALTH) SMB5_CHAN_ACTIVITY(
															"battery_rerun_aicl",
															RERUN_AICL) SMB5_CHAN_COUNT("battery_dp_dm",
																		    DP_DM)
															SMB5_CHAN_ACTIVITY(
																"battery_recharge_soc",
																RECHARGE_SOC) SMB5_CHAN_ACTIVITY("battery_force_recharge",
																				 FORCE_RECHARGE)
																SMB5_CHAN_ACTIVITY(
