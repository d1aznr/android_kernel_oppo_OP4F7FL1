...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		schedule_delayed_work(&chg->opluschg_monitor_work,
...
static void opluschg_debug_func(struct work_struct *w)
{
	...
	if (charge_debug_mask & DEBUG_MASK_VOTER) {
		for (i = 0; i < chg->usb_icl_votable->num_clients; i++) {
			if (chg->usb_icl_votable->client_strs[i])
				chg_debug(
					...
					chg->usb_icl_votable->client_strs[i],
					chg->usb_icl_votable->votes[i].enabled,
		...
		for (i = 0; i < chg->fcc_votable->num_clients; i++) {
			if (chg->fcc_votable->client_strs[i])
				chg_debug("%s: %s:\t\t\ten=%d v=%d\n",
					  ...
					  chg->fcc_votable->client_strs[i],
					  chg->fcc_votable->votes[i].enabled,
		...
		for (i = 0; i < chg->fv_votable->num_clients; i++) {
			if (chg->fv_votable->client_strs[i])
				chg_debug("%s: %s:\t\t\ten=%d v=%d\n",
					  chg->fv_votable->name,
					  chg->fv_votable->client_strs[i],
					  chg->fv_votable->votes[i].enabled,
	...
	schedule_delayed_work(&opluschg_debug_work,
			      msecs_to_jiffies(charge_debug_delay > 1000 ?
						       charge_debug_delay :
						       1000));
}
...
static void opluschg_monitor_func(struct work_struct *work)
{
	...
	if ((chip->batt_volt > rechg_vol - 10) && chip->batt_full) {
		...
		goto rerun_work;
		   ...
		   opluschg_get_vbatt_full_vol_sw(chip) - 10) {
		counts = 0;
		goto rerun_work;
	}
	...
	if (counts >= (chip->batt_full ? 8 : 3)) { //because rechg counts=6
		...
		if (stat & PRE_TERM_BIT) {
			...
			printk(KERN_ERR
			       "opluschg_monitor_func: PRE_TERM_BIT is set[0x%x], clear it\n",
			       stat);
			rc = smblib_masked_write(chg, USBIN_CMD_IL_REG,
						 USBIN_SUSPEND_BIT, 1);
			if (rc < 0) {
				printk(KERN_ERR
				       "opluschg_monitor_func: Couldn't set USBIN_SUSPEND_BIT rc=%d\n",
				       rc);
				goto rerun_work;
			}
			msleep(50);
			...
			if (rc < 0) {
				printk(KERN_ERR
				       "opluschg_monitor_func: Couldn't clear USBIN_SUSPEND_BIT rc=%d\n",
				       rc);
				goto rerun_work;
			}
			msleep(10);
			...
			if (rc < 0) {
				printk(KERN_ERR
				       "opluschg_monitor_func: Couldn't set RESTART_AICL_BIT rc=%d\n",
				       rc);
				goto rerun_work;
			}
			printk(KERN_ERR
			       "opluschg_monitor_func: ichg[%d], fv[%d]\n",
...
static void opluschg_usbtemp_dischg(void)
{
	...
	pinctrl_select_state(g_oplus_chip->normalchg_gpio.pinctrl,
			     g_oplus_chip->normalchg_gpio.dischg_enable);
...
static int opluschg_usbtemp_monitor_main(void *data)
{
	...
	bool critical_rise1 = false, critical_rise2 = false,
	     dischg_enable = false, high_temp_version = false;
	...
	while (!kthread_should_stop()) {
		if (opluschg_usbtemp_condition() == true) {
			...
			for (index = 0; index < ADC_SAMPLE_COUNT; index++) {
				voltage1 += opluschg_read_adc(
					USB_TEMPERATURE1); /* PM7150A ADC */
				voltage2 += opluschg_read_adc(
			...
			resistance1 = voltage_convert_resistance(
				voltage1, PULL_UP_VOLTAGE_1800MV,
				PULL_UP_RESISTANCE_51KOHM);
			resistance2 = voltage_convert_resistance(
				voltage2, PULL_UP_VOLTAGE_1800MV,
				PULL_UP_RESISTANCE_51KOHM);

			resistance_convert_temperature(resistance1,
						       temperature1, index,
						       ntc_table_100K);
			resistance_convert_temperature(resistance2,
						       temperature2, index,
						       ntc_table_100K);

			add_temperature_list(temperature1, temperature2,
					     detect_interval, index);
			check_temperature_list(temperature_list1, total_time,
					       critical_rise1, index);
			check_temperature_list(temperature_list2, total_time,
					       critical_rise2, index);

			if (oplus_vooc_get_fastchg_started() == true)
				bat_temp =
					oplus_gauge_get_prev_batt_temperature();
			else
				bat_temp = oplus_gauge_get_batt_temperature();

			check_dischg_enable(temperature1, temperature2,
					    critical_rise1, critical_rise2,
					    bat_temp, dischg_enable);

			if (usbtemp_debug == 1)
				chg_debug("[%d %d][%d %d %d %d][%d %d %d %d]\n",
					  bat_temp, dischg_enable, voltage1,
					  resistance1, temperature1,
					  critical_rise1, voltage2, resistance2,
			...
			wait_event_interruptible(opluschg_usbtemp_wq,
						 opluschg_usbtemp_condition() ==
							 true);
			do_gettimeofday(&time2);
			if (time2.tv_sec - time1.tv_sec >= 15) {
				if (usbtemp_debug == 1)
					chg_debug(
						"clear_temperature_list %d %d\n",
...
{
	oplushg_usbtemp_kthread = kthread_run(opluschg_usbtemp_monitor_main, 0,
					      "usbtemp_kthread");
...
static int opluschg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	/* uart gpio */
	chip->normalchg_gpio.uart_bias_disable = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "uart_bias_disable");
	chip->normalchg_gpio.uart_pull_down = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "uart_pull_down");

	/* chargerid adc */
	chip->normalchg_gpio.chargerid_adc_default = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "chargerid_adc_default");
	...
	chg_debug(
		"usb_switch_1, dischg, ship_mode, shortc, ccdetect] = [%d, %d, %d, %d, %d]\n",
		chip->normalchg_gpio.chargerid_switch_gpio,
		chip->normalchg_gpio.dischg_gpio,
		chip->normalchg_gpio.ship_gpio,
		chip->normalchg_gpio.shortc_gpio, chg->ccdetect_gpio);
...
static int opluschg_read_adc(int channel)
{
	...
	case USB_TEMPERATURE1:
		rc = iio_read_channel_processed(chg->iio.usb_temp_chan1,
						&result);
		if (rc < 0)
			chg_err("fail to read usb_temp1 adc rc = %d\n", rc);
		break;
	case USB_TEMPERATURE2:
		rc = iio_read_channel_processed(chg->iio.usb_temp_chan2,
						&result);
		if (rc < 0)
...
static int opluschg_get_chargerid(void)
{
	...
	if (boot_with_console() == false) {
		mutex_lock(
			&g_oplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
	...
	if (boot_with_console() == false) {
		...
		pinctrl_select_state(
			g_oplus_chip->normalchg_gpio.pinctrl,
			g_oplus_chip->normalchg_gpio.uart_pull_down);
		mutex_unlock(
			&g_oplus_chip->pmic_spmi.smb5_chip->chg.pinctrl_mutex);
	}
...
static int opluschg_set_icl(int current_ma)
{
	...
	if (opluschg_is_suspend_status() && opluschg_is_usb_present() &&
	    opluschg_is_normal_mode()) {
		i = i - 1;
		goto aicl_suspend;
	}
	chg_vol = opluschg_read_adc(USBIN_VOLTAGE);
