...
int bq25882_set_vindpm_vol(int vol)
{
	...
	tmp = (vol - REG02_BQ25882_INPUT_VOLTAGE_LIMIT_OFFSET) /
	      REG02_BQ25882_INPUT_VOLTAGE_LIMIT_STEP;
	rc = bq25882_config_interface(
		REG02_BQ25882_ADDRESS,
		tmp << REG02_BQ25882_INPUT_VOLTAGE_LIMIT_SHIFT,
...
int bq25882_input_Ilimit_disable(void)
{
	...
	rc = bq25882_config_interface(REG01_BQ25882_ADDRESS,
				      REG01_BQ25882_EN_ILIM_DISABLE,
				      REG01_BQ25882_EN_ILIM_MASK);
...
int bq25882_input_current_limit_write(int current_ma)
{
	...
	for (i = ARRAY_SIZE(bq25882_usbin_input_current_limit) - 1; i >= 0;
	     i--) {
	...
	if (pre_icl_index < 2) {
		...
		for (i = pre_icl_index - 1; i > 0; i--) {
			switch (i) {
			case 1:
				bq25882_config_interface(
					REG03_BQ25882_ADDRESS,
					REG03_BQ25882_INPUT_CURRENT_LIMIT_900MA,
...
aicl_pre_step:
	chg_debug(
		...
		chg_vol, i, bq25882_usbin_input_current_limit[i],
		chip->sw_aicl_point);
	goto aicl_rerun;
aicl_end:
	chg_debug(
...
int bq25882_set_complete_charge_timeout(int val)
{
	...
	if (val == OVERTIME_AC) {
		val = REG05_BQ25882_EN_TIMER_ENABLE |
		      REG05_BQ25882_CHG_TIMER_8H;
	} else if (val == OVERTIME_USB) {
		val = REG05_BQ25882_EN_TIMER_ENABLE |
		      REG05_BQ25882_CHG_TIMER_12H;
	} else {
		val = REG05_BQ25882_EN_TIMER_DISABLE |
		      REG05_BQ25882_CHG_TIMER_8H;
	}

	rc = bq25882_config_interface(REG05_BQ25882_ADDRESS, val,
				      REG05_BQ25882_EN_TIMER_MASK |
					      REG05_BQ25882_CHG_TIMER_MASK);
	if (rc < 0) {
...
int bq25882_float_voltage_write(int vfloat_mv)
{
	...
	rc = bq25882_config_interface(REG00_BQ25882_ADDRESS,
...
int bq25882_set_prechg_current(int ipre_mA)
{
	...
	rc = bq25882_config_interface(REG04_BQ25882_ADDRESS,
				      tmp << REG04_BQ25882_IPRECHG_LIMIT_SHIFT,
...
int bq25882_set_termchg_current(int term_curr)
{
	...
	rc = bq25882_config_interface(REG04_BQ25882_ADDRESS,
				      tmp << REG04_BQ25882_ITERM_LIMIT_SHIFT,
...
int bq25882_set_rechg_voltage(int recharge_mv)
{
	...
	rc = bq25882_config_interface(REG06_BQ25882_ADDRESS,
				      tmp << REG06_BQ25882_VRECHG_SHIFT,
...
int bq25882_set_wdt_timer(int reg)
{
	...
	rc = bq25882_config_interface(REG05_BQ25882_ADDRESS, reg,
				      REG05_BQ25882_WATCHDOG_MASK);
	if (rc) {
...
int bq25882_set_chging_term_disable(void)
{
	...
	rc = bq25882_config_interface(REG05_BQ25882_ADDRESS,
				      REG05_BQ25882_EN_TERM_DISABLE,
				      REG05_BQ25882_EN_TERM_MASK);
	if (rc) {
...
int bq25882_kick_wdt(void)
{
	...
	rc = bq25882_config_interface(REG07_BQ25882_ADDRESS,
				      REG07_BQ25882_WD_RST_RESET,
				      REG07_BQ25882_WD_RST_MASK);
	if (rc) {
...
int bq25882_enable_charging(void)
{
	...
	rc = bq25882_config_interface(REG06_BQ25882_ADDRESS,
				      REG06_BQ25882_EN_CHG_ENABLE,
				      REG06_BQ25882_EN_CHG_MASK);
	if (rc < 0) {
...
int bq25882_reset_charger(void)
{
	...
	rc = bq25882_config_interface(REG25_BQ25882_ADDRESS,
				      REG25_BQ25882_RESET_DEFAULT,
				      REG25_BQ25882_RESET_MASK);
	if (rc < 0) {
...
int bq25882_otg_vlim_set(int vlim)
{
	...
	rc = bq25882_config_interface(REG09_BQ25882_ADDRESS, vlim,
				      REG09_BQ25882_OTG_VLIM_MASK);
	if (rc < 0) {
...
int bq25882_otg_ilim_set(int ilim)
{
	...
	rc = bq25882_config_interface(REG09_BQ25882_ADDRESS, ilim,
				      REG09_BQ25882_OTG_ILIM_MASK);
	if (rc < 0) {
...
int bq25882_otg_enable(void)
{
	...
	rc = bq25882_config_interface(REG06_BQ25882_ADDRESS,
				      REG06_BQ25882_EN_OTG_ENABLE,
				      REG06_BQ25882_EN_OTG_MASK);
	if (rc < 0) {
...
int bq25882_ico_disable(void)
{
	...
	rc = bq25882_config_interface(REG03_BQ25882_ADDRESS,
				      REG03_BQ25882_EN_ICO_DISABLE,
				      REG03_BQ25882_EN_ICO_MASK);
	if (rc < 0) {
...
void bq25882_dump_registers(void)
{
	...
	if (dump_count == DUMP_REG_LOG_CNT_30S) {
		dump_count = 0;
		for (addr = BQ25882_FIRST_REG; addr <= BQ25882_LAST_REG;
		     addr++) {
		...
		printk(KERN_ERR
		       ...
		       val_buf[13], val_buf[14], val_buf[15], val_buf[16],
		       val_buf[17], val_buf[18], val_buf[19], val_buf[20],
		       val_buf[21], val_buf[22], val_buf[23], val_buf[24],
		       val_buf[25]);
		printk(KERN_ERR
		       "bq25882[1A-25]: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x \n",
		       val_buf[26], val_buf[27], val_buf[28], val_buf[29],
		       val_buf[30], val_buf[31], val_buf[32], val_buf[33],
		       val_buf[34], val_buf[35], val_buf[36], val_buf[37]);
	}
	dump_count++;
}
				...
				const struct i2c_device_id *id)
{
	...
	chg_ic = devm_kzalloc(&client->dev, sizeof(struct chip_bq25882),
			      GFP_KERNEL);
	if (!chg_ic) {
