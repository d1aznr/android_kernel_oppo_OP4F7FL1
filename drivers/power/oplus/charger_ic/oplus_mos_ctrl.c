...
static int oplus_mos_ctrl_parse_dt(struct oplus_mos_dev *dev)
{
	...
	rc = of_property_read_u32(
		node, "qcom,parallel_vbat_gap_abnormal",
...
static int oplus_mos_ctrl_set_enable(int enable)
{
	...
	if (!g_mos_dev->mos_en_pinctrl || !g_mos_dev->mos_en_gpio_active ||
	...
	if (enable)
		pinctrl_select_state(g_mos_dev->mos_en_pinctrl,
	...
	chg_err("enable %d, level %d\n", enable,
		gpio_get_value(g_mos_dev->mos_en_gpio));
...
static int oplus_mos_ctrl_probe(struct platform_device *pdev)
{
	...
	if (batt_authenticate != true || sub_batt_authenticate != true) {
		...
		chg_err("gauge i2c error! error_status:%d\n", batt_authenticate,
			sub_batt_authenticate, error_status);
	}
	...
	g_mos_dev->switching_chip = devm_kzalloc(
		&pdev->dev, sizeof(struct oplus_switch_chip), GFP_KERNEL);
	...
	if (abs(batt_volt - sub_batt_volt) >
	    g_mos_dev->switching_chip->parallel_vbat_gap_abnormal) {
