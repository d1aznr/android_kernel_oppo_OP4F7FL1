...
extern enum alarmtimer_restart usbtemp_alarm_timer_func(struct alarm *alarm,
							ktime_t now);
					     ...
					     struct ntc_temp *ntc_param)
{
	...
	if (res >= ntc_param->pst_temp_table[0].temperature_r) {
		...
		tap_value = (((res - res2) * tmp1) + ((res1 - res) * tmp2)) *
			    OPLUS_TCHG_01C_PRECISION / (res1 - res2);
	}
...
#define DEFAULT_CHARGERIC_I_TAP_OVER_CRITICAL_LOW                              \
	DEFAULT_BATCON_I_TAP_OVER_CRITICAL_LOW
...
static int get_chargeric_temp(void)
{
	...
	if (!is_param_init) {
		...
		ntc_param.i_tap_over_critical_low =
			DEFAULT_CHARGERIC_I_TAP_OVER_CRITICAL_LOW;
		ntc_param.i_rap_pull_up_r = g_rap_pull_up_r0;
		ntc_param.i_rap_pull_up_voltage = DEFAULT_I_RAP_UP_VOLT;
		ntc_param.i_tap_min = DEFAULT_I_TAP_MIN;
		ntc_param.i_tap_max = DEFAULT_I_TAP_MAX;
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	if (chip) {
		chip->normalchg_gpio.chargerid_switch_gpio = of_get_named_gpio(
			node, "qcom,chargerid_switch-gpio", 0);
		if (chip->normalchg_gpio.chargerid_switch_gpio <= 0) {
			...
			if (gpio_is_valid(chip->normalchg_gpio
...
int oplus_mt6765_pd_setup_forsvooc(void)
{
	...
	if (!chip->calling_on && !chip->camera_on &&
		...
		if (chip->charger_volt > PD_CHG_5V3A_VOLT_THRESHOLD &&
		    (chip->calling_on || chip->soc >= PD_CHG_SOC_THRESHOLD ||
		     chip->camera_on ||
		...
		printk(KERN_ERR "%s: pd9v svooc  default[%d]", __func__,
		       chip->batt_volt);
	}
...
int mtk_charger_enable_high_voltage_charging(struct charger_consumer *consumer,
					     bool en)
{
...
int mtk_charger_enable_power_path(struct charger_consumer *consumer, int idx,
				  bool en)
{
	int ret = 0;
	bool is_en = true;
...
int mtk_charger_enable_charging(struct charger_consumer *consumer, int idx,
				bool en)
{
	struct mtk_charger *info = consumer->cm;
	int ret = 0;
	...
	defined(CONFIG_OPLUS_CHARGER_MT6370_TYPEC)
	return -EBUSY;
#endif
...
int mtk_charger_set_charging_current_limit(struct charger_consumer *consumer,
...
bool oplus_chg_get_shortc_hw_gpio_status(void)
{
	...
	if (oplus_shortc_check_is_gpio(g_oplus_chip) == true) {
		shortc_hw_status = !!(gpio_get_value(
...
int oplus_chg_shortc_hw_parse_dt(struct oplus_chg_chip *chip)
{
	...
	if (chip) {
		chip->normalchg_gpio.shortc_gpio =
			of_get_named_gpio(node, "qcom,shortc_gpio", 0);
		if (chip->normalchg_gpio.shortc_gpio <= 0) {
			...
			if (oplus_shortc_check_is_gpio(chip) == true) {
				chg_debug(
					"This project use gpio for shortc hw check\n");
				rc = gpio_request(
					chip->normalchg_gpio.shortc_gpio,
					"shortc_gpio");
				if (rc) {
...
static void mtk_charger_external_power_changed(struct power_supply *psy)
{
	...
	pr_notice("%s event, name:%s online:%d usb_type:%d type:%d vbus:%d\n",
		  ...
		  type_prop.intval, get_vbus(info));
...
int oplus_usb_switch_gpio_gpio_init(void)
{
	...
	g_oplus_chip->normalchg_gpio.pinctrl =
		devm_pinctrl_get(g_oplus_chip->dev);
	if (IS_ERR_OR_NULL(g_oplus_chip->normalchg_gpio.pinctrl)) {
		chg_err("get normalchg_gpio.chargerid_switch_gpio pinctrl falil\n");
		return -EINVAL;
	}
	g_oplus_chip->normalchg_gpio.charger_gpio_as_output1 =
		pinctrl_lookup_state(g_oplus_chip->normalchg_gpio.pinctrl,
...
struct USB_PORT_TEMPERATURE Usb_Port_Temperature_Table[USB_NTC_TABLE_SIZE] = {
	...
	{ -400, 4397119 }, { -350, 3088599 }, { -300, 2197225 },
	{ -250, 1581881 }, { -200, 1151037 }, { -150, 846579 },
	{ -100, 628988 },  { -50, 471632 },   { 0, 357012 },
	{ 50, 272500 },	   { 100, 209710 },   { 150, 162651 },
	{ 200, 127080 },   { 250, 100000 }, /* 100K */
	{ 300, 79222 },	   { 350, 63167 },    { 400, 50677 },
	{ 410, 48528 },	   { 420, 46482 },    { 430, 44533 },
	{ 440, 42675 },	   { 450, 40904 },    { 460, 39213 },
	{ 470, 37601 },	   { 480, 36063 },    { 490, 34595 },
	{ 500, 33195 },	   { 510, 31859 },    { 520, 30584 },
	{ 530, 29366 },	   { 540, 28203 },    { 550, 27091 },
	{ 560, 26028 },	   { 570, 25013 },    { 580, 24042 },
	{ 590, 23113 },	   { 600, 22224 },    { 610, 21374 },
	{ 620, 20560 },	   { 630, 19782 },    { 640, 19036 },
	{ 650, 18322 },	   { 660, 17640 },    { 670, 16986 },
	{ 680, 16360 },	   { 690, 15759 },    { 700, 15184 },
	{ 710, 14631 },	   { 720, 14100 },    { 730, 13591 },
	{ 740, 13103 },	   { 750, 12635 },    { 760, 12187 },
	{ 770, 11756 },	   { 780, 11343 },    { 790, 10946 },
	{ 800, 10565 },	   { 810, 10199 },    { 820, 9847 },
	{ 830, 9509 },	   { 840, 9184 },     { 850, 8872 },
	{ 860, 8572 },	   { 870, 8283 },     { 880, 8005 },
	{ 890, 7738 },	   { 900, 7481 },     { 950, 6337 },
	{ 1000, 5384 },	   { 1050, 4594 },    { 1100, 3934 },
	{ 1150, 3380 },	   { 1200, 2916 },    { 1250, 2522 }
};
...
int usb_port_volt_to_temp(int volt)
{
	...
	if (usb_r >= Usb_Port_Temperature_Table[0].temperature_r) {
		usb_temp_value = -40;
	} else if (usb_r <= Usb_Port_Temperature_Table[USB_NTC_TABLE_SIZE - 1]
				    .temperature_r) {
		usb_temp_value = 125;
	} else {
		...
		for (i = 0; i < USB_NTC_TABLE_SIZE; i++) {
			if (usb_r >=
			    Usb_Port_Temperature_Table[i].temperature_r) {
				res2 = Usb_Port_Temperature_Table[i]
					       .temperature_r;
				tmp2 = Usb_Port_Temperature_Table[i].temp;
				break;
			}
			{ /* hidden else */
				res1 = Usb_Port_Temperature_Table[i]
					       .temperature_r;
		...
		usb_temp_value =
			(((usb_r - res2) * tmp1) + ((res1 - usb_r) * tmp2)) /
			(res1 - res2);
	}

	USBTEMP_LOG_PRINT("[%s] %d %d %d %d %d %d\n", __func__, res1, res2,
			  usb_r, tmp1, tmp2, usb_temp_value);
				   ...
				   const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		if (pinfo->data.parallel_vbus)
			pinfo->chg_data[CHG2_SETTING]
				.thermal_input_current_limit = reg;
		pr_debug("[Battery] %s: %x\n", __func__,
			 pinfo->chg_data[CHG1_SETTING]
				  ...
				  const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		pr_debug("[Battery] %s: %x\n", __func__,
			 pinfo->chg_data[CHG1_SETTING]
				 .thermal_charging_current_limit);
	}
	return size;
}
...
static enum alarmtimer_restart recover_charge_hrtimer_func(struct alarm *alarm,
							   ktime_t now)
{
...
{
	alarm_init(&usbotp_recover_timer, ALARM_BOOTTIME,
		   recover_charge_hrtimer_func);
...
void oplus_wake_up_usbtemp_thread(void)
{
	if (oplus_usbtemp_check_is_support() == true) {
		...
		chg_debug("wake_up_usbtemp_thread, vbus:%d, otg:%d",
...
static int oplus_chg_parse_charger_dt_2nd_override(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,iterm_ma_2nd",
				  &chip->limits.iterm_ma);
	if (rc < 0) {
		chip->limits.iterm_ma = 300;
	}

	rc = of_property_read_u32(node, "qcom,recharge-mv_2nd",
	...
	rc = of_property_read_u32(
		node, "qcom,little_cold_vfloat_over_sw_limit_2nd",
	...
	rc = of_property_read_u32(
		node, "qcom,little_cool_vfloat_over_sw_limit_2nd",
	...
	rc = of_property_read_u32(node,
				  "qcom,default_temp_normal_vfloat_mv_2nd",
	...
	rc = of_property_read_u32(
		node, "qcom,default_normal_vfloat_over_sw_limit_2nd",
...
int mtk_pdc_get_max_watt(struct mtk_charger *info)
{
	...
	if (info->pdc.pdc_max_watt_setting != -1)
		...
		if (info->chg_data[CHG1_SETTING]
			    .thermal_charging_current_limit != -1)
			charging_current =
	...
	chg_err("[%s]watt:%d:%d vbat:%d c:%d=>\n", __func__,
		info->pdc.pdc_max_watt_setting, info->pdc.pdc_max_watt, vbat,
		charging_current);
...
void pmic_set_register_value(struct regmap *map, unsigned int addr,
...
static ssize_t gsm_pmic_pwm_node_write(struct file *file,
				       ...
				       loff_t *pos)
{
	...
	if (strncmp(upmu_vs1_mode, "1", 1) == 0) {
		pmic_set_register_value(pmic_chip->regmap, MT6357_VS1_ANA_CON2,
...
static ssize_t gsm_pmic_pwm_node_read(struct file *file, char __user *buf,
...
{
	if (!proc_create("gsm_pmic_pwm", S_IWUSR | S_IWGRP | S_IWOTH, NULL,
			 &gsm_pmic_pwm_ctrl)) {
...
static int oplus_charger_probe(struct platform_device *pdev)
{
	...
	chg_err("%s: starts\n", __func__);
	oplus_chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_chg_chip),
				  GFP_KERNEL);
	if (!oplus_chip) {
	...
	if (oplus_chip->vbatt_num == 1) {
		...
		if (oplus_chip->is_double_charger_support) {
			...
			case (1 << BQ2589X | 1 << BQ2560X):
				...
				oplus_chip->chg_ops = &oplus_chg_bq2589x_ops;
#endif
				oplus_chip->sub_chg_ops =
					&oplus_chg_bq2560x_ops;
				break;
			default:
				oplus_chip->chg_ops = &oplus_chg_default_ops;
				oplus_chip->sub_chg_ops =
					&oplus_chg_default_ops;
			}
		} else {
			switch (charger_ic__det_flag) {
			case (1 << BQ2560X):
	...
	if (oplus_usbtemp_check_is_support() == true) {
		...
		if (alarmtimer_get_rtcdev()) {
			alarm_init(&oplus_chip->usbtemp_alarm_timer,
				   ALARM_BOOTTIME, usbtemp_alarm_timer_func);
		} else {
	...
	if (IS_ERR(oplus_chip->batt_psy) == 0) {
		ret = device_create_file(
			&oplus_chip->batt_psy->dev,
			&dev_attr_StopCharging_Test); /* stop charging */
		ret = device_create_file(&oplus_chip->batt_psy->dev,
