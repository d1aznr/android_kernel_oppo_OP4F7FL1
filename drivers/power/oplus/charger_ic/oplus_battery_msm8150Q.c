					     ...
					     int max_allowed_uv)
{
	...
	if (chip->vbatt_num == 2) {
		...
		if (rc < 0) {
			smblib_err(
				chg,
				"Couldn't configure adapter allowance rc=%d\n",
				rc);
		}
	}
#else
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if (chip->vbatt_num == 2) {
			if (g_oplus_chip && g_oplus_chip->ui_otg_switch !=
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	if (g_oplus_chip != NULL) {
		if (g_oplus_chip->vbatt_num == 2) {
			if (apsd_result->bit ==
			    (DCP_CHARGER_BIT | QC_2P0_BIT)) {
				chg->hvdcp_detect_ok = true;
				chg->hvdcp_detect_time =
					cpu_clock(smp_processor_id()) / 1000000;
				printk(KERN_ERR
				       " HVDCP2 detect: %d, the detect time: %lu\n",
				       chg->hvdcp_detect_ok,
				       chg->hvdcp_detect_time);
			}
		}
	...
	if (chg->pr_swap_in_progress) {
		...
		if (oplus_ccdetect_check_is_gpio(chip) == true) {
			...
			if (chg->pd_hard_reset != 0) {
				u8 typec_stat = 0;
				rc = smblib_read(chg, TYPE_C_MODE_CFG_REG,
						 &typec_stat);
				if (rc < 0) {
					printk(KERN_ERR
					       "[OPLUS_CHG][%s]:  Couldn't read 0x1368 rc=%d\n",
					       __func__, rc);
				} else {
					printk(KERN_ERR
					       "[OPLUS_CHG][%s]: reg0x1368[0x%x]\n",
					       __func__, typec_stat);
				}
				if (level != 1) {
...
int oplus_usbtemp_adc_gpio12_init(struct oplus_chg_chip *chip)
{
	...
	chg->usbtemp_gpio12_default = pinctrl_lookup_state(
		chg->usbtemp_gpio1_adc_pinctrl, "gpio12_adc_default");
	if (IS_ERR_OR_NULL(chg->usbtemp_gpio12_default)) {
		chg_err("get usbtemp_gpio12_default fail\n");
		return -EINVAL;
	}

	pinctrl_select_state(chg->usbtemp_gpio12_adc_pinctrl,
			     chg->usbtemp_gpio12_default);
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if ((chip->usbtemp_volt_l > USB_50C_VOLT) &&
		    (chip->usbtemp_volt_r >
		     USB_50C_VOLT)) //get vbus when usbtemp < 50C
		...
		if ((chip->usbtemp_volt_l > USB_50C_VOLT) &&
		    (chip->usbtemp_volt_r > USB_50C_VOLT) &&
		...
		if (level == 0) {
			if (((chip->usb_temp_l >= USB_57C) &&
				...
				count = 0;
				msleep(delay);
			} else if (((chip->usb_temp_l -
				     chip->temperature / 10) > 10) ||
				   ((chip->usb_temp_r -
				     chip->temperature / 10) > 10)) {
				if (dischg_flag == false) {
					if (count == 0) {
			...
			if (chip->vbatt_num == 2) {
				...
				wait_event_interruptible(
					oplus_usbtemp_wq,
					gpio_get_value(chg->ccdetect_gpio) ==
						0);
			}
		}
		//chg_err("==================usbtemp_volt[%d], chip->usb_temp[%d]\n", chip->usbtemp_volt,chip->usb_temp);
...
static int smb5_chg_config_init(struct smb5 *chip)
{
	...
	revid_dev_node =
		of_parse_phandle(chip->chg.dev->of_node, "qcom,pmic-revid", 0);
	if (!revid_dev_node) {
	...
	case PMI632_SUBTYPE:
		...
		chg->wa_flags |=
			WEAK_ADAPTER_WA | USBIN_OV_WA | CHG_TERMINATION_WA;
			      ...
			      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		rc = smblib_get_prop_from_bms(
			chg, POWER_SUPPLY_PROP_CURRENT_NOW, val);
		if (!rc)
			val->intval *= (-1);
		break;
#endif
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		...
		if (g_oplus_chip) {
			val->intval = g_oplus_chip->ui_soc *
				      g_oplus_chip->batt_capacity_mah * 1000 /
				      100;
		}
#endif
		break;
				...
				const unsigned int *val)
{
	...
	case DUAL_ROLE_PROP_MODE:
		if (chg->pr_swap_in_progress) {
			pr_debug(
				"Already in mode transition. Skipping request\n");
			mutex_unlock(&chg->dr_lock);
			return 0;
		}
		...
		case DUAL_ROLE_PROP_MODE_UFP:
			if (chg->typec_mode >=
				...
				rc = smblib_force_dr_mode(
					chg, DUAL_ROLE_PROP_MODE_UFP);
				if (rc < 0) {
					chg->pr_swap_in_progress = false;
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if (chip->dt.wd_snarl_time_cfg == -EINVAL)
		...
		val |= (chip->dt.wd_snarl_time_cfg
			<< SNARL_WDOG_TIMEOUT_SHIFT) &
		       SNARL_WDOG_TIMEOUT_MASK;
