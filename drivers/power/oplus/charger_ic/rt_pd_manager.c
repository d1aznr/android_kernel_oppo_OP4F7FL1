...
static const char *const iio_channel_map[] = {
	"pd_active",	    "pd_usb_suspend_supported",
	"pd_in_hard_reset", "pd_current_max",
	"pd_voltage_min",   "pd_voltage_max",
	"real_type",	    "pd_sdp",
};
...
bool oplus_pd_without_usb(void)
{
	...
	if (!tcpm_inquire_pd_connected(g_rpmd->tcpc))
		return false;
	return (tcpm_inquire_dpm_flags(g_rpmd->tcpc) &
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	rpmd->iio_channels =
		devm_kcalloc(rpmd->dev, POWER_SUPPLY_IIO_PROP_MAX,
