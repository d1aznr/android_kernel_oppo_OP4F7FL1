			    ...
			    struct smb_chg_param *param, int val_u)
{
	...
	if (value_new != value_old) {
		chg_debug(
			"name = %s, value_new = %d, value_old = %d, input_para = %d\n",
...
const struct apsd_result *smblib_update_usb_type(struct smb_charger *chg)
{
	...
	/* Yichun.Chen  PSW.BSP.CHG  2018-05-25  debug */
	chg_debug(
		"[plugin]***********  type_name = %s, real_type = %d **************\n",
...
int smblib_vbus_regulator_enable(struct regulator_dev *rdev)
{
	...
	if (chip && oplus_usbid_check_is_gpio(chip) &&
...
static int smblib_get_prop_ufp_mode(struct smb_charger *chg)
{
	...
	if (stat &
		...
		if (rc < 0)
			smblib_err(
				chg,
				"Couldn't config TYPE_C_DEBUG_ACCESS_SINK_REG rc=%d\n",
				rc);
	}
#endif
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		schedule_delayed_work(&chg->check_camera_and_hotspot_work,
		...
		if (is_client_vote_enabled(chg->usb_icl_votable,
					   THERMAL_USER_VOTER) == true) {
			rc = vote(chg->usb_icl_votable, THERMAL_USER_VOTER,
				  false, 500000);
		}
	}
#endif
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, arb_monitor_work.work);
	int rc = 0;
	...
	rc = power_supply_get_property(
		chg->usb_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_NOW, &pval);
	if (rc < 0) {
	...
	smblib_dbg(chg, PR_MISC,
		   "it's arb monitor work, ibus_current:%d, vbus_voltage:%d\n",
...
/* PD session is ongoing, ignore BC1.2 and QC detection */
	...
	if ((chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB) &&
		...
		if (rc < 0) {
			chg_debug(
				"couldn't read APSD_RESULT_STATUS_REG, rc = %d\n",
				rc);
			return IRQ_HANDLED;
		}
...
static void smblib_uusb_otg_work(struct work_struct *work)
{
	...
	chg->usb_psy->desc->get_property(chg->usb_psy,
...
static void jeita_update_work(struct work_struct *work)
{
	...
	if (IS_ERR(pnode)) {
		...
		pnode = of_batterydata_get_best_profile(
			batt_node, NONSTD_BAT_ID_OHM / 1000, NULL);
		if (IS_ERR(pnode)) {
			rc = PTR_ERR(pnode);
	...
	if (bat_thermal_1k == true) {
		rc = of_property_read_u32_array(
			pnode, "qcom,jeita-hard-thresholds_oplus",
			jeita_hard_thresholds, 2);
	} else {
		rc = of_property_read_u32_array(
			pnode, "qcom,jeita-hard-thresholds_oplus",
	...
	if (bat_thermal_1k == true) {
		rc = of_property_read_u32_array(
			pnode, "qcom,jeita-soft-thresholds_oplus",
			chg->jeita_soft_thlds, 2);
	} else {
		rc = of_property_read_u32_array(
			pnode, "qcom,jeita-soft-thresholds_oplus",
...
static void oplus_check_camera_and_hotspot_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(
		work, struct smb_charger, check_camera_and_hotspot_work.work);
	struct oplus_chg_chip *chip = g_oplus_chip;
	...
	if ((chg->ctrl_by_camera) && (chg->ctrl_by_hotspot) &&
	    (chip->tbatt_status >= BATTERY_STATUS__WARM_TEMP)) {
		if (is_client_vote_enabled(chg->usb_icl_votable,
					   THERMAL_USER_VOTER) == false) {
			rc = vote(chg->usb_icl_votable, THERMAL_USER_VOTER,
				  true, 500000);
			chg_err("Batt warm, camera and hotspot on, set usb_icl 500mA\n");
		}
	} else {
		if ((!chg->ctrl_by_camera) ||
		    (chip->tbatt_status < BATTERY_STATUS__WARM_TEMP)) {
			if (is_client_vote_enabled(chg->usb_icl_votable,
						   THERMAL_USER_VOTER) ==
			    true) {
				rc = vote(chg->usb_icl_votable,
					  THERMAL_USER_VOTER, false, 500000);
...
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
	INIT_DELAYED_WORK(&chg->check_camera_and_hotspot_work,
			  oplus_check_camera_and_hotspot_work);
...
static int smb5_parse_dt(struct smb5 *chip)
{
	...
	if (g_oplus_chip) {
		...
		if (g_oplus_chip->normalchg_gpio.chargerid_switch_gpio <= 0) {
			...
			rc = gpio_request(g_oplus_chip->normalchg_gpio
						  .chargerid_switch_gpio,
			      ...
			      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SHORT_C_BATT_CV_STATUS:
		if (g_oplus_chip) {
			val->intval = (int)oplus_short_c_batt_get_cv_status(
				g_oplus_chip);
		}
		break;
#endif
	...
	case POWER_SUPPLY_PROP_SHORT_C_HW_FEATURE:
		if (g_oplus_chip) {
			val->intval =
				g_oplus_chip->short_c_batt.is_feature_hw_on;
		}
		break;
	...
	case POWER_SUPPLY_PROP_SHORT_C_HW_STATUS:
		if (g_oplus_chip) {
			val->intval =
				g_oplus_chip->short_c_batt.shortc_gpio_status;
		}
		break;
#endif
	...
	case POWER_SUPPLY_PROP_SHORT_C_BATT_UPDATE_CHANGE:
		if (g_oplus_chip) {
			chg_debug(
				...
				val->intval);
			oplus_short_c_batt_update_change(g_oplus_chip,
							 val->intval);
...
static void otg_enable_pmic_id_value(void)
{
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: Couldn't clear 0x1544[0] rc=%d\n",
		       __func__, rc);
	}
	...
	if (rc < 0) {
		...
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: reg0x1544[0x%x], bit[2:0]=0(DRP)\n",
		       __func__, stat);
	}
...
static void otg_disable_pmic_id_value(void)
{
	...
	rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
				 TYPEC_POWER_ROLE_CMD_MASK,
				 EN_SNK_ONLY_BIT); //bit[2:0]=0x2
	if (rc < 0) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: Couldn't set 0x1544[2] rc=%d\n",
		       __func__, rc);
	}
	...
	if (rc < 0) {
		...
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: reg0x1544[0x%x], bit[2:0]=4(UFP)\n",
		       __func__, stat);
	}
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if (0) { /* when 1k BAT_THERMAL resistance    */
		...
		smblib_write(chg, 0x1095,
			     0xF2); /*      soft hot threshold   90C     */
		smblib_write(chg, 0x1096, 0x5D);
		smblib_write(chg, 0x1097,
			     0xFC); /*      soft cold threshold -30C     */
		smblib_write(chg, 0x1098, 0x07);
		smblib_write(chg, 0x1099,
			     0x67); /*      hard hot threshold   95C     */
		smblib_write(chg, 0x109A, 0x59);
		smblib_write(chg, 0x109B,
			     0x68); /*      hard cold threshold -35C     */
	} else { /* when 5p1k BAT_THERMAL resistance  */
		smblib_write(chg, 0x1094, 0x13);
		smblib_write(chg, 0x1095,
			     0xBF); /*      soft hot threshold   90C     */
		smblib_write(chg, 0x1096, 0x5E);
		smblib_write(chg, 0x1097,
			     0x68); /*      soft cold threshold -30C     */
		smblib_write(chg, 0x1098, 0x13);
		smblib_write(chg, 0x1099,
			     0x63); /*      hard hot threshold   95C     */
		smblib_write(chg, 0x109A, 0x5A);
		smblib_write(chg, 0x109B,
			     0x12); /*      hard cold threshold -35C     */
	}
#endif
...
static ssize_t show_regs_mask_write(struct file *file, const char __user *buff,
...
{
	if (!proc_create("show_regs_mask", S_IWUSR | S_IWGRP | S_IWOTH, NULL,
			 &show_regs_mask_fops)) {
...
static ssize_t show_voter_mask_write(struct file *file, const char __user *buff,
				     size_t count, loff_t *ppos)
{
	char mask[16];
...
{
	if (!proc_create("show_voter_mask", S_IWUSR | S_IWGRP | S_IWOTH, NULL,
			 &show_voter_mask_fops)) {
...
static int smbchg_set_fastchg_current_raw(int current_ma)
{
	...
	rc = vote(g_smb_chip->fcc_votable, DEFAULT_VOTER, true,
		  current_ma * 1000);
	if (rc < 0) {
...
static void oplus_chg_clear_suspend(void)
{
	...
	rc = smblib_masked_write(g_smb_chip, USBIN_CMD_IL_REG,
				 USBIN_SUSPEND_BIT, 1);
	if (rc < 0) {
...
*/
	...
	if (get_client_vote(g_smb_chip->usb_icl_votable, BOOST_BACK_VOTER) ==
		    0 &&
	...
	i = 1;
	rc = vote(g_smb_chip->usb_icl_votable, USB_PSY_VOTER, true,
		  usb_icl[i] * 1000);
	msleep(90);
...
aicl_end:
	rc = vote(g_smb_chip->usb_icl_votable, USB_PSY_VOTER, true,
		  usb_icl[i] * 1000);
	chg_debug(
		"usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_end rc = %d\n",
...
static int smbchg_float_voltage_set(int vfloat_mv)
{
	...
	rc = vote(g_smb_chip->fv_votable, BATT_PROFILE_VOTER, true,
		  vfloat_mv * 1000);
	if (rc < 0) {
...
static int smbchg_term_current_set(int term_current)
{
	...
	rc = smblib_masked_write(g_smb_chip,
...
static int oplus_usbid_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chip->normalchg_gpio.usbid_active = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "usbid_active");
	if (IS_ERR_OR_NULL(chip->normalchg_gpio.usbid_active)) {
		chg_err("get usbid_active fail\n");
		return -EINVAL;
	}

	chip->normalchg_gpio.usbid_sleep = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "usbid_sleep");
...
static void oplus_usbid_irq_init(struct oplus_chg_chip *chip)
{
	chip->normalchg_gpio.usbid_irq =
		gpio_to_irq(chip->normalchg_gpio.usbid_gpio);
}
...
static void oplus_usbid_irq_register(struct oplus_chg_chip *chip)
{
	...
	retval = devm_request_threaded_irq(
		chip->dev, chip->normalchg_gpio.usbid_irq, NULL,
	...
	power_supply_get_property(chip->usb_psy, POWER_SUPPLY_PROP_OTG_SWITCH,
				  &ret);
	if (ret.intval == false) {
...
static int oplus_chg_get_shutdown_soc(void)
{
	...
	rc = g_smb_chip->bms_psy->desc->get_property(
		g_smb_chip->bms_psy, POWER_SUPPLY_PROP_RESTORE_SOC, &ret);
	if (rc) {
...
restore_soc_err:
	rc = g_smb_chip->bms_psy->desc->get_property(
		g_smb_chip->bms_psy, POWER_SUPPLY_PROP_CAPACITY, &ret);
	if (rc) {
...
static void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	rc = iio_read_channel_processed(chip->pmic_spmi.usb_temp_v_l_chan,
					&usb_temp_v);
	if (rc < 0) {
	...
	rc = iio_read_channel_processed(chip->pmic_spmi.usb_temp_v_r_chan,
					&usb_temp_v);
	if (rc < 0) {
...
static int smb5_probe(struct platform_device *pdev)
{
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (of_find_property(oplus_chip->dev->of_node, "qcom,pmi632chg-vadc",
			     NULL)) {
		rc = smblib_get_iio_channel(chg, "usb_in_voltage",
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	oplus_chip->pmic_spmi.usb_temp_v_l_chan =
		iio_channel_get(oplus_chip->dev, "usb_temp_v_l");
	...
	oplus_chip->pmic_spmi.usb_temp_v_r_chan =
		iio_channel_get(oplus_chip->dev, "usb_temp_v_r");
