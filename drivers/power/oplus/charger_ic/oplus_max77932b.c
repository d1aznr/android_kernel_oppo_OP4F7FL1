...
int max77932b_hardware_init(void)
{
	...
	rc = max77932b_config_interface(MAX77932B_SCC_CFG1,
					MAX77932B_FIX_FREQ_AUTO,
					MAX77932B_FIX_FREQ_MASK);
	return rc;
}
				    ...
				    size_t count, loff_t *lo)
{
	...
	if (work_mode != 0) {
		max77932b_config_interface(MAX77932B_SCC_CFG1,
					   MAX77932B_FIX_FREQ_AUTO,
					   MAX77932B_FIX_FREQ_MASK);
	} else {
		max77932b_config_interface(MAX77932B_SCC_CFG1,
					   MAX77932B_FIX_FREQ_FIX,
					   MAX77932B_FIX_FREQ_MASK);
	}
...
static int max77932b_driver_probe(struct i2c_client *client,
				  const struct i2c_device_id *id)
{
	struct chip_max77932b *divider_ic;

	divider_ic = devm_kzalloc(&client->dev, sizeof(struct chip_max77932b),
				  GFP_KERNEL);
	if (!divider_ic) {
