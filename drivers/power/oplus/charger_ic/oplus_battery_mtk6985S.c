				...
				void *data)
{
	...
	case TCP_NOTIFY_BC12_COMPLETE_STATE:
		...
		pr_err("%s bc12 complete = %d\n", __func__,
		       noti->bc12_complete_state.bc12_complete);
...
void oplus_set_otg_switch_status(bool value)
{
	if (pinfo != NULL && pinfo->tcpc != NULL) {
		...
		printk(KERN_ERR "[OPLUS_CHG][%s]: otg switch[%d]\n", __func__,
		       value);
		tcpm_typec_change_role_postpone(
			pinfo->tcpc,
			value ? TYPEC_ROLE_TRY_SNK : TYPEC_ROLE_SNK, true);
	}
}
...
void oplus_ccdetect_enable(void)
{
	...
	/* set DRP mode */
	if (pinfo != NULL && pinfo->tcpc != NULL) {
		tcpm_typec_change_role_postpone(pinfo->tcpc, TYPEC_ROLE_TRY_SNK,
						true);
...
void oplus_ccdetect_disable(void)
{
	...
	/* set SINK mode */
	if (pinfo != NULL && pinfo->tcpc != NULL) {
		tcpm_typec_change_role_postpone(pinfo->tcpc, TYPEC_ROLE_SNK,
						true);
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if (pinfo->tcpc && !pinfo->wd0_detect &&
	    tcpm_is_floating_ground(pinfo->tcpc)) {
		tcpm_typec_change_role_postpone(pinfo->tcpc, TYPEC_ROLE_SNK,
						true);
	}
