...
int oplus_get_adapter_svid(void)
{
	...
	if ((vdos[0] & 0xFFFF) == OPLUS_SVID) {
		...
		for (j = 0; j < ARRAY_SIZE(bidirect_abnormal_adapter); j++) {
			if (bidirect_abnormal_adapter[j] == vdos[2] &&
			    BIDIRECT_XID == vdos[1]) {
...
int mt_power_supply_type_check(void)
{
	...
	if (g_oplus_chip) {
		if ((g_oplus_chip->charger_type != charger_type) &&
		    g_oplus_chip->usb_psy && g_oplus_chip->mmi_chg != 0)
...
static struct temp_param battery_temp_table[] = {
	...
	{ 44, 42670 },	  { 45, 40900 },    { 46, 39210 },    { 47, 37601 },
...
int oplus_force_get_battery_temp(void)
{
	...
	if (!is_param_init) {
		...
		ntc_param.i_table_size = (sizeof(battery_temp_table) /
...
int oplus_mt6360_pd_setup_forvoocphy(void)
{
	...
	printk(KERN_ERR "%s: pd_type %d pd9v svooc [%d %d %d %d %d]", __func__,
...
int oplus_chg_set_qc_config_forvoocphy(void)
{
	...
	chip->charger_volt = battery_meter_get_charger_voltage();
	printk(KERN_ERR "%s: qc9v svooc [%d %d %d %d %d]", __func__,
...
struct oplus_chg_operations mtk6360_chg_ops = {
	...
	.input_current_ctrl_by_vooc_write =
		oplus_mt6360_input_current_limit_ctrl_by_vooc_write,
...
/* add for charger_wakelock */
	...
	pinfo->batid_temp_chan =
		iio_channel_get(oplus_chip->dev, "auxadc3-bat_id_temp");
