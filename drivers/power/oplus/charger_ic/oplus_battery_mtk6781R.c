	...
	if (pinfo->data.dual_charger_support) {
		if (g_oplus_chip->dual_charger_support &&
			...
			chg_debug(
				...
				usb_icl[i] * 1000 * g_oplus_chip->slave_pct /
					100);
...
{
	int ibat = 0;
	if (g_oplus_chip->charger_exist && bq25601d_is_detected() &&
	    (g_oplus_chip->slave_charger_enable)) {
...
{
#if defined(CONFIG_OPLUS_CHECK_CHARGERID_VOLT) ||                              \
...
int oplus_chg_set_qc_config(void)
{
	...
	chg_err("%s:charger_volt:%d,temperature:%d,cool_down_force_5v:%d,batt_volt:%d\n",
		__func__, chip->charger_volt, chip->temperature,
		chip->cool_down_force_5v, chip->batt_volt);
	if (!chip->calling_on && !chip->camera_on && chip->soc < 90 &&
	    chip->batt_volt < 4550 && chip->temperature <= 420 &&
	    chip->cool_down_force_5v ==
		    false /* && (chip->batt_volt <= chip->limits.vbatt_pdqc_to_9v_thr)*/) {
		if (pdqc_flag == false) {
		...
		if ((chip->calling_on || chip->camera_on || chip->soc >= 90 ||
		     chip->batt_volt >= 4550 || chip->temperature > 420 ||
		     chip->cool_down_force_5v ==
			     true) /* || (chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr)*/) {
...
int disable_uart(struct oplus_chg_chip *chip)
{
	...
	pinctrl_rx_low =
		pinctrl_lookup_state(pinctrl, "state_uart0_rx_output0");
	if (IS_ERR(pinctrl_rx_low)) {
		pr_err("uart---pinctrl_rx_init fail\n");
		return -1;
	}
	pinctrl_tx_low =
		pinctrl_lookup_state(pinctrl, "state_uart0_tx_output0");
...
static void mt6370_step_charging_work(struct work_struct *work)
{
	...
	if (chip->tbatt_status == BATTERY_STATUS__NORMAL) {
		...
		if (pinfo->step_status != pinfo->step_status_pre) {
			...
			oplus_mt6370_charging_current_write_fast(
				pinfo->step_chg_current);
...
int is_spaceb_hc_project(void)
{
	...
	if (is_project(21690) || is_project(21691) || is_project(21692)) {
		...
		/*chg_err("[is_spaceb_hc_project]!!!!!! spaceb_18W ret=%d\n", ret);*/
		   ...
		   is_project(21686) || is_project(21687)) {
		ret = 1;
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	pinfo->usb_temp_v_l_chan =
		iio_channel_get(oplus_chip->dev, "auxadc3-usb_temp_v_l");
