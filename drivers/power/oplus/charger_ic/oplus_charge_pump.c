...
static int charge_pump_config_interface(int RegNum, int val, int MASK)
{
	...
	if (ret >= 0) {
		...
		if (RegNum == CHARGE_PUMP_ADDRESS_REG04 &&
...
void charge_pump_dump_registers(void)
{
	...
	for (addr = CHARGE_PUMP_FIRST_REG; addr <= CHARGE_PUMP_LAST_REG;
	     addr++) {
	...
	for (addr = CHARGE_PUMP_FIRST2_REG; addr <= CHARGE_PUMP_LAST2_REG;
	     addr++) {
		rc = charge_pump_read_reg(addr, &val_buf[addr]);
		if (rc) {
			chg_err("Couldn't read 0x%02x rc = %d\n", addr, rc);
...
static int charge_pump_set_work_mode(int work_mode)
{
	...
	case HWID_DA9313:
		if (work_mode != 0) {
			rc = charge_pump_config_interface(
				DA9313_WORK_MODE_REG, DA9313_WORK_MODE_AUTO,
				DA9313_WORK_MODE_MASK);
		} else {
			rc = charge_pump_config_interface(
				DA9313_WORK_MODE_REG, DA9313_WORK_MODE_FIXED,
				DA9313_WORK_MODE_MASK);
		}
		break;
	case HWID_MAX77932:
		if (work_mode != 0) {
			rc = charge_pump_config_interface(
				MAX77932_WORK_MODE_REG, MAX77932_WORK_MODE_AUTO,
				MAX77932_WORK_MODE_MASK);
		} else {
			rc = charge_pump_config_interface(
				MAX77932_WORK_MODE_REG,
				MAX77932_WORK_MODE_FIXED,
				MAX77932_WORK_MODE_MASK);
		}
		break;
	case HWID_MAX77938:
		if (work_mode != 0) {
			rc = charge_pump_config_interface(
				MAX77938_WORK_MODE_REG, MAX77938_WORK_MODE_AUTO,
				MAX77938_WORK_MODE_MASK);
		} else {
			rc = charge_pump_config_interface(
				MAX77938_WORK_MODE_REG,
				MAX77938_WORK_MODE_FIXED,
				MAX77938_WORK_MODE_MASK);
		}
		break;
	case HWID_SD77313:
		if (work_mode != 0) {
			rc = charge_pump_config_interface(
				SD77313_WORK_MODE_REG, SD77313_WORK_MODE_AUTO,
				SD77313_WORK_MODE_MASK);
		} else {
			rc = charge_pump_config_interface(
				SD77313_WORK_MODE_REG, SD77313_WORK_MODE_FIXED,
				SD77313_WORK_MODE_MASK);
		}
		break;
	default:
...
int max77932_hardware_config(void)
{
	...
	charge_pump_set_work_mode(CHARGE_PUMP_WORK_MODE_AUTO);
	charge_pump_config_interface(CHARGE_PUMP_ADDRESS_REG05,
				     (BIT(4) | BIT(5)),
				     (BIT(0) | BIT(4) | BIT(5)));
	...
	charge_pump_config_interface(CHARGE_PUMP_ADDRESS_REG09,
...
int max77938_hardware_config(void)
{
	...
	rc = charge_pump_config_interface(CHARGE_PUMP_ADDRESS_REG08,
...
int sd77313_hardware_config(void)
{
	...
	charge_pump_config_interface(CHARGE_PUMP_ADDRESS_REG10, BIT(2),
...
static void charge_pump_chip_init(struct chip_charge_pump *chip)
{
	...
	chip->charge_pump_hwid_gpio =
		of_get_named_gpio(node, "oplus,charge_pump-hwid-gpio", 0);
	if (chip->charge_pump_hwid_gpio < 0) {
	...
	chip->charge_pump_hwid_active =
		pinctrl_lookup_state(chip->pinctrl, "charge_pump_hwid_active");
	if (IS_ERR_OR_NULL(chip->charge_pump_hwid_active)) {
		chg_err("get charge_pump_hwid_active fail\n");
		goto HWID_HANDLE;
	}

	chip->charge_pump_hwid_sleep =
		pinctrl_lookup_state(chip->pinctrl, "charge_pump_hwid_sleep");
	if (IS_ERR_OR_NULL(chip->charge_pump_hwid_sleep)) {
		chg_err("get charge_pump_hwid_sleep fail\n");
		goto HWID_HANDLE;
	}

	chip->charge_pump_hwid_default =
		pinctrl_lookup_state(chip->pinctrl, "charge_pump_hwid_default");
...
static ssize_t proc_charge_pump_work_mode_read(struct file *file,
...
static ssize_t proc_charge_pump_work_mode_write(struct file *file,
...
static ssize_t proc_charge_pump_reg_write(struct file *file,
...
static int init_charge_pump_proc(struct chip_charge_pump *da)
{
	...
	prEntry_tmp = proc_create_data("work_mode", 0664, prEntry_da,
	...
	prEntry_set_reg = proc_create_data("register", 0664, prEntry_da,
					   &proc_register_ops, da);
				    ...
				    const struct i2c_device_id *id)
{
	...
	divider_ic = devm_kzalloc(&client->dev, sizeof(struct chip_charge_pump),
				  GFP_KERNEL);
	if (!divider_ic) {
