...
static int __bq25890h_read_reg(struct chip_bq25890h *chip, int reg,
			       int *returnData)
{
#ifdef CONFIG_OPLUS_CHARGER_MTK
#if defined CONFIG_OPLUS_CHARGER_MTK6763 ||                                    \
...
static int bq25890h_read_reg(struct chip_bq25890h *chip, int reg,
			     int *returnData)
{
	int ret = 0;
...
static int bq25890h_config_interface(struct chip_bq25890h *chip, int RegNum,
				     int val, int MASK)
{
	int bq25890h_reg = 0;
	int ret = 0;
...
static int bq25890h_input_current_limit_write(int value)
{
	...
	*/
	...
	if (chg_vol < 4300 && low_chg_vol_flag == 0 &&
...
*/
	...
	aicl_point_temp = charger_ic->sw_aicl_point;
#endif
	rc = bq25890h_config_interface(
		charger_ic, REG00_BQ25890H_ADDRESS,
...
aicl_pre_step:
	if ((j >= 2) &&
	    (j <= ARRAY_SIZE(bq25890h_usbin_input_current_limit) - 1))
...
static int bq25890h_input_current_limit_write(int value)
{
	...
	for (i = ARRAY_SIZE(bq25890h_usbin_input_current_limit) - 1; i >= 0;
	     i--) {
	...
	for (j = 2; j <= i; j++) {
		...
		if (chg_vol < charger_ic->sw_aicl_point) {
			...
			if ((j >= 2) &&
			    (j <=
			     ARRAY_SIZE(bq25890h_usbin_input_current_limit) -
				     1))
				aicl_result =
					bq25890h_usbin_input_current_limit[j];
			chg_debug(
				"usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d\n",
				chg_vol, j,
				bq25890h_usbin_input_current_limit[j],
				charger_ic->sw_aicl_point);
			bq25890h_config_interface(
				chip, REG00_BQ25890H_ADDRESS,
				j << REG00_BQ25890H_INPUT_CURRENT_LIMIT_SHIFT,
				REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
			return 0;
		}
...
static int bq25890h_charging_current_write_fast(int chg_cur)
{
	...
	value = (chg_cur - BQ25890H_MIN_FAST_CURRENT_MA) /
		BQ25890H_FAST_CURRENT_STEP_MA;
	value <<= REG04_BQ25890H_FAST_CHARGING_CURRENT_LIMIT_SHIFT;

	ret = bq25890h_config_interface(
		charger_ic, REG04_BQ25890H_ADDRESS, value,
...
static int bq25890h_set_vindpm_vol(struct chip_bq25890h *chip, int vol)
{
	...
	value = (vol - REG0D_BQ25890H_VINDPM_OFFSET) /
		REG0D_BQ25890H_VINDPM_STEP_MV;
	value <<= REG0D_BQ25890H_VINDPM_SHIFT;

	rc = bq25890h_config_interface(chip, REG0D_BQ25890H_ADDRESS, value,
				       REG0D_BQ25890H_VINDPM_MASK);
	return rc;
}
...
static int bq25890h_set_complete_charge_timeout(struct chip_bq25890h *chip,
						int val)
{
	int rc = 0;
	...
	if (val == OVERTIME_AC) {
		val = REG07_BQ25890H_CHARGING_SAFETY_TIME_ENABLE |
		      REG07_BQ25890H_FAST_CHARGING_TIMEOUT_8H;
	} else if (val == OVERTIME_USB) {
		val = REG07_BQ25890H_CHARGING_SAFETY_TIME_ENABLE |
		      REG07_BQ25890H_FAST_CHARGING_TIMEOUT_12H;
	} else {
		val = REG07_BQ25890H_CHARGING_SAFETY_TIME_DISABLE |
		      REG07_BQ25890H_FAST_CHARGING_TIMEOUT_8H;
	}

	rc = bq25890h_config_interface(
		charger_ic, REG07_BQ25890H_ADDRESS, val,
...
int bq25890h_float_voltage_write(int vfloat_mv)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG06_BQ25890H_ADDRESS,
				       value,
...
int bq25890h_set_prechg_current(int ipre_mA)
{
	...
	value = (ipre_mA - BQ25890H_MIN_PRE_CURRENT_MA) /
		BQ25890H_PRE_CURRENT_STEP_MA;
	value <<= REG05_BQ25890H_PRE_CHARGING_CURRENT_LIMIT_SHIFT;

	return bq25890h_config_interface(
		charger_ic, REG05_BQ25890H_ADDRESS, value,
...
static int bq25890h_set_termchg_current(int term_curr)
{
	...
	value = (term_curr - BQ25890H_MIN_TERM_CURRENT_MA) /
		BQ25890H_TERM_CURRENT_STEP_MA;
...
int bq25890h_set_rechg_voltage(int recharge_mv)
{
	...
	rc = bq25890h_config_interface(
		charger_ic, REG06_BQ25890H_ADDRESS, reg,
...
int bq25890h_set_wdt_timer(int reg)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG07_BQ25890H_ADDRESS, reg,
...
static int bq25890h_set_chging_term_disable(void)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG07_BQ25890H_ADDRESS,
				       REG07_BQ25890H_TERMINATION_DISABLE,
...
int bq25890h_kick_wdt(void)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG03_BQ25890H_ADDRESS,
				       REG03_BQ25890H_WDT_TIMER_RESET,
...
int bq25890h_enable_charging(void)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG03_BQ25890H_ADDRESS,
				       REG03_BQ25890H_CHARGING_ENABLE,
...
int bq25890h_disable_charging(void)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG03_BQ25890H_ADDRESS,
				       REG03_BQ25890H_CHARGING_DISABLE,
				       REG03_BQ25890H_CHARGING_MASK);
	if (rc < 0) {
	...
	rc = bq25890h_config_interface(
		charger_ic, REG07_BQ25890H_ADDRESS,
		REG07_BQ25890H_CHARGING_SAFETY_TIME_DISABLE,
		REG07_BQ25890H_I2C_WATCHDOG_TIME_MASK);
	if (rc < 0) {
...
int bq25890h_disable_charging(void)
{
	...
	bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS,
				  REG00_BQ25890H_INPUT_CURRENT_LIMIT_400MA,
				  REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
...
static bool bq25890h_check_charger_suspend_enable(void)
{
	...
	charger_suspend_enable =
		((reg_val & REG00_BQ25890H_SUSPEND_MODE_MASK) ==
		 REG00_BQ25890H_SUSPEND_MODE_ENABLE) ?
			true :
			false;
...
static int bq25890h_check_charging_enable(void)
{
	...
	charging_enable = ((reg_val & REG03_BQ25890H_CHARGING_MASK) ==
			   REG03_BQ25890H_CHARGING_ENABLE) ?
				  1 :
				  0;
...
int bq25890h_registers_read_full(void)
{
	...
	reg_ovp = ((reg_ovp & REG0C_BQ25890H_BATTERY_VOLATGE_MASK) ==
		   REG0C_BQ25890H_BATTERY_VOLATGE_HIGH_ERROR) ?
			  1 :
			  0;
...
#endif /* CONFIG_OPLUS_CHARGER_MTK */
	bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS,
				  REG00_BQ25890H_SUSPEND_MODE_DISABLE,
				  REG00_BQ25890H_SUSPEND_MODE_MASK);

	rc = bq25890h_config_interface(charger_ic, REG03_BQ25890H_ADDRESS,
				       REG03_BQ25890H_OTG_ENABLE,
...
int bq25890h_reset_charger(void)
{
	...
	rc = bq25890h_config_interface(charger_ic, REG00_BQ25890H_ADDRESS, 0x08,
				       0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG00 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(charger_ic, REG01_BQ25890H_ADDRESS, 0x01,
				       0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG01 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(charger_ic, REG02_BQ25890H_ADDRESS, 0x00,
				       0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG02 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(charger_ic, REG03_BQ25890H_ADDRESS, 0x1A,
				       0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG03 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(charger_ic, REG04_BQ25890H_ADDRESS, 0x08,
				       0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG04 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(charger_ic, REG05_BQ25890H_ADDRESS, 0x13,
				       0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG05 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(charger_ic, REG06_BQ25890H_ADDRESS, 0x86,
				       0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG06 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(charger_ic, REG07_BQ25890H_ADDRESS, 0xDD,
				       0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG07 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(charger_ic, REG08_BQ25890H_ADDRESS, 0x03,
				       0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG08 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(charger_ic, REG09_BQ25890H_ADDRESS, 0x40,
				       0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG09 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(charger_ic, REG0A_BQ25890H_ADDRESS, 0x73,
				       0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG0A rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(charger_ic, REG0D_BQ25890H_ADDRESS, 0x91,
				       0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG0D rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(charger_ic, REG14_BQ25890H_ADDRESS, 0x5F,
				       0xFF);
	if (rc < 0) {
...
void bq25890h_dump_registers(void)
{
	...
	if (dump_count == DUMP_REG_LOG_CNT_30S) {
		...
		printk("bq25890h_reg[b-14]:0x%02x(b),0x%02x(c),0x%02x(d),0x%02x(e),0x%02x(f),0x%02x(10),0x%02x(11),0x%02x(12),0x%02x(13),0x%02x(14)\n",
		       val_buf[11], val_buf[12], val_buf[13], val_buf[14],
		       val_buf[15], val_buf[16], val_buf[17], val_buf[18],
...
enum charger_type mt_charger_type_detection_bq25890h(void)
{
	...
	while (val_buf == 0x02 && count < 20) {
		count++;
		rc = bq25890h_read_reg(charger_ic, REG02_BQ25890H_ADDRESS,
				       &val_buf);
		val_buf &= 0x02;
		...
		if (!upmu_get_rgs_chrdet()) {
			MTK_CHR_Type_num = CHARGER_UNKNOWN;
			bq25890h_config_interface(
				charger_ic, REG02_BQ25890H_ADDRESS, 0, 0xff);
			return MTK_CHR_Type_num;
		}
	}
	...
	if (count == 20) {
		MTK_CHR_Type_num = CHARGER_UNKNOWN;
		bq25890h_config_interface(charger_ic, REG02_BQ25890H_ADDRESS, 0,
					  0xff);
		return MTK_CHR_Type_num;
	}
	...
	} else if (val_buf == 0x60 || val_buf == 0x80 || val_buf == 0xA0 ||
		   val_buf == 0xC0) {
	...
	if (MTK_CHR_Type_num == CHARGER_UNKNOWN && upmu_get_rgs_chrdet()) {
		...
		bq25890h_config_interface(charger_ic, REG02_BQ25890H_ADDRESS,
					  0x01, 0);
		bq25890h_config_interface(charger_ic, REG02_BQ25890H_ADDRESS,
					  0x02, 0);
		bq25890h_dump_registers();
		...
		while (val_buf == 0x02 && count < 20) {
			count++;
			rc = bq25890h_read_reg(
				charger_ic, REG02_BQ25890H_ADDRESS, &val_buf);
			val_buf &= 0x02;
			usleep_range(50000, 50200);
			if (!upmu_get_rgs_chrdet()) {
				MTK_CHR_Type_num = CHARGER_UNKNOWN;
				bq25890h_config_interface(
					charger_ic, REG02_BQ25890H_ADDRESS, 0,
					0xff);
				return MTK_CHR_Type_num;
			}
		}
		...
		if (val_buf == 0x0) {
			...
			MTK_CHR_Type_num = CHARGING_HOST;
		} else if (val_buf == 0x60 || val_buf == 0x80 ||
			   val_buf == 0xA0 || val_buf == 0xC0) {
			MTK_CHR_Type_num = APPLE_2_1A_CHARGER;
		} else {
			MTK_CHR_Type_num = APPLE_2_1A_CHARGER;
		}
...
static int bq25890h_driver_probe(struct i2c_client *client,
				 const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct chip_bq25890h),
			    GFP_KERNEL);
	if (!chip) {
