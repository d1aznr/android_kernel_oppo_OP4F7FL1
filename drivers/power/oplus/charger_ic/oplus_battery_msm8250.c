...
#define OPLUS_CHG_MONITOR_INTERVAL                                             \
	round_jiffies_relative(msecs_to_jiffies(5000))
#define REC_SUSPEND_WORK_SCH_DELAY                                             \
...
static int oplus_otg_en_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chg->otg_en_active =
		pinctrl_lookup_state(chg->otg_en_pinctrl, "otg_en_active");
	...
	chg->otg_en_sleep =
		pinctrl_lookup_state(chg->otg_en_pinctrl, "otg_en_sleep");
	...
	chg->otg_en_default =
		pinctrl_lookup_state(chg->otg_en_pinctrl, "otg_en_default");
...
static int oplus_wired_conn_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chg->wired_conn_active = pinctrl_lookup_state(chg->wired_conn_pinctrl,
	...
	chg->wired_conn_sleep = pinctrl_lookup_state(chg->wired_conn_pinctrl,
						     "wired_con_int_sleep");
...
static void oplus_wired_conn_int_work(struct work_struct *work)
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, wired_in_work.work);
...
static void oplus_wired_conn_irq_register(struct oplus_chg_chip *chip)
{
	...
	ret = devm_request_threaded_irq(
		chip->dev, chg->wired_conn_irq, NULL,
		oplus_wired_conn_change_handler,
		IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT,
		"wired_conn-change", chip);
	if (ret < 0) {
...
static int oplus_idt_en_gpio_init(struct oplus_chg_chip *chip)
{
	...
	if (!chg) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: tongfeng test start chg  NULL !\n",
		       __func__);
		return 0;
	}
	...
	chg->idt_en_active =
		pinctrl_lookup_state(chg->idt_en_pinctrl, "idt_en_active");
	...
	chg->idt_en_sleep =
		pinctrl_lookup_state(chg->idt_en_pinctrl, "idt_en_sleep");
	if (IS_ERR_OR_NULL(chg->idt_en_sleep)) {
		chg_err("get idt_en_sleep fail\n");
		return -EINVAL;
	}
	chg->idt_en_default =
		pinctrl_lookup_state(chg->idt_en_pinctrl, "idt_en_default");
...
void oplus_set_idt_en_val(int value) // 0  active, 1 inactive
{
	...
	chg_err("<~WPC~> set value:%d, gpio_val:%d\n", value,
		gpio_get_value(chg->idt_en_gpio));
}
...
bool oplus_get_wired_otg_online(void)
{
	...
	ret = power_supply_get_property(chg->usb_psy,
...
void oplus_switch_to_wired_charge(struct smb_charger *chg)
{
	...
	if (oplus_wpc_get_otg_charging()) {
		...
		schedule_delayed_work(&chg->wait_wired_charge_on,
...
void oplus_switch_from_wired_charge(struct smb_charger *chg)
{
	if (oplus_wpc_get_otg_charging()) {
		...
		schedule_delayed_work(&chg->wait_wired_charge_off,
				      msecs_to_jiffies(100));
	} else {
		oplus_wpc_set_vbat_en_val(0);
		if (g_oplus_chip->wpc_no_chargerpump &&
		    oplus_wpc_get_fw_updating() == false)
...
void oplus_dcin_irq_enable(bool enable)
{
	...
	if (enable && !dcin_en) {
		...
		enable_irq(chip->pmic_spmi.smb5_chip->chg.irq_info[DCIN_UV_IRQ]
				   .irq);
		enable_irq(chip->pmic_spmi.smb5_chip->chg.irq_info[DCIN_OV_IRQ]
				   .irq);
		enable_irq(
			chip->pmic_spmi.smb5_chip->chg.irq_info[DCIN_PLUGIN_IRQ]
				.irq);
		enable_irq(
			chip->pmic_spmi.smb5_chip->chg.irq_info[DCIN_REVI_IRQ]
				.irq);
		enable_irq(chip->pmic_spmi.smb5_chip->chg.irq_info[DCIN_PON_IRQ]
				   .irq);
		enable_irq(chip->pmic_spmi.smb5_chip->chg.irq_info[DCIN_EN_IRQ]
				   .irq);
	} else if (!enable && dcin_en) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: disable!\n", __func__);
		disable_irq_nosync(
			chip->pmic_spmi.smb5_chip->chg.irq_info[DCIN_UV_IRQ]
				.irq);
		disable_irq_nosync(
			chip->pmic_spmi.smb5_chip->chg.irq_info[DCIN_OV_IRQ]
				.irq);
		disable_irq_nosync(
			chip->pmic_spmi.smb5_chip->chg.irq_info[DCIN_PLUGIN_IRQ]
				.irq);
		disable_irq_nosync(
			chip->pmic_spmi.smb5_chip->chg.irq_info[DCIN_REVI_IRQ]
				.irq);
		disable_irq_nosync(
			chip->pmic_spmi.smb5_chip->chg.irq_info[DCIN_PON_IRQ]
				.irq);
		disable_irq_nosync(
			chip->pmic_spmi.smb5_chip->chg.irq_info[DCIN_EN_IRQ]
				.irq);
	}
	dcin_en = enable;
}
	...
	if (g_chg->wireless_present) {
		...
		if (!reconfiged_aicl) {
			...
			rc = smblib_masked_write(
				g_chg, USBIN_AICL_OPTIONS_CFG_REG,
				USBIN_AICL_EN_BIT |
			...
			if (rc < 0) {
				smblib_err(
					g_chg,
					"Couldn't turn on USBIN_AICL_OPTIONS_CFG_REG AICL rc=%d\n",
					rc);
				return rc;
			}
			rc = smblib_masked_write(
				g_chg, USBIN_LOAD_CFG_REG,
			...
			if (rc < 0) {
				smblib_err(
					g_chg,
					"Couldn't turn on USBIN_LOAD_CFG_REG rc=%d\n",
					rc);
				return rc;
			}
...
void smblib_bypass_vsafe0_control(struct smb_charger *chg, bool enable)
{
	...
	rc = smblib_masked_write(
		g_chg, TYPE_C_EXIT_STATE_CFG_REG,
...
int smblib_set_icl_current(struct smb_charger *chg, int icl_ua)
{
	...
	smblib_err(
		chg,
		"icl_ua=%d, connector_type=%d, real_charger_type=%d,wireles_present[%d]\n",
	...
	if (chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC) {
		...
		if (rc < 0) {
			smblib_err(
				chg,
				"Couldn't write %s to EN_PULLDOWN_USB_IN_BIT rc=%d\n",
		...
		if (chip->wireless_support) {
			...
			if (chip->wpc_no_chargerpump) {
				if (oplus_wpc_get_fw_updating() == true) {
					smblib_err(
						chg,
						"[WPC] idt_fw_update,don't enabling OTG\n");
					return rc;
				}
				oplus_wpc_dis_wireless_chg(1);
				if (oplus_wpc_get_otg_charging()) {
	...
	if (chip->vbatt_num == 2) {
		...
		if (chip->wireless_support) {
			if (chip->wpc_no_chargerpump &&
				...
				smblib_err(
					chg,
					"[WPC] idt_fw_update,don't disabling OTG\n");
				return rc;
			}
			chg->otg_disable_timeout = 1;
			cancel_delayed_work_sync(
				&chg->otg_disable_timeout_work);
			schedule_delayed_work(&chg->otg_disable_timeout_work,
					      msecs_to_jiffies(100));
...
#endif
	       JEITA_EN_HOT_SL_FCV_BIT | JEITA_EN_HOT_SL_CCC_BIT |
...
static int smblib_get_prop_ufp_mode(struct smb_charger *chg)
{
	...
	if (stat &
		...
		rc = smblib_masked_write(chg, DEBUG_ACCESS_SNK_CFG_REG, 0x1f,
					 0x17);
		if (rc < 0)
			smblib_err(
				chg,
				"Couldn't config DEBUG_ACCESS_SNK_CFG_REG rc=%d\n",
				rc);
	}
#endif
				    ...
				    union power_supply_propval *val)
{
	...
	if (chip && chip->vbatt_num == 2 &&
	    chip->chg_ops->get_charger_current) {
			     ...
			     const union power_supply_propval *val)
{
	...
	rc = smblib_masked_write(chg, TYPE_C_CC_CURRSRC_CONTROL_REG,
				 TYPE_C_CC_CURRSRC_CONTROL_MASK, val->intval);
	if (rc < 0)
		smblib_err(
			chg,
			"Couldn't write to TYPE_C_CC_CURRSRC_CONTROL_REG rc=%d\n",
			rc);
	...
	if (rc < 0) {
		smblib_err(
			chg,
			"Couldn't read TYPE_C_CC_CURRSRC_CONTROL_REG rc=%d\n",
			rc);
		return rc;
	}
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, reset_rd_work.work);
	int rc;
	u8 stat;

	rc = smblib_masked_write(chg, TYPE_C_CC_CURRSRC_CONTROL_REG,
				 TYPE_C_CC_CURRSRC_CONTROL_MASK, 0);
	if (rc < 0)
		smblib_err(
			chg,
			"Couldn't write to TYPE_C_CC_CURRSRC_CONTROL_REG rc=%d\n",
				     ...
				     const union power_supply_propval *val)
{
	...
	smblib_err(chg,
		   " power_role=%d stat =0x%x level =%d chip->otg_switch=%d\n",
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	if (g_oplus_chip && g_oplus_chip->wireless_support) {
		...
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: scheduling wireless charger!\n",
		       __func__);
	...
	if (vbus_rising) {
		...
		chg->hvdcp_detach_time =
			cpu_clock(smp_processor_id()) / 1000000;
		...
		if (!(chg->hvdcp_detect_ok &&
			...
			schedule_delayed_work(&chg->hvdcp_disable_work,
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (chg->wireless_present) {
		if (chg->wireless_high_vol_mode)
			...
			smblib_set_opt_switcher_freq(
				chg, vbus_rising ? chg->chg_freq.freq_6V_8V :
						   chg->chg_freq.freq_removal);
	} else {
		smblib_set_opt_switcher_freq(
			chg, vbus_rising ? chg->chg_freq.freq_5V :
					   chg->chg_freq.freq_removal);
		if (g_oplus_chip &&
		    g_oplus_chip->charger_type == POWER_SUPPLY_TYPE_WIRELESS) {
			g_oplus_chip->charger_type = POWER_SUPPLY_TYPE_UNKNOWN;
		}
	}
#endif // OPLUS_CUSTOM_OP_DEF
	...
	vote(chg->temp_change_irq_disable_votable, DEFAULT_VOTER, !vbus_rising,
	     0);
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (vbus_rising) {
		cancel_delayed_work_sync(&chg->chg_monitor_work);
	...
	case POWER_SUPPLY_TYPE_USB:
		...
		if (!is_client_vote_enabled(chg->usb_icl_votable,
			...
			vote(chg->usb_icl_votable, USB_PSY_VOTER, true,
			     is_flash_active(chg) ? SDP_CURRENT_UA :
						    SDP_100_MA);
		}
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, false, 0);
		break;
...
#ifdef OPLUS_CUSTOM_OP_DEF
	printk(KERN_ERR
	       "%s: !!!fg_oplus_set_input_current[%d] wireless_present[%d]\n",
...
static bool smblib_src_lpd(struct smb_charger *chg)
{
	...
	if (lpd_flag) {
		...
		smblib_err(chg, "%s: LPD_MOISTURE_DETECTED detected\n",
			   __func__);
		if ((oplus_get_usb_status() & USB_WATER_DETECT) == 0 &&
		    g_oplus_chip) {
...
irqreturn_t typec_state_change_irq_handler(int irq, void *data)
{
	...
	if (chg->wls_psy) {
		rc = power_supply_get_property(
			chg->wls_psy, POWER_SUPPLY_PROP_PRESENT, &pval);
		if (rc < 0)
	...
	if (wireless_present) {
		if (typec_mode ==
		    POWER_SUPPLY_TYPEC_NONE) // If nothing connected enable bypass vsafe for OTG detection
	...
	if (dfp_status ^ current_status) {
		...
		printk(KERN_ERR
		       "!!!!! smblib_handle_typec_cc_state_change: [%d], mode[%d]\n",
	...
	if (chg->typec_present == false &&
		...
		if (oplus_ccdetect_get_power_role() !=
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	printk(KERN_ERR
	       "!!!!! dc_plugin_irq_handler: dcin_present[%d], usbin_present[%d], cp_reason[%d]\n",
					...
					const union power_supply_propval *val)
{
	...
	if (chg->pr_swap_in_progress) {
		...
		smblib_err(
			chg,
			" reg_val=%d stat =0x%x level =%d chip->otg_switch=%d\n",
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, ccdetect_work.work);
	int level;
	...
	if (stat == U_USB_STATUS_WATER_PRESENT || stat == U_USB_FMB1_BIT ||
		...
		if (get_boot_mode() != MSM_BOOT_MODE__FACTORY)
			alarm_start_relative(
				...
				ms_to_ktime(
					MOISTURE_PROTECTION_CHECK_DELAY_MS));
	} else {
...
static void typec_disable_cmd_work(struct work_struct *work)
{
	...
	if (smblib_get_prop_typec_mode(chg) == POWER_SUPPLY_TYPEC_NONE) {
		...
		if (rc < 0)
			smblib_err(chg,
				   "Couldn't enable APSD_START_ON_CC rc=%d\n",
				   rc);
		msleep(600);
...
static void oplus_hvdcp_disable_work(struct work_struct *work)
{
	...
	bool vbus_rising;
	struct smb_charger *chg =
		container_of(work, struct smb_charger, hvdcp_disable_work.work);
	if (chg == NULL)
		return;
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, chg_monitor_work.work);
	struct oplus_chg_chip *chip = g_oplus_chip;
	int boot_mode = get_boot_mode();
	static int counts = 0;
	int rechg_vol;
	...
	if ((chip->batt_volt > rechg_vol - 10) && chip->batt_full) {
		counts = 0;
		goto rerun_work;
	} else if (chip->batt_volt >
		   oplus_chg_get_vbatt_full_vol_sw(chip) - 10) {
		counts = 0;
		goto rerun_work;
	}
	...
	if (counts >= (chip->batt_full ? 8 : 3)) { //because rechg counts=6
		...
		if (get_client_vote(chg->usb_icl_votable, BOOST_BACK_VOTER) ==
			    0 &&
		...
		if (chip->charging_state ==
		    CHARGING_STATUS_FAIL) { //for TEMP > 55 or < -3
			counts = 0;
			goto rerun_work;
		}
		if (stat & PRE_TERM_BIT) {
			...
			printk(KERN_ERR
			       "oplus_chg_monitor_work: PRE_TERM_BIT is set[0x%x], clear it\n",
			       stat);
			//rc = smblib_masked_write(chg, USBIN_CMD_IL_REG, USBIN_SUSPEND_BIT, 1);
			rc = smblib_set_usb_suspend(chg, true);
			if (rc < 0) {
				printk(KERN_ERR
				       "oplus_chg_monitor_work: Couldn't set USBIN_SUSPEND_BIT rc=%d\n",
				       rc);
				goto rerun_work;
			}
			msleep(50);
			...
			msleep(10);
			rc = smblib_masked_write(chg, AICL_CMD_REG,
						 RESTART_AICL_BIT,
						 RESTART_AICL_BIT);
			if (rc < 0) {
				printk(KERN_ERR
				       "oplus_chg_monitor_work: Couldn't set RESTART_AICL_BIT rc=%d\n",
				       rc);
				goto rerun_work;
			}
			printk(KERN_ERR
			       "oplus_chg_monitor_work: ichg[%d], fv[%d]\n",
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->wait_wired_charge_on,
			  oplus_wait_wired_charge_on_work);
	INIT_DELAYED_WORK(&chg->wait_wired_charge_off,
			  oplus_wait_wired_charge_off_work);
	INIT_DELAYED_WORK(&chg->otg_disable_timeout_work,
			  oplus_otg_disable_timeout_work);
	chg->wired_in_flag = 0;
	chg->otg_disable_timeout = 0;
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
...
#ifndef OPLUS_CUSTOM_OP_DEF
	INIT_DELAYED_WORK(&chg->switch_to_wired_work,
			  oplus_switch_to_wired_work);
#endif
#endif
	...
	INIT_DELAYED_WORK(&chg->connect_check_work,
...
static int oplus_chg_2uart_pinctrl_init(struct oplus_chg_chip *chip)
{
	...
	chg->chg_2uart_default = pinctrl_lookup_state(
		chg->chg_2uart_pinctrl, "chg_qupv3_se12_2uart_default");
	if (IS_ERR_OR_NULL(chg->chg_2uart_default)) {
		chg_err("get chg_2uart_default fail\n");
		return -EINVAL;
	}

	chg->chg_2uart_sleep = pinctrl_lookup_state(
		chg->chg_2uart_pinctrl, "chg_qupv3_se12_2uart_sleep");
...
static int oplus_chg_set_2uart_pinctrl_chgID(struct oplus_chg_chip *chip)
{
	...
	if (IS_ERR_OR_NULL(chg->chg_2uart_pinctrl) ||
	    IS_ERR_OR_NULL(chg->chg_2uart_sleep)) {
		chg_err("get 2uart chg_2uart_pinctrl fail\n");
		return -EINVAL;
	}
...
static int oplus_chg_set_2uart_pinctrl_default(struct oplus_chg_chip *chip)
{
	...
	if (IS_ERR_OR_NULL(chg->chg_2uart_pinctrl) ||
	    IS_ERR_OR_NULL(chg->chg_2uart_default)) {
		chg_err("get 2uart chg_2uart_pinctrl fail\n");
		return -EINVAL;
	}
	mutex_lock(&chg->pinctrl_mutex);
...
int smbchg_get_chargerid_volt(void)
{
	...
	if (IS_ERR_OR_NULL(chg->iio.chgid_v_chan)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: chg->iio.chgid_v_chan  is  NULL !\n",
		       __func__);
		return 0;
	}
...
static int smbchg_chargerid_switch_gpio_init(struct oplus_chg_chip *chip)
{
...
#ifdef OPLUS_CUSTOM_OP_DEF
	chip->normalchg_gpio.chargerid_switch_pvt_active = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "chargerid_switch_pvt_active");
	if (IS_ERR_OR_NULL(chip->normalchg_gpio.chargerid_switch_pvt_active)) {
		chg_err("get chargerid_switch_pvt_active fail\n");
		//return -EINVAL;
	}

	chip->normalchg_gpio.chargerid_switch_pvt_sleep = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "chargerid_switch_pvt_sleep");
	...
	if (chip->normalchg_gpio.chargerid_switch_gpio_pvt > 0) {
		gpio_direction_output(
			chip->normalchg_gpio.chargerid_switch_gpio_pvt, 0);
...
void smbchg_set_chargerid_switch_val(int value)
{
	...
	if (value) {
		gpio_direction_output(
			chip->normalchg_gpio.chargerid_switch_gpio, 1);
		pinctrl_select_state(
			chip->normalchg_gpio.pinctrl,
			chip->normalchg_gpio.chargerid_switch_default);
#ifdef OPLUS_CUSTOM_OP_DEF
		if (chip->normalchg_gpio.chargerid_switch_gpio_pvt > 0) {
			gpio_direction_output(
				chip->normalchg_gpio.chargerid_switch_gpio_pvt,
				1);
			pinctrl_select_state(
				chip->normalchg_gpio.pinctrl,
				chip->normalchg_gpio
...
static int oplus_shipmode_id_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chg->shipmode_id_active = pinctrl_lookup_state(chg->shipmode_id_pinctrl,
						       "shipmode_id_active");
...
static bool oplus_shipmode_id_check_is_gpio(struct oplus_chg_chip *chip)
{
	...
	if (gpio_is_valid(chg->shipmode_id_gpio)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: tongfeng test shipmode_id_gpio true!\n",
		       __func__);
		return true;
	}
...
int oplus_usbtemp_adc_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chg->usbtemp_gpio1_default = pinctrl_lookup_state(
		chg->usbtemp_gpio1_adc_pinctrl, "gpio1_adc_default");
	if (IS_ERR_OR_NULL(chg->usbtemp_gpio1_default)) {
		chg_err("get usbtemp_gpio1_default fail\n");
		return -EINVAL;
	}

	pinctrl_select_state(chg->usbtemp_gpio1_adc_pinctrl,
			     chg->usbtemp_gpio1_default);
	...
	chg->usbtemp_gpio8_default = pinctrl_lookup_state(
		chg->usbtemp_gpio8_adc_pinctrl, "gpio8_adc_default");
	if (IS_ERR_OR_NULL(chg->usbtemp_gpio8_default)) {
		chg_err("get usbtemp_gpio8_default fail\n");
		return -EINVAL;
	}

	pinctrl_select_state(chg->usbtemp_gpio8_adc_pinctrl,
			     chg->usbtemp_gpio8_default);
	...
	chg->usbtemp_gpio5_default = pinctrl_lookup_state(
		chg->usbtemp_gpio5_adc_pinctrl, "gpio5_adc_default");
	if (IS_ERR_OR_NULL(chg->usbtemp_gpio5_default)) {
		chg_err("get usbtemp_gpio5_default fail\n");
		return -EINVAL;
	}

	pinctrl_select_state(chg->usbtemp_gpio5_adc_pinctrl,
			     chg->usbtemp_gpio5_default);
...
int oplus_sc_mos_gpio_init(struct oplus_chg_chip *chip)
{
	...
	sc_mos_default =
		pinctrl_lookup_state(sc_mos_pinctrl, "gpio12_adc_default");
...
int oplus_ccdetect_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chg->ccdetect_active =
		pinctrl_lookup_state(chg->ccdetect_pinctrl, "ccdetect_active");
	...
	chg->ccdetect_sleep =
		pinctrl_lookup_state(chg->ccdetect_pinctrl, "ccdetect_sleep");
...
void oplus_ccdetect_enable(void)
{
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: 111 Couldn't read 0x1368 rc=%d\n",
		       __func__, rc);
	} else {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]:111 reg0x1368[0x%x], bit[2:0]=0(DRP)\n",
		       __func__, stat);
	}
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: Couldn't clear 0x1368[0] rc=%d\n",
		       __func__, rc);
	}
	...
	if (rc < 0) {
		...
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: reg0x1368[0x%x], bit[2:0]=0(DRP)\n",
		       __func__, stat);
	}
}
...
void oplus_ccdetect_disable(void)
{
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: Couldn't set 0x1368[2] rc=%d\n",
		       __func__, rc);
	}
	...
	if (rc < 0) {
		...
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: reg0x1368[0x%x], bit[2:0]=4(UFP)\n",
		       __func__, stat);
	}
}
...
int oplus_ccdetect_get_power_role(void)
{
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: Couldn't get typec power role, rc=%d\n",
		       __func__, rc);
...
static void oplus_ccdetect_irq_register(struct oplus_chg_chip *chip)
{
	...
	ret = devm_request_threaded_irq(
		chip->dev, chg->ccdetect_irq, NULL,
...
void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	if (IS_ERR_OR_NULL(chg->iio.usbtemp_v_chan)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: chg->iio.usbtemp_v_chan  is  NULL !\n",
		       __func__);
...
int oplus_chg_get_battery_btb_temp_cal(void)
{
	...
	if (IS_ERR_OR_NULL(chg->iio.btbtemp_v_chan)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: chg->iio.btbtemp_v_chan  is  NULL !\n",
		       __func__);
		return temp;
	}
...
int usbtemp_get_charger_voltage_now(void)
{
	...
	if (chg->chg_param.smb_version != PM8150B_SUBTYPE) {
		...
		if (!chg->iio.mid_chan ||
		    PTR_ERR(chg->iio.mid_chan) == -EPROBE_DEFER)
			chg->iio.mid_chan =
				iio_channel_get(chg->dev, "mid_voltage");
...
static void oplus_usbtemp_thread_init(void)
{
	...
	if (chg && chg->adc_check_pulse) {
		rc = smblib_masked_write(chg, ADC_CHECK_HIGH_LEVEL_1,
					 BIT(5) | BIT(4), 0);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't configure 0x1282 rc=%d\n",
				rc);
		}
		rc = smblib_masked_write(chg, ADC_CHECK_HIGH_LEVEL_2,
					 BIT(5) | BIT(4), 0);
		if (rc < 0) {
...
void oplus_wake_up_usbtemp_thread(void)
{
	...
	if (chg && chg->adc_check_pulse) {
		...
		if (val != 0) {
			...
			if (rc < 0) {
				dev_err(chg->dev,
					"Couldn't configure 0x3546 rc=%d\n",
					rc);
			}
		}
	}
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	if (g_oplus_chip) {
		...
		g_oplus_chip->normalchg_gpio.chargerid_switch_gpio_pvt =
			of_get_named_gpio(node,
					  "qcom,chargerid_switch-gpio-pvt", 0);
		if (g_oplus_chip->normalchg_gpio.chargerid_switch_gpio_pvt <=
		    0) {
			chg_err("*** Couldn't read chargerid_switch-gpio-pvt ***\n");
		} else {
			if (gpio_is_valid(g_oplus_chip->normalchg_gpio
						  .chargerid_switch_gpio_pvt)) {
				rc = gpio_request(
					g_oplus_chip->normalchg_gpio
						.chargerid_switch_gpio_pvt,
		...
		g_oplus_chip->normalchg_gpio.chargerid_switch_gpio =
			of_get_named_gpio(node, "qcom,chargerid_switch-gpio",
					  0);
		if (g_oplus_chip->normalchg_gpio.chargerid_switch_gpio <= 0) {
			...
			if (gpio_is_valid(g_oplus_chip->normalchg_gpio
				...
				if (rc) {
					...
					smbchg_chargerid_switch_gpio_init(
						g_oplus_chip);
				}
			}
	...
	if (g_oplus_chip) {
		...
		if (chg->shipmode_id_gpio <= 0) {
			...
			if (oplus_shipmode_id_check_is_gpio(g_oplus_chip) ==
			    true) {
				rc = gpio_request(chg->shipmode_id_gpio,
						  "qcom,shipmode-id-gpio");
				if (rc) {
					chg_err("unable to request qcom,shipmode-id-gpio:%d\n",
						chg->shipmode_id_gpio);
				} else {
					oplus_shipmode_id_gpio_init(
						g_oplus_chip);
					if (rc)
	...
	if (chip) {
		...
		if (chg->ccdetect_gpio <= 0) {
			...
			if (oplus_ccdetect_check_is_gpio(chip) == true) {
				rc = gpio_request(chg->ccdetect_gpio,
						  "ccdetect-gpio");
				if (rc) {
	...
	if (chip) {
		chg->wired_conn_gpio =
			of_get_named_gpio(node, "qcom,wired_conn-gpio", 0);
		if (chg->wired_conn_gpio <= 0) {
			...
			if (oplus_wired_conn_check_is_gpio(chip) == true) {
				rc = gpio_request(chg->wired_conn_gpio,
						  "wired_conn-gpio");
				if (rc) {
	...
	if (chip) {
		...
		if (chg->otg_en_gpio <= 0) {
			...
			if (1) { ///oplus_wired_conn_check_is_gpio(chip) == true) {
				rc = gpio_request(chg->otg_en_gpio,
						  "otg_en-gpio");
				if (rc) {
	...
	if (chip) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]:11111 tongfeng test start chg = %p!\n",
		       __func__, chg);
		chg->idt_en_gpio =
			of_get_named_gpio(node, "qcom,idt_en-gpio", 0);
		if (chg->idt_en_gpio <= 0) {
			chg_err("Couldn't read qcom,idt_en-gpio rc=%d, qcom,idt_en-gpio:%d\n",
				rc, chg->idt_en_gpio);
		} else {
			if (1) { //oplus_mps_otg_en_check_is_gpio(chip) == true) {
				rc = gpio_request(chg->idt_en_gpio,
						  "idt_en-gpio");
				if (rc) {
...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	chg->soc_ready_check =
		of_property_read_bool(node, "qcom,soc-ready-check");
	...
	chg->adc_check_pulse =
		of_property_read_bool(node, "qcom,adc-check-pulse");
	chg->low_voltage_charger =
		of_property_read_bool(node, "op,low-voltage-charger");
	...
	g_oplus_chip->usbtemp_chan_tmp =
		of_property_read_bool(node, "qcom,usbtemp_chan_tmp");
...
static int smb5_init_ac_psy(struct smb5 *chip)
{
	...
	chg->ac_psy =
		devm_power_supply_register(chg->dev, &ac_psy_desc, &ac_cfg);
...
static int smb5_configure_iterm_thresholds_adc(struct smb5 *chip)
{
	...
	if (chip->dt.term_current_thresh_hi_ma) {
		...
		if (rc < 0) {
			dev_err(chg->dev,
				"Couldn't configure ITERM threshold HIGH rc=%d\n",
				rc);
			return rc;
		}
	}
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if (rc < 0) {
		dev_err(chg->dev,
			"Couldn't disable PM660 iterm override rc=%d\n", rc);
	}
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (g_oplus_chip &&
	    oplus_ccdetect_check_is_gpio(g_oplus_chip) == true) {
		level = gpio_get_value(chg->ccdetect_gpio);
...
static void dump_regs(void)
{
	...
	int rc;
	u8 stat;
	int base[] = { 0x1000, 0x1100, 0x1200, 0x1300,
		       0x1400, 0x1600, 0x1800, 0x1900 };
	struct smb_charger *chg = NULL;
...
static void smbchg_usbin_collapse_irq_enable(bool enable)
{
	...
	if (enable && !collapse_en) {
		enable_irq(chip->pmic_spmi.smb5_chip->chg
				   .irq_info[USBIN_COLLAPSE_IRQ]
				   .irq);
	} else if (!enable && collapse_en) {
		disable_irq(chip->pmic_spmi.smb5_chip->chg
				    .irq_info[USBIN_COLLAPSE_IRQ]
				    .irq);
	}
	collapse_en = enable;
...
static int oplus_chg_set_input_current(int current_ma)
{
	...
	if (get_client_vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable,
	    ...
	    get_effective_result(
		    chip->pmic_spmi.smb5_chip->chg.usb_icl_votable) <=
		    USBIN_25MA) {
		vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable,
	...
	chg_debug(
		"usb input max current limit=%d setting %02x, pre_current[%d]\n",
	...
	if (pre_current != -1) {
		...
		chg_debug(
			"downTo: usb input max current limit = %d setting %d\n",
			current_ma, n);
		if (usb_icl[n] > 1200) {
			rc = vote(
				chip->pmic_spmi.smb5_chip->chg.usb_icl_votable,
			...
			if (usb_icl[n] >= 1200) {
				rc = vote(chip->pmic_spmi.smb5_chip->chg
						  .usb_icl_votable,
					  USB_PSY_VOTER, true,
					  usb_icl[n] * 1000);
				msleep(90);
...
aicl_suspend:
	rc = vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable, USB_PSY_VOTER,
		  true, usb_icl[i] * 1000);
	chg_debug(
		"usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_suspend\n",
		chg_vol, i, usb_icl[i], aicl_point);
...
aicl_return:
	...
	if (chip->pmic_spmi.smb5_chip->chg.typec_mode ==
		...
		if ((bool)(stat & ICL_OVERRIDE_AFTER_APSD_BIT)) {
			rc = smblib_write(&chip->pmic_spmi.smb5_chip->chg,
					  USBIN_CURRENT_LIMIT_CFG_REG, 0x14);
			if (rc < 0) {
				chg_debug(
					"Couldn't write USBIN_CURRENT_LIMIT_CFG_REG rc=%d\n",
					rc);
			} else {
...
static int smbchg_float_voltage_set(int vfloat_mv)
{
	...
	rc = vote(chip->pmic_spmi.smb5_chip->chg.fv_votable,
		  BATT_PROFILE_VOTER /*DEFAULT_VOTER*/, true, vfloat_mv * 1000);
	if (rc < 0)
...
static int smbchg_charging_enble(void)
{
	...
	rc = vote(chip->pmic_spmi.smb5_chip->chg.chg_disable_votable,
...
static int smbchg_usb_suspend_disable(void)
{
	...
	if (boot_mode == MSM_BOOT_MODE__RF ||
	    boot_mode == MSM_BOOT_MODE__WLAN) {
		chg_err("RF/WLAN, suspending...\n");
		rc = smblib_set_usb_suspend(&chip->pmic_spmi.smb5_chip->chg,
					    true);
		if (rc < 0)
			chg_err("Couldn't write enable to USBIN_SUSPEND_BIT rc=%d\n",
				rc);
...
static int oplus_chg_get_dyna_aicl_result(void)
{
	...
	if (usb_psy) {
		power_supply_get_property(
			usb_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_SETTLED,
			&pval);
...
int oplus_chg_set_pd_config()
{
	...
	if (chip->vbatt_num == 1) {
		...
		printk(KERN_ERR "%s: charger voltage=%d", __func__,
		       usbtemp_get_charger_voltage_now());
...
int oplus_chg_set_qc_config()
{
	...
	if (chip->limits.vbatt_pdqc_to_5v_thr > 0 &&
		...
		if (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY)
			oplus_voocphy_set_pdqc_config();
		smblib_masked_write(
			chg, CMD_HVDCP_2_REG, FORCE_5V_BIT,
		...
		oplus_chg_config_charger_vsys_threshold(
			0x02); //set Vsys Skip threshold 104%
		oplus_chg_enable_burst_mode(false);
		smblib_masked_write(chg, CMD_HVDCP_2_REG, FORCE_9V_BIT,
				    FORCE_9V_BIT);
		msleep(300);
		oplus_chg_unsuspend_charger();
	}
...
static int smb5_show_charger_status(struct smb5 *chip)
{
	...
	batt_health = val.intval;
		...
		if (g_oplus_chip)
			batt_health =
				oplus_chg_get_prop_batt_health(g_oplus_chip);
#endif
...
static void register_oplus_pdsvooc_svid(struct work_struct *work)
{
	...
	if (IS_ERR(pd)) {
		...
		rc = usbpd_register_svid(chg->oplus_pd,
					 &chg->oplus_svid_handler);
		if (rc) {
			//chg->oplus_svid_handler = NULL;
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	if (oplus_chip->vbatt_num == 1) {
		...
		if (oplus_gauge_ic_chip_is_null() ||
	...
	if (rc >= 0) {
		chg->iio.chgid_v_chan =
			iio_channel_get(chg->dev, "chgID_voltage_adc");
		if (IS_ERR(chg->iio.chgid_v_chan)) {
			rc = PTR_ERR(chg->iio.chgid_v_chan);
			if (rc != -EPROBE_DEFER)
				dev_err(chg->dev,
					"chgid_v_chan  get  error, %ld\n", rc);
	...
	rc = of_property_match_string(chg->dev->of_node, "io-channel-names",
				      "usb_temp_adc");
	if (rc >= 0) {
		chg->iio.usbtemp_v_chan =
			iio_channel_get(chg->dev, "usb_temp_adc");
	...
	rc = of_property_match_string(chg->dev->of_node, "io-channel-names",
				      "btb_temp_therm");
	if (rc >= 0) {
		chg->iio.btbtemp_v_chan =
			iio_channel_get(chg->dev, "btb_temp_therm");
		if (IS_ERR(chg->iio.btbtemp_v_chan)) {
			rc = PTR_ERR(chg->iio.btbtemp_v_chan);
			if (rc != -EPROBE_DEFER)
				dev_err(chg->dev,
					"btb_temp_therm  get  error, %ld\n",
					rc);
	...
	if (rc >= 0) {
		chg->iio.usbtemp_sup_v_chan =
			iio_channel_get(chg->dev, "usb_supplementary_temp_adc");
	...
	if (chg->soc_ready_check == true) {
		...
		if (bms_psy == NULL)
			return -EPROBE_DEFER;
		power_supply_get_property(
			bms_psy, POWER_SUPPLY_PROP_SOC_REPORTING_READY, &pval);
		pr_err("%s:ready_flag=%d\n", __func__, pval.intval);
		if (pval.intval == SOC_CHECK_NOT_READY) {
			reporting_not_ready_count++;
			if (reporting_not_ready_count <=
			    SOC_READY_RETRY_COUNT) {
	...
	if (main_psy) {
		...
		power_supply_set_property(
			main_psy, POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX,
			&pval);
	}
#endif
#ifndef OPLUS_FEATURE_CHG_BASIC
...
static void smb5_shutdown(struct platform_device *pdev)
{
	...
	if (g_oplus_chip && g_oplus_chip->enable_shipmode) {
		...
		smblib_masked_write(
			chg, SHIP_MODE_REG, SHIP_MODE_EN_BIT,
