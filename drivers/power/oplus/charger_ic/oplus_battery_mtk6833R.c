...
static void oplus_clear_usb_status(int status)
{
	if (g_oplus_chip->usb_status == USB_TEMP_HIGH) {
		g_oplus_chip->usb_status =
			g_oplus_chip->usb_status & (~USB_TEMP_HIGH);
	}
	usb_status = usb_status & (~status);
}
					    ...
					    int *tchg_max)
{
	...
	if (info != NULL) {
		...
		if (oplus_chg_get_voocphy_support() &&
		    oplus_ntc_ctrl_is_support()) {
			chg_err("chargeric use adc ctrl!\n");
			//oplus_chg_adc_switch_ctrl();
			chargeric_temp_thermal =
				oplus_chg_get_chargeric_temp_cal();
		} else {
			get_chargeric_temp(pdata);
		}

		consumer->support_ntc_01c_precision =
			info->support_ntc_01c_precision;
...
bool is_typec_adapter(struct charger_manager *info)
{
	...
	if (info->pd_type == MTK_PD_CONNECT_TYPEC_ONLY_SNK && rp != 500 &&
	...
	if (is_mtksvooc_project == false) {
		if (mt_get_charger_type() != CHARGER_UNKNOWN) {
			...
			charger_dev_set_input_current(
				...
				500000);
			if (g_oplus_chip->dual_charger_support) {
				charger_manager_notifier(
					pinfo, CHARGER_NOTIFY_STOP_CHARGING);
		...
		if (mt_get_charger_type() != CHARGER_UNKNOWN) {
			...
			if (mtkhv_flashled_pinctrl.hv_flashled_support) {
				...
				if (mt6360_get_vbus_rising() != true) {
					mutex_lock(&mtkhv_flashled_pinctrl
							    .chgvin_mutex);
					pinctrl_select_state(
						mtkhv_flashled_pinctrl.pinctrl,
						mtkhv_flashled_pinctrl
							.chgvin_enable);
					mutex_unlock(&mtkhv_flashled_pinctrl
							      .chgvin_mutex);
		...
		charger_dev_set_input_current(
			g_oplus_chip->chgic_mtk.oplus_info->chg1_dev, 500000);
	...
	if (plug == 1) { /* plug_in */
		if (g_oplus_chip->camera_on) {
			if (is_vooc_support_single_batt_svooc() != true) {
				mutex_lock(
					&mtkhv_flashled_pinctrl.chgvin_mutex);
				pinctrl_select_state(
					mtkhv_flashled_pinctrl.pinctrl,
				...
				chr_err("[%s] camera_on %d\n", __func__,
					g_oplus_chip->camera_on);
				pr_err("[OPLUS_CHG]   %d",
				       gpio_get_value(mtkhv_flashled_pinctrl
							      .chgvin_gpio));
			}
		}
	} else if (plug == 0) {
				...
				struct device *dev)
{
	...
	info->support_ntc_01c_precision =
		of_property_read_bool(np, "qcom,support_ntc_01c_precision");
	chr_debug("%s: support_ntc_01c_precision: %d\n", __func__,
		  info->support_ntc_01c_precision);

	info->usbtemp_lowvbus_detect =
		of_property_read_bool(np, "qcom,usbtemp_lowvbus_detect");
					...
					struct device *dev)
{
	...
	chg_err("step1_time: %d, step1_current: %d, step2_time: %d,step2_current: %d, step3_current: %d,pd_not_support: %d, qc_not_support:%d\n",
		info->data.step1_time, info->data.step1_current_ma,
		info->data.step2_time, info->data.step2_current_ma,
		info->data.step3_current_ma, info->data.pd_not_support,
		info->data.qc_not_support);

	return 0;
}

static ssize_t show_BatNotify(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
...
bool oplus_check_pdphy_ready(void)
{
	return (pinfo != NULL && pinfo->tcpc != NULL &&
		pinfo->tcpc->pd_inited_flag);
}
...
static int oplus_mt6360_charging_current_write_fast(int chg_curr)
{
	...
	if (!g_oplus_chip->authenticate) {
		...
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]:!g_oplus_chip->authenticate , charging_disable\n",
		       __func__);
		return 0;
	}
	...
	if (pinfo->data.dual_charger_support) {
		...
		if (g_oplus_chip->dual_charger_support &&
			...
			main_cur =
				(chg_curr * (100 - g_oplus_chip->slave_pct)) /
				100;
			main_cur -= main_cur % 100;
			slave_cur = chg_curr - main_cur;
			chg_debug(
				"chg_curr [%d],main_cur [%d] slave_cur [%d] \n",
				chg_curr, main_cur, slave_cur);
			rc = charger_dev_set_charging_current(chg,
							      main_cur * 1000);
			if (rc < 0) {
			...
			if (bq25601d_is_detected() == true) {
				rc = bq25601d_charging_current_write_fast(
					slave_cur);
			}
			if (bq2591x_is_detected() == true) {
			...
			if (rc < 0) {
				chg_debug(
					"set sub fast charge current:%d fail\n",
					slave_cur);
			}
		} else {
			if (g_oplus_chip->dual_charger_support &&
		...
		if (rc < 0) {
			...
			chg_debug(
				"set fast charge current:%d ret_chg_curr = %d\n",
...
static void oplus_mt6360_set_aicl_point(int vbatt)
{
	...
	if (!g_oplus_chip->authenticate) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]:!g_oplus_chip->authenticate \n",
		       __func__);
		return;
	}
	...
	if (hw_aicl_point == HW_AICL_POINT_VOL_5V_PHASE1 &&
	    vbatt > AICL_POINT_VOL_5V) {
		hw_aicl_point = HW_AICL_POINT_VOL_5V_PHASE2;
	} else if (hw_aicl_point == HW_AICL_POINT_VOL_5V_PHASE2 &&
		   vbatt <= AICL_POINT_VOL_5V) {
...
static int oplus_mt6360_input_current_limit_write(int value)
{
	...
	if (g_oplus_chip->chg_ops->oplus_chg_get_pd_type) {
		if (g_oplus_chip->chg_ops->oplus_chg_get_pd_type() == true) {
			...
			if (rc >= 0 && ibus_ma >= 500 && ibus_ma < 3000 &&
			    value > ibus_ma) {
				value = ibus_ma;
				chg_debug(
					"usb input max current limit=%d(pd)\n",
					value);
			}
		}
	}
	...
	if (g_oplus_chip->charger_current_pre == value) {
		...
		if (aicl_point != AICL_POINT_VOL_9V &&
		    pinfo->data.dual_charger_support) {
			return rc;
		}
	} else {
...
aicl_pre_step:
	chg_debug(
		"usb input max current limit aicl chg_vol=%d i[%d]=%d sw_aicl_point:%d aicl_pre_step\n",
		chg_vol, i, usb_icl[i], aicl_point);
	if (pinfo->data.dual_charger_support) {
		if (g_oplus_chip->dual_charger_support &&
		    (g_oplus_chip->slave_charger_enable || em_mode)) {
			slave_cur =
				(usb_icl[i] * g_oplus_chip->slave_pct) / 100;
			slave_cur -= slave_cur % 100;
			main_cur = usb_icl[i] - slave_cur;

			chg_debug(
				"usb input max current limit aicl: master and salve input current: %d, %d\n",
				main_cur, slave_cur);
...
aicl_end:
	chg_debug(
		"usb input max current limit aicl chg_vol=%d i[%d]=%d sw_aicl_point:%d aicl_end\n",
		chg_vol, i, usb_icl[i], aicl_point);
	...
	if (pinfo->data.dual_charger_support) {
		if (g_oplus_chip->dual_charger_support &&
			...
			if (rc >= 0) {
				if (bq25601d_is_detected() == true) {
					bq25601d_input_current_limit_write(
						slave_cur);
				}
				if (bq2591x_is_detected() == true) {
					oplus_bq2591x_set_ichg(slave_cur);
...
int mt_power_supply_type_check(void)
{
	...
	if (g_oplus_chip) {
		if ((g_oplus_chip->charger_type != charger_type) &&
		    g_oplus_chip->usb_psy) {
			chg_debug(
				"charger_type[%d] [%d] power_supply_changed.\n",
...
bool oplus_mt_get_vbus_status(void)
{
	...
	if (ret < 0) {
		if (g_oplus_chip && g_oplus_chip->unwakelock_chg == 1 &&
			...
			printk(KERN_ERR
			       "[OPLUS_CHG][%s]: unwakelock_chg=1, use pre status\n",
			       __func__);
...
int mt_get_chargerid_volt(void)
{
	...
	if (is_support_chargerid_check() == true) {
		rc = iio_read_channel_processed(pinfo->charger_id_chan,
						&chargerid_volt);
		if (rc < 0) {
...
int oplus_usb_switch_gpio_gpio_init(void)
{
	...
	chip->normalchg_gpio.charger_gpio_as_output1 = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "charger_gpio_as_output_low");
	if (IS_ERR_OR_NULL(chip->normalchg_gpio.charger_gpio_as_output1)) {
		chg_err("get charger_gpio_as_output_low fail\n");
		return -EINVAL;
	}

	chip->normalchg_gpio.charger_gpio_as_output2 = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "charger_gpio_as_output_high");
...
static int oplus_chg_chargerid_parse_dt(struct oplus_chg_chip *chip)
{
	...
	chargerid_support =
		of_property_read_bool(node, "qcom,chargerid_support");
...
static int oplus_shortc_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chip->normalchg_gpio.shortc_active = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "shortc_active");
...
bool oplus_chg_get_shortc_hw_gpio_status(void)
{
	...
	if (oplus_shortc_check_is_gpio(chip) == true) {
		shortc_hw_status =
			!!(gpio_get_value(chip->normalchg_gpio.shortc_gpio));
	}
	return shortc_hw_status;
}
...
static int oplus_chg_shortc_hw_parse_dt(struct oplus_chg_chip *chip)
{
	...
	if (chip->normalchg_gpio.shortc_gpio <= 0) {
		...
		if (oplus_shortc_check_is_gpio(chip) == true) {
			rc = gpio_request(chip->normalchg_gpio.shortc_gpio,
					  "shortc-gpio");
			if (rc) {
...
static int oplus_chg_shipmode_parse_dt(struct oplus_chg_chip *chip)
{
	...
	chip->disable_ship_mode =
		!of_property_read_bool(node, "qcom,enable-ship-mode");
	chip->normalchg_gpio.ship_gpio =
...
static struct temp_param sub_board_temp_table[] = {
	{ -40, 4397119 }, { -39, 4092874 }, { -38, 3811717 }, { -37, 3551749 },
	{ -36, 3311236 }, { -35, 3088599 }, { -34, 2882396 }, { -33, 2691310 },
	{ -32, 2514137 }, { -31, 2349778 }, { -30, 2197225 }, { -29, 2055558 },
	{ -28, 1923932 }, { -27, 1801573 }, { -26, 1687773 }, { -25, 1581881 },
	{ -24, 1483100 }, { -23, 1391113 }, { -22, 1305413 }, { -21, 1225531 },
	{ -20, 1151037 }, { -19, 1081535 }, { -18, 1016661 }, { -17, 956080 },
	{ -16, 899481 },  { -15, 846579 },  { -14, 797111 },  { -13, 750834 },
	{ -12, 707524 },  { -11, 666972 },  { -10, 628988 },  { -9, 593342 },
	{ -8, 559931 },	  { -7, 528602 },   { -6, 499212 },   { -5, 471632 },
	{ -4, 445772 },	  { -3, 421480 },   { -2, 398652 },   { -1, 377193 },
	{ 24, 1644694 },  { 25, 1637838 },  { 26, 1630827 },  { 27, 1623581 },
	{ 28, 1616120 },  { 29, 1608409 },  { 30, 1600466 },  { 31, 1592292 },
	{ 32, 1583862 },  { 33, 1575169 },  { 34, 1566264 },  { 35, 1557085 },
	{ 36, 1547687 },  { 37, 1538029 },  { 38, 1528097 },  { 39, 1517913 },
	{ 40, 1507507 },  { 41, 1496817 },  { 42, 1485919 },  { 43, 1474738 },
	{ 44, 1463363 },  { 45, 1451703 },  { 46, 1439856 },  { 47, 1427715 },
	{ 48, 1415385 },  { 49, 1402737 },  { 50, 1389977 },  { 51, 1376968 },
	{ 52, 1363742 },  { 53, 1350225 },  { 54, 1336680 },  { 55, 1322800 },
	{ 56, 1308734 },  { 57, 1294524 },  { 58, 1280219 },  { 59, 1265717 },
	{ 60, 1250885 },  { 61, 1236090 },  { 62, 1221222 },  { 63, 1206137 },
	{ 64, 1190863 },  { 65, 1175434 },  { 66, 1160114 },  { 67, 1144501 },
	{ 68, 1129109 },  { 69, 1113239 },  { 70, 1097698 },  { 71, 1082010 },
	{ 72, 1066205 },  { 73, 1050312 },  { 74, 1034368 },  { 75, 1018750 },
	{ 76, 1002834 },  { 77, 986992 },   { 78, 971271 },   { 79, 955326 },
	{ 80, 939579 },	  { 81, 924088 },   { 82, 908470 },   { 83, 893016 },
	{ 84, 877585 },	  { 85, 862256 },   { 86, 847064 },   { 87, 831946 },
	{ 88, 816931 },	  { 89, 802051 },   { 90, 787285 },   { 91, 772661 },
	{ 92, 758152 },	  { 93, 743848 },   { 94, 729655 },   { 95, 715663 },
	{ 96, 701769 },	  { 97, 688065 },   { 98, 674578 },   { 99, 661264 },
	{ 100, 648074 },  { 101, 635102 },  { 102, 622298 },  { 103, 609681 },
	{ 104, 597274 },  { 105, 585015 },  { 106, 572922 },  { 107, 561098 },
	{ 108, 549479 },  { 109, 537993 },  { 110, 526745 },  { 111, 515653 },
	{ 112, 504775 },  { 113, 494100 },  { 114, 483604 },  { 115, 473310 },
	{ 116, 463211 },  { 117, 453299 },  { 118, 443577 },  { 119, 434056 },
	{ 120, 424705 },  { 121, 415555 },  { 122, 406574 },  { 123, 397781 },
	{ 124, 389176 },  { 125, 380744 }
};

static __s16 oplus_ch_thermistor_conver_temp(__s32 res,
					     struct ntc_temp *ntc_param)
{
	int i = 0;
	int asize = 0;
	__s32 res1 = 0, res2 = 0;
	...
	if (res >= ntc_param->pst_temp_table[0].temperature_r) {
		...
		for (i = 0; i < asize; i++) {
			if (res >= ntc_param->pst_temp_table[i].temperature_r) {
				res2 = ntc_param->pst_temp_table[i]
					       .temperature_r;
...
static __s16 oplus_res_to_temp(struct ntc_temp *ntc_param)
{
	...
	dwvcrich = ((__u64)ntc_param->i_tap_over_critical_low *
		    (__u64)ntc_param->i_rap_pull_up_voltage);
	dwvcrich2 = (ntc_param->i_tap_over_critical_low +
		     ntc_param->i_rap_pull_up_r);
	...
	if (ntc_param->ui_dwvolt > ((__u32)dwvcrich)) {
		...
		tres = (ntc_param->i_rap_pull_up_r * ntc_param->ui_dwvolt) /
		       (ntc_param->i_rap_pull_up_voltage -
			ntc_param->ui_dwvolt);
	}
...
int oplus_force_get_subboard_temp(void)
{
	...
	if (!is_param_init) {
		...
		ntc_param.i_table_size = (sizeof(sub_board_temp_table) /
					  sizeof(struct temp_param));
		is_param_init = true;
		...
		chg_err("ntc_type:%d,critical_low:%d,pull_up_r=%d,pull_up_voltage=%d,tap_min=%d,tap_max=%d,table_size=%d\n",
			ntc_param.e_ntc_type, ntc_param.i_tap_over_critical_low,
			ntc_param.i_rap_pull_up_r,
			ntc_param.i_rap_pull_up_voltage, ntc_param.i_tap_min,
			ntc_param.i_tap_max, ntc_param.i_table_size);
	}
	...
	if (!is_param_init) {
		...
		if (g_rap_pull_up_r0 == PULL_UP_R0_390K) {
			...
			ntc_param.i_table_size =
				(sizeof(chargeric_temp_table_390k) /
				 sizeof(struct temp_param));
		} else {
			ntc_param.pst_temp_table = chargeric_temp_table;
			ntc_param.i_table_size = (sizeof(chargeric_temp_table) /
						  sizeof(struct temp_param));
		}
		is_param_init = true;
		chg_err("ntc_type:%d,critical_low:%d,pull_up_r=%d,pull_up_voltage=%d,tap_min=%d,tap_max=%d,table_size=%d\n",
...
int oplus_get_typec_cc_orientation(void)
{
	...
	if (pinfo != NULL && pinfo->tcpc != NULL) {
		if (tcpm_inquire_typec_attach_state(pinfo->tcpc) !=
		    TYPEC_UNATTACHED) {
...
int oplus_ccdetect_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chip->chgic_mtk.oplus_info->ccdetect_active = pinctrl_lookup_state(
		chip->chgic_mtk.oplus_info->pinctrl, "ccdetect_active");
	if (IS_ERR_OR_NULL(chip->chgic_mtk.oplus_info->ccdetect_active)) {
		chg_err("get ccdetect_active fail\n");
		return -EINVAL;
	}

	chip->chgic_mtk.oplus_info->ccdetect_sleep = pinctrl_lookup_state(
		chip->chgic_mtk.oplus_info->pinctrl, "ccdetect_sleep");
	...
	pinctrl_select_state(chip->chgic_mtk.oplus_info->pinctrl,
...
void oplus_ccdetect_irq_init(struct oplus_chg_chip *chip)
{
	...
	chip->chgic_mtk.oplus_info->ccdetect_irq =
		gpio_to_irq(chip->chgic_mtk.oplus_info->ccdetect_gpio);
	printk(KERN_ERR
	       "[OPLUS_CHG][%s]: chip->chgic_mtk.oplus_info->ccdetect_gpio[%d]!\n",
...
void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	rc = iio_read_channel_processed(pinfo->usb_temp_v_l_chan,
					&usbtemp_volt);
	if (rc < 0) {
	...
	rc = iio_read_channel_processed(pinfo->usb_temp_v_r_chan,
					&usbtemp_volt);
	if (rc < 0) {
...
static int oplus_chg_usbtemp_parse_dt(struct oplus_chg_chip *chip)
{
	...
	if (chip->normalchg_gpio.dischg_gpio <= 0) {
		...
		if (oplus_usbtemp_check_is_support() == true) {
			rc = gpio_request(chip->normalchg_gpio.dischg_gpio,
					  "dischg-gpio");
			if (rc) {
...
static int oplus_mtk_hv_flashled_dt(struct oplus_chg_chip *chip)
{
	...
	mtkhv_flashled_pinctrl.hv_flashled_support =
		of_property_read_bool(node, "qcom,hv_flashled_support");
	...
	mtkhv_flashled_pinctrl.chgvin_gpio =
		of_get_named_gpio(node, "qcom,chgvin", 0);
	mtkhv_flashled_pinctrl.pmic_chgfunc_gpio =
		of_get_named_gpio(node, "qcom,pmic_chgfunc", 0);
	if (mtkhv_flashled_pinctrl.chgvin_gpio <= 0 ||
	    mtkhv_flashled_pinctrl.pmic_chgfunc_gpio <= 0) {
		chg_err("read dts fail %d %d\n",
			mtkhv_flashled_pinctrl.chgvin_gpio,
			mtkhv_flashled_pinctrl.pmic_chgfunc_gpio);
	} else {
		if (oplus_mtk_hv_flashled_check_is_gpio() == true) {
			rc = gpio_request(mtkhv_flashled_pinctrl.chgvin_gpio,
					  "chgvin");
			if (rc) {
				...
				mtkhv_flashled_pinctrl.pinctrl =
					devm_pinctrl_get(chip->dev);
				//chgvin
				mtkhv_flashled_pinctrl.chgvin_enable =
					pinctrl_lookup_state(
						mtkhv_flashled_pinctrl.pinctrl,
						"chgvin_enable");
				if (IS_ERR_OR_NULL(mtkhv_flashled_pinctrl
							   .chgvin_enable)) {
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	if (rc) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: oplus_chg_chargerid_parse_dt fail!\n",
		       __func__);
		return -EINVAL;
	}
	...
	if (rc) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: oplus_chg_shipmode_parse_dt fail!\n",
		       __func__);
		return -EINVAL;
	}
	...
	if (rc) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: oplus_chg_shortc_hw_parse_dt fail!\n",
		       __func__);
		return -EINVAL;
	}
	...
	if (rc) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: oplus_chg_usbtemp_parse_dt fail!\n",
		       __func__);
		return -EINVAL;
	}
	...
	if (rc) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: oplus_chg_ccdetect_parse_dt fail!\n",
		       __func__);
	}
	...
	if (rc) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: oplus_mtk_hv_flashled_dt fail!\n",
		       __func__);
		return -EINVAL;
	}
	return rc;
...
static int oplus_power_supply_init(struct oplus_chg_chip *chip)
{
	...
	if (IS_ERR(mt_chg->ac_psy)) {
		dev_err(mt_chg->dev,
			"Failed to register power supply ac: %ld\n",
	...
	if (IS_ERR(mt_chg->usb_psy)) {
		dev_err(mt_chg->dev,
			"Failed to register power supply usb: %ld\n",
	...
	mt_chg->batt_psy =
		power_supply_register(mt_chg->dev, &mt_chg->battery_psd, NULL);
	if (IS_ERR(mt_chg->batt_psy)) {
		dev_err(mt_chg->dev,
			"Failed to register power supply battery: %ld\n",
...
void oplus_set_otg_switch_status(bool value)
{
	if (pinfo != NULL && pinfo->tcpc != NULL) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: otg switch[%d]\n", __func__,
		       value);
		tcpm_typec_change_role(pinfo->tcpc, value ? TYPEC_ROLE_TRY_SNK :
							    TYPEC_ROLE_SNK);
	}
}
...
int oplus_mt6360_pd_setup_forsvooc(void)
{
	...
	printk(KERN_ERR "%s: pd_type %d pd9v svooc [%d %d %d]", __func__,
	       pinfo->pd_type, chip->limits.vbatt_pdqc_to_9v_thr,
	...
	if (!chip->calling_on && !chip->camera_on &&
	    chip->charger_volt < 6500 && chip->soc < 90 &&
		...
		if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
			adapter_dev_get_cap(pinfo->pd_adapter, MTK_PD_APDO,
					    &cap);
			for (i = 0; i < cap.nr; i++) {
				printk(KERN_ERR
				       "PD APDO cap %d: mV:%d,%d mA:%d type:%d pwr_limit:%d pdp:%d\n",
				       i, cap.max_mv[i], cap.min_mv[i],
				       cap.ma[i], cap.type[i], cap.pwr_limit[i],
				       cap.pdp);
			}

			for (i = 0; i < cap.nr; i++) {
				if (cap.min_mv[i] <= VBUS_9V &&
				    VBUS_9V <= cap.max_mv[i]) {
					vbus_mv = VBUS_9V;
					ibus_ma = cap.ma[i];
					if (ibus_ma > IBUS_2A)
						ibus_ma = IBUS_2A;
		...
		printk(KERN_ERR "%s: pd9v svooc  default[%d %d]", __func__,
		       em_mode, chip->batt_volt);
	}
...
int oplus_mt6360_pd_setup_forvooc(void)
{
	...
	printk(KERN_ERR "%s: pd_type %d pd for vooc30_project [%d]", __func__,
	       pinfo->pd_type, chip->batt_volt);
	...
	if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
		...
		for (i = 0; i < cap.nr; i++) {
			if (cap.min_mv[i] <= VBUS_5V &&
			    VBUS_5V <= cap.max_mv[i]) {
				vbus_mv = VBUS_5V;
				ibus_ma = cap.ma[i];
				if (ibus_ma > IBUS_3A)
					ibus_ma = IBUS_3A;
...
int oplus_mt6360_pd_setup(void)
{
	...
	if (is_mtksvooc_project) {
		...
		printk(KERN_ERR "%s: [%d-%d-%d-%d-%d-%d]", __func__,
		...
		if (!chip->calling_on && !chip->camera_on &&
			...
			if (pinfo->pd_type ==
				...
				adapter_dev_get_cap(pinfo->pd_adapter,
						    MTK_PD_APDO, &cap);
				for (i = 0; i < cap.nr; i++) {
					printk(KERN_ERR
					       "PD APDO cap %d: mV:%d,%d mA:%d type:%d pwr_limit:%d pdp:%d\n",
			...
			} else if (pinfo->pd_type ==
					   MTK_PD_CONNECT_PE_READY_SNK ||
				   pinfo->pd_type ==
					   MTK_PD_CONNECT_PE_READY_SNK_PD30) {
				adapter_dev_get_cap(pinfo->pd_adapter, MTK_PD,
						    &cap);
				for (i = 0; i < cap.nr; i++) {
					printk(KERN_ERR
					       "PD cap %d: mV:%d,%d mA:%d type:%d\n",
			...
			if (chip->charger_volt > 7500 &&
			    (chip->calling_on || chip->camera_on ||
			     chip->soc >= 90 || chip->batt_volt >= 4450 ||
			     chip->temperature > 420 ||
			     chip->cool_down_force_5v == true)) {
				vbus_mv = VBUS_5V;
				ibus_ma = IBUS_3A;
...
void oplus_chg_choose_gauge_curve(int index_curve)
{
	...
	printk(KERN_ERR
	       "%s: index_curve() =%d  target_index_curve =%d last_curve_index =%d",
...
bool oplus_chg_check_qchv_condition(void)
{
	...
	if (!chip->calling_on && !chip->camera_on &&
	    ...
	    chip->soc < QC_SOC_HIGH && chip->temperature <= QC_TEMP_HIGH &&
...
int oplus_chg_set_qc_config_forsvooc(void)
{
	...
	printk(KERN_ERR "%s: qc9v svooc [%d %d %d]", __func__,
	...
	if (!chip->calling_on && !chip->camera_on &&
		...
		printk(KERN_ERR "%s: qc9v svooc  default[%d %d]", __func__,
		       em_mode, chip->batt_volt);
	}
		...
		if (chip->tbatt_status == BATTERY_STATUS__NORMAL) {
			...
			if (pinfo->step_status != pinfo->step_status_pre) {
				pr_err("%s, step status: %d, step charging current: %d\n",
				       ...
				       pinfo->step_chg_current);
				oplus_mt6360_charging_current_write_fast(
					pinfo->step_chg_current);
				pinfo->step_status_pre = pinfo->step_status;
			}
		}

		schedule_delayed_work(&pinfo->step_charging_work,
				      msecs_to_jiffies(5000));
	}
	...
	if (!g_oplus_chip) {
		...
		if (g_oplus_chip->dual_charger_support ||
			...
			if (g_oplus_chip->charger_exist) {
				...
				} else if (g_oplus_chip->chg_ops
						   ->get_charger_subtype() ==
					   CHARGER_SUBTYPE_PD) {
					oplus_mt6360_pd_setup();
				} else {
					oplus_chg_set_flash_led_status(
						g_oplus_chip->camera_on);
				}
		...
		if (mtkhv_flashled_pinctrl.hv_flashled_support) {
			...
			if (mtkhv_flashled_pinctrl.bc1_2_done) {
				if (val) {
					mutex_lock(&mtkhv_flashled_pinctrl
							    .chgvin_mutex);
					if (is_vooc_support_single_batt_svooc() !=
					    true) {
						pinctrl_select_state(
							mtkhv_flashled_pinctrl
								.pinctrl,
...
void oplus_set_typec_sinkonly(void)
{
	if (pinfo != NULL && pinfo->tcpc != NULL) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: usbtemp occur otg switch[0]\n",
		       __func__);
...
void sc_select_charging_current(struct charger_manager *info,
				struct charger_data *pdata)
{
...
static int get_battery_btb_temp(void)
{
	...
	if (temp_uv < BATCON_Temperature_Table[pre_temp_idx].temperature_r) {
		for (; pre_temp_idx < TEMPERATURE_TBL_SIZE - 1;
		     pre_temp_idx++) {
			if (temp_uv <= BATCON_Temperature_Table[pre_temp_idx]
					       .temperature_r &&
			    temp_uv > BATCON_Temperature_Table[pre_temp_idx + 1]
					      .temperature_r)
				break;
		}
	} else if (temp_uv >
		   BATCON_Temperature_Table[pre_temp_idx].temperature_r) {
	...
	if ((temp_uv >= BATCON_Temperature_Table[0].temperature_r) &&
	    pre_temp_idx <= 0) {
		real_temp = BATCON_Temperature_Table[0].ntc_temp *
			    OPLUS_BTB_HIGHPRICISION;
	} else if (temp_uv <= BATCON_Temperature_Table[TEMPERATURE_TBL_SIZE - 1]
				      .temperature_r &&
		   pre_temp_idx >= TEMPERATURE_TBL_SIZE - 1) {
		real_temp = BATCON_Temperature_Table[TEMPERATURE_TBL_SIZE - 1]
				    .ntc_temp *
			    OPLUS_BTB_HIGHPRICISION;
	} else {
		uv_l = BATCON_Temperature_Table[pre_temp_idx].temperature_r;
		uv_r = BATCON_Temperature_Table[pre_temp_idx + 1].temperature_r;
		real_temp = BATCON_Temperature_Table[pre_temp_idx].ntc_temp *
				    OPLUS_BTB_HIGHPRICISION +
			    ((temp_uv - uv_l) * OPLUS_BTB_HIGHPRICISION) /
				    (uv_r - uv_l);
	}
...
static int oplus_chg_get_battery_btb_temp(void)
{
	...
	rc = iio_read_channel_processed(pinfo->chargeric_temp_chan,
					&batt_btb_volt);
	if (rc < 0) {
...
static int oplus_get_flashlight_temp_val(void)
{
	...
	if (pinfo && pinfo->batcon_temp_chan) {
		rc = iio_read_channel_processed(pinfo->batcon_temp_chan,
						&temp_uv);
		if (rc < 0) {
...
static void oplus_chg_batcon_adc_switch_ctrl(struct oplus_chg_chip *chip)
{
	...
	if (!oplus_ntc_ctrl_is_support() || !pinfo ||
	    !pinfo->batcon_temp_chan) {
		return;
	}
	...
	if (chip->charger_exist) {
		if (ntcctrl_gpio_value == 0) {
			...
			flashlight_temp_uv_cal =
				oplus_get_flashlight_temp_val();
			mutex_lock(&chip->normalchg_gpio.pinctrl_mutex);
			pinctrl_select_state(chip->normalchg_gpio.pinctrl,
					     chip->normalchg_gpio.ntcctrl_high);
			mutex_unlock(&chip->normalchg_gpio.pinctrl_mutex);
			msleep(100);
			...
			mutex_lock(&chip->normalchg_gpio.pinctrl_mutex);
			pinctrl_select_state(chip->normalchg_gpio.pinctrl,
					     chip->normalchg_gpio.ntcctrl_low);
			mutex_unlock(&chip->normalchg_gpio.pinctrl_mutex);
			msleep(100);
			ret = gpio_get_value(chip->normalchg_gpio.ntcctrl_gpio);
	...
	chg_err("chargeric_temp_cal=%d, battery_btb_temp_cal=%d, flashlight_temp_uv_cal=%d\n",
		chargeric_temp_cal, battery_btb_temp_cal,
		flashlight_temp_uv_cal);
}
...
static int oplus_ntcctrl_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chip->normalchg_gpio.ntcctrl_high = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "ntcctrl_high");
	if (IS_ERR_OR_NULL(chip->normalchg_gpio.ntcctrl_high)) {
		chg_err("get ntcctrl_high fail\n");
		return -EINVAL;
	}

	chip->normalchg_gpio.ntcctrl_low = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "ntcctrl_low");
...
static int oplus_ntc_switch_parse_dt(struct oplus_chg_chip *chip)
{
	...
	chip->normalchg_gpio.ntcctrl_gpio =
		of_get_named_gpio(node, "qcom,ntcctrl-gpio", 0);
	if (chip->normalchg_gpio.ntcctrl_gpio <= 0) {
		...
		if (oplus_ntc_ctrl_is_support()) {
			rc = gpio_request(chip->normalchg_gpio.ntcctrl_gpio,
					  "ntcctrl-gpio");
			if (rc) {
...
void oplus_adc_switch_update_work(struct work_struct *work)
{
	...
	if (!oplus_voocphy_get_fastchg_start()) {
		schedule_delayed_work(&adc_switch_update_work,
				      msecs_to_jiffies(5000));
	} else {
		schedule_delayed_work(&adc_switch_update_work,
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if (oplus_ntc_ctrl_is_support()) {
		INIT_DELAYED_WORK(&adc_switch_update_work,
				  oplus_adc_switch_update_work);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	pinfo->chargeric_temp_chan =
		iio_channel_get(oplus_chip->dev, "auxadc3-chargeric_temp");
	if (IS_ERR(pinfo->chargeric_temp_chan)) {
		chg_err("Couldn't get chargeric_temp_chan...\n");
		pinfo->chargeric_temp_chan = NULL;
	}
	pinfo->subboard_temp_chan =
		iio_channel_get(oplus_chip->dev, "auxadc6-subboard_temp");
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	pinfo->batcon_temp_chan =
		iio_channel_get(oplus_chip->dev, "auxadc10-batcon_temp");
...
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
	pinfo->charger_id_chan =
		iio_channel_get(oplus_chip->dev, "auxadc3-charger_id");
	if (IS_ERR(pinfo->charger_id_chan)) {
		chg_err("Couldn't get charger_id_chan...\n");
		pinfo->charger_id_chan = NULL;
	}

	pinfo->usb_temp_v_l_chan =
		iio_channel_get(oplus_chip->dev, "auxadc4-usb_temp_v_l");
	...
	pinfo->usb_temp_v_r_chan =
		iio_channel_get(oplus_chip->dev, "auxadc5-usb_temp_v_r");
	...
	if (oplus_chip->dual_charger_support) {
		INIT_DELAYED_WORK(&pinfo->step_charging_work,
				  mt6360_step_charging_work);
	}
#endif
	return 0;
}
