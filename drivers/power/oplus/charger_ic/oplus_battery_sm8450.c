...
#define OPLUS_PD_TYPE_CHECK_INTERVAL                                           \
	round_jiffies_relative(msecs_to_jiffies(1500))
#define OPLUS_HVDCP_DISABLE_INTERVAL                                           \
...
static int oplus_chg_track_upload_icl_err_info(struct battery_chg_dev *bcdev,
					       int err_type);
...
{
	return (strncmp(oplus_chg_ops_name_get(), "ext-mp2650", CHG_OPS_LEN) ==
		0);
}
...
static void handle_oem_read_buffer(struct battery_chg_dev *bcdev,
				   struct oem_read_buffer_resp_msg *resp_msg,
				   size_t len)
{
	u32 buf_len;
	...
	memcpy(bcdev->read_buffer_dump.data_buffer, resp_msg->data_buffer,
	       buf_len);
	/*
				   ...
				   size_t len)
{
	...
	memcpy(bcdev->bcc_read_buffer_dump.data_buffer, resp_msg->data_buffer,
	       buf_len);
	...
	bcdev->bcc_read_buffer_dump.data_buffer[8] = DIV_ROUND_CLOSEST(
		(int)bcdev->bcc_read_buffer_dump.data_buffer[8], 1000);
	bcdev->bcc_read_buffer_dump.data_buffer[16] =
		oplus_chg_get_bcc_curr_done_status();
	...
	printk(KERN_ERR
		...
		soc_ext_1[%d], soc_ext_2[%d], atl_last_geat_current[%d], charging_flag[%d], bcc_curr_done[%d]",
	       __func__, bcdev->bcc_read_buffer_dump.data_buffer[0],
	       bcdev->bcc_read_buffer_dump.data_buffer[1],
	       bcdev->bcc_read_buffer_dump.data_buffer[2],
	       bcdev->bcc_read_buffer_dump.data_buffer[3],
	       bcdev->bcc_read_buffer_dump.data_buffer[4],
	       bcdev->bcc_read_buffer_dump.data_buffer[5],
	       bcdev->bcc_read_buffer_dump.data_buffer[6],
	       bcdev->bcc_read_buffer_dump.data_buffer[7],
	       bcdev->bcc_read_buffer_dump.data_buffer[8],
	       bcdev->bcc_read_buffer_dump.data_buffer[9],
	       bcdev->bcc_read_buffer_dump.data_buffer[10],
	       bcdev->bcc_read_buffer_dump.data_buffer[11],
	       bcdev->bcc_read_buffer_dump.data_buffer[12],
	       bcdev->bcc_read_buffer_dump.data_buffer[13],
	       bcdev->bcc_read_buffer_dump.data_buffer[14],
	       bcdev->bcc_read_buffer_dump.data_buffer[15],
	       bcdev->bcc_read_buffer_dump.data_buffer[16]);
	complete(&bcdev->bcc_read_ack);
}

static int adsp_track_battery_chg_write(struct battery_chg_dev *bcdev,
					void *data, int len)
{
	int rc;
...
static void
handle_adsp_track_read_buffer(struct battery_chg_dev *bcdev,
			      struct adsp_track_read_resp_msg *resp_msg,
			      size_t len)
{
	u32 buf_len;
	...
	buf_len = resp_msg->data_size;
	if (buf_len > sizeof(bcdev->adsp_track_read_buffer.data_buffer) ||
	    !buf_len) {
		pr_err("Incorrect buffer length: %u\n", buf_len);
		return;
	}
	...
	memcpy(bcdev->adsp_track_read_buffer.data_buffer, resp_msg->data_buffer,
	       buf_len);
...
static void adsp_track_notification_handler(struct work_struct *work)
{
	struct battery_chg_dev *bcdev = container_of(
		work, struct battery_chg_dev, adsp_track_notify_work.work);
				   ...
				   size_t len)
{
	...
	memcpy(bcdev->pps_read_buffer_dump.data_buffer, resp_msg->data_buffer,
	       buf_len);
...
static int oplus_get_bcc_parameters_from_adsp(char *buf)
{
	...
	for (i = 0; i < BCC_PARMS_COUNT - 1; i++) {
		len = snprintf(tmpbuf, BCC_PAGE_SIZE - idx, "%d,",
			       bcdev->bcc_read_buffer_dump.data_buffer[i]);
...
static int oplus_set_bcc_debug_parameters(const char *buf)
{
	...
	if (strlen(buf) <= BCC_PAGE_SIZE) {
		...
		strncpy(bcc_debug_buf, buf + BCC_DEBUG_PARAM_SIZE,
			BCC_PAGE_SIZE);
...
static void oplus_ccdetect_happened_to_adsp(void)
{
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: write ccdetect plugout fail, rc[%d]\n",
		       __func__, rc);
		return;
	} else {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]:write ccdetect plugout success, rc[%d]\n",
		       __func__, rc);
	}
}
...
void oplus_ccdetect_enable(void)
{
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: 111 Couldn't read 0x2b44 rc=%d\n",
		       __func__, rc);
		return;
	} else {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]:111 reg0x2b44[0x%x], bit[2:0]=0(DRP)\n",
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: Couldn't clear 0x2b44[0] rc=%d\n",
		       __func__, rc);
	}
...
void oplus_typec_disable(void)
{
	...
	rc = write_property_id(bcdev, pst, USB_TYPEC_MODE,
			       TYPEC_PORT_ROLE_DISABLE);
	if (rc < 0) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: Couldn't write 0x2b44[3] rc=%d\n",
		       __func__, rc);
	}
}
...
void oplus_ccdetect_disable(void)
{
	...
	if (rc < 0) {
		...
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]:111 reg0x2b44[0x%x], bit[2:0]=0(UFP)\n",
		       __func__, pst->prop[USB_TYPEC_MODE]);
	}
}
...
void oplus_ccdetect_irq_init(struct oplus_chg_chip *chip)
{
	...
	bcdev->ccdetect_irq =
		gpio_to_irq(bcdev->oplus_custom_gpio.ccdetect_gpio);
...
static void oplus_ccdetect_irq_register(struct oplus_chg_chip *chip)
{
	...
	ret = devm_request_threaded_irq(
		chip->dev, bcdev->ccdetect_irq, NULL,
		oplus_ccdetect_change_handler,
		IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT,
		"ccdetect-change", chip);
	if (ret < 0) {
...
{
	struct battery_chg_dev *bcdev = container_of(
		work, struct battery_chg_dev, hvdcp_disable_work.work);
...
{
	struct battery_chg_dev *bcdev = container_of(
		work, struct battery_chg_dev, pd_type_check_work.work);
...
static void oplus_adsp_voocphy_status_func(struct work_struct *work)
{
	...
	if (!chip) {
		printk(KERN_ERR
		       "!!!chip null, oplus_adsp_voocphy_status_func\n");
		return;
	}
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "!!![adsp_voocphy] read adsp voocphy status fail\n");
		return;
	}
...
{
	struct battery_chg_dev *bcdev = container_of(
		work, struct battery_chg_dev, otg_vbus_enable_work.work);
	struct oplus_chg_chip *chip = g_oplus_chip;
	bool enable = false;
	...
	if (bcdev->otg_boost_src == OTG_BOOST_SOURCE_EXTERNAL) {
		...
		otg_func_ptr = (enable ? chip->chg_ops->otg_enable :
...
void lcm_frequency_ctrl(void)
{
	...
	bcdev = chip->pmic_spmi.bcdev_chip;
	if ((oplus_chg_get_charger_voltage() > 2500) ||
...
static void battery_chg_update_usb_type_work(struct work_struct *work)
{
	...
	struct battery_chg_dev *bcdev =
		container_of(dwork, struct battery_chg_dev, usb_type_work);
#endif
	struct psy_state *pst = &bcdev->psy_list[PSY_TYPE_USB];
	int rc;
	...
	case POWER_SUPPLY_USB_TYPE_PD_DRP:
		...
		if (!bcdev->pd_type_checked) {
			...
			schedule_delayed_work(&bcdev->pd_type_check_work,
					      OPLUS_PD_TYPE_CHECK_INTERVAL);
		}
	...
	if (g_oplus_chip) {
		if (g_oplus_chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP &&
		    ...
		    bcdev->hvdcp_disable == false) {
			usb_psy_desc.type = POWER_SUPPLY_TYPE_USB_DCP;
		} else if (g_oplus_chip->charger_type ==
				...
				size_t len)
{
	...
	case BC_VOOC_VBUS_ADC_ENABLE:
		...
		schedule_delayed_work(&bcdev->adsp_voocphy_err_work,
	...
	case BC_ADSP_NOTIFY_AP_CP_BYPASS_INIT:
		...
		if (g_oplus_chip &&
		    g_oplus_chip->vooc_project == DUAL_BATT_150W)
...
static int battery_chg_callback(void *priv, void *data, size_t len)
{
	...
	if (!bcdev->initialized) {
		pr_debug(
			"Driver initialization failed: Dropping glink callback message: state %d\n",
			bcdev->state);
		return 0;
	}
...
static void oplus_chg_wls_status_keep_clean_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct battery_chg_dev *bcdev = container_of(
		dwork, struct battery_chg_dev, status_keep_clean_work);
	struct oplus_chg_chip *chip = g_oplus_chip;
...
static void
oplus_chg_wls_status_keep_delay_unlock_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct battery_chg_dev *bcdev = container_of(
		dwork, struct battery_chg_dev, status_keep_delay_unlock_work);
			    ...
			    union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		rc = oplus_chg_mod_get_property(
			chip->wls_ocm, OPLUS_CHG_PROP_VOLTAGE_NOW,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		rc = oplus_chg_mod_get_property(
			chip->wls_ocm, OPLUS_CHG_PROP_VOLTAGE_MAX,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		rc = oplus_chg_mod_get_property(
			chip->wls_ocm, OPLUS_CHG_PROP_CURRENT_NOW,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		rc = oplus_chg_mod_get_property(
			chip->wls_ocm, OPLUS_CHG_PROP_CURRENT_MAX,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_PRESENT:
		rc = oplus_chg_mod_get_property(
			chip->wls_ocm, OPLUS_CHG_PROP_PRESENT,
			(union oplus_chg_mod_propval *)pval);
		break;
	default:
			    ...
			    union power_supply_propval *pval)
{
	...
	if (USB_ONLINE == prop_id) {
		...
		if (bcdev->usb_in_status == 1 &&
		    oplus_chg_wls_is_present(g_oplus_chip) == true) {
			pval->intval = 0;
...
int oplus_get_typec_cc_orientation(void)
{
	...
	if (rc < 0) {
		printk(KERN_ERR
		       "!!![OPLUS_CHG] read typec_cc_orientation fail\n");
		return 0;
	}
...
int oplus_get_otg_switch_status(void)
{
	...
	if (get_otg_scheme(chip) != OTG_SCHEME_CCDETECT_GPIO) {
		...
		if (rc < 0) {
			printk(KERN_ERR
			       "!!![OPLUS_CHG] read otg_switch_status fail\n");
			return 0;
		}
...
int oplus_get_fast_chg_type(void)
{
	...
	if (is_wls_ocm_available(chip) && (fast_chg_type == 0) &&
	    (oplus_get_wired_chg_present() == false)) {
		rc = oplus_chg_mod_get_property(chip->wls_ocm,
				...
				union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_HEALTH:
		...
		if (is_comm_ocm_available(chip) && wls_online)
			batt_health =
				oplus_chg_comm_get_batt_health(chip->comm_ocm);
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		...
		break;
	case POWER_SUPPLY_PROP_CAPACITY:
		if (chip->vooc_show_ui_soc_decimal == true &&
	...
	case POWER_SUPPLY_PROP_TEMP:
		if (oplus_switching_support_parallel_chg()) {
			if ((chip->tbatt_temp > PARALLEL_SWITCH_HIGH_TEMP) ||
				...
				pval->intval =
					chip->tbatt_temp - chip->offset_temp;
			}
		} else {
				...
				const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (g_oplus_chip && g_oplus_chip->smart_charging_screenoff) {
			oplus_smart_charge_by_shell_temp(g_oplus_chip,
							 pval->intval);
			break;
		} else {
			return -EINVAL;
...
static int wireless_fw_update(struct battery_chg_dev *bcdev, bool force)
{
	...
	rc = wait_for_completion_timeout(
		&bcdev->fw_update_ack, msecs_to_jiffies(WLS_FW_UPDATE_TIME_MS));
	if (!rc) {
				  ...
				  const char *buf, size_t count)
{
	...
	rc = __battery_psy_set_charge_current(
		bcdev, bcdev->restrict_chg_en ? bcdev->restrict_fcc_ua :
...
static int oplus_otg_boost_en_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	bcdev->oplus_custom_gpio.otg_boost_en_pinctrl =
		devm_pinctrl_get(bcdev->dev);
	if (IS_ERR_OR_NULL(bcdev->oplus_custom_gpio.otg_boost_en_pinctrl)) {
		chg_err("get otg_boost_en_pinctrl fail\n");
		return -EINVAL;
	}
	bcdev->oplus_custom_gpio.otg_boost_en_active = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.otg_boost_en_pinctrl,
		"otg_booster_en_active");
	if (IS_ERR_OR_NULL(bcdev->oplus_custom_gpio.otg_boost_en_active)) {
		chg_err("get otg_boost_en_active\n");
		return -EINVAL;
	}
	bcdev->oplus_custom_gpio.otg_boost_en_sleep = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.otg_boost_en_pinctrl,
		"otg_booster_en_sleep");
...
static int oplus_otg_ovp_en_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	bcdev->oplus_custom_gpio.otg_ovp_en_pinctrl =
		devm_pinctrl_get(bcdev->dev);
	if (IS_ERR_OR_NULL(bcdev->oplus_custom_gpio.otg_ovp_en_pinctrl)) {
		chg_err("get otg_ovp_en_pinctrl fail\n");
		return -EINVAL;
	}
	bcdev->oplus_custom_gpio.otg_ovp_en_active = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.otg_ovp_en_pinctrl,
		"otg_ovp_en_active");
	...
	bcdev->oplus_custom_gpio.otg_ovp_en_sleep = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.otg_ovp_en_pinctrl,
		"otg_ovp_en_sleep");
...
static void oplus_set_otg_boost_en_val(int value)
{
	...
	if (value) {
		gpio_direction_output(
			bcdev->oplus_custom_gpio.otg_boost_en_gpio, 1);
		pinctrl_select_state(
			bcdev->oplus_custom_gpio.otg_boost_en_pinctrl,
			bcdev->oplus_custom_gpio.otg_boost_en_active);
	} else {
		gpio_direction_output(
			bcdev->oplus_custom_gpio.otg_boost_en_gpio, 0);
		pinctrl_select_state(
			bcdev->oplus_custom_gpio.otg_boost_en_pinctrl,
			bcdev->oplus_custom_gpio.otg_boost_en_sleep);
	}
...
static void oplus_set_otg_ovp_en_val(int value)
{
	...
	if (value) {
		gpio_direction_output(bcdev->oplus_custom_gpio.otg_ovp_en_gpio,
				      1);
		pinctrl_select_state(
			bcdev->oplus_custom_gpio.otg_ovp_en_pinctrl,
...
static int oplus_tx_boost_en_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	bcdev->oplus_custom_gpio.tx_boost_en_pinctrl =
		devm_pinctrl_get(bcdev->dev);
	if (IS_ERR_OR_NULL(bcdev->oplus_custom_gpio.tx_boost_en_pinctrl)) {
		chg_err("get tx_boost_en_pinctrl fail\n");
		return -EINVAL;
	}
	bcdev->oplus_custom_gpio.tx_boost_en_active = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.tx_boost_en_pinctrl,
		"tx_boost_en_active");
	...
	bcdev->oplus_custom_gpio.tx_boost_en_sleep = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.tx_boost_en_pinctrl,
		"tx_boost_en_sleep");
...
static void oplus_set_tx_boost_en_val(int value)
{
	...
	if (value) {
		gpio_direction_output(bcdev->oplus_custom_gpio.tx_boost_en_gpio,
				      1);
		pinctrl_select_state(
			bcdev->oplus_custom_gpio.tx_boost_en_pinctrl,
...
static int oplus_tx_ovp_en_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	bcdev->oplus_custom_gpio.tx_ovp_en_pinctrl =
		devm_pinctrl_get(bcdev->dev);
	if (IS_ERR_OR_NULL(bcdev->oplus_custom_gpio.tx_ovp_en_pinctrl)) {
		chg_err("get tx_ovp_en_pinctrl fail\n");
		return -EINVAL;
	}
	bcdev->oplus_custom_gpio.tx_ovp_en_active = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.tx_ovp_en_pinctrl, "tx_ovp_en_active");
	if (IS_ERR_OR_NULL(bcdev->oplus_custom_gpio.tx_ovp_en_active)) {
		chg_err("get tx_ovp_en_active fail\n");
		return -EINVAL;
	}
	bcdev->oplus_custom_gpio.tx_ovp_en_sleep = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.tx_ovp_en_pinctrl, "tx_ovp_en_sleep");
...
static void oplus_set_tx_ovp_en_val(int value)
{
	...
	if (value) {
		gpio_direction_output(bcdev->oplus_custom_gpio.tx_ovp_en_gpio,
				      1);
		pinctrl_select_state(bcdev->oplus_custom_gpio.tx_ovp_en_pinctrl,
...
static int oplus_wrx_ovp_off_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	bcdev->oplus_custom_gpio.wrx_ovp_off_pinctrl =
		devm_pinctrl_get(bcdev->dev);
	if (IS_ERR_OR_NULL(bcdev->oplus_custom_gpio.wrx_ovp_off_pinctrl)) {
		chg_err("get wrx_ovp_off_pinctrl fail\n");
		return -EINVAL;
	}
	bcdev->oplus_custom_gpio.wrx_ovp_off_active = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.wrx_ovp_off_pinctrl,
		"wrx_ovp_off_active");
	...
	bcdev->oplus_custom_gpio.wrx_ovp_off_sleep = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.wrx_ovp_off_pinctrl,
		"wrx_ovp_off_sleep");
...
static void oplus_set_wrx_ovp_off_val(int value)
{
	...
	if (value) {
		gpio_direction_output(bcdev->oplus_custom_gpio.wrx_ovp_off_gpio,
				      1);
		pinctrl_select_state(
			bcdev->oplus_custom_gpio.wrx_ovp_off_pinctrl,
...
static int oplus_chg_2uart_pinctrl_init(struct oplus_chg_chip *chip)
{
	...
	chg_2uart_active = pinctrl_lookup_state(chg_2uart_pinctrl,
						"chg_qupv3_se3_2uart_active");
	if (IS_ERR_OR_NULL(chg_2uart_active)) {
		chg_err("get chg_qupv3_se3_2uart_active fail\n");
		return -EINVAL;
	}

	chg_2uart_sleep = pinctrl_lookup_state(chg_2uart_pinctrl,
					       "chg_qupv3_se3_2uart_sleep");
...
static int oplus_vchg_trig_gpio_init(struct battery_chg_dev *bcdev)
{
	...
	bcdev->oplus_custom_gpio.vchg_trig_pinctrl =
		devm_pinctrl_get(bcdev->dev);

	bcdev->oplus_custom_gpio.vchg_trig_default =
		pinctrl_lookup_state(bcdev->oplus_custom_gpio.vchg_trig_pinctrl,
				     "vchg_trig_default");
...
static void oplus_vchg_trig_irq_init(struct battery_chg_dev *bcdev)
{
	...
	bcdev->vchg_trig_irq =
		gpio_to_irq(bcdev->oplus_custom_gpio.vchg_trig_gpio);
...
irqreturn_t oplus_vchg_trig_change_handler(int irq, void *data)
{
	...
	printk(KERN_ERR "[OPPO_CHG][%s]: scheduling vchg_trig work!\n",
	       __func__);
	schedule_delayed_work(&bcdev->vchg_trig_work,
			      msecs_to_jiffies(VCHG_TRIG_DELAY_MS));
...
static void oplus_vchg_trig_irq_register(struct battery_chg_dev *bcdev)
{
	...
	ret = devm_request_threaded_irq(
		bcdev->dev, bcdev->vchg_trig_irq, NULL,
		oplus_vchg_trig_change_handler,
		IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT,
		"vchg_trig_change", bcdev);
	if (ret < 0) {
...
static int oplus_ship_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chip->normalchg_gpio.ship_active = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "ship_active");
	if (IS_ERR_OR_NULL(chip->normalchg_gpio.ship_active)) {
		chg_err("get ship_active fail\n");
		return -EINVAL;
	}

	chip->normalchg_gpio.ship_sleep = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "ship_sleep");
...
static void smbchg_enter_shipmode(struct oplus_chg_chip *chip)
{
	...
	if (oplus_ship_check_is_gpio(chip) == true) {
		...
		mutex_lock(&chip->pmic_spmi.bcdev_chip->oplus_custom_gpio
				    .pinctrl_mutex);
		if (!IS_ERR_OR_NULL(chip->normalchg_gpio.ship_sleep)) {
			pinctrl_select_state(chip->normalchg_gpio.pinctrl,
					     chip->normalchg_gpio.ship_sleep);
		...
		mutex_unlock(&chip->pmic_spmi.bcdev_chip->oplus_custom_gpio
				      .pinctrl_mutex);
		chg_debug("power off after 15s\n");
	} else {
...
static int smbchg_chargerid_switch_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chip->normalchg_gpio.chargerid_switch_active = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "chargerid_switch_active");
	if (IS_ERR_OR_NULL(chip->normalchg_gpio.chargerid_switch_active)) {
		chg_err("get chargerid_switch_active fail\n");
		return -EINVAL;
	}

	chip->normalchg_gpio.chargerid_switch_sleep = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "chargerid_switch_sleep");
	if (IS_ERR_OR_NULL(chip->normalchg_gpio.chargerid_switch_sleep)) {
		chg_err("get chargerid_switch_sleep fail\n");
		return -EINVAL;
	}

	chip->normalchg_gpio.chargerid_switch_default = pinctrl_lookup_state(
		chip->normalchg_gpio.pinctrl, "chargerid_switch_default");
	...
	if (chip->normalchg_gpio.chargerid_switch_gpio > 0) {
		gpio_direction_output(
			chip->normalchg_gpio.chargerid_switch_gpio, 0);
	}
	pinctrl_select_state(chip->normalchg_gpio.pinctrl,
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	if (chip->normalchg_gpio.ship_gpio <= 0) {
		...
		if (oplus_ship_check_is_gpio(chip) == true) {
			rc = gpio_request(chip->normalchg_gpio.ship_gpio,
					  "ship-gpio");
			if (rc) {
	...
	bcdev->oplus_custom_gpio.ccdetect_gpio =
		of_get_named_gpio(node, "qcom,ccdetect-gpio", 0);
	if (bcdev->oplus_custom_gpio.ccdetect_gpio <= 0) {
	...
	if (bcdev->oplus_custom_gpio.otg_boost_en_gpio <= 0) {
		...
		if (gpio_is_valid(bcdev->oplus_custom_gpio.otg_boost_en_gpio) ==
		    true) {
			rc = gpio_request(
				bcdev->oplus_custom_gpio.otg_boost_en_gpio,
				"otg-boost-en-gpio");
	...
	if (bcdev->oplus_custom_gpio.otg_ovp_en_gpio <= 0) {
		...
		if (gpio_is_valid(bcdev->oplus_custom_gpio.otg_ovp_en_gpio) ==
		    true) {
			rc = gpio_request(
				bcdev->oplus_custom_gpio.otg_ovp_en_gpio,
				"otg-ovp-en-gpio");
	...
	bcdev->oplus_custom_gpio.tx_boost_en_gpio =
		of_get_named_gpio(node, "qcom,tx_boost_en-gpio", 0);
	if (bcdev->oplus_custom_gpio.tx_boost_en_gpio <= 0) {
	...
	bcdev->oplus_custom_gpio.tx_ovp_en_gpio =
		of_get_named_gpio(node, "qcom,tx_ovp_en-gpio", 0);
	if (bcdev->oplus_custom_gpio.tx_ovp_en_gpio <= 0) {
	...
	bcdev->oplus_custom_gpio.wrx_ovp_off_gpio =
		of_get_named_gpio(node, "qcom,wrx_ovp_off-gpio", 0);
	if (bcdev->oplus_custom_gpio.wrx_ovp_off_gpio <= 0) {
...
int oplus_chg_get_battery_btb_temp_cal(void)
{
	...
	if (!IS_ERR_OR_NULL(bcdev->iio.batt0_con_btb_chan)) {
		rc = iio_read_channel_processed(bcdev->iio.batt0_con_btb_chan,
						&temp);
		if (rc < 0) {
	...
	if (!IS_ERR_OR_NULL(bcdev->iio.batt1_con_btb_chan)) {
		rc = iio_read_channel_processed(bcdev->iio.batt1_con_btb_chan,
						&temp);
		if (rc < 0) {
			chg_err("[OPLUS_CHG] iio_read_channel_processed get error\n");
		} else {
	...
	return batt0_con_btb_temp > batt1_con_btb_temp ? batt0_con_btb_temp :
							 batt1_con_btb_temp;
}
...
int oplus_chg_get_usb_btb_temp_cal(void)
{
	...
	if (IS_ERR_OR_NULL(bcdev->iio.usbcon_btb_chan)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: bcdev->iio.usbcon_btb_chan  is  NULL !\n",
		       __func__);
		return temp;
	}
...
static int oplus_usbtemp_iio_init(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_match_string(bcdev->dev->of_node, "io-channel-names",
				      "usb_temp_adc");
	if (rc >= 0) {
		bcdev->iio.usbtemp_v_chan =
			iio_channel_get(bcdev->dev, "usb_temp_adc");
		if (IS_ERR(bcdev->iio.usbtemp_v_chan)) {
			rc = PTR_ERR(bcdev->iio.usbtemp_v_chan);
			if (rc != -EPROBE_DEFER)
				dev_err(bcdev->dev,
					"usb_temp_adc  get  error, %ld\n", rc);
	...
	if (rc >= 0) {
		bcdev->iio.usbtemp_sup_v_chan = iio_channel_get(
			bcdev->dev, "usb_supplementary_temp_adc");
	...
	rc = of_property_match_string(bcdev->dev->of_node, "io-channel-names",
				      "batt0_con_therm_adc");
	if (rc >= 0) {
		bcdev->iio.batt0_con_btb_chan =
			iio_channel_get(bcdev->dev, "batt0_con_therm_adc");
		if (IS_ERR(bcdev->iio.batt0_con_btb_chan)) {
			rc = PTR_ERR(bcdev->iio.batt0_con_btb_chan);
			if (rc != -EPROBE_DEFER) {
				dev_err(bcdev->dev,
					"batt0_con_btb_chan  get error, %ld\n",
					rc);
	...
	rc = of_property_match_string(bcdev->dev->of_node, "io-channel-names",
				      "batt1_con_therm_adc");
	if (rc >= 0) {
		bcdev->iio.batt1_con_btb_chan =
			iio_channel_get(bcdev->dev, "batt1_con_therm_adc");
		if (IS_ERR(bcdev->iio.batt1_con_btb_chan)) {
			rc = PTR_ERR(bcdev->iio.batt1_con_btb_chan);
			if (rc != -EPROBE_DEFER) {
				dev_err(bcdev->dev,
					"batt1_con_btb_chan  get error, %ld\n",
					rc);
	...
	rc = of_property_match_string(bcdev->dev->of_node, "io-channel-names",
				      "conn_therm");
	if (rc >= 0) {
		bcdev->iio.usbcon_btb_chan =
			iio_channel_get(bcdev->dev, "conn_therm");
		if (IS_ERR(bcdev->iio.usbcon_btb_chan)) {
			rc = PTR_ERR(bcdev->iio.usbcon_btb_chan);
			if (rc != -EPROBE_DEFER) {
				dev_err(bcdev->dev,
					"usbcon_btb_chan  get error, %ld\n",
					rc);
...
static int oplus_chg_track_upload_usbtemp_info(struct oplus_chg_chip *chip,
					       int condition,
					       int last_usb_temp_l,
					       int last_usb_temp_r,
					       int batt_current)
{
	...
	memset(chip->usbtemp_load_trigger.crux_info, 0,
	       sizeof(chip->usbtemp_load_trigger.crux_info));
	oplus_chg_track_obtain_power_info(chip->chg_power_info,
					  sizeof(chip->chg_power_info));
	if (condition == USBTEMP_TRIGGER_CONDITION_1) {
		index +=
			snprintf(&(chip->usbtemp_load_trigger.crux_info[index]),
		...
		index +=
			snprintf(&(chip->usbtemp_load_trigger.crux_info[index]),
				 ...
				 chip->tbatt_temp, chip->usb_temp_l,
				 last_usb_temp_l, chip->usb_temp_r,
				 last_usb_temp_r);
...
static int oplus_subboard_temp_iio_init(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_match_string(bcdev->dev->of_node, "io-channel-names",
				      "subboard_temp_adc");
	if (rc >= 0) {
		bcdev->iio.subboard_temp_v_chan =
			iio_channel_get(bcdev->dev, "subboard_temp_adc");
		if (IS_ERR(bcdev->iio.subboard_temp_v_chan)) {
			rc = PTR_ERR(bcdev->iio.subboard_temp_v_chan);
			if (rc != -EPROBE_DEFER)
				dev_err(bcdev->dev,
					"subboard_temp_v_chan  get  error, %ld\n",
					rc);
...
void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	if (IS_ERR_OR_NULL(bcdev->iio.usbtemp_v_chan)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: bcdev->iio.usbtemp_v_chan  is  NULL !\n",
		       __func__);
		chip->usbtemp_volt_l = usbtemp_volt_l_pre;
		goto usbtemp_next;
	}

	rc = iio_read_channel_processed(bcdev->iio.usbtemp_v_chan,
					&usbtemp_volt);
	if (rc < 0) {
		chg_err("[OPLUS_CHG][%s]: iio_read_channel_processed  get error\n",
			__func__);
...
usbtemp_next:
	...
	if (IS_ERR_OR_NULL(bcdev->iio.usbtemp_sup_v_chan)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: chg->iio.usbtemp_sup_v_chan  is  NULL !\n",
		       __func__);
		chip->usbtemp_volt_r = usbtemp_volt_r_pre;
		return;
	}

	rc = iio_read_channel_processed(bcdev->iio.usbtemp_sup_v_chan,
					&usbtemp_volt);
	if (rc < 0) {
		chg_err("[OPLUS_CHG][%s]: iio_read_channel_processed  get error\n",
			__func__);
...
static int oplus_get_subboard_temp(void)
{
	...
	if (IS_ERR_OR_NULL(bcdev->iio.subboard_temp_v_chan)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: bcdev->iio.subboard_temp_v_chan  is  NULL !\n",
		       __func__);
		subboard_temp = subboard_temp_pre;
		goto exit;
	}

	rc = iio_read_channel_processed(bcdev->iio.subboard_temp_v_chan,
					&subboard_temp_volt);
	if (rc < 0) {
		chg_err("[OPLUS_CHG][%s]: iio_read_channel_processed  get error\n",
			__func__);
	...
	resistance_convert_temperature_855(subboard_temp_volt, subboard_temp, i,
					   con_temp_volt_855);
...
static void oplus_usbtemp_recover_tbatt_temp(struct oplus_chg_chip *chip)
{
	...
	if (oplus_pps_get_support_type() == PPS_SUPPORT_2CP ||
		...
		g_tbatt_temp =
			batt_temp > sub_batt_temp ? batt_temp : sub_batt_temp;
	} else {
	...
	if ((chg_type == ADAPTER_ID_20W_0X13 ||
	     chg_type == ADAPTER_ID_20W_0X34 ||
	     chg_type == ADAPTER_ID_20W_0X45) &&
	    chip->usbtemp_change_gap) { /*20W*/
		chip->usbtemp_batttemp_gap = USBTEMP_BATTTEMP_GAP_HIGH;
		chip->usbtemp_batttemp_current_gap =
			USBTEMP_BATTTEMP_CURRENT_GAP_HIGH;
		chip->usbtemp_max_temp_thr = USBTEMP_MAX_TEMP_THR_HIGH;
		chip->usbtemp_max_temp_diff = USBTEMP_MAX_TEMP_DIFF_HIGH;
		chip->usbtemp_batttemp_recover_gap =
			USBTEMP_BATTTEMP_RECOVER_GAP_HIGH;
	} else if ((chg_type == ADAPTER_ID_30W_0X19 ||
		    chg_type == ADAPTER_ID_30W_0X29 ||
		    chg_type == ADAPTER_ID_30W_0X41 ||
		    chg_type == ADAPTER_ID_30W_0X42 ||
		    chg_type == ADAPTER_ID_30W_0X43 ||
		    chg_type == ADAPTER_ID_30W_0X44 ||
		...
		chip->usbtemp_batttemp_recover_gap =
			USBTEMP_BATTTEMP_RECOVER_GAP_HIGH;
	} else if ((chg_type == ADAPTER_ID_33W_0X49 ||
		    chg_type == ADAPTER_ID_33W_0X4A ||
		...
		chip->usbtemp_batttemp_recover_gap =
			USBTEMP_BATTTEMP_RECOVER_GAP_HIGH;
	} else if ((chg_type == ADAPTER_ID_50W_0X11 ||
		    chg_type == ADAPTER_ID_50W_0X12 ||
		    chg_type == ADAPTER_ID_50W_0X21 ||
		    chg_type == ADAPTER_ID_50W_0X31 ||
		    chg_type == ADAPTER_ID_50W_0X33 ||
		    chg_type == ADAPTER_ID_50W_0X62) &&
		...
		chip->usbtemp_batttemp_recover_gap =
			USBTEMP_BATTTEMP_RECOVER_GAP_HIGH;
	} else if ((chg_type == ADAPTER_ID_65W_0X14 ||
		    chg_type == ADAPTER_ID_65W_0X35 ||
		    chg_type == ADAPTER_ID_65W_0X63 ||
		    chg_type == ADAPTER_ID_65W_0X66 ||
		...
		chip->usbtemp_batttemp_recover_gap =
			USBTEMP_BATTTEMP_RECOVER_GAP_HIGH;
	} else if ((chg_type == ADAPTER_ID_66W_0X36 ||
		    chg_type == ADAPTER_ID_66W_0X64) &&
		...
		chip->usbtemp_batttemp_recover_gap =
			USBTEMP_BATTTEMP_RECOVER_GAP_HIGH;
	} else if ((chg_type == ADAPTER_ID_67W_0X6C ||
		    chg_type == ADAPTER_ID_67W_0X6D) &&
		...
		chip->usbtemp_batttemp_recover_gap =
			USBTEMP_BATTTEMP_RECOVER_GAP_HIGH;
	} else if ((chg_type == ADAPTER_ID_80W_0X4B ||
		    chg_type == ADAPTER_ID_80W_0X4C ||
		    chg_type == ADAPTER_ID_80W_0X4D ||
		    chg_type == ADAPTER_ID_80W_0X4E ||
		...
		chip->usbtemp_batttemp_recover_gap =
			USBTEMP_BATTTEMP_RECOVER_GAP_HIGH;
	} else if ((chg_type == ADAPTER_ID_100W_0X69 ||
		    chg_type == ADAPTER_ID_100W_0X6A ||
		    chg_type == ADAPTER_ID_120W_0X32 ||
		    chg_type == ADAPTER_ID_120W_0X6B) &&
...
static void oplus_usbtemp_recover_tbatt_temp_100w(struct oplus_chg_chip *chip)
{
	...
	if ((chg_type == ADAPTER_ID_20W_0X13 ||
		...
		chip->usbtemp_batttemp_recover_gap =
			USBTEMP_BATTTEMP_RECOVER_GAP_DEFAULT;
		usbtemp_batttemp_current_thr =
			USBTEMP_BATTTEMP_CURRENT_THR_DEFAULT_100W;
		usbtemp_batttemp_recover_thr =
			USBTEMP_BATTTEMP_RECOVER_THR_DEFAULT_100W;
	} else if ((chg_type == ADAPTER_ID_30W_0X19 ||
		    chg_type == ADAPTER_ID_30W_0X29 ||
		    chg_type == ADAPTER_ID_30W_0X41 ||
		    chg_type == ADAPTER_ID_30W_0X42 ||
		...
		chip->usbtemp_batttemp_current_gap =
			USBTEMP_BATTTEMP_CURRENT_GAP_HIGH_100W;
		chip->usbtemp_max_temp_thr = USBTEMP_MAX_TEMP_THR_HIGH_100W;
		chip->usbtemp_max_temp_diff = USBTEMP_MAX_TEMP_DIFF_HIGH_100W;
		chip->usbtemp_batttemp_recover_gap =
			USBTEMP_BATTTEMP_RECOVER_GAP_HIGH_100W;
		usbtemp_batttemp_current_thr =
			USBTEMP_BATTTEMP_CURRENT_THR_HIGH_100W;
		usbtemp_batttemp_recover_thr =
			USBTEMP_BATTTEMP_RECOVER_THR_HIGH_100W;
	} else if ((chg_type == ADAPTER_ID_100W_0X69 ||
		    chg_type == ADAPTER_ID_100W_0X6A ||
		    chg_type == ADAPTER_ID_120W_0X32 ||
		    chg_type == ADAPTER_ID_120W_0X6B) &&
...
static int oplus_usbtemp_dischg_action(struct oplus_chg_chip *chip)
{
	...
	if ((get_eng_version() != HIGH_TEMP_AGING) &&
		...
		if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY) {
			rc = write_property_id(bcdev, pst, USB_VOOCPHY_ENABLE,
					       false);
			if (rc < 0) {
				printk(KERN_ERR
				       "!!![OPLUS_USBTEMP] write utemp high action fail\n");
				return rc;
			}
		} else {
			if (oplus_vooc_get_fastchg_started() == true) {
		...
		if (rc < 0) {
			printk(KERN_ERR
			       "!!![OPLUS_USBTEMP] write usb typec sinkonly fail\n");
			return rc;
		}
	}
...
static void oplus_update_usbtemp_current_status(struct oplus_chg_chip *chip)
{
	...
	if ((chip->usb_temp_r - g_tbatt_temp / 10) >=
	    chip->usbtemp_batttemp_current_gap) {
		limit_cur_cnt_r++;
		...
		recover_cur_cnt_r = 0;
	} else if ((chip->usb_temp_r - g_tbatt_temp / 10) <=
		   chip->usbtemp_batttemp_recover_gap) {
		recover_cur_cnt_r++;
	...
	if ((chip->usb_temp_l - g_tbatt_temp / 10) >=
		...
		recover_cur_cnt_l = 0;
	} else if ((chip->usb_temp_l - g_tbatt_temp / 10) <=
		   chip->usbtemp_batttemp_recover_gap) {
		recover_cur_cnt_l++;
	...
	if ((RETRY_COUNT <= limit_cur_cnt_r ||
		...
		chg_err("use usbtemp cooldown g_tbatt_temp:%d, usb_temp_l:%d, usb_temp_r:%d, usbtemp_batttemp_current_gap:%d\n",
			g_tbatt_temp, chip->usb_temp_l, chip->usb_temp_r,
			chip->usbtemp_batttemp_current_gap);
...
static void
oplus_update_usbtemp_current_status_100w(struct oplus_chg_chip *chip)
{
	static int limit_cur_cnt_r = 0;
	static int limit_cur_cnt_l = 0;
	static int recover_cur_cnt_r = 0;
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if ((chip->usb_temp_l < USB_50C) &&
		    (chip->usb_temp_r <
		     USB_50C)) { /*get vbus when usbtemp < 50C*/
			vbus_volt =
				get_battery_mvolts_for_usbtemp_monitor(chip);
		} else {
			vbus_volt = 0;
		}
		...
		if (g_tbatt_temp / 10 <= USB_50C &&
		    (((chip->usb_temp_l >= chip->usbtemp_max_temp_thr) &&
		      (chip->usb_temp_l < USB_100C)) ||
		...
		if (g_tbatt_temp / 10 > USB_50C &&
		    (((chip->usb_temp_l >=
		       g_tbatt_temp / 10 + chip->usbtemp_max_temp_diff) &&
		      (chip->usb_temp_l < USB_100C)) ||
		     ((chip->usb_temp_r >=
		       g_tbatt_temp / 10 + chip->usbtemp_max_temp_diff) &&
		      (chip->usb_temp_r < USB_100C)))) {
	...
	dischg:
		...
		if ((condition1 == true || condition2 == true) &&
			...
			oplus_chg_track_upload_usbtemp_info(chip, condition,
							    last_usb_temp_l,
							    last_usb_temp_r,
							    batt_current);
...
bool oplus_usbtemp_l_trigger_current_status(void)
{
	...
	if (g_oplus_chip->usb_temp_l < USB_30C ||
	    g_oplus_chip->usb_temp_l > USB_100C) {
		return false;
	}

	if (g_oplus_chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if ((g_oplus_chip->usb_temp_l >=
		     g_oplus_chip->usbtemp_cool_down_ntc_low) ||
		    (g_oplus_chip->usb_temp_l - g_tbatt_temp / 10) >=
			    g_oplus_chip->usbtemp_cool_down_gap_low)
			return true;
		return false;
	} else if (g_oplus_chip->usbtemp_curr_status ==
		   OPLUS_USBTEMP_HIGH_CURR) {
		if ((g_oplus_chip->usb_temp_l >=
		     g_oplus_chip->usbtemp_cool_down_ntc_high) ||
		    (g_oplus_chip->usb_temp_l - g_tbatt_temp / 10) >=
...
bool oplus_usbtemp_l_recovery_current_status(void)
{
	...
	if (g_oplus_chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if ((g_oplus_chip->usb_temp_l <=
		     g_oplus_chip->usbtemp_cool_down_recover_ntc_low) &&
		    (g_oplus_chip->usb_temp_l - g_tbatt_temp / 10) <=
		...
		return false;
		   ...
		   OPLUS_USBTEMP_HIGH_CURR) {
		if ((g_oplus_chip->usb_temp_l <=
		     g_oplus_chip->usbtemp_cool_down_recover_ntc_high) &&
		    (g_oplus_chip->usb_temp_l - g_tbatt_temp / 10) <=
...
bool oplus_usbtemp_r_trigger_current_status(void)
{
	...
	if (g_oplus_chip->usb_temp_r < USB_30C ||
...
#define RETRY_COUNT 3
static void
oplus_update_usbtemp_current_status_new_method(struct oplus_chg_chip *chip)
{
	static int limit_cur_cnt_r = 0;
	static int limit_cur_cnt_l = 0;
	static int recover_cur_cnt_r = 0;
	...
	if (g_oplus_chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if (g_tbatt_temp / 10 <= g_oplus_chip->usbtemp_batt_temp_low &&
		    (((g_oplus_chip->usb_temp_l >=
		       g_oplus_chip->usbtemp_ntc_temp_low) &&
		      (g_oplus_chip->usb_temp_l < USB_100C)) ||
		...
		if (g_tbatt_temp / 10 <= g_oplus_chip->usbtemp_batt_temp_high &&
		    (((g_oplus_chip->usb_temp_l >=
		       g_oplus_chip->usbtemp_ntc_temp_high) &&
		      (g_oplus_chip->usb_temp_l < USB_100C)) ||
		     ((g_oplus_chip->usb_temp_r >=
		       g_oplus_chip->usbtemp_ntc_temp_high) &&
		      (g_oplus_chip->usb_temp_r < USB_100C))))
	...
	if (g_oplus_chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if (g_tbatt_temp / 10 > g_oplus_chip->usbtemp_batt_temp_low &&
		    (((g_oplus_chip->usb_temp_l >=
		       g_tbatt_temp / 10 +
			       g_oplus_chip
				       ->usbtemp_temp_gap_low_with_batt_temp) &&
		      (g_oplus_chip->usb_temp_l < USB_100C)) ||
		     ((g_oplus_chip->usb_temp_r >=
		       g_tbatt_temp / 10 +
			       g_oplus_chip
		...
		if (g_tbatt_temp / 10 > g_oplus_chip->usbtemp_batt_temp_high &&
		    (((g_oplus_chip->usb_temp_l >=
		       g_tbatt_temp / 10 +
			       g_oplus_chip
				       ->usbtemp_temp_gap_high_with_batt_temp) &&
		      (g_oplus_chip->usb_temp_l < USB_100C)) ||
		     ((g_oplus_chip->usb_temp_r >=
		       g_tbatt_temp / 10 +
			       g_oplus_chip
...
bool oplus_usbtemp_judge_temp_gap(int current_temp, int last_temp)
{
	...
	if (g_oplus_chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if ((current_temp - last_temp) >=
		    g_oplus_chip->usbtemp_rise_fast_temp_low)
			return true;
		return false;
	} else if (g_oplus_chip->usbtemp_curr_status ==
		   OPLUS_USBTEMP_HIGH_CURR) {
		if ((current_temp - last_temp) >=
		    g_oplus_chip->usbtemp_rise_fast_temp_high)
			return true;
		return false;
	} else {
		return false;
...
bool oplus_usbtemp_change_curr_range(struct oplus_chg_chip *chip, int retry_cnt,
				     int usbtemp_first_time_in_curr_range,
				     bool curr_range_change)
{
	...
	if (((current_curr_change_temp_l - last_curr_change_usb_temp_l) >=
	     OPLUS_USBTEMP_CURR_CHANGE_TEMP) ||
	    (current_curr_change_temp_r - last_curr_change_usb_temp_r) >=
		    OPLUS_USBTEMP_CURR_CHANGE_TEMP) {
		for (i = 1; i <= retry_cnt; i++) {
			mdelay(RETRY_CNT_DELAY);
			get_usb_temp(chip);
			if ((chip->usb_temp_r - last_curr_change_usb_temp_r) >=
				    OPLUS_USBTEMP_CURR_CHANGE_TEMP &&
			    chip->usb_temp_r < USB_100C)
				count_curr_r++;
			if ((chip->usb_temp_l - last_curr_change_usb_temp_l) >=
				    OPLUS_USBTEMP_CURR_CHANGE_TEMP &&
			    chip->usb_temp_l < USB_100C)
				count_curr_l++;
			pr_err("countl : %d,countr : %d", count_curr_l,
			       count_curr_r);
		}
		current_curr_change_temp_l = chip->usb_temp_l;
		current_curr_change_temp_r = chip->usb_temp_r;

		if ((count_curr_l >= retry_cnt && chip->usb_temp_l > USB_30C &&
		     chip->usb_temp_l < USB_100C) ||
		    (count_curr_r >= retry_cnt && chip->usb_temp_r > USB_30C &&
		     chip->usb_temp_r < USB_100C)) {
...
bool oplus_usbtemp_trigger_for_high_temp(struct oplus_chg_chip *chip,
					 int retry_cnt, int count_r,
					 int count_l)
{
	int i = 0;
	...
	if (oplus_usbtemp_condition_temp_high()) {
		...
		for (i = 1; i < retry_cnt; i++) {
			mdelay(RETRY_CNT_DELAY);
			get_usb_temp(chip);
			if (chip->usbtemp_curr_status ==
			    OPLUS_USBTEMP_LOW_CURR) {
				if (chip->usb_temp_r >=
				...
				if (chip->usb_temp_l >=
				    ...
				    chip->usb_temp_l < USB_100C)
					count_l++;
			} else if (chip->usbtemp_curr_status ==
				   OPLUS_USBTEMP_HIGH_CURR) {
				if (chip->usb_temp_r >=
...
bool oplus_usbtemp_trigger_for_rise_fast_temp(struct oplus_chg_chip *chip,
					      int retry_cnt, int count_r,
					      int count_l)
{
	int i = 0;
...
bool oplus_usbtemp_trigger_for_rise_fast_without_temp(
	struct oplus_chg_chip *chip, int retry_cnt, int count_r, int count_l,
	int total_count)
{
	static int count = 0;
	static int last_usb_temp_l = 25;
	...
	if (oplus_usbtemp_temp_rise_fast_without_batt_temp()) {
		...
		if (oplus_usbtemp_judge_temp_gap(current_temp_l,
						 last_usb_temp_l) ||
		    oplus_usbtemp_judge_temp_gap(current_temp_r,
						 last_usb_temp_r)) {
			for (i = 1; i <= retry_cnt; i++) {
				mdelay(RETRY_CNT_DELAY);
				oplus_get_usbtemp_volt(chip);
				...
				current_temp_l = chip->usb_temp_l;
				current_temp_r = chip->usb_temp_r;
				if (oplus_usbtemp_judge_temp_gap(
					    current_temp_r, last_usb_temp_r) &&
				    chip->usb_temp_r < USB_100C)
					count_r++;
				if (oplus_usbtemp_judge_temp_gap(
					    current_temp_l, last_usb_temp_l) &&
				    chip->usb_temp_l < USB_100C)
					count_l++;
				pr_err("countl : %d,countr : %d", count_l,
				       count_r);
			}
			current_temp_l = chip->usb_temp_l;
			current_temp_r = chip->usb_temp_r;
...
static int oplus_usbtemp_monitor_main_new_method(void *data)
{
	...
	while (!kthread_should_stop()) {
		wait_event_interruptible(chip->oplus_usbtemp_wq_new_method,
					 chip->usbtemp_check == true);
		if (chip->dischg_flag == true) {
			goto dischg;
		}
		...
		if (current_read_count == OPLUS_CHG_CURRENT_READ_COUNT) {
			if (oplus_switching_support_parallel_chg()) {
				...
				if (oplus_vooc_get_allow_reading()) {
					...
					chip->usbtemp_batt_current =
						-oplus_gauge_get_prev_batt_current();
				}
			}
		...
		if (usbtemp_dbg_curr_status < OPLUS_USBTEMP_LOW_CURR ||
			...
			if (chip->usbtemp_batt_current > 5000) {
				chip->usbtemp_curr_status =
				   ...
				   chip->usbtemp_batt_current <= 5000) {
				chip->usbtemp_curr_status =
					OPLUS_USBTEMP_LOW_CURR;
			}
		...
		} else if (curr_range_change == true &&
			   chip->usbtemp_batt_current >= 5000 &&
			   chip->usbtemp_pre_batt_current < 5000) {
			curr_range_change = false;
		}

		if (curr_range_change == true &&
		    chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
			if (oplus_usbtemp_change_curr_range(
				    chip, retry_cnt,
				    usbtemp_first_time_in_curr_range,
				    curr_range_change)) {
				chip->usbtemp_curr_status =
					OPLUS_USBTEMP_LOW_CURR;
				curr_range_change = false;
			...
			if (curr_range_change_last_time.tv_sec -
				    curr_range_change_first_time.tv_sec >=
		...
		/*condition1  :the temp is higher than 57*/
		if (oplus_usbtemp_trigger_for_high_temp(chip, retry_cnt,
							count_r, count_l)) {
			if (!IS_ERR_OR_NULL(
				    chip->normalchg_gpio.dischg_enable)) {
				chip->dischg_flag = true;
		...
		if (oplus_usbtemp_trigger_for_rise_fast_temp(
			    chip, retry_cnt, count_r, count_l)) {
			if (!IS_ERR_OR_NULL(
				    chip->normalchg_gpio.dischg_enable) ||
		...
		/*condition2  :the temp uprising to fast*/
		if (oplus_usbtemp_trigger_for_rise_fast_without_temp(
			    chip, retry_cnt, count_r, count_l, total_count)) {
			if (!IS_ERR_OR_NULL(
				    chip->normalchg_gpio.dischg_enable) ||
			    chip->usbtemp_dischg_by_pmic) {
				chip->dischg_flag = true;
...
static void oplus_usbtemp_thread_init(void)
{
	if (g_oplus_chip->support_usbtemp_protect_v2)
		oplus_usbtemp_kthread =
			kthread_run(oplus_usbtemp_monitor_main_new_method, 0,
				    "usbtemp_kthread");
	else
		oplus_usbtemp_kthread = kthread_run(oplus_usbtemp_monitor_main,
						    0, "usbtemp_kthread");
	...
	if (oplus_usbtemp_check_is_support() == true) {
		if (g_oplus_chip->support_usbtemp_protect_v2)
			wake_up_interruptible(
				&g_oplus_chip->oplus_usbtemp_wq_new_method);
		else
...
static int oplus_usbtemp_l_gpio_init(struct oplus_chg_chip *chip)
{
	...
	bcdev->oplus_custom_gpio.usbtemp_l_gpio_pinctrl =
		devm_pinctrl_get(bcdev->dev);
	...
	bcdev->oplus_custom_gpio.usbtemp_l_gpio_default = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.usbtemp_l_gpio_pinctrl,
		"usbtemp_l_gpio_default");
...
static int oplus_usbtemp_r_gpio_init(struct oplus_chg_chip *chip)
{
	...
	bcdev->oplus_custom_gpio.usbtemp_r_gpio_pinctrl =
		devm_pinctrl_get(bcdev->dev);
	...
	bcdev->oplus_custom_gpio.usbtemp_r_gpio_default = pinctrl_lookup_state(
		bcdev->oplus_custom_gpio.usbtemp_r_gpio_pinctrl,
		"usbtemp_r_gpio_default");
...
static int oplus_subboard_temp_gpio_init(struct oplus_chg_chip *chip)
{
	...
	bcdev->oplus_custom_gpio.subboard_temp_gpio_pinctrl =
		devm_pinctrl_get(bcdev->dev);
	if (IS_ERR_OR_NULL(
		    bcdev->oplus_custom_gpio.subboard_temp_gpio_pinctrl)) {
		chg_err("get usbtemp_r_gpio_pinctrl fail\n");
...
static int oplus_usbtemp_adc_gpio_dt(struct oplus_chg_chip *chip)
{
	...
	chip->normalchg_gpio.dischg_gpio =
		of_get_named_gpio(node, "qcom,dischg-gpio", 0);
	...
	if (dump_count == DUMP_LOG_CNT_30S) {
		...
		if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY) {
			printk(KERN_ERR
			       ...
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num - 1],
			       bcdev->read_buffer_dump.data_buffer[extra_num],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 1],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 2],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 3],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 4],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 5],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 6],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 7],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 8],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 9],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 10],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 11],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 12],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 13],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 14],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 15],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 16],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 17],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 18],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 19],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 20],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 21],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 22],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 23],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 24],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 25],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 26],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 27],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 28],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 29],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 30],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 31],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 32],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 33],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 34],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 35],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 36],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 37],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 38],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 39],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 40],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 41],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 42],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 43],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 44],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 45],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 46],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 47],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 48],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 49],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 50],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 51],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 52],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 53],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 54],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 55],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 56],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 57],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 58],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 59],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 60],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 61],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 62],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 63],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 64],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 65],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 66],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 67],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 68],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 69],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 70],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 71],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 72],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 73],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 74],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 75],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 76],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 77],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 78],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 79],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 80],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 81],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 82],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 83],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 84],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 85],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 86],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 87],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 88],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 89],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 90],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 91],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 92],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 93],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 94],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 95],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 96],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 97],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 98],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 99],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 100],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 101],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 102],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 103],
			       bcdev->read_buffer_dump
				       .data_buffer[extra_num + 104]);
		} else {
			printk(KERN_ERR
			       "sm8350_st_dump: [chg_en=%d, suspend=%d, pd_svooc=%d, subtype=0x%02x],\n",
			       smbchg_get_charge_enable(),
			       bcdev->read_buffer_dump.data_buffer[9],
			       bcdev->read_buffer_dump.data_buffer[11],
			       oplus_chg_get_charger_subtype());
...
static int oplus_chg_set_wls_input_current(int current_ma)
{
	...
	rc = write_property_id(bcdev, pst, WLS_INPUT_CURR_LIMIT,
			       current_ma * 1000);
	if (rc)
...
static void oplus_chg_status_send_adsp_work(struct work_struct *work)
{
	...
	if (!oplus_chg_get_adsp_notify_ap_suspend() && chg_enable &&
	    !chg_suspend) {
		chg_status = true;
	}
...
void oplus_adsp_voocphy_cancle_err_check(void)
{
	...
	if (!chip) {
		printk(KERN_ERR
		       "!!!chip null, oplus_adsp_voocphy_cancle_err_check\n");
		return;
	}
	...
	if (chg_vol > VOLTAGE_2000MV) {
		...
		if (count > COUNT_TEN) {
			chg_err("reset input current err_cnt: %d,chg_vol:%d,fastchg_start:%d,fastchg_ing:%d,ibus_curr:%d,fast_chg_type:%d\n",
				err_cnt, chg_vol,
				oplus_voocphy_get_fastchg_start(),
				oplus_voocphy_get_fastchg_ing(), ibus_curr,
			...
			schedule_delayed_work(
				&bcdev->recheck_input_current_work,
...
static void oplus_plugin_irq_work(struct work_struct *work)
{
	...
	if (bcdev->usb_ocm) {
		if (bcdev->usb_in_status == 1) {
			if (g_oplus_chip && g_oplus_chip->charger_type ==
	...
	if (bcdev->usb_in_status == 1) {
			...
			if (oplus_chg_get_voocphy_support() != ADSP_VOOCPHY) {
				if (oplus_vooc_get_fastchg_started() == true &&
					...
					oplus_chg_wake_update_work();
					   ...
					   false) {
					printk(KERN_ERR
					       "[%s]: plug out fastchg_to_normal/warm/dummy or not vooc\n",
					       __func__);
		...
		bcdev->hvdcp_detach_time =
			cpu_clock(smp_processor_id()) / CPU_CLOCK_TIME_MS;
		printk(KERN_ERR
		       "!!! %s: the hvdcp_detach_time:%lu, detect time %lu \n",
		       __func__, bcdev->hvdcp_detach_time,
		       bcdev->hvdcp_detect_time);
		if (bcdev->hvdcp_detach_time - bcdev->hvdcp_detect_time <=
		    OPLUS_HVDCP_DETECT_TO_DETACH_TIME) {
			bcdev->hvdcp_disable = true;
			schedule_delayed_work(&bcdev->hvdcp_disable_work,
					      OPLUS_HVDCP_DISABLE_INTERVAL);
		} else {
...
int oplus_set_otg_switch_status(bool enable)
{
	...
	if (get_otg_scheme(chip) == OTG_SCHEME_CCDETECT_GPIO) {
		...
		if (level != 1) {
			printk(KERN_ERR
			       "[OPLUS_CHG][%s]: gpio[%s], should set, return\n",
			       __func__, level ? "H" : "L");
			return rc;
		}
	} else {
...
int oplus_get_otg_online_status(void)
{
	...
	if (otg_scheme == OTG_SCHEME_CID && bcdev->cid_status == 0 &&
	...
	if (otg_scheme == OTG_SCHEME_CCDETECT_GPIO) {
		level = gpio_get_value(bcdev->oplus_custom_gpio.ccdetect_gpio);
		if (level !=
		    gpio_get_value(bcdev->oplus_custom_gpio.ccdetect_gpio)) {
			printk(KERN_ERR
			       "[OPLUS_CHG][%s]: ccdetect_gpio is unstable, try again...\n",
			       __func__);
	...
	if ((pre_cid_status ^ cid_status) || (pre_typec_otg ^ typec_otg) ||
	    (pre_level ^ level)) {
		pre_cid_status = cid_status;
		pre_typec_otg = typec_otg;
		pre_level = level;
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: level[%d], cid_status[%d], typec_otg[%d], otg_online[%d]\n",
...
void smbchg_set_chargerid_switch_val(int value)
{
	...
	if (oplus_vooc_get_adapter_update_real_status() ==
	...
	if (value) {
		gpio_direction_output(
			g_oplus_chip->normalchg_gpio.chargerid_switch_gpio, 1);
		pinctrl_select_state(
			g_oplus_chip->normalchg_gpio.pinctrl,
			g_oplus_chip->normalchg_gpio.chargerid_switch_active);
	} else {
		gpio_direction_output(
			g_oplus_chip->normalchg_gpio.chargerid_switch_gpio, 0);
		pinctrl_select_state(
			g_oplus_chip->normalchg_gpio.pinctrl,
			g_oplus_chip->normalchg_gpio.chargerid_switch_sleep);
	}
	...
	chg_err("set usb_switch_1 = %d, result = %d\n", value,
		smbchg_get_chargerid_switch_val());
	return;
}
...
int qpnp_get_prop_charger_voltage_now(void)
{
	...
	if (oplus_chg_is_wls_present()) {
		rc = oplus_chg_mod_get_property(
			chip->wls_ocm, OPLUS_CHG_PROP_VOLTAGE_NOW, &pval);
		if (rc >= 0) {
			return pval.intval;
		}
	}
...
int oplus_chg_set_pd_config(void)
{
	...
	if ((chip->limits.vbatt_pdqc_to_5v_thr > 0 &&
		...
		if (is_ext_chg_ops())
			oplus_chg_suspend_charger();
		oplus_chg_config_charger_vsys_threshold(
			0x03); /*set Vsys Skip threshold 101%*/
		...
		if (is_ext_chg_ops())
			oplus_chg_suspend_charger();
		oplus_chg_config_charger_vsys_threshold(
			0x02); /*set Vsys Skip threshold 104%*/
...
int oplus_chg_set_qc_config(void)
{
	...
	if ((chip->limits.vbatt_pdqc_to_5v_thr > 0 &&
		...
		if (oplus_chg_get_charger_voltage() < 8000) {
			...
			if (chip->pdqc_9v_voltage_adaptive && is_ext_chg_ops())
				oplus_chg_pdqc9v_vindpm_vol_switch(
					OPLUS_PDQC_5VTO9V);
		}
...
static int oplus_input_current_limit_ctrl_by_vooc_write(int current_ma)
{
	...
	if (current_ma > cur_usb_icl) {
		for (temp_curr = cur_usb_icl; temp_curr < current_ma;
		     temp_curr += 500) {
			msleep(35);
...
struct oplus_chg_operations battery_chg_ops = {
	...
	.input_current_write_without_aicl =
		oplus_chg_set_input_current_with_no_aicl,
	...
	.input_current_ctrl_by_vooc_write =
		oplus_input_current_limit_ctrl_by_vooc_write,
...
static int fg_bq27541_get_battery_mvolts(void)
{
	...
	if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY) {
		volt = DIV_ROUND_CLOSEST(bcdev->read_buffer_dump.data_buffer[2],
					 1000);
		return volt;
	}
...
static int fg_bq27541_get_battery_soc(void)
{
	...
	if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY) {
		soc = DIV_ROUND_CLOSEST(bcdev->read_buffer_dump.data_buffer[3],
					100);
		return soc;
	}
...
static int fg_bq27541_get_average_current(void)
{
	...
	if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY &&
		...
		curr = DIV_ROUND_CLOSEST(
			(int)bcdev->read_buffer_dump.data_buffer[1], 1000);
		return curr;
	}
...
static struct oplus_gauge_operations battery_gauge_ops = {
	...
	.get_prev_batt_remaining_capacity =
		fg_bq27541_get_batt_remaining_capacity,
	...
	.get_prev_battery_mvolts_2cell_max =
		fg_bq27541_get_battery_mvolts_2cell_max,
	.get_prev_battery_mvolts_2cell_min =
		fg_bq27541_get_battery_mvolts_2cell_min,
...
static int oplus_chg_usb_init_mod(struct battery_chg_dev *bcdev)
{
	...
	bcdev->usb_ocm = oplus_chg_mod_register(
		bcdev->dev, &oplus_chg_usb_mod_desc, &ocm_cfg);
...
static void oplus_otg_status_check_work(struct work_struct *work)
{
	...
	bool contion1 = false, contion2 = false, contion3 = false,
	     contion4 = false, contion5 = false;
	struct oplus_chg_chip *chip = g_oplus_chip;
	...
	if (bcdev->otg_boost_src == OTG_BOOST_SOURCE_EXTERNAL) {
		rc = oplus_get_bat_info_for_otg_status_check(&real_soc,
							     &batt_current);
		if (rc < 0) {
	...
	contion1 = ((batt_current > bcdev->otg_curr_limit_high) &&
		    (skin_temp > OTG_SKIN_TEMP_HIGH));
	contion2 = (batt_current > bcdev->otg_curr_limit_max);
	contion3 = (skin_temp > OTG_SKIN_TEMP_MAX);
	contion4 = ((real_soc < bcdev->otg_real_soc_min) &&
		    (batt_current > bcdev->otg_curr_limit_high));
	contion5 = ((skin_temp < 0) &&
		    (batt_current > bcdev->otg_curr_limit_high));
...
#ifdef PM_REG_DEBUG
static ssize_t proc_debug_reg_read(struct file *file, char __user *buf,
...
static ssize_t proc_debug_reg_write(struct file *file, const char __user *buf,
...
#ifdef WLS_QI_DEBUG
static ssize_t proc_icl_read(struct file *file, char __user *buf, size_t count,
			     loff_t *ppos)
{
	uint8_t ret = 0;
	char page[16];
...
static ssize_t proc_icl_write(struct file *file, const char __user *buf,
			      size_t count, loff_t *lo)
{
	int rc = 0;
	char buffer[16] = { 0 };
...
static ssize_t proc_fcc_read(struct file *file, char __user *buf, size_t count,
			     loff_t *ppos)
{
	uint8_t ret = 0;
	char page[16];
...
static ssize_t proc_fcc_write(struct file *file, const char __user *buf,
			      size_t count, loff_t *lo)
{
	int rc = 0;
	char buffer[16] = { 0 };
...
static int init_debug_reg_proc(struct oplus_chg_chip *da)
{
	...
	pr_entry_tmp = proc_create_data("reg", 0644, pr_entry_da,
					&proc_debug_reg_ops, da);
...
#ifdef WLS_QI_DEBUG
	pr_entry_tmp = proc_create_data("icl_ma", 0644, pr_entry_da,
					&proc_icl_ops, da);
	if (pr_entry_tmp == NULL) {
		ret = -ENOMEM;
		chg_debug("%s: Couldn't create proc entry, %d\n", __func__,
	...
	pr_entry_tmp = proc_create_data("fcc_ma", 0644, pr_entry_da,
					&proc_fcc_ops, da);
	if (pr_entry_tmp == NULL) {
		ret = -ENOMEM;
		chg_debug("%s: Couldn't create proc entry, %d\n", __func__,
...
static void oplus_chg_track_dump_reg_info(char *dump_info, int len)
{
	...
	oem_read_buffer(bcdev);
	index +=
		snprintf(&(dump_info[index]), len - index,
			 ...
			 bcdev->read_buffer_dump.data_buffer[12],
			 bcdev->cid_status, bcdev->usb_in_status);
	for (i = OPLUS_REG_INDEX_START;
	     i < OPLUS_REG_INDEX_MAX && i < (MAX_OEM_PROPERTY_DATA_SIZE - 1);
	     i += 2) {
					       ...
					       int err_type)
{
	...
	index += snprintf(&(bcdev->icl_err_load_trigger->crux_info[index]),
			  ...
			  "$$device_id@@%s", "pm8350b");
	index +=
		snprintf(&(bcdev->icl_err_load_trigger->crux_info[index]),
			 OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			 "$$err_scene@@%s", OPLUS_CHG_TRACK_SCENE_PMIC_ICL_ERR);

	oplus_chg_track_get_pmic_err_reason(err_type, bcdev->err_reason,
					    sizeof(bcdev->err_reason));
	index += snprintf(&(bcdev->icl_err_load_trigger->crux_info[index]),
			  OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$err_reason@@%s", bcdev->err_reason);

	oplus_chg_track_obtain_power_info(bcdev->chg_power_info,
					  sizeof(bcdev->chg_power_info));
	index += snprintf(&(bcdev->icl_err_load_trigger->crux_info[index]),
			  OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s",
			  bcdev->chg_power_info);
	oplus_chg_track_dump_reg_info(bcdev->dump_info,
				      sizeof(bcdev->dump_info));
	index += snprintf(&(bcdev->icl_err_load_trigger->crux_info[index]),
			  OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
...
static void oplus_chg_track_icl_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct battery_chg_dev *bcdev = container_of(
		dwork, struct battery_chg_dev, icl_err_load_trigger_work);
...
static int oplus_chg_track_debugfs_init(struct battery_chg_dev *bcdev)
{
	...
	debugfs_create_u32("debug_force_icl_err", 0644, debugfs_pm8350b,
			   &(bcdev->debug_force_icl_err));
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	if (oplus_gauge_check_chip_is_null()) {
		gauge_chip = devm_kzalloc(&pdev->dev, sizeof(*gauge_chip),
					  GFP_KERNEL);
		if (!gauge_chip) {
...
#else
	INIT_DELAYED_WORK(&bcdev->usb_type_work,
			  battery_chg_update_usb_type_work);
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
	INIT_DELAYED_WORK(&bcdev->adsp_voocphy_status_work,
			  oplus_adsp_voocphy_status_func);
	...
	INIT_DELAYED_WORK(&bcdev->usbtemp_recover_work,
			  oplus_usbtemp_recover_work);
	INIT_DELAYED_WORK(&bcdev->cid_status_change_work,
			  oplus_cid_status_change_work);
	INIT_DELAYED_WORK(&bcdev->adsp_crash_recover_work,
			  oplus_adsp_crash_recover_func);
	INIT_DELAYED_WORK(&bcdev->check_charger_out_work,
			  oplus_check_charger_out_func);
	INIT_DELAYED_WORK(&bcdev->adsp_voocphy_enable_check_work,
			  oplus_adsp_voocphy_enable_check_func);
	INIT_DELAYED_WORK(&bcdev->otg_vbus_enable_work,
			  otg_notification_handler);
	INIT_DELAYED_WORK(&bcdev->hvdcp_disable_work, oplus_hvdcp_disable_work);
	INIT_DELAYED_WORK(&bcdev->typec_state_change_work,
			  oplus_typec_state_change_work);
	INIT_DELAYED_WORK(&bcdev->chg_status_send_work,
			  oplus_chg_status_send_adsp_work);
	INIT_DELAYED_WORK(&bcdev->suspend_check_work, oplus_suspend_check_work);
	INIT_DELAYED_WORK(&bcdev->vbus_adc_enable_work,
			  oplus_vbus_enable_adc_work);
	INIT_DELAYED_WORK(&bcdev->adsp_voocphy_err_work,
			  oplus_adsp_voocphy_err_work);
	INIT_DELAYED_WORK(&bcdev->plugin_irq_work, oplus_plugin_irq_work);
	INIT_DELAYED_WORK(&bcdev->recheck_input_current_work,
			  oplus_recheck_input_current_work);
	...
	INIT_DELAYED_WORK(
		&bcdev->otg_status_check_work,
		oplus_otg_status_check_work); //OTG software OCP/OTP protect work
	INIT_DELAYED_WORK(&bcdev->adsp_track_notify_work,
			  adsp_track_notification_handler);
	...
	INIT_DELAYED_WORK(&bcdev->wait_wired_charge_on,
			  oplus_wait_wired_charge_on_work);
	INIT_DELAYED_WORK(&bcdev->wait_wired_charge_off,
			  oplus_wait_wired_charge_off_work);

	INIT_DELAYED_WORK(&bcdev->status_keep_clean_work,
			  oplus_chg_wls_status_keep_clean_work);
	INIT_DELAYED_WORK(&bcdev->status_keep_delay_unlock_work,
			  oplus_chg_wls_status_keep_delay_unlock_work);
	bcdev->status_wake_lock =
		wakeup_source_register(bcdev->dev, "status_wake_lock");
	bcdev->status_wake_lock_on = false;
#endif
	...
	bcdev->subsys_handle =
		qcom_register_ssr_notifier("lpass", &bcdev->ssr_nb);
	...
	schedule_work(&bcdev->usb_type_work);
#else
	schedule_delayed_work(&bcdev->usb_type_work,
			      round_jiffies_relative(msecs_to_jiffies(3000)));
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
	...
	if (rc < 0)
		goto error;
	bcdev->wls_boost_soft_start = of_property_read_bool(
		bcdev->dev->of_node, "oplus,wls_boost_soft_start");
	bcdev->wls_sim_detect_wr = of_property_read_bool(
		bcdev->dev->of_node, "oplus,wls_sim_detect_wr");
	rc = of_property_read_u32(bcdev->dev->of_node, "oplus,ic_type",
				  &ic_type);
	if (rc < 0) {
		pr_err("can't get ic type, rc=%d\n", rc);
		goto ic_reg_err;
	}
	rc = of_property_read_u32(bcdev->dev->of_node, "oplus,ic_index",
				  &ic_index);
	if (rc < 0) {
		pr_err("can't get ic index, rc=%d\n", rc);
		goto ic_reg_err;
	}
	bcdev->ic_dev = devm_oplus_chg_ic_register(
		dev, bcdev->dev->of_node->name, ic_index);
	if (!bcdev->ic_dev) {
		rc = -ENODEV;
...
static void battery_chg_shutdown(struct platform_device *pdev)
{
	...
	if (g_oplus_chip &&
	    g_oplus_chip->pmic_spmi.bcdev_chip->wls_sim_detect_wr &&
	...
	if (g_oplus_chip && g_oplus_chip->chg_ops->charger_suspend &&
	    g_oplus_chip->chg_ops->charger_unsuspend) {
