...
static __maybe_unused int sgm41512_write_reg(struct chip_sgm41512 *chip,
					     int reg, int data)
{
	int ret;
...
static __maybe_unused int sgm41512_config_interface(struct chip_sgm41512 *chip,
...
int sgm41512_set_vindpm_vol(int vol)
{
	...
	tmp = (vol - REG06_SGM41512_VINDPM_OFFSET) /
	      REG06_SGM41512_VINDPM_STEP_MV;
...
static int sgm41512_get_usb_icl(void)
{
	...
	tmp = (tmp & REG00_SGM41512_INPUT_CURRENT_LIMIT_MASK) >>
	      REG00_SGM41512_INPUT_CURRENT_LIMIT_SHIFT;
	return (tmp * REG00_SGM41512_INPUT_CURRENT_LIMIT_STEP +
		REG00_SGM41512_INPUT_CURRENT_LIMIT_OFFSET);
}
...
int sgm41512_input_current_limit_without_aicl(int current_ma)
{
	...
	tmp = (current_ma - REG00_SGM41512_INPUT_CURRENT_LIMIT_OFFSET) /
	      REG00_SGM41512_INPUT_CURRENT_LIMIT_STEP;
	chg_err("tmp current [%d]ma\n", current_ma);
	rc = sgm41512_config_interface(
		chip, REG00_SGM41512_ADDRESS,
...
int sgm41512_input_current_limit_write(int current_ma)
{
	...
	for (pre_icl_index = ARRAY_SIZE(sgm41512_usb_icl) - 1;
	...
	for (i = pre_icl_index; i > 1; i--) {
		rc = sgm41512_input_current_limit_without_aicl(
			sgm41512_usb_icl[i]);
		if (rc) {
	...
	if (buf != NULL && size != 0) {
		...
		if (size > 3) {
			...
			pr_err("[%s] write sgm41512 reg 0x%02x with value 0x%02x !\n",
			       ...
			       reg_value);
			ret = sgm41512_write_reg(chip, chip->reg_access,
						 reg_value);
		} else {
			ret = sgm41512_read_reg(chip, chip->reg_access,
						&reg_value);
...
static DEVICE_ATTR(sgm41512_access, 0660, sgm41512_access_show,
		   sgm41512_access_store);
...
int sgm41512_set_stat_dis(bool enable)
{
	...
	rc = sgm41512_config_interface(chip, REG00_SGM41512_ADDRESS,
				       enable ? REG00_SGM41512_STAT_DIS_ENABLE :
						REG00_SGM41512_STAT_DIS_DISABLE,
...
int sgm41512_set_int_mask(int val)
{
	...
	rc = sgm41512_config_interface(chip, REG0A_SGM41512_ADDRESS, val,
				       REG0A_SGM41512_VINDPM_INT_MASK |
					       REG0A_SGM41512_IINDPM_INT_MASK);
	if (rc) {
...
bool sgm41512_get_bus_gd(void)
{
	...
	bus_gd = ((reg_val & REG0A_SGM41512_BUS_GD_MASK) ==
		  REG0A_SGM41512_BUS_GD_YES) ?
			 1 :
			 0;
	return bus_gd;
}
...
bool sgm41512_get_power_gd(void)
{
	...
	power_gd = ((reg_val & REG08_SGM41512_POWER_GOOD_STAT_MASK) ==
		    REG08_SGM41512_POWER_GOOD_STAT_GOOD) ?
			   1 :
			   0;
	return power_gd;
}
...
int sgm41512_otg_ilim_set(int ilim)
{
	...
	rc = sgm41512_config_interface(chip, REG02_SGM41512_ADDRESS, ilim,
...
int sgm41512_check_charging_enable(void)
{
	...
	charging_enable = ((reg_val & REG01_SGM41512_CHARGING_MASK) ==
			   REG01_SGM41512_CHARGING_ENABLE) ?
				  1 :
				  0;
...
int sgm41512_unsuspend_charger(void)
{
	...
	if ((chip->before_unsuspend_icl == 0) ||
		...
		sgm41512_input_current_limit_without_aicl(
			chip->before_suspend_icl);
	}
...
void sgm41512_really_suspend_charger(bool en)
{
	...
	rc = sgm41512_config_interface(chip, REG00_SGM41512_ADDRESS,
				       en ? REG00_SGM41512_SUSPEND_MODE_ENABLE :
					    REG00_SGM41512_SUSPEND_MODE_DISABLE,
...
int sgm41512_registers_read_full(void)
{
	...
	reg_full = ((reg_full & REG08_SGM41512_CHG_STAT_MASK) ==
		    REG08_SGM41512_CHG_STAT_CHG_TERMINATION) ?
			   1 :
			   0;
	if (reg_full) {
...
int sgm41512_set_ovp(int val)
{
	...
	rc = sgm41512_config_interface(chip, REG06_SGM41512_ADDRESS, val,
...
int sgm41512_get_iindet(void)
{
	...
	is_complete = ((reg_val & REG07_SGM41512_IINDET_EN_MASK) ==
		       REG07_SGM41512_IINDET_EN_DET_COMPLETE) ?
			      1 :
			      0;
	return is_complete;
}
...
static int sgm41512_batfet_reset_disable(struct chip_sgm41512 *chip,
					 bool enable)
{
	int rc = 0;
	int val = 0;
	...
	if (enable) {
		val = SGM41512_BATFET_RST_DISABLE
		      << REG07_SGM41512_BATFET_RST_EN_SHIFT;
	} else {
		val = SGM41512_BATFET_RST_ENABLE
		      << REG07_SGM41512_BATFET_RST_EN_SHIFT;
	}

	rc = sgm41512_config_interface(chip, REG07_SGM41512_ADDRESS, val,
				       REG07_SGM41512_BATFET_RST_EN_MASK);
...
int sgm41512_hardware_init(void)
{
	...
	sgm41512_set_int_mask(REG0A_SGM41512_VINDPM_INT_NOT_ALLOW |
			      REG0A_SGM41512_IINDPM_INT_NOT_ALLOW);
	...
	sgm41512_input_current_limit_without_aicl(
		REG00_SGM41512_INIT_INPUT_CURRENT_LIMIT_500MA);
...
struct oplus_chg_operations sgm41512_chg_ops = {
	...
	.input_current_ctrl_by_vooc_write =
		sgm41512_input_current_limit_ctrl_by_vooc_write,
...
static int sgm41512_parse_dt(struct chip_sgm41512 *chip)
{
	...
	chip->irq_gpio = of_get_named_gpio(chip->client->dev.of_node,
					   "sgm41512-irq-gpio", 0);
	...
	chip->enable_gpio = of_get_named_gpio(
		chip->client->dev.of_node, "qcom,slave_charger_enable-gpio", 0);
	...
	chip->use_voocphy = of_property_read_bool(chip->client->dev.of_node,
						  "qcom,use_voocphy");
...
static void sgm41512_bc12_retry_work(struct work_struct *work)
{
	...
	struct chip_sgm41512 *chip =
		container_of(dwork, struct chip_sgm41512, bc12_retry_work);
...
static irqreturn_t sgm41512_irq_handler(int irq, void *data)
{
	...
	if (atomic_read(&chip->charger_suspended) == 1) {
		...
		wait_event_interruptible_timeout(
			chip->wait, atomic_read(&chip->charger_suspended) == 0,
				  ...
				  const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct chip_sgm41512),
			    GFP_KERNEL);
	if (!chip) {
		return -ENOMEM;
	}
	...
	if (MSM_BOOT_MODE__NORMAL == get_boot_mode())
		...
		schedule_delayed_work(&chip->init_work,
				      msecs_to_jiffies(INIT_WORK_NORMAL_DELAY));
	else
		schedule_delayed_work(&chip->init_work,
				      msecs_to_jiffies(INIT_WORK_OTHER_DELAY));
