...
int oplus_switching_set_fastcharge_current(int curr_ma)
{
	...
	return g_switching_chip->switch_ops->switching_set_fastcharge_current(
		curr_ma);
}
...
int oplus_switching_enable_charge(int en)
{
	if (!g_switching_chip || !g_switching_chip->switch_ops ||
		...
		chg_err("success\n");
		return g_switching_chip->switch_ops->switching_enable_charge(
			en);
	}
}
...
bool oplus_switching_get_charge_enable(void)
{
	if (!g_switching_chip || !g_switching_chip->switch_ops ||
		...
		return g_switching_chip->switch_ops
			->switching_get_charge_enable();
	}
}
...
int oplus_switching_get_fastcharge_current(void)
{
	if (!g_switching_chip || !g_switching_chip->switch_ops ||
		...
		return g_switching_chip->switch_ops
			->switching_get_fastcharge_current();
	}
}
...
int oplus_switching_get_discharge_current(void)
{
	if (!g_switching_chip || !g_switching_chip->switch_ops ||
		...
		return g_switching_chip->switch_ops
			->switching_get_discharge_current();
	}
}
...
int oplus_switching_get_if_need_balance_bat(int vbat0_mv, int vbat1_mv)
{
	...
	if (!g_switching_chip) {
		...
		if (chip->sub_batt_temperature == FG_I2C_ERROR ||
		    chip->temperature == FG_I2C_ERROR) {
		...
		if (oplus_vooc_get_fastchg_started() == true &&
		    atomic_read(&chip->mos_lock)) {
			if (oplus_switching_get_hw_enable() &&
			    (abs(chip->sub_batt_icharging) <
				     g_switching_chip
					     ->parallel_mos_abnormal_litter_curr ||
			     abs(chip->icharging) <
				     g_switching_chip
					     ->parallel_mos_abnormal_litter_curr) &&
		...
		if (oplus_switching_support_parallel_chg() ==
			...
			if (chip->tbatt_status != BATTERY_STATUS__WARM_TEMP &&
			    (chip->sw_sub_batt_full ||
			     chip->hw_sub_batt_full_by_sw) &&
			...
			if (diff_volt >=
			    g_switching_chip->parallel_vbat_gap_abnormal) {
			...
			if ((pre_error_reason & REASON_SUB_BATT_FULL) &&
				...
				chg_err("sub full,but diff_volt > %d need to recovery MOS\n",
					g_switching_chip
						->parallel_vbat_gap_full);
			}
			if ((pre_error_reason & REASON_VBAT_GAP_BIG) &&
			    diff_volt <
				    g_switching_chip->parallel_vbat_gap_recov) {
				error_reason &= ~REASON_VBAT_GAP_BIG;
			}
		}
		...
		if (g_switching_chip->debug_force_mos_err)
			oplus_chg_track_parallel_mos_error(
				g_switching_chip->debug_force_mos_err);
		...
		if (error_reason != 0) {
			...
			chip->parallel_error_flag &=
				~(REASON_I2C_ERROR | REASON_MOS_OPEN_ERROR |
				  REASON_SUB_BATT_FULL | REASON_VBAT_GAP_BIG);
			chip->parallel_error_flag |= error_reason;
			chg_err("mos open %d\n", error_reason);
			if ((error_reason &
			     (REASON_I2C_ERROR | REASON_MOS_OPEN_ERROR)) != 0) {
				return PARALLEL_BAT_BALANCE_ERROR_STATUS8;
			}
...
static int oplus_switching_parse_dt(struct oplus_switch_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,parallel_vbat_gap_full",
				  &chip->parallel_vbat_gap_full);
	if (rc) {
		chip->parallel_vbat_gap_full = 200;
	}

	rc = of_property_read_u32(node, "qcom,parallel_vbat_gap_recov",
				  &chip->parallel_vbat_gap_recov);
	if (rc) {
		chip->parallel_vbat_gap_recov = 100;
	}

	rc = of_property_read_u32(node,
				  "qcom,parallel_mos_abnormal_litter_curr",
				  &chip->parallel_mos_abnormal_litter_curr);
	if (rc) {
		chip->parallel_mos_abnormal_litter_curr = 100;
	}

	rc = of_property_read_u32(node, "qcom,parallel_mos_abnormal_gap_curr",
	...
	chip->normal_chg_check =
		of_property_read_bool(node, "normal_chg_check_support");

	rc = of_property_read_u32(node, "track_unbalance_high",
				  &chip->track_unbalance_high);
	if (rc) {
		chip->track_unbalance_high = 100;
	}
	rc = of_property_read_u32(node, "track_unbalance_low",
				  &chip->track_unbalance_low);
	if (rc) {
		chip->track_unbalance_low = 0;
	}
	temp_node = of_get_child_by_name(node, "parallel_bat_table");
	chip->parallel_bat_data = devm_kzalloc(
		chip->dev,
		BATTERY_STATUS__INVALID * sizeof(struct parallel_bat_table),
		GFP_KERNEL);
	if (temp_node && chip->parallel_bat_data) {
		for (i = 0; i < BATTERY_STATUS__INVALID; i++) {
			rc = of_property_count_elems_of_size(
				temp_node, batt_temp_table[i], sizeof(u32));
			if (rc > 0 &&
			    rc % (sizeof(struct batt_spec) / sizeof(int)) ==
				    0) {
				length = rc;
				chip->parallel_bat_data[i].length =
					length / (sizeof(struct batt_spec) /
						  sizeof(int));
				chip->parallel_bat_data[i]
					.batt_table = devm_kzalloc(
					chip->dev,
					length * sizeof(struct batt_spec),
					GFP_KERNEL);
				if (chip->parallel_bat_data[i].batt_table) {
					rc = of_property_read_u32_array(
						temp_node, batt_temp_table[i],
						(u32 *)chip
							->parallel_bat_data[i]
							.batt_table,
						length);
	...
	chip->parallel_mos_debug =
		of_property_read_bool(node, "track,parallel_mos_debug");
...
static void track_mos_err_load_trigger_work(struct work_struct *work)
{
	...
	struct oplus_switch_chip *chip = container_of(
		dwork, struct oplus_switch_chip, parallel_mos_trigger_work);
...
int oplus_chg_track_parallel_mos_error(int reason)
{
	...
	g_switching_chip->mos_err_load_trigger =
	...
	index += snprintf(
		&(g_switching_chip->mos_err_load_trigger->crux_info[index]),
	...
	index += snprintf(
		&(g_switching_chip->mos_err_load_trigger->crux_info[index]),
		OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "$$error_reason@@");
	switch (reason) {
	...
	case REASON_RECORD_SOC:
		...
		oplus_chg_track_get_mos_err_reason(err_type, err_reason,
						   sizeof(err_reason));
		index += snprintf(&(g_switching_chip->mos_err_load_trigger
					    ->crux_info[index]),
	...
	if (reason & REASON_I2C_ERROR) {
		memset(err_reason, 0, sizeof(err_reason));
		oplus_chg_track_get_mos_err_reason(
			TRACK_MOS_I2C_ERROR, err_reason, sizeof(err_reason));
		index += snprintf(&(g_switching_chip->mos_err_load_trigger
					    ->crux_info[index]),
				  OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s,",
				  err_reason);
	...
	if (reason & REASON_MOS_OPEN_ERROR) {
		memset(err_reason, 0, sizeof(err_reason));
		oplus_chg_track_get_mos_err_reason(
			TRACK_MOS_OPEN_ERROR, err_reason, sizeof(err_reason));
		index += snprintf(&(g_switching_chip->mos_err_load_trigger
					    ->crux_info[index]),
				  OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s,",
				  err_reason);
	...
	if (reason & REASON_SUB_BATT_FULL) {
		memset(err_reason, 0, sizeof(err_reason));
		oplus_chg_track_get_mos_err_reason(TRACK_MOS_SUB_BATT_FULL,
						   err_reason,
						   sizeof(err_reason));
		index += snprintf(&(g_switching_chip->mos_err_load_trigger
					    ->crux_info[index]),
	...
	if (reason & REASON_VBAT_GAP_BIG) {
		memset(err_reason, 0, sizeof(err_reason));
		oplus_chg_track_get_mos_err_reason(
			TRACK_MOS_VBAT_GAP_BIG, err_reason, sizeof(err_reason));
		index += snprintf(&(g_switching_chip->mos_err_load_trigger
					    ->crux_info[index]),
				  OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s",
				  err_reason);
	...
	index += snprintf(
		&(g_switching_chip->mos_err_load_trigger->crux_info[index]),
		OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "main_sub_volt %d %d, ",
		chip->batt_volt, chip->sub_batt_volt);
	index += snprintf(
		&(g_switching_chip->mos_err_load_trigger->crux_info[index]),
		OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "main_sub_curr %d %d, ",
		chip->icharging, chip->sub_batt_icharging);
	index += snprintf(
		&(g_switching_chip->mos_err_load_trigger->crux_info[index]),
		OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "main_sub_temp %d %d, ",
...
static int mos_track_debugfs_init(struct oplus_switch_chip *chip)
{
	...
	debugfs_create_u32("debug_force_mos_err", 0644, debugfs_mos,
			   &(chip->debug_force_mos_err));
...
void oplus_init_parallel_temp_threshold(void)
{
	...
	g_switching_chip->main_hot_bat_decidegc =
		chip->anti_shake_bound.hot_bound;
	g_switching_chip->main_warm_bat_decidegc =
		chip->anti_shake_bound.warm_bound;
	g_switching_chip->main_normal_bat_decidegc =
		chip->anti_shake_bound.normal_bound;
	g_switching_chip->main_little_cool_bat_decidegc =
		chip->anti_shake_bound.little_cool_bound;
	g_switching_chip->main_cool_bat_decidegc =
		chip->anti_shake_bound.cool_bound;
	g_switching_chip->main_little_cold_bat_decidegc =
		chip->anti_shake_bound.little_cold_bound;
	g_switching_chip->main_cold_bat_decidegc =
		chip->anti_shake_bound.cold_bound;
	g_switching_chip->main_removed_bat_decidegc =
		chip->limits.removed_bat_decidegc;

	g_switching_chip->sub_hot_bat_decidegc =
		chip->anti_shake_bound.hot_bound;
	g_switching_chip->sub_warm_bat_decidegc =
		chip->anti_shake_bound.warm_bound;
	g_switching_chip->sub_normal_bat_decidegc =
		chip->anti_shake_bound.normal_bound;
	g_switching_chip->sub_little_cool_bat_decidegc =
		chip->anti_shake_bound.little_cool_bound;
	g_switching_chip->sub_cool_bat_decidegc =
		chip->anti_shake_bound.cool_bound;
	g_switching_chip->sub_little_cold_bat_decidegc =
		chip->anti_shake_bound.little_cold_bound;
	g_switching_chip->sub_cold_bat_decidegc =
		chip->anti_shake_bound.cold_bound;
	g_switching_chip->sub_removed_bat_decidegc =
		chip->limits.removed_bat_decidegc;
}

static void parallel_battery_anti_shake_handle(int status_change, int main_temp,
					       int sub_temp)
{
	struct oplus_switch_chip *chip = g_switching_chip;
	...
	if (status_change == MAIN_STATUS_CHG ||
		...
		if (tbatt_cur_shake > chip->main_pre_shake) { /* get warmer */
			...
			high_shake_0c = 0;
		} else if (tbatt_cur_shake <
			   chip->main_pre_shake) { /* get cooler */
			low_shake = 0;
			high_shake = HYSTERISIS_DECIDEGC;
			low_shake_0c = 0;
			high_shake_0c = HYSTERISIS_DECIDEGC_0C;
		}
		if (chip->main_tbatt_status ==
		    BATTERY_STATUS__HIGH_TEMP) { /* >53C */
			chip->main_cold_bat_decidegc =
				chg_chip->anti_shake_bound.cold_bound;
			chip->main_little_cold_bat_decidegc =
				chg_chip->anti_shake_bound.little_cold_bound;
			chip->main_cool_bat_decidegc =
				chg_chip->anti_shake_bound.cool_bound;
			chip->main_little_cool_bat_decidegc =
				chg_chip->anti_shake_bound.little_cool_bound;
			chip->main_normal_bat_decidegc =
				chg_chip->anti_shake_bound.normal_bound;
			chip->main_warm_bat_decidegc =
				chg_chip->anti_shake_bound.warm_bound;
			chip->main_hot_bat_decidegc =
				chg_chip->anti_shake_bound.hot_bound +
				low_shake;
		} else if (chip->main_tbatt_status ==
			   BATTERY_STATUS__LOW_TEMP) { /* <-10C */
			chip->main_cold_bat_decidegc =
				chg_chip->anti_shake_bound.cold_bound +
				high_shake;
			chip->main_little_cold_bat_decidegc =
			...
			chip->main_hot_bat_decidegc =
			   ...
			   BATTERY_STATUS__NORMAL) { /* 16C~45C */
			chip->main_cold_bat_decidegc =
				chg_chip->anti_shake_bound.cold_bound;
			chip->main_little_cold_bat_decidegc =
				chg_chip->anti_shake_bound.little_cold_bound;
		...
		chg_err("MAIN_BAT [%d-%d-%d-%d-%d-%d-%d] t=[%d %d] s=%d\n",
			...
			chip->main_normal_bat_decidegc,
			chip->main_warm_bat_decidegc,
			chip->main_hot_bat_decidegc, chip->main_pre_shake,
			tbatt_cur_shake, chip->main_tbatt_status);
		chip->main_pre_shake = tbatt_cur_shake;
	}

	if (status_change == SUB_STATUS_CHG ||
	    status_change == ALL_STATUS_CHG) {
		...
		if (tbatt_cur_shake > chip->sub_pre_shake) { /* get warmer */
			...
			high_shake_0c = 0;
			   ...
			   chip->sub_pre_shake) { /* get cooler */
			low_shake = 0;
			high_shake = HYSTERISIS_DECIDEGC;
			low_shake_0c = 0;
			high_shake_0c = HYSTERISIS_DECIDEGC_0C;
		}
		if (chip->sub_tbatt_status ==
		    BATTERY_STATUS__HIGH_TEMP) { /* >53C */
		...
		chg_err("SUB_BAT [%d-%d-%d-%d-%d-%d-%d] t=[%d %d] s=%d\n",
			...
			chip->sub_little_cool_bat_decidegc,
			chip->sub_normal_bat_decidegc,
			chip->sub_warm_bat_decidegc, chip->sub_hot_bat_decidegc,
...
static void oplus_chg_check_parallel_tbatt_status(int main_temp, int sub_temp,
						  int *main_status,
						  int *sub_status)
{
	struct oplus_switch_chip *chip = g_switching_chip;
	...
	if (chip->main_tbatt_status != *main_status &&
	...
	chip->sub_tbatt_status = *sub_status;
	if (status_change)
		parallel_battery_anti_shake_handle(status_change, main_temp,
						   sub_temp);
}
					 ...
					 int *target_curr)
{
	...
	oplus_chg_check_parallel_tbatt_status(main_temp, sub_temp,
					      &main_tstatus, &sub_tstatus);
	if (!batt_temp_table[main_tstatus] || !batt_temp_table[sub_tstatus] ||
	...
	if (!oplus_switching_get_hw_enable()) {
		...
		if (main_curr_now + sub_curr_now != 0)
			curr_radio = (main_curr_now * RATIO_ACC) /
				     (main_curr_now + sub_curr_now);
		if (main_curr_now > 0)
			main_curr_radio = (main_curr_now + sub_curr_now) *
					  RATIO_ACC / main_curr_now;
		if (sub_curr_now > 0)
			sub_curr_radio = (main_curr_now + sub_curr_now) *
					 RATIO_ACC / sub_curr_now;
		if (oplus_vooc_get_fastchg_started() &&
			...
			if (unbalance_count == OUT_OF_BALANCE_COUNT) {
				...
				charger_chip->parallel_error_flag |=
					REASON_CURRENT_UNBALANCE;
			}
	...
	for (i = 0; i < chip->parallel_bat_data[main_tstatus].length; i++) {
		if (main_volt >
		    chip->parallel_bat_data[main_tstatus].batt_table[i].volt)
			continue;

		if (main_curr_now > chip->parallel_bat_data[main_tstatus]
					    .batt_table[i]
					    .main_curr) {
	...
	for (i = 0; i < chip->parallel_bat_data[sub_tstatus].length; i++) {
		if (sub_volt >
		    chip->parallel_bat_data[sub_tstatus].batt_table[i].volt)
			continue;

		if (sub_curr_now >
		    chip->parallel_bat_data[sub_tstatus].batt_table[i].sub_curr) {
	...
	chg_err("pre_spec_index: %d index_main: %d pre_sub_spec_index: %d index_sub: %d\n",
		chip->pre_spec_index, index_main, chip->pre_sub_spec_index,
		index_sub);

	if (chip->pre_spec_index != -1 && chip->pre_spec_index > index_main)
		index_main = chip->pre_spec_index;
	if (chip->pre_sub_spec_index != -1 &&
	    chip->pre_sub_spec_index > index_sub)
		index_sub = chip->pre_sub_spec_index;

	target_main_curr = chip->parallel_bat_data[main_tstatus]
				   .batt_table[index_main]
				   .main_curr;
	target_main_curr = target_main_curr * main_curr_radio / RATIO_ACC;

	target_sub_curr = chip->parallel_bat_data[sub_tstatus]
				  .batt_table[index_sub]
				  .sub_curr;
	target_sub_curr = target_sub_curr * sub_curr_radio / RATIO_ACC;

	if (main_curr_need_change == MAIN_BATT_OVER_CURR &&
	    sub_curr_need_change == SUB_BATT_OVER_CURR)
		curr_need_change = (target_main_curr > target_sub_curr) ?
					   SUB_BATT_OVER_CURR :
					   MAIN_BATT_OVER_CURR;
	...
	if (!oplus_switching_get_hw_enable())
		...
		*target_curr = (target_main_curr > target_sub_curr) ?
				       target_sub_curr :
				       target_main_curr;
	...
	if (curr_need_change)
		chg_err("%s battery over current",
			(curr_need_change == MAIN_BATT_OVER_CURR) ? "main" :
								    "sub");
	...
	chg_err("main_volt:%d main_curr_now:%d sub_volt: %d sub_curr_now: %d "
		...
		main_volt, main_curr_now, sub_volt, sub_curr_now, main_temp,
		sub_temp, batt_temp_table[main_tstatus],
		batt_temp_table[sub_tstatus], curr_radio, target_main_curr,
		target_sub_curr, *target_curr, curr_need_change);
