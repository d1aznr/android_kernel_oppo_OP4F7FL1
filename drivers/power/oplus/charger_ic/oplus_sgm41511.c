...
static __maybe_unused int sgm41511_write_reg(struct chip_sgm41511 *chip,
					     int reg, int data)
{
	int ret;
...
static __maybe_unused int sgm41511_config_interface(struct chip_sgm41511 *chip,
...
int sgm41511_set_vindpm_vol(int vol)
{
	...
	tmp = (vol - REG06_SGM41511_VINDPM_OFFSET) /
	      REG06_SGM41511_VINDPM_STEP_MV;
...
static int sgm41511_get_usb_icl(void)
{
	...
	tmp = (tmp & REG00_SGM41511_INPUT_CURRENT_LIMIT_MASK) >>
	      REG00_SGM41511_INPUT_CURRENT_LIMIT_SHIFT;
	return (tmp * REG00_SGM41511_INPUT_CURRENT_LIMIT_STEP +
		REG00_SGM41511_INPUT_CURRENT_LIMIT_OFFSET);
}
...
int sgm41511_input_current_limit_without_aicl(int current_ma)
{
	...
	tmp = (current_ma - REG00_SGM41511_INPUT_CURRENT_LIMIT_OFFSET) /
	      REG00_SGM41511_INPUT_CURRENT_LIMIT_STEP;
	chg_err("tmp current [%d]ma\n", current_ma);
	rc = sgm41511_config_interface(
		chip, REG00_SGM41511_ADDRESS,
...
int sgm41511_input_current_limit_write(int current_ma)
{
	...
	for (pre_icl_index = ARRAY_SIZE(sgm41511_usb_icl) - 1;
	...
	for (i = pre_icl_index; i > 1; i--) {
		rc = sgm41511_input_current_limit_without_aicl(
			sgm41511_usb_icl[i]);
		if (rc) {
	...
	if (buf != NULL && size != 0) {
		...
		if (size > 3) {
			...
			pr_err("[%s] write sgm41511 reg 0x%02x with value 0x%02x !\n",
			       ...
			       reg_value);
			ret = sgm41511_write_reg(chip, chip->reg_access,
						 reg_value);
		} else {
			ret = sgm41511_read_reg(chip, chip->reg_access,
						&reg_value);
...
static DEVICE_ATTR(sgm41511_access, 0660, sgm41511_access_show,
		   sgm41511_access_store);
...
int sgm41511_set_wdt_timer(int reg)
{
	...
	rc = sgm41511_config_interface(chip, REG05_SGM41511_ADDRESS, reg,
...
int sgm41511_set_stat_dis(bool enable)
{
	...
	rc = sgm41511_config_interface(chip, REG00_SGM41511_ADDRESS,
				       enable ? REG00_SGM41511_STAT_DIS_ENABLE :
						REG00_SGM41511_STAT_DIS_DISABLE,
...
int sgm41511_set_int_mask(int val)
{
	...
	rc = sgm41511_config_interface(chip, REG0A_SGM41511_ADDRESS, val,
				       REG0A_SGM41511_VINDPM_INT_MASK |
					       REG0A_SGM41511_IINDPM_INT_MASK);
	if (rc) {
...
bool sgm41511_get_bus_gd(void)
{
	...
	bus_gd = ((reg_val & REG0A_SGM41511_BUS_GD_MASK) ==
		  REG0A_SGM41511_BUS_GD_YES) ?
			 1 :
			 0;
	return bus_gd;
}
...
bool sgm41511_get_power_gd(void)
{
	...
	power_gd = ((reg_val & REG08_SGM41511_POWER_GOOD_STAT_MASK) ==
		    REG08_SGM41511_POWER_GOOD_STAT_GOOD) ?
			   1 :
			   0;
	return power_gd;
}
...
int sgm41511_otg_ilim_set(int ilim)
{
	...
	rc = sgm41511_config_interface(chip, REG02_SGM41511_ADDRESS, ilim,
...
int sgm41511_check_charging_enable(void)
{
	...
	charging_enable = ((reg_val & REG01_SGM41511_CHARGING_MASK) ==
			   REG01_SGM41511_CHARGING_ENABLE) ?
				  1 :
				  0;
...
int sgm41511_unsuspend_charger(void)
{
	...
	if ((chip->before_unsuspend_icl == 0) ||
		...
		sgm41511_input_current_limit_without_aicl(
			chip->before_suspend_icl);
	}
...
int sgm41511_registers_read_full(void)
{
	...
	reg_full = ((reg_full & REG08_SGM41511_CHG_STAT_MASK) ==
		    REG08_SGM41511_CHG_STAT_CHG_TERMINATION) ?
			   1 :
			   0;
	if (reg_full) {
...
int sgm41511_set_ovp(int val)
{
	...
	rc = sgm41511_config_interface(chip, REG06_SGM41511_ADDRESS, val,
...
int sgm41511_get_iindet(void)
{
	...
	is_complete = ((reg_val & REG07_SGM41511_IINDET_EN_MASK) ==
		       REG07_SGM41511_IINDET_EN_DET_COMPLETE) ?
			      1 :
			      0;
	return is_complete;
}
...
int sgm41511_hardware_init(void)
{
	...
	sgm41511_set_int_mask(REG0A_SGM41511_VINDPM_INT_NOT_ALLOW |
			      REG0A_SGM41511_IINDPM_INT_NOT_ALLOW);
...
struct oplus_chg_operations sgm41511_chg_ops = {
	...
	.input_current_ctrl_by_vooc_write =
		sgm41511_input_current_limit_ctrl_by_vooc_write,
...
static int sgm41511_parse_dt(struct chip_sgm41511 *chip)
{
	...
	chip->enable_gpio = of_get_named_gpio(
		chip->client->dev.of_node, "qcom,slave_charg_enable-gpio", 0);
	...
	ret = of_property_read_u32(chip->client->dev.of_node,
				   "qcom,set_sgm41511_ovp",
				   &chip->set_ovp_value);
	if (ret) {
...
static void sgm41511_bc12_retry_work(struct work_struct *work)
{
	...
	struct chip_sgm41511 *chip =
		container_of(dwork, struct chip_sgm41511, bc12_retry_work);
...
static int sgm41511_enable_gpio_init(struct chip_sgm41511 *chip)
{
	...
	chip->enable_gpio_active =
		pinctrl_lookup_state(chip->pinctrl, "charging_switch1_active");
	if (IS_ERR_OR_NULL(chip->enable_gpio_active)) {
	...
	chip->enable_gpio_sleep =
		pinctrl_lookup_state(chip->pinctrl, "charging_switch1_sleep");
	if (IS_ERR_OR_NULL(chip->enable_gpio_sleep)) {
				  ...
				  const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct chip_sgm41511),
			    GFP_KERNEL);
	if (!chip) {
		return -ENOMEM;
	}
