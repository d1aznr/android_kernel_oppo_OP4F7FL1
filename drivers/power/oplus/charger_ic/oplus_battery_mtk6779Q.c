...
static int mt_get_chargerid_volt(void)
{
	...
	if (is_support_chargerid_check() == true) {
		chargerid_volt =
			mt_vadc_read(10, Channel_13); //get the charger id volt
...
static void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	usbtemp_volt =
		mt_vadc_read(1, Channel_12); //get the typec tem adc2 volt
	if (usbtemp_volt <= 0) {
		usbtemp_volt = USB_25C_VOLT;
	}
...
static int oplus_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop() && dischg_flag == false) {
		...
		if ((chip->usb_temp_r >= USB_57C &&
			...
			count_l = 0;
			count = 0;
		} else if (((chip->usb_temp_r * 10 - chip->temperature) >
			    chip->limits.usb_high_than_bat_decidegc) ||
			   ((chip->usb_temp_l * 10 - chip->temperature) >
			    chip->limits.usb_high_than_bat_decidegc)) {
			if (count <= time_count) {
				if (count == 0) {
