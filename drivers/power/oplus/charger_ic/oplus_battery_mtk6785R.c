...
int mt_get_charger_ntc_volt(void)
{
	...
	if (!pinfo->chargeric_temp_chan) {
		...
		charger_ntc_volt = charger_ntc_volt * CHARGE_NTC_MUTIPLY /
				   CHARGE_NTC_DIVIDE;
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	if (rc) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: oplus_usb_ntc_parse_dt fail!\n",
		       __func__);
		return -EINVAL;
	}
	...
	if (rc) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: oplus_vooc30_project_parse_dt fail!\n",
		       __func__);
		return -EINVAL;
	}
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if (pinfo->chargeric_temp_chan) {
		tz_dev = thermal_zone_device_register(
			"mp2762_ntc", 0, 0, oplus_chip,
			&charger_ntc_thermal_zone_ops, NULL, 0, 0);
