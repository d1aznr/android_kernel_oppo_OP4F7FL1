...
{
	return (strncmp(oplus_chg_ops_name_get(), "ext-sy6974b", CHG_OPS_LEN) ==
		0);
}

static bool is_ext_sgm41512_chg_ops(void)
{
	return (strncmp(oplus_chg_ops_name_get(), "ext-sgm41512",
			CHG_OPS_LEN) == 0);
}
...
static int oplus_parse_dt_adc_channels(struct smb_charger *chg)
{
	...
	rc = oplus_get_iio_channel(chg, "usbtemp_r_v_chan",
				   &chg->iio.usbtemp_r_v_chan);
	if (rc < 0)
		return rc;

	rc = oplus_get_iio_channel(chg, "usbtemp_l_v_chan",
				   &chg->iio.usbtemp_l_v_chan);
	if (rc < 0)
		return rc;

	rc = oplus_get_iio_channel(chg, "batbtb_temp_chan",
				   &chg->iio.batbtb_temp_chan);
	if (rc < 0)
		return rc;

	rc = oplus_get_iio_channel(chg, "usbbtb_temp_chan",
				   &chg->iio.usbbtb_temp_chan);
	if (rc < 0)
		return rc;
...
static int oplus_chg_2uart_pinctrl_init(struct oplus_chg_chip *chip)
{
	...
	if (!chip) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: discrete_charger not ready!\n",
		       __func__);
		return -EINVAL;
	}
	...
	chg->chg_2uart_default =
		pinctrl_lookup_state(chg->chg_2uart_pinctrl, "2uart_active");
	...
	chg->chg_2uart_sleep =
		pinctrl_lookup_state(chg->chg_2uart_pinctrl, "2uart_sleep");
	...
	if (!ext_boot_with_console())
		pinctrl_select_state(chg->chg_2uart_pinctrl,
				     chg->chg_2uart_sleep);
#endif
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, keep_vbus_work.work);
...
void oplus_ccdetect_enable(void)
{
	...
	if (chg != NULL && chg->tcpc != NULL) {
		tcpm_typec_change_role_postpone(chg->tcpc, TYPEC_ROLE_TRY_SNK,
						true);
...
void oplus_ccdetect_disable(void)
{
	...
	/* set SINK mode */
	if (chg != NULL && chg->tcpc != NULL) {
		tcpm_typec_change_role_postpone(chg->tcpc, TYPEC_ROLE_SNK,
						true);
...
void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	if (IS_ERR_OR_NULL(chg->iio.usbtemp_r_v_chan)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: chg->iio.usbtemp_r_v_chan is NULL !\n",
		       __func__);
		chip->usbtemp_volt_r = USBTEMP_DEFAULT_VOLT_VALUE_MV;
	} else {
		rc = iio_read_channel_processed(chg->iio.usbtemp_r_v_chan,
						&usbtemp_volt);
		if (rc < 0) {
			chg_err("[OPLUS_CHG][%s]: iio_read_channel_processed get error\n",
				__func__);
	...
	if (IS_ERR_OR_NULL(chg->iio.usbtemp_l_v_chan)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: chg->iio.usbtemp_l_v_chan is NULL !\n",
		       __func__);
		chip->usbtemp_volt_l = USBTEMP_DEFAULT_VOLT_VALUE_MV;
	} else {
		rc = iio_read_channel_processed(chg->iio.usbtemp_l_v_chan,
						&usbtemp_volt);
		if (rc < 0) {
			chg_err("[OPLUS_CHG][%s]: iio_read_channel_processed get error\n",
				__func__);
...
bool oplus_usbtemp_condition(void)
{
	...
	if (oplus_ccdetect_check_is_gpio(g_oplus_chip)) {
		level = gpio_get_value(chg->ccdetect_gpio);
		if (level == 1 ||
		    tcpm_inquire_typec_attach_state(chg->tcpc) ==
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	if (g_oplus_chip) {
		chg->sy6974b_shipmode_enable = of_property_read_bool(
			node, "qcom,use_sy6974b_shipmode");
		chg->external_cclogic = of_property_read_bool(
			node, "qcom,use_external_cclogic");
		chg->pd_not_rise_vbus_only_5v = of_property_read_bool(
			node, "qcom,pd_not_rise_vbus_only_5v");
	}
#endif
	return rc;
}
...
int oplus_get_typec_cc_orientation(void)
{
	...
	if (chg != NULL && chg->tcpc != NULL) {
		if (tcpm_inquire_typec_attach_state(chg->tcpc) !=
		    TYPEC_UNATTACHED) {
	...
	if (oplus_ccdetect_check_is_gpio(chip) == true) {
		...
		if (level != gpio_get_value(chg->ccdetect_gpio)) {
			printk(KERN_ERR
			       ...
			       __func__);
			usleep_range(CCDETECT_RETRY_DELAY_MIN_US,
				     CCDETECT_RETRY_DELAY_MAX_US);
			level = gpio_get_value(chg->ccdetect_gpio);
		}
	} else {
...
void oplus_set_otg_switch_status(bool value)
{
	...
	if (chg != NULL && chg->tcpc != NULL) {
		...
		tcpm_typec_change_role_postpone(
			chg->tcpc, value ? TYPEC_ROLE_TRY_SNK : TYPEC_ROLE_SNK,
			true);
	} else if (chg != NULL && chg->external_cclogic) {
		if (oplus_ccdetect_check_is_gpio(g_oplus_chip) == true) {
			if (gpio_get_value(chg->ccdetect_gpio) == 0) {
	...
	if (chg->pd_active == QTI_POWER_SUPPLY_PD_PPS_ACTIVE) {
		while (1) {
			ret = tcpm_inquire_pd_source_apdo(
				chg->tcpc, TCPM_POWER_CAP_APDO_TYPE_PPS, &cap_i,
				&apdo_cap);
			...
			if (apdo_cap.min_mv <= vbus_mv &&
			    vbus_mv <= apdo_cap.max_mv) {
				vbus = vbus_mv;
				ibus = apdo_cap.ma;
				if (ibus > ibus_ma)
					ibus = ibus_ma;
...
int oplus_chg_set_pd_config(void)
{
	...
	if (chip->dual_charger_support) {
		if ((chip->charger_volt > VBUS_9V_THR_MV &&
			...
			printk(KERN_ERR
			       "%s is dual charger, but not config the vbus and ibus \n",
			       __func__);
			return -1;
		}
	} else {
		if (chip->limits.vbatt_pdqc_to_5v_thr > 0 &&
			...
			printk(KERN_ERR "%s: charger voltage=%d", __func__,
			       qpnp_get_prop_charger_voltage_now());
			oplus_chg_unsuspend_charger();
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		val->intval = chip->ui_soc * chip->batt_capacity_mah *
			      UNIT_TRANS_1000 / FULL_SOC;
		break;
...
static int oplus_discrete_iio_get_prop(struct smb_charger *chg, int channel,
				       int *val)
{
	int rc = 0;
...
static int oplus_discrete_iio_set_prop(struct smb_charger *chg, int channel,
				       int val)
{
	int rc = 0;
	switch (channel) {
		...
		if (val) {
			...
			if (qpnp_is_power_off_charging() &&
				...
				schedule_delayed_work(
					&chg->keep_vbus_work,
					msecs_to_jiffies(KEEP_VBUS_DELAY));
			}
		} else
			chg->pd_hard_reset = 0;
		break;
...
static int oplus_discrete_direct_iio_read(struct device *dev, int iio_chan_no,
					  int *val)
{
...
static int oplus_discrete_direct_iio_write(struct device *dev, int iio_chan_no,
					   int val)
{
	struct smb5 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;
...
static int oplus_discrete_iio_init(struct smb5 *chip,
				   ...
				   struct iio_dev *indio_dev)
{
	...
	for (i = 0; i < chip->nchannels; i++) {
		...
		iio_chan->datasheet_name =
			oplus_discrete_chans_pmic[i].datasheet_name;
		iio_chan->extend_name =
			oplus_discrete_chans_pmic[i].datasheet_name;
		iio_chan->info_mask_separate =
			oplus_discrete_chans_pmic[i].info_mask;
	...
	if (is_ext_mp2650_chg_ops()) {
		...
		if (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY ||
			...
			if (cp_adc_reg == 0) {
				...
				usleep_range(CP_ADC_DELAY_MIN_US,
					     CP_ADC_DELAY_MAX_US);
			}
...
static int discrete_charger_probe(struct platform_device *pdev)
{
	...
	if (oplus_chip->vbatt_num == 1) {
		...
		if (oplus_chip->is_double_charger_support) {
			...
			case (1 << SY6970 | 1 << SGM41511):
				...
				if ((NULL == oplus_chip->chg_ops) ||
				    (NULL == oplus_chip->sub_chg_ops)) {
	...
	chip->iio_chan_ids =
		devm_kcalloc(&pdev->dev, chip->nchannels,
	...
	if (of_find_property(chg->dev->of_node, "nvmem-cells", NULL)) {
		chg->soc_backup_nvmem =
			devm_nvmem_cell_get(chg->dev, "oplus_soc_backup");
