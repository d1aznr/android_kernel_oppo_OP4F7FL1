...
void smblib_hvdcp_detect_enable(struct smb_charger *chg, bool enable)
{
	...
	mask = HVDCP_EN_BIT;
	if (g_oplus_chip->vbatt_num == 2 ||
	    g_oplus_chip->dual_charger_support) {
		if (enable)
			chg_err("enable = %d\n", enable);
				    ...
				    union power_supply_propval *val)
{
	...
	if (chip && chip->vbatt_num == 2 &&
		...
		pr_err("[OPLUS_CHG], ibus current: val->intval[%d]",
		       val->intval);
		return rc;
	}
#endif
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if (g_oplus_chip &&
		    g_oplus_chip->ui_otg_switch != g_oplus_chip->otg_switch)
			oplus_set_otg_switch_status_default(
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	vbus_rising = (bool)(stat & USBIN_PLUGIN_RT_STS_BIT);
	smblib_set_opt_switcher_freq(chg, vbus_rising ?
	...
	if (g_oplus_chip->hiz_gnd_cable && !vbus_rising) {
		...
		if (!ccdetect_level && fastchging) {
			...
			schedule_delayed_work(
				&chg->connect_check_work,
				msecs_to_jiffies(
					OPLUS_CONNECT_CHK_INTERVAL_MS));
		} else if (!fastchging) {
...
void oplus_set_otg_switch_status_default(bool value)
{
	...
	if (vbus_rising == true) {
		chg_debug(
			"vbus_rising, stat 0x1544 = 0x%x, otg_switch = %d, otg_online = %d, ui_otg_switch = %d\n",
			stat, g_oplus_chip->otg_switch,
			g_oplus_chip->otg_online, g_oplus_chip->ui_otg_switch);
		return;
	}
	...
	chg_debug(
		"stat 0x1544 = 0x%x, otg_switch = %d, otg_online = %d, ui_otg_switch=%d\n",
...
irqreturn_t typec_state_change_irq_handler(int irq, void *data)
{
	...
	} else if (oplus_ccdetect_support_check() ==
		   OPLUS_NOT_SUPPORT_CCDETECT) {
		if (chg->typec_present == false) {
			if (oplus_get_otg_switch_status() == false &&
			    oplus_ccdetect_get_power_role() !=
				    POWER_SUPPLY_TYPEC_PR_SINK) {
...
int oplus_chg_set_qc_config()
{
	...
	if (chip->dual_charger_support) {
		...
		if ((chip->soc >= 90 &&
			...
			if (stat & QC_9V_BIT ||
			    chg->qc_abnormal_check_count > 3) {
				ret = smblib_masked_write(chg, CMD_HVDCP_2_REG,
							  FORCE_5V_BIT,
							  FORCE_5V_BIT);
				msleep(500);
			}
	...
	} else if (chip->limits.vbatt_pdqc_to_5v_thr > 0 &&
		   chip->charger_volt > 7500 &&
		   chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr) {
		chip->chg_ops->input_current_write(500);
		oplus_chg_suspend_charger();
		...
		oplus_chg_config_charger_vsys_threshold(
			0x02); //set Vsys Skip threshold 104%
		oplus_chg_enable_burst_mode(false);
		ret = smblib_masked_write(chg, CMD_HVDCP_2_REG, FORCE_9V_BIT,
					  FORCE_9V_BIT);
		msleep(300);
		oplus_chg_unsuspend_charger();
	}
...
static int oplus_chg_2uart_pinctrl_init(struct oplus_chg_chip *chip)
{
	...
	chg->chg_2uart_default = pinctrl_lookup_state(
		chg->chg_2uart_pinctrl, "chg_qupv3_se2_2uart_default");
	...
	chg->chg_2uart_sleep = pinctrl_lookup_state(
		chg->chg_2uart_pinctrl, "chg_qupv3_se2_2uart_sleep");
...
void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	if (IS_ERR_OR_NULL(chg->iio.usbtemp_v_chan_r)) {
		printk(KERN_ERR
		       "[OPLUS_CHG][%s]: chg->iio.usbtemp_v_chan_r  is  NULL !\n",
		       __func__);
		return;
	}

	rc = iio_read_channel_processed(chg->iio.usbtemp_v_chan_r,
					&usbtemp_volt);
	if (rc < 0) {
	...
	if (IS_ERR_OR_NULL(chg->iio.usbtemp_v_chan_l)) {
		...
		if (usbtemp_debug == 1) {
			printk(KERN_ERR
			       "[OPLUS_CHG][%s]: chg->iio.usbtemp_v_chan_l  is  NULL !\n",
			       __func__);
	...
	rc = iio_read_channel_processed(chg->iio.usbtemp_v_chan_l,
					&usbtemp_volt);
	if (rc < 0) {
		if (usbtemp_debug == 1)
...
#endif /*OPLUS_FEATURE_CHG_BASIC*/
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (!chg->pm7250b_bob_reg &&
	    of_get_property(node, "test_bob-supply", NULL)) {
...
int pm7250b_bob_regulator_get_mode(unsigned int *mode)
{
	...
	if (!g_oplus_chip) {
		printk(KERN_ERR
		       "pm7250b_bob_regulator_get_mode: g_oplus_chip NULL\n");
		return -1;
	}
	chg = &g_oplus_chip->pmic_spmi.smb5_chip->chg;
	...
	if (bob_mode != REGULATOR_MODE_FAST &&
	    bob_mode != REGULATOR_MODE_NORMAL &&
	    bob_mode != REGULATOR_MODE_IDLE &&
...
int pm7250b_bob_regulator_set_mode(unsigned int mode)
{
	...
	if (mode == REGULATOR_MODE_FAST) {
		...
		if (rc < 0) {
			printk(KERN_ERR
			       "%s: Couldn't set regulator load=%d rc=%d\n",
			       __func__, ua_load, rc);
			return -1;
		}
		pre_mode = mode;
	...
	case POWER_SUPPLY_PROP_FAST_CHG_TYPE:
		if (((oplus_vooc_get_fastchg_started() == true) ||
			...
			if (val->intval == 0 && g_oplus_chip &&
				...
				val->intval = g_oplus_chip->chg_ops
						      ->get_charger_subtype();
			}
		}
		break;
			      ...
			      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PARALLEL_CURRENT_NOW:
		rc = smblib_get_prop_from_bms(
			chg, POWER_SUPPLY_PROP_PARALLEL_CURRENT_NOW, val);
		break;
#endif
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if (rc < 0) {
		...
		rc = smblib_masked_write(chg, USBIN_AICL_OPTIONS_CFG_REG, mask,
					 val);
		if (rc < 0) {
...
static int oplus_chg_get_ibus(void)
{
	...
	rc = power_supply_get_property(
		chg->usb_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_NOW, &val);
	if (rc < 0) {
		smblib_err(
			chg,
			"Couldn't get POWER_SUPPLY_PROP_INPUT_CURRENT_NOW rc=%d\n",
			rc);
		return rc;
	}
	return val.intval;
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	chg->usbtemp_parameter = of_property_read_bool(
		chg->dev->of_node, "qcom,usbtemp_parameter");
	chg->usbtemp_parameter_20813 = of_property_read_bool(
		chg->dev->of_node, "qcom,usbtemp_parameter_20813");
...
static void smb5_shutdown(struct platform_device *pdev)
{
	...
	if (g_oplus_chip && g_oplus_chip->enable_shipmode) {
		...
		if (oplus_ship_check_is_gpio(g_oplus_chip) == true) {
			...
			msleep(1000);
			rc = smblib_masked_write(chg, SHIP_MODE_REG,
						 SHIP_MODE_EN_BIT,
						 SHIP_MODE_EN_BIT);
			pval.intval = 1;
			power_supply_set_property(
				chg->batt_psy, POWER_SUPPLY_PROP_SET_SHIP_MODE,
				&pval);
			if (rc < 0)
