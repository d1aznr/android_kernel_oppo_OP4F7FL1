...
extern int bq2597x_wl_get_adc_data(struct bq2597x_wl *bq, int channel,
				   int *result);
...
smblib_bypass_vsafe0_control(struct smb_charger *chg, bool enable)
{
	return;
}
/*----For FCC/jeita----------------------------------------*/
...
static int read_data_from_node(struct device_node *node, const char *prop_str,
			       int *addr, int len)
{
	int rc = 0, length;
...
static int wireless_chg_init(struct op_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "op,max-voltage-mv",
				  &chg_param->batt_vol_max);
	if (rc < 0) {
		pr_err("max-voltage_uv reading failed, rc=%d\n", rc);
		chg_param->batt_vol_max = 4550;
	}

	rc = of_property_read_u32(node, "op,fastchg-curr-max-ma",
				  &chg_param->fastchg_curr_max);
	if (rc < 0) {
		pr_err("fastchg-curr-max-ma reading failed, rc=%d\n", rc);
		chg_param->fastchg_curr_max = 1500;
	}

	rc = of_property_read_u32(node, "op,fastchg-curr-min-ma",
				  &chg_param->fastchg_curr_min);
	if (rc < 0) {
		pr_err("fastchg-curr-min-ma reading failed, rc=%d\n", rc);
		chg_param->fastchg_curr_min = 400;
	}

	rc = of_property_read_u32(node, "op,fastchg-vol-entry-max-mv",
				  &chg_param->fastchg_vol_entry_max);
	if (rc < 0) {
		pr_err("fastchg-vol-entry-max-mv reading failed, rc=%d\n", rc);
		chg_param->fastchg_vol_entry_max = 4380;
	}

	rc = of_property_read_u32(node, "op,fastchg-vol-normal-max-mv",
				  &chg_param->fastchg_vol_normal_max);
	if (rc < 0) {
		pr_err("fastchg-vol-normal-max-mv reading failed, rc=%d\n", rc);
		chg_param->fastchg_vol_normal_max = 4480;
	}

	rc = of_property_read_u32(node, "op,fastchg-vol-hot-max-mv",
				  &chg_param->fastchg_vol_hot_max);
	if (rc < 0) {
		pr_err("fastchg-vol-hot-max-mv reading failed, rc=%d\n", rc);
		chg_param->fastchg_vol_hot_max = 4130;
	}

	rc = of_property_read_u32(node, "op,fastchg-vol-min-mv",
				  &chg_param->fastchg_vol_min);
	if (rc < 0) {
		pr_err("fastchg-vol-min-mv reading failed, rc=%d\n", rc);
		chg_param->fastchg_vol_min = 3300;
	}

	rc = of_property_read_u32(node, "op,fastchg-temp-max",
				  &chg_param->fastchg_temp_max);
	if (rc < 0) {
		pr_err("fastchg-temp-max reading failed, rc=%d\n", rc);
		chg_param->fastchg_temp_max = 440;
	}

	rc = of_property_read_u32(node, "op,fastchg-temp-min",
				  &chg_param->fastchg_temp_min);
	if (rc < 0) {
		pr_err("fastchg-temp-min reading failed, rc=%d\n", rc);
		chg_param->fastchg_temp_min = 120;
	}

	rc = of_property_read_u32(node, "op,fastchg-soc-max",
				  &chg_param->fastchg_soc_max);
	if (rc < 0) {
		pr_err("fastchg-soc-max reading failed, rc=%d\n", rc);
		chg_param->fastchg_soc_max = 85;
	}

	rc = of_property_read_u32(node, "op,fastchg-soc-min",
				  &chg_param->fastchg_soc_min);
	if (rc < 0) {
		pr_err("fastchg-soc-min reading failed, rc=%d\n", rc);
		chg_param->fastchg_soc_min = 0;
	}

	rc = of_property_read_u32(node, "op,fastchg-soc-mid",
				  &chg_param->fastchg_soc_mid);
	if (rc < 0) {
		pr_err("fastchg-soc-mid reading failed, rc=%d\n", rc);
		chg_param->fastchg_soc_mid = 75;
	}

	rc = of_property_read_u32(node, "op,fastchg-discharge-curr-max",
				  &chg_param->fastchg_discharge_curr_max);
	if (rc < 0) {
	...
	rc = read_temp_region_data_from_node(node, "op,epp-ibatmax-ma",
					     chg_param->epp_ibatmax);
	if (rc < 0) {
	...
	rc = read_temp_region_data_from_node(node, "op,bpp-ibatmax-ma",
					     chg_param->bpp_ibatmax);
	if (rc < 0) {
	...
	rc = read_temp_region_data_from_node(node, "op,epp-iclmax-ma",
					     chg_param->epp_iclmax);
	if (rc < 0) {
	...
	rc = read_temp_region_data_from_node(node, "op,bpp-iclmax-ma",
					     chg_param->bpp_iclmax);
	if (rc < 0) {
	...
	rc = read_temp_region_data_from_node(node, "vbatdet-mv",
					     chg_param->vbatdet);
	if (rc < 0) {
	...
	rc = read_temp_region_data_from_node(node, "op,fastchg-ibatmax-ma",
					     chg_param->fastchg_ibatmax);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "op,rx-freq-threshold",
	...
	rc = of_property_read_u32(node, "cool-vbat-thr-mv",
				  &chg_param->cool_vbat_thr_mv);
	if (rc < 0) {
		pr_err("cool-vbat-thr-mv reading failed, rc=%d\n", rc);
		chg_param->cool_vbat_thr_mv = 4180;
	}

	rc = of_property_read_u32(node, "cool-epp-ibat-ma",
				  &chg_param->cool_epp_ibat_ma);
	if (rc < 0) {
		pr_err("cool-epp-ibat-ma reading failed, rc=%d\n", rc);
		chg_param->cool_epp_ibat_ma = 1500;
	}

	rc = of_property_read_u32(node, "cool-epp-icl-ma",
				  &chg_param->cool_epp_icl_ma);
	if (rc < 0) {
		pr_err("cool-epp-icl-ma reading failed, rc=%d\n", rc);
		chg_param->cool_epp_icl_ma = 650;
	}

	rc = of_property_read_u32(node, "fastchg-skin-temp-max",
	...
	rc = of_property_read_u32(node, "fastchg-skin-temp-min",
	...
	rc = of_property_read_u32(node, "epp-skin-temp-max",
				  &chg_param->epp_skin_temp_max);
	if (rc < 0) {
		pr_err("epp-skin-temp-max reading failed, rc=%d\n", rc);
		chg_param->epp_skin_temp_max = 390;
	}

	rc = of_property_read_u32(node, "epp-skin-temp-min",
				  &chg_param->epp_skin_temp_min);
	if (rc < 0) {
	...
	chg_param->fastchg_fod_enable =
		of_property_read_bool(node, "op,fastchg-fod-enable");
	if (chg_param->fastchg_fod_enable) {
		...
		rc = of_property_read_u8_array(
			node, "op,fastchg-fod-parm-new",
			(u8 *)&chg_param->fastchg_fod_parm_new,
			FOD_PARM_LENGTH);
		if (rc < 0) {
		...
		rc = of_property_read_u8_array(
			node, "op,fastchg-fod-parm-startup",
			(u8 *)&chg_param->fastchg_fod_parm_startup,
			FOD_PARM_LENGTH);
		if (rc < 0) {
			pr_err("Read op,fastchg-fod-parm failed, rc=%d\n", rc);
...
static int wlchg_fastchg_disable_vote_callback(struct votable *votable,
...
static void wlchg_reset_variables(struct op_chg_chip *chip)
{
	...
	chg_status->vol_not_step =
		false; //By default, voltage drop requires step
...
void wlchg_enable_tx_function(bool is_on)
{
	...
	if (is_on) {
		...
		smblib_set_charge_param(normal_charger,
					&normal_charger->param.otg_cl,
					REVERSE_WIRELESS_CHARGE_CURR_LIMT);
		smblib_set_charge_param(normal_charger,
					&normal_charger->param.otg_vol,
		...
		if (g_op_chip->wlchg_status.wpc_dischg_status !=
...
bool wlchg_wireless_working(void)
{
	...
	working =
		g_op_chip->charger_exist || g_op_chip->wlchg_status.tx_present;
	return working;
}
...
static int wlchg_get_skin_temp(int *temp)
{
	...
	rc = iio_read_channel_processed(normal_charger->iio.op_skin_therm_chan,
					&result);
	if (rc < 0) {
		...
		if (chip->wlchg_status.fastchg_display_delay) {
			if (chip->wlchg_status.charge_online) {
				if (chip->wlchg_status.is_deviation ||
				    ((chip->wlchg_status.temp_region !=
				      WLCHG_BATT_TEMP_PRE_NORMAL) &&
				     (chip->wlchg_status.temp_region !=
				      WLCHG_BATT_TEMP_NORMAL))) {
					chip->wlchg_status
						.fastchg_display_delay = false;
				} else {
...
int wlchg_send_msg(enum WLCHG_MSG_TYPE type, char data, char remark)
{
	...
	if (remark == WPC_RESPONE_ADAPTER_TYPE) {
		...
		if (g_op_chip->chg_base_id >=
		    ARRAY_SIZE(wlchg_base_power_table))
			g_op_chip->chg_base_power = 0;
		else
			g_op_chip->chg_base_power =
				wlchg_base_power_table[g_op_chip->chg_base_id];
		chg_err("get the charging base id = %d",
			g_op_chip->chg_base_id);
	}

	/* set the charging adapter id */
	if (remark == WPC_RESPONE_INTO_FASTCHAGE) {
		g_op_chip->adapter_id = data;
		if (g_op_chip->adapter_id >=
		    ARRAY_SIZE(wlchg_adapter_power_table))
			g_op_chip->adapter_power = 0;
		else
			g_op_chip->adapter_power =
				wlchg_adapter_power_table[g_op_chip->adapter_id];
		chg_err("get the adapter id = %d", g_op_chip->adapter_id);
	}

	if (!g_op_chip->wlchg_msg_ok) {
		mutex_lock(&g_op_chip->msg_lock);
		if ((type == WLCHG_MSG_CHG_INFO) &&
		    (remark == WLCHG_ADAPTER_MSG))
	...
	if (time_is_before_jiffies(chg_status->send_msg_timer)) {
		...
		if (cmd_info->cmd != 0) {
			if (cmd_info->cmd_retry_count != 0) {
				...
				prop->send_msg(prop, cmd_info->cmd_type,
					       cmd_info->cmd);
				if (cmd_info->cmd_retry_count > 0)
					cmd_info->cmd_retry_count--;
			} else {
				wlchg_send_msg(WLCHG_MSG_CMD_ERR, 0,
					       cmd_info->cmd);
				cmd_info->cmd = 0;
			}
		}
...
{
	struct op_chg_chip *chip = container_of(
		filp->private_data, struct op_chg_chip, wlchg_device);
...
static ssize_t wlchg_dev_read(struct file *filp, char __user *buf, size_t count,
			      loff_t *offset)
{
	...
	case WLCHG_NOTIFY_ADAPTER_TYPE:
		...
		if (chip->chg_param.fastchg_fod_enable &&
		    (chg_status->adapter_type == ADAPTER_TYPE_FASTCHAGE_DASH ||
		     chg_status->adapter_type == ADAPTER_TYPE_FASTCHAGE_WARP)) {
			if (chg_status->adapter_id == 0x00 ||
			    chg_status->adapter_id == 0x01)
				wlchg_rx_set_match_q_parm(
					g_rx_chip,
					chip->chg_param.fastchg_match_q);
			else
				wlchg_rx_set_match_q_parm(
					g_rx_chip,
					chip->chg_param.fastchg_match_q_new);
		}
		chg_info(
			"adapter arg is 0x%02x, adapter type is %d, adapter id is %d\n",
...
static int handle_batt_temp_cold(struct op_chg_chip *chip)
{
	...
	if ((chg_status->temp_region != WLCHG_BATT_TEMP_COLD) ||
		...
		chg_param->mBattTempBoundT0 =
			chg_param->BATT_TEMP_T0 + BATT_TEMP_HYST;
		chg_param->mBattTempBoundT1 = chg_param->BATT_TEMP_T1;
		chg_param->mBattTempBoundT2 = chg_param->BATT_TEMP_T2;
		chg_param->mBattTempBoundT3 = chg_param->BATT_TEMP_T3;
		chg_param->mBattTempBoundT4 = chg_param->BATT_TEMP_T4;
...
static int handle_batt_temp_little_cold(struct op_chg_chip *chip)
{
	...
	if ((chg_status->temp_region != WLCHG_BATT_TEMP_LITTLE_COLD) ||
	    chg_status->is_power_changed) {
		chg_status->is_power_changed = false;
			...
			if (chip->pmic_high_vol) {
				vote(normal_charger->usb_icl_votable,
				     ...
				     chg_param->epp_iclmax
						     [chg_status->temp_region] *
					     1000);
				vote(normal_charger->fcc_votable, WLCH_VOTER,
				     true,
				     chg_param->epp_ibatmax
						     [chg_status->temp_region] *
					     1000);
		...
		/* Update the temperature boundaries */
		chg_param->mBattTempBoundT0 = chg_param->BATT_TEMP_T0;
		chg_param->mBattTempBoundT1 =
			chg_param->BATT_TEMP_T1 + BATT_TEMP_HYST;
		chg_param->mBattTempBoundT2 = chg_param->BATT_TEMP_T2;
		chg_param->mBattTempBoundT3 = chg_param->BATT_TEMP_T3;
		chg_param->mBattTempBoundT4 = chg_param->BATT_TEMP_T4;
		chg_param->mBattTempBoundT5 = chg_param->BATT_TEMP_T5;
...
static int handle_batt_temp_cool(struct op_chg_chip *chip)
{
	...
	if ((chg_status->temp_region != WLCHG_BATT_TEMP_COOL) ||
					...
					vote(normal_charger->usb_icl_votable,
					     ...
					     chg_param->epp_iclmax
							     [chg_status
								      ->temp_region] *
						     1000);
					vote(normal_charger->fcc_votable,
					     WLCH_VOTER, true,
					     chg_param->epp_ibatmax
							     [chg_status
								      ->temp_region] *
		...
		chg_param->mBattTempBoundT0 = chg_param->BATT_TEMP_T0;
		chg_param->mBattTempBoundT1 = chg_param->BATT_TEMP_T1;
		chg_param->mBattTempBoundT2 =
			chg_param->BATT_TEMP_T2 + BATT_TEMP_HYST;
		chg_param->mBattTempBoundT3 = chg_param->BATT_TEMP_T3;
		chg_param->mBattTempBoundT4 = chg_param->BATT_TEMP_T4;
		chg_param->mBattTempBoundT5 = chg_param->BATT_TEMP_T5;
		chg_param->mBattTempBoundT6 = chg_param->BATT_TEMP_T6;
...
static int handle_batt_temp_little_cool(struct op_chg_chip *chip)
{
	...
	if ((chg_status->temp_region != WLCHG_BATT_TEMP_LITTLE_COOL) ||
	    chg_status->is_power_changed) {
		chg_status->is_power_changed = false;
		...
		chg_param->mBattTempBoundT1 = chg_param->BATT_TEMP_T1;
		chg_param->mBattTempBoundT2 = chg_param->BATT_TEMP_T2;
		chg_param->mBattTempBoundT3 =
			chg_param->BATT_TEMP_T3 + BATT_TEMP_HYST;
		chg_param->mBattTempBoundT4 = chg_param->BATT_TEMP_T4;
		chg_param->mBattTempBoundT5 = chg_param->BATT_TEMP_T5;
		chg_param->mBattTempBoundT6 = chg_param->BATT_TEMP_T6;
	}
...
static int handle_batt_temp_prenormal(struct op_chg_chip *chip)
{
	...
	if ((chg_status->temp_region != WLCHG_BATT_TEMP_PRE_NORMAL) ||
		...
		if (chg_status->charge_status ==
			...
			vote(chip->wlcs_fcc_votable, JEITA_VOTER, true,
			     FASTCHG_CURR_20W_MAX_UA);
		else
		...
		chg_param->mBattTempBoundT2 = chg_param->BATT_TEMP_T2;
		chg_param->mBattTempBoundT3 = chg_param->BATT_TEMP_T3;
		chg_param->mBattTempBoundT4 =
			chg_param->BATT_TEMP_T4 + BATT_TEMP_HYST;
		chg_param->mBattTempBoundT5 = chg_param->BATT_TEMP_T5;
		chg_param->mBattTempBoundT6 = chg_param->BATT_TEMP_T6;
	}
...
static int handle_batt_temp_warm(struct op_chg_chip *chip)
{
	...
	if ((chg_status->temp_region != WLCHG_BATT_TEMP_WARM) ||
	    chg_status->is_power_changed) {
		chg_status->is_power_changed = false;
		...
		chg_param->mBattTempBoundT3 = chg_param->BATT_TEMP_T3;
		chg_param->mBattTempBoundT4 = chg_param->BATT_TEMP_T4;
		chg_param->mBattTempBoundT5 =
			chg_param->BATT_TEMP_T5 - BATT_TEMP_HYST;
		chg_param->mBattTempBoundT6 = chg_param->BATT_TEMP_T6;
	}
...
static int handle_batt_temp_hot(struct op_chg_chip *chip)
{
	...
	if ((chg_status->temp_region != WLCHG_BATT_TEMP_HOT) ||
	    chg_status->is_power_changed) {
		chg_status->is_power_changed = false;
		if (normal_charger) {
			vote(normal_charger->fcc_votable, WLCH_VOTER, true, 0);
...
static int op_check_battery_temp(struct op_chg_chip *chip)
{
	...
	if (chip->temperature < chg_param->mBattTempBoundT0) /* COLD */
		rc = handle_batt_temp_cold(chip);
		 ...
		 chip->temperature <
			 chg_param->mBattTempBoundT1) /* LITTLE_COLD */
...
static int pmic_chan_check_skin_temp(struct op_chg_chip *chip)
{
	...
	if (skin_temp >= chg_param->epp_skin_temp_max) {
		...
		vote(normal_charger->usb_icl_votable, WLCH_SKIN_VOTER, true,
		     chg_param->epp_curr_step[chg_status->epp_curr_step] *
			     1000);
		wait_timeout = jiffies + 30 * HZ;
	} else if (skin_temp <= chg_param->epp_skin_temp_min) {
		if (chg_status->epp_curr_step < 1) {
			if (is_client_vote_enabled(
				    normal_charger->usb_icl_votable,
				    WLCH_SKIN_VOTER))
				vote(normal_charger->usb_icl_votable,
				     WLCH_SKIN_VOTER, false, 0);
			return 0;
		}
		chg_status->epp_curr_step--;
...
static int fastchg_check_skin_temp(struct op_chg_chip *chip)
{
	...
	if (skin_temp >= chg_param->fastchg_skin_temp_max) {
		...
		if (chg_status->fastchg_curr_step <= chg_status->fastchg_level)
			chg_status->fastchg_curr_step =
				chg_status->fastchg_level + 1;

		if (chg_status->fastchg_curr_step >= ffc_chg->max_step) {
			vote(chip->fastchg_disable_votable, SKIN_VOTER, true,
			     0);
...
static void fastchg_switch_next_step(struct op_chg_chip *chip)
{
	...
	struct op_fastchg_ffc_step *ffc_chg = &chip->chg_param.ffc_chg;
	u32 batt_vol_max =
		ffc_chg->ffc_step[chg_status->fastchg_level].vol_max_mv;
	...
	if (chip->batt_volt >= batt_vol_max) {
		...
		if ((chg_status->temp_region == WLCHG_BATT_TEMP_PRE_NORMAL) &&
		    (ffc_chg->ffc_step[chg_status->fastchg_level].curr_ua * 4 >=
		     chg_param->fastchg_ibatmax[0])) {
			ffc_chg->ffc_wait_timeout = jiffies;
		}
	}
}
...
static void fastchg_temp_check(struct op_chg_chip *chip)
{
	...
	if (def_curr_ua <= 0)
		def_curr_ua =
			get_client_vote(chip->wlcs_fcc_votable, DEF_VOTER);
	else
		def_curr_ua =
			min(get_client_vote(chip->wlcs_fcc_votable, DEF_VOTER),
			    def_curr_ua);
	...
	pr_debug("battery temp = %d, vol = %d, level = %d, temp_diff = %d\n",
		 batt_temp, chip->batt_volt, chg_status->fastchg_level,
		 temp_diff);

	if (chg_status->fastchg_level == 0) {
		if (def_curr_ua < ffc_curr_ua) {
			if ((chg_status->fastchg_level + 1) <
			    ffc_chg->max_step) {
				if (def_curr_ua <
				    ffc_chg->ffc_step[chg_status->fastchg_level +
						      1]
					    .curr_ua) {
					chg_info(
						"target current too low, switch next step\n");
		...
		if ((batt_temp > ffc_chg->ffc_step[chg_status->fastchg_level]
					 .high_threshold) ||
		    (chip->batt_volt >= batt_vol_max)) {
			fastchg_switch_next_step(chip);
		}
	} else if (chg_status->fastchg_level >=
		   ffc_chg->max_step) { // switch to pmic
		vote(chip->fastchg_disable_votable, FFC_VOTER, true, 0);
		chg_status->charge_status = WPC_CHG_STATUS_FAST_CHARGING_EXIT;
	} else {
		if (def_curr_ua < ffc_curr_ua) {
		...
		if (time_after(jiffies, ffc_chg->ffc_wait_timeout) ||
		    (temp_diff > 200)) {
			if ((batt_temp >
			     ffc_chg->ffc_step[chg_status->fastchg_level]
				     .high_threshold) ||
...
void wlchg_check_term_charge(struct op_chg_chip *chip)
{
	...
	if (!chg_status->cep_timeout_adjusted &&
	...
	if (wlchg_check_charge_done(chip)) {
		...
		if (chg_status->charge_voltage != WPC_CHARGE_VOLTAGE_STOP_CHG) {
			wlchg_set_rx_charge_current(
				chip, WPC_CHARGE_CURRENT_STOP_CHG);
			wlchg_set_rx_target_voltage(
				chip, WPC_CHARGE_VOLTAGE_STOP_CHG);
		}

		if (!normal_charger->chg_disabled) {
			chg_info(
				"charge full, disable little current charge to battery.");
			normal_charger->chg_disabled = true;
			vote(normal_charger->chg_disable_votable, WLCH_VOTER,
			     true, 0);
		}

		if (!chg_status->quiet_mode_enabled &&
		    skin_temp < CHARGE_FULL_FAN_THREOD_LO)
			wlchg_send_msg(WLCHG_MSG_CHG_INFO, -1,
		...
		if (chg_status->charge_voltage != WPC_CHARGE_VOLTAGE_EPP) {
			wlchg_set_rx_target_voltage(chip,
						    WPC_CHARGE_VOLTAGE_EPP);
			wlchg_set_rx_charge_current_step(
				chip, WPC_CHARGE_CURRENT_EPP);
		}
		...
		if (skin_temp < chg_param->fastchg_skin_temp_min &&
		    is_client_vote_enabled(chip->fastchg_disable_votable,
					   SKIN_VOTER)) {
			vote(chip->fastchg_disable_votable, SKIN_VOTER, false,
			     0);
			chg_info(
				"skin temp is %d(below %d), restore fastcharge.",
...
static void wlchg_fastchg_restart_check(struct op_chg_chip *chip)
{
	...
	if (is_client_vote_enabled(chip->fastchg_disable_votable, FFC_VOTER) &&
	    (chip->temperature <
	     (ffc_chg->ffc_step[ffc_chg->max_step - 1].high_threshold -
	      BATT_TEMP_HYST))) {
		vote(chip->fastchg_disable_votable, FFC_VOTER, false, 0);
		chg_status->fastchg_level = ffc_chg->max_step - 1;
	}

	if (is_client_vote_enabled(chip->fastchg_disable_votable,
				   BATT_CURR_VOTER) &&
	    (chip->icharging < 0))
	...
	if (is_client_vote_enabled(chip->fastchg_disable_votable,
		...
		vote(chip->fastchg_disable_votable, STARTUP_CEP_VOTER, false,
		     0);
}
	...
	if (!chg_status->wait_cep_stable) {
		...
		if (cep < 3) {
			...
			if ((cep_ok_count >= CEP_OK_MAX) &&
			    time_after(jiffies,
				       chg_status->cep_ok_wait_timeout) &&
			    is_client_vote_enabled(chip->wlcs_fcc_votable,
						   CEP_VOTER)) {
				...
				wait_cep_count = 0;
				vote(chip->wlcs_fcc_votable, CEP_VOTER, false,
				     0);
			}
		} else {
			cep_ok_count = 0;
			...
			if (cep_err_count >= CEP_ERR_MAX) {
				...
				wait_cep_count = 0;
				if (is_client_vote_enabled(
					    chip->wlcs_fcc_votable, CEP_VOTER))
					cep_curr_ua = get_client_vote(
						chip->wlcs_fcc_votable,
						CEP_VOTER);
				else
					cep_curr_ua = 0;
				if ((cep_curr_ua > 0) &&
				    (cep_curr_ua <= FASTCHG_CURR_MIN_UA)) {
					chg_info(
						"Energy is too low, exit fast charge\n");
					vote(chip->fastchg_disable_votable,
					     CEP_VOTER, true, 0);
					chg_status->charge_status =
						WPC_CHG_STATUS_FAST_CHARGING_EXIT;
				} else {
...
retry:
	...
	if (cp_status & (CP_DWP | CP_SWITCH_OCP | CP_CRP | CP_VOUT_OVP |
			 CP_CLP | CP_VBUS_OVP)) {
	...
	if (cp_status &
	    (CP_SWITCH_OCP | CP_CRP | CP_VOUT_OVP | CP_CLP | CP_VBUS_OVP)) {
		chg_err("charge pump status error, status=0x%02x\n", cp_status);
		goto disable_cp;
	}
wait_cp_enable:
...
static int fastchg_startup_process(struct op_chg_chip *chip)
{
	...
	if ((cp1_status != CP_REEADY) &&
		...
		if (cp1_err_count > 10) {
			...
			cp1_err_count = 0;
			vote(chip->fastchg_disable_votable, HW_ERR_VOTER, true,
			     0);
			chg_status->charge_status =
				WPC_CHG_STATUS_FAST_CHARGING_EXIT;
		} else {
		...
		if (chg_status->fastchg_startup_step !=
		    FASTCHG_EN_CHGPUMP1_STEP)
			cp1_err_count = 0;
	}
	...
	if (chg_status->vol_set_ok ||
		...
		case FASTCHG_EN_CHGPUMP1_STEP:
			if (g_rx_chip->chg_data.vout <=
			    OP20A_ENABLE_VOL_MIN_MV) {
				wlchg_set_rx_target_voltage(
					chip, WPC_CHARGE_VOLTAGE_FASTCHG_INIT);
				break;
			}
		...
		case FASTCHG_WAIT_CP1_STABLE_STEP:
			...
			if (bq_adc_vbus > temp_value) {
				if (chg_status->charge_current !=
				    WPC_CHARGE_CURRENT_WAIT_FAST) {
				...
				if (vout_mv >= WPC_CHARGE_VOLTAGE_OVP_MIN &&
				    chg_status->charge_current != 0)
					wlchg_set_rx_charge_current(chip, 0);
				chg_info(
					"chargepump 1 is not ready, wait 100ms\n");
				break;
			}
		...
		case FASTCHG_SET_CHGPUMP2_VOL_AGAIN_STEP:
			if (exchgpump_bq != NULL) {
				bq2597x_wl_check_charge_enabled(
					exchgpump_bq, &cp2_is_enabled);
				if (cp2_is_enabled) {
				...
				temp_value =
					bq_adc_vbat * 2 + bq_adc_vbat * 2 / 10;
			} else {
				temp_value = chip->batt_volt * 2 +
					     chip->batt_volt * 2 / 10;
			}
			if (bq_adc_vbus > (temp_value - 50) &&
				...
				if (chg_status->vol_set_ok && !cp2_is_enabled) {
					temp_value =
						(temp_value - bq_adc_vbus) * 2;
					wlchg_set_rx_target_voltage(
						chip, g_rx_chip->chg_data.vout +
							      temp_value);
		...
		case FASTCHG_CHECK_CHGPUMP2_AGAIN_STEP:
			...
			if (cp2_is_enabled) {
				...
				if (chip->soc < chg_param->fastchg_soc_mid) {
					if (chg_status->adapter_type ==
					    ADAPTER_TYPE_FASTCHAGE_WARP) {
						vote(chip->wlcs_fcc_votable,
						     DEF_VOTER, true,
						     FASTCHG_CURR_30W_MAX_UA);
						vote(chip->wlcs_fcc_votable,
						     MAX_VOTER, true,
					...
					vote(chip->wlcs_fcc_votable, DEF_VOTER,
					     true, FASTCHG_CURR_15W_MAX_UA);
					vote(chip->wlcs_fcc_votable, MAX_VOTER,
					     true, FASTCHG_CURR_15W_MAX_UA);
					chg_status->max_current =
						FASTCHG_CURR_15W_MAX_UA / 1000;
				}
				vote(chip->wlcs_fcc_votable, EXIT_VOTER, false,
				     0);
				vote(chip->wlcs_fcc_votable, JEITA_VOTER, false,
				     0);
				chg_status->startup_fast_chg = false;
				chg_status->fastchg_startup_step =
					FASTCHG_EN_CHGPUMP1_STEP;
				...
				if (!chg_status->fastchg_restart) {
					...
					vote(chip->wlcs_fcc_votable, FFC_VOTER,
					     true,
					     chg_param->ffc_chg
						     .ffc_step[chg_status->fastchg_level]
						     .curr_ua);
				}
				fastchg_curr_control_en(chip, true);
				chg_info(
					"enable chgpump success, try num: %d\n",
					cp2_err_count);
				cp2_err_count = 0;
			} else {
				chg_status->fastchg_startup_step =
		...
		case FASTCHG_EN_PMIC_CHG_STEP:
			temp_value = g_rx_chip->chg_data.vout *
				     CP1_STABILITY_THR / 200;
			if (bq_adc_vbus > temp_value) {
				chg_info("enable pmic charge\n");
				pmic_high_vol_en(chip, true);
				wlchg_set_rx_charge_current(
					chip, WPC_CHARGE_CURRENT_WAIT_FAST);
				curr_err_count = 0;
				temp_value = g_op_chip->batt_volt * 4;
	...
	if (!chg_status->vol_set_ok &&
		...
		if (cep_err_count > 300) { //30s
			...
			chg_status->fastchg_retry_timer =
				jiffies + 300 * HZ; //5 min
			vote(chip->fastchg_disable_votable, STARTUP_CEP_VOTER,
			     true, 0);
			chg_status->charge_status =
				WPC_CHG_STATUS_FAST_CHARGING_EXIT;
		}
...
static int wlchg_charge_status_process(struct op_chg_chip *chip)
{
	...
	if (exchgpump_bq != NULL) {
		if (exchgpump_bq->adc_enabled) {
			bq2597x_wl_get_adc_data(exchgpump_bq, ADC_VBAT,
						&bq_adc_vbat);
		} else {
	...
	if (!chg_status->ftm_mode) {
		...
		if (chg_status->temp_region == WLCHG_BATT_TEMP_COLD ||
			...
			if (chg_status->charge_current !=
				...
				wlchg_set_rx_charge_current(
					chip, WPC_CHARGE_CURRENT_ZERO);

			if (chg_status->charge_voltage !=
			    WPC_CHARGE_VOLTAGE_DEFAULT) {
				chg_status->vol_not_step = true;
				wlchg_set_rx_target_voltage(
					chip, WPC_CHARGE_VOLTAGE_DEFAULT);
			}
			...
			if (((chip->quiet_mode_need !=
			    ...
			    ((chg_status->adapter_type ==
			      ADAPTER_TYPE_FASTCHAGE_DASH) ||
			     (chg_status->adapter_type ==
			      ADAPTER_TYPE_FASTCHAGE_WARP)) &&
			    (atomic_read(&chip->hb_count) > 0) &&
			    !chg_status->charge_done) {
				if (chip->quiet_mode_need) {
					// dock should in quiet mode, goto 10w.
					chg_info(
						"send msg to dock into quiet mode.");
					wlchg_send_msg(WLCHG_MSG_CHG_INFO, -1,
						       WLCHG_QUIET_MODE_MSG);
					...
					chg_info(
						"send msg to dock restore normal mode.");
					wlchg_send_msg(WLCHG_MSG_CHG_INFO, -1,
						       WLCHG_NORMAL_MODE_MSG);
				}
			}
			...
			if (chip->disable_batt_charge &&
				...
				if ((chg_status->adapter_type ==
				     ...
				     chg_status->adapter_type ==
					     ADAPTER_TYPE_FASTCHAGE_WARP) &&
				    (chg_status->charge_type ==
				     WPC_CHARGE_TYPE_FAST) &&
				    chg_status->deviation_check_done) {
					if (chip->chg_param.fastchg_fod_enable &&
					    chg_status->startup_fod_parm) {
						if (chg_status->adapter_id ==
							    0x00 ||
						...
						chg_status->startup_fod_parm =
							false;
						chg_info(
							"write fastchg fod parm\n");
					}
					pmic_high_vol_en(chip, true);
					wlchg_set_rx_target_voltage(
						chip,
						WPC_CHARGE_VOLTAGE_STOP_CHG);
					wlchg_set_rx_charge_current(
						chip,
						WPC_CHARGE_CURRENT_STOP_CHG);
					chg_status->charge_status =
						WPC_CHG_STATUS_WAIT_DISABLE_BATT_CHARGE;
				}
			}
	...
	case WPC_CHG_STATUS_DEFAULT:
...
#ifndef IDT_LAB_TEST
		if (!chg_status->adapter_msg_send &&
		    (atomic_read(&chip->hb_count) > 0)) {
			chg_err("can't send adapter msg, try again\n");
			wlchg_send_msg(WLCHG_MSG_CHG_INFO, 5,
				       WLCHG_ADAPTER_MSG);
			break;
		}
#endif
		...
		if (chg_status->adapter_type == ADAPTER_TYPE_UNKNOWN) {
			...
			if (chg_status->charge_current !=
				...
				wlchg_set_rx_charge_current(
					chip, WPC_CHARGE_CURRENT_DEFAULT);

			if (wlchg_rx_get_run_mode(rx_chip) ==
			    RX_RUNNING_MODE_EPP) {
				...
				break;
			} else if (wlchg_rx_get_run_mode(rx_chip) ==
				   RX_RUNNING_MODE_BPP) {
#ifndef IDT_LAB_TEST
				if (!chg_status->get_adapter_err &&
				    cmd_info->cmd == 0) {
					rc = wlchg_send_msg(WLCHG_MSG_CHG_INFO,
							    5,
							    WLCHG_ADAPTER_MSG);
					if (rc < 0)
						break;
				...
				wait_fast_chg = false;
					...
					chg_status->fastchg_display_delay =
						false;
					chg_status->charge_status =
			...
			if (!chg_status->deviation_check_done &&
				...
				rc = wlchg_rx_get_work_freq(rx_chip,
							    &work_freq);
				if (rc != 0) {
	...
	case WPC_CHG_STATUS_READY_FOR_BPP:
		...
		if ((atomic_read(&chip->hb_count) > 0) &&
		    (chg_status->adapter_type == ADAPTER_TYPE_UNKNOWN)) {
			rc = wlchg_send_msg(WLCHG_MSG_CHG_INFO, 5,
					    WLCHG_TX_ID_MSG);
			if (rc) {
	...
	case WPC_CHG_STATUS_BPP:
		...
		if (rx_chip->on_op_trx) {
			wlchg_set_rx_charge_current(chip,
						    WPC_CHARGE_CURRENT_ON_TRX);
			chg_info("It's on back of OP phone Trx, set current %d",
				 WPC_CHARGE_CURRENT_ON_TRX);
		} else {
			wait_fast_chg = true;
			wlchg_set_rx_charge_current(chip,
						    WPC_CHARGE_CURRENT_BPP);
			chg_info("It's on BPP dock, set current %d",
				 WPC_CHARGE_CURRENT_BPP);
#ifndef IDT_LAB_TEST
			if (atomic_read(&chip->hb_count) > 0) {
				rc = wlchg_send_msg(WLCHG_MSG_CHG_INFO, -1,
						    WLCHG_ADAPTER_MSG);
				if (rc) {
	...
	case WPC_CHG_STATUS_READY_FOR_QUIET:
		...
		if (chg_status->quiet_mode_enabled) {
			...
			if (chg_status->charge_type != WPC_CHARGE_TYPE_FAST) {
				wlchg_set_rx_target_voltage_fast(
			...
			vote(chip->fastchg_disable_votable, QUIET_VOTER, true,
			     0);
			chg_status->charge_status =
				WPC_CHG_STATUS_FAST_CHARGING_EXIT;
		} else {
	...
	case WPC_CHG_STATUS_WAITING_FOR_TX_INTO_FASTCHG:
		...
		if (chg_status->vol_set_ok &&
			...
			if (chg_status->ftm_mode) {
				...
				wlchg_set_rx_target_voltage(
					chip, WPC_CHARGE_VOLTAGE_FTM);
				chg_status->charge_status =
					WPC_CHG_STATUS_READY_FOR_FTM;
			} else {
				if (((chg_status->temp_region ==
				      WLCHG_BATT_TEMP_PRE_NORMAL) ||
				     (chg_status->temp_region ==
				      WLCHG_BATT_TEMP_NORMAL)) &&
				    (chip->batt_volt >
					...
					if (chip->chg_param.fastchg_fod_enable) {
						wlchg_rx_set_fod_parm(
							g_rx_chip,
							chip->chg_param
								.fastchg_fod_parm_startup);
						chg_status->startup_fod_parm =
							true;
						chg_info(
							"write fastchg startup fod parm\n");
					}
				} else {
					if (chip->batt_volt >=
					    chg_param->fastchg_vol_entry_max)
						vote(chip->fastchg_disable_votable,
						     BATT_VOL_VOTER, true, 0);
		...
		chg_info("vol_set_ok=%d, charge_type=%d",
			 chg_status->vol_set_ok, chg_status->charge_type);
		break;
	...
	case WPC_CHG_STATUS_INCREASE_VOLTAGE:
		...
		if (chip->batt_volt >= chg_param->fastchg_vol_entry_max &&
			...
			chg_status->startup_fast_chg = false;
			vote(chip->fastchg_disable_votable, BATT_VOL_VOTER,
			     true, 0);
			chg_status->charge_status =
				WPC_CHG_STATUS_FAST_CHARGING_EXIT;
			break;
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHGPUMP:
		...
		if (chg_status->is_deviation) {
			temp_val = get_client_vote(chip->wlcs_fcc_votable,
						   MAX_VOTER);
	...
	case WPC_CHG_STATUS_FAST_CHARGING_EXIT:
		...
		wlchg_set_rx_charge_current(
			chip, WPC_CHARGE_CURRENT_CHGPUMP_TO_CHARGER);
		chg_status->charge_status =
	...
	case WPC_CHG_STATUS_FTM_WORKING:
		...
		if (exchgpump_bq != NULL)
			bq2597x_wl_check_charge_enabled(exchgpump_bq,
							&cp2_is_enabled);
		if (!cp2_is_enabled) {
...
void wlchg_dischg_status(struct op_chg_chip *chip)
{
	...
	if (err_flag != 0) {
		...
		if (chip->wlchg_status.wpc_dischg_status >=
		    WPC_DISCHG_IC_ERR_TX_RXAC) {
	...
	if (tx_status != 0) {
		if (TRX_READY & tx_status) {
			...
			schedule_delayed_work(&chip->dischg_work,
					      WPC_DISCHG_WAIT_READY_EVENT);
			trycount = 0;
	...
	if (chip->wireless_mode != WIRELESS_MODE_TX) {
		...
		if (normal_charger != NULL) {
			...
			normal_charger->usb_psy_desc.type =
				POWER_SUPPLY_TYPE_WIRELESS;
			vote(normal_charger->usb_icl_votable, SW_ICL_MAX_VOTER,
			     true, PMIC_ICL_MAX);
		}
...
static void wlchg_disconnect_func(struct op_chg_chip *chip)
{
	...
	if (normal_charger != NULL) {
		...
		vote(normal_charger->usb_icl_votable, WIRED_CONN_VOTER, false,
		     0);
		...
		vote(normal_charger->usb_icl_votable, WLCH_SKIN_VOTER, false,
		     0);
		normal_charger->wireless_present = false;
		// remove typec related icl vote
		vote(normal_charger->usb_icl_votable, CHG_TERMINATION_VOTER,
		     false, 0);
	...
	if (chip->wlchg_status.ftm_mode) {
		...
		pval.intval = 1;
		rc = power_supply_set_property(
			chip->batt_psy, POWER_SUPPLY_PROP_INPUT_SUSPEND, &pval);
		if (rc < 0) {
			pr_err("Couldn't set input_suspend rc=%d\n", rc);
		}
	}
...
#ifdef OPLUS_CUSTOM_OP_DEF
				smblib_bypass_vsafe0_control(normal_charger,
							     true);
#endif
			}
			if (chip->wireless_psy != NULL)
...
static void wlchg_usbin_int_func(struct work_struct *work)
{
	...
	if (level == 1) {
		if (normal_charger != NULL &&
		    normal_charger->wireless_present) {
			normal_charger->wireless_present = false;
		...
		if ((chip->wireless_mode == WIRELESS_MODE_NULL) &&
			...
			vote(normal_charger->awake_votable, WIRED_CONN_VOTER,
			     false, 0);
			return;
		}
		msleep(100);
...
static void wlchg_usbin_int_shedule_work(void)
{
	...
	if (!g_op_chip) {
		...
		if (!g_op_chip->pd_charger_online)
			...
			chg_info(
				"PD is in, usbin irq work func delay 1 seconds run.");
			schedule_delayed_work(&g_op_chip->usbin_int_work,
					      WLCHG_PD_HARDRESET_WAIT_TIME);
		}
	}
...
static int wlchg_usbin_int_eint_register(struct op_chg_chip *chip)
{
	...
	retval = devm_request_irq(
		chip->dev, chip->usbin_int_irq, irq_usbin_event_int_handler,
...
static int wlchg_gpio_init(struct op_chg_chip *chip)
{
	...
	chip->usbin_int_gpio =
		of_get_named_gpio(node, "qcom,usbin_int-gpio", 0);
	if (chip->usbin_int_gpio < 0) {
		chg_err("chip->usbin_int_gpio not specified\n");
		return -EINVAL;
	} else {
		if (gpio_is_valid(chip->usbin_int_gpio)) {
			rc = gpio_request(chip->usbin_int_gpio,
					  "usbin-int-gpio");
			if (rc) {
				...
				if (rc) {
					...
					rc = wlchg_usbin_int_eint_register(
						chip);
					if (rc < 0) {
	...
	if (chip->ap_ctrl_dcdc) {
		...
		chip->dcdc_en_gpio =
			of_get_named_gpio(node, "qcom,dcdc_en-gpio", 0);
		if (chip->dcdc_en_gpio < 0) {
			...
			if (gpio_is_valid(chip->dcdc_en_gpio)) {
				rc = gpio_request(chip->dcdc_en_gpio,
						  "dcdc_en-gpio");
				if (rc) {
					chg_err("unable to request gpio [%d]\n",
						chip->dcdc_en_gpio);
...
static void curr_vol_check_process(struct work_struct *work)
{
	...
	if (chg_status->curr_limit_mode) {
		...
		ibat_err = ((ibat_max - abs(chip->icharging)) / 4) -
			   (CURR_ERR_MIN / 2);
		/* Prevent the voltage from increasing too much, ibat exceeds expectations */
		if ((ibat_err > -(CURR_ERR_MIN / 2)) && (ibat_err < 0) &&
		    (tmp_val > 0)) {
			/*
		...
		if (tmp_val < 0) {
			if (cep_flag != 0)
				...
				cep_err_count = 0;
			if (!chg_status->curr_need_dec ||
			    cep_err_count >= CEP_ERR_MAX) {
				skip_cep_check = true;
		...
		if (cep_flag == 0 || skip_cep_check) {
			if (tmp_val > 0 || tmp_val < -CURR_ERR_MIN) {
				...
				if (chg_status->charge_type ==
				    WPC_CHARGE_TYPE_FAST) {
					if (chg_status->vol_set <
					    FASTCHG_MODE_VOL_MIN) {
						chg_status->vol_set =
							FASTCHG_MODE_VOL_MIN;
					}
				} else {
...
static void wlchg_tx_check_process(struct work_struct *work)
{
	...
	if (chip->wireless_mode == WIRELESS_MODE_TX) {
		...
		schedule_delayed_work(&chip->tx_check_work,
				      msecs_to_jiffies(500));
	} else {
		chg_err("<~WPC~ wireless mode is %d, not TX, exit.",
			chip->wireless_mode);
	}
}
...
static void wlchg_check_charge_timeout(struct op_chg_chip *chip)
{
	...
	if (chip->disable_batt_charge) {
		chg_info(
			"wireless charge is disabled, reset count and return.");
		chip->wlchg_time_count = 0;
		return;
	}
					     ...
					     loff_t *ppos)
{
	...
	len = simple_read_from_buffer(buf, count, ppos, vol_string,
				      strlen(vol_string));
	return len;
}
...
static ssize_t proc_wireless_quiet_mode_read(struct file *file,
					     char __user *buf, size_t count,
					     loff_t *ppos)
{
	uint8_t ret = 0;
...
static ssize_t proc_wireless_quiet_mode_write(struct file *file,
					      const char __user *buf,
					      size_t count, loff_t *lo)
{
	char buffer[3] = { 0 };
...
static ssize_t proc_fast_skin_threld_write(struct file *file,
					   const char __user *buf, size_t count,
					   loff_t *lo)
{
	char buffer[16] = { 0 };
...
static ssize_t proc_match_q_read(struct file *file, char __user *buf,
				 size_t count, loff_t *ppos)
{
	char string[8];
	int len = 0;
...
static ssize_t proc_match_q_write(struct file *file, const char __user *buf,
				  size_t count, loff_t *lo)
{
	char string[16];
	int match_q = 0;
...
static ssize_t proc_wireless_ftm_test_read(struct file *file, char __user *buf,
					   size_t count, loff_t *ppos)
{
	char string[2] = { 0, 0 };
	int rc;
...
static int wlchg_driver_probe(struct platform_device *pdev)
{
	...
	chip->ap_ctrl_dcdc =
		of_property_read_bool(chip->dev->of_node, "op-ap_control_dcdc");
	chg_info("%s control dcdc\n", chip->ap_ctrl_dcdc ? "AP" : "RX");
	ret = of_property_read_u32(chip->dev->of_node, "op,wls-power",
				   &chip->wls_prj_power);
	if (ret < 0) {
	...
	/*--------------------For OPLUG Interface------------------*/
	wpc_chip = devm_kzalloc(&pdev->dev, sizeof(struct oplus_wpc_chip),
				GFP_KERNEL);
	if (!wpc_chip) {
	...
	INIT_DELAYED_WORK(&chip->wlchg_connect_check_work,
			  wlchg_connect_check_work);
	INIT_DELAYED_WORK(&chip->wlchg_fcc_stepper_work,
			  wlchg_fcc_stepper_work);
#ifdef HW_TEST_EDITION
	...
	chip->wlchg_wake_lock =
		wakeup_source_register(chip->dev, "wlchg_wake_lock");
	chip->reverse_wlchg_wake_lock =
		wakeup_source_register(chip->dev, "reverse_wlchg_wake_lock");
	...
	chip->fastchg_disable_votable =
		create_votable("WLCHG_FASTCHG_DISABLE", VOTE_SET_ANY,
			       wlchg_fastchg_disable_vote_callback, chip);
