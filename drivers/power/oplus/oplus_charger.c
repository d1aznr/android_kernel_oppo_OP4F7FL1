...
/* update cycle 5s */
#define OPLUS_CHG_UPDATE_INTERVAL(time)                                        \
	round_jiffies_relative(msecs_to_jiffies(time * 1000))
...
static void
battery_notify_charge_batt_terminal_check(struct oplus_chg_chip *chip);
static int parallel_chg_check_balance_bat_status(void);
#if IS_ENABLED(CONFIG_DRM_PANEL_NOTIFY)
static void
chg_panel_notifier_callback(enum panel_event_notifier_tag tag,
...
int oplus_chg_get_wls_type(void)
{
	...
	rc = oplus_chg_mod_get_property(chip->wls_ocm, OPLUS_CHG_PROP_WLS_TYPE,
					&pval);
	if (rc < 0)
		return OPLUS_CHG_WLS_UNKNOWN;
...
bool oplus_chg_is_wls_fw_upgrading(void)
{
	...
	rc = oplus_chg_mod_get_property(chip->wls_ocm,
					OPLUS_CHG_PROP_FW_UPGRADING, &pval);
	if (rc < 0)
		return false;
			  ...
			  union power_supply_propval *val)
{
	...
	if (chip->ac_online) {
		charger_xlog_printk(CHG_LOG_CRTI,
				    ...
				    chip->mmi_fastchg, chip->charger_exist,
				    chip->ac_online);
	}
...
static int oplus_chg_voocphy_get_cp_vbat_deviation(void)
{
	...
	while (retry > 0) {
		...
		vbat_deviation = (gauge_bat > cp_vbat) ? gauge_bat - cp_vbat :
							 cp_vbat - gauge_bat;

		if (cp_vbat == 0 || gauge_bat == 0 ||
		    vbat_deviation >= DEVIATION_NUM) {
			msleep(100);
			retry--;
		} else {
			break;
	...
	case POWER_SUPPLY_PROP_CHARGE_NOW:
		if (oplus_vooc_get_fastchg_started() == true &&
			...
			val->intval = 10000;
			   ...
			   (!g_charger_chip->external_gauge &&
			    !oplus_voocphy_chip_is_null()) &&
			   oplus_vooc_get_fast_chg_type() !=
				   CHARGER_SUBTYPE_FASTCHG_VOOC) {
			val->intval = 10000;
		} else {
		...
		if (is_wls_ocm_available(chip)) {
			oplus_chg_mod_get_property(chip->wls_ocm,
						   ...
						   &temp_val);
			if (!!temp_val.intval) {
				ret = oplus_chg_mod_get_property(
					chip->wls_ocm,
			       ...
			       size_t count, loff_t *ppos)
{
	...
	if (strncmp(proc_chg_cycle_data, "en808", 5) == 0) {
		...
		if (g_charger_chip->internal_gauge_with_asic == 1 &&
		    mcu_status == 1) {
		...
		if (oplus_voocphy_get_bidirect_cp_support() &&
		    g_charger_chip->chg_ops->check_chrdet_status()) {
...
static int oplus_chg_update_slow(struct oplus_chg_chip *chip)
{
	...
	slow = !((chip->wireless_support &&
		  oplus_wpc_get_wireless_charge_start() == true) ||
		 chip->charger_exist || (ibat_average >= 500) ||
	...
	return slow ? OPLUS_CHG_UPDATE_NO_CHARGE_INTERVAL_SEC :
...
static ssize_t oplus_chg_read(struct file *fp, char __user *buff, size_t count,
			      loff_t *off)
{
	char page[256] = { 0 };
	int len;
					     ...
					     size_t count, loff_t *off)
{
	...
	len = snprintf(buf, FORCE_IBUS_BUF_LEN, "%d",
		       chip->limits.force_input_current_ma);
...
static ssize_t qg_vbat_deviation_read(struct file *filp, char __user *buff,
				      size_t count, loff_t *off)
{
	char page[256] = { 0 };
	int len = 0;
	...
	if (!first_pass) {
		if (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY ||
			...
			vbat_deviation =
				oplus_chg_voocphy_get_cp_vbat_deviation();
			...
			while (retry > 0) {
				...
				gauge_bat = oplus_gauge_get_batt_mvolts();
				vbat_deviation = (gauge_bat > plat_vbat) ?
							 gauge_bat - plat_vbat :
							 plat_vbat - gauge_bat;
				if (plat_vbat == 0 || gauge_bat == 0 ||
				    vbat_deviation >= DEVIATION_NUM) {
					msleep(100);
					retry--;
				} else {
...
static ssize_t fastcharge_fail_count_read(struct file *filp, char __user *buff,
					  size_t count, loff_t *off)
{
	char page[256] = { 0 };
	int len = 0;
					      ...
					      loff_t *off)
{
	...
	if (NULL == chip)
		return -EFAULT;
	oplus_gauge_get_external_hmac_test_result(&count_total, &count_now,
						  &fail_count);
					      ...
					      loff_t *off)
{
	...
	len = sprintf(
		...
		status, fail_count, total_count, real_fail_count,
		real_total_count);
	if (len > *off) {
		len -= *off;
	} else {
...
static ssize_t proc_chg_ctl(struct file *filp, char __user *buff, size_t count,
			    loff_t *off)
{
	struct oplus_chg_chip *chip = g_charger_chip;
	char page[256] = { 0 };
...
static int init_charger_proc(struct oplus_chg_chip *chip)
{
	...
	prEntry_tmp =
		proc_create_data("fastcharge_fail_count", 0444, prEntry_da,
				 &fastcharge_fail_count_proc_fops, chip);
	if (prEntry_tmp == NULL) {
		ret = -1;
		chg_debug(
			"%s: Couldn't create proc fastcharge_fail_count entry, %d\n",
			__func__, __LINE__);
	}
	...
	if (prEntry_tmp == NULL) {
		ret = -1;
		chg_debug(
			"%s: Couldn't create test_external_hmac proc entry, %d\n",
			__func__, __LINE__);
	}
	...
	if (prEntry_tmp == NULL) {
		ret = -1;
		chg_debug(
			"%s: Couldn't create external_hmac_status proc entry, %d\n",
			__func__, __LINE__);
	}
	...
	prEntry_tmp =
		proc_create_data("integrate_gauge_fcc_flag", 0666, prEntry_da,
				 &proc_integrate_gauge_fcc_flag_ops, chip);
	if (prEntry_tmp == NULL) {
		ret = -1;
		chg_debug(
			"%s: Couldn't create proc_integrate_gauge_fcc_flag_ops entry, %d\n",
			__func__, __LINE__);
	}
	...
	if (prEntry_tmp == NULL) {
		ret = -1;
		chg_debug(
			"%s: Couldn't create proc_reserve_soc_debug_ops entry, %d\n",
			__func__, __LINE__);
	}
	...
	if (prEntry_tmp == NULL) {
		ret = -1;
		chg_debug(
			"Couldn't create proc_force_current_limit_ops entry, %d\n");
	}
...
void oplus_chg_ui_soc_decimal_init(void)
{
	...
	pr_err("[oplus_chg_ui_soc_decimal_init]!!!soc:%d",
	       (int)((chip->batt_rm * 100000) / chip->batt_fcc));
	...
	if (chip->ui_soc == 100) {
		...
		if (chip->rsd.smooth_switch_v2 && chip->rsd.reserve_chg_soc)
			chip->ui_soc_decimal =
				chip->ui_soc_decimal * OPLUS_FULL_SOC /
				(OPLUS_FULL_SOC - chip->rsd.reserve_chg_soc);
		if ((chip->ui_soc_integer + chip->ui_soc_decimal) >
			    chip->last_decimal_ui_soc &&
		    chip->last_decimal_ui_soc != 0) {
			chip->ui_soc_decimal =
				((chip->last_decimal_ui_soc % 1000 - 5) > 0) ?
...
static int
oplus_chg_track_upload_vbatt_diff_over_info(struct oplus_chg_chip *chip,
					    int vbat_cell_max,
					    int vbat_cell_min, int status)
{
	int index = 0;

	memset(chip->vbatt_diff_over_load_trigger.crux_info, 0,
	       sizeof(chip->vbatt_diff_over_load_trigger.crux_info));

	index +=
		snprintf(&(chip->vbatt_diff_over_load_trigger.crux_info[index]),
	...
	if (status == POWER_SUPPLY_STATUS_FULL)
		index += snprintf(
			&(chip->vbatt_diff_over_load_trigger.crux_info[index]),
			OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			"$$vdiff_type@@full");
	else
		index += snprintf(
...
static int
oplus_chg_track_upload_vbatt_too_low_info(struct oplus_chg_chip *chip)
{
	int index = 0;

	memset(chip->vbatt_too_low_load_trigger.crux_info, 0,
	       sizeof(chip->vbatt_too_low_load_trigger.crux_info));

	index +=
		snprintf(&(chip->vbatt_too_low_load_trigger.crux_info[index]),
			 OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			 "$$soc@@%d"
			 "$$s_soc@@%d$$uisoc@@%d$$vbatt_max@@%d$$vbatt_min@@%d",
			 chip->soc, chip->smooth_soc, chip->ui_soc,
...
static int
oplus_chg_track_upload_uisoc_keep_1_t_info(struct oplus_chg_chip *chip)
{
	int index = 0;
	...
	index += snprintf(
		...
		chip->batt_volt_max, chip->batt_volt_min, chip->soc,
		chip->smooth_soc, chip->ui_soc, chip->uisoc_1_start_batt_rm,
...
static void oplus_chg_track_upload_fast_gpio_info(struct oplus_chg_chip *chip)
{
	...
	memset(chip->fast_gpio_err_load_trigger.crux_info, 0,
	       sizeof(chip->fast_gpio_err_load_trigger.crux_info));
	...
	if (!chip->chg_ops->get_chargerid_switch_val()) {
		oplus_chg_track_get_gpio_err_reason(TRACK_GPIO_ERR_CHARGER_ID,
						    chip->err_reason,
						    sizeof(chip->err_reason));
		index += snprintf(
			&(chip->fast_gpio_err_load_trigger.crux_info[index]),
			OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			"$$err_reason@@%s", chip->err_reason);
		need_upload = true;
	} else if (!oplus_vooc_get_vooc_switch_val()) {
		oplus_chg_track_get_gpio_err_reason(TRACK_GPIO_ERR_VOOC_SWITCH,
						    chip->err_reason,
						    sizeof(chip->err_reason));
		index += snprintf(
			&(chip->fast_gpio_err_load_trigger.crux_info[index]),
	...
	if (need_upload || chip->debug_force_fast_gpio_err) {
		...
		index += snprintf(
			...
			chip->chg_power_info);
		schedule_delayed_work(&chip->fast_gpio_err_load_trigger_work,
				      0);
	}
...
static void
oplus_chg_track_fast_gpio_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_chip *chip = container_of(
		dwork, struct oplus_chg_chip, fast_gpio_err_load_trigger_work);
...
static int
oplus_track_upload_cool_down_match_err_info(struct oplus_chg_chip *chip)
{
	int index = 0;
	int in_current;
	...
	index += snprintf(
		&(chip->cool_down_match_err_load_trigger.crux_info[index]),
		OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
		"$$cool_down@@%d"
...
static void oplus_chg_cool_down_match_err_check(struct oplus_chg_chip *chip)
{
	...
	if ((chip->charger_volt * 10) /
		    (chip->batt_volt * 10 * chip->vbatt_num) >=
	    VOLTAGE_RATIO)
		scale = 2;
	else
		scale = 1;
...
oplus_chg_track_uisoc_keep_1_t_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_chip *chip = container_of(
		dwork, struct oplus_chg_chip, uisoc_keep_1_t_load_trigger_work);
...
oplus_chg_track_vbatt_too_low_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_chip *chip = container_of(
		dwork, struct oplus_chg_chip, vbatt_too_low_load_trigger_work);
...
static void
oplus_chg_track_cool_down_match_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_chip *chip =
		container_of(dwork, struct oplus_chg_chip,
	...
	oplus_chg_track_upload_trigger_data(
		chip->cool_down_match_err_load_trigger);
}
...
static int oplus_chg_track_debugfs_init(struct oplus_chg_chip *chip)
{
	...
	debugfs_create_u32("debug_vbat_keep_soc_1", 0644, debugfs_keep_1_t,
			   &(chip->debug_vbat_keep_soc_1));
	debugfs_create_u32("debug_force_vbatt_too_low", 0644, debugfs_root,
			   &(chip->debug_force_vbatt_too_low));
	debugfs_create_u32("debug_force_usbtemp_trigger", 0644, debugfs_root,
			   &(chip->debug_force_usbtemp_trigger));
	debugfs_create_u32("debug_force_fast_gpio_err", 0644, debugfs_root,
			   &(chip->debug_force_fast_gpio_err));
...
int oplus_chg_init(struct oplus_chg_chip *chip)
{
	...
	INIT_DELAYED_WORK(&chip->turn_on_charging_work,
			  oplus_chg_turn_on_charging_work);
	INIT_DELAYED_WORK(&chip->parallel_chg_mos_test_work,
			  oplus_parallel_chg_mos_test_work);
	INIT_DELAYED_WORK(&chip->fg_soft_reset_work, oplus_fg_soft_reset_work);
	INIT_DELAYED_WORK(&chip->parallel_batt_chg_check_work,
			  oplus_parallel_batt_chg_check_work);
	chip->shortc_thread =
	...
	if (chip->active_panel) {
		cookie = panel_event_notifier_register(
			...
			chip->active_panel, &chg_panel_notifier_callback, chip);
		if (!cookie) {
	...
	if (chip->dual_panel_support && chip->active_panel_sec) {
		cookie = panel_event_notifier_register(
			...
			chip->active_panel_sec, &chg_panel_notifier_callback,
			chip);
		if (!cookie) {
...
int oplus_chg_parse_svooc_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,vooc_project",
				  &chip->vooc_project);
	if (rc < 0) {
		chip->vooc_project = 0;
	}
	...
	chip->usbtemp_dischg_by_pmic =
		of_property_read_bool(node, "qcom,usbtemp_dischg_by_pmic");
...
static int oplus_chg_check_panel_dt(struct oplus_chg_chip *chip)
{
	...
	if (chip->dual_panel_support) {
		count = of_count_phandle_with_args(
			np, "oplus,dsi-panel-secondary", NULL);
		if (count <= 0)
			return 0;

		for (i = 0; i < count; i++) {
			node = of_parse_phandle(np, "oplus,dsi-panel-secondary",
						i);
			panel = of_drm_find_panel(node);
			of_node_put(node);
			if (!IS_ERR(panel)) {
...
void oplus_chg_smooth_switch_v2_parse_dt(struct oplus_chg_chip *chip)
{
	...
	if (reserve_soc_by_rus(chip)) {
		...
		chip->rsd.smooth_switch_v2 =
			of_property_read_bool(node, "oplus,smooth_switch_v2");
		if (chip->rsd.smooth_switch_v2) {
			rc = of_property_read_u32(node, "oplus,reserve_chg_soc",
						  &chip->rsd.reserve_chg_soc);
			if (rc)
				chip->rsd.reserve_chg_soc = RESERVE_SOC_DEFAULT;

			rc = of_property_read_u32(node, "oplus,reserve_dis_soc",
						  &chip->rsd.reserve_dis_soc);
			if (rc)
...
void oplus_chg_usbtemp_v2_parse_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node,
				  "qcom,usbtemp_temp_gap_low_with_batt_temp",
	...
	rc = of_property_read_u32(node,
				  "qcom,usbtemp_temp_gap_high_with_batt_temp",
	...
	rc = of_property_read_u32(
		node, "qcom,usbtemp_temp_gap_low_without_batt_temp",
	...
	rc = of_property_read_u32(
		node, "qcom,usbtemp_temp_gap_high_without_batt_temp",
	...
	rc = of_property_read_u32(node,
				  "qcom,usbtemp_rise_fast_temp_count_high",
	...
	rc = of_property_read_u32(node,
				  "qcom,usbtemp_cool_down_recover_ntc_low",
	...
	rc = of_property_read_u32(node,
				  "qcom,usbtemp_cool_down_recover_ntc_high",
	...
	rc = of_property_read_u32(node,
				  "qcom,usbtemp_cool_down_recover_gap_low",
	...
	rc = of_property_read_u32(node,
				  "qcom,usbtemp_cool_down_recover_gap_high",
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,sub_iterm_ma",
				  &chip->limits.sub_iterm_ma);
	if (rc < 0) {
		chip->limits.sub_iterm_ma = -EINVAL;
	}
	chip->skip_usbtemp_cool_down =
		of_property_read_bool(node, "qcom,skip_usbtemp_cool_down");
	...
	rc = of_property_read_u32(
		node, "qcom,temp_cold_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_cold_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,pd_temp_cold_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(
		node, "qcom,pd_temp_cold_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,qc_temp_cold_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(
		node, "qcom,qc_temp_cold_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,pd_temp_little_cool_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(
		node, "qcom,pd_temp_little_cool_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,qc_temp_little_cool_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(
		node, "qcom,qc_temp_little_cool_fastchg_current_ma_low",
	...
	chip->vooc_enable_charger_first =
		of_property_read_bool(node, "qcom,vooc_enable_charger_first");
	chip->smart_chg_bcc_support =
		of_property_read_bool(node, "qcom,smart_chg_bcc_support");
	chg_err("qcom,smart_chg_bcc_support is %d\n",
		chip->smart_chg_bcc_support);

	chip->smart_chg_soh_support =
		of_property_read_bool(node, "qcom,smart_chg_soh_support");
	...
	if (rc < 0)
		chip->limits.tbatt_pdqc_to_9v_thr =
			chip->limits.tbatt_pdqc_to_5v_thr - TEM_ANTI_SHAKE;
	...
	chip->old_smart_charge_standard =
		of_property_read_bool(node, "qcom,old_smart_charge_standard");
	charger_xlog_printk(CHG_LOG_CRTI,
			    "qcom,old_smart_charge_standard = %d\n",
			    chip->old_smart_charge_standard);
	...
	chip->chg_ctrl_by_vooc =
		of_property_read_bool(node, "qcom,chg_ctrl_by_vooc");
	chip->chg_ctrl_by_vooc_default =
		of_property_read_bool(node, "qcom,chg_ctrl_by_vooc");
	...
	rc = of_property_read_u32(
		node, "qcom,input_current_vooc_led_ma_normal",
	...
	chip->external_authenticate =
		of_property_read_bool(node, "qcom,external_authenticate");
	chip->fg_bcl_poll =
	...
	chip->usbtemp_change_gap =
		of_property_read_bool(node, "qcom,usbtemp_change_gap");
	chip->ffc_support = of_property_read_bool(node, "qcom,ffc_support");
	chip->dual_ffc = of_property_read_bool(node, "qcom,dual_ffc");
	chip->fg_info_package_read_support = of_property_read_bool(
		node, "qcom,fg_info_package_read_support");
	...
	chip->voocphy_support_display_vooc =
		of_property_read_bool(node, "voocphy_support_display_vooc");
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
	if (get_eng_version() == HIGH_TEMP_AGING || oplus_is_ptcrb_version() ||
...
#endif
	{
		chip->limits.max_chg_time_sec = chip->batt_capacity_mah /
						OPLUS_CHG_CAPACITY_PARAMETER *
						OPLUS_CHG_ONE_HOUR;
	}
	chip->efttest_fast_switch =
		of_property_read_bool(node, "qcom,efttest_fast_switch");

	rc = of_property_read_u32(node, "qcom,voocphy_support",
	...
	charger_xlog_printk(
		...
		chip->limits.temp_cold_fastchg_current_ma_low,
		chip->limits.temp_cold_fastchg_current_ma_high,
		chip->limits.charger_current_vooc_ma_normal, chip->ffc_support,
		chip->dual_ffc, chip->new_ui_warning_support,
		chip->smooth_switch, chip->voocphy_support);
	...
	rc = of_property_read_u32(node, "qcom,internal_gauge_with_asic",
				  &chip->internal_gauge_with_asic);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,usbtemp_cool_down_temp_gap",
				  &chip->usbtemp_cool_down_temp_gap);
	if (rc) {
		chip->usbtemp_cool_down_temp_gap = 12;
	}
	rc = of_property_read_u32(node,
				  "qcom,usbtemp_cool_down_recovery_temp_gap",
	...
	rc = of_property_read_u32(
		node, "qcom,usbtemp_cool_down_recovery_temp_first_gap",
		&chip->usbtemp_cool_down_recovery_temp_first_gap);
	if (rc) {
		chip->usbtemp_cool_down_recovery_temp_first_gap = 6;
	}
	rc = of_property_read_u32(node,
				  "qcom,usbtemp_cool_down_temp_second_gap",
				  &chip->usbtemp_cool_down_temp_second_gap);
	if (rc) {
		chip->usbtemp_cool_down_temp_second_gap = 17;
	}
	rc = of_property_read_u32(
		node, "qcom,usbtemp_cool_down_recovery_temp_second_gap",
		&chip->usbtemp_cool_down_recovery_temp_second_gap);
	if (rc) {
		chip->usbtemp_cool_down_recovery_temp_second_gap = 11;
	}

	chip->new_usbtemp_cool_down_support = of_property_read_bool(
		node, "qcom,new_usbtemp_cool_down_support");

	rc = of_property_read_u32(node, "qcom,usbtemp_batttemp_recover_gap",
				  &chip->usbtemp_batttemp_recover_gap);
	if (rc) {
		chip->usbtemp_batttemp_recover_gap = 6;
	}
	rc = of_property_read_u32(node, "qcom,usbtemp_batttemp_current_gap",
				  &chip->usbtemp_batttemp_current_gap);
	if (rc) {
		chip->usbtemp_batttemp_current_gap = 12;
	}
	rc = of_property_read_u32(node, "qcom,usbtemp_max_temp_diff",
				  &chip->usbtemp_max_temp_diff);
	if (rc) {
		chip->usbtemp_max_temp_diff = 7;
	}

	rc = of_property_read_u32(node, "qcom,usbtemp_max_temp_thr",
				  &chip->usbtemp_max_temp_thr);
	if (rc) {
		chip->usbtemp_max_temp_thr = 57;
	}
	rc = of_property_read_u32(node, "qcom,usbtemp_temp_up_time_thr",
				  &chip->usbtemp_temp_up_time_thr);
	if (rc) {
		chip->usbtemp_temp_up_time_thr = 30;
	}
	charger_xlog_printk(
		CHG_LOG_CRTI,
		"chip->usbtemp_temp_up_time_thr=%d, chip->usbtemp_max_temp_thr=%d, chip->usbtemp_batttemp_gap[%d] chip->usbtemp_cool_down_temp_gap[%d]  chip->usbtemp_cool_down_temp_first_gap[%d]  chip->usbtemp_cool_down_temp_second_gap[%d]\n",
		chip->usbtemp_temp_up_time_thr, chip->usbtemp_max_temp_thr,
		chip->usbtemp_batttemp_gap, chip->usbtemp_cool_down_temp_gap,
		chip->usbtemp_cool_down_temp_first_gap,
		chip->usbtemp_cool_down_temp_second_gap);

	rc = of_property_read_u32(node, "qcom,ship-mode-config",
	...
	chip->vooc_show_ui_soc_decimal =
		of_property_read_bool(node, "qcom,vooc_show_ui_soc_decimal");
	...
	chip->support_abnormal_adapter =
		of_property_read_bool(node, "qcom,support_abnormal_adapter");
	chip->nightstandby_support =
		of_property_read_bool(node, "qcom,nightstandby_support");
	charger_xlog_printk(
		CHG_LOG_CRTI,
...
#ifdef OPLUS_CUSTOM_OP_DEF
	chip->hiz_gnd_cable =
		of_property_read_bool(node, "qcom,supported_hiz_gnd_cable");
#endif

	chip->dual_panel_support =
		of_property_read_bool(node, "qcom,dual_panel_support");
	...
	chip->support_low_soc_unlimit =
		of_property_read_bool(node, "qcom,support_low_soc_unlimit");
	...
	chip->nightstandby_support =
		of_property_read_bool(node, "qcom,nightstandby_support");
	chip->screen_off_control_by_batt_temp =
		of_property_read_bool(node, "screen_off_control_by_batt_temp");
	charger_xlog_printk(CHG_LOG_CRTI,
			    "support_low_soc_unlimit=%d, unlimit_soc=%d\n",
			    chip->support_low_soc_unlimit, chip->unlimit_soc);

	rc = of_property_count_elems_of_size(node, "usbtemp_curr_table",
					     sizeof(u32));
	if (rc < 0 || rc > (USBTEMP_CURR_TABLE_MAX * 2))
		charger_xlog_printk(
			CHG_LOG_CRTI,
			"Count usbtemp_curr_table failed, rc = %d\n", rc);
	else {
		length = rc;
		rc = of_property_read_u32_array(node, "usbtemp_curr_table",
						(u32 *)temp_curr_table, length);
		if (rc)
			charger_xlog_printk(
				CHG_LOG_CRTI,
				"Read usbtemp_curr_table failed, rc = %d\n",
				rc);
		else {
			for (i = 0; i < length / 2; i++) {
				charger_xlog_printk(
					CHG_LOG_CRTI,
					"temp_curr_table[%d, %d]\n",
					temp_curr_table[i].batt_curr,
					temp_curr_table[i].temp_delta);
			}
		}
	}
	...
	chip->support_3p6_standard =
		of_property_read_bool(node, "qcom,support_3p6_standard");
	charger_xlog_printk(CHG_LOG_CRTI, "qcom,support_3p6_standard = %d\n",
			    chip->support_3p6_standard);

	chip->suport_pd_9v2a =
		of_property_read_bool(node, "qcom,suport_pd_9v2a");
	...
	chip->support_usbtemp_protect_v2 =
		of_property_read_bool(node, "qcom,support_usbtemp_protect_v2");
	charger_xlog_printk(CHG_LOG_CRTI,
			    "qcom,support_usbtemp_protect_v2 = %d\n",
			    chip->support_usbtemp_protect_v2);
	...
	chip->pdqc_9v_voltage_adaptive =
		of_property_read_bool(node, "oplus,pdqc_9v_voltage_adaptive");
...
static void oplus_chg_set_charging_current(struct oplus_chg_chip *chip)
{
	...
	if (oplus_switching_support_parallel_chg() == PARALLEL_MOS_CTRL) {
		if (chip->batt_target_curr > 0 &&
		    chip->batt_target_curr < charging_current)
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ctrl_by_vooc && chip->vbatt_num == 2 &&
		...
		if (chip->led_on) {
			if (chip->vooc_temp_status == VOOC_TEMP_STATUS__HIGH) {
				current_limit =
					chip->limits
						.input_current_vooc_led_ma_high;
			} else if (chip->vooc_temp_status ==
				   VOOC_TEMP_STATUS__WARM) {
				current_limit =
					chip->limits
						.input_current_vooc_led_ma_warm;
			} else if (chip->vooc_temp_status ==
				   VOOC_TEMP_STATUS__NORMAL) {
				current_limit =
					chip->limits
						.input_current_vooc_led_ma_normal;
			}
		...
		if (chip->chg_ops->input_current_ctrl_by_vooc_write) {
			chip->chg_ops->input_current_ctrl_by_vooc_write(
				current_limit);
			return;
		}
	}
	...
	charger_xlog_printk(CHG_LOG_CRTI, " led_on = %d, \
		...
		cool_down_force_5v = %d.\n",
			    ...
			    chip->cool_down, chip->cool_down_force_5v);
...
static void oplus_chg_vfloat_over_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->wireless_support &&
	    (oplus_wpc_get_ffc_charging() == true || oplus_chg_is_wls_ffc())) {
		return;
	}
	...
	if (oplus_switching_support_parallel_chg()) {
		if (chip->temperature >= chip->limits.ffc2_temp_high_decidegc ||
		    chip->sub_batt_temperature >=
			    chip->limits.ffc2_temp_high_decidegc) { /*>=40C*/
			high_counts++;
			if (high_counts >= TEMP_FFC_COUNTS) {
			   ...
			   chip->sub_batt_temperature >=
				   chip->limits
					   .ffc2_temp_warm_decidegc) { /*>=35C && < 40*/
			warm_counts++;
			if (warm_counts >= TEMP_FFC_COUNTS) {
			   ...
			   chip->sub_batt_temperature >=
				   chip->limits
					   .ffc2_temp_low_decidegc) { /*>=16C&&<35C*/
			normal_counts++;
...
static void battery_temp_anti_shake_handle(struct oplus_chg_chip *chip)
{
	...
	if (chip->tbatt_status == BATTERY_STATUS__HIGH_TEMP) { /*>53C*/
		...
		chip->limits.little_cool_bat_decidegc =
			chip->anti_shake_bound.little_cool_bound;
		chip->limits.normal_bat_decidegc =
			chip->anti_shake_bound.normal_bound + low_shake;
		chip->limits.warm_bat_decidegc =
			chip->anti_shake_bound.warm_bound + high_shake;
		chip->limits.hot_bat_decidegc =
			chip->anti_shake_bound.hot_bound;
	} else if (chip->tbatt_status ==
	...
	chg_err("[%d-%d-%d-%d-%d-%d-%d] t=[%d %d] s=%d\n",
		...
		chip->limits.little_cool_bat_decidegc,
		chip->limits.normal_bat_decidegc,
		chip->limits.warm_bat_decidegc, chip->limits.hot_bat_decidegc,
		chip->tbatt_pre_shake, tbatt_cur_shake, chip->tbatt_status);
...
static void oplus_chg_check_vooc_temp_status(struct oplus_chg_chip *chip)
{
	...
	if (batt_temp > chip->limits.vooc_high_bat_decidegc) { /*>45C*/
		if (oplus_vooc_get_fastchg_started() == true) {
		   ...
		   chip->limits
			   .vooc_warm_bat_decidegc_antishake) { /*>38C && <= 45*/
		high_counts++;
		if (high_counts >= TVOOC_COUNTS ||
		   ...
		   chip->limits
			   .vooc_normal_bat_decidegc_antishake) { /*>34C && <= 38*/
		warm_counts++;
...
static void oplus_chg_check_rechg_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->tbatt_status == BATTERY_STATUS__COLD_TEMP) { //4.0
		recharging_vol = oplus_chg_get_float_voltage(chip) - 300;
		   ...
		   BATTERY_STATUS__LITTLE_COLD_TEMP) { //4.4
	...
	if ((!chip->authenticate) || (!chip->hmac)) {
		recharging_vol =
			chip->limits.non_standard_vfloat_mv - 400; //3.80
	}
...
void oplus_charger_detect_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ops->check_chrdet_status()) {
		...
		if (chip->charger_type == POWER_SUPPLY_TYPE_UNKNOWN) {
			...
			charger_xlog_printk(
				CHG_LOG_CRTI,
				"Charger in 1 charger_type=%d, unknow_type_check[%d]\n",
			...
			if (unknow_type_check >=
				    UNKNOW_TYPE_DETECT_RETRY_TIME &&
			    ((oplus_vooc_get_fastchg_dummy_started() == true) ||
			     (oplus_vooc_get_fastchg_to_normal() == true) ||
			     (oplus_vooc_get_fastchg_to_warm() == true) ||
			     (oplus_vooc_get_btb_temp_over() == true))) {
			...
			if (oplus_vooc_get_fastchg_to_normal() == true ||
			    oplus_vooc_get_fastchg_to_warm() == true ||
			    (oplus_pps_get_pps_mos_started() == true)) {
				charger_xlog_printk(
					CHG_LOG_CRTI,
			...
			} else if (oplus_vooc_get_fastchg_dummy_started() ==
					   true &&
				...
				oplus_chg_turn_on_charging(chip);
				oplus_chg_switch(chip);
			} else if (oplus_wpc_get_wireless_charge_start() ==
					   false &&
				   !oplus_chg_is_wls_present()) {
				charger_xlog_printk(
					CHG_LOG_CRTI,
			...
			if (oplus_switching_support_parallel_chg() ==
				...
				oplus_chg_parellel_variables_reset();
				schedule_delayed_work(
					&chip->parallel_batt_chg_check_work,
					OPLUS_CHG_UPDATE_INTERVAL(
						PARALLEL_CHECK_TIME));
			}
		} else {
...
static void oplus_chg_get_battery_data(struct oplus_chg_chip *chip)
{
	...
	if ((oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY ||
		...
		if (oplus_vooc_get_fastchg_started() == true) {
			...
			cp_vbat = oplus_voocphy_get_cp_vbat();
			if (cp_vbat >= chip->limits.vbatt_hv_thr ||
			    cp_vbat <= 0 || abs(cp_vbat - gauge_volt) > 800) {
				...
				if (chip->usbtemp_wq_init_finished) {
					...
					if (chip->support_usbtemp_protect_v2)
						wake_up_interruptible(
							&chip->oplus_usbtemp_wq_new_method);
					else
						wake_up_interruptible(
							&chip->oplus_usbtemp_wq);
				}
			} else {
				if ((oplus_voocphy_get_bidirect_cp_support() &&
				     oplus_vooc_get_fastchg_started())) {
	...
	if (!chip->is_gauge_ready && chip->soc >= 0 &&
	    chip->soc <= OPLUS_FULL_SOC)
	...
	if (ui_soc_cp_flag == 0) {
		...
		if ((soc_load != 0) && ((abs(soc_load - chip->soc)) <= 20)) {
			...
			} else if (chip->rsd.smooth_switch_v2 &&
				   soc_load == OPLUS_FULL_SOC &&
				   chip->soc <
					   (OPLUS_FULL_SOC -
					    chip->rsd.reserve_dis_soc - 1)) {
				chip->ui_soc = soc_load - 1;
			} else {
				chip->ui_soc = soc_load;
			}
...
static void battery_notify_parallel_error_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->parallel_error_flag &
	    (REASON_SOC_GAP_TOO_BIG | REASON_VBAT_GAP_BIG |
	...
	if (reduce_count > reduce_count_limit) {
		if (!uisoc_keep_1_recorded) {
			...
			oplus_chg_track_upload_vbatt_diff_over_info(
				chip, chip->batt_volt_max, chip->batt_volt_min,
				chip->prop_status);
...
static void oplus_chg_smooth_to_soc_v2(struct oplus_chg_chip *chip, bool force)
{
	...
	if (pre_soc == -EINVAL) { /*first entry*/
		...
		if (chip->rsd.reserve_soc >= 0 &&
		    chip->rsd.reserve_soc <= RESERVE_SOC_MAX) {
			memcpy(chip->rsd.soc_jump_array,
			       soc_jump_table[chip->rsd.reserve_soc],
			       sizeof(int) * RESERVE_SOC_MAX);
		} else {
			memcpy(chip->rsd.soc_jump_array, soc_jump_table[0],
			       sizeof(int) * RESERVE_SOC_MAX);
		}
	}
	...
	temp_soc =
		soc * OPLUS_FULL_SOC / (OPLUS_FULL_SOC - chip->rsd.reserve_soc);
	chip->rsd.smooth_soc_fifo[chip->rsd.smooth_soc_index] =
		temp_soc > OPLUS_FULL_SOC ? OPLUS_FULL_SOC : temp_soc;
	...
	if (force || pre_soc != soc || pre_status != status) {
		...
		for (jump_index = 0; jump_index < chip->rsd.reserve_soc;
		     jump_index++) {
#define SOC_JUMP_RANGE_VAL 1
			if (soc >= (chip->rsd.soc_jump_array[jump_index] -
				    SOC_JUMP_RANGE_VAL) &&
			    soc <= (chip->rsd.soc_jump_array[jump_index] +
				    SOC_JUMP_RANGE_VAL)) {
	...
	if (chip->rsd.smooth_soc_avg_cnt == 1) {
		...
		while (cnt--) {
			...
			index = (index + SMOOTH_SOC_MAX_FIFO_LEN - 1) %
				SMOOTH_SOC_MAX_FIFO_LEN;
		}
		if (valid_soc_cnt > 0) {
	...
	chg_err("soc[%d %d %d] status[%d %d] [%d %d %d %d %d]"
		...
		chip->rsd.smooth_soc_index, chip->rsd.smooth_soc_avg_cnt,
		valid_soc_cnt, sum, chip->rsd.is_soc_jump_range,
		chip->rsd.smooth_soc_fifo[0], chip->rsd.smooth_soc_fifo[1],
		chip->rsd.smooth_soc_fifo[2], chip->rsd.smooth_soc_fifo[3]);

	pre_status = status;
	pre_soc = soc;
	chip->rsd.smooth_soc_index++;
	chip->rsd.smooth_soc_index =
		chip->rsd.smooth_soc_index % SMOOTH_SOC_MAX_FIFO_LEN;
...
static void oplus_chg_update_ui_soc(struct oplus_chg_chip *chip)
{
	...
	if ((chip->charger_exist || oplus_wpc_get_status()) &&
		...
		charger_xlog_printk(
			...
			chip->ui_soc, chip->waiting_for_ffc,
			chip->fastchg_to_ffc, oplus_vooc_get_fastchg_started(),
			oplus_vooc_get_fast_chg_type());
		...
		if (chip->rsd.smooth_switch_v2 &&
		    chip->ui_soc == OPLUS_FULL_SOC && !chip->batt_full) {
			...
			if (sleep_tm > 0) {
				...
				if (chip->ui_soc != ui_soc_pre)
					chg_err("sleep_tm %d %d %d %d %d %d %d %d %d\n",
						...
						chip->save_sleep_tm_sec,
						chip->soc, chip->smooth_soc,
						chip->ui_soc, ui_soc_pre,
	...
	if (chip->ui_soc < 2 || chip->debug_force_uisoc_less_than_2) {
		...
		if (oplus_chg_soc_reduce_slow_when_1(chip) == true ||
		    oplus_chg_soc_reduce_to_shutdown(chip) == true) {
			chip->ui_soc = 0;
		} else {
			chip->ui_soc = 1;
		}
...
static void oplus_efttest_fast_switch(struct oplus_chg_chip *chip)
{
	if (reset_mcu_delay == RESET_MCU_DELAY_45S &&
		...
		mcu_status = 1;
	} else if (reset_mcu_delay == (RESET_MCU_DELAY_150S * efttest_count) &&
		   efttest_count < 4 &&
		   chip->chg_ops->get_charger_subtype() != CHARGER_SUBTYPE_QC &&
		   chip->chg_ops->get_charger_subtype() != CHARGER_SUBTYPE_PD) {
...
static void oplus_chg_fast_switch_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->balancing_bat_stop_chg == 1 ||
	...
	if (chip->pd_wait_svid && !chip->pd_svooc &&
	    chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD) {
		charger_xlog_printk(CHG_LOG_CRTI,
				    "voocphy use PD, wait 50ms\n");
		msleep(50);
	...
	if (oplus_vooc_get_abnormal_adapter_current_cnt() > 0 &&
	    chip->abnormal_adapter_dis_cnt >=
		    oplus_vooc_get_abnormal_adapter_current_cnt()) {
		charger_xlog_printk(
			CHG_LOG_CRTI,
			" abnormal adapter dis cnt >= vooc_max,return\n");
		return;
	}
	if (chip->vbatt_num == DOUBLE_BATTERY) {
		if (oplus_gauge_afi_update_done() == false) {
		...
		if (!oplus_vooc_get_fastchg_started() &&
			...
			if (reset_mcu_delay == RESET_MCU_DELAY_15S) {
				...
				if (suspend_charger == false) {
				   ...
				   (chip->vbatt_num == 2 ||
				    is_vooc_support_single_batt_svooc() ==
					    true ||
				    (oplus_chg_get_voocphy_support() ==
					     AP_SINGLE_CP_VOOCPHY ||
				     oplus_chg_get_voocphy_support() ==
			...
			high_vol_light = false;
			if (chip->internal_gauge_with_asic == 1 &&
			    oplus_vooc_get_reset_gpio_status() == 0) {
				oplus_vooc_set_ap_clk_high();
				oplus_vooc_reset_mcu();
				mcu_status = 1;
			}
...
static int oplus_chg_check_sw_sub_batt_full(struct oplus_chg_chip *chip)
{
	...
	chg_err("*****[Sub_Batt vol: %d] [Sub_Batt max vol: %d] [Sub_Batt min vol: %d] [Sub_Chg Curr: %d] [Sub_Term Vol: S%d] [Sub_Term Curr: %d]*****\n",
		chip->sub_batt_volt, chip->batt_volt_max, chip->batt_volt_min,
		chip->sub_batt_icharging, sub_vbatt_full_vol_sw,
		chip->limits.iterm_ma);
#endif
	/* use SW Vfloat to check */
	if (chip->sub_batt_volt > sub_vbatt_full_vol_sw) {
		if (chip->sub_batt_icharging < 0 &&
		    (chip->sub_batt_icharging * -1) <=
			    chip->limits.sub_iterm_ma) {
...
static int oplus_chg_check_hw_sub_batt_full(struct oplus_chg_chip *chip)
{
	...
	chg_err("*****[Batt vol: %d] [Batt max vol: %d] [Batt min vol: %d] [Chg Curr: %d] [Term Vol: H%d] [Term Curr: %d]*****\n",
		chip->sub_batt_volt, chip->batt_volt_max, chip->batt_volt_min,
		chip->icharging, sub_vbatt_full_vol_hw, chip->limits.iterm_ma);
#endif
	/* use HW Vfloat to check */
...
static bool oplus_chg_check_sub_vbat_ffc_end(struct oplus_chg_chip *chip)
{
	...
	if (chip->sub_batt_volt > ffc_vfloat_sw_limit) {
		chg_err("sub_vbat_ffc_end !!! sub_batt_volt:%d, ffc_vfloat_sw_limit:%d, fastchg_ffc_status:%d",
			chip->sub_batt_volt, ffc_vfloat_sw_limit,
...
static bool oplus_chg_check_sub_ibat_ffc_end(struct oplus_chg_chip *chip)
{
	...
	if ((chip->sub_batt_icharging * -1) < ffc_ibatt_sw_limit) {
		chg_err("sub_ibat_ffc_end !!! sub_batt_icharging:%d, ffc_ibatt_sw_limit:%d, fastchg_ffc_status:%d",
			chip->sub_batt_icharging, ffc_ibatt_sw_limit,
			chip->ffc_temp_status);
		return true;
	} else {
	...
	if (chip->fastchg_to_ffc == true) {
		if (oplus_switching_support_parallel_chg()) {
			sub_vbat_ffc_end_status =
				oplus_chg_check_sub_vbat_ffc_end(chip);
			sub_ibat_ffc_end_status =
				oplus_chg_check_sub_ibat_ffc_end(chip);
		}
		if (chip->fastchg_ffc_status == 1) {
			if (chip->batt_volt >=
				    chip->limits.ffc1_normal_vfloat_sw_limit ||
			...
			if (chip->ffc_temp_status == FFC_TEMP_STATUS__NORMAL) {
				if ((chip->icharging *
				     -1) < (chip->limits.ff1_normal_fastchg_ma -
					    chip->limits.ff1_exit_step_ma) ||
				    sub_ibat_ffc_end_status) {
					normal_counts++;
					if (normal_counts >=
				...
				if ((chip->icharging *
				     -1) < (chip->limits.ff1_warm_fastchg_ma -
					    chip->limits.ff1_warm_exit_step_ma) ||
				    sub_ibat_ffc_end_status) {
					warm_counts++;
		...
		if (chip->fastchg_ffc_status == 2) {
			...
			if (chip->ffc_temp_status == FFC_TEMP_STATUS__NORMAL) {
				if ((chip->icharging *
				     -1) < (chip->limits.ffc2_normal_fastchg_ma -
					    chip->limits.ffc2_exit_step_ma) ||
				    sub_ibat_ffc_end_status) {
					normal_counts++;
					if (normal_counts >=
...
static void oplus_chg_check_status_full(struct oplus_chg_chip *chip)
{
	...
	if (chip->wireless_support && oplus_chg_is_wls_present()) {
		if (oplus_chg_comm_get_batt_status(chip->comm_ocm) ==
		    POWER_SUPPLY_STATUS_FULL) {
			chip->batt_full = true;
		}
	...
	if (oplus_vooc_get_allow_reading() == false) {
		...
		if (((oplus_vooc_get_fastchg_to_normal() == true) ||
		     (oplus_vooc_get_fastchg_to_warm() == true) ||
		     (oplus_pps_get_ffc_started() == true)) &&
			...
			chg_err("fastchg_present_wait_count = %d, pps_to_ffc_full_count = %d,chip->batt_volt = %d\n",
				...
				pps_to_ffc_full_count, chip->batt_volt);
			is_batt_full = 0;
			fastchg_present_wait_count++;
		...
		if ((chip->recharge_after_full == true ||
			...
			if (chip->dod0_counts == DOD0_COUNTS) {
				...
				if ((chip->recharge_after_full == true &&
				    ...
				    (oplus_get_flash_screen_ctrl() == true &&
				     chip->chg_ops->charger_suspend_check &&
				     chip->chg_ops->charger_suspend_check() ==
					     false)) {
					chip->in_rechging = true;
					chip->sw_full_count = 0;
...
static void oplus_chg_print_log(struct oplus_chg_chip *chip)
{
	if (chip->vbatt_num == 1) {
		charger_xlog_printk(
			...
			chip->total_time, chip->temperature, chip->tbatt_temp,
			chip->batt_volt, chip->batt_volt_min, chip->icharging,
			chip->ibus, chip->soc, chip->ui_soc, chip->soc_load,
			chip->batt_rm, oplus_gauge_get_batt_fc(),
			oplus_gauge_get_batt_qm(), oplus_gauge_get_batt_pd(),
			oplus_gauge_get_batt_rcu(), oplus_gauge_get_batt_rcf(),
			oplus_gauge_get_batt_fcu(), oplus_gauge_get_batt_fcf(),
			oplus_gauge_get_batt_sou(), oplus_gauge_get_batt_do0(),
			oplus_gauge_get_batt_doe(), oplus_gauge_get_batt_trm(),
			oplus_gauge_get_batt_pc(), oplus_gauge_get_batt_qs(),
			...
			chip->shell_temp, oplus_voocphy_get_fastchg_start(),
		...
		charger_xlog_printk(
			...
			chip->batt_volt, chip->batt_volt_min, chip->icharging,
			chip->ibus, chip->soc, chip->ui_soc, chip->soc_load,
			chip->batt_rm, chip->vbatt_over, chip->chging_over_time,
			chip->vchg_status, chip->tbatt_status, chip->stop_voter,
			chip->notify_code, chip->usb_status, chip->otg_online,
		...
		if (chip->pd_svooc == false) {
			...
			if (oplus_pps_get_support_type() == PPS_SUPPORT_2CP) {
				...
				chip->pd_authentication =
					oplus_pps_get_adsp_authenticate();
	...
	if (chip->chg_ops->oplus_chg_get_pd_type() > 0 &&
		...
		if (chip->pd_svooc == false) {
			...
			if (chip->chg_ops->oplus_chg_get_pd_type() ==
				...
				} else if (oplus_pps_get_chg_status() ==
					   PPS_CHARGE_END) {
				...
				if ((oplus_pps_get_chg_status() ==
					...
					if (oplus_pps_check_adapter_ability()) {
						...
						if (oplus_pps_is_allow_real() ==
							...
							oplus_pps_set_pps_dummy_started(
								true);
						}
					} else {
					   ...
					   PPS_CHARGE_END) {
					if (oplus_pps_voter_charging_start() &&
					    (oplus_pps_check_adapter_ability())) {
						oplus_pps_start(
							PPS_ADAPTER_OPLUS_V2);
					}
					chg_err(":pps status:PPS_CHARGE_END \n");
...
static void oplus_chg_pd_config(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ops->oplus_chg_get_pd_type() > 0) {
		...
		if (chip->pd_chging == false) {
			...
			chip->limits.temp_little_cool_fastchg_current_ma_high =
				chip->limits
					.pd_temp_little_cool_fastchg_current_ma_high;
			chip->limits.temp_little_cool_fastchg_current_ma_low =
				chip->limits
					.pd_temp_little_cool_fastchg_current_ma_low;
			chip->limits.temp_normal_fastchg_current_ma =
				chip->limits.pd_temp_normal_fastchg_current_ma;
			chip->limits.temp_little_cold_fastchg_current_ma_high =
				chip->limits
			...
			chip->limits.temp_cold_fastchg_current_ma_high =
				chip->limits
					.pd_temp_cold_fastchg_current_ma_high;
			chip->limits.temp_cold_fastchg_current_ma_low =
				chip->limits.pd_temp_cold_fastchg_current_ma_low;
			chip->limits.temp_cool_fastchg_current_ma_high =
				chip->limits
	...
	if (chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD) {
		if ((chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr ||
			...
			if (ret >= 0) {
				...
				chip->limits.temp_cold_fastchg_current_ma_high =
					chip->limits
						.default_temp_cold_fastchg_current_ma_high;
				chip->limits.temp_cold_fastchg_current_ma_low =
					chip->limits
						.default_temp_cold_fastchg_current_ma_low;
				chip->limits.temp_cool_fastchg_current_ma_high =
					chip->limits
...
static void oplus_chg_qc_config(struct oplus_chg_chip *chip)
{
	...
	if (!chip->cool_down_force_5v &&
	    (chip->vbatt_num == 2 || chip->limits.tbatt_pdqc_to_9v_thr < 0 ||
	     chip->temperature < chip->limits.tbatt_pdqc_to_9v_thr)) {
		if (qc_chging &&
		    (chip->charger_volt < QC_CHARGER_VOLTAGE_HIGH)) {
			chip->qc_abnormal_check_count++;
			if (chip->qc_abnormal_check_count >=
			    QC_ABNORMAL_CHECK_COUNT) {
				if (chip->chg_ops->pdo_5v &&
				    chip->qc_abnormal_check_count ==
					    QC_ABNORMAL_CHECK_COUNT) {
					chip->chg_ops->pdo_5v();
					...
					chip->limits
						.temp_normal_fastchg_current_ma =
						chip->limits
							.default_temp_normal_fastchg_current_ma;
					chip->limits
						.temp_little_cool_fastchg_current_ma =
					...
					chip->limits
						...
						chip->limits
							.default_temp_little_cold_fastchg_current_ma_low;
					chip->limits
						.temp_cold_fastchg_current_ma_high =
						chip->limits
							.default_temp_cold_fastchg_current_ma_high;
					chip->limits
						.temp_cold_fastchg_current_ma_low =
						chip->limits
							.default_temp_cold_fastchg_current_ma_low;
					chip->limits
						.temp_cool_fastchg_current_ma_high =
	...
	if (chip->vbatt_num == 1) {
		if (chip->chg_ops->check_qchv_condition &&
		    chip->chg_ops->check_qchv_condition() &&
		    chip->qc_abnormal_check_count < QC_ABNORMAL_CHECK_COUNT &&
		    qc_chging && chip->charger_volt < QC_CHARGER_VOLTAGE_HIGH) {
	...
	if (!qc_chging &&
		...
		if (ret >= 0) {
			...
			chip->limits.temp_little_cool_fastchg_current_ma_high =
				chip->limits
					.qc_temp_little_cool_fastchg_current_ma_high;
			chip->limits.temp_little_cool_fastchg_current_ma_low =
				chip->limits
					.qc_temp_little_cool_fastchg_current_ma_low;
			chip->limits.temp_little_cool_fastchg_current_ma =
				chip->limits
			...
			chip->limits.temp_cold_fastchg_current_ma_high =
				chip->limits
					.qc_temp_cold_fastchg_current_ma_high;
			chip->limits.temp_cold_fastchg_current_ma_low =
				chip->limits.qc_temp_cold_fastchg_current_ma_low;
			chip->limits.temp_cool_fastchg_current_ma_high =
				chip->limits
...
{
	if (g_charger_chip)
		schedule_delayed_work(&g_charger_chip->turn_on_charging_work,
				      0);
}
...
static void oplus_parallel_batt_chg_check_work(struct work_struct *work)
{
	...
	if (chip->aicl_done && chip->charger_exist && chip->batt_exist &&
		...
		ibat_status = oplus_chg_is_parellel_ibat_over_spec(
			batt_temp, sub_batt_temp, &target_curr);
...
static void oplus_aging_check_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_chip *chip =
		container_of(dwork, struct oplus_chg_chip, aging_check_work);
...
static void oplus_fg_soft_reset_work(struct work_struct *work)
{
	...
	if (chip->vbatt_num == 2 && (chip->soc < SOFT_REST_SOC_THRESHOLD ||
	...
	if (g_charger_chip->nightstandby_support) {
		if (get_soc_feature() && !g_charger_chip->charger_exist) {
			...
			if (new_soc != g_charger_chip->soc) {
				g_charger_chip->smooth_soc -=
					(g_charger_chip->soc - new_soc);
			}
			g_charger_chip->soc = new_soc;
			if (g_charger_chip->rsd.smooth_switch_v2) {
				oplus_chg_smooth_to_soc_v2(g_charger_chip,
							   false);
...
int oplus_chg_get_cool_down_status(void)
{
...
#ifdef OPLUS_CUSTOM_OP_DEF
	if (chip->hiz_gnd_cable && chip->svooc_disconnect_count > 0 &&
		...
		if (chip->smart_charging_screenoff) {
			if ((oplus_pps_get_support_type() == PPS_SUPPORT_2CP) &&
				...
				chip->usbtemp_cool_down =
					oplus_convert_pps_current_to_level(
						chip,
						USBTEMP_CHARGING_CURRENT_LIMIT);
			} else {
				chip->usbtemp_cool_down =
	...
	if (oplus_chg_get_voocphy_support() == NO_VOOCPHY &&
	    (oplus_is_pps_charging() == false)) {
		if (chip->bcc_cool_down) {
			if (ret == 0) {
				ret = chip->bcc_cool_down;
			} else {
				ret = (chip->bcc_cool_down < ret) ?
					      chip->bcc_cool_down :
					      ret;
			}
		}
	...
	charger_xlog_printk(
		CHG_LOG_CRTI,
		"ret = 0x%x, cool_down=%d,usbtemp_cool_down=%d, bcc_cool_down=%d\n",
		ret, chip->cool_down, chip->usbtemp_cool_down,
		chip->bcc_cool_down);
...
struct current_level SVOOC_2_0[] = {
	...
	{ 13, 6500 },  { 14, 7000 },  { 15, 7500 },  { 15, 7300 },
	{ 16, 8000 },  { 17, 8500 },  { 18, 9000 },  { 19, 9500 },
	{ 20, 10000 }, { 21, 10500 }, { 22, 11000 }, { 23, 11500 },
	{ 24, 12000 }, { 25, 12500 },
};
...
struct current_level pps_oplus[] = {
	{ 1, 1000 },  { 2, 1000 },  { 3, 1200 },  { 4, 1500 },	{ 5, 1700 },
	{ 6, 2000 },  { 7, 2200 },  { 8, 2500 },  { 9, 2700 },	{ 10, 3000 },
	{ 11, 3200 }, { 12, 3500 }, { 13, 3700 }, { 14, 4000 }, { 15, 4500 },
	{ 16, 5000 }, { 17, 5500 }, { 18, 6000 }, { 19, 6300 }, { 20, 6500 },
	{ 21, 7000 }, { 22, 7500 }, { 23, 8000 }, { 24, 8500 }
};

struct current_level OLD_SVOOC_1_0[] = {
	/*20638 RT5125 50W*/
...
int oplus_convert_level_to_current(struct oplus_chg_chip *chip, int val)
{
	...
	if (!val ||
	    (!reply_bits && (oplus_chg_get_voocphy_support() != ADSP_VOOCPHY)))
		return in_current;
	...
	if (reply_bits == 7 ||
		...
		if (is_vooc_support_single_batt_svooc() == 1 ||
		    (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY ||
		     oplus_chg_get_voocphy_support() == AP_DUAL_CP_VOOCPHY)) {
			if (is_vooc_support_old_svooc_1_0() ==
			    1) { /*20638 RT5125 50W*/
				in_current = find_current_to_level(
					val, OLD_SVOOC_1_0,
					ARRAY_SIZE(
						OLD_SVOOC_1_0)); /*20638 RT5125 50W*/
				charger_xlog_printk(
					CHG_LOG_CRTI,
					"use old current level {1, 2000}, {2, 3000}, {3, 4000} ...\n");
			} else {
				in_current = find_current_to_level(
					val, SVOOC_1_0, ARRAY_SIZE(SVOOC_1_0));
			}
		} else {
			in_current = find_current_to_level(
				val, SVOOC_2_0, ARRAY_SIZE(SVOOC_2_0));
		}
	}
...
int oplus_convert_current_to_level(struct oplus_chg_chip *chip, int val)
{
	...
	if (reply_bits == 7 ||
		...
		if (is_vooc_support_single_batt_svooc() == 1 ||
			...
			level = find_level_to_current(val, SVOOC_2_0,
						      ARRAY_SIZE(SVOOC_2_0));
		}
	} else {
		level = val / 1000;
	}
...
static const int cool_down_current_limit_normal[6] = { 1200, 1500, 2000,
						       2000, 2000, 2000 };
static const int cool_down_current_limit_onebat[6] = { 1200, 1500, 2000,
						       1500, 2000, 2000 };
static const int cool_down_current_limit_onebat_nohv[6] = { 1200, 1500, 2000,
							    2000, 2000, 2000 };
static const int old_cool_down_current_limit_onebat[6] = {
	500, 900, 1200, 1500, 2000, 1500
}; /* use for old 18w smart charger */
static const int cool_down_current_limit_vooc[6] = { 1500, 1500, 2000,
						     2500, 3000, 3500 };
...
int oplus_chg_override_by_shell_temp(int temp)
{
	...
	if (g_charger_chip->charger_exist || oplus_vooc_get_fastchg_started() ||
	    oplus_vooc_get_fastchg_to_normal() ||
	    oplus_vooc_get_fastchg_to_warm() ||
	    oplus_vooc_get_fastchg_dummy_started())
		charger_exist = true;
...
int oplus_chg_get_shell_temp(void)
{
	...
	if (rc) {
		g_charger_chip->shell_themal =
			thermal_zone_get_zone_by_name("shell_back");
		rc = thermal_zone_get_temp(g_charger_chip->shell_themal,
					   &temp_val);
		if (rc) {
...
int oplus_smart_charge_by_bcc(struct oplus_chg_chip *chip, int val)
{
	...
	if (!(oplus_vooc_get_bcc_support() || chip->smart_chg_bcc_support)) {
		charger_xlog_printk(CHG_LOG_CRTI,
				    "not support smart chg bcc mode\n");
		return 0;
	}
	...
	case CHARGER_SUBTYPE_FASTCHG_SVOOC:
		...
		chip->bcc_cool_down =
			oplus_convert_current_to_level(chip, chip->bcc_current);
		break;
	default:
		if (subtype > COOL_DOWN_LEVEL_10) {
			if (chip->chg_ops &&
			    chip->chg_ops->set_bcc_curr_to_voocphy)
				ret = chip->chg_ops->set_bcc_curr_to_voocphy(
					val);
		}
		chip->bcc_cool_down =
			oplus_convert_current_to_level(chip, chip->bcc_current);
...
void oplus_smart_charge_by_shell_temp(struct oplus_chg_chip *chip, int val)
{
	...
	onebat_index_temp =
		ARRAY_SIZE(cool_down_current_limit_onebat) < c_level_index ?
			ARRAY_SIZE(cool_down_current_limit_onebat) :
			c_level_index;
	normal_index_temp =
		ARRAY_SIZE(cool_down_current_limit_normal) < c_level_index ?
			ARRAY_SIZE(cool_down_current_limit_normal) :
			c_level_index;
	vooc_index_temp =
		ARRAY_SIZE(cool_down_current_limit_vooc) < c_level_index ?
	...
	if (!val) {
		if (c_level_index <= 0) {
			...
			if (CHARGER_SUBTYPE_PD == subtype)
				chip->limits.input_current_charger_ma =
					chip->limits
						.default_pd_input_current_charger_ma;
			else if (CHARGER_SUBTYPE_QC == subtype)
				chip->limits.input_current_charger_ma =
					chip->limits
						.default_qc_input_current_charger_ma;
			else
				chip->limits.input_current_charger_ma =
					chip->limits
						.default_input_current_charger_ma;
			...
			chip->limits.input_current_vooc_ma_normal =
				chip->limits
					.default_input_current_vooc_ma_normal;
			chip->limits.pd_input_current_charger_ma =
				chip->limits.default_pd_input_current_charger_ma;
			chip->limits.qc_input_current_charger_ma =
				chip->limits.default_qc_input_current_charger_ma;
			...
			if (chip->vbatt_num == 1) {
				chip->limits.pd_input_current_charger_ma =
					choose_little_current(
						cool_down_current_limit_onebat
							[onebat_index_temp - 1],
						chip->limits
							.default_pd_input_current_charger_ma);
				chip->limits.qc_input_current_charger_ma =
				...
				if (CHARGER_SUBTYPE_PD == subtype)
					chip->limits.input_current_charger_ma =
						chip->limits
							.pd_input_current_charger_ma;
				else if (CHARGER_SUBTYPE_QC == subtype)
					chip->limits.input_current_charger_ma =
						chip->limits
							.qc_input_current_charger_ma;
				else
					chip->limits
						.input_current_charger_ma = choose_little_current(
						cool_down_current_limit_onebat
							[onebat_index_temp - 1],
						chip->limits
							.default_input_current_charger_ma);
				...
				chip->limits.pd_input_current_charger_ma =
					choose_little_current(
						cool_down_current_limit_normal
							[normal_index_temp - 1],
						chip->limits
							.default_pd_input_current_charger_ma);
				chip->limits.qc_input_current_charger_ma =
			...
			chip->limits
				.input_current_vooc_ma_high = choose_little_current(
				cool_down_current_limit_vooc[vooc_index_temp -
							     1],
				chip->limits.default_input_current_vooc_ma_high);
			chip->limits
				.input_current_vooc_ma_warm = choose_little_current(
				cool_down_current_limit_vooc[vooc_index_temp -
							     1],
				chip->limits.default_input_current_vooc_ma_warm);
			chip->limits
				.input_current_vooc_ma_normal = choose_little_current(
				cool_down_current_limit_vooc[vooc_index_temp -
							     1],
				chip->limits
					.default_input_current_vooc_ma_normal);
	...
	case CHARGER_SUBTYPE_FASTCHG_SVOOC:
		...
		if (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY ||
			...
			if (c_level_index > 0) {
				chip->cool_down =
					oplus_convert_current_to_level(chip,
								       val);
				chip->cool_down =
					chip->cool_down < c_level_index ?
						chip->cool_down :
			...
			if (c_level_index <= 0) {
				...
				chip->limits.input_current_vooc_ma_high =
					choose_little_current(
						val,
						chip->limits
							.default_input_current_vooc_ma_high);
				chip->limits.input_current_vooc_ma_warm =
					choose_little_current(
						val,
						chip->limits
							.default_input_current_vooc_ma_warm);
				chip->limits.input_current_vooc_ma_normal =
					choose_little_current(
						val,
						chip->limits
				...
				chip->limits
					...
					chip->limits
						.default_input_current_charger_ma);
				chip->limits
					.pd_input_current_charger_ma = choose_little_current(
					chip->limits.input_current_cool_down_ma,
					chip->limits
						.default_pd_input_current_charger_ma);
				chip->limits
					.qc_input_current_charger_ma = choose_little_current(
					chip->limits.input_current_cool_down_ma,
					chip->limits
						.default_qc_input_current_charger_ma);
				min_temp = choose_little_current(
					val, cool_down_current_limit_vooc
						     [vooc_index_temp - 1]);
				chip->limits.input_current_vooc_ma_high =
					choose_little_current(
						min_temp,
	...
	case CHARGER_SUBTYPE_QC:
		if (c_level_index <= 0) {
			...
			if (chip->vbatt_num == 1) {
				chip->limits.input_current_cool_down_ma =
					val < cool_down_current_limit_onebat
								[onebat_index_temp -
								 1] ?
						val :
						cool_down_current_limit_onebat
				...
				chip->limits.input_current_cool_down_ma =
					val < cool_down_current_limit_normal
								[normal_index_temp -
								 1] ?
						val :
						cool_down_current_limit_normal
			...
			if (subtype == CHARGER_SUBTYPE_PD)
				chip->limits
					.input_current_cool_down_ma = choose_little_current(
					chip->limits.input_current_cool_down_ma,
					chip->limits
						.default_pd_input_current_charger_ma);
			else
	...
	charger_xlog_printk(
		CHG_LOG_CRTI,
		"val = [%d],  subtype = [%d], vbatt_num = %d, c_level_index = %d, pd = %d, \
		qc = %d, input_current = %d, input_cool = %d, vooc = %d %d %d \n",
		val, subtype, chip->vbatt_num, c_level_index,
		chip->limits.pd_input_current_charger_ma,
		chip->limits.qc_input_current_charger_ma,
		chip->limits.input_current_charger_ma,
		chip->limits.input_current_cool_down_ma,
...
void oplus_smart_charge_by_cool_down(struct oplus_chg_chip *chip, int val)
{
	...
	onebat_index_temp = ARRAY_SIZE(cool_down_current_limit_onebat) < val ?
				    ARRAY_SIZE(cool_down_current_limit_onebat) :
				    val;
	normal_index_temp = ARRAY_SIZE(cool_down_current_limit_normal) < val ?
				    ARRAY_SIZE(cool_down_current_limit_normal) :
				    val;
	vooc_index_temp = ARRAY_SIZE(cool_down_current_limit_vooc) < val ?
				  ARRAY_SIZE(cool_down_current_limit_vooc) :
				  val;
	...
	if (chip->old_smart_charge_standard == true) {
		memcpy(m_cool_down_current_limit_onebat,
		       old_cool_down_current_limit_onebat, sizeof(int) * 6);
		cool_down_force_5v_limit_onebat = OLD_FORCE_5V_LIMIT;
	} else if (chip->chg_ops->is_support_qcpd &&
		   !chip->chg_ops->is_support_qcpd()) {
		memcpy(m_cool_down_current_limit_onebat,
		       cool_down_current_limit_onebat_nohv, sizeof(int) * 6);
		cool_down_force_5v_limit_onebat = NEW_FORCE_5V_LIMIT;
	} else {
		memcpy(m_cool_down_current_limit_onebat,
		       cool_down_current_limit_onebat, sizeof(int) * 6);
		cool_down_force_5v_limit_onebat = NEW_FORCE_5V_LIMIT;
	}
	...
	if (chip->smart_charge_version == 1) {
		...
		if (!val) {
			...
			charger_xlog_printk(
				...
				val, chip->cool_down);
			return;
		} else if ((val == SALE_MODE_COOL_DOWN ||
			    val == SALE_MODE_COOL_DOWN_TWO) &&
		...
		if (chip->vbatt_num == 1) {
			chip->limits
				.pd_input_current_charger_ma = choose_little_current(
				m_cool_down_current_limit_onebat
					[onebat_index_temp - 1],
				chip->limits
					.default_pd_input_current_charger_ma);
			chip->limits
		...
		chip->limits.input_current_vooc_ma_high = choose_little_current(
			cool_down_current_limit_vooc[vooc_index_temp - 1],
			chip->limits.default_input_current_vooc_ma_high);
		...
		if ((chip->vbatt_num == 1) &&
		    (val < cool_down_force_5v_limit_onebat)) {
		...
		if (val & SMART_VOOC_CHARGER_CURRENT_BIT0) {
			if (oplus_vooc_get_vooc_multistep_adjust_current_support() ==
			    false)
				chip->cool_down = 1;
			else
				chip->cool_down = 1;
	...
	charger_xlog_printk(
		CHG_LOG_CRTI,
		"val->intval = [%04x], set cool_down = [%d], vbatt_num = %d, pd_input = %d, \
	qc_input = %d, input_current = %d, input_current_vooc_ma_high warm low = %d %d %d\n",
		val, chip->cool_down, chip->vbatt_num,
		chip->limits.pd_input_current_charger_ma,
		chip->limits.qc_input_current_charger_ma,
		chip->limits.input_current_charger_ma,
		chip->limits.input_current_vooc_ma_high,
	...
	while (!kthread_should_stop()) {
		...
		if (oplus_chg_get_voocphy_support() == NO_VOOCPHY &&
			...
			if (oplus_switching_support_parallel_chg()) {
				...
				sub_batt_temp =
					g_charger_chip->sub_batt_temperature;
				batt_temp = main_batt_temp > sub_batt_temp ?
						    main_batt_temp :
						    sub_batt_temp;
			} else {
	...
	if (oplus_switching_support_parallel_chg()) {
		...
		if (oplus_chg_override_by_shell_temp(parallel_batt_temp)) {
			...
			if (diff < 150 && diff > -150 &&
			    (batt_temp >= 320 || sub_batt_temp >= 320) &&
			    batt_temp <
				    g_charger_chip->limits.hot_bat_decidegc &&
				...
				} else if (batt_temp >= 320 ||
					   sub_batt_temp >= 320) {
					chging_temp =
						batt_temp > sub_batt_temp ?
							batt_temp :
							sub_batt_temp;
				} else {
					chging_temp =
			...
			if (sub_batt_temp == FG_I2C_ERROR) {
				...
				chging_temp = batt_temp > sub_batt_temp ?
						      batt_temp :
						      sub_batt_temp;
			} else {
				chging_temp = batt_temp < sub_batt_temp ?
		...
		if (oplus_chg_override_by_shell_temp(batt_temp)) {
			...
			if (g_charger_chip->support_tbatt_shell) {
				...
				if (!g_charger_chip->tbatt_shell_status &&
				    (shell_temp > g_charger_chip->limits
							  .warm_bat_decidegc ||
				     (batt_temp > g_charger_chip->limits
							  .warm_bat_decidegc &&
				      batt_current <= BATT_MIN_CURR_CHECK))) {
					g_charger_chip->tbatt_shell_status =
						true;
				} else if (g_charger_chip->tbatt_shell_status &&
					   (shell_temp <
						    g_charger_chip->limits
							    .warm_bat_decidegc &&
					    batt_temp <
						    g_charger_chip->limits
							    .warm_bat_decidegc)) {
				...
				if (g_charger_chip->tbatt_shell_status)
					chging_temp = shell_temp > batt_temp ?
							      shell_temp :
							      batt_temp;
			}
		} else {
...
static void soc_modfiy_callback(void)
{
	...
	if (get_soc_feature()) {
		...
		chg_err("[soc_ajust_feature]:soc_store0[%d], soc_store1[%d], g_charger_chip->soc[%d], night_count[%d]\n",
			soc_store[0], soc_store[1], g_charger_chip->soc,
			night_count);
		if (night_count == 1) {
			g_charger_chip->soc = g_charger_chip->soc -
					      soc_store[0] + soc_store[1];
		} else {
	...
	chg_err("[soc_ajust_feature]:modify_soc[%d], soc[%d], soc_ajusting[%d], real_soc[%d]\n",
		g_charger_chip->modify_soc, g_charger_chip->soc, soc_ajusting,
		oplus_gauge_get_batt_soc());
	chg_err("[soc_ajust_feature]:soc_store[%d], [%d], [%d]\n", soc_store[0],
		soc_store[1], soc_store[2]);
}
...
static void get_time_interval(void)
{
	if (!g_charger_chip) {
		...
		g_charger_chip->second_ktime =
			ktime_to_ms(ktime_get_boottime()) / 1000;
		chg_err("[soc_ajust_feature]:interval[%d], [%d]\n",
			g_charger_chip->first_ktime,
			g_charger_chip->second_ktime);
		if ((g_charger_chip->second_ktime -
		     g_charger_chip->first_ktime) >= TIME_INTERVALE_S) {
...
static void parallel_chg_set_balance_bat_status(int status)
{
	...
	case PARALLEL_NEED_BALANCE_BAT_STATUS4__STOP_CHARGE:
		if (chip->balancing_bat_status ==
			    PARALLEL_NEED_BALANCE_BAT_STATUS7__START_CHARGE &&
		    chip->charger_exist) {
			status =
				PARALLEL_NEED_BALANCE_BAT_STATUS7__START_CHARGE;
		}
	...
	if (chip->balancing_bat_status != status) {
		...
		oplus_switching_set_balance_bat_status(
			chip->balancing_bat_status);
	}
}
...
static int parallel_chg_check_balance_bat_status(void)
{
	...
	for (i = 0; i < 3; i++) {
		status = oplus_switching_get_if_need_balance_bat(
	...
	chg_err("balancing_bat_status:%d, current_status:%d",
		chip->balancing_bat_status, status);
...
void oplus_chg_check_break(int vbus_rising)
{
	...
	if (vbus_rising == 1) {
		...
		if (((chip->svooc_detect_time - chip->svooc_detach_time) <=
		     1000) &&
		...
		} else if (((chip->svooc_detect_time -
			     chip->svooc_detach_time) <= 100)) {
...
int oplus_chg_set_complete_charge_timeout(int val)
{
	if (!g_charger_chip || !g_charger_chip->chg_ops ||
		...
		return g_charger_chip->chg_ops->set_complete_charge_timeout(
			val);
	}
}
...
int oplus_chg_set_mps_second_otg_voltage(bool is_750mv)
{
	if (!g_charger_chip || !g_charger_chip->chg_ops ||
		...
		return g_charger_chip->chg_ops->set_mps_second_otg_voltage(
			is_750mv);
	}
}
...
static void oplus_chg_check_abnormal_adapter(int vbus_rising)
{
	...
	if (!vbus_rising) {
		...
		if (!chip->mmi_chg || oplus_vooc_get_fastchg_to_warm() ||
		    oplus_vooc_get_fastchg_to_normal() ||
		    oplus_vooc_get_fastchg_dummy_started() ||
		    (oplus_vooc_get_adapter_update_status() ==
		     ADAPTER_FW_NEED_UPDATE) ||
		...
		chg_err("vbus_down [%d %d %d %d %d]\n", pre_pd_svooc,
			pre_is_abnormal_adapter, chip->mmi_chg,
	...
	chg_err("vbus_up [%d %d %d %d %d]\n", pre_pd_svooc,
		pre_is_abnormal_adapter, chip->mmi_chg, chip->icon_debounce,
		chip->abnormal_adapter_dis_cnt);
}
...
int oplus_chg_get_pps_type(void)
{
	...
	if (!g_charger_chip->chg_ops ||
	    !g_charger_chip->chg_ops->oplus_chg_get_pd_type) {
		return PD_INACTIVE;
	}
...
{
	struct oplus_chg_chip *chip = g_charger_chip;
	int diff_time = 0, gain_time_ms = 0, batt_curve_current = 0,
	    current_cool_down = 0, current_normal_cool_down = 0;
	...
	if (current_cool_down > batt_curve_current) {
		if (current_normal_cool_down > batt_curve_current) {
			...
			gain_time_ms =
				((batt_curve_current * diff_time * 1000) /
				 current_normal_cool_down) -
				(diff_time * 1000);
		}
	} else {
		gain_time_ms = ((current_cool_down * diff_time * 1000) /
				current_normal_cool_down) -
			       (diff_time * 1000);
	}

	chip->quick_mode_gain_time_ms =
		chip->quick_mode_gain_time_ms + gain_time_ms;

	chg_err("quick_mode_gain_time_ms:%d, start_cap:%d, stop_cap:%d, start_time:%d, stop_time:%d, diff_time:%d, gain_time_ms:%d\n",
		chip->quick_mode_gain_time_ms, chip->start_cap, chip->stop_cap,
		chip->start_time, chip->quick_mode_time.tv_sec, diff_time,
		gain_time_ms);
}
