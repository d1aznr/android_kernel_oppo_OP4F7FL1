...
static bool is_wls_nor_out_dis_votable_available(struct oplus_chg_comm *dev)
{
	if (!dev->wls_nor_out_dis_votable)
		dev->wls_nor_out_dis_votable =
			oplus_find_votable("WLS_NOR_OUT_DISABLE");
...
static void oplus_chg_config_update_work(struct work_struct *work)
{
	...
	struct oplus_chg_comm *comm_dev =
		container_of(dwork, struct oplus_chg_comm, config_update_work);
	int rc;

	if (!force_update && (is_usb_charger_online(comm_dev) ||
	...
	case RECEIVE_START:
		...
		if (cfg_head->magic == OPLUS_CHG_CFG_MAGIC) {
			cfg_size = cfg_head->size +
				   sizeof(struct oplus_chg_cfg_head);
...
int oplus_chg_comm_switch_ffc(struct oplus_chg_mod *comm_ocm)
{
	...
	if (ffc_temp_region == FFC_TEMP_PRE_NORMAL ||
		...
		if (is_usb_charger_online(comm_dev)) {
			...
			oplus_vote(
				comm_dev->wls_icl_votable, USER_VOTER, true,
			...
			oplus_vote(comm_dev->wls_fv_votable, USER_VOTER, true,
				   ...
				   false);
			oplus_vote(
				comm_dev->wls_fcc_votable, USER_VOTER, true,
	...
	case FFC_FAST:
		...
		if (comm_dev->ffc_step >= step_max) {
			...
			if (usb_online) {
				...
				oplus_vote(comm_dev->wls_nor_out_dis_votable,
			...
			if (usb_online) {
				...
				oplus_vote(comm_dev->wls_icl_votable,
					   USER_VOTER, true,
					   comm_cfg->wls_ffc_icl_ma
						   [comm_dev->ffc_step]
						   [ffc_temp_region - 1],
					   true);
				oplus_vote(comm_dev->wls_fv_votable, USER_VOTER,
					   true,
					   comm_cfg->wls_ffc_fv_mv
						   [comm_dev->ffc_step],
					   false);
				oplus_vote(comm_dev->wls_fcc_votable,
					   USER_VOTER, true,
					   comm_cfg->wls_ffc_fcc_ma
						   [comm_dev->ffc_step]
						   [ffc_temp_region - 1],
...
int oplus_chg_comm_get_batt_status(struct oplus_chg_mod *comm_ocm)
{
	...
	if (is_wls_nor_out_dis_votable_available(comm_dev))
		wls_enable = !oplus_get_effective_result(
	...
	if (comm_dev->chg_done ||
	    oplus_chg_get_batt_status(comm_dev) == POWER_SUPPLY_STATUS_FULL) {
		...
		if (vchg_mv > comm_dev->vchg_max_mv ||
			...
			if (detect_time <= over_volt_count) {
				...
				if (is_wls_rx_dis_votable_available(comm_dev)) {
					oplus_vote(comm_dev->wls_rx_dis_votable,
...
static void oplus_chg_heartbeat_work(struct work_struct *work)
{
	...
	if (!comm_dev->chg_ovp && comm_dev->chg_done &&
	    !comm_dev->recharge_pending && temp_region > BATT_TEMP_UNKNOWN &&
...
ssize_t oplus_chg_comm_send_mutual_cmd(struct oplus_chg_mod *comm_ocm,
				       char *buf)
{
	int rc = 0;
	struct oplus_chg_cmd cmd;
...
ssize_t oplus_chg_comm_response_mutual_cmd(struct oplus_chg_mod *comm_ocm,
					   const char *buf, size_t count)
{
	ssize_t ret = 0;
	struct oplus_chg_comm *chip;
...
int oplus_chg_common_set_mutual_cmd(struct oplus_chg_mod *comm_ocm, u32 cmd,
				    u32 data_size, const void *data_buf)
{
	int rc;
	struct oplus_chg_comm *chip;
...
#ifdef CONFIG_OPLUS_CHG_DYNAMIC_CONFIG
	INIT_DELAYED_WORK(&comm_dev->config_update_work,
			  oplus_chg_config_update_work);
