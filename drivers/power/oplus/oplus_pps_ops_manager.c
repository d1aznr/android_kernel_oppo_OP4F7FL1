...
void oplus_get_pps_ops_name_from_dt(struct device_node *node)
{
	...
	strncpy(pps_ops_name, (rc ? "mcu-op10" : pps_ops_name_dt),
		PPS_OPS_DESC_NAME_MAX_LENTH);
...
int oplus_pps_ops_register(const char *name,
			   struct oplus_pps_operations *pps_ops)
{
	...
	if (oplus_pps_ops_desc_new == NULL) {
		oplus_pps_ops_desc_new =
			kmalloc(sizeof(struct oplus_pps_ops_desc), GFP_ATOMIC);
		if (oplus_pps_ops_desc_new != NULL) {
			strncpy(oplus_pps_ops_desc_new->name, name,
				PPS_OPS_DESC_NAME_MAX_LENTH);
			(oplus_pps_ops_desc_new
				 ->name)[PPS_OPS_DESC_NAME_MAX_LENTH - 1] =
				'\0';
			oplus_pps_ops_desc_new->pps_ops = pps_ops;

			spin_lock(&g_oplus_pps_ops_mg_data.pps_ops_list_lock);
			list_add_tail(
				&oplus_pps_ops_desc_new->list,
				&g_oplus_pps_ops_mg_data.pps_ops_list_head);
			spin_unlock(&g_oplus_pps_ops_mg_data.pps_ops_list_lock);
...
void oplus_pps_ops_deinit(void)
{
	...
	spin_lock(&g_oplus_pps_ops_mg_data.pps_ops_list_lock);
	list_for_each (pos, &g_oplus_pps_ops_mg_data.pps_ops_list_head) {
		oplus_pps_ops_desc_loopup =
			list_entry(pos, struct oplus_pps_ops_desc, list);
...
struct oplus_pps_operations *oplus_pps_ops_get(void)
{
	...
	pps_ops_desc =
		oplus_pps_ops_desc_get(g_oplus_pps_ops_mg_data.pps_ops_name);
