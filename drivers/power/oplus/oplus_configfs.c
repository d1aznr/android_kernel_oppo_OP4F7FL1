...
static struct device_attribute *oplus_ac_attributes[] = { &dev_attr_online,
							  &dev_attr_type,
							  NULL };
...
static ssize_t water_detect_feature_show(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t water_detect_feature_store(struct device *dev,
					  struct device_attribute *attr,
...
static ssize_t smartchg_soh_support_show(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t soh_report_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
...
static ssize_t normal_current_now_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	int val = 0;
	...
	if (!chip->led_on) {
		if (chip->smart_normal_cool_down == 0) {
			chip->normal_cool_down =
				oplus_convert_pps_current_to_level(chip, val);
...
static ssize_t normal_cool_down_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	int val = 0;
...
static ssize_t get_quick_mode_time_gain_show(struct device *dev,
					     struct device_attribute *attr,
					     char *buf)
{
...
static ssize_t get_quick_mode_percent_gain_show(struct device *dev,
						struct device_attribute *attr,
						char *buf)
{
...
static ssize_t stop_charging_enable_show(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t stop_charging_enable_store(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	int val = 0;
...
static ssize_t adapter_fw_update_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t batt_cb_status_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t chg_i2c_err_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t chg_i2c_err_store(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t count)
{
	int val = 0;
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (is_wls_ocm_available(chip) && !val) {
		oplus_chg_mod_get_property(
			chip->wls_ocm, OPLUS_CHG_PROP_FASTCHG_STATUS, &pval);
		val = pval.intval;
	}
...
static ssize_t screen_off_by_batt_temp_show(struct device *dev,
					    struct device_attribute *attr,
					    char *buf)
{
	int val = 0;
...
static ssize_t bcc_exception_store(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t bcc_parms_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
	int ret = 0;
...
static ssize_t bcc_current_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t bcc_current_store(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t count)
{
	int val = 0, ret = 0;
...
static ssize_t parallel_chg_mos_test_show(struct device *dev,
					  struct device_attribute *attr,
					  char *buf)
{
	struct oplus_chg_chip *chip = NULL;
	...
	if (!chip->mos_test_result) {
		if (!chip->mos_test_started)
			schedule_delayed_work(&chip->parallel_chg_mos_test_work,
					      0);
	} else {
...
static ssize_t parallel_chg_mos_status_show(struct device *dev,
					    struct device_attribute *attr,
					    char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
oplus_chg_wls_upgrade_fw_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	return 0;
}
ssize_t __attribute__((weak))
oplus_chg_wls_upgrade_fw_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
	return 0;
}
static ssize_t upgrade_firmware_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct oplus_chg_chip *chip = NULL;
	...
	if (is_wls_ocm_available(chip))
		return oplus_chg_wls_upgrade_fw_show(&chip->wls_ocm->dev, attr,
						     buf);
	return 0;
}

static ssize_t upgrade_firmware_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct oplus_chg_chip *chip = NULL;
	...
	if (is_wls_ocm_available(chip))
		count = oplus_chg_wls_upgrade_fw_store(&chip->wls_ocm->dev,
						       attr, buf, count);
...
static ssize_t status_keep_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t status_keep_store(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t count)
{
	int val = 0;
...
oplus_chg_wls_get_max_wireless_power(struct device *dev)
{
	return 0;
}

static ssize_t max_w_power_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct oplus_chg_chip *chip = NULL;
	int max_wls_power = 0;
	...
	if (is_wls_ocm_available(chip))
		max_wls_power = oplus_chg_wls_get_max_wireless_power(
			&chip->wls_ocm->dev);
...
static ssize_t charge_parameter_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct oplus_chg_chip *chip = NULL;
	...
	if (is_comm_ocm_available(chip))
		return oplus_chg_comm_charge_parameter_show(
			&chip->comm_ocm->dev, attr, buf);
	return 0;
}

static ssize_t charge_parameter_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct oplus_chg_chip *chip = NULL;
	...
	if (is_comm_ocm_available(chip))
		count = oplus_chg_comm_charge_parameter_store(
			&chip->comm_ocm->dev, attr, buf, count);
...
ssize_t __attribute__((weak))
oplus_chg_comm_send_mutual_cmd(struct oplus_chg_mod *comm_ocm, char *buf)
{
	return -EINVAL;
}
ssize_t __attribute__((weak))
oplus_chg_comm_response_mutual_cmd(struct oplus_chg_mod *comm_ocm,
				   const char *buf, size_t count)
{
	return -EINVAL;
}
...
#endif
	&dev_attr_mutual_cmd, NULL
};
#ifdef OPLUS_FEATURE_CHG_BASIC
void __attribute__((weak))
oplus_pps_get_adapter_status(struct oplus_chg_chip *chip)
{
	return;
}
...
static int oplus_ac_dir_create(struct oplus_chg_chip *chip)
{
	...
	oplus_ac_dir =
		device_create(oplus_chg_class, NULL, devt, NULL, "%s", "ac");
...
static int oplus_usb_dir_create(struct oplus_chg_chip *chip)
{
	...
	oplus_usb_dir =
		device_create(oplus_chg_class, NULL, devt, NULL, "%s", "usb");
	...
	while ((attr = *attrs++)) {
		...
		if (err) {
			chg_err("device_create_file fail!\n");
			device_destroy(oplus_usb_dir->class,
				       oplus_usb_dir->devt);
			return err;
		}
	}
...
static int oplus_battery_dir_create(struct oplus_chg_chip *chip)
{
	...
	while ((attr = *attrs++)) {
		...
		if (err) {
			chg_err("device_create_file fail!\n");
			device_destroy(oplus_battery_dir->class,
				       oplus_battery_dir->devt);
			return err;
		}
	}
...
static int oplus_wireless_dir_create(struct oplus_chg_chip *chip)
{
	...
	oplus_wireless_dir = device_create(oplus_chg_class, NULL, devt, NULL,
					   "%s", "wireless");
	...
	while ((attr = *attrs++)) {
		...
		if (err) {
			chg_err("device_create_file fail!\n");
			device_destroy(oplus_wireless_dir->class,
				       oplus_wireless_dir->devt);
			return err;
		}
	}
...
static int oplus_common_dir_create(struct oplus_chg_chip *chip)
{
	...
	while ((attr = *attrs++)) {
		...
		if (err) {
			chg_err("device_create_file fail!\n");
			device_destroy(oplus_common_dir->class,
				       oplus_common_dir->devt);
			return err;
		}
	}
