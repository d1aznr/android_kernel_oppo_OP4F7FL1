...
static bool oplus_vooc_is_battemp_exit(void)
{
	...
	if ((g_vooc_chip->vooc_batt_over_high_temp != -EINVAL) &&
	    (g_vooc_chip->vooc_batt_over_low_temp != -EINVAL)) {
		high_temp = (temp > g_vooc_chip->vooc_batt_over_high_temp);
		low_temp = (temp < g_vooc_chip->vooc_batt_over_low_temp);
		status = (g_vooc_chip->fastchg_batt_temp_status ==
			  BAT_TEMP_EXIT);
...
static void oplus_vooc_setup_watchdog_timer(struct oplus_vooc_chip *chip,
					    unsigned int ms)
{
	if (!chip) {
		chg_err("oplus_vooc_chip is not ready\n");
...
static void check_charger_out_work_func(struct work_struct *work)
{
	...
	struct oplus_vooc_chip *chip = container_of(
		dwork, struct oplus_vooc_chip, check_charger_out_work);
	int chg_vol = 0;
...
static void oplus_vooc_bcc_get_curr_func(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_vooc_chip *chip = container_of(
		dwork, struct oplus_vooc_chip, bcc_get_max_min_curr);
	int bcc_batt_volt_now;
	int i;
...
bool oplus_vooc_wake_bcc_update_work(void)
{
	...
	schedule_delayed_work(&g_vooc_chip->bcc_get_max_min_curr,
			      OPLUS_VOOC_BCC_UPDATE_INTERVAL);
...
static void vooc_watchdog_work_func(struct work_struct *work)
{
	struct oplus_vooc_chip *chip =
		container_of(work, struct oplus_vooc_chip, vooc_watchdog_work);
...
static int oplus_vooc_set_current_warm_range(struct oplus_vooc_chip *chip,
					     int vbat_temp_cur)
{
...
static int
oplus_vooc_set_current_1_temp_normal_range(struct oplus_vooc_chip *chip,
					   int vbat_temp_cur)
{
	int ret = 0;
...
static int
oplus_vooc_set_current_temp_low_normal_range(struct oplus_vooc_chip *chip,
					     int vbat_temp_cur)
{
	int ret = 0;
...
static int
oplus_vooc_set_current_temp_little_cool_range(struct oplus_vooc_chip *chip,
					      int vbat_temp_cur)
{
	int ret = 0;
...
static int oplus_vooc_set_current_temp_cool_range(struct oplus_vooc_chip *chip,
						  int vbat_temp_cur)
{
	int ret = 0;
...
static int
oplus_vooc_set_current_temp_little_cold_range(struct oplus_vooc_chip *chip,
					      int vbat_temp_cur)
{
	int ret = 0;
	if (chip->vooc_batt_over_high_temp != -EINVAL &&
...
static int oplus_vooc_init_temp_range(struct oplus_vooc_chip *chip,
				      int vbat_temp_cur)
{
	...
	chg_err("chip->vooc_temp_cur_range[%d], vbat_temp_cur[%d]",
	...
	struct oplus_vooc_chip *chip, int vbat_temp_cur)
{
	...
	if (chip->vooc_temp_cur_range == FASTCHG_TEMP_RANGE_INIT) {
		if (vbat_temp_cur < chip->vooc_little_cold_temp) { /*0-5C*/
			...
			chip->fastchg_batt_temp_status = BAT_TEMP_LITTLE_COOL;
			   ...
			   chip->vooc_normal_low_temp) { /*16-25C*/
			chip->vooc_temp_cur_range =
				FASTCHG_TEMP_RANGE_NORMAL_LOW;
			chip->fastchg_batt_temp_status = BAT_TEMP_NORMAL_LOW;
		} else {
	...
	case FASTCHG_TEMP_RANGE_NORMAL_HIGH:
		ret = oplus_vooc_set_current_1_temp_normal_range(chip,
								 vbat_temp_cur);
		break;
	case FASTCHG_TEMP_RANGE_NORMAL_LOW:
		ret = oplus_vooc_set_current_temp_low_normal_range(
			chip, vbat_temp_cur);
		break;
	case FASTCHG_TEMP_RANGE_LITTLE_COOL:
		ret = oplus_vooc_set_current_temp_little_cool_range(
			chip, vbat_temp_cur);
		break;
	case FASTCHG_TEMP_RANGE_COOL:
		ret = oplus_vooc_set_current_temp_cool_range(chip,
							     vbat_temp_cur);
		break;
	case FASTCHG_TEMP_RANGE_LITTLE_COLD:
		ret = oplus_vooc_set_current_temp_little_cold_range(
			chip, vbat_temp_cur);
		break;
	default:
		break;
...
static int
oplus_vooc_set_current_2_temp_normal_range(struct oplus_vooc_chip *chip,
					   int vbat_temp_cur)
{
	int ret = 0;
	...
	case BAT_TEMP_HIGH0:
		if (vbat_temp_cur > chip->vooc_strategy2_batt_up_temp3) {
			...
			ret = chip->vooc_strategy2_high1_current;
			   ...
			   chip->vooc_normal_low_temp) { /*T<25*/
			chip->fastchg_batt_temp_status = BAT_TEMP_NORMAL_LOW;
			chip->vooc_temp_cur_range =
				FASTCHG_TEMP_RANGE_NORMAL_LOW;
			ret = chip->vooc_strategy_normal_current;
...
static int
oplus_vooc_set_current_when_up_setting_batt_temp(struct oplus_vooc_chip *chip,
						 int vbat_temp_cur)
{
	int ret = 0;
...
int vooc_hardware_init_cp(int data)
{
	...
	if (data == 0 || data == 0x13 || data == 0x19 || data == 0x29 ||
	    data == 0x34 || data == 0x41 || data == 0x46 || data == 0x49 ||
	    data == 0x4e) {
		g_vooc_cp->cp_hardware_init_vooc();
		vooc_xlog_printk(CHG_LOG_CRTI,
				 "CHARGER_SUBTYPE_FASTCHG_VOOC data:0x%x\n",
				 data);

		return CHARGER_SUBTYPE_FASTCHG_VOOC;
	} else if (data == 0x11 || data == 0x12 || data == 0x21 ||
		   data == 0x31 || data == 0x33 || data == 0x14 ||
		   data == 0x61 || data == 0x66 || data == 0x69 ||
		   data == 0x6e) {
		g_vooc_cp->cp_hardware_init_svooc();
		vooc_xlog_printk(CHG_LOG_CRTI,
				 "CHARGER_SUBTYPE_FASTCHG_SVOOC data:0x%x\n",
				 data);
		...
		if (oplus_vooc_get_fastchg_allow() == true) {
			...
			vooc_xlog_printk(CHG_LOG_CRTI,
					 "oplus_chg_disable&suspend_charge\n");
		}
		return CHARGER_SUBTYPE_FASTCHG_SVOOC;
	} else {
		vooc_xlog_printk(CHG_LOG_CRTI,
				 "CHARGER_SUBTYPE_DEFAULT data:0x%x\n", data);
...
static int oplus_get_allowed_current_max(bool fw_7bit)
{
	...
	cur_max_val =
		fw_7bit ? cur_max_7bit[cur_stage] : cur_max_4bit[cur_stage];
...
static void oplus_vooc_fastchg_func(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_vooc_chip *chip =
		container_of(dwork, struct oplus_vooc_chip, fastchg_work);
	int i = 0;
	int bit = 0;
	int data = 0;
	int data_head = 0;
...
*/
	...
	if (data == VOOC_NOTIFY_FAST_PRESENT) {
		...
		if (chip->hiz_gnd_cable)
			chip->allowed_current_max =
				oplus_get_allowed_current_max(
					chip->vooc_reply_mcu_bits == 7);
#endif
		...
		if (abnormal_dis_cnt > 0 &&
			...
			if (abnormal_dis_cnt <
				...
				chip->allowed_current_max =
					chip->abnormal_adapter_current[0];
		...
		if (chip->fastchg_dummy_started) {
			...
			if (oplus_chg_get_icon_debounce()) {
				...
				vooc_xlog_printk(
					CHG_LOG_CRTI,
					" icon_debounce, set dummy_started true\n");
				chip->fastchg_dummy_started = true;
				oplus_chg_set_charger_type_unknown();
				oplus_vooc_check_charger_out(chip);
			} else {
		...
		if (data == 0) {
			...
			chip->fast_chg_type =
				oplus_vooc_convert_fast_chg_type(data);
		}
			...
			ret_info =
				(chip->vooc_multistep_adjust_current_support &&
				 (!(chip->support_vooc_by_normal_charger_path &&
					...
					0x03;
		} else if (chip->set_vooc_current_limit ==
		...
		if (chip->vooc_multistep_adjust_current_support &&
			...
			if (select_func_flag == 1) {
				ret_info_temp =
					oplus_vooc_set_current_when_bleow_setting_batt_temp(
						chip, temp);
			} else {
				ret_info_temp =
					oplus_vooc_set_current_when_up_setting_batt_temp(
						chip, temp);
			}
			ret_rst = oplus_vooc_get_smaller_battemp_cooldown(
...
#endif
			ret_rst = oplus_vooc_get_smaller_battemp_cooldown(
				chip->allowed_current_max, ret_info);
			if (ret_rst > 0) {
				ret_info = ret_rst;
			}
#ifdef OPLUS_CUSTOM_OP_DEF
		...
		} else if ((chip->vooc_multistep_adjust_current_support ==
			    true) &&
			   (soc > 75)) {
			ret_rst = oplus_vooc_get_smaller_battemp_cooldown(
				pre_ret_info, ret_info);
		...
		vooc_xlog_printk(
			CHG_LOG_CRTI, "temp_range[%d-%d-%d-%d-%d-%d]",
			chip->vooc_low_temp, chip->vooc_little_cold_temp,
			chip->vooc_cool_temp, chip->vooc_little_cool_temp,
			chip->vooc_normal_low_temp, chip->vooc_normal_high_temp,
			chip->vooc_high_temp);
		vooc_xlog_printk(
			CHG_LOG_CRTI,
		...
		if (oplus_vooc_get_reply_bits() == 7 ||
			...
			oplus_vooc_bcc_get_temp_soc_range(
				chip->soc_range, chip->vooc_temp_cur_range);
			if (chip->vooc_temp_cur_range) {
				ret_info = (chip->soc_range << 4) |
					   (chip->vooc_temp_cur_range - 1);
			} else {
	...
	if ((data == VOOC_NOTIFY_BTB_TEMP_OVER) ||
	    (data == VOOC_NOTIFY_TEMP_OVER) ||
	    (data == VOOC_NOTIFY_BAD_CONNECTED) ||
	    (chip->support_fake_vooc_check &&
	     data == VOOC_NOTIFY_DATA_UNKNOWN) ||
...
void fw_update_thread(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_vooc_chip *chip =
		container_of(dwork, struct oplus_vooc_chip, fw_update_work);
...
void fw_update_thread_fix(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_vooc_chip *chip =
		container_of(dwork, struct oplus_vooc_chip, fw_update_work_fix);
	const struct firmware *fw = NULL;
	int ret = 1;
	int retry = 5;
	char version[10];
...
void oplus_vooc_fw_update_work_plug_in(void)
{
	...
	INIT_DELAYED_WORK(&g_vooc_chip->fw_update_work_fix,
...
static int init_vooc_proc(struct oplus_vooc_chip *chip)
{
	...
	if (get_vooc_mcu_type(chip) == OPLUS_VOOC_MCU_HWID_STM8S) {
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH,
			 "fastchg/%d/oplus_vooc_fw.bin", get_project());
	} else if (get_vooc_mcu_type(chip) == OPLUS_VOOC_MCU_HWID_N76E) {
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH,
			 "fastchg/%d/oplus_vooc_fw_n76e.bin", get_project());
	} else if (get_vooc_mcu_type(chip) == OPLUS_VOOC_ASIC_HWID_RK826) {
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH,
			 "fastchg/%d/oplus_vooc_fw_rk826.bin", get_project());
	} else if (get_vooc_mcu_type(chip) == OPLUS_VOOC_ASIC_HWID_RT5125) {
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH,
			 "fastchg/%d/oplus_vooc_fw_rt5125.bin", get_project());
	} else {
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH,
			 "fastchg/%d/oplus_vooc_fw_op10.bin", get_project());
	}
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
...
void oplus_vooc_print_log(void)
{
	...
	vooc_xlog_printk(CHG_LOG_CRTI, "VOOC[ %d / %d / %d / %d / %d / %d]\n",
			 ...
			 g_vooc_chip->fastchg_started,
			 g_vooc_chip->fastchg_dummy_started,
			 g_vooc_chip->fastchg_to_normal,
			 g_vooc_chip->fastchg_to_warm,
			 g_vooc_chip->btb_temp_over);
}
...
int oplus_vooc_get_fast_chg_type(void)
{
	...
	if (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY ||
		...
		if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY ||
		    oplus_voocphy_get_bidirect_cp_support()) {
...
int oplus_vooc_get_reset_active_status(void)
{
	...
	if (!g_vooc_chip) {
		...
		if (active_level ==
		    g_vooc_chip->vops->get_reset_gpio_val(g_vooc_chip)) {
			return 1;
		} else {
			return 0;
		}
...
int oplus_vooc_check_asic_fw_status(void)
{
	if (!g_vooc_chip) {
		...
		if (g_vooc_chip->vops->check_asic_fw_status)
			return g_vooc_chip->vops->check_asic_fw_status(
				g_vooc_chip);
		else
			return -EINVAL;
	}
...
int oplus_vooc_get_bcc_exit_curr(void)
{
	if (!g_vooc_chip) {
		...
		g_vooc_chip->bcc_exit_curr =
			oplus_chg_bcc_get_stop_curr(g_vooc_chip);
...
bool oplus_vooc_bcc_get_temp_range(void)
{
	if (!g_vooc_chip) {
		...
		if (g_vooc_chip->bcc_temp_range ==
			    FASTCHG_TEMP_RANGE_NORMAL_LOW ||
		    g_vooc_chip->bcc_temp_range ==
			    FASTCHG_TEMP_RANGE_NORMAL_HIGH) {
			return true;
		} else {
			return false;
