...
static svooc_parallel_curve_t svooc_parallel_curve[7] = {
	...
	[0].curve_val = { 25, 2800 }, [1].curve_val = { 20, 2300 },
	[2].curve_val = { 15, 1900 }, [3].curve_val = { 10, 1400 },
	[5].curve_val = { 0, 0 },     [5].curve_val = { 0, 0 },
	[6].curve_val = { 0, 0 },
};
...
bool oplus_vooc_wake_voocphy_service_work(struct oplus_voocphy_manager *chip,
					  int request);
extern bool oplus_vooc_wake_monitor_work(struct oplus_voocphy_manager *chip);
extern bool
oplus_vooc_wake_monitor_start_work(struct oplus_voocphy_manager *chip);
...
int charger_boost_min_cpu_freq_set(int *freq, unsigned int clear_time)
{
	...
	schedule_delayed_work(&g_voocphy_chip->clear_boost_work,
			      msecs_to_jiffies(clear_time));
...
void cpuboost_charge_event(int flag)
{
	...
	if (!initialized) {
		ret = freq_qos_add_request(&policy->constraints, &freq_qos_req,
					   FREQ_QOS_MIN,
					   policy->cpuinfo.min_freq);
		if (ret < 0) {
			voocphy_info(
				"%s:  freq_qos_add_request failed!!! ret=%d \n",
				__func__, ret);
			return;
		} else {
			initialized = true;
	...
	if (flag == 1) {
		ret = freq_qos_update_request(&freq_qos_req,
					      CHARGER_UP_CPU_FREQ);
	} else {
		ret = freq_qos_update_request(&freq_qos_req,
					      policy->cpuinfo.min_freq);
...
static int oplus_voocphy_batt_fake_temp_open(struct inode *inode,
					     struct file *file)
{
	int ret;
...
static int oplus_voocphy_batt_fake_soc_open(struct inode *inode,
					    struct file *file)
{
	int ret;
...
int init_voocphy_log_buf(void)
{
	if (!g_voocphy_log_buf) {
		g_voocphy_log_buf =
			kzalloc(sizeof(struct voocphy_log_buf), GFP_KERNEL);
		chg_err("sizeof(t_voocphy_log_buf) [%d] sizeof(t_voocphy_info)[%d]",
			sizeof(struct voocphy_log_buf),
			sizeof(struct voocphy_info));
		if (g_voocphy_log_buf == NULL) {
			chg_err("voocphy_log_buf kzalloc error");
			return -1;
		}
	} else {
		memset(g_voocphy_log_buf->voocphy_info_buf, 0,
		       VOOCPHY_LOG_BUF_LEN);
	}
...
int write_info_to_voocphy_log_buf(struct voocphy_info *voocphy)
{
	...
	if (g_voocphy_log_buf->point >= VOOCPHY_LOG_BUF_LEN) {
		chg_err("g_voocphy_log_buf->point[%d] >= VOOCPHY_LOG_BUF_LEN",
			g_voocphy_log_buf->point);
		return -1;
	}
	curr_point = g_voocphy_log_buf->point;
	memcpy(&g_voocphy_log_buf->voocphy_info_buf[curr_point], voocphy,
	       sizeof(struct voocphy_info));
	curr_point = g_voocphy_log_buf->point++;
...
int print_voocphy_log_buf(void)
{
	...
	if (g_voocphy_log_buf->point <= 0) {
		chg_err("g_voocphy_log_buf->point[%d]",
			g_voocphy_log_buf->point);
		return -1;
	}

	do {
		chg_err("g_voocphy_log_buf[%d, %d, %d, 0x%x, 0x%x, 0x%x]",
			curr_point,
...
int oplus_voocphy_print_dbg_info(struct oplus_voocphy_manager *chip)
{
	...
	if (oplus_voocphy_get_bidirect_cp_support()) {
		if (((bidirect_int_flag[0].mask & chip->int_column_pre[1]) ==
		     0) &&
		    bidirect_int_flag[i].mark_except) {
			fg_dump_reg = true;
			fg_send_info = true;
			memcpy(&chip->reg_dump[9], chip->int_column_pre,
			       sizeof(chip->int_column_pre));
			printk("cp int happened %s\n",
			       bidirect_int_flag[i].except_info);
			goto chg_exception;
		}
		for (i = 1; i < 7; i++) {
			if ((bidirect_int_flag[i].mask &
		...
		for (i = 0; i < IRQ_EVNET_NUM; i++) {
			if ((int_flag[i].mask & chip->int_flag) &&
			    int_flag[i].mark_except) {
				printk("cp int happened %s\n",
				       int_flag[i].except_info);
				if (int_flag[i].mask != VOUT_OVP_FLAG_MASK &&
				    int_flag[i].mask !=
					    ADAPTER_INSERT_FLAG_MASK &&
	...
	/*print irq event*/
	for (i = 0; i < IRQ_EVNET_NUM; i++) {
		if ((vooc_flag[i].mask & chip->vooc_flag) &&
		    vooc_flag[i].mark_except) {
...
chg_exception:
	...
	if (chip->voocphy_dual_cp_support && chip->slave_ops &&
	    chip->slave_ops->get_voocphy_enable) {
		value = 0;
	...
	if (fg_dump_reg) {
		...
		if (chip->voocphy_dual_cp_support && chip->slave_ops &&
		    chip->slave_ops->dump_voocphy_reg) {
	...
	printk(KERN_ERR
	       "voocphydbg data[%d %d %d %d %d], status[%d, %d], init[%d, %d], set[%d, %d]"
	       "comm[rcv:%d, 0x%0x, 0x%0x, %d, 0x%0x, 0x%0x reply:0x%0x 0x%0x], "
	       "irqinfo[%d, %d, %d, %d, %d, %d, %d, %d] other[%d]\n",
	       chip->cp_vsys, chip->cp_ichg, chip->icharging, chip->cp_vbus,
	       chip->gauge_vbatt, //data
	       ...
	       chip->vooc_head, chip->adapter_type, chip->adapter_mesg,
	       chip->fastchg_adapter_ask_cmd, chip->voocphy_tx_buff[0],
	       chip->voocphy_tx_buff[1], //comm reply
	       chip->vooc_flag, chip->int_flag, chip->irq_total_num, //irqinfo
	       chip->irq_rcvok_num, chip->irq_rcverr_num,
	       chip->irq_tx_timeout_num, chip->irq_tx_timeout,
...
static void oplus_voocphy_monitor_timer_init(struct oplus_voocphy_manager *chip)
{
	...
	chip->mornitor_evt[VOOC_THREAD_TIMER_AP].expires =
		VOOC_AP_EVENT_TIME / VOOC_BASE_TIME_STEP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_AP].data = 0;
	chip->mornitor_evt[VOOC_THREAD_TIMER_AP].mornitor_hdler =
		oplus_voocphy_ap_event_handle;
	...
	chip->mornitor_evt[VOOC_THREAD_TIMER_CURR].expires =
		VOOC_CURR_EVENT_TIME / VOOC_BASE_TIME_STEP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_CURR].data = 0;
	chip->mornitor_evt[VOOC_THREAD_TIMER_CURR].mornitor_hdler =
		oplus_voocphy_curr_event_handle;
	...
	chip->mornitor_evt[VOOC_THREAD_TIMER_SOC].expires =
		VOOC_SOC_EVENT_TIME / VOOC_BASE_TIME_STEP;
	...
	chip->mornitor_evt[VOOC_THREAD_TIMER_VOL].expires =
		VOOC_VOL_EVENT_TIME / VOOC_BASE_TIME_STEP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_VOL].data = 0;
	chip->mornitor_evt[VOOC_THREAD_TIMER_VOL].mornitor_hdler =
		oplus_voocphy_vol_event_handle;
	...
	chip->mornitor_evt[VOOC_THREAD_TIMER_TEMP].expires =
		VOOC_TEMP_EVENT_TIME / VOOC_BASE_TIME_STEP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_TEMP].data = 0;
	chip->mornitor_evt[VOOC_THREAD_TIMER_TEMP].mornitor_hdler =
		oplus_voocphy_temp_event_handle;
	...
	chip->mornitor_evt[VOOC_THREAD_TIMER_BTB].expires =
		VOOC_BTB_EVENT_TIME / VOOC_BASE_TIME_STEP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_BTB].data = 0;
	chip->mornitor_evt[VOOC_THREAD_TIMER_BTB].mornitor_hdler =
		oplus_voocphy_btb_event_handle;
	...
	chip->mornitor_evt[VOOC_THREAD_TIMER_SAFE].expires =
		VOOC_SAFE_EVENT_TIME / VOOC_BASE_TIME_STEP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_SAFE].data = 0;
	chip->mornitor_evt[VOOC_THREAD_TIMER_SAFE].mornitor_hdler =
		oplus_voocphy_safe_event_handle;
	...
	chip->mornitor_evt[VOOC_THREAD_TIMER_COMMU].expires =
		VOOC_COMMU_EVENT_TIME / VOOC_BASE_TIME_STEP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_COMMU].data = true;
	chip->mornitor_evt[VOOC_THREAD_TIMER_COMMU].mornitor_hdler =
		oplus_voocphy_commu_process_handle;
	...
	chip->mornitor_evt[VOOC_THREAD_TIMER_DISCON].expires =
		VOOC_DISCON_EVENT_TIME / VOOC_BASE_TIME_STEP;
	...
	chip->mornitor_evt[VOOC_THREAD_TIMER_FASTCHG_CHECK].expires =
		VOOC_FASTCHG_CHECK_TIME / VOOC_BASE_TIME_STEP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_FASTCHG_CHECK].data = true;
	chip->mornitor_evt[VOOC_THREAD_TIMER_FASTCHG_CHECK].mornitor_hdler =
		oplus_voocphy_fastchg_check_process_handle;
	chip->mornitor_evt[VOOC_THREAD_TIMER_FASTCHG_CHECK].status =
		VOOC_MONITOR_STOP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_FASTCHG_CHECK].cnt = 0;
	chip->mornitor_evt[VOOC_THREAD_TIMER_FASTCHG_CHECK].timeout = false;

	chip->mornitor_evt[VOOC_THREAD_TIMER_CHG_OUT_CHECK].expires =
		VOOC_CHG_OUT_CHECK_TIME / VOOC_BASE_TIME_STEP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_CHG_OUT_CHECK].data = 0;
	chip->mornitor_evt[VOOC_THREAD_TIMER_CHG_OUT_CHECK].mornitor_hdler =
		oplus_voocphy_chg_out_check_event_handle;
	chip->mornitor_evt[VOOC_THREAD_TIMER_CHG_OUT_CHECK].status =
		VOOC_MONITOR_STOP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_CHG_OUT_CHECK].cnt = 0;
	chip->mornitor_evt[VOOC_THREAD_TIMER_CHG_OUT_CHECK].timeout = false;

	chip->mornitor_evt[VOOC_THREAD_TIMER_TEST_CHECK].expires =
		VOOC_TEST_CHECK_TIME / VOOC_BASE_TIME_STEP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_TEST_CHECK].data = 0;
	chip->mornitor_evt[VOOC_THREAD_TIMER_TEST_CHECK].mornitor_hdler = NULL;
	chip->mornitor_evt[VOOC_THREAD_TIMER_TEST_CHECK].status =
		VOOC_MONITOR_STOP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_TEST_CHECK].cnt = 0;
	chip->mornitor_evt[VOOC_THREAD_TIMER_TEST_CHECK].timeout = false;

	chip->mornitor_evt[VOOC_THREAD_TIMER_IBUS_CHECK].expires =
		VOOC_IBUS_CHECK_TIME / VOOC_BASE_TIME_STEP;
	chip->mornitor_evt[VOOC_THREAD_TIMER_IBUS_CHECK].data = 0;
	chip->mornitor_evt[VOOC_THREAD_TIMER_IBUS_CHECK].mornitor_hdler =
		oplus_voocphy_ibus_check_event_handle;
	chip->mornitor_evt[VOOC_THREAD_TIMER_IBUS_CHECK].status =
		VOOC_MONITOR_STOP;
...
void oplus_voocphy_basetimer_monitor_start(struct oplus_voocphy_manager *chip)
{
	if (chip && !hrtimer_active(&chip->monitor_btimer)) {
		chip->moniotr_kt =
			ktime_set(0, 5000000); // 0s	5000000ns	= 5ms
		hrtimer_start(&chip->monitor_btimer, chip->moniotr_kt,
			      HRTIMER_MODE_REL);
...
void oplus_voocphy_basetimer_monitor_stop(struct oplus_voocphy_manager *chip)
{
	...
	oplus_voocphy_monitor_timer_stop(chip, VOOC_THREAD_TIMER_COMMU,
					 VOOC_COMMU_EVENT_TIME);
	chip->fastchg_commu_ing = false;
}

int oplus_voocphy_read_mesg_mask(unsigned char mask, unsigned char data,
...
int oplus_voocphy_write_mesg_mask(unsigned char mask, unsigned char *pdata,
...
static int oplus_voocphy_get_chg_enable(struct oplus_voocphy_manager *chip,
					u8 *data)
{
	int rc = 0;
	if (!chip) {
...
int oplus_voocphy_get_slave_ichg(void)
{
	...
	if (g_voocphy_chip->slave_ops && g_voocphy_chip->slave_ops->get_ichg) {
		slave_cp_ichg =
			g_voocphy_chip->slave_ops->get_ichg(g_voocphy_chip);
		return slave_cp_ichg;
	}
	return 0;
}

int oplus_voocphy_slave_get_chg_enable(struct oplus_voocphy_manager *chip,
				       u8 *data)
{
	int slave_cp_ichg = 0;
	if (!chip) {
...
static int oplus_voocphy_set_chg_enable(struct oplus_voocphy_manager *chip,
					bool enable)
{
	int rc = 0;
	if (!chip) {
	...
	if (chip->ops && chip->ops->set_chg_enable) {
		...
		if (rc < 0) {
			voocphy_info(
				"oplus_voocphy_set_chg_enable fail, rc=%d.\n",
				rc);
			return rc;
		}
	}
...
int oplus_voocphy_set_adc_enable(bool enable)
{
	...
	if (g_voocphy_chip->ops && g_voocphy_chip->ops->set_adc_enable) {
		rc = g_voocphy_chip->ops->set_adc_enable(g_voocphy_chip,
							 enable);
		if (rc < 0) {
			voocphy_info(
				"oplus_voocphy_set_adc_enable fail, rc=%d.\n",
				rc);
			return rc;
		}
	}
...
int oplus_voocphy_slave_set_chg_enable(struct oplus_voocphy_manager *chip,
				       bool enable)
{
	int rc = 0;
	if (!chip) {
...
static int
oplus_voocphy_slave_set_adc_enable(struct oplus_voocphy_manager *chip,
				   bool enable)
{
	int rc = 0;
	if (!chip) {
...
static int oplus_voocphy_set_dpdm_enable(struct oplus_voocphy_manager *chip,
					 bool enable)
{
	int rc = 0;
	...
	if (chip->ops && chip->ops->set_dpdm_enable) {
		...
		if (rc < 0) {
			voocphy_info(
				"oplus_voocphy_set_dpdm_enable fail, rc=%d.\n",
				rc);
			return rc;
		}
	}
...
static int
oplus_voocphy_slave_set_dpdm_enable(struct oplus_voocphy_manager *chip,
				    bool enable)
{
	int rc = 0;
...
static int oplus_voocphy_set_predata(struct oplus_voocphy_manager *chip,
				     u16 val)
{
	int rc = 0;
	if (!chip) {
 ...
 */
static int
oplus_voocphy_set_adc_forcedly_enable(struct oplus_voocphy_manager *chip,
				      int mode)
{
	int rc = 0;
	if (!chip) {
...
int oplus_voocphy_parallel_curve_convert(unsigned char curr)
{
	...
	for (i = 0; i < 7; i++) {
		if (svooc_parallel_curve[i].curve_data.svooc_current == 0)
			return svooc_parallel_curve[i - 1]
				.curve_data.parallel_switch_current;

		if (curr >= svooc_parallel_curve[i].curve_data.svooc_current) {
			return svooc_parallel_curve[i]
				.curve_data.parallel_switch_current;
		}
	}
	return 2800;
...
static void oplus_voocphy_update_data(struct oplus_voocphy_manager *chip)
{
		...
		if (chip->parallel_change_current_count == 1) {
			...
			if (chip->parallel_switch_current == 0) {
				chip->parallel_switch_current =
					oplus_voocphy_parallel_curve_convert(
						chip->current_expect);
				voocphy_info(
					"current init,parallel_switch_current:%d",
					chip->parallel_switch_current);
			}
			voocphy_info("current set parallel_switch_current:%d",
				     chip->parallel_switch_current);
			oplus_switching_set_current(
				chip->parallel_switch_current);
		} else {
			chip->parallel_change_current_count--;
			voocphy_info("waiting :%d,parallel_switch_current:%d",
				     chip->parallel_change_current_count,
				     chip->parallel_switch_current);
		}
	}
	...
	if (chip->voocphy_dual_cp_support) {
		if (!chip->slave_ops) {
			...
			div_cp_ichg = (chip->cp_ichg > slave_cp_ichg) ?
					      (chip->cp_ichg - slave_cp_ichg) :
...
static int oplus_voocphy_hw_setting(struct oplus_voocphy_manager *chip,
				    int reason)
{
	int rc = 0;
	if (!chip) {
...
int oplus_voocphy_slave_hw_setting(struct oplus_voocphy_manager *chip,
				   int reason)
{
	int rc = 0;
	if (!chip) {
...
void oplus_voocphy_get_soc_and_temp_with_enter_fastchg(
	struct oplus_voocphy_manager *chip)
{
	...
	voocphy_info(
		"current_expect:%d batt_soc_plugin:%d batt_temp_plugin:%d sys_curve_index:%d",
...
static int oplus_voocphy_set_txbuff(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->ops && chip->ops->set_txbuff) {
		//write txbuff //0x2c
		val = (chip->voocphy_tx_buff[0] << VOOCPHY_DATA_H_SHIFT) |
		...
		voocphy_info(
			"failed: chip->ops or chip->ops->set_txbuff null\n");
	}
...
int oplus_voocphy_handle_ask_fastchg_ornot_cmd(
	struct oplus_voocphy_manager *chip)
{
	int status = VOOCPHY_SUCCESS;
	...
	case ADAPTER_VOOC20:
		...
		if (chip->parallel_charge_support) {
			status = oplus_voocphy_write_mesg_mask(
				TX0_DET_BIT3_TO_BIT7_MASK,
				&chip->voocphy_tx_buff[0],
				VOOC2_CIRCUIT_PARALLEL_R_L_DEF |
					chip->fastchg_allow);
			status = oplus_voocphy_write_mesg_mask(
				TX1_DET_BIT0_TO_BIT1_MASK,
				&chip->voocphy_tx_buff[1],
		...
		if (oplus_voocphy_get_bidirect_cp_support()) {
			...
			if (chip->impedance_calculation_newmethod) {
				...
				status = oplus_voocphy_write_mesg_mask(
					TX0_DET_BIT3_TO_BIT7_MASK,
					&chip->voocphy_tx_buff[0],
					NO_VOOC2_CIRCUIT_PARALLEL_R_L_DEF |
						chip->fastchg_allow);
				status = oplus_voocphy_write_mesg_mask(
					TX1_DET_BIT0_TO_BIT1_MASK,
					&chip->voocphy_tx_buff[1],
...
int oplus_voocphy_calcucate_identification_code(
	struct oplus_voocphy_manager *chip)
{
	int status = VOOCPHY_SUCCESS;
	...
	chip->code_id_local =
		((chip->adapter_rand_h << 9) | (chip->adapter_rand_l << 1));

	//set present need send mesg
	code_id_temp_l =
		((((chip->adapter_rand_l >> 6) & 0x1) << TX0_DET_BIT0_SHIFT) |
	...
	code_id_temp_h =
		((((chip->adapter_rand_h >> 4) & 0x1) << TX1_DET_BIT0_SHIFT) |
	...
	voocphy_info("present_code_id_temp_l:%0x, present_code_id_temp_h: %0x,"
		     ...
		     code_id_temp_l, code_id_temp_h, chip->code_id_temp_l,
		     chip->code_id_temp_h);
	return status;
}
...
int oplus_voocphy_handle_identification_cmd(struct oplus_voocphy_manager *chip)
{
	...
	voocphy_info(
		"oplus_vooc_handle_identification_cmd,adapter_rand_l:%d, adapter_rand_h:%d\n",
...
int oplus_voocphy_handle_adapter_check_commu_process_cmd(
	struct oplus_voocphy_manager *chip)
{
	int status = VOOCPHY_SUCCESS;
	...
	if (chip->adapter_check_cmmu_count ==
	    5) { //send before save rand information
		status =
		...
		chip->code_id_far =
			chip->code_id_far | (chip->adapter_mesg << 8);
		status =
			oplus_voocphy_write_mesg_mask(TX0_DET_BIT5_MASK,
						      &chip->voocphy_tx_buff[0],
						      BIT_ACTIVE);
		...
		chip->code_id_far =
			chip->code_id_far | (chip->adapter_mesg << 4);
		status =
			oplus_voocphy_write_mesg_mask(TX0_DET_BIT5_MASK,
						      &chip->voocphy_tx_buff[0],
						      BIT_ACTIVE);
		...
		chip->fastchg_adapter_ask_cmd =
			VOOC_CMD_UNKNOW; //reset cmd type

		if (chip->code_id_far != chip->code_id_local &&
		    chip->irq_tx_fail_num) {
			voocphy_info(
				"adapter check fail because TXDATA_WR_FAIL");
			oplus_voocphy_set_status_and_notify_ap(
				chip, FAST_NOTIFY_ADAPTER_STATUS_ABNORMAL);
		}
	} else {
...
int oplus_voocphy_handle_tell_model_cmd(struct oplus_voocphy_manager *chip)
{
	...
	status = oplus_voocphy_write_mesg_mask(
		TX0_DET_BIT4_MASK, &chip->voocphy_tx_buff[0], BIT_ACTIVE);
	status = oplus_voocphy_write_mesg_mask(
		TX0_DET_BIT6_MASK, &chip->voocphy_tx_buff[0], BIT_ACTIVE);

	return status;
}

int oplus_voocphy_handle_tell_model_process_cmd(
	struct oplus_voocphy_manager *chip)
{
	int status = VOOCPHY_SUCCESS;
	...
	if (chip->adapter_model_count == 2) {
		...
		chip->adapter_model_ver =
			chip->adapter_model_ver | chip->adapter_mesg;
		//adapter id bit7 is need to clear
		chip->adapter_model_ver &= 0x7F;
		/* some 0x12/0x13 adapter is 20W */
		if ((chip->adapter_model_ver == 0x13 ||
		     chip->adapter_model_ver == 0x12) &&
		    chip->adapter_type != ADAPTER_SVOOC) {
...
int oplus_voocphy_handle_ask_bat_model_process_cmd(
	struct oplus_voocphy_manager *chip)
{
	int status = VOOCPHY_SUCCESS;
	...
	if (chip->ask_batt_sys) {
		if (chip->adapter_type == ADAPTER_SVOOC) { //svooc battery curve
			temp_data_l =
				(svooc_batt_sys_curve[chip->ask_batt_sys - 1][0]
				 << 0) |
				(svooc_batt_sys_curve[chip->ask_batt_sys - 1][1]
				 << 1) |
				(svooc_batt_sys_curve[chip->ask_batt_sys - 1][2]
				 << 2) |
				(svooc_batt_sys_curve[chip->ask_batt_sys - 1][3]
				 << 3) |
				(svooc_batt_sys_curve[chip->ask_batt_sys - 1][4]
				 << 4);

			temp_data_h =
				(svooc_batt_sys_curve[chip->ask_batt_sys - 1][5]
				 << 0) |
				(svooc_batt_sys_curve[chip->ask_batt_sys - 1][6]
				 << 1);
		} else if (chip->adapter_type == ADAPTER_VOOC30) { //for cp
			temp_data_l =
				(vooc_batt_sys_curve[chip->ask_batt_sys - 1][0]
				 << 0) |
				(vooc_batt_sys_curve[chip->ask_batt_sys - 1][1]
				 << 1) |
				(vooc_batt_sys_curve[chip->ask_batt_sys - 1][2]
				 << 2) |
				(vooc_batt_sys_curve[chip->ask_batt_sys - 1][3]
				 << 3) |
				(vooc_batt_sys_curve[chip->ask_batt_sys - 1][4]
				 << 4);

			temp_data_h =
				(vooc_batt_sys_curve[chip->ask_batt_sys - 1][5]
				 << 0) |
				(vooc_batt_sys_curve[chip->ask_batt_sys - 1][6]
				 << 1);
		} else {
			voocphy_info(
				"error adapter type when write batt_sys_curve");
		}
		...
		if (chip->ask_batt_sys == 1) {
			if (chip->adapter_model_ver == 0 &&
				...
				if (chip->fastchg_real_allow) {
					chip->fastchg_allow =
						true; //batt allow dchg
					chip->fastchg_start =
						true; //fastchg_start true
					chip->fastchg_to_warm = false;
					chip->fastchg_dummy_start = false;
					oplus_voocphy_set_status_and_notify_ap(
						chip, FAST_NOTIFY_PRESENT);
					voocphy_info(
						"fastchg_real_allow = true");
				} else {
					chip->fastchg_allow = false;
					...
					oplus_voocphy_set_status_and_notify_ap(
						chip, FAST_NOTIFY_DUMMY_START);
					voocphy_info(
						"fastchg_dummy_start and reset voocphy");
				}
			}
			chip->ask_bat_model_finished = true;
			chip->fastchg_adapter_ask_cmd =
				VOOC_CMD_UNKNOW; //reset cmd
		}
	} else {
		voocphy_info("should not go here");
	}
...
int oplus_voocphy_vbus_vbatt_detect(struct oplus_voocphy_manager *chip)
{
	...
	voocphy_info("!!vbus=%d, vbatt=%d, vbus_vbatt=%d, vooc_vbus_status=%d",
		     chip->vbus, chip->gauge_vbatt, vbus_vbatt,
		     chip->vooc_vbus_status);
	return status;
}
		...
		if (chip->batt_sys_curv_found == false) {
			...
			if (chip->adapter_type == ADAPTER_SVOOC) {
				if (chip->batt_soc_plugin == BATT_SOC_0_TO_50) {
					chip->batt_sys_curv_by_soc =
						svooc_curves_soc0_2_50;
				} else if (chip->batt_soc_plugin ==
					   BATT_SOC_50_TO_75) {
					chip->batt_sys_curv_by_soc =
			...
			chip->batt_sys_curv_by_tmprange = &(
				chip->batt_sys_curv_by_soc[sys_curve_temp_idx]);
			...
			if (chip->external_gauge_support) {
				if (chip->parallel_charge_support) {
					...
					chip->gauge_vbatt =
						oplus_gauge_get_prev_batt_mvolts();
				}
			} else {
				chip->gauge_vbatt = oplus_voocphy_get_cp_vbat();
			}

			batt_sys_curv_by_tmprange =
				chip->batt_sys_curv_by_tmprange;
			for (idx = 0;
			     idx <= batt_sys_curv_by_tmprange->sys_curv_num - 1;
			     idx++) { //big -> small
				batt_sys_curve =
					&(batt_sys_curv_by_tmprange
						  ->batt_sys_curve[idx]);
				convert_ibus =
					batt_sys_curve->target_ibus * 100;
				voocphy_info(
					"!batt_sys_curve[%d] gauge_vbatt,exit,target_vbat,cp_ichg,target_ibus:[%d, %d, %d, %d, %d], \n",
					idx, chip->gauge_vbatt,
					batt_sys_curve->exit,
				...
				if (batt_sys_curve->exit == false) {
					if (chip->gauge_vbatt <=
						...
						chip->batt_sys_curv_found =
							true;
						voocphy_info(
							"! found batt_sys_curve first idx[%d]\n",
							idx);
						break;
					}
				} else {
					//exit fastchg
					voocphy_info(
						"! not found batt_sys_curve first\n");
				}
			}
			if (chip->batt_sys_curv_found) {
				voocphy_info(
					"! found batt_sys_curve idx idx[%d]\n",
					idx);
				chip->current_expect =
					chip->batt_sys_curv_by_tmprange
						->batt_sys_curve[idx]
...
int oplus_voocphy_handle_ask_current_level_cmd(
	struct oplus_voocphy_manager *chip)
{
	int status = VOOCPHY_SUCCESS;
	...
	if (chip->adapter_type == ADAPTER_VOOC20) {
		if (chip->vooc2_next_cmd != VOOC_CMD_ASK_CURRENT_LEVEL) {
			...
			chip->vooc2_next_cmd =
				VOOC_CMD_GET_BATT_VOL; //vooc2_next_cmd;
		}
	} else { //svooc&vooc30
		oplus_voocphy_write_mesg_mask(
			TX0_DET_BIT3_MASK, &chip->voocphy_tx_buff[0],
			(chip->current_expect >> 6) & 0x1);
		oplus_voocphy_write_mesg_mask(
			TX0_DET_BIT4_MASK, &chip->voocphy_tx_buff[0],
			(chip->current_expect >> 5) & 0x1);
		oplus_voocphy_write_mesg_mask(
			TX0_DET_BIT5_MASK, &chip->voocphy_tx_buff[0],
			(chip->current_expect >> 4) & 0x1);
		oplus_voocphy_write_mesg_mask(
			TX0_DET_BIT6_MASK, &chip->voocphy_tx_buff[0],
			(chip->current_expect >> 3) & 0x1);
		oplus_voocphy_write_mesg_mask(
			TX0_DET_BIT7_MASK, &chip->voocphy_tx_buff[0],
			(chip->current_expect >> 2) & 0x1);
		oplus_voocphy_write_mesg_mask(
			TX1_DET_BIT0_MASK, &chip->voocphy_tx_buff[1],
			(chip->current_expect >> 1) & 0x1);
		oplus_voocphy_write_mesg_mask(
			TX1_DET_BIT1_MASK, &chip->voocphy_tx_buff[1],
...
int oplus_voocphy_non_vooc20_handle_get_batt_vol_cmd(
	struct oplus_voocphy_manager *chip)
{
	int status = VOOCPHY_SUCCESS;
	...
	if (chip->btb_temp_over) {
		...
		if (vbatt1 < VBATT_BASE_FOR_ADAPTER) {
			...
			vbatt = (vbatt1 - VBATT_BASE_FOR_ADAPTER) /
				VBATT_DIV_FOR_ADAPTER;
		}

		data_temp_l = ((vbatt >> 6) & 0x1) |
			      (((vbatt >> 5) & 0x1) << 1) |
			      (((vbatt >> 4) & 0x1) << 2) |
			      (((vbatt >> 3) & 0x1) << 3) |
			      (((vbatt >> 2) & 0x1) << 4);
		data_temp_h =
			((vbatt >> 1) & 0x1) | (((vbatt >> 0) & 0x1) << 1);
	}
	chip->vbat_calc = vbatt;

	voocphy_info(
		"gauge_vbatt=%d, vbatt=0x%0x, "
		"data_temp_l=0x%0x, data_temp_h=0x%0x cp:%d %d %d %d %d %d",
		chip->gauge_vbatt, vbatt, data_temp_l, data_temp_h,
		chip->cp_vsys, chip->cp_ichg, chip->cp_vbus, vbatt1,
		chip->main_batt_icharging, chip->main_vbatt);
...
int oplus_voocphy_vooc20_handle_get_batt_vol_cmd(
	struct oplus_voocphy_manager *chip)
{
	int status = VOOCPHY_SUCCESS;
	unsigned int vbatt = 0;
	...
	voocphy_info("data_temp_l=0x%0x, data_temp_h=0x%0x", data_temp_l,
		     data_temp_h);
	oplus_voocphy_write_mesg_mask(TX0_DET_BIT3_TO_BIT7_MASK,
				      &chip->voocphy_tx_buff[0], data_temp_l);
	oplus_voocphy_write_mesg_mask(TX1_DET_BIT0_TO_BIT1_MASK,
...
int oplus_voocphy_handle_null_cmd(struct oplus_voocphy_manager *chip)
{
	...
	if (dchg_enable_status != FAST_CHARGER_MOS_DISABLE &&
	    ...
	    chip->adjust_curr != ADJ_CUR_STEP_REPLY_VBATT) {
		voocphy_info(
			"get error cmd data, try again adapter_check_vooc_head_count =%d",
			chip->adapter_check_cmd_data_count);
		return VOOCPHY_EUNSUPPORTED;
	} else {
		voocphy_info(
			"get error cmd data, reset voocphy & fastchg rerun check");
...
int oplus_voocphy_reply_adapter_mesg(struct oplus_voocphy_manager *chip)
{
	...
	if ((chip->fastchg_adapter_ask_cmd !=
	     VOOC_CMD_ADAPTER_CHECK_COMMU_PROCESS) &&
	    (chip->fastchg_adapter_ask_cmd != VOOC_CMD_ASK_BATT_SYS_PROCESS) &&
	    (chip->fastchg_adapter_ask_cmd != VOOC_CMD_TELL_MODEL_PROCESS)) {
		chip->fastchg_adapter_ask_cmd = chip->adapter_mesg;
	}

	voocphy_info("rx::0x%0x adapter_ask:0x%0x", chip->voocphy_rx_buff,
		     chip->fastchg_adapter_ask_cmd);
	...
	case VOOC_CMD_ADAPTER_CHECK_COMMU_PROCESS:
		status = oplus_voocphy_handle_adapter_check_commu_process_cmd(
			chip);
		break;
...
void oplus_voocphy_convert_tx_bit0_to_bit9_for_move_model(
	struct oplus_voocphy_manager *chip)
{
	unsigned int i = 0;
	unsigned int src_data = 0, des_data = 0, temp_data = 0;
...
int oplus_voocphy_send_mesg_to_adapter(struct oplus_voocphy_manager *chip)
{
	...
	voocphy_info(
		"tx_buff[0]=0x%0x, tx_buff[1]=0x%0x adapter_ask_cmd = 0x%0x adjust_curr = %d",
		chip->voocphy_tx_buff[0], chip->voocphy_tx_buff[1],
		chip->fastchg_adapter_ask_cmd, chip->adjust_curr);
	...
	if (VOOC_CMD_ASK_CURRENT_LEVEL == chip->fastchg_adapter_ask_cmd &&
		...
		if (VOOC_CMD_GET_BATT_VOL == chip->fastchg_adapter_ask_cmd &&
			...
			voocphy_info(
				"cpuboost_charge_event ADJ_CUR_STEP_DEFAULT\n");
			//trace_oplus_tp_sched_change_ux(1, task_cpu(current));
			//trace_oplus_tp_sched_change_ux(0, task_cpu(current));
		}
	}
...
int oplus_voocphy_get_mesg_from_adapter(struct oplus_voocphy_manager *chip)
{
	...
	if (vooc_head == SVOOC_HEAD || vooc_move_head == SVOOC_HEAD) {
		...
		if (chip->adapter_model_detect_count) {
			...
			chip->adapter_type =
				ADAPTER_VOOC30; //detect svooc adapter
			if (oplus_voocphy_get_bidirect_cp_support() == false) {
				if (!chip->ask_vooc3_detect) {
					...
					chip->fastchg_allow = false;
					chip->vooc_head =
						VOOC3_INVERT_HEAD; /*set vooc3 head to detect*/
				} else {
					voocphy_info(
						"VOOC20_HEAD chip->fastchg_start = %d, chip->fastchg_allow = %d",
						chip->fastchg_start,
						chip->fastchg_allow);
					chip->adapter_type =
						ADAPTER_VOOC20; /*old vooc2.0 adapter*/
				...
				chip->vooc_head = VOOC2_INVERT_HEAD;
				chip->adapter_type =
					ADAPTER_VOOC20; /*power_bank mode*/
				chip->fastchg_allow =
					false; /*power_bank mode need set fastchg_allow false*/
			}
		}
	} else {
		chip->rcv_date_err_num++;
		...
		if (dchg_enable_status != FAST_CHARGER_MOS_DISABLE &&
		    chip->pre_adapter_ask_cmd == VOOC_CMD_GET_BATT_VOL &&
		    chip->adapter_check_vooc_head_count <
			    ADAPTER_CHECK_VOOC_HEAD_TIMES &&
		    chip->adjust_curr != ADJ_CUR_STEP_REPLY_VBATT) {
			voocphy_info(
				"unknow frame head error, try again adapter_check_vooc_head_count =%d",
				chip->adapter_check_vooc_head_count);
			return VOOCPHY_EUNSUPPORTED;
		} else {
			voocphy_info(
				"unknow frame head, reset voocphy & fastchg rerun check");
			oplus_voocphy_set_status_and_notify_ap(
				chip, FAST_NOTIFY_ERR_COMMU);
			return VOOCPHY_EUNSUPPORTED;
		}
	}
	...
	if (chip->fastchg_reactive == true) {
		voocphy_info(
			"(fastchg_dummy_start or err commu) && adapter_mesg != 0x04");
...
bool oplus_voocphy_ap_allow_fastchg(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->fastchg_adapter_ask_cmd == VOOC_CMD_ASK_FASTCHG_ORNOT) {
		ap_allow_fastchg =
			(chip->voocphy_tx_buff[0] & AP_ALLOW_FASTCHG);
		if (ap_allow_fastchg) {
			voocphy_info("fastchg_stage OPLUS_FASTCHG_STAGE_2\n");
			oplus_voocphy_set_fastchg_state(chip,
							OPLUS_FASTCHG_STAGE_2);
		}
...
int oplus_voocphy_set_is_vbus_ok_predata(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->adapter_type == ADAPTER_SVOOC) {
		if (chip->ask_bat_model_finished) {
			chip->ask_bat_model_finished = false;
			oplus_voocphy_set_predata(
				chip, OPLUS_IS_VUBS_OK_PREDATA_SVOOC);
		}
	} else if (chip->adapter_type == ADAPTER_VOOC30) {
		if (chip->ask_bat_model_finished) {
			chip->ask_bat_model_finished = false;
			oplus_voocphy_set_predata(
				chip, OPLUS_IS_VUBS_OK_PREDATA_VOOC30);
		}
	} else if (chip->adapter_type == ADAPTER_VOOC20) {
		oplus_voocphy_set_predata(chip,
					  OPLUS_IS_VUBS_OK_PREDATA_VOOC20);
	} else {
...
static int
oplus_voocphy_get_write_txbuff_error(struct oplus_voocphy_manager *chip)
{
	if (!chip) {
...
irqreturn_t oplus_voocphy_interrupt_handler(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->fastchg_need_reset) {
		...
		chip->fastchg_need_reset = 0;
		oplus_voocphy_set_status_and_notify_ap(
			chip, FAST_NOTIFY_USER_EXIT_FASTCHG);
		goto handle_done;
	}
	...
	if (chip->vooc_flag & RXDATA_DONE_FLAG_MASK) {
		...
		if (chip->irq_rxdone_num <= MAX_IGNORE &&
			...
			if (chip->voocphy_rx_buff == FIRST_FRAME) {
				voocphy_info(
					"valid data of the first frame , irq_num = %d\n",
					chip->irq_rxdone_num);
		...
		if (!chip->start_vaild_frame &&
			...
			chip->fastchg_dummy_start = false;
			voocphy_info(
				"invalid data of the first frame , irq_num = %d\n",
				chip->irq_rxdone_num);
			oplus_voocphy_set_status_and_notify_ap(
				chip, FAST_NOTIFY_ADAPTER_STATUS_ABNORMAL);
		}
		...
		if (!chip->fastchg_commu_stop) {
			status = oplus_voocphy_monitor_timer_stop(
				chip, VOOC_THREAD_TIMER_COMMU,
				VOOC_COMMU_EVENT_TIME);
			if (status != VOOCPHY_SUCCESS) {
	...
	if (chip->vooc_flag == 0xF) {
		voocphy_info(
			"!RX_START_FLAG & TXDATA_WR_FAIL_FLAG occured do nothing");
		...
		if (oplus_voocphy_get_bidirect_cp_support() == false) {
			...
			case ADAPTER_SVOOC:
				torch_exit_fastchg =
					oplus_voocphy_svooc_commu_with_voocphy(
						chip);
				break;
			case ADAPTER_VOOC20:
				status = oplus_vooc_adapter_work_as_power_bank(
					chip);
				break;
			default:
				voocphy_info(
					"should not go here, reset voocphy");
...
void oplus_voocphy_handle_voocphy_status(struct work_struct *work)
{
	struct oplus_voocphy_manager *chip = container_of(
		work, struct oplus_voocphy_manager, notify_fastchg_work.work);
	int intval = 0;
	...
	if (intval == FAST_NOTIFY_PRESENT) {
		...
		if (oplus_voocphy_get_bidirect_cp_support()) {
			if (chip->fastchg_start == false) {
				chip->fastchg_start =
					true; /*lkl need to avoid guage data*/
		...
		chip->fast_chg_type = chip->adapter_model_ver;
		voocphy_info(
			"fastchg dummy start: [%d], adapter version: [0x%0x]\n",
	...
	} else if (intval == FAST_NOTIFY_FULL ||
		   intval == FAST_NOTIFY_BAD_CONNECTED) {
		voocphy_info("%s\r\n", intval == FAST_NOTIFY_FULL ?
					       "FAST_NOTIFY_FULL" :
		...
		if (intval == FAST_NOTIFY_FULL &&
			...
			chip->fastchg_check_stop = false;
			oplus_voocphy_monitor_timer_start(
				chip, VOOC_THREAD_TIMER_FASTCHG_CHECK,
		...
		printk(KERN_ERR
		       "!!![voocphy] FAST_NOTIFY_BTB_TEMP_OVER: [%d]\n",
		       intval);
					    ...
					    int fastchg_notify_status)
{
	...
	case FAST_NOTIFY_ADAPTER_STATUS_ABNORMAL:
		...
		if (fastchg_notify_status == FAST_NOTIFY_DUMMY_START ||
		    fastchg_notify_status ==
			    FAST_NOTIFY_ADAPTER_STATUS_ABNORMAL) {
			voocphy_info(
				"FAST_NOTIFY_DUMMY_START or FAST_NOTIFY_ADAPTER_STATUS_ABNORMAL:%d",
				fastchg_notify_status);
	...
	case FAST_NOTIFY_SWITCH_TEMP_RANGE:
		...
		oplus_voocphy_monitor_timer_start(
			chip, VOOC_THREAD_TIMER_DISCON, VOOC_DISCON_EVENT_TIME);
		chip->fastchg_notify_status = fastchg_notify_status;
		...
		if (fastchg_notify_status == FAST_NOTIFY_BATT_TEMP_OVER) {
			...
			chip->fastchg_to_warm = true;
			voocphy_info(
				"VOOC_THREAD_TIMER_FASTCHG_CHECK start for FAST_NOTIFY_BATT_TEMP_OVER:%d",
				fastchg_notify_status);
	...
	case FAST_NOTIFY_ERR_COMMU:
		...
		oplus_voocphy_get_chg_enable(chip, &dchg_enable_status);
		if (chip->fastchg_err_commu == false &&
		    ((chip->adapter_type == ADAPTER_SVOOC &&
		      dchg_enable_status == 0) ||
		     (chip->adapter_type == ADAPTER_VOOC20 &&
		      chip->fastchg_allow))) {
			voocphy_info(
				"!VOOC_THREAD_TIMER_FASTCHG_CHECK abnormal status, should run fastchg check");
...
bool oplus_voocphy_user_exit_fastchg(struct oplus_voocphy_manager *chip)
{
	if (chip) {
		voocphy_dbg("user 1 %s fastchg\n",
			    chip->user_exit_fastchg == true ? "not allow" :
							      "allow");
...
void oplus_voocphy_notify(int event)
{
	...
	voocphy_info(" start %s flash led %d, %d\n", event ? "open" : "close",
		     chip->user_exit_fastchg, chip->fastchg_recovering);
	if (1 == event) {
		if (oplus_voocphy_get_fastchg_state() ==
		    OPLUS_FASTCHG_STAGE_2) {
			oplus_voocphy_set_status_and_notify_ap(
				chip, FAST_NOTIFY_USER_EXIT_FASTCHG);
		   ...
		   event /* && chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP*/) { //recover
		oplus_voocphy_set_user_exit_fastchg(false);
		oplus_voocphy_set_fastchg_state(chip, OPLUS_FASTCHG_STAGE_1);
		chip->fastchg_recover_cnt = 0;
		plugIn = oplus_chg_stats();
...
unsigned char
oplus_voocphy_set_fastchg_current(struct oplus_voocphy_manager *chip)
{
	unsigned char current_expect_temp = chip->current_expect;

	voocphy_info("max_curr[%d] ap_curr[%d] temp_curr[%d]",
		     chip->current_max, chip->current_ap,
		     chip->current_batt_temp);

	chip->current_expect = chip->current_max > chip->current_ap ?
				       chip->current_ap :
				       chip->current_max;

	chip->current_expect = chip->current_expect > chip->current_batt_temp ?
				       chip->current_batt_temp :
				       chip->current_expect;
	if (oplus_switching_support_parallel_chg() == PARALLEL_MOS_CTRL) {
		voocphy_info("batt_spec_curr[%d]", chip->current_spec);
		chip->current_expect =
			chip->current_expect > chip->current_spec ?
				chip->current_spec :
				chip->current_expect;
	}

	if (current_expect_temp != chip->current_expect) {
		voocphy_info("current_expect[%d --> %d]", current_expect_temp,
			     chip->current_expect);
		if (chip->parallel_charge_support &&
			...
			if (current_expect_temp <= chip->current_expect) {
				...
				chip->parallel_switch_current =
					oplus_voocphy_parallel_curve_convert(
						chip->current_expect);
				voocphy_info(
					"current up,parallel_switch_current:%d",
					chip->parallel_switch_current);
				oplus_switching_set_current(
					chip->parallel_switch_current);
			} else {
				...
				chip->parallel_switch_current =
					oplus_voocphy_parallel_curve_convert(
						chip->current_expect);
				voocphy_info(
					"current down waiting :%d,parallel_switch_current:%d",
					chip->parallel_switch_current,
					chip->parallel_change_current_count);
			}
		}
...
enum hrtimer_restart oplus_voocphy_monitor_base_timer(struct hrtimer *time)
{
	...
	for (i = 0; i < MONITOR_EVENT_NUM; i++) {
		if (chip->mornitor_evt[i].status == VOOC_MONITOR_START) {
			...
			if (chip->mornitor_evt[i].cnt ==
			    chip->mornitor_evt[i].expires) {
				chip->mornitor_evt[i].timeout = true;
				chip->mornitor_evt[i].cnt = 0;
				chip->mornitor_evt[i].status =
					VOOC_MONITOR_STOP;
	...
	hrtimer_forward(&chip->monitor_btimer,
			chip->monitor_btimer.base->get_time(),
			chip->moniotr_kt);
...
void oplus_voocphy_monitor_process_events(struct work_struct *work)
{
	...
	struct oplus_voocphy_manager *chip = container_of(
		work, struct oplus_voocphy_manager, monitor_work.work);

	//event loop
	for (evt_i = 0; evt_i < MONITOR_EVENT_NUM; evt_i++) {
		if (chip->mornitor_evt[evt_i].timeout) {
			chip->mornitor_evt[evt_i].timeout =
				false; //avoid repeating run monitor event
...
int oplus_voocphy_get_ap_current(void)
{
	...
	if (g_voocphy_chip->adapter_type == ADAPTER_SVOOC) {
		if (g_charger_chip->cool_down >
			    g_voocphy_chip->svooc_cool_down_num - 1 ||
		    g_charger_chip->cool_down < 0)
			g_charger_chip->cool_down = 0;

		smartchg_current = g_voocphy_chip->svooc_cool_down_current_limit
					   [g_charger_chip->cool_down];
		smartchg_current /= svooc_current_factor;

		if (screenoff_current == 0 ||
		    screenoff_current >
			    g_voocphy_chip->svooc_cool_down_current_limit[0] /
				    svooc_current_factor) {
			screenoff_current =
				g_voocphy_chip
					->svooc_cool_down_current_limit[0] /
				svooc_current_factor;
		}
		...
		if (g_charger_chip->cool_down >
			    g_voocphy_chip->vooc_cool_down_num - 1 ||
		    g_charger_chip->cool_down < 0)
			g_charger_chip->cool_down = 0;

		smartchg_current = g_voocphy_chip->vooc_cool_down_current_limit
					   [g_charger_chip->cool_down];

		if (screenoff_current == 0 ||
		    screenoff_current >
			    g_voocphy_chip->vooc_cool_down_current_limit[0]) {
			screenoff_current =
				g_voocphy_chip->vooc_cool_down_current_limit[0];
		}
	}

	ap_request_current = smartchg_current > screenoff_current ?
				     screenoff_current :
				     smartchg_current;
	...
	if (g_charger_chip->smart_charge_user == SMART_CHARGE_USER_USBTEMP) {
		if (g_charger_chip->smart_charging_screenoff) {
			smartchg_usbtemp_current =
				USBTEMP_CHARGING_CURRENT_LIMIT / 100;
		} else {
			if (g_voocphy_chip->adapter_type == ADAPTER_SVOOC) {
				smartchg_usbtemp_current =
					g_voocphy_chip
						->svooc_cool_down_current_limit
							[3] /
					svooc_current_factor;
			} else if (g_voocphy_chip->adapter_type ==
					   ADAPTER_VOOC20 ||
				   g_voocphy_chip->adapter_type ==
					   ADAPTER_VOOC30) {
				smartchg_usbtemp_current =
					g_voocphy_chip
						->vooc_cool_down_current_limit[3];
			}
		}
		ap_request_current =
			smartchg_usbtemp_current > ap_request_current ?
				ap_request_current :
				smartchg_usbtemp_current;
	}
	...
	voocphy_info(
		...
		g_charger_chip->led_on, g_voocphy_chip->adapter_type,
		smartchg_usbtemp_current);
...
int oplus_voocphy_ap_event_handle(unsigned long data)
{
	...
	if (!chip->btb_temp_over) { //btb temp is normal
		...
		status = oplus_voocphy_monitor_timer_start(g_voocphy_chip,
...
void oplus_voocphy_reset_temp_range(struct oplus_voocphy_manager *chip)
{
	if (chip != NULL) {
		chip->vooc_normal_high_temp =
			chip->vooc_normal_high_temp_default;
		chip->vooc_little_cold_temp =
			chip->vooc_little_cold_temp_default;
		chip->vooc_cool_temp = chip->vooc_cool_temp_default;
		chip->vooc_little_cool_temp =
			chip->vooc_little_cool_temp_default;
		chip->vooc_normal_low_temp = chip->vooc_normal_low_temp_default;
	}
}

static int oplus_vooc_set_current_warm_range(struct oplus_voocphy_manager *chip,
					     int vbat_temp_cur)
{
	...
	if (chip->vooc_batt_over_high_temp != -EINVAL &&
		...
		if (chip->vooc_strategy_change_count >= VOOC_TEMP_OVER_COUNTS) {
			...
			voocphy_info("[vooc_monitor] temp_over:%d",
				     vbat_temp_cur);
			oplus_voocphy_set_status_and_notify_ap(
				g_voocphy_chip, FAST_NOTIFY_BATT_TEMP_OVER);
		}
		...
		oplus_voocphy_set_status_and_notify_ap(
			g_voocphy_chip, FAST_NOTIFY_SWITCH_TEMP_RANGE);
	} else {
		chip->fastchg_batt_temp_status = BAT_TEMP_WARM;
		chip->vooc_temp_cur_range = FASTCHG_TEMP_RANGE_WARM;
		ret = chip->vooc_strategy_normal_current;
...
static int
oplus_voocphy_set_current_temp_normal_range(struct oplus_voocphy_manager *chip,
					    int vbat_temp_cur)
{
	int ret = 0;
	...
	case BAT_TEMP_HIGH2:
		if (chip->vooc_normal_high_temp != -EINVAL &&
			...
			oplus_voocphy_set_status_and_notify_ap(
			   ...
			   vbat_temp_cur > chip->vooc_batt_over_high_temp) {
			chip->vooc_strategy_change_count++;
			if (chip->vooc_strategy_change_count >=
			    VOOC_TEMP_OVER_COUNTS) {
				chip->vooc_strategy_change_count = 0;
				chip->fastchg_batt_temp_status = BAT_TEMP_EXIT;
				voocphy_info("[vooc_monitor] temp_over:%d",
					     vbat_temp_cur);
	...
	case BAT_TEMP_LOW0:
		if (vbat_temp_cur > chip->vooc_strategy1_batt_high_temp0) {
			...
			ret = vooc_strategy1_high_current0;
			   ...
			   chip->vooc_normal_low_temp) { /*T<25C*/
			chip->fastchg_batt_temp_status = BAT_TEMP_NORMAL_LOW;
			chip->vooc_temp_cur_range =
				FASTCHG_TEMP_RANGE_NORMAL_LOW;
			ret = chip->vooc_strategy_normal_current;
...
static int oplus_voocphy_set_current_temp_low_normal_range(
	struct oplus_voocphy_manager *chip, int vbat_temp_cur)
{
	int ret = 0;
...
static int oplus_voocphy_set_current_temp_little_cool_range(
	struct oplus_voocphy_manager *chip, int vbat_temp_cur)
{
	int ret = 0;
	...
	if (vbat_temp_cur < chip->vooc_little_cool_temp &&
		...
		if (vbat_temp_cur >= chip->vooc_little_cool_temp) {
			...
			if (oplus_chg_get_ui_soc() <= 90) {
				chip->fastchg_batt_temp_status = BAT_TEMP_EXIT;
				chip->vooc_temp_cur_range =
					FASTCHG_TEMP_RANGE_INIT;
				voocphy_info(
					"[vooc_monitor] switch temp range:%d",
					vbat_temp_cur);
				oplus_voocphy_set_status_and_notify_ap(
					g_voocphy_chip,
...
static int
oplus_voocphy_set_current_temp_cool_range(struct oplus_voocphy_manager *chip,
					  int vbat_temp_cur)
{
	int ret = 0;
	if (chip->vooc_batt_over_high_temp != -EINVAL &&
		...
		if (chip->vooc_strategy_change_count >= VOOC_TEMP_OVER_COUNTS) {
		   ...
		   vbat_temp_cur >=
			   chip->vooc_little_cold_temp) { /*5C <=T<12C*/
		chip->vooc_temp_cur_range = FASTCHG_TEMP_RANGE_COOL;
		chip->fastchg_batt_temp_status = BAT_TEMP_COOL;
		ret = chip->vooc_strategy_normal_current;
	} else {
...
static int oplus_voocphy_set_current_temp_little_cold_range(
	struct oplus_voocphy_manager *chip, int vbat_temp_cur)
{
	int ret = 0;
	if (chip->vooc_batt_over_high_temp != -EINVAL &&
...
static int
oplus_voocphy_set_current_by_batt_temp(struct oplus_voocphy_manager *chip,
				       int vbat_temp_cur)
{
	int ret = 0;
	...
	if (chip->vooc_temp_cur_range == FASTCHG_TEMP_RANGE_INIT) {
		if (vbat_temp_cur < chip->vooc_little_cold_temp) { /*0-5C*/
			...
			chip->fastchg_batt_temp_status = BAT_TEMP_COOL;
			   ...
			   chip->vooc_little_cool_temp) { /*12-16C*/
			chip->vooc_temp_cur_range =
				FASTCHG_TEMP_RANGE_LITTLE_COOL;
			chip->fastchg_batt_temp_status = BAT_TEMP_LITTLE_COOL;
		} else if (vbat_temp_cur <
			   chip->vooc_normal_low_temp) { /*16-35C*/
			chip->vooc_temp_cur_range =
				FASTCHG_TEMP_RANGE_NORMAL_LOW;
			chip->fastchg_batt_temp_status = BAT_TEMP_NORMAL_LOW;
		} else {
			if (chip->vooc_normal_high_temp == -EINVAL ||
			    vbat_temp_cur <
				    chip->vooc_normal_high_temp) { /*25C-43C*/
				chip->vooc_temp_cur_range =
					FASTCHG_TEMP_RANGE_NORMAL_HIGH;
				chip->fastchg_batt_temp_status =
	...
	case FASTCHG_TEMP_RANGE_NORMAL_HIGH:
		ret = oplus_voocphy_set_current_temp_normal_range(
			chip, vbat_temp_cur);
		break;
	case FASTCHG_TEMP_RANGE_NORMAL_LOW:
		ret = oplus_voocphy_set_current_temp_low_normal_range(
			chip, vbat_temp_cur);
		break;
	case FASTCHG_TEMP_RANGE_LITTLE_COOL:
		ret = oplus_voocphy_set_current_temp_little_cool_range(
			chip, vbat_temp_cur);
		break;
	case FASTCHG_TEMP_RANGE_COOL:
		ret = oplus_voocphy_set_current_temp_cool_range(chip,
								vbat_temp_cur);
		break;
	case FASTCHG_TEMP_RANGE_LITTLE_COLD:
		ret = oplus_voocphy_set_current_temp_little_cold_range(
			chip, vbat_temp_cur);
		break;
	default:
		break;
...
int oplus_voocphy_btb_event_handle(unsigned long data)
{
	...
	if (!chip->btb_temp_over) {
		if (btb_detect) {
			...
			oplus_voocphy_set_adc_forcedly_enable(
				chip, ADC_FORCEDLY_ENABLED);
			chip->btb_temp_over = true;
		}
		status =
			oplus_voocphy_monitor_timer_start(g_voocphy_chip,
		...
		if (!btb_err_report) {
			btb_err_report = true;
			oplus_voocphy_set_status_and_notify_ap(
				g_voocphy_chip, FAST_NOTIFY_BTB_TEMP_OVER);
		}
		status = oplus_voocphy_monitor_timer_start(
			g_voocphy_chip, VOOC_THREAD_TIMER_BTB,
...
int oplus_voocphy_temp_event_handle(unsigned long data)
{
	...
	voocphy_info("!batt_temp = %d %d %d %d\n", batt_temp,
		     allow_temp_monitor, chip->vooc_temp_cur_range,
		     chip->fastchg_batt_temp_status);

	if ((!chip->btb_temp_over) && (allow_temp_monitor == true)) {
		chip->current_batt_temp =
			oplus_voocphy_set_current_by_batt_temp(chip, batt_temp);
		...
		if (chip->voocphy_dual_cp_support) {
			...
			if (oplus_voocphy_get_bidirect_cp_support()) {
				...
				if ((g_charger_chip != NULL) &&
					...
					chip->current_batt_temp = 40;
					voocphy_info(
						"bidirect_abnormal_adapter = %d, chip->current_batt_temp = %d",
						g_charger_chip
							->bidirect_abnormal_adapter,
						chip->current_batt_temp);
				}
			} else {
				if (chip->adapter_type == ADAPTER_SVOOC) {
					chip->current_batt_temp /= 2;
				} else if (chip->adapter_type ==
						   ADAPTER_VOOC20 ||
					   chip->adapter_type ==
						   ADAPTER_VOOC30) {
	...
	if (batt_temp >= chip->vooc_batt_over_high_temp ||
	    batt_temp <= chip->vooc_batt_over_low_temp) {
		oplus_voocphy_set_status_and_notify_ap(
			g_voocphy_chip, FAST_NOTIFY_BATT_TEMP_OVER);
	}

	status = oplus_voocphy_monitor_timer_start(
		g_voocphy_chip, VOOC_THREAD_TIMER_TEMP, VOOC_TEMP_EVENT_TIME);
...
void oplus_voocphy_request_fastchg_curv(struct oplus_voocphy_manager *chip)
{
	...
	if (!chip->batt_sys_curv_found) { /*first enter and find index*/
		...
		if (batt_sys_curve->exit == false &&
		    (idx < BATT_SYS_ROW_MAX - 1)) {
			voocphy_info(
				"!fastchg curv2 [%d %d %d %d %d %d %d %d %d]\n",
				cc_cnt, idx, batt_sys_curve->exit,
				batt_sys_curve->target_time,
				batt_sys_curve->chg_time, chip->gauge_vbatt,
				chip->current_max,
				(&(batt_sys_curv_by_tmprange
					   ->batt_sys_curve[idx]))
					->target_ibus,
				...
				if (chip->sub_batt_icharging >
					...
					if (switch_ocp_cnt > 6) {
						...
						if (!batt_sys_curve_next) {
							voocphy_info(
								"batt_sys_curve is a NULL pointer!!!!\n");
							return;
						}
						chip->cur_sys_curv_idx += 1;
						batt_sys_curve_next->chg_time =
							0;
						cc_cnt = 0;
						chip->current_max =
							chip->current_max >
								...
								chip->current_max;
						voocphy_info(
							"!switch error? switch fastchg curv [%d %d %d]\n",
							chip->current_max,
							batt_sys_curve_next
								->target_ibus,
							batt_sys_curve_next
			...
			if ((chip->gauge_vbatt >
				     batt_sys_curve
					     ->target_vbat //switch by vol_thr
			     && chip->cp_ichg <
					convert_ibus + TARGET_VOL_OFFSET_THR) ||
			    (batt_sys_curve->target_time > 0 &&
			     batt_sys_curve->chg_time >=
				     batt_sys_curve
					     ->target_time) //switch by chg time
			) {
				cc_cnt++;
				if (cc_cnt > 3) {
					if (batt_sys_curve->exit == false) {
						if (batt_sys_curve->chg_time >=
						    batt_sys_curve->target_time) {
							voocphy_info(
								"switch fastchg curv by chgtime[%d, %d]\n",
								batt_sys_curve
									->chg_time,
								batt_sys_curve
									->target_time);
	...
	voocphy_info("curv info [%d %d %d %d]\n", chip->gauge_vbatt,
		     chip->cp_ichg, chip->current_max, cc_cnt);
}
...
int oplus_voocphy_vol_event_handle(unsigned long data)
{
	...
	if (chip->parallel_charge_support) {
		...
		chip->pre_gauge_vbatt = chip->main_vbatt > chip->sub_vbatt ?
						chip->main_vbatt :
						chip->sub_vbatt;
		chip->main_batt_icharging = -oplus_gauge_get_batt_current();
		chip->sub_batt_icharging = -oplus_gauge_get_sub_batt_current();
		chip->icharging =
			chip->main_batt_icharging + chip->sub_batt_icharging;
		voocphy_info(
			"batt[%d, %d], main_batt[%d, %d], sub_batt[%d, %d]\n",
			chip->pre_gauge_vbatt, chip->icharging,
			chip->main_vbatt, chip->main_batt_icharging,
			chip->sub_vbatt, chip->sub_batt_icharging);
	} else if (chip->external_gauge_support) {
		chip->gauge_vbatt = oplus_gauge_get_batt_mvolts();
	}

	voocphy_info("[oplus_voocphy_vol_event_handle] vbatt: %d",
		     chip->gauge_vbatt);
	if ((chip->fastchg_notify_status & 0xFF) == FAST_NOTIFY_PRESENT)
		fast_full_count = 0;
	...
	if (!chip->btb_temp_over) {
		...
		if ((chip->batt_temp_plugin ==
			     VOOCPHY_BATT_TEMP_LITTLE_COLD /*0-5 chg to 4430mV*/
		     &&
		     chip->gauge_vbatt > chip->vooc_little_cold_full_voltage) ||
		    (chip->batt_temp_plugin ==
			     VOOCPHY_BATT_TEMP_COOL /*5-12 chg to 4430mV*/
		     && chip->gauge_vbatt > chip->vooc_cool_full_voltage) ||
		    (chip->batt_temp_plugin ==
			     VOOCPHY_BATT_TEMP_WARM /*43-52 chg to 4130mV*/
			...
			voocphy_info("vbatt 1time fastchg full: %d",
				     chip->gauge_vbatt);
			oplus_voocphy_set_status_and_notify_ap(
				g_voocphy_chip, FAST_NOTIFY_FULL);
		} else if (chip->gauge_vbatt > chip->vooc_ntime_full_voltage) {
			fast_full_count++;
			if (fast_full_count > 5) {
				voocphy_info("vbatt ntime fastchg full: %d",
					     chip->gauge_vbatt);
				oplus_voocphy_set_status_and_notify_ap(
					g_voocphy_chip, FAST_NOTIFY_FULL);
			}
...
int oplus_voocphy_safe_event_handle(unsigned long data)
{
	...
	if (chip->usb_bad_connect) {
		oplus_voocphy_set_status_and_notify_ap(
			g_voocphy_chip, FAST_NOTIFY_BAD_CONNECTED);
		//reset vooc_phy
		oplus_voocphy_reset_voocphy();
	}
	status = oplus_voocphy_monitor_timer_start(
		g_voocphy_chip, VOOC_THREAD_TIMER_SAFE, VOOC_SAFE_EVENT_TIME);
...
int oplus_voocphy_ibus_check_event_handle(unsigned long data)
{
	...
	if (slave_cp_status == 1) {
		...
		if (chip->adapter_type == ADAPTER_SVOOC) {
			...
			if (ibus_trouble_count >= 3) {
				...
				if (chip->current_max > 30) {
					...
					if (!chip->ap_need_change_current)
						chip->ap_need_change_current =
							oplus_voocphy_set_fastchg_current(
								chip);
				}
			}
		} else if (chip->adapter_type == ADAPTER_VOOC30 ||
		...
		div_cp_ichg = (main_cp_ibus > slave_cp_ibus) ?
				      (main_cp_ibus - slave_cp_ibus) :
...
static bool oplus_voocphy_check_slave_cp_status(void)
{
	...
	if (g_voocphy_chip->slave_ops &&
	    g_voocphy_chip->slave_ops->get_cp_status) {
		for (i = 0; i < 3; i++) {
			oplus_voocphy_slave_get_chg_enable(g_voocphy_chip,
							   &slave_cp_status);
		...
		if (count >= 3) {
			voocphy_err(
				"count >= 3, return false, slave cp is in trouble!\n");
			return false;
		} else {
			voocphy_err(
				"count < 3, return true, slave cp is not in trouble!\n");
			return true;
		}
	} else {
...
static int oplus_voocphy_check_spec_curr(int *target_curr)
{
	...
	return oplus_chg_is_parellel_ibat_over_spec(batt_temp, sub_batt_temp,
						    target_curr);
}
...
bool oplus_voocphy_check_fastchg_real_allow(void)
{
	...
	if (chip->fastchg_to_warm_full == true &&
	    batt_temp >
		    chip->vooc_normal_high_temp - VOOC_WARM_TEMP_RANGE_THD) {
	...
	if (oplus_switching_support_parallel_chg() == PARALLEL_MOS_CTRL) {
		...
		if (!oplus_switching_get_hw_enable()) {
			...
			if (main_batt_soc < chip->vooc_low_soc ||
			    main_batt_soc > chip->vooc_high_soc) {
				chg_debug(
					"mos open and main soc too high, should return false\n");
				return false;
			}
		}
	}
...
bool oplus_voocphy_get_real_fastchg_allow(void)
{
	if (!g_voocphy_chip) {
		...
		g_voocphy_chip->fastchg_real_allow =
			oplus_voocphy_check_fastchg_real_allow();
...
int oplus_voocphy_fastchg_check_process_handle(unsigned long enable)
{
	...
	voocphy_info(
		"fastchg_to_warm[%d %d] fastchg_dummy_start[%d] fastchg_err_commu[%d] fastchg_check_stop[%d] enable[%d] bPlugIn[%d] fastchg_real_allow[%d]\n",
		chip->fastchg_to_warm, oplus_voocphy_get_fastchg_to_warm(),
	...
	if ((chip->fastchg_to_warm || oplus_voocphy_get_fastchg_to_warm() ||
		...
		status = oplus_voocphy_monitor_timer_stop(
			...
			VOOC_FASTCHG_CHECK_TIME);
		voocphy_info(
			"[vooc_monitor] VOOC_THREAD_TIMER_FASTCHG_CHECK stop fastchg check time stop");
	}
...
int oplus_voocphy_commu_process_handle(unsigned long enable)
{
	...
	if (enable && chip->fastchg_commu_stop == false) {
		...
		oplus_voocphy_reset_fastchg_after_usbout();
		status = oplus_voocphy_monitor_timer_stop(
			chip, VOOC_THREAD_TIMER_BTB, VOOC_BTB_EVENT_TIME);
		...
		status = oplus_voocphy_monitor_timer_stop(
			chip, VOOC_THREAD_TIMER_COMMU, VOOC_COMMU_EVENT_TIME);
		chip->fastchg_commu_ing = false;
		if (oplus_voocphy_get_bidirect_cp_support()) {
			oplus_voocphy_set_chg_auto_mode(false);
		}
		voocphy_info("[vooc_monitor] commu time stop [%d]",
...
int oplus_voocphy_curr_event_handle(unsigned long data)
{
	...
	if (chip->voocphy_dual_cp_support) {
		...
		if (chip->cp_ichg > chip->slave_cp_enable_thr &&
			...
			if (chip->adapter_type == ADAPTER_SVOOC &&
				...
				oplus_voocphy_slave_hw_setting(
					chip, SETTING_REASON_SVOOC);
			} else if ((chip->adapter_type == ADAPTER_VOOC20 ||
				    chip->adapter_type == ADAPTER_VOOC30) &&
				   slave_trouble_count == 1) {
				oplus_voocphy_slave_hw_setting(
					chip, SETTING_REASON_VOOC);
			}
			voocphy_err(
				"chip->cp_vbus = %d chip->cp_vbat = %d!!!!!\n",
				chip->cp_vbus, chip->cp_vbat);
			oplus_voocphy_slave_set_chg_enable(chip, true);
			msleep(200);
			if (oplus_voocphy_check_slave_cp_status()) {
				voocphy_err(
					"slave cp is normal, then set current!\n");
			} else {
				slave_trouble_count = slave_trouble_count + 1;
				voocphy_err(
					"slave cp is abnormal, disable it!\n");
				oplus_voocphy_slave_set_chg_enable(chip, false);
		...
		if (chip->cp_ichg < chip->slave_cp_disable_thr_high &&
			...
			if (disable_sub_cp_count == 3) {
				voocphy_err(
					"Ibus < 1.5A 3 times, disable sub cp!\n");
				oplus_voocphy_slave_set_chg_enable(chip, false);
			...
			disable_sub_cp_count = 0;
			voocphy_err(
				"Discontinuity satisfies the condition, count = 0!\n");
		}
	}

	voocphy_info(
		"[oplus_voocphy_curr_event_handle] chg_temp: %d, current: %d, vbatt: %d btb: %d current_pwd[%d %d]",
	...
	if (oplus_chg_check_pd_svooc_adapater()) {
		pd_svooc_status =
			oplus_voocphy_get_pdsvooc_adapter_config(chip);
	...
	if (!chip->btb_temp_over) { //non btb temp over
		if (vbat_temp_cur < -2000) { //BATT OUPUT 2000MA
			...
			if (curr_over_count > 3) {
				...
				oplus_voocphy_set_status_and_notify_ap(
					g_voocphy_chip, FAST_NOTIFY_ABSENT);
			}
		} else {
			curr_over_count = 0;
		}
		...
		if (chip->parallel_charge_support) {
			...
			voocphy_info(
				"main range lowcurr ntime fastchg full %d, %d, %d\n",
				temp_vbatt, temp_current, chg_temp);
		} else {
			temp_current = vbat_temp_cur;
			temp_vbatt = chip->gauge_vbatt;
		}
		if (chg_temp >= chip->low_curr_full_t1 &&
		    chg_temp <= chip->low_curr_full_t2) {
			if (chip->range1_low_curr_full_num != 0 &&
			    chip->range1_low_curr_full) {
				for (i = 0; i < chip->range1_low_curr_full_num;
				     i++) {
					if (temp_current <=
						    chip->range1_low_curr_full[i]
							    .curr &&
					    temp_vbatt >=
						    chip->range1_low_curr_full[i]
							    .vbatt) {
				...
				if (term_curr_over_count > 5) {
					voocphy_info(
						"range1 lowcurr ntime fastchg full %d, %d\n",
		...
		} else if (chg_temp > chip->low_curr_full_t2 &&
			   chg_temp <= chip->low_curr_full_t3) {
			if (chip->range2_low_curr_full_num != 0 &&
			    chip->range2_low_curr_full) {
				for (i = 0; i < chip->range2_low_curr_full_num;
				     i++) {
					if (temp_current <=
						    chip->range2_low_curr_full[i]
							    .curr &&
					    temp_vbatt >=
						    chip->range2_low_curr_full[i]
							    .vbatt) {
				...
				if (term_curr_over_count > 5) {
					voocphy_info(
						"range2 lowcurr ntime fastchg full %d, %d\n",
						temp_vbatt, temp_current);
					oplus_voocphy_set_status_and_notify_ap(
						g_voocphy_chip,
						FAST_NOTIFY_FULL);
		...
		if (chip->parallel_charge_support) {
			...
			low_curr = 0;
			voocphy_info(
				"sub range lowcurr ntime fastchg full %d, %d, %d\n",
				temp_vbatt, temp_current, chg_temp);
			if (chg_temp >= chip->low_curr_full_t1 &&
			    chg_temp <= chip->low_curr_full_t2) {
				if (chip->sub_range1_low_curr_full_num != 0 &&
				    chip->sub_range1_low_curr_full) {
					for (i = 0;
					     i <
					...
					if (sub_term_curr_over_count > 5) {
						voocphy_info(
							"sub_range1 lowcurr ntime fastchg full %d, %d\n",
							temp_vbatt,
							temp_current);
						oplus_voocphy_set_status_and_notify_ap(
							g_voocphy_chip,
				   ...
				   chg_temp <= chip->low_curr_full_t3) {
				if (chip->sub_range2_low_curr_full_num != 0 &&
				    chip->sub_range2_low_curr_full) {
					for (i = 0;
					     i <
					...
					if (sub_term_curr_over_count > 5) {
						voocphy_info(
							"sub_range2 lowcurr ntime fastchg full %d, %d\n",
							temp_vbatt,
							temp_current);
						oplus_voocphy_set_status_and_notify_ap(
							g_voocphy_chip,
		...
		if (oplus_voocphy_get_bidirect_cp_support()) {
			voocphy_info(
				...
				chip->copycat_icheck, chip->current_expect,
				vbat_temp_cur, chip->current_pwd,
				chip->adapter_type);
			if ((chip->current_expect > 23) &&
			    (vbat_temp_cur > chip->current_pwd) &&
			    (chip->adapter_type == ADAPTER_SVOOC)) {
				if (chip->curr_pwd_count) {
					chip->curr_pwd_count--;
					if (chip->copycat_icheck == false &&
					    chip->curr_pwd_count == 5) {
					...
					voocphy_info(
						"FAST_NOTIFY_ADAPTER_COPYCAT for over current");
					oplus_voocphy_set_status_and_notify_ap(
						g_voocphy_chip,
			...
			if ((chip->gauge_vbatt > chip->batt_pwd_vol_thd1) &&
				...
				if (curr_curve_pwd_count == 0) {
					voocphy_info(
						"FAST_NOTIFY_ADAPTER_COPYCAT for over vbatt && ibat[%d %d]\n",
		...
		status = oplus_voocphy_monitor_timer_start(
			chip, VOOC_THREAD_TIMER_CURR, VOOC_CURR_EVENT_TIME);
	}
...
int oplus_voocphy_monitor_stop(struct oplus_voocphy_manager *chip)
{
	...
	oplus_voocphy_monitor_timer_stop(chip, VOOC_THREAD_TIMER_AP,
					 VOOC_AP_EVENT_TIME);
	oplus_voocphy_monitor_timer_stop(chip, VOOC_THREAD_TIMER_CURR,
					 VOOC_CURR_EVENT_TIME);
	oplus_voocphy_monitor_timer_stop(chip, VOOC_THREAD_TIMER_SOC,
					 VOOC_SOC_EVENT_TIME);
	oplus_voocphy_monitor_timer_stop(chip, VOOC_THREAD_TIMER_VOL,
					 VOOC_VOL_EVENT_TIME);
	oplus_voocphy_monitor_timer_stop(chip, VOOC_THREAD_TIMER_TEMP,
					 VOOC_TEMP_EVENT_TIME);
	//oplus_voocphy_monitor_timer_stop(chip, VOOC_THREAD_TIMER_BTB, VOOC_BTB_EVENT_TIME);
	oplus_voocphy_monitor_timer_stop(chip, VOOC_THREAD_TIMER_SAFE,
					 VOOC_SAFE_EVENT_TIME);
	oplus_voocphy_monitor_timer_stop(chip, VOOC_THREAD_TIMER_IBUS_CHECK,
					 VOOC_IBUS_CHECK_TIME);
...
{
	struct oplus_voocphy_manager *chip = container_of(
		work, struct oplus_voocphy_manager, voocphy_service_work.work);
	...
	if (chip->parallel_charge_support) {
		if (chip->adapter_mesg == VOOC_CMD_IS_VUBS_OK) {
			...
			if (oplus_switching_support_parallel_chg() ==
				...
				chip->pre_gauge_vbatt =
					chip->main_vbatt > chip->sub_vbatt ?
						chip->main_vbatt :
						chip->sub_vbatt;
			chip->main_batt_icharging =
				-oplus_gauge_get_batt_current();
			chip->sub_batt_icharging =
				-oplus_gauge_get_sub_batt_current();
			chip->icharging = chip->main_batt_icharging +
					  chip->sub_batt_icharging;
			voocphy_info(
				"batt[%d, %d], main_batt[%d, %d], sub_batt[%d, %d]\n",
				chip->pre_gauge_vbatt, chip->icharging,
		...
		if ((chip->external_gauge_support) &&
		    (chip->adapter_mesg == VOOC_CMD_IS_VUBS_OK)) {
			chip->gauge_vbatt = chip->pre_gauge_vbatt =
				oplus_gauge_get_batt_mvolts();
		}
					  ...
					  int request)
{
	...
	if (oplus_voocphy_get_bidirect_cp_support()) {
		memcpy(chip->int_column_pre, chip->int_column,
		       sizeof(chip->int_column));
		voocphy_dbg(
			"request %d 09~0E[0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x]\n",
			request, chip->int_column_pre[0],
			chip->int_column_pre[1], chip->int_column_pre[2],
			chip->int_column_pre[3], chip->int_column_pre[4],
			chip->int_column_pre[5]);
	}
	return true;
}
...
{
	voocphy_info("%s %s\n", __func__,
		     flag == CPU_CHG_FREQ_STAT_UP ? "request" : "release");
...
void oplus_voocphy_modify_cpufeq_work(struct work_struct *work)
{
	...
	if (atomic_read(&g_voocphy_chip->voocphy_freq_state) == 1)
		ppm_sys_boost_min_cpu_freq_set(
			g_voocphy_chip->voocphy_freq_mincore,
			g_voocphy_chip->voocphy_freq_midcore,
			g_voocphy_chip->voocphy_freq_maxcore,
	...
	freq[BIG_CLUSTER] = g_voocphy_chip->voocphy_freq_maxcore;
	if (atomic_read(&g_voocphy_chip->voocphy_freq_state) == 1) {
		charger_boost_min_cpu_freq_set(
			freq, g_voocphy_chip->voocphy_current_change_timeout);
	} else {
		charger_boost_min_cpu_freq_clear();
	}
#endif
...
void oplus_voocphy_wake_check_chg_out_work(unsigned int delay_ms)
{
	...
	schedule_delayed_work(
		&g_voocphy_chip->check_chg_out_work,
...
void oplus_voocphy_init(struct oplus_voocphy_manager *chip)
{
	...
	INIT_DELAYED_WORK(&(chip->notify_fastchg_work),
			  oplus_voocphy_handle_voocphy_status);
	INIT_DELAYED_WORK(&(chip->monitor_work),
			  oplus_voocphy_monitor_process_events);
	INIT_DELAYED_WORK(&(chip->monitor_start_work),
			  oplus_voocphy_monitor_start_work);
	INIT_DELAYED_WORK(&(chip->modify_cpufeq_work),
			  oplus_voocphy_modify_cpufeq_work);
	INIT_DELAYED_WORK(&(chip->check_chg_out_work),
			  oplus_voocphy_check_chg_out_work);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	INIT_DELAYED_WORK(&(chip->clear_boost_work),
			  oplus_voocphy_clear_boost_work);
#endif
...
static int
oplus_voocphy_parse_svooc_batt_curves(struct oplus_voocphy_manager *chip)
{
		...
		for (j = 0; j < BATT_SYS_CURVE_MAX; j++) {
			...
			case BATT_SOC_0_TO_50:
				rc = of_property_read_u32_array(
					...
					(u32 *)svooc_curves_soc0_2_50[j]
						.batt_sys_curve,
					length);
				svooc_curves_soc0_2_50[j].sys_curv_num =
					length / 5;
				break;
			case BATT_SOC_50_TO_75:
				rc = of_property_read_u32_array(
					soc_node, strategy_temp[j],
					(u32 *)svooc_curves_soc50_2_75[j]
						.batt_sys_curve,
					length);
				svooc_curves_soc50_2_75[j].sys_curv_num =
					length / 5;
				break;
			case BATT_SOC_75_TO_85:
				rc = of_property_read_u32_array(
					soc_node, strategy_temp[j],
					(u32 *)svooc_curves_soc75_2_85[j]
						.batt_sys_curve,
					length);
				svooc_curves_soc75_2_85[j].sys_curv_num =
					length / 5;
				break;
			case BATT_SOC_85_TO_90:
				rc = of_property_read_u32_array(
					soc_node, strategy_temp[j],
					(u32 *)svooc_curves_soc85_2_90[j]
						.batt_sys_curve,
					length);
				svooc_curves_soc85_2_90[j].sys_curv_num =
					length / 5;
				break;
			default:
				break;
...
static int
oplus_voocphy_parse_vooc_batt_curves(struct oplus_voocphy_manager *chip)
{
	...
	for (i = 0; i < BATT_SOC_90_TO_100; i++) {
		...
		for (j = 0; j < BATT_SYS_CURVE_MAX; j++) {
			...
			case BATT_SOC_0_TO_50:
				rc = of_property_read_u32_array(
					soc_node, strategy_temp[j],
					(u32 *)vooc_curves_soc0_2_50[j]
						.batt_sys_curve,
					length);
				vooc_curves_soc0_2_50[j].sys_curv_num =
					length / 5;
				break;
			case BATT_SOC_50_TO_75:
				rc = of_property_read_u32_array(
					soc_node, strategy_temp[j],
					(u32 *)vooc_curves_soc50_2_75[j]
						.batt_sys_curve,
					length);
				vooc_curves_soc50_2_75[j].sys_curv_num =
					length / 5;
				break;
			case BATT_SOC_75_TO_85:
				rc = of_property_read_u32_array(
					soc_node, strategy_temp[j],
					(u32 *)vooc_curves_soc75_2_85[j]
						.batt_sys_curve,
					length);
				vooc_curves_soc75_2_85[j].sys_curv_num =
					length / 5;
				break;
			case BATT_SOC_85_TO_90:
				rc = of_property_read_u32_array(
					soc_node, strategy_temp[j],
					(u32 *)vooc_curves_soc85_2_90[j]
						.batt_sys_curve,
					length);
				vooc_curves_soc85_2_90[j].sys_curv_num =
					length / 5;
				break;
			default:
				break;
...
int oplus_voocphy_parse_batt_curves(struct oplus_voocphy_manager *chip)
{
	...
	chip->parallel_charge_support =
		of_property_read_bool(node, "parallel_charge_support");
	...
	if (chip->parallel_charge_support) {
		for (i = 0; i < 7; i++) {
			...
			printk("%s\n", buf);
			rc = of_property_read_u32_array(
				node, buf, svooc_parallel_curve[i].curve_val,
				2);
			if (rc) {
	...
	chip->voocphy_dual_cp_support =
		of_property_read_bool(node, "qcom,voocphy_dual_cp_support");
	voocphy_info("voocphy_dual_cp_support = %d\n",
		     chip->voocphy_dual_cp_support);

	chip->voocphy_bidirect_cp_support =
		of_property_read_bool(node, "qcom,voocphy_bidirect_cp_support");
	voocphy_info("voocphy_bidirect_cp_support = %d\n",
		     chip->voocphy_bidirect_cp_support);

	chip->version_judge_support =
		of_property_read_bool(node, "qcom,version_judge_support");
	voocphy_info("version_judge_support = %d\n",
		     chip->version_judge_support);

	chip->impedance_calculation_newmethod = of_property_read_bool(
		node, "qcom,impedance_calculation_newmethod");
	...
	rc = of_property_read_u32(node, "qcom,vooc-warm-allow-vol",
	...
	rc = of_property_read_u32(node, "qcom,vooc-warm-allow-soc",
	...
	rc = of_property_read_u32(node,
				  "qcom,vooc_strategy1_high_current0_vooc",
				  &chip->vooc_strategy1_high_current0_vooc);
	if (rc) {
		chip->vooc_strategy1_high_current0_vooc =
			chip->vooc_strategy1_high_current0;
	...
	rc = of_property_read_u32(node,
				  "qcom,vooc_strategy1_high_current1_vooc",
				  &chip->vooc_strategy1_high_current1_vooc);
	if (rc) {
		chip->vooc_strategy1_high_current1_vooc =
			chip->vooc_strategy1_high_current1;
	...
	rc = of_property_read_u32(node,
				  "qcom,vooc_strategy1_high_current2_vooc",
				  &chip->vooc_strategy1_high_current2_vooc);
	if (rc) {
		chip->vooc_strategy1_high_current2_vooc =
			chip->vooc_strategy1_high_current2;
	...
	rc = of_property_read_u32(node,
				  "qcom,vooc_strategy2_batt_up_down_temp2",
	...
	rc = of_property_read_u32(node,
				  "qcom,vooc_strategy2_batt_up_down_temp4",
	...
	rc = of_property_read_u32(node, "qcom,vooc-low-soc",
				  &chip->vooc_low_soc);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,vooc-high-soc",
				  &chip->vooc_high_soc);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,vooc-high-temp",
				  &chip->vooc_high_temp);
	if (rc) {
	...
	rc = of_property_read_u32_array(node, "svooc_batt_sys_curve", data,
					length);
	if (rc < 0) {
	...
	rc = of_property_read_u32_array(node, "vooc_batt_sys_curve", data,
					length);
	if (rc < 0) {
	...
	chip->external_gauge_support =
		of_property_read_bool(node, "qcom,external_gauge_support");
	voocphy_info("external_gauge_support = %d\n",
		     chip->external_gauge_support);

	rc = of_property_count_elems_of_size(
		node, "qcom,svooc_cool_down_current_limit", sizeof(u32));
	if (rc < 0) {
		voocphy_info(
			"Count svooc_cool_down_current_limit failed, rc=%d\n",
			rc);
	...
	voocphy_info(
		"parse svooc_cool_down_current_limit, size=%d svooc_cool_down_num =%d\n",
		length, chip->svooc_cool_down_num);

	rc = of_property_read_u32_array(node,
					"qcom,svooc_cool_down_current_limit",
					chip->svooc_cool_down_current_limit,
					length);
	if (rc < 0) {
		voocphy_info(
			"parse svooc_cool_down_current_limit failed, rc=%d\n",
			rc);
		return rc;
	}

	rc = of_property_count_elems_of_size(
		node, "qcom,vooc_cool_down_current_limit", sizeof(u32));
	if (rc < 0) {
		voocphy_info(
			"Count vooc_cool_down_current_limit failed, rc=%d\n",
			rc);
	...
	voocphy_info(
		"parse vooc_cool_down_current_limit, size=%d vooc_cool_down_num =%d\n",
		length, chip->vooc_cool_down_num);

	rc = of_property_read_u32_array(node,
					"qcom,vooc_cool_down_current_limit",
					chip->vooc_cool_down_current_limit,
					length);
	if (rc < 0) {
		voocphy_info(
			"parse vooc_cool_down_current_limit failed, rc=%d\n",
			rc);
		return rc;
	}
	...
	if (rc < 0) {
		voocphy_info(
			"parse vooc_little_cold_full_voltage failed, rc=%d\n",
			rc);
		chip->vooc_little_cold_full_voltage =
			TEMP0_2_TEMP45_FULL_VOLTAGE;
	}
	...
	rc = of_property_count_elems_of_size(node, "qcom,range1_low_curr_full",
					     sizeof(u32));
	if (rc < 0) {
	...
	if (length != 0 && length % 2 == 0) {
		chip->range1_low_curr_full = devm_kzalloc(
			...
			chip->range1_low_curr_full_num *
				sizeof(struct low_curr_full_condition),
			GFP_KERNEL);

		if (chip->range1_low_curr_full) {
			rc = of_property_read_u32_array(
				node, "qcom,range1_low_curr_full",
				(u32 *)chip->range1_low_curr_full, length);
			if (rc < 0) {
				voocphy_info(
					"parse range1_low_curr_full failed, rc=%d\n",
					rc);
				devm_kfree(chip->dev,
					   chip->range1_low_curr_full);
	...
	rc = of_property_count_elems_of_size(node, "qcom,range2_low_curr_full",
					     sizeof(u32));
	if (rc < 0) {
	...
	if (length != 0 && length % 2 == 0) {
		...
		if (chip->range2_low_curr_full) {
			rc = of_property_read_u32_array(
				node, "qcom,range2_low_curr_full",
				(u32 *)chip->range2_low_curr_full, length);
			if (rc < 0) {
				voocphy_info(
					"parse range2_low_curr_full failed, rc=%d\n",
					rc);
				devm_kfree(chip->dev,
					   chip->range2_low_curr_full);
	...
	if (chip->parallel_charge_support) {
		rc = of_property_count_elems_of_size(
			node, "qcom,sub_range1_low_curr_full", sizeof(u32));
		if (rc < 0) {
			voocphy_info(
				"Count sub_range1_low_curr_full failed, rc=%d\n",
				rc);
			rc = 0;
		}
		length = rc;
		...
		if (length != 0 && length % 2 == 0) {
			...
			if (chip->sub_range1_low_curr_full) {
				rc = of_property_read_u32_array(
					node, "qcom,sub_range1_low_curr_full",
					(u32 *)chip->sub_range1_low_curr_full,
					length);
				if (rc < 0) {
					voocphy_info(
						"parse sub_range1_low_curr_full failed, rc=%d\n",
						rc);
					devm_kfree(
						chip->dev,
						chip->sub_range1_low_curr_full);
					...
					voocphy_info(
						"sub_range1_low_curr_full length =%d\n",
						chip->sub_range1_low_curr_full_num);
					for (i = 0;
					     i <
					     chip->sub_range1_low_curr_full_num;
		...
		rc = of_property_count_elems_of_size(
			node, "qcom,sub_range2_low_curr_full", sizeof(u32));
		if (rc < 0) {
			voocphy_info(
				"Count sub_range2_low_curr_full failed, rc=%d\n",
				rc);
			rc = 0;
		}
		length = rc;
		...
		if (length != 0 && length % 2 == 0) {
			...
			if (chip->sub_range2_low_curr_full) {
				rc = of_property_read_u32_array(
					node, "qcom,sub_range2_low_curr_full",
					(u32 *)chip->sub_range2_low_curr_full,
					length);
				if (rc < 0) {
					voocphy_info(
						"parse sub_range2_low_curr_full failed, rc=%d\n",
						rc);
					devm_kfree(
						chip->dev,
						chip->sub_range2_low_curr_full);
					...
					voocphy_info(
						"sub_range2_low_curr_full length =%d\n",
						chip->sub_range2_low_curr_full_num);
					for (i = 0;
					     i <
					     chip->sub_range2_low_curr_full_num;
	...
	if (rc) {
		chip->voocphy_svooc_cp_max_ibus =
			VOOCPHY_SVOOC_CP_MAX_IBUS_DEFAULT;
	}
	...
	if (rc) {
		chip->voocphy_vooc_cp_max_ibus =
			VOOCPHY_VOOC_CP_MAX_IBUS_DEFAULT;
	}
	...
	rc = of_property_read_u32(node, "qcom,batt_pwd_curr_thd1",
				  &chip->batt_pwd_curr_thd1);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,batt_pwd_vol_thd1",
				  &chip->batt_pwd_vol_thd1);
	if (rc) {
...
void oplus_voocphy_set_pdqc_config(void)
{
	...
	if (g_voocphy_chip->voocphy_dual_cp_support)
		oplus_voocphy_slave_hw_setting(g_voocphy_chip,
					       SETTING_REASON_PDQC);
}

void oplus_voocphy_set_pdsvooc_adapter_config(
...
void oplus_voocphy_adapter_plugout_handler(void)
{
	...
	if (oplus_vooc_get_fastchg_started() == true &&
	    ...
	    oplus_voocphy_user_exit_fastchg(g_voocphy_chip) == false &&
		...
		printk(KERN_ERR
		       "%s:plug out fastchg_to_normal or fastchg_to_warm or 5v2a\n",
		       __func__);
...
bool oplus_voocphy_version_judge(void)
{
	...
	if ((get_PCB_Version() >= PCB_VERSION_PVT_MOZI) &&
	    (get_PCB_Version() <= PCB_VERSION_MP3_MOZI)) {
		return true;
	} else {
		return false;
	}
...
int oplus_voocphy_svooc_commu_with_voocphy(struct oplus_voocphy_manager *chip)
{
	...
	if (oplus_voocphy_ap_allow_fastchg(chip)) {
		if (chip->fastchg_stage == OPLUS_FASTCHG_STAGE_2 &&
		    oplus_chg_get_flash_led_status()) {
			chip->fastchg_need_reset = 1;
			voocphy_info(
				"OPLUS_FASTCHG_STAGE_2 and open torch exit fastchg\n");
			return 1;
		} else {
			/*oplus_chg_disable_charge();*/
			oplus_chg_suspend_charger();
			...
			if (oplus_voocphy_get_bidirect_cp_support() &&
			    oplus_voocphy_version_judge()) {
				oplus_voocphy_set_chg_auto_mode(true);
			}
			voocphy_info(
				"allow fastchg adapter type %d,get_PCB_Version() = %d\n",
		...
		if (chip->adapter_type == ADAPTER_SVOOC) {
			...
			if (oplus_chg_check_pd_svooc_adapater()) {
				...
				oplus_voocphy_set_pdsvooc_adapter_config(chip,
									 true);
			}
		} else if (chip->adapter_type == ADAPTER_VOOC20 ||
			   chip->adapter_type == ADAPTER_VOOC30) {
...
int oplus_voocphy_set_chg_auto_mode(bool enable)
{
	...
	if (g_voocphy_chip->ops && g_voocphy_chip->ops->set_chg_auto_mode) {
		rc = g_voocphy_chip->ops->set_chg_auto_mode(g_voocphy_chip,
							    enable);
		if (rc < 0) {
			voocphy_info(
				"oplus_voocphy_set_chg_auto_mode fail, rc=%d.\n",
				rc);
			return rc;
		}
	}
...
int oplus_voocphy_set_bidirection_predata(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->adapter_type == ADAPTER_SVOOC) {
		...
		oplus_voocphy_set_predata(chip,
					  OPLUS_BIDIRECTION_PREDATA_VOOC30);
	} else if (chip->adapter_type == ADAPTER_VOOC20) {
		oplus_voocphy_set_predata(chip,
					  OPLUS_BIDIRECTION_PREDATA_VOOC20);
	} else {
		voocphy_info("adapter_type error !!!\n");
		return -1;
	}
...
int oplus_vooc_adapter_work_as_power_bank(struct oplus_voocphy_manager *chip)
{
	...
	if (vooc_head != VOOC2_HEAD && vooc_move_head != VOOC2_HEAD) {
		...
		if (vooc_move_head == VOOC2_HEAD) {
			voocphy_info(
				"!!!!![power_bank] vooc20 frame move head");
			chip->vooc_move_head = true;
		}

		if (chip->vooc_move_head == false) {
			status = oplus_voocphy_read_mesg_mask(
				VOOC_MESG_READ_MASK, chip->voocphy_rx_buff,
				&chip->adapter_mesg);
		} else {
			status = oplus_voocphy_read_mesg_mask(
				VOOC_MESG_MOVE_READ_MASK, chip->voocphy_rx_buff,
				&chip->adapter_mesg);
		}
		voocphy_info("!!!!![power_bank] adapter_mesg=0x%0x",
			     chip->adapter_mesg);
		...
		status |=
			oplus_voocphy_write_mesg_mask(TX1_DET_BIT0_TO_BIT1_MASK,
						      ...
						      0);
		status |=
			oplus_voocphy_write_mesg_mask(VOOC_INVERT_HEAD_MASK,
						      &chip->voocphy_tx_buff[0],
						      chip->vooc_head);
	...
	if (chip->adapter_type == ADAPTER_VOOC20 ||
		...
		oplus_voocphy_hw_setting(chip, SETTING_REASON_VOOC);
		if (chip->voocphy_dual_cp_support)
			oplus_voocphy_slave_hw_setting(chip,
						       SETTING_REASON_VOOC);
	} else {
		voocphy_info("SETTING_REASON_DEFAULT\n");
	}
