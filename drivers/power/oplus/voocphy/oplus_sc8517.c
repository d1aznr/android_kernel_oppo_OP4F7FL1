...
static int sc8517_update_bits(struct i2c_client *client, u8 reg, u8 mask,
			      u8 data)
{
	int ret;
	u8 tmp;
...
static int sc8517_read_i2c_block(struct i2c_client *client, u8 reg, u8 length,
				 u8 *returnData)
{
	int rc = 0;
	int retry = 3;
	...
	if (rc < 0) {
		while (retry > 0) {
			...
			rc = i2c_smbus_read_i2c_block_data(client, reg, length,
							   returnData);
			if (rc < 0) {
				retry--;
			} else {
...
static int sc8517_set_chg_enable(struct oplus_voocphy_manager *chip,
				 bool enable)
{
	int ret = 0;
	if (!chip) {
	...
	sc8517_write_byte(chip->client, SC8517_REG_02,
			  0x7a); //vac range disable

	if (enable) {
		ret = sc8517_write_byte(chip->client, SC8517_REG_02,
					0x7b); //enable mos
	} else {
		ret = sc8517_write_byte(chip->client, SC8517_REG_02,
					0x78); //disable mos
...
static int sc8517_set_adc_enable(struct oplus_voocphy_manager *chip,
				 bool enable)
{
	return 0;
}
...
static int sc8517_get_voocphy_enable(struct oplus_voocphy_manager *chip,
				     u8 *data)
{
	int ret = 0;
...
static int sc8517_set_chg_auto_mode(struct oplus_voocphy_manager *chip,
				    bool enable)
{
	int ret = 0;
	if (!chip) {
	...
	if (enable && (sc8517_get_chg_auto_mode(chip) == SC8517_CHG_FIX_MODE)) {
		ret = sc8517_update_bits(chip->client, SC8517_REG_15,
		   ...
		   (sc8517_get_chg_auto_mode(chip) == SC8517_CHG_AUTO_MODE)) {
		ret = sc8517_update_bits(chip->client, SC8517_REG_15,
					 SC8517_CHG_MODE_MASK,
					 SC8517_CHG_FIX_MODE);
	} else {
...
static void sc8517_set_pd_svooc_config(struct oplus_voocphy_manager *chip,
				       bool enable)
{
	if (!chip) {
		chg_err("Failed\n");
...
{
	sc8517_write_byte(chip->client, SC8517_REG_24,
			  0x81); //enable voocphy and handshake
}
...
static int sc8517_reactive_voocphy(struct oplus_voocphy_manager *chip)
{
	...
	//vooc
	sc8517_write_byte(chip->client, SC8517_REG_29,
			  0x05); //diable vooc int flag mask
...
static int sc8517_init_device(struct oplus_voocphy_manager *chip)
{
	...
	msleep(10);
	sc8517_write_byte(chip->client, SC8517_REG_24,
			  0x00); //VOOC_CTRL:disable,no handshake
	chg_err("----\n");
...
static int sc8517_init_vooc(struct oplus_voocphy_manager *chip)
{
	...
	msleep(1);
	sc8517_write_byte(chip->client, SC8517_REG_20,
			  0x21); //VOOC_CTRL:disable,no handshake
	sc8517_write_byte(chip->client, SC8517_REG_21,
			  0x80); //VOOC_CTRL:disable,no handshake
	sc8517_write_byte(chip->client, SC8517_REG_2C,
			  0xD1); //VOOC_CTRL:disable,no handshake
	sc8517_write_byte(chip->client, SC8517_REG_29,
			  0x25); //mask sedseq flag,rx start,tx done
...
static int sc8517_hardware_init(struct oplus_voocphy_manager *chip)
{
	...
	sc8517_write_byte(chip->client, SC8517_REG_06,
			  0x85); //enable  audio mode
	sc8517_write_byte(chip->client, SC8517_REG_08, 0xA6); //REF_SKIP_R 40mv
	sc8517_write_byte(
		chip->client, SC8517_REG_29,
		0x05); //Masked Pulse_filtered, RX_Start,Tx_Done,soft intflag
	sc8517_write_byte(chip->client, SC8517_REG_10,
			  0x79); //Masked Pulse_filtered, RX_Start,Tx_Done
	return 0;
}
...
int sc8517_dump_registers(struct oplus_voocphy_manager *chip)
{
	...
	chg_err("[00~06][0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x]\n", int_column[0],
		int_column[1], int_column[2], int_column[3], int_column[4],
		int_column[5], int_column[6]);
				     ...
				     const char *buf, size_t count)
{
	...
	if (ret == 2 && reg <= 0x3C)
		sc8517_write_byte(chip->client, (unsigned char)reg,
				  (unsigned char)val);

	return count;
}

static DEVICE_ATTR(registers, 0660, sc8517_show_registers,
		   sc8517_store_register);
...
static void sc8517_charger_shutdown(struct i2c_client *client)
{
	...
	sc8517_write_byte(
		client, SC8517_REG_29,
		0x05); //Masked Pulse_filtered, RX_Start,Tx_Done,bit soft intflag
