...
static int sy6529_update_bits(struct i2c_client *client, u8 reg, u8 mask,
			      u8 data)
{
	int ret;
	u8 tmp;
...
static s32 sy6529_get_adapter_request_info(struct oplus_voocphy_manager *chip)
{
	...
	data = sy6529_read_word(
		chip->client,
		sy6529_REG_2E); /*	VOOC RDATA	& VOOC FLAG*/
...
static void sy6529_update_chg_data(struct oplus_voocphy_manager *chip)
{
	...
	*/
	...
	data_tmp =
		data & (sy6529_VBUS_OVP_FLAG_MASK | sy6529_IBUS_OCP_FLAG_MASK);
	data_tmp = data_tmp << 1;
	...
	*/
	...
	data = data &
	       (sy6529_VBUS_INSERT_FLAG_MASK | sy6529_VBAT_INSERT_FLAG_MASK);
	...
	/* Reg12-Reg13:VBUS_ADC
	   ...
	   Reg1A:TDIE_ADC
	 */
	i2c_smbus_read_i2c_block_data(chip->client, sy6529_REG_12,
				      UPDATE_DATA_BLOCK_LENGTH, data_block);
	for (i = 0; i < UPDATE_DATA_BLOCK_LENGTH; i++) {
		pr_info("data_block[%d] = %u\n", i, data_block[i]);
	}

	chip->cp_ichg =
		((data_block[2] << CHAR_BITS) | data_block[3]) *
		IBUS_VBAT_LSB; /* SY6529's ibus adc:reg14,15 LSB=0.15625mA */
	chip->cp_vbus = ((data_block[0] << CHAR_BITS) | data_block[1]) *
			VBUS_LSB; /* SY6529's Vbus adc:reg12,13 LSB=0.5mV */
	chip->cp_vbat =
		((data_block[4] << CHAR_BITS) | data_block[5]) *
...
static int sy6529_set_chg_enable(struct oplus_voocphy_manager *chip,
				 bool enable)
{
	int ret;
	u8 value;
	...
	if (enable) {
		if (chip->adapter_type == ADAPTER_VOOC20 ||
		    chip->adapter_type == ADAPTER_VOOC30) { /*mode:1:1*/
			tmp = tmp | (sy6529_CHARGE_MODE_1_1
				     << sy6529_CHARGE_MODE_SHIFT);
		} else { /*mode:2:1*/
			tmp = tmp | (sy6529_CHARGE_MODE_2_1
				     << sy6529_CHARGE_MODE_SHIFT);
		}
...
static int sy6529_get_cp_vbat(struct oplus_voocphy_manager *chip)
{
	...
	i2c_smbus_read_i2c_block_data(chip->client, sy6529_REG_16, 2,
				      data_block);

	chip->cp_vbat =
		((data_block[0] << CHAR_BITS) | data_block[1]) *
		IBUS_VBAT_LSB; /* SY6529's VBAT adc:reg16,17 LSB=0.15625mV*/
	pr_info("cp_vbat = %d\n", chip->cp_vbat);
	return chip->cp_vbat;
}

static int sy6529_direct_chg_enable(struct oplus_voocphy_manager *chip,
				    u8 *data)
{
	int ret = 0;
	...
	*data = *data &
		sy6529_CHARGE_MODE_MASK; /*SY6529 reg0x00:Bit[6:4]=001, 1_1 mode;  Bit[6:4]=010,2_1 mode*/
	*data = *data >> sy6529_CHARGE_MODE_SHIFT;

	if (*data == sy6529_CHARGE_MODE_2_1 |
	    *data ==
		    sy6529_CHARGE_MODE_1_1) /* SY6529 is working in 2_1 mode or 1_1 mode, return 1*/
		*data = 1;
	else
...
static int sy6529_get_cp_ichg(struct oplus_voocphy_manager *chip)
{
	...
	if (cp_enable == 0)
		return 0;
	/*parse data_block for improving time of interrupt*/
	i2c_smbus_read_i2c_block_data(chip->client, sy6529_REG_14, 2,
				      data_block);
	chip->cp_ichg =
		((data_block[0] << CHAR_BITS) | data_block[1]) *
		IBUS_VBAT_LSB; /* SY6529's ibus adc:reg14,15 LSB=0.15625mA*/
...
static int sy6529_set_adc_enable(struct oplus_voocphy_manager *chip,
				 bool enable)
{
	if (!chip) {
		pr_err("Failed\n");
...
static void sy6529_set_pd_svooc_config(struct oplus_voocphy_manager *chip,
				       bool enable)
{
	int ret = 0;
	u8 reg07_data = 0;
	...
	if (enable) {
		sy6529_write_byte(chip->client, sy6529_REG_07,
				  0x3F); /*SY6529: IBUS_UCP:disable*/
		sy6529_write_byte(chip->client, sy6529_REG_4D,
				  0x80); /*SY6529: bus_ucp 40ms*/
		sy6529_write_byte(chip->client, sy6529_REG_0C,
				  0x02); /*IBUS_UCP_RISE_MASK*/
		sy6529_write_byte(chip->client, sy6529_REG_00,
				  0x02); /*SY6529: WD:1000ms*/
	} else {
		sy6529_write_byte(chip->client, sy6529_REG_07,
				  0xBF); /*SY6529: IBUS_OCP:3.6A*/
	}
...
static int oplus_vooc_reset_voocphy(struct oplus_voocphy_manager *chip)
{
	...
	/*turn off mos*/
	sy6529_write_byte(
		chip->client, sy6529_REG_00,
		0x8B); /* SY6529: Charge disable, RST=1, watchdog disable */
	sy6529_write_byte(chip->client, sy6529_REG_00,
			  0x0B); /* SY6529: Charge disable, watchdog disable */

	/*hwic config with plugout*/
	sy6529_write_byte(chip->client, sy6529_REG_08,
			  0xAE); /* SY6529: set vbat_ovp=4.65V */
	sy6529_write_byte(chip->client, sy6529_REG_04,
			  0x13); /* SY6529: set vac_ovp=7V */
	sy6529_write_byte(chip->client, sy6529_REG_06,
			  0x94); /* SY6529: set VBUS_ovp=6V */
	sy6529_write_byte(
		chip->client, sy6529_REG_07,
		0xBF); /* SY6529: set UCP_TH:150/300mA  BUS OCP=3.6A(Max) */
	sy6529_write_byte(chip->client, sy6529_REG_02,
			  0xB0); /* SY6529: VBUS_LOW_ERR:1.01 */
	sy6529_write_byte(chip->client, sy6529_REG_4D,
			  0x80); /* SY6529: bus_ucp 40ms */
	sy6529_write_byte(chip->client, sy6529_REG_11,
			  0x00); /* SY6529: reset adc */
	sy6529_write_byte(chip->client, sy6529_REG_10,
			  0x80); /* SY6529: disable VBUS INSERT */
...
static int oplus_vooc_reactive_voocphy(struct oplus_voocphy_manager *chip)
{
	...
	/*vooc*/
	sy6529_write_byte(chip->client, sy6529_REG_30,
			  0x05); /* MASK irq RX_START; TX_DONE*/
...
{
	sy6529_write_byte(chip->client, sy6529_REG_00,
			  0x08); /* SY6529: disable WD */
	sy6529_write_byte(chip->client, sy6529_REG_11,
			  0x0); /* SY6529: ADC_CTRL:disable */
	sy6529_write_byte(chip->client, sy6529_REG_04,
			  0x13); /* SY6529: vac_ovp =7V */
	sy6529_write_byte(chip->client, sy6529_REG_06,
			  0x94); /* SY6529: VBUS_OVP:6V */
	sy6529_write_byte(chip->client, sy6529_REG_08,
			  0xAE); /* SY6529: VBAT_OVP:4.65V */
	sy6529_write_byte(chip->client, sy6529_REG_07,
			  0xBF); /* SY6529: IBUS_OCP:3.6A */
	sy6529_write_byte(chip->client, sy6529_REG_02,
			  0xB0); /* SY6529: VBUS_LOW_ERR:1.01 */
	...
	sy6529_write_byte(chip->client, sy6529_REG_09,
			  0x34); /* SY6529: IBAT OCP DIS */
	sy6529_write_byte(chip->client, sy6529_REG_2B,
			  0x00); /* SY6529: VOOC_CTRL:disable */
	sy6529_write_byte(chip->client, sy6529_REG_05,
			  0x35); /* SY6529: Vdrop = 300mV */
	sy6529_write_byte(chip->client, sy6529_REG_0A,
			  0x00); /* SY6529: disable REGULATION */
	sy6529_write_byte(chip->client, sy6529_REG_10,
			  0x80); /* SY6529: disable VBUS INSERT */
...
{
	sy6529_write_byte(chip->client, sy6529_REG_04,
			  0x18); /*SY6529: VAC_OVP:12v*/
	sy6529_write_byte(chip->client, sy6529_REG_06,
			  0xBC); /*SY6529: VBUS_OVP:10V*/
	sy6529_write_byte(chip->client, sy6529_REG_07,
			  0xBF); /*SY6529: IBUS_OCP:3.6A*/
	sy6529_write_byte(chip->client, sy6529_REG_02,
			  0xB0); /*SY6529: VBUS_LOW_ERR:1.01*/
	...
	sy6529_write_byte(chip->client, sy6529_REG_00,
			  0x02); /*SY6529: WD:1000ms*/
	sy6529_write_byte(chip->client, sy6529_REG_11,
			  0x80); /*SY6529: ADC_CTRL:ADC_EN*/

	sy6529_write_byte(chip->client, sy6529_REG_33,
			  0xD1); /*SY6529: Loose_det=1*/
...
static int sy6529_vooc_hw_setting(struct oplus_voocphy_manager *chip)
{
	...
	sy6529_write_byte(chip->client, sy6529_REG_06,
			  0x94); /*SY6529: VBUS_OVP:6V*/
	sy6529_write_byte(chip->client, sy6529_REG_07,
			  0xBF); /*SY6529: IBUS_OCP:3.6A(max)*/
	sy6529_write_byte(chip->client, sy6529_REG_02,
			  0xB0); /*SY6529: VBUS_LOW_ERR:1.01*/

	sy6529_write_byte(chip->client, sy6529_REG_00,
			  0x04); /*SY6529: WD:5000ms*/
	sy6529_write_byte(chip->client, sy6529_REG_11,
			  0x80); /*SY6529: ADC_CTRL:*/

	sy6529_write_byte(chip->client, sy6529_REG_33,
			  0xD1); /*SY6529: Loose_det*/
...
{
	sy6529_write_byte(chip->client, sy6529_REG_04,
			  0x13); /*SY6529: VAC_OVP:12V*/
	sy6529_write_byte(chip->client, sy6529_REG_06,
			  0x94); /*SY6529: VBUS_OVP:11.5V*/

	sy6529_write_byte(chip->client, sy6529_REG_01,
			  0x60); /*SY6529: CP Frequency 375kHz*/
	sy6529_write_byte(chip->client, sy6529_REG_00,
			  0x0B); /*SY6529: WD: disable*/

	sy6529_write_byte(chip->client, sy6529_REG_11,
			  0x00); /*SY6529: ADC_CTRL:*/
	sy6529_write_byte(chip->client, sy6529_REG_2B,
			  0x00); /*SY6529: VOOC_CTRL*/
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	for (addr = sy6529_REG_00; addr <= sy6529_REG_4D; addr++) {
		if ((addr <= sy6529_REG_1A) ||
		    (addr >= sy6529_REG_2B &&
		     addr <= sy6529_REG_33) /* show SY6529 all registers*/
		    || addr == sy6529_REG_36 || addr == sy6529_REG_3A ||
		    addr == sy6529_REG_4D) {
				     ...
				     const char *buf, size_t count)
{
	...
	if (ret == 2 && reg <= sy6529_REG_4D)
		sy6529_write_byte(chip->client, (unsigned char)reg,
				  (unsigned char)val);

	return count;
}

static DEVICE_ATTR(registers, 0660, sy6529_show_registers,
		   sy6529_store_register); /* SY6529 device ID is 0x1F*/
