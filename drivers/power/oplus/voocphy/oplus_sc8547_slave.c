...
static int sc8547_slave_get_chg_enable(struct oplus_voocphy_manager *chip,
				       u8 *data);
...
static void sc8547_slave_i2c_error(bool happen)
{
	...
	if (happen) {
		...
		if (oplus_voocphy_mg->slave_voocphy_iic_err_num >=
		    I2C_ERR_NUM) {
...
static int sc8547_slave_update_bits(struct i2c_client *client, u8 reg, u8 mask,
				    u8 data)
{
	int ret;
	u8 tmp;
...
static void sc8547_slave_update_data(struct oplus_voocphy_manager *chip)
{
	...
	ret = i2c_smbus_read_i2c_block_data(chip->slave_client, SC8547_REG_13,
					    2, data_block);
	if (ret < 0) {
		sc8547_slave_i2c_error(true);
	...
	chip->slave_cp_ichg =
		((data_block[0] << 8) | data_block[1]) * 1875 / 1000;
	pr_info("slave cp_ichg = %d int_flag = %d", chip->slave_cp_ichg,
		int_flag);
}
/*********************************************************************/
...
static int sc8547_slave_get_cp_status(struct oplus_voocphy_manager *chip)
{
	...
	ret_reg06 = sc8547_slave_read_byte(chip->slave_client, SC8547_REG_06,
					   &data_reg06);
	ret_reg07 = sc8547_slave_read_byte(chip->slave_client, SC8547_REG_07,
					   &data_reg07);
...
static int sc8547_slave_reg_reset(struct oplus_voocphy_manager *chip,
				  bool enable)
{
	int ret;
	u8 val;
...
static int sc8547_slave_set_chg_enable(struct oplus_voocphy_manager *chip,
				       bool enable)
{
	if (!chip) {
		pr_err("Failed\n");
	...
	if (enable) {
		...
		return sc8547_slave_write_byte(chip->slave_client,
					       SC8547_REG_07, 0x85);
	} else {
...
static int sc8547_slave_get_voocphy_enable(struct oplus_voocphy_manager *chip,
					   u8 *data)
{
	int ret = 0;
...
static void
sc8547_slave_dump_reg_in_err_issue(struct oplus_voocphy_manager *chip)
{
	int i = 0, p = 0;
	...
	for (i = 0; i < 37; i++) {
		p = p + 1;
		sc8547_slave_read_byte(chip->client, i,
				       &chip->slave_reg_dump[p]);
	}
	for (i = 0; i < 9; i++) {
		p = p + 1;
		sc8547_slave_read_byte(chip->client, 43 + i,
				       &chip->slave_reg_dump[p]);
	}
	p = p + 1;
	sc8547_slave_read_byte(chip->client, SC8547_REG_36,
			       &chip->slave_reg_dump[p]);
	p = p + 1;
	sc8547_slave_read_byte(chip->client, SC8547_REG_3A,
			       &chip->slave_reg_dump[p]);
	pr_err("p[%d], ", p);
...
static int sc8547_slave_init_device(struct oplus_voocphy_manager *chip)
{
	...
	sc8547_slave_write_byte(chip->slave_client, SC8547_REG_04,
				0x00); //VBUS_OVP:10 2:1 or 1:1V
	reg_data = 0x20 | (chip->ocp_reg & 0xf);
	sc8547_slave_write_byte(chip->slave_client, SC8547_REG_00,
				reg_data); //VBAT_OVP:4.65V
	sc8547_slave_write_byte(chip->slave_client, SC8547_REG_05,
				0x28); //IBUS_OCP_UCP:3.6A
	sc8547_slave_write_byte(chip->slave_client, SC8547_REG_2B,
	...
	sc8547_slave_write_byte(chip->slave_client, SC8547_REG_07,
				0x05); //adjust slave freq
...
static int sc8547_slave_svooc_hw_setting(struct oplus_voocphy_manager *chip)
{
	...
	sc8547_slave_write_byte(chip->slave_client, SC8547_REG_02,
				0x01); //VAC_OVP:12v
	sc8547_slave_write_byte(chip->slave_client, SC8547_REG_04,
				0x50); //VBUS_OVP:10v
	reg_data = 0x20 | (chip->ocp_reg & 0xf);
	...
	sc8547_slave_write_byte(chip->slave_client, SC8547_REG_09,
				0x03); //WD:1000ms
	sc8547_slave_write_byte(chip->slave_client, SC8547_REG_3C,
				0x40); //VOOC_CTRL:disable
	sc8547_slave_write_byte(chip->slave_client, SC8547_REG_11,
...
static int sc8547_slave_hw_setting(struct oplus_voocphy_manager *chip,
				   int reason)
{
	if (!chip) {
					   ...
					   char *buf)
{
	...
	for (addr = 0x0; addr <= 0x3C; addr++) {
		if ((addr < 0x24) || (addr > 0x2B && addr < 0x33) ||
		    addr == 0x36 || addr == 0x3C) {
			ret = sc8547_slave_read_byte(chip->slave_client, addr,
						     &val);
			if (ret == 0) {
					   ...
					   const char *buf, size_t count)
{
	...
	if (ret == 2 && reg <= 0x3C)
		sc8547_slave_write_byte(chip->slave_client, (unsigned char)reg,
					(unsigned char)val);

	return count;
}

static DEVICE_ATTR(registers, 0660, sc8547_slave_show_registers,
		   sc8547_slave_store_register);
