...
static void oplus_voocphy_check_charger_out_work_func(struct work_struct *work)
{
	...
	struct oplus_voocphy_manager *chip =
		container_of(dwork, struct oplus_voocphy_manager,
			     voocphy_check_charger_out_work);
	int chg_vol = 0;
...
static void oplus_voocphy_check_charger_out(struct oplus_voocphy_manager *chip)
{
	...
	schedule_delayed_work(
		&chip->voocphy_check_charger_out_work,
		round_jiffies_relative(msecs_to_jiffies(TIME_3SEC)));
}

static void
oplus_adsp_voocphy_handle_track_status(struct oplus_voocphy_manager *chip,
				       int intval)
{
	int track_status;
...
void oplus_adsp_voocphy_handle_status(struct power_supply *psy, int intval)
{
	...
	if (!chip) {
		printk(KERN_ERR
		       "!!!chip null, adsp voocphy non handle status: [%d]\n",
		       intval);
		return;
	}
	...
	if (!psy) {
		printk(KERN_ERR
		       "!!!psy null, adsp voocphy non handle status: [%d]\n",
		       intval);
		return;
	}
	...
	if ((intval & 0xFF) == ADSP_VPHY_FAST_NOTIFY_PRESENT) {
		...
		oplus_adsp_voocphy_handle_track_status(chip, intval);
		printk(KERN_ERR
		       "!!![adsp_voocphy] fastchg start: [%d], adapter version: [0x%0x]\n",
		...
		oplus_adsp_voocphy_handle_track_status(chip, intval);
		printk(KERN_ERR
		       "!!![adsp_voocphy] fastchg dummy start: [%d], adapter version: [0x%0x]\n",
		...
		if (chip->fast_chg_type == FASTCHG_CHARGER_TYPE_UNKOWN) {
			chip->fastchg_start = true;
			chip->fast_chg_type =
				oplus_adsp_voocphy_get_fast_chg_type();
			power_supply_changed(psy);
		}
		...
		power_supply_changed(psy);
		printk(KERN_ERR
		       "!!![adsp_voocphy] fastchg switch temp range: [%d]\n",
		       intval);
		...
		printk(KERN_ERR
		       "!!![adsp_voocphy] fastchg commu clk err: [%d]\n",
		       intval);
		...
		oplus_chg_unsuspend_charger();
		printk(KERN_ERR
		       "!!![adsp_voocphy] fastchg hw vbatt || tbatt high: [%d]\n",
		       intval);
