...
static int hl7138_update_bits(struct i2c_client *client, u8 reg, u8 mask,
			      u8 data)
{
	int ret;
	u8 tmp;
...
static int hl7138_get_adapter_info(struct oplus_voocphy_manager *chip)
{
	...
	data = hl7138_read_word(
		chip->client,
		HL7138_REG_3A); /* JL: 2E=RX_Rdata,change to 0x3A */
...
static void hl7138_update_data(struct oplus_voocphy_manager *chip)
{
	...
	ret = i2c_smbus_read_i2c_block_data(
		chip->client, HL7138_REG_42, HL7138_REG_NUM_12,
	...
	if (chip->adapter_type == ADAPTER_SVOOC) {
		chip->cp_ichg =
			((data_block[HL7138_REG_ADC_H_BIT_ICHG]
			  << HL7138_REG_ADC_BIT_OFFSET_4) |
...
static int hl7138_reg_reset(struct oplus_voocphy_manager *chip, bool enable)
{
	unsigned char value;
	int ret = 0;
	ret = hl7138_read_byte(chip->client, HL7138_REG_01,
			       &value); /* clear INTb */

	hl7138_write_byte(chip->client, HL7138_REG_09,
			  0x00); /* set default mode; */
	hl7138_write_byte(chip->client, HL7138_REG_0A,
			  0xAE); /* set default mode; */
	hl7138_write_byte(chip->client, HL7138_REG_0C,
			  0x03); /* set default mode; */
	hl7138_write_byte(chip->client, HL7138_REG_0F,
			  0x00); /* set default mode; */
	hl7138_write_byte(chip->client, HL7138_REG_13,
			  0x00); /* set default mode; */
	hl7138_write_byte(chip->client, HL7138_REG_15,
			  0x00); /* set default mode; */
	hl7138_write_byte(chip->client, HL7138_REG_17,
			  0x00); /* set default mode; */
	hl7138_write_byte(chip->client, HL7138_REG_37,
			  0x02); /* reset VOOC PHY; */
	hl7138_write_byte(chip->client, HL7138_REG_3F,
			  0xD1); /* bit7 T6:170us */
...
static int hl7138_get_chg_enable(struct oplus_voocphy_manager *chip, u8 *data)
{
	...
	ret = hl7138_read_byte(chip->client, HL7138_REG_12,
			       data); /* JL:RST_REG 12 change to 14; */
	if (ret < 0) {
...
#define HL7138_CHG_EN                                                          \
	(HL7138_CHG_ENABLE << HL7138_CHG_EN_SHIFT) /* 1 << 7   0x80 */
#define HL7138_CHG_DIS                                                         \
	(HL7138_CHG_DISABLE << HL7138_CHG_EN_SHIFT) /* 0 << 7   0x00 */
#define HL7138_IBUS_UCP_EN                                                     \
	(HL7138_IBUS_UCP_ENABLE                                                \
	 << HL7138_IBUS_UCP_DIS_SHIFT) /* 1 << 2   0x04 */
#define HL7138_IBUS_UCP_DIS                                                    \
	(HL7138_IBUS_UCP_DISABLE                                               \
	 << HL7138_IBUS_UCP_DIS_SHIFT) /* 0 << 2   0x00 */
#define HL7138_IBUS_UCP_DEFAULT 0x01

static int hl7138_set_chg_enable(struct oplus_voocphy_manager *chip,
				 bool enable)
{
	if (!chip) {
		chg_err("Failed\n");
	...
	if (enable) {
		if (oplus_chg_check_pd_svooc_adapater()) {
			return hl7138_write_byte(
				...
				HL7138_CHG_EN | HL7138_IBUS_UCP_DIS |
...
static int hl7138_get_cp_ichg(struct oplus_voocphy_manager *chip)
{
	...
	if (cp_enable == 0)
		return 0;
	/*parse data_block for improving time of interrupt*/
	i2c_smbus_read_i2c_block_data(chip->client, HL7138_REG_44,
				      HL7138_REG_NUM_2, data_block);

	if (chip->adapter_type == ADAPTER_SVOOC) {
		chip->cp_ichg =
			((data_block[0] << HL7138_REG_ADC_BIT_OFFSET_4) |
			 data_block[1]) *
			HL7138_SVOOC_IBUS_FACTOR; /* Iin_lbs=1.10mA@CP; */
	} else {
		chip->cp_ichg =
...
int hl7138_get_cp_vbat(struct oplus_voocphy_manager *chip)
{
	...
	/*parse data_block for improving time of interrupt*/
	i2c_smbus_read_i2c_block_data(chip->client, HL7138_REG_46,
				      HL7138_REG_NUM_2, data_block);

	chip->cp_vbat = ((data_block[0] << HL7138_REG_ADC_BIT_OFFSET_4) |
			 data_block[1]) *
			HL7138_FACTOR_125_100;

	return chip->cp_vbat;
}

static void hl7138_set_pd_svooc_config(struct oplus_voocphy_manager *chip,
				       bool enable)
{
	int ret = 0;
	u8 reg = 0;
	...
	if (enable)
		hl7138_write_byte(chip->client, HL7138_REG_12,
				  0x01); /* disable ucp */
	else
		hl7138_write_byte(chip->client, HL7138_REG_12,
				  0x85); /* enable ucp */
...
static int hl7138_set_adc_enable(struct oplus_voocphy_manager *chip,
				 bool enable)
{
	if (!chip) {
		chg_err("Failed\n");
 ...
 * @return 0:success
 */
static int hl7138_set_adc_forcedly_enable(struct oplus_voocphy_manager *chip,
					  int mode)
{
	int ret = 0;
	if (!chip) {
...
int hl7138_reset_voocphy(struct oplus_voocphy_manager *chip)
{
	...
	hl7138_write_byte(chip->client, HL7138_REG_11,
			  0xDC); /* JL:Dis VBAT,IBAT reg; */
	hl7138_write_byte(
		chip->client, HL7138_REG_08,
		0x38); /* JL:vbat_ovp=4.65V;00->08;(4.65-0.09)/10=54; */
	hl7138_write_byte(chip->client, HL7138_REG_0B,
			  0x83); /* JL:VBUS_OVP=7V;4+val*lsb; */
	/* hl7138_write_byte(chip->client, HL7138_REG_0C, 0x0F);		//JL:vbus_ovp=10V;04->0c;10.5/5.25V; */
	hl7138_write_byte(
		chip->client, HL7138_REG_0E,
		0x32); /* JL:UCP_deb=5ms;IBUS_OCP=3.6A;05->0e;3.5A_max; */
	hl7138_write_byte(chip->client, HL7138_REG_40,
			  0x00); /* JL:Dis_ADC;11->40; */
	hl7138_write_byte(chip->client, HL7138_REG_02,
			  0xE0); /* JL:mask all INT_FLAG */
	hl7138_write_byte(chip->client, HL7138_REG_10,
			  0xEC); /* JL:Dis IIN_REG; */

	/* turn off mos */
	hl7138_write_byte(chip->client, HL7138_REG_12,
			  0x05); /* JL:Fsw=500KHz;07->12; */
	...
	hl7138_write_byte(chip->client, HL7138_REG_3C,
			  0xff); /* JL:30->3C,VOOC_PHY FLAG ALL MASK; */
...
int hl7138_reactive_voocphy(struct oplus_voocphy_manager *chip)
{
	...
	/* clear tx data */
	hl7138_write_byte(chip->client, HL7138_REG_38,
			  0x00); /* JL:2C->38,Dis all; */
	hl7138_write_byte(chip->client, HL7138_REG_39,
			  0x00); /* JL:2D->39,Dis all; */

	/* vooc */
	hl7138_write_byte(chip->client, HL7138_REG_3C,
			  0x85); /* JL:30->3C,JUST enable RX_START & TX_DONE; */
...
{
	hl7138_write_byte(chip->client, HL7138_REG_40,
			  0x00); /* ADC_CTRL:disable,JL:11-40; */
	hl7138_write_byte(chip->client, HL7138_REG_0B,
			  0x83); /* VBUS_OVP=7V,JL:02->0B; */
	...
	hl7138_write_byte(chip->client, HL7138_REG_08,
			  0x38); /* VBAT_OVP:4.56	4.56+0.09*/
	hl7138_write_byte(chip->client, HL7138_REG_0E,
			  0x32); /* IBUS_OCP:3.5A      ocp:100mA */
	/* hl7138_write_byte(chip->client, HL7138_REG_0A, 0x2E);		//IBAT_OCP:max;JL:01-0A;0X2E=6.6A,MAX; */
	hl7138_write_byte(chip->client, HL7138_REG_37,
			  0x00); /* VOOC_CTRL:disable;JL:2B->37; */

	hl7138_write_byte(chip->client, HL7138_REG_3C,
			  0x85); /* diff mask inter; */
	hl7138_write_byte(chip->client, HL7138_REG_02,
			  0xE0); /* JL:mask all INT_FLAG */
	hl7138_write_byte(chip->client, HL7138_REG_10,
			  0xEC); /* JL:Dis IIN_REG; */
	...
	hl7138_write_byte(chip->client, HL7138_REG_16,
			  0x2C); /* JL:OV=500, UV=250 */
...
static int hl7138_svooc_hw_setting(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->high_curr_setting)
		...
		hl7138_write_byte(
			chip->client, HL7138_REG_0E,
			0x32); /* IBUS_OCP:3.6A,UCP_DEB=5ms;JL:05-0E; */

	hl7138_write_byte(chip->client, HL7138_REG_14,
			  0x02); /* WD:1000ms,JL:09-14; */
	hl7138_write_byte(chip->client, HL7138_REG_15,
			  0x00); /* enter cp mode */
	hl7138_write_byte(chip->client, HL7138_REG_16,
			  0x2C); /* JL:OV=500, UV=250 */
...
static int hl7138_vooc_hw_setting(struct oplus_voocphy_manager *chip)
{
	...
	hl7138_write_byte(chip->client, HL7138_REG_0C,
			  0x0F); /* VIN_OVP:5.85v,,JL:04-0C; */

	hl7138_write_byte(chip->client, HL7138_REG_0E,
			  0x1A); /* IBUS_OCP:4.6A,(16+9)*0.1+0.1+2=4.6A; */
...
static int hl7138_5v2a_hw_setting(struct oplus_voocphy_manager *chip)
{
	...
	hl7138_write_byte(chip->client, HL7138_REG_0B, 0x83); /* VBUS_OVP=7V */
	hl7138_write_byte(chip->client, HL7138_REG_0C,
			  0x0F); /* VIN_OVP:11.7v,,JL:04-0C; */
	hl7138_write_byte(chip->client, HL7138_REG_0E,
			  0xAF); /* IBUS_OCP:3.6A,UCP_DEB=5ms;JL:05-0E; */
	hl7138_write_byte(chip->client, HL7138_REG_14,
			  0x08); /* WD:DIS,JL:09-14; */
	hl7138_write_byte(chip->client, HL7138_REG_15, 0x80); /* JL:bp mode; */
				     ...
				     const char *buf, size_t count)
{
	...
	if (ret == 2 && reg <= 0x4F)
		hl7138_write_byte(chip->client, (unsigned char)reg,
				  (unsigned char)val);

	return count;
}
static DEVICE_ATTR(registers, 0660, hl7138_show_registers,
		   hl7138_store_register);
...
static int hl7138_parse_dt(struct oplus_voocphy_manager *chip)
{
	...
	chip->high_curr_setting =
		of_property_read_bool(node, "qcom,high_curr_setting");
...
{
	if (oplus_voocphy_mg) {
		hl7138_write_byte(oplus_voocphy_mg->client, HL7138_REG_40,
				  0x00); /* disable */
