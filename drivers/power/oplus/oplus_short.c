...
oplus_short_c_battery_status_init(char *str)
{
...
static int __init __attribute__((unused))
oplus_short_c_switch_status_init(char *str)
{
...
static int __init __attribute__((unused))
oplus_short_c_feature_sw_status_init(char *str)
{
...
static int __init __attribute__((unused))
oplus_short_c_feature_hw_status_init(char *str)
{
...
void oplus_chg_short_c_battery_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		if (oplus_is_power_off_chg(chip) == true) {
			...
			if (data_mounted_count < 30 &&
			    oplus_short_c_batt_data_is_mounted() ==
				    true) { //3min
				oplus_short_c_batt_read_file(chip);
	...
	if (chip->short_c_batt.err_code == SHORT_C_BATT_STATUS__CV_ERR_CODE1 ||
	    (chip->short_c_batt.err_code ==
		     SHORT_C_BATT_STATUS__FULL_ERR_CODE2 &&
	     chip->batt_full !=
		     true) //set errcode2 at the 1st time, need to check errcode3
	    || chip->short_c_batt.err_code ==
		       SHORT_C_BATT_STATUS__FULL_ERR_CODE3 ||
	    (chip->short_c_batt.err_code ==
		     SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4 &&
	     chip->batt_full !=
		     true) //set errcode4 at the 1st time, need to check errcode3
	    || chip->short_c_batt.err_code ==
		       SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE5) {
	...
	if (chip->temperature <= LITTLE_COLD_TEMP_THRESHOLD ||
	    chip->temperature > WARM_TEMP_THRESHOLD) { //0~45
	...
	if (chip->batt_full == true && chip->chging_on == false &&
		...
		if (full_timer5_count <= chip->short_c_batt.full_timer5) {
			...
			oplus_short_c_batt_get_average_vbatt_mv(chip,
								true); //reset
			return;
		}
