...
void oplus_get_chg_ops_name_from_dt(struct device_node *node)
{
	...
	strncpy(chg_ops_name, (rc ? "plat-pmic" : chg_ops_name_dt),
		CHG_OPS_DESC_NAME_MAX_LENTH);
			   ...
			   struct oplus_chg_operations *chg_ops)
{
	...
	if (oplus_chg_ops_desc_new == NULL) {
		oplus_chg_ops_desc_new =
			kmalloc(sizeof(struct oplus_chg_ops_desc), GFP_ATOMIC);
		if (oplus_chg_ops_desc_new != NULL) {
			strncpy(oplus_chg_ops_desc_new->name, name,
				CHG_OPS_DESC_NAME_MAX_LENTH);
			(oplus_chg_ops_desc_new
				 ->name)[CHG_OPS_DESC_NAME_MAX_LENTH - 1] =
				'\0';
			oplus_chg_ops_desc_new->chg_ops = chg_ops;

			spin_lock(&g_oplus_chg_ops_mg_data.chg_ops_list_lock);
			list_add_tail(
				&oplus_chg_ops_desc_new->list,
...
void oplus_chg_ops_deinit(void)
{
	...
	spin_lock(&g_oplus_chg_ops_mg_data.chg_ops_list_lock);
	list_for_each (pos, &g_oplus_chg_ops_mg_data.chg_ops_list_head) {
		oplus_chg_ops_desc_loopup =
			list_entry(pos, struct oplus_chg_ops_desc, list);
...
struct oplus_chg_operations *oplus_chg_ops_get(void)
{
	...
	chg_ops_desc =
		oplus_chg_ops_desc_get(g_oplus_chg_ops_mg_data.chg_ops_name);
