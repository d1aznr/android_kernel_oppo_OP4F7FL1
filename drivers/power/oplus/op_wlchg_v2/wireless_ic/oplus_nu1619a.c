...
static int nu1619_track_upload_i2c_err_info(struct oplus_nu1619 *chip,
...
static int nu1619_read_data(struct oplus_nu1619 *chip, u16 addr, u8 *buf,
			    int len)
{
	char cmd_buf[2] = { addr >> 8, addr & 0xff };
	int rc = 0;
...
__maybe_unused static int nu1619_write_byte_mask(struct oplus_nu1619 *chip,
						 u16 addr, u8 mask, u8 data)
{
	u8 temp = 0;
	int rc = 0;
...
static void nu1619_track_i2c_err_load_trigger_work(struct work_struct *work)
{
	...
	struct oplus_nu1619 *chip = container_of(dwork, struct oplus_nu1619,
...
static int nu1619_track_upload_wls_rx_err_info(struct oplus_nu1619 *chip,
					       int err_type)
{
	int index = 0;
	int curr_time;
	...
	oplus_chg_track_get_wls_trx_err_reason(err_type, chip->err_reason,
					       sizeof(chip->err_reason));
	index += snprintf(&(chip->rx_err_load_trigger->crux_info[index]),
			  OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$err_reason@@%s", chip->err_reason);
...
static int nu1619_track_upload_wls_tx_err_info(struct oplus_nu1619 *chip,
					       int err_type)
{
	int index = 0;
	int curr_time;
...
static int nu1619_track_upload_wls_update_err_info(struct oplus_nu1619 *chip,
						   int err_type)
{
	int index = 0;
	int curr_time;
...
static int nu1619_track_debugfs_init(struct oplus_nu1619 *chip)
{
	...
	debugfs_create_u32("debug_force_i2c_err", 0644, debugfs_nu1619,
			   &(chip->debug_force_i2c_err));
	debugfs_create_u32("debug_force_rx_err", 0644, debugfs_nu1619,
			   &(chip->debug_force_rx_err));
	debugfs_create_u32("debug_force_tx_err", 0644, debugfs_nu1619,
			   &(chip->debug_force_tx_err));
	debugfs_create_u32("debug_force_update_err", 0644, debugfs_nu1619,
			   &(chip->debug_force_update_err));
...
static int nu1619_set_trx_boost_enable(struct oplus_nu1619 *chip, bool en)
{
	...
	pval.intval = en;
	return oplus_chg_mod_set_property(chip->wls_ocm,
					  OPLUS_CHG_PROP_TRX_POWER_EN, &pval);
}
...
static int nu1619_set_trx_boost_vol(struct oplus_nu1619 *chip, int vol_mv)
{
	...
	pval.intval = vol_mv;
	return oplus_chg_mod_set_property(chip->wls_ocm,
					  OPLUS_CHG_PROP_TRX_POWER_VOL, &pval);
}

__maybe_unused static int
nu1619_set_trx_boost_curr_limit(struct oplus_nu1619 *chip, int curr_ma)
{
	union oplus_chg_mod_propval pval;
	...
	pval.intval = curr_ma;
	return oplus_chg_mod_set_property(
		chip->wls_ocm, OPLUS_CHG_PROP_TRX_POWER_CURR_LIMIT, &pval);
}

__maybe_unused static int
nu1619_get_rx_event_gpio_val(struct oplus_nu1619 *chip)
{
...
static bool nu1619_is_in_tx_mode(struct oplus_nu1619 *chip)
{
	...
	oplus_chg_mod_get_property(chip->wls_ocm, OPLUS_CHG_PROP_WLS_TYPE,
				   &pval);
...
static int nu1619_get_rx_mode(struct oplus_chg_ic_dev *dev,
...
static int nu1619_set_rx_mode(struct oplus_chg_ic_dev *dev,
			      enum oplus_chg_wls_rx_mode rx_mode)
{
	struct oplus_nu1619 *chip;
	int rc = 0;
...
static int nu1619_set_dcdc_enable(struct oplus_chg_ic_dev *dev, bool en)
{
	...
	rc = pinctrl_select_state(chip->pinctrl, en ? chip->dcdc_en_active :
						      chip->dcdc_en_sleep);
	if (rc < 0)
...
static int nu1619_send_msg(struct oplus_chg_ic_dev *dev, unsigned char msg[],
			   int len)
{
	struct oplus_nu1619 *chip;
...
static int nu1619_register_msg_callback(struct oplus_chg_ic_dev *dev,
					void *dev_data,
...
static int nu1619_wirte_firmware_data(struct oplus_nu1619 *chip,
				      unsigned short addr,
...
static bool nu1619_download_firmware_data(struct oplus_nu1619 *chip, char area)
{
	...
	if (area == TX_AREA) {
		ret = nu1619_wirte_firmware_data(chip, 4864,
						 chip->fw_tx_length);
		if (ret < 0) {
			return false;
		}
...
static bool nu1619_req_checksum_and_fw_version(
	struct oplus_nu1619 *chip, char *boot_check_sum, char *rx_check_sum,
...
static bool nu1619_check_firmware_version(struct oplus_nu1619 *chip)
{
	...
	pr_err("<FW UPDATE> boot_no_need_update=%d, rx_no_need_update=%d, tx_no_need_update=%d\n",
	       chip->boot_no_need_update, chip->rx_no_need_update,
	       chip->tx_no_need_update);

	if ((chip->boot_no_need_update) && (chip->rx_no_need_update) &&
	    (chip->tx_no_need_update))
		return false;
	return true;
}
...
static int nu1619_check_idt_fw_update(struct oplus_chg_ic_dev *dev,
				      char *fw_img, int fw_len)
{
	int rc = 0;
	struct oplus_nu1619 *chip;
...
static bool nu1619_upgrade_firmware(struct oplus_nu1619 *chip, u8 *fw_buf,
				    int fw_size)
{
	u16 addr = 0;
	u8 addr_h = 0;
...
static int nu1619_upgrade_firmware_by_buf(struct oplus_chg_ic_dev *dev,
...
static void nu1619_tx_event_config(struct oplus_nu1619 *chip, int status,
				   int err)
{
...
static void nu1619_event_process(struct oplus_nu1619 *chip)
{
	...
	if (!nu1619_rx_is_enable(chip->ic_dev) &&
	    nu1619_is_in_tx_mode(chip) == false) {
		...
		if (temp[1] & NU1619_RX_ERR_UV_ALARM) {
			if (is_wls_ocm_available(chip)) {
				...
				oplus_chg_mod_set_property(
					chip->wls_ocm,
					OPLUS_CHG_PROP_RX_VOUT_UVP, &temp_val);
			}
		}
	...
	retry:
		...
		if (rc != 3) {
			...
			if (chip->rx_msg.msg_call_back != NULL)
				chip->rx_msg.msg_call_back(
					chip->rx_msg.dev_data, val_buf);
		}
	}
		...
		if (iout >= LDO_ON_MA) {
			if (is_wls_ocm_available(chip))
				oplus_chg_anon_mod_event(
					chip->wls_ocm, OPLUS_CHG_EVENT_ONLINE);
			chip->connected_ldo_on = true;
		} else {
			schedule_delayed_work(&chip->check_ldo_on_work,
					      NU1619_CHECK_LDO_ON_DELAY);
		}
	}
}
...
static int nu1619_gpio_init(struct oplus_nu1619 *chip)
{
	...
	chip->rx_event_default =
		pinctrl_lookup_state(chip->pinctrl, "rx_event_default");
	...
	rc = devm_request_irq(chip->dev, chip->rx_event_irq,
			      nu1619_event_handler, IRQF_TRIGGER_FALLING,
			      "rx_event_irq", chip);
	if (rc < 0) {
	...
	chip->rx_con_default =
		pinctrl_lookup_state(chip->pinctrl, "rx_con_default");
	...
	rc = devm_request_irq(chip->dev, chip->rx_con_irq,
			      nu1619_connect_handler,
	...
	chip->rx_en_active =
		pinctrl_lookup_state(chip->pinctrl, "rx_en_active");
	...
	chip->mode_sw_active =
		pinctrl_lookup_state(chip->pinctrl, "mode_sw_active");
	if (IS_ERR_OR_NULL(chip->mode_sw_active)) {
		pr_err("get mode_sw_active fail\n");
		goto free_mode_sw_gpio;
	}
	chip->mode_sw_sleep =
		pinctrl_lookup_state(chip->pinctrl, "mode_sw_sleep");
			       ...
			       const struct i2c_device_id *id)
{
	...
	pr_err("call !\n");
	chip = devm_kzalloc(&client->dev, sizeof(struct oplus_nu1619),
			    GFP_KERNEL);
	if (!chip) {
	...
	chip->support_epp_11w =
		of_property_read_bool(node, "oplus,support_epp_11w");
