...
static u32 oplus_chg_cfg_get_data_size(struct oplus_chg_param_head *param_head)
{
	...
	for (index = 0; index < param_head->size;) {
		data_head =
			(struct oplus_chg_cfg_data_head *)(param_head->data +
							   index);
		...
		index += data_head->size +
			 sizeof(struct oplus_chg_cfg_data_head);
	}
			     ...
			     u8 *out_buf)
{
	...
	param_head =
		(struct oplus_chg_param_head *)((unsigned char *)src +
						cfg_head->param_index[type]);
	...
	for (data_index = 0; data_index < param_head->size;) {
		...
		memcpy(out_buf + data_head->index, data_head->data,
		       data_head->size);
		data_index += data_head->size +
			      sizeof(struct oplus_chg_cfg_data_head);
	}
