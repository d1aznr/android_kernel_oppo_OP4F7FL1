...
static bool oplus_chg_wls_track_set_prop(struct oplus_chg_wls *wls_dev,
...
static int oplus_chg_wls_get_tripartite_r_power(u8 f2_data)
{
	...
	for (i = 0; i < ARRAY_SIZE(oplus_chg_wls_tripartite_pwr_table); i++) {
		if (oplus_chg_wls_tripartite_pwr_table[i].f2_id ==
		    (f2_data & 0x7F)) {
			r_pwr = oplus_chg_wls_tripartite_pwr_table[i].r_power *
				1000;
			break;
		}
	}
...
static int oplus_chg_wls_set_wrx_enable(struct oplus_chg_wls *wls_dev, bool en)
{
	...
	pr_err("wrx: set value:%d, gpio_val:%d\n", en,
	       gpio_get_value(wls_dev->wrx_en_gpio));
...
static int
oplus_chg_wls_nor_input_disable_vote_callback(struct votable *votable,
					      void *data, int disable,
					      const char *client, bool step)
{
	struct oplus_chg_wls *wls_dev = data;
...
static int oplus_chg_wls_set_ext_pwr_enable(struct oplus_chg_wls *wls_dev,
					    bool en)
{
	int rc;
	...
	rc = pinctrl_select_state(wls_dev->pinctrl,
				  en ? wls_dev->ext_pwr_en_active :
				       wls_dev->ext_pwr_en_sleep);
	if (rc < 0)
	...
	pr_err("ext_pwr: set value:%d, gpio_val:%d\n", en,
	       gpio_get_value(wls_dev->ext_pwr_en_gpio));
...
int oplus_chg_wls_set_config(struct oplus_chg_mod *wls_ocm, u8 *buf)
{
	...
	wls_cfg_temp = kmalloc(sizeof(struct oplus_chg_wls_dynamic_config),
			       GFP_KERNEL);
	if (wls_cfg_temp == NULL) {
		pr_err("alloc wls_cfg buf error\n");
		return -ENOMEM;
	}
	memcpy(wls_cfg_temp, wls_cfg,
	       sizeof(struct oplus_chg_wls_dynamic_config));
	rc = oplus_chg_cfg_load_param(buf, OPLUS_CHG_WLS_PARAM,
				      (u8 *)wls_cfg_temp);
	if (rc < 0)
		goto out;
	memcpy(wls_cfg, wls_cfg_temp,
	       sizeof(struct oplus_chg_wls_dynamic_config));
...
static void
oplus_chg_wls_get_third_part_verity_data_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev =
		container_of(dwork, struct oplus_chg_wls,
			     wls_get_third_part_verity_data_work);
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;

	oplus_chg_common_set_mutual_cmd(wls_dev->comm_ocm,
					CMD_WLS_THIRD_PART_AUTH,
					sizeof(wls_status->vendor_id),
		...
		if (rx_msg->msg_type == WLS_CMD_GET_VENDOR_ID) {
			...
			if (is_comm_ocm_available(wls_dev) &&
				...
				schedule_delayed_work(
					&wls_dev->wls_get_third_part_verity_data_work,
					0);
	...
	case WLS_RESPONE_ADAPTER_TYPE:
		if (rx_msg->msg_type == WLS_CMD_INDENTIFY_ADAPTER) {
			...
			pwr_mw = oplus_chg_wls_get_base_power_max(
				wls_status->adapter_id);
			oplus_vote(wls_dev->fcc_votable, BASE_MAX_VOTER, true,
				   (pwr_mw * 1000 / WLS_RX_VOL_MAX_MV), false);
			if (wls_dev->wls_ocm)
				oplus_chg_mod_changed(wls_dev->wls_ocm);
			if ((wls_status->adapter_type ==
			     WLS_ADAPTER_TYPE_VOOC) ||
			    (wls_status->adapter_type ==
			     WLS_ADAPTER_TYPE_SVOOC) ||
			    (wls_status->adapter_type ==
			     WLS_ADAPTER_TYPE_PD_65W)) {
				q_value = oplus_chg_wls_get_q_value(
					wls_dev, wls_status->adapter_id);
				(void)oplus_chg_wls_rx_send_match_q(
					wls_dev->wls_rx, q_value);
				msleep(200);
			}
		}
		break;
		...
		if (rx_msg->msg_type == WLS_CMD_INTO_FASTCHAGE) {
			...
			if (wls_status->adapter_id != WLS_ADAPTER_THIRD_PARTY)
				wls_status->pwr_max_mw =
					oplus_chg_wls_get_r_power(
						wls_dev,
						wls_status->adapter_power);
			else
				wls_status->pwr_max_mw =
					oplus_chg_wls_get_tripartite_r_power(
						wls_status->adapter_power);
			if (wls_status->pwr_max_mw > 0) {
				oplus_vote(wls_dev->fcc_votable, RX_MAX_VOTER,
					   true,
			...
			if (!wls_status->verity_started)
				oplus_vote(wls_dev->fcc_votable, VERITY_VOTER,
					   true,
					   dynamic_cfg->verity_curr_max_ma,
					   false);
		}
		break;
...
static void oplus_chg_wls_rx_msg_callback(void *dev_data, u8 data[])
{
	...
	case WLS_MSG_TYPE_STANDARD_MSG:
		...
		if (!((data[1] >= WLS_RESPONE_ENCRYPT_DATA1 &&
		      ...
		      (data[1] >= WLS_RESPONE_SET_AES_DATA1 &&
		       data[1] <= WLS_RESPONE_GET_AES_DATA6) ||
		      data[1] == WLS_RESPONE_PRODUCT_ID ||
		      data[1] == WLS_RESPONE_BATT_TEMP_SOC)) {
...
retry:
	...
	if (!wls_status->verity_pass && wls_status->rx_online) {
		...
		oplus_vote(wls_dev->rx_disable_votable, VERITY_VOTER, true, 1,
			   false);
...
retry:
	if (!wls_status->aes_verity_data_ok) {
		pr_err("verity data not ready\n");
		if (is_comm_ocm_available(wls_dev))
			ret = oplus_chg_common_set_mutual_cmd(
				wls_dev->comm_ocm, CMD_WLS_THIRD_PART_AUTH,
				sizeof(wls_status->vendor_id),
				&(wls_status->vendor_id));
		else
			goto done;
	...
	wls_status->verity_count = 0;
	do {
		rc = oplus_chg_wls_send_data(wls_dev, WLS_CMD_SET_AES_DATA1,
					     buf, 5);
		if (rc < 0) {
			if (rc != -EAGAIN)
				wls_status->verity_count++;
	...
	wls_status->verity_count = 0;
	do {
		rc = oplus_chg_wls_send_data(wls_dev, WLS_CMD_SET_AES_DATA2,
					     buf, 5);
		if (rc < 0) {
			if (rc != -EAGAIN)
				wls_status->verity_count++;
	...
	wls_status->verity_count = 0;
	do {
		rc = oplus_chg_wls_send_data(wls_dev, WLS_CMD_SET_AES_DATA3,
					     buf, 5);
		if (rc < 0) {
			if (rc != -EAGAIN)
				wls_status->verity_count++;
	...
	wls_status->verity_count = 0;
	do {
		rc = oplus_chg_wls_send_data(wls_dev, WLS_CMD_SET_AES_DATA4,
					     buf, 5);
		if (rc < 0) {
			if (rc != -EAGAIN)
				wls_status->verity_count++;
	...
	wls_status->verity_count = 0;
	do {
		rc = oplus_chg_wls_send_data(wls_dev, WLS_CMD_SET_AES_DATA5,
					     buf, 5);
		if (rc < 0) {
			if (rc != -EAGAIN)
				wls_status->verity_count++;
	...
	wls_status->verity_count = 0;
	do {
		rc = oplus_chg_wls_send_data(wls_dev, WLS_CMD_SET_AES_DATA6,
					     buf, 5);
		if (rc < 0) {
			if (rc != -EAGAIN)
				wls_status->verity_count++;
	...
	wls_status->verity_count = 0;
	do {
		rc = oplus_chg_wls_send_msg(wls_dev, WLS_CMD_GET_AES_DATA1,
					    0xff, 5);
		if (rc < 0) {
			if (rc != -EAGAIN)
				wls_status->verity_count++;
	...
	wls_status->verity_count = 0;
	do {
		rc = oplus_chg_wls_send_msg(wls_dev, WLS_CMD_GET_AES_DATA2,
					    0xff, 5);
		if (rc < 0) {
			if (rc != -EAGAIN)
				wls_status->verity_count++;
	...
	wls_status->verity_count = 0;
	do {
		rc = oplus_chg_wls_send_msg(wls_dev, WLS_CMD_GET_AES_DATA3,
					    0xff, 5);
		if (rc < 0) {
			if (rc != -EAGAIN)
				wls_status->verity_count++;
	...
	wls_status->verity_count = 0;
	do {
		rc = oplus_chg_wls_send_msg(wls_dev, WLS_CMD_GET_AES_DATA4,
					    0xff, 5);
		if (rc < 0) {
			if (rc != -EAGAIN)
				wls_status->verity_count++;
	...
	wls_status->verity_count = 0;
	do {
		rc = oplus_chg_wls_send_msg(wls_dev, WLS_CMD_GET_AES_DATA5,
					    0xff, 5);
		if (rc < 0) {
			if (rc != -EAGAIN)
				wls_status->verity_count++;
	...
	wls_status->verity_count = 0;
	do {
		rc = oplus_chg_wls_send_msg(wls_dev, WLS_CMD_GET_AES_DATA6,
					    0xff, 5);
		if (rc < 0) {
			if (rc != -EAGAIN)
				wls_status->verity_count++;
	...
	pr_info("aes encrypt_data: 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x \
		...
		wls_status->aes_encrypt_data[0],
		wls_status->aes_encrypt_data[1],
		wls_status->aes_encrypt_data[2],
		wls_status->aes_encrypt_data[3],
		wls_status->aes_encrypt_data[4],
		wls_status->aes_encrypt_data[5],
		wls_status->aes_encrypt_data[6],
		wls_status->aes_encrypt_data[7],
		wls_status->aes_encrypt_data[8],
		wls_status->aes_encrypt_data[9],
		wls_status->aes_encrypt_data[10],
		wls_status->aes_encrypt_data[11],
		wls_status->aes_encrypt_data[12],
		wls_status->aes_encrypt_data[13],
		wls_status->aes_encrypt_data[14],
		wls_status->aes_encrypt_data[15]);

	if (memcmp(&wls_status->aes_encrypt_data,
		   &wls_status->aes_verfity_data.aes_encode_num,
...
static void oplus_chg_wls_reset_variables(struct oplus_chg_wls *wls_dev)
{
	...
	oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true,
		   wls_status->rx_present ? 0 : 100, false);
	...
	oplus_vote(wls_dev->fcc_votable, VERITY_VOTER, false, 0, false);
	oplus_vote(wls_dev->fastchg_disable_votable, QUIET_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->fastchg_disable_votable, CEP_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->fastchg_disable_votable, FCC_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->fastchg_disable_votable, SKIN_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->fastchg_disable_votable, BATT_VOL_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->fastchg_disable_votable, BATT_CURR_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->fastchg_disable_votable, RX_FAST_ERR_VOTER, false,
		   0, false);
	oplus_vote(wls_dev->fastchg_disable_votable, IOUT_CURR_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->fastchg_disable_votable, STARTUP_CEP_VOTER, false,
		   0, false);
	oplus_vote(wls_dev->fastchg_disable_votable, HW_ERR_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->fastchg_disable_votable, CURR_ERR_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->fastchg_disable_votable, COOL_DOWN_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->nor_out_disable_votable, FFC_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->nor_out_disable_votable, USER_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->nor_out_disable_votable, STOP_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->nor_out_disable_votable, UOVP_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->nor_input_disable_votable, UOVP_VOTER, false, 0,
		   false);
	oplus_vote(wls_dev->nor_input_disable_votable, USER_VOTER, false, 0,
		   false);
...
static int
oplus_chg_wls_track_upload_trx_general_info(struct oplus_chg_wls *wls_dev,
					    char *trx_crux_info,
					    bool usb_present_once)
{
	...
	memset(wls_dev->trx_info_load_trigger.crux_info, 0,
	       sizeof(wls_dev->trx_info_load_trigger.crux_info));
	...
	if (trx_crux_info && strlen(trx_crux_info))
		index += snprintf(
			&(wls_dev->trx_info_load_trigger.crux_info[index]),
			OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s",
			trx_crux_info);
...
static int oplus_chg_wls_set_trx_enable(struct oplus_chg_wls *wls_dev, bool en)
{
	...
	if (en) {
		...
		if (!wls_dev->usb_present)
			oplus_vote(wls_dev->wrx_en_votable, TRX_EN_VOTER, true,
				   1, false);
		else
			oplus_vote(wls_dev->wrx_en_votable, TRX_EN_VOTER, false,
				   0, false);
		msleep(20);
#endif
		...
		if (wls_status->trx_rxac == true)
			oplus_vote(wls_dev->rx_disable_votable, RXAC_VOTER,
				   true, 1, false);
		cancel_delayed_work_sync(&wls_dev->wls_trx_sm_work);
#ifdef CONFIG_OPLUS_CHARGER_MTK
		oplus_vote(wls_dev->rx_disable_votable, TRX_EN_VOTER, true, 0,
			   false);
#endif
		oplus_chg_wls_nor_set_boost_en(wls_dev->wls_nor, false);
		msleep(100);
#ifdef CONFIG_OPLUS_CHARGER_MTK
		oplus_vote(wls_dev->wrx_en_votable, TRX_EN_VOTER, false, 0,
			   false);
		oplus_vote(wls_dev->rx_disable_votable, TRX_EN_VOTER, false, 0,
			   false);
#endif
		if (wls_status->trx_rxac == true)
			oplus_vote(wls_dev->rx_disable_votable, RXAC_VOTER,
		...
		if (wls_status->trx_online &&
			...
			oplus_chg_wls_update_track_info(wls_dev, trx_crux_info,
							false);
				   ...
				   enum oplus_chg_wls_path_ctrl_type type)
{
	...
	case OPLUS_CHG_WLS_PATH_CTRL_TYPE_DISABLE_ALL:
		...
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 0,
			   false);
		break;
	case OPLUS_CHG_WLS_PATH_CTRL_TYPE_ENABLE_NOR:
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 300,
			   false);
		break;
...
static void oplus_chg_wls_set_cool_down_iout(struct oplus_chg_wls *wls_dev)
{
	...
	if (cool_down == 0 || wls_dev->factory_mode) {
		oplus_vote(wls_dev->nor_icl_votable, COOL_DOWN_VOTER, false, 0,
			   false);
		oplus_vote(wls_dev->fcc_votable, COOL_DOWN_VOTER, false, 0,
			   false);
		icl_ma = 0;
	} else {
		if (icl_ma > 0) {
			oplus_vote(wls_dev->nor_icl_votable, COOL_DOWN_VOTER,
				   true, icl_ma, false);
			oplus_vote(wls_dev->fcc_votable, COOL_DOWN_VOTER, true,
				   icl_ma, false);
		}
	}
	...
	if ((batt_temp_plugin != WLS_FAST_TEMP_MAX) &&
		...
		if (wls_dev->wls_status.adapter_id != WLS_ADAPTER_THIRD_PARTY) {
			wls_dev->wls_fcc_step.max_step =
				wls_dev->fcc_steps[batt_soc_plugin]
					.fcc_step[batt_temp_plugin]
					.max_step;
			memcpy(&wls_dev->wls_fcc_step.fcc_step,
			       wls_dev->fcc_steps[batt_soc_plugin]
				       .fcc_step[batt_temp_plugin]
				       .fcc_step,
			       sizeof(struct oplus_chg_wls_range_data) *
				       wls_dev->wls_fcc_step.max_step);
		} else {
			wls_dev->wls_fcc_step.max_step =
				wls_dev->fcc_third_part_steps[batt_soc_plugin]
					.fcc_step[batt_temp_plugin]
					.max_step;
			memcpy(&wls_dev->wls_fcc_step.fcc_step,
			       wls_dev->fcc_third_part_steps[batt_soc_plugin]
...
static void oplus_chg_wls_config(struct oplus_chg_wls *wls_dev)
{
	...
	case BATT_TEMP_HOT:
		if (!pre_temp_abnormal) {
			...
			wls_status->online_keep = true;
			oplus_vote(wls_dev->rx_disable_votable, JEITA_VOTER,
				   true, 1, false);
			oplus_vote(wls_dev->nor_icl_votable, JEITA_VOTER, true,
				   WLS_CURR_JEITA_CHG_MA, true);
	...
	default:
		...
		oplus_vote(wls_dev->rx_disable_votable, JEITA_VOTER, false, 0,
			   false);
		oplus_vote(wls_dev->nor_icl_votable, JEITA_VOTER, false, 0,
			   true);
		break;
	}
	...
	oplus_vote(wls_dev->nor_fcc_votable, MAX_VOTER, true, fcc_max_ma,
		   false);
...
int oplus_chg_wls_get_max_wireless_power(struct device *dev)
{
	...
	max_wls_r_power =
		oplus_chg_wls_get_r_power(wls_dev, wls_status->adapter_power);
	max_wls_r_power = max_wls_base_power > max_wls_r_power ?
				  max_wls_r_power :
				  max_wls_base_power;
	max_wls_power = max_wls_r_power > wls_dev->wls_power_mw ?
					     ...
					     const char *buf, size_t count)
{
	...
	case OPLUS_CHG_WLS_FORCE_TYPE_FAST:
		if (fast_curr_ma > 0) {
			...
			oplus_vote(wls_dev->fcc_votable, DEBUG_VOTER, true,
				   fast_curr_ma, false);
		} else {
			wls_status->fastchg_started = false;
			oplus_vote(wls_dev->fcc_votable, DEBUG_VOTER, false, 0,
				   false);
		}
		if (nor_curr_ma >= 0)
			oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true,
...
static enum oplus_chg_mod_property oplus_chg_wls_props[] = {
	...
	OPLUS_CHG_PROP_DOCK_TYPE,	OPLUS_CHG_PROP_WLS_SKEW_CURR,
	OPLUS_CHG_PROP_VERITY,		OPLUS_CHG_PROP_CHG_ENABLE,
	...
	OPLUS_CHG_PROP_RX_POWER,	OPLUS_CHG_PROP_RX_MAX_POWER,
	OPLUS_CHG_PROP_FOD_CAL,		OPLUS_CHG_PROP_BATT_CHG_ENABLE,
	...
	OPLUS_CHG_PROP_RX_VOUT_UVP,	OPLUS_CHG_PROP_FW_UPGRADING,
};
				  ...
				  const union oplus_chg_mod_propval *pval)
{
	...
	case OPLUS_CHG_PROP_CURRENT_MAX:
		if (wls_status->fastchg_started)
			rc = oplus_vote(wls_dev->fcc_votable, MAX_VOTER, true,
	...
	case OPLUS_CHG_PROP_CHG_ENABLE:
		...
		oplus_vote(wls_dev->rx_disable_votable, DEBUG_VOTER,
	...
	case OPLUS_CHG_PROP_RX_VOUT_UVP:
		if (!!pval->intval) {
			oplus_vote(wls_dev->nor_input_disable_votable,
				   UOVP_VOTER, true, 0, false);
		} else {
			oplus_vote(wls_dev->nor_input_disable_votable,
					     ...
					     unsigned long val, void *v)
{
	...
	case OPLUS_CHG_EVENT_OTG_ENABLE:
		oplus_vote(wls_dev->wrx_en_votable, OTG_EN_VOTER, true, 1,
			   false);
		break;
	case OPLUS_CHG_EVENT_OTG_DISABLE:
		oplus_vote(wls_dev->wrx_en_votable, OTG_EN_VOTER, false, 0,
			   false);
		break;
					   ...
					   unsigned long val, void *v)
{
	...
	case OPLUS_CHG_EVENT_POWER_CHANGED:
		temp_region = oplus_chg_comm_get_temp_region(wls_dev->comm_ocm);
		if ((BATT_TEMP_LITTLE_COLD <= temp_region) &&
	...
	case OPLUS_CHG_EVENT_CALL_ON:
		pr_info("call on\n");
		oplus_vote(wls_dev->fcc_votable, CALL_VOTER, true,
	...
	case OPLUS_CHG_EVENT_CAMERA_ON:
		pr_info("camera on\n");
		oplus_vote(wls_dev->fcc_votable, CAMERA_VOTER, true,
	...
	case OPLUS_CHG_EVENT_RX_IIC_ERR:
		if (wls_status->rx_present) {
			...
			oplus_vote(wls_dev->rx_disable_votable, RX_IIC_VOTER,
				   true, 1, false);
	...
	case OPLUS_CHG_EVENT_RX_FAST_ERR:
		if (wls_status->rx_present) {
			...
			schedule_delayed_work(&wls_dev->fast_fault_check_work,
					      0);
		}
		break;
	case OPLUS_CHG_EVENT_TX_EPP_CAP:
		...
		if (rx_mode == OPLUS_CHG_WLS_RX_MODE_EPP_5W ||
			...
			oplus_chg_wls_nor_set_vindpm(wls_dev->wls_nor,
						     WLS_VINDPM_EPP);
		break;
	default:
		break;
...
static int oplus_chg_wls_init_mod(struct oplus_chg_wls *wls_dev)
{
	...
	wls_dev->wls_aes_nb.notifier_call =
		oplus_chg_wls_aes_mutual_notifier_call;
...
static void oplus_chg_wls_usb_int_work(struct work_struct *work)
{
	...
	if (wls_dev->usb_present) {
		oplus_vote(wls_dev->rx_disable_votable, USB_VOTER, true, 1,
			   false);
		...
		oplus_vote(wls_dev->rx_disable_votable, USB_VOTER, false, 0,
			   false);
	}
}
...
static void oplus_chg_wls_start_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev =
		container_of(dwork, struct oplus_chg_wls, wls_start_work);
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
...
static void oplus_chg_wls_break_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev =
		container_of(dwork, struct oplus_chg_wls, wls_break_work);
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
...
static void oplus_chg_wls_connect_work(struct work_struct *work)
{
	...
	if (wls_status->rx_online) {
...
#endif
#ifdef CONFIG_OPLUS_CHARGER_MTK
		oplus_vote(wls_dev->wrx_en_votable, WLS_ONLINE_VOTER, true, 1,
			   false);
#endif
		oplus_chg_wls_reset_variables(wls_dev);
		/*
		...
		oplus_vote(wls_dev->nor_fcc_votable, USER_VOTER, true, 1500,
			   false);
		oplus_rerun_election(wls_dev->nor_fcc_votable, false);
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 100,
			   false);
		oplus_rerun_election(wls_dev->nor_icl_votable, false);
		/* reset charger status */
		oplus_vote(wls_dev->nor_out_disable_votable, USER_VOTER, true,
			   1, false);
		oplus_vote(wls_dev->nor_out_disable_votable, USER_VOTER, false,
			   0, false);
		...
		oplus_vote(wls_dev->rx_disable_votable, CONNECT_VOTER, true, 1,
			   false);
...
#ifdef CONFIG_OPLUS_CHARGER_MTK
			msleep(100);
			oplus_vote(wls_dev->wrx_en_votable, WLS_ONLINE_VOTER,
				   false, 0, false);
#endif
		}
		oplus_chg_wls_nor_set_vindpm(wls_dev->wls_nor,
		...
		oplus_vote(wls_dev->rx_disable_votable, CONNECT_VOTER, false, 0,
			   false);
		oplus_chg_wls_reset_variables(wls_dev);
...
static void oplus_chg_wls_fast_switch_next_step(struct oplus_chg_wls *wls_dev)
{
	...
	if (wls_status->fastchg_level >= fcc_chg->max_step) {
		if (batt_vol_mv >= batt_vol_max) {
			...
			oplus_chg_wls_track_set_prop(wls_dev,
...
static int oplus_chg_wls_fast_temp_check(struct oplus_chg_wls *wls_dev)
{
	...
	if ((temp_region < BATT_TEMP_LITTLE_COLD) ||
		...
		oplus_vote(wls_dev->fastchg_disable_votable, FCC_VOTER, true, 1,
			   false);
		return -EPERM;
	}
	rc = oplus_chg_wls_get_vbat(wls_dev, &batt_vol_mv);
	...
	if (def_curr_ma <= 0)
		def_curr_ma =
			oplus_get_client_vote(wls_dev->fcc_votable, MAX_VOTER);
	else
		def_curr_ma = min(oplus_get_client_vote(wls_dev->fcc_votable,
							MAX_VOTER),
				  def_curr_ma);
	...
	if (wls_status->fastchg_level == 0) {
		...
		oplus_vote(wls_dev->fcc_votable, FCC_VOTER, true,
			   fcc_chg->fcc_step[wls_status->fastchg_level].curr_ma,
			   false);
	} else if (wls_status->fastchg_level >=
		   fcc_chg->max_step) { // switch to pmic
		oplus_vote(wls_dev->fastchg_disable_votable, FCC_VOTER, true, 1,
			   false);
		return -EPERM;
	} else {
	...
	if (!wls_status->wait_cep_stable) {
		...
		if (cep < 3) {
			...
			if ((cep_ok_count >= CEP_OK_MAX) &&
			    ...
			    oplus_is_client_vote_enabled(wls_dev->fcc_votable,
							 CEP_VOTER)) {
				...
				wait_cep_count = 0;
				oplus_vote(wls_dev->fcc_votable, CEP_VOTER,
					   false, 0, false);
			}
		} else {
			cep_ok_count = 0;
			...
			if (cep_err_count >= CEP_ERR_MAX) {
				...
				wls_status->wait_cep_stable = true;
				wait_cep_count = 0;
				if (oplus_is_client_vote_enabled(
					    wls_dev->fcc_votable, CEP_VOTER))
					cep_curr_ma = oplus_get_client_vote(
						wls_dev->fcc_votable,
						CEP_VOTER);
				else
					cep_curr_ma = 0;
				if ((cep_curr_ma > 0) &&
					...
					oplus_vote(wls_dev->fcc_votable,
						   CEP_VOTER, true, curr_ma,
						   false);
				}
				wls_status->cep_ok_wait_timeout =
					jiffies + CEP_OK_TIMEOUT_MAX * HZ;
...
static int oplus_chg_wls_fast_ibat_check(struct oplus_chg_wls *wls_dev)
{
	...
	if (ibat_ma >= WLS_FASTCHG_CURR_DISCHG_MAX_MA) {
		...
		oplus_vote(wls_dev->fastchg_disable_votable, BATT_CURR_VOTER,
			   true, 1, false);
		return -1;
	}
...
static int oplus_chg_wls_fast_iout_check(struct oplus_chg_wls *wls_dev)
{
	...
	if (iout_ma <= WLS_FASTCHG_IOUT_CURR_MIN_MA) {
		...
		if (wls_status->iout_ma_conunt >= IOUT_SMALL_COUNT) {
			...
			oplus_vote(wls_dev->fastchg_disable_votable,
				   IOUT_CURR_VOTER, true, 1, false);
			return -1;
		}
	} else {
		wls_status->iout_ma_conunt = 0;
...
static void oplus_chg_wls_fast_skin_temp_check(struct oplus_chg_wls *wls_dev)
{
	...
	if (wls_status->led_on) {
		...
		if (wls_dev->wls_fast_chg_led_on_strategy.initialized) {
			...
			oplus_vote(wls_dev->fcc_votable, SKIN_VOTER, true,
				   curr_ma, false);
		}
#else
		oplus_vote(wls_dev->fcc_votable, SKIN_VOTER, false, 0, false);
...
static void oplus_chg_wls_set_quiet_mode(struct oplus_chg_wls *wls_dev,
					 bool quiet_mode)
{
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
	...
	if (wls_status->adapter_id == WLS_ADAPTER_MODEL_0)
		(void)oplus_chg_wls_send_msg(wls_dev,
					     quiet_mode ?
						     WLS_CMD_SET_QUIET_MODE :
						     WLS_CMD_SET_NORMAL_MODE,
					     0xff, 2);
	else
		(void)oplus_chg_wls_send_msg(wls_dev, WLS_CMD_SET_FAN_SPEED,
					     quiet_mode ? fan_pwm_pulse_silent :
							  fan_pwm_pulse_fastchg,
					     2);

	(void)oplus_chg_wls_send_msg(
		wls_dev, WLS_CMD_SET_LED_BRIGHTNESS,
...
static void oplus_chg_wls_check_quiet_mode(struct oplus_chg_wls *wls_dev)
{
	...
	if (wls_status->switch_quiet_mode) {
		if ((!wls_status->quiet_mode ||
			...
			(void)oplus_chg_wls_set_quiet_mode(
				wls_dev, wls_status->switch_quiet_mode);
	} else {
		if (wls_status->quiet_mode ||
		    wls_status->quiet_mode_init == false)
			(void)oplus_chg_wls_set_quiet_mode(
...
static void oplus_chg_wls_check_term_charge(struct oplus_chg_wls *wls_dev)
{
	...
	curr_ma =
		oplus_get_client_vote(wls_dev->nor_icl_votable, CHG_DONE_VOTER);
	rc = oplus_chg_wls_get_skin_temp(wls_dev, &skin_temp);
	if (rc < 0) {
		pr_err("can't get skin temp, rc=%d\n", rc);
		skin_temp = 250;
	...
	if (wls_status->chg_done) {
		if (curr_ma <= 0) {
			...
			oplus_vote(wls_dev->nor_icl_votable, CHG_DONE_VOTER,
				   true, WLS_CURR_STOP_CHG_MA, true);
		}
...
static int oplus_chg_wls_fastchg_restart_check(struct oplus_chg_wls *wls_dev)
{
	...
	if (oplus_is_client_vote_enabled(wls_dev->fastchg_disable_votable,
					 RX_FAST_ERR_VOTER) &&
	    (wls_status->fastchg_err_count < 2) &&
	    time_is_before_jiffies(wls_status->fastchg_err_timer))
		oplus_vote(wls_dev->fastchg_disable_votable, RX_FAST_ERR_VOTER,
			   false, 0, false);

	if (oplus_is_client_vote_enabled(wls_dev->fastchg_disable_votable,
					 FCC_VOTER)) {
		oplus_vote(wls_dev->fastchg_disable_votable, FCC_VOTER, false,
			   0, false);
	}
	...
	if (oplus_is_client_vote_enabled(wls_dev->fastchg_disable_votable,
		...
		oplus_vote(wls_dev->fastchg_disable_votable, STARTUP_CEP_VOTER,
			   false, 0, false);

	if (oplus_is_client_vote_enabled(wls_dev->fastchg_disable_votable,
					 QUIET_VOTER))
		oplus_vote(wls_dev->fastchg_disable_votable, QUIET_VOTER, false,
			   0, false);
...
static int oplus_chg_wls_set_non_ffc_current(struct oplus_chg_wls *wls_dev)
{
	...
	if (dynamic_cfg->non_ffc_step[i][wls_status->non_fcc_level].curr_ma >
	    0) {
		if ((wls_status->non_fcc_level == non_ffc_max_step - 1) &&
		    (wls_status->non_fcc_level > 0)) {
			if (batt_vol_mv <=
			    dynamic_cfg->non_ffc_step
					    [i][wls_status->non_fcc_level - 1]
						    .vol_max_mv -
				    50)
				fallback_count++;
			else
			...
			if (fallback_count >= FALLBACK_COUNT_MAX) {
				wls_status->non_fcc_level =
					non_ffc_max_step - 2;
				oplus_vote(
					wls_dev->nor_icl_votable, NON_FFC_VOTER,
					true,
					dynamic_cfg
						->non_ffc_step
							[i]
			...
			fallback_count = 0;
			oplus_vote(
				wls_dev->nor_icl_votable, NON_FFC_VOTER, true,
				dynamic_cfg
					->non_ffc_step[i]
						      [wls_status->non_fcc_level]
					.curr_ma,
	...
	if (batt_vol_mv >= dynamic_cfg->cv_step[i][j].vol_max_mv &&
	    cv_icl > 0) {
		oplus_vote(wls_dev->nor_icl_votable, CV_VOTER, true, cv_icl,
			   true);
	}
...
static int
oplus_chg_wls_get_third_adapter_ext_cmd_p_id(struct oplus_chg_wls *wls_dev)
{
	int rc = 0;
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
	int try_count = 0;
	...
	if (!wls_status->tx_extern_cmd_done) {
		rc = oplus_chg_wls_send_msg(wls_dev, WLS_CMD_GET_EXTERN_CMD,
					    0xff, 5);
		if (rc < 0) {
	...
	if (!wls_status->tx_product_id_done) {
		...
		do {
			rc = oplus_chg_wls_send_data(
				wls_dev, WLS_CMD_GET_PRODUCT_ID, buf, 5);
			if (rc < 0) {
				if (rc != -EAGAIN)
					try_count++;
				msleep(200);
...
static int oplus_chg_wls_get_third_adapter_v_id(struct oplus_chg_wls *wls_dev)
{
	...
	if (!wls_status->verify_by_aes) {
		rc = oplus_chg_wls_send_msg(wls_dev, WLS_CMD_GET_VENDOR_ID,
					    0xff, 5);
		if (rc < 0) {
...
static int oplus_chg_wls_rx_handle_state_default(struct oplus_chg_wls *wls_dev)
{
	...
	oplus_vote(wls_dev->nor_input_disable_votable, USER_VOTER, true, 0,
		   false);

	if (wls_status->verity_pass) {
		(void)oplus_chg_wls_send_msg(wls_dev, WLS_CMD_INDENTIFY_ADAPTER,
					     0xff, 0);
...
static int oplus_chg_wls_rx_exit_state_default(struct oplus_chg_wls *wls_dev)
{
	...
	case OPLUS_CHG_WLS_RX_STATE_FAST:
		...
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 500,
			   false);
		break;
...
static int oplus_chg_wls_rx_enter_state_bpp(struct oplus_chg_wls *wls_dev)
{
	...
	case 0:
		oplus_vote(wls_dev->nor_fcc_votable, USER_VOTER, true, 1200,
			   false);
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 750,
			   true);
	...
	switch (wls_status->state_sub_step) {
	case 0:
		oplus_vote(wls_dev->nor_fcc_votable, USER_VOTER, true, 2000,
			   false);
		wait_time_ms = 300;
		wls_status->state_sub_step = 1;
		break;
	case 1:
		oplus_chg_wls_nor_set_aicl_enable(wls_dev->wls_nor, true);
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 200,
			   false);
		wait_time_ms = 300;
		wls_status->state_sub_step = 2;
		break;
	case 2:
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 400,
			   false);
		wait_time_ms = 300;
		wls_status->state_sub_step = 3;
		break;
	case 3:
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 700,
			   false);
		wait_time_ms = 300;
		wls_status->state_sub_step = 4;
		break;
	case 4:
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 1000,
			   true);
		wls_status->state_sub_step = 0;
		wls_status->current_rx_state = OPLUS_CHG_WLS_RX_STATE_BPP;
		break;
...
static int oplus_chg_wls_rx_handle_state_bpp(struct oplus_chg_wls *wls_dev)
{
	...
	if (wls_dev->factory_mode && wls_dev->support_get_tx_pwr) {
		...
		if (wls_status->adapter_info_cmd_count < A1_COUNT_MAX &&
	...
	if (wls_dev->batt_charge_enable) {
		if (!!oplus_get_client_vote(wls_dev->nor_out_disable_votable,
					    STOP_VOTER)) {
			oplus_vote(wls_dev->nor_icl_votable, STOP_VOTER, false,
				   0, true);
			oplus_vote(wls_dev->nor_out_disable_votable, STOP_VOTER,
				   false, 0, false);
		}
	} else {
		if (!oplus_get_client_vote(wls_dev->nor_out_disable_votable,
					   STOP_VOTER)) {
			oplus_vote(wls_dev->nor_icl_votable, STOP_VOTER, true,
	...
	case WLS_ADAPTER_TYPE_PD_65W:
		...
		if (wls_dev->support_fastchg) {
			...
			rc = oplus_chg_wls_get_real_soc(wls_dev, &real_soc);
			if ((rc < 0) ||
			    (real_soc >= dynamic_cfg->fastchg_max_soc)) {
				pr_err("can't get real soc or soc is too high, rc=%d\n",
				       rc);
				wls_status->target_rx_state =
					OPLUS_CHG_WLS_RX_STATE_DONE;
...
static int oplus_chg_wls_rx_enter_state_epp(struct oplus_chg_wls *wls_dev)
{
	...
	switch (wls_status->state_sub_step) {
	case 0:
		oplus_vote(wls_dev->nor_fcc_votable, USER_VOTER, true, 2800,
			   false);
		if (wls_status->adapter_type == WLS_ADAPTER_TYPE_VOOC ||
			...
			(void)oplus_chg_wls_rx_set_vout(
				wls_dev->wls_rx, dynamic_cfg->vooc_vol_mv, 0);
			if (wls_dev->support_fastchg)
				(void)oplus_chg_wls_send_msg(
			...
			(void)oplus_chg_wls_rx_set_vout(
				wls_dev->wls_rx, dynamic_cfg->vooc_vol_mv, 15);

			if (wls_status->pwr_max_mw > 0) {
				oplus_vote(wls_dev->nor_icl_votable,
					   RX_MAX_VOTER, true,
					   wls_status->pwr_max_mw * 1000 /
						   dynamic_cfg->vooc_vol_mv,
					   false);
			}
		}
		wait_time_ms = 100;
	...
	case 1:
		if (wls_status->adapter_type == WLS_ADAPTER_TYPE_VOOC ||
			...
			if (wls_status->vout_mv > 11500) {
				...
				oplus_chg_wls_nor_set_vindpm(
					wls_dev->wls_nor, WLS_VINDPM_AIRVOOC);
				oplus_vote(wls_dev->nor_icl_votable, USER_VOTER,
					   true, icl_max_ma, true);
				wls_status->state_sub_step = 0;
				wls_status->current_rx_state =
					OPLUS_CHG_WLS_RX_STATE_EPP;
			} else {
	...
	case 2:
		...
		break;
	case 3:
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 900,
			   false);
		wls_status->state_sub_step = 0;
		wls_status->current_rx_state = OPLUS_CHG_WLS_RX_STATE_EPP;
		break;
	...
	if (wls_status->current_rx_state == OPLUS_CHG_WLS_RX_STATE_EPP)
		schedule_delayed_work(&wls_dev->wls_skewing_work,
				      msecs_to_jiffies(SKEWING_WORK_DELAY));
...
static int oplus_chg_wls_rx_handle_state_epp(struct oplus_chg_wls *wls_dev)
{
	...
	if (wls_status->epp_5w || wls_status->vout_mv < 8000) {
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 450,
			   true);
...
static int oplus_chg_wls_rx_enter_state_epp_plus(struct oplus_chg_wls *wls_dev)
{
	...
	switch (wls_status->state_sub_step) {
	case 0:
		oplus_vote(wls_dev->nor_fcc_votable, USER_VOTER, true, 4000,
			   false);
		if (wls_status->adapter_type == WLS_ADAPTER_TYPE_VOOC ||
		    wls_status->adapter_type == WLS_ADAPTER_TYPE_SVOOC ||
		    wls_status->adapter_type == WLS_ADAPTER_TYPE_PD_65W) {
	...
	switch (wls_status->state_sub_step) {
	case 0:
		oplus_vote(wls_dev->nor_fcc_votable, USER_VOTER, true, 3000,
			   false);
		wait_time_ms = 300;
		wls_status->state_sub_step = 1;
		break;
	...
	case 3:
		...
		break;
	case 4:
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 1250,
			   false);
		wls_status->state_sub_step = 0;
		wls_status->current_rx_state = OPLUS_CHG_WLS_RX_STATE_EPP_PLUS;
		break;
...
static int oplus_chg_wls_rx_enter_state_fast(struct oplus_chg_wls *wls_dev)
{
	...
	case OPLUS_CHG_WLS_FAST_SUB_STATE_WAIT_FAST:
		...
		oplus_vote(wls_dev->nor_input_disable_votable, USER_VOTER,
			   false, 0, false);
		if (wls_status->rx_online)
		...
		if (wls_status->rx_online &&
		    wls_status->adapter_id == WLS_ADAPTER_THIRD_PARTY) {
			rc = oplus_chg_wls_get_third_adapter_ext_cmd_p_id(
				wls_dev);
			if (rc < 0) {
				if (!wls_status->rx_online)
					return 0;
				...
				wls_status->online_keep = true;
				oplus_vote(wls_dev->rx_disable_votable,
					   VERITY_VOTER, true, 1, false);
				schedule_delayed_work(
					&wls_dev->rx_verity_restore_work,
					msecs_to_jiffies(500));
				return 500;
			}
		}
	...
	case OPLUS_CHG_WLS_FAST_SUB_STATE_WAIT_IOUT:
		if (wls_status->iout_ma < 100) {
			if (curr_err_count == CURR_ERR_COUNT_MAX / 2)
				oplus_chg_wls_nor_set_aicl_rerun(
					wls_dev->wls_nor);
			curr_err_count++;
			delay_ms = 100;
		} else {
	...
	case OPLUS_CHG_WLS_FAST_SUB_STATE_WAIT_VOUT:
		rc = oplus_chg_wls_rx_set_vout_step(
			wls_dev->wls_rx, 4 * vbat_mv + CP_OPEN_OFFSET + 200,
			1000, 50);
		if (rc < 0) {
			...
			wls_status->fastchg_retry_timer =
				jiffies + (unsigned long)(300 * HZ); //5 min
			oplus_vote(wls_dev->fastchg_disable_votable,
	...
	case OPLUS_CHG_WLS_FAST_SUB_STATE_CHECK_IOUT:
		...
		if (vout_mv < vbat_mv * 4 + CP_OPEN_OFFSET) {
			...
			(void)oplus_chg_wls_rx_set_vout_step(
				wls_dev->wls_rx,
				4 * vbat_mv + CP_OPEN_OFFSET + 300, 1000, 10);
			delay_ms = 0;
			break;
		}
	...
	case OPLUS_CHG_WLS_FAST_SUB_STATE_START:
		...
		if ((wls_status->wls_type == OPLUS_CHG_WLS_SVOOC) ||
			...
			if (wls_status->wls_type == OPLUS_CHG_WLS_SVOOC &&
			    ...
			    wls_status->adapter_power <=
				    WLS_ADAPTER_POWER_45W_MAX_ID)
				iout_max_ma = WLS_FASTCHG_CURR_45W_MAX_MA;
			oplus_vote(wls_dev->fcc_votable, MAX_VOTER, true,
				   iout_max_ma, false);
		} else {
			wls_status->fastchg_ibat_max_ma =
		...
		if (!wls_status->fastchg_restart) {
			for (i = 0; i < fcc_chg->max_step; i++) {
				if ((batt_temp <
				     fcc_chg->fcc_step[i].high_threshold) &&
...
static int oplus_chg_wls_rx_handle_state_fast(struct oplus_chg_wls *wls_dev)
{
	...
	if ((wls_status->iout_ma > 500) &&
		...
		oplus_vote(wls_dev->nor_input_disable_votable, USER_VOTER, true,
			   0, false);
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 0,
			   false);
	}
...
static int oplus_chg_wls_rx_exit_state_fast(struct oplus_chg_wls *wls_dev)
{
	...
	oplus_vote(wls_dev->fcc_votable, EXIT_VOTER, true,
		   WLS_FASTCHG_CURR_EXIT_MA, false);
...
static int oplus_chg_wls_rx_enter_state_ffc(struct oplus_chg_wls *wls_dev)
{
	...
	switch (wls_status->state_sub_step) {
	case 0:
		oplus_vote(wls_dev->nor_out_disable_votable, FFC_VOTER, true, 1,
			   false);
	...
	case 1:
		...
		oplus_vote(wls_dev->nor_out_disable_votable, FFC_VOTER, false,
			   0, false);
		if (is_comm_ocm_available(wls_dev)) {
...
static int oplus_chg_wls_rx_enter_state_done(struct oplus_chg_wls *wls_dev)
{
	...
	case 2:
		if (wls_status->vout_mv > 11500) {
			oplus_vote(wls_dev->nor_fcc_votable, USER_VOTER, true,
				   3600, false);
			oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true,
				   950, true);
			oplus_chg_wls_set_non_ffc_current(wls_dev);
...
static int oplus_chg_wls_rx_enter_state_stop(struct oplus_chg_wls *wls_dev)
{
	...
	case 1:
		...
		(void)oplus_chg_wls_rx_set_vout(wls_dev->wls_rx,
						WLS_VOUT_FASTCHG_INIT_MV, -1);
		oplus_vote(wls_dev->nor_icl_votable, STOP_VOTER, true, 300,
			   true);
		oplus_vote(wls_dev->nor_out_disable_votable, STOP_VOTER, true,
			   1, false);
...
static int oplus_chg_wls_rx_handle_state_stop(struct oplus_chg_wls *wls_dev)
{
	...
	if (wls_dev->batt_charge_enable) {
		oplus_vote(wls_dev->nor_icl_votable, STOP_VOTER, false, 0,
			   true);
		oplus_vote(wls_dev->nor_out_disable_votable, STOP_VOTER, false,
			   0, false);
		if (wls_status->switch_quiet_mode)
			wls_status->target_rx_state =
				OPLUS_CHG_WLS_RX_STATE_QUIET;
...
static int oplus_chg_wls_rx_enter_state_debug(struct oplus_chg_wls *wls_dev)
{
	...
	case OPLUS_CHG_WLS_FORCE_TYPE_EPP:
		...
		(void)oplus_chg_wls_rx_set_vout(wls_dev->wls_rx,
						WLS_VOUT_EPP_MV, -1);
		msleep(500);
		oplus_vote(wls_dev->nor_icl_votable, USER_VOTER, true, 850,
			   false);
		if (wls_dev->factory_mode)
			(void)oplus_chg_wls_send_msg(
				wls_dev, WLS_CMD_GET_TX_PWR, 0xff, -1);
...
static void oplus_chg_wls_trx_sm(struct work_struct *work)
{
	...
	if (trx_status & WLS_TRX_STATUS_READY) {
		...
		if (!wls_status->trx_usb_present_once)
			wls_status->trx_usb_present_once =
				oplus_chg_wls_is_usb_present(wls_dev);
		if (is_batt_psy_available(wls_dev))
			power_supply_changed(wls_dev->batt_psy);
	...
	if (pre_trx_online && !wls_status->trx_online) {
		...
		pr_info("trx_online:%d, trx_start_time:%d, trx_end_time:%d,"
			...
			wls_status->trx_transfer_end_time,
			wls_status->trx_usb_present_once);
		if (wls_status->trx_transfer_end_time -
			    wls_status->trx_transfer_start_time >
		    WLS_TRX_INFO_UPLOAD_THD_2MINS) {
			oplus_chg_wls_update_track_info(wls_dev, trx_crux_info,
							false);
...
static void oplus_chg_wls_upgrade_fw_work(struct work_struct *work)
{
	...
	if (boot_mode == META_BOOT || boot_mode == FACTORY_BOOT ||
		...
		oplus_vote(wls_dev->rx_disable_votable, UPGRADE_VOTER, true, 1,
			   false);
		msleep(1000);
		oplus_vote(wls_dev->rx_disable_votable, UPGRADE_VOTER, false, 0,
			   false);
		retry_num = 0;
		return;
	}
	...
	if (wls_dev->fw_upgrade_by_buf) {
		...
		oplus_vote(wls_dev->rx_disable_votable, UPGRADE_VOTER, true, 1,
			   false);
		oplus_vote(wls_dev->wrx_en_votable, UPGRADE_FW_VOTER, true, 1,
			   false);
		rc = oplus_chg_wls_rx_upgrade_firmware_by_buf(
			wls_dev->wls_rx, wls_dev->fw_buf, wls_dev->fw_size);
		oplus_vote(wls_dev->wrx_en_votable, UPGRADE_FW_VOTER, false, 0,
			   false);
		oplus_vote(wls_dev->rx_disable_votable, UPGRADE_VOTER, false, 0,
			   false);
		if (rc < 0)
...
static void oplus_chg_wls_data_update_work(struct work_struct *work)
{
	...
	pr_info("Iout: target=%d, out=%d, ibat_max=%d, ibat=%d; vout=%d; vrect=%d; cep=%d\n",
		wls_status->fastchg_target_curr_ma, wls_status->iout_ma,
		wls_status->fastchg_ibat_max_ma, ibat_ma, wls_status->vout_mv,
		wls_status->vrect_mv, cep);
...
static void oplus_chg_wls_skewing_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev =
		container_of(dwork, struct oplus_chg_wls, wls_skewing_work);
	struct oplus_chg_wls_dynamic_config *dynamic_cfg =
		&wls_dev->dynamic_config;
	struct oplus_chg_wls_status *wls_status = &wls_dev->wls_status;
	int cep = 0;
	...
	if (cep < CEP_SKEWING_VALUE) {
		...
		cep_err_count = 0;
		if (cep_ok_count >= CEP_OK_MAX &&
		    dynamic_cfg->skewing_step[i][wls_status->skewing_level]
			    .fallback_step &&
	...
	skewing_icl =
		dynamic_cfg->skewing_step[i][wls_status->skewing_level].curr_ma;
	if (skewing_icl > 0 && pre_level != wls_status->skewing_level)
		oplus_vote(wls_dev->nor_icl_votable, CEP_VOTER, true,
			   skewing_icl, true);
...
out:
	schedule_delayed_work(&wls_dev->wls_skewing_work,
			      msecs_to_jiffies(SKEWING_INTERVAL));
}
...
static void oplus_chg_wls_fast_fault_check_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev = container_of(
		dwork, struct oplus_chg_wls, fast_fault_check_work);
	char *fast_fault;
	int rc;
	...
	kfree(fast_fault);
alloc_error:
	oplus_vote(wls_dev->fastchg_disable_votable, RX_FAST_ERR_VOTER, true, 1,
		   false);
}
...
static int oplus_chg_wls_parse_dt(struct oplus_chg_wls *wls_dev)
{
	...
	wls_dev->support_tx_boost =
		of_property_read_bool(node, "oplus,support_tx_boost");
	...
	rc = of_property_count_elems_of_size(node, "oplus,fastchg-match-q",
					     sizeof(u8));
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "oplus,verity_curr_max_ma",
				  &dynamic_cfg->verity_curr_max_ma);
	if (rc < 0) {
	...
	wls_strategy_node =
		of_get_child_by_name(node, "wireless_fastchg_strategy");
	...
	for (i = 0; i < WLS_FAST_SOC_MAX; i++) {
		soc_strategy_node = of_get_child_by_name(wls_strategy_node,
							 strategy_soc[i]);
		...
		for (j = 0; j < WLS_FAST_TEMP_MAX; j++) {
			rc = of_property_count_elems_of_size(soc_strategy_node,
							     strategy_temp[j],
							     sizeof(u32));
			if (rc < 0) {
				pr_err("Count %s failed, rc=%d\n",
	...
	for (i = 0; i < WLS_FAST_SOC_MAX; i++) {
		for (j = 0; j < WLS_FAST_TEMP_MAX; j++) {
			for (k = 0;
			     k < wls_dev->fcc_steps[i].fcc_step[j].max_step;
			     k++) {
				pr_err("%s: %d %d %d %d %d\n", __func__,
				       wls_dev->fcc_steps[i]
					       .fcc_step[j]
	...
	wls_third_part_strategy_node = of_get_child_by_name(
		node, "wireless_fastchg_third_part_strategy");
	if (!wls_third_part_strategy_node) {
		pr_err("Can not find wls third_part_strategy node\n");
		memcpy(wls_dev->fcc_third_part_steps, wls_dev->fcc_steps,
		       sizeof(wls_dev->fcc_steps));
		goto non_ffc;
	}
	...
	for (i = 0; i < WLS_FAST_SOC_MAX; i++) {
		...
		for (j = 0; j < WLS_FAST_TEMP_MAX; j++) {
			...
			rc = of_property_read_u32_array(
				soc_strategy_node, strategy_temp[j],
				(u32 *)wls_dev->fcc_third_part_steps[i]
					.fcc_step[j]
					.fcc_step,
				length);
			wls_dev->fcc_third_part_steps[i].fcc_step[j].max_step =
				length / 5;
		}
	}
	...
	for (i = 0; i < WLS_FAST_SOC_MAX; i++) {
		for (j = 0; j < WLS_FAST_TEMP_MAX; j++) {
			for (k = 0; k < wls_dev->fcc_third_part_steps[i]
						.fcc_step[j]
						.max_step;
			     k++) {
				pr_err("third_part %s: %d %d %d %d %d\n",
				       __func__,
				       wls_dev->fcc_third_part_steps[i]
					       .fcc_step[j]
					       .fcc_step[k]
					       .low_threshold,
				       wls_dev->fcc_third_part_steps[i]
...
non_ffc:
	/* non-ffc */
	for (i = 0; i < OPLUS_WLS_CHG_MODE_MAX; i++) {
		rc = of_property_count_elems_of_size(node, strategy_non_ffc[i],
						     sizeof(u32));
		if (rc < 0) {
			pr_err("Count %s failed, rc=%d\n", strategy_non_ffc[i],
			       rc);
	...
	/* cv */
	for (i = 0; i < OPLUS_WLS_CHG_MODE_MAX; i++) {
		rc = of_property_count_elems_of_size(node, strategy_cv[i],
						     sizeof(u32));
		if (rc < 0) {
			pr_err("Count %s failed, rc=%d\n", strategy_cv[i], rc);
			return rc;
	...
	/* cool down */
	rc = of_property_read_u32(node, "oplus,cool-down-12v-thr",
				  &dynamic_cfg->cool_down_12v_thr);
	if (rc < 0) {
	...
	for (i = 0; i < OPLUS_WLS_CHG_MODE_MAX; i++) {
		rc = of_property_count_elems_of_size(
			node, strategy_cool_down[i], sizeof(u32));
		if (rc < 0) {
			pr_err("Count %s failed, rc=%d\n",
	...
	for (i = 0; i < OPLUS_WLS_SKEWING_MAX; i++) {
		rc = of_property_count_elems_of_size(node, strategy_skewing[i],
						     sizeof(u32));
		if (rc < 0) {
			pr_err("Count %s failed, rc=%d\n", strategy_skewing[i],
			       rc);
	...
	rc = of_property_read_u32(node, "oplus,vooc-vol-mv",
				  &dynamic_cfg->vooc_vol_mv);
	if (rc < 0) {
		pr_err("oplus,vooc-vol-mv reading failed, rc=%d\n", rc);
		dynamic_cfg->vooc_vol_mv = 10000;
	}
	rc = of_property_read_u32(node, "oplus,svooc-vol-mv",
				  &dynamic_cfg->svooc_vol_mv);
	if (rc < 0) {
...
static int oplus_chg_wls_gpio_init(struct oplus_chg_wls *wls_dev)
{
	...
	wls_dev->ext_pwr_en_gpio =
		of_get_named_gpio(node, "oplus,ext_pwr_en-gpio", 0);
	...
	wls_dev->ext_pwr_en_active =
		pinctrl_lookup_state(wls_dev->pinctrl, "ext_pwr_en_active");
	if (IS_ERR_OR_NULL(wls_dev->ext_pwr_en_active)) {
		pr_err("get ext_pwr_en_active fail\n");
		goto free_ext_pwr_en_gpio;
	}
	wls_dev->ext_pwr_en_sleep =
		pinctrl_lookup_state(wls_dev->pinctrl, "ext_pwr_en_sleep");
...
static void
oplus_chg_wls_track_trx_info_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_wls *wls_dev = container_of(
		dwork, struct oplus_chg_wls, trx_info_load_trigger_work);
...
static int oplus_chg_wls_driver_probe(struct platform_device *pdev)
{
	...
	wls_dev->fcc_votable = oplus_create_votable(
	...
	wls_dev->fastchg_disable_votable = oplus_create_votable(
		"WLS_FASTCHG_DISABLE", VOTE_SET_ANY,
	...
	wls_dev->wrx_en_votable =
		oplus_create_votable("WLS_WRX_EN", VOTE_SET_ANY,
	...
	wls_dev->nor_icl_votable =
		oplus_create_votable("WLS_NOR_ICL", VOTE_MIN,
	...
	wls_dev->nor_fcc_votable =
		oplus_create_votable("WLS_NOR_FCC", VOTE_MIN,
	...
	wls_dev->nor_fv_votable =
		oplus_create_votable("WLS_NOR_FV", VOTE_MIN,
	...
	wls_dev->nor_out_disable_votable = oplus_create_votable(
		"WLS_NOR_OUT_DISABLE", VOTE_SET_ANY,
	...
	wls_dev->nor_input_disable_votable = oplus_create_votable(
		"WLS_NOR_INPUT_DISABLE", VOTE_SET_ANY,
	...
	wls_dev->rx_disable_votable =
		oplus_create_votable("WLS_RX_DISABLE", VOTE_SET_ANY,
	...
	INIT_DELAYED_WORK(&wls_dev->fast_fault_check_work,
			  oplus_chg_wls_fast_fault_check_work);
...
#endif
	INIT_DELAYED_WORK(&wls_dev->wls_skewing_work,
			  oplus_chg_wls_skewing_work);
