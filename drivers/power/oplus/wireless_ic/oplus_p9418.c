...
int p9418_hall_notifier_callback(struct notifier_block *nb, unsigned long event,
				 void *data);
...
static int __p9418_read_reg(struct oplus_p9418_ic *chip, int reg,
...
static int p9418_write_reg_multi_byte(struct oplus_p9418_ic *chip, int reg,
...
static int p9418_read_reg(struct oplus_p9418_ic *chip, int reg,
			  char *returnData, int count)
{
	int ret = 0;
...
static int p9418_config_interface(struct oplus_p9418_ic *chip, int RegNum,
				  int val, int MASK)
{
	char p9418_reg = 0;
	int ret = 0;
...
static void p9418_check_clear_irq(struct oplus_p9418_ic *chip)
{
	...
	if (rc) {
		...
		p9418_write_reg_multi_byte(chip, P9418_REG_INT_CLR,
...
static void check_int_enable(struct oplus_p9418_ic *chip)
{
	...
	if (rc) {
		...
		if ((reg_int[0] != 0xFF) || (reg_int[1] != 0x07)) {
			reg_int[0] = 0xFF;
			reg_int[1] = 0x07;
			p9418_write_reg_multi_byte(chip, P9418_REG_INT_EN,
						   reg_int, 2);
		}
	}
}
...
static void p9418_set_tx_mode(int value)
{
	...
	if (rc) {
		...
		if (value && (P9418_RTX_READY & reg_tx)) {
			...
			p9418_config_interface(chip, P9418_REG_RTX_CMD, 0x01,
					       0xFF);
		} else if (!value && (P9418_RTX_TRANSFER & reg_tx)) {
			chg_err("set tx disable\n");
			p9418_config_interface(chip, P9418_REG_RTX_CMD, 0x04,
					       0xFF);
		} else {
...
static void p9418_set_protect_parameter(struct oplus_p9418_ic *chip)
{
	...
	p9418_config_interface(chip, P9418_REG_OCP_THRESHOLD,
			       chip->ocp_threshold, 0xFF);
	p9418_config_interface(chip, P9418_REG_OVP_THRESHOLD,
			       chip->ovp_threshold, 0xFF);
	p9418_config_interface(chip, P9418_REG_LVP_THRESHOLD,
			       chip->lvp_threshold, 0xFF);
	p9418_config_interface(chip, P9418_REG_POCP_THRESHOLD1,
			       chip->pcop_threshold1, 0xFF);
	p9418_config_interface(chip, P9418_REG_POCP_THRESHOLD2,
			       chip->pcop_threshold2, 0xFF);
	p9418_config_interface(chip, P9418_REG_FOD_THRESHOLD,
			       chip->fod_threshold, 0xFF);
...
static void p9418_send_uevent(struct device *dev, bool status,
...
static int p9418_load_bootloader(struct oplus_p9418_ic *chip)
{
	...
	rc = __p9418_write_reg(chip, 0x3040,
			       0x11); /* Enable MTP access via I2C */
	if (rc != 0) {
	...
	rc = __p9418_write_reg(chip, 0x3048,
...
static int p9418_load_fw(struct oplus_p9418_ic *chip, unsigned char *fw_data,
			 int CodeLength)
{
...
static int p9418_MTP(struct oplus_p9418_ic *chip, unsigned char *fw_buf,
		     int fw_size)
{
	int rc;
	int i, j;
	...
	for (i = 0; i < pure_fw_size; i += 128) {
		...
		for (; j >= 0; j--)
			CheckSum +=
				fw_data[j + 8]; /* add the non zero values */

		CheckSum +=
			CodeLength; /* finish calculation of the check sum */
	...
	rc = __p9418_write_reg(chip, 0x404,
			       pure_fw_size &
				       0xff); /* write FW length low byte */
	if (rc != 0) {
		chg_err("<IDT UPDATE>Write 0x404 reg error!\n");
		return rc;
	}
	rc = __p9418_write_reg(chip, 0x405,
			       (pure_fw_size >> 8) &
...
void update_powerdown_info(struct oplus_p9418_ic *chip, uint8_t type)
{
	...
	chip->charger_done_time =
		(ktime_to_ms(ktime_get()) - chip->tx_start_time) / MIN_TO_MS;
}
...
void p9418_ept_type_detect_func(struct oplus_p9418_ic *chip)
{
	...
	if (rc) {
		...
		if ((recv_data[1] & P9418_RTX_ERR_TX_POCP) ||
			...
			chg_err("ept_type detect error %02x%02x!", recv_data[0],
				recv_data[1]);
			return;
		}
	}
...
static void p9418_idt_int_eint_register(struct oplus_p9418_ic *chip)
{
	...
	retval = request_threaded_irq(chip->idt_int_irq, NULL,
...
void p9418_timer_inhall_function(struct work_struct *work)
{
	...
	while (count++ < P9418_WAIT_TIME && !chip->present &&
	       chip->is_power_on) {
		msleep(100);
	}
...
void power_expired_check_func(struct work_struct *work)
{
	...
	struct oplus_p9418_ic *chip =
		container_of(dwork, struct oplus_p9418_ic, power_check_work);
...
static void p9418_do_switch(struct oplus_p9418_ic *chip, uint8_t status)
{
	...
	if (status == PEN_STATUS_NEAR) { /* hall near, power on*/
		...
		notify_pen_state(0);
		p9418_send_uevent(chip->wireless_dev, chip->present,
				  chip->ble_mac_addr);
	}
...
{
	struct oplus_p9418_ic *chip =
		container_of(work, struct oplus_p9418_ic, power_switch_work);
...
{
	struct oplus_p9418_ic *chip =
		container_of(work, struct oplus_p9418_ic, power_enable_work);
...
static ssize_t p9418_reg_store(struct file *filp, const char __user *buff,
...
static ssize_t p9418_reg_show(struct file *filp, char __user *buff,
...
static ssize_t p9418_data_log_write(struct file *filp, const char __user *buff,
				    size_t len, loff_t *data)
{
	char write_data[32] = { 0 };
	int critical_log = 0;
...
static void init_p9418_data_log(void)
{
	...
	p = proc_create("p9418_data_log", 0664, NULL,
			&p9418_data_log_proc_fops);
	if (!p)
...
static void p9418_update_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p9418_ic *chip =
		container_of(dwork, struct oplus_p9418_ic, p9418_update_work);
	int rc;
	...
	if (rc == -1) {
		...
		schedule_delayed_work(&chip->p9418_update_work,
				      P9418_UPDATE_RETRY_INTERVAL);
	}
}
...
static struct device_attribute *pencil_attributes[] = {
	&dev_attr_ble_mac_addr,	  &dev_attr_present,
	&dev_attr_fw_version,	  &dev_attr_tx_current,
	&dev_attr_tx_voltage,	  &dev_attr_rx_soc,
	&dev_attr_wireless_debug, NULL
};
...
static int p9418_identity_check(struct oplus_p9418_ic *chip)
{
	...
	if ((buf[NUM_0] == P9418_ID_H) &&
	    (buf[NUM_4] == P9415_ID_L || buf[NUM_4] == P9418_ID_L)) {
		rc = NUM_1;
	} else {
		rc = ERR_NUM;
	}
			      ...
			      const struct i2c_device_id *id)
{
	...
	chg_err(" call \n");
	chip = devm_kzalloc(&client->dev, sizeof(struct oplus_p9418_ic),
			    GFP_KERNEL);
	if (!chip) {
	...
	chip->bus_wakelock =
		wakeup_source_register(NULL, "p9418_wireless_wakelock");
	present_wakelock =
		wakeup_source_register(NULL, "p9418_present_wakelock");
