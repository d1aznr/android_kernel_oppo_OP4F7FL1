...
static int __sp6001_read_reg(struct oplus_sp6001_ic *chip, int reg,
...
static int sp6001_read_reg(struct oplus_sp6001_ic *chip, int reg,
			   char *returnData, int count)
{
	int ret = 0;
...
static int sp6001_config_interface(struct oplus_sp6001_ic *chip, int RegNum,
				   int val, int MASK)
{
	char sp6001_reg = 0;
	int ret = 0;
...
int sp6001_get_vrect_iout(struct oplus_sp6001_ic *chip)
{
	...
	for (i = 1; i < 10; i++) {
		chip->p922x_chg_status.iout_array[i] =
			chip->p922x_chg_status.iout_array[i - 1];
	}
...
static int p922x_RXTX_message_process(struct oplus_sp6001_ic *chip)
{
	...
	case P9237_RESPONE_ADAPTER_TYPE:
		chip->p922x_chg_status.adapter_type =
			chip->p922x_chg_status.tx_data;
		chg_err("<~WPC~> get adapter type = 0x%02X\n",
			chip->p922x_chg_status.adapter_type);
		if ((chip->p922x_chg_status.adapter_type ==
		     ADAPTER_TYPE_FASTCHAGE_VOOC) ||
		    (chip->p922x_chg_status.adapter_type ==
		     ADAPTER_TYPE_FASTCHAGE_SVOOC)) {
			       ...
			       const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct oplus_sp6001_ic),
			    GFP_KERNEL);
	if (!chip) {
