...
nu1619_backcover_color_info_init(char *str)
{
...
static struct wls_pwr_table oplus_chg_wls_pwr_table[] = {
	...
	{ 0x00, 12, 15 },   { 0x01, 12, 20 },	{ 0x02, 12, 30 },
	{ 0x03, 35, 50 },   { 0x04, 45, 65 },	{ 0x05, 50, 75 },
	{ 0x06, 60, 85 },   { 0x07, 65, 95 },	{ 0x08, 75, 105 },
	{ 0x09, 80, 115 },  { 0x0A, 90, 125 },	{ 0x0B, 20, 20 },
	{ 0x0C, 100, 140 }, { 0x0D, 115, 160 }, { 0x0E, 130, 180 },
	{ 0x0F, 145, 200 }, { 0x11, 35, 50 },	{ 0x12, 35, 50 },
	{ 0x13, 12, 20 },   { 0x14, 45, 65 },	{ 0x19, 12, 30 },
	{ 0x21, 35, 50 },   { 0x29, 12, 30 },	{ 0x31, 35, 50 },
	{ 0x32, 45, 65 },   { 0x33, 35, 50 },	{ 0x34, 12, 20 },
	{ 0x35, 45, 65 },   { 0x41, 12, 30 },	{ 0x42, 12, 30 },
	{ 0x43, 12, 30 },   { 0x44, 12, 30 },	{ 0x45, 12, 30 },
	{ 0x46, 12, 30 },   { 0x49, 12, 33 },	{ 0x4A, 12, 33 },
	{ 0x4B, 12, 33 },   { 0x4C, 12, 33 },	{ 0x4D, 12, 33 },
	{ 0x4E, 12, 33 },   { 0x61, 12, 33 },	{ 0x62, 35, 50 },
	{ 0x63, 45, 65 },   { 0x64, 45, 66 },	{ 0x65, 50, 80 },
...
#define target_ichg_to_input_current(vbatt, target_ichg, table, i,             \
				     input_current)                            \
	do {                                                                   \
		for (i = 0; i < ARRAY_SIZE(table); i++) {                      \
			if (table[i].vbat == 0 && table[i].target_ichg == 0 && \
			    table[i].input_current == 0) {                     \
				break;                                         \
			}                                                      \
			if (vbatt < table[i].vbat &&                           \
...
#define input_current_to_target_ichg(vbatt, input_current, table, i,           \
				     target_ichg)                              \
	do {                                                                   \
		for (i = 0; i < ARRAY_SIZE(table); i++) {                      \
			if (table[i].vbat == 0 && table[i].target_ichg == 0 && \
...
static int __nu1619_read_reg(struct oplus_nu1619_ic *chip, int reg,
...
static int nu1619_write_reg_multi_byte(struct oplus_nu1619_ic *chip, int reg,
...
static int nu1619_read_reg(struct oplus_nu1619_ic *chip, int reg,
			   char *returnData, int count)
{
	int ret = 0;
...
static int nu1619_config_interface(struct oplus_nu1619_ic *chip, int RegNum,
				   int val, int MASK)
{
	char nu1619_reg = 0;
	int ret = 0;
...
static void nu1619_clear_irq(struct oplus_nu1619_ic *chip, char mark0,
			     char mark1)
{
...
static void nu1619_set_FOD_parameter(struct oplus_nu1619_ic *chip,
				     char parameter)
{
...
static int nu1619_set_tx_charger_fastcharge(struct oplus_nu1619_ic *chip)
{
	...
	if (chip->nu1619_chg_status.mldo_en == false &&
	    chip->nu1619_chg_status.iout < 100) {
...
static bool oplus_get_chg_smem_info(struct oplus_nu1619_ic *chip)
{
	...
	if (IS_ERR(smem_addr)) {
		...
		if (smem_data == ERR_PTR(-EPROBE_DEFER)) {
			...
			memcpy(chip->nu1619_chg_status.random_num,
			       &smem_data->wls_auth_data.random_num,
			       WLS_AUTH_RANDOM_LEN);
			memcpy(chip->nu1619_chg_status.rx_encode_num,
			       &smem_data->wls_auth_data.encode_num,
			       WLS_AUTH_ENCODE_LEN);
			chg_err("<~WPC~><~VRY~> random number: %02x %02x %02x %02x %02x %02x %02x %02x\n",
				chip->nu1619_chg_status.random_num[0],
				chip->nu1619_chg_status.random_num[1],
				chip->nu1619_chg_status.random_num[2],
				chip->nu1619_chg_status.random_num[3],
				chip->nu1619_chg_status.random_num[4],
				chip->nu1619_chg_status.random_num[5],
				chip->nu1619_chg_status.random_num[6],
				chip->nu1619_chg_status.random_num[7]);
...
static void nu1619_set_rx_charge_current(struct oplus_nu1619_ic *chip,
					 int chg_current)
{
	int current_ma = 0;
	...
	current_ma = chip->nu1619_chg_status.charge_current *
...
static void nu1619_set_rx_terminate_voltage(struct oplus_nu1619_ic *chip,
					    int vol_value)
{
	if (!g_oplus_chip) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: oplus_chip not ready!\n",
...
static int nu1619_increase_vc_by_step(struct oplus_nu1619_ic *chip,
				      int src_value, int limit_value,
				      int step_value)
{
	int temp_value;
...
static int nu1619_decrease_vc_by_step(struct oplus_nu1619_ic *chip,
				      int src_value, int limit_value,
				      int step_value)
{
	int temp_value = 0;
...
static int nu1619_decrease_vout_to_target(struct oplus_nu1619_ic *chip,
					  int vout_target, int vout_min)
{
	...
	chg_err("<~WPC~> vout_setting[%d], vout_sample[%d],target_voltage[%d]\n",
		...
		chip->nu1619_chg_status.vout, vout_target);

	if (chip->nu1619_chg_status.vout > vout_target) {
		if (chip->nu1619_chg_status.charge_voltage > vout_target) {
			temp_value = nu1619_decrease_vc_by_step(
				chip, chip->nu1619_chg_status.charge_voltage,
				vout_target, WPC_CHARGE_VOLTAGE_CHANGE_STEP_1V);
		} else {
			temp_value = nu1619_decrease_vc_by_step(
				chip, chip->nu1619_chg_status.charge_voltage,
				vout_min, WPC_CHARGE_VOLTAGE_CHANGE_STEP_100MV);
		}
...
static int nu1619_increase_vout_to_target(struct oplus_nu1619_ic *chip,
					  int vout_target, int vout_max)
{
	...
	if (chip->nu1619_chg_status.vout < vout_target) {
		if (chip->nu1619_chg_status.charge_voltage < vout_target) {
			temp_value = nu1619_increase_vc_by_step(
				chip, chip->nu1619_chg_status.charge_voltage,
				vout_target, WPC_CHARGE_VOLTAGE_CHANGE_STEP_1V);
		} else {
			temp_value = nu1619_increase_vc_by_step(
				chip, chip->nu1619_chg_status.charge_voltage,
				vout_max, WPC_CHARGE_VOLTAGE_CHANGE_STEP_100MV);
		}
...
static void nu1619_reset_variables(struct oplus_nu1619_ic *chip)
{
	...
	chip->nu1619_chg_status.skewing_fastcharge_level =
...
static void nu1619_charger_init(struct oplus_nu1619_ic *chip)
{
	...
	if (chip->nu1619_chg_status.rx_runing_mode == RX_RUNNING_MODE_EPP ||
		...
		if (is_ext_chg_ops()) {
			...
			g_oplus_chip->chg_ops->wls_input_current_write(
				chip->nu1619_chg_status.epp_current_step);
		}
		...
		nu1619_set_rx_charge_current(
			chip, chip->nu1619_chg_status.BPP_current_limit);
		chip->nu1619_chg_status.BPP_current_step_cnt =
			BPP_CURRENT_INCREASE_TIME;
	}
...
int nu1619_wpc_get_adapter_type(void)
{
	if (!nu1619_chip) {
		...
		if (nu1619_chip->nu1619_chg_status.support_airsvooc == 0) {
			if (nu1619_chip->nu1619_chg_status.adapter_type ==
				    ADAPTER_TYPE_SVOOC_50W ||
...
static void oplus_wpc_update_track_info(struct oplus_nu1619_ic *chip,
					char *crux_info)
{
	int index = 0;
...
void nu1619_set_rtx_function(bool is_on)
{
	...
	if (is_on) {
		if (chip->nu1619_chg_status.wpc_dischg_status !=
		...
		if (chip->nu1619_chg_status.tx_online &&
			...
			oplus_wpc_track_upload_trx_general_info(
				chip, trx_crux_info,
				chip->nu1619_chg_status.trx_usb_present_once);
		}
		chip->nu1619_chg_status.vout = 0;
...
5V_en[%d], EXT2_otg_en[%d], EXT1_otg_en[%d], internal_5V_ldo_en[%d], wls_pg[%d]\n",
		nu1619_get_vt_sleep_val(), nu1619_get_idt_con_val(),
		nu1619_get_idt_int_val(), oplus_get_idt_en_val(),
		nu1619_get_booster_en_val(), chargepump_get_chargepump_en_val(),
		oplus_get_wrx_en_val(), oplus_get_wrx_otg_en_val(),
		nu1619_get_cp_ldo_5v_val(),
		nu1619_get_ext2_wireless_otg_en_val(),
		nu1619_get_ext1_wired_otg_en_val(), nu1619_get_vbat_en_val(),
...
[Batt vol max: %d] [Batt vol min: %d]\n",
		g_oplus_chip->temperature, g_oplus_chip->soc,
		g_oplus_chip->icharging, g_oplus_chip->batt_volt,
...
static int nu1619_write_firmware_data(struct oplus_nu1619_ic *chip,
				      unsigned short addr,
...
static bool nu1619_download_firmware_data(struct oplus_nu1619_ic *chip,
					  char area)
{
	int ret;
...
static bool nu1619_req_checksum_and_fw_version(
	struct oplus_nu1619_ic *chip, char *boot_check_sum, char *rx_check_sum,
...
{
	char boot_checksum, rx_checksum, tx_checksum, boot_version, rx_version,
		tx_version;

	nu1619_req_checksum_and_fw_version(chip, &boot_checksum, &rx_checksum,
					   &tx_checksum, &boot_version,
					   &rx_version, &tx_version);
	chg_err("check_sum: 0x%x,0x%x,0x%x  Version:0x%x,0x%x,0x%x \n",
		boot_checksum, rx_checksum, tx_checksum, boot_version,
		rx_version, tx_version);
	...
	chg_err(",g_boot_no_need_update=%d, g_rx_no_need_update=%d, g_tx_no_need_update=%d \n",
		g_boot_no_need_update, g_rx_no_need_update,
		g_tx_no_need_update);
	if ((g_boot_no_need_update) && (g_rx_no_need_update) &&
	    (g_tx_no_need_update)) {
		return false;
	}
	return true;
...
static bool nu1619_check_firmware(struct oplus_nu1619_ic *chip)
{
	...
	if ((boot_checksum == 0x66) && (rx_checksum == 0x66) &&
	    (tx_checksum == 0x66)) {
		return true;
	} else {
		return false;
...
static int nu1619_check_idt_fw_update(struct oplus_nu1619_ic *chip)
{
	...
	if (g_oplus_chip->charger_exist == 1 ||
	    g_oplus_chip->charger_volt > 2500 || g_oplus_chip->ui_soc < 15) {
...
READ_IOUT_RETRY:
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHGPUMP:
		if ((nu1619_get_CEP_flag(chip) == 0) &&
		    (abs(chip->nu1619_chg_status.charge_voltage - vout_value) >
		     2000)) {
	...
	if (chip->nu1619_chg_status.charge_status ==
...
int nu1619_get_CEP_flag(struct oplus_nu1619_ic *chip)
{
	...
	if ((chip->nu1619_chg_status.CEP_value == 0) ||
	    (chip->nu1619_chg_status.CEP_value == 1) ||
	    (chip->nu1619_chg_status.CEP_value == 2) ||
	    (chip->nu1619_chg_status.CEP_value == 0xFF) ||
...
{
	if ((nu1619_chip->nu1619_chg_status.charge_online) &&
	    (chip->nu1619_chg_status.CEP_ready)) {
...
static void nu1619_ready_to_switch_to_charger(struct oplus_nu1619_ic *chip,
...
static int nu1619_fastcharge_debug_by_adb(struct oplus_nu1619_ic *chip)
{
	...
	if (!chip->nu1619_chg_status.iout_debug_mode &&
	    !chip->nu1619_chg_status.vout_debug_mode) {
		return -1;
	}
	...
	iout_max_value = chip->nu1619_chg_status.iout_stated_current +
			 WPC_CHARGE_CURRENT_OFFSET;
	if (chip->nu1619_chg_status.iout_stated_current >
	    WPC_CHARGE_CURRENT_OFFSET) {
		iout_min_value = chip->nu1619_chg_status.iout_stated_current -
				 WPC_CHARGE_CURRENT_OFFSET;
	} else {
		iout_min_value = 0;
	}
	...
	if (chip->nu1619_chg_status.iout > iout_max_value) {
		...
		if (chip->nu1619_chg_status.charge_voltage >
			...
			temp_value =
				chip->nu1619_chg_status.iout - iout_max_value;
		...
		if (chip->nu1619_chg_status.charge_voltage <
			...
			temp_value =
				iout_min_value - chip->nu1619_chg_status.iout;
			if ((temp_value > 100) &&
			    (chip->nu1619_chg_status.iout < 1800)) {
				nu1619_set_rx_charge_voltage(
					chip,
					chip->nu1619_chg_status.charge_voltage +
						100);
...
#else
static void nu1619_fastcharge_current_adjust_40w(struct oplus_nu1619_ic *chip,
						 bool is_init)
{
	...
	if ((g_oplus_chip->batt_volt >= 4500) &&
	    (!chip->nu1619_chg_status.wpc_ffc_charge)) {
		chg_err("<~WPC~> batt_volt[%d] >= 4.5V, Not in FFC\n",
			g_oplus_chip->batt_volt);
		nu1619_ready_to_switch_to_charger(chip, true);
		return;
	} else if (chip->nu1619_chg_status.wpc_ffc_charge &&
		   (g_oplus_chip->batt_volt >= 4500) &&
		   ((g_oplus_chip->icharging < 0) &&
		    ((-1 * g_oplus_chip->icharging) < 900))) {
	...
	if (g_oplus_chip->temperature <
		    g_oplus_chip->limits.little_cool_bat_decidegc ||
	    g_oplus_chip->temperature >
		    g_oplus_chip->limits.warm_bat_decidegc) {
	...
	if (g_oplus_chip->batt_volt >= 4370 &&
	    ((g_oplus_chip->icharging < 0) &&
	     ((-1 * g_oplus_chip->icharging) < 800)) &&
	    chip->nu1619_chg_status.wpc_reach_stable_charge == true) {
		chg_err("<~WPC~> icharging < 800. Exit FFC.\n");
		chip->nu1619_chg_status.wpc_ffc_charge = false;
		nu1619_set_rx_terminate_voltage(chip,
						WPC_TERMINATION_VOLTAGE_CV);
		nu1619_ready_to_switch_to_charger(chip, true);
		goto ADJUST_FINISH;
	}

	if ((nu1619_get_CEP_flag(chip) != 0) ||
	    chip->nu1619_chg_status.wpc_skewing_proc) {
		return;
	}
	...
	if (chip->nu1619_chg_status.wpc_reach_stable_charge) {
		if (!chip->nu1619_chg_status.wpc_reach_4370mv &&
		    (g_oplus_chip->batt_volt >= 4370)) {
			chg_err("<~WPC~> First reach 4370mV.\n");
			if (fasctchg_current[chip->nu1619_chg_status
						     .fastcharge_level] >
				...
				if ((g_oplus_chip->icharging < 0) &&
				    ((-1 * g_oplus_chip->icharging) <
				     WPC_CHARGE_CURRENT_FFC_TO_CV)) {
					chg_err("<~WPC~> reach 4370mV, icharging < 1A. Exit FFC.\n");
					chip->nu1619_chg_status.wpc_ffc_charge =
						false;
					nu1619_set_rx_terminate_voltage(
						chip,
						WPC_TERMINATION_VOLTAGE_CV);
					goto ADJUST_FINISH;
				}
	...
	if (g_oplus_chip->batt_volt >= 4500) {
		...
		if (chip->nu1619_chg_status.fastcharge_level <
		    FASTCHARGE_LEVEL_5) {
			chg_err("<~WPC~> turn to %dMA charge\n",
			...
			goto ADJUST_FINISH;
		} else if (chip->nu1619_chg_status.fastcharge_level ==
			   FASTCHARGE_LEVEL_5) {
			chg_err("<~WPC~> turn to %dMA charge\n",
	...
	if (g_oplus_chip->temperature >= 420) {
		chg_err("<~WPC~> The tempearture is >= 42. fastchg current: %d\n",
			fasctchg_current[chip->nu1619_chg_status
						 .fastcharge_level]);
...
static void nu1619_fastcharge_skewing_proc_40w(struct oplus_nu1619_ic *chip,
					       bool is_init)
{
	...
	if (chip->nu1619_chg_status.iout <=
		...
		chip->nu1619_chg_status.iout_stated_current =
			chip->nu1619_chg_status.iout;
		nu1619_set_rx_charge_voltage(chip,
...
static void nu1619_TX_message_process(struct oplus_nu1619_ic *chip)
{
	...
	if (chip->nu1619_chg_status.send_msg_timer > 2) {
		...
		if (chip->nu1619_chg_status.adapter_type ==
		    ADAPTER_TYPE_UNKNOW) {
			nu1619_set_tx_charger_dect(chip);
		} else if (chip->nu1619_chg_status.send_message ==
			   P9221_CMD_INTO_FASTCHAGE) {
			nu1619_set_tx_charger_fastcharge(chip);
		} else if (chip->nu1619_chg_status.send_message ==
			   P9221_CMD_SET_CEP_TIMEOUT) {
			if (chip->nu1619_chg_status.send_msg_cnt > 0) {
				chip->nu1619_chg_status.send_msg_cnt--;
				nu1619_set_tx_cep_timeout(chip);
			} else {
				chip->nu1619_chg_status.send_message =
					P9221_CMD_NULL;
			}
		} else if (chip->nu1619_chg_status.send_message ==
			   P9221_CMD_SET_PWM_PULSE) {
			if (chip->nu1619_chg_status.send_msg_cnt > 0) {
				chip->nu1619_chg_status.send_msg_cnt--;
			...
			if (chip->nu1619_chg_status.send_msg_cnt > 0) {
			   ...
			   P9221_CMD_SEND_1ST_RANDOM_DATA) {
			nu1619_set_tx_1st_random_data(chip);
		} else if (chip->nu1619_chg_status.send_message ==
			   P9221_CMD_SEND_2ND_RANDOM_DATA) {
			nu1619_set_tx_2nd_random_data(chip);
		} else if (chip->nu1619_chg_status.send_message ==
			   P9221_CMD_SEND_3RD_RANDOM_DATA) {
			nu1619_set_tx_3rd_random_data(chip);
		} else if (chip->nu1619_chg_status.send_message ==
			   P9221_CMD_GET_1ST_ENCODE_DATA) {
			nu1619_get_1st_encode_data(chip);
		} else if (chip->nu1619_chg_status.send_message ==
			   P9221_CMD_GET_2ND_ENCODE_DATA) {
			nu1619_get_2nd_encode_data(chip);
		} else if (chip->nu1619_chg_status.send_message ==
			   P9221_CMD_GET_3RD_ENCODE_DATA) {
...
static int oplus_wpc_chg_parse_fw(struct oplus_nu1619_ic *chip)
{
	...
	chip->fw_boot_data = (char *)of_get_property(node, "oplus,fw_boot_data",
						     &chip->fw_boot_lenth);
	...
	chip->fw_rx_data = (char *)of_get_property(node, "oplus,fw_rx_data",
						   &chip->fw_rx_lenth);
	if (!chip->fw_rx_data) {
	...
	chip->fw_tx_data = (char *)of_get_property(node, "oplus,fw_tx_data",
						   &chip->fw_tx_lenth);
	if (!chip->fw_tx_data) {
...
static int oplus_wpc_parse_target_ichg_table(struct oplus_nu1619_ic *chip)
{
	...
	rc = of_property_count_elems_of_size(node, "ffc_target_ichg_table",
					     sizeof(u32));
	if (rc < 0) {
	...
	length = rc;
	rc = of_property_read_u32_array(node, "ffc_target_ichg_table",
					(u32 *)ffc_table, length);
	if (rc) {
	...
	for (i = 0; i < length / 3; i++) {
		chg_err("ffc_table[%d,%d,%d]\n", ffc_table[i].vbat,
			ffc_table[i].target_ichg, ffc_table[i].input_current);
	}

	rc = of_property_count_elems_of_size(node, "svooc_target_ichg_table",
					     sizeof(u32));
	if (rc < 0) {
	...
	length = rc;
	rc = of_property_read_u32_array(node, "svooc_target_ichg_table",
					(u32 *)svooc_table, length);
	if (rc) {
	...
	for (i = 0; i < length / 3; i++) {
		chg_err("svooc_table[%d,%d,%d]\n", svooc_table[i].vbat,
			svooc_table[i].target_ichg,
			svooc_table[i].input_current);
	}

	rc = of_property_count_elems_of_size(node, "vooc_target_ichg_table",
					     sizeof(u32));
	if (rc < 0) {
	...
	length = rc;
	rc = of_property_read_u32_array(node, "vooc_target_ichg_table",
					(u32 *)vooc_table, length);
	if (rc) {
	...
	for (i = 0; i < length / 3; i++) {
		chg_err("vooc_table[%d,%d,%d]\n", vooc_table[i].vbat,
			vooc_table[i].target_ichg, vooc_table[i].input_current);
	}

	rc = of_property_count_elems_of_size(node, "epp_target_ichg_table",
					     sizeof(u32));
	if (rc < 0) {
	...
	length = rc;
	rc = of_property_read_u32_array(node, "epp_target_ichg_table",
					(u32 *)epp_table, length);
	if (rc) {
	...
	for (i = 0; i < length / 3; i++) {
		chg_err("epp_table[%d,%d,%d]\n", epp_table[i].vbat,
			epp_table[i].target_ichg, epp_table[i].input_current);
	}

	rc = of_property_count_elems_of_size(node, "bpp_target_ichg_table",
					     sizeof(u32));
	if (rc < 0) {
	...
	length = rc;
	rc = of_property_read_u32_array(node, "bpp_target_ichg_table",
					(u32 *)bpp_table, length);
	if (rc) {
	...
	for (i = 0; i < length / 3; i++) {
		chg_err("bpp_table[%d,%d,%d]\n", bpp_table[i].vbat,
			bpp_table[i].target_ichg, bpp_table[i].input_current);
	}

	rc = of_property_count_elems_of_size(node, "fasctchg_current_level",
					     sizeof(int));
	if (rc < 0) {
	...
	length = rc;
	rc = of_property_read_u32_array(node, "fasctchg_current_level",
					(int *)fasctchg_current, length);
	if (rc) {
	...
	rc = of_property_count_elems_of_size(node, "ffc_max_vol_level",
					     sizeof(int));
	if (rc < 0) {
	...
	length = rc;
	rc = of_property_read_u32_array(node, "ffc_max_vol_level",
					(int *)ffc_max_vol, length);
	if (rc) {
	...
	rc = of_property_count_elems_of_size(node, "non_ffc_max_vol_level",
					     sizeof(int));
	if (rc < 0) {
	...
	length = rc;
	rc = of_property_read_u32_array(node, "non_ffc_max_vol_level",
					(int *)non_ffc_max_vol, length);
	if (rc) {
...
static int oplus_wpc_chg_parse_chg_dt(struct oplus_nu1619_ic *chip)
{
	...
	rc = of_property_read_u32(
		node, "qcom,epp_temp_warm_input_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,bpp_temp_cold_fastchg_ma",
	...
	rc = of_property_read_u32(node, "qcom,vooc_temp_little_cold_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .vooc_temp_little_cold_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.vooc_temp_little_cold_fastchg_ma = 1200;
	}
	...
	rc = of_property_read_u32(node, "qcom,svooc_temp_little_cold_iout_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .svooc_temp_little_cold_iout_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_temp_little_cold_iout_ma = 650;
	}
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_temp_little_cold_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .svooc_temp_little_cold_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_temp_little_cold_fastchg_ma = 1200;
	}
	...
	rc = of_property_read_u32(node, "qcom,bpp_temp_little_cold_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .bpp_temp_little_cold_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.bpp_temp_little_cold_fastchg_ma = 600;
	}
	...
	rc = of_property_read_u32(node, "qcom,epp_temp_little_cold_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .epp_temp_little_cold_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp_temp_little_cold_fastchg_ma = 1400;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,epp_15w_temp_little_cold_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .epp_15w_temp_little_cold_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp_15w_temp_little_cold_fastchg_ma = 2000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,svooc_temp_cool_iout_ma",
	...
	rc = of_property_read_u32(node, "qcom,svooc_temp_cool_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .svooc_temp_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_temp_cool_fastchg_ma = 1200;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,bpp_temp_cool_fastchg_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,epp_temp_cool_fastchg_ma",
	...
	rc = of_property_read_u32(node, "qcom,epp_15w_temp_cool_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .epp_15w_temp_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp_15w_temp_cool_fastchg_ma = 2000;
	}
	...
	rc = of_property_read_u32(node, "qcom,vooc_temp_little_cool_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .vooc_temp_little_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.vooc_temp_little_cool_fastchg_ma = 1200;
	}
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_temp_little_cool_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .svooc_temp_little_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_temp_little_cool_fastchg_ma = 1200;
	}
	...
	rc = of_property_read_u32(node, "qcom,bpp_temp_little_cool_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .bpp_temp_little_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.bpp_temp_little_cool_fastchg_ma = 600;
	}
	...
	rc = of_property_read_u32(node, "qcom,epp_temp_little_cool_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .epp_temp_little_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp_temp_little_cool_fastchg_ma = 1400;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,epp_15w_temp_little_cool_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .epp_15w_temp_little_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp_15w_temp_little_cool_fastchg_ma = 2000;
	}
	...
	rc = of_property_read_u32(node, "qcom,vooc_temp_normal_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .vooc_temp_normal_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.vooc_temp_normal_fastchg_ma = 1200;
	}
	...
	rc = of_property_read_u32(node, "qcom,svooc_temp_normal_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .svooc_temp_normal_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_temp_normal_fastchg_ma = 1200;
	}
	...
	rc = of_property_read_u32(node, "qcom,bpp_temp_normal_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .bpp_temp_normal_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.bpp_temp_normal_fastchg_ma = 1200;
	}
	...
	rc = of_property_read_u32(node, "qcom,epp_temp_normal_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .epp_temp_normal_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp_temp_normal_fastchg_ma = 1400;
	}
	...
	rc = of_property_read_u32(node, "qcom,epp_15w_temp_normal_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .epp_15w_temp_normal_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp_15w_temp_normal_fastchg_ma = 2000;
	}
	...
	rc = of_property_read_u32(node, "qcom,svooc_temp_warm_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .svooc_temp_warm_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_temp_warm_fastchg_ma = 1200;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,bpp_temp_warm_fastchg_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,epp_temp_warm_fastchg_ma",
	...
	rc = of_property_read_u32(node, "qcom,epp_15w_temp_warm_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .epp_15w_temp_warm_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp_15w_temp_warm_fastchg_ma = 2000;
	}
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_target_ichg_cold_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .svooc_target_ichg_cold_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_target_ichg_cold_fastchg_ma = 500;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,svooc_target_ichg_little_cold_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .svooc_target_ichg_little_cold_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_target_ichg_little_cold_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,vooc_target_ichg_little_cold_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .vooc_target_ichg_little_cold_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.vooc_target_ichg_little_cold_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,bpp_target_ichg_little_cold_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .bpp_target_ichg_little_cold_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.bpp_target_ichg_little_cold_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,epp1_target_ichg_little_cold_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .epp1_target_ichg_little_cold_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp1_target_ichg_little_cold_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,epp2_target_ichg_little_cold_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .epp2_target_ichg_little_cold_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp2_target_ichg_little_cold_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,epp3_target_ichg_little_cold_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .epp3_target_ichg_little_cold_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp3_target_ichg_little_cold_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_target_ichg_cool_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .svooc_target_ichg_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_target_ichg_cool_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(node, "qcom,vooc_target_ichg_cool_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .vooc_target_ichg_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.vooc_target_ichg_cool_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(node, "qcom,bpp_target_ichg_cool_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .bpp_target_ichg_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.bpp_target_ichg_cool_fastchg_ma = 500;
	}
	...
	rc = of_property_read_u32(node, "qcom,epp1_target_ichg_cool_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .epp1_target_ichg_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp1_target_ichg_cool_fastchg_ma = 500;
	}
	...
	rc = of_property_read_u32(node, "qcom,epp2_target_ichg_cool_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .epp2_target_ichg_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp2_target_ichg_cool_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(node, "qcom,epp3_target_ichg_cool_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .epp3_target_ichg_cool_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp3_target_ichg_cool_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,svooc_target_ichg_normal_region1_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .svooc_target_ichg_normal_region1_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_target_ichg_normal_region1_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,vooc_target_ichg_normal_region1_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .vooc_target_ichg_normal_region1_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.vooc_target_ichg_normal_region1_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,bpp_target_ichg_normal_region1_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .bpp_target_ichg_normal_region1_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.bpp_target_ichg_normal_region1_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,epp1_target_ichg_normal_region1_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .epp1_target_ichg_normal_region1_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp1_target_ichg_normal_region1_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,epp2_target_ichg_normal_region1_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .epp2_target_ichg_normal_region1_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp2_target_ichg_normal_region1_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,epp3_target_ichg_normal_region1_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .epp3_target_ichg_normal_region1_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp3_target_ichg_normal_region1_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,svooc_target_ichg_normal_region2_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .svooc_target_ichg_normal_region2_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_target_ichg_normal_region2_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,vooc_target_ichg_normal_region2_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .vooc_target_ichg_normal_region2_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.vooc_target_ichg_normal_region2_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,bpp_target_ichg_normal_region2_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .bpp_target_ichg_normal_region2_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.bpp_target_ichg_normal_region2_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,epp1_target_ichg_normal_region2_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .epp1_target_ichg_normal_region2_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp1_target_ichg_normal_region2_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,epp2_target_ichg_normal_region2_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .epp2_target_ichg_normal_region2_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp2_target_ichg_normal_region2_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,epp3_target_ichg_normal_region2_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .epp3_target_ichg_normal_region2_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp3_target_ichg_normal_region2_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,svooc_target_ichg_normal_region3_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .svooc_target_ichg_normal_region3_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_target_ichg_normal_region3_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,vooc_target_ichg_normal_region3_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .vooc_target_ichg_normal_region3_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.vooc_target_ichg_normal_region3_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,bpp_target_ichg_normal_region3_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .bpp_target_ichg_normal_region3_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.bpp_target_ichg_normal_region3_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(
		node, "qcom,epp_target_ichg_normal_region3_fastchg_ma",
		&chip->nu1619_chg_status.wpc_chg_param
			 .epp_target_ichg_normal_region3_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.epp_target_ichg_normal_region3_fastchg_ma = 1000;
	}
	...
	rc = of_property_read_u32(node,
				  "qcom,svooc_target_ichg_warm_fastchg_ma",
				  &chip->nu1619_chg_status.wpc_chg_param
					   .svooc_target_ichg_warm_fastchg_ma);
	if (rc) {
		chip->nu1619_chg_status.wpc_chg_param
			.svooc_target_ichg_warm_fastchg_ma = 1000;
	}
	...
	if (rc) {
		chip->nu1619_chg_status.max_charge_current =
			NU1619_FASTCHG_CURR_MAX;
	}
	chg_err("max_charge_current[%d]\n",
		chip->nu1619_chg_status.max_charge_current);

	rc = of_property_read_u32(
		node, "qcom,wireless_chg_current_index",
		&chip->nu1619_chg_status.charge_current_index);
	if (rc) {
...
*/
	if (chip->nu1619_chg_status.adapter_type == ADAPTER_TYPE_VOOC &&
	    (temp < chip->nu1619_chg_status.wpc_chg_param
			     .wireless_vooc_min_temp ||
	     temp > chip->nu1619_chg_status.wpc_chg_param
			     .wireless_vooc_max_temp)) {
		chip->nu1619_chg_status.fastchg_allow = false;
		return chip->nu1619_chg_status.fastchg_allow;
	}
...
static void nu1619_charge_check_normal_temp_region(struct oplus_nu1619_ic *chip)
{
	...
	if (now_tbatt != BATTERY_STATUS__LITTLE_COOL_TEMP &&
		...
		chip->nu1619_chg_status.normal_temp_region =
			NORMAL_TEMP_REGION_UNKNOW;
		pre_normal_region = NORMAL_TEMP_REGION_UNKNOW;
		return;
	}
	if (chip->nu1619_chg_status.normal_temp_region ==
	    NORMAL_TEMP_REGION_UNKNOW) {
		work_delay = 0;
		low_shake_temp = 0;
		high_shake_temp = 0;
	...
	if (g_oplus_chip->temperature >= NORMAL_TEMP_HIGH + high_shake_temp) {
		...
		low_shake_temp = 0;
		high_shake_temp = -10;
	} else if ((g_oplus_chip->temperature >=
...
static int nu1916_charge_get_skewing_input_current(struct oplus_nu1619_ic *chip)
{
	...
	if (chip->nu1619_chg_status.skewing_fastcharge_level ==
	    FASTCHARGE_LEVEL_UNKNOW) {
	...
	if (nu1619_get_CEP_flag(chip) == 0 & adjust_current_delay == 0) {
		cep_zero_cnt++;
		cep_nonzero_cnt = 0;
	} else if (nu1619_get_CEP_flag(chip) == 1 &&
		   adjust_current_delay == 0) {
		cep_nonzero_cnt++;
		cep_zero_cnt = 0;
	} else {
	...
	if (cep_zero_cnt > 10) {
		...
		if (chip->nu1619_chg_status.normal_temp_region ==
			...
			if (adapter_type == AIR_SVOOC) {
				if (chip->nu1619_chg_status
					    .skewing_fastcharge_level >=
				    FASTCHARGE_LEVEL_4)
	...
	if (cep_nonzero_cnt > 3) {
		...
		if (adapter_type == AIR_SVOOC) {
			input_current_to_target_ichg(vbatt, input_current,
						     svooc_table, i,
						     target_ichg);
	...
	if (adapter_type == AIR_VOOC) {
		if (g_oplus_chip->icharging > -800 &&
		    oplus_gauge_get_batt_current() > -800) {
			self_reset_cnt++;
	...
	target_ichg = fasctchg_current[chip->nu1619_chg_status
					       .skewing_fastcharge_level];
	target_ichg_to_input_current(vbatt, target_ichg, ffc_table, i,
				     input_current);
	temp_input_current = input_current;
	if (adapter_type == AIR_SVOOC)
		target_ichg_to_input_current(vbatt, target_ichg, svooc_table, i,
					     input_current);
	else if (adapter_type == AIR_VOOC)
		target_ichg_to_input_current(vbatt, target_ichg, vooc_table, i,
					     input_current);
	...
	if (chip->nu1619_chg_status.wpc_ffc_charge == true) {
		...
		if ((chip->nu1619_chg_status.fastcharge_level <=
		    ...
		    (chip->nu1619_chg_status.fastcharge_level ==
			     FASTCHARGE_LEVEL_7 &&
		     g_oplus_chip->batt_volt > P922X_CC2CV_CHG_THD_HI &&
		     (-1 * g_oplus_chip->icharging) < 600) ||
		    (chip->nu1619_chg_status.fastcharge_level ==
			     FASTCHARGE_LEVEL_8 &&
		     g_oplus_chip->batt_volt > P922X_CC2CV_CHG_THD_HI &&
		     (-1 * g_oplus_chip->icharging) < 400) ||
		    (chip->nu1619_chg_status.fastcharge_level ==
			     FASTCHARGE_LEVEL_10 &&
		     g_oplus_chip->batt_volt > P922X_CC2CV_CHG_THD_HI &&
		     (-1 * g_oplus_chip->icharging) < 200)) {
			...
			if (chip->nu1619_chg_status.fastcharge_level ==
				...
				nu1619_set_rx_charge_current(
					...
					fasctchg_current[FASTCHARGE_LEVEL_8]);
					 ...
					 FASTCHARGE_LEVEL_10 &&
				 chip->nu1619_chg_status.charge_current !=
					 fasctchg_current[FASTCHARGE_LEVEL_10])
				nu1619_set_rx_charge_current(
					chip,
					fasctchg_current[FASTCHARGE_LEVEL_10]);
		} else if (chip->nu1619_chg_status.charge_current !=
			   NU1619_FASTCHG_CURR_MAX) {
			nu1619_set_rx_charge_current(chip,
						     NU1619_FASTCHG_CURR_MAX);
		}
	}
}
...
static int nu1619_charge_get_ffc_input_current(struct oplus_nu1619_ic *chip)
{
	...
	if (ffc_level == FASTCHARGE_LEVEL_NUM ||
	...
	if (chip->nu1619_chg_status.wpc_ffc_charge == true) {
		...
		if (ffc_level != FASTCHARGE_LEVEL_UNKNOW &&
		    ffc_level < ARRAY_SIZE(ffc_max_vol)) {
			if (g_oplus_chip->batt_volt >= ffc_max_vol[ffc_level]) {
				...
				if (ffc_level >= ARRAY_SIZE(ffc_max_vol)) {
					...
					chip->nu1619_chg_status.chg_in_cv =
						true;
		...
		if (non_ffc_level == FASTCHARGE_LEVEL_UNKNOW) {
			...
			pre_rx_runing_mode =
				chip->nu1619_chg_status.rx_runing_mode;
		...
		if (non_ffc_level != FASTCHARGE_LEVEL_UNKNOW &&
		    non_ffc_level < ARRAY_SIZE(non_ffc_max_vol)) {
			if (g_oplus_chip->batt_volt >=
			    non_ffc_max_vol[non_ffc_level]) {
				non_ffc_level++;
				if (non_ffc_max_vol[non_ffc_level] == 0)
					non_ffc_level++;
				if (chip->nu1619_chg_status.rx_runing_mode ==
	...
	if (chip->nu1619_chg_status.rx_runing_mode == RX_RUNNING_MODE_EPP_15W &&
		...
		target_ichg_to_input_current(vbatt, target_ichg, epp_table, i,
					     input_current);
	if (chip->nu1619_chg_status.adapter_type != ADAPTER_TYPE_SVOOC_50W &&
	    chip->nu1619_chg_status.adapter_type != ADAPTER_TYPE_SVOOC &&
	    chip->nu1619_chg_status.adapter_type != ADAPTER_TYPE_VOOC &&
	    chip->nu1619_chg_status.adapter_type != ADAPTER_TYPE_EPP)
		target_ichg_to_input_current(vbatt, target_ichg, bpp_table, i,
					     input_current);
...
static void nu1619_charge_set_target_ichg(struct oplus_nu1619_ic *chip)
{
	...
	if (skewing_input_current > 0 &&
	    skewing_input_current < chip->nu1619_chg_status.target_iin)
		chip->nu1619_chg_status.target_iin = skewing_input_current;

	if (abs(chip->nu1619_chg_status.iout_stated_current -
		chip->nu1619_chg_status.target_iin) > 200) {
		if (chip->nu1619_chg_status.iout_stated_current >
		    chip->nu1619_chg_status.target_iin) {
			tmp_target_iin =
				chip->nu1619_chg_status.iout_stated_current -
				200;
	...
	if (chip->nu1619_chg_status.wpc_chg_param.pre_target_ichg !=
		    target_ichg ||
	    chip->nu1619_chg_status.iout_stated_current !=
		    chip->nu1619_chg_status.target_iin) {
		if (tmp_target_iin == chip->nu1619_chg_status.target_iin) {
			chip->nu1619_chg_status.wpc_chg_param.pre_target_ichg =
				target_ichg;
			if (chip->nu1619_chg_status.adapter_type ==
			    ADAPTER_TYPE_EPP)
		...
		if (tmp_target_iin != 0) {
			chip->nu1619_chg_status.iout_stated_current =
				tmp_target_iin;
			if (is_ext_chg_ops()) {
				oplus_chg_set_input_current_without_aicl(
					tmp_target_iin);
			} else {
				g_oplus_chip->chg_ops->wls_input_current_write(
					tmp_target_iin);
	...
	case ADAPTER_TYPE_EPP:
		if ((chip->nu1619_chg_status.iout_stated_current ==
			     epp_table[0].input_current &&
		     chip->nu1619_chg_status.iout <
			     epp_table[0].input_current - 100 &&
		     epp_table[0].input_current > 0) ||
		    (chip->nu1619_chg_status.iout_stated_current ==
			     epp_table[1].input_current &&
		     chip->nu1619_chg_status.iout <
			     epp_table[1].input_current - 100 &&
		     epp_table[1].input_current > 0) ||
		    (chip->nu1619_chg_status.iout_stated_current ==
			     epp_table[2].input_current &&
		     chip->nu1619_chg_status.iout <
			     epp_table[2].input_current - 100 &&
	...
	default:
		if (chip->nu1619_chg_status.iout_stated_current ==
			    bpp_table[0].input_current &&
		    chip->nu1619_chg_status.iout <
			    bpp_table[0].input_current - 100 &&
	...
	chg_err("<~WPC~> t_iin-iout[%d-%d]: t-cool-cv_ichg[%d-%d-%d], input[%d]; %sffc-skew_ichg[%d-%d], input[%d-%d]; vbatt[%d]\n",
		...
		chip->nu1619_chg_status.iout_stated_current,
		chip->nu1619_chg_status.wpc_chg_param.target_ichg,
		cool_down_target_ichg, cv_target_ichg, input_current,
		(chip->nu1619_chg_status.wpc_ffc_charge == true) ? "" : "non-",
		fasctchg_current[chip->nu1619_chg_status.fastcharge_level],
		fasctchg_current[chip->nu1619_chg_status
					 .skewing_fastcharge_level],
...
static void nu1619_update_debug_info(struct oplus_nu1619_ic *chip)
{
	chip->nu1619_chg_status.highest_temp =
		max(chip->nu1619_chg_status.highest_temp,
		    g_oplus_chip->temperature);

	chip->nu1619_chg_status.max_iout = max(chip->nu1619_chg_status.max_iout,
					       chip->nu1619_chg_status.iout);
	...
	if (g_oplus_chip->cool_down != 0)
		chip->nu1619_chg_status.min_cool_down =
			min(chip->nu1619_chg_status.min_cool_down,
			    g_oplus_chip->cool_down);
	...
	if (chip->nu1619_chg_status.skewing_fastcharge_level !=
	    FASTCHARGE_LEVEL_UNKNOW)
		chip->nu1619_chg_status.min_skewing_current = min(
			chip->nu1619_chg_status.min_skewing_current,
			fasctchg_current[chip->nu1619_chg_status
						 .skewing_fastcharge_level]);
#ifdef SUPPORT_OPLUS_WPC_VERIFY
	...
	case BATTERY_STATUS__NORMAL:
		...
		case ADAPTER_TYPE_VOOC:
			chip->nu1619_chg_status.fastchg_current_limit =
				chip->nu1619_chg_status.wpc_chg_param
					.vooc_temp_normal_fastchg_ma;
			charging_current = chip->nu1619_chg_status.wpc_chg_param
						   .vooc_temp_normal_fastchg_ma;
			if (chip->nu1619_chg_status.normal_temp_region ==
			    NORMAL_TEMP_REGION3) {
				if (chip->nu1619_chg_status.charge_status !=
		...
		if (pre_tbatt == BATTERY_STATUS__REMOVED ||
		    ...
		    (pre_tbatt == BATTERY_STATUS__COLD_TEMP &&
		     ...
		     chip->nu1619_chg_status.adapter_type !=
			     ADAPTER_TYPE_EPP)) {
			if ((g_oplus_chip->temperature >
			     g_oplus_chip->limits.little_cold_bat_decidegc) &&
			    (g_oplus_chip->temperature <
			     g_oplus_chip->limits.warm_bat_decidegc)) {
				chip->nu1619_chg_status.adapter_type =
					ADAPTER_TYPE_UNKNOW;
...
static int oplus_wpc_set_input_current(struct oplus_nu1619_ic *chip)
{
	...
	if ((g_oplus_chip->sw_full || g_oplus_chip->hw_full_by_sw) &&
	    g_oplus_chip->in_rechging == false) {
		chip->nu1619_chg_status.wpc_chg_param.pre_input_ma = 0;
		current_limit = 300;
...
static int nu1619_charge_status_process(struct oplus_nu1619_ic *chip)
{
...
#ifndef FASTCHG_TEST_BY_TIME
	if ((g_oplus_chip->temperature <
	     g_oplus_chip->limits.cold_bat_decidegc) ||
	    (g_oplus_chip->temperature >
	     g_oplus_chip->limits.hot_bat_decidegc)) {
		chg_err("<~WPC~> The temperature is abnormal, stop charge!\n");
		if (!is_ext_chg_ops())
			oplus_chg_set_vindpm_vol(4100);
		if (chip->nu1619_chg_status.charge_current !=
		    WPC_CHARGE_CURRENT_ZERO) {
			nu1619_set_rx_charge_current(chip,
						     WPC_CHARGE_CURRENT_ZERO);
		}
	...
	case WPC_CHG_STATUS_DEFAULT:
		...
		if ((chip->nu1619_chg_status.adapter_type ==
			...
			if (g_oplus_chip->batt_volt_max > 4100) {
				...
				freq_thr =
					chip->nu1619_chg_status.freq_threshold +
					(g_oplus_chip->batt_volt_max - 3700) /
						200;
			} else {
				freq_thr =
					chip->nu1619_chg_status.freq_threshold;
	...
	case WPC_CHG_STATUS_READY_FOR_EPP:
		...
		if (chip->nu1619_chg_status.epp_current_step ==
		    WPC_CHARGE_CURRENT_EPP_INIT) {
		...
		if (chip->nu1619_chg_status.rx_runing_mode ==
			...
			if ((chip->nu1619_chg_status.epp_current_limit >=
				     chip->nu1619_chg_status.wpc_chg_param
					     .epp_temp_warm_input_ma &&
			     oplus_chg_get_tbatt_status() ==
	...
	case WPC_CHG_STATUS_READY_FOR_FASTCHG:
		...
		if (is_ext_chg_ops()) {
			g_oplus_chip->chg_ops->input_current_write_without_aicl(
				WPC_CHARGE_CURRENT_LIMIT_300MA);
		} else {
			g_oplus_chip->chg_ops->wls_input_current_write(
				WPC_CHARGE_CURRENT_LIMIT_300MA);
		}
		...
		schedule_delayed_work(
			&chip->charger_suspend_work,
			round_jiffies_relative(msecs_to_jiffies(12000)));
		nu1619_set_rx_charge_voltage(chip,
					     WPC_CHARGE_VOLTAGE_FASTCHG_INIT);
		if (is_ext_chg_ops()) {
			oplus_chg_disable_buck_switch();
			oplus_chg_disable_async_mode();
		}
	...
	case WPC_CHG_STATUS_WAITING_FOR_TX_INTO_FASTCHG:
		...
		if (chip->nu1619_chg_status.vout >= 11500 &&
		    charger_suspend == true) {
			msleep(300);
			g_oplus_chip->chg_ops->charger_unsuspend();
		...
		if (chip->nu1619_chg_status.charge_type ==
		    WPC_CHARGE_TYPE_FAST) {
			if (nu1619_get_CEP_flag(chip) != 0) {
#ifndef FASTCHG_TEST_BY_TIME
				cep_nonzero_cnt++;
				if (cep_nonzero_cnt > 10) {
					...
					chip->nu1619_chg_status.skewing_info =
						true;
					if (g_oplus_chip->wpc_no_chargerpump)
						nu1619_test_reset_record_seconds();
					if (chip->nu1619_chg_status.cep_info <
					    to_cep_info(chip->nu1619_chg_status
								.CEP_value))
						chip->nu1619_chg_status
							.cep_info = to_cep_info(
							chip->nu1619_chg_status
								.CEP_value);
				}
#endif
		...
		if (chip->nu1619_chg_status.idt_adc_test_enable) {
			if (chip->nu1619_chg_status.iout < 200) {
				chg_err("<~WPC~> idt_adc_test iout: %dmA < 200mA\n",
					chip->nu1619_chg_status.iout);
				chip->nu1619_chg_status.idt_adc_test_result =
					false;
				break;
			} else {
				chg_err("<~WPC~> idt_adc_test iout: %dmA >= 200mA\n",
	...
	case WPC_CHG_STATUS_INCREASE_VOLTAGE_FOR_CHARGEPUMP:
		...
		if (nu1619_get_CEP_flag(chip) == 0) {
			temp_value = nu1619_increase_vout_to_target(
				chip, g_oplus_chip->batt_volt * 4 + 300,
				WPC_CHARGE_VOLTAGE_CHGPUMP_MAX);
			if (temp_value != 0) {
				nu1619_reset_CEP_flag(chip);
			} else {
	...
	case WPC_CHG_STATUS_CHARGER_FASTCHG_INIT:
		...
		nu1619_set_rx_charge_voltage(
			chip, NU1619_WPC_CHARGE_VOLTAGE_FASTCHG_MIN);
		cep_zero_cnt = 0;
		cep_nonzero_cnt = 0;
		chip->nu1619_chg_status.wpc_ffc_charge = false;
		...
		if (chip->nu1619_chg_status.adapter_type == ADAPTER_TYPE_VOOC)
			nu1619_set_rx_charge_current(
				chip, NU1619_FASTCHG_CURR_MIDDLE);
		else
			nu1619_set_rx_charge_current(chip,
						     NU1619_FASTCHG_CURR_MAX);
		...
		if (oplus_chg_get_tbatt_status() != BATTERY_STATUS__COLD_TEMP &&
		    oplus_chg_get_tbatt_status() != BATTERY_STATUS__WARM_TEMP)
			nu1619_set_rx_terminate_voltage(
				chip, WPC_TERMINATION_VOLTAGE_CV);
		chip->nu1619_chg_status.charge_status =
	...
	case WPC_CHG_STATUS_CHARGEPUMP_INIT:
		...
		if (chip->nu1619_chg_status.vout <
		    (g_oplus_chip->batt_volt * 4 + 300)) {
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHGPUMP:
		...
		if ((g_oplus_chip->batt_volt >= 4500) &&
			...
			break;
			     ...
			     WPC_CHARGE_FFC_TEMP_MIN) ||
			    (g_oplus_chip->temperature >
			     WPC_CHARGE_FFC_TEMP_MAX)) &&
			   (g_oplus_chip->batt_volt > 4370)) {
			chg_err("<~WPC~> batt_volt[%d], temperature[%d]\n",
				g_oplus_chip->batt_volt,
				g_oplus_chip->temperature);
		...
		if ((nu1619_get_CEP_flag(chip) == 0) &&
		    (chip->nu1619_chg_status.wpc_skewing_proc == false)) {
			cep_nonzero_cnt = 0;
			if (chip->nu1619_chg_status.work_silent_mode ||
			    chip->nu1619_chg_status.call_mode) {
				cep_nonzero_cnt = 0;
				chip->nu1619_chg_status.wpc_ffc_charge = false;
			...
			if (nu1619_get_CEP_flag(chip) == 0) {
				...
				if (cep_zero_cnt >= 10) {
					cep_zero_cnt = 0;
					chip->nu1619_chg_status
						.wpc_skewing_proc = false;
					chg_err("<~WPC~> turn to wpc_skewing_proc = false\n");

					if (nu1619_resume_chargepump_fastchg(
						    chip) == 0) {
						chip->nu1619_chg_status
							.wpc_ffc_charge = false;
						nu1619_ready_to_switch_to_charger(
							chip, true);
						break;
					}
				}
				...
				if (chip->nu1619_chg_status.wpc_skewing_proc ==
				    true) {
					nu1619_fastcharge_skewing_proc_40w(
						chip, false);
				} else {
					cep_nonzero_cnt++;
					if (cep_nonzero_cnt >= 3) {
		...
		if (nu1619_get_CEP_flag(chip) == 0) {
			...
			if (chip->nu1619_chg_status.iout_stated_current >
			    WPC_CHARGE_CURRENT_OFFSET) {
				iout_min_value = chip->nu1619_chg_status
							 .iout_stated_current -
						 WPC_CHARGE_CURRENT_OFFSET;
			} else {
				iout_min_value = 0;
			...
			if (chip->nu1619_chg_status.iout > iout_max_value) {
			...
			} else if (chip->nu1619_chg_status.iout <
				   iout_min_value) {
				chg_err("<~WPC~> The Iout < %d.\n",
					iout_min_value);
				if (chip->nu1619_chg_status.charge_voltage <
				...
				if (!chip->nu1619_chg_status
					     .wpc_reach_stable_charge) {
					chg_err("<~WPC~> set wpc_reach_stable_charge = true\n");
					chip->nu1619_chg_status
						.wpc_reach_stable_charge = true;
					if (g_oplus_chip->batt_volt >= 4370) {
	...
	case WPC_CHG_STATUS_DECREASE_VOUT_TO_12V:
		if ((nu1619_get_CEP_flag(chip) == 0) ||
		    chip->nu1619_chg_status.wpc_skewing_proc) {
			temp_value = nu1619_decrease_vout_to_target(
				chip, WPC_CHARGE_VOLTAGE_CHGPUMP_TO_CHARGER,
				WPC_CHARGE_VOLTAGE_CHGPUMP_MIN);
			if (temp_value == 0) {
				if (is_ext_chg_ops()) {
					g_oplus_chip->chg_ops
						->input_current_write_without_aicl(
							1000);
				} else {
					g_oplus_chip->chg_ops
						->wls_input_current_write(1000);
				}
				chip->nu1619_chg_status.iout_stated_current =
					1000;
				self_reset_cnt = 0;
	...
	case WPC_CHG_STATUS_INCREASE_CURRENT_FOR_CHARGER:
		...
		temp_value = nu1619_increase_vc_by_step(
			chip, chip->nu1619_chg_status.charge_current,
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHARGER:
		...
		nu1619_charge_check_ffc_status(chip);
		if (chip->nu1619_chg_status.terminate_voltage !=
			    WPC_TERMINATION_VOLTAGE_FFC &&
		    chip->nu1619_chg_status.wpc_ffc_charge == true)
			nu1619_set_rx_terminate_voltage(
				chip, WPC_TERMINATION_VOLTAGE_FFC);
		if (chip->nu1619_chg_status.terminate_voltage !=
			    WPC_TERMINATION_VOLTAGE_CV &&
		    chip->nu1619_chg_status.wpc_ffc_charge == false &&
...
#ifdef SUPPORT_OPLUS_WPC_VERIFY
		if (chip->nu1619_chg_status.target_iin ==
			    chip->nu1619_chg_status.iout_stated_current &&
		    work_delay <= 0) {
			if (chip->nu1619_chg_status.dock_verify_retry > 0 &&
			    chip->nu1619_chg_status.dock_verify_status ==
				    DOCK_VERIFY_UNKOWN) {
				if (chip->nu1619_chg_status.dock_verify_retry ==
				    DOCK_VERIFY_RETRY_TIMES) {
...
static int oplus_wpc_track_upload_trx_err_info(struct oplus_nu1619_ic *chip,
					       char *trx_crux_info, u8 trx_err)
{
	int index = 0;
	...
	memset(chip->trx_err_load_trigger.crux_info, 0,
	       sizeof(chip->trx_err_load_trigger.crux_info));
	index += snprintf(&(chip->trx_err_load_trigger.crux_info[index]),
			  OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$err_reason@@%s", trx_err_reason);
	if (trx_crux_info && strlen(trx_crux_info))
		index +=
			snprintf(&(chip->trx_err_load_trigger.crux_info[index]),
				 OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s",
				 trx_crux_info);
...
static void nu1619_idt_dischg_status(struct oplus_nu1619_ic *chip)
{
	...
	if (regdata[1] != 0) {
		...
		case WPC_DISCHG_IC_ERR_TX_RXEPT:
			...
			if (chip->nu1619_chg_status.wpc_dischg_status !=
			    WPC_DISCHG_IC_ERR_TX_RXEPT) {
				chip->nu1619_chg_status.wpc_chg_err =
					chip->nu1619_chg_status
						.wpc_dischg_status;
				oplus_wpc_update_track_info(chip,
							    trx_crux_info);
				oplus_wpc_track_upload_trx_err_info(
					chip, trx_crux_info,
					chip->nu1619_chg_status
						.wpc_dischg_status);
			}
...
static void nu1619_idt_dischg_work(struct work_struct *work)
{
	...
	struct oplus_nu1619_ic *chip =
		container_of(dwork, struct oplus_nu1619_ic, idt_dischg_work);
		...
		if (rc != 3) {
			...
			if (val_buf[0] == 0x4F || val_buf[0] == 0x5F) {
				...
				chg_err("<~WPC~> Received TX command: [0x%02X, 0x%02X], data: [0x%02X, 0x%02X]\n",
					tx_command, tx_command_r, tx_data,
					tx_data_r);
				switch (tx_command) {
				case P9237_RESPONE_ADAPTER_TYPE:
					if ((tx_command == tx_command_r) &&
					    (tx_data == tx_data_r)) {
						nu1619_set_FOD_parameter(chip,
									 1);
						chip->nu1619_chg_status
							.adapter_type =
							(tx_data & 0x07);
						...
						chg_err("<~WPC~> get adapter type = 0x%02X, dock hw version = 0x%02X\n",
							...
							chip->nu1619_chg_status
								.dock_version);
						nu1619_config_fan_pwm_pulse_value(
							chip);
						if (chip->nu1619_chg_status
							    .adapter_type ==
						    ADAPTER_TYPE_PD_65W)
				...
				case P9237_RESPONE_SEND_1ST_DECODE_DATA:
					...
					chip->nu1619_chg_status
						.tx_encode_num[0] = val_buf[2];
					chip->nu1619_chg_status
						.tx_encode_num[1] = val_buf[3];
					chip->nu1619_chg_status
						.tx_encode_num[2] = val_buf[4];
					chip->nu1619_chg_status.send_message =
						P9221_CMD_GET_2ND_ENCODE_DATA;
					break;
				case P9237_RESPONE_SEND_2ND_DECODE_DATA:
					...
					chip->nu1619_chg_status
						.tx_encode_num[3] = val_buf[2];
					chip->nu1619_chg_status
						.tx_encode_num[4] = val_buf[3];
					chip->nu1619_chg_status
						.tx_encode_num[5] = val_buf[4];
					chip->nu1619_chg_status.send_message =
						P9221_CMD_GET_3RD_ENCODE_DATA;
					break;
				case P9237_RESPONE_SEND_3RD_DECODE_DATA:
					...
					chip->nu1619_chg_status
						.tx_encode_num[6] = val_buf[2];
					chip->nu1619_chg_status
						.tx_encode_num[7] = val_buf[3];
...
static void nu1619_idt_event_int_func(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip =
		container_of(dwork, struct oplus_nu1619_ic, idt_event_int_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip = container_of(
		dwork, struct oplus_nu1619_ic, idt_event_int_probe_work);
...
static void nu1619_idt_connect_int_func(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip = container_of(
		dwork, struct oplus_nu1619_ic, idt_connect_int_work);
	int con_counts = 0;
	...
	if (nu1619_chip->booster_en_gpio <= 0 &&
	    oplus_get_wired_otg_online() == true &&
	...
	if (nu1619_get_idt_con_val() == 1) {
		...
		if (nu1619_chip->nu1619_chg_status.charge_online == false) {
			...
			} else if (chip->nu1619_chg_status.adapter_type ==
				   ADAPTER_TYPE_UNKNOW) {
				chg_err("<~WPC~> tx A1\n");
				nu1619_set_tx_charger_dect(chip);
			}
		...
		if (nu1619_chip->nu1619_chg_status.charge_online == true) {
			...
			schedule_delayed_work(
				&chip->charger_disconnect_work,
				round_jiffies_relative(msecs_to_jiffies(6000)));

			if (charger_suspend ||
			    atomic_read(&chip->volt_low_flag) == 1) {
				g_oplus_chip->chg_ops->charger_unsuspend();
				charger_suspend = false;
				atomic_set(&chip->volt_low_flag, 0);
			}
...
static void nu1619_idt_int_eint_register(struct oplus_nu1619_ic *chip)
{
	...
	retval = request_irq(chip->idt_int_irq, irq_idt_event_int_handler,
			     IRQF_TRIGGER_FALLING, "nu1619_idt_int", chip);
	if (retval < 0) {
...
static int nu1619_ext1_wired_otg_gpio_init(struct oplus_nu1619_ic *chip)
{
	...
	chip->ext1_wired_otg_en_default = pinctrl_lookup_state(
		chip->pinctrl, "ext1_wired_otg_en_default");
...
void nu1619_set_ext1_wired_otg_en_val(int value)
{
	...
	chg_err("<~WPC~>set value:%d, gpio_val:%d\n", value,
		gpio_get_value(chip->ext1_wired_otg_en_gpio));
}
...
static int nu1619_ext2_wireless_otg_gpio_init(struct oplus_nu1619_ic *chip)
{
	...
	chip->ext2_wireless_otg_en_active = pinctrl_lookup_state(
		chip->pinctrl, "ext2_wireless_otg_en_active");
	if (IS_ERR_OR_NULL(chip->ext2_wireless_otg_en_active)) {
		chg_err("get ext2_wireless_otg_en_active fail\n");
		return -EINVAL;
	}

	chip->ext2_wireless_otg_en_sleep = pinctrl_lookup_state(
		chip->pinctrl, "ext2_wireless_otg_en_sleep");
	if (IS_ERR_OR_NULL(chip->ext2_wireless_otg_en_sleep)) {
		chg_err("get ext2_wireless_otg_en_sleep fail\n");
		return -EINVAL;
	}

	chip->ext2_wireless_otg_en_default = pinctrl_lookup_state(
		chip->pinctrl, "ext2_wireless_otg_en_default");
...
void nu1619_set_ext2_wireless_otg_en_val(int value)
{
	...
	chg_err("<~WPC~>set value:%d, gpio_val:%d\n", value,
		gpio_get_value(chip->ext2_wireless_otg_en_gpio));
}
...
static int nu1619_ext3_wireless_otg_gpio_init(struct oplus_nu1619_ic *chip)
{
	...
	chip->ext3_wireless_otg_en_active = pinctrl_lookup_state(
		chip->pinctrl, "ext3_wireless_otg_en_active");
	if (IS_ERR_OR_NULL(chip->ext3_wireless_otg_en_active)) {
		chg_err("get ext3_wireless_otg_en_active fail\n");
		return -EINVAL;
	}

	chip->ext3_wireless_otg_en_sleep = pinctrl_lookup_state(
		chip->pinctrl, "ext3_wireless_otg_en_sleep");
	if (IS_ERR_OR_NULL(chip->ext3_wireless_otg_en_sleep)) {
		chg_err("get ext3_wireless_otg_en_sleep fail\n");
		return -EINVAL;
	}

	chip->ext3_wireless_otg_en_default = pinctrl_lookup_state(
		chip->pinctrl, "ext3_wireless_otg_en_default");
...
void nu1619_set_ext3_wireless_otg_en_val(int value)
{
	...
	chg_err("<~WPC~>set value:%d, gpio_val:%d\n", value,
		gpio_get_value(chip->ext3_wireless_otg_en_gpio));
}
...
void nu1619_set_vt_sleep_val(int value)
{
	...
	chg_err("<~WPC~>set value:%d, gpio_val:%d\n", value,
		gpio_get_value(chip->vt_sleep_gpio));
}
...
static int oplus_wrx_otg_en_gpio_init(struct oplus_nu1619_ic *chip)
{
	...
	chip->wrx_otg_en_active =
		pinctrl_lookup_state(chip->pinctrl, "wrx_otg_en_active");
	if (IS_ERR_OR_NULL(chip->wrx_otg_en_active)) {
		chg_err("get wrx_otg_en_active fail\n");
		return -EINVAL;
	}
	chip->wrx_otg_en_sleep =
		pinctrl_lookup_state(chip->pinctrl, "wrx_otg_en_sleep");
	if (IS_ERR_OR_NULL(chip->wrx_otg_en_sleep)) {
		chg_err("get wrx_otg_en_sleep fail\n");
		return -EINVAL;
	}
	chip->wrx_otg_en_default =
		pinctrl_lookup_state(chip->pinctrl, "wrx_otg_en_default");
...
static int oplus_wls_pg_gpio_init(struct oplus_nu1619_ic *chip)
{
	...
	chip->wls_pg_active =
		pinctrl_lookup_state(chip->pinctrl, "wls_pg_active");
	...
	chip->wls_pg_default =
		pinctrl_lookup_state(chip->pinctrl, "wls_pg_default");
...
static int nu1619_idt_gpio_init(struct oplus_nu1619_ic *chip)
{
	...
	chip->ext1_wired_otg_en_gpio =
		of_get_named_gpio(node, "qcom,ext1_wired_otg_en-gpio", 0);
	if (chip->ext1_wired_otg_en_gpio < 0) {
		pr_err("chip->ext1_wired_otg_en_gpio not specified\n");
	} else {
		if (gpio_is_valid(chip->ext1_wired_otg_en_gpio)) {
			rc = gpio_request(chip->ext1_wired_otg_en_gpio,
					  "ext1_wired_otg_en-gpio");
			if (rc) {
				pr_err("unable to request gpio [%d]\n",
				       chip->ext1_wired_otg_en_gpio);
			} else {
	...
	chip->ext2_wireless_otg_en_gpio =
		of_get_named_gpio(node, "qcom,ext2_wireless_otg_en-gpio", 0);
	if (chip->ext2_wireless_otg_en_gpio < 0) {
		pr_err("chip->ext2_wireless_otg_en_gpio not specified\n");
	} else {
		if (gpio_is_valid(chip->ext2_wireless_otg_en_gpio)) {
			rc = gpio_request(chip->ext2_wireless_otg_en_gpio,
					  "ext2_wireless_otg_en-gpio");
			if (rc) {
				pr_err("unable to request gpio [%d]\n",
				       chip->ext2_wireless_otg_en_gpio);
			} else {
	...
	chip->ext3_wireless_otg_en_gpio =
		of_get_named_gpio(node, "qcom,ext3_wireless_otg_en-gpio", 0);
	if (chip->ext3_wireless_otg_en_gpio < 0) {
		pr_err("chip->ext3_wireless_otg_en_gpio not specified\n");
	} else {
		if (gpio_is_valid(chip->ext3_wireless_otg_en_gpio)) {
			rc = gpio_request(chip->ext3_wireless_otg_en_gpio,
					  "ext3_wireless_otg_en-gpio");
			if (rc) {
				pr_err("unable to request gpio [%d]\n",
				       chip->ext3_wireless_otg_en_gpio);
			} else {
	...
	chip->booster_en_gpio =
		of_get_named_gpio(node, "qcom,booster_en-gpio", 0);
	if (chip->booster_en_gpio < 0) {
	...
	chip->cp_ldo_5v_gpio =
		of_get_named_gpio(node, "qcom,cp_ldo_5v-gpio", 0);
	if (chip->cp_ldo_5v_gpio < 0) {
		pr_err("chip->cp_ldo_5v_gpio not specified\n");
	} else {
		if (gpio_is_valid(chip->cp_ldo_5v_gpio)) {
			rc = gpio_request(chip->cp_ldo_5v_gpio,
					  "cp-ldo-5v-gpio");
			if (rc) {
				pr_err("unable to request gpio [%d]\n",
				       chip->cp_ldo_5v_gpio);
	...
	chip->wrx_otg_en_gpio =
		of_get_named_gpio(node, "qcom,wrx_otg_en-gpio", 0);
	if (chip->wrx_otg_en_gpio < 0) {
		pr_err("chip->wrx_otg_en_gpio not specified\n");
	} else {
		if (gpio_is_valid(chip->wrx_otg_en_gpio)) {
			rc = gpio_request(chip->wrx_otg_en_gpio,
					  "wrx-otg-en-gpio");
			if (rc) {
	...
	if (chip->wls_pg_gpio < 0) {
		...
		if (oplus_wls_pg_is_support(chip)) {
			if (gpio_is_valid(chip->wls_pg_gpio)) {
				rc = gpio_request(chip->wls_pg_gpio,
						  "wls-pg-gpio");
				if (rc) {
...
static ssize_t nu1619_reg_store(struct file *filp, const char __user *buff,
...
static ssize_t nu1619_reg_show(struct file *filp, char __user *buff,
...
static void init_nu1619_add_log(void)
{
	...
	p = proc_create("nu1619_add_log", 0664, NULL,
			&nu1619_add_log_proc_fops);
	if (!p) {
		pr_err("proc_create init_nu1619_add_log_proc_fops fail!\n");
	}
}

static ssize_t nu1619_data_log_write(struct file *filp, const char __user *buff,
				     size_t len, loff_t *data)
{
	char write_data[32] = { 0 };
	int critical_log = 0;
	...
	rc = nu1619_config_interface(nu1619_chip, nu1619_add, critical_log,
				     0xFF);
	if (rc) {
...
static void init_nu1619_data_log(void)
{
	...
	p = proc_create("nu1619_data_log", 0664, NULL,
			&nu1619_data_log_proc_fops);
	if (!p) {
...
static void nu1619_task_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip =
		container_of(dwork, struct oplus_nu1619_ic, nu1619_task_work);
	...
	if (nu1619_get_idt_con_val() == 0) {
		...
		if (idt_disconnect_cnt >= 2) {
			if (nu1619_chip->nu1619_chg_status.charge_online) {
				...
				schedule_delayed_work(
					&nu1619_chip->idt_connect_int_work, 0);
			}
		}
	...
	if (nu1619_chip->nu1619_chg_status.charge_online) {
		...
		case WPC_CHG_STATUS_START_VERIFY:
			schedule_delayed_work(
				...
				round_jiffies_relative(msecs_to_jiffies(1000)));
		case WPC_CHG_STATUS_WAITING_VERIFY:
			schedule_delayed_work(&chip->nu1619_task_work,
...
static void nu1619_CEP_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip =
		container_of(dwork, struct oplus_nu1619_ic, nu1619_CEP_work);

	if (nu1619_chip->nu1619_chg_status.charge_online) {
		nu1619_detect_CEP(chip);
		schedule_delayed_work(&chip->nu1619_CEP_work,
				      P922X_CEP_INTERVAL);
	}
}
...
static void nu1619_update_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip =
		container_of(dwork, struct oplus_nu1619_ic, nu1619_update_work);
	int rc;
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
...
static void nu1619_self_reset_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip = container_of(
		dwork, struct oplus_nu1619_ic, nu1619_self_reset_work);
...
static void nu1619_test_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip =
		container_of(dwork, struct oplus_nu1619_ic, nu1619_test_work);
...
static void charger_suspend_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip = container_of(
		dwork, struct oplus_nu1619_ic, charger_suspend_work);
...
static void charger_disconnect_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip = container_of(
		dwork, struct oplus_nu1619_ic, charger_disconnect_work);
...
static void charger_start_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip =
		container_of(dwork, struct oplus_nu1619_ic, charger_start_work);
...
start:
	...
	case UPGRADE_START:
		...
		if (fw_head->magic[0] == 0x02 && fw_head->magic[1] == 0x00 &&
			...
			chg_err("<IDT UPDATE>image header verification succeeded, fw_size=%d\n",
				fw_size);
			if (copy_from_user(
				    fw_buf, buf + sizeof(struct idt_fw_head),
	...
	case UPGRADE_END:
		...
		if (is_ext_chg_ops()) {
			...
			oplus_wireless_set_otg_en_val(0);
			msleep(100);
			rc = nu1619_upgrade_firmware(nu1619_chip, fw_buf,
						     fw_size, fw_type);
			g_oplus_chip->chg_ops->otg_disable();
			oplus_set_wrx_en_value(0);
			oplus_set_wls_pg_value(1);
...
static ssize_t proc_wireless_rx_freq_read(struct file *file, char __user *buf,
					  size_t count, loff_t *ppos)
{
	int rc;
	char string[8];
						   ...
						   size_t len, loff_t *lo)
{
	...
	if (pmw_pulse == FASTCHG_MODE) {
		...
		rc = nu1619_set_dock_fan_pwm_pulse(
			chip->nu1619_chg_status.dock_fan_pwm_pulse_silent);
		if (rc == 0) {
			chip->quiet_mode_ack = false;
...
static ssize_t proc_wireless_rx_power_read(struct file *file, char __user *buf,
					   size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[16] = { 0 };
...
static ssize_t proc_wireless_rx_power_write(struct file *file,
					    const char __user *buf,
					    size_t count, loff_t *lo)
{
	return count;
...
static ssize_t proc_wireless_tx_power_read(struct file *file, char __user *buf,
					   size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[16] = { 0 };
...
static ssize_t proc_wireless_tx_power_write(struct file *file,
					    const char __user *buf,
					    size_t count, loff_t *lo)
{
	return count;
...
static ssize_t proc_wireless_rx_version_read(struct file *file,
					     char __user *buf, size_t count,
					     loff_t *ppos)
{
	uint8_t ret = 0;
...
static ssize_t proc_wireless_rx_version_write(struct file *file,
					      const char __user *buf,
					      size_t count, loff_t *lo)
{
	return count;
...
static ssize_t proc_wired_otg_online_read(struct file *file, char __user *buf,
					  size_t count, loff_t *ppos)
{
	uint8_t ret = 0;
	char page[16] = { 0 };
...
static ssize_t proc_wired_otg_online_write(struct file *file,
					   const char __user *buf, size_t count,
					   loff_t *lo)
{
	return count;
...
static int init_wireless_charge_proc(struct oplus_nu1619_ic *chip)
{
	...
	prEntry_tmp =
		proc_create_data("user_sleep_mode", 0664, prEntry_da,
...
static void oplus_chg_wls_status_keep_clean_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip = container_of(
		dwork, struct oplus_nu1619_ic, status_keep_clean_work);
	struct power_supply *wls_psy;
...
static enum power_supply_property nu1619_wireless_props[] = {
	...
	POWER_SUPPLY_PROP_REAL_TYPE,	  POWER_SUPPLY_PROP_TX_VOLTAGE_NOW,
	POWER_SUPPLY_PROP_TX_CURRENT_NOW, POWER_SUPPLY_PROP_CP_VOLTAGE_NOW,
	POWER_SUPPLY_PROP_CP_CURRENT_NOW, POWER_SUPPLY_PROP_WIRELESS_MODE,
	...
	case POWER_SUPPLY_PROP_CEP_INFO:
		if (val->intval == 0) {
			...
			if (!nu1619_chip->nu1619_chg_status.wpc_skewing_proc) {
				nu1619_chip->nu1619_chg_status.skewing_info =
					false;
...
static int nu1619_wpc_get_max_wireless_power(void)
{
	...
	adapter_wireless_power =
		nu1619_get_r_power(chip->nu1619_chg_status.adapter_power);
	...
	max_wireless_power =
		adapter_wireless_power >
			...
			chip->nu1619_chg_status.wireless_power :
			adapter_wireless_power;
	max_wireless_power = max_wireless_power > base_wireless_power ?
				     base_wireless_power :
				     max_wireless_power;
...
static void
nu1619_wpc_track_trx_info_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip = container_of(
		dwork, struct oplus_nu1619_ic, trx_info_load_trigger_work);
...
static void nu1619_wpc_track_trx_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_nu1619_ic *chip = container_of(
		dwork, struct oplus_nu1619_ic, trx_err_load_trigger_work);
...
static int nu1619_wpc_track_init(struct oplus_nu1619_ic *chip)
{
	...
	chip->trx_err_load_trigger.type_reason =
		TRACK_NOTIFY_TYPE_DEVICE_ABNORMAL;
	chip->trx_err_load_trigger.flag_reason =
		TRACK_NOTIFY_FLAG_WLS_TRX_ABNORMAL;
	chip->trx_info_load_trigger.type_reason =
		TRACK_NOTIFY_TYPE_GENERAL_RECORD;
	chip->trx_info_load_trigger.flag_reason =
			       ...
			       const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct oplus_nu1619_ic),
			    GFP_KERNEL);
	if (!chip) {
	...
	INIT_DELAYED_WORK(&chip->idt_event_int_probe_work,
			  nu1619_idt_event_int_probe_func);
	INIT_DELAYED_WORK(&chip->idt_connect_int_work,
			  nu1619_idt_connect_int_func);
	...
	INIT_DELAYED_WORK(&chip->nu1619_update_work,
			  nu1619_update_work_process);
	INIT_DELAYED_WORK(&chip->nu1619_self_reset_work,
			  nu1619_self_reset_process);
#ifdef FASTCHG_TEST_BY_TIME
	INIT_DELAYED_WORK(&chip->nu1619_test_work, nu1619_test_work_process);
#endif
	INIT_DELAYED_WORK(&chip->charger_suspend_work,
			  charger_suspend_work_process);
	INIT_DELAYED_WORK(&chip->charger_disconnect_work,
			  charger_disconnect_work_process);
	INIT_DELAYED_WORK(&chip->charger_start_work,
			  charger_start_work_process);
	...
	if (g_oplus_chip && !g_oplus_chip->charger_exist) {
		...
		nu1619_idt_connect_shedule_work();
		schedule_delayed_work(&nu1619_chip->idt_event_int_probe_work,
...
#ifndef CONFIG_OPLUS_CHARGER_MTK
	INIT_DELAYED_WORK(&chip->status_keep_clean_work,
			  oplus_chg_wls_status_keep_clean_work);
	chip->status_wake_lock =
		wakeup_source_register(chip->dev, "status_wake_lock");
	chip->status_wake_lock_on = false;
#endif
#endif
