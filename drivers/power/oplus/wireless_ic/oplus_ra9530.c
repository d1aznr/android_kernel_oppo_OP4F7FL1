...
int ra9530_hall_notifier_callback(struct notifier_block *nb,
				  unsigned long event, void *data);
static void ra9530_power_onoff_switch(int value);

void __attribute__((weak)) notify_pen_state(bool state, unsigned int tp_index)
{
	return;
}
...
static int __ra9530_read_reg(struct oplus_ra9530_ic *chip, int reg,
...
static int ra9530_write_reg_multi_byte(struct oplus_ra9530_ic *chip, int reg,
...
static int ra9530_read_reg(struct oplus_ra9530_ic *chip, int reg,
			   char *returnData, int count)
{
	int ret = 0;
...
static int ra9530_config_interface(struct oplus_ra9530_ic *chip, int RegNum,
				   int val, int MASK)
{
	char ra9530_reg = 0;
	int ret = 0;
...
static void ra9530_check_clear_irq(struct oplus_ra9530_ic *chip)
{
	...
	if (rc) {
		...
		ra9530_write_reg_multi_byte(chip, RA9530_REG_INT_CLR,
					    chip->int_flag_data, 4);
...
static void check_int_enable(struct oplus_ra9530_ic *chip)
{
	...
	if (rc) {
		...
		if ((reg_int[0] != 0xDF) || (reg_int[1] != 0x81) ||
			...
			reg_int[1] = 0x81;
			reg_int[2] = 0x02;
			ra9530_write_reg_multi_byte(chip, RA9530_REG_INT_EN,
						    reg_int, 3);
		}
	}
}
...
void ra9530_check_point_function(struct work_struct *work)
{
	...
	oplus_chg_debug_info.wirelesspen_info.support =
		1; /* wireless pen check point */
	/* count ble addr get timeout */
	do {
		msleep(500);
		cnt++;
		if (cnt >
		    RA9530_GET_BLE_ADDR_TIMEOUT) { /* 5s get ble addr time out*/
			g_ble_timeout_cnt++;
...
static void ra9530_set_tx_mode(int value)
{
	...
	if (rc) {
		chg_err("Couldn't read 0x%04x rc = %x\n", RA9530_REG_RTX_STATUS,
			rc);
	} else {
		if (value && (RA9530_RTX_READY & reg_tx)) {
			chg_err("set tx enable\n");
			ra9530_config_interface(chip, RA9530_REG_RTX_CMD, 0x01,
						0xFF);
		} else if (!value && (RA9530_RTX_TRANSFER & reg_tx)) {
			chg_err("set tx disable\n");
			ra9530_config_interface(chip, RA9530_REG_RTX_CMD, 0x04,
						0xFF);
		} else {
...
static void ra9530_set_ble_addr(struct oplus_ra9530_ic *chip)
{
	...
	chip->ble_mac_addr = ble_addr << 24 | decode_addr[2] << 16 |
...
static void ra9530_set_protect_parameter(struct oplus_ra9530_ic *chip)
{
	...
	chg_err("config ocp_threshold = 0x%02x, ovp_threshold = 0x%02x \
		...
		chip->ocp_threshold, chip->ovp_threshold, chip->lvp_threshold,
		chip->fod_threshold, chip->pcop_threshold1,
...
static void ra9530_send_uevent(struct device *dev, bool status,
...
static int RA9530_load_bootloader(struct oplus_ra9530_ic *chip)
{
	...
	rc = __ra9530_write_reg(chip, REG_3040,
				HEX_10); /* halt microcontroller M0*/
	if (rc != NUM_0) {
	...
	rc = __ra9530_write_reg(chip, 0x3040,
				0x10); /* halt microcontroller M0*/
	if (rc != 0) {
		chg_err("<IDT UPDATE>Write 0x3040 reg error!\n");
		return rc;
	}
	...
	package_count =
		(sizeof(MTPBootloader9530) + RA9530_FW_CODE_LENGTH - 1) /
	...
	for (index = 0; index < package_count - 1; index++) {
		rc = ra9530_write_reg_multi_byte(
			chip, RA9530_FW_CODE_LENGTH * index,
	...
	if (package_count) { /* write the last bootloader part */
		rc = ra9530_write_reg_multi_byte(
			chip, RA9530_FW_CODE_LENGTH * (package_count - 1),
			MTPBootloader9530 +
				RA9530_FW_CODE_LENGTH * (package_count - 1),
			sizeof(MTPBootloader9530) -
				RA9530_FW_CODE_LENGTH *
					(package_count -
					 1)); /* load provided by IDT array */
		if (rc != NUM_0) {
	...
	rc = __ra9530_write_reg(chip, 0x3048,
				0xF0); /* map RAM address 0x1c00 to OTP 0x0000*/
	if (rc != 0) {
...
static int RA9530_e2p_check(struct oplus_ra9530_ic *chip)
{
	...
	/* check status*/
	chip->boot_check_status = ((write_ack >> 4) & 0x07) << 8 |
				  (chip->boot_check_status & 0xFF);
	...
	rc = __ra9530_write_reg(
		chip, REG_3048,
		HEX_F0); /* map RAM address 0x1c00 to OTP 0x0000*/
	if (rc != NUM_0) {
		chg_err("[%s] <IDT UPDATE>Write 0x3048 reg error!\n", __func__);
		return rc;
	}
	...
	msleep(MS_100);
	rc = ra9530_read_reg(chip, REG_1000, (unsigned char *)&boot_version,
			     NUM_2);
	if (rc) {
	...
	chip->boot_check_status =
		(chip->boot_check_status & 0xFF00) | write_ack;
...
static int RA9530_load_fw(struct oplus_ra9530_ic *chip, unsigned char *fw_data,
			  int codelength)
{
...
static int RA9530_Crc32_fw(struct oplus_ra9530_ic *chip, int start_addr,
			   int fw_size, unsigned int crc)
{
	unsigned char write_ack = 0;
	int rc = NUM_0, retry_times = NUM_0;
...
static int RA9530_MTP(struct oplus_ra9530_ic *chip, unsigned char *fw_buf,
		      int fw_size)
{
	int rc = -1;
	int i = 0, j = 0;
				      ...
				      bool force_update)
{
	...
	power_state =
		chip->is_power_on; /* remember current power status before start upgrade */
	if (power_state) {
		ra9530_power_enable(chip, false);
		msleep(10);
	}
	...
	if (rc) {
		...
		chg_err("<IDT UPDATE>The idt fw version: %02x %02x %02x %02x beta version:%02x old_flag:%02x %02x\n",
			...
			temp[NUM_4], flag[NUM_0], flag[NUM_1]);
		...
		chg_err("<IDT UPDATE>The new fw version: %02x %02x %02x %02x beta version:%02x\n",
			...
			fw_buf[fw_ver_start_addr + 0x08],
			fw_buf[fw_ver_start_addr]);
		...
		if ((temp[NUM_0] == HEX_1) && (temp[NUM_1] == HEX_0) &&
			...
			chg_err("<IDT UPDATE> The fw version is 0x04020001-09, no More update!\n");
			   ...
			   (((temp[NUM_0] !=
			      fw_buf[fw_ver_start_addr + HEX_5]) ||
			     (temp[NUM_1] !=
			      fw_buf[fw_ver_start_addr + HEX_6]) ||
			     (temp[NUM_2] !=
			      fw_buf[fw_ver_start_addr + HEX_7]) ||
			     (temp[NUM_3] !=
			      fw_buf[fw_ver_start_addr + HEX_8]) ||
	...
	chg_err("ra9530 idt_fw_version 0x%04x,dec:%d", chip->idt_fw_version,
		chip->idt_fw_version);
...
void ra9530_ept_type_detect_func(struct oplus_ra9530_ic *chip)
{
	...
	rc = ra9530_read_reg(chip, RA9530_REG_RTX_ERR_STATUS,
			     (char *)&recv_data, 2);
	if (rc) {
		chg_err("Couldn't read 0x%04x rc = %x\n",
		...
		if ((recv_data & RA9530_RTX_ERR_TX_POCP) ||
			...
			ra9530_update_powerdown_info(chip,
...
static void ra9530_commu_data_process(struct oplus_ra9530_ic *chip)
{
	...
	chg_err("int: %02x %02x %02x %02x\n", chip->int_flag_data[0],
		chip->int_flag_data[1], chip->int_flag_data[2],
	...
	if (chip->int_flag_data[1] & RA9530_INT_FLAG_BLE_ADDR) {
		if (!chip->ble_mac_addr) {
			...
			if (ra9530_valid_check(chip)) {
				if (PEN_REASON_RECHARGE !=
				    chip->power_enable_reason) {
					ra9530_send_uevent(chip->wireless_dev,
							   chip->present,
							   chip->ble_mac_addr);
				}
			} else {
...
static void ra9530_idt_event_int_func(struct work_struct *work)
{
	...
	struct oplus_ra9530_ic *chip =
		container_of(dwork, struct oplus_ra9530_ic, idt_event_int_work);

	chg_err("ra9530_idt_event_int_func triggered!!!\n");

	__pm_stay_awake(chip->bus_wakelock);
	wait_event_interruptible_timeout(i2c_waiter, chip->i2c_ready,
					 msecs_to_jiffies(50));
	mutex_lock(&chip->flow_mutex);
...
{
	schedule_delayed_work(&ra9530_chip->idt_event_int_work,
			      msecs_to_jiffies(0));
	return IRQ_HANDLED;
}
...
static void ra9530_idt_int_eint_register(struct oplus_ra9530_ic *chip)
{
	...
	retval = request_irq(chip->idt_int_irq, irq_idt_event_int_handler,
			     IRQF_TRIGGER_FALLING, "ra9530_idt_int", chip);
	if (retval < 0) {
...
void ra9530_timer_inhall_function(struct work_struct *work)
{
	...
	while (count++ < RA9530_WAIT_TIME && !chip->present &&
	       chip->is_power_on) {
		msleep(MS_100);
	}
...
void ra9530_power_expired_do_check(struct oplus_ra9530_ic *chip)
{
	...
	if (chip->is_power_on) {
		do_gettimeofday(&now_time);
		time_offset = now_time.tv_sec * 1000 + now_time.tv_usec / 1000 -
			      chip->tx_start_time;
...
void ra9530_power_expired_check_func(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_ra9530_ic *chip =
		container_of(dwork, struct oplus_ra9530_ic, power_check_work);
...
{
	struct oplus_ra9530_ic *chip =
		container_of(work, struct oplus_ra9530_ic, power_switch_work);
...
static void ra9530_enable_func(struct work_struct *work)
{
	struct timeval now_time;
	struct oplus_ra9530_ic *chip =
		container_of(work, struct oplus_ra9530_ic, power_enable_work);
	...
	if ((chip->rx_soc > chip->soc_threshould) ||
	    ...
	    (PEN_REASON_CHARGE_FULL !=
	     chip->power_disable_reason)) { /* check second time */
		chg_err("enable failed for (%d %d %d %d).\n", chip->rx_soc,
			chip->pen_status, chip->is_power_on,
...
static ssize_t ra9530_reg_store(struct file *filp, const char __user *buff,
...
static ssize_t ra9530_reg_show(struct file *filp, char __user *buff,
...
static void init_ra9530_add_log(void)
{
	...
	p = proc_create("ra9530_add_log", 0664, NULL,
			&ra9530_add_log_proc_fops);
	if (!p) {
		chg_err("proc_create init_ra9530_add_log_proc_fops fail!\n");
	}
}

static ssize_t ra9530_data_log_write(struct file *filp, const char __user *buff,
				     size_t len, loff_t *data)
{
	char write_data[32] = { 0 };
	int critical_log = 0;
	...
	rc = ra9530_config_interface(ra9530_chip, ra9530_add, critical_log,
				     0xFF);
	if (rc) {
...
static void init_ra9530_data_log(void)
{
	...
	p = proc_create("ra9530_data_log", 0664, NULL,
			&ra9530_data_log_proc_fops);
	if (!p)
...
static void ra9530_update_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_ra9530_ic *chip =
		container_of(dwork, struct oplus_ra9530_ic, ra9530_update_work);
	int rc = 0;
	...
	if (rc == ERR_NUM) {
		...
		schedule_delayed_work(&chip->ra9530_update_work,
				      RA9530_UPDATE_RETRY_INTERVAL);
	}
}
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	if (chip->is_power_on) {
		if (chip->ble_mac_addr) {
			return sprintf(buf, "0x%lx_(Time:%dms)",
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	if (chip->is_power_on) {
		...
		chip->idt_fw_version = (temp[3] << 24) | (temp[2] << 16) |
				       (temp[1] << 8) | temp[0];
...
static ssize_t rx_soc_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	struct oplus_ra9530_ic *chip = NULL;
...
static ssize_t rx_soc_store(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t count)
{
	...
	if ((chip->rx_soc <= chip->soc_threshould) &&
	    ...
	    (false == chip->is_power_on) &&
	    (PEN_REASON_CHARGE_FULL ==
	     chip->power_disable_reason)) { /* check first time */
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < PEN_OFF_REASON_MAX; i++) {
		count += sprintf(tmp_buf + count, ":%d",
				 chip->power_disable_times[i]);
	}

	return sprintf(buf, "%d:%d\n%d\n%d:%d\n%d:%d\n%d %s\n", chip->rx_soc,
		       chip->soc_threshould, chip->is_power_on,
				    ...
				    const char *buf, size_t count)
{
	...
	if (val <= 100) {
		...
		chip->power_expired_time =
			val -
			FULL_SOC; /* we use more than 100 as power expired time */
	}
...
static ssize_t ping_time_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct oplus_ra9530_ic *chip = NULL;
...
static ssize_t tx_status_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct oplus_ra9530_ic *chip = NULL;
	int rc;
...
static ssize_t e2p_check_store(struct device *dev,
...
static ssize_t e2p_check_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct oplus_ra9530_ic *chip = NULL;
...
static int init_wireless_device(struct oplus_ra9530_ic *chip)
{
	...
	chip->wireless_dev =
		device_create(wireless_class, NULL, devt, NULL, "%s", "pencil");
			       ...
			       const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct oplus_ra9530_ic),
			    GFP_KERNEL);
	if (!chip) {
	...
	chip->bus_wakelock =
		wakeup_source_register(NULL, "ra9530_wireless_wakelock");
	present_wakelock =
		wakeup_source_register(NULL, "ra9530_present_wakelock");
	...
	INIT_DELAYED_WORK(&chip->ra9530_update_work,
			  ra9530_update_work_process);
	INIT_WORK(&ra9530_idt_timer_work, ra9530_timer_inhall_function);
	INIT_DELAYED_WORK(&chip->power_check_work,
			  ra9530_power_expired_check_func);
	INIT_DELAYED_WORK(&chip->check_point_dwork,
			  ra9530_check_point_function);
