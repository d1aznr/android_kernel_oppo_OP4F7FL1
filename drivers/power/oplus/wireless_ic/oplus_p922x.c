...
static int p922x_set_tx_charger_dect(struct oplus_p922x_ic *chip)
{
	...
	p922x_config_interface(chip, 0x0052, (~P9221_CMD_INDENTIFY_ADAPTER),
			       0xFF);
...
static int p922x_set_tx_cep_timeout(struct oplus_p922x_ic *chip)
{
	...
	p922x_config_interface(chip, 0x0052, (~P9221_CMD_SET_CEP_TIMEOUT),
			       0xFF);
	p922x_config_interface(chip, 0x0053, 0xFF, 0xFF);
	p922x_config_interface(chip, 0x0054, 0x00, 0xFF);
...
static int p922x_set_tx_led_brightness(struct oplus_p922x_ic *chip)
{
	...
	p922x_config_interface(chip, 0x0050, 0x48, 0xFF);
	p922x_config_interface(chip, 0x0051, P9221_CMD_SET_LED_BRIGHTNESS,
			       0xFF);
	p922x_config_interface(chip, 0x0052, (~P9221_CMD_SET_LED_BRIGHTNESS),
			       0xFF);
	p922x_config_interface(chip, 0x0053, pwm_pulse, 0xFF);
	p922x_config_interface(chip, 0x0054, ~pwm_pulse, 0xFF);
					...
					int chg_current)
{
	...
	mp2650_charging_current_write_fast(
					 ...
					 int vout_target, int vout_min)
{
	...
	if (chip->p922x_chg_status.vout > vout_target) {
		if (chip->p922x_chg_status.charge_voltage > vout_target) {
			temp_value = p922x_decrease_vc_by_step(
				chip, chip->p922x_chg_status.charge_voltage,
					 ...
					 int vout_target, int vout_max)
{
	...
	if (chip->p922x_chg_status.vout < vout_target) {
		if (chip->p922x_chg_status.charge_voltage < vout_target) {
			temp_value = p922x_increase_vc_by_step(
				chip, chip->p922x_chg_status.charge_voltage,
				vout_target, WPC_CHARGE_VOLTAGE_CHANGE_STEP_1V);
		} else {
			temp_value = p922x_increase_vc_by_step(
...
static void p922x_charger_init(struct oplus_p922x_ic *chip)
{
	...
	if (chip->p922x_chg_status.rx_runing_mode == RX_RUNNING_MODE_EPP) {
		...
		p922x_set_rx_charge_current(
			chip, chip->p922x_chg_status.BPP_current_limit);
	}
...
void p922x_set_rtx_function(bool is_on)
{
	...
	if (is_on) {
		if (p922x_chip->p922x_chg_status.wpc_dischg_status !=
...
5V_en[%d], EXT2_otg_en[%d], EXT1_otg_en[%d]\n",
		p922x_get_vbat_en_val(), p922x_get_idt_con_val(),
		p922x_get_idt_int_val(), oplus_get_idt_en_val(),
		p922x_get_booster_en_val(), chargepump_get_chargepump_en_val(),
		oplus_get_wrx_en_val(), oplus_get_wrx_otg_en_val(),
		mp2650_wireless_get_mps_otg_en_val(), p922x_get_cp_ldo_5v_val(),
...
[Batt vol max: %d] [Batt vol min: %d] [Term Vol: %d] [Term Curr: %d]\n",
		...
		mp2650_get_termchg_voltage(), mp2650_get_termchg_current());
		     ...
		     int fw_size)
{
	...
	rc = __p922x_write_reg(chip, 0x405,
			       (pure_fw_size >> 8) &
				       0xff); // write FW length high byte
	if (rc != 0) {
...
static int p922x_check_idt_fw_update(struct oplus_p922x_ic *chip)
{
	...
	if (rc) {
		...
		chg_err("<IDT UPDATE>The new fw version: %02x %02x %02x %02x\n",
			...
			fw_buf[fw_ver_start_addr + 0x05],
			fw_buf[fw_ver_start_addr + 0x06],
		...
		if ((temp[0] != fw_buf[fw_ver_start_addr + 0x04]) ||
			...
			if (p922x_MTP(chip, fw_buf, fw_size) == 0) {
				...
				if (idt_update_retry_cnt > 5) {
					chip->p922x_chg_status.check_fw_update =
						true;
				} else {
...
static int p922x_get_vrect_iout(struct oplus_p922x_ic *chip)
{
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHGPUMP:
		if ((p922x_get_CEP_flag(chip) == 0) &&
		    (abs(chip->p922x_chg_status.charge_voltage - vout_value) >
		     2000)) {
...
static int p922x_fastcharge_debug_by_adb(struct oplus_p922x_ic *chip)
{
	...
	if (chip->p922x_chg_status.iout > iout_max_value) {
		...
		if (chip->p922x_chg_status.charge_voltage >
		    WPC_CHARGE_VOLTAGE_CHGPUMP_MIN) {
			temp_value =
				chip->p922x_chg_status.iout - iout_max_value;
		...
		if (chip->p922x_chg_status.charge_voltage <
			...
			temp_value =
				iout_min_value - chip->p922x_chg_status.iout;
			if ((temp_value > 100) &&
			    (chip->p922x_chg_status.iout < 1800)) {
				p922x_set_rx_charge_voltage(
					chip,
					chip->p922x_chg_status.charge_voltage +
						100);
						...
						bool is_init)
{
	...
	if (g_oplus_chip->batt_volt >= 4370 &&
		...
		chip->p922x_chg_status.wpc_ffc_charge = false;
		p922x_set_rx_terminate_voltage(chip,
	...
	if (chip->p922x_chg_status.wpc_reach_stable_charge) {
		if (!chip->p922x_chg_status.wpc_reach_4370mv &&
			...
			if (fasctchg_current[chip->p922x_chg_status
						     .fastcharge_level] >
				...
				if ((g_oplus_chip->icharging < 0) &&
					...
					chip->p922x_chg_status.wpc_ffc_charge =
						false;
					p922x_set_rx_terminate_voltage(
						chip,
						WPC_TERMINATION_VOLTAGE_CV);
					goto ADJUST_FINISH;
				}
	...
	if (g_oplus_chip->batt_volt >= 4500) {
		...
		if (chip->p922x_chg_status.fastcharge_level <
		    FASTCHARGE_LEVEL_5) {
			chg_err("<~WPC~> turn to %dMA charge\n",
			...
			goto ADJUST_FINISH;
		} else if (chip->p922x_chg_status.fastcharge_level ==
			   FASTCHARGE_LEVEL_5) {
			chg_err("<~WPC~> turn to %dMA charge\n",
	...
	if (g_oplus_chip->temperature >= 420) {
		chg_err("<~WPC~> The tempearture is >= 42. fastchg current: %d\n",
			fasctchg_current[chip->p922x_chg_status
						 .fastcharge_level]);
...
static void p922x_TX_message_process(struct oplus_p922x_ic *chip)
{
	...
	if (chip->p922x_chg_status.send_msg_timer > 2) {
		...
		if (chip->p922x_chg_status.adapter_type ==
		    ADAPTER_TYPE_UNKNOW) {
			...
			if (chip->p922x_chg_status.send_msg_cnt > 0) {
				...
				chip->p922x_chg_status.send_message =
					P9221_CMD_NULL;
			}
		} else if (chip->p922x_chg_status.send_message ==
...
static int oplus_wpc_chg_parse_chg_dt(struct oplus_p922x_ic *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,vooc_temp_little_cold_fastchg_ma",
				  &chip->p922x_chg_status.wpc_chg_param
	...
	rc = of_property_read_u32(
		node, "qcom,vooc_temp_cool_fastchg_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,vooc_temp_warm_fastchg_ma",
	...
	case BATTERY_STATUS__NORMAL:
		...
		case ADAPTER_TYPE_VOOC:
			chip->p922x_chg_status.fastchg_current_limit =
				chip->p922x_chg_status.wpc_chg_param
					.vooc_temp_normal_fastchg_ma;
			charging_current = chip->p922x_chg_status.wpc_chg_param
						   .vooc_temp_normal_fastchg_ma;
			break;
...
static int oplus_wpc_set_input_current(struct oplus_p922x_ic *chip)
{
	...
	default:
		if (chip->p922x_chg_status.BPP_current_step_cnt > 0) {
			chip->p922x_chg_status.BPP_current_step_cnt--;
			if (chip->p922x_chg_status.BPP_current_step_cnt == 0) {
				if (chip->p922x_chg_status.BPP_current_limit <
				    500) {
					chg_err("<~WPC~> set BPP current limit 500ma\n");
					chip->p922x_chg_status
						.BPP_current_limit = 500;
					mp2650_set_vindpm_vol(4800);
					chip->p922x_chg_status
						.BPP_current_step_cnt =
						BPP_CURRENT_INCREASE_TIME;
				} else if (chip->p922x_chg_status
						   .BPP_current_limit < 700) {
...
static const int cool_down_current_limit_wireless[6] = { 600,  600,  600,
							 1000, 1250, 1250 };
...
static int p922x_charge_status_process(struct oplus_p922x_ic *chip)
{
	...
	if ((g_oplus_chip->temperature <
		...
		if (chip->p922x_chg_status.charge_current !=
			...
			p922x_set_rx_charge_current(chip,
						    WPC_CHARGE_CURRENT_ZERO);
		}
	...
	case WPC_CHG_STATUS_DEFAULT:
		if (chip->p922x_chg_status.rx_runing_mode ==
		    RX_RUNNING_MODE_EPP) {
	...
	case WPC_CHG_STATUS_READY_FOR_EPP:
		...
		if (chip->p922x_chg_status.epp_current_step ==
		    WPC_CHARGE_CURRENT_EPP_INIT) {
	...
	case WPC_CHG_STATUS_WAITING_FOR_TX_INTO_FASTCHG:
		...
		if (chip->p922x_chg_status.charge_type ==
			...
			if (p922x_get_CEP_flag(chip) != 0) {
				...
				if (cep_nonzero_cnt > 10) {
					...
					chip->p922x_chg_status.skewing_info =
						true;
					if (chip->p922x_chg_status.cep_info <
					    to_cep_info(chip->p922x_chg_status
								.CEP_value))
						chip->p922x_chg_status
		...
		if (chip->p922x_chg_status.idt_adc_test_enable) {
			if (chip->p922x_chg_status.iout < 200) {
				chg_err("<~WPC~> idt_adc_test iout: %dmA < 200mA\n",
					chip->p922x_chg_status.iout);
				chip->p922x_chg_status.idt_adc_test_result =
					false;
				break;
			} else {
				chg_err("<~WPC~> idt_adc_test iout: %dmA >= 200mA\n",
	...
	case WPC_CHG_STATUS_INCREASE_VOLTAGE_FOR_CHARGEPUMP:
		...
		if (p922x_get_CEP_flag(chip) == 0) {
			temp_value = p922x_increase_vout_to_target(
	...
	case WPC_CHG_STATUS_CHARGER_FASTCHG_INIT:
		...
		p922x_set_rx_charge_current(
			chip, chip->p922x_chg_status.fastchg_current_limit);
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHGPUMP:
		...
		if ((p922x_get_CEP_flag(chip) == 0) &&
			...
			if (p922x_get_CEP_flag(chip) == 0) {
				...
				if (cep_zero_cnt >= 10) {
					...
					if (p922x_resume_chargepump_fastchg(
						    chip) == 0) {
						chip->p922x_chg_status
							.wpc_ffc_charge = false;
						p922x_ready_to_switch_to_charger(
							chip, true);
						break;
					}
				}
				...
				if (chip->p922x_chg_status.wpc_skewing_proc ==
				    true) {
					p922x_fastcharge_skewing_proc_40w(
						chip, false);
				} else {
					cep_nonzero_cnt++;
		...
		if (p922x_get_CEP_flag(chip) == 0) {
			...
			if (chip->p922x_chg_status.iout_stated_current >
			    WPC_CHARGE_CURRENT_OFFSET) {
				iout_min_value = chip->p922x_chg_status
							 .iout_stated_current -
						 WPC_CHARGE_CURRENT_OFFSET;
			} else {
				iout_min_value = 0;
			...
			} else if (chip->p922x_chg_status.iout <
				   iout_min_value) {
				chg_err("<~WPC~> The Iout < %d.\n",
					iout_min_value);
	...
	case WPC_CHG_STATUS_DECREASE_VOUT_TO_12V:
		if ((p922x_get_CEP_flag(chip) == 0) ||
		    chip->p922x_chg_status.wpc_skewing_proc) {
			temp_value = p922x_decrease_vout_to_target(
	...
	case WPC_CHG_STATUS_INCREASE_CURRENT_FOR_CHARGER:
		...
		temp_value = p922x_increase_vc_by_step(
			chip, chip->p922x_chg_status.charge_current,
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHARGER:
		...
		if (chip->p922x_chg_status.adapter_type == ADAPTER_TYPE_VOOC) {
			if (chip->p922x_chg_status.fastchg_current_limit >
				...
				chip->p922x_chg_status.fastchg_current_limit =
					1200;
				p922x_set_rx_charge_current(
					chip, chip->p922x_chg_status
						      .fastchg_current_limit);
		...
		if (p922x_get_CEP_flag(chip) == 0) {
			...
			if ((chip->p922x_chg_status.adapter_type ==
			     ADAPTER_TYPE_SVOOC) &&
			    (chip->p922x_chg_status.work_silent_mode ==
			     false) &&
				...
				if (chip->p922x_chg_status.wpc_skewing_proc ==
					...
					if (cep_zero_cnt > 10) {
						...
						chip->p922x_chg_status
							...
							false;
						chg_err("<~WPC~> turn to wpc_skewing_proc = false\n");
						p922x_ready_to_switch_to_chargepump(
							chip);
						break;
					}
				} else {
...
static void p922x_idt_dischg_work(struct work_struct *work)
{
	...
	struct oplus_p922x_ic *chip =
		container_of(dwork, struct oplus_p922x_ic, idt_dischg_work);
				...
				case P9237_RESPONE_ADAPTER_TYPE:
					if ((tx_command == tx_command_r) &&
						...
						chg_err("<~WPC~> get adapter type = 0x%02X, get dock hw version = 0x%02X\n",
							...
							chip->p922x_chg_status
								.dock_version);
						p922x_config_fan_pwm_pulse_value(
							chip);
						if (chip->p922x_chg_status
							    .adapter_type ==
						    ADAPTER_TYPE_PD_65W)
						...
						oplus_wpc_get_fastchg_allow(
							chip);
						p922x_set_tx_Q_value(chip);
						p922x_set_FOD_parameter(chip,
									1);
...
static void p922x_get_running_mode(struct oplus_p922x_ic *chip)
{
	...
	if (rc) {
		...
		if (temp == 0x31) {
			...
			chip->p922x_chg_status.adapter_type = ADAPTER_TYPE_EPP;
			chip->p922x_chg_status.rx_runing_mode =
				RX_RUNNING_MODE_EPP;
		} else if (temp == 0x04) {
...
static void p922x_idt_event_int_func(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p922x_ic *chip =
		container_of(dwork, struct oplus_p922x_ic, idt_event_int_work);
...
static void p922x_idt_connect_int_func(struct work_struct *work)
{
	...
	if (p922x_get_idt_con_val() == 1) {
		...
		if (p922x_chip->p922x_chg_status.charge_online == false) {
			...
			} else if (chip->p922x_chg_status.adapter_type ==
				   ADAPTER_TYPE_UNKNOW) {
				chg_err("<~WPC~> tx A1\n");
				p922x_set_tx_charger_dect(chip);
			}
...
static void init_p922x_data_log(void)
{
	...
	p = proc_create("p922x_data_log", 0664, NULL,
			&p922x_data_log_proc_fops);
	if (!p)
...
static void p922x_task_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p922x_ic *chip =
		container_of(dwork, struct oplus_p922x_ic, p922x_task_work);
	...
	if (p922x_get_idt_con_val() == 0) {
		...
		if (idt_disconnect_cnt >= 2) {
			if (p922x_chip->p922x_chg_status.charge_online) {
				...
				schedule_delayed_work(
					&p922x_chip->idt_connect_int_work, 0);
			}
		}
...
static void p922x_CEP_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p922x_ic *chip =
		container_of(dwork, struct oplus_p922x_ic, p922x_CEP_work);

	if (p922x_chip->p922x_chg_status.charge_online) {
		p922x_detect_CEP(chip);
		schedule_delayed_work(&chip->p922x_CEP_work,
				      P922X_CEP_INTERVAL);
	}
}
...
static void p922x_update_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p922x_ic *chip =
		container_of(dwork, struct oplus_p922x_ic, p922x_update_work);
	int rc;
...
static void p922x_test_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_p922x_ic *chip =
		container_of(dwork, struct oplus_p922x_ic, p922x_test_work);
...
start:
	...
	case UPGRADE_START:
		...
		if (fw_head->magic[0] == 0x02 && fw_head->magic[1] == 0x00 &&
			...
			chg_info(
				"<IDT UPDATE>Receiving image, fw_size=%d, fw_index=%d\n",
				fw_size, fw_index);
			if (fw_index >= fw_size) {
...
static int p922x_wpc_get_max_wireless_power(void)
{
	...
	max_wireless_power =
		adapter_wireless_power > chip->p922x_chg_status.wireless_power ?
			chip->p922x_chg_status.wireless_power :
	...
	case POWER_SUPPLY_PROP_CEP_INFO:
		if (val->intval == 0) {
			...
			if (!p922x_chip->p922x_chg_status.wpc_skewing_proc) {
				p922x_chip->p922x_chg_status.skewing_info =
					false;
			      ...
			      const struct i2c_device_id *id)
{
	...
	chip = devm_kzalloc(&client->dev, sizeof(struct oplus_p922x_ic),
			    GFP_KERNEL);
	if (!chip) {
	...
	INIT_DELAYED_WORK(&chip->p922x_self_reset_work,
			  p922x_self_reset_process);
#ifdef FASTCHG_TEST_BY_TIME
