...
static int vooc_uart_write_some_addr(u8 *fw_buf, int length)
{
	...
	while (1) {
		...
		if (write_addr == 0) {
			write_addr =
				(fw_buf[fw_count + 1] << 8) | fw_buf[fw_count];
		}
		vooc_uart_irq_fiq_enable(false);
...
static bool vooc_uart_read_addr_line_and_check(unsigned int addr)
{
	...
	if (!(addr % 0x20)) {
		...
		if ((addr - 16) ==
			...
			for (i = 0; i < 16; i++) {
				if (fw_check_buf[i + 2] !=
				    adapter_stm8s_firmware_data[fw_line * 34 + 2 +
								HALF_ONE_LINE +
								i]) {
					goto read_addr_line_err;
				}
...
static bool vooc_adapter_update_handle(unsigned long tx_pin,
				       unsigned long rx_pin)
{
	...
	vooc_uart_irq_fiq_enable(false);
	vooc_uart_tx_byte(Last_Line_Addr & 0xff);
	for (rx_last_line_count = 0; rx_last_line_count < 18;
	     rx_last_line_count++) {
		rx_last_line[rx_last_line_count] =
			vooc_uart_rx_byte(Read_Addr_Line_Cmd);
		if (chip->rx_timeout) {
			break;
		}
	}
	...
	rc = vooc_uart_write_some_addr(
		&adapter_stm8s_firmware_data[0],
	...
	rc = vooc_uart_write_some_addr(
		&adapter_stm8s_firmware_data[sizeof(adapter_stm8s_firmware_data) -
					     34],
		34);
	if (rc) {
...
bool oplus_vooc_adapter_update_is_tx_gpio(unsigned long gpio_num)
{
	...
	if (the_chip->adapter_update_ing &&
	    gpio_num == the_chip->uart_tx_gpio) {
		return true;
	} else {
		return false;
	}
...
bool oplus_vooc_adapter_update_is_rx_gpio(unsigned long gpio_num)
{
	...
	if (the_chip->adapter_update_ing &&
	    gpio_num == the_chip->uart_rx_gpio) {
		return true;
	} else {
		return false;
	}
