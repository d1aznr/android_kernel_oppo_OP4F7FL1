...
static int oplus_pps_parse_charge_strategy(struct oplus_pps_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,pps_warm_allow_soc",
	...
	rc = of_property_read_u32(node, "qcom,pps_full_ffc_vbat",
	...
	if (rc >= 0) {
		...
		rc = of_property_read_u32_array(
			node, "oplus,pps_strategy_batt_high_temp",
	...
	if (rc >= 0) {
		...
		if (length > 6)
			length = 6;
		rc = of_property_read_u32_array(
			node, "oplus,pps_strategy_high_current",
...
static int oplus_pps_parse_batt_curves_third(struct oplus_pps_chip *chip)
{
	...
	for (i = 0; i < chip->limits.pps_strategy_soc_num - 1; i++) {
		...
		for (j = 0; j < chip->limits.pps_strategy_temp_num - 1; j++) {
			rc = of_property_count_elems_of_size(
				soc_node, pps_strategy_temp[j], sizeof(u32));
			if (rc < 0) {
...
static int oplus_pps_get_charging_data(struct oplus_pps_chip *chip)
{
	...
	if (oplus_pps_get_support_type() == PPS_SUPPORT_MCU) {
		...
		chip->ap_input_current =
			chip->cp_master_ibus + chip->cp_slave_ibus;
	}
	chip->vbat0 = oplus_pps_get_vbat0(chip);
	pps_err(" [%d, %d, %d, %d, %d], [%d, %d, %d, %d, %d, %d]\n",
		chip->ap_batt_volt, chip->ap_batt_current, chip->ap_input_volt,
		chip->ap_input_current, chip->charger_output_volt,
		chip->charger_output_current, chip->cp_master_ibus,
		chip->cp_slave_ibus, chip->cp_master_tdie, chip->cp_slave_tdie,
		chip->vbat0);
	return 0;
}
...
static int oplus_pps_check_bcc_curr(struct oplus_pps_chip *chip)
{
	...
	bcc_max = chip->batt_curves.batt_curves[index_min].target_ibus *
		  oplus_pps_get_curve_vbus(chip) / 10000 / 100;
	...
	pps_err("result index_vbat = %d,  [%d, %d, %d, %d]\n", index_vbat,
		chip->batt_curve_index, chip->bcc_max_curr, chip->bcc_min_curr,
		chip->bcc_exit_curr);
	return 0;
}
...
static int oplus_pps_choose_curves(struct oplus_pps_chip *chip)
{
	...
	pps_err("[%d, %d, %d, %d, %d, %d, %d]", chip->pps_adapter_type,
		chip->ap_batt_temperature, chip->ap_batt_soc, batt_soc_plugin,
	...
	chip->bcc_exit_curr =
		chip->batt_curves
			...
			.target_ibus *
		(oplus_pps_get_curve_vbus(chip) / 10000);
...
static int oplus_pps_charging_enable_master(struct oplus_pps_chip *chip,
					    bool on)
{
	int status = 0;
	if (!chip || !chip->ops || !chip->pps_support_type) {
	...
	if (chip->cp_master_abnormal) {
		pps_err("cp_master_abnormal = %d, cp_slave_abnormal = %d, cp_master_ibus = %d, cp_slave_ibus = %d\n",
			chip->cp_master_abnormal, chip->cp_slave_abnormal,
			chip->cp_master_ibus, chip->cp_slave_ibus);
		return status;
	}
	if (on) {
...
static int
oplus_pps_set_current_temp_low_normal_range(struct oplus_pps_chip *chip,
					    int vbat_temp_cur)
{
	int ret = chip->limits.pps_strategy_normal_current;
...
static int
oplus_pps_set_current_temp_little_cool_range(struct oplus_pps_chip *chip,
					     int vbat_temp_cur)
{
	int ret = 0;
...
static int
oplus_pps_set_current_temp_little_cold_range(struct oplus_pps_chip *chip,
					     int vbat_temp_cur)
{
	int ret = 0;
	if (chip->limits.pps_batt_over_low_temp != -EINVAL &&
...

{
	...
	if (chip->pps_temp_cur_range == PPS_TEMP_RANGE_INIT) {
		if (vbat_temp_cur <
		    chip->limits.pps_little_cold_temp) { /*0-5C*/
			chip->pps_temp_cur_range = PPS_TEMP_RANGE_LITTLE_COLD;
			chip->pps_fastchg_batt_temp_status =
				PPS_BAT_TEMP_LITTLE_COLD;
		} else if (vbat_temp_cur <
			   chip->limits.pps_cool_temp) { /*5-12C*/
			chip->pps_temp_cur_range = PPS_TEMP_RANGE_COOL;
			chip->pps_fastchg_batt_temp_status = PPS_BAT_TEMP_COOL;
		} else if (vbat_temp_cur <
			   chip->limits.pps_little_cool_temp) { /*12-16C*/
			chip->pps_temp_cur_range = PPS_TEMP_RANGE_LITTLE_COOL;
			chip->pps_fastchg_batt_temp_status =
				PPS_BAT_TEMP_LITTLE_COOL;
		} else if (vbat_temp_cur <
			   chip->limits.pps_normal_low_temp) { /*16-35C*/
			chip->pps_temp_cur_range = PPS_TEMP_RANGE_NORMAL_LOW;
			chip->pps_fastchg_batt_temp_status =
				PPS_BAT_TEMP_NORMAL_LOW;
		} else {
			if (chip->limits.pps_normal_high_temp == -EINVAL ||
			    vbat_temp_cur <
				    chip->limits
					    .pps_normal_high_temp) { /*25C-43C*/
				chip->pps_temp_cur_range =
					PPS_TEMP_RANGE_NORMAL_HIGH;
				chip->pps_fastchg_batt_temp_status =
					PPS_BAT_TEMP_NORMAL_HIGH;
			} else {
				chip->pps_temp_cur_range = PPS_TEMP_RANGE_WARM;
...
static int oplus_pps_get_batt_curve_curr(struct oplus_pps_chip *chip)
{
	...
	if (oplus_pps_get_curve_time(chip) > 0) {
		if ((curve_volt_change) &&
		    (chip->timer.batt_curve_time <
		     (oplus_pps_get_curve_time(chip) + toal_curve_time))) {
	...
	pps_err(" [%d, %d, %d, %d, %d] [%d, %d, %d, %d, %d]\n",
		...
		chip->ap_batt_volt, oplus_pps_get_curve_vbat(chip),
	...
	if ((curve_volt_change) || (curve_time_change)) {
		...
		if ((chip->batt_curve_index > 0) &&
		    (oplus_pps_get_last_curve_vbat(chip) !=
		     oplus_pps_get_curve_vbat(chip)))
			oplus_pps_check_bcc_curr(chip);
...
static const int Cool_Down_Third_Curve[] = { 3000, 1000, 1000, 1200, 1500, 1700,
					     2000, 2200, 2500, 2700, 3000 };
static const int Cool_Down_Oplus_Curve[] = { 8000, 1000, 1000, 1200, 1500,
					     1700, 2000, 2200, 2500, 2700,
...
static int oplus_pps_get_cool_down_curr(struct oplus_pps_chip *chip)
{
	...
	if (chip->pps_adapter_type == PPS_ADAPTER_THIRD) {
		if (cool_down >=
		    (sizeof(Cool_Down_Third_Curve) / sizeof(int))) {
			cool_down =
				sizeof(Cool_Down_Third_Curve) / sizeof(int) - 1;
		}
		chip->current_cool_down = Cool_Down_Third_Curve[cool_down];
	} else {
		if (cool_down >=
		    (sizeof(Cool_Down_Oplus_Curve) / sizeof(int))) {
			cool_down =
...
static void oplus_pps_check_cp_abnormal(struct oplus_pps_chip *chip)
{
	...
	/*return;*/
	if ((chip->cp_master_ibus < chip->cp_slave_ibus) &&
	    (chip->cp_master_ibus < PPS_CP_IBUS_ABORNAL_MIN))
		chip->cp_master_abnormal = true;
	else if ((chip->cp_master_ibus > chip->cp_slave_ibus) &&
		 (chip->cp_slave_ibus < PPS_CP_IBUS_ABORNAL_MIN))
...
static int oplus_pps_check_ibus_curr(struct oplus_pps_chip *chip)
{
	...
	if ((chip->cp_slave_enable) &&
	    ((abs(chip->cp_master_ibus - chip->cp_slave_ibus) >
	      PPS_CP_IBUS_DIFF) ||
	...
	pps_err("[%d, %d, %d, %d, %d]\n", chip->cp_slave_enable,
		chip->cp_master_ibus, chip->cp_slave_ibus, chip->cp_ibus_down,
		chip->count.ibus_over);
	return 0;
}
...
static int oplus_pps_check_tdie_curr(struct oplus_pps_chip *chip)
{
	...
	if ((chip->cp_master_tdie > PPS_CP_TDIE_MAX) ||
	    (chip->cp_slave_tdie > PPS_CP_TDIE_MAX)) {
	...
	if ((chip->cp_tdie_down == OPLUS_PPS_CURRENT_LIMIT_3A) &&
	    ...
	    ((chip->cp_master_tdie > PPS_CP_TDIE_MAX) ||
	     (chip->cp_slave_tdie > PPS_CP_TDIE_MAX))) {
	...
	pps_err("[%d, %d, %d, %d, %d]\n", chip->cp_master_tdie,
		chip->cp_slave_tdie, chip->count.tdie_over,
...
static int oplus_pps_get_target_current(struct oplus_pps_chip *chip)
{
	...
	pps_err("[%d, %d, %d, %d, %d, %d, %d]\n", chip->current_batt_curve,
		...
		chip->current_bcc, chip->cp_ibus_down, chip->cp_tdie_down,
		chip->cp_r_down);
	...
	target_current_temp = target_current_temp < chip->cp_ibus_down ?
				      target_current_temp :
				      chip->cp_ibus_down;
	target_current_temp = target_current_temp < chip->cp_tdie_down ?
				      target_current_temp :
				      chip->cp_tdie_down;
...
static void oplus_pps_check_temp(struct oplus_pps_chip *chip)
{
	...
	pps_err("set_temp_flag = %d, btb_counts = %d, ap_batt_temperature = %d,tbatt_counts = %d!\n",
		chip->timer.set_temp_flag, chip->count.btb_high,
		chip->ap_batt_temperature, chip->count.tbatt_over);
}
...
static void oplus_pps_check_low_curr_full(struct oplus_pps_chip *chip)
{
	...
	for (i = 0;
		...
		pps_err("temp_current = %d, temp_vbatt =  %d, temp_status = %d full_counts = %d, pps_stop_status = %d, i = %d,iterm = %d,vterm = %d\n",
			...
			chip->count.low_curr_full, chip->pps_stop_status, i,
			iterm, vterm);
...
PPS_R_COOL_DOWN_ILIMIT oplus_pps_get_cool_down_by_resistense(void)
{
	...
	PPS_R_COOL_DOWN_ILIMIT cool_down_r0 = R_COOL_DOWN_NOLIMIT,
			       cool_down_r1 = R_COOL_DOWN_NOLIMIT;
	PPS_R_COOL_DOWN_ILIMIT cool_down_r2 = R_COOL_DOWN_NOLIMIT,
	...
	if (ibus_master > PPS_R_IBUS_MIN) {
		r_tmp.r1 =
			(vac_master -
			 chip->rmos_mohm * (ibus_master + ibus_slave) / 1000 -
			 vbus_master) *
			1000 / (ibus_master);
	...
	pps_err("[%d, %d, %d, %d, %d] [%d, %d, %d, %d, %d] [%d, %d, %d, %d, %d] [%d, %d, %d, %d, %d, %d, %d]",
		...
		ibus_slave, vpps, ipps, vac_master, ibus_master, ibus_slave,
	...
	if ((ibus_master + ibus_slave) < OPLUS_PPS_CURRENT_LIMIT_2A) {
		pps_err("!!!!!ibus_master = %d, ibus_slave: %d", ibus_master,
			ibus_slave);
		pps_err("!!!!!vpps = %d, ipps = %d, vac_master: %d, ibus_master: %d, ibus_slave: %d, \
			...
			vbus_master, vbus_slave, vout_master, vout_slave,
			vbatt);
		pps_err("!!!!!tmp r0: %d, r1: %d, r2: %d, r3: %d, r4: %d",
			r_tmp.r0, r_tmp.r1, r_tmp.r2, r_tmp.r3, r_tmp.r4);
	...
	pps_err("!!!!!vpps = %d, ipps = %d, vac_master: %d, ibus_master: %d, ibus_slave: %d, vbus_master : %d, \
		...
		vpps, ipps, vac_master, ibus_master, ibus_slave, vbus_master,
		vbus_slave, vout_master, vout_slave, vbatt);
	pps_err("!!!!!tmp r0: %d, r1: %d, r2: %d, r3: %d, r4: %d", r_tmp.r0,
		r_tmp.r1, r_tmp.r2, r_tmp.r3, r_tmp.r4);
	...
	pps_err("!!!!!cool_down_target = %d, cool_down_r0: %d, cool_down_r1: %d, cool_down_r2: %d, \
		cool_down_r3: %d, cool_down_r4: %d",
		cool_down_target, cool_down_r0, cool_down_r1, cool_down_r2,
...
static void oplus_pps_master_enable_check(struct oplus_pps_chip *chip)
{
	...
	for (cnt_fail = 0; cnt_fail < PPS_MASTER_ENALBE_CHECK_CNTS;
	     cnt_fail++) {
		msleep(200);
...
static void oplus_pps_slave_enable_check(struct oplus_pps_chip *chip)
{
	...
	if ((chip->ask_charger_current >= PPS_IBUS_SLAVE_ENABLE_MIN) &&
	    (!chip->cp_slave_enable)) {
		...
		for (cnt_fail = 0; cnt_fail < PPS_SLAVLE_ENALBE_CHECK_CNTS;
			...
			if (chip->ops->pps_get_cp_slave_ibus() <
			    PPS_IBUS_ABNORMAL_MIN) {
				ret = oplus_pps_charging_enable_slave(chip,
								      true);
				msleep(200);
...
bool oplus_pps_is_allow_real(void)
{
	...
	pps_err("chip->fg[%d %d %d] temp[%d %d %d %d] warm[%d %d %d]\n",
		...
		chip->ap_batt_volt, chip->limits.pps_batt_over_high_temp,
...
int oplus_chg_pps_get_batt_curve_current(void)
{
	...
	batt_curve_current =
		chip->current_batt_curve < chip->current_batt_temp ?
			chip->current_batt_curve :
			chip->current_batt_temp;
	batt_curve_current = batt_curve_current < chip->current_bcc ?
				     batt_curve_current :
				     chip->current_bcc;
	batt_curve_current = batt_curve_current < chip->cp_ibus_down ?
				     batt_curve_current :
				     chip->cp_ibus_down;
	batt_curve_current = batt_curve_current < chip->cp_tdie_down ?
				     batt_curve_current :
				     chip->cp_tdie_down;
	batt_curve_current = batt_curve_current < chip->cp_r_down ?
				     batt_curve_current :
				     chip->cp_r_down;
