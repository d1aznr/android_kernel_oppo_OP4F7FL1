	...
	*returnData = i2c_smbus_read_word_data(chip->client, cmd);
	...
	if (*returnData < 0) {
		pr_err("%s read err, rc = %d suspend = %d\n", __func__,
		       *returnData, atomic_read(&chip->suspended));
...
int bq27541_i2c_txsubcmd(struct chip_bq27541 *chip, int cmd, int writeData)
{
	...
	if (rc < 0) {
		while (retry > 0) {
			usleep_range(5000, 5000);
			rc = i2c_smbus_write_word_data(chip->client, cmd,
						       writeData);
			if (rc < 0) {
				retry--;
			} else {
...
static int bq27541_write_i2c_block(struct chip_bq27541 *chip, u8 cmd, u8 length,
				   u8 *writeData)
{
	int rc = 0;
	int retry = 3;
	...
	if (rc < 0) {
		while (retry > 0) {
			usleep_range(5000, 5000);
			rc = i2c_smbus_write_i2c_block_data(chip->client, cmd,
							    length, writeData);
			if (rc < 0) {
				retry--;
			} else {
...
static int bq27541_read_i2c_block(struct chip_bq27541 *chip, u8 cmd, u8 length,
				  u8 *returnData)
{
	int rc = 0;
	int retry = 3;
	...
	if (rc < 0) {
		while (retry > 0) {
			usleep_range(5000, 5000);
			rc = i2c_smbus_read_i2c_block_data(chip->client, cmd,
							   length, returnData);
			if (rc < 0) {
				retry--;
			} else {
...
static int bq27541_read_i2c_onebyte(struct chip_bq27541 *chip, u8 cmd,
				    u8 *returnData)
{
	int retry = 3;
	s32 ret = 0;
...
static int bq27541_i2c_txsubcmd_onebyte(struct chip_bq27541 *chip, u8 cmd,
					u8 writeData)
{
	int rc = 0;
	int retry = 3;
	...
	if (rc < 0) {
		while (retry > 0) {
			usleep_range(5000, 5000);
			rc = i2c_smbus_write_byte_data(chip->client, cmd,
						       writeData);
			if (rc < 0) {
				retry--;
			} else {
...
static int bq27541_get_gauge_i2c_err(void)
{
	...
	pr_err("%s gauge_i2c_err = %d suspend = %d\n", __func__, gauge_i2c_err,
	       atomic_read(&gauge_ic->suspended));
...
static int zy0603_check_comm_gfconfig(struct chip_bq27541 *chip, int cmd,
				      u8 target_val)
{
	u8 r_val;
	int retry_cnt = 0;
...
fg_read_altmac_block_end:
	...
	pr_info("%s [0x%02x][0x%02x][0x%02x][0x%02x] fg_version %d %s\n",
		__func__, version_data[0], version_data[1], version_data[2],
		version_data[3], *fwversion,
...
static bool zy0603_fg_softreset(struct chip_bq27541 *chip)
{
	...
	if (ret == 0) {
		bq27541_i2c_txsubcmd(chip, ZYO603_CMD_ALTMAC,
				     ZY0603_SOFT_RESET_CMD);
		pr_info("%s w 0x3E \n", __func__);
		if (gauge_i2c_err)
...
static int zy0603_start_checksum_cal(struct chip_bq27541 *chip)
{
	...
	return bq27541_write_i2c_block(chip, BQ28Z610_REG_CNTL1, 2,
				       update_checksum);
}

static int zy0603_disable_ssdf_and_pf(struct chip_bq27541 *chip)
{
	u8 write_protectioncfgb[5] = { 0x0F, 0x41, 0x00, 0xAF,
				       0x05 }; /*zy 20211227*/
	...
	if (!zy0603_unseal(chip, 0)) { /* ok */
		...
		bq27541_write_i2c_block(chip, BQ28Z610_REG_CNTL1, 3,
					write_protectioncfgb);
		...
		if (!((read_buf[0] == write_protectioncfgb[0]) &&
		      (read_buf[1] == write_protectioncfgb[1]) &&
		...
		pr_err("%s 0x3E -->read [0x%02x][0x%02x] [0x%02x]\n", __func__,
		       read_buf[0], read_buf[1], read_buf[2]);
		if (!((read_buf[0] == write_ssdftime[0]) &&
		      (read_buf[1] == write_ssdftime[1]) &&
		...
		if (chip->b_soft_reset_for_zy) {
			/* modelratio */
			bq27541_write_i2c_block(chip, BQ28Z610_REG_CNTL1, 4,
						w_modelratio);
			...
			bq27541_read_i2c_block(chip, BQ28Z610_REG_CNTL1, 4,
					       read_buf);
			...
			bq27541_read_i2c_block(chip, BQ28Z610_REG_CNTL1, 3,
					       read_buf);
		...
		bq27541_write_i2c_block(chip, BQ28Z610_REG_CNTL1, 4,
					write_static_df_cheksum);
		pr_err("%s write static_df_checksum_3e --> 0x3e\n", __func__);
		usleep_range(U_DELAY_5_MS, U_DELAY_5_MS);

		bq27541_write_i2c_block(chip, BQ28Z610_REG_CNTL2, 2,
					write_static_df_cheksum + 4);
		...
		pr_err("%s 0x3E -->read [0x%02x][0x%02x] [0x%02x][0x%02x]\n",
		       __func__, read_buf[0], read_buf[1], read_buf[2],
		       read_buf[3]);
		if (!((read_buf[0] == write_static_df_cheksum[0]) &&
		      (read_buf[1] == write_static_df_cheksum[1]) &&
		      (read_buf[2] == write_static_df_cheksum[2]) &&
		      (read_buf[3] == write_static_df_cheksum[3]))) {
			rc = -1;
			return rc;
		}
		/* disable PF_EN */
		do {
			bq27541_write_i2c_block(chip, BQ28Z610_REG_CNTL1, 2,
						pf_en_cmd);
		...
		if (!((read_buf[0] == mfg_status_cm[0]) &&
		      (read_buf[1] == mfg_status_cm[1]) &&
		      ((read_buf[3] & 0x40) == 0))) {
			rc = -1;
...
static int zy0603_disable_ssdf_and_pf_check(struct chip_bq27541 *chip)
{
	...
	do {
		...
		pr_err("%s 0x3E -->read [0x%02x][0x%02x] [0x%02x]\n", __func__,
		       read_buf[0], read_buf[1], read_buf[2]);
		error_flag = 0;
		if (((read_buf[0] == ProtectionCfgB[0]) &&
		     (read_buf[1] == ProtectionCfgB[1]) &&
	...
	if (chip->b_soft_reset_for_zy) {
		...
		for (i_index = 0; i_index < 3; i_index++) {
			rc = zy0603_check_comm_gfconfig(
				chip, comm_cfconfig_cmd[i_index],
...
static int zy0603_QmaxFgStatus_check(struct chip_bq27541 *chip)
{
	...
	if (!(qmax_cell1 >= chip->zy_dts_qmax_min &&
	      qmax_cell1 <= chip->zy_dts_qmax_max) ||
	    !(qmax_cell2 >= chip->zy_dts_qmax_min &&
	      qmax_cell2 <= chip->zy_dts_qmax_max) ||
	    !(fg_status == 0x33)) {
		rc = 1; /* exception */
...
static int zy0603_update_afi_buf(struct chip_bq27541 *chip, u16 base_addr,
				 u16 len, const u8 *afi_buf1, char *msg)
{
	u8 checksum = 0xFF;
	u8 len1 = len / 16;
	u8 len2 = len % 16;
	...
	for (i = 0; i < len1; i++) {
		...
		for (j = 0; j < 16; j++) {
			write_buf[2 + j] =
				afi_buf1[j + i * 16]; /* 0x4000 + 30 = 0x401E */
		...
		retry = 0;
		do {
			bq27541_write_i2c_block(
				chip, BQ28Z610_REG_CNTL1, 18,
				write_buf); /* addr + valid data */
			usleep_range(U_DELAY_1_MS, U_DELAY_1_MS);
			bq27541_write_i2c_block(
				chip, BQ28Z610_REG_CNTL2, 2,
				write_buf + 18); /* checksum + self (18 + 19) */
			usleep_range(U_DELAY_5_MS, U_DELAY_5_MS);
			bq27541_read_i2c_block(
				chip, BQ28Z610_REG_CNTL1, 18,
				read_buf); /* addr + valid data */
			for (k = 0; k < 18; k++) {
				error_flag = 0;
				if (write_buf[k] != read_buf[k]) {
					pr_err("%s 1 w r[%d][%d] = 0x%0x 0x%0x",
					       msg, i, k, write_buf[k],
	...
	if (len2 > 0) { /* tail: len2 < 16 */
		...
		retry = 0;
		do {
			bq27541_write_i2c_block(chip, BQ28Z610_REG_CNTL1,
						len2 + 2, write_buf);
			usleep_range(U_DELAY_1_MS, U_DELAY_1_MS);
			bq27541_write_i2c_block(chip, BQ28Z610_REG_CNTL2, 2,
						write_buf + len2 + 2);
			usleep_range(U_DELAY_5_MS, U_DELAY_5_MS);
			bq27541_read_i2c_block(chip, BQ28Z610_REG_CNTL1,
					       len2 + 2, read_buf);
			for (k = 0; k < len2 + 2; k++) {
				error_flag = 0;
				if (write_buf[k] != read_buf[k]) {
					pr_err("%s 2 [%d] = 0x%0x 0x%0x", msg,
					       k, write_buf[k], read_buf[k]);
					error_flag = 1;
					retry++;
...
static int zy0603_afi_param_update(struct chip_bq27541 *chip)
{
	...
	/* write afi_buf1 */
	base_addr = 0x4000;
	zy0603_update_afi_buf(chip, base_addr, chip->afi_buf_len[0],
			      chip->afi_buf[0], "afi_buf1");

	/* write afi_buf2 */
	base_addr = 0x40FF;
	zy0603_update_afi_buf(chip, base_addr, chip->afi_buf_len[1],
			      chip->afi_buf[1], "afi_buf2");

	/* write afi_buf3 */
	base_addr = 0x4200;
	zy0603_update_afi_buf(chip, base_addr, chip->afi_buf_len[2],
			      chip->afi_buf[2], "afi_buf3");

	/* write afi_buf4 */
	base_addr = 0x4245;
	zy0603_update_afi_buf(chip, base_addr, chip->afi_buf_len[3],
			      chip->afi_buf[3], "afi_buf4");

	/* write afi_buf5 */
	base_addr = 0x427A;
	zy0603_update_afi_buf(chip, base_addr, chip->afi_buf_len[4],
			      chip->afi_buf[4], "afi_buf5");

	/* write afi_buf6 */
	base_addr = 0x4400;
	zy0603_update_afi_buf(chip, base_addr, chip->afi_buf_len[5],
			      chip->afi_buf[5], "afi_buf6");

	/* write afi_buf7 */
	base_addr = 0x4600;
	zy0603_update_afi_buf(chip, base_addr, chip->afi_buf_len[6],
			      chip->afi_buf[6], "afi_buf7");
	return 0;
}

#define OPPO_AFI_UPDATE_INTERVAL_SEC 5
#define OPPO_AFI_UPDATE_INTERVAL                                               \
	round_jiffies_relative(                                                \
		msecs_to_jiffies(OPPO_AFI_UPDATE_INTERVAL_SEC * 1000))
...
static void zy0603_afi_update_work(struct work_struct *work)
{
	...
	struct chip_bq27541 *chip =
		container_of(dwork, struct chip_bq27541, afi_update);
	int ret = 0;
	int error_flag = 0;
	...
	if (oplus_check_afi_update_condition()) {
		...
		if (oplus_vooc_get_allow_reading()) {
			if (!zy0603_unseal(chip, 0)) {
				...
				if (ret < 0) {
					printk(KERN_ERR
					       "[%s] zy0603_afi_param_update fail\n",
					       __func__);
				}
...
static bool zy0603_afi_update_done(void)
{
	...
	if (gauge_ic->batt_zy0603 && gauge_ic->afi_count > 0 &&
	    !gauge_ic->afi_update_done) {
		printk(KERN_ERR
		       "[%s] return for afi_update_done not finished\n");
		return false;
	} else {
		return true;
	}
...
static int zy0603_protect_check(void)
{
	...
	if (oplus_vooc_get_allow_reading() && gauge_ic->disabled &&
	    gauge_ic->need_check) {
...
static int zy0603_parse_afi_buf(struct chip_bq27541 *chip)
{
	...
	rc = of_property_read_string(afi_data_node, "battery,name",
				     &battery_name);
	if (rc) {
	...
	rc = of_property_read_u32(afi_data_node, "qmax_min",
				  &chip->zy_dts_qmax_min);
	if (rc) {
	...
	dev_err(chip->dev, "%s, qmax_min = %d\n", __func__,
		chip->zy_dts_qmax_min);
	rc = of_property_read_u32(afi_data_node, "qmax_max",
				  &chip->zy_dts_qmax_max);
	if (rc) {
	...
	dev_err(chip->dev, "%s, static_df_checksum_3e = [%x,%x]\n", __func__,
		chip->static_df_checksum_3e[0], chip->static_df_checksum_3e[1]);
	...
	dev_err(chip->dev, "%s, static_df_checksum_60 = [%x,%x]\n", __func__,
		chip->static_df_checksum_60[0], chip->static_df_checksum_60[1]);
	rc = of_property_read_u32(afi_data_node, "afi_buf_num",
				  &chip->afi_count);
	...
	dev_err(chip->dev, "zy0603_parse_afi_buf afi_count %d\n",
		chip->afi_count);
	chip->afi_buf = devm_kzalloc(
		&chip->client->dev, chip->afi_count * sizeof(u8 *), GFP_KERNEL);
	chip->afi_buf_len = devm_kzalloc(&chip->client->dev,
					 chip->afi_count * sizeof(unsigned int),
					 GFP_KERNEL);
	for (i = 0; i < chip->afi_count; i++) {
		...
		dev_err(chip->dev,
			"zy0603_parse_afi_buf chwit i = %d, len= %d  end [%x]\n",
...
static int bq27541_get_battery_cc(void) /*  sjc20150105  */
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_cc,
				       &cc);
		if (ret) {
...
static int bq27541_get_sub_battery_cc(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(sub_gauge_ic,
				       sub_gauge_ic->cmd_addr.reg_cc, &cc);
		if (ret) {
...
static int bq27541_get_battery_fcc(void) /*  sjc20150105  */
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_fcc,
				       &fcc);
		if (ret) {
...
static int bq27541_get_sub_gauge_battery_fcc(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(sub_gauge_ic,
				       sub_gauge_ic->cmd_addr.reg_fcc, &fcc);
		if (ret) {
...
static int bq27541_get_battery_soh(void) /*  sjc20150105  */
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_soh,
				       &soh);
		if (ret) {
...
static int bq27541_get_sub_battery_soh(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(sub_gauge_ic,
				       sub_gauge_ic->cmd_addr.reg_soh, &soh);
		if (ret) {
...
static int bq27541_get_battery_mvolts(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_volt,
				       &volt);
		if (ret) {
...
static int bq27541_get_sub_battery_mvolts(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(sub_gauge_ic,
				       sub_gauge_ic->cmd_addr.reg_volt, &volt);
		if (ret) {
			dev_err(sub_gauge_ic->dev,
				"error reading voltage, ret:%d\n", ret);
			sub_gauge_ic->batt_cell_max_vol =
				sub_gauge_ic->max_vol_pre;
			sub_gauge_ic->batt_cell_min_vol =
				sub_gauge_ic->min_vol_pre;
			return sub_gauge_ic->batt_vol_pre;
		}
...
static int bq27541_get_battery_fc(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_fc,
				       &val);
		if (ret) {
...
static int bq27541_get_battery_qm(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_qm,
				       &val);
		if (ret) {
...
static int bq27541_get_battery_pd(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_pd,
				       &val);
		if (ret) {
...
static int bq27541_get_battery_rcu(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_rcu,
				       &val);
		if (ret) {
...
static int bq27541_get_battery_rcf(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_rcf,
				       &val);
		if (ret) {
...
static int bq27541_get_battery_fcu(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_fcu,
				       &val);
		if (ret) {
...
static int bq27541_get_battery_fcf(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_fcf,
				       &val);
		if (ret) {
...
static int bq27541_get_battery_sou(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_sou,
				       &val);
		if (ret) {
...
static int bq27541_get_battery_do0(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_do0,
				       &val);
		if (ret) {
...
static int bq27541_get_battery_doe(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_doe,
				       &val);
		if (ret) {
...
static int bq27541_get_battery_trm(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_trm,
				       &val);
		if (ret) {
...
static int bq27541_get_battery_pc(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_pc,
				       &val);
		if (ret) {
...
static int bq27541_get_battery_qs(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_qs,
				       &val);
		if (ret) {
...
static int bq28z610_get_device_chemistry(struct chip_bq27541 *chip)
{
	...
	if (chip->batt_bq28z610) {
		if (oplus_vooc_get_allow_reading() == true) {
			mutex_lock(&bq28z610_alt_manufacturer_access);
			bq27541_i2c_txsubcmd(chip,
					     BQ28Z610_DEVICE_CHEMISTRY_EN_ADDR,
					     BQ28Z610_DEVICE_CHEMISTRY_CMD);
			usleep_range(1000, 1000);
			ret = bq27541_read_i2c_block(
				chip, BQ28Z610_DEVICE_CHEMISTRY_ADDR,
...
static int bq28z610_get_balancing_config(struct chip_bq27541 *chip)
{
	...
	if (chip->batt_bq28z610) {
		if (oplus_vooc_get_allow_reading() == true) {
			mutex_lock(&bq28z610_alt_manufacturer_access);
			bq27541_i2c_txsubcmd(chip,
					     BQ28Z610_OPERATION_STATUS_EN_ADDR,
					     BQ28Z610_OPERATION_STATUS_CMD);
			usleep_range(1000, 1000);
			ret = bq27541_read_i2c_block(
				chip, BQ28Z610_OPERATION_STATUS_ADDR,
			...
			if (balancing_config ^ chip->pre_balancing_config ||
			    chip->pre_balancing_count >= 10) {
...
static int bq27541_get_battery_temperature(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_temp,
				       &temp);
		if (ret) {
			count++;
...
static int bq27541_get_sub_battery_temperature(void)
{
	...
	if (atomic_read(&sub_gauge_ic->suspended) == 1) {
		return sub_gauge_ic->temp_pre +
		       ZERO_DEGREE_CELSIUS_IN_TENTH_KELVIN;
	}
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(sub_gauge_ic,
				       sub_gauge_ic->cmd_addr.reg_temp, &temp);
		if (ret) {
			count++;
			dev_err(sub_gauge_ic->dev,
...
static bool bq27541_is_battery_present(void)
{
	...
	ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.subcmd_chem_id,
			       &flags);
	if (ret < 0) {
		pr_err("read bq27541  fail\n");
		mdelay(100);
		ret = bq27541_read_i2c(
			gauge_ic, gauge_ic->cmd_addr.subcmd_chem_id, &flags);
		if (ret < 0) {
...
static int bq27541_get_batt_remaining_capacity(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_rm,
				       &cap);
		if (ret) {
...
static int bq27541_get_sub_gauge_batt_remaining_capacity(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(sub_gauge_ic,
				       sub_gauge_ic->cmd_addr.reg_rm, &cap);
		if (ret) {
...
static int bq28z610_get_dod0_parameters(int *batt_dod0_1, int *batt_dod0_2,
					int *batt_dod0_passed_q)
{
	unsigned char dod0_buffer[BQ28Z610_MAC_CELL_DOD0_SIZE] = { 0, 0, 0,
								   0, 0, 0 };
	...
	if (!gauge_ic || !gauge_ic->batt_bq28z610 ||
	    !oplus_vooc_get_allow_reading()) {
		dev_err(gauge_ic->dev,
			"gauge ic is NULL or is not 28z610 or not allow_reading!\n");
		return -1;
	}
	...
	*batt_dod0_passed_q = dod0_passed_q_temp;
	dev_err(gauge_ic->dev,
		"!!![bq28z610_get_dod0_parameters]dod0 passed q: %d, dod0 1: %d, dod0 2: %d",
		*batt_dod0_passed_q, *batt_dod0_1, *batt_dod0_2);

	return 0;
}

static int bq28z610_get_qmax_parameters(int *batt_qmax_1, int *batt_qmax_2,
					int *batt_qmax_passed_q)
{
	unsigned char qmax_buffer_1[BQ28Z610_MAC_CELL_QMAX_SIZE_A] = { 0, 0, 0,
								       0 };
	...
	*batt_qmax_passed_q = qmax_passed_q_temp;
	dev_err(gauge_ic->dev,
		"!!![bq28z610_get_qmax_parameters]qmax passed q: %d, qmax 1: %d, qmax 2: %d",
...
static int bq27541_get_battery_soc(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_soc,
				       &soc);
		if (ret) {
		...
		if ((soc > 100) || ((soc & 0xff) > 100) ||
			...
			ret = bq27541_read_i2c(
				gauge_ic, gauge_ic->cmd_addr.reg_soc, &soc);
			if (ret) {
...
static int bq27541_get_sub_battery_soc(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(sub_gauge_ic,
				       sub_gauge_ic->cmd_addr.reg_soc, &soc);
		if (ret) {
...
static int bq27541_get_average_current(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(gauge_ic, gauge_ic->cmd_addr.reg_ai,
				       &curr);
		if (ret) {
			while (retry > 0) {
				usleep_range(5000, 5000);
				ret = bq27541_read_i2c(
					gauge_ic, gauge_ic->cmd_addr.reg_ai,
					&curr);
				if (ret < 0) {
					retry--;
				} else {
...
static int bq27541_get_sub_battery_average_current(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		ret = bq27541_read_i2c(sub_gauge_ic,
				       sub_gauge_ic->cmd_addr.reg_ai, &curr);
		if (ret) {
			while (retry > 0) {
				usleep_range(5000, 5000);
				ret = bq27541_read_i2c(
...
static int bq27411_write_soc_smooth_parameter_for_zy(struct chip_bq27541 *chip,
						     bool is_powerup);
...
int bq27541_get_passedchg(int *val)
{
	...
	if (gauge_ic->batt_bq28z610) {
		rc = bq27541_i2c_txsubcmd_onebyte(
			gauge_ic, BQ27411_BLOCK_DATA_CONTROL, 0x00);
		if (rc) {
		...
		bq27541_i2c_txsubcmd(gauge_ic, BQ27411_DATA_CLASS_ACCESS,
				     0x0074);
...
static int bq28z610_set_sleep_mode_withoutunseal(struct chip_bq27541 *chip,
						 bool val)
{
	int rc = -1;
	...
	if (val) {
		...
		rc = bq27541_write_i2c_block(chip, BQ28Z610_REG_CNTL1,
					     BQ28Z610_REG_I2C_SIZE,
					     &enable_data[0]);
		msleep(10);
...
static void bq28z610_bcc_set_buffer(int *buffer)
{
	...
	bq28z610_get_qmax_parameters(&batt_qmax_1, &batt_qmax_2,
				     &batt_qmax_passed_q);
	bq28z610_get_dod0_parameters(&batt_dod0_1, &batt_dod0_2,
				     &batt_dod0_passed_q);
...
static int bq28z610_get_battery_bcc_parameters(char *buf)
{
	...
	if (oplus_vooc_bcc_get_temp_range() == false ||
	    (oplus_pps_get_pps_fastchg_started() &&
	     oplus_pps_bcc_get_temp_range() == false)) {
		buffer[9] = 0;
		buffer[10] = 0;
		buffer[14] = 0;
		buffer[15] = 0;
...
static struct oplus_gauge_operations bq27541_sub_gauge_ops = {
	...
	.get_batt_remaining_capacity =
		bq27541_get_sub_gauge_batt_remaining_capacity,
	.get_prev_batt_remaining_capacity =
		bq27541_get_sub_gauge_prev_batt_remaining_capacity,
...
static void bq27541_hw_config(struct chip_bq27541 *chip)
{
	...
	if (device_type == DEVICE_TYPE_BQ27411) {
		...
		chip->device_type_for_vooc = DEVICE_TYPE_FOR_VOOC_BQ27541;
		if (!chip->batt_bq28z610) {
			bq27541_cntl_cmd(chip,
					 BQ27541_BQ27411_SUBCMD_CTNL_STATUS);
			udelay(66);
			bq27541_cntl_cmd(chip,
					 BQ27541_BQ27411_SUBCMD_ENABLE_IT);
		}
	}
	gauge_set_cmd_addr(chip, chip->device_type);
	if (device_type == DEVICE_TYPE_BQ28Z610 ||
	    device_type == DEVICE_TYPE_ZY0603) {
...
static void bq27541_parse_dt(struct chip_bq27541 *chip)
{
	...
	chip->b_soft_reset_for_zy =
		of_property_read_bool(node, "zy,b_soft_reset_for_zy");
	...
	rc = of_property_read_u32(node, "oplus,capacity-pct",
				  &chip->capacity_pct);
	if (rc) {
		chip->capacity_pct = 0;
	}
...
static int zy0603_battery_current_check_cmd(struct chip_bq27541 *chip,
					    bool enable)
{
	...
	if (rc == 0) {
		...
		if ((chip->batt_zy0603) && (chip->afi_count > 0)) {
			do {
				error_flag = 0;
				if (GAUGE_OK !=
					    zy0603_disable_ssdf_and_pf_check(
						    chip) ||
				    GAUGE_OK != zy0603_check_ssdf_pf_checksum(
							chip)) {
					retry_cnt++;
					error_flag = 1;
...
static int bq27441_battery_full_sleep_mode_cmd(struct chip_bq27541 *chip,
					       bool enable)
{
	u8 rc = 0;
	u8 CNTL1_VAL_1[10] = { 0xC1, 0x11, 0x00, 0x00, 0x00,
			       0x00, 0x00, 0x00, 0x00, 0x00 };
	u8 CNTL1_VAL_3[10] = { 0xC1, 0x31, 0x00, 0x00, 0x00,
			       0x00, 0x00, 0x00, 0x00, 0x00 };
...
static int bq8z610_sealed(struct chip_bq27541 *chip)
{
	...
	usleep_range(10000, 10000);
	bq27541_read_i2c_block(chip, BQ28Z610_REG_CNTL1,
			       BQ28Z610_REG_CNTL1_SIZE, CNTL1_VAL);
...
int bq28z610_write_soc_smooth_parameter(struct chip_bq27541 *chip)
{
	...
	bq27541_write_i2c_block(chip, BQ28Z610_REG_CNTL1,
				BQ28Z610_REG_CNTL1_SIZE, &CNTL1_write1[0]);
	msleep(100);
	...
	bq27541_write_i2c_block(chip, BQ28Z610_REG_CNTL1,
				BQ28Z610_REG_CNTL1_SIZE, &CNTL1_write2[0]);
	msleep(100);
	//bq8z610_cntl2_cmd(0x06E9);
	...
	bq27541_write_i2c_block(chip, BQ28Z610_REG_CNTL1,
				BQ28Z610_REG_CNTL1_SIZE, &CNTL1_write3[0]);
	msleep(100);
...
static int bq28z610_get_2cell_voltage(void)
{
	...
	u8 cell_vol_zy0603[BQ28Z610_MAC_CELL_VOLTAGE_SIZE + 2] = { 0, 0, 0,
								   0, 0, 0 };
	...
	if (!gauge_ic->batt_zy0603) {
		mutex_lock(&bq28z610_alt_manufacturer_access);
		bq27541_i2c_txsubcmd(gauge_ic,
				     BQ28Z610_MAC_CELL_VOLTAGE_EN_ADDR,
				     BQ28Z610_MAC_CELL_VOLTAGE_CMD);
		usleep_range(1000, 1000);
		bq27541_read_i2c_block(gauge_ic, BQ28Z610_MAC_CELL_VOLTAGE_ADDR,
				       BQ28Z610_MAC_CELL_VOLTAGE_SIZE,
				       cell_vol);
		mutex_unlock(&bq28z610_alt_manufacturer_access);
		...
		bq27541_i2c_txsubcmd(gauge_ic,
				     ...
				     BQ28Z610_MAC_CELL_VOLTAGE_CMD);
		usleep_range(U_DELAY_1_MS, U_DELAY_1_MS);
		bq27541_read_i2c_block(gauge_ic,
				       BQ28Z610_MAC_CELL_VOLTAGE_EN_ADDR,
				       BQ28Z610_MAC_CELL_VOLTAGE_SIZE + 2,
				       cell_vol_zy0603);
		...
		if (cell_vol_zy0603[0] == 0x71 && cell_vol_zy0603[1] == 0x00) {
			gauge_ic->batt_cell_1_vol =
				(cell_vol_zy0603[3] << 8) | cell_vol_zy0603[2];
			gauge_ic->batt_cell_2_vol =
				(cell_vol_zy0603[5] << 8) | cell_vol_zy0603[4];
		}
	}
...
const char *oplus_chg_get_cmdline(void)
{
	...
	if (of_chosen) {
		ogauge_auth =
			(char *)of_get_property(of_chosen, "ogauge_auth", NULL);
		if (!ogauge_auth)
	...
	struct bq27541_authenticate_data *authenticate_data)
{
	...
	if (gauge_ic->sha1_key_index == ZY0602_KEY_INDEX) {
		...
		ret = bq27541_i2c_txsubcmd_onebyte(gauge_ic, DATAFLASHBLOCK,
						   authen_cmd_buf[0]);
		if (ret < 0) {
			chg_err("%s i2c write error\n", __func__);
			return -1;
		...
		bq27541_i2c_txsubcmd_onebyte(gauge_ic, AUTHENCHECKSUM,
					     checksum_buf[0]);
		if (gauge_ic->batt_zy0603) {
			msleep(10);
		} else {
...
static void bq27541_reset(struct i2c_client *client_chip)
{
	...
	if (gauge_chip->gauge_ops->get_battery_mvolts() <= 3300 &&
	    gauge_chip->gauge_ops->get_battery_mvolts() > 2500 && ui_soc == 0 &&
		...
		if (bq27541_chip->device_type == DEVICE_BQ27541 ||
			...
			bq27541_cntl_cmd(bq27541_chip,
					 BQ27411_RESET_SUBCMD); /*  27411  */
		}
		msleep(50);
...
rerun:
	...
	if (fg_ic->batt_zy0603) {
		...
		dev_err(&client->dev,
			"zy0603 support and can't config gauge param\n");
	} else {
...
*/
	...
	if (fg_ic->bq28z610_need_balancing)
		fg_ic->bq28z610_device_chem =
			bq28z610_get_device_chemistry(fg_ic);
