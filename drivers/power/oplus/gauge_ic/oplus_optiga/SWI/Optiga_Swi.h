...
BOOL Swi_WriteRegisterSpace(uint16_t uw_Address, uint8_t ub_Data,
			    uint8_t ub_BitSelect, BOOL b_WaitForInterrupt,
			    BOOL b_ImmediateInterrupt, BOOL *bp_IrqDetected);
BOOL Swi_WriteRegisterSpaceNoIrq(uint16_t uw_Address, uint8_t ub_Data,
				 uint8_t ub_BitSelect);
BOOL Swi_ReadConfigSpace(uint16_t uw_Address, uint8_t *ubp_Data);
BOOL Swi_WriteConfigSpace(uint16_t uw_Address, uint8_t ub_Data,
			  uint8_t ub_BitSelect);
BOOL Swi_SearchPuid(uint8_t ub_BitsToSearch, S_OPTIGA_PUID *stp_DetectedPuid);
BOOL Swi_SearchMultiplePuid(uint8_t ub_BitsToSearch,
			    S_OPTIGA_PUID *stp_DetectedPuid,
			    uint8_t *ubp_DevCnt);
BOOL Swi_SelectByPuid(uint8_t ub_BitsToExecute,
		      S_OPTIGA_PUID *stp_DeviceToSelect,
		      uint16_t uw_AssignDevAdr);
...
BOOL Swi_SearchMultiplePuid(uint8_t ub_BitsToSearch,
			    S_OPTIGA_PUID *p_DetectedPuid, uint8_t *ubp_DevCnt);

/* *** SWI Low Level Functions *** */
/* *** SWI Low Level Functions *** */
void Swi_SendRawWord(uint8_t ub_Code, uint8_t ub_Data, BOOL b_WaitForInterrupt,
		     BOOL b_ImmediateInterrupt, BOOL *bp_IrqDetected);
...
#define REGISTER_OVERWRITE                                                     \
	(0xFFu) /*!< Register value will be completely overwritten */

#define SWI_FRAME_BIT_SIZE                                                     \
	(13u) /*!< SWI consists of 2 training + 2 CMD + 8 DATA + 1 INV bits */
...
#define NVM_ADDR_REG__LSC_DEC_STS                                              \
	(0x80u) /*!< Bit7 Lifespan Counter decrement status */
#define NVM_ADDR_REG__KILL (0x40u) /*!< Bit6 kill instruction*/
#define NVM_ADDR_REG__DEC_LFSP                                                 \
	(0x20u) /*!< Bit5 decrease lifespan by 1 command */
#define NVM_ADDR_REG__NVM_DATA_LEN                                             \
	(0x18u) /*!< Bit4 and Bit3 NVM data length select  */
#define NVM_ADDR_REG__BYTE_AD                                                  \
	(0x07u) /*!< Bit2, Bit1 and Bit0 Byte address offset  */

/*!< Work In Progress (WIP2) and WIP3 is used for Data read from NVM */
#define NVM_WIP2_REG                                                           \
	(0x0010u) /*!< Base Address of WIP2. Same as ECC RX Response1 */
 ...
 */
#define INF_SWI                                                                \
	(TRUSTB_L_INF + 0x0100u) /*!< 0x1100 - SWI Interface return code. */
#define APP_ECC                                                                \
	(TRUSTB_L_APP +                                                        \
	 0x0100u) /*!< 0x3100 - Application ECC layer return code. */
#define APP_NVM                                                                \
	(TRUSTB_L_APP +                                                        \
	 0x0200u) /*!< 0x3200 - Application NVM layer return code. */
#define APP_LSC                                                                \
	(TRUSTB_L_APP +                                                        \
	 0x0300u) /*!< 0x3300 - Application LSC layer return code. */
#define APP_SID                                                                \
	(TRUSTB_L_APP +                                                        \
	 0x0400u) /*!< 0x3400 - Application search UID layer return code. */
#define APP_CRC                                                                \
	(TRUSTB_L_APP +                                                        \
	 0x0500u) /*!< 0x3500 - Application CRC layer return code. */
...
#define INF_SWI_E_NO_BIT_SEL                                                   \
	(INF_SWI + 0x05u) /*!< 0x1105 - No bit selected for writing */
#define INF_SWI_E_READ_ODC_TIMEOUT                                             \
	(INF_SWI + 0x06u) /*!< 0x1106 - Read ODC timeout */
#define INF_SWI_E_READ_ODC_DATA                                                \
	(INF_SWI + 0x07u) /*!< 0x1107 - Read ODC info error */
#define INF_SWI_E_BURST_TYPE                                                   \
	(INF_SWI + 0x08u) /*!< 0x1108 - Invalid burst type */
#define INF_SWI_E_READ_TYPE (INF_SWI + 0x09u) /*!< 0x1109 - Invalid read type */
#define INF_SWI_E_WRITE_TYPE                                                   \
	(INF_SWI + 0x0Au) /*!< 0x110A - Invalid write type */
#define INF_SWI_E_TIMING_INIT                                                  \
	(INF_SWI + 0x0Bu) /*!< 0x110B - SWI timing initialized */
...
#define APP_ECC_E_READ_PK                                                      \
	(APP_ECC + 0x03u) /*!< 0x3103 - Read ECC Public Key error */
...
#define APP_ECC_E_CHECKVALUE                                                   \
	(APP_ECC + 0x06u) /*!< 0x3106 - Generate Checkvalue error */
#define APP_ECC_E_SEND_CHG                                                     \
	(APP_ECC + 0x07u) /*!< 0x3107 - Send Challenge error */
#define APP_ECC_E_TIMEOUT (APP_ECC + 0x08u) /*!< 0x3108 - ECC timeout */
#define APP_ECC_E_READ_RESP                                                    \
	(APP_ECC + 0x09u) /*!< 0x3109 - Get response error */
#define APP_ECC_E_READ_RESP_X                                                  \
	(APP_ECC + 0x0Au) /*!< 0x310A - Get Response X error */
#define APP_ECC_E_READ_RESP_Z                                                  \
	(APP_ECC + 0x0Bu) /*!< 0x310B - Get Response Z error */
#define APP_ECC_E_READ_RESP_Z1                                                 \
	(APP_ECC + 0x0Cu) /*!< 0x310C - Get Response Z last byte error */
#define APP_ECC_E_VERIFY_RESP                                                  \
	(APP_ECC + 0x0Du) /*!< 0x310D - ECC Verify response error */
#define APP_ECC_E_NO_IRQ (APP_ECC + 0x0Eu) /*!< 0x310E - No IRQ received */
#define APP_ECC_E_GEN_CHALLENGE                                                \
	(APP_ECC + 0x0Fu) /*!< 0x310F - Generate challenge fail */
#define APP_ECC_E_WRITE_CAP7                                                   \
	(APP_ECC + 0x10u) /*!< 0x3110 - Write Reg CAP7 error */
#define APP_ECC_E_WRITE_INT0                                                   \
	(APP_ECC + 0x11u) /*!< 0x3111 - Write Reg INT0 error */
 ...
 */
#define APP_ECC_E_CRC_WRITE                                                    \
	(APP_ECC + 0x50u) /*!< 0x3150 - ECC Pass but CRC page writing fail */
...
#define APP_NVM_E_ILLEGAL_PARA                                                 \
	(APP_NVM + 0x06u) /*!< 0x3206 - Illegal parameter */
#define APP_NVM_E_READ_STATUS                                                  \
	(APP_NVM + 0x07u) /*!< 0x3207 - Read NVM status error */
#define APP_NVM_E_PROG_PAGE                                                    \
	(APP_NVM + 0x08u) /*!< 0x3208 - NVM program page error */
#define APP_NVM_E_READ_DEVICEADDR                                              \
	(APP_NVM + 0x09u) /*!< 0x3209 - Unable to read device address */
#define APP_NVM_E_WRITE_DEVICEADDR                                             \
	(APP_NVM + 0x0Au) /*!< 0x320A - Unable to write device address */
#define APP_NVM_E_DEVICEADDR                                                   \
	(APP_NVM + 0x0Bu) /*!< 0x320B - Invalid device address */
#define APP_NVM_E_LOCK_PAGE                                                    \
	(APP_NVM + 0x0Cu) /*!< 0x320C - Invalid NVM locked page */
#define APP_NVM_E_INVALID_PAGE                                                 \
	(APP_NVM + 0x0Du) /*!< 0x320D - Invalid NVM page number */
#define APP_NVM_E_READBACK                                                     \
	(APP_NVM + 0x0Eu) /*!< 0x320E - Write and read back verify error */
#define APP_NVM_E_READ_BURST (APP_NVM + 0x0Fu) /*!< 0x320F - Read burst error */
#define APP_NVM_E_READ_KILL                                                    \
	(APP_NVM + 0x10u) /*!< 0x3210 - Read reading the kill status register */
#define APP_NVM_E_READ_CAP4                                                    \
	(APP_NVM +                                                             \
	 0x11u) /*!< 0x3211 - Read reading the kill capability register */

#define APP_NVM_READY (APP_NVM + 0xF0u) /*!< 0x32F0 - NVM ready for access */
#define APP_NVM_PAGE_LOCKED                                                    \
	(APP_NVM + 0xF1u) /*!< 0x32F1 - NVM Page locked unable to write */
#define APP_NVM_PAGE_NOT_LOCKED                                                \
	(APP_NVM + 0xF2u) /*!< 0x32F2 - NVM Page not lock */

#define APP_NVM_DEV_KILLED (APP_NVM + 0xF3u) /*!< 0x32F3 - Device is killed */
#define APP_NVM_DEV_NOT_KILL                                                   \
	(APP_NVM + 0xF4u) /*!< 0x32F4 - Device cannot be killed */
#define APP_NVM_KILL_CAP (APP_NVM + 0xF5u) /*!< 0x32F5 - Device can be killed */
#define APP_NVM_KILL_NOT_CAP                                                   \
	(APP_NVM + 0xF6u) /*!< 0x32F6 - Device cannot not killed */
...
uint16_t Swi_ReadUID_burst(uint8_t *UID, UID_BYTE_ORDER order,
			   SWI_BURST_SPEED ReadMode);
