...
static BOOL Optiga_SendChallenge(uint16_t *challenge, BOOL bPolling,
				 uint8_t cmd);
static BOOL Optiga_ReadResponse(uint16_t *Z_resp, uint16_t *X_resp,
				uint8_t cmd);
...
BOOL Ecc_ReadODC(uint32_t *gf2n_ODC, uint32_t *gf2n_PublicKey)
{
	...
	if (Nvm_ReadData(0x0100 + (8 * 29), 18, (uint8_t *)gf2n_PublicKey) ==
	    FALSE)
		return FALSE;
...
/* ****************************************************************************
   ...
   date:
 ************************************************************************* */
BOOL Ecc_SendChallengeAndGetResponse(uint16_t *gf2n_Challenge,
				     uint16_t *gf2n_XResponse,
				     uint16_t *gf2n_ZResponse, BOOL bPolling,
				     uint8_t bEccMode)
{
	BOOL ret;
	BOOL bEccIrq;
	...
	for (ubIndex = 0u; ubIndex < 16u; ubIndex++) {
		...
		if ((ubIndex & 0x07u) == 0u) {
			Swi_SendRawWordNoIrq(
				SWI_ERA, (((0x0040u + ubIndex) >> 8u) & 0xFFu));
			Swi_SendRawWordNoIrq(SWI_WRA,
					     ((0x0040u + ubIndex) & 0xFFu));
		}

		ubWordIndex = (ubIndex >> 1u);
		if ((ubIndex & 1u) == 1u) {
			ubData = (uint8_t)((challenge[ubWordIndex] >> 8u) &
					   0x00FFu);
		} else {
