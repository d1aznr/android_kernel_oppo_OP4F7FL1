...
BOOL move_register_pointer(void)
{
	...
	do {
		ubUID10 = 0x00;
		if ((Swi_ReadConfigSpace(SWI_UID10, &ubUID10) == FALSE) ||
		    (cnt++ > 30)) {
			return FALSE;
		}
	} while (ubUID10 != ubUID);
 ...
 ************************************************************************* */
BOOL Nvm_ProgramData(BOOL b_WaitForFinish, BOOL b_VerifyData,
		     uint16_t uw_Address, uint8_t ub_BytesToProgram,
		     uint8_t *ubp_Data)
{
	BOOL bResult;
	uint8_t ubData;
	...
	for (uwDataCount = 0u; uwDataCount < ub_BytesToProgram; uwDataCount++) {
		...
		do {
			if (Swi_ReadRegisterSpace(SWI_OPTIGA_CTRL2_NVM,
						  &ubData) == FALSE) {
				return FALSE;
			}
			/* check for timeout */
		...
		(void)Swi_WriteRegisterSpaceNoIrq(SWI_NVM_WIP0 | ubAddressL,
						  ubp_Data[uwDataCount], 0xFFu);
		(void)Swi_WriteRegisterSpaceNoIrq(SWI_OPTIGA_NVM_ADDR,
						  ubAddressL, 0xFFu);
		if (move_register_pointer() == FALSE)
			return FALSE;
		(void)Swi_WriteRegisterSpaceNoIrq(SWI_OPTIGA_CTRL2_NVM,
						  ubAddressH, 0xFFu);
	}
	...
	if (b_VerifyData == TRUE) {
		for (uwDataCount = 0u; uwDataCount < ub_BytesToProgram;
		     uwDataCount++) {
			bResult = Nvm_ReadData(uw_Address + uwDataCount, 1u,
					       &ubData);
			if ((bResult == FALSE) ||
...
/* ****************************************************************************
   ...
   date:			2010-03-04: time out added.
 ************************************************************************* */
BOOL Nvm_ReadData(uint16_t uw_Address, uint8_t ub_BytesToRead,
		  uint8_t *ubp_Data)
{
	uint16_t uwDataCount;
	uint8_t ubData;
	...
	for (uwDataCount = 0u; uwDataCount < ub_BytesToRead; uwDataCount++) {
		...
		if (Swi_ReadRegisterSpace(SWI_NVM_WIP2 | ubAddressL, &ubData) ==
		    FALSE) {
			return FALSE;
		}
...
BOOL Nvm_DecreaseLifeSpanCounter(void)
{
	...
	ulNvmTimeOut = g_culNvmTimeout;
	do {
		if (Swi_ReadRegisterSpace(SWI_OPTIGA_CTRL2_NVM, &ubData) ==
		    FALSE) {
			return FALSE;
		}
		/* check for timeout */
...
{
	return Swi_WriteRegisterSpaceNoIrq(SWI_OPTIGA_NVM_LOCK,
					   (1 << ub_PageNumber), 0xff);
}
...
static uint16_t Nvm_IsReady(void)
{
	...
	do {
		...
		ic_udelay(1);
		NvmRetryCount--;
	} while ((ubData & CTRL2_NVM_REG__NVM_ACT) !=
		 0u); /*!< Checks Bit7 NVM active process */
...
{
	uint8_t _ubData[8 * (NVM_ODC_PAGE_COUNT + NVM_PUK_PAGE_COUNT)] = {
		0
	}; /*!< allocate buffer for 9 pages */
	...
	for (j = 0; j < (NVM_ODC_PAGE_COUNT + NVM_PUK_PAGE_COUNT);
		...
		ret = Swi_ReadWIPRegisterBurst(NVM_WIP2_REG,
					       _ubData + (NVM_PAGE_SIZE * j));
		if (ret != INF_SWI_SUCCESS) {
	...
	for (j = 0; j < (NVM_ODC_PAGE_COUNT + NVM_PUK_PAGE_COUNT); j++) {
		if (j < NVM_ODC_PAGE_COUNT) {
			memcpy(ubODC + (NVM_PAGE_SIZE * j),
			       _ubData + (NVM_PAGE_SIZE * j), NVM_PAGE_SIZE);
		} else {
			memcpy(ubPUBKEY + (NVM_PAGE_SIZE * (j - 6)),
			       _ubData + (NVM_PAGE_SIZE * j), NVM_PAGE_SIZE);
		}
	}
