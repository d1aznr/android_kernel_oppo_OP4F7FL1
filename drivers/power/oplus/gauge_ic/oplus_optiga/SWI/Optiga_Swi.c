			    ...
			    BOOL b_ImmediateInterrupt, BOOL *bp_IrqDetected)
{
	...
	Swi_SendRawWord(SWI_WD, ub_Data, b_WaitForInterrupt,
			b_ImmediateInterrupt, &bIntOccured);
...
{
	return Swi_WriteRegisterSpace(uw_Address, ub_Data, ub_BitSelect, FALSE,
				      FALSE, NULL);
}
...
{
	Swi_SendRawWordNoIrq(SWI_EDA,
			     ((uint8_t)((uw_DeviceAddress >> 8u) & 0xFFu)));
...
void Swi_SetAddress(uint16_t uw_DeviceAddress)
{
	...
	Swi_WriteConfigSpace(SWI_DADR1, ((uw_DeviceAddress >> 8u) & 0xFFu),
			     0xFFu);
}
...
BOOL Swi_ReadUID(uint8_t *UID)
{
	...
	for (i = 0; i < 12; i++) {
		if (Swi_ReadConfigSpace(SWI_UID0 + 11 - i,
					((uint8_t *)&value)) == FALSE) {
...
BOOL Swi_ReceiveRawWord(uint8_t *up_Byte)
{
	...
	*up_Byte = (((ulTimes[8] > ulThreshold) ? 1u : 0u) << 7) |
	...
	if (((ulTimes[12] > ulThreshold) ? 1u : 0u) ==
	    ((ulTimes[11] > ulThreshold) ? 1u : 0u)) {
...
BOOL Swi_SearchMultiplePuid(uint8_t ub_BitsToSearch,
			    S_OPTIGA_PUID *pst_DetectedPuid,
			    uint8_t *ubp_DevCnt)
{
	...
	do {
		...
		for (; ubBitCnt > 0; ubBitCnt--) {
			...
			if (b_UidSearch_GetDipDoneBit(
				    ubBitInfo[ubCurrentIdPtr]) ==
			    FALSE) // need to do DIP
			{
				...
				if ((bFound_0 == TRUE) &&
				    (bFound_1 ==
				     TRUE)) // both DIP0 and DIP1 has positive response
				{
					...
					Swi_SendRawWordNoIrq(SWI_BC, SWI_DIE0);
					b_UidSearch_SetDIE0Info(
						&ubBitInfo[ubLastIdPtr], TRUE);
					b_UidSearch_SetDIE1Info(
						&ubBitInfo[ubCurrentIdPtr]);
				} else if ((bFound_0 == TRUE) &&
					   (bFound_1 == FALSE)) // DIE0
				{
				...
				b_UidSearch_SetDipDoneBit(
					&ubBitInfo[ubCurrentIdPtr], TRUE);
			...
			} else // DIPDone == TRUE
			{
				...
				if (b_UidSearch_GetDIE0Info(
					    ubBitInfo[ubCurrentIdPtr])) {
					ubpBytes[ubByteIndex] &= 0xFEu;
					Swi_SendRawWordNoIrq(SWI_BC, SWI_DIE0);
				} else if (b_UidSearch_GetDIE1Info(
						   ubBitInfo[ubCurrentIdPtr])) {
					ubpBytes[ubByteIndex] |= 0x01u;
					Swi_SendRawWordNoIrq(SWI_BC, SWI_DIE1);
				}
			...
			if (ubCurrentIdPtr ==
			    0) // clear dip done from last bit index that has two positive response
			{
				if (ub_SizeOfStack() == 0)
					bSearchDone = TRUE;
				else {
				...
				for (i = 0; i < ubLastIdPtr; i++) {
					b_UidSearch_SetDipDoneBit(&ubBitInfo[i],
								  FALSE);
				}

				b_UidSearch_SetDIE0Info(&ubBitInfo[ubLastIdPtr],
							FALSE); // clear
		...
		(void)Swi_WriteConfigSpace(SWI_DADR0, (ubSlaveCnt & 0xFFu),
					   0xFFu);
		(void)Swi_WriteConfigSpace(SWI_DADR1,
					   ((ubSlaveCnt >> 8u) & 0xFFu), 0xFFu);
...
uint16_t Swi_ReadWIPRegisterBurst(uint16_t uw_Address, uint8_t *ubp_Data)
{
	...
	Swi_SendRawWordNoIrq(SWI_BC,
			     SWI_RBL3); /*!< set burst length to 8 byte */
	Swi_SendRawWordNoIrq(SWI_BC, SWI_EREG); /*!< select EREG register set */
	Swi_SendRawWordNoIrq(
		SWI_ERA,
		HIGH_BYTE_ADDR(uw_Address)); /*!< send high byte of address */

#if (SWI_FLOW_CONTROL_ENABLE == 1)
	Swi_ChangeTauRate(
		LOW_SPEED); /*!< Lower the speed to make sure burst receive is working */
#endif
	Swi_SendRawWordNoIrq(
		SWI_RRA,
		LOW_BYTE_ADDR(uw_Address)); /*!< send low byte of address */
...
*/
uint16_t Swi_ReadConfigSpaceBurst(uint16_t uw_Address, uint8_t *ubp_Data,
				  uint8_t len)
{
	uint16_t ret = INF_SWI_INIT;
	uint8_t index = 0;
	...
	Swi_SendRawWordNoIrq(SWI_BC,
			     SWI_ECFG); /*!< select ECFG register space */

	while (burst8_cnt != 0) {
		Swi_SendRawWordNoIrq(
			SWI_BC,
			SWI_RBL3); /*!< set read burst length to 8 bytes */
	...
	if (remaining_cnt >= BURST_LEN4) {
		Swi_SendRawWordNoIrq(
			SWI_BC,
			SWI_RBL2); /*!< set read burst length to 4 bytes */
		Swi_SendRawWordNoIrq(SWI_ERA, HIGH_BYTE_ADDR(uw_Address));
#if (SWI_FLOW_CONTROL_ENABLE == 1)
		Swi_ChangeTauRate(
			LOW_SPEED); /*!< Lower the speed to make sure burst receive is working */
	...
	if (remaining_cnt != 0) {
		Swi_SendRawWordNoIrq(
			SWI_BC,
			SWI_RBL0); /*!< set read burst length to 1 byte */

		while (remaining_cnt != 0) {
			Swi_SendRawWordNoIrq(SWI_ERA,
					     HIGH_BYTE_ADDR(uw_Address));
#if (SWI_FLOW_CONTROL_ENABLE == 1)
			Swi_ChangeTauRate(
				LOW_SPEED); /*!< Lower the speed to make sure burst receive is working */
#endif
			Swi_SendRawWordNoIrq(SWI_RRA,
					     LOW_BYTE_ADDR(uw_Address));
...
#if (SWI_FLOW_CONTROL_ENABLE == 1)
	Swi_ChangeTauRate(
		HIGH_SPEED); /*!< Lower the speed to make sure burst receive is working */
#endif
			   ...
			   SWI_BURST_SPEED ReadMode)
{
	...
	if (ReadMode == BURST_BYTES_READ) {
		ret = Swi_ReadConfigSpaceBurst(UID0_REG, burstvalue,
					       UID_BYTE_LENGTH);
		...
		if (order == LOW_BYTE_FIRST) {
			...
			while (i) {
				*(UID + i) =
					burstvalue[UID_BYTE_LENGTH - 1 - i];
				i--;
			}
		...
		for (i = 0; i < UID_BYTE_LENGTH; i++) {
			ret = Swi_ReadConfigSpace(
				(uint16_t)(UID0_REG + (UID_BYTE_LENGTH - 1) -
					   i),
				value);
			if (ret != INF_SWI_SUCCESS) {
				return ret;
