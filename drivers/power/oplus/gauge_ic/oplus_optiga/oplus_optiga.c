...
int oplus_optiga_parse_dt(struct oplus_optiga_chip *chip)
{
	...
	chip->optiga_active =
		pinctrl_lookup_state(chip->pinctrl, "optiga_active");
		...
		if (gpio_is_valid(chip->data_gpio)) {
			...
			if (rc) {
				...
				gpio_direction_output(
					g_oplus_optiga_chip->data_gpio, 0);
			}
		} else {
	...
	chip->support_optiga_in_lk =
		of_property_read_bool(node, "support_optiga_in_lk");
...
int optiga_authenticate(void)
{
	...
	if (first_read_uid == 0) {
#ifdef BURST_READ
		if (Swi_ReadUID_burst((uint8_t *)&stDetectedPuids[0],
				      HIGH_BYTE_FIRST,
				      BURST_BYTES_READ) != INF_SWI_SUCCESS)
#else
...
#endif
		{
			...
			Swi_PowerDownCmd();
			spin_unlock_irqrestore(&g_oplus_optiga_chip->slock,
					       flags);
			return ret;
		}
	}
	...
	if (Ecc_DoAuthenticationEnhanced(
		    (uint8_t *)&stDetectedPuids[devloop]) == FALSE) {
		chg_err("ECCE Failed\n");
	} else {
		chg_err("ECCE Passed\n");
		first_read_uid = 1;
...
int oplus_optiga_get_test_result(int *count_total, int *count_now,
				 int *fail_count)
{
	...
	if (ret == false) {
		...
		if (try_count < g_oplus_optiga_chip->try_count) {
			schedule_delayed_work_on(
				7, &g_oplus_optiga_chip->auth_work, 0);
			return;
		} else {
...
int oplus_optiga_auth(void)
{
	...
	if (!wait_for_completion_timeout(
		    &g_oplus_optiga_chip->is_complete,
...
static void oplus_optiga_test_func(struct work_struct *work)
{
	...
	while (g_oplus_optiga_chip->test_result.test_count_now <
	       g_oplus_optiga_chip->test_result.test_count_total) {
...
static const char *oplus_optiga_get_cmdline(void)
{
	...
	if (of_chosen) {
		optiga_auth =
			(char *)of_get_property(of_chosen, "optiga_auth", NULL);
		if (!optiga_auth)
...
static bool oplus_optia_check_auth_msg(void)
{
	...
	if (0 == memcmp(str, OPLUS_OPTIGA_AUTH_SUCCESS,
			sizeof(OPLUS_OPTIGA_AUTH_SUCCESS))) {
		ret = true;
		pr_info("oplus_chg_check_auth_msg: %s\n",
...
static int oplus_optiga_probe(struct platform_device *pdev)
{
	...
	g_oplus_optiga_chip = devm_kzalloc(
		&pdev->dev, sizeof(*g_oplus_optiga_chip), GFP_KERNEL);
	...
	INIT_DELAYED_WORK(&g_oplus_optiga_chip->auth_work,
			  oplus_optiga_auth_work);
	INIT_DELAYED_WORK(&g_oplus_optiga_chip->test_work,
			  oplus_optiga_test_func);

	if (g_oplus_optiga_chip->support_optiga_in_lk &&
	    oplus_optia_check_auth_msg()) {
	...
	external_auth_chip =
		devm_kzalloc(&pdev->dev,
			     sizeof(struct oplus_external_auth_chip),
			     GFP_KERNEL);
	if (!external_auth_chip) {
		ret = -ENOMEM;
		goto error;
	}
	external_auth_chip->get_external_auth_hmac =
		oplus_optiga_get_external_auth_hmac;
