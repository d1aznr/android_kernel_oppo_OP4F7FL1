...
#define ROTR(A, B)                                                             \
	(((A) >> (B)) | ((A) << (32 - (B)))) // rotate right operation
...
#define BIG_U8TO32(c)                                                          \
	(((uint32_t)T8(*(c)) << 24) | ((uint32_t)T8(*((c) + 1)) << 16) |       \
...
#define gf2n_add(A, B, C)                                                      \
	actual_gf2n_add(A, B, C) /* addition: out = op1 + op2 */
#define gf2n_sum(A, B) actual_gf2n_sum(A, B) /* addition: out += op */
#define gf2n_mul(A, B, C)                                                      \
	actual_gf2n_mul(A, B, C) /* multiplication: out = op1 * op2 */
#define gf2n_square(A, B) actual_gf2n_square(A, B) /* squaring: out = op^2 */
#define dwordvec_copy(A, B)                                                    \
	actual_dwordvec_copy(A, B) /* copy of a GF(2^131)-element */
...
BOOL verify_response(dwordvec_t gf2n_XResponse, dwordvec_t gf2n_ZResponse,
		     dwordvec_t gf2n_CheckValue,
		     const curve_parameter_t *curve);
BOOL verify_mac80(const mac_t mac_value, const dwordvec_t Z,
...
void mac_algorithm_80(mac_t mac_value, const mac_t data,
		      const mac_t session_key);
