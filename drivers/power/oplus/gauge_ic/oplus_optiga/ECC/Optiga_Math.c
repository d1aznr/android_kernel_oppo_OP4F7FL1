...
const uint32_t irred_polynomial_131[ARRAY_LEN(GF2_131)] = { 0x10d, 0x0, 0x0,
							    0x0, 0x8 };
static const uint32_t irred_polynomial_163[ARRAY_LEN(GF2_163)] = { 0xc9, 0x0,
								   0x0,	 0x0,
								   0x0,	 0x8 };
...
static const uint16_t square_tab[256] = {
	0x0,	0x1,	0x4,	0x5,	0x10,	0x11,	0x14,	0x15,	0x40,
 ...
 ********************************************************************/
BOOL generate_challenge(dwordvec_t xA, const curve_parameter_t *curve,
			dwordvec_t lambda)
{
	dwordvec_t A, B, C, D;
 ...
 * return FALSE if everything is ok, TRUE otherwise
 ********************************************************************/
BOOL generate_checkvalue(dwordvec_t xC, const dwordvec_t xT,
			 const curve_parameter_t *curve, dwordvec_t lambda)
{
	dwordvec_t A, B, C, D;
 ...
 * return FALSE if everything is ok, TRUE otherwise
 ********************************************************************/
BOOL verify_response(dwordvec_t gf2n_XResponse, dwordvec_t gf2n_ZResponse,
		     dwordvec_t gf2n_CheckValue, const curve_parameter_t *curve)
{
	uint32_t uwResult;
	dwordvec_t gf2n_AV;
 ...
 */
BOOL verify_mac64(const mac_t mac_value, const dwordvec_t Z,
		  const dwordvec_t xC, const mac_t data,
...
void gf2_131_square(dwordvec_t out, const dwordvec_t op)
{
	...
	t = op[0];
	temp[0] = ((uint32_t)(square_tab[t & 0xff]) ^
		   ((uint32_t)(square_tab[t >> 8 & 0xff]) << 16));
	temp[1] = ((uint32_t)(square_tab[t >> 16 & 0xff]) ^
		   ((uint32_t)(square_tab[t >> 24]) << 16));
	t = op[1];
	temp[2] = ((uint32_t)(square_tab[t & 0xff]) ^
		   ((uint32_t)(square_tab[t >> 8 & 0xff]) << 16));
	temp[3] = ((uint32_t)(square_tab[t >> 16 & 0xff]) ^
		   ((uint32_t)(square_tab[t >> 24]) << 16));
	t = op[2];
	temp[4] = ((uint32_t)(square_tab[t & 0xff]) ^
		   ((uint32_t)(square_tab[t >> 8 & 0xff]) << 16));
	temp[5] = ((uint32_t)(square_tab[t >> 16 & 0xff]) ^
		   ((uint32_t)(square_tab[t >> 24]) << 16));
	t = op[3];
	temp[6] = ((uint32_t)(square_tab[t & 0xff]) ^
		   ((uint32_t)(square_tab[t >> 8 & 0xff]) << 16));
	temp[7] = ((uint32_t)(square_tab[t >> 16 & 0xff]) ^
		   ((uint32_t)(square_tab[t >> 24]) << 16));
	t = op[4];
...
void gf2_163_square(dwordvec_t out, const dwordvec_t op)
{
	...
	temp[7] = ((uint32_t)(square_tab[t >> 16 & 0xff]) ^
		   ((uint32_t)(square_tab[t >> 24]) << 16));
	t = op[4];
	temp[8] = ((uint32_t)(square_tab[t & 0xff]) ^
		   ((uint32_t)(square_tab[t >> 8 & 0xff]) << 16));
	temp[9] = ((uint32_t)(square_tab[t >> 16 & 0xff]) ^
		   ((uint32_t)(square_tab[t >> 24]) << 16));
	t = op[5];
...
uint32_t dwordvec_sub(dwordvec_t c, const dwordvec_t a, const dwordvec_t b)
{
	...
	for (i = 1; i < ARRAY_LEN(actual_degree); i++) {
		...
		c[i] = a[i] -
		       borrow; /* subtract borrow of preceeding loop from a */
 ...
 *
 ********************************************************************/
void mont_ecc_add(dwordvec_t E, dwordvec_t F, const dwordvec_t A,
		  const dwordvec_t B, const dwordvec_t C, const dwordvec_t D,
		  const dwordvec_t point_x)
{
	dwordvec_t G;
 ...
 *
 ********************************************************************/
void mont_ecc_double(dwordvec_t E, dwordvec_t F, const dwordvec_t A,
		     const dwordvec_t B)
{
	dwordvec_t C;
 ...
 */
void mac_algorithm_64(mac_t mac_value, const mac_t data,
		      const mac_t session_key)
{
	...
	for (i = 0; i < 113; i++) {
		...
		y1 = ((x3 | ~x2) & x0) ^ (((~x3 & x2) ^ x3) & x1) ^ (x3 & x2) ^
		     x3;
		y2 = (((x2 ^ x3) | ~x1) & x0) ^ (((~x3 & x2) ^ x3) & x1) ^
		     (~x3 & x2);
 ...
 *                [in] scalar - scalar
 ********************************************************************/
void ecc_mul_projective(dwordvec_t X, dwordvec_t Y, dwordvec_t Z,
			const eccpoint_t *p, const dwordvec_t scalar)
{
	...
	gf2n_mul(t2, t2,
		 t3); /* (X1+x*Z1)*((X1+x*Z1)*(X2+x*Z2)+(x^2+y)*Z1*Z2) */
	gf2n_mul(t1, p->y_coord, Z); /* x*y*Z1^2*Z2 */
	gf2n_add(
		Y, t1,
		t2); /* (X1+x*Z1)*((X1+x*Z1)*(X2+x*Z2)+(x^2+y)*Z1*Z2)+x*y*Z1^2*Z2 */
}
 ...
 ********************************************************************/
BOOL ecc_add_point_x(dwordvec_t point_x, const dwordvec_t X1,
		     const dwordvec_t Y1, const dwordvec_t Z1,
		     const dwordvec_t X2, const dwordvec_t Y2,
		     const dwordvec_t Z2)
{
	...
	if (dwordvec_iszero(t2)) {
		...
		return gf2n_divide(
			point_x, t1,
			t2); /* TRUE, when doubling point of order 2 */
	}
	...
	gf2n_sum(
		t3,
		t5); /* (Y1*Z2+Y2*Z1)^2+(Y1*Z2+Y2*Z1)*(X1*Z2+X2*Z1)+a*(X1*Z2+X2*Z1)^2 */
	gf2n_mul(t5, Z1, Z2); /* Z1*Z2 */
	gf2n_mul(
		t3, t3,
		t5); /* Z1*Z2*((Y1*Z2+Y2*Z1)^2+(Y1*Z2+Y2*Z1)*(X1*Z2+X2*Z1)+a*(X1*Z2+X2*Z1)^2) */
	gf2n_sum(
		t3,
		t2); /* Z1*Z2*((Y1*Z2+Y2*Z1)^2+(Y1*Z2+Y2*Z1)*(X1*Z2+X2*Z1)+a*(X1*Z2+X2*Z1)^2)+(X1*Z2+X2*Z1)^3 */
 ...
 ********************************************************************/
BOOL ECDSA_verify(const signature_t *sig, const uint8_t *hash_data,
		  const eccpoint_t *pub_key, const curve_parameter_t *curve)
{
	int i;
	...
	order_length =
		actual_degree; // compute bit length of actual_base_point_order_OUL
	while ((actual_base_point_order[order_length / 32] &
		1UL << order_length % 32) == 0) {
		if (order_length < 160)
	...
	for (i = ARRAY_LEN(order_length) - 1; i >= 0; i--) {
		...
		if (order_length % 32 == 0) {
			...
			hash[i] = t1 << (order_length % 32) |
				  t2 >> (32 - order_length % 32);
		}
		t1 = t2;
		ptr += 4;
	}
...
void sha256_compress(sha256_context_t *context)
{
	...
	for (i = 0; i < 64; i++) {
		if (i > 15) {
			W[i & 15] =
				LIT_SIG_1(W[(i - 2) & 15]) + W[(i - 7) & 15] +
				LIT_SIG_0(W[(i - 15) & 15]) + W[(i - 16) & 15];
		}
 ...
 * [in,out] context pointer to sha256 context
 ********************************************************************/
static void sha256_update(const uint8_t *input_data,
 ...
 ********************************************************************/
void sha256(uint8_t *hash_value, const uint8_t *input_data,
	    const uint32_t input_length)
{
