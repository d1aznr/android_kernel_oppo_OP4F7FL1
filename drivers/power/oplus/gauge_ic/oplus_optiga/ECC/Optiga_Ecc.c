...
BOOL Ecc_DoAuthentication()
{
	...
	if (Ecc_GenerateChallenge(gf2nChallenge, gf2nRandomValue, MODE_ECCS) ==
	    FALSE) {
		return FALSE;
	}

	if (Ecc_GenerateCheckValue(gf2nCheck, gf2nRandomValue, g_public_key_131,
				   MODE_ECCS) == FALSE) {
		return FALSE;
	}

	if (Ecc_StartECC(gf2nChallenge, gf2nReturnX, gf2nReturnZ, TRUE,
			 MODE_ECCS) == FALSE) {
		return FALSE;
	}

	if (Ecc_VerifyResponse(gf2nReturnX, gf2nReturnZ, gf2nCheck,
			       g_public_key_131, MODE_ECCS) == FALSE) {
		return FALSE;
	}
...
BOOL Ecc_DoAuthenticationEnhanced(uint8_t *gf2nUid)
{
...
#ifdef BURST_READ
		if (Ecc_ReadODC_Burst(gf2nODC_first, gf2nPublicKey_first) !=
		    INF_SWI_SUCCESS)
#else
	...
	if (Ecc_GenerateChallenge(gf2nChallenge, gf2nRandomValue, MODE_ECCE) ==
	    FALSE) {
		return FALSE;
	}

	if (Ecc_GenerateCheckValue(gf2nCheck, gf2nRandomValue, gf2nPublicKey,
				   MODE_ECCE) == FALSE) {
		return FALSE;
	}
...
/* ****************************************************************************
   ...
   date:      .
 ************************************************************************* */
BOOL Ecc_GenerateChallenge(dwordvec_t gf2n_Challenge,
			   dwordvec_t gf2n_RandomValue, uint8_t bEccMode)
{
	/* get new random value and calculate challenge */
	Ecc_Rng128(gf2n_RandomValue);

	if (bEccMode == MODE_ECCS)
		generate_challenge(gf2n_Challenge, &ECCS_131_Curve,
				   gf2n_RandomValue);
	else
		generate_challenge(gf2n_Challenge, ECCE_131_Curve,
				   gf2n_RandomValue);
...
/* ****************************************************************************
   ...
   date:      .
 ************************************************************************* */
BOOL Ecc_GenerateCheckValue(dwordvec_t gf2n_CheckValue,
			    dwordvec_t gf2n_RandomValue,
			    dwordvec_t gf2n_PublicKey, uint8_t bEccMode)
{
	if (bEccMode == MODE_ECCS)
		generate_checkvalue(gf2n_CheckValue, gf2n_PublicKey,
				    &ECCS_131_Curve, gf2n_RandomValue);
	else
		generate_checkvalue(gf2n_CheckValue, gf2n_PublicKey,
				    ECCE_131_Curve, gf2n_RandomValue);
...
/* ****************************************************************************
   ...
   date:      .
 ************************************************************************* */
BOOL Ecc_VerifyResponse(dwordvec_t gf2n_XResponse, dwordvec_t gf2n_ZResponse,
			dwordvec_t gf2n_CheckValue, dwordvec_t gf2n_PublicKey,
			uint8_t bEccMode)
{
#if defined(SLE95150) || defined(SLE95250) || defined(SLE95300)
	mac_t hostMac;
#endif
	if (bEccMode == MODE_ECCS)
		return verify_response(gf2n_XResponse, gf2n_ZResponse,
				       gf2n_CheckValue, &ECCS_131_Curve);
#if defined(SLE95150) || defined(SLE95250) || defined(SLE95300)
	else
		return verify_mac64(gf2n_XResponse, gf2n_ZResponse,
				    gf2n_CheckValue, gf2n_PublicKey,
				    ECCE_131_Curve, hostMac);
#endif
}
#if defined(SLE95250) || defined(SLE95300)
/* ****************************************************************************
   ...
   date:      .
 ************************************************************************* */
BOOL Ecc_VerifyODC(uint32_t *gf2n_ODC, uint32_t *gf2n_PublicKey,
		   uint8_t *gf2n_Uid)
{
	...
	memset((uint8_t *)(sigSignature[0].r_value), 0,
	       sizeof(sigSignature[0].r_value));
	memset((uint8_t *)(sigSignature[0].s_value), 0,
	       sizeof(sigSignature[0].s_value));

	memcpy((uint8_t *)(sigSignature[0].r_value), (uint8_t *)gf2n_ODC, 21);
	memcpy((uint8_t *)(sigSignature[0].s_value), ((uint8_t *)gf2n_ODC) + 24,
	       21);
	...
	return ECDSA_verify(sigSignature, (uint8_t *)ubHashOut,
			    ODC_163_PubKeyXY, &ODC_163_Curve);
}
#endif
...
/* ****************************************************************************
   ...
   date:
 ************************************************************************* */
BOOL Ecc_StartECC(dwordvec_t gf2n_Challenge, dwordvec_t gf2n_XResponse,
		  dwordvec_t gf2n_ZResponse, BOOL bPolling, uint8_t bEccMode)
{
	BOOL ret;
#if defined(SLE95150) || defined(SLE95250) || defined(SLE95300)
	...
	ret = Ecc_SendChallengeAndGetResponse((uint16_t *)gf2n_Challenge,
					      (uint16_t *)gf2n_XResponse,
					      (uint16_t *)gf2n_ZResponse,
					      bPolling, bEccMode);
...
static BOOL Ecc_EncodeChlg(dwordvec_t Chlg)
{
	...
	for (i = 0; i < 17; i++) {
		*(((uint8_t *)Chlg) + i) =
			ChallengeByteEncode(*(((uint8_t *)Chlg) + i));
	}
...
static BOOL Ecc_DecodeRes(dwordvec_t Res, mac_t Mac)
{
	...
	for (i = 0; i < 17; i++) {
		*(((uint8_t *)Res) + i) =
			RespByteDecode(*(((uint8_t *)Res) + i));
	}
	for (i = 0; i < 8; i++) {
		*(((uint8_t *)Mac) + i) =
			RespByteDecode(*(((uint8_t *)Mac) + i));
	}
