...
int oplus_gauge_get_batt_temperature(void)
{
	...
	if (!g_gauge_chip) {
		...
		if (gauge_dbg_tbat != 0 || sub_gauge_dbg_tbat != 0) {
			printk(KERN_ERR
			       "[OPLUS_CHG]debug enabled, gauge_dbg_tbat[%d],  sub_gauge_dbg_ibat[%d]\n",
			       gauge_dbg_tbat, sub_gauge_dbg_tbat);
			return gauge_dbg_tbat != 0 ? gauge_dbg_tbat :
						     sub_gauge_dbg_tbat;
		}
...
int oplus_gauge_get_batt_soc(void)
{
	...
	if (!g_gauge_chip) {
		...
		if (gauge_dbg_soc != 0) {
			...
			soc_remainder =
				(batt_soc * g_gauge_chip->capacity_pct +
				 sub_batt_soc *
					 g_sub_gauge_chip->capacity_pct) %
				100;
			...
			printk(KERN_ERR
			       "batt_soc:%d,  sub_batt_soc:%d, soc_remainder:%d, soc:%d,\n",
...
int oplus_gauge_get_external_hmac_test_result(int *count_total, int *count_now,
					      int *fail_count)
{
	...
	if (!g_external_auth_chip) {
		...
		return g_external_auth_chip->get_hmac_test_result(
			count_total, count_now, fail_count);
	}
}

int oplus_gauge_get_external_hmac_status(int *status, int *fail_count,
					 int *total_count, int *real_fail_count,
...
{
	if (g_gauge_chip) {
		g_gauge_chip->gauge_ops->set_float_uv_ma(iterm_ma,
							 float_volt_uv);
	}
}
...
int oplus_gauge_get_sub_batt_mvolts(void)
{
	if (!g_sub_gauge_chip || !g_sub_gauge_chip->gauge_ops ||
		...
		if (sub_gauge_dbg_vbat != 0) {
			printk(KERN_ERR
			       "[OPLUS_CHG]%s:debug enabled,voltage sub_gauge_dbg_vbat[%d] \n",
...
int oplus_gauge_get_sub_batt_current(void)
{
	if (!g_sub_gauge_chip || !g_sub_gauge_chip->gauge_ops ||
		...
		if (sub_gauge_dbg_ibat != 0) {
			printk(KERN_ERR
			       "[OPLUS_CHG]debug enabled,current sub_gauge_dbg_ibat[%d] \n",
			       sub_gauge_dbg_ibat);
	...
	if (!g_sub_gauge_chip || !g_sub_gauge_chip->gauge_ops ||
		...
		if (sub_gauge_dbg_tbat != 0 || gauge_dbg_tbat != 0) {
			printk(KERN_ERR
			       "[OPLUS_CHG]debug enabled, gauge_dbg_tbat[%d],  sub_gauge_dbg_ibat[%d]\n",
			       gauge_dbg_tbat, sub_gauge_dbg_tbat);
			return sub_gauge_dbg_tbat != 0 ? sub_gauge_dbg_tbat :
							 gauge_dbg_tbat;
		}
		batt_temp =
			g_sub_gauge_chip->gauge_ops->get_battery_temperature();
...
int oplus_gauge_get_bcc_parameters(char *buf)
{
	if (!g_gauge_chip)
		...
		if (g_gauge_chip->gauge_ops &&
		    g_gauge_chip->gauge_ops->get_bcc_parameters) {
...
int oplus_gauge_fastchg_update_bcc_parameters(char *buf)
{
	if (!g_gauge_chip)
		...
		if (g_gauge_chip->gauge_ops &&
		    g_gauge_chip->gauge_ops->get_update_bcc_parameters) {
...
int oplus_gauge_get_prev_bcc_parameters(char *buf)
{
	if (!g_gauge_chip)
		...
		if (g_gauge_chip->gauge_ops &&
		    g_gauge_chip->gauge_ops->get_prev_bcc_parameters) {
...
int oplus_gauge_set_bcc_parameters(const char *buf)
{
	if (!g_gauge_chip)
		...
		if (g_gauge_chip->gauge_ops &&
		    g_gauge_chip->gauge_ops->set_bcc_parameters) {
...
bool oplus_gauge_check_rc_sfr(void)
{
	if (!g_gauge_chip)
		...
		if (g_gauge_chip->gauge_ops &&
		    g_gauge_chip->gauge_ops->check_rc_sfr) {
...
int oplus_gauge_soft_reset_rc_sfr(void)
{
	if (!g_gauge_chip)
		...
		if (g_gauge_chip->gauge_ops &&
		    g_gauge_chip->gauge_ops->soft_reset_rc_sfr) {
