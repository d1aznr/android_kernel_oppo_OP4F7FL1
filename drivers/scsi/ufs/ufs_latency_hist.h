...
struct io_latency_state {
	struct io_latency_chunck_size
		latency_y_axis[ARRAY_SIZE(latency_x_axis_us) + 1];
	u_int64_t latency_elems;
	u_int64_t latency_sum;
};

static inline void io_update_latency_hist(struct io_latency_state *s,
					  u_int64_t delta_us,
					  unsigned int length)
{
	int i;
...
ssize_t io_latency_hist_show(char *name, struct io_latency_state *s, char *buf,
			     int buf_size);
