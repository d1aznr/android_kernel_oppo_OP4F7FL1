...
static void ufshpb_set_read16_cmd(struct ufshpb_lu *hpb,
				  struct ufshcd_lrb *lrbp, u64 ppn,
...
void ufshpb_prep_fn(struct ufsf_feature *ufsf, struct ufshcd_lrb *lrbp)
{
	...
	if (!ufshpb_is_support_chunk(transfer_len)) {
		TMSG_CMD(hpb, "READ_10 doesn't support chunk size", rq, rgn_idx,
			 srgn_idx);
		goto put_hpb;
	}
	...
	spin_unlock_irqrestore(&hpb->hpb_lock, flags);
	if (unlikely(error)) {
		ERR_MSG("get_ppn failed.. err %d region %d subregion %d", error,
			rgn_idx, srgn_idx);
		ufshpb_lu_put(hpb);
			     ...
			     struct ufshpb_map_ctx *mctx, const char *str)
{
	...
	if (!error)
		val[1] =
			ufshpb_get_ppn(mctx, hpb->entries_per_srgn - 1, &error);
	if (error)
		val[0] = val[1] = 0;
...
static int ufshpb_map_req_error(struct ufshpb_req *map_req)
{
	...
	if (rgn->rgn_state == HPB_RGN_PINNED) {
		...
		HPB_DEBUG(hpb, "pinned rb %d - %d(dirty)", map_req->rb.rgn_idx,
			  map_req->rb.srgn_idx);
...
static inline struct ufshpb_rsp_field *
ufshpb_get_hpb_rsp(struct ufshcd_lrb *lrbp)
{
				...
				struct ufshpb_subregion *srgn)
{
	...
	ufshpb_set_map_req(hpb, srgn->rgn_idx, srgn->srgn_idx, srgn->mctx,
			   map_req);
	...
	if (ret) {
		ERR_MSG("issue map_req failed. [%d-%d] err %d", srgn->rgn_idx,
			srgn->srgn_idx, ret);
		ufshpb_lu_put(hpb);
		goto wakeup_ee_worker;
	}
	...
	if (rgn->rgn_state == HPB_RGN_INACTIVE) {
		if (atomic64_read(&lru_info->active_cnt) ==
			...
			TMSG(hpb->ufsf, hpb->lun, "Noti: VT RG %d",
			     victim_rgn->rgn_idx);
			HPB_DEBUG(
				hpb, "LRU MAX(=%lld). victim choose %d",
					 ...
					 struct ufshpb_rsp_field *rsp_field)
{
	...
	for (num = 0; num < rsp_field->active_rgn_cnt; num++) {
		rgn_idx = be16_to_cpu(
			rsp_field->hpb_active_field[num].active_rgn);
		srgn_idx = be16_to_cpu(
			rsp_field->hpb_active_field[num].active_srgn);
...
static int ufshpb_issue_map_req_from_list(struct ufshpb_lu *hpb)
{
	...
	while ((srgn = list_first_entry_or_null(&hpb->lh_pinned_srgn,
		...
		TMSG(hpb->ufsf, hpb->lun, "Noti: I RB %d - %d", srgn->rgn_idx,
		     srgn->srgn_idx);
		...
		TMSG(hpb->ufsf, hpb->lun, "Noti: C RB %d - %d", srgn->rgn_idx,
		     srgn->srgn_idx);
...
static int ufshpb_check_pm(struct ufshpb_lu *hpb)
{
	...
	if (hba->pm_op_in_progress ||
		...
		INFO_MSG("hba current power state %d pm_progress %d",
			 hba->curr_dev_pwr_mode, hba->pm_op_in_progress);
		return -ENODEV;
	}
	return 0;
}
...
static void ufshpb_retry_work_handler(struct work_struct *work)
{
	...
	list_for_each_entry_safe (map_req, next, &retry_list, list_req) {
		if (ufshpb_get_state(hpb->ufsf) == HPB_SUSPEND) {
			INFO_MSG(
				"suspend state. Issue READ_BUFFER in the next turn");
			spin_lock_bh(&hpb->retry_list_lock);
...
static void ufshpb_run_active_subregion_list(struct ufshpb_lu *hpb)
{
	...
	while ((srgn = list_first_entry_or_null(&hpb->lh_act_srgn,
		...
		if (ufshpb_get_state(hpb->ufsf) == HPB_SUSPEND) {
			INFO_MSG(
				"suspend state. Issuing READ_BUFFER the next turn");
			break;
		}
					    ...
					    struct ufshpb_region *rgn)
{
	...
	for (srgn_idx = 0; srgn_idx < rgn->srgn_cnt; srgn_idx++) {
		...
		srgn->mctx = ufshpb_get_map_ctx(hpb, &err);
		if (err) {
			ERR_MSG("get mctx err %d srgn %d free_table %d", err,
				srgn_idx, hpb->debug_free_table);
			goto release;
		}
				    ...
				    struct ufshpb_lu *hpb)
{
	...
	/*	From descriptors	*/
	rgn_unit_size =
		(unsigned long long)SECTOR * (0x01 << hpb_dev_info->rgn_size);
	rgn_mem_size = rgn_unit_size / BLOCK * HPB_ENTRY_SIZE;

	hpb->srgn_unit_size =
		(unsigned long long)SECTOR * (0x01 << hpb_dev_info->srgn_size);
	 ...
	 */
	hpb->rgns_per_lu = ((unsigned long long)hpb->lu_num_blocks +
			    (rgn_mem_size / HPB_ENTRY_SIZE) - 1) /
			   (rgn_mem_size / HPB_ENTRY_SIZE);
	hpb->srgns_per_lu = ((unsigned long long)hpb->lu_num_blocks +
...
static int ufshpb_lu_hpb_init(struct ufsf_feature *ufsf, int lun)
{
	...
	if (total_srgn_cnt != 0) {
		ERR_MSG("error total_subregion_count: %d", total_srgn_cnt);
		goto release_srgns;
	}
...
{
	INFO_MSG(
		"Support HPB Spec : UFSHPB_VER_2_0_0 = (%.4X)   UFSHPB_VER_2_0_1 = (%.4X)   Device = (%.4X)",
		UFSHPB_VER_2_0_0, UFSHPB_VER_2_0_1, hpb_dev_info->version);

	INFO_MSG("HPB Driver Version : (%.6X%s)", UFSHPB_DD_VER,
		 UFSHPB_DD_VER_POST);
...
void ufshpb_get_lu_info(struct ufsf_feature *ufsf, int lun, u8 *unit_buf)
{
	...
	INFO_MSG("LUN(%d) [0A] bLogicalBlockSize (%d)", lun,
		 lu_desc.lu_logblk_size);
	INFO_MSG("LUN(%d) [0B] qLogicalBlockCount (%llu)", lun,
		 lu_desc.lu_logblk_cnt);
	INFO_MSG("LUN(%d) [03] bLuEnable (%d)", lun, lu_desc.lu_enable);
	INFO_MSG("LUN(%d) [06] bLuQueueDepth (%d)", lun,
		 lu_desc.lu_queue_depth);
	INFO_MSG("LUN(%d) [23:24] wLUMaxActiveHPBRegions (%d)", lun,
		 lu_desc.lu_max_active_hpb_rgns);
	INFO_MSG("LUN(%d) [25:26] wHPBPinnedRegionStartIdx (%d)", lun,
		 lu_desc.lu_hpb_pinned_rgn_startidx);
	INFO_MSG("LUN(%d) [27:28] wNumHPBPinnedRegions (%d)", lun,
		 lu_desc.lu_num_hpb_pinned_rgns);
	INFO_MSG("LUN(%d) PINNED Start (%d) End (%d)", lun,
		 lu_desc.lu_hpb_pinned_rgn_startidx,
		 lu_desc.lu_hpb_pinned_end_offset);
...
static int ufshpb_init(struct ufsf_feature *ufsf)
{
	...
	seq_scan_lu(lun) if (ufsf->hpb_lup[lun])
	{
		hpb = ufsf->hpb_lup[lun];
...
void ufshpb_remove(struct ufsf_feature *ufsf, int state)
{
	...
	INFO_MSG("kref count (%d)", atomic_read(&ufsf->hpb_kref.refcount.refs));
...
void ufshpb_init_handler(struct work_struct *work)
{
	...
	ret = wait_event_timeout(ufsf->hpb_wait, ufshpb_probe_lun_done(ufsf),
...
void ufshpb_resume(struct ufsf_feature *ufsf)
{
	...
	seq_scan_lu(lun)
	{
		...
		if (hpb) {
			...
			INFO_MSG(
				"ufshpb_lu %d resume. do_task_work %d retry %d",
...
static ssize_t ufshpb_sysfs_debug_store(struct ufshpb_lu *hpb, const char *buf,
					size_t cnt)
{
	unsigned long debug;
...
static ssize_t ufshpb_sysfs_version_show(struct ufshpb_lu *hpb, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "HPB version %.4X D/D version %.6X%s\n",
		       hpb->ufsf->hpb_dev_info.version, UFSHPB_DD_VER,
		       UFSHPB_DD_VER_POST);
...
static ssize_t ufshpb_sysfs_hit_show(struct ufshpb_lu *hpb, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE,
		       ...
		       hit_cnt, hit_4k_cnt, hit_8_32k_cnt, span_cnt,
		       span_hit_cnt);
	INFO_MSG("%s", buf);
...
static ssize_t ufshpb_sysfs_map_req_show(struct ufshpb_lu *hpb, char *buf)
{
	...
	INFO_MSG("rb_noti %lld ACT %lld INACT %lld map_req_count %lld",
		 rb_noti_cnt, rb_active_cnt, rb_inactive_cnt, map_req_cnt);
...
static ssize_t ufshpb_sysfs_region_stat_show(struct ufshpb_lu *hpb, char *buf)
{
	...
	INFO_MSG("Total %d pinned %d active %d inactive %d", hpb->rgns_per_lu,
		 pin_cnt, act_cnt, inact_cnt);
					   ...
					   const char *buf, size_t cnt)
{
	...
	if (value > hpb->lu_num_blocks * SECTORS_PER_BLOCK) {
		ERR_MSG("value %lu > lu_num_blocks %d error", value,
			hpb->lu_num_blocks);
		return -EINVAL;
	}
						 ...
						 const char *buf, size_t cnt)
{
	...
	return cnt;
err_out:
	INFO_MSG("ref_cnt %d", atomic_read(&hpb->ufsf->hpb_kref.refcount.refs));
	ufshpb_failed(hpb, __func__);
...
static struct ufshpb_sysfs_entry ufshpb_sysfs_entries[] = {
	__ATTR(hpb_read_disable, 0644, ufshpb_sysfs_prep_disable_show,
	       ufshpb_sysfs_prep_disable_store),
	__ATTR(map_cmd_disable, 0644, ufshpb_sysfs_map_disable_show,
	       ufshpb_sysfs_map_disable_store),
	__ATTR(throttle_map_req, 0644, ufshpb_sysfs_throttle_map_req_show,
	...
	__ATTR(pre_req_min_tr_len, 0644, ufshpb_sysfs_pre_req_min_tr_len_show,
	       ufshpb_sysfs_pre_req_min_tr_len_store),
	__ATTR(pre_req_max_tr_len, 0644, ufshpb_sysfs_pre_req_max_tr_len_show,
...
static int ufshpb_create_sysfs(struct ufsf_feature *ufsf, struct ufshpb_lu *hpb)
{
	...
	if (!err) {
		for (entry = hpb->sysfs_entries; entry->attr.name != NULL;
		     entry++) {
			INFO_MSG("ufshpb_lu%d sysfs attr creates: %s", hpb->lun,
				 entry->attr.name);
				  ...
				  size_t cnt, loff_t *data)
{
	...
	seq_scan_lu(lun)
	{
		if (ufsf_para.ufsf->hpb_lup[lun])
			hpbfn_enable_ctrl(ufsf_para.ufsf->hpb_lup[lun], val);
	}
