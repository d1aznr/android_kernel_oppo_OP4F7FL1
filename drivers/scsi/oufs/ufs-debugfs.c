...
#define UFS_ERR_STATS_PRINT(file, error_index, string, error_seen)             \
	do {                                                                   \
		if (err_stats[error_index]) {                                  \
			seq_printf(file, string, err_stats[error_index]);      \
			error_seen = true;                                     \
		}                                                              \
	} while (0)
...
static const int err_inject_pwr_change_err_codes[] = {
	-EIO,	       -ETIMEDOUT,	-1, PWR_REMOTE, PWR_BUSY,
	PWR_ERROR_CAP, PWR_FATAL_ERROR,
};
...
static bool inject_fatal_err_tm(struct ufs_hba *hba, u8 ocs_err)
{
	...
	(&hba->utmrdl_base_addr[tag])->header.dword_2 = cpu_to_be32(ocs_err);
...
static bool ufsdbg_find_err_code(enum ufsdbg_err_inject_scenario usecase,
				 int *ret, u32 *index)
{
				    ...
				    int success_value, int *ret_value)
{
	...
	if ((unlikely(!hba || !ret_value)) ||
	    (likely(!(hba->debugfs_files.err_inj_scenario_mask & BIT(usecase)))))
		goto out;
...
should_fail:
	*ret_value = opt_ret;
	...
	pr_debug(
		"%s: error code index [%d], error code %d (0x%x) is injected for scenario \"%s\"\n",
...
{
	return single_open(file, ufsdbg_err_inj_scenario_read,
			   inode->i_private);
}
...
static void ufsdbg_setup_fault_injection(struct ufs_hba *hba)
{
	...
	if (IS_ERR(fault_dir)) {
		dev_err(hba->dev,
			"%s: failed to create debugfs entry for fault injection\n",
			__func__);
		return;
	}
				      ...
				      loff_t *ppos)
{
	...
	if (!val) {
		...
		memset(hba->ufs_stats.tag_stats[0], 0,
		       sizeof(**hba->ufs_stats.tag_stats) * TS_NUM_STATS *
			       hba->nutrs);
...
static int ufsdbg_query_stats_show(struct seq_file *file, void *data)
{
	...
	static const char *opcode_name[UPIU_QUERY_OPCODE_MAX] = {
		"QUERY_OPCODE_NOP:",	     "QUERY_OPCODE_READ_DESC:",
		"QUERY_OPCODE_WRITE_DESC:",  "QUERY_OPCODE_READ_ATTR:",
		"QUERY_OPCODE_WRITE_ATTR:",  "QUERY_OPCODE_READ_FLAG:",
		"QUERY_OPCODE_SET_FLAG:",    "QUERY_OPCODE_CLEAR_FLAG:",
	...
	seq_puts(
		file,
		"The following table shows how many TIMES each IDN was sent to device for each QUERY OPCODE:\n");
	seq_puts(file, "\n");
...
static int ufsdbg_err_stats_show(struct seq_file *file, void *data)
{
	...
	UFS_ERR_STATS_PRINT(
		file, UFS_ERR_HIBERN8_EXIT,
		"controller reset due to hibern8 exit error:\t %d\n",
		error_seen);

	UFS_ERR_STATS_PRINT(
		file, UFS_ERR_VOPS_SUSPEND,
	...
	UFS_ERR_STATS_PRINT(
		file, UFS_ERR_CLEAR_PEND_XFER_TM,
	...
	UFS_ERR_STATS_PRINT(file, UFS_ERR_RESUME, "resume error:\t\t %d\n",
			    error_seen);

	UFS_ERR_STATS_PRINT(file, UFS_ERR_SUSPEND, "suspend error:\t\t %d\n",
			    error_seen);
...
static int ufshcd_init_statistics(struct ufs_hba *hba)
{
	...
	stats->enabled = false;
	stats->tag_stats =
		kcalloc(hba->nutrs, sizeof(*stats->tag_stats), GFP_KERNEL);
	if (!hba->ufs_stats.tag_stats)
		goto no_mem;

	stats->tag_stats[0] =
		kzalloc(sizeof(**stats->tag_stats) * TS_NUM_STATS * hba->nutrs,
			GFP_KERNEL);
...
static int ufsdbg_show_hba_show(struct seq_file *file, void *data)
{
	...
	seq_printf(
		file, "dl_tcx_replay_timer_expired_err_cnt = %d\n",
		hba->ufs_stats.dl_err_cnt[UFS_EC_DL_TCx_REPLAY_TIMER_EXPIRED]);
	seq_printf(
		file, "dl_afcx_request_timer_expired_err_cnt = %d\n",
		hba->ufs_stats.dl_err_cnt[UFS_EC_DL_AFCx_REQUEST_TIMER_EXPIRED]);
	seq_printf(
		file, "dl_fcx_protection_timer_expired_err_cnt = %d\n",
	...
	seq_printf(
		file, "dl_max_frame_length_exceeded_err_cnt = %d\n",
...
{
	return single_open(file, ufsdbg_dump_device_desc_show,
			   inode->i_private);
}
...
static int ufsdbg_power_mode_show(struct seq_file *file, void *data)
{
	...
	static const char *const names[] = {
		"INVALID MODE", "FAST MODE",	 "SLOW MODE",
		"INVALID MODE", "FASTAUTO MODE", "SLOWAUTO MODE",
		"INVALID MODE",
	};
	...
	/* Print usage */
	seq_puts(file, "To change power mode write 'GGLLMM' where:\n"
		       "G - selected gear\n"
		       "L - number of lanes\n"
		       "M - power mode:\n"
		       "\t1 = fast mode\n"
				       ...
				       loff_t *ppos)
{
	...
	ret = simple_write_to_buffer(pwr_mode_str, BUFF_LINE_SIZE, &buff_pos,
				     ubuf, cnt);
	...
	if (ret == -EBUSY)
		...
		dev_err(hba->dev, "%s: ufshcd_config_pwr_mode failed, ret=%d\n",
			__func__, ret);
...
static int ufsdbg_dme_read(void *data, u64 *attr_val, bool peer)
{
	...
	if (!ret) {
		if ((attr_id >= MPHY_RX_ATTR_ADDR_START) &&
			...
			attr_sel = UIC_ARG_MIB_SEL(
				attr_id, UIC_ARG_MPHY_RX_GEN_SEL_INDEX(0));
		else
...
DEFINE_DEBUGFS_ATTRIBUTE(ufsdbg_dme_local_read_ops, ufsdbg_dme_local_read,
...
DEFINE_DEBUGFS_ATTRIBUTE(ufsdbg_dme_peer_read_ops, ufsdbg_dme_peer_read,
...
DEFINE_DEBUGFS_ATTRIBUTE(ufsdbg_dbg_print_en_ops, ufsdbg_dbg_print_en_read,
			 ufsdbg_dbg_print_en_set, "%llu\n");
...
static int ufsdbg_req_stats_show(struct seq_file *file, void *data)
{
	...
	/* Header */
	seq_printf(file, "\t%-10s %-10s %-10s %-10s %-10s %-10s", "All",
		   "Write", "Read", "Read(urg)", "Write(urg)", "Flush");
...
DEFINE_DEBUGFS_ATTRIBUTE(ufsdbg_err_state, ufsdbg_read_err_state,
...
void ufsdbg_add_debugfs(struct ufs_hba *hba)
{
	...
	hba->debugfs_files.debugfs_root =
		debugfs_create_dir(dev_name(hba->dev), NULL);
	...
	if (!hba->debugfs_files.debugfs_root) {
		 ...
		 */
		dev_err(hba->dev, "%s: NULL debugfs root directory, exiting\n",
			__func__);
		goto err_no_root;
	}

	hba->debugfs_files.stats_folder =
		debugfs_create_dir("stats", hba->debugfs_files.debugfs_root);
	if (!hba->debugfs_files.stats_folder) {
		dev_err(hba->dev, "%s: NULL stats_folder, exiting\n", __func__);
		goto err;
	}
	...
	hba->debugfs_files.host_regs =
		debugfs_create_file("host_regs", 0400,
				    hba->debugfs_files.debugfs_root, hba,
	...
	hba->debugfs_files.show_hba =
		debugfs_create_file("show_hba", 0400,
				    hba->debugfs_files.debugfs_root, hba,
	...
	if (!hba->debugfs_files.dump_dev_desc) {
		dev_err(hba->dev, "%s:  NULL dump_device_desc file, exiting\n",
			__func__);
		goto err;
	}
	...
	if (!hba->debugfs_files.power_mode) {
		dev_err(hba->dev, "%s:  NULL power_mode_desc file, exiting\n",
			__func__);
		goto err;
	}
	...
	if (!hba->debugfs_files.err_state) {
		dev_err(hba->dev, "%s: failed create err_state debugfs entry\n",
			__func__);
		goto err;
	}

	if (!debugfs_create_bool("crash_on_err", 0600,
				 hba->debugfs_files.debugfs_root,
				 &hba->crash_on_err))
		goto err;
