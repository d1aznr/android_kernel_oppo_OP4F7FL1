...
*/
static ssize_t latency_hist_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	...
	} else if (value == IO_LAT_HIST_ENABLE || value == IO_LAT_HIST_DISABLE)
			  ...
			  char *buf)
{
	...
	written_bytes +=
		io_latency_hist_show("Read", &hba->io_lat_read,
				     buf + written_bytes, 8192 - written_bytes);
	written_bytes +=
		io_latency_hist_show("Write", &hba->io_lat_write,
				     buf + written_bytes, 8192 - written_bytes);
	written_bytes +=
		io_latency_hist_show("Other", &hba->io_lat_other,
				     buf + written_bytes, 8192 - written_bytes);
