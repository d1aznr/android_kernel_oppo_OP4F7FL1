...
static void recordTimeStamp(struct signal_quality *record,
...
void recordUniproErr(struct unipro_signal_quality_ctrl *signalCtrl, u32 reg,
		     enum unipro_err_type type)
{
...
#define SEQ_UPLOAD_PRINT(x)                                                    \
	seq_printf(s, #x ": %d\n",                                             \
		   signalCtrl->record.x - signalCtrl->record_upload.x);        \
...
int create_signal_quality_proc(struct unipro_signal_quality_ctrl *signalCtrl)
{
	...
	d_entry =
		proc_create_data("record_upload", S_IRUGO, signalCtrl->ctrl_dir,
