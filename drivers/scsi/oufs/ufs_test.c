...
#define LONG_TEST_SIZE_FRACTION(x)                                             \
	(BYTE_TO_MB_x_10(x) - (LONG_TEST_SIZE_INTEGER(x) * 10))
...
#define add_test(utd, test_name, upper_case_name)                              \
	ufs_test_add_test(utd, UFS_TEST_##upper_case_name,                     \
			  "ufs_test_" #test_name,                              \
			     ...
			     const struct file_operations *test_fops)
{
	...
	utd->test_list[test_id] = debugfs_create_file(
		test_str, 0600, tests_root, NULL, test_fops);
...
static struct test_scenario *get_scenario(struct test_data *td,
					  enum scenario_id id)
{
...
{
	*num_of_bios =
		ufs_test_pseudo_random_seed(seed, 1, TEST_MAX_BIOS_PER_REQ);
	if (!(*num_of_bios))
...
static int ufs_test_run_write_read_test(struct test_data *td)
{
	...
	ret = test_iosched_add_wr_rd_test_req(0, READ, start_sec, num_bios,
					      TEST_PATTERN_5A, NULL);
...
static int ufs_test_run_lun_depth_test(struct test_data *td)
{
	...
	num_req[i++] = ufs_test_pseudo_random_seed(&utd->random_test_seed, 1,
						   lun_qdepth - 2);
	...
	num_req[i++] =
		nutrs + 1 +
		ufs_test_pseudo_random_seed(&utd->random_test_seed, 1, 10);
...
static int run_long_test(struct test_data *td)
{
	...
	switch (td->test_info.testcase) {
	case UFS_TEST_LONG_RANDOM_READ:
		utd->long_test_num_reqs =
			(utd->sector_range * SECTOR_SIZE) /
	...
	do {
		...
		inserted_requests++;
		if (utd->test_stage == UFS_TEST_LONG_SEQUENTIAL_MIXED_STAGE2) {
			ret = test_iosched_add_wr_rd_test_req(
				0, READ, sector, num_bios_per_request,
...
static int ufs_test_run_data_integrity_test(struct test_data *td)
{
	...
	for (i = 0; i < QUEUE_MAX_REQUESTS; i++) {
		ret = test_iosched_add_wr_rd_test_req(0, READ, sectors[i], 1, i,
						      long_test_free_end_io_fn);
...
static int ufs_test_debugfs_init(void)
{
	...
	utd->test_list =
		kmalloc(sizeof(struct dentry *) * NUM_TESTS, GFP_KERNEL);
	if (!utd->test_list)
		return -ENODEV;
	...
	utd->random_test_seed_dentry = debugfs_create_u32(
		"random_test_seed", 0666, utils_root, &utd->random_test_seed);
