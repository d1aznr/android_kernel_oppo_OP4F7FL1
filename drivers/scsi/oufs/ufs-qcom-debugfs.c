...
DEFINE_DEBUGFS_ATTRIBUTE(ufs_qcom_dbg_print_en_ops, ufs_qcom_dbg_print_en_read,
			 ufs_qcom_dbg_print_en_set, "%llu\n");
...
DEFINE_DEBUGFS_ATTRIBUTE(ufs_qcom_dbg_testbus_en_ops,
			 ...
			 ufs_qcom_dbg_testbus_en_set, "%llu\n");
...
DEFINE_DEBUGFS_ATTRIBUTE(ufs_qcom_dbg_testbus_bus_ops,
			 ufs_qcom_dbg_testbus_bus_read, NULL, "%llu\n");
...
static int ufs_qcom_dbg_dbg_regs_open(struct inode *inode, struct file *file)
{
	return single_open(file, ufs_qcom_dbg_dbg_regs_show, inode->i_private);
}
...
static int ufs_qcom_dbg_pm_qos_open(struct inode *inode, struct file *file)
{
...
void ufs_qcom_dbg_add_debugfs(struct ufs_hba *hba, struct dentry *root)
{
	...
	host->debugfs_files.testbus =
		debugfs_create_dir("testbus", host->debugfs_files.debugfs_root);
	if (!host->debugfs_files.testbus) {
		dev_err(host->hba->dev, "%s: failed create testbus directory\n",
			__func__);
		goto err;
	}

	host->debugfs_files.testbus_en =
		debugfs_create_file("enable", 0600, host->debugfs_files.testbus,
				    host, &ufs_qcom_dbg_testbus_en_ops);
