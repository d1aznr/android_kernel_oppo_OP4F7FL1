...
static bool ice_cap_idx_valid(struct ufs_hba *hba, unsigned int cap_idx)
{
					     ...
					     unsigned int slot)
{
	...
	if (!(data_unit_mask & hba->crypto_cap_array[crypto_alg_id].sdus_mask))
		return -EINVAL;
...
static enum blk_crypto_mode_num ufshcd_blk_crypto_qti_mode_num_for_alg_dusize(
	enum ufs_crypto_alg ufs_crypto_alg, enum ufs_crypto_key_size key_size)
{
	/*
...
static int
ufshcd_hba_init_crypto_qti_spec(struct ufs_hba *hba,
				const struct keyslot_mgmt_ll_ops *ksm_ops)
{
	...
	for (cap_idx = 0; cap_idx < hba->crypto_capabilities.num_crypto_cap;
	     cap_idx++) {
		hba->crypto_cap_array[cap_idx].reg_val = cpu_to_le32(
			ufshcd_readl(hba, REG_UFS_CRYPTOCAP +
	...
	hba->ksm =
		keyslot_manager_create(hba->dev, ufshcd_num_keyslots(hba),
				       ksm_ops,
				  ...
				  const struct keyslot_mgmt_ll_ops *ksm_ops)
{
	...
	mem_res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "ufs_ice");
	...
	err = crypto_qti_init_crypto(hba->dev, mmio_base,
