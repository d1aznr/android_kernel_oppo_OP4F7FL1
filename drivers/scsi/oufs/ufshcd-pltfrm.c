...
static int ufshcd_parse_reset_info(struct ufs_hba *hba)
{
	...
	hba->core_reset = devm_reset_control_get(hba->dev, "core_reset");
...
static int ufshcd_parse_clock_info(struct ufs_hba *hba)
{
	...
	clkfreq = devm_kcalloc(dev, sz, sizeof(*clkfreq), GFP_KERNEL);
	if (!clkfreq) {
		ret = -ENOMEM;
		goto out;
	}

	ret = of_property_read_u32_array(np, "freq-table-hz", clkfreq, sz);
	if (ret && (ret != -EINVAL)) {
