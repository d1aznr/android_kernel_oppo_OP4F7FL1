...
static int ufs_hisi_check_hibern8(struct ufs_hba *hba)
{
	...
	do {
		...
		err |= ufshcd_dme_get(hba,
				      UIC_ARG_MIB_SEL(MPHY_TX_FSM_STATE, 1),
				      &tx_fsm_val_1);
	...
	if (err) {
		...
		err = -1;
		dev_err(hba->dev,
			"%s: invalid TX_FSM_STATE, lane0 = %d, lane1 = %d\n",
...
static int ufs_hisi_get_pwr_dev_param(struct ufs_hisi_dev_params *hisi_param,
				      ...
				      struct ufs_pa_layer_attr *agreed_pwr)
{
	...
	if (!is_dev_sup_hs && is_hisi_max_hs) {
		 ...
		 */
		agreed_pwr->pwr_rx = agreed_pwr->pwr_tx = hisi_param->rx_pwr_hs;
	} else {
		/*
...
static int
ufs_hi3660_pwr_change_notify(struct ufs_hba *hba,
			     enum ufs_notify_change_status status,
			     ...
			     struct ufs_pa_layer_attr *dev_req_params)
{
	...
	case PRE_CHANGE:
		...
		ret = ufs_hisi_get_pwr_dev_param(&ufs_hisi_cap, dev_max_params,
						 dev_req_params);
		if (ret) {
			dev_err(hba->dev,
