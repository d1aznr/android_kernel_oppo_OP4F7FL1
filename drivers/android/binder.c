				     ...
				     struct binder_proc *proc, char *name)
{
	...
	if (NULL != tr && tr->target.handle == 0 && NULL != proc &&
	    NULL != proc->context) {
		if (!strcmp(proc->context->name, "hwbinder")) {
			...
			for (i = 0; (2 * i) < tr->data_size; i++) {
				...
				if (len >= (OPLUS_MAX_SERVICE_NAME_LEN - 1))
					break;
				tmp = (char *)(uintptr_t)(tr->data.ptr.buffer +
							  (2 * i));
				get_user(c, tmp);
				if (c >= 32 &&
				    c <= 126) { // visible character range [32, 126]
					if (len <
					    OPLUS_MAX_SERVICE_NAME_LEN - 1)
						len += sprintf(sname + len,
							       "%c", c);
					else
						break;
				}
		...
		pr_info("context.name[%s] tr.size:%lu service:%s\n",
			proc->context->name, (unsigned long)tr->data_size,
			sname);
	} else {
...
void ob_sysctrace_c(struct binder_proc *proc, struct binder_thread *thread)
{
	...
	snprintf(buf, sizeof(buf), "C|%d|oplus_bt%s|%d", proc->pid,
		 thread->task->comm, 1);
...
static char *obs_blacklist[] = { "rocess.contacts" };
...
bool obs_black_list(struct binder_transaction *t)
{
	...
	for (i = 0; i < ARRAY_SIZE(obs_blacklist); i++) {
		if (strstr(t->from->proc->tsk->comm, obs_blacklist[i])) {
			pr_debug("%s is obs blacklist, don't limit it!!!",
				 obs_blacklist[i]);
			return true;
		}
	}
...
bool obwork_is_restrict(struct binder_transaction *t)
{
	...
	if (obs_black_list(t))
		return false;
	return (obtrans_is_from_background(t)) &&
	       obtrans_is_from_third_party(t) && (!obtrans_is_from_main(t)) &&
	       (!test_task_ux(t->from->task));
}
				   ...
				   struct binder_transaction *t, bool oneway)
{
	struct binder_thread *thread = NULL;
	if (sysctl_ob_control_enable && (proc == ob_target.ob_proc) &&
	    obwork_is_restrict(t) && !oneway) {
		list_for_each_entry (thread, &proc->waiting_threads,
				     waiting_thread_node)
			if (thread &&
			    (thread->looper & BINDER_LOOPER_STATE_BACKGROUND)) {
				list_del_init(&thread->waiting_thread_node);
				pr_info("%s :bg thread->name:%s thread->pid:%d",
					__func__, thread->task->comm,
					thread->task->pid);
				return thread;
			}
		return NULL;
...
int sysctl_ob_control_handler(struct ctl_table *table, int write,
				    ...
				    struct binder_thread *thread)
{
	...
	if (NULL != node && NULL != proc->tsk) {
		...
		strncpy(binder_notify_obj.service_name, node->service_name,
			OPLUS_MAX_SERVICE_NAME_LEN);
		binder_notify_obj.service_name[OPLUS_MAX_SERVICE_NAME_LEN - 1] =
			'\0';
	...
	if (thread) {
		...
		if (NULL != thread && NULL != thread->task) {
			...
			call_binderevent_notifiers(0,
						   (void *)&binder_notify_obj);
		}
#endif
#ifdef OPLUS_FEATURE_SCHED_ASSIST
...
#ifdef CONFIG_OPLUS_FEATURE_INPUT_BOOST_V4
		if (t->from) {
			binder_thread_set_fbg(thread->task, t->from->task,
					      oneway);
		}
#endif /* CONFIG_OPLUS_FEATURE_INPUT_BOOST_V4 */
	...
	if (!pending_async) {
		...
		if (thread && thread->task) {
			...
			if (grp_leader && is_sf(curr) &&
			    test_task_ux(thread->task->group_leader) &&
			    oneway) {
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (reply) {
		...
		if (!(tr->flags & TF_ONE_WAY) //report sync binder call
			...
			hans_report(SYNC_BINDER, task_tgid_nr(proc->tsk),
				    task_uid(proc->tsk).val,
				    task_tgid_nr(target_proc->tsk),
		...
		if (security_binder_transaction(proc->cred, target_proc->cred) <
		    0) {
			return_error = BR_FAILED_REPLY;
	...
	if ((tr->flags & TF_ONE_WAY) //report async binder call
		...
		if (!copy_from_user(buf_data, (char *)tr->data.ptr.buffer,
			...
			hans_report(ASYNC_BINDER, task_tgid_nr(proc->tsk),
				    task_uid(proc->tsk).val,
				    task_tgid_nr(target_proc->tsk),
				    task_uid(target_proc->tsk).val, buf,
				    tr->code);
		}
	}
#endif /*OPLUS_FEATURE_HANS_FREEZE*/
...
#endif /* OPLUS_FEATURE_SCHED_ASSIST */
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_CPU_JANKINFO)
		android_vh_binder_wait_for_work_hanlder(NULL, do_proc_work,
							thread, proc);
#endif
		binder_inner_proc_unlock(proc);
...
#ifdef OPLUS_FEATURE_SCHED_ASSIST
			binder_transaction_priority(thread, current, t,
						    node_prio,
...
#ifdef OPLUS_FEATURE_SCHED_ASSIST
			if (sysctl_sched_assist_enabled) {
				binder_set_inherit_ux(thread->task,
						      t_from->task);
			}
#endif /* OPLUS_FEATURE_SCHED_ASSIST */
		} else {
				    ...
				    struct flat_binder_object *fbo)
{
	...
	if (NULL != context->binder_context_mgr_node &&
		...
		snprintf(context->binder_context_mgr_node->service_name,
			 OPLUS_MAX_SERVICE_NAME_LEN, "%s",
			 context->binder_context_mgr_node->proc->tsk->comm);
		context->binder_context_mgr_node
			->service_name[OPLUS_MAX_SERVICE_NAME_LEN - 1] = '\0';
	}
#endif
				       ...
				       enum message_type type)
{
	...
	for (n = rb_first(&proc->threads); n != NULL; n = rb_next(n)) {
		...
		if (thread->task != NULL) {
			...
			if (!empty) {
				binder_inner_proc_unlock(proc);
				hans_report(type, -1, -1, -1, uid,
					    "FROZEN_TRANS_THREAD", 1);
				return;
			}
