...
static int pm8008_masked_write(struct regmap *regmap, u16 reg, u8 mask, u8 val)
{
	int rc;
...
static int pm8008_regulator_get_voltage(struct regulator_dev *rdev)
{
	...
	rc = pm8008_read(pm8008_reg->regmap, LDO_VSET_LB_REG(pm8008_reg->base),
			 vset_raw, 2);
	if (rc < 0) {
...
static int pm8008_regulator_is_enabled(struct regulator_dev *rdev)
{
	...
	rc = pm8008_read(pm8008_reg->regmap, LDO_ENABLE_REG(pm8008_reg->base),
			 &reg, 1);
	if (rc < 0) {
...
static int pm8008_regulator_enable(struct regulator_dev *rdev)
{
	...
	if (rc < 0) {
		pm8008_err(pm8008_reg, "failed to enable en_supply rc=%d\n",
			   rc);
		return rc;
	}
	...
	if (pm8008_reg->parent_supply) {
		rc = regulator_set_voltage(
			pm8008_reg->parent_supply,
			current_uv + pm8008_reg->min_dropout_uv, INT_MAX);
		if (rc < 0) {
			pm8008_err(
				pm8008_reg,
	...
	rc = pm8008_masked_write(pm8008_reg->regmap,
				 LDO_ENABLE_REG(pm8008_reg->base), ENABLE_BIT,
				 ENABLE_BIT);
	if (rc < 0) {
		pm8008_err(pm8008_reg, "failed to enable regulator rc=%d\n",
			   rc);
		goto remove_vote;
	}
...
remove_vote:
	if (pm8008_reg->parent_supply) {
		...
		if (rc2 < 0)
			pm8008_err(pm8008_reg,
				   "failed to disable parent supply rc=%d\n",
				   rc2);
		...
		if (rc2 < 0)
			pm8008_err(
				pm8008_reg,
				"failed to remove voltage vote for parent supply rc=%d\n",
				rc2);
	}
...
static int pm8008_regulator_disable(struct regulator_dev *rdev)
{
	...
	rc = pm8008_masked_write(pm8008_reg->regmap,
				 ...
				 0);
	if (rc < 0) {
		pm8008_err(pm8008_reg, "failed to disable regulator rc=%d\n",
			   rc);
		return rc;
	}
	...
	if (pm8008_reg->parent_supply) {
		...
		rc = regulator_set_voltage(pm8008_reg->parent_supply, 0,
					   INT_MAX);
		if (rc < 0) {
			pm8008_err(pm8008_reg,
...
static int pm8008_regulator_set_voltage(struct regulator_dev *rdev, int min_uv,
					int max_uv, unsigned int *selector)
{
	...
	if (pm8008_reg->parent_supply && enabled && rounded_uv >= current_uv) {
		...
		rc = regulator_set_voltage(
			pm8008_reg->parent_supply,
			rounded_uv + pm8008_reg->min_dropout_uv, INT_MAX);
		if (rc < 0) {
			pm8008_err(
				pm8008_reg,
				"failed to request parent supply voltage rc=%d\n",
...
static unsigned int pm8008_regulator_get_mode(struct regulator_dev *rdev)
{
	...
	rc = pm8008_read(pm8008_reg->regmap, LDO_STATUS1_REG(pm8008_reg->base),
			 &reg, 1);
	if (rc < 0) {
...
static int pm8008_ldo_cb(struct notifier_block *nb, ulong event, void *data)
{
	struct pm8008_regulator *pm8008_reg =
		container_of(nb, struct pm8008_regulator, nb);
	u8 val;
	int rc;
			       ...
			       const char *name)
{
	...
	reg_data = pm8008_reg->pmic_subtype == PM8008_SUBTYPE ?
			   pm8008_reg_data :
			   pm8010_reg_data;
	...
	init_data =
		of_get_regulator_init_data(dev, reg_node, &pm8008_reg->rdesc);
	if (init_data == NULL) {
