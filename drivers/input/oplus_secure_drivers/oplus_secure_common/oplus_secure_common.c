...
static int secure_common_parse_parent_dts(struct secure_data *secure_data)
{
	...
	ret = of_property_read_u32(np, SEC_REG_NODE,
				   &(secure_data->sec_reg_num));
	if (ret) {
	...
	ret = of_property_read_u32(np, SEC_ENABLE_ANTIROLLBACK_REG,
				   &(secure_data->sec_en_anti_reg));
	if (ret) {
		dev_err(secure_data->dev, "the param %s is not found !\n",
	...
	ret = of_property_read_u32(np, SEC_OVERRIDE_1_REG,
				   &(secure_data->sec_override1_reg));
	if (ret) {
		dev_err(secure_data->dev, "the param %s is not found !\n",
			SEC_OVERRIDE_1_REG);
		ret = -SECURE_ERROR_GENERAL;
	...
	ret = of_property_read_u32(np, OVERRIDE_1_ENABLED_VALUE,
				   &(secure_data->override1_en_value));
	if (ret) {
		dev_err(secure_data->dev, "the param %s is not found !\n",
	...
	ret2 = of_property_read_u32(np, CRYPTOKEY_UNSUPPORT_STATUS,
				    &oem_cryptokey_unsupport);
	if (ret2) {
		dev_err(secure_data->dev, "the param %s is not found !\n",
	...
	dev_info(
		secure_data->dev,
		"sec_reg_num: %d sec_en_anti_reg: %d sec_override1_reg: %d override1_en_value: %d\n",
		secure_data->sec_reg_num, secure_data->sec_en_anti_reg,
		secure_data->sec_override1_reg,
		secure_data->override1_en_value);
...
#else
	return strstr(saved_command_line, "androidboot.sbootstate=on") ? true :
									 false;
#endif
}
#endif
				       ...
				       size_t count, loff_t *off)
{
	...
	if (oem_sec_override1_reg == 0x7860C4) {
		...
		if (get_secureType() == SECURE_BOOT_ON_STAGE_2 &&
		    secure_override1_config != oem_override1_en_value) {
			secureSNBound_state =
...
static int secure_register_proc_fs(struct secure_data *secure_data)
{
	...
	oplus_secure_common_dir =
		proc_mkdir(oplus_secure_common_dir_name, NULL);
	if (!oplus_secure_common_dir) {
		dev_err(secure_data->dev,
			"can't create oplus_secure_common_dir proc\n");
		return SECURE_ERROR_GENERAL;
	}

	/*  make the proc /proc/oplus_secure_common/secureType  */
	pentry = proc_create("secureType", 0664, oplus_secure_common_dir,
	...
	pentry = proc_create("secureSNBound", 0444, oplus_secure_common_dir,
	...
	pentry =
		proc_create("CryptoKeyUnsupport", 0444, oplus_secure_common_dir,
			    &CryptoKeyUnsupport_proc_fops);
	if (!pentry) {
		dev_err(secure_data->dev,
			"create CryptoKeyUnsupport proc failed.\n");
		return SECURE_ERROR_GENERAL;
	}

#ifdef QCOM_QSEELOG_ENCRYPT
	/*  make the proc /proc/oplus_secure_common/oemLogEncrpt  */
	pentry = proc_create("oemLogEncrypt", 0444, oplus_secure_common_dir,
			     &oemLogEncrypt_proc_fops);
	if (!pentry) {
		dev_err(secure_data->dev,
			"create oemLogEncrypt proc failed.\n");
		return SECURE_ERROR_GENERAL;
	}
#endif //QCOM_QSEELOG_ENCRYPT
	...
	if (g_rpmb_enabled != -1) {
		...
		pentry = proc_create("rpmbEnableStatus", 0444,
				     oplus_secure_common_dir,
				     &rpmbEnableStatus_proc_fops);
		if (!pentry) {
			dev_err(secure_data->dev,
				"create rpmbEnableStatus proc failed.\n");
			return SECURE_ERROR_GENERAL;
		}
	}
#endif
