...
static int dma_alloc(struct core_i2c_data *i2c)
{
	if (i2c->client != NULL) {
		...
		ilitek_dma_va =
			(u8 *)dma_alloc_coherent(&i2c->client->dev,
						 DMA_VA_BUFFER, &ilitek_dma_pa,
						 GFP_KERNEL);
...
int core_i2c_write(uint8_t nSlaveId, uint8_t *pBuf, uint16_t nSize)
{
	...
	if (nSize > 8) {
		...
		msgs[0].ext_flag = (core_i2c->client->ext_flag |
	...
	if (protocol->major >= 5 && protocol->mid >= 4) {
		if (!core_config->icemodeenable && pBuf[0] == 0xF1 &&
			...
			txbuf = (uint8_t *)kcalloc(nSize + 1, sizeof(uint8_t),
						   GFP_KERNEL);
...
int core_i2c_segmental_read(uint8_t nSlaveId, uint8_t *pBuf, uint16_t nSize)
{
	...
	struct i2c_msg msgs[] = {
		{
			.addr = nSlaveId, .flags = I2C_M_RD, .len = nSize,
			/* .buf = pBuf, */
		},
	};
...
int core_Rx_check(uint16_t check)
{
	...
	for (i = 0; i < count; i++) {
		...
		txbuf[3] = 0x0;
		txbuf[4] = 0x2;
		if (ilitek_core_spi_write_then_read(core_spi->spi, txbuf, 5,
						    txbuf, 0) < 0) {
			size = -EIO;
		...
		txbuf[0] = SPI_READ;
		if (ilitek_core_spi_write_then_read(core_spi->spi, txbuf, 1,
						    rxbuf, 4) < 0) {
			size = -EIO;
			TPD_INFO("spi Write Error, res = %d\n", size);
			return size;
...
int core_ice_mode_read_9881H11(uint8_t *data, uint32_t size)
{
	...
	txbuf[3] = 0x0;
	txbuf[4] = 0x2;
	if (ilitek_core_spi_write_then_read(core_spi->spi, txbuf, 5, txbuf, 0) <
	    0) {
		res = -EIO;
		TPD_INFO("spi Write Error, res = %d\n", res);
		return res;
	...
	//read data
	txbuf[0] = SPI_READ;
	if (ilitek_core_spi_write_then_read(core_spi->spi, txbuf, 1, data,
					    size) < 0) {
		res = -EIO;
		TPD_INFO("spi Write Error, res = %d\n", res);
		return res;
	...
	txbuf[7] = (char)0x98;
	txbuf[8] = (char)0x81;
	if (ilitek_core_spi_write_then_read(core_spi->spi, txbuf, 9, txbuf, 0) <
	    0) {
		res = -EIO;
		TPD_INFO("spi Write Error, res = %d\n", res);
	}
...
int core_ice_mode_write_9881H11(uint8_t *data, uint32_t size)
{
	...
	if (ilitek_core_spi_write_then_read(core_spi->spi, txbuf, wsize + 5,
					    txbuf, 0) < 0) {
		res = -EIO;
		TPD_INFO("spi Write Error, res = %d\n", res);
		goto out;
...
int core_ice_mode_enable_9881H11(void)
{
	...
	txbuf[3] = 0x10;
	txbuf[4] = 0x18;
	if (ilitek_core_spi_write_then_read(core_spi->spi, txbuf, 1, rxbuf, 1) <
	    0) {
		res = -EIO;
		TPD_INFO("spi Write Error, res = %d\n", res);
		return res;
	...
	if (ilitek_core_spi_write_then_read(core_spi->spi, txbuf, 5, rxbuf, 0) <
	    0) {
		res = -EIO;
		TPD_INFO("spi Write Error, res = %d\n", res);
	}
...
int core_spi_write(uint8_t *pBuf, uint16_t nSize)
{
	...
	if (ilitek_core_spi_write_then_read(core_spi->spi, txbuf, nSize + 1,
					    txbuf, 0) < 0) {
		if (core_config->do_ic_reset) {
...
int core_spi_read(uint8_t *pBuf, uint16_t nSize)
{
	...
	if (ilitek_core_spi_write_then_read(core_spi->spi, txbuf, 1, pBuf,
					    nSize) < 0) {
		if (core_config->do_ic_reset) {
			/* ignore spi error if doing ic reset */
			res = 0;
