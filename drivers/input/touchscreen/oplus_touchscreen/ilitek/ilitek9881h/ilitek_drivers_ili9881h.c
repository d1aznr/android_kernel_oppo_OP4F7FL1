	...
	if (ipd->reset_gpio) {
		if (isEnable) {
			...
			for (i = 0; i < 3; i++) {
				...
				ret = core_firmware_upgrade(UPDATE_FW_PATH,
							    true);
				if (ret >= 0) {
					break;
				} else {
				...
				int is_suspended)
{
	...
	g_mutual_data.points = (struct point_info *)kcalloc(
		10, sizeof(struct point_info), GFP_KERNEL);
	if (ERR_ALLOC_MEM(g_mutual_data.points)) {
		TPD_INFO(
			"Failed to allocate g_mutual_data.points memory, %ld\n",
