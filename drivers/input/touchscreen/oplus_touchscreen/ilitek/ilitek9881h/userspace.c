					     ...
					     loff_t *pPos)
{
	...
	if (ipd->debug_node_open) {
		if (ERR_ALLOC_MEM(ipd->debug_buf)) {
			ipd->debug_buf = (unsigned char **)kmalloc(
				1024 * sizeof(unsigned char *), GFP_KERNEL);
			if (!ERR_ALLOC_MEM(ipd->debug_buf)) {
				for (i = 0; i < 1024; i++) {
					ipd->debug_buf[i] = (char *)kmalloc(
