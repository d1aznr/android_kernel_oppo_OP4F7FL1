...
static void i2cuart_recv_packet(void)
{
	...
	if (need_read_len > actual_len) {
		...
		res = core_read(core_config->slave_i2c_addr, g_fr_uart->data,
				g_fr_uart->len);
		if (res < 0)
...
void core_fr_touch_press(int32_t x, int32_t y, uint32_t pressure, int32_t id)
{
	...
	if (core_fr->isEnablePressure)
		input_report_abs(core_fr->input_device, ABS_MT_PRESSURE,
				 pressure);
#else
...
#ifdef MT_B_TYPE
	input_mt_slot(core_fr->input_device, id);
	input_mt_report_slot_state(core_fr->input_device, MT_TOOL_FINGER,
				   false);
#else
...
static int parse_touch_package_v5_0(uint8_t pid)
{
	...
	if (pid == protocol->demo_pid) {
		...
		for (i = 0; i < MAX_TOUCH_NUM; i++) {
			...
			if (g_mutual_data.mtp[g_mutual_data.touch_num]
				    .pressure == 0) {
				TPD_INFO("pressure = 0 force set 1\n");
				g_mutual_data.mtp[g_mutual_data.touch_num]
					.pressure = 1;
			}
...
static int finger_report_ver_5_0(void)
{
...
#else
#ifdef I2C_SEGMENT
	res = core_i2c_segmental_read(core_config->slave_i2c_addr,
				      g_fr_node->data, g_fr_node->len);
#else
	res = core_read(core_config->slave_i2c_addr, g_fr_node->data,
			g_fr_node->len);
#endif
#endif
	if (res < 0) {
	...
	if (pid == protocol->ges_pid && core_config->isEnableGesture) {
		...
		memcpy(g_mutual_data.gesture_data, g_fr_node->data,
		       (g_total_len > 170 ? 170 : g_total_len));
		goto out;
	}
...
void core_fr_input_set_param(struct input_dev *input_device)
{
	...
	TPD_INFO(
		"input resolution : max_x = %d, max_y = %d, min_x = %d, min_y = %d\n",
		max_x, max_y, min_x, min_y);
	TPD_INFO("input touch number: max_tp = %d\n", max_tp);

#if (TP_PLATFORM != PT_MTK)
	input_set_abs_params(core_fr->input_device, ABS_MT_POSITION_X, min_x,
			     max_x - 1, 0, 0);
	input_set_abs_params(core_fr->input_device, ABS_MT_POSITION_Y, min_y,
			     max_y - 1, 0, 0);

	input_set_abs_params(core_fr->input_device, ABS_MT_TOUCH_MAJOR, 0, 255,
			     0, 0);
	input_set_abs_params(core_fr->input_device, ABS_MT_WIDTH_MAJOR, 0, 255,
			     0, 0);
#endif /* PT_MTK */
	TPD_INFO("\n");
	if (core_fr->isEnablePressure)
		input_set_abs_params(core_fr->input_device, ABS_MT_PRESSURE, 0,
				     255, 0, 0);
...
#else
	input_set_abs_params(core_fr->input_device, ABS_MT_TRACKING_ID, 0,
			     max_tp, 0, 0);
#endif /* MT_B_TYPE */
	TPD_INFO("\n");
