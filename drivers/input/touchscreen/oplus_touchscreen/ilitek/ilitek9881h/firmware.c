...
__attribute__((weak)) void set_tp_fw_done(void)
{
	printk("set_tp_fw_done() in tp\n");
}
...
static uint32_t calc_crc32(uint32_t start_addr, uint32_t end_addr,
			   uint8_t *data)
{
	int i = 0;
	int j = 0;
...
static uint32_t tddi_check_data(uint32_t start_addr, uint32_t end_addr)
{
	...
	if (write_len > core_firmware->max_count) {
		TPD_INFO(
			"The length (%x) written to firmware is greater than max count (%x)\n",
	...
	if (timer >= 0) {
		...
		iram_check = core_firmware->isCRC ?
				     core_config_ice_mode_read(0x4101C) :
...
static int do_program_flash(uint32_t start_addr)
{
	...
	if (core_write(core_config->slave_i2c_addr, buf,
		       flashtab->program_page + 4) < 0) {
		TPD_INFO(
			"Failed to write data at start_addr = 0x%X, k = 0x%X, addr = 0x%x\n",
	...
	core_firmware->update_status =
		(start_addr * 101) / core_firmware->end_addr;
...
static int flash_program_sector(void)
{
	...
	for (i = 0; i < g_section_len + 1; i++) {
		...
		for (j = g_flash_sector[i].ss_addr;
		     j < g_flash_sector[i].se_addr;
...
static int iram_upgrade(void)
{
	...
	TPD_DEBUG(
		...
		core_firmware->checksum);

	/* write hex to the addr of iram */
	TPD_INFO("Writing data into IRAM ...\n");
	for (i = core_firmware->start_addr; i < core_firmware->end_addr;
	     i += upl) {
		...
		if (core_write(core_config->slave_i2c_addr, buf, upl + 4)) {
			TPD_INFO(
				"Failed to write data via i2c, address = 0x%X, start_addr = 0x%X, end_addr = 0x%X\n",
				(int)i, (int)core_firmware->start_addr,
				(int)core_firmware->end_addr);
			res = -EIO;
			return res;
		}

		core_firmware->update_status =
			(i * 101) / core_firmware->end_addr;
		printk("%cupgrade firmware(ap code), %02d%c", 0x0D,
		       core_firmware->update_status, '%');
...
int check_hex_crc(void)
{
	...
	int addr_len = 0;
	addr_len = g_flash_block_info[0].end_addr -
		   g_flash_block_info[0].start_addr;
	if ((addr_len >= 4) && (addr_len < MAX_AP_FIRMWARE_SIZE)) {
		ap_crc = calc_crc32(0, addr_len - 3, ap_fw);
		hex_ap_crc = ((ap_fw[addr_len - 3] << 24) +
			      (ap_fw[addr_len - 2] << 16) +
	...
	addr_len = g_flash_block_info[1].end_addr -
		   g_flash_block_info[1].start_addr;
	if ((addr_len >= 4) && (addr_len < MAX_DLM_FIRMWARE_SIZE)) {
		dlm_crc = calc_crc32(0, addr_len - 3, dlm_fw);
		hex_dlm_crc = ((dlm_fw[addr_len - 3] << 24) +
			       (dlm_fw[addr_len - 2] << 16) +
	...
	addr_len = g_flash_block_info[2].end_addr -
		   g_flash_block_info[2].start_addr;
	if ((addr_len >= 4) && (addr_len < MAX_MP_FIRMWARE_SIZE)) {
		mp_crc = calc_crc32(0, addr_len - 3, mp_fw);
		hex_mp_crc = ((mp_fw[addr_len - 3] << 24) +
			      (mp_fw[addr_len - 2] << 16) +
	...
	addr_len = g_flash_block_info[3].end_addr -
		   g_flash_block_info[3].start_addr;
	if ((addr_len >= 4) && (addr_len < MAX_GESTURE_FIRMWARE_SIZE)) {
		gesture_crc = calc_crc32(0, addr_len - 3, gesture_fw);
		hex_gesture_crc = ((gesture_fw[addr_len - 3] << 24) +
				   (gesture_fw[addr_len - 2] << 16) +
				   (gesture_fw[addr_len - 1] << 8) +
				   gesture_fw[addr_len]);
	} else {
		check_fail = true;
	...
	if ((check_fail) || (ap_crc != hex_ap_crc) || (mp_crc != hex_mp_crc) ||
		...
		TPD_INFO("mp block, driver crc = 0x%x, hex mp crc = 0x%x\n",
			 mp_crc, hex_mp_crc);
		TPD_INFO(
			"gesture block, driver crc = 0x%x, hex gesture crc = 0x%x\n",
			gesture_crc, hex_gesture_crc);
		return -1;
	} else {
...
int write_download(uint32_t start, uint32_t size, uint8_t *w_buf,
		   uint32_t w_len)
{
	int res = 0;
	int addr = 0;
	...
	for (addr = start, i = 0; addr < end; addr += w_len, i += w_len) {
		...
		if (core_write(core_config->slave_i2c_addr, buf, w_len + 4)) {
			TPD_INFO(
				"Failed to write data via SPI, address = 0x%X, start_addr = 0x%X, end_addr = 0x%X\n",
				(int)addr, 0, end);
			res = -EIO;
			goto write_error;
		}
...
int host_download(bool mode)
{
	...
	if (core_fr->actual_fw_mode == P5_0_FIRMWARE_TEST_MODE) {
		...
		if (write_download(0, MAX_MP_FIRMWARE_SIZE, mp_fw,
				   SPI_UPGRADE_LEN) < 0) {
			TPD_INFO("SPI Write MP code data error\n");
		}
		if (read_download(0, MAX_MP_FIRMWARE_SIZE, read_mp_buf,
				  SPI_UPGRADE_LEN)) {
		...
		if (mode) {
			...
			if (write_download(core_gesture->ap_start_addr,
					   core_gesture->length, gesture_fw,
					   core_gesture->length) < 0) {
				TPD_INFO("SPI Write Gesture code data error\n");
			}
			if (read_download(core_gesture->ap_start_addr,
					  core_gesture->length,
					  read_gesture_buf,
					  core_gesture->length)) {
				TPD_INFO("SPI Read Gesture code data error\n");
			}
			if (memcmp(gesture_fw, read_gesture_buf,
				   core_gesture->length) == 0) {
			...
			memcpy(gesture_ap_buf,
			       ap_fw + core_gesture->ap_start_addr,
			       core_gesture->ap_length);
			//TPD_INFO("Writing data into AP code ...\n");
			if (write_download(core_gesture->ap_start_addr,
					   core_gesture->ap_length,
					   gesture_ap_buf,
					   core_gesture->ap_length) < 0) {
				TPD_INFO("SPI Write AP code data error\n");
			}
			if (read_download(core_gesture->ap_start_addr,
					  core_gesture->ap_length, read_ap_buf,
					  core_gesture->ap_length)) {
				TPD_INFO("SPI Read AP code data error\n");
			}
			if (memcmp(gesture_ap_buf, read_ap_buf,
				   core_gesture->ap_length) == 0) {
		...
		/* write hex to the addr of AP code */
		//TPD_INFO("Writing data into AP code ...\n");
		if (write_download(0, MAX_AP_FIRMWARE_SIZE, ap_fw,
				   SPI_UPGRADE_LEN) < 0) {
			TPD_INFO("SPI Write AP code data error\n");
		}
		/* write hex to the addr of DLM code */
		//TPD_INFO("Writing data into DLM code ...\n");
		if (write_download(DLM_START_ADDRESS, MAX_DLM_FIRMWARE_SIZE,
				   dlm_fw, SPI_UPGRADE_LEN) < 0) {
		...
		if (method >= CORE_TYPE_E) {
			...
			TPD_INFO("AP CRC %s (%x) : (%x)\n",
				 (ap_crc != ap_dma ? "Invalid !" : "Correct !"),
				 ap_crc, ap_dma);

			TPD_INFO("DLM CRC %s (%x) : (%x)\n",
				 (dlm_crc != dlm_dma ? "Invalid !" :
						       "Correct !"),
				 dlm_crc, dlm_dma);
			...
			if (CHECK_EQUAL(ap_crc, ap_dma) == UPDATE_FAIL ||
				...
				TPD_INFO(
					"Check AP/DLM Mode upgrade: FAIL read data check\n");
				res = UPDATE_FAIL;
				read_download(0, MAX_AP_FIRMWARE_SIZE,
					      read_ap_buf, SPI_UPGRADE_LEN);
				read_download(DLM_START_ADDRESS,
					      MAX_DLM_FIRMWARE_SIZE,
					      read_dlm_buf, SPI_UPGRADE_LEN);

				if (memcmp(ap_fw, read_ap_buf,
					   MAX_AP_FIRMWARE_SIZE) != 0 ||
				    memcmp(dlm_fw, read_dlm_buf,
					   MAX_DLM_FIRMWARE_SIZE) != 0) {
					TPD_INFO(
						"Check AP/DLM Mode upgrade: FAIL\n");
					res = UPDATE_FAIL;
					goto out;
				} else {
					TPD_INFO(
						"Check AP/DLM Mode upgrade: SUCCESS\n");
					res = 0;
				}
				//goto out;
			}
		} else {
			read_download(0, MAX_AP_FIRMWARE_SIZE, read_ap_buf,
				      SPI_UPGRADE_LEN);
			read_download(DLM_START_ADDRESS, MAX_DLM_FIRMWARE_SIZE,
				      read_dlm_buf, SPI_UPGRADE_LEN);

			if (memcmp(ap_fw, read_ap_buf, MAX_AP_FIRMWARE_SIZE) !=
				    0 ||
			    memcmp(dlm_fw, read_dlm_buf,
				   MAX_DLM_FIRMWARE_SIZE) != 0) {
...
int core_load_gesture_code(void)
{
	...
	core_firmware->core_version = (ap_fw[0xFFF4] << 24) +
				      (ap_fw[0xFFF5] << 16) +
				      (ap_fw[0xFFF6] << 8) + ap_fw[0xFFF7];
	...
	if (core_firmware->core_version >= 0x01000600) {
		core_gesture->area_section =
			(ap_fw[0xFFC4 + 3] << 24) + (ap_fw[0xFFC4 + 2] << 16) +
			(ap_fw[0xFFC4 + 1] << 8) + ap_fw[0xFFC4];
		core_gesture->ap_start_addr =
			(ap_fw[0xFFC4 + 7] << 24) + (ap_fw[0xFFC4 + 6] << 16) +
			(ap_fw[0xFFC4 + 5] << 8) + ap_fw[0xFFC4 + 4];
		core_gesture->start_addr = (ap_fw[0xFFC4 + 15] << 24) +
					   (ap_fw[0xFFC4 + 14] << 16) +
					   (ap_fw[0xFFC4 + 13] << 8) +
					   ap_fw[0xFFC4 + 12];
	} else {
		...
		core_gesture->start_addr = (ap_fw[0xFFDB] << 24) +
					   (ap_fw[0xFFDA] << 16) +
	...
	TPD_DEBUG(
		"area = %d, ap_start_addr = 0x%x, core_gesture->ap_length = 0x%x\n",
...
int core_load_ap_code(void)
{
	...
	ap_end_addr =
		(temp[8] << 24) + (temp[9] << 16) + (temp[10] << 8) + temp[11];
	gesture_start_addr = (temp[12] << 24) + (temp[13] << 16) +
			     (temp[14] << 8) + temp[15];
	gesture_end_addr = (temp[16] << 24) + (temp[17] << 16) +
			   (temp[18] << 8) + temp[19];
...
static int convert_hex_array(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(core_firmware->old_fw_ver); i++) {
		if (core_firmware->old_fw_ver[i] !=
		    core_firmware->new_fw_ver[i]) {
			TPD_INFO(
				"FW version is different, preparing to upgrade FW\n");
			break;
		}
	}
	TPD_INFO("hw fw: %d.%d.%d.%d", core_firmware->old_fw_ver[0],
		 core_firmware->old_fw_ver[1], core_firmware->old_fw_ver[2],
		 core_firmware->old_fw_ver[3]);
	TPD_INFO("hex fw: %d.%d.%d.%d", core_firmware->new_fw_ver[0],
		 core_firmware->new_fw_ver[1], core_firmware->new_fw_ver[2],
	...
	if (block > 0) {
		...
		for (i = 0; i < block; i++) {
			...
			g_flash_block_info[i].hex_crc =
				(CTPM_FW[64 + end_addr - 3] << 24) +
				(CTPM_FW[64 + end_addr - 2] << 16) +
				(CTPM_FW[64 + end_addr - 1] << 8) +
				CTPM_FW[64 + (end_addr)];
			g_flash_block_info[i].block_crc = tddi_check_data(
				start_addr,
				end_addr - start_addr - crc_byte_len + 1);
			TPD_INFO(
				"block=%d, start_addr=0x%06x, end_addr=0x%06x, H_CRC=0x%06x, B_CRC=0x%06x\n",
				i, start_addr, end_addr,
				g_flash_block_info[i].hex_crc,
				g_flash_block_info[i].block_crc);
			bindex += blen;
			if (g_flash_block_info[i].hex_crc !=
			    g_flash_block_info[i].block_crc) {
	...
	for (i = 0; i < ARRAY_SIZE(CTPM_FW) - 64; i++) {
		...
		if (!g_flash_sector[index].data_flag) {
			g_flash_sector[index].ss_addr =
				index * flashtab->sector;
	...
	if (g_flash_sector[g_section_len].se_addr > flashtab->mem_size) {
		TPD_INFO(
			"The size written to flash is larger than it required (%x) (%x)\n",
			g_flash_sector[g_section_len].se_addr,
			flashtab->mem_size);
		goto out;
	}
	...
	for (i = 0; i < g_total_sector; i++) {
		...
		if (g_flash_sector[i].ss_addr == 0x0 &&
		    g_flash_sector[i].se_addr == 0x0) {
			g_flash_sector[i].ss_addr = tmp_addr;
			g_flash_sector[i].se_addr =
				(i + 1) * flashtab->sector - 1;
		}
		...
		if (core_firmware->hasBlockInfo) {
			for (j = 0; j < ARRAY_SIZE(g_flash_block_info); j++) {
				if (g_flash_sector[i].ss_addr >=
					    g_flash_block_info[j].start_addr &&
				    g_flash_sector[i].se_addr <=
					    g_flash_block_info[j].end_addr) {
		 ...
		 */
		if (g_flash_sector[i].ss_addr == g_start_resrv &&
		    g_flash_sector[i].se_addr == g_end_resrv) {
...
int core_firmware_boot_upgrade(void)
{
	...
	g_flash_sector = kcalloc(g_total_sector, sizeof(struct flash_sector),
				 GFP_KERNEL);
...
int core_firmware_get_h_file_data(void)
{
	...
	memcpy(gesture_fw, flash_fw + core_gesture->start_addr,
	       core_gesture->length);
...
int core_firmware_boot_host_download(void)
{
	...
	TPD_INFO(
		"MAX_AP_FIRMWARE_SIZE + MAX_DLM_FIRMWARE_SIZE + MAX_MP_FIRMWARE_SIZE = 0x%X\n",
		MAX_AP_FIRMWARE_SIZE + MAX_DLM_FIRMWARE_SIZE +
			MAX_MP_FIRMWARE_SIZE);
...
static int convert_hex_file(uint8_t *pBuf, uint32_t nSize, bool isIRAM)
{
	...
	for (; i < nSize;) {
		...
		if (nType == 0xAE) {
			...
			if (block < 4) {
				g_flash_block_info[block].start_addr =
					HexToDec(&pBuf[i + 9], 6);
				g_flash_block_info[block].end_addr =
					HexToDec(&pBuf[i + 9 + 6], 6);
				TPD_DEBUG(
					"Block[%d]: start_addr = %x, end = %x\n",
					block,
					g_flash_block_info[block].start_addr,
					g_flash_block_info[block].end_addr);
			}
				...
				if (isIRAM) {
					...
					if ((nAddr + k) == (0xFFF7)) {
						core_firmware->core_version =
							...
							ap_fw[0xFFF7];
						TPD_INFO(
							"core_firmware->core_version = 0x%X\n",
							core_firmware
								->core_version);
						if (core_firmware->core_version >=
						    0x01000600) {
							core_gesture
								->area_section =
								(ap_fw[0xFFC4 +
					...
					if ((nAddr + k) != 0) {
						index = ((nAddr + k) /
							 flashtab->sector);
						if (!g_flash_sector[index]
							     .data_flag) {
							g_flash_sector[index]
								.ss_addr =
								index *
								flashtab->sector;
							g_flash_sector[index]
	...
	if (g_flash_sector[g_section_len - 1].se_addr > flashtab->mem_size) {
		TPD_INFO(
			"The size written to flash is larger than it required (%x) (%x)\n",
			g_flash_sector[g_section_len - 1].se_addr,
			flashtab->mem_size);
		goto out;
	}
...
int core_firmware_get_hostdownload_data(const char *pFilePath)
{
	...
	if (!ERR_ALLOC_MEM(core_firmware->fw) &&
	    (!ERR_ALLOC_MEM(core_firmware->fw->data)) &&
	    (core_firmware->fw->size != 0) && (ipd->common_reset == 1)) {
	...
	hex_buffer =
		vmalloc(fsize); //kcalloc(fsize, sizeof(uint8_t), GFP_KERNEL);
...
convert_hex:
	if (!ERR_ALLOC_MEM(core_firmware->fw) &&
	    (!ERR_ALLOC_MEM(core_firmware->fw->data)) &&
	    (core_firmware->fw->size != 0) && (ipd->common_reset == 1)) {
		res = convert_hex_file((uint8_t *)core_firmware->fw->data,
				       core_firmware->fw->size, true);
	} else {
...
int core_firmware_upgrade(const char *pFilePath, bool isIRAM)
{
	...
	g_flash_sector =
		kcalloc(g_total_sector, sizeof(*g_flash_sector), GFP_KERNEL);
	if (ERR_ALLOC_MEM(g_flash_sector)) {
		TPD_INFO("Failed to allocate g_flash_sector memory, %ld\n",
			 PTR_ERR(g_flash_sector));
		res = -ENOMEM;
	...
	if (core_firmware->core_version < 0x01000600) {
		for (i = 0; i < 14; i++) {
			...
			if (res < 0) {
				TPD_INFO(
					"Failed to read tp set ddi trim code %d\n",
					res);
			}
			if (cmd[0] == 0x55) {
				TPD_INFO(
					"TP set ddi trim code ok read value 0x%X i = %d\n",
					cmd[0], i);
				break;
			} else if (cmd[0] == 0x35) {
				TPD_INFO(
					"TP set ddi trim code bypass read value 0x%X\n",
					cmd[0]);
				break;
			}
			mdelay(5);
...
int core_firmware_init(void)
{
	...
	for (; i < ARRAY_SIZE(ipio_chip_list); i++) {
		if (ipio_chip_list[i] == TP_TOUCH_IC) {
			for (j = 0; j < 4; j++) {
				core_firmware->old_fw_ver[i] =
					core_config->firmware_ver[i];
