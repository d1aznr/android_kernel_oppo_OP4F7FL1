		...
		for (j = 0, index1 = 0;
			...
			if (ilitek_ini_file_data[i].pKeyValue[j] == ';' ||
				...
				if (record != '.') {
					...
					if (count >= frame_len) {
						ipio_err(
							"count(%d) is larger than frame length, break\n",
							count);
						break;
					}
					type_ptr[count] = temp;
				 ...
				 char *desp, int frame_len)
{
	...
	snprintf(benchmark_str, BENCHMARK_STR_SIZE, "%s%s%s", desp, "_",
		 BENCHMARK_KEY_NAME);
	...
	for (i = 0; i < g_ini_items; i++) {
		...
		if (strcmp(keyname, ilitek_ini_file_data[i].pKeyName) == 0) {
			...
			if (rv_len > ilitek_ini_file_data[i].iKeyValueLen) {
				rv_len = ilitek_ini_file_data[i].iKeyValueLen +
					 1;
			}

			ret = snprintf(rv, rv_len, "%s",
...
static void mp_print_csv_header(char *csv, int *csv_len, int *csv_line)
{
	...
	tmp_len += snprintf(
		csv + tmp_len, CSV_FILE_SIZE - tmp_len,
		"==============================================================================\n");

	tmp_line++;
	tmp_len +=
		snprintf(csv + tmp_len, CSV_FILE_SIZE - tmp_len,
	...
	if (strncmp(name, "open test(integration)_sp", strlen(name)) == 0) {
		for (i = 0; i < ARRAY_SIZE(open_sp_cmd); i++) {
			slen = parser_get_int_data("pv5_4 command",
						   open_sp_cmd[i], str, 128);
			if (slen < 0)
				ipio_err(
					"Failed to get CDC command %s from ini\n",
					open_sp_cmd[i]);
			else
				tmp_len += snprintf(csv + tmp_len,
						    CSV_FILE_SIZE - tmp_len,
...
int full_open_rate_compare(s32 *full_open, s32 *cbk, int x, int y,
...
void allnode_open_cdc_result(int index, int *buf, int *dac, int *raw)
{
	...
	if (strncmp(name, "open_integration_sp", strlen(name)) == 0) {
		...
		for (i = 0; i < core_mp.frame_len; i++)
			buf[i] = idev->chip->open_c_formula(
				dac[i], raw[i],
				open_c_spec.tvch - open_c_spec.tvcl,
				open_c_spec.gain);
	}
}
	...
	if (core_mp.protocol_ver >= PROTOCOL_VER_540) {
		...
		for (j = 0; j < core_mp.frame_len; j++)
			tItems[index].buf[frame_index * core_mp.frame_len + j] =
				codeToOhm(frame_buf[j], v_tdf, h_tdf);
	} else {
		for (j = 0; j < core_mp.frame_len; j++)
			tItems[index].buf[frame_index * core_mp.frame_len + j] =
				frame_buf[j];
...
static int allnode_key_cdc_data(int index)
{
	...
	ilitek_dump_data(key_buf, 32, core_mp.frame_len, core_mp.xch_len,
...
static int allnode_mutual_cdc_data(int index)
{
	...
	ilitek_dump_data(frame_buf, 32, core_mp.frame_len, core_mp.xch_len,
			 "Mutual CDC combined");
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		if (core_mp.tx_delta_buf == NULL) {
			...
			if (ERR_ALLOC_MEM(core_mp.tx_delta_buf)) {
				ipio_err(
					"Failed to allocate tx_delta_buf mem\n");
		...
		if (core_mp.rx_delta_buf == NULL) {
			...
			if (ERR_ALLOC_MEM(core_mp.rx_delta_buf)) {
				ipio_err(
					"Failed to allocate rx_delta_buf mem\n");
		...
		if (tItems[index].spec_option == BENCHMARK) {
			if (tItems[index].bench_mark_max == NULL) {
				...
				if (ERR_ALLOC_MEM(
					    tItems[index].bench_mark_max)) {
					ipio_err(
						"Failed to allocate bench_mark_max mem\n");
			...
			if (tItems[index].bench_mark_min == NULL) {
				...
				if (ERR_ALLOC_MEM(
					    tItems[index].bench_mark_min)) {
					ipio_err(
						"Failed to allocate bench_mark_min mem\n");
					ipio_kfree((void **)&tItems[index]
							   .bench_mark_min);
					return -ENOMEM;
				}
...
static int open_test_sp(int index)
{
	...
	TPD_DEBUG(
		"open_test_sp: frame_cont %d, AA %d, Border %d, Notch %d, full_open_rate %d\n",
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		ilitek_dump_data(open[i].cbk_700, 10, core_mp.frame_len,
				 core_mp.xch_len, "cbk 700");
		ilitek_dump_data(open[i].cbk_250, 10, core_mp.frame_len,
				 core_mp.xch_len, "cbk 250");
		ilitek_dump_data(open[i].cbk_200, 10, core_mp.frame_len,
		...
		if (ipio_debug_level) {
			ilitek_dump_data(open[i].charg_rate, 10,
					 core_mp.frame_len, core_mp.xch_len,
					 "origin charge rate");
			ilitek_dump_data(open[i].full_Open, 10,
					 core_mp.frame_len, core_mp.xch_len,
					 "origin full open");
		}
		...
		if (ipio_debug_level)
			ilitek_dump_data(
				&tItems[index].buf[(i * core_mp.frame_len)], 10,
				core_mp.frame_len, core_mp.xch_len,
...
static int open_test_cap(int index)
{
	...
	TPD_DEBUG(
		"open_test_c: frame_cont = %d, gain = %d, tvch = %d, tvcl = %d\n",
		tItems[index].frame_count, open_c_spec.gain, open_c_spec.tvch,
		open_c_spec.tvcl);
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		ilitek_dump_data(open[i].dcl_cap, 10, core_mp.frame_len,
...
static int mp_comp_result_before_retry(int index)
{
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		...
		if (tItems[index].trimmed_mean &&
			...
			mp_compare_cdc_result(index, tItems[index].buf,
					      max_threshold, min_threshold,
					      &test_result);
			mp_compare_cdc_result(index, tItems[index].buf,
					      max_threshold, min_threshold,
...
{
	int ret, i, x, y, j, csv_len = 0, pass_item_count = 0, line_count = 0,
			     get_frame_cont = 1;
	s32 *max_threshold = NULL, *min_threshold = NULL;
	char *csv = NULL;
	char csv_name[128] = { 0 };
	...
	for (i = 0; i < ARRAY_SIZE(tItems); i++) {
		...
		if (tItems[i].spec_option == BENCHMARK) {
			...
			mp_compare_cdc_show_result(i, tItems[i].bench_mark_max,
						   csv, &csv_len,
						   TYPE_BENCHMARK,
						   max_threshold, min_threshold,
						   "Max_Bench");
			mp_compare_cdc_show_result(i, tItems[i].bench_mark_min,
						   csv, &csv_len,
						   TYPE_BENCHMARK,
						   max_threshold, min_threshold,
						   "Min_Bench");
		...
		if (tItems[i].catalog == KEY_TEST) {
			...
			if (tItems[i].trimmed_mean &&
				...
				mp_compare_cdc_show_result(
					i, tItems[i].max_buf, csv, &csv_len,
					TYPE_JUGE, max_threshold, min_threshold,
					"Max Hold");
				mp_compare_cdc_show_result(
					i, tItems[i].min_buf, csv, &csv_len,
					TYPE_JUGE, max_threshold, min_threshold,
					"Min Hold");
			}
	...
	if (core_mp.final_result == MP_FAIL)
		snprintf(csv_name, 128, "%s/%s_%s_%s_%s.csv", csv_path,
			 tp_module, lcm_on ? "lcm_on" : "lcm_off",
			 ret_fail_name, get_date_time_str());
	else
		snprintf(csv_name, 128, "%s/%s_%s_%s_%s.csv", csv_path,
			 tp_module, lcm_on ? "lcm_on" : "lcm_off",
			 ret_pass_name, get_date_time_str());
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		if (strncmp(item, tItems[i].desp, strlen(item)) == 0) {
			...
			if (tItems[i].catalog == SHORT_TEST) {
				...
				tItems[i].v_tdf_1 = parser_get_tdf_value(
					str, tItems[i].catalog);
				parser_get_int_data(item, "v_tdf_2", str, 512);
				tItems[i].v_tdf_2 = parser_get_tdf_value(
					str, tItems[i].catalog);
				parser_get_int_data(item, "h_tdf_1", str, 512);
				tItems[i].h_tdf_1 = parser_get_tdf_value(
					str, tItems[i].catalog);
				parser_get_int_data(item, "h_tdf_2", str, 512);
				tItems[i].h_tdf_2 = parser_get_tdf_value(
					str, tItems[i].catalog);
			} else {
			...
			if (strcmp(item, "tx/rx delta") == 0) {
				...
				TPD_DEBUG(
					...
					tItems[i].desp, core_mp.TxDeltaMax,
					core_mp.TxDeltaMin, core_mp.RxDeltaMax,
					core_mp.RxDeltaMin);
			} else {
			...
			TPD_DEBUG(
				"v_tdf_1 = %d, v_tdf_2 = %d, h_tdf_1 = %d, h_tdf_2 = %d\n",
				tItems[i].v_tdf_1, tItems[i].v_tdf_2,
				tItems[i].h_tdf_1, tItems[i].h_tdf_2);
	...
	if (0 == test_failed_items) {
		if (!ERR_ALLOC_MEM(message)) {
			snprintf(message, MESSAGE_SIZE, "%d errors. %s",
				 test_failed_items, "All test passed.");
		}
		if (!ERR_ALLOC_MEM(s)) {
			seq_printf(s, "%d errors. %s", test_failed_items,
				   "All test passed.");
		}
	}
}

int ilitek_tddi_mp_test_main(char *apk, struct seq_file *s, char *message,
			     bool lcm_on)
{
	int ret = 0;
	...
	if (ERR_ALLOC_MEM(ilitek_ini_file_data)) {
		...
		if (!ERR_ALLOC_MEM(message)) {
			snprintf(message, MESSAGE_SIZE,
				 "malloc ilitek_ini_file_data failed\n");
		}
		if (!ERR_ALLOC_MEM(s)) {
	...
	if (idev->protocol->ver >= PROTOCOL_VER_540) {
		if (lcm_on) {
			...
			mp_test_run(
				"short test -ili9881"); //compatible with old ini version.
