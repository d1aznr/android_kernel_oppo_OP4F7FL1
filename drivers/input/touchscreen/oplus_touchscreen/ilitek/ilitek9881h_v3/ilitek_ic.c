...
static int ilitek_tddi_ic_check_support(u32 pid, u16 id)
{
	...
	if (id == ILI9881_CHIP) {
		...
		idev->chip->hd_dma_check_crc_off =
			firmware_hd_dma_crc_off_ili9881;
		...
		idev->chip->hd_dma_check_crc_off =
			firmware_hd_dma_crc_off_ili7807;
		idev->chip->no_bk_shift = RAWDATA_NO_BK_SHIFT_9881H;
	}
...
int ilitek_ice_mode_ctrl(bool enable, bool mcu)
{
	...
	if (enable) {
		...
		do {
			...
			ret = ilitek_ice_mode_read(idev->chip->pid_addr, &pid,
						   sizeof(u32));
			if (ret < 0) {
				continue;
			}
...
int ilitek_tddi_ic_watch_dog_ctrl(bool write, bool enable)
{
	...
	if (!write) {
		ret = ilitek_ice_mode_read(idev->chip->wdt_addr, &reg_data,
					   sizeof(u8));
		TPD_INFO("Read WDT: %s\n", (reg_data ? "ON" : "OFF"));
		return reg_data;
	}
	...
	if (enable) {
		...
		ilitek_ice_mode_write(idev->chip->wdt_addr,
				      (idev->chip->wtd_key & 0xff), 1);
		ilitek_ice_mode_write(idev->chip->wdt_addr,
				      (idev->chip->wtd_key >> 8), 1);
		/* need to delay 300us after stop mcu to wait fw relaod */
		udelay(300);
	}

	while (timeout > 0) {
		ret = ilitek_ice_mode_read(TDDI_WDT_ACTIVE_ADDR, &reg_data,
					   sizeof(u8));
	...
	if (timeout <= 0) {
		ipio_err(
			"WDT turn on/off timeout !, reg_data = %x, pc = 0x%x\n",
...
int ilitek_tddi_ic_func_ctrl(const char *name, int ctrl)
{
	...
	if (idev->protocol->ver >= PROTOCOL_VER_560) {
		if (strncmp(func_ctrl[i].name, "gesture", strlen("gesture")) ==
			    0 ||
...
int ilitek_tddi_ic_whole_reset(void)
{
	...
	if (ilitek_ice_mode_write(idev->chip->reset_key, idev->chip->reset_addr,
				  sizeof(u32)) < 0) {
...
int ilitek_tddi_ic_get_fw_ver(void)
{
	...
	if (idev->ts->panel_data.manufacture_info.version) {
		...
		if (idev->ts->panel_data.vid_len == 0) {
			...
			strlcpy(&(idev->ts->panel_data.manufacture_info
					  .version[12]),
				dev_version, 3);
		} else {
...
static void ilitek_tddi_ic_check_protocol_ver(u32 pver)
{
	...
	TPD_INFO(
		"Not found a correct protocol version in list, use newest version\n");
...
int ilitek_tddi_ic_get_info(void)
{
	...
	ilitek_ice_mode_read(idev->chip->pid_addr, &(idev->chip->pid),
			     sizeof(u32));
	idev->chip->id = idev->chip->pid >> 16;
	idev->chip->type_hi = idev->chip->pid & 0x0000FF00;
	idev->chip->type_low = idev->chip->pid & 0xFF;

	ilitek_ice_mode_read(idev->chip->otp_addr, &(idev->chip->otp_id),
			     sizeof(u32));
	ilitek_ice_mode_read(idev->chip->ana_addr, &(idev->chip->ana_id),
			     sizeof(u32));
	idev->chip->otp_id &= 0xFF;
	idev->chip->ana_id &= 0xFF;

	TPD_INFO(
		"CHIP INFO: PID = %x, ID = %x, TYPE = %x, OTP = %x, ANA = %x\n",
		idev->chip->pid, idev->chip->id,
		((idev->chip->type_hi << 8) | idev->chip->type_low),
		idev->chip->otp_id, idev->chip->ana_id);
