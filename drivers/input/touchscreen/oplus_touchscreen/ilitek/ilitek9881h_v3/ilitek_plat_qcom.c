...
static int core_spi_setup(u32 freq)
{
	...
	TPD_INFO("name = %s, bus_num = %d,cs = %d, mode = %d, speed = %d\n",
		 idev->spi->modalias, idev->spi->master->bus_num,
		 idev->spi->chip_select, idev->spi->mode,
...
int ilitek_tddi_mp_test_handler(char *apk, struct seq_file *s, char *message,
				bool lcm_on)
{
	int ret = 0;
	...
	if (!idev->chip->open_c_formula || !idev->chip->open_sp_formula) {
...
int ilitek_tddi_switch_mode(u8 *data)
{
	...
	if (mode == P5_X_FW_DEMO_MODE || mode == P5_X_FW_GESTURE_MODE ||
	...
	case P5_X_FW_DEMO_MODE:
		...
		if (ret < 0) {
			...
			if (ret < 0) {
				ipio_err(
					"Failed to switch demo mode, do reset/reload instead\n");
...
static int ilitek_tddi_move_gesture_code_iram(int mode)
{
	...
	if (i >= retry) {
		ipio_err(
			"Gesture is not ready, 0x%x running gesture esd flow\n",
			cmd[0]);
		return -1;
	}
...
static void ilitek_tddi_touch_esd_gesture_iram(void)
{
	...
	if (ilitek_ice_mode_write(SPI_ESD_GESTURE_PWD_ADDR, ESD_GESTURE_PWD,
				  4) < 0)
	...
	do {
		ilitek_ice_mode_read(SPI_ESD_GESTURE_PWD_ADDR, &answer,
				     sizeof(u32));
...
static int ilitek_tddi_sleep_handler(enum TP_SLEEP_STATUS mode)
{
	...
	case TP_SUSPEND:
		...
		if (idev->gesture) {
			ret = ilitek_tddi_move_gesture_code_iram(
				idev->gesture_mode);
			if (ret < 0) {
				atomic_set(&idev->esd_stat, START);
				ilitek_tddi_touch_esd_gesture_iram();
...
static int ilitek_tddi_get_tp_recore_data(u16 *out_buf, u32 out_len)
{
	...
	TPD_INFO(
		"addr = 0x%x, len = %d, lndex = %d, frame_cnt = %d, record_case = 0x%x\n",
	...
	if (frame_cnt > 0 && index >= 0 && index < 3) {
		...
		if (out_buf) {
			...
			if (out_len > 0) {
				memcpy(out_buf, &raw[index * frame_len],
				       out_len);
			}
		}
		for (i = 0; i < frame_cnt; i++) {
			...
			ilitek_dump_data(raw_ptr, 16, frame_len, idev->xch_num,
					 "recore_data");
			index--;
			if (index < 0)
				index = frame_cnt - 1;
...
static void ilitek_tddi_touch_send_debug_data(u8 *buf, int len)
{
	...
	if (idev->debug_node_open && !(ERR_ALLOC_MEM(idev->debug_buf)) &&
		...
		memset(idev->debug_buf[idev->debug_data_frame], 0x00,
		       (u8)sizeof(u8) * DEBUG_DATA_MAX_LENGTH);
		ipio_memcpy(idev->debug_buf[idev->debug_data_frame], buf, len,
...
static void ilitek_tddi_report_ap_mode(u8 *buf, int len)
{
	...
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		...
		TPD_DEBUG("scale x = %d, y = %d, p = %d\n", idev->points[i].x,
			  idev->points[i].y, idev->points[i].z);
	}
...
static int ilitek_tddi_report_handler(void)
{
	...
	case P5_X_DEMO_DEBUG_INFO_PACKET_ID:
		if (rlen < P5_X_DEMO_MODE_PACKET_LENGTH + 2) {
			ipio_err(
				"read demo debug info mode data len < 45 len = %d\n",
				rlen);
			ret = -1;
		} else {
...
void tp_goto_sleep_ftm(void)
{
	...
	if (idev != NULL && idev->ts != NULL) {
		...
		if ((idev->ts->boot_mode == META_BOOT ||
		     idev->ts->boot_mode == FACTORY_BOOT)) {
#else
				   ...
				   struct gesture_info *gesture)
{
	...
	TPD_DEBUG("gesture data 0-17 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X "
		  ...
		  point_data[12], point_data[13], point_data[14],
		  point_data[15], point_data[16], point_data[17]);

	TPD_DEBUG(
		"gesture data 18-35 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X "
		"0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X "
		"0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X\n",
	...
	gesture->Point_start.x =
		gesture->Point_start.x * idev->panel_wid / TPD_WIDTH;
	gesture->Point_start.y =
		gesture->Point_start.y * idev->panel_hei / TPD_HEIGHT;
	gesture->Point_end.x =
		gesture->Point_end.x * idev->panel_wid / TPD_WIDTH;
	gesture->Point_end.y =
		gesture->Point_end.y * idev->panel_hei / TPD_HEIGHT;
	gesture->Point_1st.x =
		gesture->Point_1st.x * idev->panel_wid / TPD_WIDTH;
	gesture->Point_1st.y =
		gesture->Point_1st.y * idev->panel_hei / TPD_HEIGHT;

	gesture->Point_2nd.x =
		gesture->Point_2nd.x * idev->panel_wid / TPD_WIDTH;
	gesture->Point_2nd.y =
		gesture->Point_2nd.y * idev->panel_hei / TPD_HEIGHT;

	gesture->Point_3rd.x =
		gesture->Point_3rd.x * idev->panel_wid / TPD_WIDTH;
	gesture->Point_3rd.y =
		gesture->Point_3rd.y * idev->panel_hei / TPD_HEIGHT;

	gesture->Point_4th.x =
		gesture->Point_4th.x * idev->panel_wid / TPD_WIDTH;
	gesture->Point_4th.y =
		gesture->Point_4th.y * idev->panel_hei / TPD_HEIGHT;
...
static int ilitek_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_GESTURE:
		...
		if (chip_info->sleep_type == DEEP_SLEEP_IN) {
			TPD_INFO(
				"TP in deep sleep not support gesture flag = %d\n",
				flag);
			break;
		}
...
int __maybe_unused ilitek_platform_probe(struct spi_device *spi)
{
	...
	/*for gesture ws*/
	idev->gesture_process_ws =
		wakeup_source_register(ts->dev, "gesture_wake_lock");
	if (!idev->gesture_process_ws) {
