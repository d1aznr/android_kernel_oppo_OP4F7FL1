...
#define TPD_DEBUG(a, arg...)                                                   \
	do {                                                                   \
		if (LEVEL_DEBUG == tp_debug || ipio_debug_level)               \
			pr_err("[TP]" TPD_DEVICE ": (%s, %d): " a, __func__,   \
			       __LINE__, ##arg);                               \
	} while (0)
...
enum TP_FW_UPGRADE_TARGET { ILI_FILE = 0, HEX_FILE };
...
static inline s32 open_sp_formula_ili9881(int dac, int raw)
{
	return (int)((int)(dac * 2 * 10000 * 161 / 100) -
		     (int)(16384 / 2 - (int)raw) * 20000 * 7 / 16384 * 36 /
			     10) /
	       31 / 2;
}
...
{
	return (int)((int)(dac * 414 * 39 / 2) +
		     (int)(((int)raw - 8192) * 36 * (7 * 100 - 22) * 10 /
			   16384)) /
	       tvch / 100 / gain;
}
