...
static int ilitek_tddi_fw_iram_upgrade(u8 *pfw)
{
	...
	for (i = 0; i < ARRAY_SIZE(fbi); i++) {
		if (fbi[i].mode == mode && fbi[i].len != 0) {
			TPD_INFO(
				"Download %s code from hex 0x%x to IRAM 0x%x, len = 0x%x\n",
				fbi[i].name, fbi[i].start, fbi[i].mem_start,
				fbi[i].len);

			ilitek_tddi_fw_iram_program(fbi[i].mem_start,
						    fbi[i].len,
						    (fw_ptr + fbi[i].start));

			crc = CalculateCRC32(fbi[i].start, fbi[i].len - 4,
					     fw_ptr);
			dma = 0;
			ret = host_download_dma_check(fbi[i].mem_start,
						      fbi[i].len - 4, &dma);

			TPD_INFO("%s CRC is %s (%x) : (%x)\n", fbi[i].name,
				 (crc != dma ? "Invalid !" : "Correct !"), crc,
				 dma);

			if (ret < 0 || crc != dma) {
				ilitek_tddi_fw_print_iram_data(fbi[i].mem_start,
							       fbi[i].len);
				return UPDATE_FAIL;
			}
		}
...
static void ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	if (tfd.hex_tag == BLOCK_TAG_AF) {
		fbi[AP].mem_start = (fbi[AP].fix_mem_start != INT_MAX) ?
					    fbi[AP].fix_mem_start :
					    0;
		...
		fbi[MP].mem_start = (fbi[MP].fix_mem_start != INT_MAX) ?
		...
		if (fbi[GESTURE].len > 8 * K) {
			TPD_INFO(
				"WARRING!!!The gesture len is too long!!!The size is %d.\n",
				fbi[GESTURE].len);
			fbi[GESTURE].len = 8 * K;
		}
		fbi[GESTURE].start = 0;
	...
	/* Copy gesture data */
	if (fbi[GESTURE].mem_start != 0xffffffff &&
	    ges_fw_start != 0xffffffff && fbi[GESTURE].mem_start != 0 &&
	    ges_fw_start != 0)
		ipio_memcpy(gestrue_fw, (pfw + ges_fw_start), fbi[GESTURE].len,
			    sizeof(gestrue_fw));
	else
		ipio_err("There is no gesture data inside fw\n");

	TPD_INFO(
		"gesture memory start = 0x%x, upgrade lenth = 0x%x, hex area = %d, ges_start_addr = 0x%x, ges_end_addr = 0x%x",
		fbi[GESTURE].mem_start, MAX_GESTURE_FIRMWARE_SIZE,
		ges_area_section, ges_fw_start, ges_fw_end);
	...
	/* Get hex fw vers */
	tfd.new_fw_cb = (pfw[FW_VER_ADDR] << 24) |
			(pfw[FW_VER_ADDR + 1] << 16) |
...
static int ilitek_tddi_fw_ili_convert(u8 *pfw)
{
	...
	if (!idev->p_firmware_headfile->firmware_data ||
	    idev->p_firmware_headfile->firmware_size <= ILI_FILE_HEADER ||
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		if (((block_enable >> i) & 0x01) == 0x01) {
			num = i + 1;
			if ((num) == 6) {
				fbi[num].start = (CTPM_FW[0] << 16) +
						 (CTPM_FW[1] << 8) +
						 (CTPM_FW[2]);
				fbi[num].end = (CTPM_FW[3] << 16) +
					       (CTPM_FW[4] << 8) + (CTPM_FW[5]);
				fbi[num].fix_mem_start = INT_MAX;
			} else {
				fbi[num].start = (CTPM_FW[34 + i * 6] << 16) +
						 (CTPM_FW[35 + i * 6] << 8) +
						 (CTPM_FW[36 + i * 6]);
				fbi[num].end = (CTPM_FW[37 + i * 6] << 16) +
					       (CTPM_FW[38 + i * 6] << 8) +
					       (CTPM_FW[39 + i * 6]);
				fbi[num].fix_mem_start = INT_MAX;
			}
	...
	if ((block_enable & 0x80) == 0x80) {
		for (i = 0; i < 3; i++) {
			Addr = (CTPM_FW[6 + i * 4] << 16) +
			       (CTPM_FW[7 + i * 4] << 8) + (CTPM_FW[8 + i * 4]);
			block = CTPM_FW[9 + i * 4];

			if ((block != 0) && (Addr != 0x000000)) {
				fbi[block].fix_mem_start = Addr;
				TPD_INFO(
					"Tag 0xB0: change Block[%d] to addr = 0x%x\n",
...
static int ilitek_tddi_fw_hex_convert(u8 *pfw)
{
	...
	for (; i < size;) {
		...
		if (type == 0x04) {
			...
			fbi[num].fix_mem_start = INT_MAX;
			fbi[num].len = fbi[num].end - fbi[num].start + 1;
			TPD_INFO("Block[%d]: start_addr = %x, end = %x", num,
				 fbi[num].start, fbi[num].end);
	...
	for (i = 0; i < ARRAY_SIZE(fbi); i++) {
		...
		hex_crc = pfw[ex_addr - 3] << 24 | pfw[ex_addr - 2] << 16 |
			  pfw[ex_addr - 1] << 8 | pfw[ex_addr];
		TPD_DEBUG("data crc = %x, hex crc = %x\n", data_crc, hex_crc);
		if (data_crc != hex_crc) {
			ipio_err(
				"Content of hex file is broken. (%d, %x, %x)\n",
				i, data_crc, hex_crc);
			return -1;
		}
	}
...
int ilitek_tddi_fw_upgrade(void)
{
	...
	if (ret == UPDATE_PASS) {
		...
		if (idev->debug_mode_sta &&
		    idev->actual_tp_mode != P5_X_FW_GESTURE_MODE) {
			if (idev->ts->apk_op &&
			    idev->ts->apk_op->apk_debug_set) {
				idev->ts->apk_op->apk_debug_set((void *)idev,
								true);
			}
		}
