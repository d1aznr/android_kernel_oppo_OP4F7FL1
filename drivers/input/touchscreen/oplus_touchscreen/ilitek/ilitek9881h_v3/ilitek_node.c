...
static int file_write(struct file_buffer *file, bool new_open)
{
	...
	if (new_open)
		f = filp_open(file->file_name, O_WRONLY | O_CREAT | O_TRUNC,
			      644);
	else
		f = filp_open(file->file_name, O_WRONLY | O_CREAT | O_APPEND,
			      644);
			       ...
			       u32 frame_count)
{
	...
	while ((write_index < frame_count) && (timeout > 0)) {
		TPD_INFO("frame = %d,index = %d,count = %d\n", write_index,
			 write_index % 1024, idev->debug_data_frame);
		if ((write_index % 1024) < idev->debug_data_frame) {
			mutex_lock(&idev->touch_mutex);
			file->file_len = 0;
			memset(file->ptr, 0, file->file_max_zise);
			file->file_len +=
				snprintf(file->ptr + file->file_len,
					 file->file_max_zise - file->file_len,
					 "\n\nFrame%d,", write_index);
			for (j = 0; j < col; j++)
				file->file_len += snprintf(
					file->ptr + file->file_len,
					file->file_max_zise - file->file_len,
					"[X%d] ,", j);
					       ...
					       loff_t *pos)
{
	...
	// print delta data
	for (y = 0; y < row; y++) {
		size += snprintf(g_user_buf + size, PAGE_SIZE - size, "[%2d] ",
				 (y + 1));
		TPD_INFO("[%2d] ", (y + 1));
					  ...
					  size_t size, loff_t *pos)
{
	...
	if (type == read) {
		...
		TPD_INFO("READ:addr = 0x%06x, read = 0x%08x\n", addr,
			 read_data);
		size = snprintf(g_user_buf, PAGE_SIZE,
					     ...
					     loff_t *pos)
{
	...
	if (idev->debug_node_open) {
		if (ERR_ALLOC_MEM(idev->debug_buf)) {
			idev->debug_buf = (unsigned char **)kmalloc(
				1024 * sizeof(unsigned char *), GFP_KERNEL);
			if (!ERR_ALLOC_MEM(idev->debug_buf)) {
				for (i = 0; i < 1024; i++) {
					idev->debug_buf[i] = (char *)kmalloc(
	...
	size = snprintf(g_user_buf, USER_STR_BUFF, "debug_node_open : %s\n",
			idev->debug_node_open ? "Enabled" : "Disabled");
					      ...
					      loff_t *pos)
{
	...
	if (ERR_ALLOC_MEM(tmpbuf)) {
		...
		send_data_len +=
			snprintf(tmpbufback + send_data_len,
				 128 - send_data_len, "buffer vmalloc error\n");
		ret = copy_to_user(buff, tmpbufback,
				   send_data_len); /*idev->debug_buf[0] */
		goto out;
	}
	...
	if (idev->debug_data_frame > 0) {
		if (idev->debug_buf[0][0] == P5_X_DEMO_PACKET_ID) {
			...
			data_count =
				idev->debug_buf[0][1] * idev->debug_buf[0][2];
						    ...
						    size_t size, loff_t *pos)
{
	...
	csv.file_len += snprintf(csv.ptr + csv.file_len,
				 ...
				 "\n\n=======Delta data=======");
	file_write(&csv, false);
	ret = debug_mode_get_data(&csv, P5_X_FW_DELTA_DATA_MODE,
				  idev->delta_count);
	if (ret < 0)
		goto out;
			      ...
			      unsigned long arg)
{
	...
	case ILITEK_IOCTL_TP_MODE_STATUS:
		TPD_INFO("ioctl: current firmware mode = %d",
			 idev->actual_tp_mode);
		ret = copy_to_user((int *)arg, &idev->actual_tp_mode,
				   sizeof(int));
		if (ret < 0)
			ipio_err("Failed to copy chip id to user space\n");
		break;
	...
	case ILITEK_IOCTL_TP_INTERFACE_TYPE:
		...
		if (ret < 0) {
			ipio_err(
				"Failed to copy interface type to user space\n");
		}
		break;
...
void ilitek_tddi_node_init(void)
{
	...
	for (; i < ARRAY_SIZE(proc_table); i++) {
		proc_table[i].node =
			proc_create(proc_table[i].name, 0644, proc_dir_ilitek,
				    proc_table[i].fops);
