...
int ilitek_platform_tp_hw_reset(bool isEnable)
{
	...
	if (ipd->hw_res->reset_gpio) {
		if (isEnable) {
			...
			if (ret < 0) {
				TPD_INFO(
					"get host download data fail use default data\n");
				//goto out;
			}
			for (i = 0; i < 3; i++) {
				TPD_DEBUG("HW Reset: HIGH\n");
				gpio_direction_output(ipd->hw_res->reset_gpio,
						      1);
...
static int ilitek_core_spi_write_then_read(struct spi_device *spi,
					   uint8_t *txbuf, int w_len,
					   uint8_t *rxbuf, int r_len)
{
	int res = 0;
	int retry = 2;
...
static int core_Rx_check(struct spi_device *spi, uint16_t check)
{
	...
	for (i = 0; i < count; i++) {
		...
		txbuf[3] = 0x0;
		txbuf[4] = 0x2;
		if (ilitek_core_spi_write_then_read(spi, txbuf, 5, txbuf, 0) <
		    0) {
			size = -EIO;
		...
		txbuf[0] = SPI_READ;
		if (ilitek_core_spi_write_then_read(spi, txbuf, 1, rxbuf, 4) <
		    0) {
			size = -EIO;
			TPD_INFO("spi Write Error, res = %d\n", size);
			return size;
...
static int core_ice_mode_read_9881H11(struct spi_device *spi, uint8_t *data,
				      uint32_t size)
{
	int res = 0;
	uint8_t txbuf[64] = { 0 };
...
static int core_ice_mode_write_9881H11(struct spi_device *spi, uint8_t *data,
				       uint32_t size)
{
	int res = 0;
	uint8_t check_sum = 0;
	...
	if (ilitek_core_spi_write_then_read(spi, txbuf, wsize + 5, txbuf, 0) <
	    0) {
		res = -EIO;
		TPD_INFO("spi Write Error, res = %d\n", res);
		goto out;
...
static int core_spi_read_9881H11(struct spi_device *spi, uint8_t *pBuf,
				 uint16_t nSize)
{
	int res = 0;
	int size = 0;
...
static int core_spi_check_header(struct spi_device *spi, uint8_t *data,
				 uint32_t size)
{
	int res = 0;
	uint8_t txbuf[5] = { 0 };
...
static int core_spi_write_9881H11(struct spi_device *spi, uint8_t *pBuf,
				  uint16_t nSize)
{
	int res = 0;
	uint8_t *txbuf;
...
int core_spi_write(uint8_t *pBuf, uint16_t nSize)
{
	...
	if (ilitek_core_spi_write_then_read(temp_spi, txbuf, nSize + 1, txbuf,
					    0) < 0) {
...
int core_spi_read(uint8_t *pBuf, uint16_t nSize)
{
	...
	if (ilitek_core_spi_write_then_read(temp_spi, txbuf, 1, pBuf, nSize) <
	    0) {
		if (core_config->do_ic_reset) {
			/* ignore spi error if doing ic reset */
			res = 0;
...
static int parse_touch_package_v5_0(uint8_t pid)
{
	...
	uint32_t nX = 0;
	uint32_t nY = 0;
	uint32_t count = (core_config->tp_info->nXChannelNum *
	...
	if (pid == protocol->demo_pid) {
		...
		for (i = 0; i < MAX_TOUCH_NUM; i++) {
			if ((g_fr_node->data[(4 * i) + 1] == 0xFF) &&
			    (g_fr_node->data[(4 * i) + 2] == 0xFF) &&
			    (g_fr_node->data[(4 * i) + 3] == 0xFF)) {
				continue;
			}
			nX = (((g_fr_node->data[(4 * i) + 1] & 0xF0) << 4) |
			      (g_fr_node->data[(4 * i) + 2]));
			nY = (((g_fr_node->data[(4 * i) + 1] & 0x0F) << 8) |
			      (g_fr_node->data[(4 * i) + 3]));
			if (!core_fr->isSetResolution) {
				g_mutual_data.points[i].x =
					nX * (ipd->resolution_x) / TPD_WIDTH;
				g_mutual_data.points[i].y =
					nY * (ipd->resolution_y) / TPD_HEIGHT;
			} else {
				g_mutual_data.points[i].x = nX;
				g_mutual_data.points[i].y = nY;
			}
			g_mutual_data.points[i].z =
				g_fr_node->data[(4 * i) + 4];
			if (g_mutual_data.points[i].z == 0) {
				TPD_INFO("pressure = 0 force set 1\n");
				g_mutual_data.points[i].z = 1;
			}
			g_mutual_data.pointid_info =
				g_mutual_data.pointid_info | (1 << i);
			g_mutual_data.points[i].width_major =
				g_mutual_data.points[i].z;
			g_mutual_data.points[i].touch_major =
				g_mutual_data.points[i].z;
		...
		for (i = 0; i < MAX_TOUCH_NUM; i++) {
			if ((g_fr_node->data[(3 * i) + 5] == 0xFF) &&
			    (g_fr_node->data[(3 * i) + 6] == 0xFF) &&
			    (g_fr_node->data[(3 * i) + 7] == 0xFF)) {
				continue;
			}

			nX = (((g_fr_node->data[(3 * i) + 5] & 0xF0) << 4) |
			      (g_fr_node->data[(3 * i) + 6]));
			nY = (((g_fr_node->data[(3 * i) + 5] & 0x0F) << 8) |
			      (g_fr_node->data[(3 * i) + 7]));
			if (!core_fr->isSetResolution) {
				g_mutual_data.points[i].x =
					nX * (ipd->resolution_x) / TPD_WIDTH;
				g_mutual_data.points[i].y =
		...
		if (ipd->oplus_read_debug_data &&
		    (!ERR_ALLOC_MEM(ipd->oplus_debug_buf))) {
			for (index = 0, i = 35; i < count + 35;
			     i += 2, index++) {
				if ((uint8_t)(g_fr_node->data[i] & 0x80) ==
				    (uint8_t)0x80) {
					TPD_DEBUG("%d, ",
						  (((g_fr_node->data[i] << 8) +
						    g_fr_node->data[i + 1]) -
						   0x10000));
					ipd->oplus_debug_buf[index] =
						(((g_fr_node->data[i] << 8) +
						  g_fr_node->data[i + 1]) -
						 0x10000);
				} else {
...
static int finger_report_ver_5_0(void)
{
	...
	res = core_spi_read_data_after_checksize(g_fr_node->data,
						 g_fr_node->len);
	...
	if (pid == protocol->ges_pid && core_config->isEnableGesture) {
		...
		memcpy(g_mutual_data.gesture_data, g_fr_node->data,
		       (g_total_len > GESTURE_INFO_LENGTH ?
				GESTURE_INFO_LENGTH :
				g_total_len));
		goto out;
	}
	...
	if (core_fr->isEnableFR && core_fr->handleint) {
		...
		if (g_total_len > 0) {
			...
			if (ERR_ALLOC_MEM(g_fr_node)) {
				TPD_INFO(
					"Failed to allocate g_fr_node memory %ld\n",
					PTR_ERR(g_fr_node));
				goto out;
			}

			g_fr_node->data = kcalloc(g_total_len, sizeof(uint8_t),
						  GFP_ATOMIC);
			...
			memset(g_fr_node->data, 0xFF,
			       (uint8_t)sizeof(uint8_t) * g_total_len);

			while (i < ARRAY_SIZE(fr_t)) {
				if (protocol->major ==
				    fr_t[i].protocol_marjor_ver) {
					...
					if (g_total_len < MAX_INT_DATA_LEN) {
						...
						memcpy(tdata, g_fr_node->data,
						       g_fr_node->len);
					} else {
						TPD_INFO(
							"total length (%d) is too long than user can handle\n",
							g_total_len);
						goto out;
					}
					...
					if (ipd->debug_node_open) {
						mutex_lock(
							&ipd->ilitek_debug_mutex);
						if (!ERR_ALLOC_MEM(
							    ipd->debug_buf) &&
						    !ERR_ALLOC_MEM(
							    ipd->debug_buf
								    [ipd->debug_data_frame])) {
							memset(ipd->debug_buf
								       [ipd->debug_data_frame],
							       0x00,
							       (uint8_t)sizeof(
								       uint8_t) *
								       MAX_INT_DATA_LEN);
							memcpy(ipd->debug_buf
								       [ipd->debug_data_frame],
							       tdata,
							       g_total_len);
						} else {
							TPD_INFO(
								"Failed to malloc debug_buf\n");
						}
						ipd->debug_data_frame++;
						if (ipd->debug_data_frame > 1) {
							TPD_INFO(
								"ipd->debug_data_frame = %d\n",
								ipd->debug_data_frame);
						}
						if (ipd->debug_data_frame >=
						    MAX_DEBUG_DATA_FRAME) {
							TPD_INFO(
								"ipd->debug_data_frame = %d > 1023\n",
								ipd->debug_data_frame);
							ipd->debug_data_frame =
								MAX_DEBUG_DATA_FRAME -
								1;
			...
			if (i >= ARRAY_SIZE(fr_t))
				TPD_INFO(
					"Can't find any callback functions to handle INT event\n");
		} else {
			g_mutual_data.pointid_info = -1;
...
static void copy_fw_to_buffer(struct ilitek_chip_data_9881h *chip_info,
			      const struct firmware *fw)
{
	if (fw) {
		...
		if (chip_info->tp_firmware.data == NULL) {
			...
			chip_info->tp_firmware.data =
				kmalloc(fw->size, GFP_KERNEL);
			if (chip_info->tp_firmware.data == NULL) {
				TPD_INFO(
					"retry kmalloc tp firmware data error\n");
				return;
			}
		}

		//copy bin fw to data buffer
		memcpy((u8 *)chip_info->tp_firmware.data, (u8 *)(fw->data),
		       fw->size);
		if (0 == memcmp((u8 *)chip_info->tp_firmware.data,
				(u8 *)(fw->data), fw->size)) {
...
static int ilitek_reset_for_esd(void *chip_data)
{
	...
	if (P5_0_FIRMWARE_GESTURE_MODE != core_firmware->enter_mode) {
		...
		if (ret < 0) {
			...
			temp[1] = 0x0A;
			temp[2] = 0x06;
			if ((core_write(core_config->slave_i2c_addr, temp, 3)) <
			    0) {
...
static u8 ilitek_trigger_reason(void *chip_data, int gesture_enable,
				int is_suspended)
{
	struct ilitek_chip_data_9881h *chip_info =
		(struct ilitek_chip_data_9881h *)chip_data;
				   ...
				   int max_num)
{
	...
	memcpy(points, g_mutual_data.points,
	       sizeof(struct point_info) * (chip_info->ts->max_num));
				   ...
				   struct gesture_info *gesture)
{
	...
	gesture->Point_start.x =
		(((point_data[4] & 0xF0) << 4) | (point_data[5]));
	gesture->Point_start.y =
		(((point_data[4] & 0x0F) << 8) | (point_data[6]));
	gesture->Point_end.x =
		(((point_data[7] & 0xF0) << 4) | (point_data[8]));
	gesture->Point_end.y =
		(((point_data[7] & 0x0F) << 8) | (point_data[9]));

	gesture->Point_1st.x =
		(((point_data[16] & 0xF0) << 4) | (point_data[17]));
	gesture->Point_1st.y =
		(((point_data[16] & 0x0F) << 8) | (point_data[18]));
	gesture->Point_2nd.x =
		(((point_data[19] & 0xF0) << 4) | (point_data[20]));
	gesture->Point_2nd.y =
		(((point_data[19] & 0x0F) << 8) | (point_data[21]));
	gesture->Point_3rd.x =
		(((point_data[22] & 0xF0) << 4) | (point_data[23]));
	gesture->Point_3rd.y =
		(((point_data[22] & 0x0F) << 8) | (point_data[24]));
	...
	case GESTURE_TWOLINE_DOWN:
		...
		gesture->Point_1st.x =
			(((point_data[10] & 0xF0) << 4) | (point_data[11]));
		gesture->Point_1st.y =
			(((point_data[10] & 0x0F) << 8) | (point_data[12]));
		gesture->Point_2nd.x =
			(((point_data[13] & 0xF0) << 4) | (point_data[14]));
		gesture->Point_2nd.y =
			(((point_data[13] & 0x0F) << 8) | (point_data[15]));
		break;
	...
	TPD_DEBUG(
		"gesture data 0-17 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X "
	...
	TPD_DEBUG(
		"gesture data 18-35 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X "
		"0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X\n",
	...
	TPD_INFO("gesture debug data 160-168 0x%02X 0x%02X "
		 ...
		 point_data[160], point_data[161], point_data[162],
		 point_data[163], point_data[164], point_data[165],
		 point_data[166], point_data[167], point_data[168]);

	TPD_DEBUG(
		"before scale gesture_id: 0x%x, score: %d, gesture_type: %d, clockwise: %d,"
	...
	if (!core_fr->isSetResolution) {
		gesture->Point_start.x = gesture->Point_start.x *
					 (chip_info->resolution_x) / TPD_WIDTH;
		gesture->Point_start.y = gesture->Point_start.y *
					 (chip_info->resolution_y) / TPD_HEIGHT;
	...
	TPD_INFO(
		"gesture_id: 0x%x, score: %d, gesture_type: %d, clockwise: %d, points:"
...
static int ilitek_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	if ((ERR_ALLOC_MEM(core_mp) ||
	     (!ERR_ALLOC_MEM(core_mp) && (core_mp->run == false))) &&
	    (((!ERR_ALLOC_MEM(core_firmware))) &&
...
//lcd will goto sleep when tp suspend, close lcd esd check
			...
			if (P5_0_FIRMWARE_GESTURE_MODE !=
			    core_firmware->enter_mode) {
				core_config_ic_suspend();
			} else {
				core_config->ili_sleep_type =
					SLEEP_IN_GESTURE_PS;
				/* sleep in */
				core_config_sleep_ctrl(false);
			}
		...
		case MODE_GESTURE:
			...
			if (core_config->ili_sleep_type == SLEEP_IN_DEEP) {
				TPD_INFO(
					"TP in deep sleep mode is not support gesture mode flag = %d\n",
					flag);
				break;
			}
...
//lcd will goto sleep when tp suspend, close lcd esd check
				...
				if (P5_0_FIRMWARE_GESTURE_MODE !=
					...
					temp[1] = 0x0A;
					TPD_INFO(
						"write prepare gesture command 0xF6 0x0A \n");
					if ((core_write(
						    core_config->slave_i2c_addr,
						    temp, 2)) < 0) {
						TPD_INFO(
							"write prepare gesture command error\n");
					}
					temp[0] = 0x01;
					temp[1] = 0x0A;
					temp[2] = core_gesture->mode + 1;
					TPD_INFO(
						"write gesture command 0x01 0x0A, 0x%02X\n",
						core_gesture->mode + 1);
					if ((core_write(
						    core_config->slave_i2c_addr,
						    temp, 3)) < 0) {
						TPD_INFO(
							"write gesture command error\n");
					}
				}
				core_fr->handleint = true;
			}
		...
		TPD_INFO(
			"not ready switch mode work_mode mode = %d flag = %d\n",
			mode, flag);
	}
...
ilitek_fw_update_common(void *chip_data, const struct firmware *fw, bool force)
{
	int ret = 0;
	struct ilitek_chip_data_9881h *chip_info =
		(struct ilitek_chip_data_9881h *)chip_data;
...
static int ilitek_get_vendor(void *chip_data, struct panel_info *panel_data)
{
	...
	TPD_INFO(
		"chip_info->tp_type = %d, panel_data->fw_name = %s panel_data->test_limit_name = %s\n",
...
static int ilitek_esd_handle(void *chip_data)
{
	...
	if ((timer > ILITEK_TOUCH_ESD_CHECK_PERIOD) &&
	    chip_info->esd_check_enabled) {
				  ...
				  DEBUG_READ_TYPE read_type)
{
	...
	chip_info->oplus_debug_buf =
		(int *)kzalloc((xch * ych) * sizeof(int), GFP_KERNEL);
	...
	for (i = 0; i < ych; i++) {
		...
		for (j = 0; j < xch; j++) {
			seq_printf(s, "%5d, ",
				   chip_info->oplus_debug_buf[i * xch + j]);
		}
		seq_printf(s, "\n");
	}
...
static int ilitek_platform_probe(struct spi_device *spi)
{
	...
	TPD_INFO("reset_gpio = %d irq_gpio = %d irq = %d ipd->fw_name = %s\n",
		 ipd->hw_res->reset_gpio, ipd->hw_res->irq_gpio, ipd->isr_gpio,
		 ipd->fw_name);
	...
	if (ts->esd_handle_support) {
		ts->esd_info.esd_work_time = msecs_to_jiffies(
			ILITEK_TOUCH_ESD_CHECK_PERIOD); // change esd check interval to 1.5s
...
err_register_driver:
	if ((ts->boot_mode == MSM_BOOT_MODE__FACTORY ||
		...
		gesture_process_ws =
			wakeup_source_register("gesture_wake_lock");
