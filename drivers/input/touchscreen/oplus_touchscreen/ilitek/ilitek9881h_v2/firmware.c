...
static int read_download(uint32_t start, uint32_t size, uint8_t *r_buf,
			 uint32_t r_len)
{
	int res = 0;
	int addr = 0;
...
static int write_download(uint32_t start, uint32_t size, uint8_t *w_buf,
			  uint32_t w_len)
{
	int res = 0;
	int addr = 0;
...
int core_firmware_get_project_h_file_data(void)
{
	...
	if (do_once == 0) {
		...
		if (ipd->p_firmware_headfile->firmware_data) {
			if (MAX_CTPM_FW_LEN >=
			    ipd->p_firmware_headfile->firmware_size) {
				memcpy(CTPM_FW,
				       ipd->p_firmware_headfile->firmware_data,
				       ipd->p_firmware_headfile->firmware_size);
				ret = 0;
			} else {
				ipio_info(
					"p_firmware_headfile->firmware_data is more than %d,exit firmware update!\n",
					MAX_CTPM_FW_LEN);
				ret = -1;
			}
		} else {
			TPD_INFO(
				"p_firmware_headfile->firmware_data is NULL! exit firmware update!\n");
			ret = -1;
		}
	}
	return ret;
