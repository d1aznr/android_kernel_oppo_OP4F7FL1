...
struct mp_test_items tItems[] = {
	...
	{ .name = "self_dac",
	  ...
	  .catalog = SELF_TEST },
	{ .name = "self_bg",
	  .desp = "Baselin Data(BG,Self_Tx,Self_Rx)",
	  .result = "FAIL",
	  .catalog = SELF_TEST },
	{ .name = "self_signal",
	...
	{ .name = "open_integration",
	  ...
	  .result = "FAIL",
	  .catalog = OPEN_TEST },
	{ .name = "open_integration_sp",
	  .desp = "Open Test(integration)_SP",
	  .result = "FAIL",
	  .catalog = OPEN_TEST },
	{ .name = "open_cap",
	  .desp = "Open Test(Cap)",
	...
	{ .name = "doze_p2p",
	  ...
	  .result = "FAIL",
	  .catalog = PEAK_TO_PEAK_TEST },
	{ .name = "doze_raw_td_lcm_off",
	  .desp = "Raw Data_TD (LCM OFF)",
	  .result = "FAIL",
	  .catalog = MUTUAL_TEST },
	{ .name = "doze_p2p_td_lcm_off",
...
static void mp_compare_cdc_result(int32_t *tmp, int32_t *max_ts,
				  int32_t *min_ts, int *result)
{
	int x = 0;
	int y = 0;
...
static void mp_compare_cdc_show_result(int32_t *tmp, char *csv, int *csv_len,
				       int type, int32_t *max_ts,
				       int32_t *min_ts, const char *desp)
{
	int x = 0;
	int y = 0;
	int tmp_len = *csv_len;
		...
		for (x = 0; x < core_mp->xch_len; x++) {
			...
			if ((tmp[shift] <= max_ts[shift] &&
				...
				if ((tmp[shift] == INT_MAX ||
					...
					DUMP(DEBUG_MP_TEST, " %7d ",
					     tmp[shift]);
					tmp_len +=
						sprintf(csv + tmp_len, " %7d, ",
							tmp[shift]);
				}
			} else {
				if (tmp[shift] > max_ts[shift]) {
...
static int create_mp_test_frame_buffer(int index, int frame_count)
{
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		...
		core_mp->tx_delta_buf =
			vmalloc(core_mp->frame_len * sizeof(int32_t));
		ipio_vfree((void **)&core_mp->rx_delta_buf);
		core_mp->rx_delta_buf =
			vmalloc(core_mp->frame_len * sizeof(int32_t));
		...
		if (tItems[index].spec_option == BENCHMARK) {
			...
			if (ERR_ALLOC_MEM(tItems[index].bench_mark_max) ||
				...
				TPD_INFO(
					"Failed to allocate bench_mark FRAME buffer\n");
				return -ENOMEM;
			}
		}
...
static void mp_calc_nodp(bool long_v)
{
	...
	if (!long_v) {
		...
		tmp = ((tshd << 2) - (at << 6) - tp_tsdh_wait -
		       ddi_width * (multi_term_num - 1) - 64 - dp2tp - twc) *
		      5;
		tmp1 = (phase << 5) -
		       ((twcm * 5 + (phase << 5) + (tp2dp * 5 << 6)) *
			(multi_term_num - 1));
		real_tx_dura = (tmp - tmp1) / (multi_term_num * 5);

		core_mp->nodp.first_tp_width =
			(dp2tp * 5 + twc * 5 + (phase << 5) +
			 real_tx_dura * 5) /
			5;
		core_mp->nodp.tp_width =
			(((tp2dp * 10 + phase) << 6) + real_tx_dura * 10) / 10;
		...
		real_tx_dura = (((tshd << 2) - (at << 6) - ddi_width * (11) -
				 64 - dp2tp - twc) *
					5 -
				(phase << 5) -
				((twcm * 5 + (phase << 5) + (tp2dp * 5 << 6)) *
...
static int allnode_key_cdc_data(int index)
{
	...
	if (key_buf == NULL) {
		key_buf =
			kcalloc(core_mp->key_len, sizeof(int32_t), GFP_KERNEL);
	...
	dump_data(key_buf, 32, core_mp->frame_len, core_mp->xch_len,
...
int core_mp_calc_timing_nodp(void)
{
     ...
     */
	if (LEVEL_DEBUG == tp_debug) {
		dump_data(timing_cmd, 8, sizeof(timing_cmd), 0,
			  "Timing command");
	}
	ret = core_write(core_config->slave_i2c_addr, timing_cmd,
			 sizeof(timing_cmd));
	if (ret < 0) {
	...
	ret = core_read(core_config->slave_i2c_addr, get_timing,
			sizeof(get_timing));
	if (ret < 0) {
...
static int mp_cdc_get_pv5_4_command(uint8_t *cmd, int len, int index)
{
	...
	else if (strncmp(key, "Noise Peak to Peak(With Panel) (LCM OFF)",
			 strlen(key)) == 0)
...
static int allnode_mutual_cdc_data(int index)
{
	...
	if (frame_buf == NULL) {
		frame_buf = kcalloc(core_mp->frame_len, sizeof(int32_t),
				    GFP_KERNEL);
	...
	for (i = 0; i < core_mp->frame_len; i++) {
		if (tItems[index].cmd == protocol->mutual_dac) {
			...
			if (strncmp(tItems[index].name, "mutual_no_bk",
				    strlen("mutual_no_bk")) == 0 ||
			    strncmp(tItems[index].name, "mutual_no_bk_lcm_off",
				    strlen("mutual_no_bk_lcm_off")) == 0) {
				if (core_config->chip_id == CHIP_TYPE_ILI9881) {
					if (core_config->chip_type ==
					    ILI9881_TYPE_H)
						frame_buf[i] -=
							RAWDATA_NO_BK_DATA_SHIFT_9881H;
					if (core_config->chip_type ==
					    ILI9881_TYPE_F)
						frame_buf[i] -=
							RAWDATA_NO_BK_DATA_SHIFT_9881F;
				}
			}
		}
	}

	dump_data(frame_buf, 32, core_mp->frame_len, core_mp->xch_len,
		  "Mutual CDC combined");
...
int full_open_rate_compare(int32_t *full_open, int32_t *cbk, int x, int y,
			   int32_t inNodeType, int full_open_rate)
{
	int ret = true;
	...
	for (i = 0; i < core_mp->frame_len; i++) {
		if (mode == 0) {
			...
			if ((tmp & 0x8000) == 0x8000)
				...
				buf[i] = tmp;
			buf[i] = (int)((int)(dac[i] * 2 * 10000 * 161 / 100) -
				       (int)(16384 / 2 - (int)buf[i]) * 20000 *
					       7 / 16384 * 36 / 10) /
				 31 / 2;
		}
	}
	dump_data(buf, 10, core_mp->frame_len, core_mp->xch_len,
...
static int open_test_sp(int index)
{
	...
	TPD_DEBUG("index = %d, name = %s, CMD = 0x%x, Frame Count = %d\n",
		  index, tItems[index].name, tItems[index].cmd,
	...
	if (tItems[index].test_count == 0) {
		...
		if (ERR_ALLOC_MEM(frame1_cbk700) ||
		    ERR_ALLOC_MEM(frame1_cbk250) ||
		    ERR_ALLOC_MEM(frame1_cbk200)) {
	...
	tItems[index].node_type =
		kcalloc(core_mp->frame_len, sizeof(int32_t), GFP_KERNEL);
	...
	if (tItems[index].spec_option == BENCHMARK) {
		core_parser_benchmark(tItems[index].bench_mark_max,
				      tItems[index].bench_mark_min,
	...
	res = core_parser_get_int_data(tItems[index].desp, "Charge_Border",
				       str);
	if (res || res == 0)
	...
	TPD_DEBUG(
		...
		tItems[index].frame_count, Charge_AA, Charge_Border,
		Charge_Notch, full_open_rate);

	for (i = 0; i < tItems[index].frame_count; i++) {
		open[i].cbk_700 = kcalloc(core_mp->frame_len, sizeof(int32_t),
					  GFP_KERNEL);
		open[i].cbk_250 = kcalloc(core_mp->frame_len, sizeof(int32_t),
					  GFP_KERNEL);
		open[i].cbk_200 = kcalloc(core_mp->frame_len, sizeof(int32_t),
					  GFP_KERNEL);
		open[i].charg_rate = kcalloc(core_mp->frame_len,
					     sizeof(int32_t), GFP_KERNEL);
		open[i].full_Open = kcalloc(core_mp->frame_len, sizeof(int32_t),
					    GFP_KERNEL);
		open[i].dac = kcalloc(core_mp->frame_len, sizeof(int32_t),
				      GFP_KERNEL);
		open[i].cdc = kcalloc(core_mp->frame_len, sizeof(int32_t),
				      GFP_KERNEL);

		if (ERR_ALLOC_MEM(open[i].cbk_700) ||
		    ERR_ALLOC_MEM(open[i].cbk_250) ||
		    ERR_ALLOC_MEM(open[i].cbk_200) ||
		    ERR_ALLOC_MEM(open[i].charg_rate) ||
		    ERR_ALLOC_MEM(open[i].full_Open) ||
		    ERR_ALLOC_MEM(open[i].dac) || ERR_ALLOC_MEM(open[i].cdc)) {
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if (i == 0) {
			memcpy(frame1_cbk700, open[i].cbk_700,
			       core_mp->frame_len * sizeof(int32_t));
			memcpy(frame1_cbk250, open[i].cbk_250,
			       core_mp->frame_len * sizeof(int32_t));
			memcpy(frame1_cbk200, open[i].cbk_200,
			       core_mp->frame_len * sizeof(int32_t));
		}

		dump_data(open[i].cbk_700, 10, core_mp->frame_len,
			  core_mp->xch_len, "cbk 700");
		dump_data(open[i].cbk_250, 10, core_mp->frame_len,
			  core_mp->xch_len, "cbk 250");
		dump_data(open[i].cbk_200, 10, core_mp->frame_len,
		...
		if (LEVEL_DEBUG == tp_debug) {
			dump_data(open[i].charg_rate, 10, core_mp->frame_len,
				  core_mp->xch_len, "origin charge rate");
			dump_data(open[i].full_Open, 10, core_mp->frame_len,
		...
		if (LEVEL_DEBUG == tp_debug)
			dump_data(&tItems[index].buf[(i * core_mp->frame_len)],
...
int codeToOhm(int32_t Code)
{
	...
	if (Code == 0) {
		...
		temp = ((douTVCH - douTVCL) * douVariation *
			(douTDF1 - douTDF2) * (1 << 12) /
			(9 * Code * douCint)) *
		       100;
...
int mp_test_data_sort_average(int32_t *oringin_data, int index,
			      int32_t *avg_result)
{
	int i = 0;
	int j = 0;
...
int core_mp_compare_retry_cdc_result(int i)
{
	...
	if (tItems[i].catalog == TX_RX_DELTA) {
		if (ERR_ALLOC_MEM(core_mp->rx_delta_buf) ||
			...
			TPD_INFO(
				"This test item (%s) has no data inside its buffer\n",
				tItems[i].desp);
			test_result = MP_FAIL;
			goto fail_open;
		}
		...
		mp_compare_cdc_result(core_mp->tx_max_buf, max_threshold,
				      min_threshold, &test_result);
		mp_compare_cdc_result(core_mp->tx_min_buf, max_threshold,
				      min_threshold, &test_result);
		for (j = 0; j < core_mp->frame_len; j++) {
			max_threshold[j] = core_mp->RxDeltaMax;
			min_threshold[j] = core_mp->RxDeltaMin;
		}
		mp_compare_cdc_result(core_mp->rx_max_buf, max_threshold,
				      min_threshold, &test_result);
		mp_compare_cdc_result(core_mp->rx_min_buf, max_threshold,
				      min_threshold, &test_result);
	} else {
		if (ERR_ALLOC_MEM(tItems[i].buf) ||
		    ERR_ALLOC_MEM(tItems[i].max_buf) ||
		    ERR_ALLOC_MEM(tItems[i].min_buf) ||
		    ERR_ALLOC_MEM(tItems[i].result_buf)) {
			TPD_INFO(
				"This test item (%s) has no data inside its buffer\n",
				tItems[i].desp);
			test_result = MP_FAIL;
		...
		if (tItems[i].trimmed_mean &&
			...
			mp_test_data_sort_average(tItems[i].buf, i,
						  tItems[i].result_buf);
			mp_compare_cdc_result(tItems[i].result_buf,
					      max_threshold, min_threshold,
					      &test_result);
		} else {
			mp_compare_cdc_result(tItems[i].max_buf, max_threshold,
					      min_threshold, &test_result);
			mp_compare_cdc_result(tItems[i].min_buf, max_threshold,
					      min_threshold, &test_result);
		}
	}
...
void core_mp_show_result(void)
{
	...
	if (protocol->mid >= 0x3) { /*line7*/
		...
		csv_len +=
			sprintf(csv + csv_len, "Firmware Version ,V%d.%d.%d\n",
				core_config->firmware_ver[1],
				core_config->firmware_ver[2],
				core_config->firmware_ver[3]);
	}
	...
	for (i = 0; i < ARRAY_SIZE(tItems); i++) {
		...
		if (tItems[i].spec_option == BENCHMARK) {
			...
			mp_compare_cdc_show_result(tItems[i].bench_mark_max,
						   csv, &csv_len,
						   TYPE_BENCHMARK,
						   max_threshold, min_threshold,
						   "Max_Bench");
			mp_compare_cdc_show_result(tItems[i].bench_mark_min,
						   csv, &csv_len,
						   TYPE_BENCHMARK,
						   max_threshold, min_threshold,
						   "Min_Bench");
		...
		if (strcmp(tItems[i].name, "open_integration_sp") == 0) {
			mp_compare_cdc_show_result(frame1_cbk700, csv, &csv_len,
						   TYPE_NO_JUGE, max_threshold,
						   min_threshold,
						   "frame1 cbk700");
			mp_compare_cdc_show_result(frame1_cbk250, csv, &csv_len,
						   TYPE_NO_JUGE, max_threshold,
						   min_threshold,
						   "frame1 cbk250");
			mp_compare_cdc_show_result(frame1_cbk200, csv, &csv_len,
						   TYPE_NO_JUGE, max_threshold,
						   min_threshold,
						   "frame1 cbk200");
		...
		if (tItems[i].catalog == KEY_TEST) {
			for (x = 0; x < core_mp->key_len; x++) {
				...
				csv_len +=
					sprintf(csv + csv_len, "KEY_%02d,", x);
			}
			...
			for (y = 0; y < core_mp->key_len; y++) {
				DUMP(DEBUG_MP_TEST, " %3d   ",
				     tItems[i].buf[y]);
				csv_len += sprintf(csv + csv_len, " %3d, ",
						   tItems[i].buf[y]);
			}
			...
			mp_compare_cdc_show_result(core_mp->tx_max_buf, csv,
						   &csv_len, TYPE_JUGE,
						   max_threshold, min_threshold,
						   "TX Max Hold");
			mp_compare_cdc_show_result(core_mp->tx_min_buf, csv,
						   &csv_len, TYPE_JUGE,
						   max_threshold, min_threshold,
						   "TX Min Hold");
			...
			mp_compare_cdc_show_result(core_mp->rx_max_buf, csv,
						   &csv_len, TYPE_JUGE,
						   max_threshold, min_threshold,
						   "RX Max Hold");
			mp_compare_cdc_show_result(core_mp->rx_min_buf, csv,
						   &csv_len, TYPE_JUGE,
						   max_threshold, min_threshold,
						   "RX Min Hold");
			...
			if (tItems[i].trimmed_mean &&
				...
				mp_compare_cdc_show_result(
					tItems[i].result_buf, csv, &csv_len,
					TYPE_JUGE, max_threshold, min_threshold,
					"Mean result");
			} else {
				mp_compare_cdc_show_result(
					tItems[i].max_buf, csv, &csv_len,
					TYPE_JUGE, max_threshold, min_threshold,
					"Max Hold");
				mp_compare_cdc_show_result(
					tItems[i].min_buf, csv, &csv_len,
					TYPE_JUGE, max_threshold, min_threshold,
					"Min Hold");
			}
	...
	if (!(core_mp->oplus_lcm) && core_mp->osc_test) {
		csv_len += sprintf(
			...
			core_mp->osc_org_data[0], core_mp->osc_org_data[1],
			core_mp->osc_org_data[2], core_mp->osc_org_data[3]);
		csv_len +=
			sprintf(csv + csv_len,
				"OSC threshold(%s%d.%d%% ~ +%d.%d%%)\n",
				(core_mp->osc_threshold_min < 0 &&
				 core_mp->osc_threshold_min > -10) ?
					"-" :
					" ",
				core_mp->osc_threshold_min / 10,
				(int)Mathabs(core_mp->osc_threshold_min % 10),
				core_mp->osc_threshold_max / 10,
				core_mp->osc_threshold_max % 10);
		csv_len += sprintf(
			csv + csv_len,
			"trim count is %d OSC offset Percent result: %s%d.%d%%  %s \n",
			((core_mp->osc_org_data[0] << 8) +
			 core_mp->osc_org_data[1]),
			(core_mp->osc_offset < 0 && core_mp->osc_offset > -10) ?
				"-" :
				" ",
			core_mp->osc_offset / 10,
			(int)(Mathabs(core_mp->osc_offset % 10)),
			(core_mp->osc_result == MP_PASS) ? "OSC test pass" :
							   "OSC test NG");
	...
	for (i = 0; i < core_mp->mp_items; i++) {
		if (strncmp(item, tItems[i].desp, strlen(item)) == 0) {
			if (ini) {
				...
				core_parser_get_int_data(item, "SPEC Option",
							 str);
				tItems[i].spec_option = katoi(str);
				core_parser_get_int_data(item, "Type Option",
							 str);
				tItems[i].type_option = katoi(str);
				core_parser_get_int_data(item, "Frame Count",
							 str);
				tItems[i].frame_count = katoi(str);
				core_parser_get_int_data(item, "Trimmed Mean",
							 str);
				tItems[i].trimmed_mean = katoi(str);
				core_parser_get_int_data(
					item, "Lowest Percentage", str);
				tItems[i].lowest_percentage = katoi(str);
				core_parser_get_int_data(
					item, "Highest Percentage", str);
				tItems[i].highest_percentage = katoi(str);

				/* Get threshold from ini structure in parser */
				if (strcmp(item, "Tx/Rx Delta") == 0) {
					core_parser_get_int_data(item, "Tx Max",
								 str);
					core_mp->TxDeltaMax = katoi(str);
					core_parser_get_int_data(item, "Tx Min",
								 str);
					core_mp->TxDeltaMin = katoi(str);
					core_parser_get_int_data(item, "Rx Max",
								 str);
					core_mp->RxDeltaMax = katoi(str);
					core_parser_get_int_data(item, "Rx Min",
								 str);
					core_mp->RxDeltaMin = katoi(str);
					TPD_DEBUG(
						"%s: Tx Max = %d, Tx Min = %d, Rx Max = %d,  Rx Min = %d\n",
						tItems[i].desp,
						core_mp->TxDeltaMax,
						core_mp->TxDeltaMin,
						core_mp->RxDeltaMax,
					...
					core_parser_get_int_data(item, "Max",
								 str);
					tItems[i].max = katoi(str);
					core_parser_get_int_data(item, "Min",
								 str);
				...
				TPD_DEBUG(
					"%s: run = %d, max = %d, min = %d, frame_count = %d\n",
					tItems[i].desp, tItems[i].run,
					tItems[i].max, tItems[i].min,
					tItems[i].frame_count);
			}

			if (tItems[i].run) {
				/* LCM off */
				if (strnstr(tItems[i].desp, "LCM",
					    strlen(tItems[i].desp)) != NULL) {
					if (!core_mp->oplus_run &&
					    !core_mp->oplus_lcm)
				...
				tItems[i].test_count++;
				test_result =
					core_mp_compare_retry_cdc_result(i);

				/* To see if this item needs to do retry  */
				if (core_mp->retry &&
				    (test_result == MP_FAIL)) {
					for (count = 0; count < RETRY_COUNT;
					     count++) {
						TPD_INFO(
							"retry = %d, item = %s\n",
							(count + 1),
						...
						core_mp_retry(i);
						test_result =
							core_mp_compare_retry_cdc_result(
								i);
	...
	for (i = 0; i < 3; i++) {
		...
		if (res < 0) {
			...
			TPD_INFO("read osc value 0x%X 0x%X 0x%X 0x%X\n",
				 test_cmd[0], test_cmd[1], test_cmd[2],
				 test_cmd[3]);
			...
			TPD_INFO(
				"osc offset org value is %d,  trim count is %d",
				core_mp->osc_offset,
				((test_cmd[0] << 8) + test_cmd[1]));
			break;
		}
	}
	if (i >= 3) {
		...
		if (core_mp->osc_offset > core_mp->osc_threshold_max ||
		    core_mp->osc_offset < core_mp->osc_threshold_min) {
			TPD_INFO(
				"OSC check failed, osc_offset = %d osc_threshold_max = %d osc_threshold_min = %d\n",
				core_mp->osc_offset, core_mp->osc_threshold_max,
				core_mp->osc_threshold_min);
			core_mp->osc_result = MP_FAIL;
		} else {
			TPD_INFO(
				"OSC check pass, osc_offset = %d osc_threshold_max = %d osc_threshold_min = %d\n",
				core_mp->osc_offset, core_mp->osc_threshold_max,
				core_mp->osc_threshold_min);
			core_mp->osc_result = MP_PASS;
		}
...
int ilitek_mp_test(void *chip_data, struct seq_file *s)
{
	...
	struct ilitek_chip_data_9881h *chip_info =
		(struct ilitek_chip_data_9881h *)chip_data;
	...
	ilitek_ini_file_data = (struct ini_file_data *)vmalloc(
	...
	if (!ERR_ALLOC_MEM(s)) {
		if (core_config->core_type == CORE_TYPE_B) {
			seq_printf(
				s, "%d error(s). %s\n", final_result,
				final_result ?
					"panel test failed. This panel not use in MP" :
					"All test passed.");
		} else {
			if (osc_test) {
				seq_printf(
					s,
					"%d error(s). %s %s    OSC read data 0x%X 0x%X 0x%X 0x%X;    OSC test threshold(%s%d.%d%% ~ +%d.%d%%);    OSC offset Percent result: %s%d.%d%%\n",
					final_result + osc_result,
					(final_result + osc_result) ?
						"test failed." :
						"All test passed.",
					final_result ? "MP test data fail" :
						       "MP test data ok",
					osc_org_data[0], osc_org_data[1],
					osc_org_data[2], osc_org_data[3],
					(osc_threshold_min < 0 &&
					 osc_threshold_min > -10) ?
						"-" :
						" ",
					osc_threshold_min / 10,
					(int)(Mathabs(osc_threshold_min % 10)),
					osc_threshold_max / 10,
					osc_threshold_max % 10,
					(osc_offset < 0 && osc_offset > -10) ?
						"-" :
						" ",
					osc_offset / 10,
					(int)Mathabs(osc_offset % 10));
			} else {
...
out:
	...
	if (core_config->core_type == CORE_TYPE_B) {
		sprintf(message, "%d errors. %s", final_result,
			final_result ?
				"panel black screen test failed. This panel not use in MP" :
				"All test passed.");
	} else {
		sprintf(message, "%d errors. %s", final_result,
			final_result ? "black screen test failed." :
				       "All test passed.");
	}
	if (!ERR_ALLOC_MEM(ilitek_ini_file_data)) {
		vfree(ilitek_ini_file_data);
...
int core_mp_init(void)
{
	...
	if (!ERR_ALLOC_MEM(core_config->tp_info)) {
		...
		TPD_INFO(
			"Failed to allocate core_mp mem as did not find TP info\n");
		res = -ENOMEM;
	}
...
static int get_ini_phy_data(char *data, int fsize)
{
	...
	while (true) {
		...
		n = get_ini_phy_line(data + offset, ini_buf,
				     ((offset + PARSER_MAX_CFG_BUF) >= fsize) ?
					     (fsize - offset) :
					     PARSER_MAX_CFG_BUF);
		if (n < 0) {
		...
		if (n > 2 && ((ini_buf[0] == M_CFG_SSL &&
			...
			if (ini_buf[0] == M_CFG_SSL) {
				ilitek_ini_file_data[g_ini_items]
					.iSectionNameLen = n - 2;
				if (ilitek_ini_file_data[g_ini_items]
					    .iSectionNameLen >
				    PARSER_MAX_KEY_NAME_LEN) {
					TPD_INFO(
						"MAX_KEY_NAME_LEN: Out Of Length\n");
		...
		strcpy(ilitek_ini_file_data[g_ini_items].pSectionName,
		       tmpSectionName);
		ilitek_ini_file_data[g_ini_items].iSectionNameLen =
		...
		if (isEqualSign == 0) {
			...
			if (strstr(ilitek_ini_file_data[g_ini_items]
					   .pSectionName,
				   "Benchmark_Data") > 0) {
				banchmark_flag = 1;
				isEqualSign = -1;
			} else if (strstr(ilitek_ini_file_data[g_ini_items]
						  .pSectionName,
					  "Node Type") > 0) {
				nodetype_flag = 1;
				isEqualSign = -1;
		...
		if (banchmark_flag) {
			/* Get Key names */
			ilitek_ini_file_data[g_ini_items].iKeyNameLen =
				strlen(BENCHMARK_KEY_NAME);
			strcpy(ilitek_ini_file_data[g_ini_items].pKeyName,
			       BENCHMARK_KEY_NAME);
			...
			ilitek_ini_file_data[g_ini_items].iKeyNameLen =
				isEqualSign;
			if (ilitek_ini_file_data[g_ini_items].iKeyNameLen >
			    PARSER_MAX_KEY_NAME_LEN) {
			...
			memcpy(ilitek_ini_file_data[g_ini_items].pKeyName,
			       ini_buf,
			       ilitek_ini_file_data[g_ini_items].iKeyNameLen);
			ilitek_ini_file_data[g_ini_items].iKeyValueLen =
				n - isEqualSign - 1;
		}

		/* Get a value assigned to a key */

		if (ilitek_ini_file_data[g_ini_items].iKeyValueLen >
...
static void init_ilitek_ini_data(void)
{
	...
	for (i = 0; i < PARSER_MAX_KEY_NUM; i++) {
		memset(ilitek_ini_file_data[i].pSectionName, 0,
		       PARSER_MAX_KEY_NAME_LEN);
		memset(ilitek_ini_file_data[i].pKeyName, 0,
		       PARSER_MAX_KEY_NAME_LEN);
		memset(ilitek_ini_file_data[i].pKeyValue, 0,
		       PARSER_MAX_KEY_VALUE_LEN);
...
static int get_ini_key_value(char *section, char *key, char *value)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		...
		if (strcmp(key, ilitek_ini_file_data[i].pKeyName) == 0) {
			memcpy(value, ilitek_ini_file_data[i].pKeyValue,
			       ilitek_ini_file_data[i].iKeyValueLen);
...
void core_parser_nodetype(int32_t *type_ptr, char *desp)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		if ((strstr(ilitek_ini_file_data[i].pSectionName, desp) ==
		     NULL) ||
		    strcmp(ilitek_ini_file_data[i].pKeyName,
			   NODE_TYPE_KEY_NAME) != 0) {
			continue;
		}

		record = ',';
		for (j = 0, index1 = 0;
		     j <= ilitek_ini_file_data[i].iKeyValueLen; j++) {
			if (ilitek_ini_file_data[i].pKeyValue[j] == ';' ||
			    j == ilitek_ini_file_data[i].iKeyValueLen) {
				if (record != '.') {
					memset(str, 0, sizeof(str));
					memcpy(str,
					       &ilitek_ini_file_data[i]
							.pKeyValue[index1],
					       (j - index1));
					temp = katoi(str);
					type_ptr[count] = temp;
...
void core_parser_benchmark(int32_t *max_ptr, int32_t *min_ptr, int8_t type,
			   char *desp)
{
	int i = 0;
	int j = 0;
	...
	for (i = 0; i < g_ini_items; i++) {
		if ((strcmp(ilitek_ini_file_data[i].pSectionName,
			    Benchmark_desp) != 0) ||
		    strcmp(ilitek_ini_file_data[i].pKeyName,
			   BENCHMARK_KEY_NAME) != 0) {
			continue;
		}
