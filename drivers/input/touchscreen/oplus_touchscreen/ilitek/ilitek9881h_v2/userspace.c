					     ...
					     loff_t *pPos)
{
	...
	if (chip_info->debug_node_open) {
		if (ERR_ALLOC_MEM(chip_info->debug_buf)) {
			chip_info->debug_buf = (unsigned char **)kmalloc(
				MAX_DEBUG_DATA_FRAME * sizeof(unsigned char *),
				GFP_KERNEL);
			if (!ERR_ALLOC_MEM(chip_info->debug_buf)) {
				for (i = 0; i < MAX_DEBUG_DATA_FRAME; i++) {
					chip_info
						->debug_buf[i] = (char *)kmalloc(
						MAX_INT_DATA_LEN *
					      ...
					      loff_t *pPos)
{
	...
	while (chip_info->debug_data_frame <= 0) {
		...
		wait_event_interruptible(chip_info->inq,
					 chip_info->debug_data_frame > 0);
	}
	...
	if (ERR_ALLOC_MEM(tmpbuf)) {
		...
		send_data_len += sprintf(tmpbufback + send_data_len,
					 "buffer vmalloc error\n");
		ret = copy_to_user(
			buff, tmpbufback,
			send_data_len); /*ts->chip_data->debug_buf[0] */
	} else {
		if (chip_info->debug_data_frame > 0) {
			if (chip_info->debug_buf[0][0] == 0x5A) {
				...
				data_count = chip_info->debug_buf[0][1] *
					     chip_info->debug_buf[0][2];
			...
			send_data_len +=
				sprintf(tmpbuf + send_data_len, "\n\n");

			if (p == 5 || size == (MAX_INT_DATA_LEN * 2) ||
			    size == MAX_INT_DATA_LEN) {
				chip_info->debug_data_frame--;
				if (chip_info->debug_data_frame < 0) {
					chip_info->debug_data_frame = 0;
				}

				for (i = 1; i <= chip_info->debug_data_frame;
				     i++) {
					memcpy(chip_info->debug_buf[i - 1],
					       chip_info->debug_buf[i],
					       MAX_INT_DATA_LEN);
				}
			}
		} else {
...
static ssize_t ilitek_proc_mp_black_screen_test_read(struct file *filp,
						     char __user *buff,
						     size_t size, loff_t *pPos)
{
	int ret = 0;
					...
					size_t size, loff_t *pPos)
{
	...
	res = copy_to_user((uint32_t *)buff, &core_config->isEnableGesture,
			   len);
	if (res < 0) {
		TPD_INFO("Failed to copy data to user space\n");
	}
					   ...
					   size_t size, loff_t *pPos)
{
	...
	res = copy_to_user((uint32_t *)buff, &core_firmware->update_status,
			   len);
	if (res < 0) {
		TPD_INFO("Failed to copy data to user space");
	}
 ...
 */
static ssize_t ilitek_proc_fw_upgrade_read(struct file *filp, char __user *buff,
					   size_t size, loff_t *pPos)
{
	int res = 0;
	uint32_t len = 0;
	...
	if (strcmp(cmd, "reset") == 0) {
		...
		TPD_INFO("test gettpregdata set reg is 0x%X\n",
			 (data[1] << 24 | data[2] << 16 | data[3] << 8 |
			  data[4]));
		core_config_get_reg_data(data[1] << 24 | data[2] << 16 |
					 data[3] << 8 | data[4]);
			      ...
			      unsigned long arg)
{
	...
	case ILITEK_IOCTL_I2C_WRITE_DATA:
		...
		if (res < 0) {
			...
			res = core_write(core_config->slave_i2c_addr, &szBuf[0],
					 i2c_rw_length);
			if (res < 0) {
	...
	case ILITEK_IOCTL_I2C_READ_DATA:
		res = core_read(core_config->slave_i2c_addr, szBuf,
				i2c_rw_length);
		if (res < 0) {
			TPD_INFO("Failed to read data via i2c\n");
		} else {
			res = copy_to_user((uint8_t *)arg, szBuf,
					   i2c_rw_length);
			if (res < 0) {
				TPD_INFO("Failed to copy data to user space\n");
			}
	...
	case ILITEK_IOCTL_TP_CHIP_ID:
		...
		if (res < 0) {
			...
			id_to_user = core_config->chip_id << 16 |
				     core_config->chip_type;

			res = copy_to_user((uint32_t *)arg, &id_to_user,
					   sizeof(uint32_t));
			if (res < 0) {
				TPD_INFO(
	...
	case ILITEK_IOCTL_TP_NETLINK_STATUS:
		...
		res = copy_to_user((int *)arg, &core_fr->isEnableNetlink,
				   sizeof(int));
		if (res < 0) {
	...
	case ILITEK_IOCTL_TP_MODE_STATUS:
		TPD_DEBUG("Current firmware mode : %d",
			  core_fr->actual_fw_mode);
		res = copy_to_user((int *)arg, &core_fr->actual_fw_mode,
				   sizeof(int));
		if (res < 0) {
			TPD_INFO("Failed to copy chip id to user space\n");
		}
...
static proc_node_t proc_table[] = {
	...
	{ "mp_black_screen_test", NULL, &proc_mp_black_screen_test_fops,
	  false },
...
static int netlink_init(void)
{
	...
	_gNetLinkSkb = netlink_kernel_create(
...
int ilitek_proc_init(struct touchpanel_data *ts)
{
	...
	for (; i < ARRAY_SIZE(proc_table); i++) {
		proc_table[i].node = proc_create_data(proc_table[i].name, 0666,
						      proc_dir_ilitek,
						      proc_table[i].fops, ts);
