...
static void create_func_hash(void)
{
	...
	insert_func(0, protocol->func_ctrl_len, protocol->sense_ctrl,
		    "sense_ctrl");
	insert_func(1, protocol->func_ctrl_len, protocol->sleep_ctrl,
		    "sleep_ctrl");
	insert_func(2, protocol->func_ctrl_len, protocol->glove_ctrl,
		    "glove_ctrl");
	insert_func(3, protocol->func_ctrl_len, protocol->stylus_ctrl,
		    "stylus_ctrl");
	insert_func(4, protocol->func_ctrl_len, protocol->tp_scan_mode,
		    "tp_scan_mode");
	insert_func(5, protocol->func_ctrl_len, protocol->lpwg_ctrl,
		    "lpwg_ctrl");
	insert_func(6, protocol->func_ctrl_len, protocol->gesture_ctrl,
		    "gesture_ctrl");
	insert_func(7, protocol->func_ctrl_len, protocol->phone_cover_ctrl,
		    "phone_cover_ctrl");
	insert_func(8, protocol->func_ctrl_len, protocol->finger_sense_ctrl,
		    "finger_sense_ctrl");
	insert_func(9, protocol->window_len, protocol->phone_cover_window,
		    "phone_cover_window");
	insert_func(10, protocol->func_ctrl_len, protocol->finger_sense_ctrl,
		    "finger_sense_ctrl");
	insert_func(11, protocol->func_ctrl_len, protocol->proximity_ctrl,
		    "proximity_ctrl");
	insert_func(12, protocol->func_ctrl_len, protocol->plug_ctrl,
		    "plug_ctrl");
	insert_func(13, protocol->func_ctrl_len, protocol->edge_limit_ctrl,
		    "edge_limit_ctrl");
	insert_func(14, protocol->func_ctrl_len, protocol->lock_point_ctrl,
		    "lock_point_ctrl");
}
...
void core_protocol_func_control(int key, int ctrl)
{
	...
	if (tmp != NULL) {
		...
		if (tmp->key != 9) {
			if (tmp->key == 1 && ctrl == 0) {
				if (core_config->ili_sleep_type ==
				    SLEEP_IN_GESTURE_PS) {
					TPD_INFO(
						"Gesture mode TP enter sleep in\n");
					tmp->cmd[tmp->len - 1] = 0x00;
				} else if (core_config->ili_sleep_type ==
					   SLEEP_IN_DEEP) {
					...
					tmp->cmd[tmp->len - 1] = 0x03;
					   ...
					   SLEEP_IN_BEGIN_FTM) {
					TPD_INFO(
						"TP enter sleep in at begin of ftm, but not close lcd\n");
					tmp->cmd[tmp->len - 1] = 0x04;
				} else if (core_config->ili_sleep_type ==
					   SLEEP_IN_END_FTM) {
					TPD_INFO(
						"TP enter sleep in at end of ftm, close lcd to reduce 1ma\n");
		...
		TPD_INFO(
			"Found func's name: %s, key = %d cmd: 0x%02X 0x%02X 0x%02X\n",
...
int core_protocol_update_ver(uint8_t major, uint8_t mid, uint8_t minor)
{
	...
	struct protocol_sup_list pver[] = {
		{ 0x5, 0x0, 0x0 }, { 0x5, 0x1, 0x0 }, { 0x5, 0x2, 0x0 },
		{ 0x5, 0x3, 0x0 }, { 0x5, 0x4, 0x0 },
	};

	for (i = 0; i < ARRAY_SIZE(pver); i++) {
		if (pver[i].major == major &&
		    pver[i].mid == mid /* && pver[i].minor == minor*/) {
			protocol->major = major;
			protocol->mid = mid;
			protocol->minor = minor;
			TPD_INFO("protocol: major = %d, mid = %d, minor = %d\n",
				 protocol->major, protocol->mid,
				 protocol->minor);
...
void core_flash_init(uint16_t mid, uint16_t did)
{
	...
	if (i >= ARRAY_SIZE(ft)) {
		TPD_INFO(
			"Can't find them in flash table, apply default flash config\n");
		flashtab->mid = mid;
		flashtab->dev_id = did;
...
void core_config_mode_control(uint8_t *from_user)
{
	...
	TPD_DEBUG("mode = %x, b1 = %x, b2 = %x, b3 = %x\n", from_user[0],
		  from_user[1], from_user[2], from_user[3]);
	...
	if (protocol->major == 0x5) {
		if (mode == protocol->i2cuart_mode) {
			...
			TPD_INFO(
				"Switch to I2CUART mode, cmd = %x, b1 = %x, b2 = %x\n",
				cmd[0], cmd[1], cmd[2]);

			if ((core_write(core_config->slave_i2c_addr, cmd, 3)) <
			    0) {
				TPD_INFO("Failed to switch I2CUART mode\n");
				goto out;
			}

		} else if (mode == protocol->demo_mode ||
			   mode == protocol->debug_mode) {
			...
			TPD_INFO(
				"Switch to Demo/Debug mode, cmd = 0x%x, b1 = 0x%x\n",
				cmd[0], cmd[1]);

			if ((core_write(core_config->slave_i2c_addr, cmd, 2)) <
			    0) {
...
int core_config_get_tp_info(void)
{
	...
	TPD_DEBUG(
		"side_touch_type = %d, max_touch_num= %d, touch_key_num = %d, max_key_num = %d\n",
...
int core_config_get_protocol_ver(void)
{
	...
	if (g_read_buf[0] != cmd[1]) {
		TPD_INFO(
			"Failed to read protocol version packet data[0] = 0x%X\n",
			g_read_buf[0]);
		goto out;
	}
...
int core_config_get_core_ver(void)
{
	...
	TPD_DEBUG("Core Version = %d.%d.%d.%d\n", core_config->core_ver[1],
		  core_config->core_ver[2], core_config->core_ver[3],
...
{
	core_config =
		kzalloc(sizeof(*core_config) * sizeof(uint8_t) * 6, GFP_KERNEL);
	...
	core_config->tp_info =
		kzalloc(sizeof(*core_config->tp_info), GFP_KERNEL);
