				 ...
				 char *desp, int frame_len, char *bchmrk_name)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		if ((ipio_strcmp(ini_info[i].section_name, benchmark_str) !=
		     0)) {
			continue;
		}
...
static int parser_get_ini_phy_data(char *data, int fsize)
{
	...
	while (true) {
		...
		if (n > 2 && ((ini_buf[0] == M_CFG_SSL &&
			...
			if (ini_buf[0] == M_CFG_SSL) {
				...
				strncpy(seq_item[scount], tmpSectionName,
					PARSER_MAX_KEY_NAME_LEN - 1);
				seq_item[scount][PARSER_MAX_KEY_NAME_LEN - 1] =
					'\0';
				scount++;
		...
		ini_info[g_ini_items].section_name[PARSER_MAX_KEY_NAME_LEN - 1] =
			'\0';
		...
		if (isEqualSign == 0) {
			...
			if (strstr(ini_info[g_ini_items].section_name,
				...
				strncpy(ini_info[g_ini_items].key_name,
					...
					(PARSER_MAX_KEY_NAME_LEN *
					 sizeof(char)));
				ini_info[g_ini_items]
					.key_name[PARSER_MAX_KEY_NAME_LEN - 1] =
					'\0';
	...
	for (y = 0; y < core_mp.ych_len; y++) {
		for (x = 0; x < core_mp.xch_len; x++) {
			...
			if (y != (core_mp.ych_len - 1)) {
				core_mp.tx_delta_buf[shift] = Mathabs(
					p_comb[shift] -
					p_comb[(shift + core_mp.xch_len)]);
			}

			/* Rx Delta */
			if (x != (core_mp.xch_len - 1)) {
				core_mp.rx_delta_buf[shift] = Mathabs(
					p_comb[shift] - p_comb[shift + 1]);
			}
		}
	}
}
				...
				int file_size)
{
	...
	/* print final result*/
	tmp_len += snprintf(
		csv + tmp_len, CSV_FILE_SIZE - tmp_len, "Final Result ,%s\n",
...
static void allnode_open_cdc_result(int index, int *buf, int *dac, int *raw)
{
	...
	if (ipio_strcmp(desp, "open test(integration)_sp") == 0) {
		...
		if (parser_get_int_data(desp, "accuracy", str, sizeof(str)) >
		    0) {
	...
	for (seq = 0; seq < ri.count; seq++) {
		...
		if (tItems[i].catalog == KEY_TEST) {
			...
			for (j = 0; j < get_frame_cont; j++) {
				...
				snprintf(frame_name, (CSV_FILE_SIZE - csv_len),
					 "Frame %d", (j + 1));
				mp_compare_cdc_show_result(
					i,
	...
	if (core_mp.final_result == MP_DATA_FAIL) {
		...
		if (lcm_on) {
			...
			snprintf(csv_name, sizeof(csv_name),
				 "%s/%s_lcm_on_%s_%s.csv", CSV_LCM_ON_NG_PATH,
				 ilits->ts->panel_data.manufacture_info
					 .manufacture,
				 get_date_time_str(), ret_fail_name);
#else
			snprintf(csv_name, sizeof(csv_name),
				 "%s/lcm_on_%s_%s.csv", CSV_LCM_ON_NG_PATH,
				 get_date_time_str(), ret_fail_name);
#endif
		} else {
...
#if 1
			snprintf(csv_name, sizeof(csv_name),
				 "%s/%s_lcm_off_%s_%s.csv", CSV_LCM_OFF_NG_PATH,
				 ilits->ts->panel_data.manufacture_info
					 .manufacture,
				 get_date_time_str(), ret_fail_name);
#else
			snprintf(csv_name, sizeof(csv_name),
				 "%s/lcm_off_%s_%s.csv", CSV_LCM_OFF_NG_PATH,
				 get_date_time_str(), ret_fail_name);
#endif
		}
	} else {
...
#if 1
			snprintf(csv_name, sizeof(csv_name),
				 "%s/%s_lcm_on_%s_%s.csv", CSV_LCM_ON_OK_PATH,
				 ilits->ts->panel_data.manufacture_info
					 .manufacture,
				 get_date_time_str(), ret_pass_name);
#else
			snprintf(csv_name, sizeof(csv_name),
				 "%s/lcm_on_%s_%s.csv", CSV_LCM_ON_OK_PATH,
				 get_date_time_str(), ret_pass_name);
#endif
		} else {
...
#if 1
			snprintf(csv_name, sizeof(csv_name),
				 "%s/%s_lcm_off_%s_%s.csv", CSV_LCM_OFF_OK_PATH,
				 ilits->ts->panel_data.manufacture_info
					 .manufacture,
				 get_date_time_str(), ret_pass_name);
#else
			snprintf(csv_name, sizeof(csv_name),
				 "%s/lcm_off_%s_%s.csv", CSV_LCM_OFF_OK_PATH,
				 get_date_time_str(), ret_pass_name);
#endif
		}
	}
...
static void ilitek_tddi_mp_all_pass_check(void)
{
	...
	if (temp[2] == ili_calc_packet_checksum(temp, sizeof(temp) - 1) &&
	    temp[0] == 0xFD &&
	    temp[1] == 0x02) { //0xFD:back door command header, 0x02:Normal mode
		core_mp.all_pass = true;
...
int ili_mp_test_main(char *apk, bool lcm_on, struct seq_file *s, char *message)
{
	...
	if (ilits->xch_num <= 0 || ilits->ych_num <= 0) {
		...
		if (!ERR_ALLOC_MEM(message)) {
			snprintf(message, MESSAGE_SIZE,
				 "Invalid frame length (xch %d, ych %d)\n",
	...
	if ((ili_str2hex(ver)) != (core_mp.protocol_ver >> 8)) {
		...
		if (!ERR_ALLOC_MEM(message)) {
			snprintf(
				...
				"ERROR! MP Protocol version is invaild, 0x%x\n",
				ili_str2hex(ver));
		}
	...
	if (mp_get_timing_info() < 0) {
		...
		if (!ERR_ALLOC_MEM(message)) {
			snprintf(message, MESSAGE_SIZE,
				 "Failed to get timing info from ini\n");
		}
	...
	if (ili_ic_int_trigger_ctrl(true) < 0) {
		...
		if (!ERR_ALLOC_MEM(message)) {
			snprintf(message, MESSAGE_SIZE,
				 "Failed to set INT as Level trigger\n");
		}
...
out:
	...
	if (ili_ic_int_trigger_ctrl(false) < 0) {
		...
		if (!ERR_ALLOC_MEM(message)) {
			snprintf(message, MESSAGE_SIZE,
				 "Failed to set INT back to pluse trigger\n");
		}
