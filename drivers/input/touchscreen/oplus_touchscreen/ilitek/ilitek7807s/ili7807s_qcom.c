...
extern void __attribute__((weak)) switch_spi7cs_state(bool normal)
{
	return;
}
...
int ili_spi_write_then_read_split(struct spi_device *spi, const void *txbuf,
				  unsigned n_tx, void *rxbuf, unsigned n_rx)
{
	...
	case SPI_WRITE:
		...
		for (xfercnt = 0; xfercnt < xferloop; xfercnt++) {
			...
			xfer[xfercnt].tx_buf =
				ilits->spi_tx + xfercnt * DMA_TRANSFER_MAX_LEN;
	...
	case SPI_READ:
		...
		for (xfercnt = 0; xfercnt < xferloop; xfercnt++) {
			...
			xfer[xfercnt].rx_buf =
				ilits->spi_rx + xfercnt * DMA_TRANSFER_MAX_LEN;
			spi_message_add_tail(&xfer[xfercnt], &message);
			xferlen = duplex_len -
				  (xfercnt + 1) * DMA_TRANSFER_MAX_LEN;
		}
...
#else
int ili_spi_write_then_read_direct(struct spi_device *spi, const void *txbuf,
				   unsigned n_tx, void *rxbuf, unsigned n_rx)
{
	int status = -1, duplex_len = 0;
	int offset = 0;
			   ...
			   bool spi_irq, bool i2c_irq)
{
	...
	case SPI_WRITE:
		...
		/*
            ...
            */
		if (atomic_read(&ilits->mp_stat) &&
		    wdata[index] == P5_X_SET_CDC_INIT) {
			checksum = ili_calc_packet_checksum(&wdata[index],
							    wlen - index);
			wdata[wlen] = checksum;
			wlen++;
			wdata[1] = (wlen - index) >> 8;
			wdata[2] = (wlen - index) & 0xff;
			ili_dump_data(wdata, 8, wlen, 0,
				      "mp cdc cmd with checksum");
		}

		ret = ilits->spi_write_then_read(ilits->spi, wdata, wlen, txbuf,
						 0);
	...
	case SPI_READ:
		...
		wdata[0] = SPI_READ;
		ret = ilits->spi_write_then_read(ilits->spi, wdata, 1, rxbuf,
						 rlen);
	...
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		...
		if (ilits->trans_xy) {
			...
			if (ilits->position_high_resolution == OFF) {
				...
				ilits->points[i].x = xop * ilits->panel_wid /
						     TPD_HIGH_RESOLUTION_WIDTH;
				ilits->points[i].y = yop * ilits->panel_hei /
		...
		if (ilits->position_high_resolution == OFF) {
			...
			touch_major = buf[(5 * i) +
					  P5_X_DEMO_MODE_PACKET_INFO_LEN + 5];
		}
		...
		if (ilits->ts->snr_read_support && snr->doing) {
			...
			snr->channel_x = snr->x * ilits->hw_res->TX_NUM /
					 ilits->ts->resolution_info.max_x;
			snr->channel_y = snr->y * ilits->hw_res->RX_NUM /
					 ilits->ts->resolution_info.max_y;
			ILI_INFO("snr: [%d %d, %d] {%d %d} \n", snr->x, snr->y,
				 i, snr->channel_x, snr->channel_y);
		}
	}
	...
	if (!transfer) {
		if (ilits->position_high_resolution == OFF) {
			...
			gc->pos_start.y = gc->pos_start.y * ilits->panel_hei /
					  TPD_HIGH_RESOLUTION_HEIGHT;
			gc->pos_end.x = gc->pos_end.x * ilits->panel_wid /
					TPD_HIGH_RESOLUTION_WIDTH;
			gc->pos_end.y = gc->pos_end.y * ilits->panel_hei /
					TPD_HIGH_RESOLUTION_HEIGHT;
			gc->pos_1st.x = gc->pos_1st.x * ilits->panel_wid /
					TPD_HIGH_RESOLUTION_WIDTH;
			gc->pos_1st.y = gc->pos_1st.y * ilits->panel_hei /
					TPD_HIGH_RESOLUTION_HEIGHT;
			gc->pos_2nd.x = gc->pos_2nd.x * ilits->panel_wid /
					TPD_HIGH_RESOLUTION_WIDTH;
			gc->pos_2nd.y = gc->pos_2nd.y * ilits->panel_hei /
					TPD_HIGH_RESOLUTION_HEIGHT;
			gc->pos_3rd.x = gc->pos_3rd.x * ilits->panel_wid /
					TPD_HIGH_RESOLUTION_WIDTH;
			gc->pos_3rd.y = gc->pos_3rd.y * ilits->panel_hei /
					TPD_HIGH_RESOLUTION_HEIGHT;
			gc->pos_4th.x = gc->pos_4th.x * ilits->panel_wid /
					TPD_HIGH_RESOLUTION_WIDTH;
			gc->pos_4th.y = gc->pos_4th.y * ilits->panel_hei /
					TPD_HIGH_RESOLUTION_HEIGHT;
		}
	}
...
int ili_mp_test_handler(char *apk, bool lcm_on, struct seq_file *s,
			char *message)
{
	int ret = 0;
#if 0
...
int ili_tddi_init(void)
{
	...
	if (ilits->ts->boot_mode == RECOVERY_BOOT || is_oem_unlocked() ||
	    ilits->ts->fw_update_in_probe_with_headfile) {
		if (ili_fw_upgrade_handler() < 0) {
			ILI_ERR("FW upgrade failed\n");
		}
	}
...
{
	u32 freq[] = { TP_SPI_CLK_1M,  TP_SPI_CLK_2M,  TP_SPI_CLK_3M,
		       TP_SPI_CLK_4M,  TP_SPI_CLK_5M,  TP_SPI_CLK_6M,
		       TP_SPI_CLK_7M,  TP_SPI_CLK_8M,  TP_SPI_CLK_9M,
		       TP_SPI_CLK_10M, TP_SPI_CLK_11M, TP_SPI_CLK_12M,
		       TP_SPI_CLK_13M, TP_SPI_CLK_14M, TP_SPI_CLK_15M };
	...
	ILI_INFO("name = %s, bus_num = %d,cs = %d, mode = %d, speed = %d\n",
		 ilits->spi->modalias, ilits->spi->master->bus_num,
		 ilits->spi->chip_select, ilits->spi->mode,
...
static void ilitek_ftm_process_extra(void)
{
	...
	if (ilits != NULL && ilits->ts != NULL) {
		...
		if ((ilits->ts->boot_mode == META_BOOT ||
		     ilits->ts->boot_mode == FACTORY_BOOT))
#else
...
#endif
		{
			...
			if (ret < 0) {
				ILI_INFO(
					"Failed to upgrade firmware, ret = %d\n",
					ret);
			}
				...
				int is_suspended)
{
	...
	if (!chip_info->fw_update_stat || !ilits->report ||
	    atomic_read(&ilits->tp_reset) || atomic_read(&ilits->fw_stat) ||
	    atomic_read(&ilits->tp_sw_mode) || atomic_read(&ilits->mp_stat) ||
	    atomic_read(&ilits->tp_sleep) || atomic_read(&ilits->esd_stat)) {
...
{
	struct ilitek_ts_data *chip_info = (struct ilitek_ts_data *)chip_data;
	memcpy(points, chip_info->points,
	       sizeof(struct point_info) * MAX_TOUCH_NUM);
				  ...
				  u8 read_type)
{
	...
	if (i < 10) {
		...
		for (i = 0; i < ych; i++) {
			...
			for (j = 0; j < xch; j++) {
				s16 temp;
				temp = (s16)((buf[(i * xch + j) * 2 + offset_len]
					      << 8) +
					     buf[(i * xch + j) * 2 +
						 offset_len + 1]);
		...
		seq_printf(s, "Y Data:");
		for (i = 2 * xch * ych + offset_len;
		     i < 2 * xch * ych + 2 * ych + offset_len; i += 2) {
			s16 temp;
		...
		for (i = 2 * xch * ych + 2 * ych + offset_len;
		     i < 2 * xch * ych + 2 * ych + 2 * xch + offset_len;
		     i += 2) {
			s16 temp;
			temp = (s16)((buf[i] << 8) + buf[i + 1]);
			seq_printf(s, "%5d,", temp);
...
START_TO_READ:
	if (is_delta_snr) {
		...
		if (ilits->position_high_resolution == OFF) {
			temp = (s16)((buf[(snr->channel_y * xch +
					   snr->channel_x) *
						  2 +
					  35]
				      << 8) +
				     buf[(snr->channel_y * xch + snr->channel_x) *
						 2 +
					 35 + 1]);
		} else {
			temp = (s16)((buf[(snr->channel_y * xch +
...
static void ilitek_delta_snr_read(struct seq_file *s, void *chip_data,
				  uint32_t count)
{
	struct ilitek_ts_data *chip_info = (struct ilitek_ts_data *)chip_data;
	struct touchpanel_snr *snr = &ilits->ts->snr;
	...
	for (i = 0; i < count; i++) {
		ilitek_read_delta_snr_data(s, chip_info,
					   P5_X_FW_DELTA_SNR_DATA_MODE, i);
	}
	/* change to demo mode */
	if (ili_set_tp_data_len(DATA_FORMAT_DEMO, false, NULL) < 0) {
		ILI_ERR("Failed to set tp data length\n");
...
{
	return single_open(file, ilitek_tp_auto_test_read_func,
			   PDE_DATA(inode));
}
...
static int ili_apk_tp_info_get(void *chip_data, char *buf, int len)
{
	...
	ret = snprintf(buf, len, "IC:ILI%04X\nFW_VER:0x%02X\nCH:%dX%d\n",
		       chip_info->chip->id, chip_info->chip->fw_ver >> 8 & 0xFF,
...
{
	ilits = devm_kzalloc(&spi->dev, sizeof(struct ilitek_ts_data),
			     GFP_KERNEL);
...
int __maybe_unused ilitek7807s_spi_probe(struct spi_device *spi)
{
	...
	ILI_INFO(
		"ts->resolution_info.max_x = %d ts->resolution_info.max_y = %d\n",
