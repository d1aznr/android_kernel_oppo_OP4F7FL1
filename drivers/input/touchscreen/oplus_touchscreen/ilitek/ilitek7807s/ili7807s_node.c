...
int ili_get_tp_recore_ctrl(int data)
{
	...
	case DATA_RECORD:
		...
		ret = ili_get_tp_recore_data(NULL, ilits->xch_num *
							   ilits->ych_num * 2);
			      ...
			      unsigned long arg)
{
	...
	case ILITEK_IOCTL_TP_CORE_VER:
		...
		if (ilits->protocol->core_ver_len ==
