...
#define TR_BUF_LIST_SIZE                                                       \
	(1 * K) /* Buffer size of touch report for debug data */
...
#define ILI_INFO(fmt, arg...)                                                  \
	({                                                                     \
		pr_info("[TP]ILITEK: [INFO](%s, %d): " fmt, __func__,          \
			__LINE__, ##arg);                                      \
	})

#define ILI_ERR(fmt, arg...)                                                   \
	({                                                                     \
		pr_err("[TP]ILITEK: [ERR](%s, %d): " fmt, __func__, __LINE__,  \
		       ##arg);                                                 \
	})
...
#define ILI_DBG(fmt, arg...)                                                   \
	do {                                                                   \
		if (LEVEL_DEBUG == tp_debug || ili_debug_en)                   \
			pr_info("[TP]ILITEK: [DEBUG](%s, %d): " fmt, __func__, \
				__LINE__, ##arg);                              \
	} while (0)
...
extern int ili_mp_test_main(char *apk, bool lcm_on, struct seq_file *s,
			    char *message);
...
extern int ili_mp_test_handler(char *apk, bool lcm_on, struct seq_file *s,
			       char *message);
