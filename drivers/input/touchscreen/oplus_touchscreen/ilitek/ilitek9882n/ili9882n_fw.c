...
int ili_fw_dump_iram_data(u32 start, u32 end, bool save, bool mcu)
{
	...
	if (save) {
		f = filp_open(DUMP_IRAM_PATH, O_WRONLY | O_CREAT | O_TRUNC,
			      644);
				       ...
				       u32 split_len)
{
	...
	if (split_len != 0) {
		for (addr = start, i = 0; addr < end;
			...
			if (fix_4_alignment) {
				...
				for (j = 0; j < (4 - (split_len % 4)); j++) {
					ilits->update_buf[5 + j + split_len] =
						0xFF;
					ILI_INFO(
						"idev->update_buf[5 + 0x%X] = 0x%X\n",
						j + split_len,
						ilits->update_buf[5 + j +
								  split_len]);
				}
			...
			if (ilits->spi_write_then_read(
				    ilits->spi, ilits->update_buf,
				    split_len + 5, NULL, 0)) {
		...
		if (ilits->spi_write_then_read(ilits->spi, ilits->update_buf,
					       w_len + 5, NULL, 0) < 0) {
			ILI_ERR("Failed to write data via SPI in host download (%x)\n",
				w_len + 5);
			return -EIO;
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		if (fbi[i].mode == mode && fbi[i].len != 0) {
			...
			if (ilitek_tddi_fw_iram_program(
				    fbi[i].mem_start, (fw_ptr + fbi[i].start),
				    fbi[i].len, SPI_UPGRADE_LEN) < 0) {
...
#endif
			crc = CalculateCRC32(fbi[i].start, fbi[i].len - crc_len,
					     fw_ptr);
			...
			ILI_INFO(
				"%s CRC is %s hex(%x) : dma(%x) : iram(%x), calculation len is 0x%x\n",
				fbi[i].name,
				((crc != dma) || (iram_crc_err)) ? "Invalid !" :
								   "Correct !",
...
{
	u32 ges_area_section = 0, ges_info_addr = 0, ges_fw_start = 0,
	    ges_fw_end = 0;
	...
	fbi[AP].mem_start =
		(fbi[AP].fix_mem_start != INT_MAX) ? fbi[AP].fix_mem_start : 0;
	...
	fbi[MP].mem_start =
		(fbi[MP].fix_mem_start != INT_MAX) ? fbi[MP].fix_mem_start : 0;
	fbi[GESTURE].mem_start = (fbi[GESTURE].fix_mem_start != INT_MAX) ?
					 fbi[GESTURE].fix_mem_start :
					 0;
	...
	ILI_INFO(
		"gesture move to ap addr => start = 0x%x, ap_end = 0x%x, ap_len = 0x%x\n",
		fbi[GESTURE].mem_start, ap_end, ap_len);
	ILI_INFO(
		"gesture hex addr => start = 0x%x, gesture_end = 0x%x, gesture_len = 0x%x\n",
	...
	ILI_INFO(
		"report_info_block : nIsI2C = %d, nReserved00 = %d, nReserved01 = %x, nReserved02 = %x,  nReserved03 = %x\n",
...
static int ilitek_tddi_fw_ili_convert(u8 *pfw)
{
	...
	if (!ilits->p_firmware_headfile->firmware_data ||
	    ilits->p_firmware_headfile->firmware_size <= ILI_FILE_HEADER ||
...
int ili_fw_upgrade(int op)
{
	...
	if (ilits->oplus_fw_update || ilits->force_fw_update ||
	    ERR_ALLOC_MEM(pfw)) {
