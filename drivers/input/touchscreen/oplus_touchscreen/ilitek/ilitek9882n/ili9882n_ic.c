...
static u32 ic_sup_list[CHIP_SUP_NUM] = { [0] = ILI9881_CHIP,
					 [1] = ILI7807_CHIP,
					 [2] = ILI9881N_AA,
					 [3] = ILI9881O_AA,
					 [4] = ILI9882_CHIP };
...
void ili_ic_get_pc_counter(int stat)
{
	...
	if (ili_ice_mode_read(ilits->chip->pc_counter_addr, &pc, sizeof(u32)) <
	    0) {
		ILI_ERR("Read pc conter error\n");
	}

	if (ili_ice_mode_read(ilits->chip->pc_latch_addr, &latch, sizeof(u32)) <
	    0) {
...
int ili_ic_check_busy(int count, int delay)
{
	...
	do {
		...
		if (ilits->wrapper(&cmd[1], sizeof(u8), &busy, sizeof(u8), ON,
				   OFF) < 0) {
...
int ili_ic_get_core_ver(void)
{
	...
	do {
		...
		if (ilits->wrapper(&cmd[1], sizeof(u8), buf,
				   ilits->protocol->core_ver_len, ON,
				   OFF) < 0) {
...
out:
	...
	if (ilits->ts->panel_data.manufacture_info.version) {
		...
		if (ilits->ts->panel_data.vid_len == 0) {
			...
			if (ver_len <= 11) {
				snprintf(ilits->ts->panel_data.manufacture_info
							 .version +
						 9,
					 sizeof(dev_version), "%s",
					 dev_version);
			} else {
				strlcpy(&ilits->ts->panel_data.manufacture_info
						 .version[12],
					dev_version, 3);
			}
		} else {
