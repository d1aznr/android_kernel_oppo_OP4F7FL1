...
static void tp_touch_handle(struct touchpanel_data *ts)
{
	...
	if (!ts->enable_point_auto_change) {
		...
		obj_attention =
			ts->ts_ops->get_touch_points_auto(ts->chip_data, points,
							  ts->max_num,
							  &ts->resolution_info);
		if (obj_attention == -EINVAL) {
			TPD_INFO("Invalid points, ignore..\n");
...
static int init_touchpanel_proc(struct touchpanel_data *ts)
{
	...
	if (ts->edge_limit_support || ts->fw_edge_limit_support) {
		...
		if (ts->edge_limit_support) {
			prEntry_tmp =
				proc_create_data("oplus_tp_limit_area", 0664,
						 prEntry_tp,
	...
	if (ts->sensitive_level_array_support) {
		prEntry_tmp =
			proc_create_data("sensitive_level", 0666, prEntry_tp,
	...
	if (ts->pen_support) {
		prEntry_tmp =
			proc_create_data("pencil_connected", 0666, prEntry_tp,
	...
	if (ts->auto_test_need_cal_support) {
		...
		prEntry_tmp =
			proc_create_data("calibration_status", 0666, prEntry_tp,
