...
static int gpio_set_int_level(struct hw_resource *hw_res, bool level)
{
	...
	if (level) {
		ret = pinctrl_select_state(hw_res->pinctrl,
					   hw_res->pin_set_high);
		if (ret)
...
static void gt1x_select_addr(struct i2c_client *client,
...
static int goodix_reset(void *chip_data)
{
	...
	struct chip_data_gt5688 *chip_info =
		(struct chip_data_gt5688 *)chip_data;
...
static s32 gt1x_send_cfg(struct i2c_client *client, u8 *config, int cfg_len)
{
	...
	while (retry++ < 5) {
		ret = touch_i2c_write_block(client, GTP_REG_CONFIG_DATA,
					    cfg_len, config);
		if (ret == cfg_len) {
 ...
 */
static int gt1x_init_panel(struct chip_data_gt5688 *chip_info,
			   cfg_config cfg_config)
{
	...
	case CFG_NOISE:
		...
		chip_info->config_info.gt1x_cfg_length =
			sizeof(GTP_NOISE_CFG_GROUP_OFLIM);
		if (chip_info->tp_type == TP_BIEL) {
			chip_info->config_info.gt1x_config =
				GTP_NOISE_CFG_GROUP_BIEL;
			chip_info->config_info.gt1x_cfg_length =
				sizeof(GTP_NOISE_CFG_GROUP_BIEL);
		} else if (chip_info->tp_type == TP_TRULY) {
			chip_info->config_info.gt1x_config =
				GTP_NOISE_CFG_GROUP_TRULY;
			chip_info->config_info.gt1x_cfg_length =
				sizeof(GTP_NOISE_CFG_GROUP_TRULY);
		} else if (chip_info->tp_type == TP_BOE) {
			chip_info->config_info.gt1x_config =
				GTP_NOISE_CFG_GROUP_BOE;
			chip_info->config_info.gt1x_cfg_length =
				sizeof(GTP_NOISE_CFG_GROUP_BOE);
		}
		break;
	...
	if (chip_info->config_info.gt1x_cfg_length < GTP_CONFIG_MIN_LENGTH ||
	    chip_info->config_info.gt1x_cfg_length > GTP_CONFIG_MAX_LENGTH) {
		TPD_INFO(
	...
	if ((chip_info->config_info.gt1x_abs_x_max == 0) &&
	    (chip_info->config_info.gt1x_abs_y_max == 0)) {
		chip_info->config_info.gt1x_abs_x_max =
			(chip_info->config_info.gt1x_config[RESOLUTION_LOC + 1]
			 << 8) +
			chip_info->config_info.gt1x_config[RESOLUTION_LOC];
		chip_info->config_info.gt1x_abs_y_max =
			(chip_info->config_info.gt1x_config[RESOLUTION_LOC + 3]
			 << 8) +
			chip_info->config_info.gt1x_config[RESOLUTION_LOC + 2];
		chip_info->config_info.gt1x_int_type =
			(chip_info->config_info.gt1x_config[TRIGGER_LOC]) &
			0x03;
		chip_info->config_info.gt1x_wakeup_level = !(
			chip_info->config_info.gt1x_config[MODULE_SWITCH3_LOC] &
			0x20);
	} else {
		chip_info->config_info.gt1x_config[RESOLUTION_LOC] =
			(u8)chip_info->config_info.gt1x_abs_x_max;
		chip_info->config_info.gt1x_config[RESOLUTION_LOC + 1] =
			(u8)(chip_info->config_info.gt1x_abs_x_max >> 8);
		chip_info->config_info.gt1x_config[RESOLUTION_LOC + 2] =
			(u8)chip_info->config_info.gt1x_abs_y_max;
		chip_info->config_info.gt1x_config[RESOLUTION_LOC + 3] =
			(u8)(chip_info->config_info.gt1x_abs_y_max >> 8);
		set_reg_bit(
			chip_info->config_info.gt1x_config[MODULE_SWITCH3_LOC],
			5, !chip_info->config_info.gt1x_wakeup_level);
		chip_info->config_info.gt1x_config[TRIGGER_LOC] =
			(chip_info->config_info.gt1x_config[TRIGGER_LOC] &
			 0xFC) |
			chip_info->config_info.gt1x_int_type;
	...
	ret = gt1x_send_cfg(chip_info->client,
			    chip_info->config_info.gt1x_config,
			    chip_info->config_info.gt1x_cfg_length);
 ...
 */
s32 gt1x_i2c_read_dbl_check(struct i2c_client *client, u16 addr, u8 *buffer,
			    s32 len)
{
	u8 buf[16] = { 0 };
	u8 confirm_buf[16] = { 0 };
...
static int gt1x_i2c_write_with_readback(struct i2c_client *client, u16 addr,
...
static s32 gt1x_read_version(struct chip_data_gt5688 *chip_info)
{
	...
	while (retry--) {
		ret = gt1x_i2c_read_dbl_check(
			chip_info->client, GTP_REG_VERSION, buf, sizeof(buf));
		if (!ret) {
			checksum = 0;
	...
	TPD_INFO(
		"IC VERSION:GT%s(Product)_%06X(Patch)_%04X(Mask)_%02X(SensorID)\n",
...
static int gt1x_recall_check(struct i2c_client *client, u8 *chk_src,
			     u16 start_addr, u16 chk_length)
{
	u8 rd_buf[PACK_SIZE];
	s32 ret = 0;
	u16 len = 0;
	...
	while (chk_length > 0) {
		...
		ret = touch_i2c_read_block(client, start_addr + compared_length,
					   len, rd_buf);
		if (ret < 0) {
 ...
 */
static u8 *gt1x_get_fw_data(struct chip_data_gt5688 *chip_info, u32 offset,
			    int length)
{
...
static int __gt1x_hold_ss51_dsp_20(struct i2c_client *client)
{
	...
	while (retry++ < 30) {
		// Hold ss51 & dsp
		buf[0] = 0x0C;
		ret = touch_i2c_write_byte(client, _rRW_MISCTL__SWRST_B0_,
					   buf[0]);
		if (ret < 0) {
...
static int gt1x_hold_ss51_dsp(struct i2c_client *client)
{
	...
	buffer[0] = 0x00;
	ret = gt1x_i2c_write_with_readback(client, _bRW_MISCTL__DSP_MCU_PWR_,
					   buffer, 1);
	if (ret) {
	...
	// disable watchdog
	buffer[0] = 0x00;
	ret = gt1x_i2c_write_with_readback(client, _bRW_MISCTL__TMR0_EN, buffer,
					   1);
	if (ret) {
	...
	// clear cache
	buffer[0] = 0x00;
	ret = gt1x_i2c_write_with_readback(client, _bRW_MISCTL__CACHE_EN,
					   buffer, 1);
	if (ret) {
	...
	// soft reset
	buffer[0] = 0x01;
	ret = touch_i2c_write_block(client, _bWO_MISCTL__CPU_SWRST_PULSE, 1,
				    buffer);
	if (ret < 0) {
	...
	// set scramble
	buffer[0] = 0x00;
	ret = gt1x_i2c_write_with_readback(client, _rRW_MISCTL__BOOT_OPT_B0_,
					   buffer, 1);
	if (ret) {
...
static int gt1x_run_ss51_isp(struct i2c_client *client, u8 *ss51_isp,
			     int length)
{
	int ret;
	u8 buffer[10];
	...
	// select bank4
	buffer[0] = 0x04;
	ret = gt1x_i2c_write_with_readback(client, _bRW_MISCTL__SRAM_BANK,
					   buffer, 1);
	if (ret) {
	...
	buffer[0] = 0x01;
	ret = gt1x_i2c_write_with_readback(client, _bRW_MISCTL__PATCH_AREA_EN_,
					   buffer, 1);
	if (ret) {
	...
	// release ss51
	buffer[0] = 0x08;
	ret = gt1x_i2c_write_with_readback(client, _rRW_MISCTL__SWRST_B0_,
					   buffer, 1);
	if (ret) {
...
static int gt1x_error_erase(struct chip_data_gt5688 *chip_info)
{
	...
	while (retry-- > 0) {
		...
		ret = gt1x_i2c_write_with_readback(chip_info->client, 0x8100,
						   buffer, 4);
		if (ret) {
		...
		ret = gt1x_recall_check(chip_info->client, fw, 0x8100 + 4,
					block_len);
		if (ret) {
			continue;
		}
		...
		buffer[0] = 0;
		ret = gt1x_i2c_write_with_readback(chip_info->client, 0x8022,
						   buffer, 1);
		if (ret) {
		...
		ret = gt1x_i2c_write_with_readback(chip_info->client, 0x8020,
						   buffer, 2);
		if (ret) {
		...
		while (wait_time > 0) {
			wait_time--;
			msleep(5);
			ret = gt1x_i2c_read_dbl_check(chip_info->client, 0x8022,
						      buffer, 1);
			if (ret) {
				continue;
			}
...
static int gt1x_burn_subsystem(struct chip_data_gt5688 *chip_info,
			       struct fw_subsystem_info *subsystem)
{
	...
	while (length > 0 && retry > 0) {
		...
		fw = gt1x_get_fw_data(chip_info, subsystem->offset + burn_len,
				      block_len);
		if (fw == NULL) {
			return -1;
		}
...
static int gt1x_check_subsystem_in_flash(struct chip_data_gt5688 *chip_info,
					 struct fw_subsystem_info *subsystem)
{
	int block_len;
	int checked_len = 0;
	...
	while (length > 0) {
		...
		ret = gt1x_read_flash(chip_info->client,
		...
		ret = gt1x_recall_check(chip_info->client, fw, 0x8100,
					block_len);
		if (ret) {
...
s32 gt1x_request_event_handler(struct chip_data_gt5688 *chip_info)
{
	...
	u8 rqst_data = 0;
	u8 noise_data = 0;
	ret = touch_i2c_read_block(chip_info->client, GTP_REG_RQST, 1,
				   &rqst_data);
	if (ret < 0) {
	...
	case GTP_RQST_CONFIG:
		...
		if (ret) {
			...
			ret = touch_i2c_write_block(
				chip_info->client, GTP_REG_RQST, 1, &rqst_data);
		}
		break;
	...
	case GTP_RQST_RESET:
		...
		//  gt1x_reset_guitar();
		rqst_data = GTP_RQST_RESPONDED;
		ret = touch_i2c_write_block(chip_info->client, GTP_REG_RQST, 1,
					    &rqst_data);
		break;
	...
	//noise
	ret = touch_i2c_read_block(chip_info->client, GTP_NOISE_DETECT_REG, 1,
				   &noise_data);
	if (ret < 0) {
		TPD_INFO("I2C transfer error. errno:%d\n", ret);
		return -1;
...
static int goodix_get_vendor(void *chip_data, struct panel_info *panel_data)
{
	...
	chip_info->config_info.config_version =
				      ...
				      struct panel_info *panel_data)
{
	...
	do {
		gt1x_i2c_read_dbl_check(chip_info->client,
					GTP_REG_FW_CHK_MAINSYS, reg_val, 1);
		gt1x_i2c_read_dbl_check(chip_info->client,
					GTP_REG_FW_CHK_SUBSYS, &reg_val[1], 1);
	...
	if (panel_data->manufacture_info.version)
		strlcpy(&panel_data->manufacture_info.version[8], fw_id,
				...
				int is_suspended)
{
     ...
     * keycode + checksum
     */
	ret = touch_i2c_read_block(chip_info->client, (GTP_READ_COOR_ADDR + 1),
				   8 * touch_num + 2,
				   &chip_info->touch_data[1]);
	if (ret < 0) {
	...
	} else if ((chip_info->touch_data[0] & 0x10) &&
		   (chip_info->vk_status & 0x0F)) {
	...
	if (CHK_BIT(cur_event, IRQ_TOUCH) ||
	    CHK_BIT(trigger_event, IRQ_TOUCH)) {
		SET_BIT(result_event, IRQ_TOUCH);
	}

	if ((chip_info->touch_data[0] & 0x10) ||
	    CHK_BIT(trigger_event, IRQ_BTN_KEY)) {
	...
	TPD_DEBUG("point data is 0x%2x %hhu, vk_status:%hhu\n", ret,
		  result_event, chip_info->vk_status);
				   ...
				   struct gesture_info *gesture)
{
	...
	if (doze_buf[0] != 0) {
		...
		ret = touch_i2c_read_block(client,
		...
		case DTAP_DETECT:
			...
			gesture->Point_start.x = coordinate_single[0] |
						 (coordinate_single[1] << 8);
			gesture->Point_start.y = coordinate_single[2] |
						 (coordinate_single[3] << 8);
			gesture->Point_end.x = coordinate_single[8] |
					       (coordinate_single[9] << 8);
			gesture->Point_end.y = coordinate_single[10] |
					       (coordinate_single[11] << 8);
		...
		case UP_VEE_DETECT:
			...
			gesture->Point_end.y = coordinate_single[10] |
					       (coordinate_single[11] << 8);
			gesture->Point_1st.x = coordinate_single[4] |
					       (coordinate_single[5] << 8);
			gesture->Point_1st.y = coordinate_single[6] |
					       (coordinate_single[7] << 8);
			break;
		...
		case RIGHT_VEE_DETECT:
			...
			gesture->Point_start.y = coordinate_single[2] |
						 (coordinate_single[3] << 8);
			gesture->Point_end.x = coordinate_single[16] |
					       (coordinate_single[17] << 8);
			gesture->Point_end.y = coordinate_single[18] |
					       (coordinate_single[19] << 8);
			gesture->Point_1st.x = coordinate_single[8] |
					       (coordinate_single[9] << 8);
			gesture->Point_1st.y = coordinate_single[10] |
					       (coordinate_single[11] << 8);
			break;
		...
		case CIRCLE_DETECT:
			...
			for (i = 0; i < coordinate_size; i++) {
				Point_input[i].x =
					coordinate_single[j] |
					(coordinate_single[j + 1] << 8);
				Point_input[i].y =
					coordinate_single[j + 2] |
					(coordinate_single[j + 3] << 8);
				j = j + 4;
				//TPD_INFO("Point_input[%d].x = %d, Point_input[%d].y = %d\n", i, Point_input[i].x, i, Point_input[i].y);
			}

			gesture->clockwise =
				ClockWise(&Point_input[0], coordinate_size - 2);
			GetCirclePoints(&Point_input[0], coordinate_size,
					Point_output);
			gesture->Point_start.x = Point_input[0].x;
			gesture->Point_start.y = Point_input[0].y;
			gesture->Point_end.x =
				Point_input[coordinate_size - 1].x;
			gesture->Point_end.y =
				Point_input[coordinate_size - 1].y;
		...
		case DOUSWIP_DETECT:
			...
			gesture->Point_start.y = coordinate_single[2] |
						 (coordinate_single[3] << 8);
			gesture->Point_end.x = coordinate_single[4] |
					       (coordinate_single[5] << 8);
			gesture->Point_end.y = coordinate_single[6] |
					       (coordinate_single[7] << 8);
			gesture->Point_1st.x = coordinate_single[8] |
					       (coordinate_single[9] << 8);
			gesture->Point_1st.y = coordinate_single[10] |
					       (coordinate_single[11] << 8);
			gesture->Point_2nd.x = coordinate_single[12] |
					       (coordinate_single[13] << 8);
			gesture->Point_2nd.y = coordinate_single[14] |
					       (coordinate_single[15] << 8);
			break;
		...
		case M_DETECT:
			...
			gesture->Point_1st.y = coordinate_single[6] |
					       (coordinate_single[7] << 8);
			gesture->Point_2nd.x = coordinate_single[8] |
					       (coordinate_single[9] << 8);
			gesture->Point_2nd.y = coordinate_single[10] |
					       (coordinate_single[11] << 8);
			gesture->Point_3rd.x = coordinate_single[12] |
					       (coordinate_single[13] << 8);
			gesture->Point_3rd.y = coordinate_single[14] |
					       (coordinate_single[15] << 8);
			break;
		...
		TPD_INFO(
			"%s, gesture_sign = 0x%x, gesture_type = %d | %d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d:%d,%d\n",
...
static int goodix_enable_edge_limit(struct chip_data_gt5688 *chip_info,
				    bool enable)
{
	int ret = -1;
...
static int goodix_set_charger_state(struct chip_data_gt5688 *chip_info,
				    bool enable)
{
	int ret = -1;
...
static int goodix_esd_handle(void *chip_data)
{
	...
	for (i = 0; i < 3; i++) {
		ret = touch_i2c_read_block(chip_info->client, GTP_REG_CMD, 4,
					   esd_buf);
		TPD_DEBUG(
			"[Esd]0x8040 = 0x%02X, 0x8043 = 0x%02X gt1x_rawdiff_mode = %d\n",
				   ...
				   debug_type debug_type)
{
	...
	//wait for data ready
	while (i++ < 10) {
		ret = touch_i2c_read_block(client, GTP_READ_COOR_ADDR, 1,
					   kernel_buf);
		TPD_INFO("ret = %d \t kernel_buf = %d\n", ret, kernel_buf[0]);
	...
	ret = touch_i2c_read_block(client, addr, TX_NUM * RX_NUM * 2,
				   kernel_buf);
	msleep(5);
	...
	for (i = 0; i < RX_NUM; i++) {
		...
		for (j = 0; j < TX_NUM; j++) {
			...
			seq_printf(
				s, "%4d ",
				(s16)(((s16)kernel_buf[i * TX_NUM * 2 + j * 2]
				       << 8) +
				      kernel_buf[i * TX_NUM * 2 + j * 2 + 1]));
		}
		//printk("\n");
		seq_printf(s, "\n");
	}
...
static void goodix_config_info_read(struct seq_file *s, void *chip_data)
{
	...
	for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
		seq_printf(s, "0x%02X, ",
			   chip_info->config_info.gt1x_config[i]);
		if (i % 10 == 9)
			seq_printf(s, "\n");
	}
	...
	ret = touch_i2c_read_block(client, GTP_REG_CONFIG_DATA,
	...
	for (i = RX_NUM - SPURIOUS_FP_RX_NUM; i < RX_NUM - 1; i++) {
		...
		for (j = 1; j < TX_NUM - 1; j++) {
			raw_data_get =
				...
				raw_data[i * TX_NUM * 2 + j * 2 + 1];
			raw_data_base =
				chip_info->spuri_fp_touch_raw_data[i * TX_NUM +
								   j];
			delta_data = raw_data_get - raw_data_base;
			printk("%d ", delta_data);
			if (delta_data + 90 < 0) {
				TPD_INFO(
					"delta_data too large, delta_data = %d\n",
					delta_data);
				error_count++;
			}
		}
			 ...
			 const struct i2c_device_id *id)
{
	...
	ts->tp_suspend_order =
		TP_LCD_SUSPEND; /* for oncell panel, set tp suspend first */
	//step6: debug tools
...
static const struct i2c_device_id gt1x_ts_id[] = { { GTP_I2C_NAME, 0 }, {} };
