...
#define DOZE_CLOSE_OK_FLAG 0xBB
	...
	if (enable) {
		...
		if (doze_mode_set_count == 0) {
			...
			for (i = 0; i < 3; i++) {
				if (touch_i2c_write_block(client,
							  GTP_REG_DOZE_CTRL, 1,
							  &w_data) >= 0) {
					result = 0;
					goto exit;
		...
		if (doze_mode_set_count == 1) {
			...
			if (touch_i2c_write_block(client, GTP_REG_DOZE_CTRL, 1,
						  &w_data) < 0) {
				TPD_INFO(
					"doze mode comunition disable FAILED\n");
				goto exit;
			}
			usleep_range(8000, 8100);
			for (i = 0; i < 20; i++) {
				if (touch_i2c_read_block(client,
							 GTP_REG_DOZE_STAT, 1,
							 &r_data) < 0) {
					TPD_INFO(
						"doze mode comunition disable FAILED\n");
				...
				if (DOZE_CLOSE_OK_FLAG == r_data) {
					...
					if (touch_i2c_write_block(
						    client, GTP_REG_DOZE_CTRL,
						    1, &w_data) < 0) {
						TPD_INFO(
							"doze mode comunition disable FAILED, 0x%x!=0x%x\n",
							r_data,
							DOZE_DISABLE_FLAG);
						goto exit;
					}
				...
				if (retry_times > 5) {
					TPD_INFO(
						"doze mode wait flag timeout,should clear int\n");
					w_data = 0;
					touch_i2c_write_block(client, 0x4100, 1,
							      &w_data);
				}
...
{
	s32 ret = -1;
	struct chip_data_gt9886 *chip_info =
		(struct chip_data_gt9886 *)chip_data;
	...
	ret = touch_i2c_write_block(chip_info->client,
				    chip_info->reg_info.GTP_REG_CMD, 3,
				    &buffer[0]);
	if (ret < 0) {
	...
	while (retry--) {
		ret = touch_i2c_read_block(chip_info->client,
					   chip_info->reg_info.GTP_REG_CMD, 1,
					   &buf_read_back);
		if (ret < 0) {
			usleep_range(10000, 11000);
		...
		if (buf_read_back != cmd || cmd == TS_CMD_REG_READY) {
			...
			TPD_DEBUG(
				"%s: cmd isn't been handle, wait for it at: %d, readbak: 0x%x\n",
		...
		if (retry_times > 5) {
			TPD_INFO("%s: wait cmd flag failed, clear int\n",
				 __func__);
			buf_read_back = 0;
			touch_i2c_write_block(chip_info->client, 0x4100, 1,
					      &buf_read_back);
			chip_info->send_cmd_err_count++;
			tp_healthinfo_report(chip_info->monitor_data_v2,
					     HEALTH_REPORT, "SENT_CMD_ERR");
		}
		usleep_range(10000, 11000);
	}
	if (retry) {
...
static int goodix_enable_gesture(struct chip_data_gt9886 *chip_info,
				 bool enable)
{
	int ret = -1;
				    ...
				    int state)
{
	...
	if (state == 1 || VERTICAL_SCREEN == chip_info->touch_direction) {
		ret = goodix_send_cmd(chip_info, GTM_CMD_EDGE_LIMIT_VERTICAL,
				      0x00);
	} else {
		if (LANDSCAPE_SCREEN_90 == chip_info->touch_direction) {
			ret = goodix_send_cmd(
				chip_info, GTM_CMD_EDGE_LIMIT_LANDSCAPE, 0x00);
		} else if (LANDSCAPE_SCREEN_270 == chip_info->touch_direction) {
			ret = goodix_send_cmd(
				chip_info, GTM_CMD_EDGE_LIMIT_LANDSCAPE, 0x01);
		}
	}
...
static int goodix_reset_esd_status(struct chip_data_gt9886 *chip_info)
{
	...
	ret = gt9886_i2c_write(chip_info->client,
			       chip_info->reg_info.GTP_REG_ESD_WRITE, 1,
			       &value);
	if (ret < 0) {
			       ...
			       struct panel_info *panel_data)
{
	...
	while (retry--) {
		ret = gt9886_i2c_read(chip_info->client,
				      chip_info->reg_info.GTP_REG_PRODUCT_VER,
				      sizeof(buf), buf);
		if (ret > 0) {
			checksum = 0;
			...
			if (checksum ==
				    0 && /* first 3 bytes must be number or char */
			    IS_NUM_OR_CHAR(buf[9]) &&
			    IS_NUM_OR_CHAR(buf[10]) &&
			    IS_NUM_OR_CHAR(buf[11]) &&
	...
	patch_id = (u32)((buf[17] << 24) | (buf[18] << 16) | buf[19] << 8 |
			 buf[20]);
	...
	TPD_INFO(
		"goodix fw VERSION:GT%s(Product)_%08X(Patch)_%06X(MaskID)_%02X(SensorID)\n",
...
static int goodix_check_cfg_valid(u8 *cfg, int length)
{
	...
	for (j = 0; j < bag_num; j++) {
		if (bag_start >= length - 1) {
			TPD_INFO("ERROR, overflow!!bag_start:%d, cfg_len:%d",
				 bag_start, length);
			ret = -EINVAL;
			goto exit;
		}
		...
		for (i = bag_start; i < bag_end; i++)
			checksum += cfg[i];
		if (checksum != 0) {
			TPD_INFO("cfg INVALID, bag:%d checksum ERROR:0x%02x", j,
				 checksum);
			ret = -EINVAL;
			goto exit;
		}
				     ...
				     u8 right_cmd, u8 send_cmd)
{
	...
	for (try_times = 0; try_times < TS_WAIT_CFG_READY_RETRY_TIMES;
	     try_times++) {
		if (gt9886_i2c_read(chip_info->client, command_reg, 3,
				    cmd_buf) < 0) {
...
RETRY:
	...
	for (try_times = 0; try_times < TS_WAIT_CMD_FREE_RETRY_TIMES;
	     try_times++) {
		if (gt9886_i2c_read(chip_info->client, command_reg, 1, &buf) >=
			    0 &&
	...
	if (try_times >= TS_WAIT_CMD_FREE_RETRY_TIMES) {
		...
		if (count == 1) {
			count = 0;
			buf = 0x00;
			touch_i2c_write_block(chip_info->client, GTP_REG_COOR,
					      1, &buf);
			goto RETRY;
		}
		r = -EINVAL;
	...
	if (goodix_send_cmd(chip_info, COMMAND_START_SEND_LARGE_CFG, 0)) {
		TPD_INFO(
			"Send large cfg FAILED, send COMMAND_START_SEND_LARGE_CFG ERROR");
		r = -EINVAL;
		goto exit;
	}
	...
	if (goodix_wait_cfg_cmd_ready(chip_info, COMMAND_SEND_CFG_PREPARE_OK,
		...
		TPD_INFO("Send large cfg FAILED, reg:0x%04x is not 0x82",
			 command_reg);
		r = -EINVAL;
		goto exit;
	}
...
static s32 goodix_request_event_handler(struct chip_data_gt9886 *chip_info)
{
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.GTP_REG_RQST, 1,
				   &rqst_data);
	if (ret < 0) {
				      ...
				      struct panel_info *panel_data)
{
	...
	do {
		goodix_i2c_read_dbl_check(
			chip_info->client,
			chip_info->reg_info.GTP_REG_FW_CHK_MAINSYS, reg_val, 1);

		if (reg_val[0] != 0xFC) {
			TPD_INFO(
				"Check fw status failed: reg[0x21e4] = 0x%2X!\n",
				reg_val[0]);
		} else {
			break;
		}
...
static int goodix_parse_firmware(struct firmware_data *fw_data)
{
	...
	if (fw_info->subsys_num > FW_SUBSYS_MAX_NUM) {
		TPD_INFO("Bad firmware, invalid subsys num: %d",
			 fw_info->subsys_num);
		r = -EINVAL;
		goto err_size;
	}
	...
	TPD_INFO("Fimware VID:%02X%02X%02X%02X", fw_info->fw_vid[0],
...
#define VID_LEN 4
    ...
    * incorrect,so we need to update firmware*/
	...
	TPD_INFO("PID:%02x %02x %02x %02x", buffer[0], buffer[1], buffer[2],
		 buffer[3]);

	if (memcmp(buffer, fw_info->fw_pid, PID_LEN)) {
		TPD_INFO(
			"Product is not match,may header's fw is other product's");
		goto need_update;
	}
	...
	TPD_INFO("VID:%02x %02x %02x %02x", buffer[0], buffer[1], buffer[2],
		 buffer[3]);
			   ...
			   struct firmware_data *fw_data)
{
	...
	TPD_DEBUG("Success select bank0, Set 0x%x -->0x01",
		  HW_REG_ACCESS_PATCH0);

	r = goodix_reg_write_confirm(client, HW_REG_ISP_ADDR,
	...
	for (i = 0; i < TS_CHECK_ISP_STATE_RETRY_TIMES; i++) {
		r = touch_i2c_read_block(client, HW_REG_ISP_RUN_FLAG, 2,
					 reg_val);
				 ...
				 struct fw_update_ctrl *fwu_ctrl)
{
	...
	do {
		reg_val[0] = 0x24;
		r = goodix_reg_write_confirm(chip_info->client, HW_REG_CPU_CTRL,
					     reg_val, 1);
		if (r < 0) {
	...
	reg_val[0] = 0x00;
	r = goodix_reg_write_confirm(chip_info->client, HW_REG_DSP_MCU_POWER,
				     reg_val, 1);
	if (r < 0) {
	...
	reg_val[0] = 0x95;
	r = touch_i2c_write_block(chip_info->client, HW_REG_ESD_KEY, 1,
				  reg_val);
	reg_val[0] = 0x00;
	r |= touch_i2c_write_block(chip_info->client, HW_REG_WTD_TIMER, 1,
				   reg_val);
	...
	/* set scramble */
	reg_val[0] = 0x00;
	r = touch_i2c_write_block(chip_info->client, HW_REG_SCRAMBLE, 1,
				  reg_val);
	if (r < 0) {
 ...
 * return 0 ok, <0 error
 */
static int goodix_send_fw_packet(struct i2c_client *client, u8 type, u8 *pkt,
				 u32 len)
{
	u8 reg_val[4];
	int r, i;
	...
	total_size = len;
	while (total_size > 0) {
		data_size = total_size > I2C_DATA_MAX_BUFFERSIZE ?
				    I2C_DATA_MAX_BUFFERSIZE :
				    total_size;
		TPD_INFO("I2C firmware to %08x,size:%u bytes all len:%u bytes",
			 offset, data_size, len);
		r = goodix_reg_write_confirm(client, HW_REG_ISP_BUFFER + offset,
					     &pkt[offset], data_size);
		if (r < 0) {
	...
	for (i = 0; i < TS_READ_FLASH_STATE_RETRY_TIMES; i++) {
		...
		if (reg_val[0] == ISP_FLASH_SUCCESS &&
			...
			r = touch_i2c_read_block(client, HW_REG_FLASH_FLAG, 2,
						 reg_val);
	...
	TPD_INFO("Wait for flash end timeout, 0x6022= %x %x", reg_val[0],
		 reg_val[1]);
	return -EAGAIN;
}
				  ...
				  struct fw_subsys_info *subsys)
{
	...
	while (total_size > 0) {
		...
		for (i = 0; i < 3; i++) {
			r = goodix_send_fw_packet(client, subsys->type,
				...
				struct fw_update_ctrl *fwu_ctrl)
{
	...
	/* hold ss51 */
	reg_val[0] = 0x24;
	r = touch_i2c_write_block(chip_info->client, HW_REG_CPU_CTRL, 1,
				  reg_val);
	if (r < 0)
	...
	r = touch_i2c_write_block(chip_info->client, HW_REG_CPU_RUN_FROM, 8,
				  reg_val);
	if (r < 0) {
			       ...
			       u8 *cfg, int *cfg_len, u8 sid)
{
	...
	if (cfgPackageLen > cfg_bin->size) {
		TPD_INFO(
			"%s:Bad firmware!,cfg package len:%d,firmware size:%d\n",
	...
	if ((checksum) != cfg_checksum) {
		TPD_INFO(
			"%s:Bad firmware!(checksum: 0x%04X, header define: 0x%04X)\n",
	...
	cfg_offset = CFG_HEAD_BYTES +
		     bin_group_num * bin_cfg_num * CFG_INFO_BLOCK_BYTES;
	for (i = 0; i < bin_group_num * bin_cfg_num; i++) {
		/* find cfg's sid in cfg.bin */
		one_cfg_count =
			getU16(&cfg_bin->data[CFG_HEAD_BYTES + 2 +
					      i * CFG_INFO_BLOCK_BYTES]);
		if (sid ==
		    (cfg_bin->data[CFG_HEAD_BYTES + i * CFG_INFO_BLOCK_BYTES])) {
			sid_is_exist = GTX8_EXIST;
			if (config_status ==
			    (cfg_bin->data[CFG_HEAD_BYTES + 1 +
					   i * CFG_INFO_BLOCK_BYTES])) {
				memcpy(cfg, &cfg_bin->data[cfg_offset],
				       one_cfg_count);
				*cfg_len = one_cfg_count;
				TPD_DEBUG(
					"%s:one_cfg_count = %d, cfg_data1 = 0x%02x, cfg_data2 = 0x%02x\n",
					__func__, one_cfg_count, cfg[0],
					cfg[1]);
				break;
			}
		}
...
static int gtx8_get_cfg_data(void *chip_data_info,
			     const struct firmware *cfg_bin, char *config_name,
			     struct gtx8_ts_config *config)
{
	struct chip_data_gt9886 *chip_info =
		(struct chip_data_gt9886 *)chip_data_info;
	u8 *cfg_data = NULL;
	int cfg_len = 0;
	int ret = NO_ERR;
	...
	/* parse config data */
	ret = gtx8_parse_cfg_data(cfg_bin, config_name, cfg_data, &cfg_len,
			      ...
			      const struct firmware *firmware)
{
	...
	if (ret < 0) {
		...
		TPD_INFO(
			"%s: Failed to parse noise_test_config data,use normal_config data\n",
			__func__);
	} else {
			     ...
			     struct firmware *fw_firmware)
{
	...
	if (!(firmware->data[cfgPackageLen + 0] == 'G' &&
	      firmware->data[cfgPackageLen + 1] == 'X' &&
	      firmware->data[cfgPackageLen + 2] == 'F' &&
	      firmware->data[cfgPackageLen + 3] == 'W')) {
		TPD_INFO("%s can't find fw package\n", __func__);
		TPD_INFO("Data type:%c %c %c %c,dest type is:GXFW\n",
			 firmware->data[cfgPackageLen + 0],
			 firmware->data[cfgPackageLen + 1],
			 firmware->data[cfgPackageLen + 2],
	...
	if ((fwPackageLen + 16 + cfgPackageLen) > firmware->size) {
		TPD_INFO(
			"%s bad firmware,need len:%d,actual firmware size:%d\n",
			__func__, fwPackageLen + 16 + cfgPackageLen,
			(int)firmware->size);
		ret = -1;
		goto exit;
	}
	...
	TPD_DEBUG("fw tail info:%*ph\n", 4,
		  &fw_firmware->data[fwPackageLen - 4 - 1]);
	ret = 0;
...
static fw_update_state goodix_fw_update(void *chip_data,
					const struct firmware *cfg_fw_firmware,
					bool force)
{
#define FW_UPDATE_RETRY 2
	...
	if (force == false) {
		r = goodix_check_update(chip_info->client,
...
start_update:
	...
	if ((r == -EBUS || r == -5) && --retry0 > 0) {
		TPD_INFO("Bus error, retry prepare ISP:%d",
	...
	if ((r == -EBUS || r == -ETIMEOUT) && --retry1 > 0) {
         ...
         * flashing
         */
		TPD_INFO("Bus error, retry firmware update:%d",
...
static u32 goodix_u32_trigger_reason(void *chip_data, int gesture_enable,
				     int is_suspended)
{
	int ret = -1;
	u8 touch_num = 0;
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.GTP_REG_READ_COOR,
	...
	if ((chip_info->touch_data[0] & GOODIX_TOUCH_EVENT) ==
	    GOODIX_TOUCH_EVENT) {
		touch_num = chip_info->touch_data[1] & 0x0F;
		touch_num =
			touch_num < MAX_POINT_NUM ? touch_num : MAX_POINT_NUM;
		if (chip_info->kernel_grip_support) {
			if (touch_num > 0) {
				ret = touch_i2c_read_block(
					chip_info->client,
					chip_info->reg_info.GTP_REG_EDGE_INFO,
					BYTES_PER_EDGE * touch_num,
					chip_info->edge_data);
				if (ret < 0) {
					TPD_INFO("%s: i2c transfer error!\n",
						 __func__);
		...
		if (touch_num > 1) {
			...
			if (ret < 0) {
				TPD_INFO(
					"read touch point data from coor_addr failed!\n");
				goto IGNORE_CLEAR_IRQ;
			}
		}

		check_sum = 0;
		for (i = 0; i < 2 + BYTES_PER_COORD * touch_num + 2;
		     i++) { //do checksum
	...
	} else if (unlikely(((chip_info->touch_data[0] &
			      GOODIX_GESTURE_EVENT) != GOODIX_GESTURE_EVENT) &&
			    ((chip_info->touch_data[0] &
		...
		if ((check_sum && chip_info->touch_data[0] == 0) || all_zero) {
			...
			TPD_INFO(
				"irq checksum is invalid for first 12 bytes\n");
			goto IGNORE_CLEAR_IRQ;
		}
	}
	...
	if (unlikely((chip_info->touch_data[0] &
		      (GOODIX_TOUCH_EVENT | GOODIX_FINGER_IDLE_EVENT)) ==
		     GOODIX_FINGER_IDLE_EVENT)) {
				   ...
				   int max_num)
{
	...
	for (i = 0; i < max_num; i++) {
		if (i == id) {
			...
			if (coor_data[0] & 0x10) {
				chip_info->fp_coor_report.fp_x_coor =
					points[i].x;
				chip_info->fp_coor_report.fp_y_coor =
					points[i].y;
				chip_info->fp_coor_report.fp_area =
					coor_data[7];
			}
				   ...
				   struct gesture_info *gesture)
{
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.GTP_REG_WAKEUP_GESTURE,
				   sizeof(doze_buf), doze_buf);
	if (ret < 0) {
	...
	if (gesture_id == 0 || ((doze_buf[0] & GOODIX_GESTURE_EVENT) ==
				0)) { //no gesture type, no need handle
	...
	case FP_DOWN_DETECT:
		...
		chip_info->fp_coor_report.fp_x_coor =
			(doze_buf[4] & 0xFF) | (doze_buf[5] & 0x0F) << 8;
		chip_info->fp_coor_report.fp_y_coor =
			(doze_buf[6] & 0xFF) | (doze_buf[7] & 0x0F) << 8;
		chip_info->fp_coor_report.fp_area =
			(doze_buf[12] & 0xFF) | (doze_buf[13] & 0x0F) << 8;
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.GTP_REG_GESTURE_COOR,
				   point_num * 4 + 1,
	...
	case RIGHT_SLIDE_DETECT:
		...
		gesture->Point_start.x =
			(point_data[0] & 0xFF) | (point_data[1] & 0x0F) << 8;
		gesture->Point_start.y =
			(point_data[2] & 0xFF) | (point_data[3] & 0x0F) << 8;
		gesture->Point_end.x =
	...
	case DOWN_VEE_DETECT:
		...
		getSpecialCornerPoint(&point_data[0], point_num,
				      &limitPoint[0]);
		gesture->Point_start.x =
			(point_data[0] & 0xFF) | (point_data[1] & 0x0F) << 8;
		gesture->Point_start.y =
...
REPORT_GESTURE:
	TPD_INFO(
		"%s: gesture_id = 0x%x, gesture_type = %d, clockWise = %d, point:(%d %d)(%d %d)(%d %d)(%d %d)(%d %d)(%d %d)\n",
				   ...
				   struct monitor_data *mon_data)
{
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.GTP_REG_DEBUG,
				   sizeof(struct goodix_health_info),
	...
	if (health_info->shield_freq != 0) {
		...
		if (tp_debug != 0) {
			TPD_INFO("%s: freq before: %d HZ, freq after: %d HZ\n",
				 __func__, health_info->freq_before,
				 health_info->freq_after);
		}
	}
	...
	ret = touch_i2c_write_block(chip_info->client,
				    chip_info->reg_info.GTP_REG_DEBUG, 1,
				    &clear_flag);
	if (ret < 0) {
...
static void goodix_get_health_info_v2(void *chip_data,
				      struct monitor_data_v2 *mon_data_v2)
{
	struct chip_data_gt9886 *chip_info =
		(struct chip_data_gt9886 *)chip_data;
	struct goodix_health_info *health_info;
	...
	if (health_info->fw_rst != 0) {
		...
		case RST_MAIN_REG:
			tp_healthinfo_report(mon_data_v2, HEALTH_REPORT,
					     HEALTH_REPORT_RST_HARD);
			break;
		case RST_OVERLAY_ERROR:
			tp_healthinfo_report(mon_data_v2, HEALTH_REPORT,
					     HEALTH_REPORT_RST_INST);
			break;
		case RST_LOAD_OVERLAY:
			tp_healthinfo_report(mon_data_v2, HEALTH_REPORT,
					     HEALTH_REPORT_RST_PARITY);
			break;
		case RST_CHECK_PID:
			tp_healthinfo_report(mon_data_v2, HEALTH_REPORT,
					     HEALTH_REPORT_RST_WD);
			break;
		case RST_CHECK_RAM:
			tp_healthinfo_report(mon_data_v2, HEALTH_REPORT,
					     HEALTH_REPORT_RST_OTHER);
			break;
		case RST_CHECK_RAWDATA:
			tp_healthinfo_report(mon_data_v2, HEALTH_REPORT,
					     HEALTH_REPORT_RST_OTHER);
...
static int goodix_esd_handle(void *chip_data) //Jarvis:have not finished
{
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.GTP_REG_ESD_READ, 1,
				   &esd_buf);
	if ((ret < 0) || esd_buf == 0xAA) {
		...
		tp_healthinfo_report(chip_info->monitor_data_v2, HEALTH_REPORT,
				     HEALTH_REPORT_SHIELD_ESD);
		return -1;
	} else {
		esd_buf = 0xAA;
		ret = touch_i2c_write_block(
...
static void
goodix_screenon_fingerprint_info(void *chip_data,
				   ...
				   debug_type debug_type)
{
	...
	kernel_buf =
		kzalloc(TX_NUM * RX_NUM * 2 > PAGE_SIZE ? TX_NUM * RX_NUM * 2 :
							  PAGE_SIZE,
			GFP_KERNEL);
	if (kernel_buf == NULL) {
	...
	msleep(20);
	gt9886_i2c_write(chip_info->client,
			 chip_info->reg_info.GTP_REG_READ_COOR, 1,
			 &clear_state);
	...
	ret = gt9886_i2c_read(chip_info->client, addr, TX_NUM * RX_NUM * 2,
			      kernel_buf);
	msleep(5);
	...
	for (i = 0; i < RX_NUM; i++) {
		seq_printf(s, "[%2d] ", i);
		for (j = 0; j < TX_NUM; j++) {
			data = (kernel_buf[j * RX_NUM * 2 + i * 2] << 8) +
			       kernel_buf[j * RX_NUM * 2 + i * 2 + 1];
...
static void goodix_detailed_debug_info_read(struct seq_file *s, void *chip_data,
					    debug_type debug_type)
{
	...
	log_buf = 0xAA;
	gt9886_i2c_write(chip_info->client,
			 chip_info->reg_info.GTP_REG_DETAILED_DEBUG_INFO, 1,
			 &log_buf);
	msleep(10);
	for (i = 0; i < 3; i++) {
		ret = gt9886_i2c_read(
	...
	for (read_data_times = 0; read_data_times < 5; read_data_times++) {
		...
		ret = gt9886_i2c_read(chip_info->client, addr,
				      TX_NUM * RX_NUM * 2, kernel_buf);
		msleep(5);
...
static void goodix_down_diff_info_read(struct seq_file *s, void *chip_data)
{
	...
	ret = gt9886_i2c_read(chip_info->client,
			      chip_info->reg_info.GTP_REG_DOWN_DIFFDATA, 2,
			      kernel_buf);
	if (ret < 0) {
	...
	ret = gt9886_i2c_read(chip_info->client,
			      chip_info->reg_info.GTP_REG_DOWN_DIFFDATA,
			      TX_NUM * RX_NUM * 2 + 2, kernel_buf);
	if (ret < 0) {
		TPD_INFO("%s failed to read down diff data.\n", __func__);
		goto exit;
	}
	...
	for (i = 0; i < RX_NUM; i++) {
		...
		for (j = 0; j < TX_NUM; j++) {
			diff_data =
				(kernel_buf[j * RX_NUM * 2 + i * 2 + 2] << 8) +
				kernel_buf[j * RX_NUM * 2 + i * 2 + 1 + 2];
...
static void goodix_main_register_read(
	struct seq_file *s,
...
static int goodix_do_read_config(void *chip_data, u32 base_addr, u8 *buf)
{
	...
	ret = gt9886_i2c_read(chip_info->client, base_addr, TS_CFG_HEAD_LEN,
			      buf);
	if (ret < 0)
		goto err_out;
	...
	if (checksum) {
		TPD_INFO("Config head checksum err:0x%x,data:%*ph", checksum,
			 TS_CFG_HEAD_LEN, buf);
		ret = -EINVAL;
		goto err_out;
	}
	...
	for (i = 0; i < sub_bags; i++) {
		...
		ret = gt9886_i2c_read(chip_info->client, base_addr + offset, 2,
				      buf + offset);
		if (ret < 0)
			goto err_out;
		...
		TPD_DEBUG("sub bag num:%u,sub bag length:%u", buf[offset],
			  subbag_len);
		ret = gt9886_i2c_read(chip_info->client, base_addr + offset + 2,
				      subbag_len + 1, buf + offset + 2);
		if (ret < 0)
			goto err_out;
		...
		offset += subbag_len + 3;
		TPD_DEBUG("sub bag %d, data:%*ph", buf[offset],
...
int goodix_read_config(void *chip_data, u8 *config_data, u32 config_len)
{
	...
	if (config_len) {
		...
		ret = gt9886_i2c_read(chip_info->client, cmd_reg + 16,
				      config_len, config_data);
		if (ret < 0)
			TPD_INFO("Failed read config data");
		else
			ret = config_len;
	} else {
		ret = goodix_do_read_config(chip_info, cmd_reg + 16,
					    config_data);
		if (ret < 0)
			TPD_INFO("Failed read config data");
		if (ret > 0)
...
static void goodix_config_info_read(struct seq_file *s, void *chip_data)
{
	...
	for (i = 0; i < TS_CFG_MAX_LEN && i < chip_info->normal_cfg.length;
	     i++) {
	...
	for (i = 0; i < TS_CFG_MAX_LEN && i < chip_info->noise_test_cfg.length;
	     i++) {
	...
	if (!chip_info->reg_info.GTP_REG_CMD) {
		TPD_INFO("command register ERROR:0x%04x",
			 chip_info->reg_info.GTP_REG_CMD);
		return;
	}
	...
	if (ret < 0) {
		TPD_INFO(
			"goodix_config_info_read goodix_read_config error:%d\n",
			ret);
		goto exit;
	}
	...
	gt9886_i2c_read(chip_info->client,
			chip_info->reg_info.GTP_REG_PRODUCT_VER, 72, temp_data);
			   ...
			   struct goodix_ts_config *test_config)
{
	...
	TPD_INFO("disable_hopping:0x%02x_%d", test_config->data[offset],
		 offset);
...
static int init_test_config(struct gtx8_ts_test *ts_test)
{
	int ret = 0;
	struct chip_data_gt9886 *chip_info =
		(struct chip_data_gt9886 *)ts_test->ts;
...
static int gtx8_init_testlimits(struct gtx8_ts_test *ts_test)
{
	...
	para_data32 = getpara_for_item(p_testdata->fw, TYPE_NOISE_DATA_LIMIT,
				       &para_num);
	if (para_data32) {
		ts_test->is_item_support[TYPE_NOISE_DATA_LIMIT] =
			para_data32[0];
		if (para_num >= 2) {
	...
	para_data32 = getpara_for_item(p_testdata->fw, TYPE_SHORT_THRESHOLD,
				       &para_num);
	if (para_data32) {
	...
	p_test_item_info =
		get_test_item_info(p_testdata->fw, TYPE_SPECIAL_RAW_MAX_MIN);
	if (p_test_item_info) {
		if (p_test_item_info->para_num) {
			if (p_test_item_info->p_buffer[0]) {
				TPD_INFO(
					"get TYPE_SPECIAL_RAW_MAX_MIN data,len:%d\n",
					para_num);
				ts_test->is_item_support
					[TYPE_SPECIAL_RAW_MAX_MIN] =
					p_test_item_info->p_buffer[0];
				if (p_test_item_info->item_limit_type ==
				    LIMIT_TYPE_MAX_MIN_DATA) {
					test_params->max_limits =
						(uint32_t
							 *)(p_testdata->fw->data +
							    p_test_item_info
								    ->top_limit_offset);
					test_params->min_limits =
						(uint32_t
							 *)(p_testdata->fw->data +
							    p_test_item_info
								    ->floor_limit_offset);
					...
					TPD_INFO(
						"item: %d get_test_item_info fail\n",
						TYPE_SPECIAL_RAW_MAX_MIN);
				}
			}
		}
	...
	p_test_item_info =
		get_test_item_info(p_testdata->fw, TYPE_SPECIAL_RAW_DELTA);
	if (p_test_item_info) {
		if (p_test_item_info->para_num) {
			if (p_test_item_info->p_buffer[0]) {
				TPD_INFO(
					"get TYPE_SPECIAL_RAW_DELTA data,len:%d\n",
					para_num);
				ts_test->is_item_support[TYPE_SPECIAL_RAW_DELTA] =
					p_test_item_info->p_buffer[0];
				if (p_test_item_info->item_limit_type ==
				    LIMIT_TYPE_MAX_MIN_DATA) {
					test_params->deviation_limits =
						(uint32_t
							 *)(p_testdata->fw->data +
							    p_test_item_info
								    ->top_limit_offset);
	...
	para_data32 = getpara_for_item(p_testdata->fw,
				       TYPE_NOISE_SLEFDATA_LIMIT, &para_num);
	if (para_data32) {
		ts_test->is_item_support[TYPE_NOISE_SLEFDATA_LIMIT] =
			para_data32[0];
		if (para_num >= 2) {
			/* store data to test_parms */
	...
	p_test_item_info = get_test_item_info(p_testdata->fw,
		...
		if (p_test_item_info->para_num) {
			if (p_test_item_info->p_buffer[0]) {
				ts_test->is_item_support
					...
					p_test_item_info->p_buffer[0];
				if (p_test_item_info->item_limit_type ==
				    IMIT_TYPE_SLEFRAW_DATA) {
					TPD_INFO(
						"get IMIT_TYPE_SLEFRAW_DATA data\n");
					test_params->self_max_limits =
						(int32_t *)(p_testdata->fw->data +
							    p_test_item_info
								    ->top_limit_offset);
					test_params->self_min_limits =
						(int32_t *)(p_testdata->fw->data +
							    p_test_item_info
								    ->floor_limit_offset);
				} else {
				...
				TPD_INFO(
					"TYPE_SPECIAL_SELFRAW_MAX_MIN para_num is invalid\n");
			}
		}
	} else {
		ret = -1;
...
static int gtx8_tptest_prepare(struct gtx8_ts_test *ts_test)
{
	...
	if (ret < 0) {
		...
		ts_test->error_count++;
		seq_printf(ts_test->p_seq_file,
			   "Failed to init testlimits from csv\n");
	}
	ret = goodix_send_config(chip_info, chip_info->noise_test_cfg.data,
				 chip_info->noise_test_cfg.length);
	if (ret < 0) {
		TPD_INFO(
			"Failed to send noise test config config:%d,noise_config len:%d\n",
				      ...
				      unsigned char *data, unsigned int len)
{
	...
	TPD_INFO(
		"%s run,reg_addr:0x%04x,len:%d,sync_addr:0x%04x,sync_mask:0x%x\n",
...
static int gtx8_cache_rawdata(struct gtx8_ts_test *ts_test)
{
	...
	for (j = 0; j < GTX8_RETRY_NUM_3; j++) {
		/* read rawdata */
		ret = normandy_sync_read_rawdata(
			chip_info->client,
			chip_info->reg_info.GTP_REG_READ_COOR, 0x80,
			rawdata_addr, (u8 *)&ts_test->rawdata.data[0],
...
static int gtx8_noisetest_prepare(struct gtx8_ts_test *ts_test)
{
	...
	TPD_INFO("%s: noise test prepare rawdata addr:0x%04x,len:%d", __func__,
		 ts_test->rawdata_cmd.cmd_reg, ts_test->rawdata_cmd.length);
	/* change to rawdata mode */
	ret = touch_i2c_write_block(chip_info->client,
				    ts_test->rawdata_cmd.cmd_reg,
				    ts_test->rawdata_cmd.length,
				    ts_test->rawdata_cmd.cmds);
	if (ret < 0) {
...
static int gtx8_cache_noisedata(struct gtx8_ts_test *ts_test)
{
	...
	ret1 = normandy_sync_read_rawdata(
		chip_info->client, chip_info->reg_info.GTP_REG_READ_COOR, 0x80,
					 ...
					 struct ts_test_params *test_params)
{
	...
	for (i = 0; i < rawdata->size; i++) {
		if (rawdata->data[i] > test_params->self_max_limits[i]) {
			TPD_INFO(
				"self_rawdata[%d][%d]:%u >self_max_limit:%u, NG\n",
	...
	for (i = 0; i < rawdata->size; i++) {
		if (rawdata->data[i] > test_params->max_limits[i]) {
			TPD_INFO("rawdata[%d][%d]:%u > max_limit:%u, NG\n",
				 (u32)div_s64(i, test_params->sen_num),
				 i % test_params->sen_num, rawdata->data[i],
	...
	for (i = 0; i < sc_data_num; i++) {
		...
		if (rawdata_val) {
			...
			if (max_val > test_params->deviation_limits[i]) {
				TPD_INFO(
					"deviation[%d][%d]:%u > delta_limit:%u, NG\n",
			...
			TPD_INFO(
				"Find rawdata=0 when calculate deltacapacitance:[%d][%d]\n",
				(u32)div_s64(i, cols), i % cols);
			ret = RESULT_ERR;
		}
	}
...
static void gtx8_intgpio_test(struct gtx8_ts_test *p_gtx8_ts_test)
{
	...
	int eint_status = 0, eint_count = 0,
	    read_gpio_num = 0; //not test int pin
	...
	while (read_gpio_num--) {
		msleep(5);
		eint_status =
			gpio_get_value(p_gtx8_ts_test->p_testdata->irq_gpio);
		if (eint_status == 1) {
			eint_count--;
		} else {
			eint_count++;
	...
	if (eint_count == 10) {
		...
		seq_printf(p_gtx8_ts_test->p_seq_file,
...
static int gtx8_short_test_prepare(struct gtx8_ts_test *ts_test)
{
	...
	while (--retry) {
		...
		ret = gt9886_i2c_write(chip_info->client, short_cmd_reg, 3,
				       short_cmd);
		if (ret < 0) {
	...
	ret = touch_i2c_write_block(chip_info->client, WATCH_DOG_TIMER_REG, 1,
				    data);
	if (ret < 0) {
	...
	ret = touch_i2c_write_block(chip_info->client, TXRX_THRESHOLD_REG, 2,
				    data);
	if (ret < 0) {
		TPD_INFO(
			"Failed write tx/tx, tx/rx, rx/rx short threshold value\n");
		return ret;
	}
	...
	ret = touch_i2c_write_block(chip_info->client, GNDVDD_THRESHOLD_REG, 2,
				    data);
	if (ret < 0) {
	...
	ret = touch_i2c_write_block(chip_info->client, ADC_DUMP_NUM_REG, 2,
				    data);
	if (ret < 0) {
	...
	data[0] = 0x01;
	ret = touch_i2c_write_block(chip_info->client, SHORT_STATUS_REG, 1,
				    data);
	if (ret < 0) {
...
static int gtx8_shortcircut_analysis(struct gtx8_ts_test *ts_test)
{
	...
	ret = touch_i2c_read_block(chip_info->client, TEST_RESTLT_REG, 1,
				   &short_flag);
	if (ret < 0) {
	...
	if (short_flag & 0x08) {
		...
		ret = touch_i2c_read_block(chip_info->client, DIFF_CODE_REG,
					   size, data_buf);
		if (ret < 0) {
    ...
    */
	...
	for (i = 0; i < short_status[0]; i++) {
		...
		ret = touch_i2c_read_block(chip_info->client, data_addr, size,
					   data_buf);
		if (ret < 0) {
		...
		for (j = short_die_num + 1; j < MAX_DRV_NUM; j++) {
			...
			if (adc_signal > test_params->short_threshold) {
				...
				if (short_r < r_threshold) {
					...
					TPD_INFO(
						"Tx/Tx short circut:R=%dK,R_Threshold=%dK\n",
						short_r, r_threshold);
	...
	for (i = 0; i < short_status[1]; i++) {
		...
		for (j = short_die_num + 1; j < MAX_SEN_NUM; j++) {
			...
			if (adc_signal > ts_test->test_params.short_threshold) {
				...
				if (self_capdata[short_die_num +
						 test_params->max_drv_num] ==
					    0xffff ||
				    self_capdata[short_die_num +
						 test_params->max_drv_num] ==
					    0) {
					TPD_INFO(
						"invalid self_capdata:0x%x\n",
						self_capdata
							[short_die_num +
							 test_params
								 ->max_drv_num]);
				...
				if (short_r < r_threshold) {
					...
					slave_pin_num = map_die2pin(
						...
						temp_short_info.slave);
					TPD_INFO(
						"Rx/Rx short circut:R=%dK,R_Threshold=%dK\n",
						short_r, r_threshold);
					TPD_INFO("%s%d--%s%d shortcircut\n",
						 (master_pin_num &
						  DRV_CHANNEL_FLAG) ?
			...
			if (adc_signal > ts_test->test_params.short_threshold) {
				...
				if (short_r < r_threshold) {
					...
					slave_pin_num = map_die2pin(
						...
						temp_short_info.slave);
					TPD_INFO(
						"Rx/Tx short circut:R=%dK,R_Threshold=%dK\n",
						short_r, r_threshold);
					TPD_INFO("%s%d--%s%d shortcircut\n",
						 (master_pin_num &
						  DRV_CHANNEL_FLAG) ?
...
static void gtx8_shortcircut_test(struct gtx8_ts_test *ts_test)
{
	...
	for (i = 0; i < 150; i++) {
		...
		ret = touch_i2c_read_block(chip_info->client, SHORT_TESTEND_REG,
					   1, data);
		if (ret < 0)
	...
	if (i < 150) {
		...
		for (i = 0; i < 8; i++) {
			udelay(1000);
			data[0] = 0x00;
			ret = touch_i2c_read_block(chip_info->client, 0x2244, 2,
						   data);
			if (ret < 0) {
...
static void gtx8_put_test_result(struct gtx8_ts_test *ts_test)
{
	uint8_t data_buf[64];
	...
	for (i = 0; i < MAX_TEST_ITEMS; i++) {
		...
		if (ts_test->test_result[i]) {
			...
			} else if (GTP_PANEL_REASON ==
				   ts_test->test_result[i]) {
				ts_test->error_count++;
			} else if (SYS_SOFTWARE_REASON ==
				   ts_test->test_result[i]) {
				ts_test->error_count++;
			}
		}
	...
	if (ts_test->rawdata.size) {
		if (!IS_ERR_OR_NULL(p_testdata->fp)) {
			...
			vfs_write(p_testdata->fp, data_buf, strlen(data_buf),
				  &p_testdata->pos);
		}
		for (i = 0; i < ts_test->rawdata.size; i++) {
			if (!IS_ERR_OR_NULL(p_testdata->fp)) {
				snprintf(data_buf, 64, "%d,",
					 ts_test->rawdata.data[i]);
				vfs_write(p_testdata->fp, data_buf,
					  strlen(data_buf), &p_testdata->pos);
				if (!((i + 1) % p_testdata->RX_NUM) &&
				    (i != 0)) {
			...
			if (GTP_TEST_PASS == ts_test->test_result[i]) {
				...
				if (!IS_ERR_OR_NULL(p_testdata->fp)) {
					...
					vfs_write(p_testdata->fp, data_buf,
						  strlen(data_buf),
						  &p_testdata->pos);
					seq_printf(ts_test->p_seq_file, "%s",
						   data_buf);
				}
			} else if (SYS_SOFTWARE_REASON ==
				   ts_test->test_result[i]) {
...
static void goodix_auto_test(struct seq_file *s, void *chip_data,
			     struct goodix_testdata *p_testdata)
{
	int ret = 0;
	...
	p_rawdata =
		kzalloc(sizeof(u16) * (p_testdata->TX_NUM * p_testdata->RX_NUM),
			GFP_KERNEL);
	if (!p_rawdata) {
...
exit_finish:
	tp_healthinfo_report(chip_info->monitor_data_v2, HEALTH_TEST_AUTO,
			     &gts_test->error_count);
	seq_printf(s, "imageid = %lld, deviceid = %lld\n", p_testdata->TP_FW,
		   gts_test->device_tp_fw);
	TPD_INFO("imageid= %lld, deviceid= %lld\n", p_testdata->TP_FW,
		 gts_test->device_tp_fw);
	seq_printf(s, "%d error(s). %s\n", gts_test->error_count,
		   gts_test->error_count ? "" : "All test passed.");
			   ...
			   const struct i2c_device_id *id)
{
	...
	chip_info->detail_debug_info_support = of_property_read_bool(
		ts->dev->of_node, "goodix_detail_debug_info_support");
