...
#define TPD_DEBUG_ARRAY(array, num)                                            \
	do {                                                                   \
		s32 i;                                                         \
		u8 *a = array;                                                 \
		if (tp_debug) {                                                \
...
int gt1x_init_tool_node(struct touchpanel_data *ts,
			struct fw_update_info *update_info)
{
	...
	gt_tool_info->cmd_head.wr =
		1; //if the first operation is read, will return fail.
	...
	gt1x_tool_proc_entry = proc_create_data("goodix_tool", 0666, NULL,
						&gt1x_tool_fops, gt_tool_info);
...
static u8 relation(u8 src, u8 dst, u8 rlt)
{
	...
	case 1:
		ret = (src == dst) ? true : false;
		TPD_DEBUG("equal:src:0x%02x   dst:0x%02x   ret:%d.", src, dst,
			  (s32)ret);
		break;
...
static u8 comfirm(struct Goodix_tool_info *gt_tool_info)
{
	...
	memcpy(buf, gt_tool_info->cmd_head.flag_addr,
	       gt_tool_info->cmd_head.addr_len);
	...
	for (i = 0; i < gt_tool_info->cmd_head.times; i++) {
		...
		if (true == relation(buf[GTP_ADDR_LENGTH],
				     gt_tool_info->cmd_head.flag_val,
				     gt_tool_info->cmd_head.flag_relation)) {
			TPD_DEBUG("value at flag addr:0x%02x.",
				  buf[GTP_ADDR_LENGTH]);
			TPD_DEBUG("flag value:0x%02x.",
				  gt_tool_info->cmd_head.flag_val);
			break;
		}
			       ...
			       size_t len, loff_t *data)
{
	...
	ret = copy_from_user(cmd_head, (struct st_cmd_head __user *)buff,
			     CMD_HEAD_LENGTH);
	if (ret) {
	...
	if (1 == cmd_head->wr) {
		...
		while (data_len > 0) {
			...
			if (tool_i2c_write(client, cmd_head->data,
		...
		if (gt_tool_info->esd_handle_support)
			esd_handle_switch(gt_tool_info->esd_info,
					  false); //disable esd check
		...
		if (gt_tool_info->esd_handle_support)
			esd_handle_switch(gt_tool_info->esd_info,
					  true); //enable esd check
		return CMD_HEAD_LENGTH;
	} else if (17 == cmd_head->wr) {
		cmd_head->data_len = cmd_head->data_len > DATA_LENGTH ?
					     DATA_LENGTH :
					     cmd_head->data_len;
		ret = copy_from_user(&cmd_head->data[GTP_ADDR_LENGTH],
			      ...
			      size_t count, loff_t *ppos)
{
	...
	if (cmd_head->wr % 2) {
		...
		while (data_len > 0) {
			...
			cmd_head->data[0] = (addr >> 8) & 0xFF;
			cmd_head->data[1] = (addr & 0xFF);
			if (tool_i2c_read(gt_tool_info->client, cmd_head->data,
					  len) <= 0) {
		...
		*ppos += cmd_head->data_len;
		...
		TPD_DEBUG("Return ic type:%s len:%d.", buffer,
