				...
				uint8_t item_index)
{
	...
	for (i = 0; i < item_cnt; i++) {
		item_header =
			(struct auto_test_item_header *)(fw->data +
							 p_item_offset[i]);
			  ...
			  uint32_t *para_num)
{
	...
	if (item_header->para_num == 0) {
		...
		p_buffer = (int32_t *)(fw->data + item_offset +
...
void GetCirclePoints(struct Coordinate *input_points, int number,
		     struct Coordinate *pPnts)
{
	int i = 0;
	int k = 0, j = 0, m = 0, n = 0;
...
static ssize_t tp_devices_check_read_func(struct file *file, char __user *page,
					  size_t size, loff_t *ppos)
{
	char pagesize[64] = { 0 };
	int ret = 0;
	struct touchpanel_data *ts =
		(struct touchpanel_data *)PDE_DATA(file_inode(file));
	if (!ts)
		return 0;

	ret = sprintf(pagesize, "%d\n", (int)(ts->panel_data.tp_type));
	ret = simple_read_from_buffer(page, size, ppos, pagesize,
				      strlen(pagesize));
	return ret;
}
...
static ssize_t proc_health_info_read(struct file *file, char __user *user_buf,
...
static ssize_t proc_health_info_write(struct file *file,
...
{
	return single_open(file, proc_data_config_version_read,
			   PDE_DATA(inode));
}
...
static ssize_t goodix_water_protect_read(struct file *file, char *buff,
...
static ssize_t goodix_water_protect_write(struct file *file, const char *buff,
					  size_t len, loff_t *pos)
{
	struct touchpanel_data *ts = PDE_DATA(file_inode(file));
	struct goodix_proc_operations *syna_ops;
...
void goodix_limit_read(struct seq_file *s, struct touchpanel_data *ts)
{
	...
	for (m = 0; m < item_cnt; m++) {
		...
		if (item_head->item_limit_type == LIMIT_TYPE_NO_DATA) {
			...
			p_data32 = (int32_t *)(fw->data +
					       item_head->top_limit_offset);
			for (i = 0; i < (ts->hw_res.TX_NUM * ts->hw_res.RX_NUM);
			     i++) {
		       ...
		       struct goodix_proc_operations *goodix_ops)
{
	...
	prEntry_tmp = proc_create_data("TP_AUTO_TEST_ID", 0777, ts->prEntry_tp,
				       &gt1x_devices_check, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__,
			 __LINE__);
	...
	prEntry_tmp =
		proc_create_data("health_info_enable", 0777, ts->prEntry_tp,
				 &goodix_health_info_ops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__,
			 __LINE__);
	...
	prEntry_tmp = proc_create_data("config_version", 0666, prEntry_gt,
				       &gt1x_tp_config_version_proc_fops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__,
			 __LINE__);
	...
	prEntry_tmp = proc_create_data("water_protect", 0666, prEntry_gt,
				       &goodix_water_protect_debug_ops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
