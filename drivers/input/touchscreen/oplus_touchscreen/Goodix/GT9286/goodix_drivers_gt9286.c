...
static void goodix_enable_game_mode(struct chip_data_gt9286 *chip_info,
				    bool enable);
 ...
 *@len    : data length we want to send
 return  0: success, non-0: failed
*/
static int goodix_i2c_write_with_readback(struct i2c_client *client, u16 addr,
 ...
 *@chk_len: total check length
 return  0: success, non-0: failed
*/
static int goodix_recall_check(struct i2c_client *client, u8 *chk_src, u16 addr,
			       u16 chk_len)
{
	u8 rd_buf[PACK_SIZE];
	s32 ret = 0;
	...
	while (chk_len > 0) {
		...
		ret = touch_i2c_read_block(client, addr + compared_len, len,
					   rd_buf);
		if (ret < 0) {
...
{
	s32 ret;
	struct chip_data_gt9286 *chip_info =
		(struct chip_data_gt9286 *)chip_data;
	...
	ret = touch_i2c_write_block(chip_info->client,
				    chip_info->reg_info.GTP_REG_CMD + 1, 2,
				    &buffer[1]);
	ret |= touch_i2c_write_byte(chip_info->client,
				    chip_info->reg_info.GTP_REG_CMD, buffer[0]);
	if (ret < 0) {
...
static int goodix_clear_irq(void *chip_data)
{
	...
	if (!gt1x_rawdiff_mode) {
		ret = touch_i2c_write_byte(
			chip_info->client,
			chip_info->reg_info.GTP_REG_READ_COOR, 0);
		if (ret < 0) {
...
static void goodix_esd_check_enable(struct chip_data_gt9286 *chip_info,
				    bool enable)
{
...
static int goodix_enable_edge_limit(struct chip_data_gt9286 *chip_info,
				    bool enable)
{
	int ret = -1;
	...
	if (enable) {
		...
		if (edge_limit_last_status) {
			ret = goodix_send_cmd(chip_info, GTP_CMD_DISABLE_EDGE,
					      0);
		}
	}
...
static int goodix_enable_charge_mode(struct chip_data_gt9286 *chip_info,
				     bool enable)
{
	int ret = -1;
...
static int goodix_set_reset_status(struct chip_data_gt9286 *chip_info)
{
	...
	ret = touch_i2c_write_block(chip_info->client,
				    chip_info->reg_info.GTP_REG_CMD + 1, 3,
				    &value[1]);
	if (ret < 0) {
	...
	return touch_i2c_write_byte(chip_info->client,
				    chip_info->reg_info.GTP_REG_CMD, value[0]);
}

static s32 goodix_read_version(struct chip_data_gt9286 *chip_info,
			       struct panel_info *panel_data)
{
	...
	while (retry--) {
		ret = goodix_i2c_read_dbl_check(
			chip_info->client,
			chip_info->reg_info.GTP_REG_PRODUCT_VER, buf,
			sizeof(buf));
		if (!ret) {
			checksum = 0;
	...
	TPD_INFO(
		"goodix fw VERSION:GT%s(Product)_%06X(Patch)_%04X(Mask)_%02X(SensorID)\n",
		product_id, patch_id, mask_id >> 8, sensor_id);

	if (panel_data->manufacture_info.version) {
		snprintf(&(panel_data->manufacture_info.version[8]),
			 MAX_DEVICE_VERSION_LENGTH - 6, "%6x",
			 patch_id); //MAX_DEVICE_VERSION_LENGTH = 16
	}
...
static s32 goodix_send_config(struct chip_data_gt9286 *chip_info, u8 *config,
			      int cfg_len)
{
	int i;
	s32 ret = 0;
	...
	while (retry++ < 5) {
		ret = touch_i2c_write_block(
			chip_info->client,
			chip_info->reg_info.GTP_REG_CONFIG_DATA, cfg_len,
			config);
		if (ret == cfg_len) {
...
static int goodix_init_panel(struct chip_data_gt9286 *chip_info)
{
	...
	chip_info->config_info.goodix_config_len =
		sizeof(GTP_CFG_GROUP_SAMSUNG);

	TPD_DEBUG("config sizeof(chip_info->goodix_config) is %d\n",
		  chip_info->config_info.goodix_config_len);

	if (chip_info->config_info.goodix_config_len < GTP_CONFIG_MIN_LENGTH ||
	    chip_info->config_info.goodix_config_len > GTP_CONFIG_MAX_LENGTH) {
		TPD_INFO(
			"Config is INVALID! You need to check you header file CFG_GROUP section!\n");
		return -1;
	}
	...
	if ((chip_info->config_info.goodix_abs_x_max == 0) &&
	    (chip_info->config_info.goodix_abs_y_max == 0)) {
		chip_info->config_info.goodix_abs_x_max =
			(chip_info->config_info
				 .goodix_config[RESOLUTION_LOCATION + 1]
			 << 8) +
			chip_info->config_info
				.goodix_config[RESOLUTION_LOCATION];
		chip_info->config_info.goodix_abs_y_max =
			(chip_info->config_info
				 .goodix_config[RESOLUTION_LOCATION + 3]
			 << 8) +
			chip_info->config_info
				.goodix_config[RESOLUTION_LOCATION + 2];
		chip_info->config_info.goodix_int_type =
			(chip_info->config_info.goodix_config[TRIGGER_LOCATION]) &
			0x03;
		chip_info->config_info.goodix_wakeup_level =
			!(chip_info->config_info
				  .goodix_config[MODULE_SWITCH3_LOCATION] &
			  0x20);
	} else {
		chip_info->config_info.goodix_config[RESOLUTION_LOCATION] =
			(u8)chip_info->config_info.goodix_abs_x_max;
		chip_info->config_info.goodix_config[RESOLUTION_LOCATION + 1] =
			(u8)(chip_info->config_info.goodix_abs_x_max >> 8);
		chip_info->config_info.goodix_config[RESOLUTION_LOCATION + 2] =
			(u8)chip_info->config_info.goodix_abs_y_max;
		chip_info->config_info.goodix_config[RESOLUTION_LOCATION + 3] =
			(u8)(chip_info->config_info.goodix_abs_y_max >> 8);
		set_reg_bit(chip_info->config_info
				    .goodix_config[MODULE_SWITCH3_LOCATION],
			    5, !chip_info->config_info.goodix_wakeup_level);
		chip_info->config_info.goodix_config[TRIGGER_LOCATION] =
			(chip_info->config_info.goodix_config[TRIGGER_LOCATION] &
			 0xFC) |
			chip_info->config_info.goodix_int_type;
	...
	TPD_INFO(
		"X_MAX = %d, Y_MAX = %d, TRIGGER = 0x%02x, WAKEUP_LEVEL = %d\n",
		chip_info->config_info.goodix_abs_x_max,
		chip_info->config_info.goodix_abs_y_max,
		chip_info->config_info.goodix_int_type,
		chip_info->config_info.goodix_wakeup_level);

	ret = goodix_send_config(chip_info,
				 chip_info->config_info.goodix_config,
				 chip_info->config_info.goodix_config_len);
...
static s32 goodix_request_event_handler(struct chip_data_gt9286 *chip_info)
{
	...
	case GTP_RQST_CONFIG:
		...
		ret = goodix_send_config(
			chip_info, chip_info->config_info.goodix_config,
			chip_info->config_info.goodix_config_len);
		if (ret) {
...
static u8 *goodix_get_fw_data(struct chip_data_gt9286 *chip_info, u32 offset,
			      int length)
{
	memcpy(chip_info->update_info.buffer,
	       chip_info->update_info.firmware_file_data + offset, length);
...
static int goodix_check_firmware_data(struct chip_data_gt9286 *chip_info)
{
	...
	if (getU32(p) + 6 != chip_info->update_info.fw_length) {
		TPD_INFO(
			"firmware error, (file actual length: %d, header define length: %d)\n",
	...
	TPD_INFO(
		"firmware actual checksum: 0x%04X, header define checksum: 0x%04X\n",
	...
	TPD_INFO("Firmware patch: %02X%02X%02X\n",
		 chip_info->update_info.firmware->version[0],
		 chip_info->update_info.firmware->version[1],
...
static int __goodix_hold_ss51_dsp_20(struct chip_data_gt9286 *chip_info)
{
	...
	while (retry++ < 30) {
		...
		ret = touch_i2c_write_byte(
			chip_info->client,
			chip_info->reg_info._rRW_MISCTL__SWRST_B0_, buf[0]);
		if (ret < 0) {
		...
		ret = touch_i2c_read_byte(
			chip_info->client,
			chip_info->reg_info._rRW_MISCTL__SWRST_B0_);
		if (ret < 0) {
			TPD_INFO("Hold ss51 & dsp I2C error, retry:%d\n",
				 retry);
			continue;
...
static int goodix_hold_ss51_dsp(struct chip_data_gt9286 *chip_info)
{
	...
	do {
		ret = touch_i2c_read_block(chip_info->client, 0x4220, 1,
					   buffer);
	} while (retry-- && ret < 0);
	...
	buffer[0] = 0x00;
	ret = goodix_i2c_write_with_readback(
		chip_info->client,
		chip_info->reg_info._bRW_MISCTL__DSP_MCU_PWR_, buffer, 1);
	if (ret) {
	...
	buffer[0] = 0x00;
	ret = goodix_i2c_write_with_readback(
		chip_info->client, chip_info->reg_info._bRW_MISCTL__TMR0_EN,
		buffer, 1);
	if (ret) {
	...
	buffer[0] = 0x00;
	ret = goodix_i2c_write_with_readback(
		chip_info->client, chip_info->reg_info._bRW_MISCTL__CACHE_EN,
		buffer, 1);
	if (ret) {
	...
	ret = touch_i2c_write_block(
		chip_info->client,
		chip_info->reg_info._bWO_MISCTL__CPU_SWRST_PULSE, 1, buffer);
	if (ret < 0) {
	...
	ret = goodix_i2c_write_with_readback(
		chip_info->client,
		chip_info->reg_info._rRW_MISCTL__BOOT_OPT_B0_, buffer, 1);
	if (ret) {
...
static int goodix_run_ss51_isp(struct chip_data_gt9286 *chip_info, u8 *ss51_isp,
			       int length)
{
	int ret;
	u8 buffer[10];
	...
	buffer[0] = 0x04;
	ret = goodix_i2c_write_with_readback(
		chip_info->client, chip_info->reg_info._bRW_MISCTL__SRAM_BANK,
		buffer, 1);
	if (ret) {
	...
	ret = goodix_i2c_write_with_readback(
		chip_info->client,
		chip_info->reg_info._bRW_MISCTL__PATCH_AREA_EN_, buffer, 1);
	if (ret) {
	...
	TPD_INFO("ss51_isp length: %d, checksum: 0x%04X\n", length,
		 goodix_calc_checksum(ss51_isp, length));
	// load ss51 isp
	ret = touch_i2c_write_block(chip_info->client, 0xC000, length,
				    ss51_isp);
	if (ret < 0) {
	...
	ret = goodix_i2c_write_with_readback(chip_info->client, 0x8140, buffer,
					     10);
	...
	ret = goodix_i2c_write_with_readback(chip_info->client, 0x8006, buffer,
					     8);
	if (ret) {
	...
	buffer[0] = 0x08;
	ret = goodix_i2c_write_with_readback(
		chip_info->client, chip_info->reg_info._rRW_MISCTL__SWRST_B0_,
		buffer, 1);
	if (ret) {
...
static int goodix_error_erase(struct chip_data_gt9286 *chip_info)
{
	...
	fw = goodix_get_fw_data(
		chip_info, chip_info->update_info.firmware->subsystem[0].offset,
		chip_info->update_info.firmware->subsystem[0].length);
	if (fw == NULL) {
		TPD_INFO("get isp fail\n");
		return -1;
	}
	ret = goodix_run_ss51_isp(
		chip_info, fw,
		chip_info->update_info.firmware->subsystem[0].length);
	if (ret) {
	...
	while (retry-- > 0) {
		...
		ret = goodix_i2c_write_with_readback(chip_info->client, 0x8100,
						     buffer, 4);
		if (ret) {
		...
		ret = touch_i2c_write_block(chip_info->client, 0x8100 + 4,
					    block_len, fw);
		if (ret < 0) {
		...
		ret = goodix_recall_check(chip_info->client, fw, 0x8100 + 4,
					  block_len);
		if (ret) {
			continue;
		}
		...
		ret = goodix_i2c_write_with_readback(
			chip_info->client, 0x8100 + 4 + block_len, buffer, 2);
		if (ret) {
		...
		buffer[0] = 0;
		ret = goodix_i2c_write_with_readback(chip_info->client, 0x8022,
						     buffer, 1);
		if (ret) {
		...
		ret = goodix_i2c_write_with_readback(chip_info->client, 0x8020,
						     buffer, 2);
		if (ret) {
		...
		while (wait_time > 0) {
			wait_time--;
			msleep(5);
			ret = goodix_i2c_read_dbl_check(chip_info->client,
							0x8022, buffer, 1);
			if (ret) {
				continue;
			}
...
static int goodix_burn_subsystem(struct chip_data_gt9286 *chip_info,
				 struct fw_subsystem_info *subsystem)
{
	int block_len;
	u16 checksum;
	int burn_len = 0;
	...
	while (length > 0 && retry > 0) {
		...
		fw = goodix_get_fw_data(chip_info, subsystem->offset + burn_len,
					block_len);
		if (fw == NULL) {
			return -1;
		}
		...
		if (block_len % 2048) {
			...
			if (ret < 0) {
				TPD_INFO(
					"the last pack, write fw data fail!\n");
			}
		}
		...
		while (wait_time-- > 0) {
			...
			ret = touch_i2c_read_block(chip_info->client, 0x8022, 1,
						   buffer);
			if (ret < 0) {
				continue;
			}
...
static int goodix_check_subsystem_in_flash(struct chip_data_gt9286 *chip_info,
					   struct fw_subsystem_info *subsystem)
{
	int block_len;
	int checked_len = 0;
	...
	TPD_INFO("Subsystem: %d, Length: %d, Address: 0x%08X", subsystem->type,
		 subsystem->length, subsystem->address);
	...
	while (length > 0) {
		...
		fw = goodix_get_fw_data(
			chip_info, subsystem->offset + checked_len, block_len);
		if (fw == NULL) {
			return -1;
		}
		ret = goodix_read_flash(chip_info->client,
		...
		ret = goodix_recall_check(chip_info->client, fw, 0x8100,
					  block_len);
		if (ret) {
...
static void goodix_finger_protect_data_get(struct chip_data_gt9286 *chip_info)
{
	...
	chip_info->spuri_fp_touch_raw_data =
		kzalloc(TX_NUM * RX_NUM * sizeof(u16), GFP_KERNEL);
	...
	while (i++ < 10) {
		ret = touch_i2c_read_block(
			chip_info->client,
			chip_info->reg_info.GTP_REG_READ_COOR, 1, raw_data);
	...
	ret = touch_i2c_read_block(chip_info->client, addr, TX_NUM * RX_NUM * 2,
				   raw_data);

	for (i = 0; i < RX_NUM; i++) {
		//TPD_INFO("[%2d] ", i);
		for (j = 0; j < TX_NUM; j++) {
			chip_info->spuri_fp_touch_raw_data[i * TX_NUM + j] =
				(s16)(raw_data[i * TX_NUM * 2 + j * 2] << 8) +
				raw_data[i * TX_NUM * 2 + j * 2 + 1];
				      ...
				      struct panel_info *panel_data)
{
	...
	do {
		...
		goodix_i2c_read_dbl_check(
			chip_info->client,
			chip_info->reg_info.GTP_REG_FW_CHK_SUBSYS, &reg_val[1],
			1);

		if (reg_val[0] != 0xBE || reg_val[1] == 0xAA) {
			TPD_INFO(
				"Check fw status failed: reg[0x41E4] = 0x%2X, reg[0x5095] = 0x%2X!\n",
				reg_val[0], reg_val[1]);
		} else {
			break;
		}
	...
	ret = touch_i2c_read_byte(chip_info->client,
				  chip_info->reg_info.GTP_REG_CONFIG_DATA);
	if (ret < 0) {
		TPD_INFO(
			"%s: read config version failed, need update firmware!\n",
			__func__);
		return FW_ABNORMAL;
	} else {
					...
					const struct firmware *fw, bool force)
{
	...
	chip_info->update_info.firmware =
		(struct fw_info *)kzalloc(sizeof(struct fw_info), GFP_KERNEL);
	...
	chip_info->update_info.max_progress =
		6 + chip_info->update_info.firmware->subsystem_count;
	...
	do {
		...
		if (reg_val[0] != 0xBE || reg_val[1] == 0xAA) {
			TPD_INFO(
				"Check fw status reg not pass, reg[0x814E] = 0x%2X, reg[0x5095] = 0x%2X!, retry = %d\n",
	...
	if (retry) { // if tp firmware is ok, judge the fw id, else  update immediately
		fw_ver_info.mask_id =
			(chip_info->update_info.firmware->target_mask_version[0]
			 << 16) |
			(chip_info->update_info.firmware->target_mask_version[1]
			 << 8) |
			(chip_info->update_info.firmware
				 ->target_mask_version[2]);
		fw_ver_info.patch_id =
			(chip_info->update_info.firmware->version[0] << 16) |
			(chip_info->update_info.firmware->version[1] << 8) |
			(chip_info->update_info.firmware->version[2]);
		memcpy(fw_ver_info.product_id,
		       chip_info->update_info.firmware->pid, 4);
	...
	for (i = 1; i < chip_info->update_info.firmware->subsystem_count; i++) {
		...
		ret = goodix_burn_subsystem(
			chip_info,
			&(chip_info->update_info.firmware->subsystem[i]));
		if (ret) {
	...
	for (i = 1; i < chip_info->update_info.firmware->subsystem_count; i++) {
		...
		ret = goodix_check_subsystem_in_flash(
			chip_info,
			&(chip_info->update_info.firmware->subsystem[i]));
		if (ret) {
...
update_exit:
	...
	if (ret) {
		chip_info->update_info.progress =
			2 * chip_info->update_info.max_progress;
		TPD_INFO("Update firmware failed!\n");
		return FW_UPDATE_ERROR;
	} else {
		goodix_init_panel(
			chip_info); //after update, need send config again
...
static u8 goodix_trigger_reason(void *chip_data, int gesture_enable,
				int is_suspended)
{
	int ret = -1;
	u8 touch_num = 0;
	...
	if (gesture_enable &&
	    is_suspended) { //check whether the gesture trigger
		return IRQ_GESTURE;
	...
	if ((chip_info->touch_data[0] & 0x80) ==
	    0) { //touch data not ready, wait for next interrupt
		return IRQ_IGNORE;
	}

	touch_num = touch_num < MAX_POINT_NUM ? touch_num : MAX_POINT_NUM;
	ret = touch_i2c_read_block(
		chip_info->client, chip_info->reg_info.GTP_REG_READ_COOR + 1,
		8 * touch_num + 2,
		&chip_info->touch_data[1]); //read out point data
	if (ret < 0) {
	...
	if (check_sum) { /* checksum should be zero, check again when it's no zero */
		ret = touch_i2c_read_block(
			chip_info->client,
			chip_info->reg_info.GTP_REG_READ_COOR,
			3 + 8 * touch_num, chip_info->touch_data);
		if (ret < 0) {
			return IRQ_IGNORE;
		}
				   ...
				   struct gesture_info *gesture)
{
	...
	ret = touch_i2c_read_block(chip_info->client,
				   ...
				   point_num * 4, point_data);
	touch_i2c_write_byte(chip_info->client,
			     chip_info->reg_info.GTP_REG_WAKEUP_GESTURE,
			     0x00); //clear gesture int
	if (ret < 0) {
...
static int goodix_esd_handle(void *chip_data)
{
	...
	for (i = 0; i < 3; i++) {
		ret = touch_i2c_read_block(chip_info->client,
					   chip_info->reg_info.GTP_REG_CMD, 4,
					   esd_buf);
		TPD_INFO(
			"[Esd]0x8040 = 0x%02X, 0x8043 = 0x%02X goodix_rawdiff_mode = %d\n",
	...
	if (likely(i < 3)) {
		...
		TPD_DEBUG(
			"time_before(jiffies, timeout) is %d(%lu,%lu), 0x8040 = 0x%02X, 0x8043 = 0x%02X gt1x_rawdiff_mode = %d\n",
			time_before(jiffies, timeout), jiffies, timeout,
			esd_buf[0], esd_buf[3], gt1x_rawdiff_mode);
		if (gt1x_rawdiff_mode == 1 ||
		    (time_before(jiffies, timeout) && esd_buf[0] == 0x00 &&
		     esd_buf[3] == 0xAA)) {
		...
		touch_i2c_write_block(chip_info->client, 0x4226,
				      sizeof(esd_buf), esd_buf);
		msleep(50);
				      ...
				      uint8_t *result, uint8_t length)
{
	...
	ret = touch_i2c_read_block(chip_info->client, register_addr, length,
				   result); /*read data*/
}
				   ...
				   debug_type debug_type)
{
	...
	while (i++ < 10) {
		ret = touch_i2c_read_block(
			chip_info->client,
			chip_info->reg_info.GTP_REG_READ_COOR, 1, kernel_buf);
		TPD_INFO("ret = %d \t kernel_buf = %d\n", ret, kernel_buf[0]);
	...
	if (debug_type == GTP_DIFFDATA) {
		ret = touch_i2c_read_block(chip_info->client, addr,
					   TX_NUM * RX_NUM, kernel_buf);
		msleep(5);
		...
		for (i = 0; i < RX_NUM; i++) {
			...
			for (j = 0; j < TX_NUM; j++) {
				...
				seq_printf(s, "%4d ",
					   (kernel_buf[j * RX_NUM * 2 + i * 2]
					    << 8) + kernel_buf[j * RX_NUM * 2 +
							       i * 2 + 1]);
			}
			//printk("\n");
			seq_printf(s, "\n");
...
static void goodix_main_register_read(struct seq_file *s, void *chip_data)
{
	...
	ret = touch_i2c_read_block(chip_info->client, 0x88B8, 392,
				   noise_detect);
	for (i = 0; i < 392; i++) {
	...
	ret = touch_i2c_read_block(chip_info->client, 0xD62C, 128,
				   noise_rawdata);
	for (i = 0; i < 128; i++) {
	...
	ret = touch_i2c_read_block(chip_info->client, 0x8E80, 2432,
				   XDATA2_bottom);
	for (i = 0; i < 76; i++) {
		seq_printf(s, "[%d]:", i);
		for (j = 0; j < 32; j++) {
	...
	ret = touch_i2c_read_block(
		chip_info->client,
		chip_info->reg_info.GTP_REG_WATER_PROTECT_QUERY, 1, XDATA1);
	...
	for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
		seq_printf(s, "0x%02X, ",
			   chip_info->config_info.goodix_config[i]);
		if (i % 10 == 9)
			seq_printf(s, "\n");
	}
	seq_printf(s, "\n");
...
static void goodix_config_info_read(struct seq_file *s, void *chip_data)
{
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.GTP_REG_CONFIG_DATA,
				   GTP_CONFIG_MAX_LENGTH, temp_data);
	...
	touch_i2c_read_block(chip_info->client,
			     chip_info->reg_info.GTP_REG_PRODUCT_VER, 12,
			     temp_data);
...
static size_t goodix_water_protect_mode_read(struct file *file, char *buf,
					     size_t len, loff_t *pos)
{
	...
	struct chip_data_gt9286 *chip_info =
		(struct chip_data_gt9286 *)ts->chip_data;
...
static size_t goodix_water_protect_mode_write(struct file *file,
					      ...
					      loff_t *pos)
{
	...
	if (buf_tmp[0] == '1') {
		ret = goodix_send_cmd(chip_info, GTP_CMD_ENTER_WATER_PROTECT,
				      0);
			   ...
			   const struct i2c_device_id *id)
{
	...
	read_cfg_from_dts = of_property_read_bool(ts->dev->of_node,
						  "read_cfg_group_from_dts");
	if (read_cfg_from_dts) {
		ret = of_property_read_u8_array(ts->dev->of_node,
						"goodix,gtp_cfg_group",
						GTP_CFG_GROUP_SAMSUNG,
						sizeof(GTP_CFG_GROUP_SAMSUNG));
		if (ret) {
