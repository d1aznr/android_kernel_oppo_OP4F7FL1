...
static int async_read(void __user *arg)
{
	...
	reg_addr = i2c_msg_head[0] + (i2c_msg_head[1] << 8) + i2c_msg_head[2] +
		   (i2c_msg_head[3] << 8);
	...
	if (touch_i2c_read_block(gtx8_tool_info.client, (u16)reg_addr, length,
				 databuf) >= 0) {
...
static int async_write(void __user *arg)
{
	...
	if (touch_i2c_write_block(gtx8_tool_info.client, (u16)reg_addr, length,
				  databuf) < 0) {
		ret = -EBUSY;
			       ...
			       unsigned long arg)
{
	...
	if (_IOC_NR(cmd) > GOODIX_TS_IOC_MAXNR) {
		TPD_INFO("Bad cmd num:%d > %d", _IOC_NR(cmd),
			 GOODIX_TS_IOC_MAXNR);
		return -ENOTTY;
	}
