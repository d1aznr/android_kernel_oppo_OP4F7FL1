...
static int sec_stop_filter_set(struct chip_data_s6sy792 *chip_info, int level,
			       int usb_state);
				    ...
				    bool enable)
{
	...
	if (enable) {
		...
		touch_i2c_write_word(chip_info->client,
				     SEC_CMD_WAKEUP_GESTURE_MODE, config);
			       ...
			       int usb_state)
{
	...
	TPD_INFO("now usb state is %d, level is %d, %s", usb_state, level,
		 level < 255 ? "success" : "default");
...
static int sec_smooth_lv_set(void *chip_data, int level)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_REG_SMOOTH_LEVEL,
				   level);
...
static int sec_sensitive_lv_set(void *chip_data, int level)
{
	...
	if (ts->is_usb_checked && level < 255) {
		sec_stop_filter_set(chip_info, ts->stop_filter_set + 10,
				    ts->is_usb_checked);
	} else if (!ts->is_usb_checked && level < 255) {
		sec_stop_filter_set(chip_info, ts->stop_filter_set,
				    ts->is_usb_checked);
	} else if (level == 255) {
...
static void sec_main_register_read(struct seq_file *s, void *chip_data)
{
	...
	memset(buf, 0, 4);
	touch_i2c_read_block(chip_info->client, SEC_CMD_REG_STOP_LOCK_POINT, 4,
			     buf);
