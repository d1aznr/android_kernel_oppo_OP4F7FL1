...
__attribute__((weak)) int set_lsi_tp_watchdog_state(int state)
{
	return 1;
}
...
static int sec_enable_black_gesture(struct chip_data_s6d7at0 *chip_info,
				    bool enable)
{
	int ret = -1;
	//int i = 0;
	...
	if (enable) {
		ret = touch_i2c_write_byte(chip_info->client,
					   SEC_CMD_WAKEUP_GESTURE_MODE, 0x00);
		ret = touch_i2c_write_byte(chip_info->client,
					   SEC_CMD_SET_POWER_MODE, 0x01);
		ret = touch_i2c_write_byte(chip_info->client,
					   SEC_CMD_AFE_SENSING, 0x0D);
		sec_mdelay(50);
		TPD_INFO("%s, last_state = %d,ret = %d\n", __func__, last_state,
			 ret);
	} else {
		ret = touch_i2c_write_byte(chip_info->client,
					   SEC_CMD_WAKEUP_GESTURE_MODE, 0x01);
		ret = touch_i2c_write_byte(chip_info->client,
					   SEC_CMD_AFE_SENSING, 0x0C);
		TPD_INFO("%s, ret = %d\n", __func__, ret);
		return 0;
	}
...
static int sec_enable_edge_limit(struct chip_data_s6d7at0 *chip_info,
				 bool enable)
{
	int ret = -1;
	u8 data[4] = {
	...
	if (chip_info->edge_limit_support) {
		ret = touch_i2c_write_byte(chip_info->client,
					   SEC_CMD_EDGE_DEADZONE, 0x00);
	} else if (chip_info->fw_edge_limit_support) {
		if (enable == 1 ||
		    VERTICAL_SCREEN == chip_info->touch_direction) {
			ret = touch_i2c_write_byte(chip_info->client,
						   SEC_CMD_EDGE_DEADZONE, 0x01);
		} else {
...
static int sec_enable_charge_mode(struct chip_data_s6d7at0 *chip_info,
				  bool enable)
{
	int ret = -1;
	int retry = 0;
	...
	touchfunction = touch_i2c_read_byte(chip_info->client,
					    SEC_CMD_SET_TOUCHFUNCTION);
	sec_mdelay(20);

	if (enable) {
		touchfunction = touchfunction | SEC_BIT_SETFUNC_CHARGER |
				SEC_BIT_SETFUNC_TOUCH;
	} else {
		touchfunction = ((touchfunction & (~SEC_BIT_SETFUNC_CHARGER)) |
				 SEC_BIT_SETFUNC_TOUCH);
	}
	ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_SET_TOUCHFUNCTION,
				   touchfunction);
	sec_mdelay(150);

	while (retry++ < 50) {
		readfunction = touch_i2c_read_byte(chip_info->client,
						   SEC_CMD_SET_TOUCHFUNCTION);
		TPD_DEBUG("%s: retry %d (%02X)\n", __func__, retry,
			  readfunction);
		if ((touchfunction & SEC_BIT_SETFUNC_CHARGER) ==
		    (readfunction & SEC_BIT_SETFUNC_CHARGER))
			break;
		sec_mdelay(20);
	}
...
static int sec_enable_earsense_mode(struct chip_data_s6d7at0 *chip_info,
				    bool enable)
{
	int ret = -1;
...
static int sec_enable_palm_reject(struct chip_data_s6d7at0 *chip_info,
				  bool enable)
{
	int ret = -1;
	int retry = 0;
	...
	if (enable) {
		touchfunction = touchfunction | SEC_BIT_SETFUNC_PALM |
				SEC_BIT_SETFUNC_TOUCH;
	} else {
		touchfunction = ((touchfunction & (~SEC_BIT_SETFUNC_PALM)) |
				 SEC_BIT_SETFUNC_TOUCH);
	...
	while (retry++ < 50) {
		...
		TPD_DEBUG("%s: retry %d (%02X)\n", __func__, retry,
			  readfunction);
		if ((touchfunction & SEC_BIT_SETFUNC_PALM) ==
		    (readfunction & SEC_BIT_SETFUNC_PALM))
			break;
		sec_mdelay(20);
	}
...
static int sec_enable_game_mode(struct chip_data_s6d7at0 *chip_info,
				bool enable)
{
	int ret = -1;

	ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_STOP_FILTER,
				   enable ? 0x01 : 0x00);
	TPD_INFO("%s: state: %d %s!\n", __func__, enable,
		 ret < 0 ? "failed" : "success");
	return ret;
...
static int sec_enable_headset_mode(struct chip_data_s6d7at0 *chip_info,
				   bool enable)
{
	int ret = -1;
	int retry = 0;
	...
	if (enable) {
		touchfunction = touchfunction | SEC_BIT_SETFUNC_HEADSET |
				SEC_BIT_SETFUNC_TOUCH;
	} else {
		touchfunction = ((touchfunction & (~SEC_BIT_SETFUNC_HEADSET)) |
				 SEC_BIT_SETFUNC_TOUCH);
	...
	while (retry++ < 50) {
		...
		TPD_DEBUG("%s: retry %d (%02X)\n", __func__, retry,
			  readfunction);
		if ((touchfunction & SEC_BIT_SETFUNC_HEADSET) ==
		    (readfunction & SEC_BIT_SETFUNC_HEADSET))
			break;
		sec_mdelay(20);
	}
...
static int sec_ts_flash_set_datanum(struct chip_data_s6d7at0 *chip_info,
				    u16 num)
{
	u8 tData[2];
	int ret;
	...
	ret = touch_i2c_write_block(chip_info->client, SEC_CMD_FLASH_READ_SIZE,
				    sizeof(tData), tData);
	if (ret < 0)
...
static int sec_ts_flash_cs_control(struct chip_data_s6d7at0 *chip_info,
				   bool cs_level)
{
	u8 tData;
	int ret;
	...
	ret = touch_i2c_write_block(chip_info->client, SEC_CMD_CS_CONTROL, 1,
				    &tData);
	if (ret < 0)
...
static int sec_ts_wren(struct chip_data_s6d7at0 *chip_info)
{
	...
	ret = touch_i2c_write_block(chip_info->client, SEC_CMD_FLASH_SEND_DATA,
				    1, tData);
	if (ret < 0)
...
static u8 sec_ts_rdsr(struct chip_data_s6d7at0 *chip_info)
{
	...
	ret = touch_i2c_read_block(chip_info->client, SEC_CMD_FLASH_READ_DATA,
				   1, tData);
...
static int sec_ts_cmd_flash_se(struct chip_data_s6d7at0 *chip_info,
			       u32 flash_addr)
{
	int ret;
	u8 tBuf[5];
...
static int sec_flash_sector_erase(struct chip_data_s6d7at0 *chip_info,
				  u32 sector_idx)
{
	u32 addr;
	int ret;
...
static int sec_flash_page_write(struct chip_data_s6d7at0 *chip_info,
...
static int sec_block_read(struct chip_data_s6d7at0 *chip_info, u32 mem_addr,
			  int mem_size, u8 *buf)
{
	int ret;
	u8 cmd[5];
...
static int sec_memory_read(struct chip_data_s6d7at0 *chip_info, u32 mem_addr,
...
static int sec_flash_write(struct chip_data_s6d7at0 *chip_info, u32 mem_addr,
			   u8 *mem_data, u32 mem_size)
{
	int ret = -1;
	...
	for (page_idx = (int)((page_num - 1) / 16); page_idx >= 0; page_idx--) {
		ret = sec_flash_sector_erase(
			chip_info, (u32)(page_idx_start + page_idx * 16));
		if (ret < 0) {
	...
	for (page_idx = page_num - 1;; page_idx--) {
		...
		ret = sec_flash_page_write(
			chip_info, (page_idx + page_idx_start), page_buf);
		if (ret < 0) {
			sec_mdelay(50);
...
static int sec_chunk_update(struct chip_data_s6d7at0 *chip_info, u32 addr,
			    u32 size, u8 *data)
{
	int ii = 0;
	int ret = 0;
...
int sec_execute_force_calibration(struct chip_data_s6d7at0 *chip_info)
{
	...
	rc = touch_i2c_write_block(chip_info->client, SEC_CMD_MIS_CAL_CHECK, 0,
...
static int sec_reset(void *chip_data)
{
	struct chip_data_s6d7at0 *chip_info =
		(struct chip_data_s6d7at0 *)chip_data;
				     ...
				     bool force)
{
	...
	if (cal_status == 0xFF || (config_in_bin != config_in_ic) ||
	    ((vers_in_ic == 0x05) && (version_in_ic == 0xE1))) {
		...
		while (i < 2) {
			...
			if (cal_status != 0x00) {
				TPD_INFO(
					"calibration result(%02X) failed(%d), try again.\n",
					cal_status, i++);
				continue;
			}
			     ...
			     int is_suspended)
{
	...
	if (event_id == SEC_STATUS_EVENT) {
		...
		if ((p_event_status->stype == TYPE_STATUS_EVENT_INFO) &&
		    (p_event_status->status_id == SEC_ACK_PALM_MODE)) {
			ret = touch_i2c_write_block(chip_info->client,
		...
		if ((p_event_status->stype == TYPE_STATUS_EVENT_VENDOR_INFO) &&
			...
			TPD_INFO(
				"%s: IC Event Queue is full! Send Clear Event Queue\n",
				__func__);
			ret = touch_i2c_write_block(chip_info->client,
						    SEC_CMD_CLEAR_EVENT_STACK,
						    0, NULL);
				...
				int max_num)
{
	...
	for (i = 0; i < left_event; i++) {
		...
		if ((t_id < max_num) &&
			...
			TPD_DEBUG(
				"event[%d]: 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x\n",
				i, event_buff[i][0], event_buff[i][1],
				event_buff[i][2], event_buff[i][3],
				event_buff[i][4], event_buff[i][5],
				event_buff[i][6], event_buff[i][7]);
		}
	}
...
static int sec_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	case GESTURE_O:
		...
		gesture->Point_start.y =
			(coord[1] << 4) | ((coord[2] >> 0) & 0x0F);
		limitPoint[0].x =
			(coord[3] << 4) | ((coord[5] >> 4) & 0x0F); //ymin
		limitPoint[0].y = (coord[4] << 4) | ((coord[5] >> 0) & 0x0F);
		limitPoint[1].x =
			(coord[6] << 4) | ((coord[8] >> 4) & 0x0F); //xmin
		limitPoint[1].y = (coord[7] << 4) | ((coord[8] >> 0) & 0x0F);
		limitPoint[2].x =
			(coord[9] << 4) | ((coord[11] >> 4) & 0x0F); //ymax
		limitPoint[2].y = (coord[10] << 4) | ((coord[11] >> 0) & 0x0F);
		limitPoint[3].x =
			(coord[12] << 4) | ((coord[14] >> 4) & 0x0F); //xmax
...
static int sec_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	last_state =
		mode; //update the "last_state" value when the mode change sequence finish
...
*/
static int sec_read_self(struct chip_data_s6d7at0 *chip_info, u8 type,
			 char *data, int len)
{
	int ret = 0;
...
static int sec_read_mutual(struct chip_data_s6d7at0 *chip_info, u8 type,
			   char *data, int len)
{
	int ret = 0;
	u8 buf[2] = { 0 };
...
static void sec_main_register_read(struct seq_file *s, void *chip_data)
{
	...
	seq_printf(
		s,
		"power mode: 0x%02X[normal-0x02/lpwg-0x05], 0x%02X[idle-0x00/active-0x02]\n",
		buf[1], buf[3]);
	return;
}
...
static void sec_reserve_read(struct seq_file *s, void *chip_data)
{
	...
	if (int_state) {
		...
		touch_i2c_write_byte(chip_info->client,
				     SEC_CMD_INTERRUPT_SWITCH,
				     1); //disable interrupt
	} else {
		int_state = 1;
		touch_i2c_write_block(chip_info->client,
				      SEC_CMD_CLEAR_EVENT_STACK, 0,
...
#define SELFTEST_RESULT(result, n)                                             \
	(((result) & (1 << (n))) == 0 ? "PASS" : "FAIL")
#define I2C_MAX 256
				...
				struct sec_testdata *sec_testdata)
{
	...
	touch_i2c_write_byte(chip_info->client, SEC_CMD_INTERRUPT_SWITCH,
			     1); //disable interrupt
	touch_i2c_write_byte(chip_info->client, SEC_CMD_INTERRUPT_LEVEL,
			     1); //disable interrupt
	touch_i2c_write_block(chip_info->client, SEC_CMD_CLEAR_EVENT_STACK, 0,
			      NULL); //clear event buffer
	...
	tData[0] = 0x1E;
	tData[1] = 0x7f;
	ret = touch_i2c_write_block(chip_info->client, SEC_CMD_SELFTEST_CHOICE,
				    sizeof(tData), tData);
	/* execute selftest */
...
static int sec_execute_lp_selftest(struct chip_data_s6d7at0 *chip_info,
				   struct sec_testdata *sec_testdata)
{
	int ret = -1;
	...
	/* execute selftest */
	ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_LP_SELFTEST,
				   0x03);
	if (ret < 0) {
...
static void sec_print_selftest_frame(struct chip_data_s6d7at0 *chip_info,
				     int test_type,
				     struct sec_testdata *sec_testdata,
				     short *frame)
{
	...
	for (i = 0; i < gap_para_x; i++) {
		...
		for (j = 0; j < gap_para_y; j++) {
			snprintf(
				pTmp, sizeof(pTmp), ",%3d",
				frame[((gap_para_y - 1 - j) * gap_para_x) + i]);
			strlcat(pStr, pTmp, 6 * (gap_para_y + 1));
		}
		TPD_INFO("%s\n", pStr);
		store_to_file(sec_testdata->fd, "%s\n", pStr);
...
static int sec_read_flash(struct chip_data_s6d7at0 *chip_info, u32 mem_addr,
			  u8 *buff, int mem_size)
{
	int ret;
	u8 cmd[5];
	u8 *data;
...
static int sec_print_selftest_report(struct chip_data_s6d7at0 *chip_info,
				     struct sec_testdata *sec_testdata,
				     int test_type, u8 test_result)
{
	int ret = -1;
	int i = 0;
	...
	case TOUCH_SELFTEST_ITEM_SENSOR_UNI:
		...
		TPD_INFO("%s: Sensor Uniformity: %s\n", __func__,
			 SELFTEST_RESULT(test_result, test_type));
	...
	if (read_bytes == sec_testdata->TX_NUM * sec_testdata->RX_NUM) {
		...
		for (i = 0; i < read_bytes; i += 2)
			print_buff[i / 2] =
				frame_buff[i] + (frame_buff[i + 1] << 8);
	}

	sec_print_selftest_frame(chip_info, test_type, sec_testdata,
				 print_buff);
	ret = SELFTEST_RESULT_CNT(test_result,
				  test_type >= 10 ? test_type - 10 : test_type);
...
static int sec_read_selftest_report(struct chip_data_s6d7at0 *chip_info,
				    struct sec_testdata *sec_testdata)
{
	int ret = 0;
	int i = 0;
	...
	/* read fw version */
	ret = touch_i2c_read_block(chip_info->client, SEC_READ_IMG_VERSION, 4,
				   report_buff);
	if (ret < 0) {
		TPD_INFO("%s: read FW version failed\n", __func__);
		return ret;
	}
	TPD_INFO("%s: FW version: %02X%02X%02X%02X\n", __func__, report_buff[0],
		 report_buff[1], report_buff[2], report_buff[3]);
	store_to_file(sec_testdata->fd, "FW version: %02X%02X%02X%02X\n",
		      report_buff[0], report_buff[1], report_buff[2],
		      report_buff[3]);
	...
	store_to_file(sec_testdata->fd, "%s: Test Report: %02X%02X%02X%02X\n\n",
		      __func__, report_buff[0], report_buff[1], report_buff[2],
		      report_buff[3]);

	/* print each result */
	for (i = 0; i < 10; i++) {
		ret = sec_print_selftest_report(chip_info, sec_testdata, i,
						report_buff[0]);
		if (ret < 0) {
	...
	if ((report_buff[0] & 0xff) == 0) {
		...
		TPD_INFO(
			"%s: Test Result FAIL... Sensor_Uniformity: %s, Raw_Variance_X: %s, Raw_Variance_Y: %s, P2P_min: %s, P2P_max: %s, Open: %s, Short: %s, High-Z: %s\n",
			__func__, SELFTEST_RESULT(report_buff[0], 0),
			SELFTEST_RESULT(report_buff[0], 1),
			SELFTEST_RESULT(report_buff[0], 2),
			SELFTEST_RESULT(report_buff[0], 3),
			SELFTEST_RESULT(report_buff[0], 4),
			SELFTEST_RESULT(report_buff[0], 5),
			SELFTEST_RESULT(report_buff[0], 6),
			SELFTEST_RESULT(report_buff[0], 7));
		store_to_file(
			sec_testdata->fd,
			"%s: Test Result FAIL... Sensor_Uniformity: %s Raw_Variance_X: %s Raw_Variance_Y: %s P2P_min: %s P2P_max: %s Open: %s Short: %s High-Z: %s\n",
			__func__, SELFTEST_RESULT(report_buff[0], 0),
...
static int sec_read_lp_selftest_report(struct chip_data_s6d7at0 *chip_info,
				       struct sec_testdata *sec_testdata)
{
	int ret = 0;
	int i = 0;
	...
	if ((report_buff[0] & 0xff) == 0) {
		...
		TPD_INFO(
			"%s: LP Test Result FAIL... RawData: %s, P2P_min: %s, P2P_max: %s\n",
			__func__, SELFTEST_RESULT(report_buff[0], 0),
			SELFTEST_RESULT(report_buff[0], 1),
			SELFTEST_RESULT(report_buff[0], 2));
		store_to_file(
...
static int sec_run_mis_cal(struct chip_data_s6d7at0 *chip_info)
{
	...
	TPD_INFO("%s: Mis cal Check\n", __func__);
	ret = touch_i2c_write_block(chip_info->client, SEC_CMD_MIS_CAL_CHECK, 0,
				    NULL); //set to verify calibration
	sec_mdelay(200);
	return ret;
}
			  ...
			  struct sec_testdata *sec_testdata)
{
	...
	do {
		...
		if (tBuff[0] == 0x00) {
			break;
		} else if ((tBuff[0] & MASK_2_BITS) == SEC_COORDINATE_EVENT ||
			   ((tBuff[0] & MASK_2_BITS) == SEC_STATUS_EVENT &&
			    ((tBuff[0] >> 2) & 0xF) == TYPE_STATUS_EVENT_INFO &&
			    tBuff[1] == SEC_ACK_PALM_MODE)) {
			...
			const struct i2c_device_id *id)
{
	...
	chip_info->sec_fw_watchdog_surport = of_property_read_bool(
		ts->dev->of_node, "sec_fw_watchdog_surport");

	//reset esd handle time interval
	if (ts->esd_handle_support) {
		sec_reset_esd(chip_info);
		ts->esd_info.esd_work_time = msecs_to_jiffies(
			SEC_TOUCH_ESD_CHECK_PERIOD); // change esd check interval to 0.2s
	...
	ret = touch_i2c_read_block(chip_info->client, SEC_READ_BOOT_STATUS, 1,
				   &data[0]);
	if (ret < 0)
	...
	ret = touch_i2c_read_block(chip_info->client, SEC_READ_IMG_VERSION, 8,
				   &data[0]);
	if (ret < 0)
