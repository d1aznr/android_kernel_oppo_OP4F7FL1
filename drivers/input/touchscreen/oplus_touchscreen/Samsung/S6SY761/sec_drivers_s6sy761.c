				 ...
				 bool enable)
{
	int ret = -1;
	u8 buf[3] = { 0x01, 0x00,
		      0x0A }; //direction enable, landscape grip area
	...
	edge_range[1] = (chip_info->grip_area.ver_edgescreen_y1 & 0x0F) << 4 |
			((chip_info->grip_area.ver_edgescreen_y2 >> 8) & 0x0F);
	...
	ret = touch_i2c_write_block(chip_info->client, SEC_CMD_GRIP_PCORNER,
				    sizeof(p_corner), p_corner);
	ret |= touch_i2c_write_block(chip_info->client, SEC_CMD_GRIP_LCORNER,
				     sizeof(l_corner), l_corner);
	ret |= touch_i2c_write_word(chip_info->client, SEC_CMD_GRIP_PZONE,
				    (chip_info->curved_rejsize << 8) & 0xFF00);
	ret |= touch_i2c_write_block(chip_info->client, SEC_CMD_ELI_LCORNER,
				     sizeof(l_eli_corner), l_eli_corner);
	ret |= touch_i2c_write_block(chip_info->client, SEC_CMD_GRIP_DIRECTION,
				     sizeof(buf), buf);
...
static int sec_wait_for_ready(struct chip_data_s6sy761 *chip_info,
			      unsigned int ack)
{
	int rc = -1;
...
static bool corner_point_filtered(struct chip_data_s6sy761 *chip_info,
				  uint16_t x, uint16_t y)
{
	uint16_t vertical_height = 120, hori_width = 100, width = 32,
		 corner_len = 60;
			   ...
			   char *data, int len)
{
	...
	touch_i2c_write_block(chip_info->client, SEC_CMD_TOUCH_RAWDATA_SETLEN,
			      2, buf);
	ret = touch_i2c_read_block(chip_info->client,
				   SEC_READ_TOUCH_SETLEN_RAWDATA, len, data);
	if (ret < 0) {
...
static void sec_main_register_read(struct seq_file *s, void *chip_data)
{
	...
	seq_printf(s,
		   "landscape eliminate area: 0x%02x, 0x%02x, 0x%02x, 0x%02x\n",
		   buf[0], buf[1], buf[2], buf[3]);
...
static void sec_set_grip_handle(void *chip_data, int para_num, char *buf)
{
	...
	case (1 << PORTRAIT_GRIP_BIT):
		sscanf(left_para, "%d,%d", &chip_info->grip_area.ver_grip_x,
		       &chip_info->grip_area.ver_grip_y);
		TPD_DETAIL("portrait grip:%d, %d\n",
			   chip_info->grip_area.ver_grip_x,
			   chip_info->grip_area.ver_grip_y);
		break;

	case (1 << LANDSCAPE_GRIP_BIT):
		sscanf(left_para, "%d,%d", &chip_info->grip_area.hor_grip_x,
		       &chip_info->grip_area.hor_grip_y);
		TPD_DETAIL("landscape grip:%d, %d\n",
			   chip_info->grip_area.hor_grip_x,
			   chip_info->grip_area.hor_grip_y);
		break;

	case (1 << LANDSCAPE_ELIMINATION_BIT):
		sscanf(left_para, "%d,%d", &chip_info->grip_area.hor_elim_x,
		       &chip_info->grip_area.hor_elim_y);
		TPD_DETAIL("landscape elimination:%d, %d\n",
			   chip_info->grip_area.hor_elim_x,
			   chip_info->grip_area.hor_elim_y);
		break;
...
int sec_kernel_grip_print_func(struct seq_file *s,
			       struct chip_data_s6sy761 *chip_info)
{
	if (!chip_info) {
...
static int sec_kernel_grip_read_func(struct seq_file *s, void *v)
{
	...
	struct chip_data_s6sy761 *chip_info =
		(struct chip_data_s6sy761 *)ts->chip_data;
...
static int sec_str_parse(char *in, char *name, uint16_t max_len,
			 uint16_t *array, uint16_t array_max)
{
		...
		if (in[i] < '0' || in[i] > '9') {
			if ((' ' == in[i]) || (0 == in[i]) || ('\n' == in[i]) ||
				...
				if (value_cnt <= array_max) {
					array[value_cnt++] = sec_str_to_int(
						in, start_pos, i - 1);
					start_pos = i + 1;
				} else {
					TPD_INFO(
...
static int sec_kernel_grip_parse(struct chip_data_s6sy761 *chip_info,
				 char *input, int len)
{
	bool cmd = false;
	bool object = false;
...
static ssize_t sec_kernel_grip_write(struct file *file,
				     ...
				     loff_t *ppos)
{
	...
	mutex_lock(&ts->mutex);
	sec_kernel_grip_parse((struct chip_data_s6sy761 *)ts->chip_data, buf,
			      count);
...
static void sec_init_kernel_grip_proc(struct proc_dir_entry *prEntry_tp,
