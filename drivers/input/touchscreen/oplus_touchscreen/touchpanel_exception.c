...
static inline int tp_memcpy(void *dest, unsigned int dest_size, void *src,
			    unsigned int src_size, unsigned int count)
{
...
static int tp_olc_raise_exception(tp_excep_type excep_tpye, void *summary,
				  unsigned int summary_size)
{
	...
	exp_info->time = 0;
	exp_info->id =
		(TP_RESERVED_ID << 20) | (TP_MODULE_ID << 12) | excep_tpye;
	exp_info->pid = 0;
	exp_info->exceptionType = EXCEPTION_KERNEL;
	exp_info->faultLevel = 0;
	exp_info->logOption = LOG_KERNEL | LOG_MAIN;
	tp_memcpy(exp_info->module, sizeof(exp_info->module), TP_MODULE_NAME,
		  sizeof(TP_MODULE_NAME), sizeof(TP_MODULE_NAME));
	tp_memcpy(exp_info->logPath, sizeof(exp_info->logPath), TP_LOG_PATH,
		  sizeof(TP_LOG_PATH), sizeof(TP_LOG_PATH));

	tp_memcpy(exp_info->summary, sizeof(exp_info->summary), summary,
...
int tp_exception_report(void *tp_exception_data, tp_excep_type excep_tpye,
			void *summary, unsigned int summary_size)
{
	int ret = -1;

	struct exception_data *exception_data =
		(struct exception_data *)tp_exception_data;
	...
	case EXCEP_BUS:
		...
		if (exception_data->bus_error_count > MAX_BUS_ERROR_COUNT &&
			...
			ret = tp_olc_raise_exception(excep_tpye, summary,
						     summary_size);
		}
		break;
	default:
