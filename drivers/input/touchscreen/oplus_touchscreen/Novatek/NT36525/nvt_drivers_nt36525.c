...
static int32_t nvt_read_fw_noise(struct chip_data_nt36525 *chip_info,
				 int32_t config_Diff_Test_Frame, int32_t *xdata,
				 int32_t *xdata_n, int32_t xdata_len);
static int8_t nvt_switch_FreqHopEnDis(struct chip_data_nt36525 *chip_info,
				      uint8_t FreqHopEnDis);
static void nvt_read_mdata(struct chip_data_nt36525 *chip_info,
...
static void nvt_esd_check_enable(struct chip_data_nt36525 *chip_info,
				 bool enable);
...
int32_t nvt_i2c_write(struct i2c_client *client, uint16_t address, uint8_t *buf,
		      uint16_t len)
{
	int ret = -1;
...
int32_t nvt_i2c_read(struct i2c_client *client, uint16_t address, uint8_t *buf,
		     uint16_t len)
{
	int ret = -1;
	uint16_t addr_cpy = client->addr;
...
static int8_t nvt_check_chip_ver_trim(struct chip_data_nt36525 *chip_info)
{
	...
	for (retry = 5; retry > 0; retry--) {
		...
		buf[1] = 0x01;
		buf[2] = 0xF6;
		ret |= nvt_i2c_write(chip_info->client, I2C_BLDR_Address, buf,
				     3);
		...
		buf[5] = 0x00;
		buf[6] = 0x00;
		ret |= nvt_i2c_read(chip_info->client, I2C_BLDR_Address, buf,
				    7);
		if (ret < 0) {
		...
		for (list = 0; list < (sizeof(trim_id_table) /
				       sizeof(struct nvt_trim_id_table));
		     list++) {
			found_nvt_chip = 0;
...
int32_t nvt_check_fw_reset_state(struct chip_data_nt36525 *chip_info,
...
{
	int ret = -1;
	struct chip_data_nt36525 *chip_info =
		(struct chip_data_nt36525 *)chip_data;
...
static int nvt_enable_black_gesture(struct chip_data_nt36525 *chip_info,
				    bool enable)
{
	int8_t ret = -1;
...
static int nvt_enable_edge_limit(struct chip_data_nt36525 *chip_info,
				 bool enable)
{
	int8_t ret = -1;
...
static int nvt_enable_charge_mode(struct chip_data_nt36525 *chip_info,
				  bool enable)
{
	int8_t ret = -1;
...
int32_t Resume_PD(struct chip_data_nt36525 *chip_info)
{
	...
	while (1) {
		...
		if (unlikely(retry > 20)) {
			TPD_INFO(
				"Check 0xAA (Resume Command) error!! status=0x%02X\n",
				buf[1]);
			return -1;
		}
	}
...
int32_t Check_CheckSum(struct chip_data_nt36525 *chip_info,
		       const struct firmware *fw)
{
	...
	for (i = 0; i < BLOCK_64KB_NUM; i++) {
		if (fw_bin_size > (i * SIZE_64KB)) {
			...
			ret = nvt_i2c_write(chip_info->client, I2C_HW_Address,
					    buf, 7);
			if (ret < 0) {
			...
			while (1) {
				...
				buf[0] = 0x00;
				buf[1] = 0x00;
				ret = nvt_i2c_read(chip_info->client,
						   I2C_HW_Address, buf, 2);
				if (ret < 0) {
					TPD_INFO(
						"Check 0xAA (Fast Read Command) error!\n");
					return ret;
				}
				if (buf[1] == 0xAA) {
					break;
				...
				if (unlikely(retry > 5)) {
					TPD_INFO(
						"Check 0xAA (Fast Read Command) failed, buf[1]=0x%02X, retry=%d\n",
						buf[1], retry);
					return -1;
				}
			}
			...
			ret = nvt_i2c_write(chip_info->client, I2C_BLDR_Address,
					    buf, 3);
			if (ret < 0) {
				TPD_INFO(
					"Read Checksum (write addr high byte & middle byte) error!\n");
				return ret;
			}
			// Read Checksum
			...
			buf[1] = 0x00;
			buf[2] = 0x00;
			ret = nvt_i2c_read(chip_info->client, I2C_BLDR_Address,
					   buf, 3);
			if (ret < 0) {
			...
			TPD_INFO(
				"RD_Filechksum[%d]=0x%04X, WR_Filechksum[%d]=0x%04X\n",
...
int32_t Check_FW_Ver(struct chip_data_nt36525 *chip_info,
		     const struct firmware *fw)
{
	uint8_t buf[16] = { 0 };
	int32_t ret = 0;
...
int32_t Init_BootLoader(struct chip_data_nt36525 *chip_info)
{
	...
	while (1) {
		...
		if (unlikely(retry > 20)) {
			TPD_INFO(
				"Check 0xAA (Inittial Flash Block) error!! status=0x%02X\n",
				buf[1]);
			return -1;
		}
	}
...
int32_t Erase_Flash(struct chip_data_nt36525 *chip_info, uint32_t fw_len)
{
	...
	while (1) {
		...
		if (ret < 0) {
			TPD_INFO(
				"Check 0xAA (Write Enable for Write Status Register) error!!\n");
			return ret;
		}
		if (buf[1] == 0xAA) {
			break;
		...
		if (unlikely(retry > 20)) {
			TPD_INFO(
				"Check 0xAA (Write Enable for Write Status Register) error! status=0x%02X\n",
				buf[1]);
			return -1;
		}
	}
	...
	while (1) {
		...
		if (ret < 0) {
			TPD_INFO(
				"Check 0xAA (Write Status Register) error!!\n");
			return ret;
		}
		if (buf[1] == 0xAA) {
			break;
		...
		if (unlikely(retry > 20)) {
			TPD_INFO(
				"Check 0xAA (Write Status Register) error! status=0x%02X\n",
				buf[1]);
			return -1;
		}
	}
	...
	while (1) {
		...
		if (ret < 0) {
			TPD_INFO(
				"Read Status (for Write Status Register) error!!(%d)\n",
				ret);
			return ret;
		}
		...
		if (ret < 0) {
			TPD_INFO(
				"Check 0xAA (Read Status for Write Status Register) error!\n");
			return ret;
		}
		...
		if (unlikely(retry > 100)) {
			TPD_INFO(
				"Check 0xAA (Read Status for Write Status Register) failed, buf[1]=0x%02X, buf[2]=0x%02X, retry=%d\n",
				buf[1], buf[2], retry);
			return -1;
		}
	}
	...
	for (i = 0; i < count; i++) {
		...
		while (1) {
			...
			buf[0] = 0x00;
			buf[1] = 0x00;
			ret = nvt_i2c_read(chip_info->client, I2C_HW_Address,
					   buf, 2);
			if (ret < 0) {
				TPD_INFO(
					"Check 0xAA (Write Enable) error!!(%d)\n",
					i);
				return ret;
			}
			if (buf[1] == 0xAA) {
			...
			if (unlikely(retry > 20)) {
				TPD_INFO(
					"Check 0xAA (Write Enable) error!! status=0x%02X\n",
					buf[1]);
				return -1;
			}
		}
		...
		while (1) {
			...
			if (unlikely(retry > 20)) {
				TPD_INFO(
					"Check 0xAA (Sector Erase) failed, buf[1]=0x%02X, retry=%d\n",
					buf[1], retry);
				return -1;
			}
		}
		...
		while (1) {
			...
			if (ret < 0) {
				TPD_INFO(
					"Check 0xAA (Read Status) error!!(%d)\n",
					i);
				return ret;
			}
			if ((buf[1] == 0xAA) && (buf[2] == 0x00)) {
			...
			if (unlikely(retry > 100)) {
				TPD_INFO(
					"Check 0xAA (Read Status) failed, buf[1]=0x%02X, buf[2]=0x%02X, retry=%d\n",
					buf[1], buf[2], retry);
				return -1;
			}
		}
...
int32_t Write_Flash(struct chip_data_nt36525 *chip_info,
		    const struct firmware *fw)
{
	uint8_t buf[64] = { 0 };
	...
	for (i = 0; i < count; i++) {
		...
		while (1) {
			...
			if (unlikely(retry > 20)) {
				TPD_INFO(
					"Check 0xAA (Page Program) failed, buf[1]=0x%02X, retry=%d\n",
					buf[1], retry);
				return -1;
			}
		}
...
int32_t Verify_Flash(struct chip_data_nt36525 *chip_info,
		     const struct firmware *fw)
{
	uint8_t buf[64] = { 0 };
	uint32_t XDATA_Addr = chip_info->mmap->READ_FLASH_CHECKSUM_ADDR;
				     ...
				     bool force)
{
	...
	if (*(fw->data + FW_BIN_VER_OFFSET) +
	    ...
	    0xFF) {
		TPD_INFO(
			"bin file FW_VER(0x%02x) + FW_VER_BAR(0x%02x) should be 0xFF\n",
	...
	if (!force) {
		...
		ret = Check_CheckSum(chip_info,
				     fw); //-1:error, 0:not match, 1:match
		if (((ret == 0) && (Check_FW_Ver(chip_info, fw) == 1)) ||
		    (ret == 1)) {
				...
				int max_num)
{
	...
	ret = nvt_i2c_read(chip_info->client, I2C_FW_Address, buf,
			   POINT_DATA_LEN + 1);
	if (ret < 0) {
...
static int nvt_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	ret = nvt_i2c_read(chip_info->client, I2C_FW_Address, point_data,
			   POINT_DATA_LEN + 1);
	if (ret < 0) {
...
static void nvt_black_screen_test(void *chip_data, char *message)
{
	...
	if (nvt_get_fw_pipe(chip_info) == 0)
		nvt_read_mdata(chip_info, chip_info->mmap->RAW_PIPE0_ADDR,
			       raw_data, buf_len);
	else
		nvt_read_mdata(chip_info, chip_info->mmap->RAW_PIPE1_ADDR,
			       raw_data, buf_len);
			   ...
			   int32_t xdata_len)
{
	...
	for (i = 0; i < ((dummy_len + data_len) / XDATA_SECTOR_SIZE); i++) {
		...
		for (j = 0; j < (XDATA_SECTOR_SIZE / I2C_TANSFER_LENGTH); j++) {
			...
			nvt_i2c_read(chip_info->client, I2C_FW_Address, buf,
	...
	case NVT_RAWDATA:
		...
		if (pipe == 0)
			nvt_read_mdata(chip_info,
				       chip_info->mmap->RAW_PIPE0_ADDR, xdata,
				       buf_len);
		else
			nvt_read_mdata(chip_info,
				       chip_info->mmap->RAW_PIPE1_ADDR, xdata,
				       buf_len);
		break;
	case NVT_DIFFDATA:
		...
		if (pipe == 0)
			nvt_read_mdata(chip_info,
				       chip_info->mmap->DIFF_PIPE0_ADDR, xdata,
				       buf_len);
		else
			nvt_read_mdata(chip_info,
				       chip_info->mmap->DIFF_PIPE1_ADDR, xdata,
				       buf_len);
		break;
	case NVT_BASEDATA:
		seq_printf(s, "basline_data:\n");
		nvt_read_mdata(chip_info, chip_info->mmap->BASELINE_ADDR, xdata,
			       buf_len);
		break;
	default:
...
static void nvt_enable_noise_collect(struct chip_data_nt36525 *chip_info,
				     int32_t frame_num)
{
	int ret = -1;
	uint8_t buf[8] = { 0 };
...
static int32_t
nvt_polling_hand_shake_status(struct chip_data_nt36525 *chip_info)
{
	uint8_t buf[8] = { 0 };
	int32_t i = 0;
	const int32_t retry = 50;
				 ...
				 int32_t *xdata_n, int32_t xdata_len)
{
	...
	if (nvt_get_fw_pipe(chip_info) == 0)
		nvt_read_mdata(chip_info, chip_info->mmap->DIFF_PIPE0_ADDR,
			       xdata, xdata_len);
	else
		nvt_read_mdata(chip_info, chip_info->mmap->DIFF_PIPE1_ADDR,
			       xdata, xdata_len);
...
static void nvt_enable_doze_noise_collect(struct chip_data_nt36525 *chip_info,
					  int32_t frame_num)
{
	int ret = -1;
	uint8_t buf[8] = { 0 };
...
static int32_t nvt_read_doze_fw_noise(struct chip_data_nt36525 *chip_info,
				      ...
				      int32_t *xdata_n, int32_t xdata_len)
{
	...
	for (x = 0; x < doze_X_Channel; x++) {
		//---change xdata index---
		buf[0] = 0xFF;
		buf[1] = ((chip_info->mmap->DIFF_PIPE0_ADDR +
			   rx_num * doze_X_Channel * x) >>
			  16) &
			 0xFF;
		buf[2] = ((chip_info->mmap->DIFF_PIPE0_ADDR +
			   rx_num * doze_X_Channel * x) >>
			  8) &
			 0xFF;
		nvt_i2c_write(chip_info->client, I2C_FW_Address, buf, 3);
		...
		//---read data---
		buf[0] = (chip_info->mmap->DIFF_PIPE0_ADDR +
			  rx_num * doze_X_Channel * x) &
			 0xFF;
		nvt_i2c_read(chip_info->client, I2C_FW_Address, buf,
			     rx_num * 2 + 1);
...
static int32_t nvt_read_doze_baseline(struct chip_data_nt36525 *chip_info,
				      int32_t doze_X_Channel, int32_t *xdata,
				      int32_t xdata_len)
{
	uint8_t buf[128] = { 0 };
	...
	for (x = 0; x < doze_X_Channel; x++) {
		//---change xdata index---
		buf[0] = 0xFF;
		buf[1] = ((chip_info->mmap->DOZE_GM_S1D_SCAN_RAW_ADDR +
			   rm_num * doze_X_Channel * x) >>
			  16) &
			 0xFF;
		buf[2] = ((chip_info->mmap->DOZE_GM_S1D_SCAN_RAW_ADDR +
			   rm_num * doze_X_Channel * x) >>
			  8) &
			 0xFF;
		nvt_i2c_write(chip_info->client, I2C_FW_Address, buf, 3);
		...
		//---read data---
		buf[0] = (chip_info->mmap->DOZE_GM_S1D_SCAN_RAW_ADDR +
			  rm_num * doze_X_Channel * x) &
			 0xFF;
		nvt_i2c_read(chip_info->client, I2C_FW_Address, buf,
			     rm_num * 2 + 1);
...
static int32_t nvt_read_fw_short(struct chip_data_nt36525 *chip_info,
				 int32_t *xdata, int32_t xdata_len)
{
	...
	for (y = 0; y < chip_info->hw_res->RX_NUM; y++) {
		//---change xdata index---
		buf[0] = 0xFF;
		buf[1] = (uint8_t)(((raw_pipe_addr +
				     y * chip_info->hw_res->TX_NUM * 2) >>
				    16) &
				   0xFF);
		buf[2] = (uint8_t)(((raw_pipe_addr +
				     y * chip_info->hw_res->TX_NUM * 2) >>
				    8) &
				   0xFF);
		nvt_i2c_write(chip_info->client, I2C_FW_Address, buf, 3);
...
static int32_t nvt_read_fw_open(struct chip_data_nt36525 *chip_info,
				int32_t *xdata, int32_t xdata_len)
{
	uint32_t raw_pipe_addr = 0;
	uint8_t *rawdata_buf = NULL;
	...
	for (y = 0; y < rx_num; y++) {
		//---change xdata index---
		buf[0] = 0xFF;
		buf[1] = (uint8_t)(((raw_pipe_addr + y * tx_num * 2) >> 16) &
				   0xFF);
		buf[2] = (uint8_t)(((raw_pipe_addr + y * tx_num * 2) >> 8) &
				   0xFF);
		nvt_i2c_write(chip_info->client, I2C_FW_Address, buf, 3);
			  ...
			  struct nvt_testdata *nvt_testdata)
{
	...
	memset(raw_data, 0, buf_len);
	nvt_read_mdata(chip_info, chip_info->mmap->BASELINE_ADDR, raw_data,
		       buf_len);
...
static int nvt_tp_probe(struct i2c_client *client,
