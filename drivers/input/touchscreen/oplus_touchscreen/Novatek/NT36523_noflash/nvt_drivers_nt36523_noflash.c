...
static void nvt_printk_fw_history(void *chip_data,
				  uint32_t NVT_MMAP_HISTORY_ADDR)
{
	uint8_t i = 0;
	uint8_t buf[66];
	char str[128];
	struct chip_data_nt36523 *chip_info =
		(struct chip_data_nt36523 *)chip_data;
...
static void nvt_esd_check_enable(struct chip_data_nt36523 *chip_info,
				 bool enable)
{
...
void nvt_esd_reverse_phase(struct chip_data_nt36523 *chip_info)
{
	...
	if (buf[1] == 0xA5) {
		...
		if (buf[1] == 0x00)
			...
			TPD_INFO(
				"Failed to switch back to phase II, buf[1]=0x%02X\n",
				buf[1]);
	}
}
...
*******************************************************/
static int32_t
nvt_check_fw_reset_state_noflash(struct chip_data_nt36523 *chip_info,
	...
	for (list = 0; list < chip_info->partition; list++) {
		...
		if ((list >= chip_info->ilm_dlm_num) &&
			...
			if (find_bin_header == 0) {
				...
				pos = 0x30 +
				      (0x10 * (list - chip_info->ilm_dlm_num));
			} else if (find_bin_header &&
				   chip_info->cascade_2nd_header_info) {
			...
			if ((chip_info->bin_map[list].BIN_addr < tmp_end) &&
		...
		if (list >= (chip_info->ilm_dlm_num + info_sec_num)) {
			...
			chip_info->bin_map[list].crc =
				byte_to_word(&fwdata[pos + 12]);
			snprintf(
				chip_info->bin_map[list].name, 12, "Overlay-%u",
				(list - chip_info->ilm_dlm_num - info_sec_num));
		}
...
static int32_t Download_Init(struct chip_data_nt36523 *chip_info)
{
	...
	if (chip_info->fwbuf == NULL) {
		chip_info->fwbuf = (uint8_t *)kzalloc(
...
void nvt_tx_auto_copy_mode(struct chip_data_nt36523 *chip_info)
{
	...
	nvt_write_addr(chip_info->s_client,
		       chip_info->trim_id_table.mmap->TX_AUTO_COPY_EN, 0x69);
...
int32_t nvt_check_spi_dma_tx_info(struct chip_data_nt36523 *chip_info)
{
	...
	for (i = 0; i < retry; i++) {
		...
		nvt_set_page(chip_info,
			     chip_info->trim_id_table.mmap->SPI_DMA_TX_INFO);
...
nvt_f2c_retry:
	...
	f2c_control = buf[1] | (0x01 << BIT_F2C_EN) |
	...
	if (((buf[2] & 0x04) >> 2) != 0x01) {
		...
		if (f2c_retry <= retry_max) {
			...
			TPD_INFO(
				"check TH_CPU_CHK failed!, buf[1]=0x%02X, buf[2]=0x%02X\n",
				buf[1], buf[2]);
			return -EIO;
		}
	}
	...
	fail:
		...
		if (unlikely(retry > 2) || chip_info->using_headfile) {
			...
			if (nvt_check_crc_done_ilm_err(chip_info)) {
				TPD_INFO(
					"set display off to trigger display esd recovery.\n");
...
*/
static u32 nvt_trigger_reason(void *chip_data, int gesture_enable,
			      int is_suspended)
{
	struct chip_data_nt36523 *chip_info =
		(struct chip_data_nt36523 *)chip_data;
	...
	ret = CTP_SPI_READ(chip_info->s_client, chip_info->point_data,
			   POINT_DATA_LEN + 1);
	if (ret < 0) {
	...
	ret = nvt_ts_point_data_checksum(chip_info->point_data,
	...
	if (ts->pen_support) {
		ret = nvt_ts_pen_data_checksum(chip_info->point_data + 66,
					       PEN_DATA_LEN);
		if (ret) {
			return IRQ_IGNORE;
		}
...
static int32_t nvt_ts_pen_data_checksum(uint8_t *buf, uint8_t length)
{
	...
	if (checksum != buf[length - 1]) {
		TPD_INFO(
			"pen packet checksum not match. (buf[%d]=0x%02X, checksum=0x%02X)\n",
				    ...
				    uint8_t u8SubCmd1)
{
	...
	if (unlikely(i == retry)) {
		TPD_INFO(
			"send Cmd 0x%02X 0x%02X 0x%02X failed, buf[1]=0x%02X\n",
			u8Cmd, u8SubCmd, u8SubCmd1, buf[1]);
		return -1;
	} else {
		TPD_INFO(
			"send Cmd 0x%02X 0x%02X 0x%02X success, tried %d times\n",
...
static void
nvt_read_debug_gesture_coordinate_buffer(struct chip_data_nt36523 *chip_info,
					 ...
					 int32_t xdata_len)
{
	...
	if (residual_len != 0) {
		...
		for (j = 0; j < (residual_len / SPI_TANSFER_LENGTH + 1); j++) {
			...
			nvt_set_page(chip_info,
				     xdata_addr + xdata_len - residual_len +
					     (SPI_TANSFER_LENGTH * j));
			...
			//---copy buf to xdata_tmp---
			for (k = 0; k < SPI_TANSFER_LENGTH; k++) {
				xdata_tmp[(dummy_len + xdata_len - residual_len) +
					  SPI_TANSFER_LENGTH * j + k] =
					buf[k + 1];
...
static void
nvt_dbg_gesture_record_coor_read(struct chip_data_nt36523 *chip_info,
				 u8 pointdata)
{
	u8 *xdata = NULL;
	uint32_t buf_len = 0;
	...
	nvt_read_debug_gesture_coordinate_buffer(
		chip_info, NVT_MMAP_DEBUG_STATUS_CHANGE_DIFFDATA, xdata,
		buf_len);
...
static int nvt_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	if (chip_info->dev_pm_suspend) {
		ret = wait_for_completion_timeout(
			&chip_info->dev_pm_resume_completion,
			msecs_to_jiffies(500));
		if (!ret) {
			TPD_INFO(
				"system (i2c/spi) can't finished resuming procedure, skip it!\n");
			return -1;
		}
	}
	...
	TPD_INFO(
		"%s, gesture_id: 0x%x, func_type: 0x%x, gesture_type: %d, clockwise: %d, points: (%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)\n",
...
static __maybe_unused int nvt_enable_debug_gesture_coordinate_record_mode(
	struct chip_data_nt36523 *chip_info, bool enable)
{
	int8_t ret = -1;
...
nvt_enable_debug_gesture_coordinate_mode(struct chip_data_nt36523 *chip_info,
					 bool enable)
{
	int8_t ret = -1;
...
static int nvt_enable_black_gesture(struct chip_data_nt36523 *chip_info,
				    bool enable)
{
	int ret = -1;
	...
	if (enable) {
		...
		if ((chip_info->gesture_state & (1 << PENDETECT)) &&
		    *chip_info->is_pen_connected &&
		    !(*chip_info->is_pen_attracted)) {
...
static int nvt_enable_edge_limit(struct chip_data_nt36523 *chip_info, int state)
{
	...
	struct touchpanel_data *ts = spi_get_drvdata(chip_info->s_client);
	TPD_INFO(
		"%s:state = %d, limit_corner = %d, level = %d, chip_info->is_sleep_writed = %d\n",
...
nvt_set_jitter_para(struct chip_data_nt36523 *chip_info, int level)
{
	int8_t ret = -1;
	...
	ret = nvt_extend_cmd2_store(chip_info, EVENTBUFFER_EXT_CMD,
				    EVENTBUFFER_EXT_JITTER_LEVEL, level);

	return ret;
}

static int __maybe_unused
nvt_set_smooth_para(struct chip_data_nt36523 *chip_info, int level)
{
	int8_t ret = -1;
	...
	ret = nvt_extend_cmd2_store(chip_info, EVENTBUFFER_EXT_CMD,
				    EVENTBUFFER_EXT_SMOOTH_LEVEL, level);
...
static int nvt_enable_pen_mode(struct chip_data_nt36523 *chip_info, bool enable)
{
	...
	if (enable) {
		ret = nvt_extend_cmd_store(chip_info, EVENTBUFFER_EXT_CMD,
					   EVENTBUFFER_EXT_PEN_MODE_ON);
	} else {
		ret = nvt_extend_cmd_store(chip_info, EVENTBUFFER_EXT_CMD,
					   EVENTBUFFER_EXT_PEN_MODE_OFF);
	}
...
#ifdef CONFIG_OPLUS_TP_APK
static __maybe_unused int
nvt_enable_hopping_polling_mode(struct chip_data_nt36523 *chip_info,
				bool enable)
{
	int8_t ret = -1;
...
static __maybe_unused int
nvt_enable_hopping_fix_freq_mode(struct chip_data_nt36523 *chip_info,
				 bool enable)
{
	int8_t ret = -1;
	...
	if (enable)
		...
		ret = nvt_cmd_store(chip_info,
				    EVENTBUFFER_HOPPING_FIX_FREQ_OFF);

	return ret;
}

static __maybe_unused int
nvt_enable_debug_msg_diff_mode(struct chip_data_nt36523 *chip_info, bool enable)
{
	int8_t ret = -1;
...
static __maybe_unused int
nvt_enable_water_polling_mode(struct chip_data_nt36523 *chip_info, bool enable)
{
	int8_t ret = -1;
		...
		if (panel_data->manufacture_info.version) {
			...
			if (panel_data->vid_len == 0) {
				...
				if (ver_len <= 11) {
					...
					snprintf(panel_data->manufacture_info
								 .version +
							 9,
						 sizeof(dev_version),
						 dev_version);
...
static int32_t
nvt_polling_hand_shake_status(struct chip_data_nt36523 *chip_info)
{
	uint8_t buf[8] = { 0 };
	int32_t i = 0;
					 ...
					 const struct firmware *fw, bool force)
{
	...
	if (ret) {
		if (fw != request_fw_headfile) {
			TPD_INFO(
				"Image fw check checksum failed, reload fw from array\n");
...
static void nvt_enable_doze_noise_collect(struct chip_data_nt36523 *chip_info,
					  int32_t frame_num)
{
	int ret = -1;
	uint8_t buf[8] = { 0 };
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((raw_data[iArrayIndex] >
					...
					TPD_INFO(
						"LPWG_Rawdata Test failed at rawdata[%d][%d] = %d[%d %d]\n",
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((noise_p_data[iArrayIndex] >
					...
					TPD_INFO(
						"LPWG Noise RawData_Diff_Max Test failed at rawdata[%d][%d] = %d[%d %d]\n",
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((noise_n_data[iArrayIndex] >
					...
					TPD_INFO(
						"LPWG Noise RawData_Diff_Min Test failed at rawdata[%d][%d] = %d[%d %d]\n",
...
TEST_END:
	...
	if (!err_cnt) {
		...
		snprintf(
			data_buf, 128,
			"/sdcard/TpTestReport/screenOff/OK/tp_testlimit_%s_%02d%02d%02d-%02d%02d%02d-utc.csv",
		...
		snprintf(
			data_buf, 128,
			"/sdcard/TpTestReport/screenOff/NG/tp_testlimit_%s_%02d%02d%02d-%02d%02d%02d-utc.csv",
			(char *)all_test_result,
			(rtc_now_time.tm_year + 1900) % 100,
			rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
			rtc_now_time.tm_hour, rtc_now_time.tm_min,
...
static void nvt_read_fw_history(struct seq_file *s, void *chip_data,
				uint32_t NVT_MMAP_HISTORY_ADDR)
{
	uint8_t i = 0;
	uint8_t buf[66];
			  ...
			  struct nvt_testdata *nvt_testdata)
{
	...
	int32_t *pen_ring_x_noise_data_P = NULL,
		*pen_ring_x_noise_data_N = NULL;
	int32_t *pen_ring_y_noise_data_P = NULL,
		*pen_ring_y_noise_data_N = NULL;
#endif
	struct timespec now_time;
	struct rtc_time rtc_now_time;
	char *p_node = NULL;
...
#ifdef CONFIG_TOUCHPANEL_NT_PEN_SUPPORT
	      && pen_tip_x_data && pen_tip_y_data && pen_ring_x_data &&
	      pen_ring_y_data && pen_tip_x_noise_p_data &&
	      pen_tip_x_noise_n_data && pen_tip_y_noise_p_data &&
	      pen_tip_y_noise_n_data && pen_ring_x_noise_p_data &&
	      pen_ring_x_noise_n_data && pen_ring_y_noise_p_data &&
	      pen_ring_y_noise_n_data
#endif
...
#ifdef CONFIG_TOUCHPANEL_NT_PEN_SUPPORT
	      && pen_tip_x_record && pen_tip_y_record && pen_ring_x_record &&
	      pen_ring_y_record && pen_tip_x_noise_p_record &&
	      pen_tip_x_noise_n_record && pen_tip_y_noise_p_record &&
	      pen_tip_y_noise_n_record && pen_ring_x_noise_p_record &&
	      pen_ring_x_noise_n_record && pen_ring_y_noise_p_record &&
	      pen_ring_y_noise_n_record
#endif
	...
	digital_diff_rawdata_P =
		(int32_t *)(nvt_testdata->fw->data +
			    ph->array_FW_Digital_Diff_P_offset);
	digital_diff_rawdata_N =
		(int32_t *)(nvt_testdata->fw->data +
			    ph->array_FW_Digital_Diff_N_offset);
#endif
#ifdef CONFIG_TOUCHPANEL_NT_PEN_SUPPORT
	if (ts->pen_support) {
		pen_tip_x_data_P =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_tip_x_data_p_offset);
		pen_tip_x_data_N =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_tip_x_data_n_offset);
		pen_tip_y_data_P =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_tip_y_data_p_offset);
		pen_tip_y_data_N =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_tip_y_data_n_offset);
		pen_ring_x_data_P =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_ring_x_data_p_offset);
		pen_ring_x_data_N =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_ring_x_data_n_offset);
		pen_ring_y_data_P =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_ring_y_data_p_offset);
		pen_ring_y_data_N =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_ring_y_data_n_offset);
		pen_tip_x_noise_data_P =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_tip_x_diff_p_offset);
		pen_tip_x_noise_data_N =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_tip_x_diff_n_offset);
		pen_tip_y_noise_data_P =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_tip_y_diff_p_offset);
		pen_tip_y_noise_data_N =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_tip_y_diff_n_offset);
		pen_ring_x_noise_data_P =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_ring_x_diff_p_offset);
		pen_ring_x_noise_data_N =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_ring_x_diff_n_offset);
		pen_ring_y_noise_data_P =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_ring_y_diff_p_offset);
		pen_ring_y_noise_data_N =
			(int32_t *)(nvt_testdata->fw->data +
				    ph->array_pen_ring_y_diff_n_offset);
	} /* ts->pen_support */
#endif
	...
	if (ts->pen_support) {
		...
		nvt_read_get_num_mdata(
			chip_info,
			chip_info->trim_id_table.mmap->PEN_2D_BL_TIP_X_ADDR,
			pen_tip_x_data, tx_num * pen_rx_num);
		nvt_read_get_num_mdata(
			chip_info,
			chip_info->trim_id_table.mmap->PEN_2D_BL_TIP_Y_ADDR,
			pen_tip_y_data, pen_tx_num * rx_num);
		nvt_read_get_num_mdata(
			chip_info,
			chip_info->trim_id_table.mmap->PEN_2D_BL_RING_X_ADDR,
			pen_ring_x_data, tx_num * pen_rx_num);
		nvt_read_get_num_mdata(
			chip_info,
			chip_info->trim_id_table.mmap->PEN_2D_BL_RING_Y_ADDR,
		...
		for (j = 0; j < pen_rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((pen_tip_x_data[iArrayIndex] >
				     pen_tip_x_data_P[iArrayIndex]) ||
				    (pen_tip_x_data[iArrayIndex] <
				     pen_tip_x_data_N[iArrayIndex])) {
					pen_tip_x_result = NVT_MP_FAIL;
					pen_tip_x_record[iArrayIndex] = 1;
					TPD_INFO(
						"pen tip x data Test failed at data[%d][%d] = %d [%d,%d]\n",
						i, j,
						pen_tip_x_data[iArrayIndex],
						pen_tip_x_data_N[iArrayIndex],
						pen_tip_x_data_P[iArrayIndex]);
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < pen_tx_num; i++) {
				...
				if ((pen_tip_y_data[iArrayIndex] >
				     pen_tip_y_data_P[iArrayIndex]) ||
				    (pen_tip_y_data[iArrayIndex] <
				     pen_tip_y_data_N[iArrayIndex])) {
					pen_tip_y_result = NVT_MP_FAIL;
					pen_tip_y_record[iArrayIndex] = 1;
					TPD_INFO(
						"pen tip y data Test failed at data[%d][%d] = %d [%d,%d]\n",
						i, j,
						pen_tip_y_data[iArrayIndex],
						pen_tip_y_data_N[iArrayIndex],
						pen_tip_y_data_P[iArrayIndex]);
		...
		for (j = 0; j < pen_rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((pen_ring_x_data[iArrayIndex] >
				     pen_ring_x_data_P[iArrayIndex]) ||
				    (pen_ring_x_data[iArrayIndex] <
				     pen_ring_x_data_N[iArrayIndex])) {
					pen_ring_x_result = NVT_MP_FAIL;
					pen_ring_x_record[iArrayIndex] = 1;
					TPD_INFO(
						"pen ring x data Test failed at data[%d][%d] = %d [%d,%d]\n",
						i, j,
						pen_ring_x_data[iArrayIndex],
						pen_ring_x_data_N[iArrayIndex],
						pen_ring_x_data_P[iArrayIndex]);
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < pen_tx_num; i++) {
				...
				if ((pen_ring_y_data[iArrayIndex] >
				     pen_ring_y_data_P[iArrayIndex]) ||
				    (pen_ring_y_data[iArrayIndex] <
				     pen_ring_y_data_N[iArrayIndex])) {
					pen_ring_y_result = NVT_MP_FAIL;
					pen_ring_y_record[iArrayIndex] = 1;
					TPD_INFO(
						"pen ring y data Test failed at data[%d][%d] = %d [%d,%d]\n",
						i, j,
						pen_ring_y_data[iArrayIndex],
						pen_ring_y_data_N[iArrayIndex],
						pen_ring_y_data_P[iArrayIndex]);
	...
	if (ts->pen_support) {
		...
		nvt_read_get_num_mdata(
			chip_info,
			chip_info->trim_id_table.mmap->PEN_2D_DIFF_TIP_X_ADDR,
			pen_tip_x_noise_p_data, tx_num * pen_rx_num);
		nvt_read_get_num_mdata(
			chip_info,
			chip_info->trim_id_table.mmap->PEN_2D_RAW_TIP_X_ADDR,
			pen_tip_x_noise_n_data, tx_num * pen_rx_num);
		nvt_read_get_num_mdata(
			chip_info,
			chip_info->trim_id_table.mmap->PEN_2D_DIFF_TIP_Y_ADDR,
			pen_tip_y_noise_p_data, pen_tx_num * rx_num);
		nvt_read_get_num_mdata(
			chip_info,
			chip_info->trim_id_table.mmap->PEN_2D_RAW_TIP_Y_ADDR,
			pen_tip_y_noise_n_data, pen_tx_num * rx_num);
		nvt_read_get_num_mdata(
			chip_info,
			chip_info->trim_id_table.mmap->PEN_2D_DIFF_RING_X_ADDR,
			pen_ring_x_noise_p_data, tx_num * pen_rx_num);
		nvt_read_get_num_mdata(
			chip_info,
			chip_info->trim_id_table.mmap->PEN_2D_RAW_RING_X_ADDR,
			pen_ring_x_noise_n_data, tx_num * pen_rx_num);
		nvt_read_get_num_mdata(
			chip_info,
			chip_info->trim_id_table.mmap->PEN_2D_DIFF_RING_Y_ADDR,
			pen_ring_y_noise_p_data, pen_tx_num * rx_num);
		nvt_read_get_num_mdata(
			chip_info,
			chip_info->trim_id_table.mmap->PEN_2D_RAW_RING_Y_ADDR,
		...
		for (j = 0; j < pen_rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((pen_tip_x_noise_p_data[iArrayIndex] >
				     pen_tip_x_noise_data_P[iArrayIndex]) ||
				    (pen_tip_x_noise_p_data[iArrayIndex] <
				     pen_tip_x_noise_data_N[iArrayIndex])) {
					pen_tip_x_noise_p_result = NVT_MP_FAIL;
					pen_tip_x_noise_p_record[iArrayIndex] =
						1;
					TPD_INFO(
						"pen tip x noise max data Test failed at data[%d][%d] = %d [%d,%d]\n",
						i, j,
						pen_tip_x_noise_p_data
							[iArrayIndex],
						pen_tip_x_noise_data_N
							[iArrayIndex],
						pen_tip_x_noise_data_P
		...
		for (j = 0; j < pen_rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((pen_tip_x_noise_n_data[iArrayIndex] >
					...
					pen_tip_x_noise_n_record[iArrayIndex] =
						1;
					TPD_INFO(
						"pen tip x noise min data Test failed at data[%d][%d] = %d [%d,%d]\n",
						i, j,
						pen_tip_x_noise_n_data
							[iArrayIndex],
						pen_tip_x_noise_data_N
							[iArrayIndex],
						pen_tip_x_noise_data_P
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < pen_tx_num; i++) {
				...
				if ((pen_tip_y_noise_p_data[iArrayIndex] >
				     pen_tip_y_noise_data_P[iArrayIndex]) ||
				    (pen_tip_y_noise_p_data[iArrayIndex] <
				     pen_tip_y_noise_data_N[iArrayIndex])) {
					pen_tip_y_noise_p_result = NVT_MP_FAIL;
					pen_tip_y_noise_p_record[iArrayIndex] =
						1;
					TPD_INFO(
						"pen tip y noise max data Test failed at data[%d][%d] = %d [%d,%d]\n",
						i, j,
						pen_tip_y_noise_p_data
							[iArrayIndex],
						pen_tip_y_noise_data_N
							[iArrayIndex],
						pen_tip_y_noise_data_P
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < pen_tx_num; i++) {
				...
				if ((pen_tip_y_noise_n_data[iArrayIndex] >
					...
					pen_tip_y_noise_n_record[iArrayIndex] =
						1;
					TPD_INFO(
						"pen tip y noise min data Test failed at data[%d][%d] = %d [%d,%d]\n",
						i, j,
						pen_tip_y_noise_n_data
							[iArrayIndex],
						pen_tip_y_noise_data_N
							[iArrayIndex],
						pen_tip_y_noise_data_P
		...
		for (j = 0; j < pen_rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((pen_ring_x_noise_p_data[iArrayIndex] >
				     pen_ring_x_noise_data_P[iArrayIndex]) ||
				    (pen_ring_x_noise_p_data[iArrayIndex] <
				     pen_ring_x_noise_data_N[iArrayIndex])) {
					pen_ring_x_noise_p_result = NVT_MP_FAIL;
					pen_ring_x_noise_p_record[iArrayIndex] =
						1;
					TPD_INFO(
						"pen ring x noise max data Test failed at data[%d][%d] = %d [%d,%d]\n",
						i, j,
						pen_ring_x_noise_p_data
							[iArrayIndex],
						pen_ring_x_noise_data_N
							[iArrayIndex],
						pen_ring_x_noise_data_P
		...
		for (j = 0; j < pen_rx_num; j++) {
			for (i = 0; i < tx_num; i++) {
				...
				if ((pen_ring_x_noise_n_data[iArrayIndex] >
					...
					pen_ring_x_noise_n_record[iArrayIndex] =
						1;
					TPD_INFO(
						"pen ring x noise min data Test failed at data[%d][%d] = %d [%d,%d]\n",
						i, j,
						pen_ring_x_noise_n_data
							[iArrayIndex],
						pen_ring_x_noise_data_N
							[iArrayIndex],
						pen_ring_x_noise_data_P
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < pen_tx_num; i++) {
				...
				if ((pen_ring_y_noise_p_data[iArrayIndex] >
				     pen_ring_y_noise_data_P[iArrayIndex]) ||
				    (pen_ring_y_noise_p_data[iArrayIndex] <
				     pen_ring_y_noise_data_N[iArrayIndex])) {
					pen_ring_y_noise_p_result = NVT_MP_FAIL;
					pen_ring_y_noise_p_record[iArrayIndex] =
						1;
					TPD_INFO(
						"pen ring y noise max data Test failed at data[%d][%d] = %d [%d,%d]\n",
						i, j,
						pen_ring_y_noise_p_data
							[iArrayIndex],
						pen_ring_y_noise_data_N
							[iArrayIndex],
						pen_ring_y_noise_data_P
		...
		for (j = 0; j < rx_num; j++) {
			for (i = 0; i < pen_tx_num; i++) {
				...
				if ((pen_ring_y_noise_n_data[iArrayIndex] >
					...
					pen_ring_y_noise_n_record[iArrayIndex] =
						1;
					TPD_INFO(
						"pen ring y noise min data Test failed at data[%d][%d] = %d [%d,%d]\n",
						i, j,
						pen_ring_y_noise_n_data
							[iArrayIndex],
						pen_ring_y_noise_data_N
	...
	for (j = 0; j < rx_num; j++) {
		for (i = 0; i < tx_num; i++) {
			...
			if ((digital_noise_p_data[iArrayIndex] >
			     digital_diff_rawdata_P[iArrayIndex]) ||
			    (digital_noise_p_data[iArrayIndex] <
			     digital_diff_rawdata_N[iArrayIndex])) {
				digital_noise_p_result = NVT_MP_FAIL;
				digital_noise_p_record[iArrayIndex] = 1;
				TPD_INFO(
					"Digital Noise Max Test failed at data[%d][%d] = %d [%d,%d]\n",
	...
	for (j = 0; j < rx_num; j++) {
		for (i = 0; i < tx_num; i++) {
			...
			if ((digital_noise_n_data[iArrayIndex] >
				...
				TPD_INFO(
					"Digital Noise Min Test failed at data[%d][%d] = %d [%d,%d]\n",
...
TEST_END:
	...
	if (!err_cnt) {
		...
		snprintf(
			data_buf, 128,
			"/sdcard/TpTestReport/screenOn/OK/tp_testlimit_%s_%02d%02d%02d-%02d%02d%02d-utc.csv",
			(char *)all_test_result,
			(rtc_now_time.tm_year + 1900) % 100,
			rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
			rtc_now_time.tm_hour, rtc_now_time.tm_min,
		...
		snprintf(
			data_buf, 128,
			"/sdcard/TpTestReport/screenOn/NG/tp_testlimit_%s_%02d%02d%02d-%02d%02d%02d-utc.csv",
			(char *)all_test_result,
			(rtc_now_time.tm_year + 1900) % 100,
			rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
			rtc_now_time.tm_hour, rtc_now_time.tm_min,
	...
	if (ts->pen_support) {
		/* 3. pen tip x rawdata    */
		nvt_store_testrecord_to_file(nvt_testdata->fd,
					     "pen tip x rawData",
					     pen_tip_x_result, pen_tip_x_record,
					     tx_num, pen_rx_num);

		/* 4. pen tip y rawdata    */
		nvt_store_testrecord_to_file(nvt_testdata->fd,
					     "pen tip y rawData",
					     pen_tip_y_result, pen_tip_y_record,
					     pen_tx_num, rx_num);
	...
	if (ts->pen_support) {
		...
		nvt_store_testrecord_to_file(nvt_testdata->fd,
					     "pen tip x noise max",
					     pen_tip_x_noise_p_result,
					     pen_tip_x_noise_p_record, tx_num,
					     pen_rx_num);
		/*10. pen tip x noise min    */
		nvt_store_testrecord_to_file(nvt_testdata->fd,
					     "pen tip x noise min",
					     pen_tip_x_noise_n_result,
					     pen_tip_x_noise_n_record, tx_num,
					     pen_rx_num);

		/*11. pen tip y noise max    */
		nvt_store_testrecord_to_file(nvt_testdata->fd,
					     "pen tip y noise max",
					     pen_tip_y_noise_p_result,
					     pen_tip_y_noise_p_record,
					     pen_tx_num, rx_num);
		/*12. pen tip y noise min    */
		nvt_store_testrecord_to_file(nvt_testdata->fd,
					     "pen tip y noise min",
					     pen_tip_y_noise_n_result,
					     pen_tip_y_noise_n_record,
					     pen_tx_num, rx_num);

		/*13. pen ring x noise max    */
		nvt_store_testrecord_to_file(nvt_testdata->fd,
					     "pen ring x noise max",
					     pen_ring_x_noise_p_result,
					     pen_ring_x_noise_p_record, tx_num,
					     pen_rx_num);
		/*14. pen ring x noise min    */
		nvt_store_testrecord_to_file(nvt_testdata->fd,
					     "pen ring x noise min",
					     pen_ring_x_noise_n_result,
					     pen_ring_x_noise_n_record, tx_num,
					     pen_rx_num);

		/*15. pen ring y noise max    */
		nvt_store_testrecord_to_file(nvt_testdata->fd,
					     "pen ring y noise max",
					     pen_ring_y_noise_p_result,
					     pen_ring_y_noise_p_record,
					     pen_tx_num, rx_num);
		/*16. pen ring y noise min    */
		nvt_store_testrecord_to_file(nvt_testdata->fd,
					     "pen ring y noise min",
					     pen_ring_y_noise_p_result,
					     pen_ring_y_noise_p_record,
					     pen_tx_num, rx_num);
	} /* ts->pen_support */
	...
	/* 20. noise max      */
	nvt_store_testrecord_to_file(nvt_testdata->fd,
				     "Digital_RawData_Diff_Max",
				     digital_noise_p_result,
				     digital_noise_p_record, tx_num, rx_num);
	/* 21. noise min      */
	nvt_store_testrecord_to_file(nvt_testdata->fd,
				     "Digital_RawData_Diff_Min",
				     digital_noise_n_result,
				     digital_noise_n_record, tx_num, rx_num);
#endif
	/*22. short          */
	...
	if (ts->pen_support) {
		/* 3. pen tip x rawdata    */
		nvt_store_testdata_to_file(nvt_testdata->fd,
					   "pen tip x rawData",
					   pen_tip_x_result, pen_tip_x_data,
					   tx_num, pen_rx_num);

		/* 4. pen tip y rawdata    */
		nvt_store_testdata_to_file(nvt_testdata->fd,
					   "pen tip y rawData",
					   pen_tip_y_result, pen_tip_y_data,
					   pen_tx_num, rx_num);

		/* 5. pen ring x rawdata    */
		nvt_store_testdata_to_file(nvt_testdata->fd,
					   "pen ring x rawData",
					   pen_ring_x_result, pen_ring_x_data,
					   tx_num, pen_rx_num);

		/* 6. pen ring y rawdata    */
		nvt_store_testdata_to_file(nvt_testdata->fd,
					   "pen ring y rawData",
					   pen_ring_y_result, pen_ring_y_data,
					   pen_tx_num, rx_num);
	} /* ts->pen_support */
#endif
	/* 7. noise max      */
	...
	if (ts->pen_support) {
		...
		nvt_store_testdata_to_file(nvt_testdata->fd,
					   "pen tip x noise max",
					   pen_tip_x_noise_p_result,
					   pen_tip_x_noise_p_data, tx_num,
					   pen_rx_num);
		/*10. pen tip x noise min    */
		nvt_store_testdata_to_file(nvt_testdata->fd,
					   "pen tip x noise min",
					   pen_tip_x_noise_n_result,
					   pen_tip_x_noise_n_data, tx_num,
					   pen_rx_num);
		...
		nvt_store_testdata_to_file(nvt_testdata->fd,
					   "pen tip y noise max",
					   pen_tip_y_noise_p_result,
					   pen_tip_y_noise_p_data, pen_tx_num,
					   rx_num);
		/*12. pen tip y noise min    */
		nvt_store_testdata_to_file(nvt_testdata->fd,
					   "pen tip y noise min",
					   pen_tip_y_noise_n_result,
					   pen_tip_y_noise_n_data, pen_tx_num,
					   rx_num);
		...
		nvt_store_testdata_to_file(nvt_testdata->fd,
					   "pen ring x noise max",
					   pen_ring_x_noise_p_result,
					   pen_ring_x_noise_p_data, tx_num,
					   pen_rx_num);
		/*14. pen ring x noise min    */
		nvt_store_testdata_to_file(nvt_testdata->fd,
					   "pen ring x noise min",
					   pen_ring_x_noise_n_result,
					   pen_ring_x_noise_n_data, tx_num,
					   pen_rx_num);
		...
		nvt_store_testdata_to_file(nvt_testdata->fd,
					   "pen ring y noise max",
					   pen_ring_y_noise_p_result,
					   pen_ring_y_noise_p_data, pen_tx_num,
					   rx_num);
		/*16. pen ring y noise min    */
		nvt_store_testdata_to_file(nvt_testdata->fd,
					   "pen ring y noise min",
	...
	nvt_store_testdata_to_file(nvt_testdata->fd, "Digital RawData_Diff_Max",
				   digital_noise_p_result, digital_noise_p_data,
				   tx_num, rx_num);
	/* 21. noise min      */
	nvt_store_testdata_to_file(nvt_testdata->fd, "Digital RawData_Diff_Min",
				   digital_noise_n_result, digital_noise_n_data,
				   tx_num, rx_num);
#endif
	/*22. short          */
