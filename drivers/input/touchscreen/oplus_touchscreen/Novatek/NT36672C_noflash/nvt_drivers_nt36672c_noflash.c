	...
	for (list = 0; list < chip_info->partition; list++) {
		...
		if ((list >= chip_info->ilm_dlm_num) &&
			...
			if ((chip_info->bin_map[list].BIN_addr == 0) &&
				...
				snprintf(chip_info->bin_map[list].name, 12,
					 "Info-%d",
					 (int)(list - chip_info->ilm_dlm_num));
			}
		}
	...
	if (fw == NULL) {
		...
		if (chip_info->g_fw_sta) {
			...
			if (chip_info->p_firmware_headfile->firmware_data) {
				...
				tp_healthinfo_report(monitor_data,
						     HEALTH_FW_UPDATE,
						     "FW_DT_NL_failed");
				TPD_INFO(
...
OUT:
	...
	tp_healthinfo_report(chip_info->monitor_data_v2,
			     HEALTH_TEST_BLACKSCREEN, &err_cnt);
...
static int nova_apk_tp_info_get(void *chip_data, char *buf, int len)
{
	...
	ret = snprintf(buf, len, "IC:NOVA%04X\nFW_VER:0x%02X\nCH:%dX%d\n",
		       (unsigned int)0x672C, (unsigned int)chip_info->fw_ver,
		       chip_info->hw_res->TX_NUM, chip_info->hw_res->RX_NUM);
	if (ret > len) {
		ret = len;
	}
...
static int nt36672c_parse_dts(struct chip_data_nt36672c *chip_info,
			      struct spi_device *client)
{
	...
	chip_info->spi_fast_read_support =
		of_property_read_bool(np, "spi_fast_read_support");
