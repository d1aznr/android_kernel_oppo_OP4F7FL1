...
static void nvt_esd_check_enable(struct chip_data_nt36525 *chip_info,
				 bool enable);
static uint8_t nvt_wdt_fw_recovery(struct chip_data_nt36525 *chip_info,
				   uint8_t *point_data);
static int8_t nvt_cmd_store(struct chip_data_nt36525 *chip_info, uint8_t u8Cmd);
static fw_update_state nvt_fw_update_choice(struct chip_data_nt36525 *chip_info,
...
*******************************************************/
static int32_t
nvt_check_fw_reset_state_noflash(struct chip_data_nt36525 *chip_info,
...
static void copy_fw_to_buffer(struct chip_data_nt36525 *chip_info,
			      const struct firmware *fw)
{
	if (fw) {
...
static int nvt_enable_edge_limit(struct chip_data_nt36525 *chip_info, int state)
{
	...
	if (ts->fw_edge_limit_support) {
		...
		if (state) {
			ret = nvt_cmd_store(chip_info,
					    EVENTBUFFER_EDGE_LIMIT_ON);
		} else {
			ret = nvt_cmd_store(chip_info,
					    EVENTBUFFER_EDGE_LIMIT_OFF);
		}
	}
...
static int nvt_enable_headset_mode(struct chip_data_nt36525 *chip_info,
				   bool enable)
{
	int8_t ret = -1;
...
static int32_t nvt_bin_header_parser(struct chip_data_nt36525 *chip_info,
...
return:
        n.a.
*******************************************************/
static void nvt_read_ram_test(struct chip_data_nt36525 *chip_info,
...
return:
        n.a.
*******************************************************/
static int32_t Write_Partition(struct chip_data_nt36525 *chip_info,
			       const u8 *fwdata, size_t fwsize)
{
	uint32_t list = 0;
	char *name;
...
static void nvt_set_bld_crc(struct chip_data_nt36525 *chip_info,
...
return:
        n.a.
*******************************************************/
static int32_t Check_HW_CRC(struct chip_data_nt36525 *chip_info,
			    uint8_t is_ilm_dlm)
{
	uint32_t list = 0;
	...
	if (is_ilm_dlm) {
		...
		while (1) {
			nvt_set_page(chip_info, chip_info->trim_id_table.mmap
							->BLD_ILM_DLM_CRC_ADDR);
			chip_info->fwbuf[0] = chip_info->trim_id_table.mmap
						      ->BLD_ILM_DLM_CRC_ADDR &
					      0x7F;
			chip_info->fwbuf[1] = 0xFF;
			ret = CTP_SPI_READ(chip_info->s_client,
					   chip_info->fwbuf, 2);
			if (ret) {
			...
			if (unlikely(retry > 20)) {
				...
				TPD_INFO(
					...
					byte_to_word(&chip_info->fwbuf[1]));
				TPD_INFO(
					"DLM_BIN_CRC=0x%08X, DLM_HW_CRC=0x%08X\n",
					chip_info->bin_map[1].crc,
					byte_to_word(&chip_info->fwbuf[5]));
				return -EIO;
			}
		}
	} else {
		for (list = ilm_dlm_num; list < chip_info->partition; list++) {
			...
			if ((chip_info->bin_map[list].BIN_addr == 0) &&
			    (chip_info->bin_map[list].size != 0))
				chip_info->bin_map[list].size =
					chip_info->bin_map[list].size - 4;

			/* write register bank */
			nvt_set_bld_crc(
				chip_info,
				chip_info->trim_id_table.mmap->BLD_DES_ADDR,
				chip_info->bin_map[list].SRAM_addr,
				chip_info->trim_id_table.mmap->BLD_LENGTH_ADDR,
				chip_info->bin_map[list].size,
				chip_info->trim_id_table.mmap
					->G_DLM_CHECKSUM_ADDR,
			...
			nvt_set_page(
				chip_info,
				chip_info->trim_id_table.mmap->DMA_CRC_EN_ADDR);
			chip_info->fwbuf[0] =
				chip_info->trim_id_table.mmap->DMA_CRC_EN_ADDR &
				0x7F;
		...
		/* write register bank */
		nvt_set_bld_crc(
			chip_info, chip_info->trim_id_table.mmap->ILM_DES_ADDR,
		...
		/* write register bank */
		nvt_set_bld_crc(
			chip_info, chip_info->trim_id_table.mmap->DLM_DES_ADDR,
...
void nvt_boot_ready(struct chip_data_nt36525 *chip_info, uint8_t ready)
{
	...
	//---write POR_CD cmds---
	nvt_write_addr(chip_info->s_client,
		       chip_info->trim_id_table.mmap->POR_CD_ADDR, 0xA0);
...
return:
        n.a.
*******************************************************/
static int32_t Download_Firmware_HW_CRC(struct chip_data_nt36525 *chip_info,
					const struct firmware *fw)
{
	uint8_t retry = 0;
	int32_t ret = 0;
	...
	while (1) {
		nvt_esd_check_enable(chip_info,
				     true); //update esd trigger base time
		/*
...
return:
        n.a.
*******************************************************/
static int32_t Download_Firmware(struct chip_data_nt36525 *chip_info,
				 const struct firmware *fw)
{
	uint8_t retry = 0;
	int32_t ret = 0;
...
static int nvt_enable_jitter_mode(struct chip_data_nt36525 *chip_info,
				  bool enable)
{
	int8_t ret = -1;
...
nvt_fw_update_for_common(void *chip_data, const struct firmware *fw, bool force)
{
	int ret = 0;
	struct chip_data_nt36525 *chip_info =
		(struct chip_data_nt36525 *)chip_data;
				     ...
				     bool chk_ignore)
{
	...
	if (trigger_count >= 5) {
		trigger_count = 0;
		TPD_INFO(
			"TP firmware download failed exceed 5 times, triger panic\n");
				   ...
				   uint8_t *point_data)
{
	...
	if (chip_info->recovery_cnt) {
		TPD_INFO("%s recovery_cnt=%d  point_data[1]=0x%x ", __func__,
			 chip_info->recovery_cnt, point_data[i]);
	}
...
static int nvt_spi_suspend(struct device *dev)
{
	...
	struct chip_data_nt36525 *chip_info =
		(struct chip_data_nt36525 *)(ts->chip_data);
