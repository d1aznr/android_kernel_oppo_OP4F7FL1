...
static int8_t nvt_cmd_store(struct chip_data_nt36525b *chip_info,
			    uint8_t u8Cmd);
...
static uint8_t nvt_wdt_fw_recovery(struct chip_data_nt36525b *chip_info,
				   uint8_t *point_data)
{
...
static void nvt_esd_check_enable(struct chip_data_nt36525b *chip_info,
				 bool enable)
{
...
static int nvt_esd_handle(void *chip_data)
{
	struct chip_data_nt36525b *chip_info =
		(struct chip_data_nt36525b *)chip_data;
...
*******************************************************/
static int32_t
nvt_check_fw_reset_state_noflash(struct chip_data_nt36525b *chip_info,
...
static int32_t nvt_bin_header_parser(struct chip_data_nt36525b *chip_info,
...
return:
        n.a.
*******************************************************/
static void nvt_read_ram_test(struct chip_data_nt36525b *chip_info,
...
static int32_t Write_Partition(struct chip_data_nt36525b *chip_info,
			       const u8 *fwdata, size_t fwsize)
{
	uint32_t list = 0;
	char *name;
...
return:
        n.a.
*******************************************************/
static int32_t Download_Firmware_HW_CRC(struct chip_data_nt36525b *chip_info,
...
static void nova_write_log_buf(struct chip_data_nt36525b *chip_info, u8 main_id,
			       u8 sec_id)
{
...
static int8_t nvt_extend_cmd_store(struct chip_data_nt36525b *chip_info,
...
static void
nvt_read_debug_gesture_coordinate_buffer(struct chip_data_nt36525b *chip_info,
...
static void
nvt_dbg_gesture_record_coor_read(struct chip_data_nt36525b *chip_info,
				 u8 pointdata)
{
	u8 *xdata = NULL;
	uint32_t buf_len = 0;
...
static __maybe_unused int nvt_enable_debug_gesture_coordinate_record_mode(
	struct chip_data_nt36525b *chip_info, bool enable)
{
	int8_t ret = -1;
...
nvt_enable_debug_gesture_coordinate_mode(struct chip_data_nt36525b *chip_info,
					 bool enable)
{
	int8_t ret = -1;
...
static int nvt_enable_black_gesture(struct chip_data_nt36525b *chip_info,
				    bool enable)
{
	int ret = -1;
...
static int nvt_enable_edge_limit(struct chip_data_nt36525b *chip_info,
				 int state)
{
	int8_t ret = -1;
	struct touchpanel_data *ts = spi_get_drvdata(chip_info->s_client);
...
static int nvt_enable_charge_mode(struct chip_data_nt36525b *chip_info,
				  bool enable)
{
	int8_t ret = -1;
...
static int nvt_enable_game_mode(struct chip_data_nt36525b *chip_info,
				bool enable)
{
	int8_t ret = -1;
...
static int nvt_enable_headset_mode(struct chip_data_nt36525b *chip_info,
				   bool enable)
{
	int8_t ret = -1;
...
#ifdef CONFIG_OPLUS_TP_APK
static __maybe_unused int
nvt_enable_hopping_polling_mode(struct chip_data_nt36525b *chip_info,
				bool enable)
{
	int8_t ret = -1;
...
static __maybe_unused int
nvt_enable_hopping_fix_freq_mode(struct chip_data_nt36525b *chip_info,
				 bool enable)
{
	int8_t ret = -1;
...
static __maybe_unused int
nvt_enable_debug_msg_diff_mode(struct chip_data_nt36525b *chip_info,
			       bool enable)
{
	int8_t ret = -1;
...
static __maybe_unused int
nvt_enable_water_polling_mode(struct chip_data_nt36525b *chip_info, bool enable)
{
	int8_t ret = -1;
	...
	if (ret < 0) {
		...
		if (panel_data->manufacture_info.version) {
			if (panel_data->vid_len == 0) {
				...
				if (ver_len <= 8) {
					strlcat(panel_data->manufacture_info
							.version,
						dev_version,
...
static void nvt_enable_noise_collect(struct chip_data_nt36525b *chip_info,
				     int32_t frame_num)
{
	int ret = -1;
	uint8_t buf[8] = { 0 };
...
static int8_t nvt_switch_FreqHopEnDis(struct chip_data_nt36525b *chip_info,
...
static void nvt_read_mdata(struct chip_data_nt36525b *chip_info,
...
static int32_t
nvt_polling_hand_shake_status(struct chip_data_nt36525b *chip_info)
{
	uint8_t buf[8] = { 0 };
	int32_t i = 0;
...
static int32_t nvt_read_fw_noise(struct chip_data_nt36525b *chip_info,
...
static void nvt_enable_doze_noise_collect(struct chip_data_nt36525b *chip_info,
					  int32_t frame_num)
{
	int ret = -1;
	uint8_t buf[8] = { 0 };
...
static int32_t nvt_read_doze_fw_noise(struct chip_data_nt36525b *chip_info,
...
static int32_t nvt_read_doze_baseline(struct chip_data_nt36525b *chip_info,
				      int32_t doze_X_Channel, int32_t *xdata,
				      int32_t xdata_len)
{
	uint8_t buf[256] = { 0 };
...
TEST_END:
	...
	if (!err_cnt) {
		...
		snprintf(
			...
			all_test_result, (rtc_now_time.tm_year + 1900) % 100,
...
static int32_t nvt_read_fw_short(struct chip_data_nt36525b *chip_info,
...
static int32_t nvt_read_fw_open(struct chip_data_nt36525b *chip_info,
				int32_t *xdata, int32_t xdata_len)
{
	uint32_t raw_pipe_addr = 0;
	uint8_t *rawdata_buf = NULL;
...
static int nova_apk_tp_info_get(void *chip_data, char *buf, int len)
{
	...
	ret = snprintf(buf, len, "IC:NOVA%04X\nFW_VER:0x%02X\nCH:%dX%d\n",
		       0x525B, chip_info->fw_ver, chip_info->hw_res->TX_NUM,
