...
void nvt_read_mdata(struct chip_data_nt36672 *chip_info, uint32_t xdata_addr,
...
void nvt_stop_crc_reboot(struct chip_data_nt36672 *chip_info)
{
	...
	if (((buf[1] == 0xFC) && (buf[2] == 0xFC) && (buf[3] == 0xFC)) ||
		...
		for (retry = 5; retry > 0; retry--) {
			...
			buf[0] = 0x00;
			buf[1] = 0xA5;
			nvt_i2c_write(chip_info->client, I2C_HW_Address, buf,
				      2);
			...
			buf[1] = 0x03;
			buf[2] = 0xF1;
			nvt_i2c_write(chip_info->client, I2C_BLDR_Address, buf,
				      3);
			...
			buf[0] = 0x35;
			buf[1] = 0x00;
			nvt_i2c_read(chip_info->client, I2C_BLDR_Address, buf,
				     2);
...
static int8_t nvt_check_chip_ver_trim(struct chip_data_nt36672 *chip_info)
{
	...
	for (retry = 5; retry > 0; retry--) {
		...
		for (list = 0; list < (sizeof(trim_id_table) /
			...
			if (found_nvt_chip) {
				TPD_INFO(
					"This is NVT touch IC, Chip Index %d\n",
					list);
				chip_info->mmap = trim_id_table[list].mmap;
				chip_info->carrier_system =
					trim_id_table[list].carrier_system;
				memcpy(chip_info->id, trim_id_table[list].id,
				       NVT_ID_BYTE_MAX); /* store id */
				ret = 0;
				goto out;
			} else {
				chip_info->mmap = NULL;
...
int32_t nvt_check_fw_reset_state(struct chip_data_nt36672 *chip_info,
...
static int nvt_enable_edge_limit(struct chip_data_nt36672 *chip_info,
				 bool enable)
{
	int8_t ret = -1;
...
int32_t Check_CheckSum(struct chip_data_nt36672 *chip_info,
...
int32_t Check_FW_Ver(struct chip_data_nt36672 *chip_info,
		     const struct firmware *fw)
{
	uint8_t buf[16] = { 0 };
	int32_t ret = 0;
...
int32_t Write_Flash(struct chip_data_nt36672 *chip_info,
		    const struct firmware *fw)
{
	uint8_t buf[64] = { 0 };
...
int32_t Verify_Flash(struct chip_data_nt36672 *chip_info,
		     const struct firmware *fw)
{
	uint8_t buf[64] = { 0 };
	uint32_t XDATA_Addr = chip_info->mmap->READ_FLASH_CHECKSUM_ADDR;
...
{
	int i = 0;
	uint8_t nvt_36672A[NVT_ID_BYTE_MAX] = { 0x0A, 0xFF, 0xFF,
						0x72, 0x66, 0x03 };
	...
	for (y = 0; y < rx_num; y++) {
		for (x = 0; x < tx_num; x++) {
			if (nvt_is_NT36672A(chip_info)) { /*for 36672A*/
				...
				xdata[(rx_num - y - 1) * tx_num +
				      (tx_num - x - 1)] =
					(int16_t)((
						rawdata_buf[(y * tx_num + x) *
							    2] +
						256 * rawdata_buf[(y * tx_num +
								   x) * 2 +
								  1]));
			}
		}
