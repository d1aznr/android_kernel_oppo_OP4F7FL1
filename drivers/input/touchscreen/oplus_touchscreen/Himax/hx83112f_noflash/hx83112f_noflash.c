...
static ssize_t hx83112f_spi_sync(struct touchpanel_data *ts,
...
static int hx83112f_spi_read(uint8_t *command, uint8_t command_len,
...
void hx83112f_flash_write_burst_length(uint8_t *reg_byte, uint8_t *write_data,
				       int length)
{
	...
	memset(g_internal_buffer, 0x00,
	       sizeof(uint8_t) * HX_SPI_GEN_MAX_WRITE_SZ);
			    ...
			    uint8_t *read_data, bool hx83112f_cfg_flag)
{
	...
	if (hx83112f_cfg_flag == false) {
		...
		if (hx83112f_bus_read(read_addr[0], read_length, read_data) <
		    0) {
			TPD_INFO("%s: spi bus access fail!\n", __func__);
			return;
		}
...
void hx83112f_register_write(uint8_t *write_addr, int write_length,
			     uint8_t *write_data, bool hx83112f_cfg_flag)
{
	int i = 0;
	int address = 0;
	if (hx83112f_cfg_flag == false) {
		...
		for (i = address; i < address + write_length; i++) {
			...
			hx83112f_flash_write_burst_length(
				write_addr, write_data, write_length);
		}
	} else if (hx83112f_cfg_flag == true) {
		if (hx83112f_bus_write(write_addr[0], write_length,
				       write_data) < 0) {
			TPD_INFO("%s: spi bus access fail!\n", __func__);
			return;
		}
...
static int hx83112f_mcu_register_write(uint8_t *write_addr,
				       uint32_t write_length,
				       uint8_t *write_data,
				       uint8_t hx83112f_cfg_flag)
{
	...
	for (i = 0; i < (total_read_times); i++) {
		if (total_size_temp >= max_bus_size) {
			memcpy(tmp_data, write_data + (i * max_bus_size),
			       max_bus_size);
			hx83112f_flash_write_burst_length(tmp_addr, tmp_data,
							  max_bus_size);
...
void hx83112f_hx_chk_write_register(uint8_t *addr, uint8_t *data)
{
	...
	TPD_INFO("%s: Now write  addr=0x%02X%02X%02X%02X", __func__, addr[3],
	...
	do {
		...
		msleep(1);
		TPD_INFO(
			"%s: times %d,Now read[3]=0x%02X,read[2]=0x%02X,read[1]=0x%02X,read[0]=0x%02X\n",
			__func__, retry, read_data[3], read_data[2],
			read_data[1], read_data[0]);
		retry--;
	} while (retry > 0 &&
		 (data[3] != read_data[3] || data[2] != read_data[2] ||
		  data[1] != read_data[1] || data[0] != read_data[0]));
...
bool hx83112f_sense_off(void)
{
	...
	do {
		if (cnt == 0 || (tmp_data[0] != 0xA5 && tmp_data[0] != 0x87) ||
		    (tmp_data[0] != 0x00)) {
			tmp_addr[3] = 0x90;
			tmp_addr[2] = 0x00;
			tmp_addr[1] = 0x00;
	...
	do {
		...
		if (tmp_data[0] == 0x0C) {
			...
			msleep(10);
#ifdef HX_RST_PIN_FUNC
			hx83112f_resetgpio_set(hx83112f_g_chip_info->hw_res,
					       false);
			hx83112f_resetgpio_set(hx83112f_g_chip_info->hw_res,
					       true);
#else
...
int hx83112f_hx_dis_rload_0f(void)
{
	...
	do {
		hx83112f_flash_write_burst(pzf_op->addr_dis_flash_reload,
					   pzf_op->data_dis_flash_reload);
		hx83112f_register_read(pzf_op->addr_dis_flash_reload, 4,
				       tmp_data, false);
		TPD_DETAIL(
	...
	TPD_DETAIL(
		"tmp_data[3]=0x%02X,tmp_data[2]=0x%02X,tmp_data[1]=0x%02X,tmp_data[0]=0x%02X\n",
		tmp_data[3], tmp_data[2], tmp_data[1], tmp_data[0]);
	if (tmp_data[3] != 0x00 || tmp_data[2] != 0x00 || tmp_data[1] != 0x00 ||
	    tmp_data[0] != 0x00) {
...
static void hx83112f_mcu_write_sram_0f(const struct firmware *fw_entry,
				       ...
				       uint32_t write_len)
{
	...
	if (hx83112f_g_chip_info->tmp_data == NULL) {
		...
		hx83112f_g_chip_info->tmp_data = kzalloc(
			sizeof(uint8_t) * firmware_update_space, GFP_KERNEL);
		if (hx83112f_g_chip_info->tmp_data == NULL) {
			TPD_INFO(
				"%s, alloc hx83112f_g_chip_info->tmp_data failed\n",
				__func__);
			return;
		}
...
int hx83112f_sram_write_crc_check(const struct firmware *fw_entry,
				  uint8_t *addr, int strt_idx, uint32_t len)
{
	...
	do {
		hx83112f_g_core_fp.fp_write_sram_0f(fw_entry, addr, strt_idx,
						    len);
		crc = hx83112f_hw_check_CRC(pzf_op->data_sram_start_addr,
					    HX64K);
		retry++;
...
int hx83112f_mcu_Calculate_CRC_with_AP(unsigned char *FW_content,
...
bool hx83112f_hx_parse_bin_cfg_data(const struct firmware *fw_entry)
{
	...
	hx83112f_nf_cfg_crc = hx83112f_mcu_Calculate_CRC_with_AP(
...
void hx83112f_mcu_firmware_update_0f(const struct firmware *fw_entry)
{
	...
	if (fw_entry == NULL || reload) {
		...
		if (request_fw_headfile == NULL) {
			tp_healthinfo_report(
				hx83112f_g_chip_info->monitor_data_v2,
				HEALTH_FW_UPDATE, "FW_HF_KZL_failed");
		...
		if (hx83112f_g_chip_info->g_fw_sta) {
			...
			request_fw_headfile->size =
				hx83112f_g_chip_info->g_fw_len;
			request_fw_headfile->data =
				hx83112f_g_chip_info->g_fw_buf;
			tmp_fw_entry = request_fw_headfile;

		} else {
			TPD_INFO(
				"request firmware failed, get from headfile\n");
			if (hx83112f_g_chip_info->p_firmware_headfile
				    ->firmware_data) {
				request_fw_headfile->size =
					hx83112f_g_chip_info
						->p_firmware_headfile
				...
				tp_healthinfo_report(
					hx83112f_g_chip_info->monitor_data_v2,
					HEALTH_FW_UPDATE, "FW_DT_NL_failed");
				TPD_INFO(
	...
	hx83112f_fw_check(hx83112f_private_ts->chip_data,
			  &hx83112f_private_ts->resolution_info,
			  &hx83112f_private_ts->panel_data);
...
void hx83112f_mcu_0f_operation(struct work_struct *work)
{
...
#ifdef CONFIG_TOUCHPANEL_MTK_PLATFORM
	if (hx83112f_private_ts->boot_mode != RECOVERY_BOOT &&
	    !is_oem_unlocked())
#else
	/*Himax_DB_Test Start*/
	if (hx83112f_private_ts->boot_mode != MSM_BOOT_MODE__RECOVERY &&
	    !is_oem_unlocked())
	...
	if (hx83112f_g_chip_info->debug_mode_sta) {
		if (hx83112f_private_ts->apk_op &&
		    hx83112f_private_ts->apk_op->apk_debug_set) {
			hx83112f_private_ts->apk_op->apk_debug_set(
				hx83112f_private_ts->chip_data, true);
		}
	}
...
int hx83112f_hx_0f_init(void)
{
	...
	hx83112f_in_parse_assign_cmd(zf_addr_dis_flash_reload,
				     pzf_op->addr_dis_flash_reload,
				     sizeof(pzf_op->addr_dis_flash_reload));
	hx83112f_in_parse_assign_cmd(zf_data_dis_flash_reload,
				     pzf_op->data_dis_flash_reload,
				     sizeof(pzf_op->data_dis_flash_reload));
	hx83112f_in_parse_assign_cmd(zf_addr_system_reset,
				     pzf_op->addr_system_reset,
				     sizeof(pzf_op->addr_system_reset));
	hx83112f_in_parse_assign_cmd(zf_data_system_reset,
				     pzf_op->data_system_reset,
				     sizeof(pzf_op->data_system_reset));
	hx83112f_in_parse_assign_cmd(zf_data_sram_start_addr,
				     pzf_op->data_sram_start_addr,
				     sizeof(pzf_op->data_sram_start_addr));
	hx83112f_in_parse_assign_cmd(zf_data_sram_clean,
				     pzf_op->data_sram_clean,
				     sizeof(pzf_op->data_sram_clean));
	hx83112f_in_parse_assign_cmd(zf_data_cfg_info, pzf_op->data_cfg_info,
				     sizeof(pzf_op->data_cfg_info));
	hx83112f_in_parse_assign_cmd(zf_data_fw_cfg_p1, pzf_op->data_fw_cfg_p1,
				     sizeof(pzf_op->data_fw_cfg_p1));
	hx83112f_in_parse_assign_cmd(zf_data_fw_cfg_p2, pzf_op->data_fw_cfg_p2,
				     sizeof(pzf_op->data_fw_cfg_p2));
	hx83112f_in_parse_assign_cmd(zf_data_fw_cfg_p3, pzf_op->data_fw_cfg_p3,
				     sizeof(pzf_op->data_fw_cfg_p3));
	hx83112f_in_parse_assign_cmd(zf_data_adc_cfg_1, pzf_op->data_adc_cfg_1,
				     sizeof(pzf_op->data_adc_cfg_1));
	hx83112f_in_parse_assign_cmd(zf_data_adc_cfg_2, pzf_op->data_adc_cfg_2,
				     sizeof(pzf_op->data_adc_cfg_2));
	hx83112f_in_parse_assign_cmd(zf_data_adc_cfg_3, pzf_op->data_adc_cfg_3,
				     sizeof(pzf_op->data_adc_cfg_3));
	hx83112f_in_parse_assign_cmd(zf_data_map_table, pzf_op->data_map_table,
				     sizeof(pzf_op->data_map_table));
	hx83112f_in_parse_assign_cmd(zf_data_mode_switch,
...
bool hx83112f_ic_package_check(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x11) &&
			...
			g_i_FW_VER = i_CTPM_FW[HX83112F_FW_VER_MAJ_FLASH_ADDR]
					     << 8 |
				     i_CTPM_FW[HX83112F_FW_VER_MIN_FLASH_ADDR];
			g_i_CFG_VER =
				i_CTPM_FW[HX83112F_CFG_VER_MAJ_FLASH_ADDR]
					<< 8 |
				i_CTPM_FW[HX83112F_CFG_VER_MIN_FLASH_ADDR];
			g_i_CID_MAJ =
				i_CTPM_FW[HX83112F_CID_VER_MAJ_FLASH_ADDR];
			g_i_CID_MIN =
				i_CTPM_FW[HX83112F_CID_VER_MIN_FLASH_ADDR];
#endif
...
void hx83112f_read_OPLUS_FW_ver(struct chip_data_hx83112f *chip_info)
{
	...
	if (hx83112f_private_ts->panel_data.manufacture_info.version) {
		if (hx83112f_private_ts->panel_data.vid_len == 0) {
			strlcpy(&(hx83112f_private_ts->panel_data
					  .manufacture_info.version[12]),
				dev_version, 3);
		} else {
			...
			strlcpy(&hx83112f_private_ts->panel_data
					 .manufacture_info.version[ver_len],
				dev_version,
...
int hx83112f_report_data_init(int max_touch_point, int tx_num, int rx_num)
{
	...
	hx83112f_hx_touch_data->touch_all_size =
	...
	if ((max_touch_point % 4) == 0)
		...
		HX83112F_HX_TOUCH_INFO_POINT_CNT +=
	...
	if (hx83112f_hx_touch_data->raw_cnt_rmd != 0x00) { //more than 4 fingers
		hx83112f_hx_touch_data->rawdata_size = hx83112f_cal_data_len(
			hx83112f_hx_touch_data->raw_cnt_rmd, max_touch_point,
			hx83112f_hx_touch_data->raw_cnt_max);
		hx83112f_hx_touch_data->touch_info_size =
			(max_touch_point + hx83112f_hx_touch_data->raw_cnt_max +
			 2) *
			4;
	...
	if ((tx_num * rx_num + tx_num + rx_num) %
		    hx83112f_hx_touch_data->rawdata_size ==
	    0) {
		hx83112f_hx_touch_data->rawdata_frame_size =
	...
	TPD_INFO("%s: rawdata_frame_size = %d ", __func__,
		 hx83112f_hx_touch_data->rawdata_frame_size);
	TPD_INFO(
		"%s: max_touch_point:%d, hx_raw_cnt_max:%d, hx_raw_cnt_rmd:%d, g_hx_rawdata_size:%d, hx83112f_hx_touch_data->touch_info_size:%d\n",
		__func__, max_touch_point, hx83112f_hx_touch_data->raw_cnt_max,
		hx83112f_hx_touch_data->raw_cnt_rmd,
		hx83112f_hx_touch_data->rawdata_size,
		hx83112f_hx_touch_data->touch_info_size);

	hx83112f_hx_touch_data->hx_coord_buf = kzalloc(
		sizeof(uint8_t) * (hx83112f_hx_touch_data->touch_info_size),
		GFP_KERNEL);
	if (hx83112f_hx_touch_data->hx_coord_buf == NULL) {
		goto mem_alloc_fail;
	}

	hx83112f_hx_touch_data->diag_mutual =
	...
	hx83112f_hx_touch_data->hx_rawdata_buf = kzalloc(
		sizeof(uint8_t) * (hx83112f_hx_touch_data->touch_all_size -
				   hx83112f_hx_touch_data->touch_info_size),
		GFP_KERNEL);
	...
	//#if defined(HX_SMART_WAKEUP)
	hx83112f_hx_touch_data->hx_event_buf =
		kzalloc(sizeof(uint8_t) * (hx83112f_hx_touch_data->event_size),
			GFP_KERNEL);
...
static int hx83112f_ic_esd_recovery(int hx_esd_event, int hx_zero_event,
				    int length)
{
...
int hx83112f_checksum_cal(struct chip_data_hx83112f *chip_info, uint8_t *buf,
			  int ts_status)
{
	...
	if (ts_status == HX_REPORT_COORD) {
		...
		if (hx_esd_event == length || hx_zero_event == length) {
			shaking_ret = hx83112f_ic_esd_recovery(
			...
			TPD_INFO(
				"[HX83112F_HX_ESD_RESET_ACTIVATE]:%s: Back from reset, ready to serve.\n",
				__func__);
			goto checksum_fail;
			...
			TPD_INFO(
				"[hx83112f_hx_hw_reset_activate]:%s: Back from reset, ready to serve.\n",
				__func__);
			goto ready_to_serve;
		}
	}
...
void hx83112f_log_touch_data(
	uint8_t *buf, struct hx83112f_report_data *hx83112f_hx_touch_data)
{
	int loop_i = 0;
	int print_size = 0;
				  ...
				  char *filepath)
{
	...
	vfs_write(raw_file, rslt_buf,
		  hx83112f_g_1kind_raw_size * hx83112f_hx_criteria_item *
			  sizeof(char),
		  &pos);
	if (raw_file != NULL)
...
int hx83112f_hx_test_data_get(struct chip_data_hx83112f *chip_info,
...
int hx83112f_get_rawdata(struct chip_data_hx83112f *chip_info, uint32_t *RAW,
			 uint32_t datalen)
{
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
...
uint32_t hx83112f_check_mode(uint8_t checktype)
{
	...
	hx83112f_register_read(tmp_addr, 4, tmp_data, false);
	TPD_INFO(
		"%s: hx83112f_wait_sorting_mode, tmp_data[0]=%x, tmp_data[1]=%x\n",
		__func__, tmp_data[0], tmp_data[1]);
...
int hx83112f_mpTestFunc(struct chip_data_hx83112f *chip_info, uint8_t checktype,
			uint32_t datalen)
{
	...
	case HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA:
		...
		if (ret != THP_AFE_INSPECT_ERAW)
			TPD_INFO(
				"%s: HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA PASS\n",
				__func__);
		break;
	case HIMAX_INSPECTION_LPWUG_IDLE_NOISE:
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
		     ...
		     i++) {
			if (hx83112f_isread_csv == false) {
				if ((int)RAW[i] > LPWUG_IDLE_NOISE_MAX ||
				    (int)RAW[i] < LPWUG_IDLE_NOISE_MIN) {
					TPD_INFO(
	...
	hx83112f_hx_test_data_get(chip_info, RAW, start_log, rslt_log,
				  checktype);
...
int hx83112f_saperate_comma(const struct firmware *file_entry, char **result,
			    int str_size)
{
	int count = 0;
...
int hx83112f_hx_find_crtra_id(char *input)
{
	...
	for (i = 0; i < hx83112f_hx_criteria_size; i++) {
		if (hx83112f_hx_diff_str(hx83112f_g_hx_inspt_crtra_name[i],
					 input) == 0) {
			result = i;
...
int hx83112f_hx_print_crtra_after_parsing(struct chip_data_hx83112f *chip_info)
{
	...
	for (i = 0; i < hx83112f_hx_criteria_size; i++) {
		...
		if (hx83112f_nf_inspt_crtra_flag[i] == 1) {
			...
			printk(KERN_CONT
			       "No this Item in this criteria file!\n");
		}
		printk(KERN_CONT "\n");
	}
...
int hx83112f_hx_get_crtra_by_name(struct chip_data_hx83112f *chip_info,
            ...
            */
			...
			if (count_type < 0) {
				...
				TPD_INFO("Now str=%s, idx=%d\n",
					 hx83112f_g_hx_inspt_crtra_name
						 [count_type],
					 count_type);
...
void hx83112f_limit_get(struct touchpanel_data *ts, struct hx_limit_data *limit)
{
	...
	int all_mut_len = hx83112f_g_chip_info->hw_res->TX_NUM *
			  hx83112f_g_chip_info->hw_res->RX_NUM;
	int str_max_len = 0;
	...
	hx83112f_hx_criteria_item =
		hx83112f_hx_get_size_str_arr(hx83112f_g_himax_inspection_mode);
	hx83112f_hx_criteria_size =
		hx83112f_hx_get_size_str_arr(hx83112f_g_hx_inspt_crtra_name);
	TPD_INFO(
		"%s:There is %d hx83112f_hx_criteria_item and %d hx83112f_hx_criteria_size\n",
	...
	if (!hx83112f_nf_inspt_crtra_flag)
		hx83112f_nf_inspt_crtra_flag = kzalloc(
			hx83112f_hx_criteria_size * sizeof(int), GFP_KERNEL);
	if (!hx83112f_nf_inspection_criteria)
		hx83112f_nf_inspection_criteria = kzalloc(
			sizeof(int *) * hx83112f_hx_criteria_size, GFP_KERNEL);
	...
	for (i = 0; i < hx83112f_hx_criteria_size; i++) {
		if (!hx83112f_nf_inspection_criteria[i]) {
			hx83112f_nf_inspection_criteria[i] =
				kcalloc((hx83112f_g_chip_info->hw_res->TX_NUM *
					 hx83112f_g_chip_info->hw_res->RX_NUM),
	...
	err = request_firmware(&file_entry,
			       hx83112f_g_chip_info->test_limit_name,
			       hx83112f_private_ts->dev);
	if (err < 0) {
	...
	limit->item_name =
		kzalloc(sizeof(char *) * limit->item_size, GFP_KERNEL);
	for (i = 0; i < limit->item_size; i++) {
		if (hx83112f_g_hx_inspt_crtra_name[i] != NULL) {
			limit->item_name[i] = kzalloc(
				sizeof(char) *
					strlen(hx83112f_g_hx_inspt_crtra_name[i]),
				GFP_KERNEL);
			memcpy(limit->item_name[i],
			       hx83112f_g_hx_inspt_crtra_name[i],
			       sizeof(char) *
				       strlen(hx83112f_g_hx_inspt_crtra_name[i]));
		}
	...
	limit->crtra_val =
		kzalloc(sizeof(int *) * limit->item_size, GFP_KERNEL);
	for (i = 0; i < limit->item_size; i++) {
		limit->crtra_val[i] =
			kzalloc(sizeof(int) * limit->rawdata_size, GFP_KERNEL);
	}
	...
	err = hx83112f_hx_get_crtra_by_name(hx83112f_g_chip_info, result,
					    size_of_result_str);
...
int hx83112f_parse_criteria_file(struct chip_data_hx83112f *chip_info)
{
	...
	if (result == NULL) {
		TPD_INFO(
			"%s: result is NULL, alloc memory.%d %lu result = %lu\n",
			__func__, data_size, sizeof(char *),
			(data_size * sizeof(char *)));
		result = (char **)vzalloc(data_size * sizeof(char *));
		if (result != NULL) {
			for (i = 0; i < data_size; i++) {
				result[i] = (char *)kcalloc(
					str_max_len, sizeof(char), GFP_KERNEL);
				if (result[i] == NULL) {
					TPD_INFO(
						"%s: rst_arr Memory allocation falied!\n",
						__func__);
			...
			TPD_INFO(
				"%s: vzalloc result Memory allocation falied!\n",
				__func__);
			goto rst_mem_alloc_failed;
		}
	} else {
	...
	err = hx83112f_hx_get_crtra_by_name(chip_info, result,
					    size_of_result_str);
	if (err != THP_AFE_INSPECT_OK) {
		TPD_INFO("%s:Load criteria from file fail, go end!\n",
			 __func__);
...
int hx83112f_self_test_data_init(struct chip_data_hx83112f *chip_info)
{
	...
	hx83112f_hx_criteria_size =
		hx83112f_hx_get_size_str_arr(hx83112f_g_hx_inspt_crtra_name);
	TPD_INFO(
		"There is %d hx83112f_hx_criteria_item and %d hx83112f_hx_criteria_size\n",
		hx83112f_hx_criteria_item, hx83112f_hx_criteria_size);

	/* init criteria data*/
	hx83112f_nf_inspt_crtra_flag =
		kzalloc(hx83112f_hx_criteria_size * sizeof(int), GFP_KERNEL);
	hx83112f_nf_inspection_criteria =
		kzalloc(sizeof(int *) * hx83112f_hx_criteria_size, GFP_KERNEL);
	if (hx83112f_nf_inspt_crtra_flag == NULL ||
	    hx83112f_nf_inspection_criteria == NULL) {
		TPD_INFO("%s: %d, Memory allocation falied!\n", __func__,
			 __LINE__);
...
void hx83112f_black_screen_test(void *chip_data, char *message)
{
	...
	if (hx83112f_g_rslt_data == NULL) {
		TPD_INFO(
			"%s hx83112f_g_rslt_data is NULL, will vzalloc %d %d %lu hx83112f_g_rslt_data = %lu\n",
			__func__, hx83112f_g_1kind_raw_size,
			hx83112f_hx_criteria_item, sizeof(char),
			(hx83112f_g_1kind_raw_size * hx83112f_hx_criteria_item *
			 sizeof(char)));
		hx83112f_g_rslt_data = (char *)vzalloc(
			hx83112f_g_1kind_raw_size * hx83112f_hx_criteria_item *
			sizeof(char));
		if (!hx83112f_g_rslt_data) {
			TPD_INFO(
				"%s:%d hx83112f_g_rslt_data vzalloc buf error\n",
				__func__, __LINE__);
			goto RET_OUT;
		}
	} else {
		memset(hx83112f_g_rslt_data, 0x00,
		       hx83112f_g_1kind_raw_size * hx83112f_hx_criteria_item *
			       sizeof(char));
	}
	/*Init Log Data */
	...
	do {
		error = hx83112f_mpTestFunc(chip_info,
	...
	hx83112f_nf_fail_write_count +=
		snprintf(g_Test_list_log + hx83112f_nf_fail_write_count, 15,
			 "test Item:\n");
	hx83112f_nf_fail_write_count +=
	...
	if (error) {
		...
		hx83112f_hx_test_data_pop_out(chip_info, g_Test_list_log,
					      g_Company_info_log,
					      g_project_test_info_log,
					      hx83112f_g_rslt_data,
					      hx83112f_g_file_path_NG);
	} else {
	...
	TPD_INFO("%d errors. %s\n", error_num,
		 error_num ? "" : "All test passed.");
	tp_healthinfo_report(hx83112f_g_chip_info->monitor_data_v2,
			     HEALTH_TEST_BLACKSCREEN, &error_num);
...
int hx83112f_chip_self_test(struct seq_file *s,
			    struct chip_data_hx83112f *chip_info,
			    char *g_Test_list_log)
{
	int error = 0;
	...
	do {
		error = hx83112f_mpTestFunc(chip_info, HIMAX_INSPECTION_OPEN,
					    (chip_info->hw_res->TX_NUM *
					     chip_info->hw_res->RX_NUM) +
						    chip_info->hw_res->TX_NUM +
						    chip_info->hw_res->RX_NUM);
...
*/
	...
	do {
		error = hx83112f_mpTestFunc(chip_info, HIMAX_INSPECTION_SHORT,
					    (chip_info->hw_res->TX_NUM *
					     chip_info->hw_res->RX_NUM) +
						    chip_info->hw_res->TX_NUM +
						    chip_info->hw_res->RX_NUM);
	...
	TPD_INFO("[MP_RAW_TEST_RAW]\n");
	do {
		error = hx83112f_mpTestFunc(chip_info, HIMAX_INSPECTION_RAWDATA,
					    (chip_info->hw_res->TX_NUM *
					     chip_info->hw_res->RX_NUM) +
						    chip_info->hw_res->TX_NUM +
						    chip_info->hw_res->RX_NUM);
	...
	do {
		error = hx83112f_mpTestFunc(chip_info, HIMAX_INSPECTION_NOISE,
					    (chip_info->hw_res->TX_NUM *
					     chip_info->hw_res->RX_NUM) +
						    chip_info->hw_res->TX_NUM +
						    chip_info->hw_res->RX_NUM);
					  ...
					  size_t len, loff_t *pos)
{
	...
	if (!HX83112F_HX_PROC_SEND_FLAG) {
		...
		TPD_INFO("himax_register_show: %02X, %02X, %02X, %02X\n",
			 hx83112f_register_command[3],
			 hx83112f_register_command[2],
			 hx83112f_register_command[1],
			 hx83112f_register_command[0]);

		hx83112f_register_read(hx83112f_register_command, max_bus_size,
				       data, hx83112f_cfg_flag);
	...
	if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' &&
		...
		if (buf[0] == 'r') {
			...
			if (!kstrtoul(buf_tmp, 16, &result)) {
				for (loop_i = 0; loop_i < hx83112f_byte_length;
				     loop_i++) {
					hx83112f_register_command[loop_i] =
			...
			if (count < 3) {
				...
				hx83112f_register_write(
					hx83112f_register_command,
					hx83112f_byte_length, w_data,
					hx83112f_cfg_flag);
			} else {
...
int hx83112f_write_read_reg(uint8_t *tmp_addr, uint8_t *tmp_data, uint8_t hb,
			    uint8_t lb)
{
	int cnt = 0;
	...
	if (cnt >= 99) {
		TPD_INFO(
			"hx83112f_write_read_reg ERR Now register 0x%08X : high byte = 0x%02X, low byte = 0x%02X\n",
...
void hx83112f_get_DSRAM_data(uint8_t *info_data, uint8_t x_num, uint8_t y_num)
{
	...
	for (i = 2; i < total_size; i = i + 2) { /* 2:PASSWORD NOT included */
		...
		printk(KERN_CONT "0x%2x:0x%4x ", temp_info_data[i],
		       check_sum_cal);
		if (i % 32 == 0)
			printk(KERN_CONT "\n");
	}
...
void hx83112f_ts_diag_func(struct chip_data_hx83112f *chip_info,
			   int32_t *mutual_data)
{
	int i = 0;
	int j = 0;
	...
	hx83112f_burst_enable(1);
	hx83112f_get_DSRAM_data(info_data, chip_info->hw_res->RX_NUM,
				chip_info->hw_res->TX_NUM);
...
void hx83112f_diag_parse_raw_data(
	struct hx83112f_report_data *hx83112f_hx_touch_data, int mul_num,
	...
	int32_t *self_data)
{
	...
	if (hx83112f_hx_touch_data->hx_rawdata_buf[0] == 0x3A &&
		...
		index = (hx83112f_hx_touch_data->hx_rawdata_buf[2] - 1) *
			RawDataLen_word;
		...
		for (i = 0; i < RawDataLen_word; i++) {
			...
			if (temp1 < mul_num) {
				//mutual
				mutual_data[index + i] =
					((int8_t)hx83112f_hx_touch_data
						 ->hx_rawdata_buf[i * 2 + 4 +
								  1]) *
						256 +
					hx83112f_hx_touch_data->hx_rawdata_buf
						[i * 2 +
...
bool hx83112f_diag_check_sum(
	struct hx83112f_report_data
		*hx83112f_hx_touch_data) /*return checksum value  */
{
	uint16_t check_sum_cal = 0;
	int i;
	...
	for (i = 0, check_sum_cal = 0;
	     i < (hx83112f_hx_touch_data->touch_all_size -
		  hx83112f_hx_touch_data->touch_info_size);
	     i = i + 2) {
		check_sum_cal +=
			(hx83112f_hx_touch_data->hx_rawdata_buf[i + 1] * 256 +
			 hx83112f_hx_touch_data->hx_rawdata_buf[i]);
	}
	if (check_sum_cal % 0x10000 != 0) {
				       ...
				       size_t len, loff_t *pos)
{
	...
	if (hx83112f_g_diag_command > 0 && rawdata_type == 0) {
		TPD_INFO(
			"[Himax]hx83112f_g_diag_command = 0x%x, storage_type=%d, rawdata_type=%d! Maybe no support!\n",
			hx83112f_g_diag_command, storage_type, rawdata_type);
		hx83112f_g_diag_command = 0x00;
	} else {
		TPD_INFO(
			"[Himax]hx83112f_g_diag_command = 0x%x, storage_type=%d, rawdata_type=%d\n",
			hx83112f_g_diag_command, storage_type, rawdata_type);
	}
	...
	if (!HX83112F_HX_PROC_SEND_FLAG) {
		...
		//if ((hx83112f_g_diag_command >= 1 && hx83112f_g_diag_command <= 7) || dsram_type > 0)
		{
			...
			for (i = 0; i < (chip_info->hw_res->TX_NUM *
					 chip_info->hw_res->RX_NUM);
			     i++) {
				if (hx83112f_hx_touch_data->diag_mutual[i] >
				    hx83112f_g_max_mutual) {
					hx83112f_g_max_mutual =
						hx83112f_hx_touch_data
							->diag_mutual[i];
				}
				if (hx83112f_hx_touch_data->diag_mutual[i] <
				    hx83112f_g_min_mutual) {
					hx83112f_g_min_mutual =
						hx83112f_hx_touch_data
							->diag_mutual[i];
				}
			...
			for (i = 0; i < (chip_info->hw_res->TX_NUM +
					 chip_info->hw_res->RX_NUM);
			     i++) {
				if (hx83112f_diag_self[i] >
				    hx83112f_g_max_self) {
					hx83112f_g_max_self =
						hx83112f_diag_self[i];
				}
				if (hx83112f_diag_self[i] <
				    hx83112f_g_min_self) {
					hx83112f_g_min_self =
...
int hx83112f_ic_reset(struct chip_data_hx83112f *chip_info, uint8_t loadconfig,
		      uint8_t int_off)
{
	int ret = 0;
				     ...
				     int max_num)
{
	...
	if (hx83112f_g_diag_command)
		...
		ret = hx83112f_read_event_stack(
			buf, hx83112f_hx_touch_data->touch_info_size);
	if (!ret) {
	...
	check_sum_cal =
		hx83112f_checksum_cal(chip_info, buf, ts_status); //????checksum
	...
	if (ts_status == HX_REPORT_COORD) {
		memcpy(hx83112f_hx_touch_data->hx_coord_buf, &buf[0],
		       hx83112f_hx_touch_data->touch_info_size);
		if (buf[hx_state_info_pos] != 0xFF &&
		    buf[hx_state_info_pos + 1] != 0xFF) {
			memcpy(hx83112f_hx_touch_data->hx_state_info,
			...
			memset(hx83112f_hx_touch_data->hx_state_info, 0x00,
			       sizeof(hx83112f_hx_touch_data->hx_state_info));
		}
	}
	if (hx83112f_g_diag_command) {
		mutual_num =
		...
		self_num =
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM;
		TPD_INFO(
			"hx83112f_hx_touch_data->touch_all_size= %d hx83112f_hx_touch_data->touch_info_size = %d, %d\n",
			hx83112f_hx_touch_data->touch_all_size,
			hx83112f_hx_touch_data->touch_info_size,
			hx83112f_hx_touch_data->touch_all_size -
				hx83112f_hx_touch_data->touch_info_size);
		memcpy(hx83112f_hx_touch_data->hx_rawdata_buf,
		       &buf[hx83112f_hx_touch_data->touch_info_size],
		       hx83112f_hx_touch_data->touch_all_size -
			       hx83112f_hx_touch_data->touch_info_size);
		...
		hx83112f_diag_parse_raw_data(
			hx83112f_hx_touch_data, mutual_num, self_num,
			hx83112f_g_diag_command,
			hx83112f_hx_touch_data->diag_mutual,
			hx83112f_diag_self);
	}
	...
	if (hx83112f_hx_touch_data
		    ->hx_coord_buf[HX83112F_HX_TOUCH_INFO_POINT_CNT] ==
	    0xff) //HX83112F_HX_TOUCH_INFO_POINT_CNT buf???????
		hx_point_num = 0;
	else
		hx_point_num =
			hx83112f_hx_touch_data
	...
	for (i = 0; i < 10; i++) {
		x = hx83112f_hx_touch_data->hx_coord_buf[i * 4] << 8 |
		    hx83112f_hx_touch_data->hx_coord_buf[i * 4 + 1];
		y = (hx83112f_hx_touch_data->hx_coord_buf[i * 4 + 2] << 8 |
		     hx83112f_hx_touch_data->hx_coord_buf[i * 4 + 3]);
		z = hx83112f_hx_touch_data->hx_coord_buf[i + 40];
		if (x >= 0 && x <= hx83112f_private_ts->resolution_info.max_x &&
		    y >= 0 && y <= hx83112f_private_ts->resolution_info.max_y) {
			points[i].x = x;
			points[i].y = y;
	...
	if (panel_data->manufacture_info.version) {
		...
		if (ver_len <= 11) {
			...
			snprintf(panel_data->manufacture_info.version + 9,
				 sizeof(dev_version), dev_version);
		} else {
			strlcpy(&panel_data->manufacture_info.version[12],
				dev_version, 3);
		}
	}
...
int hx83112f_enable_black_gesture(struct chip_data_hx83112f *chip_info,
				  bool enable)
{
	int ret = 0;
	int retry_cnt = 0;
			...
			if (!p_sensor_rec) {
				...
				do {
                    ...
                    * A55A : FW wait for 11 29*/
					...
					tmp_data[1] = 0xA5;
					tmp_data[0] = 0x5A;
					hx83112f_flash_write_burst(tmp_addr,
								   tmp_data);
					back_data[3] = 0XA5;
					back_data[2] = 0X5A;
					back_data[1] = 0XA5;
					back_data[0] = 0X5A;
					hx83112f_register_read(tmp_addr, 4,
							       tmp_data, false);
					TPD_INFO(
						"%s: tmp_data[0] = 0x%02X, retry_cnt=%d \n",
						__func__, tmp_data[0],
						retry_cnt);
					retry_cnt++;
				} while ((tmp_data[3] != back_data[3] ||
	...
	if (ts->headset_pump_support) {
		if (enable) { /* insert headset */
			do {
				...
				hx83112f_register_read(tmp_addr, 4, tmp_data,
						       false);
				TPD_DETAIL(
					"%s:retry times %d, current tmp_data[0, 1, 2, 3] = 0x%2.2X, 0x%2.2X, 0x%2.2X, 0x%2.2X\n",
					__func__, rtimes, tmp_data[0],
				     ...
				     struct gesture_info *gesture)
{
	...
	buf = kzalloc(hx83112f_hx_touch_data->event_size * sizeof(uint8_t),
		      GFP_KERNEL);
	if (!buf) {
	...
	hx83112f_burst_enable(0);
	if (!hx83112f_read_event_stack(buf,
				       hx83112f_hx_touch_data->event_size)) {
	...
	if (buf[GEST_PTLG_ID_LEN] == GEST_PTLG_HDR_ID1 &&
		...
		if (hx83112f_check_point_format == 0) {
			...
			while (i < (gest_len + 1) / 2) {
				if (i == 6) {
					hx83112f_gest_pt_x[hx83112f_gest_pt_cnt] =
						buf[GEST_PTLG_ID_LEN + 4 +
						    i * 2];
				} else {
					hx83112f_gest_pt_x[hx83112f_gest_pt_cnt] =
						buf[GEST_PTLG_ID_LEN + 4 +
				...
				hx83112f_gest_pt_y[hx83112f_gest_pt_cnt] =
			...
			if (hx83112f_private_ts->gesture_buf) {
				...
				hx83112f_private_ts->gesture_buf[0] =
					gesture_sign;
				hx83112f_private_ts->gesture_buf[1] = buf[127];

				if (hx83112f_private_ts->gesture_buf[0] ==
				    0x07) {
					...
					hx83112f_private_ts->gesture_buf[2] =
						pt_num;
				} else {
					hx83112f_private_ts->gesture_buf[2] =
						gest_len;
					memcpy(&hx83112f_private_ts
							->gesture_buf[3],
					       &buf[24], 80);
				}
			}
		...
		if (hx83112f_gest_pt_cnt) {
			...
			gesture->Point_start.x =
				hx83112f_gest_pt_x[0]; /* start x */
			gesture->Point_start.y =
				hx83112f_gest_pt_y[0]; /* start y */
			gesture->Point_end.x =
				hx83112f_gest_pt_x[1]; /* end x */
			gesture->Point_end.y =
				hx83112f_gest_pt_y[1]; /* end y */
				 ...
				 char *g_Test_list_log)
{
	...
	if (eint_count == 10) {
		...
		hx83112f_nf_fail_write_count += snprintf(
			g_Test_list_log + hx83112f_nf_fail_write_count, 45,
			       ...
			       struct syna_testdata *syna_testdata)
{
	...
	if (hx83112f_g_rslt_data == NULL) {
		...
		if (!hx83112f_g_rslt_data) {
			TPD_INFO(
				"%s:%d hx83112f_g_rslt_data kzalloc buf error\n",
				__func__, __LINE__);
			goto RET_OUT;
		}
	} else {
	...
	error_count += hx83112f_int_pin_test(s, chip_info, syna_testdata,
					     g_Test_list_log);
...
static fw_update_state hx83112f_fw_update(void *chip_data,
...

{
	...
	hx83112f_hx_touch_data =
		kzalloc(sizeof(struct hx83112f_report_data), GFP_KERNEL);
	if (hx83112f_hx_touch_data == NULL) {
		goto err_register_driver;
	}

	g_internal_buffer =
		(uint8_t *)vzalloc(sizeof(uint8_t) * HX_SPI_GEN_MAX_WRITE_SZ);
	...
	memcpy(&chip_info->hx_spi_mcc, &hx83112f_hx_spi_ctrdata,
	...
	INIT_DELAYED_WORK(&chip_info->work_0f_update,
			  hx83112f_mcu_0f_operation);

	hx83112f_power_on_init();

	//touch data init
	ret = hx83112f_report_data_init(ts->max_num, ts->hw_res.TX_NUM,
					ts->hw_res.RX_NUM);
	if (ret) {
		goto err_register_driver;
	}
