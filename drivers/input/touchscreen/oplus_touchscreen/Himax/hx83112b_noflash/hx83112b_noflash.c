...
static ssize_t hx83112b_nf_spi_sync(struct touchpanel_data *ts,
...
static int hx83112b_nf_spi_read(uint8_t *command, uint8_t command_len,
...
static int hx83112b_nf_bus_write(uint8_t command, uint32_t length,
				 uint8_t *data)
{
...
static uint32_t hx83112b_nf_hw_check_CRC(uint8_t *start_addr,
					 int reload_length);
static fw_check_state
hx83112b_nf_hw_fw_check(void *chip_data,
...
void hx83112b_nf_flash_write_burst_length(uint8_t *reg_byte,
...
void hx83112b_nf_register_read(uint8_t *read_addr, int read_length,
			       uint8_t *read_data, bool hx83112b_nf_cfg_flag)
{
	uint8_t tmp_data[4];
	int ret;
	if (hx83112b_nf_cfg_flag == false) {
		...
		if (hx83112b_nf_bus_read(read_addr[0], read_length, read_data) <
		    0) {
			TPD_INFO("%s: i2c access fail!\n", __func__);
			return;
		}
...
void hx83112b_nf_register_write(uint8_t *write_addr, int write_length,
				uint8_t *write_data, bool hx83112b_nf_cfg_flag)
{
	uint8_t tmp_addr[4];
	uint8_t *tmp_data;
	...
	if (hx83112b_nf_cfg_flag == false) {
		...
		for (i = 0; i < (total_read_times); i++) {
			...
			if (total_size_temp >= max_bus_size) {
				memcpy(tmp_data,
				       write_data + (i * max_bus_size),
				       max_bus_size);
				hx83112b_nf_flash_write_burst_length(
				...
				memcpy(tmp_data,
				       write_data + (i * max_bus_size), test);
		...
		if (hx83112b_nf_bus_write(write_addr[0], write_length,
					  write_data) < 0) {
			TPD_INFO("%s: i2c access fail!\n", __func__);
			return;
		}
...
void hx83112b_nf_sense_on(uint8_t FlashMode)
{
	...
	if (!FlashMode) {
		...
		do {
			...
			tmp_data[1] = 0x00;
			tmp_data[0] = 0x53;
			hx83112b_nf_register_write(tmp_addr, 4, tmp_data,
						   false);
 ...
 */
static int hx83112b_nf_enable_interrupt(struct chip_data_hx83112b_nf *chip_info,
					bool enable)
{
...
{
	hx83112b_nf_register_write(hx83112b_nf_pzf_op->addr_system_reset, 4,
				   hx83112b_nf_pzf_op->data_system_reset,
				   false);
}
...
int hx83112b_nf_dis_rload_0f(int disable)
{
	...
	do {
		hx83112b_nf_flash_write_burst(
			hx83112b_nf_pzf_op->addr_dis_flash_reload,
			hx83112b_nf_pzf_op->data_dis_flash_reload);
		hx83112b_nf_register_read(
			hx83112b_nf_pzf_op->addr_dis_flash_reload, 4, tmp_data,
			false);
		TPD_DETAIL(
...
void hx83112b_nf_mcu_clean_sram_0f(uint8_t *addr, int write_len, int type)
{
	...
	for (i = 0; i < (total_read_times); i++) {
		...
		if (total_size_temp >= max_bus_size) {
			hx83112b_nf_flash_write_burst_length(tmp_addr, tmp_data,
							     max_bus_size);
...
void hx83112b_nf_mcu_write_sram_0f(const struct firmware *fw_entry,
				   ...
				   uint32_t write_len)
{
	...
	if (hx83112b_nf_chip_info->tmp_data == NULL) {
		...
		hx83112b_nf_chip_info->tmp_data =
			kzalloc(sizeof(uint8_t) * total_size, GFP_KERNEL);
		if (hx83112b_nf_chip_info->tmp_data == NULL) {
			TPD_INFO(
				"%s, alloc hx83112b_nf_chip_info->tmp_data failed\n",
				__func__);
			return;
		}
...
int hx83112b_nf_parse_bin_cfg_data(const struct firmware *fw_entry,
...
static int hx83112b_nf_mcu_Calculate_CRC_with_AP(unsigned char *FW_content,
...
static int hx83112b_nf_zf_part_info(const struct firmware *fw_entry) //zax
{
	...
	/*CFG*/
	cfg_sz = hx83112b_nf_parse_bin_cfg_data(fw_entry, zf_info_arr, FW_buf,
						sram_min);
	...
	if (cfg_sz < 0) {
		...
		cfg_crc_sw = hx83112b_nf_mcu_Calculate_CRC_with_AP(FW_buf, 0,
								   cfg_sz);
	...
	if (HX83112B_NF_POWERONOF == 1) {
		...
		retry = 0;
		do {
			hx83112b_nf_core_fp.fp_write_sram_0f(
				fw_entry, zf_info_arr[0].sram_addr,
				zf_info_arr[0].fw_addr,
				zf_info_arr[0].write_size);
			crc = hx83112b_nf_hw_check_CRC(
			...
			retry++;
			 ...
			 (hx83112b_nf_chip_info->using_headfile && retry < 15));
		...
		retry = 3;
		TPD_DETAIL(
			"Now sram_min[0]=0x%02X,[1]=0x%02X,[2]=0x%02X,[3]=0x%02X\n",
		...
		do {
            ...
            */
			...
			if (cfg_crc_hw != cfg_crc_sw) {
				TPD_INFO(
					"Config CRC FAIL, HW CRC = %X,SW CRC = %X, retry = %d\n",
		...
		hx83112b_nf_core_fp.fp_clean_sram_0f(
			zf_info_arr[0].sram_addr, zf_info_arr[0].write_size, 2);
	}
...
fw_reload:
	if (fw_entry == NULL || reload) {
		...
		if (hx83112b_nf_chip_info->p_firmware_headfile->firmware_data) {
			request_fw_headfile->size =
				hx83112b_nf_chip_info->p_firmware_headfile
					->firmware_size;
	...
	if ((int)tmp_fw_entry->size > HX64K) {
		...
		do {
			hx83112b_nf_core_fp.fp_write_sram_0f(
				tmp_fw_entry,
				hx83112b_nf_pzf_op->data_sram_start_addr, 0,
				HX_48K_SZ);
			crc = hx83112b_nf_hw_check_CRC(
				hx83112b_nf_pzf_op->data_sram_start_addr,
				HX_48K_SZ);
			...
			temp_addr[1] = 0xBF;
			temp_addr[0] = 0xFC;
			hx83112b_nf_register_read(temp_addr, 4, temp_data,
						  false);
			...
			retry++;
			   ...
			   retry < 80) ||
			  (crc != 0 && retry < 30)) &&
			 !(hx83112b_nf_chip_info->using_headfile && retry < 3));
		...
		if (HX83112B_NF_POWERONOF == 1) {
			...
			do {
				hx83112b_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112b_nf_pzf_op->data_cfg_info,
					0xC000, 128); //132
				crc = hx83112b_nf_hw_check_CRC(
					hx83112b_nf_pzf_op->data_cfg_info, 128);
				if (crc == 0) {
					TPD_DETAIL(
				...
				retry++;
				 ...
				 (hx83112b_nf_chip_info->using_headfile &&
				  retry < 15));
			...
			hx83112b_nf_core_fp.fp_clean_sram_0f(
				hx83112b_nf_pzf_op->data_cfg_info, 128, 2);
		}
		/*FW config*/
		if (HX83112B_NF_POWERONOF == 1) {
			retry = 0;
			do {
				hx83112b_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112b_nf_pzf_op->data_fw_cfg_p1,
					0xC0FE, 528); //482
				crc = hx83112b_nf_hw_check_CRC(
					hx83112b_nf_pzf_op->data_fw_cfg_p1,
					528);
			...
			hx83112b_nf_core_fp.fp_clean_sram_0f(
				hx83112b_nf_pzf_op->data_fw_cfg_p1, 528, 1);
		}
		if (HX83112B_NF_POWERONOF == 1) {
			retry = 0;
			do {
				hx83112b_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112b_nf_pzf_op->data_fw_cfg_p2,
					0xCA80, 48);
				crc = hx83112b_nf_hw_check_CRC(
					hx83112b_nf_pzf_op->data_fw_cfg_p2, 48);
				if (crc == 0) {
					TPD_DETAIL(
						"%s, 2 FW config ok in %d time \n",
						__func__, retry);
					break;
				} else {
					TPD_INFO(
						"%s, 2 FW config fail in %d time !\n",
						__func__, retry);
				}
				retry++;
			} while ((crc != 0 && retry < 30) ||
			...
			hx83112b_nf_core_fp.fp_clean_sram_0f(
				hx83112b_nf_pzf_op->data_fw_cfg_p2, 48, 1);
		}
		if (HX83112B_NF_POWERONOF == 1) {
			retry = 0;
			do {
				hx83112b_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112b_nf_pzf_op->data_fw_cfg_p3,
					0xCA00, 128);
				crc = hx83112b_nf_hw_check_CRC(
					hx83112b_nf_pzf_op->data_fw_cfg_p3,
					128);
			...
			hx83112b_nf_core_fp.fp_clean_sram_0f(
				hx83112b_nf_pzf_op->data_fw_cfg_p3, 128, 1);
		}
		...
		if (HX83112B_NF_POWERONOF == 1) {
			...
			do {
				hx83112b_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112b_nf_pzf_op->data_adc_cfg_1,
					0xD6B0, 1284);
				crc = hx83112b_nf_hw_check_CRC(
					hx83112b_nf_pzf_op->data_adc_cfg_1,
					1284);
			...
			hx83112b_nf_core_fp.fp_clean_sram_0f(
				hx83112b_nf_pzf_op->data_adc_cfg_1, 1284, 2);
		}
		...
		if (HX83112B_NF_POWERONOF == 1) {
			...
			do {
				hx83112b_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112b_nf_pzf_op->data_adc_cfg_2,
					0xD358, 856);
				crc = hx83112b_nf_hw_check_CRC(
					hx83112b_nf_pzf_op->data_adc_cfg_2,
					856);
			...
			hx83112b_nf_core_fp.fp_clean_sram_0f(
				hx83112b_nf_pzf_op->data_adc_cfg_2, 856, 2);
		}
		...
		if (HX83112B_NF_POWERONOF == 1) {
			...
			do {
				hx83112b_nf_core_fp.fp_write_sram_0f(
					tmp_fw_entry,
					hx83112b_nf_pzf_op->data_map_table,
					0xE000, 1536);
				crc = hx83112b_nf_hw_check_CRC(
					hx83112b_nf_pzf_op->data_map_table,
					1536);
			...
			hx83112b_nf_core_fp.fp_clean_sram_0f(
				hx83112b_nf_pzf_op->data_map_table, 1536, 2);
		}
	}
	/* switch mode*/
	if (HX83112B_NF_POWERONOF == 1) {
		hx83112b_nf_core_fp.fp_write_sram_0f(
			tmp_fw_entry, hx83112b_nf_pzf_op->data_mode_switch,
			0xC30C, 4);
	} else {
		hx83112b_nf_core_fp.fp_clean_sram_0f(
			hx83112b_nf_pzf_op->data_mode_switch, 4, 2);
	}

	hx83112b_nf_hw_fw_check(hx83112b_nf_pri_ts->chip_data,
				&hx83112b_nf_pri_ts->resolution_info,
				&hx83112b_nf_pri_ts->panel_data);
	...
	timeDelta1.tv_nsec =
		(timeEnd1.tv_sec * 1000000000 + timeEnd1.tv_nsec) -
		(timeStart1.tv_sec * 1000000000 + timeStart1.tv_nsec);
...
int hx83112b_nf_0f_op_file_dirly(char *file_name)
{
	...
	/* trigger reset */
	hx83112b_nf_resetgpio_set(hx83112b_nf_chip_info->hw_res,
				  false); // reset gpio
	hx83112b_nf_resetgpio_set(hx83112b_nf_chip_info->hw_res,
				  true); // reset gpio
...
int hx83112b_nf_mcu_0f_operation_dirly(void)
{
	...
	if (hx83112b_nf_chip_info->g_fw_entry == NULL && !is_oem_unlocked()) {
		...
		if (hx83112b_nf_pri_ts->fw_update_app_support) {
			err = request_firmware_select(
				&hx83112b_nf_chip_info->g_fw_entry,
				hx83112b_nf_pri_ts->panel_data.fw_name,
				hx83112b_nf_pri_ts->dev);
		} else {
	...
	hx83112b_nf_core_fp.fp_firmware_update_0f(
		hx83112b_nf_chip_info->g_fw_entry);
		...
		if (hx83112b_nf_chip_info->g_fw_entry == NULL) {
			...
			if (err < 0) {
				TPD_INFO(
					...
					__func__, __LINE__, err);
				if (hx83112b_nf_chip_info->g_fw_entry != NULL) {
					release_firmware(hx83112b_nf_chip_info
								 ->g_fw_entry);
					hx83112b_nf_chip_info->g_fw_entry =
						NULL;
				}
...
#ifdef HX_0F_DEBUG
void hx83112b_nf_mcu_read_sram_0f(const struct firmware *fw_entry,
...
void hx83112b_nf_mcu_firmware_read_0f(const struct firmware *fw_entry, int type)
{
	...
	TPD_INFO("%s, Entering \n", __func__);
	if (type == 0) { /* first 48K */
		hx83112b_nf_core_fp.fp_read_sram_0f(
			fw_entry, hx83112b_nf_pzf_op->data_sram_start_addr, 0,
			HX_48K_SZ);
		hx83112b_nf_core_fp.fp_read_all_sram(tmp_addr, 0xC000);
	} else { /*last 16k*/
		hx83112b_nf_core_fp.fp_read_sram_0f(
			fw_entry, hx83112b_nf_pzf_op->data_cfg_info, 0xC000,
			132);
		hx83112b_nf_core_fp.fp_read_sram_0f(
			fw_entry, hx83112b_nf_pzf_op->data_fw_cfg, 0xC0FE, 512);
		hx83112b_nf_core_fp.fp_read_sram_0f(
			fw_entry, hx83112b_nf_pzf_op->data_adc_cfg_1, 0xD000,
			376);
		hx83112b_nf_core_fp.fp_read_sram_0f(
			fw_entry, hx83112b_nf_pzf_op->data_adc_cfg_2, 0xD178,
			376);
		hx83112b_nf_core_fp.fp_read_sram_0f(
			fw_entry, hx83112b_nf_pzf_op->data_adc_cfg_3, 0xD000,
			376);
		hx83112b_nf_core_fp.fp_read_all_sram(
			hx83112b_nf_pzf_op->data_sram_clean, HX_32K_SZ);
	}
	TPD_INFO("%s, END \n", __func__);
}
...
void hx83112b_nf_mcu_0f_operation_check(int type)
{
	...
	TPD_INFO("first 4 bytes 0x%2X, 0x%2X, 0x%2X, 0x%2X !\n",
		 hx83112b_nf_chip_info->g_fw_entry->data[0],
		 hx83112b_nf_chip_info->g_fw_entry->data[1],
		 hx83112b_nf_chip_info->g_fw_entry->data[2],
		 hx83112b_nf_chip_info->g_fw_entry->data[3]);
	TPD_INFO("next 4 bytes 0x%2X, 0x%2X, 0x%2X, 0x%2X !\n",
		 hx83112b_nf_chip_info->g_fw_entry->data[4],
		 hx83112b_nf_chip_info->g_fw_entry->data[5],
		 hx83112b_nf_chip_info->g_fw_entry->data[6],
		 hx83112b_nf_chip_info->g_fw_entry->data[7]);
	TPD_INFO("and next 4 bytes 0x%2X, 0x%2X, 0x%2X, 0x%2X !\n",
		 hx83112b_nf_chip_info->g_fw_entry->data[8],
		 hx83112b_nf_chip_info->g_fw_entry->data[9],
		 hx83112b_nf_chip_info->g_fw_entry->data[10],
		 hx83112b_nf_chip_info->g_fw_entry->data[11]);

	hx83112b_nf_core_fp.fp_firmware_read_0f(
		hx83112b_nf_chip_info->g_fw_entry, type);
...
{
	hx83112b_nf_pzf_op =
		kzalloc(sizeof(struct hx83112b_nf_operation), GFP_KERNEL);
	...
	hx83112b_nf_core_fp.fp_firmware_update_0f =
		hx83112b_nf_mcu_firmware_update_0f;
	...
	hx83112b_nf_core_fp.fp_firmware_read_0f =
		hx83112b_nf_mcu_firmware_read_0f;
	hx83112b_nf_core_fp.fp_0f_operation_check =
		hx83112b_nf_mcu_0f_operation_check;
#endif

	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_addr_dis_flash_reload,
		hx83112b_nf_pzf_op->addr_dis_flash_reload,
		sizeof(hx83112b_nf_pzf_op->addr_dis_flash_reload));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_dis_flash_reload,
		hx83112b_nf_pzf_op->data_dis_flash_reload,
		sizeof(hx83112b_nf_pzf_op->data_dis_flash_reload));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_addr_system_reset,
		hx83112b_nf_pzf_op->addr_system_reset,
		sizeof(hx83112b_nf_pzf_op->addr_system_reset));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_system_reset,
		hx83112b_nf_pzf_op->data_system_reset,
		sizeof(hx83112b_nf_pzf_op->data_system_reset));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_sram_start_addr,
		hx83112b_nf_pzf_op->data_sram_start_addr,
		sizeof(hx83112b_nf_pzf_op->data_sram_start_addr));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_sram_clean,
		hx83112b_nf_pzf_op->data_sram_clean,
		sizeof(hx83112b_nf_pzf_op->data_sram_clean));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_cfg_info, hx83112b_nf_pzf_op->data_cfg_info,
		sizeof(hx83112b_nf_pzf_op->data_cfg_info));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_fw_cfg_p1, hx83112b_nf_pzf_op->data_fw_cfg_p1,
		sizeof(hx83112b_nf_pzf_op->data_fw_cfg_p1));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_fw_cfg_p2, hx83112b_nf_pzf_op->data_fw_cfg_p2,
		sizeof(hx83112b_nf_pzf_op->data_fw_cfg_p2));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_fw_cfg_p3, hx83112b_nf_pzf_op->data_fw_cfg_p3,
		sizeof(hx83112b_nf_pzf_op->data_fw_cfg_p3));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_adc_cfg_1, hx83112b_nf_pzf_op->data_adc_cfg_1,
		sizeof(hx83112b_nf_pzf_op->data_adc_cfg_1));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_adc_cfg_2, hx83112b_nf_pzf_op->data_adc_cfg_2,
		sizeof(hx83112b_nf_pzf_op->data_adc_cfg_2));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_adc_cfg_3, hx83112b_nf_pzf_op->data_adc_cfg_3,
		sizeof(hx83112b_nf_pzf_op->data_adc_cfg_3));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_map_table, hx83112b_nf_pzf_op->data_map_table,
		sizeof(hx83112b_nf_pzf_op->data_map_table));
	hx83112b_nf_in_parse_assign_cmd(
		hx83112b_nf_data_mode_switch,
		hx83112b_nf_pzf_op->data_mode_switch,
		sizeof(hx83112b_nf_pzf_op->data_mode_switch));
...
bool hx83112b_nf_ic_package_check(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (1) {
			...
			HX83112B_NF_IC_CHECKSUM =
				HX83112B_NF_TP_BIN_CHECKSUM_CRC;
			hx83112b_nf_0f_init();
			...
			g_i_FW_VER =
				i_CTPM_FW[HX83112B_NF_FW_VER_MAJ_FLASH_ADDR]
					<< 8 |
				i_CTPM_FW[HX83112B_NF_FW_VER_MIN_FLASH_ADDR];
			g_i_CFG_VER =
				i_CTPM_FW[HX83112B_NF_CFG_VER_MAJ_FLASH_ADDR]
					<< 8 |
				i_CTPM_FW[HX83112B_NF_CFG_VER_MIN_FLASH_ADDR];
			g_i_CID_MAJ =
				i_CTPM_FW[HX83112B_NF_CID_VER_MAJ_FLASH_ADDR];
			g_i_CID_MIN =
				i_CTPM_FW[HX83112B_NF_CID_VER_MIN_FLASH_ADDR];
#endif
...
static void hx83112b_nf_read_FW_ver(void)
{
	...
	while (reload_status == 0) {
		cmd[3] =
			0x10; // oplus fw id bin address : 0xc014   , 49172    Tp ic address : 0x 10007014
		cmd[2] = 0x00;
		cmd[1] = 0x7f;
		cmd[0] = 0x00;
...
void hx83112b_nf_read_OPLUS_FW_ver(void)
{
	...
	cmd[3] =
		0x10; // oplus fw id bin address : 0xc014    Tp ic address : 0x 10007014
	cmd[2] = 0x00;
	cmd[1] = 0x70;
	cmd[0] = 0x14;
	hx83112b_nf_register_read(cmd, 4, data, false);
...
int hx83112b_nf_report_data_init(int max_touch_point, int tx_num, int rx_num)
{
	...
	hx83112b_nf_touch_data->touch_all_size =
		128; //himax_get_touch_data_size();
	...
	if ((max_touch_point % 4) == 0)
		...
		HX83112B_NF_TOUCH_INFO_POINT_CNT +=
	...
	if (hx83112b_nf_touch_data->raw_cnt_rmd != 0x00) { //more than 4 fingers
		hx83112b_nf_touch_data->rawdata_size = hx83112b_nf_cal_data_len(
			hx83112b_nf_touch_data->raw_cnt_rmd, max_touch_point,
			hx83112b_nf_touch_data->raw_cnt_max);
		hx83112b_nf_touch_data->touch_info_size =
			(max_touch_point + hx83112b_nf_touch_data->raw_cnt_max +
			 2) *
			4;
	...
	if ((tx_num * rx_num + tx_num + rx_num) %
		    hx83112b_nf_touch_data->rawdata_size ==
	    0) {
		hx83112b_nf_touch_data->rawdata_frame_size =
	...
	TPD_INFO("%s: rawdata_frame_size = %d ", __func__,
		 hx83112b_nf_touch_data->rawdata_frame_size);
	TPD_INFO(
		"%s: max_touch_point:%d,hx_raw_cnt_max:%d,hx_raw_cnt_rmd:%d,g_hx_rawdata_size:%d,hx83112b_nf_touch_data->touch_info_size:%d\n",
		__func__, max_touch_point, hx83112b_nf_touch_data->raw_cnt_max,
		hx83112b_nf_touch_data->raw_cnt_rmd,
		hx83112b_nf_touch_data->rawdata_size,
		hx83112b_nf_touch_data->touch_info_size);

	hx83112b_nf_touch_data->hx_coord_buf = kzalloc(
		sizeof(uint8_t) * (hx83112b_nf_touch_data->touch_info_size),
		GFP_KERNEL);
	if (hx83112b_nf_touch_data->hx_coord_buf == NULL) {
		goto mem_alloc_fail;
	}

	hx83112b_nf_touch_data->diag_mutual =
	...
	hx83112b_nf_touch_data->hx_rawdata_buf = kzalloc(
		sizeof(uint8_t) * (hx83112b_nf_touch_data->touch_all_size -
				   hx83112b_nf_touch_data->touch_info_size),
		GFP_KERNEL);
	...
	//#if defined(HX_SMART_WAKEUP)
	hx83112b_nf_touch_data->hx_event_buf =
		kzalloc(sizeof(uint8_t) * (hx83112b_nf_touch_data->event_size),
			GFP_KERNEL);
...
void hx83112b_nf_esd_hw_reset(struct chip_data_hx83112b_nf *chip_info)
{
	...
	do {
		hx83112b_nf_resetgpio_set(chip_info->hw_res,
					  true); // reset gpio
		hx83112b_nf_resetgpio_set(chip_info->hw_res,
					  false); // reset gpio
		hx83112b_nf_resetgpio_set(chip_info->hw_res,
		...
		load_fw_times--;
		hx83112b_nf_core_fp.fp_0f_operation_dirly();
		ret = hx83112b_nf_core_fp.fp_reload_disable(
...
int hx83112b_nf_checksum_cal(struct chip_data_hx83112b_nf *chip_info,
			     uint8_t *buf, int ts_status)
{
	...
	/* SMWP */
		...
		length = (HX83112B_NF_GEST_PTLG_ID_LEN +
			  HX83112B_NF_GEST_PTLG_HDR_LEN);
	} else {
	...
	if (ts_status == HX83112B_NF_REPORT_COORD) {
		...
		if ((hx_esd_event == length || hx_zero_event == length) &&
			...
			shaking_ret = hx83112b_nf_ic_esd_recovery(
			...
			HX83112B_NF_ESD_RESET_ACTIVATE = 0;
			TPD_INFO(
				"[HX83112B_NF_ESD_RESET_ACTIVATE]:%s: Back from reset, ready to serve.\n",
				__func__);
			goto checksum_fail;
			...
			HX83112B_NF_HW_RESET_ACTIVATE = 0;
			TPD_INFO(
				"[HX83112B_NF_HW_RESET_ACTIVATE]:%s: Back from reset, ready to serve.\n",
				__func__);
			goto ready_to_serve;
		}
	}
...
void hx83112b_nf_log_touch_data(
	uint8_t *buf, struct hx83112b_nf_report_data *hx83112b_nf_touch_data)
{
	int loop_i = 0;
	int print_size = 0;
...
int hx83112b_nf_get_rawdata(struct chip_data_hx83112b_nf *chip_info,
			    uint32_t *RAW, uint32_t datalen)
{
	...
	while (retry < 200) {
		...
		hx83112b_nf_register_read(tmp_addr, 4, tmp_data, false);
		if ((tmp_data[0] == HX83112B_NF_Data_PWD0 &&
		     tmp_data[1] == HX83112B_NF_Data_PWD1) ||
		    (tmp_data[0] == HX83112B_NF_Data_PWD1 &&
		     tmp_data[1] == HX83112B_NF_Data_PWD0)) {
			break;
		}
	...
	while (retry < 200) {
		if (tmp_data[0] == HX83112B_NF_Data_PWD1 &&
		    tmp_data[1] == HX83112B_NF_Data_PWD0) {
			break;
		}
...
uint32_t hx83112b_nf_check_mode(uint8_t checktype)
{
	...
	hx83112b_nf_register_read(tmp_addr, 4, tmp_data, false);
	TPD_INFO(
		"%s: hx83112b_nf_wait_sorting_mode, tmp_data[0]=%x,tmp_data[1]=%x\n",
		__func__, tmp_data[0], tmp_data[1]);
...
int hx83112b_nf_mpTestFunc(int fd, struct chip_data_hx83112b_nf *chip_info,
			   uint8_t checktype, uint32_t datalen)
{
	...
	if (hx83112b_nf_check_mode(checktype)) {
		...
		if (checktype == HIMAX_INSPECTION_NOISE) {
			hx83112b_nf_set_N_frame(HX83112B_NF_NOISEFRAME,
						checktype);
			...
			hx83112b_nf_set_N_frame(
				HX83112B_NF_DOZE_RAWDATA_NOISEFRAME, checktype);
		} else if (checktype >= HIMAX_INSPECTION_LPWUG_RAWDATA) {
			TPD_INFO("N frame = %d\n",
				 HX83112B_NF_LPWUG_RAWDATAFRAME);
			hx83112b_nf_set_N_frame(HX83112B_NF_LPWUG_RAWDATAFRAME,
						checktype);
		} else {
			hx83112b_nf_set_N_frame(HX83112B_NF_OTHERSFRAME,
						checktype);
		}
	...
	case HIMAX_INSPECTION_SORTING:
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (RAW[i] < HX83112B_NF_SORTINGMIN) {
				hx83112b_nf_store_to_file(fd, "==>%d, ",
							  RAW[i]);
				is_test_failed = true;
			} else {
				hx83112b_nf_store_to_file(fd, "%d, ", RAW[i]);
	...
	case HIMAX_INSPECTION_OPEN:
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (RAW[i] > HX83112B_NF_OPENMAX ||
			    RAW[i] < HX83112B_NF_OPENMIN) {
				hx83112b_nf_store_to_file(fd, "==>%d, ",
							  RAW[i]);
				is_test_failed = true;
			} else {
	...
	case HIMAX_INSPECTION_MICRO_OPEN:
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (RAW[i] > HX83112B_NF_M_OPENMAX ||
			    RAW[i] < HX83112B_NF_M_OPENMIN) {
				hx83112b_nf_store_to_file(fd, "==>%d, ",
							  RAW[i]);
				is_test_failed = true;
			} else {
		...
		TPD_INFO(
			"HX83112B_NF_M_OPENMAX = %d,HX83112B_NF_M_OPENMIN = %d\n",
	...
	case HIMAX_INSPECTION_SHORT:
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (RAW[i] > HX83112B_NF_SHORTMAX ||
			    RAW[i] < HX83112B_NF_SHORTMIN) {
				hx83112b_nf_store_to_file(fd, "==>%d, ",
							  RAW[i]);
				is_test_failed = true;
			} else {
	...
	case HIMAX_INSPECTION_RAWDATA:
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (RAW[i] > HX83112B_NF_RAWMAX ||
			    RAW[i] < HX83112B_NF_RAWMIN) {
				hx83112b_nf_store_to_file(fd, "==>%d, ",
							  RAW[i]);
				is_test_failed = true;
			} else {
	...
	case HIMAX_INSPECTION_NOISE:
		...
		for (i = 0; i < chip_info->hw_res->RX_NUM; i++) {
			for (j = 0; j < chip_info->hw_res->TX_NUM; j++) {
				if ((j == HX83112B_NF_SKIPRXNUM) &&
				    (i >= SKIPTXNUM_START) &&
				    (i >= SKIPTXNUM_END)) {
					*(pInspectGridData +
					  ((chip_info->hw_res->TX_NUM - j) *
						   chip_info->hw_res->RX_NUM -
					   i - 1)) = 0;
				} else {
					*(pInspectGridData +
					  ((chip_info->hw_res->TX_NUM - j) *
						   chip_info->hw_res->RX_NUM -
					   i - 1)) = (uint16_t)RAW_Rawdata
						[i * chip_info->hw_res->TX_NUM +
						 j];
				}
			}
		}
	...
	case HIMAX_INSPECTION_LPWUG_RAWDATA:
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (RAW[i] > HX83112B_NF_LPWUG_RAWDATA_MAX ||
			    RAW[i] < HX83112B_NF_LPWUG_RAWDATA_MIN) {
				hx83112b_nf_store_to_file(fd, "==>%d, ",
							  RAW[i]);
				is_test_failed = true;
			} else {
	...
	case HIMAX_INSPECTION_LPWUG_NOISE:
		...
		hx83112b_nf_store_to_file(fd, "\n Noise weight: %d", weight);
		if (weight > HX83112B_NF_LPWUG_NOISEMAX ||
		    weight < HX83112B_NF_LPWUG_NOISE_MIN) {
			TPD_INFO("%s: HIMAX_INSPECTION_LPWUG_NOISE FAIL\n",
				 __func__);
			return THP_AFE_INSPECT_ENOISE;
		}
	...
	case HIMAX_INSPECTION_DOZE_RAWDATA:
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (RAW[i] > HX83112B_NF_DOZE_RAWDATA_MAX ||
			    RAW[i] < HX83112B_NF_DOZE_RAWDATA_MIN) {
				hx83112b_nf_store_to_file(fd, "==>%d, ",
							  RAW[i]);
				is_test_failed = true;
			} else {
	...
	case HIMAX_INSPECTION_DOZE_NOISE:
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (RAW[i] > HX83112B_NF_DOZE_NOISE_MAX ||
			    RAW[i] < HX83112B_NF_DOZE_NOISE_MIN) {
				hx83112b_nf_store_to_file(fd, "==>%d, ",
							  RAW[i]);
				is_test_failed = true;
			} else {
	...
	case HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA:
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (RAW[i] > HX83112B_NF_LPWUG_IDLE_RAWDATA_MAX ||
			    RAW[i] < HX83112B_NF_LPWUG_IDLE_RAWDATA_MIN) {
				hx83112b_nf_store_to_file(fd, "==>%d, ",
							  RAW[i]);
				is_test_failed = true;
			} else {
	...
	case HIMAX_INSPECTION_LPWUG_IDLE_NOISE:
		...
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (RAW[i] > HX83112B_NF_LPWUG_IDLE_NOISE_MAX ||
			    RAW[i] < HX83112B_NF_LPWUG_IDLE_NOISE_MIN) {
				hx83112b_nf_store_to_file(fd, "==>%d, ",
							  RAW[i]);
				is_test_failed = true;
			} else {
...
static void hx83112b_nf_black_screen_test(void *chip_data, char *message)
{
	...
	mm_segment_t old_fs;
	struct chip_data_hx83112b_nf *chip_info =
		(struct chip_data_hx83112b_nf *)chip_data;
	...
	do {
		error = hx83112b_nf_mpTestFunc(
			fd, chip_info, HIMAX_INSPECTION_LPWUG_RAWDATA,
			(chip_info->hw_res->TX_NUM *
			 chip_info->hw_res->RX_NUM) +
				chip_info->hw_res->TX_NUM +
				chip_info->hw_res->RX_NUM);
	...
	do {
		error = hx83112b_nf_mpTestFunc(
			fd, chip_info, HIMAX_INSPECTION_LPWUG_NOISE,
			(chip_info->hw_res->TX_NUM *
			 chip_info->hw_res->RX_NUM) +
				chip_info->hw_res->TX_NUM +
				chip_info->hw_res->RX_NUM);
	...
	do {
		error = hx83112b_nf_mpTestFunc(
			fd, chip_info, HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA,
			(chip_info->hw_res->TX_NUM *
			 chip_info->hw_res->RX_NUM) +
				chip_info->hw_res->TX_NUM +
				chip_info->hw_res->RX_NUM);
	...
	do {
		error = hx83112b_nf_mpTestFunc(
			fd, chip_info, HIMAX_INSPECTION_LPWUG_IDLE_NOISE,
			(chip_info->hw_res->TX_NUM *
			 chip_info->hw_res->RX_NUM) +
				chip_info->hw_res->TX_NUM +
				chip_info->hw_res->RX_NUM);
...
int hx83112b_nf_chip_self_test(struct seq_file *s,
			       struct chip_data_hx83112b_nf *chip_info,
			       struct syna_testdata *syna_testdata)
{
	...
	do {
		error = hx83112b_nf_mpTestFunc(
			syna_testdata->fd, chip_info, HIMAX_INSPECTION_SORTING,
			(chip_info->hw_res->TX_NUM *
			 chip_info->hw_res->RX_NUM) +
				chip_info->hw_res->TX_NUM +
				chip_info->hw_res->RX_NUM);
	...
	do {
		error = hx83112b_nf_mpTestFunc(
			syna_testdata->fd, chip_info, HIMAX_INSPECTION_SHORT,
			(chip_info->hw_res->TX_NUM *
			 chip_info->hw_res->RX_NUM) +
				chip_info->hw_res->TX_NUM +
				chip_info->hw_res->RX_NUM);
	...
	do {
		error = hx83112b_nf_mpTestFunc(
			syna_testdata->fd, chip_info, HIMAX_INSPECTION_RAWDATA,
			(chip_info->hw_res->TX_NUM *
			 chip_info->hw_res->RX_NUM) +
				chip_info->hw_res->TX_NUM +
				chip_info->hw_res->RX_NUM);
	...
	do {
		error = hx83112b_nf_mpTestFunc(
			syna_testdata->fd, chip_info, HIMAX_INSPECTION_NOISE,
			(chip_info->hw_res->TX_NUM *
			 chip_info->hw_res->RX_NUM) +
				chip_info->hw_res->TX_NUM +
				chip_info->hw_res->RX_NUM);
	...
	do {
		...
		retry_cnt++;
		 ...
		 retry_cnt < HX83112B_NF_REG_RETRY_TIMES);
...
void hx83112b_nf_flash_programming(uint8_t *FW_content, int FW_Size)
{
	...
	for (page_prog_start = 0; page_prog_start < FW_Size;
		...
		for (j = 0; j < 5; j++) {
			...
			if (hx83112b_nf_bus_write(0x00, program_length + 4,
						  buring_data) < 0) {
				TPD_INFO("%s: i2c access fail!\n", __func__);
				return;
			}
...
int hx83112b_nf_fts_ctpm_fw_upgrade_with_sys_fs_64k(
...
static size_t hx83112b_nf_proc_register_read(struct file *file, char *buf,
					     size_t len, loff_t *pos)
{
	...
	if (!HX83112B_NF_HX_PROC_SEND_FLAG) {
		...
		TPD_INFO("himax_register_show: %02X,%02X,%02X,%02X\n",
			 hx83112b_nf_register_command[3],
			 hx83112b_nf_register_command[2],
			 hx83112b_nf_register_command[1],
			 hx83112b_nf_register_command[0]);

		hx83112b_nf_register_read(hx83112b_nf_register_command,
					  max_bus_size, data,
					  hx83112b_nf_cfg_flag);
...
static size_t hx83112b_nf_proc_register_write(struct file *file,
	...
	if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' &&
		...
		if (buf[0] == 'r') {
			...
			if (!kstrtoul(buf_tmp, 16, &result)) {
				for (loop_i = 0;
				     loop_i < hx83112b_nf_byte_length;
				     loop_i++) {
					hx83112b_nf_register_command[loop_i] =
			...
			if (count < 3) {
				...
				hx83112b_nf_register_write(
					hx83112b_nf_register_command,
					hx83112b_nf_byte_length, w_data,
					hx83112b_nf_cfg_flag);
			} else {
				hx83112b_nf_byte_length =
					x_pos[1] - x_pos[0] - 2;
				...
				hx83112b_nf_register_write(
					hx83112b_nf_register_command,
					hx83112b_nf_byte_length, &w_data[0],
					hx83112b_nf_cfg_flag);
			}
		} else {
			return len;
...
int hx83112b_nf_write_read_reg(uint8_t *tmp_addr, uint8_t *tmp_data, uint8_t hb,
			       uint8_t lb)
{
	int cnt = 0;
	...
	if (cnt >= 99) {
		TPD_INFO(
			"hx83112b_nf_write_read_reg ERR Now register 0x%08X : high byte=0x%02X,low byte=0x%02X\n",
...
void hx83112b_nf_get_DSRAM_data(uint8_t *info_data, uint8_t x_num,
				uint8_t y_num)
{
	int i = 0;
	//int cnt = 0;
	...
	tmp_data[0] = 0xA5;
	fw_run_flag =
		hx83112b_nf_write_read_reg(tmp_addr, tmp_data, 0xA5, 0x5A);
	if (fw_run_flag < 0) {
...
void hx83112b_nf_ts_diag_func(struct chip_data_hx83112b_nf *chip_info,
			      int32_t *mutual_data)
{
	int i = 0;
	int j = 0;
	...
	hx83112b_nf_burst_enable(1);
	hx83112b_nf_get_DSRAM_data(info_data, chip_info->hw_res->RX_NUM,
				   chip_info->hw_res->TX_NUM);
...
void hx83112b_nf_diag_parse_raw_data(
	struct hx83112b_nf_report_data *hx83112b_nf_touch_data, int mul_num,
	...
	int32_t *self_data)
{
	...
	if (hx83112b_nf_touch_data->hx_rawdata_buf[0] == 0x3A &&
		...
		index = (hx83112b_nf_touch_data->hx_rawdata_buf[2] - 1) *
			RawDataLen_word;
		...
		for (i = 0; i < RawDataLen_word; i++) {
			...
			if (temp1 < mul_num) {
				//mutual
				mutual_data[index + i] =
					((int8_t)hx83112b_nf_touch_data
						 ->hx_rawdata_buf[i * 2 + 4 +
								  1]) *
						256 +
					hx83112b_nf_touch_data->hx_rawdata_buf
						[i * 2 +
...
bool hx83112b_nf_diag_check_sum(
	struct hx83112b_nf_report_data
		*hx83112b_nf_touch_data) //return checksum value
{
	uint16_t check_sum_cal = 0;
	int i;
	...
	for (i = 0, check_sum_cal = 0;
	     i < (hx83112b_nf_touch_data->touch_all_size -
		  hx83112b_nf_touch_data->touch_info_size);
	     i = i + 2) {
		check_sum_cal +=
			(hx83112b_nf_touch_data->hx_rawdata_buf[i + 1] * 256 +
			 hx83112b_nf_touch_data->hx_rawdata_buf[i]);
	}
	if (check_sum_cal % 0x10000 != 0) {
...
static size_t hx83112b_nf_proc_diag_write(struct file *file, const char *buff,
					  size_t len, loff_t *pos)
{
	char messages[80] = { 0 };
	...
	struct touchpanel_data *ts = PDE_DATA(file_inode(file));
	struct chip_data_hx83112b_nf *chip_info =
		(struct chip_data_hx83112b_nf *)ts->chip_data;
	...
	TPD_INFO(" messages       = %s\n"
		 ...
		 " storage_type   = 0x%x\n"
		 " rawdata_type   = 0x%x\n",
		 messages, hx83112b_nf_diag_command, storage_type,
		 rawdata_type);

	if (hx83112b_nf_diag_command > 0 && rawdata_type == 0) {
		TPD_INFO(
			"[Himax]hx83112b_nf_diag_command=0x%x ,storage_type=%d, rawdata_type=%d! Maybe no support!\n",
			hx83112b_nf_diag_command, storage_type, rawdata_type);
		hx83112b_nf_diag_command = 0x00;
	} else {
		TPD_INFO(
			"[Himax]hx83112b_nf_diag_command=0x%x ,storage_type=%d, rawdata_type=%d\n",
			hx83112b_nf_diag_command, storage_type, rawdata_type);
	}
...
static size_t hx83112b_nf_proc_diag_read(struct file *file, char *buff,
					 size_t len, loff_t *pos)
{
	size_t ret = 0;
	char *temp_buf;
	...
	if (!HX83112B_NF_HX_PROC_SEND_FLAG) {
		...
		//if ((hx83112b_nf_diag_command >= 1 && hx83112b_nf_diag_command <= 7) || dsram_type > 0)
		{
			...
			for (i = 0; i < (chip_info->hw_res->TX_NUM *
					 chip_info->hw_res->RX_NUM);
			     i++) {
				if (hx83112b_nf_touch_data->diag_mutual[i] >
				    hx83112b_nf_max_mutual) {
					hx83112b_nf_max_mutual =
						hx83112b_nf_touch_data
							->diag_mutual[i];
				}
				if (hx83112b_nf_touch_data->diag_mutual[i] <
				    hx83112b_nf_min_mutual) {
					hx83112b_nf_min_mutual =
						hx83112b_nf_touch_data
							->diag_mutual[i];
				}
			...
			for (i = 0; i < (chip_info->hw_res->TX_NUM +
					 chip_info->hw_res->RX_NUM);
			     i++) {
				if (hx83112b_nf_diag_self[i] >
				    hx83112b_nf_max_self) {
					hx83112b_nf_max_self =
						hx83112b_nf_diag_self[i];
				}
				if (hx83112b_nf_diag_self[i] <
				    hx83112b_nf_min_self) {
					hx83112b_nf_min_self =
						hx83112b_nf_diag_self[i];
				}
			}
...
uint8_t hx83112b_nf_read_DD_status(struct chip_data_hx83112b_nf *chip_info,
				   uint8_t *hx83112b_nf_cmd_set,
				   uint8_t *tmp_data)
{
	int cnt = 0;
	...
	TPD_INFO(
		"%s: hx83112b_nf_cmd_set[0] = 0x%02X,hx83112b_nf_cmd_set[1] = 0x%02X,hx83112b_nf_cmd_set[2] = 0x%02X,hx83112b_nf_cmd_set[3] = 0x%02X\n",
		__func__, hx83112b_nf_cmd_set[0], hx83112b_nf_cmd_set[1],
		hx83112b_nf_cmd_set[2], hx83112b_nf_cmd_set[3]);
...
static size_t hx83112b_nf_proc_DD_debug_read(struct file *file, char *buf,
					     size_t len, loff_t *pos)
{
	int ret = 0;
	uint8_t tmp_data[64];
	...
	if (!HX83112B_NF_HX_PROC_SEND_FLAG) {
		...
		if (hx83112b_nf_mutual_set_flag == 1) {
			if (hx83112b_nf_read_DD_status(chip_info,
						       hx83112b_nf_cmd_set,
						       tmp_data) == NO_ERR) {
				for (loop_i = 0;
				     loop_i < hx83112b_nf_cmd_set[0];
				     loop_i++) {
					if ((loop_i % 8) == 0) {
						ret += snprintf(
							temp_buf + ret,
...
static size_t hx83112b_nf_proc_DD_debug_write(struct file *file,
					      const char *buff, size_t len,
					      loff_t *pos)
{
	uint8_t i = 0;
	...
	if (buf_tmp[2] == 'x' && buf_tmp[6] == 'x' && buf_tmp[10] == 'x') {
		...
		TPD_INFO(
			"hx83112b_nf_cmd_set[2] = %02X, hx83112b_nf_cmd_set[1] = %02X, hx83112b_nf_cmd_set[0] = %02X\n",
			hx83112b_nf_cmd_set[2], hx83112b_nf_cmd_set[1],
			hx83112b_nf_cmd_set[0]);
	} else
...
int hx83112b_nf_read_FW_status(struct chip_data_hx83112b_nf *chip_info,
			       uint8_t *state_addr, uint8_t *tmp_addr)
{
	...
	if (state_addr[0] == 0x01) {
		...
		req_size = 0x04;
		hx83112b_nf_sense_off();
		hx83112b_nf_register_read(status_addr, req_size, tmp_addr,
					  false);
		hx83112b_nf_sense_on(1);
					     ...
					     size_t len, loff_t *pos)
{
	...
	if (!HX83112B_NF_HX_PROC_SEND_FLAG) {
		...
		if (hx83112b_nf_read_FW_status(chip_info, hx83112b_nf_cmd_set,
					       tmp_data) == NO_ERR) {
			ret += snprintf(temp_buf + ret, len - ret,
					"0x%02X%02X%02X%02X :\t",
					hx83112b_nf_cmd_set[5],
					hx83112b_nf_cmd_set[4],
					hx83112b_nf_cmd_set[3],
					hx83112b_nf_cmd_set[2]);
			for (loop_i = 0; loop_i < hx83112b_nf_cmd_set[1];
			     loop_i++) {
				ret += snprintf(temp_buf + ret, len - ret,
						"%5d\t", tmp_data[loop_i]);
			}
...
static int
hx83112b_nf_configuration_init(struct chip_data_hx83112b_nf *chip_info,
			       bool config)
{
	int ret = 0;
...
int hx83112b_nf_ic_reset(struct chip_data_hx83112b_nf *chip_info,
			 uint8_t loadconfig, uint8_t int_off)
{
	...
	if (chip_info->hw_res->reset_gpio) {
		if (int_off) {
			...
			if (ret < 0) {
				TPD_INFO(
					"%s: hx83112b enable interrupt failed.\n",
					__func__);
				return ret;
			}
		}
		...
		if (loadconfig) {
			...
			if (ret < 0) {
				TPD_INFO(
					"%s: hx83112b configuration init failed.\n",
					__func__);
				return ret;
			}
...
static size_t hx83112b_nf_proc_sense_on_off_write(struct file *file,
						  const char *buff, size_t len,
						  loff_t *pos)
{
	char buf[80] = { 0 };
...
static int hx83112b_nf_get_touch_points(void *chip_data,
					struct point_info *points, int max_num)
{
	...
	if (hx83112b_nf_diag_command)
		...
		ret = hx83112b_nf_read_event_stack(
			buf, hx83112b_nf_touch_data->touch_info_size);
	if (!ret) {
	...
	check_sum_cal = hx83112b_nf_checksum_cal(chip_info, buf,
						 ts_status); //????checksum
	...
	if (ts_status == HX83112B_NF_REPORT_COORD) {
		memcpy(hx83112b_nf_touch_data->hx_coord_buf, &buf[0],
		       hx83112b_nf_touch_data->touch_info_size);
		if (buf[hx_state_info_pos] != 0xFF &&
		    buf[hx_state_info_pos + 1] != 0xFF) {
			memcpy(hx83112b_nf_touch_data->hx_state_info,
			       &buf[hx_state_info_pos], 2);
		} else {
			memset(hx83112b_nf_touch_data->hx_state_info, 0x00,
			       sizeof(hx83112b_nf_touch_data->hx_state_info));
		}
	}
	if (hx83112b_nf_diag_command) {
		mutual_num =
		...
		self_num =
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM;
		TPD_INFO(
			"hx83112b_nf_touch_data->touch_all_size= %d hx83112b_nf_touch_data->touch_info_size = %d ,  %d\n",
			hx83112b_nf_touch_data->touch_all_size,
			hx83112b_nf_touch_data->touch_info_size,
			hx83112b_nf_touch_data->touch_all_size -
				hx83112b_nf_touch_data->touch_info_size);
		memcpy(hx83112b_nf_touch_data->hx_rawdata_buf,
		       &buf[hx83112b_nf_touch_data->touch_info_size],
		       hx83112b_nf_touch_data->touch_all_size -
			       hx83112b_nf_touch_data->touch_info_size);
		...
		hx83112b_nf_diag_parse_raw_data(
			hx83112b_nf_touch_data, mutual_num, self_num,
			hx83112b_nf_diag_command,
			hx83112b_nf_touch_data->diag_mutual,
			hx83112b_nf_diag_self);
	}

	if (hx83112b_nf_touch_data
		    ->hx_coord_buf[HX83112B_NF_TOUCH_INFO_POINT_CNT] ==
	    0xff) //HX83112B_NF_TOUCH_INFO_POINT_CNT buf???????
		hx_point_num = 0;
	else
		hx_point_num =
			hx83112b_nf_touch_data
	...
	for (i = 0; i < 10; i++) {
		x = hx83112b_nf_touch_data->hx_coord_buf[i * 4] << 8 |
		    hx83112b_nf_touch_data->hx_coord_buf[i * 4 + 1];
		y = (hx83112b_nf_touch_data->hx_coord_buf[i * 4 + 2] << 8 |
		     hx83112b_nf_touch_data->hx_coord_buf[i * 4 + 3]);
		z = hx83112b_nf_touch_data->hx_coord_buf[i + 40];
		if (x >= 0 && x <= hx83112b_nf_pri_ts->resolution_info.max_x &&
		    y >= 0 && y <= hx83112b_nf_pri_ts->resolution_info.max_y) {
			points[i].x = x;
			points[i].y = y;
...
static int hx83112b_nf_get_vendor(void *chip_data,
				  struct panel_info *panel_data)
{
	struct chip_data_hx83112b_nf *chip_info =
		(struct chip_data_hx83112b_nf *)chip_data;
			...
			struct panel_info *panel_data)
{
	...
	if (panel_data->manufacture_info.version)
		sprintf(panel_data->manufacture_info.version, "0x%x-%d",
			panel_data->TP_FW, hx83112b_nf_lcd_vendor);

	return FW_NORMAL;
}

static u8 hx83112b_nf_trigger_reason(void *chip_data, int gesture_enable,
				     int is_suspended)
{
...
static int
hx83112b_nf_enable_black_gesture(struct chip_data_hx83112b_nf *chip_info,
				 bool enable)
{
	int ret = 0;
...
static int
hx83112b_nf_enable_charge_mode(struct chip_data_hx83112b_nf *chip_info,
			       bool enable)
{
	int ret = 0;
	uint8_t tmp_addr[4];
...
//on = 1:on   0:off
static int hx83112b_nf_jitter_switch(struct chip_data_hx83112b_nf *chip_info,
				     bool on)
{
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
...
static int
hx83112b_nf_enable_headset_mode(struct chip_data_hx83112b_nf *chip_info,
				bool enable)
{
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
	...
	if (ts->headset_pump_support) {
		if (enable) { //insert headset
			do {
				...
				tmp_data[1] = 0xA5;
				tmp_data[0] = 0x5A;
				hx83112b_nf_flash_write_burst(tmp_addr,
							      tmp_data);

				hx83112b_nf_register_read(tmp_addr, 4, tmp_data,
							  false);
				TPD_DETAIL(
					"%s:retry times %d, current tmp_data[0,1,2,3] = 0x%2.2X,0x%2.2X,0x%2.2X,0x%2.2X\n",
					__func__, rtimes, tmp_data[0],
...
static int hx83112b_nf_rotative_switch(struct chip_data_hx83112b_nf *chip_info,
				       int mode)
{
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
	...
	if (ts->fw_edge_limit_support) {
		if (mode == 1 ||
		    VERTICAL_SCREEN == chip_info->touch_direction) { //vertical
			do {
				if (rtimes > 10) {
...
static int hx83112b_nf_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_EDGE:
		...
		if (ret < 0) {
			TPD_INFO(
				"%s: hx83112b enable edg & corner limit failed.\n",
				__func__);
			return ret;
		}
...
static int hx83112b_nf_get_gesture_info(void *chip_data,
					struct gesture_info *gesture)
{
	...
	buf = kzalloc(hx83112b_nf_touch_data->event_size * sizeof(uint8_t),
		      GFP_KERNEL);
	if (!buf) {
	...
	hx83112b_nf_burst_enable(0);
	if (!hx83112b_nf_read_event_stack(buf,
					  hx83112b_nf_touch_data->event_size)) {
	...
	if (buf[HX83112B_NF_GEST_PTLG_ID_LEN] !=
		    HX83112B_NF_GEST_PTLG_HDR_ID1 ||
	    buf[HX83112B_NF_GEST_PTLG_ID_LEN + 1] !=
		    HX83112B_NF_GEST_PTLG_HDR_ID2) {
		goto RET_OUT;
	}
	...
	if (buf[HX83112B_NF_GEST_PTLG_ID_LEN] ==
		...
		while (i < (gest_len + 1) / 2) {
			if (i == 6) {
				hx83112b_nf_gest_pt_x[hx83112b_nf_gest_pt_cnt] =
					buf[HX83112B_NF_GEST_PTLG_ID_LEN + 4 +
					    i * 2];
			} else {
				hx83112b_nf_gest_pt_x[hx83112b_nf_gest_pt_cnt] =
					buf[HX83112B_NF_GEST_PTLG_ID_LEN + 4 +
			...
			hx83112b_nf_gest_pt_y[hx83112b_nf_gest_pt_cnt] =
				buf[HX83112B_NF_GEST_PTLG_ID_LEN + 4 + i * 2 +
				    1] *
				hx83112b_nf_pri_ts->resolution_info.max_y / 255;
			i++;
		...
		if (hx83112b_nf_gest_pt_cnt) {
			...
			gesture->Point_start.x =
				hx83112b_nf_gest_pt_x[0]; //start x
			gesture->Point_start.y =
				hx83112b_nf_gest_pt_y[0]; //start y
...
static int hx83112b_nf_int_pin_test(struct seq_file *s, void *chip_data,
				    struct syna_testdata *syna_testdata)
{
...
static void hx83112b_nf_auto_test(struct seq_file *s, void *chip_data,
				  struct syna_testdata *syna_testdata)
{
	int error_count = 0;
	int ret = 0;
...
static void hx83112b_nf_read_debug_data(struct seq_file *s, void *chip_data,
					int HX83112B_NF_DEBUG_DATA_TYPE)
{
	uint16_t mutual_num;
	uint16_t self_num;
	uint16_t width;
...
hx83112b_nf_fw_update(void *chip_data, const struct firmware *fw, bool force)
{
	...
	if (hx83112b_nf_chip_info->g_fw_entry == NULL) {
		if (hx83112b_nf_pri_ts->fw_update_app_support) {
			if (fw_name_fae == NULL) {
				...
				err = request_firmware(
					&hx83112b_nf_chip_info->g_fw_entry,
					fw_name_fae, hx83112b_nf_pri_ts->dev);
			}
		} else {
			err = request_firmware(
				&hx83112b_nf_chip_info->g_fw_entry,
...
static int hx83112b_nf_tp_probe(struct spi_device *spi)
{
	...
	hx83112b_nf_touch_data =
		kzalloc(sizeof(struct hx83112b_nf_report_data), GFP_KERNEL);
	...
	memcpy(&chip_info->hx_spi_mcc, &hx83112b_nf_spi_ctrdata,
	...
	//touch data init
	ret = hx83112b_nf_report_data_init(ts->max_num, ts->hw_res.TX_NUM,
					   ts->hw_res.RX_NUM);
	if (ret) {
		goto err_register_driver;
	}
