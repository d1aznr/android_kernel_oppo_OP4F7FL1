...
bool himax_sense_off(void)
{
	...
	do {
		...
		if (tmp_data[0] == 0x0C) {
			...
			usleep_range(10000, 10001);
#ifdef HX_RST_PIN_FUNC
			hx83102d_resetgpio_set(g_chip_info->hw_res,
					       false); // reset gpio
			hx83102d_resetgpio_set(g_chip_info->hw_res,
					       true); // reset gpio
#else
...
static int hx_turn_on_mp_func(int on)
{
	...
	if (on) {
		...
		himax_in_parse_assign_cmd(PWD_TURN_ON_MPAP_OVL, tmp_data,
					  sizeof(tmp_data));
		do {
			himax_register_write(tmp_addr, 4, tmp_data, 0);
			usleep_range(10000, 10001);
			himax_register_read(tmp_addr, 4, tmp_read, false);
        ...
        */
		if (total_size_temp >= max_bus_size) {
			himax_flash_write_burst_length(
				tmp_addr,
				&(g_chip_info->tmp_data[start_index +
							i * max_bus_size]),
				max_bus_size);
			total_size_temp = total_size_temp - max_bus_size;
		} else {
...
static int hx83102d_0f_overlay(int ovl_type, int mode)
{
	...
	if (g_chip_info->g_fw_sta) {
		...
		if (g_chip_info->p_firmware_headfile->firmware_data) {
			request_fw_headfile->size =
				g_chip_info->p_firmware_headfile->firmware_size;
			request_fw_headfile->data =
				g_chip_info->p_firmware_headfile->firmware_data;
			tmp_fw_entry = request_fw_headfile;
	...
	if (IN_SELF_TEST == 0) {
		...
		if (himax_sram_write_crc_check(tmp_fw_entry, sram_addr, offset,
					       size) != 0)
		...
		count2 = 0;
		do {
			himax_register_write(handshaking_addr, 4, send_data,
					     false);
			msleep(1);
			himax_register_read(handshaking_addr, 4, recv_data,
					    false);
...
bool hx_parse_bin_cfg_data(const struct firmware *fw_entry)
{
	...
	for (i = 0; i < part_num; i++) {
		...
		g_zf_info_arr[i].cfg_addr += g_zf_info_arr[i].sram_addr[2]
					     << 16;
		g_zf_info_arr[i].cfg_addr += g_zf_info_arr[i].sram_addr[3]
					     << 24;
...
#endif
		memcpy(hx83102d_nf_FW_buf +
			       (g_zf_info_arr[i].cfg_addr - dsram_base),
		       (unsigned char *)&fw_entry
			       ->data[g_zf_info_arr[i].fw_addr],
		       g_zf_info_arr[i].write_size);
	}
	hx83102d_nf_cfg_crc = himax_mcu_Calculate_CRC_with_AP(
		hx83102d_nf_FW_buf, 0, hx83102d_nf_cfg_sz);
...
static int hx83102d_nf_zf_part_info(const struct firmware *fw_entry)
{
	...
	if (flag_1k_header == true)
		crc = himax_sram_write_crc_check(fw_entry,
						 pzf_op->data_sram_start_addr,
						 HX1K,
						 g_zf_info_arr[0].write_size);
	else
		crc = himax_sram_write_crc_check(fw_entry,
						 pzf_op->data_sram_start_addr,
						 0,
	...
	if (crc != 0)
		TPD_INFO("size=%d CRC Failed! CRC = %X",
			 g_zf_info_arr[0].write_size, crc);
	do {
		himax_mcu_register_write(hx83102d_nf_sram_min,
					 hx83102d_nf_cfg_sz, hx83102d_nf_FW_buf,
					 0);
		crc = himax_hw_check_CRC(hx83102d_nf_sram_min,
					 hx83102d_nf_cfg_sz);
		if (crc != hx83102d_nf_cfg_crc)
			TPD_INFO(
				"Config CRC FAIL, HW CRC = %X, SW CRC = %X, retry time = %d",
				crc, hx83102d_nf_cfg_crc, retry);
		retry++;
	} while (!ret && retry < 10);
	...
	if (g_zf_info_arr[ovl_idx_t].write_size == 0) {
		...
		if (himax_sram_write_crc_check(
			    fw_entry, g_zf_info_arr[ovl_idx_t].sram_addr,
...
fw_reload:
	...
	if ((int)tmp_fw_entry->size > HX64K) {
		...
		do {
			...
			TPD_DETAIL(
				"%s, 64K LAST 4 BYTES: data[3] = 0x%02X, data[2] = 0x%02X, data[1] = 0x%02X, data[0] = 0x%02X\n",
		...
		if (G_POWERONOF == 1) {
			...
			do {
				g_core_fp.fp_write_sram_0f(
					tmp_fw_entry, pzf_op->data_cfg_info,
					0xC000, 128); //132
...
/*Himax_DB_Test End*/
	...
	TPD_INFO("%s, END first_download_finished = %d\n", __func__,
		 g_chip_info->first_download_finished);
...
static void himax_read_FW_ver(void)
{
	...
	if (private_ts->panel_data.manufacture_info.version) {
		if (private_ts->panel_data.vid_len == 0) {
			strlcpy(&(private_ts->panel_data.manufacture_info
					  .version[12]),
				dev_version, 3);
		} else {
...
int himax_report_data_init(int max_touch_point, int tx_num, int rx_num)
{
	...
	TPD_INFO("%s: rawdata_frame_size = %d ", __func__,
		 hx_touch_data->rawdata_frame_size);
	TPD_INFO(
		"%s: max_touch_point:%d, hx_raw_cnt_max:%d, hx_raw_cnt_rmd:%d, g_hx_rawdata_size:%d, hx_touch_data->touch_info_size:%d\n",
		       ...
		       int ts_status)
{
	...
	if (ts_status == HX_REPORT_COORD) {
		...
		if (hx_esd_event == length || hx_zero_event == length) {
			...
			TPD_INFO(
				"[HX_ESD_RESET_ACTIVATE]:%s: Back from reset, ready to serve.\n",
				__func__);
			goto checksum_fail;
			...
			TPD_INFO(
				"[HX_HW_RESET_ACTIVATE]:%s: Back from reset, ready to serve.\n",
				__func__);
			goto ready_to_serve;
		}
	}
...
int hx_test_data_pop_out(struct chip_data_hx83102d *chip_info,
...
int hx_test_data_get(struct chip_data_hx83102d *chip_info, uint32_t RAW[],
	...
	case HIMAX_INSPECTION_OPEN:
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (isRead_csv == false) {
				...
				if (RAW[i] > hx83102d_nf_inspection_criteria
						     [IDX_OPENMAX][i] ||
				    RAW[i] < hx83102d_nf_inspection_criteria
						     [IDX_OPENMIN][i]) {
					TPD_INFO("%s: open test FAIL\n",
						 __func__);
					ret = RESULT_ERR;
	...
	case HIMAX_INSPECTION_LPWUG_RAWDATA:
		for (i = 0; i < (chip_info->hw_res->TX_NUM *
			...
			if (isRead_csv == false) {
				...
				if (RAW[i] >
					    hx83102d_nf_inspection_criteria
	...
	case HIMAX_INSPECTION_LPWUG_NOISE:
		if (isBD12proj || isEA006proj) {
			...
			if (isRead_csv == false) {
				...
				if (weight > LPWUG_NOISEMAX ||
				    weight < hx83102d_nf_inspection_criteria
	...
	for (i = 0; i < HX_CRITERIA_SIZE; i++) {
		...
		if (hx83102d_nf_inspt_crtra_flag[i] == 1) {
			for (j = 0; j < all_mut_len; j++) {
				TPD_DEBUG(
					"%d, ",
					hx83102d_nf_inspection_criteria[i][j]);
				if (j % 16 == 15)
					TPD_DEBUG("\n");
			}
		} else {
...
static int hx_get_crtra_by_name(struct chip_data_hx83102d *chip_info,
...
static void hx83102d_black_screen_test(void *chip_data, char *message)
{
	...
	struct chip_data_hx83102d *chip_info =
		(struct chip_data_hx83102d *)chip_data;
	int i = 0;
	...
	if (g_rslt_data == NULL) {
		TPD_INFO(
			"%s g_rslt_data is NULL,will vzalloc %d %d %lu g_rslt_data = %lu\n",
			__func__, g_1kind_raw_size, HX_CRITERIA_ITEM,
			sizeof(char),
			(g_1kind_raw_size * HX_CRITERIA_ITEM * sizeof(char)));
		g_rslt_data = (char *)vzalloc(g_1kind_raw_size *
					      HX_CRITERIA_ITEM * sizeof(char));
		if (!g_rslt_data) {
	...
	TPD_INFO("%s : p_node=%s, copy_len=%d,postfix:%s, fw_name: %s\n",
		 __func__, p_node, copy_len, postfix,
		 private_ts->panel_data.fw_name);
	...
	snprintf(buf, 128, "6. MP_LPWUG_TEST_RAW: %s\n",
		 error ? "Error" : "Ok");
	hx83102d_nf_fail_write_count +=
		snprintf(g_Test_list_log + hx83102d_nf_fail_write_count, 15,
			 "test Item:\n");
	hx83102d_nf_fail_write_count +=
		snprintf(g_Test_list_log + hx83102d_nf_fail_write_count, 32,
	...
	hx83102d_nf_fail_write_count +=
		snprintf(g_Test_list_log + hx83102d_nf_fail_write_count, 22,
			 "Final_result: %s\n", error_num ? "Fail" : "Pass");
	if (error) {
		...
		hx_test_data_pop_out(chip_info, g_Test_list_log,
				     g_Company_info_log,
				     g_project_test_info_log, g_rslt_data,
				     g_file_path_NG);
	} else {
...
RET_OUT:
	...
	if (hx83102d_nf_inspection_criteria != NULL) {
		...
		hx83102d_nf_inspection_criteria = NULL;
		TPD_INFO(
			"Now it have free the hx83102d_nf_inspection_criteria!\n");
	} else {
			 ...
			 uint8_t lb)
{
	...
	if (cnt >= 99) {
		TPD_INFO(
			"himax_write_read_reg ERR Now register 0x%08X : high byte = 0x%02X, low byte = 0x%02X\n",
				       ...
				       size_t len, loff_t *pos)
{
	...
	struct touchpanel_data *ts = PDE_DATA(file_inode(file));
	struct chip_data_hx83102d *chip_info =
		(struct chip_data_hx83102d *)ts->chip_data;
	...
	if (g_diag_command > 0 && rawdata_type == 0) {
		TPD_INFO(
			"[Himax]g_diag_command = 0x%x, storage_type=%d, rawdata_type=%d! Maybe no support!\n",
			g_diag_command, storage_type, rawdata_type);
		g_diag_command = 0x00;
	} else {
		TPD_INFO(
			"[Himax]g_diag_command = 0x%x, storage_type=%d, rawdata_type=%d\n",
			g_diag_command, storage_type, rawdata_type);
	}
			     ...
			     uint8_t *cmd_set, uint8_t *tmp_data)
{
	...
	TPD_INFO(
		"%s: cmd_set[0] = 0x%02X, cmd_set[1] = 0x%02X, cmd_set[2] = 0x%02X, cmd_set[3] = 0x%02X\n",
		   ...
		   uint8_t int_off)
{
	...
	if (chip_info->hw_res->reset_gpio) {
		if (int_off) {
			...
			if (ret < 0) {
				TPD_INFO(
					"%s: hx83102d enable interrupt failed.\n",
					__func__);
				return ret;
			}
		}
		...
		if (loadconfig) {
			...
			if (ret < 0) {
				TPD_INFO(
					"%s: hx83102d configuration init failed.\n",
					__func__);
				return ret;
			}
				     ...
				     int max_num)
{
	...
	if (g_diag_command)
		...
		ret = himax_read_event_stack(buf,
					     hx_touch_data->touch_info_size);
	if (!ret) {
	...
	if (g_diag_command) {
		...
		self_num =
			chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM;
		TPD_INFO(
			"hx_touch_data->touch_all_size= %d hx_touch_data->touch_info_size = %d, %d\n",
	...
	if (hx_touch_data->hx_coord_buf[HX_TOUCH_INFO_POINT_CNT] ==
...
static int hx83102d_ftm_process(void *chip_data)
{
	...
	switch_spi7cs_state(
		false); /* in case of current leakaging in ftm mode */
					 ...
					 bool enable)
{
	...
	if (ts->is_suspended) {
		...
		if (enable) {
			...
			if (ovl_done == 1) {
#ifdef HX_RST_PIN_FUNC
				hx83102d_resetgpio_set(chip_info->hw_res,
						       true); // reset gpio
				hx83102d_resetgpio_set(chip_info->hw_res,
						       false); // reset gpio
				hx83102d_resetgpio_set(chip_info->hw_res,
...
static int hx83102d_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_EDGE:
		...
		if (ret < 0) {
			TPD_INFO(
				"%s: hx83102d enable edg & corner limit failed.\n",
				__func__);
			return ret;
		}
				 ...
				 char *g_Test_list_log)
{
	...
	if (eint_count == 10) {
		...
		hx83102d_nf_fail_write_count += snprintf(
			g_Test_list_log + hx83102d_nf_fail_write_count, 45,
...
static int himax_apk_tp_info_get(void *chip_data, char *buf, int len)
{
	...
	ret = snprintf(buf, len, "IC:HIMAX%06X\nFW_VER:0x%04X\nCH:%dX%d\n",
		       0x83102D, chip_info->fw_ver, chip_info->hw_res->TX_NUM,
		       chip_info->hw_res->RX_NUM);
	if (ret > len) {
		ret = len;
	}
...
int __maybe_unused hx83102d_tp_probe(struct spi_device *spi)
{
	...
	if (is_oem_unlocked()) {
		TPD_INFO(
			"Replace system image for cts, download fw by headfile\n");
		queue_delayed_work(chip_info->himax_0f_update_wq,
				   &chip_info->work_0f_update,
