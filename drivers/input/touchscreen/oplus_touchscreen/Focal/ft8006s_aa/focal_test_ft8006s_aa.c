...
static int short_get_adcdata_incell(u8 retval, u8 ch_num, int byte_num,
				    int *adc_buf)
{
	int ret = 0;
	int times = 0;
...
static int chip_clb(void)
{
	...
	while (times++ < FACTORY_TEST_RETRY) {
		...
		if ((0 == ret) && (0x02 == val)) {
			...
			TPD_DEBUG("reg%x=%x,retry:%d", FACTORY_REG_CLB, val,
				  times);
	}
...
static int fts_short_test(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	if (!ts_data->short_data || !ts_data->fts_autotest_offset ||
	    !ts_data->fts_autotest_offset->node_valid) {
		FTS_TEST_SAVE_ERR("short_data/fts_autotest_offset is null\n");
		return -EINVAL;
	}

	if (!ts_data->fts_autotest_offset->fts_short_data_P ||
	    !ts_data->fts_autotest_offset->fts_short_data_N) {
		FTS_TEST_SAVE_ERR(
			"fts_short_data_P || fts_short_data_N is NULL\n");
		return -EINVAL;
	}
	...
	ret = short_get_adcdata_incell(TEST_RETVAL_AA, rx_num, byte_num,
				       ts_data->short_data);
	if (ret < 0) {
	...
	for (i = 0; i < node_num; i++) {
		...
		ts_data->short_data[i] =
			(6090 * 4096 / (74 * tmp_adc - 17 * 4096)) - 20;
...
static int fts_open_test(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	if (!ts_data->open_data || !ts_data->fts_autotest_offset ||
	    !ts_data->fts_autotest_offset->node_valid) {
	...
	if (!ts_data->fts_autotest_offset->fts_open_data_P ||
		...
		FTS_TEST_SAVE_ERR(
			"fts_open_data_P || fts_open_data_N is NULL\n");
		return -EINVAL;
	}
...
static int fts_cb_test(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	if (!ts_data->cb_data || !ts_data->fts_autotest_offset ||
	    !ts_data->fts_autotest_offset->node_valid) {
...
static int fts_rawdata_test(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	if (!ts_data->raw_data || !ts_data->fts_autotest_offset ||
	    !ts_data->fts_autotest_offset->node_valid) {
	...
	/* read rawdata */
	ret = read_mass_data(FACTORY_REG_RAWDATA_ADDR, byte_num,
			     ts_data->raw_data);
	if (ret < 0) {
...
static int fts_lcdnoise_test(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	if (!ts_data->lcdnoise_data || !ts_data->fts_autotest_offset ||
	    !ts_data->fts_autotest_offset->node_valid) {
		FTS_TEST_SAVE_ERR(
			"lcdnoise_data/fts_autotest_offset is null\n");
		return -EINVAL;
	}

	if (!ts_data->fts_autotest_offset->fts_lcdnoise_data_P ||
	    !ts_data->fts_autotest_offset->fts_lcdnoise_data_N) {
		FTS_TEST_SAVE_ERR(
			"fts_lcdnoise_data_P || fts_lcdnoise_data_N is NULL\n");
		return -EINVAL;
	}
	...
	for (i = 0; i < node_num; i++) {
		...
		if ((ts_data->lcdnoise_data[i] <
		     ts_data->fts_autotest_offset->fts_lcdnoise_data_N[i]) ||
		    (ts_data->lcdnoise_data[i] >
			...
			TPD_INFO(
				...
				ts_data->lcdnoise_data[i],
				ts_data->fts_autotest_offset
					->fts_lcdnoise_data_N[i],
				ts_data->fts_autotest_offset
					->fts_lcdnoise_data_P[i]);
			tmp_result = false;
		}
	}
...
static void fts_auto_write_result(struct fts_ts_data *ts_data, int failed_count)
{
	...
	if (ts_data->black_screen_test) {
		if (failed_count) {
			snprintf(
				file_data_buf, 128,
				"/sdcard/TpTestReport/screenOff/NG/tp_testlimit_%02d%02d%02d-%02d%02d%02d-fail-utc.csv",
			...
			snprintf(
				file_data_buf, 128,
				"/sdcard/TpTestReport/screenOff/OK/tp_testlimit_%02d%02d%02d-%02d%02d%02d-pass-utc.csv",
				(rtc_now_time.tm_year + 1900) % 100,
				rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
				rtc_now_time.tm_hour, rtc_now_time.tm_min,
				rtc_now_time.tm_sec);
	...
	if (ts_data->black_screen_test) {
...
#endif /*CONFIG_ARCH_HAS_SYSCALL_WRAPPER*/
		buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
				  "LCD Noise Test", 27, tx_num, rx_num,
				  11 + tx_num * 2, 1);
#ifdef CONFIG_ARCH_HAS_SYSCALL_WRAPPER
		ksys_write(ts_data->csv_fd, data_buf, buflen);
#else
		...
		sys_write(ts_data->csv_fd, data_buf, buflen);
#endif /*CONFIG_ARCH_HAS_SYSCALL_WRAPPER*/
		fts_test_save_data("CB Test", ts_data->cb_data, node_num,
				   rx_num, ts_data->csv_fd);
		fts_test_save_data("RawData Test", ts_data->raw_data, node_num,
				   rx_num, ts_data->csv_fd);
		fts_test_save_data("LCD Noise Test", ts_data->lcdnoise_data,
...
#endif /*CONFIG_ARCH_HAS_SYSCALL_WRAPPER*/
		buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
				  "Short Circuit Test", 15, tx_num, rx_num, 11,
				  1);
#ifdef CONFIG_ARCH_HAS_SYSCALL_WRAPPER
		ksys_write(ts_data->csv_fd, data_buf, buflen);
#else
...
#endif /*CONFIG_ARCH_HAS_SYSCALL_WRAPPER*/
		buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
				  "Open Test", 25, tx_num, rx_num, 11 + tx_num,
				  1);
#ifdef CONFIG_ARCH_HAS_SYSCALL_WRAPPER
		ksys_write(ts_data->csv_fd, data_buf, buflen);
#else
		...
		sys_write(ts_data->csv_fd, data_buf, buflen);
#endif /*CONFIG_ARCH_HAS_SYSCALL_WRAPPER*/
		fts_test_save_data("Short Circuit Test", ts_data->short_data,
				   node_num, rx_num, ts_data->csv_fd);
		fts_test_save_data("Open Test", ts_data->open_data, node_num,
				   rx_num, ts_data->csv_fd);
		fts_test_save_data("CB Test", ts_data->cb_data, node_num,
				   rx_num, ts_data->csv_fd);
		fts_test_save_data("RawData Test", ts_data->raw_data, node_num,
				      ...
				      const struct firmware *limit_fw)
{
	...
	ts_data->fts_autotest_offset->node_valid =
		kzalloc(node_num * sizeof(int32_t), GFP_KERNEL);
	...
	if (FTS_THR_DEBUG) {
		...
		ts_data->fts_autotest_offset->fts_lcdnoise_data_N =
			kzalloc(node_num * sizeof(int32_t), GFP_KERNEL);
		ts_data->fts_autotest_offset->fts_lcdnoise_data_P =
			kzalloc(node_num * sizeof(int32_t), GFP_KERNEL);
		for (i = 0; i < node_num; i++) {
			ts_data->fts_autotest_offset->fts_short_data_N[i] =
				1000;
...
static void fts_print_threshold(struct fts_ts_data *ts_data)
{
	...
	print_buffer(ts_data->fts_autotest_offset->fts_short_data_P, node_num,
		     rx_num);
	print_buffer(ts_data->fts_autotest_offset->fts_short_data_N, node_num,
		     rx_num);

	TPD_DEBUG("open threshold max/min:\n");
	print_buffer(ts_data->fts_autotest_offset->fts_open_data_P, node_num,
		     rx_num);
	print_buffer(ts_data->fts_autotest_offset->fts_open_data_N, node_num,
		     rx_num);

	TPD_DEBUG("cb threshold max/min:\n");
	print_buffer(ts_data->fts_autotest_offset->fts_cb_data_P, node_num,
		     rx_num);
	print_buffer(ts_data->fts_autotest_offset->fts_cb_data_N, node_num,
		     rx_num);
	...
	print_buffer(ts_data->fts_autotest_offset->fts_lcdnoise_data_P,
		     node_num, rx_num);
	print_buffer(ts_data->fts_autotest_offset->fts_lcdnoise_data_N,
		     node_num, rx_num);
}

static int fts_enter_test_environment(struct touchpanel_data *ts,
				      bool test_state)
{
	int ret = 0;
	u8 detach_flag = 0;
	...
	if (test_state) {
		...
		ret = request_firmware_select(&fw, ts->panel_data.fw_name,
					      ts->dev);
	}
