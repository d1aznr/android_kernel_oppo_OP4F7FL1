			      ...
			      size_t count, loff_t *ppos)
{
	...
	case PROC_READ_DATA:
		...
		ret = fts_read(ts_data->spi, proc->cmd, proc->cmd_len, readbuf,
			       num_read_chars);
		if (ret < 0) {
...
static int fts_dpram_write(struct fts_ts_data *ts_data, u32 saddr,
...
static int fts_ecc_cal_tp(struct fts_ts_data *ts_data, u32 ecc_saddr,
			  u32 ecc_len, u16 *ecc_value)
{
	int ret = 0;
	int i = 0;
...
static int fts_pram_write_ecc(struct fts_ts_data *ts_data, u8 *buf, u32 len)
{
	...
	ret = fts_ecc_cal_tp(ts_data, pram_start_addr, pram_app_size,
			     &ecc_in_tp);
	if (ret < 0) {
...
static int fts_fw_write_start(struct fts_ts_data *ts_data, u8 *buf, u32 len,
			      bool need_reset)
{
	int ret = 0;
...
static int fts_fw_download(struct fts_ts_data *ts_data, u8 *buf, u32 len,
			   bool need_reset)
{
	int ret = 0;
	int i = 0;
...
static int fts_get_rawdata(struct fts_ts_data *ts_data, int *raw, bool is_diff)
{
	...
	if (is_diff) {
		ret = fts_read_reg(ts_data->spi, FACTORY_REG_DATA_SELECT,
				   &regval);
		ret = fts_write_reg(ts_data->spi, FACTORY_REG_DATA_SELECT,
				    0x01);
		if (ret < 0) {
				   ...
				   struct panel_info *panel_data)
{
	...
	if (i >= 10) {
		fts_read(ts_data->spi, &cmd, 1, id, 2);
		TPD_INFO("%s:boot id:0x%02x%02x, fw abnormal", __func__, id[0],
			 id[1]);
		return FW_ABNORMAL;
	}
...
static int fts_fw_recovery(struct fts_ts_data *ts_data)
{
	...
	ts_data->h_fw_file =
		(u8 *)ts->panel_data.firmware_headfile.firmware_data;
...
static u8 fts_trigger_reason(void *chip_data, int gesture_enable,
			     int is_suspended)
{
	struct fts_ts_data *ts_data = (struct fts_ts_data *)chip_data;
	int ret = 0;
				...
				int max_num)
{
	...
	for (i = 0; i < max_num; i++) {
		...
		touch_point++;
		points[pointid].x =
			((buf[2 + base] & 0x0F) << 8) + (buf[3 + base] & 0xFF);
		points[pointid].y =
			((buf[4 + base] & 0x0F) << 8) + (buf[5 + base] & 0xFF);
...
static int fts_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	if ((gesture->gesture_type != FingerprintDown) &&
		...
		gesture->Point_start.x =
			(u16)((gesture_buf[4] << 8) + gesture_buf[5]);
		gesture->Point_start.y =
			(u16)((gesture_buf[6] << 8) + gesture_buf[7]);
		gesture->Point_end.x =
			(u16)((gesture_buf[8] << 8) + gesture_buf[9]);
		gesture->Point_end.y =
			(u16)((gesture_buf[10] << 8) + gesture_buf[11]);
		gesture->Point_1st.x =
			(u16)((gesture_buf[12] << 8) + gesture_buf[13]);
		gesture->Point_1st.y =
			(u16)((gesture_buf[14] << 8) + gesture_buf[15]);
		gesture->Point_2nd.x =
			(u16)((gesture_buf[16] << 8) + gesture_buf[17]);
		gesture->Point_2nd.y =
			(u16)((gesture_buf[18] << 8) + gesture_buf[19]);
		gesture->Point_3rd.x =
			(u16)((gesture_buf[20] << 8) + gesture_buf[21]);
		gesture->Point_3rd.y =
			(u16)((gesture_buf[22] << 8) + gesture_buf[23]);
		gesture->Point_4th.x =
			(u16)((gesture_buf[24] << 8) + gesture_buf[25]);
		gesture->Point_4th.y =
			(u16)((gesture_buf[26] << 8) + gesture_buf[27]);
	}
...
*/
	...
	memcpy(&ts_data->spi_ctrl, &spi_ctrdata,
	...
	{
		TPD_INFO(
			"In Recovery mode,or oem unlocked,or fw_update_in_probe_with_headfile is enable,no-flash download fw by headfile\n");
		disable_irq_nosync(ts_data->ts->irq);
