...
static int ft3681_test_read(u8 addr, u8 *readbuf, int readlen)
{
	...
	for (i = 1; i < packet_num; i++) {
		...
		ret = ft3681_test_bus_read(&addr, 1, &readbuf[offset],
					   packet_length);
...
static void ft3681_test_save_data(char *name, int *data, int datacnt, int line,
				  int fd)
{
	char *data_buf = NULL;
	u32 cnt = 0;
...
static int enter_work_mode(void)
{
	...
	for (i = 0; i < ENTER_WORK_FACTORY_RETRIES; i++) {
		...
		if (ret >= 0) {
			...
			for (j = 0; j < 20; j++) {
				ret = ft3681_test_read_reg(DEVIDE_MODE_ADDR,
							   &mode);
...
static int
ft3681_special_operation_for_samsung(struct chip_data_ft3681 *ts_data)
{
	int ret = 0;
	...
	if (true ==
		...
		if (ret < 0) {
			FTS_TEST_SAVE_ERR(
				"write FT3681_REG_SAMSUNG_SPECIFAL fail at %s,ret=%d\n",
				__func__, ret);
			return -EIO;
		}
	}
	...
	for (i = 0; i < ENTER_WORK_FACTORY_RETRIES; i++) {
		...
		if (ret >= 0) {
			...
			for (j = 0; j < 20; j++) {
				...
				if ((ret >= 0) &&
				    (FT3681_FACTORY_MODE == mode)) {
					TPD_INFO("enter factory mode success");
					sys_delay(200);
					ft3681_special_operation_for_samsung(
						ts_data);
					return 0;
...
static int short_get_adc_data_mc(u8 retval, int byte_num, int *adc_buf, u8 mode)
{
	...
	for (i = 0; i < FACTORY_TEST_RETRY; i++) {
		...
		ret = ft3681_test_read_reg(FACTROY_REG_SHORT2_TEST_STATE,
					   &short_state);
...
static int short_test_ch_to_all(struct chip_data_ft3681 *ts_data, int *res,
				u8 *ab_ch, bool *result)
{
	int ret = 0;
	int i = 0;
	int tx_num = ts_data->hw_res->TX_NUM;
...
static int ft3681_noise_autotest(struct chip_data_ft3681 *ts_data,
				 bool *test_result)
{
	int ret = 0;
	int i = 0;
	...
	if (!ts_data->ft3681_autotest_offset->ft3681_noise_data_P ||
	...
	ret = ft3681_test_write_reg(FACTORY_REG_FRAME_NUM,
				    (frame_num >> 8) & 0xFF);
	ret = ft3681_test_write_reg(FACTORY_REG_FRAME_NUM + 1,
				    frame_num & 0xFF);
	...
	if (ts_data->ft3681_autotest_offset->ft3681_noise_data_P &&
		...
		for (i = 0; i < node_num; i++) {
			...
			if (ts_data->noise_rawdata[i] >
			    ts_data->ft3681_autotest_offset
				    ->ft3681_noise_data_P[i]) {
				TPD_INFO(
...
static int ft3681_rawdata_autotest(struct chip_data_ft3681 *ts_data,
				   bool *test_result)
{
	int ret = 0;
	int i = 0;
	...
	if (!ts_data->ft3681_autotest_offset->ft3681_raw_data_P ||
	    !ts_data->ft3681_autotest_offset->ft3681_raw_data_N) {
		TPD_INFO("ft3681_raw_data_P || ft3681_raw_data_N is NULL");
		return 0;
	}

	if (!thr || !thr->rawdata_h_min || !thr->rawdata_h_max ||
	    !thr->node_valid || !ts_data->rawdata) {
		FTS_TEST_SAVE_ERR(
			"thr/rawdata_h_min/max/node/rawdata is null\n");
		ret = -EINVAL;
		goto test_err;
	}
	...
	for (i = 0; i < node_num; i++) {
		...
		if ((ts_data->rawdata[i] <
		     ts_data->ft3681_autotest_offset->ft3681_raw_data_N[i]) ||
		    (ts_data->rawdata[i] >
		     ts_data->ft3681_autotest_offset->ft3681_raw_data_P[i])) {
...
static int ft3681_uniformity_autotest(struct chip_data_ft3681 *ts_data,
				      bool *test_result)
{
	int ret = 0;
	int row = 0;
	...
	if (!ts_data->ft3681_autotest_offset->ft3681_uniformity_data_P ||
	    !ts_data->ft3681_autotest_offset->ft3681_uniformity_data_N) {
		TPD_INFO(
			"ft3681_uniformity_data_P || ft3681_uniformity_data_N is NULL");
		return 0;
	}
	...
	if (!thr || !thr->tx_linearity_max || !thr->rx_linearity_max ||
	    !thr->node_valid || !ts_data->rawdata) {
		FTS_TEST_SAVE_ERR(
			"thr/tx/rx_lmax/node_valid/rawdata is null\n");
		ret = -EINVAL;
		goto test_err;
	}
	...
	for (i = 0; i < node_num; i++) {
		...
		if ((rl_tmp[i] < ts_data->ft3681_autotest_offset
					 ->ft3681_uniformity_data_N[i]) ||
		    (rl_tmp[i] > ts_data->ft3681_autotest_offset
					 ->ft3681_uniformity_data_P[i])) {
			TPD_INFO(
...
static int ft3681_scap_cb_autotest(struct chip_data_ft3681 *ts_data,
				   bool *test_result)
{
	int ret = 0;
	...
	if (!ts_data->ft3681_autotest_offset->ft3681_scap_cb_data_P ||
		...
		TPD_INFO(
			"ft3681_scap_cb_data_P || ft3681_scap_cb_data_N || ft3681_scap_cb_data_waterproof_N || ft3681_scap_cb_data_waterproof_P is NULL");
		return 0;
	}

	if (!thr || !thr->node_valid_sc || !thr->scap_cb_on_min ||
	    !thr->scap_cb_on_max || !thr->scap_cb_off_min ||
	    !thr->scap_cb_off_max) {
		FTS_TEST_SAVE_ERR(
			"thr/node_valid_sc/scap_cb_on/off_min/max is null\n");
		ret = -EINVAL;
		goto test_err;
	}
	ret = enter_factory_mode(ts_data);
	...
	if (fw_wp_check) {
		...
		ret = ft3681_test_write_reg(FACTORY_REG_MC_SC_MODE,
					    WATER_PROOF_ON);
		...
		ret = ft3681_test_read_reg(FACTROY_REG_SCAP_GCB_RX,
					   (u8 *)&ts_data->scap_gcb_rx);
		if (ret) {
			FTS_TEST_SAVE_ERR("read GCB_RX fail,ret=%d\n", ret);
			goto restore_reg;
		}

		ret = ft3681_test_read_reg(FACTROY_REG_SCAP_GCB_TX,
					   (u8 *)&ts_data->scap_gcb_tx);
		if (ret) {
			FTS_TEST_SAVE_ERR("read GCB_TX fail,ret=%d\n", ret);
			goto restore_reg;
		}
		TPD_INFO("GCB:RX=%d,TX=%d", ts_data->scap_gcb_rx,
			 ts_data->scap_gcb_tx);

		/* compare */
		tx_check = get_fw_wp(wc_sel, WATER_PROOF_ON_TX);
		rx_check = get_fw_wp(wc_sel, WATER_PROOF_ON_RX);
		tmp_result = compare_mc_sc(
			ts_data, tx_check, rx_check, scb_tmp,
			ts_data->ft3681_autotest_offset
				->ft3681_scap_cb_data_waterproof_N,
			ts_data->ft3681_autotest_offset
				->ft3681_scap_cb_data_waterproof_P);
...
static int ft3681_scap_rawdata_autotest(struct chip_data_ft3681 *ts_data,
					bool *test_result)
{
	int ret = 0;
	bool tmp_result = false;
	...
	if (!ts_data->ft3681_autotest_offset->ft3681_scap_raw_data_P ||
		...
		TPD_INFO(
			"ft3681_scap_raw_data_P || ft3681_scap_raw_data_N || ft3681_scap_raw_waterproof_data_N || ft3681_scap_raw_waterproof_data_P is NULL");
		return 0;
	}

	if (!thr || !thr->node_valid_sc || !thr->scap_rawdata_on_min ||
	    !thr->scap_rawdata_on_max || !thr->scap_rawdata_off_min ||
	    !thr->scap_rawdata_off_max) {
		FTS_TEST_SAVE_ERR(
			"thr/node_valid_sc/scap_rawdata_on/off_min/max is null\n");
		ret = -EINVAL;
		goto test_err;
	}
	ret = enter_factory_mode(ts_data);
	...
	if (fw_wp_check) {
		...
		tmp_result = compare_mc_sc(
			...
			ts_data->ft3681_autotest_offset
				->ft3681_scap_raw_waterproof_data_N,
			ts_data->ft3681_autotest_offset
				->ft3681_scap_raw_waterproof_data_P);
...
static int ft3681_short_test(struct chip_data_ft3681 *ts_data,
			     bool *test_result)
{
	int ret = 0;
	int adc[256] = { 0 };
...
static int ft3681_panel_differ_test(struct chip_data_ft3681 *ts_data,
				    bool *test_result)
{
	int ret = 0;
	int i = 0;
	...
	if (!ts_data->ft3681_autotest_offset->ft3681_panel_differ_data_P ||
		...
		TPD_INFO(
			"ft3681_panel_differ_data_P || ft3681_panel_differ_data_N is NULL");
		return 0;
	}
	...
	for (i = 0; i < node_num; i++) {
		...
		if ((ts_data->panel_differ[i] <
		     ts_data->ft3681_autotest_offset
			     ->ft3681_panel_differ_data_N[i]) ||
		    (ts_data->panel_differ[i] >
		     ts_data->ft3681_autotest_offset
			     ->ft3681_panel_differ_data_P[i])) {
			TPD_INFO("raw data ERR [%d]: [%d] > [%d] > [%d] \n", i,
				 ts_data->ft3681_autotest_offset
					 ->ft3681_panel_differ_data_P[i],
				 ts_data->panel_differ[i],
...
static void ft3681_auto_write_result(struct chip_data_ft3681 *ts_data,
				     int failed_count)
{
	//uint8_t  data_buf[64];
	...
	line_num += 2;
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "SCAP CB Test", 9, 2, 1, line_num, 2);
	ksys_write(ts_data->csv_fd, data_buf, buflen);

	line_num += 2;
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "SCAP CB Test", 9, 2, rx_num, line_num, 3);
	ksys_write(ts_data->csv_fd, data_buf, buflen);

	line_num += 2;
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "SCAP CB Test", 9, 2, 1, line_num, 4);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	...
	line_num += 2;
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "Panel Differ Test", 20, tx_num, rx_num, line_num, 1);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	...
	line_num = 11;
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ", "Noise",
			  14, tx_num, rx_num, line_num, 1);
	sys_write(ts_data->csv_fd, data_buf, buflen);
	...
	ft3681_test_save_data("Noise Test", ts_data->noise_rawdata, node_num,
			      rx_num, ts_data->csv_fd);
	ft3681_test_save_data("Null Noise Test", &ts_data->null_noise_value, 1,
			      1, ts_data->csv_fd);
	ft3681_test_save_data("Rawdata Test", ts_data->rawdata, node_num,
			      rx_num, ts_data->csv_fd);
	ft3681_test_save_data("Rawdata Uniformity Test",
			      ts_data->rawdata_linearity, ts_data->rl_cnt,
			      rx_num, ts_data->csv_fd);
	ft3681_test_save_data("SCAP CB Test", ts_data->scap_cb, channel_num,
			      rx_num, ts_data->csv_fd);
	ft3681_test_save_data("SCAP CB Test", &ts_data->scap_gcb_rx, 1, 1,
			      ts_data->csv_fd);
	ft3681_test_save_data("SCAP CB Test", ts_data->scap_cb + channel_num,
			      channel_num, rx_num, ts_data->csv_fd);
	ft3681_test_save_data("SCAP CB Test", &ts_data->scap_gcb_tx, 1, 1,
			      ts_data->csv_fd);
	ft3681_test_save_data("SCAP Rawdata Test", ts_data->scap_rawdata,
			      channel_num, rx_num, ts_data->csv_fd);
	ft3681_test_save_data("SCAP Rawdata Test",
			      ts_data->scap_rawdata + channel_num, channel_num,
			      rx_num, ts_data->csv_fd);
	ft3681_test_save_data("Panel Differ Test", ts_data->panel_differ,
			      node_num, rx_num, ts_data->csv_fd);
...
static void ft3681_autotest_endoperation(struct chip_data_ft3681 *ts_data,
...
static int ft3681_get_threshold_from_img(struct chip_data_ft3681 *ts_data,
					 char *data,
					 const struct firmware *limit_fw)
{
	int ret = 0;
	...
	ts_data->ft3681_autotest_offset =
		kzalloc(sizeof(struct ft3681_autotest_offset), GFP_KERNEL);

	ret = request_real_test_limit(ts, &limit_fw,
		...
		if (item_head->item_limit_type == LIMIT_TYPE_NO_DATA) {
			...
			if (false == ts_data->use_panelfactory_limit) {
				...
				if (item_head->item_bit == TYPE_NOISE_DATA) {
					...
					ts_data->ft3681_autotest_offset
						...
						(int32_t *)(limit_fw->data +
							    item_head
								    ->floor_limit_offset);
					TPD_INFO(
						"item_head->top_limit_offset = %d, ft3681_noise_data_N = %d \n",
					...
					ts_data->ft3681_autotest_offset
						...
						(int32_t *)(limit_fw->data +
							    item_head
								    ->floor_limit_offset);
					TPD_INFO(
						"ft3681_raw_data_P = %p, ft3681_raw_data_N = %p \n",
						ts_data->ft3681_autotest_offset
							->ft3681_raw_data_P,
						ts_data->ft3681_autotest_offset
							->ft3681_raw_data_N);
					...
					ts_data->ft3681_autotest_offset
						->ft3681_uniformity_data_P =
						(int32_t *)(limit_fw->data +
							    item_head
								    ->top_limit_offset);
					ts_data->ft3681_autotest_offset
						->ft3681_uniformity_data_N =
						(int32_t *)(limit_fw->data +
							    item_head
								    ->floor_limit_offset);
					TPD_INFO(
						"ft3681_uniformity_data_P = %p, ft3681_uniformity_data_N = %p \n",
						ts_data->ft3681_autotest_offset
							->ft3681_uniformity_data_P,
						ts_data->ft3681_autotest_offset
							->ft3681_uniformity_data_N);
					...
					ts_data->ft3681_autotest_offset
						->ft3681_panel_differ_data_P =
						(int32_t *)(limit_fw->data +
							    item_head
								    ->top_limit_offset);
					ts_data->ft3681_autotest_offset
						->ft3681_panel_differ_data_N =
						(int32_t *)(limit_fw->data +
							    item_head
								    ->floor_limit_offset);
					TPD_INFO(
						"ft3681_panel_differ_data_P = %p, ft3681_panel_differ_data_N = %p \n",
						ts_data->ft3681_autotest_offset
							->ft3681_panel_differ_data_P,
						ts_data->ft3681_autotest_offset
							->ft3681_panel_differ_data_N);
					...
					ts_data->ft3681_autotest_offset
						...
						(int32_t *)(limit_fw->data +
							    item_head
								    ->floor_limit_offset);
					TPD_INFO(
						"ft3681_noise_data_P = %p, ft3681_noise_data_N = %p \n",
						ts_data->ft3681_autotest_offset
							->ft3681_noise_data_P,
						ts_data->ft3681_autotest_offset
							->ft3681_noise_data_P);
			...
			if (false == ts_data->use_panelfactory_limit) {
				...
				if (item_head->item_bit == TYPE_SCAP_CB_DATA) {
					ts_data->ft3681_autotest_offset
						->ft3681_scap_cb_data_P =
						(int32_t *)(limit_fw->data +
							    item_head
								    ->top_limit_offset);
					ts_data->ft3681_autotest_offset
						->ft3681_scap_cb_data_N =
						(int32_t *)(limit_fw->data +
							    item_head
								    ->floor_limit_offset);
					TPD_INFO(
						"ft3681_scap_cb_data_P = %p, ft3681_scap_cb_data_P = %p \n",
						ts_data->ft3681_autotest_offset
							->ft3681_scap_cb_data_P,
						ts_data->ft3681_autotest_offset
							->ft3681_scap_cb_data_N);
					   ...
					   TYPE_SCAP_RAW_DATA) {
					ts_data->ft3681_autotest_offset
						->ft3681_scap_raw_data_P =
						(int32_t *)(limit_fw->data +
							    item_head
								    ->top_limit_offset);
					ts_data->ft3681_autotest_offset
						->ft3681_scap_raw_data_N =
						(int32_t *)(limit_fw->data +
							    item_head
								    ->floor_limit_offset);
					TPD_INFO(
						"ft3681_scap_raw_data_P = %p, ft3681_scap_raw_data_N = %p \n",
						ts_data->ft3681_autotest_offset
							->ft3681_scap_raw_data_P,
						ts_data->ft3681_autotest_offset
							->ft3681_scap_raw_data_N);
					...
					ts_data->ft3681_autotest_offset
						...
						(int32_t *)(limit_fw->data +
							    item_head
								    ->floor_limit_offset);
					TPD_INFO(
						"ft3681_scap_cb_data_waterproof_P = %p, ft3681_scap_cb_data_waterproof_N = %p \n",
						ts_data->ft3681_autotest_offset
							->ft3681_scap_cb_data_waterproof_P,
						ts_data->ft3681_autotest_offset
							->ft3681_scap_cb_data_waterproof_N);
					...
					ts_data->ft3681_autotest_offset
						...
						(int32_t *)(limit_fw->data +
							    item_head
								    ->floor_limit_offset);
					TPD_INFO(
						"ft3681_scap_raw_waterproof_data_P = %p, ft3681_scap_raw_waterproof_data_N = %p \n",
						ts_data->ft3681_autotest_offset
							->ft3681_scap_raw_waterproof_data_P,
						ts_data->ft3681_autotest_offset
							->ft3681_scap_raw_waterproof_data_N);
...
static void ft3681_print_threshold(struct chip_data_ft3681 *ts_data)
{
	...
	ft3681_print_buffer(
		ts_data->ft3681_autotest_offset->ft3681_noise_data_P, node_num,
		rx_num);
	ft3681_print_buffer(
		ts_data->ft3681_autotest_offset->ft3681_noise_data_N, node_num,
		rx_num);

	TPD_INFO("rawdata threshold max/min:");
	ft3681_print_buffer(ts_data->ft3681_autotest_offset->ft3681_raw_data_P,
			    node_num, rx_num);
	ft3681_print_buffer(ts_data->ft3681_autotest_offset->ft3681_raw_data_N,
			    node_num, rx_num);
	...
	ft3681_print_buffer(ts_data->ft3681_autotest_offset
				    ->ft3681_scap_cb_data_waterproof_P,
			    channel_num, channel_num);
	ft3681_print_buffer(ts_data->ft3681_autotest_offset
				    ->ft3681_scap_cb_data_waterproof_N,
