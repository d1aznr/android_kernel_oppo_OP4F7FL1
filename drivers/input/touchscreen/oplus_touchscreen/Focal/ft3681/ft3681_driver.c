...
static int ft3681_get_gesture_info(void *chip_data,
...
static int ft3681_spi_transfer_direct(u8 *writebuf, u32 writelen, u8 *readbuf,
				      u32 readlen)
{
	int ret = 0;
	struct chip_data_ft3681 *ts_data = g_ft3681_data;
	...
	ret = ft3681_spi_transfer(ts_data->ft_spi, txbuf, rxbuf, txlen);
	if (ret < 0) {
		TPD_INFO("data read(addr:%x) fail,status:%x,ret:%d", txbuf[0],
			 rxbuf[3], ret);
		goto err_spi_dir;
	}
					    ...
					    int retries_delay)
{
	...
	TPD_INFO("flash status fail,ok:%04x read:%04x, retries:%d",
		 flash_status, read_status, i);
...
static int ft3681_pram_write_buf(u8 *buf, u32 len)
{
	...
	for (i = 0; i < packet_number; i++) {
		...
		ret = ft3681_write(packet_buf,
				   FT3681_ROMBOOT_CMD_SET_PRAM_ADDR_LEN);
...
static int ft3681_fwupg_enter_into_boot(struct chip_data_ft3681 *ts_data)
{
	...
	do {
		...
		if ((id[0] == FT3681_VAL_BT_ID) &&
		    (id[1] == FT3681_VAL_BT_ID2)) {
			break;
		}
...
static int ft3681_fwupg_erase(struct chip_data_ft3681 *ts_data, u32 delay)
{
	...
	flag = ft3681_fwupg_check_flash_status(ts_data,
					       FT3681_CMD_FLASH_STATUS_ERASE_OK,
					       FT3681_RETRIES_REASE,
					       FT3681_RETRIES_DELAY_REASE);
...
int ft3681_fwupg_ecc_cal_tp(struct chip_data_ft3681 *ts_data, u32 saddr,
			    u32 len)
{
	int ret = 0;
	u8 wbuf[7] = { 0 };
	...
	bflag = ft3681_fwupg_check_flash_status(ts_data,
						FT3681_CMD_FLASH_STATUS_ECC_OK,
...
void ft3681_auto_test(struct seq_file *s, void *chip_data,
			      ...
			      bool is_diff)
{
	...
	ret = ft3681_enter_factory_work_mode(ts_data,
					     FT3681_FACTORY_MODE_VALUE);
...
static int ft3681_enable_edge_limit(struct chip_data_ft3681 *ts_data,
				    int enable)
{
	u8 edge_mode = 0;
...
static int ft3681_enable_charge_mode(struct chip_data_ft3681 *ts_data,
				     int enable)
{
...
static fw_update_state ft3681_fw_update(void *chip_data,
				   ...
				   int max_num)
{
	...
	if (finger_num > max_num) {
		TPD_INFO("invalid point_num(%d),max_num(%d)", finger_num,
			 max_num);
		return -EIO;
	}
	...
	for (i = 0; i < max_num; i++) {
		...
		if (!ts_data->high_resolution_support) {
			points[pointid].x =
				((touch_buf[2 + base_position] & 0x0F) << 8) +
				(touch_buf[3 + base_position] & 0xFF);
			points[pointid].y =
				((touch_buf[4 + base_position] & 0x0F) << 8) +
				(touch_buf[5 + base_position] & 0xFF);

			points[pointid].touch_major =
				touch_buf[7 + base_position];
			points[pointid].width_major =
				touch_buf[7 + base_position];
			points[pointid].z = touch_buf[7 + base_position];

			if (ft3681_grip_v2_support) {
				points[pointid].tx_press =
					touch_buf[62 + base_prevent];
				points[pointid].rx_press =
					touch_buf[63 + base_prevent];
				points[pointid].tx_er =
					touch_buf[65 + base_prevent];
				points[pointid].rx_er =
					touch_buf[64 + base_prevent];
			}

			TPD_DEBUG(
				"[prevent-ft] x:%3d y:%3d | tx_press:%3d rx_press:%3d tx_er:%3d rx_er:%3d",
				points[pointid].x, points[pointid].y,
				points[pointid].tx_press,
				points[pointid].rx_press, points[pointid].tx_er,
				points[pointid].rx_er);
		...
		if (snr->doing) {
			...
			GET_LEN_BY_WIDTH_MAJOR(snr->width_major,
					       &snr->area_len);
			TPD_INFO("snr%d: [%d %d, %d] {%d %d} len %d \n",
				 pointid, snr->x, snr->y, snr->width_major,
				 snr->channel_x, snr->channel_y, snr->area_len);
		}
	...
	if (gesture == NULL) {
		TPD_INFO("gesture == NULL, return\n\
			...
			gesture->Point_4th.y = %d\n;",
			 ...
			 ((buf[20] << 8) + buf[21]), ((buf[22] << 8) + buf[23]),
			 ((buf[24] << 8) + buf[25]),
			 ((buf[26] << 8) + buf[27]));
		return ret;
	}
...
static void ft3681_enable_fingerprint_underscreen(void *chip_data,
						  uint32_t enable)
{
	int ret = 0;
	u8 val = 0;
	...
	if (ret < 0) {
		TPD_INFO("%s: write FOD enable(%x=%x) fail", __func__,
			 FT3681_REG_FOD_EN, val);
	}
}

static void
ft3681_screenon_fingerprint_info(void *chip_data,
...
static int ft3681_refresh_switch(void *chip_data, int fps)
{
	...
	return ft3681_write_reg(FT3681_REG_REPORT_RATE,
				(fps == 60 ? FT3681_120HZ_REPORT_RATE :
					     FT3681_180HZ_REPORT_RATE));
}
...
static void ft3681_enable_gesture_mask(void *chip_data, uint32_t enable)
{
	...
	if (ret < 0) {
		TPD_INFO(
			"%s: write FT3681_REG_GESTURE_CONFIG1 enable(%x=%x) fail",
			__func__, FT3681_REG_GESTURE_CONFIG1, config1);
	}
	...
	if (ret < 0) {
		TPD_INFO(
			"%s: write FT3681_REG_GESTURE_CONFIG2 enable(%x=%x) fail",
			__func__, FT3681_REG_GESTURE_CONFIG2, config2);
	}
	...
	if (ret < 0) {
		TPD_INFO(
			"%s: write FT3681_REG_GESTURE_CONFIG4 enable(%x=%x) fail",
			__func__, FT3681_REG_GESTURE_CONFIG4, config4);
	}

	msleep(1);
	TPD_INFO(
		"%s, enable[%d] register[FT3681_REG_GESTURE_CONFIG1. FT3681_REG_GESTURE_CONFIG2. FT3681_REG_GESTURE_CONFIG4]",
		__func__, enable);
}
...
static ssize_t ft3681_debug_read(struct file *filp, char __user *buff,
				 size_t count, loff_t *ppos)
{
	...
	case PROC_READ_DATA:
		...
		ret = ft3681_read(proc->cmd, proc->cmd_len, readbuf,
				  num_read_chars);
		if (ret < 0) {
	...
	case PROC_READ_DATA_DIRECT:
		num_read_chars = buflen;
		ret = ft3681_spi_transfer_direct(proc->cmd, proc->cmd_len,
						 readbuf, num_read_chars);
		if (ret < 0) {
	...
	case PROC_GET_DRIVER_INFO:
		if (buflen >= 64) {
			num_read_chars = buflen;
			readbuf[0] = 3;
			snprintf(&readbuf[32], buflen - 32,
				 "Focaltech V3.4 20211214");
		}
		break;
...
static void ft3681_delta_snr_read(struct seq_file *s, void *chip_data,
				  uint32_t count)
{
	int ret = 0;
	struct chip_data_ft3681 *ts_data = (struct chip_data_ft3681 *)chip_data;
	...
	for (i = 0; i < count; i++) {
		...
		if (snr->point_status) {
			if (i) {
				snr->max = raw[rx_num * snr->channel_x +
					       snr->channel_y] > snr->max ?
						   raw[rx_num * snr->channel_x +
						       snr->channel_y] :
						   snr->max;
				snr->min = raw[rx_num * snr->channel_x +
					       snr->channel_y] < snr->min ?
						   raw[rx_num * snr->channel_x +
						       snr->channel_y] :
						   snr->min;
			} else {
			...
			snr->sum +=
				raw[rx_num * snr->channel_x + snr->channel_y];
	...
	TPD_INFO("snr-cover [%d %d] %d %d %d %d\n", snr->channel_x,
		 snr->channel_y, snr->max, snr->min, snr->sum, snr->noise);
...
static ssize_t ft3681_debug_write(struct file *filp, const char __user *buff,
				  size_t count, loff_t *ppos)
{
	u8 *writebuf = NULL;
	u8 tmpbuf[PROC_BUF_SIZE] = { 0 };
	...
	case PROC_READ_DATA:
		...
		if (writelen >= FT3681_MAX_COMMMAND_LENGTH) {
			TPD_INFO("cmd(PROC_READ_DATA) length(%d) fail",
				 writelen);
			goto proc_write_err;
		}
	...
	case PROC_READ_DATA_DIRECT:
		writelen = buflen - 1;
		if (writelen >= FT3681_MAX_COMMMAND_LENGTH) {
			TPD_INFO("cmd(PROC_READ_DATA_DIRECT) length(%d) fail",
				 writelen);
			goto proc_write_err;
		}
		memcpy(proc->cmd, writebuf + 1, writelen);
	...
	case PROC_WRITE_DATA_DIRECT:
		writelen = buflen - 1;
		ret = ft3681_spi_transfer_direct(writebuf + 1, writelen, NULL,
						 0);
		if (ret < 0) {
	...
	case PROC_CONFIGURE:
		...
		TPD_INFO("spi,mode=%d,bits=%d,speed=%d", ts_data->ft_spi->mode,
			 ts_data->ft_spi->bits_per_word,
			 ts_data->ft_spi->max_speed_hz);
...
static int ft3681_create_apk_debug_channel(struct chip_data_ft3681 *ts_data)
{
	...
	proc->proc_entry = proc_create_data("ftxxxx-debug", 0777, NULL,
					    &ft3681_proc_fops, ts_data);
...
static void ft3681_start_aging_test(void *chip_data)
{
	...
	if (ret < 0) {
		TPD_INFO("%s: enable(%x=%x) fail", __func__,
			 FT3681_REG_GAME_MODE_EN, 2);
	}
	ret = ft3681_write_reg(FT3681_REG_POWER_MODE, 0);
	if (ret < 0) {
		TPD_INFO("%s: enable(%x=%x) fail", __func__,
			 FT3681_REG_POWER_MODE, 0);
	}
}
...
static const struct spi_device_id tp_id[] = { { TPD_DEVICE, 0 }, {} };
