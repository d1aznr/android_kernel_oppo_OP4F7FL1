...
static int focal_lcd_cfg_upgrade_use_buf(void *chip_data, u8 *pbt_buf,
					 u32 dw_lenth);
static void focal_esd_check_enable(void *chip_data, bool enable);
static int focal_get_chip_info(void *chip_data);
void __attribute__((weak)) lcd_esd_recovery_mutex(bool enable)
{
	return;
} //lcd esd recovery mutex
...
static int focal_enter_sleep(struct chip_data_ft8006 *chip_info, bool config)
{
	...
	if (config) {
		ret = touch_i2c_write_byte(
			chip_info->client,
			chip_info->reg_info.FTS_REG_POWER_MODE, 0x03);
		if (ret < 0) {
...
{
	struct chip_data_ft8006 *chip_info =
		(struct chip_data_ft8006 *)chip_data;
...
static int focal_enable_black_gesture(struct chip_data_ft8006 *chip_info,
				      bool enable)
{
	int state = -1;
	int ret = -1;
	...
	for (i = 0; i < 5; i++) {
		ret = touch_i2c_read_byte(chip_info->client,
					  chip_info->reg_info.FTS_REG_CHIP_ID);
		if (ret != 0x80) {
			TPD_INFO(
				"%s: read chip id failed,ret:0x%02x, try %d(<4 is ok) times\n",
				__func__, ret, i);
			msleep(10);
		} else {
			break;
	...
	if (enable) {
		for (i = 0; i < 5; i++) {
			...
			touch_i2c_write_byte(
				chip_info->client,
				chip_info->reg_info.FTS_REG_GESTURE_EN, 0x01);
			msleep(1);
			state = touch_i2c_read_byte(
				chip_info->client,
				chip_info->reg_info.FTS_REG_GESTURE_EN);
			if (state == 1)
				break;
		}
	} else {
		for (i = 0; i < 5; i++) {
			touch_i2c_write_byte(
				chip_info->client,
				chip_info->reg_info.FTS_REG_GESTURE_EN, 0x00);
			msleep(1);
			state = touch_i2c_read_byte(
				chip_info->client,
				chip_info->reg_info.FTS_REG_GESTURE_EN);
...
static int focal_enable_edge_limit(struct chip_data_ft8006 *chip_info,
				   bool enable)
{
	int ret = -1;
	...
	if (enable) {
		ret = touch_i2c_write_byte(chip_info->client,
					   chip_info->reg_info.FTS_REG_EDG_CTR,
					   0x00);
		ret |= touch_i2c_write_byte(
			chip_info->client,
			chip_info->reg_info.FTS_REG_EDGESCREEN_Y1,
			chip_info->grip_area.ver_edgescreen_y1 / 8);
		ret |= touch_i2c_write_byte(
			chip_info->client,
			chip_info->reg_info.FTS_REG_EDGESCREEN_Y2,
			chip_info->grip_area.ver_edgescreen_y2 / 8);
		ret |= touch_i2c_write_byte(
			chip_info->client,
			chip_info->reg_info.FTS_REG_EDGESCREEN_DIR,
			chip_info->grip_area.edgescreen_switch - 1);
	} else {
		ret = touch_i2c_write_byte(chip_info->client,
					   chip_info->reg_info.FTS_REG_EDG_CTR,
					   0x01);
...
static int focal_enable_charge_mode(struct chip_data_ft8006 *chip_info,
				    bool enable)
{
	int ret = -1;
	...
	if (enable) {
		ret = touch_i2c_write_byte(
			chip_info->client,
			chip_info->reg_info.FTS_REG_CHARGER_MODE_EN, 0x01);
	} else {
		ret = touch_i2c_write_byte(
			chip_info->client,
			chip_info->reg_info.FTS_REG_CHARGER_MODE_EN, 0x00);
	}
...
static fw_check_state focal_fw_check(void *chip_data,
				     ...
				     struct panel_info *panel_data)
{
	...
	if (ret != 0x80) {
		TPD_INFO(
			"Read chip Id failed, need Update the Firmware, ret: %d\n",
			ret);
		return FW_ABNORMAL;
	}

	ret = touch_i2c_read_byte(chip_info->client,
				  chip_info->reg_info.FTS_REG_FW_VER);
	if (ret <= 0) {
		TPD_INFO(
			"Read firmware version failed, need Update the Firmware, ret: %d\n",
			ret);
		return FW_ABNORMAL;
	} else {
...
static fw_update_state focal_fw_update(void *chip_data,
				       const struct firmware *fw, bool force)
{
	...
	if (fw_len < APP_FILE_MIN_SIZE || fw_len > APP_FILE_MAX_SIZE) {
		TPD_INFO(
			"%s: firmware size have some problem, no need update\n",
			__func__);
	...
	PANEL_ID_IN_FLASH = touch_i2c_read_byte(
		chip_info->client, 0xE3); //read out panel id in flash
	TPD_INFO(
		"PANEL_ID_IN_BIN = %02x, PANEL_ID_IN_FLASH= %02x [0x8a-tianma/0xb0-truly/0xc3-boeb3/0xc8-boeb8]\n",
	...
	TP_FIRMWARE_ID = touch_i2c_read_byte(
		chip_info->client, chip_info->reg_info.FTS_REG_FW_VER);
	TPD_INFO(
		"%s, TP_FIRMWARE_ID(in .bin) = %02x, TP_FIRMWARE_ID(in flash) = %02x\n",
	...
	if (!force) {
		if (TP_CURRENT_FIRMWARE_ID == TP_FIRMWARE_ID) {
			...
			ret = focal_upgrade_use_buf(chip_info,
						    fw_data + APP_OFFSET,
						    fw_len - APP_OFFSET);
			if (ret != 0) {
		...
		ret = focal_upgrade_use_buf(chip_info, fw_data + APP_OFFSET,
					    fw_len - APP_OFFSET);
		if (ret != 0) {
			update_state = FW_UPDATE_ERROR;
			TPD_INFO("%s: tp fw update failed!\n", __func__);
	...
	if (i == 3) {
		TPD_INFO(
			"%s: Read chip Id failed(ret:%d), no need Update the Firmware\n",
			__func__, ret);
		update_state = FW_UPDATE_ERROR;
		goto OUT;
	}
	...
	if (0xFF != fw_data[initcode_len] + fw_data[initcode_len + 1]) {
		...
		LCD_FIRMWARE_ID = touch_i2c_read_byte(
			chip_info->client,
			chip_info->reg_info.FTS_REG_INITCODE_VER);
		TPD_INFO(
			"%s: LCD_FIRMWARE_ID(in .bin) = %02x, LCD_FIRMWARE_ID(in flash) = %02x\n",
			__func__, LCD_CURRENT_FIRMWARE_ID, LCD_FIRMWARE_ID);
		if (!force) {
			if (LCD_FIRMWARE_ID ==
			    0xA5) { //0xA5: the lcd initcode is stable
				TPD_INFO(
					"%s : check lcd version is A5, must no update initcode\n",
					__func__);
				goto OUT;
			}
			if ((LCD_CURRENT_FIRMWARE_ID != LCD_FIRMWARE_ID) ||
			    (LCD_FIRMWARE_ID ==
			     0xFF)) { //0xFF: lcd initcode must update
				ret = focal_lcd_cfg_upgrade_use_buf(
					chip_info, fw_data, 4096);
				if (ret != 0) {
					update_state = FW_UPDATE_ERROR;
					TPD_INFO(
						"%s: init code update failed!\n",
						__func__);
				} else {
					TPD_INFO(
						"%s: init code update success!\n",
						__func__);
				}
			} else {
			...
			ret = focal_lcd_cfg_upgrade_use_buf(chip_info, fw_data,
							    4096);
			if (ret != 0) {
				update_state = FW_UPDATE_ERROR;
...
static u8 focal_trigger_reason(void *chip_data, int gesture_enable,
			       int is_suspended)
{
	int state = 0;
	struct chip_data_ft8006 *chip_info =
...
static int focal_get_touch_points(void *chip_data, struct point_info *points,
				  int max_num)
{
	int obj_attention = 0;
	uint8_t pointid = 0;
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.FTS_REG_TOUCH_DATA, 3,
				   buf);
	if (ret < 0) {
	...
	if ((0x0F != point_num) && (0x00 != point_num)) {
		for (i = 0; i < point_num; i++) {
			...
			points[pointid].x = ((buf[i * 6 + 3] & 0x0f) << 8) |
					    (buf[i * 6 + 4] & 0xff);
			points[pointid].y = ((buf[i * 6 + 5] & 0x0f) << 8) |
					    (buf[i * 6 + 6] & 0xff);
			points[pointid].z = buf[i * 6 + 7];
			points[pointid].width_major = buf[i * 6 + 8] >> 4;
			points[pointid].status = 1;
			point_state =
				buf[i * 6 + 3] >> 6; /*0&2 is down, 1 is up*/
...
static int focal_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	focal_esd_check_enable(chip_info, false);
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.FTS_REG_GESTURE_DATA, 8,
				   &(buf[0]));
	if (ret < 0) {
	...
	point_num = buf[1];
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.FTS_REG_GESTURE_DATA,
				   4 * point_num + 2, &(buf[0]));
	focal_esd_check_enable(chip_info, true);
	if (ret < 0) {
	...
	case GESTURE_LEFT:
		...
		gesture->Point_end.x = (buf[4 * (point_num - 1) + 2] & 0x0F)
					       << 8 |
				       (buf[4 * (point_num - 1) + 3] & 0xFF);
		gesture->Point_end.y = (buf[4 * (point_num - 1) + 4] & 0x0F)
					       << 8 |
				       (buf[4 * (point_num - 1) + 5] & 0xFF);
		break;
	case GESTURE_RIGHT:
...
static int focal_esd_handle(void *chip_data)
{
	...
	ret = touch_i2c_read_byte(chip_info->client,
				  chip_info->reg_info.FTS_REG_FLOW_WORK_CNT);
	if (ret < 0) {
...
static fp_touch_state focal_spurious_fp_check(void *chip_data)
{
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.FTS_REG_BASELINE_DATA,
...
static void focal_finger_proctect_data_get(void *chip_data)
{
	...
	chip_info->spuri_fp_data = kzalloc(
		TX_NUM * SPURIOUS_FP_RX_NUM * (sizeof(int16_t)), GFP_KERNEL);
	if (!chip_info->spuri_fp_data) {
		TPD_INFO("chip_info->spuri_fp_data kzalloc error\n");
		kfree(raw_data);
		ret = -ENOMEM;
...
static void focal_register_info_read(void *chip_data, uint16_t register_addr,
...
int focal_start_fw_upgrade(void *chip_data)
{
	...
	i_ret = touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.FTS_RST_CMD_REG, 0xAA);
	msleep(10);
	i_ret = touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.FTS_RST_CMD_REG, 0x55);
	msleep(200);
...
int focal_erase_flash(void *chip_data)
{
	...
	for (i = 0; i < 15; i++) {
		...
		touch_i2c_read(chip_info->client, auc_i2c_write_buf, 1, reg_val,
			       2);

		if (0xF0 == reg_val[0] &&
		    0xAA == reg_val[1]) { /*erase flash success*/
			break;
		}
		msleep(50);
	}
	...
	if ((0xF0 != reg_val[0] || 0xAA != reg_val[1]) && (i >= 15)) {
		TPD_INFO(
			"[UPGRADE]: erase app error.need reset tp and reload FW!!\n");
		return -EIO;
	}
...
static int focal_read_3gamma(struct i2c_client *client, u8 **gamma, u16 *len)
{
	...
	for (i = 0; i < packet_num; i++) {
		...
		ret = touch_i2c_write(client, cmd, 4);
		msleep(10);
		ret = touch_i2c_read(client, NULL, 0,
				     pgamma + i * FTS_PACKET_LENGTH,
				     packet_len);
		if (ret < 0) {
...
find_gamma_bank_err:
	TPD_INFO("3-gamma bank(%02x %02x) not find", gamma[gamma_pos],
		 gamma[gamma_pos + 1]);
	return -ENODATA;
}
...
static int focal_read_replace_3gamma(struct i2c_client *client, u8 *buf)
{
	...
	ret = focal_read_3gamma(
		client, &gamma,
		&gamma_len); //read out 3gamma data from ic backup
	if (ret < 0) {
		TPD_INFO("no valid 3-gamma data, not replace\n");
		goto OUT;
	}

	ret = focal_replace_3gamma(
		buf, gamma,
		gamma_len); //replace initcode in fw with reading out 3gamma
	if (ret < 0) {
...
static int focal_upgrade_use_buf(void *chip_data, u8 *pbt_buf, u32 dw_lenth)
{
	...
	TPD_INFO(
		"[UPGRADE]**********send 0xAA and 0x55 to FW, start upgrade**********\n");
	...
	TPD_INFO(
		"[UPGRADE]**********check if run in bootloader mode**********\n");
	...
	for (j = 0; j < packet_number; j++) {
		...
		touch_i2c_write(chip_info->client, packet_buf,
				FTS_PACKET_LENGTH + 6);
		//msleep(1);
		...
		for (i = 0; i < 30; i++) {
			auc_i2c_write_buf[0] = 0x6a;
			reg_val[0] = reg_val[1] = 0x00;
			touch_i2c_read(chip_info->client, auc_i2c_write_buf, 1,
				       reg_val, 2);
			...
			if (i > 15) {
				msleep(1);
				TPD_INFO(
					"[UPGRADE]: write flash: host : %x status : %x!!\n",
					(j + 0x1000 +
	...
	if ((dw_lenth) % FTS_PACKET_LENGTH > 0) {
		...
		for (i = 0; i < 30; i++) {
			...
			if ((0x1000 +
			     ((0x5000 + packet_number * FTS_PACKET_LENGTH) /
					 ...
					 u32 dw_lenth)
{
	...
	TPD_INFO(
		"[UPGRADE]**********erase app and panel paramenter area**********\n");
	...
	TPD_INFO(
		"[UPGRADE]: erase app area reg_val[0] = %x reg_val[1] = %x!!\n",
		reg_val[0], reg_val[1]);
...
static int focal_get_fw_version(void *chip_data)
{
	struct chip_data_ft8006 *chip_info =
		(struct chip_data_ft8006 *)chip_data;
	return touch_i2c_read_byte(chip_info->client,
				   chip_info->reg_info.FTS_REG_FW_VER);
}
...
static int focal_dump_reg_state(void *chip_data, char *buf)
{
	...
	regvalue = touch_i2c_read_byte(chip_info->client,
				       chip_info->reg_info.FTS_REG_POWER_MODE);
	...
	//Vendor ID
	regvalue = touch_i2c_read_byte(chip_info->client,
				       chip_info->reg_info.FTS_REG_VENDOR_ID);
	...
	// 3 charge in
	regvalue = touch_i2c_read_byte(
		chip_info->client, chip_info->reg_info.FTS_REG_CHARGER_MODE_EN);
	count += sprintf(buf + count, "charge stat:0x%02x\n", regvalue);

	//Interrupt counter
	regvalue = touch_i2c_read_byte(chip_info->client,
				       chip_info->reg_info.FTS_REG_INT_CNT);
	count += sprintf(buf + count, "INT count:0x%02x\n", regvalue);

	//Flow work counter
	regvalue = touch_i2c_read_byte(
		chip_info->client, chip_info->reg_info.FTS_REG_FLOW_WORK_CNT);
...
static void focal_delta_read(struct seq_file *s, void *chip_data)
{
	...
	while (--retry) {
		ret = touch_i2c_read_byte(
			chip_info->client,
			chip_info->reg_info.FTS_REG_RAWDIFF_CONTR);
		if (!ret) {
			break;
		}
		msleep(1);
	...
	msleep(10);
	ret = touch_i2c_read_block(
		chip_info->client, chip_info->reg_info.FTS_REG_DIFF_DATA,
...
static void focal_baseline_read(struct seq_file *s, void *chip_data)
{
	...
	msleep(10);
	ret = touch_i2c_read_block(
		chip_info->client, chip_info->reg_info.FTS_REG_RAW_DATA,
		chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM * 2,
		baseline_data); //read data
	for (x = 0; x < chip_info->hw_res->TX_NUM; x++) {
		seq_printf(s, "\n[%2d]", x);
		for (y = 0; y < chip_info->hw_res->RX_NUM; y++) {
			z = chip_info->hw_res->RX_NUM * x + y;
			temp_delta = baseline_data[z * 2 + 1] |
				     (baseline_data[z * 2] << 8);
			seq_printf(s, "%4d, ", temp_delta);
		}
	}
...
static void focal_main_register_read(struct seq_file *s, void *chip_data)
{
	...
	//initcode version
	regvalue = touch_i2c_read_byte(
		chip_info->client, chip_info->reg_info.FTS_REG_INITCODE_VER);
	...
	seq_printf(
		s,
		"PANEL ID(0x8a-tianma/0xb0-truly/0xc3-boeb3/0xc8-boeb8):0x%02x\n",
		regvalue);
...
static void focal_cb_read(struct seq_file *s, void *chip_data)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, 0x18,
				   0x00); //Address offset high 8 bit
	ret = touch_i2c_write_byte(chip_info->client, 0x19,
				   0x00); //Address offset low 8 bit
	ret = touch_i2c_read_block(chip_info->client, 0x6E,
...
static int focal_tp_probe(struct i2c_client *client,
			  const struct i2c_device_id *id)
{
	...
	if (ts->esd_handle_support) {
		ts->esd_info.esd_work_time =
			HZ; // change esd check interval to 1s
