...
int fts_pinctrl_select_active(struct fts_ts_data *fts_data)
{
	...
	if (fts_data->hw_res->pinctrl && fts_data->hw_res->pin_set_reset_high) {
		ret = pinctrl_select_state(
			fts_data->hw_res->pinctrl,
			fts_data->hw_res->pin_set_reset_high);
		TPD_INFO("%s : ret = %d\n", __func__, ret);
		if (ret < 0) {
...
int fts_pinctrl_select_suspend(struct fts_ts_data *fts_data)
{
	...
	if (fts_data->hw_res->pinctrl && fts_data->hw_res->pin_set_reset_low) {
		ret = pinctrl_select_state(fts_data->hw_res->pinctrl,
					   fts_data->hw_res->pin_set_reset_low);
		TPD_INFO("%s : ret = %d\n", __func__, ret);
		if (ret < 0) {
...
static int fts_upgrade(struct fts_ts_data *ts_data, u8 *buf, u32 len)
{
	...
	if (ret < 0) {
		tp_healthinfo_report(monitor_data, HEALTH_FW_UPDATE,
				     "Enter pramboot/bootloader failed");
...
static int fts_enable_game_mode(struct fts_ts_data *ts_data, bool enable)
{
	...
	ret = touch_i2c_write_byte(ts_data->client, FTS_REG_GAME_MODE_EN,
				   !enable);
	if (ret < 0) {
...
static void fts_health_report_v2(void *chip_data,
			    ...
			    struct i2c_client *client)
{
	...
	TPD_INFO("%s:high_resolution_support is:%d %d\n", __func__,
		 ts_data->high_resolution_support,
		 ts_data->high_resolution_support_x8);
	ts_data->need_pinctrl_pull_up_reset =
		of_property_read_bool(np, "need_pinctrl_pull_up_reset");
	...
	if (ts_data->need_pinctrl_pull_up_reset) {
		...
		if (IS_ERR_OR_NULL(ts_data->hw_res->pinctrl)) {
			...
			ts_data->hw_res->pin_set_reset_high =
				pinctrl_lookup_state(ts_data->hw_res->pinctrl,
						     "pin_set_reset_high");
			if (IS_ERR_OR_NULL(
				    ts_data->hw_res->pin_set_reset_high)) {
				TPD_INFO(
					"Failed to get reset high state pinctrl handle\n");
				ret = PTR_ERR(
					ts_data->hw_res->pin_set_reset_high);
				goto err_pinctrl_lookup;
			}

			ts_data->hw_res->pin_set_reset_low =
				pinctrl_lookup_state(ts_data->hw_res->pinctrl,
						     "pin_set_reset_low");
			if (IS_ERR_OR_NULL(
				    ts_data->hw_res->pin_set_reset_low)) {
				TPD_INFO(
					" Failed to get reset low state pinctrl handle\n");
				ret = PTR_ERR(
					ts_data->hw_res->pin_set_reset_low);
				goto err_pinctrl_lookup;
			}
