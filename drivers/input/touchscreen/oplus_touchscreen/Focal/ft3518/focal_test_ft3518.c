...
static bool short_test_get_channel_num(struct fts_ts_data *ts_data,
				       int *fm_short_resistance, int *adc_data,
				       int offset, bool *is_weak_short_mut)
{
	int ret = 0;
	int i;
	int count = 0;
	...
	for (i = 0; i < 1; i++) {
		ret = short_get_adc_data_mc(TEST_RETVAL_00,
					    all_adc_data_num * 2, adc_data,
					    FACTROY_REG_SHORT_CA);
		sys_delay(50);
		if (ret) {
			tmp_result = false;
			FTS_TEST_SAVE_ERR(
				"failed to get weak short data,ret= %d\n", ret);
			goto TEST_END;
		}
	}
	...
	for (i = 0; i < total_num; i++) {
		...
		fm_short_resistance[i] = (adc_data[i] - offset + 395) *
						 res_stalls /
						 (code_1 - adc_data[i]) -
					 3;
...
static bool short_test_channel_to_gnd(struct fts_ts_data *ts_data,
				      int *fm_short_resistance, int *adc_data,
				      int offset, bool *is_weak_short_gnd)
{
	int ret = 0;
	int error_num = 0;
	int min_70k_num = 0;
	...
	if (error_num > 0) {
		...
		ret = fts_test_write(FACTROY_REG_SHORT_AB_CH, w_buf,
				     error_num + 1);
		for (i = 0; i < 1; i++) {
			ret = short_get_adc_data_mc(TEST_RETVAL_00,
						    error_num * 2, tmp_adc_data,
	...
	if (min_70k_num > 0) {
		...
		for (i = 0; i < min_70k_num; i++) {
			...
			fvalue = (tmp_adc_data[i] - offset + 395) *
					 res_stalls0 /
					 (code_0 - tmp_adc_data[i]) -
				 3;
...
static bool short_test_channel_to_channel(struct fts_ts_data *ts_data,
					  int *fm_short_resistance,
					  int *adc_data, int offset)
{
	int ret = 0;
	int error_num = 0;
	int min_70k_num = 0;
	...
	if (error_num > 1) {
		...
		ret = fts_test_write(FACTROY_REG_SHORT_AB_CH, w_buf,
				     error_num + 1);
		for (j = 0; j < 1; j++) {
			ret = short_get_adc_data_mc(
				TEST_RETVAL_00,
				error_num * (error_num - 1) * 2 / 2,
		...
		for (i = 0; i < error_num; i++) {
			for (j = i + 1; j < error_num; j++) {
				...
				fvalue = (tmp_adc_data[adc_count] - offset +
					  395) * res_stalls /
						 (code_1 -
				...
				if (fvalue < 0)
					fvalue = abs(fvalue);
				if (min_cc > fvalue) {
					fm_short_resistance[error_ch[i] - 1] =
						fvalue;
					fm_short_resistance[error_ch[j] - 1] =
						fvalue;
					adc_data[error_ch[i] - 1] =
						tmp_adc_data[adc_count];
					adc_data[error_ch[j] - 1] =
						tmp_adc_data[adc_count];
		...
		for (i = 0; i < min_70k_num; i++) {
			for (j = i + 1; j < min_70k_num; j++) {
				...
				if (fvalue < 0)
					fvalue = abs(fvalue);
				if (min_cc > fvalue) {
					fm_short_resistance[min_70k_ch[i] - 1] =
						fvalue;
					fm_short_resistance[min_70k_ch[j] - 1] =
						fvalue;
					adc_data[min_70k_ch[i] - 1] =
						tmp_adc_data[adc_count];
					adc_data[min_70k_ch[j] - 1] =
						tmp_adc_data[adc_count];
					if (min_70k_ch[i] <= tx_num) {
						FTS_TEST_SAVE_INFO(
							"Tx%d ",
...
static int fts_noise_autotest(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	for (i = 0; i < 3; i++) {
		...
		ret = read_rawdata(FACTORY_REG_LINE_ADDR, 0xAA, rawdata_addr,
				   byte_num, ts_data->noise_rawdata);
		if (ret < 0) {
			FTS_TEST_SAVE_ERR("read rawdata fail\n");
		}
	}
...
static int fts_short_test(struct fts_ts_data *ts_data, bool *test_result)
{
	...
	tmp_result = short_test_get_channel_num(ts_data, fm_short_resistance,
						adc_data, offset,
						&is_weak_short_mut);
	...
	if (is_weak_short_mut) {
		tmp_result =
			short_test_channel_to_gnd(ts_data, fm_short_resistance,
						  adc_data, offset,
						  &is_weak_short_gnd);
	}
	...
	if (is_weak_short_mut) {
		tmp_result &= short_test_channel_to_channel(
			ts_data, fm_short_resistance, adc_data, offset);
	}
...
static void fts_auto_write_result(struct fts_ts_data *ts_data, int failed_count)
{
	...
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "Noise Test", 14, tx_num, rx_num, 11, 1);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "Rawdata Test", 7, tx_num, rx_num, 11 + tx_num, 2);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
	...
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "SCAP CB Test", 9, 2, rx_num, 11 + tx_num * 4, 1);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "SCAP CB Test", 9, 2, rx_num, 11 + tx_num * 4 + 2, 2);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "SCAP Rawdata Test", 10, 2, rx_num,
			  11 + tx_num * 4 + 4, 1);
	ksys_write(ts_data->csv_fd, data_buf, buflen);
	buflen = snprintf(data_buf, 256, "%s, %d, %d, %d, %d, %d, ",
			  "SCAP Rawdata Test", 10, 2, rx_num,
