...
static int zeroflash_check_uboot(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to read PDT entry\n");
		return retval;
	}
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to read F$35 query\n");
		return retval;
	}
...
static int zeroflash_download_disp_config(void)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Downloading display config\n");
	...
	retval = tcm_hcd->write_message(
		tcm_hcd, CMD_DOWNLOAD_CONFIG, zeroflash_hcd->out.buf,
		zeroflash_hcd->out.data_length, &zeroflash_hcd->resp.buf,
		&zeroflash_hcd->resp.buf_size, &zeroflash_hcd->resp.data_length,
		&response_code, 0);
	if (retval < 0) {
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Display config downloaded\n");
...
static int zeroflash_download_app_config(void)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Downloading application config\n");
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Application config downloaded\n");
...
static void zeroflash_download_config_work(struct work_struct *work)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Start of config download\n");
...
static int zeroflash_download_app_fw(void)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Downloading application firmware\n");
	...
	retval = syna_tcm_rmi_write(tcm_hcd,
				    zeroflash_hcd->f35_addr.control_base +
					    F35_CTRL3_OFFSET,
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Application firmware downloaded\n");
...
static void zeroflash_download_firmware_work(struct work_struct *work)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to read F$35 data\n");
		goto exit;
	}
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Start of firmware download\n");
	...
	LOGN(tcm_hcd->pdev->dev.parent, "End of firmware download\n");
...
static int zeroflash_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	INIT_WORK(&zeroflash_hcd->config_work, zeroflash_download_config_work);
