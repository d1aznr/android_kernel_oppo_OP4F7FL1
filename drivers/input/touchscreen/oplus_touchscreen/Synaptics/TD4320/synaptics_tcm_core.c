...
retry:
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to read from device\n");
		UNLOCK_BUFFER(tcm_hcd->in);
		if (retry) {
	...
	LOGD(tcm_hcd->pdev->dev.parent, "Header code = 0x%02x\n",
				  ...
				  unsigned int polling_delay_ms)
{
	...
	LOGE(tcm_hcd->pdev->dev.parent, "Command = 0x%02x\n", command);
...
static int syna_tcm_config_gpio(struct syna_tcm_hcd *tcm_hcd)
{
	...
	syna_tcm_set_gpio(tcm_hcd, bdata->display_reset_gpio, true, 1, 1);
	syna_tcm_set_gpio(tcm_hcd, bdata->display_reset_gpio, false, 0, 0);
...
static int syna_tcm_get_boot_info(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to copy boot info\n");
		goto exit;
	}
...
static int syna_tcm_run_bootloader_firmware(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval = tcm_hcd->write_message(tcm_hcd, CMD_RUN_BOOTLOADER_FIRMWARE,
					NULL, 0, &resp_buf, &resp_buf_size,
					&resp_length, NULL,
					MODE_SWITCH_DELAY_MS);
	if (retval < 0) {
...
static int syna_tcm_resume(struct device *dev)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to exit deep sleep\n");
		goto exit;
	}

	retval = syna_tcm_rezero(tcm_hcd);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to rezero\n");
		goto exit;
	}
...
static int syna_tcm_early_suspend(struct device *dev)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to enter deep sleep\n");
		return retval;
	}
#endif
...
static int syna_tcm_probe(struct platform_device *pdev)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to get regulators\n");
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to configure GPIO's\n");
