					     ...
					     const char *buf, size_t count)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to do recovery\n");
		goto exit;
	}
					 ...
					 char *buf, loff_t pos, size_t count)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to copy ihex data\n");
...
static int recovery_parse_ihex(void)
{
	...
	retval = recovery_add_padding(&words);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to add padding\n");
		return retval;
	}
...
static int recovery_check_status(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to read status\n");
		return retval;
	}
...
static int recovery_do_recovery(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to parse ihex data\n");
		return retval;
	}
