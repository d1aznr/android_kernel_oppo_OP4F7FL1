									...
									testing_sysfs_show(
										...
										testing_sysfs_show(
											...
											testing_sysfs_show(
												...
												static ssize_t
	testing_sysfs_trx_trx_shorts_show(struct device *dev,
...
static ssize_t
testing_sysfs_trx_sensor_opens_show(struct device *dev,
...
static ssize_t
testing_sysfs_trx_ground_shorts_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	int retval;
	struct syna_tcm_hcd *tcm_hcd = testing_hcd->tcm_hcd;
				       ...
				       char *buf, loff_t pos, size_t count)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to copy report data\n");
	} else {
		retval = readlen;
	}
...
static void testing_doze_frame_output(unsigned int rows, unsigned int cols)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to copy test data\n");
		UNLOCK_BUFFER(testing_hcd->resp);
		UNLOCK_BUFFER(testing_hcd->output);
		return;
	}
...
static void testing_standard_frame_output(bool image_only)
{
	...
	retval = secure_memcpy(testing_hcd->output.buf,
			       ...
			       header_size);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to copy header data\n");
		UNLOCK_BUFFER(testing_hcd->output);
		return;
	}
...
static int testing_dynamic_range_doze(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to run test\n");
		goto exit;
	}
...
static int testing_dynamic_range_lpwg(void)
{
	...
	retval = tcm_hcd->set_dynamic_config(tcm_hcd, DC_IN_WAKEUP_GESTURE_MODE,
					     1);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
...
static int testing_noise_lpwg(void)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to do noise test\n");
		return retval;
	}
...
static int testing_open_short_detector(void)
{
	...
	if (data_size * 2 != testing_hcd->resp.data_length) {
		LOGE(tcm_hcd->pdev->dev.parent, "Data size mismatch\n");
		UNLOCK_BUFFER(testing_hcd->resp);
		retval = -EINVAL;
		goto exit;
	}
...
static void testing_lockdown_output(void)
{
	...
	retval = syna_tcm_alloc_mem(tcm_hcd, &testing_hcd->output,
				    testing_hcd->resp.data_length);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
		     "Failed to allocate memory for testing_hcd->output.buf\n");
...
static int testing_lockdown(void)
{
	...
	if (tcm_hcd->read_flash_data == NULL) {
		LOGE(tcm_hcd->pdev->dev.parent, "Unable to read from flash\n");
		return -EINVAL;
	}
