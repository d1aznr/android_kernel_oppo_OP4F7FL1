...
static int parse_dt(struct device *dev, struct syna_tcm_board_data *bdata)
{
	...
	bdata->display_reset_gpio = of_get_named_gpio_flags(
		np, "synaptics,display-reset-gpio", 0, NULL);
				  ...
				  unsigned int length)
{
	...
	if (retval < 0) {
		LOGE(&spi->dev, "Failed to copy write data\n");
		goto exit;
	}
			     ...
			     unsigned int length)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to allocate memory\n");
		goto exit;
	}
...
static int syna_tcm_spi_probe(struct spi_device *spi)
{
	...
	if (retval < 0) {
		LOGE(&spi->dev, "Failed to set up SPI protocol driver\n");
		return retval;
	}
	...
	if (retval < 0) {
		LOGE(&spi->dev, "Failed to add platform device\n");
		return retval;
	}
