...
#define reflash_erase(p_name)                                                  \
	...
	{                                                                      \
		...
		LOGD(tcm_hcd->pdev->dev.parent, "Page start = %d\n",           \
		     page_start);                                              \
                                                                               \
		LOGD(tcm_hcd->pdev->dev.parent, "Page count = %d\n",           \
		     page_count);                                              \
                                                                               \
				      ...
				      char *buf, loff_t pos, size_t count)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to read LCM data\n");
		goto exit;
	}
				      ...
				      char *buf, loff_t pos, size_t count)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to read OEM data\n");
		goto exit;
	}
...
exit:
	if (update_area == NONE) {
		LOGN(tcm_hcd->pdev->dev.parent, "No need to do reflash\n");
	} else {
		LOGN(tcm_hcd->pdev->dev.parent, "Updating %s\n",
		     update_area == FIRMWARE_CONFIG ? "firmware and config" :
						      "config only");
	}
			      ...
			      unsigned int datalen)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to copy read data\n");
		UNLOCK_BUFFER(reflash_hcd->resp);
		return retval;
	}
			     ...
			     struct syna_tcm_buffer *output)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to read from flash\n");
		UNLOCK_BUFFER(reflash_hcd->read);
		goto run_app_firmware;
	}
	...
	reflash_write(disp_config)
		...
		reflash_write(prod_test_firmware)
			...
			reflash_write(app_firmware)
				...
				static int reflash_erase_flash(
					unsigned int page_start,
					unsigned int page_count)
{
	int retval;
	...
	retval = tcm_hcd->write_message(tcm_hcd, CMD_ERASE_FLASH, out_buf,
					sizeof(out_buf), &reflash_hcd->resp.buf,
					&reflash_hcd->resp.buf_size,
	...
	reflash_erase(disp_config)
		...
		reflash_erase(prod_test_firmware)
			...
			reflash_erase(app_firmware)
				...
				static int reflash_update_custom_otp(
					const unsigned char *data,
					unsigned int offset,
					unsigned int datalen)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to write to flash\n");
		goto run_app_firmware;
	}
...
static int reflash_update_boot_config(bool lock)
{
	...
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to read boot config\n");
		goto reset;
	}
