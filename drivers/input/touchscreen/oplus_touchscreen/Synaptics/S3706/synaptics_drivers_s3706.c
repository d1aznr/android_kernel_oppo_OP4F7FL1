				      ...
				      struct point_info *points, int max_num)
{
	...
	for (i = 0; i < fingers_to_process; i++) {
		points[i].x = ((buf[i * 8 + 2] & 0x0f) << 8) |
			      (buf[i * 8 + 1] & 0xff);
		points[i].y = ((buf[i * 8 + 4] & 0x0f) << 8) |
			      (buf[i * 8 + 3] & 0xff);
...
static void synaptics_health_zone_init(struct chip_data_s3706 *chip_info)
{
	...
	if (1 == ret) {
		...
		l_tmp = touch_i2c_read_byte(
			chip_info->client,
			chip_info->reg_info.F51_CUSTOM_QUERY_BASE + 0x05);
		h_tmp = touch_i2c_read_byte(
			chip_info->client,
...
static int synaptics_get_chip_info(void *chip_data)
{
	...
	TPD_INFO("F51_CUSTOM_QUERY_BASE  = 0x%x \n\
		 ...
		 reg_info->F51_CUSTOM_CTRL_BASE,
		 reg_info->F51_CUSTOM_DATA_BASE);
	...
	reg_info->F51_CUSTOM_CTRL31 =
		reg_info->F51_CUSTOM_CTRL_BASE + 11; /*no use*/
	...
	reg_info->F51_CUSTOM_CTRL30 =
		reg_info->F51_CUSTOM_CTRL_BASE + 0x38; /*switch for game mode*/
...
synaptics_fw_check(void *chip_data, struct resolution_info *resolution_info,
		   struct panel_info *panel_data)
{
	...
	bootloader_mode = touch_i2c_read_byte(
		chip_info->client, chip_info->reg_info.F01_RMI_DATA_BASE);
	...
	if (chip_info->report_120hz_support) {
		chip_info->default_lock_point_level =
			synaptics_get_lock_point_level(chip_info);
		TPD_INFO(
			"%s: default_nosie_level = 0x%x, default_lock_point_level = 0x%x .\n",
			__func__, chip_info->default_nosie_level,
			chip_info->default_lock_point_level);
	}
	...
	if (config) { //configed  && out of sleep mode
		...
		if (ctrl_buf[2] & 0x02) {
			...
			if (retval != 0) {
				touch_i2c_write_byte(
					chip_info->client,
					chip_info->reg_info.F51_CUSTOM_CTRL00 +
						3,
					0x00);
				touch_i2c_write_byte(chip_info->client, 0xff,
						     0x01); /* page 1*/
				touch_i2c_write_byte(
					chip_info->client,
					chip_info->reg_info
					  ...
					  bool enable)
{
	...
	wakeup_gesture_enable_buf =
		0xef; /* all kinds of gesture except triangle*/
					 ...
					 bool enable)
{
	...
	pre_set_1 = touch_i2c_read_byte(
		chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL04_05);
	pre_set_2 = touch_i2c_read_byte(
		chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL04_06);
	pre_set_3 = touch_i2c_read_byte(
		chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL04_08);
				       ...
				       bool enable)
{
	...
	if (chip_info->report_120hz_support) {
		...
		if (ret < 0) {
			TPD_INFO(
				"%s:set F51_CUSTOM_CTRL30 for game mode fail\n",
				__func__);
			return;
		}
				      ...
				      struct gesture_info *gesture)
{
	...
	ret = touch_i2c_read_block(
		chip_info->client, chip_info->reg_info.F51_CUSTOM_DATA_BASE, 25,
	...
	if (gesture->gesture_type != UnkownGesture) {
		for (i = 0; i < 23; i += 2) {
			trspoint = coordinate_buf[i] | coordinate_buf[i + 1]
							       << 8;
			TPD_DEBUG(
				   ...
				   uint8_t *data_buf)
{
	...
	ret = touch_i2c_read_block(
		chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 3,
	...
	for (x = 0; x < syna_testdata->TX_NUM; x++) {
		...
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
...
#ifdef CONFIG_ARCH_HAS_SYSCALL_WRAPPER
				ksys_write(syna_testdata->fd, data_buf,
					   strlen(data_buf));
#else
			...
			if ((y <
			     syna_testdata->RX_NUM - syna_testdata->key_RX) &&
			    (x <
			...
			} else if ((x == syna_testdata->TX_NUM -
						 syna_testdata->key_TX) &&
				   (y >= syna_testdata->RX_NUM -
						 syna_testdata->key_RX)) {
				TPD_DEBUG(
	...
	baseline_data_test = (uint16_t *)(syna_testdata->fw->data +
				    ...
				    struct syna_testdata *syna_testdata)
{
	...
	for (x = 0; x < 7; x++) {
		...
		if (buffer[x] != 0) {
			if ((x == 0) &&
			    (buffer[x] &
			     0x01)) { //pin - 0, these four pins should be test in RT100
				     ...
				     uint8_t *raw_data)
{
	...
	p_data_delta = kzalloc(syna_testdata->TX_NUM * syna_testdata->RX_NUM *
				       2 * (sizeof(uint8_t)),
			       GFP_KERNEL);
	if (!p_data_delta) {
		error_count++;
	...
	rx_physical =
		kzalloc(syna_testdata->RX_NUM * (sizeof(uint8_t)), GFP_KERNEL);
	minRX = kzalloc((syna_testdata->RX_NUM + 1) * (sizeof(uint8_t)),
			GFP_KERNEL);
	maxRX = kzalloc((syna_testdata->RX_NUM + 1) * (sizeof(uint8_t)),
			GFP_KERNEL);
	...
	for (i = 0; i < 4; i++) {
		...
		logical_pin = GetLogicalPin(ExtendRT26_pin[i],
					    syna_testdata->RX_NUM, rx_physical);
		...
		for (x = 0; x < syna_testdata->TX_NUM; x++) {
			for (y = 0; y < syna_testdata->RX_NUM; y++) {
				p_data_delta[x * syna_testdata->RX_NUM + y] = ABS(
					p_data_baseline1
						[x * syna_testdata->RX_NUM + y],
					p_data_baseline2
						[x * syna_testdata->RX_NUM + y]);
				...
				if (maxRX[y] <
					...
					maxRX[y] = p_data_delta
						[x * syna_testdata->RX_NUM + y];

				if (minRX[y] >
				    p_data_delta[x * syna_testdata->RX_NUM + y])
					minRX[y] = p_data_delta
						[x * syna_testdata->RX_NUM + y];
			}
		}
				     ...
				     uint8_t *data_buf)
{
	...
	for (y = 0; y < syna_testdata->RX_NUM + syna_testdata->TX_NUM -
				     ...
				     uint8_t *data_buf)
{
	...
	for (y = 0; y < chip_info->hw_res->RX_NUM + chip_info->hw_res->TX_NUM;
		...
		if ((syna_testdata->key_RX != 0 ||
		     syna_testdata->key_TX != 0) &&
		    (((y >
		...
		if (unsigned_baseline_data < 50 ||
		    unsigned_baseline_data > 975) {
			error_count++;
			TPD_INFO(
					...
					uint8_t *data_buf)
{
	...
	ret |= touch_i2c_write_byte(chip_info->client,
				    ...
				    0x9B); /*select report type 0x9B*/
	ret |= touch_i2c_write_word(
		chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE + 1,
		0x00); /*set fifo 00*/
	ret |= touch_i2c_write_byte(chip_info->client,
				    chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
				    0x01); /*get report*/
				    ...
				    uint8_t *data_buf)
{
	...
	const int data_cycle = 20,
		  node_num = syna_testdata->TX_NUM + syna_testdata->RX_NUM,
		  judge_threshold = 300;
				    ...
				    uint8_t *data_buf)
{
	...
	for (i = 0; i < syna_testdata->RX_NUM; i++) {
		node_data = (raw_data[i * 4] | (raw_data[i * 4 + 1] << 8) |
			     (raw_data[i * 4 + 2] << 16) |
			     (raw_data[i * 4 + 3] << 24));
		if (syna_testdata->fd >= 0) {
				...
				struct syna_testdata *syna_testdata)
{
	...
	raw_data = kzalloc(syna_testdata->TX_NUM * syna_testdata->RX_NUM * 2 *
				   (sizeof(uint8_t)),
			   GFP_KERNEL);
	if (!raw_data) {
	...
	ret = synaptics_capacity_test(s, chip_info, syna_testdata, ph, raw_data,
				      data_buf);
	if (ret > 0) {
	...
	if (!syna_testdata->fingerprint_underscreen_support) {
		ret = synaptics_auto_test_rt155(s, chip_info, syna_testdata,
						data_buf);
		if (ret > 0) {
			TPD_INFO(
	...
	if (syna_testdata->fd_support) {
		if (chip_info->rt155_fdreplace_rt59_support) {
			ret = synaptics_auto_test_rt155_fd(
				s, chip_info, syna_testdata, data_buf);
		} else {
			ret = synaptics_auto_test_rt59(s, chip_info,
		...
		ret = synaptics_auto_test_rt63(s, chip_info, syna_testdata,
					       data_buf);
		if (ret > 0) {
	...
	do {
		...
		for (x = 0; x < chip_info->hw_res->TX_NUM; x++) {
			...
			for (y = 0; y < chip_info->hw_res->RX_NUM; y++) {
				...
				baseline_data = (raw_data[z * 2 + 1] << 8) |
						raw_data[z * 2];
...
static void synaptics_get_gesture_coord(void *chip_data, uint32_t gesture_type)
{
	...
	if (loop == 0) {
#ifdef CONFIG_ARCH_HAS_SYSCALL_WRAPPER
		fd = ksys_open(gesture_file_bf, O_WRONLY | O_CREAT | O_TRUNC,
			       0);
#else
...
static int fwu_recovery_check_status(struct chip_data_s3706 *chip_info)
{
	...
	retval = touch_i2c_read_block(chip_info->client,
				      data_base + F35_ERROR_CODE_OFFSET, 1,
				      &status);
	if (retval < 0) {
...
static int fwu_scan_pdt(struct chip_data_s3706 *chip_info)
{
	...
	for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE) {
		retval = touch_i2c_read_block(chip_info->client, addr,
					      sizeof(rmi_fd),
		...
		if (rmi_fd.fn_number) {
			...
			case SYNAPTICS_RMI4_F34: /*found*/
				...
				case F34_V1:
					...
					break;
				case F34_V2:
					chip_info->fwu->bl_version =
						BL_V7; /* why not direct set BL_V8*/
					break;
				default:
					TPD_INFO(
	...
	rmi4_data->intr_mask[0] |=
		chip_info->fwu
			->intr_mask; //Roland interrupts mode for write flash, like checkFlashState in 3508 notice

	addr = rmi4_data->f01_ctrl_base_addr + 1;

	retval = touch_i2c_write_block(chip_info->client, addr,
					 ...
					 unsigned char cmd)
{
	...
	retval = touch_i2c_write_block(
		chip_info->client, data_base + chip_info->fwu->off.partition_id,
...
static int
fwu_write_f34_v7_command_single_transaction(struct chip_data_s3706 *chip_info,
					    unsigned char cmd)
{
	int retval;
	unsigned char data_base;
				    ...
				    unsigned char cmd)
{
	...
	case CMD_ERASE_UTILITY_PARAMETER:
	case CMD_ENABLE_FLASH_PROG:
		retval = fwu_write_f34_v7_command_single_transaction(chip_info,
								     cmd);
		if (retval < 0) {
			return retval;
		} else {
					   ...
					   unsigned char *partition_table)
{
	...
	retval = touch_i2c_write_block(
		chip_info->client, data_base + chip_info->fwu->off.block_number,
	...
	length[0] = (unsigned char)(chip_info->fwu->flash_config_length &
				    MASK_8BIT);
...
static int fwu_read_f34_v7_queries(struct chip_data_s3706 *chip_info)
{
	...
	retval = touch_i2c_read_block(chip_info->client, query_base,
	...
	retval = touch_i2c_read_block(chip_info->client, query_base + offset,
	...
	chip_info->fwu->block_size =
		query_1_7.block_size_15_8 << 8 | query_1_7.block_size_7_0;

	chip_info->fwu->flash_config_length = query_1_7.flash_config_length_15_8
						      << 8 |
	...
	fwu_parse_partition_table(
		chip_info, ptable, &chip_info->fwu->blkcount,
				   ...
				   bool force)
{
	...
	chip_info->fwu->rmi4_data->firmware_id =
		(unsigned int)build_id[0] + (unsigned int)build_id[1] * 0x100 +
...
static void
fwu_parse_image_header_10_bootloader(struct chip_data_s3706 *chip_info,
				     const unsigned char *image)
{
	...
	for (ii = 1; ii <= num_of_containers; ii++) {
		addr = le_to_uint(chip_info->fwu->img.bootloader.data +
				  (ii * 4));
...
static int fwu_parse_image_info(struct chip_data_s3706 *chip_info)
{
	...
	if (chip_info->fwu->bl_version == BL_V7 ||
		...
		fwu_parse_partition_table(
			chip_info, chip_info->fwu->img.fl_config.data,
				     ...
				     unsigned int *fw_id)
{
	...
	if (chip_info->fwu->img.contains_firmware_id) {
		*fw_id = chip_info->fwu->img.firmware_id;
		...
		strptr = strnstr(chip_info->fwu->image_name, "PR",
				 MAX_IMAGE_NAME_LEN);
		if (!strptr) {
			TPD_INFO(
...
exit:
	...
	if (flash_area == NONE) {
		...
		TPD_INFO("%s: Updating %s\n", __func__,
			 flash_area == UI_FIRMWARE ? "UI firmware and config" :
						     "UI config only");
	}
...
static int fwu_enter_flash_prog(struct chip_data_s3706 *chip_info)
{
	...
	retval = fwu_write_f34_command(
		chip_info, CMD_ENABLE_FLASH_PROG); //enter flash program mode
	if (retval < 0) {
		return retval;
	}
				...
				unsigned short block_cnt, unsigned char cmd)
{
	...
	if (chip_info->fwu->bl_version == BL_V7 ||
	    chip_info->fwu->bl_version == BL_V8) {
		retval = fwu_write_f34_v7_blocks(chip_info, block_ptr,
						 block_cnt, cmd);
	} else {
...
static int fwu_write_lockdown(struct chip_data_s3706 *chip_info)
{
	...
	return fwu_write_f34_blocks(
		chip_info, (unsigned char *)chip_info->fwu->img.lockdown.data,
...
{
	return fwu_write_f34_blocks(
		chip_info, (unsigned char *)chip_info->fwu->config_data,
...
static int fwu_check_ui_firmware_size(struct chip_data_s3706 *chip_info)
{
	...
	block_count = chip_info->fwu->img.ui_firmware.size /
		      chip_info->fwu->block_size;
...
static int fwu_check_guest_code_size(struct chip_data_s3706 *chip_info)
{
	...
	block_count = chip_info->fwu->img.guest_code.size /
		      chip_info->fwu->block_size;
...
static int fwu_write_bootloader(struct chip_data_s3706 *chip_info)
{
	...
	retval = fwu_write_f34_blocks(
		chip_info, (unsigned char *)chip_info->fwu->img.bl_image.data,
...
static int fwu_write_utility_parameter(struct chip_data_s3706 *chip_info)
{
	...
	for (ii = 0; ii < MAX_UTILITY_PARAMS; ii++) {
		...
		if (fwu->img.utility_param_id[ii] == FORCE_PARAMETER) {
			...
			retval = secure_memcpy(&(pbuf[4]), remaining_size - 4,
					       fwu->cal_data,
	...
	retval = fwu_write_f34_blocks(
		chip_info, (unsigned char *)chip_info->fwu->read_config_buf,
...
static int fwu_write_partition_table_v7(struct chip_data_s3706 *chip_info)
{
	...
	retval = fwu_allocate_read_config_buf(chip_info,
...
static int fwu_write_guest_code(struct chip_data_s3706 *chip_info)
{
	...
	retval = fwu_write_f34_blocks(
		chip_info, (unsigned char *)chip_info->fwu->img.guest_code.data,
...
synaptics_fw_update(void *chip_data, const struct firmware *fw, bool force)
{
	int retval = 0;
	struct chip_data_s3706 *chip_info = (struct chip_data_s3706 *)chip_data;
...
static fp_touch_state synaptics_spurious_fp_check(void *chip_data)
{
	...
	raw_data = kzalloc(TX_NUM * SPURIOUS_FP_RX_NUM * 2 * (sizeof(uint8_t)),
			   GFP_KERNEL);
	if (!raw_data) {
		TPD_INFO("raw_data kzalloc error\n");
		return fp_touch_state;
	...
	touch_i2c_write_byte(chip_info->client,
			     ...
			     0x00); /*set LSB*/
	touch_i2c_write_byte(chip_info->client,
			     chip_info->reg_info.F54_ANALOG_DATA_BASE + 2,
			     0x00); /*set MSB*/
	touch_i2c_write_byte(chip_info->client,
			     chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
			     0x01); /*get report*/
	...
	touch_i2c_read_block(chip_info->client,
			     chip_info->reg_info.F54_ANALOG_DATA_BASE + 3,
			     TX_NUM * SPURIOUS_FP_RX_NUM * 2,
			     raw_data); /*read baseline data*/
	...
	TPD_INFO(
		"%s:%d chip_info->reg_info.F54_ANALOG_COMMAND_BASE=0x%x set 0, \n",
		__func__, __LINE__,
		chip_info->reg_info
			.F54_ANALOG_COMMAND_BASE); /*add for Prevent TP failure*/
	touch_i2c_write_byte(chip_info->client,
...
RE_TRY:
	...
	TPD_INFO(
		"%s F54_ANALOG_COMMAND_BASE=0x%x set 0, \n", __func__,
		chip_info->reg_info
			.F54_ANALOG_COMMAND_BASE); /*add for Prevent TP failure*/
	touch_i2c_write_byte(chip_info->client,
			     chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0);
				    ...
				    struct monitor_data *mon_data)
{
	...
	ret = touch_i2c_write_byte(
		chip_info->client, 0xff,
		(uint8_t)(chip_info->health_zone.loglength_addr >>
			  8)); /* page 4*/
	data_length = touch_i2c_read_byte(
		chip_info->client,
				   ...
				   void *chip_data)
{
	...
	ret = touch_i2c_read_byte(
		chip_info->client,
		chip_info->reg_info.F01_RMI_CTRL00); /* page 4*/
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F01_RMI_CTRL00,
				   ret | 0x08); /* page 4*/
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x04); /* page 4*/
	if (coord_arg[0] == DouTap) {
		ret = touch_i2c_write_word(chip_info->client, 0x25,
					   coord_arg[1]); /*write length*/
		ret = touch_i2c_write_byte(chip_info->client, 0x28,
					   2); /*Data Function*/
	} else if (coord_arg[0] == DouSwip) {
		ret = touch_i2c_write_word(
			chip_info->client, 0x25,
...
static int synaptics_get_face_state(void *chip_data)
{
	...
	if ((ret > 0) &&
	    time_before(jiffies, chip_info->rotation_changed_time + HZ) &&
...
static void
synaptics_screenon_fingerprint_info(void *chip_data,
...
static void synaptics_enable_gesture_mask(void *chip_data, uint32_t enable)
{
	...
	touch_i2c_write_byte(chip_info->client, 0xff, 0x4);
	gesture_mask = touch_i2c_read_byte(
		chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL20);
	if (enable) {
			      ...
			      const struct i2c_device_id *id)
{
	...
	chip_info->fw_corner_limit_support = of_property_read_bool(
		ts->dev->of_node, "fw_corner_limit_support");
	chip_info->rt155_fdreplace_rt59_support = of_property_read_bool(
		ts->dev->of_node, "rt155_fdreplace_rt59_support");
