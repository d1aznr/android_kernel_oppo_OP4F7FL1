...
static int synaptics_get_chip_info(void *chip_data)
{
	...
	reg_info->F01_RMI_QUERY11 =
		reg_info->F01_RMI_QUERY_BASE + 0x0b; // product id
	...
	reg_info->F12_2D_CTRL08 =
		reg_info->F12_2D_CTRL_BASE; // max XY Coordinate
	...
	reg_info->F12_2D_DATA15 =
		reg_info->F12_2D_DATA_BASE + 2; //object attention
	reg_info->F12_2D_CMD00 = reg_info->F12_2D_CMD_BASE;
	reg_info->F12_2D_CTRL20 =
		reg_info->F12_2D_CTRL_BASE + 0x06; // wakeup Gesture enable
	reg_info->F12_2D_CTRL27 =
		reg_info->F12_2D_CTRL_BASE + 0x0B; //motion suppression
	...
	reg_info->F51_CUSTOM_CTRL36 =
		reg_info->F51_CUSTOM_CTRL_BASE + 0x14; //042e for esd test
	reg_info->F51_CUSTOM_DATA =
		reg_info->F51_CUSTOM_DATA_BASE; //wakeup gesture data
	reg_info->F51_CUSTOM_DATA57 =
		reg_info->F51_CUSTOM_DATA_BASE + 0x19; // esd status

	reg_info->F51_CUSTOM_GRIP_CTRL =
		reg_info->F51_CUSTOM_CTRL_BASE +
		0x05; //041F edge function register,the enable bit of Grip suppression
	...
	reg_info->F54_ANALOG_CTRL91 =
		reg_info->F54_ANALOG_CONTROL_BASE + 0x25; // 0132
	reg_info->F54_ANALOG_CTRL99 =
		reg_info->F54_ANALOG_CONTROL_BASE + 0x2A; //0137
	reg_info->F54_ANALOG_CTRL182 =
		reg_info->F54_ANALOG_CONTROL_BASE + 0x2F; //013C
	reg_info->F54_ANALOG_CTRL225 =
		reg_info->F54_ANALOG_CONTROL_BASE + 0x32; //013F
	...
	TPD_INFO("F55_SENSOR_QUERY_BASE  = 0x%x \n\
		 ...
		 reg_info->F55_SENSOR_CTRL_BASE,
		 reg_info->F55_SENSOR_DATA_BASE);
				   ...
				   int is_suspended)
{
	...
	if ((device_status & 0xC0) ==
	    0x80) { /*unconfigured and not in flash mode*/
		TPD_INFO(
...
static void synaptics_exit_esd_mode(void *chip_data)
{
	...
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F51_CUSTOM_CMD_BASE,
				   0x04);
	if (ret < 0) {
...
static int synaptics_reset(void *chip_data)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x00);
	ret = touch_i2c_write_byte(chip_info->client,
				   chip_info->reg_info.F01_RMI_CMD_BASE,
				   0x01); // reset register
	if (ret < 0) {
				      ...
				      struct gesture_info *gesture)
{
	...
	ret = touch_i2c_read_block(chip_info->client,
				   ...
				   &(gesture_buffer[0]));
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x4);
	regswipe = touch_i2c_read_byte(
		chip_info->client, chip_info->reg_info.F51_CUSTOM_DATA + 0x18);

	TPD_DEBUG(
		"gesture_buffer[0] = 0x%x, regswipe = 0x%x, gesture_buffer[1] = 0x%x, gesture_buffer[4] = 0x%x\n",
		gesture_buffer[0], regswipe, gesture_buffer[1],
		gesture_buffer[4]);
				  ...
				  int rx)
{
	...
	p_right_column_buf = (signed short *)kzalloc(
		right_size * rx_num * sizeof(short), GFP_KERNEL);
	...
	p_left_column_buf = (signed short *)kzalloc(
		left_size * rx_num * sizeof(short), GFP_KERNEL);
	...
	for (i = 0; i < rx_num; i++) {
		p_left_median[i] = find_median(
			p_left_column_buf + i * left_size, left_size);
		p_right_median[i] = find_median(
			p_right_column_buf + i * right_size, right_size);
	}
...
static void synaptics_black_screen_RT136_test(void *chip_data, char *message)
{
	...
	ret = touch_i2c_write_byte(client, reg_info->F54_ANALOG_DATA_BASE,
				   136); //select report type 136
	if (ret < 0) {
		error_count++;
		TPD_INFO(
	...
	ret = touch_i2c_write_byte(client, reg_info->F54_ANALOG_COMMAND_BASE,
				   0X02); //force Cal
	checkCMD(chip_info);
	TPD_DEBUG("Force Cal oK\n");
	ret = touch_i2c_write_word(client, reg_info->F54_ANALOG_DATA_BASE + 1,
				   0x00); //set fifo 00
	ret = touch_i2c_write_byte(client, reg_info->F54_ANALOG_COMMAND_BASE,
				   0x01); //get report
	checkCMD(chip_info);
					...
					struct syna_testdata *syna_testdata)
{
	...
	ret = touch_i2c_write_byte(client, reg_info->F54_ANALOG_CTRL225,
				   0x01); //Force Doze
	ret = touch_i2c_write_byte(client, reg_info->F54_ANALOG_COMMAND_BASE,
				   COMMAND_FORCE_UPDATE); //force update
	checkCMD(chip_info);

	mdelay(1000); // Doze data update every 1s, make sure data updated

	ret = touch_i2c_write_byte(client, reg_info->F54_ANALOG_DATA_BASE,
				   3); //select report type 3
	if (ret < 0) {
		TPD_INFO(
			"[line:%d]read_baseline: touch_i2c_write_byte failed \n",
			__LINE__);
				    ...
				    struct syna_testdata *syna_testdata)
{
	...
	buffer[1] = (ELEC_OPEN_TEST_TX_ON_COUNT >> 8) &
		    0xff; //cbc_timing_ctrl_tx_msb
	buffer[2] = ELEC_OPEN_TEST_RX_ON_COUNT; //cbc_timing_ctrl_rx_lsb
	buffer[3] = (ELEC_OPEN_TEST_RX_ON_COUNT >> 8) &
		    0xff; //cbc_timing_ctrl_rx_msb
	ret = touch_i2c_write_block(chip_info->client,
	...
	raw_data_2 = kzalloc(syna_testdata->TX_NUM * syna_testdata->RX_NUM * 2 *
				     (sizeof(uint8_t)),
			     GFP_KERNEL);
	if (!raw_data_2) {
		TPD_INFO("failed to alloc memory\n");
	...
	touch_i2c_read_block(chip_info->client,
			     chip_info->reg_info.F55_SENSOR_CTRL43,
			     sizeof(ctrl_43), ctrl_43.data); //read data
	synaptics_read_F54_base_reg(chip_info);
				     ...
				     struct syna_testdata *syna_testdata)
{
	...
	tddi_rt95_raw_data = kzalloc(2 * tx_2d_num * syna_testdata->RX_NUM * 2 *
					     (sizeof(uint8_t)),
				     GFP_KERNEL);
	...
	tddi_rt95_part_one = kzalloc(tx_2d_num * syna_testdata->RX_NUM *
					     sizeof(signed short),
				     GFP_KERNEL);
	...
	tddi_rt95_part_two = kzalloc(tx_2d_num * syna_testdata->RX_NUM *
					     sizeof(signed short),
				     GFP_KERNEL);
	...
	touch_i2c_read_block(chip_info->client,
			     chip_info->reg_info.F54_ANALOG_DATA_BASE + 3,
			     2 * tx_2d_num * syna_testdata->RX_NUM * 2,
			     tddi_rt95_raw_data); //read data
	...
	for (x = 0; x < tx_2d_num; x++) {
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			store_to_file(
				syna_testdata->fd, "%d, ",
				tddi_rt95_part_one[x * syna_testdata->RX_NUM +
						   y]);
	...
	for (x = 0; x < tx_2d_num; x++) {
		...
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			...
			tmp_arg2 = tddi_rt95_raw_data[buffer_size + z * 2 + 1] &
				   0xff;
	...
	for (x = 0; x < tx_2d_num; x++) {
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			store_to_file(
				syna_testdata->fd, "%d, ",
				tddi_rt95_part_two[x * syna_testdata->RX_NUM +
						   y]);
				...
				struct syna_testdata *syna_testdata)
{
	...
	error_count += synaptics_tddi_RT3_doze_test(
		s, chip_data, syna_testdata); // test latest
...
static void synaptics_baseline_read(struct seq_file *s, void *chip_data)
{
	...
	for (i = 0; i < 2; i++) {
		if (i == 0) {
			...
			ret = touch_i2c_write_byte(
				chip_info->client,
				chip_info->reg_info.F54_ANALOG_CTRL225,
				0x01); //Force Doze
			ret = touch_i2c_write_byte(
				chip_info->client,
				chip_info->reg_info.F54_ANALOG_COMMAND_BASE,
...
static void synaptics_delta_read(struct seq_file *s, void *chip_data)
{
	...
	seq_printf(s, "TD4310 Type:%d FW:0x%8x\n", chip_info->tp_type,
		   synaptics_get_fw_id(chip_info)); // add fw info and type
...
static void synaptics_main_register_read(struct seq_file *s, void *chip_data)
{
	...
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.F12_2D_DATA_BASE, 8,
				   buf);
	if (ret < 0) {
...
static void re_scan_PDT(struct chip_data_td4310 *chip_info)
{
	...
	touch_i2c_read_block(
		chip_info->client,
		chip_info->reg_info.SynaF34ReflashQuery_FirmwareBlockSize, 2,
		buf);
...
synaptics_fw_update(void *chip_data, const struct firmware *fw, bool force)
{
	...
	ret = touch_i2c_read_block(
		chip_info->client,
		chip_info->reg_info.SynaF34ReflashQuery_BootID, 8,
		&(bootloder_id[0]));
	TPD_DEBUG("bootloader id is %x \n",
		  (bootloder_id[1] << 8) | bootloder_id[0]);
	ret = touch_i2c_write_block(
		chip_info->client, chip_info->reg_info.SynaF34Reflash_BlockData,
		2, &(bootloder_id[0x0]));
	TPD_DEBUG(
	...
	touch_i2c_write_byte(
		chip_info->client, chip_info->reg_info.SynaF34_FlashControl,
		0x03); //erase flash except display config  ,0x03 erase all flash
#else
	touch_i2c_write_byte(
		chip_info->client, chip_info->reg_info.SynaF34_FlashControl,
		0x22); //erase flash except display config  ,0x03 erase all flash
#endif
	msleep(2000);
	ret = touch_i2c_read_byte(chip_info->client,
				  chip_info->reg_info.SynaF34_FlashControl);
	...
	for (j = 0; j < firmware; j++) {
		touch_i2c_write_block(
			chip_info->client,
			chip_info->reg_info.SynaF34Reflash_BlockData, block,
...
static void synaptics_write_ps_status(void *chip_data, int ps_status)
{
	...
	tmp = touch_i2c_read_byte(chip_info->client,
				  chip_info->reg_info.F51_CUSTOM_CTRL36);
	if (ps_status == 1) {
