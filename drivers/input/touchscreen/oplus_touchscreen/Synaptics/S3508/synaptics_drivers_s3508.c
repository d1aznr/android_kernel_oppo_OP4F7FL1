...
static void synaptics_health_zone_init(struct chip_data_s3508 *chip_info)
{
	...
	if (1 == ret) {
		...
		h_tmp = touch_i2c_read_byte(
			chip_info->client,
			chip_info->reg_info.F51_CUSTOM_QUERY_BASE + 0x07);
				...
				struct syna_testdata *syna_testdata)
{
	...
	ret = touch_i2c_write_byte(client,
				   reg_info->F54_ANALOG_CONTROL_BASE + 0x14,
				   0x01); //No SignalClarity
	ret = touch_i2c_read_byte(client,
				  reg_info->F54_ANALOG_CONTROL_BASE +
					  0x1B); //0129  CBC Xmtr carrier select
	tmp_arg1 = ret & 0xff;
	...
	ret = touch_i2c_write_byte(
		client, reg_info->F54_ANALOG_CONTROL_BASE + 0x1B,
		(tmp_arg1 & 0xdf)); //Set CBC, F54_ANALOG_CTRL88, close CBC
	ret = touch_i2c_write_word(client, reg_info->F54_ANALOG_COMMAND_BASE,
				   0x04); //force update
	...
	do {
		...
		if (enable_cbc) {
			...
			ret = touch_i2c_write_byte(
				client, reg_info->F54_ANALOG_CONTROL_BASE + 23,
				(tmp_arg1 | 0x10));
			ret = touch_i2c_write_word(
				client, reg_info->F54_ANALOG_COMMAND_BASE,
				0x04);
			checkCMD(chip_info, 30);
			ret = touch_i2c_read_byte(
				client,
				reg_info->F54_ANALOG_CONTROL_BASE +
					27); //0129  CBC Xmtr carrier select
			tmp_arg2 = ret | 0x20;
			touch_i2c_write_byte(
				client, reg_info->F54_ANALOG_CONTROL_BASE + 27,
				tmp_arg2);
			ret = touch_i2c_write_word(
				client, reg_info->F54_ANALOG_COMMAND_BASE,
				0x04);
			...
			ret = touch_i2c_write_byte(
				client, reg_info->F54_ANALOG_CONTROL_BASE + 23,
				(tmp_arg1 & 0xef)); //0125 0D CBC Settings
			ret = touch_i2c_write_word(
				client, reg_info->F54_ANALOG_COMMAND_BASE,
				0x04);
			ret = touch_i2c_read_byte(
			...
			touch_i2c_write_byte(
				client, reg_info->F54_ANALOG_CONTROL_BASE + 27,
				tmp_arg2); //0129  CBC Xmtr carrier select
			ret = touch_i2c_write_word(
				client, reg_info->F54_ANALOG_COMMAND_BASE,
				0x04); // force update
			ret = touch_i2c_write_byte(
				client, reg_info->F54_ANALOG_CONTROL_BASE + 7,
		...
		/******write No Relax to 1******/
		ret = touch_i2c_write_word(client,
					   reg_info->F54_ANALOG_COMMAND_BASE,
					   0x04); // force update
		checkCMD(chip_info, 30);
		...
		TPD_DEBUG("Force Cal oK\n");
		ret = touch_i2c_write_word(client,
					   reg_info->F54_ANALOG_DATA_BASE + 1,
					   0x00); //set fifo 00
		ret = touch_i2c_write_byte(client,
					   reg_info->F54_ANALOG_COMMAND_BASE,
					   0x01); //get report
		...
		for (x = 0; x < syna_testdata->TX_NUM; x++) {
			...
			for (y = 0; y < syna_testdata->RX_NUM; y++) {
				...
				if ((y < syna_testdata->RX_NUM -
						 syna_testdata->key_RX) &&
				    (x < syna_testdata->TX_NUM -
						 syna_testdata->key_TX)) {
					TPD_DEBUG_NTAG("%d, ", baseline_data);
					if ((baseline_data <
					     *(baseline_data_test +
	...
	//msleep(100);
	checkCMD(chip_info, 30);
	ret = touch_i2c_read_block(client, reg_info->F54_ANALOG_DATA_BASE + 3,
				   7, buffer);
	...
	for (y = 0; y < chip_info->hw_res->RX_NUM + chip_info->hw_res->TX_NUM;
		...
		if ((syna_testdata->key_RX != 0 ||
		    ...
		    ((y > chip_info->hw_res->RX_NUM - syna_testdata->key_RX -
				      1 &&
...
static void synaptics_baseline_read(struct seq_file *s, void *chip_data)
{
	...
	do {
		...
		ret = touch_i2c_read_byte(
			chip_info->client,
			chip_info->reg_info.F54_ANALOG_CONTROL_BASE + 23);
		tmp_arg1 = ret & 0xff;
...
synaptics_fw_update(void *chip_data, const struct firmware *fw, bool force)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x0);
	ret = touch_i2c_read_block(chip_info->client,
				   chip_info->reg_info.F01_RMI_QUERY_BASE + 11,
				   8, productid);
	productid[7] = '\0';
	...
	for (j = 0; j < firmware; j++) {
		...
		touch_i2c_write_block(
			chip_info->client,
			chip_info->reg_info.SynaF34Reflash_BlockNum, 2, buf);
		touch_i2c_write_block(
			chip_info->client,
			chip_info->reg_info.SynaF34Reflash_BlockData, 16,
			&Firmware_Data[j * 16]);
		touch_i2c_write_byte(chip_info->client,
				     chip_info->reg_info.SynaF34_FlashControl,
				     0x02);
				    ...
				    struct monitor_data *mon_data)
{
	...
	if (data_length < 1 || data_length > 255) {
		...
		if (health_info->reset_reason != 0) {
			...
			ret = touch_i2c_read_byte(
				chip_info->client,
				chip_info->reg_info.F51_CUSTOM_CTRL67);
			touch_i2c_write_byte(
				chip_info->client,
				chip_info->reg_info.F51_CUSTOM_CTRL67,
				ret | 0x80); /* page 4*/
			TPD_INFO(
				"chip_info->reg_info.F51_CUSTOM_CTRL67= %x  ret=%x\n",
...
static void synaptics_bootup_test(void *chip_data, const struct firmware *fw,
				  struct monitor_data *mon_data,
				  struct hw_resource *hw_res)
{
	...
	raw_data =
		kzalloc(hw_res->TX_NUM * hw_res->RX_NUM * 2 * (sizeof(uint8_t)),
			GFP_KERNEL);
	if (!raw_data) {
		TPD_INFO("raw_data kzalloc error\n");
		return;
	...
	for (x = 0; x < hw_res->TX_NUM; x++) {
		...
		for (y = 0; y < hw_res->RX_NUM; y++) {
			...
			tmp_arg2 = raw_data[z * 2 + 1] & 0xff;
			baseline_data = (tmp_arg2 << 8) | tmp_arg1;
			if ((y < hw_res->RX_NUM - hw_res->key_RX) &&
			    (x < hw_res->TX_NUM - hw_res->key_TX)) {
				TPD_DEBUG_NTAG("%d, ", baseline_data);
				if ((baseline_data <
				     *(baseline_data_test + count * 2)) ||
				    (baseline_data >
			...
			} else if ((x == hw_res->TX_NUM - hw_res->key_TX) &&
				   (y >= hw_res->RX_NUM - hw_res->key_RX)) {
				TPD_DEBUG(
					"touchkey test xy(%d,%d),baseline_data=%d,limite( %d, %d)\n ",
					x, y, baseline_data,
					*(baseline_data_test + count * 2),
	...
	ret = touch_i2c_read_byte(client, reg_info->F54_ANALOG_CONTROL_BASE +
						  23); //0125 0D CBC Settings
	tmp_arg1 = ret & 0xff;
	...
	ret = touch_i2c_write_byte(chip_info->client,
				   ...
				   0x00); // Disable No SignalClarity
	ret = touch_i2c_write_byte(client,
				   reg_info->F54_ANALOG_CONTROL_BASE + 23,
				   (tmp_arg1 | 0x10));
	ret = touch_i2c_write_word(client, reg_info->F54_ANALOG_COMMAND_BASE,
				   0x04);
	checkCMD(chip_info, 30);
	ret = touch_i2c_read_byte(client,
				  reg_info->F54_ANALOG_CONTROL_BASE +
					  27); //0129  CBC Xmtr carrier select
	tmp_arg2 = ret | 0x20;
	touch_i2c_write_byte(client, reg_info->F54_ANALOG_CONTROL_BASE + 27,
			     tmp_arg2);
	ret = touch_i2c_write_word(client, reg_info->F54_ANALOG_COMMAND_BASE,
				   0x04);
	TPD_DEBUG("Test open cbc\n");
	...
	touch_i2c_write_byte(client, 0xff, 0x3);
	touch_i2c_read_block(client, reg_info->F55_SENSOR_CTRL01,
			     hw_res->RX_NUM, buffer_rx);
	touch_i2c_read_block(client, reg_info->F55_SENSOR_CTRL02,
			     hw_res->TX_NUM, buffer_tx);
