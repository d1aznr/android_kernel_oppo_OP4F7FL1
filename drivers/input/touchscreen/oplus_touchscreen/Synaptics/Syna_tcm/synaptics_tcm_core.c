...
syna_td4330_remote_device_init(struct syna_tcm_hcd *tcm_hcd);
...
static int syna_tcm_run_bootloader_firmware(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval = syna_tcm_write_message(tcm_hcd, CMD_RUN_BOOTLOADER_FIRMWARE,
					NULL, 0, &resp_buf, &resp_buf_size,
					&resp_length, 0);
	if (retval < 0) {
		TPD_INFO("Failed to write command %s\n",
...
static int syna_tcm_reset(void *chip_data)
{
	...
	retval = syna_tcm_write_message(tcm_hcd, CMD_RESET, NULL, 0, &resp_buf,
					&resp_buf_size, &resp_length, 0);
	if (retval < 0) {
		TPD_INFO("Failed to write command %s\n", STR(CMD_RESET));
		goto exit;
...
static int syna_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_EDGE:
		ret = syna_tcm_set_dynamic_config(
			tcm_hcd, DC_GRIP_SUPPRESSION_ENABLED, flag ? 1 : 0);
		if (ret < 0) {
				...
				unsigned int page_count)
{
	...
	ret = syna_tcm_write_message(tcm_hcd, CMD_ERASE_FLASH, out_buf,
				     sizeof(out_buf), &resp_buf, &resp_buf_size,
				...
				unsigned int datalen)
{
	...
	while (remaining_len) {
		...
		retval =
			syna_tcm_write_message(tcm_hcd, CMD_WRITE_FLASH,
					       out.buf, xfer_len + 2, &resp_buf,
					       &resp_buf_size, &resp_length,
					  ...
					  const struct firmware *fw, bool force)
{
	...
	if (force) {
		...
		if (ret > 0) {
			ret = syna_tcm_switch_mode(
				tcm_hcd,
				FW_MODE_APPLICATION); /* switch to app mode to get app info */
			if (ret < 0) {
	...
	ret = syna_tcm_write_flash(tcm_hcd, &reflash_hcd, flash_addr, data,
				   size);
	if (ret < 0) {
