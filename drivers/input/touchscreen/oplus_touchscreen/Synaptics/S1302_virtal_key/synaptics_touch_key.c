...
extern void input_report_key_oplus(struct input_dev *dev, unsigned int code,
				   int value);
/*------------------------------------------Fuction Declare----------------------------------------------*/
...
/*-------------------------------Using Struct----------------------------------*/
static const struct i2c_device_id synaptics_ts_id[] = { { TPD_DEVICE, 0 }, {} };
...
static int synaptics_read_register_map(struct synaptics_ts_data *ts)
{
	...
	TPD_INFO("F11_2D_QUERY_BASE = %x \n \
            ...
            F11_2D_DATA_BASE    = %x \n\
            ",
		 F11_2D_QUERY_BASE, F11_2D_CMD_BASE, F11_2D_CTRL_BASE,
		 F11_2D_DATA_BASE);
	...
	TPD_DEBUG("F01_RMI_QUERY_BASE = %x \n\
            ...
            F01_RMI_DATA_BASE    = %x \n\
            ",
		  F01_RMI_QUERY_BASE, F01_RMI_CMD_BASE, F01_RMI_CTRL00,
		  F01_RMI_DATA_BASE);
	...
	TPD_DEBUG("F34_FLASH_QUERY_BASE = %x \n\
            ...
            ",
		  F34_FLASH_QUERY_BASE, F34_FLASH_CMD_BASE, F34_FLASH_CTRL_BASE,
		  F34_FLASH_DATA_BASE);
...
static int synaptics_init_panel(struct synaptics_ts_data *ts)
{
	...
	if (ret < 0) {
		msleep(150);
		ret = synaptics_rmi4_i2c_write_byte(ts->client, F01_RMI_CTRL00,
						    0x80);
		if (ret < 0) {
...
static int synaptics_enable_interrupt(struct synaptics_ts_data *ts, int enable)
{
	...
	ret = synaptics_rmi4_i2c_write_byte(ts->client, F01_RMI_CTRL01,
					    abs_status_int);
	if (ret < 0) {
...
static void int_key(struct synaptics_ts_data *ts)
{
	...
	case 3: //force cal
		...
		synaptics_rmi4_i2c_write_word(ts->client,
					      F54_ANALOG_COMMAND_BASE,
					      0x04); // force update
		checkCMD();
		synaptics_rmi4_i2c_write_byte(
			ts->client, F54_ANALOG_COMMAND_BASE, 0X02); //force Cal
		checkCMD();
...
static int synatpitcs_fw_update(struct device *dev, bool force)
{
	...
	if (ret < 0) {
		...
		ret = synapitcs_ts_update(ts->client, fw->data, fw->size,
					  force);
		if (ret < 0) {
			TPD_INFO(
	...
	ret = synaptics_rmi4_i2c_write_byte(ts->client, 0xff, 0x0);
	ret = synaptics_rmi4_i2c_read_block(ts->client, F34_FLASH_CTRL_BASE, 4,
					    buf);
	CURRENT_FIRMWARE_ID =
					  ...
					  size_t count, loff_t *ppos)
{
	...
	wbyte = buf[3];
	if (0xFF ==
	    temp_block) //the  mark is to write register else read register
	{
		for (i = 0; i < wbyte; i++) {
			reg[i] = (char)buf[4 + i];
		}
		ret = synaptics_rmi4_i2c_write_byte(ts->client, 0xff, page);
		ret = synaptics_rmi4_i2c_write_block(ts->client, (char)address,
						     wbyte, reg);
						  ...
						  bool savefile)
{
	...
	if (savefile) {
		...
		sprintf(data_buf,
			...
			rtc_now_time.tm_hour + 8 > 23 ?
				(rtc_now_time.tm_hour + 8 - 24) :
	...
	ret = synaptics_rmi4_i2c_write_byte(ts_g->client, F54_ANALOG_DATA_BASE,
	...
	ret = synaptics_rmi4_i2c_write_word(
		ts_g->client, F54_ANALOG_COMMAND_BASE, 0x04); // force update

	checkCMD();
	TPD_DEBUG("forbid Forbid NoiseMitigation oK\n");
	ret = synaptics_rmi4_i2c_write_byte(
		ts_g->client, F54_ANALOG_COMMAND_BASE, 0X02); //force Cal
	checkCMD();
	TPD_DEBUG("Force Cal oK\n");
	ret = synaptics_rmi4_i2c_write_word(
		ts_g->client, F54_ANALOG_DATA_BASE + 1, 0x00); //set fifo 00
	ret = synaptics_rmi4_i2c_write_byte(
		ts_g->client, F54_ANALOG_COMMAND_BASE, 0x01); //get report
	checkCMD();

	count = 0;
	for (x = 0; x < 2; x++) {
		for (y = 0; y < 5; y++) {
			ret = synaptics_rmi4_i2c_read_byte(
				ts_g->client, F54_ANALOG_DATA_BASE + 3);
			tmp_l = ret & 0xff;
			ret = synaptics_rmi4_i2c_read_byte(
				ts_g->client, F54_ANALOG_DATA_BASE + 3);
			tmp_h = ret & 0xff;
	...
	ret = synaptics_rmi4_i2c_write_word(ts_g->client,
...
END:
	...
	//TPD_DEBUG("buf = %s\n", buf);
	ret = synaptics_rmi4_i2c_write_byte(ts_g->client,
					...
					loff_t *ppos)
{
	...
	ret = synaptics_rmi4_i2c_write_byte(ts_g->client, 0xff,
					    tp_reg_address >> 8);
	if (ret < 0) {
		TPD_DEBUG("%s error %d\n", __func__, __LINE__);
		return -1;
	...
	ret = synaptics_rmi4_i2c_read_byte(ts_g->client,
					   tp_reg_address & 0x00ff);
					 ...
					 size_t count, loff_t *ppos)
{
	...
	data = synaptics_rmi4_i2c_read_byte(ts_g->client,
					    tp_reg_address & 0x00ff);
					  ...
					  loff_t *ppos)
{
	...
	if (!ts_g)
		return baseline_ret;
	if (baseline_ret ==
	    0) //proc node will auto read two time, but only need tp test one time
	{
		count = synaptics_rmi4_baseline_show_s1302(ts_g->dev, page, 1);
		baseline_ret = simple_read_from_buffer(user_buf, count, ppos,
						       page, strlen(page));
	} else {
		baseline_ret = 0;
	}
			     ...
			     loff_t *ppos)
{
	...
	ret = simple_read_from_buffer(buf, size, ppos, kernel_buf,
				      strlen(kernel_buf));
	kfree(kernel_buf);
	return ret;
}
				...
				size_t size, loff_t *ppos)
{
	...
	TPD_DEBUG("ret = %x, tmp_old = %x, tmp_new = %x\n", ret, tmp_old,
		  (tmp_old & 0xef));
	ret = synaptics_rmi4_i2c_write_byte(
		ts_g->client, F54_ANALOG_CONTROL_BASE + 86, (tmp_old & 0xef));
	ret = synaptics_rmi4_i2c_write_word(ts_g->client,
					    F54_ANALOG_COMMAND_BASE, 0x04);

	ret = synaptics_rmi4_i2c_write_byte(ts_g->client,
					    F54_ANALOG_CONTROL_BASE + 29,
...
static int synaptics_s1302_proc(void)
{
	...
	proc_entry = proc_create_data("fw_update", 0444, procdir,
				      &proc_firmware_update, NULL);
	proc_entry = proc_create_data("key_rep", 0666, procdir,
				      &proc_reverse_key, NULL);
	proc_entry = proc_create_data("radd", 0666, procdir, &proc_radd, NULL);
	proc_entry =
		proc_create_data("reset", 0666, procdir, &proc_reset, NULL);
	proc_entry = proc_create_data("oplus_tp_debug", 0666, procdir,
				      &proc_debug, NULL);

	proc_entry = proc_create_data("baseline_test", 0666, procdir,
				      &tp_baseline_test_proc_fops, NULL);
	proc_entry = proc_create_data("oplus_tp_delta_data", 0644, procdir,
				      &oplus_tp_delta_data_proc_fops, NULL);
	proc_entry = proc_create_data("oplus_tp_baseline_image", 0644, procdir,
				      &oplus_tp_baseline_image_proc_fops, NULL);
...
static void re_scan_PDT(struct i2c_client *client)
{
	...
	i2c_smbus_read_i2c_block_data(
		client, SynaF34ReflashQuery_FirmwareBlockSize, 2, buf);
			       ...
			       uint32_t data_len, bool force)
{
	...
	ret = synaptics_rmi4_i2c_read_block(client, F34_FLASH_CTRL_BASE, 4,
					    buf);
	CURRENT_FIRMWARE_ID =
		(buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
	...
	ret = i2c_smbus_read_i2c_block_data(client, SynaF34ReflashQuery_BootID,
					    8, &(bootloder_id[0]));
	TPD_DEBUG("bootloader id is %x \n",
		  (bootloder_id[1] << 8) | bootloder_id[0]);
	ret = i2c_smbus_write_i2c_block_data(client, SynaF34Reflash_BlockData,
					     2, &(bootloder_id[0x0]));
	TPD_DEBUG(
...
static int synaptics_parse_dts(struct device *dev, struct synaptics_ts_data *ts)
{
	...
	np = dev->of_node;
	ts->irq_gpio = of_get_named_gpio_flags(np, "synaptics,irq-gpio", 0,
					       &(ts->irq_flags));
	if (ts->irq_gpio < 0) {
			      ...
			      const struct i2c_device_id *id)
{
	...
	ret = request_threaded_irq(ts->irq, NULL, synaptics_irq_thread_fn,
				   ts->irq_flags | IRQF_ONESHOT, TPD_DEVICE,
				   ts);
	if (ret < 0)
		TPD_INFO("%s request_threaded_irq ret is %d\n", __func__, ret);

	//   ret = synaptics_soft_reset(ts);
	ret = synaptics_init_panel(
		ts); // switch to normal work mode, can not go to sleep when power on
	if (ret < 0) {
