...
static int synaptics_get_chip_info(void *chip_data)
{
	...
	reg_info->F12_2D_CTRL20 =
		reg_info->F12_2D_CTRL_BASE + 0x06; //motion suppression
	reg_info->F12_2D_CTRL27 =
		reg_info->F12_2D_CTRL_BASE + 0x0b; // wakeup Gesture enable
	...
	reg_info->F51_CUSTOM_GRIP_CTRL =
		reg_info->F51_CUSTOM_CTRL_BASE +
		0x1D; //edge function register,the enable bit of Grip suppression
				    ...
				    struct syna_testdata *syna_testdata)
{
	...
	buffer[1] =
		(ELEC_OPEN_INT_DUR_ONE >> 8) & 0xff; //integration_duration_msb
	ret = touch_i2c_write_block(chip_info->client,
				    chip_info->reg_info.F54_ANALOG_CTRL99, 3,
				    buffer);
	if (ret < 0) {
	...
	buffer[1] =
		(ELEC_OPEN_INT_DUR_TWO >> 8) & 0xff; //integration_duration_msb
	ret = touch_i2c_write_block(chip_info->client,
				    chip_info->reg_info.F54_ANALOG_CTRL99, 3,
				    buffer);
	if (ret < 0) {
		...
		for (y = 0; y < syna_testdata->RX_NUM; y++) {
			store_to_file(
				...
				raw_data_delta_image[x * syna_testdata->RX_NUM +
						     y]);
			if (x < tx_2d_num &&
			    raw_data_delta_image[x * syna_testdata->RX_NUM + y] <
				    ELEC_OPEN_TEST_LIMIT_ONE) {
				TPD_INFO(
