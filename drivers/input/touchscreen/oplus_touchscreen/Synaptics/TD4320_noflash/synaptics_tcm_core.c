...
syna_td4320_nf_remote_device_init(struct syna_tcm_hcd *tcm_hcd);
...
int zeroflash_check_uboot(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval = syna_tcm_td4320_nf_rmi_read(tcm_hcd, 0x00ee, &fn_number,
					     sizeof(fn_number));
	if (retval < 0) {
...
static int syna_parse_report(struct syna_tcm_hcd *tcm_hcd)
{
	...
	while (idx < config_size) {
		...
		case TOUCH_TIMESTAMP:
			...
			retval = syna_get_report_data(tcm_hcd, offset, bits,
						      &data);
			if (retval < 0) {
...
static int syna_get_input_params(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval = syna_tcm_write_message(tcm_hcd, CMD_GET_TOUCH_REPORT_CONFIG,
...
static int syna_get_default_report_config(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval =
		syna_tcm_write_message(tcm_hcd, CMD_GET_TOUCH_REPORT_CONFIG,
				       NULL, 0, &tcm_hcd->default_config.buf,
				       &tcm_hcd->default_config.buf_size,
				       &tcm_hcd->default_config.data_length, 0);
	if (retval < 0) {
		TPD_INFO("Failed to write command %s\n",
			 STR(CMD_GET_TOUCH_REPORT_CONFIG));
		goto exit;
...
static int syna_set_normal_report_config(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval = syna_tcm_write_message(tcm_hcd, CMD_SET_TOUCH_REPORT_CONFIG,
...
static int syna_tcm_continued_read(struct syna_tcm_hcd *tcm_hcd)
{
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		retval = secure_memcpy(&tcm_hcd->in.buf[offset],
				       total_length - offset,
				       &tcm_hcd->temp.buf[2], xfer_length,
				       xfer_length);
		if (retval < 0) {
			TPD_INFO("Failed to copy payload\n");
			UNLOCK_BUFFER(tcm_hcd->temp);
 ...
 */
int syna_tcm_raw_read(struct syna_tcm_hcd *tcm_hcd, unsigned char *in_buf,
		      unsigned int length)
{
	int retval;
	unsigned char code;
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		if (idx == 0) {
			retval =
				secure_memcpy(&in_buf[0], length,
					      &tcm_hcd->temp.buf[0],
					      xfer_length + 2, xfer_length + 2);
		} else {
			if (code != STATUS_CONTINUED_READ) {
				TPD_INFO("Incorrect header code (0x%02x)\n",
					 code);
			      ...
			      unsigned int length)
{
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		if (xfer_length) {
			retval =
				secure_memcpy(&tcm_hcd->out.buf[1], xfer_length,
...
static int syna_tcm_write_message_zeroflash(
	struct syna_tcm_hcd *tcm_hcd, unsigned char command,
	unsigned char *payload, unsigned int length, unsigned char **resp_buf,
...
get_app_info:
	retval = syna_tcm_write_message(tcm_hcd, CMD_GET_APPLICATION_INFO, NULL,
...
static int syna_tcm_get_boot_info(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval = syna_tcm_write_message(tcm_hcd, CMD_GET_BOOT_INFO, NULL, 0,
...
static int syna_tcm_identify(struct syna_tcm_hcd *tcm_hcd, bool id)
{
	...
	retval = syna_tcm_write_message(tcm_hcd, CMD_IDENTIFY, NULL, 0,
					&resp_buf, &resp_buf_size, &resp_length,
					0);
	if (retval < 0) {
		TPD_INFO("Failed to write command %s\n", STR(CMD_IDENTIFY));
...
retry:
	retval = syna_tcm_write_message(tcm_hcd, CMD_RUN_APPLICATION_FIRMWARE,
				       ...
				       unsigned short *value)
{
	...
	retval = syna_tcm_write_message(tcm_hcd, CMD_GET_DYNAMIC_CONFIG,
				       ...
				       unsigned short value)
{
	...
	retval = syna_tcm_write_message(tcm_hcd, CMD_SET_DYNAMIC_CONFIG,
					out_buf, sizeof(out_buf), &resp_buf,
					&resp_buf_size, &resp_length,
					RESPONSE_TIMEOUT_MS_SHORT);
	if (retval < 0) {
...
static int syna_tcm_sleep(struct syna_tcm_hcd *tcm_hcd, bool en)
{
	...
	retval = syna_tcm_write_message(tcm_hcd, command, NULL, 0, &resp_buf,
...
static int syna_get_chip_info(void *chip_data)
{
	...
	ret = syna_tcm_reset(
		tcm_hcd); // reset to get bootloader info or boot info
	if (ret < 0) {
				 ...
				 int max_num)
{
	...
	for (idx = 0; idx < touch_hcd->max_objects; idx++) {
		...
		points[idx].z =
			(object_data[idx].x_width + object_data[idx].y_width) /
			2;
...
static int syna_tcm_set_gesture_mode(struct syna_tcm_hcd *tcm_hcd, bool enable)
{
	...
	retval = syna_tcm_get_dynamic_config(tcm_hcd, DC_IN_WAKEUP_GESTURE_MODE,
					     &config);
	if (retval < 0) {
	...
	if (enable) {
		if (!config) {
			...
			retval = syna_tcm_set_dynamic_config(
				tcm_hcd, DC_IN_WAKEUP_GESTURE_MODE, true);
			if (retval < 0) {
				TPD_INFO(
...
static int syna_tcm_normal_mode(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval = syna_tcm_set_dynamic_config(tcm_hcd, DC_IN_WAKEUP_GESTURE_MODE,
					     false);
	if (retval < 0) {
...
static int synaptics_corner_limit_handle(struct syna_tcm_hcd *tcm_hcd,
					 bool enable)
{
	int ret = -1;
	...
	if ((LANDSCAPE_SCREEN_90 == tcm_hcd->touch_direction) && !enable) {
		...
		ret = syna_tcm_set_dynamic_config(
			tcm_hcd, DC_GRIP_ROATE_TO_HORIZONTAL_LEVEL, 0x01);
		if (ret < 0) {
			TPD_INFO(
		...
		ret = syna_tcm_set_dynamic_config(tcm_hcd, DC_DARKZONE_ENABLE,
						  0x03);
		if (ret < 0) {
		...
		ret = syna_tcm_set_dynamic_config(
			tcm_hcd, DC_GRIP_DARKZONE_X,
			tcm_hcd->grip_darkzone_x); //x part
		if (ret < 0) {
		...
		ret = syna_tcm_set_dynamic_config(
			tcm_hcd, DC_GRIP_DARKZONE_Y,
			tcm_hcd->grip_darkzone_y); //y part
		if (ret < 0) {
	...
	} else if ((LANDSCAPE_SCREEN_270 == tcm_hcd->touch_direction) &&
		   !enable) {
		ret = syna_tcm_set_dynamic_config(
			tcm_hcd, DC_GRIP_ROATE_TO_HORIZONTAL_LEVEL, 0x01);
		if (ret < 0) {
		...
		ret = syna_tcm_set_dynamic_config(
			tcm_hcd, DC_GRIP_ROATE_TO_HORIZONTAL_LEVEL, 0x00);
		if (ret < 0) {
			TPD_INFO(
				"%s:failed to set DC_GRIP_ROATE_TO_HORIZONTAL_LEVEL\n",
				__func__);
...
static int synaptics_enable_edge_limit(struct syna_tcm_hcd *tcm_hcd,
				       bool enable)
{
	int ret;

	ret = syna_tcm_set_dynamic_config(tcm_hcd,
					  DC_GRIP_SUPPRESSION_ENABLED_NEW, 1);
...
static int synaptics_enable_headset_mode(struct syna_tcm_hcd *tcm_hcd,
					 bool enable)
{
	int8_t ret = -1;
...
static int synaptics_enable_game_mode(struct syna_tcm_hcd *tcm_hcd, bool enable)
{
	...
	if (enable) {
		ret = syna_tcm_set_dynamic_config(tcm_hcd, DC_GAME_MODE_ENABLED,
						  1);
		if (ret < 0) {
...
static int syna_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_SLEEP:
		...
		break;
	case MODE_CHARGE:
		ret = syna_tcm_set_dynamic_config(tcm_hcd, DC_CHARGER_CONNECTED,
						  flag ? 1 : 0);
		if (ret < 0) {
				    ...
				    struct panel_info *panel_data)
{
	...
	if (!tcm_hcd->using_fae) {
		retval = wait_for_completion_timeout(
			&tcm_hcd->config_complete,
			msecs_to_jiffies(RESPONSE_TIMEOUT_MS_LONG * 2));
		if (retval == 0) {
			TPD_INFO(
				"Timed out waiting for response config_complete\n");
		}
	}
...
void syna_fw_version_update(void *chip_data)
{
	...
	if (ts->panel_data.manufacture_info.version) {
		sprintf(ts->panel_data.manufacture_info.version, "0x%s",
			fw_ver);
	}
...
static void syna_tcm_helper_work(struct work_struct *work)
{
	...
	struct syna_tcm_hcd *tcm_hcd =
		container_of(work, struct syna_tcm_hcd, helper_work);
...
*/
extern int td4320_nf_try_to_recovery_ic(struct syna_tcm_hcd *tcm_hcd,
					char *iHex);
			     ...
			     struct syna_testdata *syna_testdata)
{
	...
	retval = syna_tcm_write_message(tcm_hcd, CMD_PRODUCTION_TEST, out, 1,
					&resp_buf, &resp_buf_size, &resp_length,
...
static void syna_tcm_black_screen_test(void *chip_data, char *message)
{
	...
	retval = request_firmware(&fw, tcm_hcd->limit_name,
	...
	error_count += syna_testing_noise(buffer, tcm_hcd, &syna_testdata,
					  "GestureNoise");
	error_count += syna_testing_dynamic_range(
		buffer, tcm_hcd, &syna_testdata, "GestureDynamic");
			   ...
			   struct syna_testdata *syna_testdata)
{
	...
	if (0) {
		retval = syna_tcm_set_dynamic_config(tcm_hcd, DC_DISABLE_ESD,
						     false);
	}
	if (retval < 0) {
		error_count++;
	...
	error_count += syna_testing_dynamic_range(buffer, tcm_hcd,
						  syna_testdata, "Dynamic");
	error_count += syna_testing_pt11(buffer, tcm_hcd, syna_testdata);

	/*start Doze test*/
	error_count += syna_testing_Doze_dynamic_range(
		buffer, tcm_hcd, syna_testdata, "DozeDynamic");
	error_count += syna_testing_Doze_noise(buffer, tcm_hcd, syna_testdata,
					       "DozeNoise");
				    ...
				    unsigned int num_of_reports)
{
	...
	retval = syna_tcm_write_message(tcm_hcd, CMD_ENABLE_REPORT, out, 1,
					&resp_buf, &resp_buf_size, &resp_length,
					0);
	if (retval < 0) {
	...
	retval = syna_tcm_write_message(tcm_hcd, CMD_DISABLE_REPORT, out, 1,
					&resp_buf, &resp_buf_size, &resp_length,
					0);
	if (retval < 0) {
		TPD_INFO("Failed to write message %s\n",
...
static void syna_tcm_test_report(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (test_hcd->report_index == 0) {
		retval = syna_tcm_alloc_mem(tcm_hcd, &test_hcd->report,
					    report_size *
...
static void syna_main_register(struct seq_file *s, void *chip_data)
{
	...
	retval = syna_tcm_write_message(tcm_hcd, CMD_GET_NSM_INFO, NULL, 0,
					&resp_buf, &resp_buf_size, &resp_length,
					0);
	if (retval < 0) {
...
static void syna_freq_hop_trigger(void *chip_data)
{
	...
	retval = syna_tcm_set_dynamic_config(tcm_hcd, DC_FREQUENCE_HOPPING,
					     freq_point);
	if (retval < 0) {
...
static enum hrtimer_restart syna_tcm_reset_timeout(struct hrtimer *timer)
{
	...
	hrtimer_forward_now(&tcm_hcd->watchdog,
			    ktime_set(0, RESET_TIMEOUT_TIME));
...
static int syna_tcm_init_device(struct syna_tcm_hcd *tcm_hcd)
{
	...
	tcm_hcd->touch_hcd = (struct touch_hcd *)kzalloc(
	...
	tcm_hcd->test_hcd = (struct syna_tcm_test *)kzalloc(
...
static void syna_tcm_parse_dts(struct syna_tcm_hcd *tcm_hcd,
			       struct spi_device *spi)
{
	...
	dev = &spi->dev;
	np = dev->of_node;
	rc = of_property_read_u32_array(np, "synaptics,grip-darkzone-area",
					temp_array, 2);
	if (rc) {
	...
	tcm_hcd->irq_trigger_hdl_support =
		of_property_read_bool(np, "synaptics,irq_trigger_hdl_support");
}
...
static const struct spi_device_id syna_tmc_id[] = { { TPD_DEVICE, 0 }, {} };
