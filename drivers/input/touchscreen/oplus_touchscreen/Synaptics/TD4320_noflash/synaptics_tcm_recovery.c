	...
	for (record = 0; record < recovery_hcd->ihex_records; record++) {
		...
		if (type == 0x00) {
			if ((words % F35_CHUNK_SIZE_WORDS) == 0) {
				...
				retval = recovery_add_data_entry(recovery_hcd,
								 addr);
				if (retval < 0) {
					TPD_INFO("Failed to add data entry\n");
					return retval;
...
static int recovery_write_flash(struct recovery_hcd *recovery_hcd)
{
	...
	while (entries_written < recovery_hcd->data_entries) {
		...
		retval = syna_tcm_td4320_nf_rmi_write(
			...
			recovery_hcd->chunk_buf, chunk_buf_size);
		if (retval < 0) {
...
static int recovery_poll_erase_completion(struct recovery_hcd *recovery_hcd)
{
	...
	do {
		...
		retval = syna_tcm_td4320_nf_rmi_write(
			recovery_hcd->tcm_hcd,
			recovery_hcd->f35_addr.command_base, &command,
			sizeof(command));
		if (retval < 0) {
			TPD_INFO("Failed to write F$35 command\n");
			return retval;
		...
		retval = syna_tcm_td4320_nf_rmi_read(
			recovery_hcd->tcm_hcd, data_base + F35_DATA5_OFFSET,
			&status, sizeof(status));
		if (retval < 0) {
...
static int recovery_get_fw_ihex(struct recovery_hcd *recovery_hcd, char *iHex)
{
	...
	retval = request_firmware(&recovery_hcd->fw_entry, iHex,
				  &recovery_hcd->tcm_hcd->s_client->dev);
	if (retval < 0) {
