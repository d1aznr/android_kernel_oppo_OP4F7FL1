...
static void device_capture_touch_report(unsigned int count)
{
	...
	case REPORT_TOUCH:
		...
		retval = syna_tcm_alloc_mem(tcm_hcd, &g_device_hcd->report,
					    remaining_size);
		if (retval < 0) {
			   ...
			   loff_t *f_pos)
{
	...
	if (g_device_hcd->raw_mode) {
		retval =
			syna_tcm_alloc_mem(tcm_hcd, &g_device_hcd->resp, count);
		if (retval < 0) {
		...
		retval = g_device_hcd->read_message(
			tcm_hcd, g_device_hcd->resp.buf, count);
		if (retval < 0) {
			    ...
			    size_t count, loff_t *f_pos)
{
	...
	retval = syna_tcm_alloc_mem(tcm_hcd, &g_device_hcd->out,
	...
	if (g_device_hcd->raw_mode) {
		...
		retval = g_device_hcd->write_message(
			tcm_hcd, g_device_hcd->out.buf[0],
			&g_device_hcd->out.buf[1], count - 1,
			&g_device_hcd->resp.buf, &g_device_hcd->resp.buf_size,
...
static int device_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	g_device_hcd->device =
		device_create(g_device_hcd->class, NULL, g_device_hcd->dev_num,
