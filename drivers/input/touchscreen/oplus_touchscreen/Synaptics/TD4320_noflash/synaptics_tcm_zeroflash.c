...
static int zeroflash_check_uboot(void)
{
	...
	retval = syna_tcm_td4320_nf_rmi_read(tcm_hcd, PDT_END_ADDR, &fn_number,
					     sizeof(fn_number));
	if (retval < 0) {
	...
	retval = syna_tcm_td4320_nf_rmi_read(tcm_hcd, PDT_START_ADDR,
	...
	retval = syna_tcm_td4320_nf_rmi_read(
		tcm_hcd, g_zeroflash_hcd->f35_addr.query_base,
...
int zeroflash_parse_fw_image(void)
{
	...
	for (idx = 0; idx < num_of_areas; idx++) {
		...
		if (0 == strncmp((char *)descriptor->id_string, BOOT_CONFIG_ID,
			...
			TPD_INFO("Application firmware size = %d\n", length);
			TPD_INFO(
			...
			if (checksum != (crc32(~0, content, length) ^ ~0)) {
				TPD_INFO("Application config checksum error\n");
				return -EINVAL;
			}
			...
			TPD_INFO("Application config size = %d\n", length);
			TPD_INFO("Application config flash address = 0x%08x\n",
				 flash_addr);
		} else if (0 == strncmp((char *)descriptor->id_string,
					DISP_CONFIG_ID,
					strlen(DISP_CONFIG_ID))) {
			if (checksum != (crc32(~0, content, length) ^ ~0)) {
				TPD_INFO("Display config checksum error\n");
				return -EINVAL;
			}
			image_info->disp_config.size = length;
			image_info->disp_config.data = content;
			image_info->disp_config.flash_addr = flash_addr;
			TPD_INFO("Display config size = %d\n", length);
			TPD_INFO("Display config flash address = 0x%08x\n",
				 flash_addr);
		}
	}
	...
	do {
		...
		if (fw_name_fae == NULL) {
			if (ts->fw_update_app_support) {
				retval = request_firmware_select(
					&g_zeroflash_hcd->fw_entry,
...
void zeroflash_check_download_config(void)
{
	...
	//msleep(50);
	queue_work(g_zeroflash_hcd->config_workqueue,
		   &g_zeroflash_hcd->config_work);
...
void zeroflash_download_config(void)
{
	...
	TPD_INFO(
		"zeroflash_download_config fw status:need app:%d need display:%d\n",
		g_zeroflash_hcd->fw_status.need_app_config,
		g_zeroflash_hcd->fw_status.need_disp_config);
	if (!fw_status->need_app_config && !fw_status->need_disp_config) {
		/*
	...
	if (zeroflash_init_done) {
		if (zeroflash_check_uboot() < 0) {
			if (g_zeroflash_hcd->tcm_hcd->health_monitor_support) {
				g_zeroflash_hcd->tcm_hcd->monitor_data
					->reserve1++;
			}
			TPD_INFO("uboot check fail\n");
			msleep(50);
			enable_irq(g_zeroflash_hcd->tcm_hcd->s_client->irq);
			return;
		}

		queue_work(g_zeroflash_hcd->firmware_workqueue,
			   &g_zeroflash_hcd->firmware_work);
	} else {
	}
	return;
}
...
static int zeroflash_download_disp_config(void)
{
	...
	TPD_DETAIL("Downloading display config\n");

	image_info = &g_zeroflash_hcd->image_info;

	if (image_info->disp_config.size == 0) {
		TPD_INFO("No display config in image file\n");
		return -EINVAL;
	}

	LOCK_BUFFER(g_zeroflash_hcd->out);

	retval = syna_tcm_alloc_mem(tcm_hcd, &g_zeroflash_hcd->out,
	...
	retval = secure_memcpy(&g_zeroflash_hcd->out.buf[2],
			       ...
			       image_info->disp_config.size);
	if (retval < 0) {
		TPD_INFO("Failed to copy display config data\n");
		goto unlock_out;
	}
	...
	retval = tcm_hcd->write_message(
		tcm_hcd, CMD_DOWNLOAD_CONFIG, g_zeroflash_hcd->out.buf,
		g_zeroflash_hcd->out.data_length, &g_zeroflash_hcd->resp.buf,
		&g_zeroflash_hcd->resp.buf_size,
		&g_zeroflash_hcd->resp.data_length, &response_code, 0);
	if (retval < 0) {
	...
	retval = secure_memcpy((unsigned char *)&g_zeroflash_hcd->fw_status,
			       ...
			       sizeof(g_zeroflash_hcd->fw_status));
	TPD_INFO(
		"zeroflash_download_disp_config fw status:need app:%d need display:%d\n",
		g_zeroflash_hcd->fw_status.need_app_config,
		g_zeroflash_hcd->fw_status.need_disp_config);

	if (retval < 0) {
		TPD_INFO("Failed to copy firmware status\n");
		goto unlock_resp;
	}
...
static int zeroflash_download_app_config(void)
{
	...
	if (image_info->app_config.size == 0) {
		TPD_INFO("No application config in image file\n");
		return -EINVAL;
	}
	...
	retval = secure_memcpy(&g_zeroflash_hcd->out.buf[2],
			       ...
			       image_info->app_config.size);
	if (retval < 0) {
		TPD_INFO("Failed to copy application config data\n");
		goto unlock_out;
	}
	...
	TPD_INFO(
		"zeroflash_download_app_config fw status:need app:%d need display:%d\n",
		g_zeroflash_hcd->fw_status.need_app_config,
		g_zeroflash_hcd->fw_status.need_disp_config);
...
static void zeroflash_download_config_work(struct work_struct *work)
{
	...
	if (retval < 0) {
		TPD_INFO("Failed to get firmware image\n");
		return;
	}
	...
	if (g_zeroflash_hcd->fw_status.need_app_config) {
		...
		if (retval < 0) {
			...
			TPD_INFO("Failed to download application config\n");
			return;
		}
		goto exit;
	}
	...
	if (g_zeroflash_hcd->fw_status.need_disp_config) {
		...
		if (retval < 0) {
			...
			TPD_INFO("Failed to download display config\n");
			return;
		}
		goto exit;
	}
...
static int zeroflash_download_app_fw(void)
{
	...
	if (image_info->app_firmware.size == 0) {
		TPD_INFO("No application firmware in image file\n");
		return -EINVAL;
	}
	...
	retval = secure_memcpy(g_zeroflash_hcd->out.buf,
			       ...
			       image_info->app_firmware.size);
	if (retval < 0) {
		TPD_INFO("Failed to copy application firmware data\n");
		UNLOCK_BUFFER(g_zeroflash_hcd->out);
		return retval;
	}
...
retry_app_download:
...
#endif
#endif
	retval = syna_tcm_td4320_nf_rmi_write(
		tcm_hcd,
	...
	retval = syna_tcm_td4320_nf_rmi_write(
		...
		g_zeroflash_hcd->out.buf, g_zeroflash_hcd->out.data_length);
	if (retval < 0) {
		TPD_INFO("Failed to write application firmware data\n");
		UNLOCK_BUFFER(g_zeroflash_hcd->out);
		return retval;
	}
...
int zeroflash_download_firmware_directly(void *chip_data,
					 const struct firmware *fw)
{
	...
	if (retval < 0) {
		TPD_INFO("Microbootloader support unavailable\n");
		goto exit;
	}
	...
	retval = syna_tcm_td4320_nf_rmi_read(
		tcm_hcd, g_zeroflash_hcd->f35_addr.data_base,
		(unsigned char *)&data, sizeof(data));
	if (retval < 0) {
		TPD_INFO("Failed to read F$35 data\n");
		goto exit;
	}

	if (data.error_code != REQUESTING_FIRMWARE) {
		TPD_INFO("Microbootloader error code = 0x%02x\n",
			 data.error_code);
...
reload_fw:
	...
	if (retval < 0) {
		TPD_INFO("Failed to download application firmware\n");
		goto update_fail;
	}
...
exit:
	...
	if (DOWNLOAD_RETRY_COUNT && retry_count > DOWNLOAD_RETRY_COUNT) {
		...
		if (retval < 0) {
			...
			msleep(20);
			TPD_INFO(
				"something wrong happen, add hw reset try to download\n");
		}
		//msleep(20);
...
static int zeroflash_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (!g_zeroflash_hcd) {
		TPD_INFO("Failed to allocate memory for zeroflash_hcd\n");
		return -ENOMEM;
	}
