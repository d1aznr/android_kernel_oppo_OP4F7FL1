...
static int synaptics_get_chip_info(void *chip_data)
{
	...
	reg_info->F12_2D_CTRL32 =
		reg_info->F12_2D_CTRL_BASE + 0x0f; //moisture enable
	reg_info->F12_2D_DATA04 = reg_info->F12_2D_DATA_BASE + 1; //gesture type
	reg_info->F12_2D_DATA15 =
		reg_info->F12_2D_DATA_BASE + 3; //object attention
	reg_info->F12_2D_CMD00 = reg_info->F12_2D_CMD_BASE;
	reg_info->F12_2D_CTRL20 =
		reg_info->F12_2D_CTRL_BASE + 0x07; //motion suppression
	...
	reg_info->F51_CUSTOM_CTRL42 =
		reg_info->F51_CUSTOM_CTRL_BASE + 0x14; //0D Open Test,0x42D
	reg_info->F51_CUSTOM_CTRL50 = reg_info->F51_CUSTOM_CTRL_BASE +
				      0x0D; //edge function register 0x427,
				...
				struct syna_testdata *syna_testdata)
{
	...
	ret = touch_i2c_write_byte(
		...
		0x01); //Disable Noise Mitigation, F54_ANALOG_CTRL20
	TPD_DEBUG("forbid Forbid NoiseMitigation oK\n");

	ret = touch_i2c_read_byte(
		client,
		reg_info->F54_ANALOG_CONTROL_BASE +
			40); //Read Original CBC settings from F54_ANALOG_CTRL88
	tmp_arg1 = ret & 0xff;
	...
	TPD_DEBUG("ret = %x, tmp_arg1 = %x, tmp_arg2 = %x\n", ret, tmp_arg1,
		  (tmp_arg1 & 0xdf));
	ret = touch_i2c_write_byte(
		client, reg_info->F54_ANALOG_CONTROL_BASE + 40,
	...
	do {
		...
		if (enable_cbc) {
			...
			ret = touch_i2c_write_byte(
				client, reg_info->F54_ANALOG_CONTROL_BASE + 40,
				(tmp_arg1 | 0x20)); //Set CBC, F54_ANALOG_CTRL88
			...
			ret = touch_i2c_write_byte(
				client, reg_info->F54_ANALOG_CONTROL_BASE + 40,
				(tmp_arg1 & 0xdf)); //Set CBC, F54_ANALOG_CTRL88
			ret = touch_i2c_write_word(
				client, reg_info->F54_ANALOG_COMMAND_BASE,
				0x04); //force update
			checkCMD(chip_info);
	...
	ret = touch_i2c_write_byte(client, reg_info->F01_RMI_CMD_BASE,
				   0x01); //software reset TP
	msleep(100);
	ret = touch_i2c_write_byte(client, reg_info->F54_ANALOG_DATA_BASE,
	...
	checkCMD(chip_info);
	ret = touch_i2c_read_block(
		client, reg_info->F54_ANALOG_DATA_BASE + 3, 4,
		buffer); //for s3320 only need to check 32 Rx channels
...
static void synaptics_self_raw_read(struct seq_file *s, void *chip_data)
{
	...
	for (x = 0; x < 1; x++) {
		...
		seq_printf(s, "\n[%2d]", x);
		for (y = 0;
		     y < chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM;
		     y++) { //self Abs raw data
			ret = touch_i2c_read_byte(
				chip_info->client,
				chip_info->reg_info.F54_ANALOG_DATA_BASE + 3);
...
static fp_touch_state synaptics_spurious_fp_check(void *chip_data)
{
	...
	touch_i2c_write_byte(chip_info->client,
			     chip_info->reg_info.F01_RMI_CTRL00,
			     (uint8_t)(ret) | 0x80); //change to normal mode
	msleep(1);
