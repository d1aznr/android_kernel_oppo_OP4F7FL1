...
static int synaptics_get_chip_info(void *chip_data)
{
	...
	reg_info->F12_2D_CTRL32 =
		reg_info->F12_2D_CTRL_BASE + 0x10; //moisture enable
	...
	reg_info->F12_2D_CTRL27 =
		reg_info->F12_2D_CTRL_BASE + 0x0c; // wakeup Gesture enable
	...
	reg_info->F51_CUSTOM_CTRL50 =
		reg_info->F51_CUSTOM_CTRL_BASE +
		0x08; //edge function register, realy use F51_CUSTOM_CTRL30 register
				...
				struct syna_testdata *syna_testdata)
{
	...
	touch_i2c_read_block(
		client, reg_info->F54_ANALOG_DATA_BASE + 3,
		(syna_testdata->TX_NUM + syna_testdata->RX_NUM) * 2, raw_data);
	if (syna_testdata->fd >= 0) {
		sys_write(syna_testdata->fd, "\nstep5: check doze mode\n", 1);
	}
...
static void synaptics_abs_doze_read(struct seq_file *s, void *chip_data)
{
	...
	ret |= touch_i2c_write_byte(chip_info->client, 0x67,
				    0x01); //enter doze mode
	msleep(100);
