			...
			struct point_info end_point)
{
	...
	for (i = length; i > 0; i--) {
		memcpy(&swipes_record->start_points[i],
		       &swipes_record->start_points[i - 1],
		       sizeof(struct Coordinate));
		memcpy(&swipes_record->end_points[i],
		       &swipes_record->end_points[i - 1],
		       sizeof(struct Coordinate));
	}
...
{
	int i = 0;
	int length = points_record->count < RECORD_POINTS_COUNT ?
			     points_record->count :
			     RECORD_POINTS_COUNT;
...
{
	int i = 0;
	int length = swipes_record->count < RECORD_POINTS_COUNT ?
			     swipes_record->count :
			     RECORD_POINTS_COUNT;
	...
	for (i = 0; i < length; i++) {
		if (s) {
			seq_printf(s, "%sswipe[%d]:[%d %d] to [%d %d]\n",
				   prefix ? prefix : "", i,
				   swipes_record->start_points[i].x,
				   swipes_record->start_points[i].y,
				   swipes_record->end_points[i].x,
				   swipes_record->end_points[i].y);
		}
			    ...
			    value_record_type value_type)
{
	...
	list_for_each (pos, list) {
		...
		case TYPE_RECORD_STR:
			if (vc->value_type == value_type &&
			    !strcmp((char *)vc->value, value_str)) {
				vc->count++;
				TPD_DETAIL("%s str=%s, count=%d\n", __func__,
					   (char *)vc->value, vc->count);
				kfree(vc_value);
				return vc->count;
			}
			break;
	...
	if (vc) {
		...
		case TYPE_RECORD_STR:
			vc->value =
				kzalloc(sizeof(char) * (strlen(value_str) + 1),
					GFP_KERNEL);
			if (vc->value) {
				strncpy((char *)vc->value, value_str,
					strlen(value_str) + 1);
				vc->value_type = value_type;
				vc->count = 1;
				list_add_tail(&vc->head, list);
		      ...
		      bool need_feedback)
{
	...
	if (need_feedback) {
		tmp_str_feedback =
			kzalloc(linebreak * DEFAULT_CHILD_STR_LEN, GFP_KERNEL);
	}
	...
	for (i = 0; i < len; i++) {
		...
		snprintf(tmp_str_child, DEFAULT_CHILD_STR_LEN, "0x%02x, ",
			 tmp_uint8[i]);
		...
		if (i % linebreak == linebreak - 1) {
			...
			if (tmp_str_feedback && (i / linebreak == 0)) {
				strncpy(tmp_str_feedback, tmp_str,
					strlen(tmp_str));
			}
			memset(tmp_str, 0, linebreak * DEFAULT_CHILD_STR_LEN);
		}
	...
	list_for_each (pos, list) {
		...
		if (!list_num) {
			...
			if (vc->value) {
				...
				vc->count = recordlen;
				TPD_DETAIL("len = %d\n", vc->count);
				feedback = print_as_matrix(
					NULL, vc->value, vc->count,
			...
			vc->count = pre_count;
			TPD_DETAIL("len = %d\n", vc->count);
			print_as_matrix(NULL, vc->value, vc->count,
					DEFAULT_BUF_MATRIX_LINEBREAK, false);
		}
		list_num++;
	}
...
void print_buffer_list(struct seq_file *s, struct list_head *list, char *prefix)
{
	...
	list_for_each (pos, list) {
		...
		if (s) {
			seq_printf(s, "%s%d:len=%d, ", prefix ? prefix : "",
				   i++, vc->count);
		}
		print_as_matrix(s, vc->value, vc->count,
				DEFAULT_BUF_MATRIX_LINEBREAK, false);
	}
}
...
int catch_delta_data(struct monitor_data_v2 *monitor_data, int32_t *delta_data)
{
	...
	memset(delta_data, 0,
	       sizeof(int32_t) * monitor_data->TX_NUM * monitor_data->RX_NUM);
	debug_info_ops->get_delta_data(monitor_data->chip_data, delta_data);

	print_delta_data(NULL, delta_data, monitor_data->TX_NUM,
			 monitor_data->RX_NUM);

	return 0;
}

int point_state_up_handle(struct monitor_data_v2 *monitor_data, int i,
			  int direction)
{
	...
	if (monitor_data->points_state[i].down_up_state == STATE_DOWN) {
		...
		if (monitor_data->points_state[i].max_swipe_distance_sq == 0) {
			monitor_data->points_state[i].touch_action =
				ACTION_CLICK;
			monitor_data->click_count++;
			if ((monitor_data->points_state[i].last_point.y <
			     monitor_data->max_y) &&
			    (monitor_data->points_state[i].last_point.x <
			     monitor_data->max_x)) {
				monitor_data->click_count_array
					[monitor_data->points_state[i]
							 .last_point.y *
						 CLICK_COUNT_ARRAY_HEIGHT /
						 monitor_data->max_y *
...
int tp_touch_healthinfo_handle(struct monitor_data_v2 *monitor_data,
			       int obj_attention, struct point_info *points,
			       int finger_num, int max_num, int direction)
{
	int i = 0, j = 0;
	...
	if (!monitor_data || !monitor_data->points_state ||
	    (!points && finger_num)) {
		return -1;
	}
	...
	if (finger_num) {
		while (points_handled < finger_num && i < max_num) {
			...
			if (monitor_data->points_state[i].down_up_state ==
				    STATE_DOWN &&
			    !is_point_reporting(obj_attention, points, i)) {
				point_state_up_handle(monitor_data, i,
						      direction);
			} else if (is_point_reporting(obj_attention, points,
						      i)) {
				if (monitor_data->points_state[i]
					    .down_up_state == STATE_UP) {
					is_first_point = true;
					monitor_data->points_state[i]
						.jumping_times++;
					monitor_data->points_state[i]
						.down_up_state = STATE_DOWN;
					memcpy(&monitor_data->points_state[i]
							.first_point,
					       &points[i],
					...
					if (monitor_data->points_state[i]
					    ...
					    !check_healthinfo_time_counter_timeout(
						    ...
						    monitor_data->in_game_mode ?
							    ...
							    (SWIPE_BROKEN_FRAMES_MS /
							     monitor_data
								     ->report_rate))) { //up and down in short time(2.5 frames) and short distance. judge as broken swipe
					...
					if (monitor_data->points_state[i]
					    ...
					    !check_healthinfo_time_counter_timeout(
						    ...
						    monitor_data->in_game_mode ?
							    ...
							    (JUMPING_POINT_FRAMES_MS /
							     monitor_data
								     ->report_rate))) { //time between last click and this down less than 1.5 frames, and distance less than 20px, judge as jumping point
						    ...
						    int_sqrt(dist_sq) -
								    int_sqrt(
									    monitor_data
										    ...
										    .last_swipe_distance_sq) >
							    monitor_data
								    ->long_swipe_judge_distance) { //distance between points changed sunddently when swiping
							add_swipe_to_record(
								&monitor_data
									 ->long_swipes,
								monitor_data
				...
				if (monitor_data->points_state[i].jumping_times >
					...
					if (!monitor_data->points_state[i]
						...
						catch_delta_data(
							monitor_data,
							monitor_data
								->jumping_point_delta_data); //catch delta data
						monitor_data->points_state[i]
							.is_down_handled = true;
					}
					if (monitor_data->points_state[i]
						    .jumping_times >
				...
				if (!monitor_data->points_state[i]
					...
					catch_delta_data(
						monitor_data,
						monitor_data
							->stuck_point_delta_data); //coordinate not change over 15s, catch delta data
					/*
	...
	if (finger_num != monitor_data->current_touch_num) {
		if (monitor_data->current_touch_num) {
			touch_time = check_healthinfo_time_counter_timeout(
				monitor_data->touch_timer, 0);
			if (monitor_data->in_game_mode) {
				monitor_data->total_touch_time_in_game[0] +=
					touch_time;
				monitor_data->total_touch_time_in_game
					[monitor_data->current_touch_num] +=
					touch_time;
			} else {
			...
			if (!finger_num) {
				if (monitor_data->holding_touch_time >
				    monitor_data->max_holding_touch_time) {
					monitor_data->max_holding_touch_time =
						monitor_data->holding_touch_time;
				}
...
int tp_grip_up_healthinfo_handle(struct monitor_data_v2 *monitor_data,
				 uint8_t up_id, int direction)
{
	int ret = 0;
...
int tp_gesture_healthinfo_handle(struct monitor_data_v2 *monitor_data,
				 int gesture_type)
{
	if (!monitor_data) {
		return 0;
	...
	if (monitor_data->is_gesture_waiting_read) {
		update_value_count_list(
			&monitor_data->invalid_gesture_values_list,
				      ...
				      int gesture_type)
{
	...
	if (monitor_data->is_gesture_waiting_read) {
		if (check_healthinfo_time_counter_timeout(
			    monitor_data->gesture_received_time,
			    GESTURE_RESPONSE_TIME) ||
		    gesture_type != monitor_data->gesture_waiting) {
			update_value_count_list(
				&monitor_data->invalid_gesture_values_list,
				&monitor_data->gesture_waiting,
				TYPE_RECORD_INT);

		} else {
			update_value_count_list(
				&monitor_data->gesture_values_list,
				&monitor_data->gesture_waiting,
				TYPE_RECORD_INT);
		}
...
int tp_fingerprint_healthinfo_handle(struct monitor_data_v2 *monitor_data,
				     uint8_t area_rate)
{
	int rate_rate = 0;
	...
	return update_value_count_list(&monitor_data->fp_area_rate_list,
				       &rate_rate, TYPE_RECORD_INT);
	;
}

int tp_face_detect_healthinfo_handle(struct monitor_data_v2 *monitor_data,
				     int ps_state)
{
	if (!ps_state) {
		return 0;
...
int tp_report_healthinfo_handle(struct monitor_data_v2 *monitor_data,
				char *report)
{
	int ret = 0;
...
void reset_healthinfo_grip_time_record(void *tp_monitor_data,
				       void *tp_grip_info)
{
	struct monitor_data_v2 *monitor_data =
		(struct monitor_data_v2 *)tp_monitor_data;
...
int tp_probe_healthinfo_handle(struct monitor_data_v2 *monitor_data,
			       u64 start_time)
{
	if (!monitor_data) {
		return 0;
	...
	monitor_data->probe_time =
		check_healthinfo_time_counter_timeout(start_time, 0);
...
int update_max_time(struct monitor_data_v2 *monitor_data, u64 *max_time,
		    u64 start_time)
{
	u64 time_cost = 0;
...
int tp_resume_healthinfo_handle(struct monitor_data_v2 *monitor_data,
				u64 start_time)
{
	if (!monitor_data) {
		return 0;
	...
	update_max_time(monitor_data, &monitor_data->max_resume_time,
			start_time);
...
int tp_suspend_healthinfo_handle(struct monitor_data_v2 *monitor_data,
				 u64 start_time)
{
	u64 screenon_time = 0;
	...
	tp_touch_healthinfo_handle(monitor_data, 0, monitor_data->touch_points,
				   0, monitor_data->max_finger_support,
				   monitor_data->direction);

	update_max_time(monitor_data, &monitor_data->max_suspend_time,
			start_time);
...
int tp_test_healthinfo_handle(struct monitor_data_v2 *monitor_data,
...
int tp_bus_err_healthinfo_handle(struct monitor_data_v2 *monitor_data,
				 int errno, unsigned char *writebuf,
				 uint16_t writelen)
{
	char *report = NULL;
	char *buff_part = NULL;
	...
	buff_part =
		record_buffer_data(&monitor_data->bus_errs_buff_list, writebuf,
				   writelen, MAX_BUS_ERROR_BUFF_RECORD);

	if (buff_part) {
		report = kzalloc(DEFAULT_REPORT_STR_LEN + strlen(buff_part) + 1,
				 GFP_KERNEL);
		if (report) {
			snprintf(report, DEFAULT_REPORT_STR_LEN,
				 "BusErr$$Errno@@%d$$Buff@@%s", errno,
				 buff_part);
			upload_touchpanel_kevent_data(report);
			kfree(report);
		} else {
...
int tp_alloc_healthinfo_handle(struct monitor_data_v2 *monitor_data,
			       long alloc_size, bool alloc_success)
{
	...
	if (alloc_success) {
		...
		if (!monitor_data->max_alloc_err_size &&
		...
		if (report) {
			...
			if (func) {
				...
				snprintf(report, DEFAULT_REPORT_STR_LEN, "%pS",
					 (void *)frame.pc);
				strncpy(func, report,
					strstr(report, "+") - report);
				update_value_count_list(
					&monitor_data->alloc_err_funcs_list,
					func, TYPE_RECORD_STR);
			}
			kfree(report);
			kfree(func);
		} else {
...
int tp_fw_update_healthinfo_handle(struct monitor_data_v2 *monitor_data,
				   char *result)
{
	int ret = 0;
	char *report = NULL;
	...
	ret = update_value_count_list(&monitor_data->fw_update_result_list,
				      result, TYPE_RECORD_STR);

	if (!strstr(result, FIRMWARE_UPDATE_SUCCESS_KEYWORD)) {
		report = kzalloc(DEFAULT_REPORT_STR_LEN, GFP_KERNEL);
		if (report) {
			snprintf(report, DEFAULT_REPORT_STR_LEN,
				 "FwUpdateErr$$ErrMsg@@%s", result);
			upload_touchpanel_kevent_data(report);
			kfree(report);
		}
	}
			 ...
			 void *value)
{
	...
	case HEALTH_TOUCH:
		if (monitor_data->touch_points) {
			ret = tp_touch_healthinfo_handle(
				monitor_data, *value_int,
				monitor_data->touch_points,
				monitor_data->touch_num,
	...
	case HEALTH_GESTURE_READ:
		ret = tp_gesture_read_healthinfo_handle(monitor_data,
							*value_int);
		break;
	case HEALTH_FINGERPRINT:
		ret = tp_fingerprint_healthinfo_handle(monitor_data,
						       *value_uint8);
		break;
	case HEALTH_FACE_DETECT:
		ret = tp_face_detect_healthinfo_handle(monitor_data,
						       *value_int);
		break;
	case HEALTH_REPORT:
	...
	case HEALTH_BUS:
		if (monitor_data->bus_buf) {
			ret = tp_bus_err_healthinfo_handle(
				monitor_data, *value_int, monitor_data->bus_buf,
				monitor_data->bus_len);
			monitor_data->bus_buf = NULL;
		}
		break;
	case HEALTH_ALLOC_SUCCESS:
		ret = tp_alloc_healthinfo_handle(monitor_data, *value_long,
						 true);
		break;
	...
	case HEALTH_FW_UPDATE_COST:
		ret = update_max_time(monitor_data,
				      &monitor_data->max_fw_update_time,
				      *value_u64);
		break;
	case HEALTH_GRIP_UP:
...
int tp_healthinfo_read(struct seq_file *s, void *tp_monitor_data)
{
	...
	if (monitor_data->max_touch_num_in_game) {
		seq_printf(
			...
			monitor_data->total_touch_time_in_game[0] /
				MS_PER_SECOND,
			monitor_data->total_touch_time_in_game[1] /
				MS_PER_SECOND,
			monitor_data->total_touch_time_in_game[2] /
				MS_PER_SECOND,
			monitor_data->total_touch_time_in_game[3] /
				MS_PER_SECOND,
			monitor_data->total_touch_time_in_game[4] /
				MS_PER_SECOND,
			monitor_data->total_touch_time_in_game[5] /
				MS_PER_SECOND,
			monitor_data->total_touch_time_in_game[6] /
				MS_PER_SECOND,
			monitor_data->total_touch_time_in_game[7] /
				MS_PER_SECOND,
			monitor_data->total_touch_time_in_game[8] /
				MS_PER_SECOND,
			monitor_data->total_touch_time_in_game[9] /
				MS_PER_SECOND,
			monitor_data->total_touch_time_in_game[10] /
				MS_PER_SECOND);
	}
	...
	print_delta_data(s, monitor_data->click_count_array,
			 CLICK_COUNT_ARRAY_HEIGHT, CLICK_COUNT_ARRAY_WIDTH);
	...
	if (monitor_data->fw_version) {
		seq_printf(s,
			   ...
			   monitor_data->fw_version,
			   monitor_data->max_fw_update_time);
	}
	print_value_count_list(s, &monitor_data->fw_update_result_list,
			       TYPE_RECORD_STR, PREFIX_FW_UPDATE_RESULT);
	...
	if (monitor_data->min_alloc_err_size) {
		seq_printf(s, "alloc_error_size:%ld~%ld\n",
			   monitor_data->min_alloc_err_size,
			   monitor_data->max_alloc_err_size);
		print_value_count_list(s, &monitor_data->alloc_err_funcs_list,
				       TYPE_RECORD_STR, PREFIX_ALLOC_ERR_FUNC);
	}

	//debug info
	print_value_count_list(s, &monitor_data->health_report_list,
			       TYPE_RECORD_STR, PREFIX_HEALTH_REPORT);
	...
	if (monitor_data->max_jumping_times > JUMPING_POINT_TIMES) {
		...
		if (!is_delta_data_allzero(
			    monitor_data->jumping_points_count_array,
			    monitor_data->RX_NUM / 2,
			    monitor_data->TX_NUM / 2)) {
			seq_printf(s, "%spoint:", PREFIX_POINT_JUMPING);
			print_delta_data(
				s, monitor_data->jumping_points_count_array,
				monitor_data->RX_NUM / 2,
				monitor_data->TX_NUM / 2);
		}
		if (!is_delta_data_allzero(
	...
	if (!is_delta_data_allzero(monitor_data->stuck_points_count_array,
		...
		print_delta_data(s, monitor_data->stuck_points_count_array,
				 monitor_data->RX_NUM / 2,
				 monitor_data->TX_NUM / 2);
	}
	if (!is_delta_data_allzero(
		...
		print_delta_data(
			s, monitor_data->lanscape_stuck_points_count_array,
			monitor_data->RX_NUM / 2, monitor_data->TX_NUM / 2);
	}
	if (!is_delta_data_allzero(monitor_data->stuck_point_delta_data,
		...
		print_delta_data(s, monitor_data->stuck_point_delta_data,
				 monitor_data->TX_NUM, monitor_data->RX_NUM);
	}
	...
	if (vc_value) {
		...
		list_for_each (pos, &monitor_data->gesture_values_list) {
			...
			seq_printf(s, "%s%s:%d\n", PREFIX_GESTURE,
				   ...
				   *vc_value == Mgestrue    ? "(M)" :
				   *vc_value == Wgestrue    ? "(W)" :
				   *vc_value == FingerprintDown ?
							   "(fingerprintdown)" :
				   *vc_value == FingerprintUp ?
							   "(fingerprintup)" :
	...
	print_value_count_list(s, &monitor_data->fp_area_rate_list,
			       TYPE_RECORD_INT, PREFIX_FP_AREA_RATE);

	//face detect
	print_value_count_list(s, &monitor_data->fd_values_list,
			       TYPE_RECORD_INT, PREFIX_FD);
...
int tp_healthinfo_init(struct device *dev, void *tp_monitor_data)
{
	...
	TPD_INFO(
		"long_swipe_judge_distance=%d swipe_broken_judge_distance=%d jumping_point_judge_distance=%d\n",
		monitor_data->long_swipe_judge_distance,
		monitor_data->swipe_broken_judge_distance,
		monitor_data->jumping_point_judge_distance);
	...
	ret = of_property_read_string(dev->of_node, "chip-name",
				      &monitor_data->tp_ic);
	if (ret) {
	...
	monitor_data->jumping_point_delta_data = kzalloc(
		sizeof(int32_t) * monitor_data->TX_NUM * monitor_data->RX_NUM,
		GFP_KERNEL);
	...
	monitor_data->stuck_point_delta_data = kzalloc(
		sizeof(int32_t) * monitor_data->TX_NUM * monitor_data->RX_NUM,
		GFP_KERNEL);
	...
	monitor_data->points_state =
		kzalloc(sizeof(struct point_state_monitor) *
				monitor_data->max_finger_support,
			GFP_KERNEL);
	if (!monitor_data->points_state) {
	...
	monitor_data->click_count_array =
		kzalloc(sizeof(int32_t) * CLICK_COUNT_ARRAY_HEIGHT *
				CLICK_COUNT_ARRAY_WIDTH,
			GFP_KERNEL);
	...
	monitor_data->jumping_points_count_array =
		kzalloc(sizeof(int32_t) * (monitor_data->TX_NUM / 2) *
				(monitor_data->RX_NUM / 2),
			GFP_KERNEL);
	...
	monitor_data->total_touch_time_in_game =
		kzalloc(sizeof(u64) * (monitor_data->max_finger_support + 1),
			GFP_KERNEL);
