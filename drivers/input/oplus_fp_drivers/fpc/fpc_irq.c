...
int fpc_power_on(struct fpc1020_data *fpc_dev)
{
	...
	for (index = 0; index < fpc_dev->power_num; index++) {
		...
		case FP_POWER_MODE_LDO:
			rc = vreg_setup(fpc_dev, &(fpc_dev->pwr_list[index]),
					true);
...
int fpc_power_off(struct fpc1020_data *fpc_dev)
{
	...
	for (index = 0; index < fpc_dev->power_num; index++) {
		...
		case FP_POWER_MODE_GPIO:
			gpio_set_value(
				fpc_dev->pwr_list[index].pwr_gpio,
				(fpc_dev->pwr_list[index].poweron_level == 0 ?
					 1 :
					 0));
			pr_info("set pwr_gpio %d\n",
				(fpc_dev->pwr_list[index].poweron_level == 0 ?
				   ...
				   const char *buffer, size_t count)
{
	...
	if (1 == sscanf(buffer, "%d", &op)) {
		if (op == WAKELOCK_ENABLE) {
			__pm_wakeup_event(&fpc1020->fpc_wl,
					  FPC_WL_WAKELOCK_TIMEOUT);
...
static int fpc1020_irq_probe(struct platform_device *pldev)
{
	...
	if (rc) {
		dev_err(dev,
			"failed to request fpc,use_gpio_power_enable, ret = %d\n",
			rc);
		g_use_gpio_power_enable = 0;
	}
	...
	if (g_use_gpio_power_enable == 1) {
		...
		fpc1020->pstate_cs_func =
			pinctrl_lookup_state(fpc1020->pinctrl, "fpc_cs_func");
		if (IS_ERR(fpc1020->pstate_cs_func)) {
			dev_err(&pldev->dev,
				"Can't find fpc_cs_func pinctrl state\n");
	...
	if (g_use_gpio_power_enable == 1) {
		/*get cs resource*/
		fpc1020->cs_gpio =
			of_get_named_gpio(pldev->dev.of_node, "fpc,gpio_cs", 0);
		...
		if (fpc1020->cs_gpio_set) {
			dev_info(
				fpc1020->dev,
				"---- pull CS up and set CS from gpio to func ----");
			gpio_set_value(fpc1020->cs_gpio, 1);
			pinctrl_select_state(fpc1020->pinctrl,
					     fpc1020->pstate_cs_func);
			fpc1020->cs_gpio_set = false;
		}
	}
