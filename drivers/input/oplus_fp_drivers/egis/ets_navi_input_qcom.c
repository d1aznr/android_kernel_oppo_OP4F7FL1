...
enum { KEY_RELEASE, KEY_PRESS, KEY_PRESS_RELEASE };
...
static void send_key_event(struct egistec_data *egistec, unsigned int code,
			   int value)
{
...
static void long_touch_handler(unsigned long arg)
{
	...
	if (g_KeyEventRaised == false) {
		...
		/* Long touch event */
		send_key_event(egistec, KEYEVENT_LONGTOUCH,
			       KEYEVENT_LONGTOUCH_ACTION);
	}
}
#endif
...
void translated_command_converter(char cmd, struct egistec_data *egistec)
{
	...
	case NAVI_EVENT_ON: /* finger down */
		...
		mod_timer(&long_touch_timer,
			  jiffies + (HZ * LONGTOUCH_INTERVAL / 1000));
#endif
		break;
		...
		if (g_KeyEventRaised == false) {
			...
			pr_info("Egis : g_SingleClick %u tap interval =%u double tap interval = %u time= %u",
				...
				jiffies_to_msecs(jiffies -
						 g_SingleClickJiffies),
				jiffies_to_msecs(jiffies -
						 g_DoubleClickJiffies),
				jiffies_to_msecs(jiffies));
#if ENABLE_TRANSLATED_SINGLE_CLICK
			if ((jiffies - g_SingleClickJiffies) <
			    (HZ * SINGLECLICK_INTERVAL / 1000)) {
				/* Click event */
				send_key_event(egistec, KEYEVENT_CLICK,
					       KEYEVENT_CLICK_ACTION);
				g_SingleClick++;
				if (g_SingleClick == 1) {
			...
			if (g_SingleClick >= 2) {
				if ((jiffies - g_DoubleClickJiffies) <
				    (HZ *
				     (SINGLECLICK_INTERVAL +
				      DOUBLECLICK_INTERVAL) /
				     1000)) {
					/* Double click event */
					send_key_event(
						egistec, KEYEVENT_DOUBLECLICK,
						KEYEVENT_DOUBLECLICK_ACTION);
					g_SingleClick = 0;
				} else {
					g_SingleClick = 1;
					g_DoubleClickJiffies = jiffies;
...
#if ENABLE_FINGER_DOWN_UP
			send_key_event(egistec, KEYEVENT_OFF,
				       KEYEVENT_OFF_ACTION);
#endif
		}
#if ENABLE_FINGER_DOWN_UP
	...
	case NAVI_EVENT_UP:
		if (g_KeyEventRaised == false) {
			...
			send_key_event(egistec, KEYEVENT_UP,
				       KEYEVENT_UP_ACTION);
			send_key_event(egistec, KEYEVENT_OFF,
				       KEYEVENT_OFF_ACTION);
	...
	case NAVI_EVENT_DOWN:
		...
		if (g_KeyEventRaised == false) {
			g_KeyEventRaised = true;
#if ENABLE_SWIPE_UP_DOWN
			send_key_event(egistec, KEYEVENT_DOWN,
				       KEYEVENT_DOWN_ACTION);
			send_key_event(egistec, KEYEVENT_OFF,
				       KEYEVENT_OFF_ACTION);
	...
	case NAVI_EVENT_RIGHT:
		...
		if (g_KeyEventRaised == false) {
			g_KeyEventRaised = true;
			send_key_event(egistec, KEYEVENT_RIGHT,
				       KEYEVENT_RIGHT_ACTION);
			send_key_event(egistec, KEYEVENT_OFF,
				       KEYEVENT_OFF_ACTION);
		}
#endif
	...
	case NAVI_EVENT_LEFT:
		...
		if (g_KeyEventRaised == false) {
			g_KeyEventRaised = true;
			send_key_event(egistec, KEYEVENT_LEFT,
				       KEYEVENT_LEFT_ACTION);
			send_key_event(egistec, KEYEVENT_OFF,
				       KEYEVENT_OFF_ACTION);
		}
#endif
...
void straight_command_converter(char cmd, struct egistec_data *egistec)
{
	...
	case NAVI_EVENT_CANCEL:
		...
		send_key_event(egistec, KEYEVENT_CANCEL,
			       KEYEVENT_CANCEL_ACTION);
#endif
...
{
	struct navi_struct *command =
		container_of(work, struct navi_struct, workq);
					      ...
					      char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "%s",
			 egistec->property_navigation_enable ? "enable" :
							       "disable");
}
 ...
 *  Sysfs node creation
 */
static struct attribute *attributes[] = { &dev_attr_navigation_event.attr,
...
static int nav_input_thread(void *et_spi)
{
	...
	while (1) {
		wait_event_interruptible(
			nav_input_wait, nav_input_sig || kthread_should_stop());
		mutex_lock(&driver_mode_lock);
...
void uinput_egis_init(struct egistec_data *egistec)
{
	...
	egistec->property_navigation_enable =
		PROPERTY_NAVIGATION_ENABLE_DEFAULT;
	...
	nav_input_sig = 0;
	if (!nav_kthread) {
		nav_kthread = kthread_run(nav_input_thread, (void *)egistec,
					  "nav_thread");
	}
#if ENABLE_TRANSLATED_LONG_TOUCH
