...
int Interrupt_Init(struct egistec_data *egistec, int int_mode,
		   int detect_period, int detect_threshold)
{
	...
	DEBUG_PRINT(
		"FP --  %s request_irq_done = %d gpio_irq = %d  pin = %d  \n",
	...
	if (request_irq_done == 0) {
		...
		DEBUG_PRINT(
			"[Interrupt_Init] flag current: %d disable: %d enable: %d\n",
		...
		if (int_mode == EDGE_TRIGGER_RISING) {
			...
			err = request_irq(gpio_irq, fp_eint_func_ll,
					  IRQ_TYPE_LEVEL_LOW, "fp_detect-eint",
					  egistec);
			if (err) {
				pr_err("request_irq failed==========%s,%d\n",
				       __func__, __LINE__);
			...
			err = request_irq(gpio_irq, fp_eint_func_ll,
					  IRQ_TYPE_LEVEL_HIGH, "fp_detect-eint",
					  egistec);
			if (err) {
				pr_err("request_irq failed==========%s,%d\n",
				       __func__, __LINE__);
...
unsigned int fps_interrupt_poll(struct file *file,
...
static ssize_t egistec_read(struct file *filp, char __user *buf, size_t count,
			    loff_t *f_pos)
{
...
static ssize_t egistec_write(struct file *filp, const char __user *buf,
...
static long egistec_ioctl(struct file *filp, unsigned int cmd,
			  unsigned long arg)
{
	int retval = 0;
	...
	case INT_TRIGGER_INIT:
		...
		retval = Interrupt_Init(egistec, data.int_mode,
					data.detect_period,
...
#ifdef CONFIG_COMPAT
static long egistec_compat_ioctl(struct file *filp, unsigned int cmd,
				 unsigned long arg)
{
...
*/
		if (g_egis_pmic_disable == 1) {
			finger_regulator =
				devm_regulator_get(&pdev->dev, "vmch");
			...
			status = regulator_set_voltage(finger_regulator,
						       3300000, 3300000);
			...
			status = pinctrl_select_state(egistec->pinctrl_gpios,
						      egistec->pins_power_high);
			mdelay(2);
			if (!status) {
				DEBUG_PRINT(
					"%s successful to select power active state\n",
					__func__);
			} else {
				DEBUG_PRINT(
					"%s Failed to select power active state=%d\n",
					__func__, status);
				goto err;
			}
		}
	...
	if (g_spi_cs_mode == 1) {
		if (egistec->pins_spi_cs_mode) {
			pinctrl_select_state(egistec->pinctrl_gpios,
					     egistec->pins_spi_cs_mode);
		}
		pinctrl_select_state(egistec->pinctrl_gpios,
				     egistec->pins_reset_high);
	}
...
static int egistec_parse_dt(struct device *dev, struct egistec_data *data)
{
...
*/
			rc = of_property_read_u32(node, "egis,pmic_disable",
						  &g_egis_pmic_disable);
			if (rc) {
				dev_err(&pdev->dev,
					"failed to request egis,egis_pmic_disable, ret = %d\n",
					rc);
				g_egis_pmic_disable = 0;
			}

			rc = of_property_read_u32(node, "egis,spi_cs_mode",
						  &g_spi_cs_mode);
			if (rc) {
				dev_err(&pdev->dev,
					"failed to request egis,spi_cs_mode, ret = %d\n",
					rc);
				g_spi_cs_mode = 0;
			}

			data->pins_reset_high = pinctrl_lookup_state(
				data->pinctrl_gpios, "rst-high");
			...
			data->pins_reset_low = pinctrl_lookup_state(
				data->pinctrl_gpios, "rst-low");
			...
			if (g_egis_pmic_disable != 1) {
				data->pins_power_high = pinctrl_lookup_state(
					data->pinctrl_gpios, "power_high");
				if (IS_ERR(data->pins_power_high)) {
					ret = PTR_ERR(data->pins_power_high);
					printk(KERN_ERR
					       "%s can't find fingerprint pinctrl power_high\n",
					       __func__);
					goto pinctrl_err;
				}
				data->pins_power_low = pinctrl_lookup_state(
					data->pinctrl_gpios, "power_low");
				if (IS_ERR(data->pins_power_low)) {
					ret = PTR_ERR(data->pins_power_low);
					printk(KERN_ERR
					       "%s can't find fingerprint pinctrl power_low\n",
					       __func__);
					goto pinctrl_err;
				}
			}
