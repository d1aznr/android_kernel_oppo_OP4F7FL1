...
void egis_irq_enable(bool irq_wake, bool enable)
{
	...
	DEBUG_PRINT(
		"irq_type = %d, g_data->irq_enable_flag = %d,desc->depth = %d\n",
...
int Interrupt_Init(struct egistec_data *etspi, int int_mode, int detect_period,
		   int detect_threshold)
{
	int err = 0;
	int status = 0;
...
static ssize_t etspi_read(struct file *filp, char __user *buf, size_t count,
			  loff_t *f_pos)
{
...
static ssize_t etspi_write(struct file *filp, const char __user *buf,
...
static struct attribute *attributes[] = { &dev_attr_etspi_enable.attr, NULL };
...
static long etspi_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case INT_TRIGGER_INIT:
		...
		retval =
			Interrupt_Init(etspi, data.int_mode, data.detect_period,
...
#ifdef CONFIG_COMPAT
static long etspi_compat_ioctl(struct file *filp, unsigned int cmd,
			       unsigned long arg)
{
...
static int etspi_parse_dt(struct device *device, struct egistec_data *data)
{
...
static const struct file_operations etspi_fops = { .owner = THIS_MODULE,
						   .write = etspi_write,
						   .read = etspi_read,
						   .unlocked_ioctl =
							   etspi_ioctl,
