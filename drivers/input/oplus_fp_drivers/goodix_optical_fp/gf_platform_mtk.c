...
void gf_cleanup_pwr_list(struct gf_dev *gf_dev)
{
	...
	for (index = 0; index < gf_dev->power_num; index++) {
		...
		if (gf_dev->pwr_list[index].pwr_type == FP_POWER_MODE_LDO)
			memset(&(gf_dev->pwr_list[index]), 0,
			       sizeof(fp_power_info_t));
	}
}
...
int gf_parse_dts(struct gf_dev *gf_dev)
{
	...
	if (FP_GOODIX_5658 == get_fpsensor_type() ||
	    ...
	    FP_GOODIX_3636 ==
		    get_fpsensor_type()) { //add new fpsensor if needed
	...
	rc = of_property_read_u32(node, "gf,cs_gpio_disable",
				  &g_cs_gpio_disable);
	if (rc) {
		dev_err(&pdev->dev,
			"failed to request gf,cs_gpio_disable, ret = %d\n", rc);
		g_cs_gpio_disable = 0;
	}
	...
	if (g_cs_gpio_disable != 1) {
		gf_dev->pstate_cs_func =
			pinctrl_lookup_state(gf_dev->pinctrl, "gf_cs_func");
		if (IS_ERR(gf_dev->pstate_cs_func)) {
			dev_err(&pdev->dev,
				"Can't find gf_cs_func pinctrl state\n");
			return PTR_ERR(gf_dev->pstate_cs_func);
		}
	}

	if (gf_dev->is_optical == true) {
		gf_dev->pstate_spi_6mA = pinctrl_lookup_state(
			gf_dev->pinctrl, "gf_spi_drive_6mA");
		if (IS_ERR(gf_dev->pstate_spi_6mA)) {
			dev_err(&pdev->dev,
				"Can't find gf_spi_drive_6mA pinctrl state\n");
			return PTR_ERR(gf_dev->pstate_spi_6mA);
		}
		pinctrl_select_state(gf_dev->pinctrl, gf_dev->pstate_spi_6mA);

		gf_dev->pstate_default =
			pinctrl_lookup_state(gf_dev->pinctrl, "default");
		if (IS_ERR(gf_dev->pstate_default)) {
			dev_err(&pdev->dev,
				"Can't find default pinctrl state\n");
			return PTR_ERR(gf_dev->pstate_default);
		}
		pinctrl_select_state(gf_dev->pinctrl, gf_dev->pstate_default);
	} else {
		gf_dev->pstate_irq_no_pull = pinctrl_lookup_state(
			gf_dev->pinctrl, "goodix_irq_no_pull");
		if (IS_ERR(gf_dev->pstate_irq_no_pull)) {
			dev_err(&pdev->dev,
				"Can't find irq_no_pull pinctrl state\n");
			// return PTR_ERR(gf_dev->pstate_irq_no_pull);
		} else {
			pinctrl_select_state(gf_dev->pinctrl,
					     gf_dev->pstate_irq_no_pull);
		}
	}
	/*get reset resource*/
	gf_dev->reset_gpio =
		of_get_named_gpio(pdev->dev.of_node, "goodix,gpio_reset", 0);
	...
	/*get cs resource*/
	if (g_cs_gpio_disable != 1) {
		gf_dev->cs_gpio = of_get_named_gpio(pdev->dev.of_node,
						    "goodix,gpio_cs", 0);
		...
		if (rc) {
			dev_err(&gf_dev->spi->dev,
				"Failed to request CS GPIO. rc = %d\n", rc);
			return -1;
		}
	...
	/*get irq resourece*/
	gf_dev->irq_gpio =
		of_get_named_gpio(pdev->dev.of_node, "goodix,gpio_irq", 0);
	if (!gpio_is_valid(gf_dev->irq_gpio)) {
