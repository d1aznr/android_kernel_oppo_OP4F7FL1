...
static int cdfinger_free_gpio(struct cdfingerfp_data *cdfinger)
{
	...
	if (gpio_is_valid(cdfinger->irq_num)) {
		gpio_free(cdfinger->irq_num);
		if (isInit == 1) {
			free_irq(gpio_to_irq(cdfinger->irq_num),
				 (void *)cdfinger);
			isInit = 0;
		}
	}
...
#ifdef POWER_GPIO
	cdfinger->pwr_num =
		of_get_named_gpio(dev->of_node, "cdfinger,pwr_gpio", 0);
#endif
	cdfinger->reset_num =
...
static int cdfinger_init_irq(struct cdfingerfp_data *pdata)
{
	...
	error = request_irq(gpio_to_irq(pdata->irq_num), cdfinger_eint_handler,
			    IRQF_TRIGGER_RISING, "cdfinger_eint", NULL);
	if (error < 0) {
...
static void cdfinger_wake_lock(struct cdfingerfp_data *pdata, int arg)
{
	if (arg) {
		...
		wake_lock_timeout(&pdata->cdfinger_lock,
...
static int cdfinger_probe(struct platform_device *pdev)
{
	...
	for (idx = 0; idx < ARRAY_SIZE(maps); idx++) {
		input_set_capability(cdfingerdev->cdfinger_input,
				     maps[idx].type, maps[idx].code);
	}
