...
#define fpsensor_debug(level, fmt, args...)                                    \
	do {                                                                   \
...
#define FUNC_ENTRY()                                                           \
	fpsensor_debug(DEBUG_LOG, "%s, %d, entry\n", __func__, __LINE__)
#define FUNC_EXIT()                                                            \
	fpsensor_debug(DEBUG_LOG, "%s, %d, exit\n", __func__, __LINE__)
...
#define FPSENSOR_IOC_ENABLE_REPORT_BLANKON                                     \
	_IOWR(FPSENSOR_IOC_MAGIC, 21, uint32_t)
...
typedef struct {
	...
	struct pinctrl_state *eint_as_int, *fp_rst_low, *fp_rst_high,
		*fp_cs_mode, *fp_cs_as_gpio, *fp_spi_mo, *fp_spi_mi,
		*fp_spi_clk, *fp_mo_low, *fp_mi_low, *fp_cs_low, *fp_clk_low,
		*fp_pwr_high, *fp_pwr_low, *fp_irq;
