...
static int fpsensor_irq_gpio_cfg(fpsensor_data_t *fpsensor)
{
	...
	if (node) {
		...
		fpsensor_debug(ERR_LOG,
			       "node = of_find_matching_node fail error  \n");
		return -EINVAL;
	}
	if (CHIPONE_LDO_DISABLE == 1) {
		int ret = 0;
		fpsensor->fp_irq =
			pinctrl_lookup_state(fpsensor->pinctrl1, "irq-init");
		if (IS_ERR(fpsensor->fp_irq)) {
			ret = PTR_ERR(fpsensor->fp_irq);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor Cannot find fp pinctrl fp_irq!\n");
			return ret;
		}

		pinctrl_select_state(fpsensor->pinctrl1,
				     fpsensor->fp_irq); //cfg irq
	}
	/*
...
void fpsensor_gpio_output_dts(int gpio, int level)
{
	...
	fpsensor_debug(
		DEBUG_LOG,
		"[fpsensor]fpsensor_gpio_output_dts: gpio= %d, level = %d\n",
		gpio, level);
	if (gpio == FPSENSOR_RST_PIN) {
		if (level) {
			pinctrl_select_state(g_fpsensor->pinctrl1,
					     g_fpsensor->fp_rst_high);
		} else {
			pinctrl_select_state(g_fpsensor->pinctrl1,
					     g_fpsensor->fp_rst_low);
		}
	}
	mutex_unlock(&spidev_set_gpio_mutex);
}
...
static void fpsensor_spi_mode_enable(u8 bonoff)
{
	if (IS_ERR(g_fpsensor->pinctrl1)) {
		fpsensor_debug(
			ERR_LOG,
			"fpsensor_spi_mode_enable Cannot find pinctrl1.\n");
		return;
	}
	if (bonoff == 0) {
		if (IS_ERR(g_fpsensor->fp_cs_low)) {
			fpsensor_debug(
				ERR_LOG,
				"fpsensor_spi_mode_enable Cannot find fp_cs_low .\n");
			return;
		}
		pinctrl_select_state(g_fpsensor->pinctrl1,
				     g_fpsensor->fp_cs_low);
		if (IS_ERR(g_fpsensor->fp_clk_low)) {
			fpsensor_debug(
				ERR_LOG,
				"fpsensor_spi_mode_enable Cannot find fp_clk_low .\n");
			return;
		}
		pinctrl_select_state(g_fpsensor->pinctrl1,
				     g_fpsensor->fp_clk_low);
		if (IS_ERR(g_fpsensor->fp_mo_low)) {
			fpsensor_debug(
				ERR_LOG,
				"fpsensor_spi_mode_enable Cannot find fp_mo_low .\n");
			return;
		}
		pinctrl_select_state(g_fpsensor->pinctrl1,
				     g_fpsensor->fp_mo_low);
		if (IS_ERR(g_fpsensor->fp_mi_low)) {
			fpsensor_debug(
				ERR_LOG,
				"fpsensor_spi_mode_enable Cannot find fp_mi_low .\n");
			return;
		}
		pinctrl_select_state(g_fpsensor->pinctrl1,
				     g_fpsensor->fp_mi_low);
	} else if (bonoff == 1) {
		if (IS_ERR(g_fpsensor->fp_cs_mode)) {
			fpsensor_debug(
				ERR_LOG,
				"fpsensor_spi_mode_enable Cannot find fp_cs_mode .\n");
			return;
		}
		pinctrl_select_state(g_fpsensor->pinctrl1,
				     g_fpsensor->fp_cs_mode);
		if (IS_ERR(g_fpsensor->fp_spi_clk)) {
			fpsensor_debug(
				ERR_LOG,
				"fpsensor_spi_mode_enable Cannot find fp_spi_clk .\n");
			return;
		}
		pinctrl_select_state(g_fpsensor->pinctrl1,
				     g_fpsensor->fp_spi_clk);
		if (IS_ERR(g_fpsensor->fp_spi_mo)) {
			fpsensor_debug(
				ERR_LOG,
				"fpsensor_spi_mode_enable Cannot find fp_spi_mo .\n");
			return;
		}
		pinctrl_select_state(g_fpsensor->pinctrl1,
				     g_fpsensor->fp_spi_mo);
		if (IS_ERR(g_fpsensor->fp_spi_mi)) {
			fpsensor_debug(
				ERR_LOG,
				"fpsensor_spi_mode_enable Cannot find fp_spi_mi .\n");
			return;
		}
		pinctrl_select_state(g_fpsensor->pinctrl1,
				     g_fpsensor->fp_spi_mi);
	}
}
	...
	if (node) {
		...
		if (pdev) {
			...
			if (IS_ERR(fpsensor->pinctrl1)) {
				...
				fpsensor_debug(
					ERR_LOG,
					"fpsensor Cannot find fp pinctrl1.\n");
				return ret;
			}
		} else {
			fpsensor_debug(ERR_LOG,
				       "fpsensor Cannot find device.\n");
			return -ENODEV;
		}

		fpsensor->fp_rst_low = pinctrl_lookup_state(
			fpsensor->pinctrl1, "fpsensor_finger_rst_low");
		if (IS_ERR(fpsensor->fp_rst_low)) {
			ret = PTR_ERR(fpsensor->fp_rst_low);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor Cannot find fp pinctrl fp_rst_low!\n");
			return ret;
		}
		fpsensor->fp_rst_high = pinctrl_lookup_state(
			fpsensor->pinctrl1, "fpsensor_finger_rst_high");
		if (IS_ERR(fpsensor->fp_rst_high)) {
			ret = PTR_ERR(fpsensor->fp_rst_high);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor Cannot find fp pinctrl fp_rst_high!\n");
			return ret;
		}

		fpsensor->fp_cs_mode = pinctrl_lookup_state(
			fpsensor->pinctrl1, "fpsensor_finger_cs_mode");
		if (IS_ERR(fpsensor->fp_cs_mode)) {
			ret = PTR_ERR(fpsensor->fp_cs_mode);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor Cannot find fp pinctrl fp_cs_mode!\n");
			//return ret;
		}

		fpsensor->fp_spi_clk = pinctrl_lookup_state(
			fpsensor->pinctrl1, "fpsensor_finger_clk");
		if (IS_ERR(fpsensor->fp_spi_clk)) {
			ret = PTR_ERR(fpsensor->fp_spi_clk);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor Cannot find fp pinctrl fp_spi_clk!\n");
			//return ret;
		}

		fpsensor->fp_spi_mo = pinctrl_lookup_state(
			fpsensor->pinctrl1, "fpsensor_finger_mosi");
		if (IS_ERR(fpsensor->fp_spi_mo)) {
			ret = PTR_ERR(fpsensor->fp_spi_mo);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor Cannot find fp pinctrl fp_spi_mo!\n");
			//return ret;
		}

		fpsensor->fp_spi_mi = pinctrl_lookup_state(
			fpsensor->pinctrl1, "fpsensor_finger_miso");
		if (IS_ERR(fpsensor->fp_spi_mi)) {
			ret = PTR_ERR(fpsensor->fp_spi_mi);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor Cannot find fp pinctrl fp_spi_mi!\n");
			//return ret;
		}

		fpsensor->fp_clk_low = pinctrl_lookup_state(
			fpsensor->pinctrl1, "fpsensor_finger_clk_low");
		if (IS_ERR(fpsensor->fp_clk_low)) {
			ret = PTR_ERR(fpsensor->fp_clk_low);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor Cannot find fp pinctrl fp_clk_low!\n");
			//return ret;
		}

		fpsensor->fp_mo_low = pinctrl_lookup_state(
			fpsensor->pinctrl1, "fpsensor_finger_mosi_low");
		if (IS_ERR(fpsensor->fp_mo_low)) {
			ret = PTR_ERR(fpsensor->fp_mo_low);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor Cannot find fp pinctrl fp_mo_low!\n");
			//return ret;
		}

		fpsensor->fp_mi_low = pinctrl_lookup_state(
			fpsensor->pinctrl1, "fpsensor_finger_miso_low");
		if (IS_ERR(fpsensor->fp_mi_low)) {
			ret = PTR_ERR(fpsensor->fp_mi_low);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor Cannot find fp pinctrl fp_mi_low!\n");
			//return ret;
		}

		fpsensor->fp_cs_low = pinctrl_lookup_state(
			fpsensor->pinctrl1, "fpsensor_finger_cs_low");
		if (IS_ERR(fpsensor->fp_cs_low)) {
			ret = PTR_ERR(fpsensor->fp_cs_low);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor Cannot find fp pinctrl fp_cs_low!\n");
			//return ret;
		}
...
static long fpsensor_ioctl(struct file *filp, unsigned int cmd,
			   unsigned long arg)
{
	...
	case FPSENSOR_IOC_INIT:
		...
		if (free_irq_flag == 0) {
			...
			retval = request_threaded_irq(fpsensor_dev->irq,
						      fpsensor_irq, NULL, irqf,
						      FPSENSOR_DEV_NAME,
						      fpsensor_dev);
			if (retval == 0) {
				fpsensor_debug(
					INFO_LOG,
					" irq thread reqquest success!\n");
				free_irq_flag = 1;
			} else {
				fpsensor_debug(
					ERR_LOG,
					" irq thread request failed , retval =%d \n",
					retval);
				break;
			}
...
*/
	...
	case FPSENSOR_IOC_GET_INT_VAL:
		...
		if (copy_to_user((void __user *)arg, (void *)&val,
				 sizeof(uint32_t))) {
			fpsensor_debug(ERR_LOG,
	...
	case FPSENSOR_IOC_ENABLE_POWER:
		fpsensor_debug(INFO_LOG,
			       "%s: FPSENSOR_IOC_ENABLE_POWER ======\n",
			       __func__);
		//if (CHIPONE_LDO_DISABLE == 1)
		fpsensor_power_onoff(1);
		break;
...
*/
	...
	case FPSENSOR_IOC_GET_FP_STATUS:
		...
		fpsensor_debug(INFO_LOG,
			       "%s: FPSENSOR_IOC_GET_FP_STATUS  %d \n",
	...
	case FPSENSOR_IOC_ENABLE_REPORT_BLANKON:
		...
		fpsensor_debug(INFO_LOG,
			       "%s: FPSENSOR_IOC_ENABLE_REPORT_BLANKON: %d\n",
			       __func__, val);
		break;
	case FPSENSOR_IOC_UPDATE_DRIVER_SN:
		if (copy_from_user(&g_cmd_sn, (void __user *)arg,
				   sizeof(uint32_t))) {
			fpsensor_debug(
				ERR_LOG,
				"Failed to copy g_cmd_sn from user to kernel\n");
			retval = -EFAULT;
			break;
		}
	...
	default:
		fpsensor_debug(ERR_LOG,
			       "fpsensor doesn't support this command(0x%x)\n",
			       cmd);
		break;
	}
...
static long fpsensor_compat_ioctl(struct file *filp, unsigned int cmd,
				  unsigned long arg)
{
	return fpsensor_ioctl(filp, cmd, (unsigned long)(arg));
}

static unsigned int fpsensor_poll(struct file *filp,
...
static ssize_t fpsensor_read(struct file *filp, char __user *buf, size_t count,
			     loff_t *f_pos)
{
...
static int fpsensor_dev_setup(fpsensor_data_t *fpsensor)
{
	...
	if (fpsensor_dev_major) {
		...
		ret = register_chrdev_region(dev_no, FPSENSOR_NR_DEVS,
					     FPSENSOR_DEV_NAME);
	} else {
		ret = alloc_chrdev_region(&dev_no, fpsensor_dev_minor,
					  FPSENSOR_NR_DEVS, FPSENSOR_DEV_NAME);
		fpsensor_dev_major = MAJOR(dev_no);
		fpsensor_dev_minor = MINOR(dev_no);
		fpsensor_debug(INFO_LOG,
			       "fpsensor device major is %d, minor is %d\n",
	...
	dev = device_create(fpsensor->class, &fpsensor->spi->dev, dev_no,
...
#endif
{
	...
	if (!fpsensor_dev) {
		...
		fpsensor_debug(
			ERR_LOG,
			"%s, Failed to alloc memory for fpsensor device.\n",
			__func__);
		goto out;
	}
	...
	if (status) {
		fpsensor_debug(ERR_LOG, "fpsensor setup char device failed, %d",
			       status);
		goto release_drv_data;
	}
	//fpsensor_spi_clk_enable(1);
	init_waitqueue_head(&fpsensor_dev->wq_irq_return);
	wake_lock_init(&g_fpsensor->ttw_wl, WAKE_LOCK_SUSPEND,
		       "fpsensor_ttw_wl");
	fpsensor_dev->device_available = 1;
#if FP_NOTIFY
	...
	if (node) {
		...
		if (pdev) {
			...
			if (IS_ERR(fpsensor_dev->pinctrl1)) {
				...
				fpsensor_debug(
					ERR_LOG,
					"fpsensor_dev Cannot find fp pinctrl1.\n");
			}
		} else {
			fpsensor_debug(ERR_LOG,
				       "fpsensor_dev Cannot find device.\n");
		}
	}

	ret = of_property_read_u32(node, "chipone,ldo_disable",
				   &CHIPONE_LDO_DISABLE);
	if (ret) {
		fpsensor_debug(
			ERR_LOG,
			"failed to request chipone,ldo_disable, ret = %d\n",
			ret);
		CHIPONE_LDO_DISABLE = 0;
	}

	if (CHIPONE_LDO_DISABLE == 1) {
		fpsensor_dev->fp_pwr_high = pinctrl_lookup_state(
			fpsensor_dev->pinctrl1, "power_high");
		if (IS_ERR(fpsensor_dev->fp_pwr_high)) {
			ret = PTR_ERR(fpsensor_dev->fp_pwr_high);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor_dec Cannot find fp pinctrl fp_pwr_high!\n");
			return ret;
		}
		fpsensor_dev->fp_pwr_low = pinctrl_lookup_state(
			fpsensor_dev->pinctrl1, "power_low");
		if (IS_ERR(fpsensor_dev->fp_pwr_low)) {
			ret = PTR_ERR(fpsensor_dev->fp_pwr_low);
			fpsensor_debug(
				ERR_LOG,
				"fpsensor_dev Cannot find fp pinctrl fp_pwr_low!\n");
			return ret;
		}
	}
	...
	if (CHIPONE_LDO_DISABLE == 0) {
		...
		status = regulator_set_voltage(finger_regulator, 3300000,
					       3300000);
