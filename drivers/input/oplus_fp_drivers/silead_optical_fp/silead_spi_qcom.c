...
static int silfp_fb_callback(struct notifier_block *notif, unsigned long event,
			     void *data)
{
	struct silfp_data *fp_dev =
	...
	if (event == MSM_DRM_ONSCREENFINGERPRINT_EVENT) {
		...
		default:
			LOG_MSG_DEBUG(
				INFO_LOG,
				"[%s] Unknown MSM_DRM_ONSCREENFINGERPRINT_EVENT\n",
				__func__);
			break;
		}
		return retval;
...
#endif /* PROC_DIR */
		fp_dev->proc_entry = proc_create(
			PROC_NODE, 0666, fp_dev->proc_root, &silfp_proc_fops);
		if (fp_dev->proc_entry == NULL) {
			LOG_MSG_DEBUG(ERR_LOG,
...
static long silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SIFP_IOC_RESET: {
		...
		if (arg) {
			if (copy_from_user(&delay, (void __user *)arg,
					   sizeof(char))) {
				retval = -EFAULT;
				break;
			}
	...
	case SIFP_IOC_WAIT_IRQ:
		...
		retval = __put_user(silfp_wait_irq(fp_dev) ? 1 : 0,
				    (__u8 __user *)arg);
		break;

	case SIFP_IOC_IRQ_STATUS:
		LOG_MSG_DEBUG(INFO_LOG, "[%s] irq status\n", __func__);
		retval = __put_user((char)silfp_irq_status(fp_dev),
				    (__u8 __user *)arg);
		break;
	...
	case SIFP_IOC_SCR_STATUS:
		if (arg) {
			LOG_MSG_DEBUG(INFO_LOG, "[IOC_SCR_STATUS] put v = %d\n",
				      (u8)(!fp_dev->scr_off));
			retval = __put_user((u8)(!fp_dev->scr_off),
					    (__u8 __user *)arg);
		} else {
			LOG_MSG_DEBUG(INFO_LOG,
				      ...
				      (u8)(!fp_dev->scr_off));
			silfp_netlink_send(
				fp_dev, fp_dev->scr_off ? SIFP_NETLINK_SCR_OFF :
							  SIFP_NETLINK_SCR_ON);
		}
		break;
	...
	case SIFP_IOC_DBG_LEVEL:
		if (arg) {
			...
			if (copy_from_user(&level, (void __user *)arg,
					   sizeof(char)) ||
			...
			LOG_MSG_DEBUG(ERR_LOG, "debug level %d-->%d\n",
				      debug_level, level);
			retval = __put_user((u8)(debug_level),
					    (__u8 __user *)arg);
	...
	case SIFP_IOC_WAKELOCK:
		if (arg) {
			...
			if (copy_from_user(&lock, (void __user *)arg,
					   sizeof(char))) {
				LOG_MSG_DEBUG(
					ERR_LOG,
	...
	case SIFP_IOC_PWDN:
		if (arg) {
			...
			if (copy_from_user(&flag_avdd, (void __user *)arg,
					   sizeof(char))) {
				LOG_MSG_DEBUG(
					ERR_LOG,
	...
	case SIFP_IOC_PROC_NODE:
		if (arg) {
			if (copy_from_user(vendor_name, (void __user *)arg,
					   PROC_VND_ID_LEN)) {
				LOG_MSG_DEBUG(
					ERR_LOG,
	...
	case SIFP_IOC_GET_TP_TOUCH_INFO:
		...
		if (arg) {
			if (copy_to_user((void __user *)arg, (void *)&fp_tpinfo,
					 sizeof(fp_tpinfo))) {
				LOG_MSG_DEBUG(
					ERR_LOG,
					"[SIFP_IOC_GET_TP_TOUCH_INFO] copy_to fail\n");
				retval = -EFAULT;
			}
		}
		break;
	case SIFP_IOC_SET_TP_MSG_REPORT_MODE:
		LOG_MSG_DEBUG(
			INFO_LOG,
			"[SET_TP_MSG_REPORT_MODE]enter,arg =%d,lasttouchmode=%d\n",
		...
		if (1 == lasttouchmode) {
			...
			LOG_MSG_DEBUG(ERR_LOG,
				      "arg null,or mode is 0, do nothing!\n");
		}
		break;
	default:
		retval = -ENOTTY;
...
static int silfp_probe(struct platform_device *spi)
{
	...
	if (FP_DEV_MAJOR > 0) {
		...
		status = alloc_chrdev_region(&fp_dev->devt, fp_dev->ref++, 1,
					     FP_DEV_NAME);
	}
	if (status < 0) {
	...
	fp_dev->dev = device_create(silfp_class, &fp_dev->spi->dev,
