...
int ff_log_printf(ff_log_level_t level, const char *tag, const char *fmt, ...)
{
	...
	if (likely(ff_ctx) && unlikely(ff_ctx->ff_config.logcat_driver) &&
		...
		kobject_uevent_env(&ff_ctx->fp_dev->kobj, KOBJ_CHANGE,
				   uevent_env);
	}
...
#define FF_IOCTL_SPI_SYNC                                                      \
	_IOWR(FF_IOCTL_SPI_DEVICE, 0x01, struct ff_spi_sync_data)
...
#define FF_IOCTL_SPI_SYNC_2                                                    \
	_IOWR(FF_IOCTL_SPI_DEVICE, 0x10, struct ff_spi_sync_data_2)
...
/* spi interface */
static int ff_spi_sync(struct ff_spi_context *spi_ctx, u8 *tx_buf, u8 *rx_buf,
		       u32 len)
{
	int ret = 0;
...
static int ff_ioctl_set_spi_mode(struct ff_spi_context *spi_ctx,
				 unsigned long arg)
{
	int ret = 0;
	u32 mode = 0;
...
static int ff_ioctl_set_spi_speed(struct ff_spi_context *spi_ctx,
				  unsigned long arg)
{
	int ret = 0;
	u32 speed = 0;
...
{
	struct ff_spi_context *spi_ctx =
		container_of(filp->private_data, struct ff_spi_context, mdev);
...
static long ff_spi_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FF_IOCTL_GET_SPI_SPEED:
		ret = __put_user(spi_ctx->spi->max_speed_hz,
				 (__u32 __user *)arg);
		break;
...
static int ff_spi_probe(struct spi_device *spi)
{
	...
	if (ff_ctx->b_read_chipid || ff_ctx->b_ree) {
		...
		spi_ctx = (struct ff_spi_context *)kzalloc(sizeof(*spi_ctx),
							   GFP_KERNEL);
		if (!spi_ctx) {
...
static int ff_set_irq(ff_context_t *ff_ctx, bool enable)
{
	...
	if ((ff_ctx->irq_num <= 0) || (!ff_ctx->b_driver_inited)) {
		FF_LOGE("ff_ctx/irq_num(%d)/b_driver_inited is invalid",
			ff_ctx->irq_num);
		return -EINVAL;
	}
...
static int ff_power_on(ff_context_t *ff_ctx, bool on)
{
	...
	FF_LOGI("set power:%d->%d,use_regulator:%d", ff_ctx->b_power_on, on,
		ff_ctx->b_use_regulator);
			...
			if (ff_ctx->b_use_pinctrl && ff_ctx->pinctrl) {
				...
				if (on && ff_ctx->pins_power_high) {
					ret = pinctrl_select_state(
						ff_ctx->pinctrl,
						ff_ctx->pins_power_high);
				} else if (!on && ff_ctx->pins_power_low) {
					ret = pinctrl_select_state(
						ff_ctx->pinctrl,
						ff_ctx->pins_power_low);
				}
			}

			if (!ff_ctx->b_use_pinctrl &&
			    gpio_is_valid(ff_ctx->vdd_gpio)) {
				FF_LOGI("use gpio(%d) to control power",
					ff_ctx->vdd_gpio);
				ret = gpio_direction_output(ff_ctx->vdd_gpio,
							    on);
			}
		}
...
static void ff_spi_mode_enable(ff_context_t *ff_ctx, u8 bonoff)
{
	...
	if (bonoff == 0) {
		...
		pinctrl_select_state(ff_ctx->pinctrl,
				     ff_ctx->pins_spi_mosi_low);
		pinctrl_select_state(ff_ctx->pinctrl,
				     ff_ctx->pins_spi_miso_low);
	} else if (bonoff == 1) {
...
static int ff_set_spiclk(ff_context_t *ff_ctx, bool enable)
{
	...
	if (ff_ctx && ff_ctx->b_spiclk && ff_ctx->spi && ff_ctx->b_power_on) {
		FF_LOGD("set spi clock:%d->%d", ff_ctx->b_spiclk_enabled,
			enable);
		if (!ff_ctx->b_spiclk_enabled && enable) {
			...
			FF_LOGD("can't set spi clock , becasue : ff_ctx->b_spiclk is %d , enable is %d ",
				ff_ctx->b_spiclk, enable);
		}
	} else {
...
static int ff_register_input(ff_context_t *ff_ctx)
{
	...
	for (i_try = 0; i_try < FF_GK_MAX; i_try++) {
		if (config->gesture_keycode[i_try] > 0) {
			FF_LOGD("register gesture keycode:%d",
				config->gesture_keycode[i_try]);
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FF_IOC_GET_FEATURE:
		...
		ret = copy_to_user((ff_feature_t __user *)arg, &ff_ctx->feature,
				   sizeof(ff_feature_t));
		break;
...
static unsigned int ff_ctl_poll(struct file *filp,
...
{
	ff_context_t *ff_ctx =
		container_of(inode->i_cdev, ff_context_t, ff_cdev);
	FF_LOGV("'%s' enter.", __func__);
...
/* ff_log node */
static ssize_t ff_log_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	ssize_t count = 0;
	FF_LOGV("'%s' enter.", __func__);
...
static ssize_t ff_log_store(struct device *dev, struct device_attribute *attr,
...
/* ff_reset interface */
static ssize_t ff_reset_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	ssize_t count = 0;
	...
	if (ff_ctx && ff_ctx->b_driver_inited) {
		...
		count = snprintf(buf, PAGE_SIZE,
				 "fingerprint device has been reset\n");
	}
	FF_LOGV("'%s' leave.", __func__);
	return count;
}

static ssize_t ff_reset_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	struct platform_device *pdev = to_platform_device(dev);
	ff_context_t *ff_ctx = platform_get_drvdata(pdev);
...
/* ff_irq interface */
static ssize_t ff_irq_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	ssize_t count = 0;
	struct platform_device *pdev = to_platform_device(dev);
...
static ssize_t ff_irq_store(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t count)
{
	struct platform_device *pdev = to_platform_device(dev);
	ff_context_t *ff_ctx = platform_get_drvdata(pdev);
...
/* ff_info interface */
static ssize_t ff_info_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	ssize_t count = 0;
	struct platform_device *pdev = to_platform_device(dev);
	...
	if (ff_ctx) {
		count += snprintf(buf + count, PAGE_SIZE,
				  "gpio,reset:%d,int:%d-%d,power:%d\n",
				  ff_ctx->reset_gpio, ff_ctx->irq_gpio,
				  ff_ctx->irq_num, ff_ctx->vdd_gpio);
		count += snprintf(
			buf + count, PAGE_SIZE,
			"power_always_on:%d,use_regulator:%d,use_pinctrl:%d\n",
			ff_ctx->b_power_always_on, ff_ctx->b_use_regulator,
			ff_ctx->b_use_pinctrl);
		count += snprintf(buf + count, PAGE_SIZE,
				  "read_chip:%d,ree:%d,screen on/off:%d\n",
				  ff_ctx->b_read_chipid, ff_ctx->b_ree,
				  ff_ctx->b_screen_onoff_event);

		count += snprintf(
			buf + count, PAGE_SIZE,
			"driver_init:%d,power_on:%d,irq_en:%d,spiclk_en:%d\n",
			ff_ctx->b_driver_inited, ff_ctx->b_power_on,
			ff_ctx->b_irq_enabled, ff_ctx->b_spiclk_enabled);

		count += snprintf(buf + count, PAGE_SIZE,
				  "gesture_en:%d,%d-%d-%d-%d\n",
				  ff_ctx->b_gesture_support,
				  ff_ctx->ff_config.gesture_keycode[0],
				  ff_ctx->ff_config.gesture_keycode[1],
				  ff_ctx->ff_config.gesture_keycode[2],
				  ff_ctx->ff_config.gesture_keycode[3]);
	}
	FF_LOGV("'%s' leave.", __func__);
	return count;
}

static ssize_t ff_info_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	struct platform_device *pdev = to_platform_device(dev);
	ff_context_t *ff_ctx = platform_get_drvdata(pdev);
...
static struct attribute *ff_attrs[] = { &dev_attr_ff_log.attr,
					&dev_attr_ff_reset.attr,
					&dev_attr_ff_irq.attr,
					&dev_attr_ff_info.attr, NULL };
static struct attribute_group ff_attrs_group = { .attrs = ff_attrs };
...
static void ff_work_device_event(struct work_struct *ws)
{
	...
	wake_lock_timeout(&ff_ctx->wake_lock,
			  msecs_to_jiffies(FF_WAKELOCK_TIMEOUT));
...
static void ff_parse_dt(struct device *dev, ff_context_t *ff_ctx)
{
	...
	ff_ctx->b_screen_onoff_event =
		!of_property_read_bool(np, "focaltech_fp,no-screen-on-off");

	/*check if power is always on*/
	ff_ctx->b_power_always_on =
		of_property_read_bool(np, "focaltech_fp,power-always-on");

	/*check if power regulator is used*/
	ff_ctx->b_use_regulator =
		of_property_read_bool(np, "focaltech_fp,use-regulator");

	/*check if pinctrl(irq and reset) is used*/
	ff_ctx->b_use_pinctrl =
		of_property_read_bool(np, "focaltech_fp,use-pinctrl");

	/*check if need read chip id*/
	ff_ctx->b_read_chipid =
		of_property_read_bool(np, "focaltech_fp,read-chip");
	...
	/*check gpios*/
	ff_ctx->reset_gpio =
		of_get_named_gpio(np, "focaltech_fp,reset-gpio", 0);
	if (!gpio_is_valid(ff_ctx->reset_gpio)) {
	...
	FF_LOGD("power_always_on:%d, use_regulator:%d, use_pinctrl:%d",
		ff_ctx->b_power_always_on, ff_ctx->b_use_regulator,
		ff_ctx->b_use_pinctrl);
	FF_LOGD("read_chip:%d, ree:%d, spiclk:%d, screen on/off:%d",
		ff_ctx->b_read_chipid, ff_ctx->b_ree, ff_ctx->b_spiclk,
		ff_ctx->b_screen_onoff_event);
	FF_LOGV("'%s' leave.", __func__);
}
...
static int ff_init_pins(ff_context_t *ff_ctx)
{
	...
	if (ff_ctx->b_use_pinctrl) {
		...
		if (IS_ERR_OR_NULL(ff_ctx->pinctrl)) {
			...
			FF_LOGE("Failed to get pinctrl, please check dts,ret=%d",
				ret);
			return ret;
		}

		ff_ctx->pins_reset_low = pinctrl_lookup_state(
			ff_ctx->pinctrl, "ff_pins_reset_low");
		...
		ff_ctx->pins_reset_high = pinctrl_lookup_state(
			ff_ctx->pinctrl, "ff_pins_reset_high");
		...
		ff_ctx->pins_irq_as_int = pinctrl_lookup_state(
			ff_ctx->pinctrl, "ff_pins_irq_as_int");
		...
		ff_ctx->pins_spi_cs_mode = pinctrl_lookup_state(
			ff_ctx->pinctrl, "ff_pins_cs_mode");
		...
		ff_ctx->pins_spi_clk = pinctrl_lookup_state(ff_ctx->pinctrl,
							    "ff_pins_spi_clk");
		...
		ff_ctx->pins_spi_mosi = pinctrl_lookup_state(
			ff_ctx->pinctrl, "ff_pins_spi_mosi");
		...
		ff_ctx->pins_spi_miso = pinctrl_lookup_state(
			ff_ctx->pinctrl, "ff_pins_spi_miso");
		...
		ff_ctx->pins_spi_clk_low = pinctrl_lookup_state(
			ff_ctx->pinctrl, "ff_pins_spi_clk_low");
		...
		ff_ctx->pins_spi_mosi_low = pinctrl_lookup_state(
			ff_ctx->pinctrl, "ff_pins_spi_mosi_low");
		...
		ff_ctx->pins_spi_miso_low = pinctrl_lookup_state(
			ff_ctx->pinctrl, "ff_pins_spi_miso_low");
		...
		ff_ctx->pins_spi_cs_low = pinctrl_lookup_state(
			ff_ctx->pinctrl, "ff_pins_spi_cs_low");
	...
	if (ff_ctx->b_use_regulator) {
		...
		if (regulator_count_voltages(ff_ctx->ff_vdd) > 0) {
			ret = regulator_set_voltage(
				ff_ctx->ff_vdd, FF_VTG_MIN_UV, FF_VTG_MAX_UV);
			if (ret) {
				FF_LOGD("vdd regulator set_voltage failed ret=%d",
					ret);
				regulator_put(ff_ctx->ff_vdd);
				return ret;
			}
...
static int ff_register_irq(ff_context_t *ff_ctx)
{
	...
	if (ff_ctx->irq_num > 0) {
		...
		ret = request_irq(ff_ctx->irq_num, ff_irq_handler, irqflags,
				  FF_DRV_NAME, ff_ctx);
		ff_ctx->b_irq_enabled = true;
...
static int ff_register_device(ff_context_t *ff_ctx)
{
	...
	ff_ctx->fp_dev = device_create(ff_ctx->ff_class, NULL, ff_devno, ff_ctx,
				       FF_DRV_NAME);
				...
				unsigned long event, void *data)
{
	...
	if (ff_ctx->b_screen_onoff_event &&
		...
		ff_ctx->b_screen_on = this_screen_on;
		if (ff_ctx->event_type == FF_EVENT_POLL) {
			ff_ctx->poll_event |= this_screen_on ?
						      FF_POLLEVT_SCREEN_ON :
						      FF_POLLEVT_SCREEN_OFF;
			wake_up_interruptible(&ff_ctx->wait_queue_head);
		} else if (ff_ctx->event_type == FF_EVENT_NETLINK) {
			uevent_env[1] = NULL;
			kobject_uevent_env(&ff_ctx->fp_dev->kobj, KOBJ_CHANGE,
...
static int ff_probe(struct platform_device *pdev)
{
	...
	ff_ctx->feature.fbs_power_const =
		(ff_ctx->b_power_always_on) ? true : false;
	ff_ctx->feature.fbs_screen_onoff =
		(ff_ctx->b_screen_onoff_event) ? true : false;
	...
	wake_lock_init(&ff_ctx->wake_lock_ctl, WAKE_LOCK_SUSPEND,
		       "ff_wake_lock_ctl");
