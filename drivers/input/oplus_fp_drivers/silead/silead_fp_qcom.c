...
static void silfp_hw_poweron(struct silfp_data *fp_dev)
{
	...
	if (fp_dev->sld_ext_pmic_flag == 1) {
		if (fingerprint_ldo_enable(fp_dev->sld_ext_pmic_ldo_num,
					   fp_dev->sld_ext_pmic_ldo_mv_max)) {
			LOG_MSG_DEBUG(ERR_LOG, "%s: enable EXT_LDO7 failed",
				      __func__);
			goto fp_out;
		} else {
			LOG_MSG_DEBUG(ERR_LOG, "%s: enable EXT_LDO7 success",
				      __func__);
		}
	}
	...
	if (fp_dev->avdd_ldo) {
		err = regulator_set_voltage(fp_dev->avdd_ldo, AVDD_MIN,
					    AVDD_MAX); /*set 2.8v*/
		err = regulator_set_load(fp_dev->avdd_ldo, 200000); //100ma
		if (err < 0) {
			LOG_MSG_DEBUG(
				INFO_LOG,
				"poweron: Failed to set avdd_ldo vreg mode(err:%d)\n",
				err);
		}
	...
	if (fp_dev->vddio_ldo) {
		...
		if (err < 0) {
			LOG_MSG_DEBUG(
				INFO_LOG,
				"poweron: Failed to set vddio vreg mode(err:%d)\n",
				err);
		}
	...
	if (fp_dev->pin.pins_avdd_h) {
		err = pinctrl_select_state(fp_dev->pin.pinctrl,
...
static void silfp_hw_poweroff(struct silfp_data *fp_dev)
{
	...
	if (fp_dev->sld_ext_pmic_flag == 1) {
		fingerprint_ldo_disable(fp_dev->sld_ext_pmic_ldo_num,
	...
	if ((fp_dev->power_is_off == 0) && fp_dev->avdd_ldo &&
		...
		if (err < 0) {
			LOG_MSG_DEBUG(
				INFO_LOG,
				"poweroff: Failed to set avdd_ldo vreg mode(err:%d)\n",
				err);
		}
	}
	if ((fp_dev->power_is_off == 0) && fp_dev->vddio_ldo &&
		...
		if (err < 0) {
			LOG_MSG_DEBUG(
				INFO_LOG,
				"poweroff: Failed to set vddio vreg mode(err:%d)\n",
				err);
		}
	}
#endif /* BSP_SIL_POWER_SUPPLY_REGULATOR */
...
static void silfp_power_deinit(struct silfp_data *fp_dev)
{
	...
	if (fp_dev->avdd_ldo) {
		...
		if (err < 0) {
			LOG_MSG_DEBUG(
				INFO_LOG,
				"deinit: Failed to set avdd_ldo vreg mode(err:%d)\n",
				err);
		}
...
static int silfp_parse_dts(struct silfp_data *fp_dev)
{
	...
	if (of_property_read_bool(fp_dev->spi->dev.of_node,
				  "sld,enable-via-gpio")) {
	...
	if (fp_dev->sld_ext_pmic_flag) {
		ret = of_property_read_u32(fp_dev->spi->dev.of_node,
					   "sld,ext-pmic-ldo-num",
...
static int spi_set_pinctrl(struct silfp_data *fp_dev, bool active)
{
	...
	if (IS_ERR_OR_NULL(fp_dev->pin.pinctrl) ||
	    IS_ERR_OR_NULL(fp_dev->pin.active)) {
		LOG_MSG_DEBUG(ERR_LOG, "%s: not support\n", __func__);
		return ret;
	}
	if (active) { /* Change to active settings */
		ret = pinctrl_select_state(fp_dev->pin.pinctrl,
					   fp_dev->pin.active);
	} else {
		ret = pinctrl_select_state(fp_dev->pin.pinctrl,
					   fp_dev->pin.sleep);
...
static int spi_set_clks(struct silfp_data *fp_dev, bool enable)
{
	...
	if (enable) {
		...
		ret = clk_set_rate(fp_dev->pin.core_clk,
		...
		if (ret) {
			...
			if (ret) {
				LOG_MSG_DEBUG(
					ERR_LOG,
					"%s: Error setting clk_rate:%d, ret=%d\n",
					__func__, clk, ret);
			} else {
		...
		if (ret) {
			LOG_MSG_DEBUG(ERR_LOG,
				      "%s: Error enabling core clk, ret=%d\n",
				      __func__, ret);
		}
		ret = clk_prepare_enable(fp_dev->pin.iface_clk);
		if (ret) {
			LOG_MSG_DEBUG(ERR_LOG,
				      "%s: Error enabling iface clk, ret=%d\n",
				      __func__, ret);
		}
	} else {
		/* Disable the clocks */
...
#ifndef QSEE_V4
	if (IS_ERR_OR_NULL(fp_dev->pin.pinctrl) ||
	    IS_ERR_OR_NULL(fp_dev->pin.active) || !fp_dev->pin.qup_id) {
		LOG_MSG_DEBUG(ERR_LOG, "%s: not support!\n", __func__);
		return ret;
	}
			       ...
			       struct fp_dev_init_t *dev_info)
{
	...
	if (fp_dev->sld_gpio_pwr_flag == 1) {
		fp_dev->avdd_port = of_get_named_gpio(fp_dev->spi->dev.of_node,
						      "avdd-gpios", 0);
		...
		if (fp_dev->vddio_port > 0) {
			gpio_free(fp_dev->vddio_port);
			ret = gpio_request(fp_dev->vddio_port,
					   "SILFP_VDDIO_PIN");
			if (ret < 0) {
				LOG_MSG_DEBUG(
					ERR_LOG,
	...
	msleep(5);
	fp_dev->int_port =
		of_get_named_gpio(fp_dev->spi->dev.of_node, "irq-gpios", 0);
	fp_dev->rst_port =
		of_get_named_gpio(fp_dev->spi->dev.of_node, "rst-gpios", 0);
