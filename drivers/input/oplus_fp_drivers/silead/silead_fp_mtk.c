...
int vmch_enable = 0;
const static uint8_t TANAME[] = { 0x51, 0x1E, 0xAD, 0x0D, 0x00, 0x00,
	...
	if (SIL_LDO_DISBALE == 0) {
		...
		if (fp_dev->avdd_ldo) {
			...
			err = regulator_enable(
				fp_dev->avdd_ldo); /*enable regulator*/
		...
		if (fp_dev->vddio_ldo) {
			...
			err = regulator_enable(
				fp_dev->vddio_ldo); /*enable regulator*/
			if (err) {
				goto last;
			}
		}
...
static void silfp_hw_poweroff(struct silfp_data *fp_dev)
{
	...
	if (SIL_LDO_DISBALE == 0) {
		...
		if (fp_dev->avdd_ldo &&
			...
			regulator_disable(
				fp_dev->avdd_ldo); /*disable regulator*/
		}
		if (fp_dev->vddio_ldo &&
		    (regulator_is_enabled(fp_dev->vddio_ldo) > 0)) {
			regulator_disable(
				fp_dev->vddio_ldo); /*disable regulator*/
		}
	}
#endif /* BSP_SIL_POWER_SUPPLY_REGULATOR */
	...
	if (SIL_LDO_DISBALE == 1) { /* BSP_SIL_POWER_SUPPLY_PINCTRL */
		...
		if (fp_dev->pin.pins_vddio_h) {
			...
			if (err) {
				LOG_MSG_DEBUG(
					ERR_LOG,
					"[%s] enter,pinctrl_select_state fail\n",
					__func__);
			}
		}
	}
...
static int silfp_set_spi_cs_low(struct silfp_data *fp_dev)
{
	...
	struct pinctrl_state *spi_cs =
		pinctrl_lookup_state(fp_dev->pin.pinctrl, "cs-low");
...
static int silfp_parse_dts(struct silfp_data *fp_dev)
{
	...
	if (node) {
		...
		if (pdev == NULL) {
			LOG_MSG_DEBUG(
				ERR_LOG,
				"%s can not find finger-fp device by node \n",
				__func__);
			return -1;
		}
		LOG_MSG_DEBUG(ERR_LOG,
			      "%s find compatible mediatek,finger-fp node \n",
			      __func__);
		//return -1;
	}
	...
	if (ret) {
		LOG_MSG_DEBUG(ERR_LOG,
			      "failed to request silfp,ldo_disable, ret = %d\n",
			      ret);
		SIL_LDO_DISBALE = 0;
	}
	...
	fp_dev->irq_gpio =
		of_get_named_gpio(pdev->dev.of_node, "sil,silead_irq", 0);
	...
	if (SIL_LDO_DISBALE == 1) { /* BSP_SIL_POWER_SUPPLY_PINCTRL */
		fp_dev->pin.pins_avdd_h =
			pinctrl_lookup_state(fp_dev->pin.pinctrl, "power_high");
		if (IS_ERR_OR_NULL(fp_dev->pin.pins_avdd_h)) {
			fp_dev->pin.pins_avdd_h = NULL;
			LOG_MSG_DEBUG(ERR_LOG,
				      "%s can't find silfp avdd-enable\n",
				      __func__);
			// Ignore error
		}

		fp_dev->pin.pins_vddio_h =
			pinctrl_lookup_state(fp_dev->pin.pinctrl, "power_low");
		if (IS_ERR_OR_NULL(fp_dev->pin.pins_vddio_h)) {
			fp_dev->pin.pins_vddio_h = NULL;
			LOG_MSG_DEBUG(ERR_LOG,
				      "%s can't find silfp vddio-enable\n",
				      __func__);
			// Ignore error
		}
	}
	...
	if (SIL_LDO_DISBALE == 0) {
		...
		if (vmch_enable == 1) {
			fp_dev->avdd_ldo =
				regulator_get(&fp_dev->spi->dev, "vmch");
		} else {
			fp_dev->avdd_ldo =
				regulator_get(&fp_dev->spi->dev, "dvdd");
		}
	}
...
static int silfp_set_spi_default_status(struct silfp_data *fp_dev)
{
	int ret = 0;
#ifdef BSP_SIL_CTRL_SPI
	fp_dev->pin.spi_default =
		pinctrl_lookup_state(fp_dev->pin.pinctrl, "spi-default");
	if (IS_ERR(fp_dev->pin.spi_default)) {
		ret = PTR_ERR(fp_dev->pin.spi_default);
		pr_info("%s can't find silfp spi-default\n", __func__);
		return ret;
