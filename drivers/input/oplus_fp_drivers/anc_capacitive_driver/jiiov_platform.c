				     ...
				     unsigned long val, void *data)
{
	...
	if (evdata && evdata->data && (val == MSM_DRM_EARLY_EVENT_BLANK) &&
		...
		case MSM_DRM_BLANK_POWERDOWN:
			...
			anc_cap_netlink_send_message_to_user(
				&netlink_msg, sizeof(netlink_msg));
			break;
					 ...
					 const char *p_buffer, size_t count)
{
	...
	return anc_cap_netlink_send_message_to_user(&netlink_msg,
						    sizeof(netlink_msg));
}
...
static void anc_power_onoff(struct anc_data *data, int power_onoff)
{
	pr_info("%s: power_onoff = %d powered_flag:%d \n", __func__,
		power_onoff, g_anc_data->is_powered_on);
	if (power_onoff == 1) {
		if (g_anc_data->is_powered_on == 0) {
			...
			if (data->vdd_use_ext_pmic) {
				if (fingerprint_ldo_enable(
					    data->ext_pmic_ldo_num,
					    data->ext_pmic_ldo_mv_max)) {
					pr_err("%s, enable external ldo failed",
					       __func__);
				} else {
					pr_err("%s: enable external ldo success",
					       __func__);
				}
			}
		}
		...
		if (g_anc_data->is_powered_on == 1) {
			...
			if (data->vdd_use_ext_pmic) {
				fingerprint_ldo_disable(
					data->ext_pmic_ldo_num,
					data->ext_pmic_ldo_mv_min);
			}
			mdelay(10);
		}
...
static int anc_gpio_init(struct device *dev, struct anc_data *data)
{
	...
	data->vdd_use_ext_pmic =
		of_property_read_bool(np, "anc,enable-external-pmic");
	...
	if (data->vdd_use_ext_pmic) {
		rc = of_property_read_u32(np, "anc,ext-pmic-ldo-num",
					  &data->ext_pmic_ldo_num);
		if (rc) {
		...
		rc = of_property_read_u32(np, "anc,ext-pmic-ldo-mv-max",
					  &data->ext_pmic_ldo_mv_max);
		if (rc) {
		...
		rc = of_property_read_u32(np, "anc,ext-pmic-ldo-mv-min",
					  &data->ext_pmic_ldo_mv_min);
		if (rc) {
...
{
	int rc = 0;
	int irqf = IRQF_TRIGGER_RISING |
		   IRQF_ONESHOT; // IRQF_TRIGGER_FALLING or IRQF_TRIGGER_RISING
	if (data->irq_init == 0) {
		rc = devm_request_threaded_irq(data->dev, data->irq, NULL,
...
static int anc_report_key_event(struct anc_data *data)
{
	...
	pr_info("%s: key = %d, value = %d\n", __func__, data->key_event.key,
		data->key_event.value);
...
static long anc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ANC_IOC_REPORT_KEY_EVENT:
		if (copy_from_user(&(dev_data->key_event), (ANC_KEY_EVENT *)arg,
