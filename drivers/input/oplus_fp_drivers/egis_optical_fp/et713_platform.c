...
static int vreg_setup(struct egis_data *egis, const char *name, bool enable)
{
	...
	if (enable) {
		...
		if (regulator_count_voltages(vreg) > 0) {
			...
			if (rc)
				DEBUG_PRINT(
					"[egis] Unable to set voltage on %s, %d\n",
					name, rc);
		}
				      ...
				      unsigned long val, void *data)
{
	...
	if (val == MSM_DRM_ONSCREENFINGERPRINT_EVENT) {
		...
		default:
			DEBUG_PRINT(
				"[egis] Unknown MSM_DRM_ONSCREENFINGERPRINT_EVENT!\n");
			break;
		}
		return retval;
	}
...
int Interrupt_Init(struct egis_data *egis, int int_mode, int detect_period,
		   int detect_threshold)
{
	int err = 0;
	int status = 0;
	...
	DEBUG_PRINT(
		"[egis] %s: request_irq_done = %d gpio_irq = %d  pin = %d\n",
	...
	if (request_irq_done == 0) {
		...
		DEBUG_PRINT(
			"[egis] %s: flag current: %d disable: %d enable: %d\n",
			__func__, fps_ints.drdy_irq_flag, DRDY_IRQ_DISABLE,
			DRDY_IRQ_ENABLE);
...
static ssize_t egis_read(struct file *filp, char __user *buf, size_t count,
			 loff_t *f_pos)
{
	/*if needed*/
	return 0;
...
static ssize_t egis_write(struct file *filp, const char __user *buf,
...
static long egis_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case INT_TRIGGER_INIT:
		...
		retval = Interrupt_Init(egis, data.int_mode, data.detect_period,
...
#ifdef CONFIG_COMPAT
static long egis_compat_ioctl(struct file *filp, unsigned int cmd,
			      unsigned long arg)
{
	DEBUG_PRINT("[egis] %s", __func__);
...
int egis_platformInit(struct egis_data *egis)
{
	...
	if (egis != NULL) {
		...
		if (status < 0) {
			DEBUG_PRINT(
				"[egis] %s gpio_direction_input IRQ failed\n",
				__func__);
		...
		if (status < 0) {
			DEBUG_PRINT(
				"[egis] %s gpio_direction_output rstPin failed\n",
				__func__);
			goto egis_platformInit_gpio_init_failed;
		}
	}
...
static const struct file_operations egis_fops = { .owner = THIS_MODULE,
						  .write = egis_write,
						  .read = egis_read,
...
static int egis_probe(struct platform_device *pdev)
{
	...
	if (minor < N_PD_MINORS) {
		...
		fdev = device_create(egis_class, &pdev->dev, egis->devt, egis,
				     "esfp0");
