...
static int egis_opticalfp_irq_handler(struct fp_underscreen_info *tp_info)
{
	...
	wake_lock_timeout(&et713_wake_lock,
				      ...
				      unsigned long val, void *data)
{
	...
	if (val == MTK_ONSCREENFINGERPRINT_EVENT) {
		...
		default:
			DEBUG_PRINT(
				"[egis] Unknown MTK_ONSCREENFINGERPRINT_EVENT!\n");
			break;
		}
		return retval;
	}
		   ...
		   int detect_period, int detect_threshold)
{
	...
	DEBUG_PRINT(
		"EGISTEC   %s request_irq_done = %d gpio_irq = %d  pin = %d  \n",
	...
	if (request_irq_done == 0) {
		...
		DEBUG_PRINT(
			"[EGISTEC Interrupt_Init] flag current: %d disable: %d enable: %d\n",
			fps_ints.drdy_irq_flag, DRDY_IRQ_DISABLE,
			DRDY_IRQ_ENABLE);
...
ssize_t egistec_read(struct file *filp, char __user *buf, size_t count,
		     loff_t *f_pos)
{
...
ssize_t egistec_write(struct file *filp, const char __user *buf, size_t count,
		      loff_t *f_pos)
{
	/*Implement by vendor if needed*/
	return 0;
...
int egistec_spi_pin(struct egistec_data *egistec, int spi_pulllow)
{
	...
	if (spi_pulllow) {
		pinctrl_select_state(egistec->pinctrl_gpios,
				     egistec->pstate_spi_6mA_pull_low);
	} else {
		pinctrl_select_state(egistec->pinctrl_gpios,
				     egistec->pstate_spi_6mA);
	}
	return 0;
}
...
long egistec_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FP_SENSOR_RESET:
		DEBUG_PRINT(
			"EGISTEC fp_ioctl ioc->opcode == FP_SENSOR_RESET \n");
	...
	case FP_POWER_ONOFF:
		...
		egis_power_onoff(
			egistec,
			data.int_mode); // Use data.int_mode as power setting. 1 = on, 0 = off.
...
#ifdef CONFIG_COMPAT
long egistec_compat_ioctl(struct file *filp, unsigned int cmd,
			  unsigned long arg)
{
...
int egistec_parse_dt(struct device *dev, struct egistec_data *data)
{
	...
	if (node) {
		...
		if (pdev) {
			...
			data->pstate_spi_6mA = pinctrl_lookup_state(
				data->pinctrl_gpios, "gf_spi_drive_6mA");
			if (IS_ERR(data->pstate_spi_6mA)) {
				ret = PTR_ERR(data->pstate_spi_6mA);
				printk(KERN_ERR
				       "%s can't find fingerprint pinctrl pstate_spi_6mA\n",
				       __func__);
				return ret;
			}
			data->pstate_spi_6mA_pull_low = pinctrl_lookup_state(
				data->pinctrl_gpios,
				"gf_spi_drive_6mA_pulllow");
			if (IS_ERR(data->pstate_spi_6mA_pull_low)) {
				ret = PTR_ERR(data->pstate_spi_6mA_pull_low);
				printk(KERN_ERR
				       "%s can't find fingerprint pinctrl pstate_spi_6mA_pull_low\n",
				       __func__);
				return ret;
			}
			data->pstate_default = pinctrl_lookup_state(
				data->pinctrl_gpios, "default");
			if (IS_ERR(data->pstate_default)) {
				ret = PTR_ERR(data->pstate_default);
				printk(KERN_ERR
				       "%s can't find fingerprint pinctrl pstate_default\n",
				       __func__);
				return ret;
			}
			pinctrl_select_state(data->pinctrl_gpios,
					     data->pstate_default);
...
#endif
const struct file_operations egistec_fops = { .owner = THIS_MODULE,
					      .write = egistec_write,
					      .read = egistec_read,
					      .unlocked_ioctl = egistec_ioctl,
					      .compat_ioctl =
						      egistec_compat_ioctl,
					      .open = egistec_open,
