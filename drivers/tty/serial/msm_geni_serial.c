...
#define UART_LOG_DBG(ctxt, dev, fmt...)                                        \
	do {                                                                   \
		ipc_log_string(ctxt, fmt);                                     \
		ftrace_dbg(dev, fmt);                                          \
	} while (0)

#define ftrace_dbg(dev, fmt, ...) __ftrace_dbg(dev, fmt, ##__VA_ARGS__)
...
static void wait_for_transfers_inflight(struct uart_port *uport)
{
	...
	if (check_transfers_inflight(uport)) {
		...
		u32 rx_fifo_status = geni_read_reg_nolog(
			uport->membase, SE_GENI_RX_FIFO_STATUS);
		u32 rx_dma =
			geni_read_reg_nolog(uport->membase, SE_DMA_RX_LEN_IN);
							...
							unsigned int mctrl)
{
	...
	UART_LOG_DBG(port->ipc_log_misc, uport->dev,
		     ...
		     current->comm, mctrl, uart_manual_rfr,
		...
		if (timeout) {
			...
			if (timeout) {
				...
				geni_read_reg_nolog(uport->membase,
							SE_GENI_STATUS));
				UART_LOG_DBG(
					msm_port->ipc_log_misc, uport->dev,
					"%s: tx abort failed 0x%x\n", __func__,
					geni_read_reg_nolog(uport->membase,
							    SE_GENI_STATUS));
				msm_geni_update_uart_error_code(msm_port,
...
static void stop_tx_sequencer(struct uart_port *uport)
{
	...
	UART_LOG_DBG(port->ipc_log_misc, uport->dev, "%s: Start GENI: 0x%x\n",
					...
					struct msm_geni_serial_port *port)
{
	...
	if (!enable) {
		...
		uart_manual_rfr = geni_read_reg_nolog(port->uport.membase,
							SE_UART_MANUAL_RFR);
		UART_LOG_DBG(
			port->ipc_log_misc, port->uport.dev,
			...
			bool drop_rx)
{
	...
	dump_ipc(uport, msm_port->ipc_log_rx, "Rx", (char *)msm_port->rx_fifo,
		 0, rx_bytes);
	return ret;
}
	...
	if (dma_rx_status) {
		...
		if (dma_rx_status & UART_DMA_RX_ERRS) {
			...
			UART_LOG_DBG(msm_port->ipc_log_misc, uport->dev,
				     ...
				     dma_rx_status, uport->icount.brk);
			msm_geni_update_uart_error_code(msm_port,
			UART_ERROR_RX_BREAK_ERROR);
		}
	...
	if (!dma) {
		...
		if (s_irq_status & (S_GP_IRQ_0_EN | S_GP_IRQ_1_EN)) {
			...
			UART_LOG_DBG(msm_port->ipc_log_misc, uport->dev,
				     ...
				     s_irq_status, uport->icount.parity);
			drop_rx = true;
	...
	if (!uart_console(uport)) {
		if (msm_port->ioctl_count) {
			...
			for (i = 0; i < msm_port->ioctl_count; i++) {
				UART_LOG_DBG(
					msm_port->ipc_log_pwr, uport->dev,
				...
				struct ktermios *termios, struct ktermios *old)
{
	...
	UART_LOG_DBG(port->ipc_log_misc, uport->dev, "%s: baud %d\n", __func__,
		     baud);
	UART_LOG_DBG(port->ipc_log_misc, uport->dev,
		     "Tx: trans_cfg%d parity %d\n", tx_trans_cfg,
		     tx_parity_cfg);
	UART_LOG_DBG(port->ipc_log_misc, uport->dev,
		     "Rx: trans_cfg%d parity %d\n", rx_trans_cfg,
		     rx_parity_cfg);
...
static int msm_geni_serial_get_ver_info(struct uart_port *uport)
{
	...
	UART_LOG_DBG(msm_port->ipc_log_misc, uport->dev, "%s: FW Ver:0x%x%x\n",
		     __func__, msm_port->ver_info.m_fw_ver,
...
#ifdef OPLUS_FEATURE_CHG_BASIC
static bool oplus_charge_id_reconfig(struct platform_device *pdev,
				     struct uart_driver *drv)
{
	...
	if (drv == &msm_geni_console_driver) {
		...
		if (IS_ERR_OR_NULL(serial_pinctrl)) {
			dev_err(&pdev->dev,
				"No serial_pinctrl config specified!\n");
		} else {
			serial_pinctrl_state_disable = pinctrl_lookup_state(
				serial_pinctrl, PINCTRL_SLEEP);
			if (IS_ERR_OR_NULL(serial_pinctrl_state_disable)) {
				dev_err(&pdev->dev,
					"No serial_pinctrl_state_disable config specified!\n");
			} else {
