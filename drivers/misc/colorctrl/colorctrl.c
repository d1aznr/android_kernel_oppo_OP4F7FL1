...
static void
colorctrl_transparent_recharge_operation(struct color_ctrl_device *cd);
...
static int colorctrl_power_control(struct color_ctrl_hw_resource *hw_res,
				   int vol, bool on)
{
	int ret = 0;
	...
	if (on) {
		if (!IS_ERR_OR_NULL(hw_res->vm)) {
			COLOR_INFO("enable the vm power to voltage : %d(mV).",
				   vol);
			if (vol) {
				ret = regulator_set_voltage(hw_res->vm,
							    vol * UV_PER_MV,
							    vol * UV_PER_MV);
				if (ret) {
					COLOR_INFO(
						"Regulator vm set voltage failed, ret = %d",
						ret);
					return ret;
				}
			}
			...
			if (ret) {
				COLOR_INFO(
					"Regulator vm enable failed, ret = %d",
					ret);
				return ret;
			}
			if (gpio_is_valid(hw_res->vm_enable_gpio)) {
				...
				ret = gpio_direction_output(
					hw_res->vm_enable_gpio, GPIO_HIGH);
				if (ret) {
					COLOR_INFO(
						"enable vm_enable_gpio fail.");
					return ret;
				}
			}
		}
		...
		if (!IS_ERR_OR_NULL(hw_res->vm)) {
			if (gpio_is_valid(hw_res->vm_enable_gpio)) {
				...
				ret = gpio_direction_output(
					hw_res->vm_enable_gpio, GPIO_LOW);
				if (ret) {
					COLOR_INFO(
						"disable vm_enable_gpio fail.");
					return ret;
				}
			}
			...
			if (ret) {
				COLOR_INFO(
					"Regulator vm disable failed, ret = %d",
					ret);
				return ret;
			}
		}
...
static void colorctrl_recharge_work(struct work_struct *work)
{
	...
	if (cd->color_status == BLUE) {
		...
		COLOR_INFO(
			"not need to do the recharge work, color_status : %d.",
			cd->color_status);
	}
...
static void colorctrl_reset_hrtimer(struct color_ctrl_device *cd)
{
	...
	hrtimer_start(&cd->hrtimer, ktime_set(cd->recharge_time, 0),
		      HRTIMER_MODE_REL);
}
...
static int colorctrl_get_fitting_temperature(struct color_ctrl_device *cd,
					     s64 *fitting_temp)
{
	...
	for (i = 0; i < cd->thermal_zone_device_num; i++) {
		...
		if (ret) {
			...
			COLOR_INFO("current %s temperature is : %d",
...
static void colorctrl_blue_recharge_operation(struct color_ctrl_device *cd)
{
	...
	} else if (vm_volt >= para->recharge_volt_thd_1 &&
		   vm_volt < para->recharge_volt_thd_2) {
	...
	for (i = 0; i < 5; i++) {
		if (vm_volt < cd->open_circuit_thd) {
			COLOR_INFO(
				"volt is too low, try to do normal charging.");
			colorctrl_power_control(cd->hw_res, charge_volt, true);
			gpio_direction_output(hw_res->sleep_en_gpio, GPIO_HIGH);
			colorctrl_msleep(charge_time);
			gpio_direction_output(hw_res->sleep_en_gpio, GPIO_LOW);
			colorctrl_power_control(cd->hw_res, 0, false);
		} else if (vm_volt >= cd->open_circuit_thd &&
			   vm_volt < cd->blue_short_circuit_thd) {
			COLOR_INFO("volt is too low, try to do recharging.");
			colorctrl_power_control(cd->hw_res, para->recharge_volt,
						true);
			gpio_direction_output(hw_res->sleep_en_gpio, GPIO_HIGH);
			colorctrl_msleep(para->recharge_time);
			gpio_direction_output(hw_res->sleep_en_gpio, GPIO_LOW);
...
OUT:
	...
	gpio_direction_output(hw_res->sleep_en_gpio, GPIO_LOW);
	COLOR_INFO("recharge color to blue %s.",
...
static void colorctrl_blue_operation(struct color_ctrl_device *cd, bool is_ftm)
{
	...
	if (cd->temp_status == ABNORMAL_TEMP) {
		COLOR_INFO(
			"abnormal temperature occur, can not do normal charge operation");
		return;
	}
	...
	if (is_ftm) {
		...
		cd->color_status = BLUE;
		COLOR_INFO(
			"ftm mode operation, no need to do voltage detection.");
		goto OUT;
	}
	...
	for (i = 0; i < 5; i++) {
		if (vm_volt < cd->open_circuit_thd) {
			...
			colorctrl_power_control(cd->hw_res, 0, false);
			   ...
			   vm_volt < cd->transparent_short_circuit_thd) {
			COLOR_INFO("volt is too low, try to do recharging.");
			colorctrl_power_control(cd->hw_res, para->recharge_volt,
						true);
			gpio_direction_output(hw_res->sleep_en_gpio, GPIO_HIGH);
...
OUT:
	...
	gpio_direction_output(hw_res->si_in_2_gpio, GPIO_LOW);
	gpio_direction_output(hw_res->sleep_en_gpio, GPIO_LOW);
	COLOR_INFO("recharge color to transparent %s.",
		   ret < 0 ? "failed" : "success");

	return;
}

static void colorctrl_transparent_operation(struct color_ctrl_device *cd,
					    bool is_ftm)
{
	struct color_ctrl_hw_resource *hw_res = NULL;
	int ret = 0, vm_volt = 0, i = 0;
...
OUT:
	...
	gpio_direction_output(hw_res->si_in_2_gpio, GPIO_LOW);
	gpio_direction_output(hw_res->sleep_en_gpio, GPIO_LOW);
	COLOR_INFO("change color to transparent %s.",
		   ret < 0 ? "failed" : "success");
...
static void colorctrl_light_blue_operation(struct color_ctrl_device *cd)
{
	...
	COLOR_INFO("change color to light blue %s.",
		   ret < 0 ? "failed" : "success");
				    ...
				    loff_t *ppos)
{
	...
	if (cd->color_status == OPEN_CIRCUIT ||
	    cd->color_status == SHORT_CIRCUIT) {
		COLOR_INFO(
			"device is in bad status, can not do any operation, color_status : %d",
			cd->color_status);
		return count;
	}
	...
	case RESET_FTM: {
		...
		if (cd->color_status == UNKNOWN || cd->color_status == BLUE) {
			...
			COLOR_INFO(
				"current color status is : %d, not need to do any operation.",
				cd->color_status);
		}
		break;
	}
				      ...
				      size_t count, loff_t *ppos)
{
	...
	default:
		COLOR_INFO(
			"not support voltage read type, current color status : %d.",
			cd->color_status);
		goto OUT;
		break;
	}
...
static int colorctrl_main_parameter_read(struct seq_file *s, void *v)
{
	...
	seq_printf(
		s,
		"blue charge control parameter :\n        temp_range_1:[%d, %d], temp_range_2:[%d, %d], temp_range_3:[%d, %d]\n\
        temp_range_4:[%d, %d], temp_range_5:[%d, %d], temp_range_6:[%d, %d]\n",
		blue_para->charge_volt[0], blue_para->charge_time[0],
		blue_para->charge_volt[1], blue_para->charge_time[1],
		blue_para->charge_volt[2], blue_para->charge_time[2],
		blue_para->charge_volt[3], blue_para->charge_time[3],
		blue_para->charge_volt[4], blue_para->charge_time[4],
		blue_para->charge_volt[5], blue_para->charge_time[5]);
	seq_printf(
		s,
		"transparent charge control parameter :\n        temp_range_1:[%d, %d], temp_range_2:[%d, %d], temp_range_3:[%d, %d]\n\
        temp_range_4:[%d, %d], temp_range_5:[%d, %d], temp_range_6:[%d, %d]\n",
		transparent_para->charge_volt[0],
		transparent_para->charge_time[0],
		transparent_para->charge_volt[1],
		transparent_para->charge_time[1],
		transparent_para->charge_volt[2],
		transparent_para->charge_time[2],
		transparent_para->charge_volt[3],
		transparent_para->charge_time[3],
		transparent_para->charge_volt[4],
		transparent_para->charge_time[4],
		transparent_para->charge_volt[5],
		transparent_para->charge_time[5]);
	...
	seq_printf(s, "transparent recharge control parameter : [%d, %d]\n",
		   transparent_para->recharge_volt,
		   transparent_para->recharge_time);
	seq_printf(s, "blue recharge voltage thd : [%d(mV), %d(mV)]\n",
		   blue_para->recharge_volt_thd_1,
		   blue_para->recharge_volt_thd_2);
	seq_printf(s, "transparent recharge voltage thd : [%d(mV), %d(mV)]\n",
		   transparent_para->recharge_volt_thd_1,
		   transparent_para->recharge_volt_thd_2);
	mutex_unlock(&cd->rw_lock);
						   ...
						   size_t count, loff_t *ppos)
{
	...
	if (!strcmp(name, "recharge_time") && (value_cnt == 1)) {
	...
	} else if (!strcmp(name, "intermediate_wait_time") &&
		   (value_cnt == 1)) {
		cd->intermediate_wait_time = split_value[0];
		COLOR_INFO("%s is change to %d.", name, split_value[0]);
	} else if (!strcmp(name, "volt_measure_interval_time") &&
		   (value_cnt == 1)) {
		...
		COLOR_INFO(
			"%s is change : low temp [%d, %d], normal temp [%d, %d], high temp [%d, %d].",
			name, split_value[0], split_value[1], split_value[2],
			split_value[3], split_value[4], split_value[5]);
		...
		COLOR_INFO("%s is change : [%d, %d, %d].", name, split_value[0],
			   split_value[1], split_value[2]);
		...
		COLOR_INFO(
			"%s is change : temp range(1) [%d, %d], temp range(2) [%d, %d], temp range(3) [%d, %d], temp range(4) [%d, %d], temp range(5) [%d, %d], temp range(6) [%d, %d].",
			name, split_value[0], split_value[1], split_value[2],
			split_value[3], split_value[4], split_value[5],
			split_value[6], split_value[7], split_value[8],
			split_value[9], split_value[10], split_value[11]);
		...
		COLOR_INFO("%s is change : [%d, %d].", name, split_value[0],
			   split_value[1]);
	} else if (!strcmp(name, "transparent_charge_para") &&
		   (value_cnt == 12)) {
		...
		COLOR_INFO("%s is not support or input value count is wrong.",
			   name);
	}
...
static int colorctrl_init_proc(struct color_ctrl_device *cd)
{
	...
	prEntry_tmp = proc_create_data("color_ctrl", 0666, prEntry_cr,
	...
	prEntry_tmp = proc_create_data("temperature", 0666, prEntry_cr,
	...
	prEntry_tmp = proc_create_data("voltage", 0666, prEntry_cr,
	...
	prEntry_tmp = proc_create_data("main_parameter", 0666, prEntry_cr,
...
static int colorctrl_parse_dt(struct device *dev, struct color_ctrl_device *cd)
{
	...
	blue_para = devm_kzalloc(dev, sizeof(struct color_ctrl_control_para),
				 GFP_KERNEL);
	if (!blue_para) {
	...
	COLOR_INFO(
		"Parse dt ok, get gpios:[sleep_en_gpio:%d si_in_1_gpio:%d si_in_2_gpio:%d]",
		hw_res->sleep_en_gpio, hw_res->si_in_1_gpio,
		hw_res->si_in_2_gpio);

	ret = of_property_read_u32_array(
		dev->of_node, "colorctrl,recharge_time", temp_array, 1);
	if (ret) {
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "colorctrl,intermediate_wait_time",
					 temp_array, 1);
	if (ret) {
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "colorctrl,volt_measure_interval_time",
					 temp_array, 1);
	if (ret) {
	...
	ret = of_property_read_u32_array(
		dev->of_node, "colorctrl,temperature_thd", temp_array, 7);
	if (ret) {
		cd->temp_thd_1 = 0;
		cd->temp_thd_2 = 5000;
		cd->temp_thd_3 = 10000;
	...
	ret = of_property_read_u32_array(
		dev->of_node, "colorctrl,abnormal_volt_thd", temp_array, 3);
	if (ret) {
	...
	ret = of_property_read_u32_array(
		dev->of_node, "colorctrl,blue_charge_para", temp_array, 12);
	if (ret) {
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "colorctrl,transparent_charge_para",
					 temp_array, 12);
	if (ret) {
	...
	ret = of_property_read_u32_array(
		dev->of_node, "colorctrl,blue_recharge_para", temp_array, 2);
	if (ret) {
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "colorctrl,transparent_recharge_para",
					 temp_array, 2);
	if (ret) {
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "colorctrl,blue_recharge_volt_thd",
					 temp_array, 2);
	if (ret) {
	...
	ret = of_property_read_u32_array(
		dev->of_node, "colorctrl,transparent_recharge_volt_thd",
		temp_array, 2);
	if (ret) {
		transparent_para->recharge_volt_thd_1 = 100;
		transparent_para->recharge_volt_thd_2 = 400;
		COLOR_INFO(
			"transparent recharge voltage threshold using default.");
	} else {
...
static int colorctrl_probe(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(hw_res->vm_enable_gpio)) {
		ret = devm_gpio_request(&pdev->dev, hw_res->vm_enable_gpio,
					"vm_enable_gpio");
		if (ret) {
			COLOR_INFO("request vm_enable_gpio fail.");
			goto PROBE_ERR;
		} else {
			ret = gpio_direction_output(hw_res->vm_enable_gpio,
						    GPIO_LOW);
			if (ret) {
				COLOR_INFO(
					"Config vm_enable_gpio gpio output direction fail.");
				goto PROBE_ERR;
			}
		}
	}
