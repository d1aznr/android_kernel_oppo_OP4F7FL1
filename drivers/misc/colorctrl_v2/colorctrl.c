...
static ssize_t proc_ageing_para_read(struct file *file, char __user *user_buf,
				     size_t count, loff_t *ppos);
static ssize_t proc_ageing_para_write(struct file *file,
...
static int check_adc_status(struct color_ctrl_device *cd)
{
	...
	if (ret < 0) {
		COLOR_INFO(
			"iio_read_channel_processed get error ret = %d, err_vm = %d",
			ret, vm_volt);
		goto OUT;
	}
...
static int coloring_power_control(struct color_ctrl_device *cd, int vol,
				  bool on)
{
	struct color_ctrl_hw_resource *hw_res = cd->hw_res;
	int ret = 0;
	...
	if (on) {
		if (!IS_ERR_OR_NULL(hw_res->coloring)) {
			COLOR_INFO(
				"enable the coloring power to voltage : %dmV.",
				vol);

			if (vol) {
				ret = regulator_set_voltage(hw_res->coloring,
							    vol * UV_PER_MV,
							    vol * UV_PER_MV);

				if (ret) {
					COLOR_INFO(
						"Regulator coloring set voltage failed, ret = %d",
						ret);
					goto OUT;
				}
			}
			...
			if (ret) {
				COLOR_INFO(
					"Regulator coloring enable failed, ret = %d",
					ret);
				goto OUT;
			}
			...
			if (gpio_is_valid(hw_res->vm_coloring_gpio)) {
				...
				ret = gpio_direction_output(
					hw_res->vm_coloring_gpio, GPIO_HIGH);

				if (ret) {
					COLOR_INFO(
						"enable vm_coloring_gpio fail.");
					goto OUT;
				}
			}
		}
		...
		if (!IS_ERR_OR_NULL(hw_res->coloring)) {
			if (gpio_is_valid(hw_res->vm_coloring_gpio)) {
				...
				ret = gpio_direction_output(
					hw_res->vm_coloring_gpio, GPIO_LOW);

				if (ret) {
					COLOR_INFO(
						"disable vm_coloring_gpio fail.");
					goto OUT;
				}
			}
			...
			if (ret) {
				COLOR_INFO(
					"Regulator coloring disable failed, ret = %d",
					ret);
				goto OUT;
			}
		}
...
static int fade_power_control(struct color_ctrl_device *cd, int vol, bool on)
{
	...
	if (on) {
		if (!IS_ERR_OR_NULL(hw_res->fade)) {
			COLOR_INFO("enable the fade power to voltage : %dmV.",
				   vol);

			if (vol) {
				ret = regulator_set_voltage(hw_res->fade,
							    vol * UV_PER_MV,
							    vol * UV_PER_MV);

				if (ret) {
					COLOR_INFO(
						"Regulator fade set voltage failed, ret = %d",
						ret);
					goto OUT;
				}
			}
			...
			if (ret) {
				COLOR_INFO(
					"Regulator fade enable failed, ret = %d",
					ret);
				goto OUT;
			}
		...
		if (!IS_ERR_OR_NULL(hw_res->fade)) {
			if (gpio_is_valid(hw_res->vm_fade_gpio)) {
				...
				ret = gpio_direction_output(
					hw_res->vm_fade_gpio, GPIO_LOW);

				if (ret) {
					COLOR_INFO(
						"disable vm_fade_gpio fail.");
					goto OUT;
				}
			}
			...
			if (ret) {
				COLOR_INFO(
					"Regulator fade disable failed, ret = %d",
					ret);
				goto OUT;
			}
		}
...
static void ready_to_recharge(struct color_ctrl_device *cd)
{
	...
	case COLOR: /*red_open_circuit_vol  = <1350 1550 1550>;*/
		if (cd->is_first_boot) {
			...
			if (vm_volt > cd->red_open_circuit_vol[1]) {
				COLOR_INFO(
					"[normal boot] vm_volt is %dmV>%dmV, nothing to red recharge, reset timer\n",
					vm_volt, cd->red_open_circuit_vol[1]);
				colorctrl_reset_hrtimer(
					cd, cd->coloring_recharge_waitime);
				goto OUT;
			} else if (vm_volt <= cd->red_open_circuit_vol[1]) {
				COLOR_INFO(
					"[normal boot] vm_volt is %dmV<=%dmV\n, need to recharge",
					vm_volt, cd->red_open_circuit_vol[1]);
				ec_recharge_operation(cd, false);
			}
		}
	...
	case TRANSPARENT: /*fade_open_circuit_vol = <1100 1300 1350>;*/
		...
		if (vm_volt > cd->fade_open_circuit_vol[1]) {
			...
			COLOR_INFO("vm_volt is %dmV>%dmV , need to fade charge",
				   vm_volt, cd->fade_open_circuit_vol[1]);
			ec_charge_operation(cd, false);
			cd->operation = UNKNOWN;
		} else if (vm_volt <= cd->fade_open_circuit_vol[1] &&
			   vm_volt >= cd->fade_open_circuit_vol[0]) {
			COLOR_INFO(
				"vm_volt is %dmV->[%dmV,%dmV] , need to fade recharge",
				vm_volt, cd->fade_open_circuit_vol[0],
				cd->fade_open_circuit_vol[1]);
			ec_recharge_operation(cd, false);
		} else if (vm_volt < cd->fade_open_circuit_vol[0]) {
			COLOR_INFO(
				"vm_volt is %dmV<%dmV, nothing to fade recharge, reset timer\n",
				vm_volt, cd->fade_open_circuit_vol[0]);
...
{
	struct color_ctrl_device *cd =
		container_of(work, struct color_ctrl_device, recharge_work);
	...
	COLOR_INFO("is call. color_status: %d, operation: %d", cd->color_status,
		   cd->operation);

	if (!cd->need_recharge) {
		COLOR_INFO(
			"not need to do the recharge work due to force close.");
		goto OUT;
	}
...
{
	struct color_ctrl_device *cd =
		container_of(timer, struct color_ctrl_device, hrtimer);
...
static int get_fitting_temperature(struct color_ctrl_device *cd,
				   s64 *fitting_temp)
{
	...
	for (i = 0; i < cd->thermal_zone_device_num; i++) {
		...
		if (ret) {
			COLOR_INFO("fail to get %s temperature: %d",
				   cd->thermal_zone_device_name[i], ret);
			return -1;
		}

		*fitting_temp += (s64)temp * cd->thermal_zone_device_weight[i];
		COLOR_DEBUG("current %s , temperature is : %d, fit_temp is %d",
			    cd->thermal_zone_device_name[i], temp,
			    *fitting_temp);
	}

	*fitting_temp = *fitting_temp / 10000000;
	*fitting_temp +=
		cd->thermal_zone_device_weight[cd->thermal_zone_device_num] /
		10000;
...
static void update_temperature_status(struct color_ctrl_device *cd)
{
	...
	for (i = 0; i < cd->temp_num; i++) {
		if (temp >= cd->temperature_thd[i] &&
		    temp < cd->temperature_thd[i + 1]) {
			cd->temp_status = i;
			COLOR_DEBUG(
				"match %d success !, temp * 1000 is %d include (%d ~ %d)",
				i, temp, cd->temperature_thd[i],
				cd->temperature_thd[i + 1]);
			return;
		}
	}
...
static int do_operation(struct color_ctrl_device *cd, int charge_volt,
			int charge_time)
{
	int ret = 0;
	...
	COLOR_INFO(
		"charge_volt is %dmV, charge time is %dms, status: %d, operation: %d",
	...
	case DO_COLORING:
		if (cd->ec_step_voltage_support && cd->pcb_version >= PVT) {
			...
			COLOR_DEBUG("step voltage is %dmV, time is %ds",
				    charge_volt, charge_time);
			if (0 == charge_volt && 0 == charge_time &&
			    cd->ec_step_voltage_support &&
			    cd->pcb_version >= PVT) {
		...
		if (charge_volt >= MAX_CHANGE_VOLTAGE) {
			COLOR_INFO(
				"now vol is %dmV, need to enable PM8008 power.",
				charge_volt);
		...
		if (charge_volt >= MAX_CHANGE_VOLTAGE) {
			COLOR_INFO(
				"now vol is %dmV, need to disable PM8008 power.",
				charge_volt);
...
static int get_cur_para(struct color_ctrl_device *cd,
			struct color_ctrl_control_para *para)
{
	...
	if ((cd->color_status == COLOR || cd->operation == DO_COLORING_EVT ||
	     cd->operation == DO_COLORING) &&
	...
	if (!ageing_flag) {
		cd->charge_volt =
			para->coloring_charge_para_1[cd->temp_status][0];
		cd->charge_time =
			para->coloring_charge_para_1[cd->temp_status][1];
		cd->recharge_volt =
			para->coloring_charge_para_1[cd->temp_status][2];
		cd->recharge_time =
			para->coloring_charge_para_1[cd->temp_status][3];

		if (cd->ec_step_voltage_support == true &&
		    cd->operation == DO_COLORING) {
			cd->step_para[0] =
				para->coloring_charge_para_1[cd->temp_status][4];
			cd->step_para[1] =
				para->coloring_charge_para_1[cd->temp_status][5];
			COLOR_DEBUG("step para is %d and %d", cd->step_para[0],
				    cd->step_para[1]);
		}
	} else if (ageing_flag) {
		cd->charge_volt =
			para->coloring_charge_para_2[cd->temp_status][0];
		cd->charge_time =
			para->coloring_charge_para_2[cd->temp_status][1];
		cd->recharge_volt =
			para->coloring_charge_para_2[cd->temp_status][2];
		cd->recharge_time =
			para->coloring_charge_para_2[cd->temp_status][3];
	}

	COLOR_DEBUG(
		" ageing_flag:[%d] color_status: %d, operation: %d, get_cur_para[%d]: charge_volt[%d], charge_time[%d], recharge_volt[%d], recharge_time[%d]",
		ageing_flag, cd->color_status, cd->operation, cd->temp_status,
		cd->charge_volt, cd->charge_time, cd->recharge_volt,
		cd->recharge_time);
...
static int effective_check(struct color_ctrl_device *cd)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if (vm_volt < 0) {
			COLOR_INFO(
				"iio_read_channel_processed get error ret = %d",
				vm_volt);
			goto OUT;
		}
		get_open_circuit_vol[i] = vm_volt;
	}
	COLOR_INFO("V1:%dmV, V2:%dmV, do operation=%d", get_open_circuit_vol[0],
		   get_open_circuit_vol[1], cd->operation);
	switch (cd->operation) {
	...
	case DO_COLORING_EVT:
		if (get_open_circuit_vol[0] > cd->fade_to_red_check_volt[1] &&
			...
			COLOR_INFO(
				"(%ldmV<V1:%ldmV<%ldmV) && (%ldmV<V2:%ldmV<%ldmV)",
				cd->fade_to_red_check_volt[1],
				get_open_circuit_vol[0],
				cd->fade_to_red_check_volt[2],
				cd->fade_to_red_check_volt[1],
				get_open_circuit_vol[1],
				cd->fade_to_red_check_volt[2]);
			cd->open_circuit_flag++;
			if (cd->open_circuit_flag > cd->fade_to_red_retry_cnt) {
				cd->color_status = OPEN_CIRCUIT;
				COLOR_INFO(
					"WARNING!!!  fade to coloring [OPEN_CIRCUIT]");
				goto OUT;
			} else {
		...
		} else if (get_open_circuit_vol[1] <
			   cd->fade_to_red_check_volt[0]) {
			COLOR_INFO("V2:%ldmV<%ldmV", get_open_circuit_vol[1],
				   cd->fade_to_red_check_volt[0]);
			cd->recharge_time_flag++;
			if (cd->recharge_time_flag >
			    cd->fade_to_red_retry_cnt) {
				cd->color_status = SHORT_CIRCUIT;
				COLOR_INFO(
					"WARNING!!!  fade to coloring [SHORT_CIRCUIT]");
				goto OUT;
			} else {
				COLOR_INFO("try again coloring recharge");
				cd->color_status = COLOR;
			   ...
			   cd->fade_to_red_check_volt[0]) {
			COLOR_INFO(
				"SUCCESS!!!   do coloring [V2:%ldmV>=%ldmV] start %ds timer",
				get_open_circuit_vol[1],
				cd->fade_to_red_check_volt[0],
	...
	case DO_RECHARGE_FADE:
		if (get_open_circuit_vol[1] > cd->red_to_fade_check_volt[0]) {
			...
			if (cd->open_circuit_flag > cd->red_to_fade_retry_cnt) {
				if (get_open_circuit_vol[1] >
					    cd->red_to_fade_check_volt[1] &&
				    get_open_circuit_vol[1] <
					    cd->red_to_fade_check_volt[2]) {
					COLOR_INFO(
						"WARNING!!!  coloring to fade[OPEN_CIRCUIT]");
					cd->color_status = OPEN_CIRCUIT;
				} else if (get_open_circuit_vol[1] >
						   cd->red_to_fade_check_volt[0] &&
					   get_open_circuit_vol[1] <=
						   cd->red_to_fade_check_volt[1]) {
					COLOR_INFO(
						"WARNING!!!  coloring to fade[SHORT_CIRCUIT]");
					cd->color_status = SHORT_CIRCUIT;
				}
				goto OUT;
			} else {
			   ...
			   cd->red_to_fade_check_volt[0]) {
			COLOR_INFO(
				"SUCCESS!!!   do fade [V2:%ldmV<=%ldmV] start %ds timer",
				get_open_circuit_vol[1],
				cd->red_to_fade_check_volt[0],
...
OUT:
	...
	COLOR_INFO("effect check over, operation: %d, color_status:%s",
		   cd->operation,
		   cd->color_status > 2 ?
			   (cd->color_status > 3 ? "open circurt" :
						   "short circurt") :
			   (cd->color_status > 1 ? "fade" : "red"));
	return 0;
}
...
static int ec_recharge_operation(struct color_ctrl_device *cd, bool is_ftm)
{
	...
	COLOR_DEBUG("ec_recharge_operation, status:%d operation:%d",
		    cd->color_status, cd->operation);
	...
	if (cd->temp_status == ABNORMAL_ZONE) {
		COLOR_INFO(
			"abnormal temperature occur, can not do recharge operation");
		goto OUT;
	}
	...
	if (is_ftm) {
		if (cd->operation == DO_COLORING_EVT ||
		    cd->operation == DO_RECHARGE_COLORING ||
		    cd->operation == DO_COLORING) {
			cd->color_status = COLOR;
			colorctrl_reset_hrtimer(cd,
						cd->coloring_recharge_waitime);
		} else if (cd->operation == DO_FADE ||
			   cd->operation == DO_RECHARGE_FADE) {
			cd->color_status = TRANSPARENT;
			colorctrl_reset_hrtimer(cd, cd->fade_recharge_waitime);
		}
...
OUT:
	...
	COLOR_INFO("color status is %s\n",
		   cd->color_status > 2 ?
			   (cd->color_status > 3 ? "short circuit" :
						   "open circuit") :
			   (cd->color_status > 1 ? "fade" : "red"));
	close_all_gpio_operation(cd);
	return 0;
...
static int ec_charge_operation(struct color_ctrl_device *cd, bool is_ftm)
{
	...
	COLOR_DEBUG("ec_charge_operation, status:%d operation:%d",
		    cd->color_status, cd->operation);
...
static int ec_reset_operation(struct color_ctrl_device *cd)
{
	...
	if (vm_volt > cd->reset_ftm_vol[0] && vm_volt <= cd->reset_ftm_vol[2]) {
		COLOR_INFO(
			"reset_ftm_vol:%dmv is (%dmV , %dmV], need to recharge",
			vm_volt, cd->reset_ftm_vol[0], cd->reset_ftm_vol[2]);
		cd->color_status = COLOR;
		ec_recharge_operation(cd, false);
	} else if (vm_volt <= cd->reset_ftm_vol[0]) {
		COLOR_INFO("reset_ftm_vol:%dmv < %dmV, need to charge", vm_volt,
			   cd->reset_ftm_vol[0]);
		cd->color_status = UNKNOWN;
		if (cd->pcb_version >= PVT) {
			cd->operation = DO_COLORING;
		...
		COLOR_INFO("reset_ftm_vol:%dmv > %dmV, not to charge", vm_volt,
			   cd->reset_ftm_vol[2]);
	}
...
static ssize_t proc_colorctrl_write(struct file *file,
				    const char __user *buffer, size_t count,
				    loff_t *ppos)
{
	char buf[8] = { 0 };
	...
	if (cd->color_status == OPEN_CIRCUIT ||
	    cd->color_status == SHORT_CIRCUIT) {
...
static ssize_t proc_colorctrl_read(struct file *file, char __user *user_buf,
				   size_t count, loff_t *ppos)
{
	ssize_t ret = 0;
...
static ssize_t proc_other_status_read(struct file *file, char __user *user_buf,
				      size_t count, loff_t *ppos)
{
	ssize_t ret = 0;
	char page[PAGESIZE] = { 0 };
	...
	if (vm_volt < 0) {
		COLOR_INFO("iio_read_channel_processed get error ret = %d",
			   vm_volt);
	...
	snprintf(page, PAGESIZE - 1, "adc: %2dmV, version status: %s\n",
		 vm_volt,
		 cd->pcb_version > DVT ? "PVT" :
		 cd->pcb_version < DVT ? "EVT" :
					 "DVT");
...
static ssize_t proc_cover_color_read(struct file *file, char __user *user_buf,
				     size_t count, loff_t *ppos)
{
	ssize_t ret = 0;
	char page[PAGESIZE] = { 0 };
...
static ssize_t proc_temperature_control_read(struct file *file,
					     ...
					     size_t count, loff_t *ppos)
{
	...
	if (ret < 0) {
		...
		snprintf(page, PAGESIZE - 1,
			 "failed to get fitting temperature\n");
	} else {
...
static ssize_t colorctrl_voltage_read(struct file *file, char __user *user_buf,
				      size_t count, loff_t *ppos)
{
	ssize_t ret = 0;
	int vm_volt = 0;
...
OUT:
	close_all_gpio_operation(cd);
	mutex_unlock(&cd->rw_lock);
	return simple_read_from_buffer(user_buf, count, ppos, page,
				       strlen(page));
}
...
static int colorctrl_str_parse(char *in, char *name, unsigned int max_len,
		...
		if (in[i] < '0' || in[i] > '9') {
			if ((' ' == in[i]) || (0 == in[i]) || ('\n' == in[i]) ||
				...
				if (value_cnt <= array_max) {
					array[value_cnt++] =
						colorctrl_str_to_int(
							in, start_pos, i - 1);
					start_pos = i + 1;
				} else {
					COLOR_INFO("too many parameter(%s).",
						   in);
					return -1;
				}
			} else {
				COLOR_INFO("incorrect char 0x%02x in %s.",
					   in[i], in);
				return -1;
			}
		}
...
static int colorctrl_main_parameter_read(struct seq_file *s, void *v)
{
	...
	seq_printf(
		s,
		"now ageing enable : coloring_ageing: %d, fade_ageing:%d, s\n",
	...
	/*wait_check_time*/
	seq_printf(s, "wait_check_time <%d , %d>\n", cd->wait_check_time[0],
		   cd->wait_check_time[1]);
	seq_printf(s, "--------\n");

	/***open_circuit_vol***/
	seq_printf(s, "fade_open_circuit_vol: [%d][%d][%d]\n",
		   cd->fade_open_circuit_vol[0], cd->fade_open_circuit_vol[1],
		   cd->fade_open_circuit_vol[2]);
	seq_printf(s, "red_open_circuit_vol: [%d][%d][%d]\n",
		   cd->red_open_circuit_vol[0], cd->red_open_circuit_vol[1],
		   cd->red_open_circuit_vol[2]);
	seq_printf(s, "--------\n");
	...
	seq_printf(s, "fade_to_red_check_volt:[%d][%d][%d]\n",
		   cd->fade_to_red_check_volt[0], cd->fade_to_red_check_volt[1],
		   cd->fade_to_red_check_volt[2]);
	seq_printf(s, "red_to_fade_check_volt:[%d][%d][%d]\n",
		   cd->red_to_fade_check_volt[0], cd->red_to_fade_check_volt[1],
		   cd->red_to_fade_check_volt[2]);
	seq_printf(s, "--------\n");
	...
	for (i = 0; i < cd->temp_num; i++) {
		seq_printf(s, " [%d] ",
			   cd->temperature_thd[i] + cd->temperature_thd_offect);
	}
	seq_printf(s, "\n");
	seq_printf(s, "--------\n");
	...
	for (i = 0; i < cd->red_para_1_format[0]; i++) {
		seq_printf(s, "%2d ~ %2d:", cd->temperature_thd[i] / 1000,
			   cd->temperature_thd[i + 1] / 1000);
		for (j = 0; j < cd->red_para_1_format[1]; j++) {
			seq_printf(s, " %6d ",
				   color_para->coloring_charge_para_1[i][j]);
		}
		seq_printf(s, "\n");
	}
	seq_printf(s, "--------\n");
	...
	for (i = 0; i < cd->fade_para_1_format[0]; i++) {
		...
		for (j = 0; j < cd->fade_para_1_format[1]; j++) {
			seq_printf(
				s, " %6d ",
				transparent_para->coloring_charge_para_1[i][j]);
		}
		seq_printf(s, "\n");
	}
	seq_printf(s, "--------\n");
	...
	for (i = 0; i < cd->red_para_2_format[0]; i++) {
		...
		for (j = 0; j < cd->red_para_2_format[1]; j++) {
			seq_printf(s, " %6d ",
				   color_para->coloring_charge_para_2[i][j]);
		}
		seq_printf(s, "\n");
	}
	seq_printf(s, "--------\n");
	...
	for (i = 0; i < cd->fade_para_2_format[0]; i++) {
		...
		for (j = 0; j < cd->fade_para_2_format[1]; j++) {
			seq_printf(
				s, " %6d ",
				transparent_para->coloring_charge_para_2[i][j]);
		}
		seq_printf(s, "\n");
	}
	seq_printf(s, "--------\n");
...
{
	return single_open(file, colorctrl_main_parameter_read,
			   PDE_DATA(inode));
}

static ssize_t proc_colorctrl_main_parameter_write(struct file *file,
						   ...
						   size_t count, loff_t *ppos)
{
	...
	color_para = cd->color_control_para;
	transparent_para = cd->transparent_control_para;
	value_cnt = colorctrl_str_parse(buf, name, NAME_TAG_SIZE, split_value,
					MAX_PARAMETER);
	...
	if (strstr(name, "reset_ftm_vol") && (value_cnt == 3)) {
		for (i = 0; i < 3; i++) {
			...
			COLOR_DEBUG("reset_ftm_vol success is %d",
				    cd->fade_to_red_check_volt[i]);
		}
	...
	if (strstr(name, "fade_recharge_waitime") && (value_cnt == 1)) {
		...
		colorctrl_reset_hrtimer(cd, cd->fade_recharge_waitime);
		COLOR_INFO(
			"fade_recharge_waitime success,need to retimer -> %d, fade_mandatory_flag is %d",
			cd->fade_recharge_waitime, cd->fade_mandatory_flag);
	} else if (strstr(name, "coloring_recharge_waitime") &&
		   (value_cnt == 1)) {
		cd->coloring_recharge_waitime = split_value[0];
		COLOR_INFO("coloring_recharge_waitime success is %d",
			   cd->coloring_recharge_waitime);
	} else if (strstr(name, "coloring_recharge_waitime") &&
		   (value_cnt == 2)) {
		val = split_value[0];
		...
		COLOR_INFO(
			"coloring_recharge_waitime success,need to retimer -> %d, val is %d",
		...
		for (i = 0; i < 3; i++) {
			...
			COLOR_INFO("red_to_fade_check_volt success is %d",
				   cd->fade_to_red_check_volt[i]);
		}
		...
		for (i = 0; i < 3; i++) {
			...
			COLOR_INFO("red_to_fade_check_volt success is %d",
				   cd->red_to_fade_check_volt[i]);
		}
	} else if (strstr(name, "temperature_thd") &&
		   (value_cnt == cd->temp_num)) {
		for (i = 0; i < cd->temp_num; i++) {
			cd->temperature_thd[i] =
				split_value[i] - cd->temperature_thd_offect;
			COLOR_INFO("temperature_thd success is %d",
				   cd->temperature_thd[i]);
		}
	} else if (strstr(name, "temperature_thd_offect") && (value_cnt == 1)) {
		cd->temperature_thd_offect = split_value[0];
		COLOR_INFO("temperature_thd_offect success is %d",
			   cd->temperature_thd_offect);
	}

	/*red_charge_para_1 && fade_charge_para_1*/
	if (strstr(name, "red_charge_para_1") &&
	    (value_cnt == cd->red_para_1_format[1] + 1)) {
		i = split_value[0];
		for (j = 0; j < cd->red_para_1_format[1]; j++) {
			color_para->coloring_charge_para_1[i][j] =
				split_value[j + 1];
	...
	} else if (strstr(name, "fade_charge_para_1") &&
		   (value_cnt == cd->fade_para_1_format[1] + 1)) {
		i = split_value[0];
		for (j = 0; j < cd->fade_para_1_format[1]; j++) {
			transparent_para->coloring_charge_para_1[i][j] =
				split_value[j + 1];
	...
	if (strstr(name, "red_charge_para_2") &&
	    (value_cnt == cd->red_para_2_format[1] + 1)) {
		i = split_value[0];
		for (j = 0; j < cd->red_para_2_format[1]; j++) {
			color_para->coloring_charge_para_2[i][j] =
				split_value[j + 1];
	...
	} else if (strstr(name, "fade_charge_para_2") &&
		   (value_cnt == cd->fade_para_2_format[1] + 1)) {
		i = split_value[0];
		for (j = 0; j < cd->fade_para_2_format[1]; j++) {
			transparent_para->coloring_charge_para_2[i][j] =
				split_value[j + 1];
				      ...
				      loff_t *ppos)
{
	...
	COLOR_DEBUG("aging_vol:%d < MAX_AGING_VOL_DATA:%d", sizeof(aging_vol),
		    MAX_AGING_VOL_DATA);
	if (sizeof(aging_vol) > MAX_AGING_VOL_DATA) {
		COLOR_INFO("aging_vol:%dmV > MAX_AGING_VOL_DATA:%d",
			   sizeof(aging_vol), MAX_AGING_VOL_DATA);
		goto OUT;
	}
	...
	COLOR_DEBUG(
		"open file success , now to write , path: %s, str:%s, vol: %dmV\n",
...
static ssize_t proc_ec_debug_level_read(struct file *file,
...
static ssize_t proc_ec_debug_level_write(struct file *file,
					 const char __user *buffer,
					 size_t count, loff_t *ppos)
{
	struct color_ctrl_device *cd = PDE_DATA(file_inode(file));
...
static int colorctrl_init_proc(struct color_ctrl_device *cd)
{
	...
	prEntry_tmp = proc_create_data("color_ctrl", EC_CHMOD, prEntry_cr,
				       &proc_colorctrl_ops, cd);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("temperature", EC_CHMOD, prEntry_cr,
				       &proc_temperature_control_ops, cd);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("voltage", EC_CHMOD, prEntry_cr,
				       &proc_adc_voltage_ops, cd);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("main_parameter", EC_CHMOD, prEntry_cr,
				       &proc_main_parameter_ops, cd);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("other_status", EC_CHMOD, prEntry_cr,
				       &proc_other_status_ops, cd);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create_data("cover_color", EC_CHMOD, prEntry_cr,
				       &proc_cover_color_ops, cd);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		COLOR_INFO("Couldn't create adc proc entry");
	}
	...
	prEntry_tmp = proc_create_data("ave_aging_vol", EC_CHMOD, prEntry_cr,
				       &proc_ave_aging_vol_ops, cd);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		COLOR_INFO("Couldn't create adc proc entry");
	}
	...
	/*debug level proc*/
	prEntry_tmp = proc_create_data("debug_level", EC_CHMOD, prEntry_cr,
				       &proc_ec_debug_level_ops, cd);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		COLOR_INFO("Couldn't create adc proc entry");
	}
...
static int colorctrl_parse_dt(struct device *dev, struct color_ctrl_device *cd)
{
	...
	color_para = devm_kzalloc(dev, sizeof(struct color_ctrl_control_para),
				  GFP_KERNEL);
	...
	transparent_para = devm_kzalloc(
		dev, sizeof(struct color_ctrl_control_para), GFP_KERNEL);

	if (!transparent_para) {
		COLOR_INFO(
			"Malloc memory for color control transparent para fail.");
		return -ENOMEM;
	}
	...
	cd->project_num =
		of_property_count_u32_elems(np, "platform_support_project");

	if (cd->project_num <= 0) {
		COLOR_INFO(
			"project not specified, need to config the support project.");
		goto OUT;
	} else {
		ret = of_property_read_u32_array(np, "platform_support_project",
						 cd->platform_support_project,
						 cd->project_num);
		...
		for (i = 0; i < cd->project_num; i++) {
			if (prj_id == cd->platform_support_project[i]) {
				COLOR_INFO("project [%d] match OK!",
					   cd->platform_support_project[i]);
				cd->match_project =
					cd->platform_support_project[i];
				break;
			}

			COLOR_INFO("project [%d] is not match",
				   cd->platform_support_project[i]);
		}
	...
	cd->colorctrl_v2_support =
		of_property_read_bool(np, "colorctrl_v2_support");
	cd->ec_step_voltage_support =
		of_property_read_bool(np, "ec_step_voltage_support");
	...
	cd->thermal_zone_device_num =
		of_property_count_strings(np, "thermal_zone_device_names");

	if (cd->thermal_zone_device_num <= 0 ||
	    cd->thermal_zone_device_num > MAX_THERMAL_ZONE_DEVICE_NUM) {
		COLOR_INFO(
			"thermal zone device num not specified or device num is too large, thermal_zone_device_num : %d.",
			cd->thermal_zone_device_num);
		goto OUT;
	} else {
		ret = of_property_read_u32_array(
			np, "thermal_zone_device_weight",
			cd->thermal_zone_device_weight,
			cd->thermal_zone_device_num + 1);

		if (ret) {
			COLOR_INFO(
				"thermal zone device weight is not specified, please check the dts config.");
			goto OUT;
		}

		ret = of_property_read_u32_array(
			np, "thermal_zone_device_weight_sigh_bit",
			cd->thermal_zone_device_weight_sigh_bit,
			cd->thermal_zone_device_num + 1);

		if (ret) {
			COLOR_INFO(
				"thermal zone device weight sigh bit is not specified, please check the dts config.");
			goto OUT;
		}

		for (i = 0; i < cd->thermal_zone_device_num + 1; i++) {
			cd->thermal_zone_device_weight[i] =
				cd->thermal_zone_device_weight[i] *
				(cd->thermal_zone_device_weight_sigh_bit[i] >
						 0 ?
					 1 :
					 -1);
			COLOR_INFO("%d thermal zone device weight is %d.", i,
				   cd->thermal_zone_device_weight[i]);
		}

		for (i = 0; i < cd->thermal_zone_device_num; i++) {
			cd->thermal_zone_device_name[i] =
				devm_kzalloc(dev,
					     MAX_THERMAL_ZONE_DEVICE_NAME_LEN,
					     GFP_KERNEL);
			ret = of_property_read_string_index(
				np, "thermal_zone_device_names", i,
				(const char **)&cd->thermal_zone_device_name[i]);

			if (ret) {
				COLOR_INFO(
					"thermal zone device name is not specified, please check the dts config.");
				goto OUT;
			}

			/*Request thermal device*/
			cd->thermal_zone_device[i] =
			...
			if (IS_ERR(cd->thermal_zone_device[i])) {
				...
				COLOR_INFO(
					"fail to get %s thermal_zone_device: %d",
					cd->thermal_zone_device_name[i], ret);
				goto OUT;
			} else {
				COLOR_INFO(
					"success get %s thermal_zone_device.",
					cd->thermal_zone_device_name[i]);
			}
		}
	}
	...
	hw_res->vm_coloring_gpio =
		of_get_named_gpio(np, "enable_coloring_gpio", 0);
	...
	if (IS_ERR(hw_res->vm_v_chan)) {
		...
		COLOR_INFO(
			"vm_v_chan get error or voltage read is not support, ret = %d\n",
			ret);
	} else {
	...
	COLOR_DEBUG("Parse V2 dt ok, [sleep_en_gpio:%d][si_in_1_gpio:%d]\
		    ...
		    hw_res->sleep_en_gpio, hw_res->si_in_1_gpio,
		    hw_res->si_in_2_gpio, hw_res->adc_on_gpio,
		    hw_res->oa_en_gpio, hw_res->vm_coloring_gpio,
		    hw_res->vm_fade_gpio);

	/*******************para parse********************/

	/*fade_recharge_waitime*/
	ret = of_property_read_u32(np, "fade_recharge_waitime",
				   &cd->fade_recharge_waitime);
	if (ret) {
	...
	ret = of_property_read_u32(np, "coloring_recharge_waitime",
				   &cd->coloring_recharge_waitime);
	if (ret) {
		COLOR_INFO("coloring_recharge_waitime not set ,err quit!!");
		goto OUT;
	}
	COLOR_DEBUG("coloring_recharge_waitime [%d]",
		    cd->coloring_recharge_waitime);

	/*wait_check_time*/
	ret = of_property_read_u32_array(np, "wait_check_time",
					 cd->wait_check_time, 2);
	if (ret) {
		COLOR_INFO("intermediate state wait time not set ,err quit!!");
		goto OUT;
	}
	COLOR_DEBUG("wait_check_time-[%d]-[%d]", cd->wait_check_time[0],
		    cd->wait_check_time[1]);

	/*fade_open_circuit_vol = <1200 1450 1550>*/
	if (cd->pcb_version >= PVT) {
		ret = of_property_read_u32_array(np, "fade_open_circuit_vol",
						 temp_array, 3);
	} else {
		ret = of_property_read_u32_array(
			np, "evt1_fade_open_circuit_vol", temp_array, 3);
	}
	...
	COLOR_DEBUG("fade_open_circuit_vol-[%d]-[%d]-[%d]",
		    cd->fade_open_circuit_vol[0], cd->fade_open_circuit_vol[1],
		    cd->fade_open_circuit_vol[2]);

	/*red_open_circuit_vol  = <1350 1400 1400>*/
	if (cd->pcb_version >= PVT) {
		ret = of_property_read_u32_array(np, "red_open_circuit_vol",
						 temp_array, 3);
	} else {
		ret = of_property_read_u32_array(
			np, "evt1_red_to_fade_check_volt", temp_array, 3);
	}
	...
	COLOR_DEBUG("red_open_circuit_vol-[%d]-[%d]-[%d]",
		    cd->red_open_circuit_vol[0], cd->red_open_circuit_vol[1],
		    cd->red_open_circuit_vol[2]);

	/*fade_to_red_check_volt = <1550 1780 1820>*/
	ret = of_property_read_u32_array(np, "fade_to_red_check_volt",
					 temp_array, 3);
	if (ret) {
	...
	COLOR_DEBUG("fade_to_red_check_volt-[%d]-[%d]-[%d]",
		    cd->fade_to_red_check_volt[0],
		    cd->fade_to_red_check_volt[1],
		    cd->fade_to_red_check_volt[2]);

	/*fade_to_red_retry_cnt*/
	ret = of_property_read_u32(np, "fade_to_red_retry_cnt",
				   &cd->fade_to_red_retry_cnt);
	if (ret) {
	...
	if (cd->pcb_version >= PVT) {
		ret = of_property_read_u32_array(np, "red_to_fade_check_volt",
						 temp_array, 3);
	} else {
		ret = of_property_read_u32_array(
			np, "evt1_red_to_fade_check_volt", temp_array, 3);
	...
	COLOR_DEBUG("red_to_fade_check_volt-[%d]-[%d]-[%d]",
		    cd->red_to_fade_check_volt[0],
		    cd->red_to_fade_check_volt[1],
		    cd->red_to_fade_check_volt[2]);

	/*reset_ftm_voltage = <1350 1450 1450>*/
	ret = of_property_read_u32_array(np, "reset_ftm_voltage", temp_array,
					 3);
	if (ret) {
	...
	ret = of_property_read_u32(np, "red_to_fade_retry_cnt",
				   &cd->red_to_fade_retry_cnt);
	if (ret) {
	...
	/*temperature_thd_offect*/
	if (cd->pcb_version >= PVT) {
		ret = of_property_read_u32(np, "temperature_thd_offect",
					   &cd->temperature_thd_offect);
	} else {
		ret = of_property_read_u32(np, "evt1_temperature_thd_offect",
					   &cd->temperature_thd_offect);
	}
	...
	if (cd->pcb_version >= PVT) {
		cd->temp_num =
			of_property_count_u32_elems(np, "temperature_thd");
	} else {
		cd->temp_num =
			of_property_count_u32_elems(np, "evt1_temperature_thd");
	}
	...
	if (cd->pcb_version >= PVT) {
		ret = of_property_read_u32_array(np, "temperature_thd",
						 temp_array, cd->temp_num);
	} else {
		ret = of_property_read_u32_array(np, "evt1_temperature_thd",
						 temp_array, cd->temp_num);
	}
	...
	for (i = 0; i < cd->temp_num; i++) {
		cd->temperature_thd[i] =
			temp_array[i] - cd->temperature_thd_offect;
	}

	/*coloring_charge_para_1*/
	if (cd->pcb_version >= PVT) {
		cd->red_para_1_num =
			of_property_count_u32_elems(np, "red_charge_para_1");
	} else {
		cd->red_para_1_num = of_property_count_u32_elems(
			np, "evt1_red_charge_para_1");
	}
	...
	if (cd->pcb_version >= PVT) {
		ret = of_property_read_u32_array(np, "red_para_1_format",
						 temp_array, 2);
	} else {
		ret = of_property_read_u32_array(np, "evt1_red_para_1_format",
						 temp_array, 2);
	}
	...
	if (ret) {
		...
		COLOR_DEBUG("red_para_1_format is %d, %d",
			    cd->red_para_1_format[0], cd->red_para_1_format[1]);
	}
	...
	if (ret) {
		...
		for (j = 0; j < cd->red_para_1_format[0]; j++) {
			for (i = 0; i < cd->red_para_1_format[1]; i++) {
				color_para->coloring_charge_para_1[j][i] =
					temp_array[num];
				num++;
			}
		}
	...
	if (cd->pcb_version >= PVT) {
		cd->fade_para_1_num =
			of_property_count_u32_elems(np, "fade_charge_para_1");
	} else {
		cd->fade_para_1_num = of_property_count_u32_elems(
			np, "evt1_fade_charge_para_1");
	}
	...
	if (cd->pcb_version >= PVT) {
		ret = of_property_read_u32_array(
			dev->of_node, "fade_para_1_format", temp_array, 2);
	} else {
		ret = of_property_read_u32_array(
			dev->of_node, "evt1_fade_para_1_format", temp_array, 2);
	}
	...
	if (ret) {
		...
		COLOR_DEBUG("fade_para_1_format is %d, %d",
			    cd->fade_para_1_format[0],
			    cd->fade_para_1_format[1]);
	}
	...
	if (ret) {
		...
		for (j = 0; j < cd->fade_para_1_format[0]; j++) {
			for (i = 0; i < cd->fade_para_1_format[1]; i++) {
				transparent_para->coloring_charge_para_1[j][i] =
					temp_array[num];
				num++;
			}
		}
	...
	cd->red_para_2_num =
		of_property_count_u32_elems(np, "red_charge_para_2");
	if (cd->red_para_2_num <= 0) {
		COLOR_INFO("red_para_2_num not specified");
		goto OUT;
	}

	ret = of_property_read_u32_array(dev->of_node, "red_para_2_format",
					 temp_array, 2);
	if (ret) {
		...
		COLOR_DEBUG("red_para_2_format is %d, %d",
			    cd->red_para_2_format[0], cd->red_para_2_format[1]);
	}

	ret = of_property_read_u32_array(dev->of_node, "red_charge_para_2",
					 temp_array, cd->red_para_2_num);
	if (ret) {
		...
		for (j = 0; j < cd->red_para_2_format[0]; j++) {
			for (i = 0; i < cd->red_para_2_format[1]; i++) {
				color_para->coloring_charge_para_2[j][i] =
					temp_array[num];
				num++;
			}
		}
	...
	/*fade_charge_para_2*/
	cd->fade_para_2_num =
		of_property_count_u32_elems(np, "fade_charge_para_2");
	if (cd->fade_para_2_num <= 0) {
		COLOR_INFO("fade_para_2_num not specified");
		goto OUT;
	}

	ret = of_property_read_u32_array(dev->of_node, "fade_para_2_format",
					 temp_array, 2);
	if (ret) {
		...
		COLOR_DEBUG("fade_para_2_format is %d, %d",
			    cd->fade_para_2_format[0],
			    cd->fade_para_2_format[1]);
	}

	ret = of_property_read_u32_array(dev->of_node, "fade_charge_para_2",
					 temp_array, cd->fade_para_2_num);
	if (ret) {
		...
		for (j = 0; j < cd->fade_para_2_format[0]; j++) {
			for (i = 0; i < cd->fade_para_2_format[1]; i++) {
				transparent_para->coloring_charge_para_2[j][i] =
					temp_array[num];
				num++;
			}
		}
...
static int colorctrl_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		hw_res = devm_kzalloc(&pdev->dev,
				      sizeof(struct color_ctrl_hw_resource),
				      GFP_KERNEL);
	...
	cd = devm_kzalloc(&pdev->dev, sizeof(struct color_ctrl_device),
			  GFP_KERNEL);
	...
	if (gpio_is_valid(hw_res->sleep_en_gpio)) {
		ret = devm_gpio_request(&pdev->dev, hw_res->sleep_en_gpio,
					"sleep_en_gpio");
		...
		if (ret) {
			...
			ret = gpio_direction_output(hw_res->sleep_en_gpio,
						    GPIO_LOW);

			if (ret) {
				COLOR_INFO(
					"Config sleep_en_gpio gpio output direction fail.");
				goto PROBE_ERR;
			}
		}
	} else {
	...
	if (gpio_is_valid(hw_res->si_in_1_gpio)) {
		ret = devm_gpio_request(&pdev->dev, hw_res->si_in_1_gpio,
					"si_in_1_gpio");
		...
		if (ret) {
			...
			ret = gpio_direction_output(hw_res->si_in_1_gpio,
						    GPIO_LOW);

			if (ret) {
				COLOR_INFO(
					"Config si_in_1_gpio gpio output direction fail.");
				goto PROBE_ERR;
			}
		}
	} else {
	...
	if (gpio_is_valid(hw_res->si_in_2_gpio)) {
		ret = devm_gpio_request(&pdev->dev, hw_res->si_in_2_gpio,
					"si_in_2_gpio");
		...
		if (ret) {
			...
			ret = gpio_direction_output(hw_res->si_in_2_gpio,
						    GPIO_LOW);

			if (ret) {
				COLOR_INFO(
					"Config si_in_2_gpio gpio output direction fail.");
				goto PROBE_ERR;
			}
		}
	} else {
	...
	if (gpio_is_valid(hw_res->vm_coloring_gpio)) {
		ret = devm_gpio_request(&pdev->dev, hw_res->vm_coloring_gpio,
					"vm_coloring_gpio");
		...
		if (ret) {
			...
			ret = gpio_direction_output(hw_res->vm_coloring_gpio,
						    GPIO_LOW);

			if (ret) {
				COLOR_INFO(
					"Config vm_coloring_gpio gpio output direction fail.");
				goto PROBE_ERR;
			}
		}
	}
	...
	if (gpio_is_valid(hw_res->vm_fade_gpio)) {
		ret = devm_gpio_request(&pdev->dev, hw_res->vm_fade_gpio,
					"vm_fade_gpio");
		...
		if (ret) {
			...
			ret = gpio_direction_output(hw_res->vm_fade_gpio,
						    GPIO_LOW);

			if (ret) {
				COLOR_INFO(
					"Config vm_fade_gpio gpio output direction fail.");
				goto PROBE_ERR;
			}
		}
	}
	...
	if (gpio_is_valid(hw_res->adc_on_gpio)) {
		ret = devm_gpio_request(&pdev->dev, hw_res->adc_on_gpio,
					"adc_on_gpio");
		...
		if (ret) {
			...
			ret = gpio_direction_output(hw_res->adc_on_gpio,
						    GPIO_LOW);

			if (ret) {
				COLOR_INFO(
					"Config adc_on_gpio  output direction fail.");
				goto PROBE_ERR;
			}
		}
	} else {
	...
	if (gpio_is_valid(hw_res->oa_en_gpio)) {
		ret = devm_gpio_request(&pdev->dev, hw_res->oa_en_gpio,
					"oa_en_gpio");
		...
		if (ret) {
			...
			ret = gpio_direction_output(hw_res->oa_en_gpio,
						    GPIO_LOW);

			if (ret) {
				COLOR_INFO(
					"Config oa_en_gpio gpio output direction fail.");
				goto PROBE_ERR;
			}
		}
	} else {
	...
	COLOR_INFO(
		"color ctrl device probe : normal end, now default is %d mV. color_status: %d",
