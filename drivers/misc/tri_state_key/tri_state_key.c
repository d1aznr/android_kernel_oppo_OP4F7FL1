...
static void tri_key_dev_work(struct work_struct *work)
{
	...
	if (key[0] && key[1] && key[2]) {
		TRI_KEY_LOG(
			"key[0]=%d,key[1]=%d,key[2]=%d***pre_key0=%d, pre_key1=%d, pre_key2=%d\n",
		...
		if (pre_key0 && pre_key1 && pre_key2) {
			trikey_data->state = 3;
			TRI_KEY_LOG(
				"Debounce timeout: Set the tri key state to 0,1,1\n");
		...
		if (!del_timer(&trikey_data->s_timer))
			TRI_KEY_LOG(
				"timer already expired for floating state\n");
	}
	if (!key[0] && !key[1] && !key[2])
		return;
	if (!key[0] && !key[1] && key[2])
...
static int tri_key_dev_probe(struct platform_device *pdev)
{
	...
	trikey_data->set_state = pinctrl_lookup_state(
		trikey_data->key_pinctrl, "pmx_tri_state_key_active");
	...
	ret = gpio_request(trikey_data->key2_gpio, "tristate_key2");
	if (ret < 0)
		goto err_request_gpio;
	...
	ret = gpio_request(trikey_data->key3_gpio, "tristate_key3");
	if (ret < 0)
		goto err_request_gpio;
