...
__attribute__((weak)) void external_report_touch(int id, bool down_status,
						 int x, int y)
{
	return;
}
 ...
 */
static int cs_i2c_read(struct cs_device *cd, unsigned char reg,
 ...
 * Returning transfer length(transfer success) or most likely negative errno(transfer error)
 */
static int cs_i2c_write(struct cs_device *cd, unsigned char reg,
			unsigned char *datbuf, int byte_len)
{
...
static int cs_read_eeprom(struct cs_device *cd, unsigned short reg,
			  unsigned char *datbuf, int byte_len)
{
	struct i2c_msg msg[2];
	int ret = 0;
...
static int cs_write_eeprom(struct cs_device *cd, unsigned short reg,
			   unsigned char *datbuf, int byte_len)
{
	unsigned char *buf = NULL;
	struct i2c_msg msg;
...
static int cs_eeprom_erase(struct cs_device *cd)
{
	...
	/*erase flash*/
	if (cs_write_eeprom(cd, IIC_EEPROM, erase_cmd, sizeof(erase_cmd)) <=
	    0) {
...
static int cs_parse_dt(struct cs_device *cd)
{
	...
	if (IS_ERR_OR_NULL(cd->vdd_2v8)) {
		...
		if (regulator_count_voltages(cd->vdd_2v8) > 0) {
			ret = regulator_set_voltage(cd->vdd_2v8, 2800000,
						    3300000);
			if (ret) {
	...
	cd->irq_gpio = of_get_named_gpio_flags(
		cd->dev->of_node, "press,irq-gpio", 0, &cd->irq_flag);
	...
	cd->reset_gpio =
		of_get_named_gpio(cd->dev->of_node, "press,rst-gpio", 0);
...
static int cs_power_control(struct cs_device *cd, bool on)
{
	...
	if (on) {
		...
		if (gpio_is_valid(cd->reset_gpio)) {
			gpio_direction_output(cd->reset_gpio,
					      0); //default to low state
		}
	} else {
		if (gpio_is_valid(cd->reset_gpio)) {
			gpio_direction_output(cd->reset_gpio,
					      1); //pull to reset state
		}
...
*/
static int set_press_threshold(struct cs_device *cd, int touchTh, int leaveTh,
			       char channel)
{
	int ret = 0;
...
*/
static int get_press_threshold(struct cs_device *cd, unsigned int *touchTh,
			       unsigned int *leaveTh, char channel)
{
	...
	*touchTh =
		(unsigned int)(short)(datbuf[0] | ((datbuf[1] << 8) & 0xff00));
...
int cs_get_pressure(struct cs_device *cd)
{
	...
	pressdata =
		((int)(reg_data[0] & 0xff) | ((int)(reg_data[1] & 0xff) << 8));
	return pressdata;
}
...
static int cs_write_calibrate_param(struct cs_device *cd, unsigned int data1,
				    char channel)
{
	unsigned char datbuf[8] = { 0 };
	...
	datbuf[6] = datbuf[0] + datbuf[1] + datbuf[2] + datbuf[3] + datbuf[4] +
		    datbuf[5]; /*checksum*/
...
static int cs_read_calibrate_param(struct cs_device *cd, unsigned int *data,
				   char channel)
{
	unsigned char datbuf[8] = { 0 };
	...
	CS_LOG("R:%02x,%02x,%02x,%02x,%02x\n", datbuf[0], datbuf[1], datbuf[2],
	       datbuf[3], datbuf[4]);
	if (datbuf[0] == 4) {
		*data = (unsigned int)(datbuf[1] + (datbuf[2] << 8) +
				       (datbuf[3] << 16) + (datbuf[4] << 24));
...
int cs_compare_cali_param(struct cs_device *cd)
{
	...
	ret = sscanf(temp, "%d %d %d %d %d", &is_cal, &slp1, &itr1, &slp2,
		     &itr2);
	CS_LOG("read ROM Cali:%d %d %d %d %d\n", is_cal, slp1, itr1, slp2,
	       itr2);
...
static irqreturn_t cs_irq_thread_func(int irq, void *dev_id)
{
	...
	if (MODE_REPORT_KEY == cd->report_mode) {
		...
		external_report_touch(10, key_status > 0, cd->current_point.x,
				      cd->current_point.y);
...
static ssize_t cs_write(struct file *file, const char __user *buf, size_t count,
			loff_t *offset)
{
	int err = 0;
	char *kbuf = NULL;
				    ...
				    size_t size, loff_t *lo)
{
	...
	image_version =
		(fw->data[CS_FW_VERPOS] << 8) | fw->data[CS_FW_VERPOS + 1];
	...
	fw_len = fw->data[CS_FW_LENPOS + 3] |
		 (fw->data[CS_FW_LENPOS + 2] << 8) |
		 (fw->data[CS_FW_LENPOS + 1] << 16) |
		 (fw->data[CS_FW_LENPOS] << 24);
...
static int rawdata_read_func(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < len / 2; i++) {
		raw_data = ((reg_data[i * 2] & 0xff) |
			    ((reg_data[i * 2 + 1] & 0xff) << 8));
...
static ssize_t proc_debug_th_read(struct file *file, char __user *user_buf,
...
static ssize_t proc_debug_th_write(struct file *file, const char __user *buffer,
				   size_t count, loff_t *ppos)
{
	char buf[64];
	int ret = 0, channel = 0, touch_th = 0, leave_th = 0;
...
static ssize_t proc_report_switch_write(struct file *file,
...
static ssize_t proc_report_switch_read(struct file *file, char __user *user_buf,
				       size_t count, loff_t *ppos)
{
	char buf[16];
	int ret = 0;
...
static ssize_t proc_coordinate_write(struct file *file,
				     const char __user *buffer, size_t count,
				     loff_t *ppos)
{
	char buf[64];
...
static int cs_init_proc(struct cs_device *cd)
{
	...
	ptr_tmp = proc_create_data("fw_update", 0666, ptr_cs,
				   &proc_fw_update_ops, cd);
	if (ptr_tmp == NULL) {
		ret = -ENOMEM;
	...
	ptr_tmp = proc_create_data("fw_info", 0444, ptr_cs, &proc_fw_info_ops,
				   cd);
	if (ptr_tmp == NULL) {
		ret = -ENOMEM;
		CS_LOG("%s: Couldn't create proc entry, %d\n", __func__,
	...
	ptr_tmp = proc_create_data("rawdata", 0444, ptr_cs, &proc_rawdata_ops,
				   cd);
	if (ptr_tmp == NULL) {
		ret = -ENOMEM;
		CS_LOG("%s: Couldn't create proc entry, %d\n", __func__,
	...
	ptr_tmp = proc_create_data("diffdata", 0444, ptr_cs, &proc_diffdata_ops,
				   cd);
	if (ptr_tmp == NULL) {
		ret = -ENOMEM;
		CS_LOG("%s: Couldn't create proc entry, %d\n", __func__,
	...
	ptr_tmp = proc_create_data("press_thd", 0666, ptr_cs,
				   &proc_debug_th_ops, cd);
	if (ptr_tmp == NULL) {
		ret = -ENOMEM;
		CS_LOG("%s: Couldn't create proc entry, %d\n", __func__,
		       __LINE__);
	...
	ptr_tmp = proc_create_data("report_mode", 0666, ptr_cs,
				   &proc_report_switch_ops, cd);
	if (ptr_tmp == NULL) {
		ret = -ENOMEM;
		CS_LOG("%s: Couldn't create proc entry, %d\n", __func__,
		       __LINE__);
	...
	ptr_tmp = proc_create_data("touch_coordinate", 0666, ptr_cs,
				   &proc_coordinate_ops, cd);
	if (ptr_tmp == NULL) {
		ret = -ENOMEM;
		CS_LOG("%s: Couldn't create proc entry, %d\n", __func__,
		       __LINE__);
...
static int cs_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = request_threaded_irq(cd->irq, NULL, cs_irq_thread_func,
				   cd->irq_flag | IRQF_ONESHOT, CS_CHRDEV_NAME,
				   cd);
	if (ret) {
