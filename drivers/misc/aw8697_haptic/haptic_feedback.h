 ...
 *
 *********************************************************/
#define haptic_fb_err(format, ...) pr_err("[haptic_hv]" format, ##__VA_ARGS__)

#define haptic_fb_info(format, ...) pr_info("[haptic_hv]" format, ##__VA_ARGS__)

#define haptic_fb_dbg(format, ...) pr_debug("[haptic_hv]" format, ##__VA_ARGS__)
...
struct haptic_mem_alloc_track_event {
	struct haptic_mem_alloc_event_info
		mem_event_que[MAX_MEM_ALLOC_EVENT_QUEUE_LEN];
	uint32_t que_front;
	uint32_t que_rear;
