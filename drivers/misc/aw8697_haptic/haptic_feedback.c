...
static void oplus_haptic_event_queue_update(void)
{
	...
	memset(dev_event->dev_event_que, 0,
	       sizeof(struct haptic_dev_event_info) * MAX_DEV_EVENT_QUEUE_LEN);
	dev_event->que_front = 0;
	dev_event->que_rear = 0;

	/* clear mem_event_que */
	mem_event = &chip->mem_alloc_track_event;
	memset(mem_event->mem_event_que, 0,
	       sizeof(struct haptic_mem_alloc_event_info) *
		       MAX_MEM_ALLOC_EVENT_QUEUE_LEN);
	mem_event->que_front = 0;
	mem_event->que_rear = 0;
}

static struct haptic_fb_info g_haptic_fb_table[] = {
	{ HAPTIC_I2C_READ_TRACK_ERR, "i2c_read_err",
	  HAPTIC_TRACK_EVENT_DEVICE_ERR },
	{ HAPTIC_I2C_WRITE_TRACK_ERR, "i2c_write_err",
	  HAPTIC_TRACK_EVENT_DEVICE_ERR },

	{ HAPTIC_F0_CALI_TRACK, "f0_cali_err",
	  HAPTIC_TRACK_EVENT_FRE_CALI_ERR },
	{ HAPTIC_OSC_CALI_TRACK, "osc_cali_err",
	  HAPTIC_TRACK_EVENT_FRE_CALI_ERR },

	{ HAPTIC_MEM_ALLOC_TRACK, "mem_alloc_err",
	  HAPTIC_TRACK_EVENT_MEM_ALLOC_ERR },
};
...
static int oplus_haptic_event_payload_pack(struct haptic_fb_detail *fb_info)
{
	...
	snprintf(chip->dcs_info->payload, MAX_PAYLOAD_LEN,
		 ...
		 fb_event_field, fb_event_desc, fb_info->detailData);
...
static int
oplus_haptic_track_upload_trigger_data(struct haptic_fb_detail *fb_detail)
{
	int rc = 0;
	struct oplus_haptic_track *chip = g_haptic_track_chip;
...
int oplus_haptic_track_dev_err(uint32_t track_type, uint32_t reg_addr,
			       uint32_t err_code)
{
	struct oplus_haptic_track *chip = g_haptic_track_chip;
...
int oplus_haptic_track_fre_cail(uint32_t track_type, uint32_t cali_data,
				uint32_t result_flag, char *fail_info)
{
	struct oplus_haptic_track *chip = g_haptic_track_chip;
	...
	memset(track_event->fre_event.fail_info, 0,
	       sizeof(char) * MAX_FAIL_INFO_LEN);
	...
	strncpy(track_event->fre_event.fail_info, fail_info,
		MAX_FAIL_INFO_LEN - 1);
	track_event->fre_event.fail_info[MAX_FAIL_INFO_LEN - 1] = '\0';

	schedule_delayed_work(&track_event->track_fre_cail_load_trigger_work,
			      0);
...
int oplus_haptic_track_mem_alloc_err(uint32_t track_type, uint32_t alloc_len,
				     const char *fun_name)
{
	struct oplus_haptic_track *chip = g_haptic_track_chip;
	...
	strncpy(track_event->mem_event_que[que_rear].fun_name, fun_name,
		MAX_FUN_NAME_LEN - 1);
	track_event->mem_event_que[que_rear].fun_name[MAX_FUN_NAME_LEN - 1] =
		'\0';
	track_event->que_rear++;

	schedule_delayed_work(
		&track_event->track_mem_alloc_err_load_trigger_work, 0);
...
static void
oplus_haptic_track_dev_err_load_trigger_work(struct work_struct *work)
{
	uint32_t que_front;
	struct oplus_haptic_track *chip = g_haptic_track_chip;
	...
	snprintf(fb_detail->detailData, MAX_DETAIL_INFO_LEN,
		 "reg_addr:0x%x,err_code:%u",
		 dev_event->dev_event_que[que_front].reg_addr,
...
static void
oplus_haptic_track_fre_cail_load_trigger_work(struct work_struct *work)
{
	struct oplus_haptic_track *chip = g_haptic_track_chip;
	struct delayed_work *dwork = to_delayed_work(work);
	...
	memset(fb_detail->detailData, 0, MAX_DETAIL_INFO_LEN);
	snprintf(fb_detail->detailData, MAX_DETAIL_INFO_LEN,
		 "cali_data:%u,cali_ret:0x%x,fail_info:%s",
		 cail_event->fre_event.cali_data,
		 cail_event->fre_event.result_flag,
		 cail_event->fre_event.fail_info);

	oplus_haptic_track_upload_trigger_data(fb_detail);
}

static void
oplus_haptic_track_mem_alloc_load_trigger_work(struct work_struct *work)
{
	uint32_t que_front;
	struct oplus_haptic_track *chip = g_haptic_track_chip;
	struct delayed_work *dwork = to_delayed_work(work);
	...
	memset(fb_detail->detailData, 0, MAX_DETAIL_INFO_LEN);
	snprintf(fb_detail->detailData, MAX_DETAIL_INFO_LEN,
		 "alloc_len:0x%x,fun_name:%s",
		 mem_event->mem_event_que[que_front].alloc_len,
...
static void oplus_haptic_track_upload_info_dwork(struct work_struct *work)
{
	int ret = 0;
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_haptic_track *chip = container_of(
		dwork, struct oplus_haptic_track, upload_info_dwork);
...
static int oplus_haptic_track_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (!chip->trigger_data_ok)
			haptic_fb_err(
				"oplus haptic track false wakeup, rc=%d\n", rc);
		mutex_lock(&chip->trigger_data_lock);
...
static void oplus_haptic_track_init(struct oplus_haptic_track *track_dev)
{
	...
	INIT_DELAYED_WORK(
		&mem_alloc_event->track_mem_alloc_err_load_trigger_work,
...
static ssize_t dev_event_track_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	return oplus_haptic_track_dev_err(event_info_node.track_type,
					  event_info_node.reg_addr,
					  event_info_node.err_code);
}

static ssize_t dev_event_track_store(struct device *dev,
...
static ssize_t dev_fre_cail_track_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	return oplus_haptic_track_fre_cail(fre_cail_node.track_type,
					   fre_cail_node.cali_data,
					   fre_cail_node.result_flag,
					   fre_cail_node.fail_info);
}

static ssize_t dev_fre_cail_track_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t len)
{
	uint8_t track_type = 0;
	...
	if (sscanf(buf, "%d %d %d %127s", &track_type, &cali_data, &result_flag,
		   &fail_info[0]) == 4) {
		if (track_type >= HAPTIC_TRACK_TYPE_MAX) {
			haptic_fb_err("%s: first value out of range!\n",
				      __func__);
		...
		strncpy(fre_cail_node.fail_info, fail_info,
			MAX_FAIL_INFO_LEN - 1);
...
static ssize_t dev_mem_alloc_track_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	return oplus_haptic_track_mem_alloc_err(
		mem_alloc_node.track_type, mem_alloc_node.alloc_len, __func__);
}

static ssize_t dev_mem_alloc_track_store(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t len)
{
	uint8_t track_type = 0;
...
static int oplus_haptic_feedback_probe(struct platform_device *pdev)
{
	...
	hapric_track = devm_kzalloc(
		&pdev->dev, sizeof(struct oplus_haptic_track), GFP_KERNEL);
	if (!hapric_track) {
		pr_err("alloc memory error\n");
		return -ENOMEM;
	}

	hapric_track->dcs_info = (struct kernel_packet_info *)devm_kzalloc(
		&pdev->dev, sizeof(char) * OPLUS_HSPTIC_TRIGGER_MSG_LEN,
		GFP_KERNEL);
	...
	hapric_track->track_upload_kthread =
		kthread_run(oplus_haptic_track_thread, (void *)hapric_track,
	...
	hapric_track->trigger_upload_wq =
		create_workqueue("haptic_chg_trigger_upload_wq");
...
static int oplus_haptic_feedback_remove(struct platform_device *pdev)
{
	...
	cancel_delayed_work_sync(
		&hapric_track->dev_track_event.track_dev_err_load_trigger_work);
	cancel_delayed_work_sync(&hapric_track->fre_cail_track_event
					  .track_fre_cail_load_trigger_work);
	cancel_delayed_work_sync(
		&hapric_track->mem_alloc_track_event
			 .track_mem_alloc_err_load_trigger_work);
	devm_kfree(&pdev->dev, hapric_track->dcs_info);
	devm_kfree(&pdev->dev, hapric_track);
	return 0;
