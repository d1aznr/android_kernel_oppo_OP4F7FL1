...
struct uid_entry {
	...
	atomic64_t cg_checkout
		[UID_GROUP_SIZE]; /* MUSTFIX only inst pevent for calc */
#endif
};
...
static int uid_perf_event_type[UID_PERF_EVENTS] = { PERF_TYPE_RAW,
						    PERF_TYPE_RAW,
						    PERF_TYPE_RAW };

static int uid_perf_event_pin[UID_PERF_EVENTS] = { 0, 0, 0 };
...
static struct list_head uid_perf_remove_list =
	LIST_HEAD_INIT(uid_perf_remove_list);
...
static inline void uid_remove_and_disable_one_pevent(struct task_struct *task,
						     int idx)
{
...
static void uid_create_and_enable_one_pevent(struct task_struct *task, int idx,
					     bool grouping)
{
	...
	if (!task->uid_pevents[idx]) {
		...
		pevent = perf_event_create_kernel_counter(&attr, -1, task, NULL,
							  NULL);
		if (IS_ERR(pevent)) {
...
void uid_check_out_pevent(struct task_struct *task)
{
	...
	for (i = 0; i < UID_PERF_EVENTS; ++i) {
		if (task->uid_pevents[i]) {
			val = perf_event_read_value(task->uid_pevents[i],
						    &enabled, &running);
			...
			if (i == 0) {
				...
				if (idx > -1)
					...
					pr_err("%s: idx is invalid value idx=%d task=%s pid=%d",
					       __func__, idx, task->comm,
					       task->pid);
			}
			...
			if (tgid) {
				...
				if (uid_perf_debug)
					pr_err("task %s %d %d uid %d leave event %d val: %llu cur %llu\n",
					       task->comm, task->pid,
					       task->tgid, uid, i, delta,
					       tgid->uid_leaving_counts[i]);
			}
		}
	}
...
static int uid_perf_show(struct seq_file *m, void *v)
{
	...
	do_each_thread(temp, task)
	{
		...
		if (!(task->flags & PF_EXITING)) {
			...
			seq_printf(m, "%s,%d,%d,%d", task->comm, task->pid,
				   task->tgid, uid);
			for (i = 0; i < UID_PERF_EVENTS; ++i) {
				val[i] = 0;

				if (task->uid_pevents[i]) {
					val[i] = perf_event_read_value(
						task->uid_pevents[i], &enabled,
						&running);
					task->uid_prev_counts[i] = val[i];
				}
				seq_printf(m, ",%llu",
					   val[i] +
						   task->uid_leaving_counts[i]);
			}
			...
			if (idx > -1) {
				...
				for (i = 0; i < 1; ++i) {
					...
					if (task->uid_group_prev_counts[idx] >
					    0) {
						task->uid_group[idx] +=
							(val[i] -
							 task->uid_group_prev_counts
								 [idx]);
						if (uid_perf_debug)
	...
	hash_for_each (hash_table, bkt, uid_entry, hash) {
		...
		for (i = 0; i < UID_PERF_EVENTS; ++i)
			seq_printf(m, ",%llu",
				   atomic64_read(&uid_entry->checkout[i]));
		/* cgroup counters */
		for (i = 0; i < UID_GROUP_SIZE; ++i)
			seq_printf(m, ",%llu",
				   atomic64_read(&uid_entry->cg_checkout[i]));
		seq_puts(m, "\n");
	}
...
static int __uid_perf_add_work(void *unused)
{
	...
	while (!kthread_should_stop()) {
		...
		while (!list_empty(&uid_perf_add_list)) {
			struct uid_work *work = list_first_entry(
				&uid_perf_add_list, struct uid_work, node);
			...
			if (task && !(task->flags & PF_EXITING)) {
				for (i = 0; i < UID_PERF_EVENTS; ++i)
					uid_create_and_enable_one_pevent(
						task, i, false);
			}
			put_task_struct(task);
			spin_lock_irqsave(&uid_perf_lock, flag);
...
static int __uid_perf_remove_work(void *unused)
{
	...
	while (!kthread_should_stop()) {
		...
		while (!list_empty(&uid_perf_remove_list)) {
			struct uid_work *work = list_first_entry(
				&uid_perf_remove_list, struct uid_work, node);
			struct task_struct *task = work->task;
			...
			if (task && !(task->flags & PF_EXITING)) {
				for (i = 0; i < UID_PERF_EVENTS; ++i)
					uid_remove_and_disable_one_pevent(task,
									  i);
			}
			put_task_struct(task);
			spin_lock_irqsave(&uid_perf_lock, flag);
...
#ifdef CONFIG_OPLUS_FEATURE_UID_PERF
	proc_create_data("show_uid_perf", 0444, cpu_parent, &uid_perf_fops,
			 NULL);
	uid_perf_add_thread =
		kthread_create(__uid_perf_add_work, NULL, "uid_add_thread");
	uid_perf_remove_thread = kthread_create(__uid_perf_remove_work, NULL,
						"uid_remove_thread");
#endif
