...
static void dump_kswapd_debug_info(void)
{
	...
	for_each_online_pgdat (pgdat) {
		pr_info("Node %d kswapd has run %lu ms %lu times:\n",
			pgdat->node_id,
			div_u64(atomic_long_read(&pgdat->kswapd_running_ns),
				NS_PER_MS),
		...
		for (order = 0; order <= MAX_ORDER; order++)
			pr_info("order %4d %12lu %12lu ms\n", order,
				...
				div_u64(atomic_long_read(
						&entry[order].runtime_ns),
					NS_PER_MS));
	}
}
			       ...
			       size_t count, loff_t *off)
{
	...
	for_each_online_pgdat (pgdat) {
		len += scnprintf(
			kbuf + len, KSWAPD_DEBUG_INFO_KBUF_LEN - len,
...
static long get_mem_usage_pages(enum mem_type type)
{
	...
	case MEM_GPU:
#ifdef CONFIG_QCOM_KGSL
		ret = atomic_long_read(&kgsl_driver.stats.page_alloc) >>
		      PAGE_SHIFT;
...
static void oplus_show_mem(void)
{
	...
	long free, slab_rec, slab_unrec, vmalloc, anon, file, pagetbl,
		kernel_stack;
	...
	unaccounted = total - free - slab_rec - slab_unrec - vmalloc - anon -
		      file - pagetbl - kernel_stack - ion_used - ion_cache -
		      gpu;

	pr_info("%s:%lukB %s:%lukB %s:%lukB %s:%lukB %s:%lukB "
		"%s:%lukB %s:%lukB %s:%lukB %s:%lukB %s:%lukB "
		"%s:%lukB %s:%lukB %s:%lukB Unaccounted:%lukB",
		mem_type_text[MEM_TOTAL], K(total), mem_type_text[MEM_FREE],
		K(free), mem_type_text[MEM_SLAB_RECLAIMABLE], K(slab_rec),
		mem_type_text[MEM_SLAB_UNRECLAIMABLE], K(slab_unrec),
		mem_type_text[MEM_VMALLOC], K(vmalloc), mem_type_text[MEM_ANON],
		K(anon), mem_type_text[MEM_FILE], K(file),
		...
		mem_type_text[MEM_GPU], K(gpu), mem_type_text[MEM_DT_RESERVED],
		K(dt_reserved), K(unaccounted));
}
...
static int dump_tasks_info(bool verbose)
{
	...
	for_each_process (p) {
		...
		if (!verbose && tsk_oom_adj && (tsk_oom_adj <= service_adj) &&
		    (get_mm_rss(tsk->mm) +
		...
		tsk_nr_ptes = PTRS_PER_PTE * sizeof(pte_t) *
			      atomic_long_read(&tsk->mm->nr_ptes);
#else
		...
		pr_info("[%5d] %5d %5d %8lu %8lu %8lu %8lu %8lu %5hd %c %s%c\n",
			tsk->pid, from_kuid(&init_user_ns, task_uid(tsk)),
			tsk->tgid, tsk->mm->total_vm, get_mm_rss(tsk->mm),
			tsk_nr_ptes / SZ_1K,
			get_mm_counter(tsk->mm, MM_SWAPENTS),
			get_mm_counter(tsk->mm, MM_SHMEMPAGES), tsk_oom_adj,
			task_state, tsk->comm, frozen_mark);
		task_unlock(tsk);
		/* add process cmdline info */
		if (strstr(tsk->comm, "vendor.qti") != NULL) {
			memset(cmdline, 0, MAX_OOM_DUMP_TASK_CMDLINE);
			get_cmdline(tsk, cmdline,
				    MAX_OOM_DUMP_TASK_CMDLINE - 1);
			pr_info("[%5d] %s\n", tsk->pid, cmdline);
		}
	}
	rcu_read_unlock();
	...
	for (n = rb_first(&dev->clients); n; n = rb_next(n)) {
		...
		for (m = rb_first(&client->handles); m; m = rb_next(m)) {
			struct ion_handle *handle =
				rb_entry(m, struct ion_handle, node);
	...
	for (n = rb_first(&dev->buffers); n; n = rb_next(n)) {
		...
		if (!buffer->handle_count) {
			pr_info("[%-5d] %-8d %-16s 0x%p %d %d\n", buffer->pid,
				buffer->size / SZ_1K, buffer->task_comm, buffer,
				buffer->kmap_cnt,
...
static int acct_dma_dize(const void *data, struct file *file, unsigned int n)
{
	...
	if (likely(!verbose)) {
		...
		if (max_cachep) {
			...
			pr_info("%-17s %6lu %6lu %6u %4u %4d : tunables %4u %4u %4u : slabdata %6lu %6lu %6lu\n",
				...
				(1 << sinfo.cache_order), sinfo.limit,
				sinfo.batchcount, sinfo.shared,
		...
		list_for_each_entry (cachep, &slab_caches, list) {
			...
			pr_info("%-17s %6lu %6lu %6u %4u %4d : tunables %4u %4u %4u : slabdata %6lu %6lu %6lu\n",
				cachep->name, sinfo.active_objs, sinfo.num_objs,
				cachep->size, sinfo.objects_per_slab,
				(1 << sinfo.cache_order), sinfo.limit,
				sinfo.batchcount, sinfo.shared,
				sinfo.active_slabs, sinfo.num_slabs,
				sinfo.shared_avail);
...
static struct shrinker lowmem_shrinker = { .scan_objects = lowmem_scan,
...
static __init int oplus_lowmem_dbg_init(void)
{
	...
	pr_info("init watermark %s:%lukB %s:%lukB %s:%lukB %s:%lukB "
		...
		mem_type_text[MEM_GPU], K(pcfg->wms[MEM_GPU]), K(pcfg->wm_low),
		K(pcfg->wm_critical));
	return 0;
}
...
int kswapd_debug_init(struct proc_dir_entry *parent)
{
	...
	entry = proc_create("kswapd_debug", S_IRUGO, parent,
			    &kswapd_debug_operations);
	if (!entry) {
