				     ...
				     const struct track *track)
{
	...
	for (i = 0; i < TRACK_ADDRS_COUNT; i++) {
		...
		if (track->addr == addr || ((track->addr - 4) == addr))
#else
	...
	for (;;) {
		...
		if (!memcmp(caddrs, taddrs,
			    OPLUS_MEMCFG_SLABTRACE_CNT *
				    sizeof(unsigned long))) {
			l = &t->loc[pos];
			l->count++;
			//sort by pid
			if ((isbypid && !isbystacks) &&
			    !strcmp(s->name, sort_cache_name))
...
static int oplus_memcfg_slabtrace_show(struct seq_file *m, void *p)
{
	...
	pid_cnt_cachep = kmem_cache_create(
		"pid_cnt_cache", sizeof(struct pid_cnt_struct), 0, 0, NULL);
	if (!pid_cnt_cachep) {
...
static int oplus_memcfg_slabcfg_show(struct seq_file *m, void *p)
{
	...
	seq_printf(m,
		   "Reset config:echo \"reset\" > /proc/slab_memcfg/slabcfg\n");
	seq_printf(
		m,
			     ...
			     loff_t *data)
{
	...
	pr_info("slabtrace:%s %s %lx %lx %lx %lx\n", argv1, argv2,
		sort_addrs[0], sort_addrs[1], sort_addrs[2], sort_addrs[3]);
