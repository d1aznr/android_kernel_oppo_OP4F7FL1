...
void save_dump_reason_to_smem(char *info, char *function_name)
{
	...
	if (IS_ERR_OR_NULL(dp_info)) {
		...
		strlinfo = strlinfo < DUMP_REASON_SIZE ? strlinfo :
							 DUMP_REASON_SIZE;
		strlfun =
			strlfun < DUMP_REASON_SIZE ? strlfun : DUMP_REASON_SIZE;
		if ((strlen(dp_info->dump_reason) + strlinfo) <
		    DUMP_REASON_SIZE)
		...
		pr_debug(
			"\r%s: dump_reason : %s strl=%d function caused panic :%s strl1=%d \n",
			__func__, dp_info->dump_reason, strlinfo, function_name,
			strlfun);
