...
#ifdef CONFIG_OPLUS_SYSTEM_KERNEL_QCOM
	smem_addr = qcom_smem_get(QCOM_SMEM_HOST_ANY, SMEM_OPLUS_FEATURE_MAP,
				  &smem_size);
#endif
...
static unsigned int mdmfeature_poll(struct file *file,
	...
	if (manifest_parent) {
		...
		if (update_type == MDMFEATURE_UPDATE_SSSS) {
			telephony_manifest_ssss =
				proc_symlink("network_manifest",
					     manifest_parent,
					     telephony_manifest_src[0]);
		} else {
			telephony_manifest_dsds =
				proc_symlink("network_manifest",
					     manifest_parent,
					     telephony_manifest_src[1]);
		}
	}
...
static long mdmfeature_ioctl(struct file *file, unsigned int cmd,
			     unsigned long arg)
{
	long ret = 0;
...
static struct miscdevice mdmfeature_device = { MISC_DYNAMIC_MINOR, "mdmfeature",
					       &mdmfeature_device_fops };
