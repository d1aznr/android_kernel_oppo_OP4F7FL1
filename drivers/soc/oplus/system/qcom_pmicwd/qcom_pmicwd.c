...
static ssize_t proc_force_shutdown_read(struct file *file, char __user *buf,
...
{
	return pon->subtype == PON_GEN3_PBS || pon->subtype == PON_GEN3_HLOS;
}
...
int qpnp_pon_wd_timer(unsigned char timer, enum pon_power_off_type reset_type)
{
	...
	if (rc)
		dev_err(pon->dev, "Unable to write to addr=%x, rc(%d)\n",
...
static int pmicwd_kthread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		schedule_timeout_interruptible(msecs_to_jiffies(
			(((pon->pmicwd_state >> 8) & 0xff) * 1000) / 2));
				...
				size_t count, loff_t *off)
{
	...
	len = snprintf(&page[len], 128 - len,
		       "enable = %d timeout = %d rstype = %d\n",
				 ...
				 size_t count, loff_t *off)
{
	...
	ret = sscanf(buffer, "%d %d %d", &tmp_enable, &tmp_timeout,
		     &tmp_rstypt);
	if (ret <= 0) {
 ...
 */
static int pmicWd_pm_notifier(struct notifier_block *nb, unsigned long event,
			      void *unused)
{
	struct qpnp_pon *pon = sys_reset_dev;
	switch (event) {
...
static int pmicwd_test_set(void *data, u64 val)
{
	...
	if (val) {
		...
		if (pon->wd_task) {
			...
			dev_info(
				pon->dev,
				"%s: pmicwd test start: wd task not run, ignore! \n",
				__func__);
		}
		...
		if (!pon->wd_task) {
			pon->wd_task =
				kthread_create(pmicwd_kthread, pon, "pmicwd");
			if (pon->wd_task) {
				oplus_qpnp_pon_wd_config(1);
				wake_up_process(pon->wd_task);
			}
...
DEFINE_DEBUGFS_ATTRIBUTE(pmicwd_test_fops, pmicwd_test_get, pmicwd_test_set,
			 "%llu\n");
...
static void pmicwd_debugfs_init(struct qpnp_pon *pon)
{
	...
	file = debugfs_create_file("pmicwd_config", 0666, pon->debugfs, NULL,
				   &pmicwd_config_fops);
	if (IS_ERR_OR_NULL(file))
		dev_err(pon->dev,
			"%s: Couldn't create pmicwd_config debugfs file\n",
			__func__);

	file = debugfs_create_file("pmicwd_test", 0666, pon->debugfs, pon,
				   &pmicwd_test_fops);
	if (IS_ERR_OR_NULL(file))
		dev_err(pon->dev,
			"%s: Couldn't create pmicwd_test debugfs file\n",
			__func__);
		 ...
		 bool sys_reset)
{
	...
	if (!sys_reset)
		return;
	pon->pmicwd_state =
		of_property_read_bool(pdev->dev.of_node, "qcom,pmicwd");
	pon->wd_task = NULL;
	pon->suspend_state = 0;
	...
	if (sys_reset && pon->pmicwd_state) {
		if (get_eng_version() == AGING) {
			pon->pmicwd_state = PMIC_WD_DEFAULT_ENABLE |
					    (PMIC_WD_DEFAULT_TIMEOUT << 8) |
		...
		if (pon->wd_task) {
			if (get_eng_version() == AGING) {
				qpnp_pon_wd_timer(PMIC_WD_DEFAULT_TIMEOUT,
		...
		if (rc) {
			dev_err(pon->dev,
				"%s: pmicWd power state notif error %d\n",
				__func__, rc);
		}
	}
