				 ...
				 size_t cnt, loff_t *data)
{
	...
	if (val == 1) {
		...
		rc = qpnp_pon_masked_write(pon, QPNP_PON_KPDPWR_S1_TIMER(pon),
					   QPNP_PON_S1_TIMER_MASK,
					   QPNP_PON_KPDPWR_S1_TIMER_TIME);
		if (rc)
			dev_err(pon->dev,
				"Unable to write to addr=%x, rc(%d)\n",
		...
		rc = qpnp_pon_masked_write(pon, QPNP_PON_KPDPWR_S2_TIMER(pon),
					   QPNP_PON_S2_TIMER_MASK,
					   QPNP_PON_KPDPWR_S2_TIMER_TIME);
		if (rc)
			dev_err(pon->dev,
				"Unable to write to addr=%x, rc(%d)\n",
	...
	if (val == 101) {
		pr_err("QPNP_PON_RESET_S2_CNTL2(pon) addr:%x",
		       QPNP_PON_RESET_S2_CNTL2(pon));
