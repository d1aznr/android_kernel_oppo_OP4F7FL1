...
static int do_cmd_add_dir(unsigned long args)
{
	...
	list_for_each_entry (dir_node, &acm_dir_list.head, lnode) {
		if (strncmp(fwk_dir->dir, dir_node->afd.dir, ACM_DIR_MAX - 1) ==
		    0) {
...
static int do_cmd_del_dir(unsigned long args)
{
	...
	list_for_each_entry_safe (dir_node, n, &acm_dir_list.head, lnode) {
		if (strncmp(dir_node->afd.dir, fwk_dir->dir, ACM_DIR_MAX - 1) ==
		    0) {
...
static int do_cmd_add_nomediadir(unsigned long args)
{
	...
	list_for_each_entry (dir_node, &acm_nomediadir_list.head, lnode) {
		if (strncasecmp(fwk_ndir->dir, dir_node->afnd.dir,
				ACM_DIR_MAX - 1) == 0) {
			spin_unlock(&acm_nomediadir_list.spinlock);
			goto add_dir_ret;
		}
...
static int do_cmd_del_nomediadir(unsigned long args)
{
	...
	spin_lock(&acm_nomediadir_list.spinlock);
	list_for_each_entry_safe (dir_node, n, &acm_nomediadir_list.head,
				  lnode) {
		if (strncasecmp(dir_node->afnd.dir, fwk_ndir->dir,
				ACM_DIR_MAX - 1) == 0) {
				 ...
				 struct dentry *dentry, int file_type, int op)
{
	...
	if (dir_flag == 0) {
		...
		if (acm_opstat(ACM_FLAG_LOGGING)) {
			if (delete_log_upload(pkgname, taskuid, dentry,
					      file_type, op)) {
		...
		if (acm_opstat(ACM_FLAG_LOGGING) && !acm_opstat(ACM_FLAG_DEL)) {
			return DEL_ALLOWED;
		}
...
static int inquiry_create_nomedia_policy(struct dentry *dentry)
{
	...
	list_for_each_entry (dir_node, &acm_nomediadir_list.head, lnode) {
		if (strlen(dentry_path) >= strlen(dir_node->afnd.dir)) {
			if (strncasecmp(dentry_path +
						(strlen(dentry_path) -
						 strlen(dir_node->afnd.dir)),
					dir_node->afnd.dir,
					strlen(dir_node->afnd.dir)) == 0) {
				if (strstr(dir_node->afnd.dir, "/") == NULL) {
					if (IS_ROOT(dget_parent(
						    dget_parent(parent)))) {
						ret = CRT_NOT_ALLOWED;
						break;
					}
...
int acm_search(struct dentry *dentry, int file_type, int op)
{
	...
	*/
	...
	if ((acm_opstat(ACM_FLAG_DEL | ACM_FLAG_LOGGING)) &&
	    (op == FUSE_UNLINK || op == FUSE_RMDIR || op == FUSE_RENAME ||
	     op == FUSE_RENAME2)) {
		ret = inquiry_delete_policy(pkgname, taskuid, dentry, file_type,
					    op);
		if (ret != DEL_ALLOWED) {
			err = delete_log_upload_fwk(pkgname, taskuid, dentry,
						    op);
			if (err) {
...
static int is_video_file(struct dentry *dentry)
{
	static const char *const ext[] = {
		...
		"3g2",	"3gpp2", "mkv",	 "webm", "ts",	"avi", "f4v",
		"flv",	"m2ts",	 "divx", "wmv",	 "asf", "amr", NULL
	};
	int i;
...
{
	static const char *const ext[] = { ".amr", NULL };
	int i;
...
static void set_logging_uevent_env(struct acm_lnode *node)
{
	...
	snprintf(logging_env.path, sizeof(logging_env.path), "PATH=%s",
		 node->path);
	snprintf(logging_env.depth, sizeof(logging_env.depth), "DEPTH=%d",
		 node->depth);
	snprintf(logging_env.file_type, sizeof(logging_env.file_type),
		 "FTYPE=%d", node->file_type);
	...
	pr_info("ACM: %s %s %s %s %s %s %s\n", logging_env.envp[0],
		logging_env.envp[1], logging_env.envp[2], logging_env.envp[4],
		logging_env.envp[5], logging_env.envp[6], logging_env.envp[7]);
}
...
static void set_fwk_uevent_env(struct acm_lnode *node)
{
	...
	snprintf(fwk_env.path, sizeof(fwk_env.path), "FILE_PATH=%s",
		 node->path);
...
static int acm_task_init(void)
{
	...
	acm_logging_task =
		kthread_run(acm_logging_loop, NULL, "acm_logging_loop");
...
static int acm_chr_dev_init(void)
{
	...
	acm_device =
		device_create(acm_class, NULL, acm_devno, NULL, ACM_DEV_NAME);
	...
	err = kobject_add(&(acm_cdev->kobj), &(acm_kset->kobj),
			  "acm_cdev_kobj");
	if (err) {
