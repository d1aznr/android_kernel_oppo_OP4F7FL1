...
int white_list_check(struct task_struct *p)
{
	...
	for (i = 0; i < ARRAY_SIZE(white_list); i++) {
		if (!strncmp(p->comm, white_list[i].comm,
			     strlen(white_list[i].comm))) {
...
static void fdleak_check(struct task_struct *p, int fd)
{
	...
	if (p->sighand->action[SIG_FDLEAK_CHECK_TRIGGER - 1].sa.sa_handler ==
	    SIG_DFL) {
		return;
	}
	...
	if (ots->fdleak_flag == FDLEAK_ALREADY_DUMP_FLAG || p->pid != p->tgid) {
		return;
	} else if (ots->fdleak_flag == FDLEAK_ALREADY_TRIGGER_FLAG &&
		   !white_list_check(p) && fd >= dump_threshold) {
		send_sig(BIONIC_SIGNAL_FDTRACK, p, 0);
		ots->fdleak_flag = FDLEAK_ALREADY_DUMP_FLAG;
	} else if (ots->fdleak_flag != FDLEAK_ALREADY_TRIGGER_FLAG &&
		   !white_list_check(p) && fd >= load_threshold) {
...
static int __init fdleak_check_init(void)
{
	...
	if (ret < 0) {
		pr_err(FDLEAK_CHECK_LOG_TAG
		       "oplus_fdleak_check, register_kretprobe failed, return %d\n",
		       ret);
		return ret;
	}
	pr_info(FDLEAK_CHECK_LOG_TAG
		"oplus_fdleak_check, planted kretprobe at %p\n",
		g_krp.kp.addr);
