...
static struct nla_policy olc_genl_policy[OLC_ATTR_MAX + 1] = {
	...
	[OLC_ATTR_EXCEPTION_INFO] = { .type = NLA_BINARY,
				      .len = sizeof(struct exception_info) },
...
static int olc_netlink_notify_user_info(struct sk_buff *skb,
...
static int olc_netlink_get_record(struct sk_buff *skb, struct genl_info *info)
{
	...
	msg_head = genlmsg_put(msg, olc_user_pid, 0, &olc_genl_family, 0,
			       OLC_CMD_GET_RECORD);
	if (msg_head == NULL) {
	...
	if (nla_put(msg, OLC_ATTR_STATISTIC_DATA,
		    sizeof(struct statistic_header), &payload_head) != 0) {
	...
	list_for_each_entry (node, &flow_control.exception_list, list) {
		if (nla_put(msg, OLC_ATTR_STATISTIC_DATA,
			    sizeof(struct exception_element),
			    &node->data) != 0) {
	...
	list_for_each_entry (node, &flow_control.exception_list, list) {
		if (node->data.exception_id == exp_id) {
			...
			if ((enabled == 0) || (node->data.expire > jiffies)) {
				...
				node->data.expire =
					jiffies +
					flow_control.interval_second * HZ;
				ret = 0;
			}
			break;
		}
...
static int flow_control_exit(void)
{
	...
	mutex_lock(&flow_control.lock);
	list_for_each_entry_safe (node, next, &flow_control.exception_list,
				  list) {
		list_del(&node->list);
		kfree(node);
	}
...
int olc_raise_exception(struct exception_info *exp)
{
	...
	pr_notice(
		"[olc]exception:id=0x%x,time=%ld,level=%d,atomicLogs=0x%lx,logParams=%s\n",
		exp->exceptionId, exp->time, exp->level, exp->atomicLogs,
		exp->logParams);
	return olc_netlink_send_msg(OLC_CMD_NEW_EXCEPTION,
				    OLC_ATTR_EXCEPTION_INFO, (void *)exp,
