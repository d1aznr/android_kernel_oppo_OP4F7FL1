...
static ssize_t proc_generate_exception_read(struct file *file, char __user *buf,
					     ...
					     size_t size, loff_t *ppos)
{
	...
	while (token != NULL) {
		if (strncmp(token, "id", strlen("id")) == 0) {
			...
			if (value != NULL) {
				exp.exceptionId =
					simple_strtoul(value + 1, NULL, 16);
			}
		...
		} else if (strncmp(token, "exceptionType",
				   strlen("exceptionType")) == 0) {
			value = strchr(token, '=');
			if (value != NULL) {
				exp.exceptionType =
					simple_strtoul(value + 1, NULL, 10);
			}
		} else if (strncmp(token, "atomicLogs", strlen("atomicLogs")) ==
			   0) {
			value = strchr(token, '=');
			if (value != NULL) {
				exp.atomicLogs =
					simple_strtoull(value + 1, NULL, 10);
			}
		} else if (strncmp(token, "logParams", strlen("logParams")) ==
			   0) {
			value = strchr(token, '=');
			if (value != NULL) {
				strncpy(exp.logParams, value + 1,
					sizeof(exp.logParams) - 1);
			}
		}
...
int olc_proc_debug_init(void)
{
	...
	if (!proc_create("generate-exception", S_IFREG | 0600, olc_proc_dir,
			 &proc_generate_exception_fops)) {
