...
static void init_project_version(void)
{
	...
	if (np) {
		ret = of_property_read_u32(np, "nVersion",
					   &(local_ProjectInfoCDT.Version));
		if (ret) {
		...
		ret = of_property_read_u32(
			np, "nDtsi", &(local_ProjectInfoCDT.nDataBCDT.DtsiNo));
		if (ret) {
		...
		ret = of_property_read_u32(
			np, "nRF", &(local_ProjectInfoCDT.nDataSCDT.RF));
		if (ret) {
		...
		for (i = 0; i < FEATURE_COUNT; i++) {
			...
			if (ret) {
				pr_err("init_project_version_newcdt nFeature[%d]: %s %d fail",
				       i, feature_node,
				       local_ProjectInfoCDT.nDataBCDT
					       .Feature[i]);
			}
		}

		ret = of_property_read_u32(
			np, "nPCB", &(local_ProjectInfoCDT.nDataSCDT.PCB));
		if (ret) {
		...
		pr_err("KE new cdt newcdt:%d nVersion:%d Project:%d,Prj:%d, Dtsi:%d, Audio:%d, nRF:%d, PCB:%d Feature0-9:%d %d %d %d %d %d %d %d %d %d eng_version:%d confidential:%d\n",
		       newcdt, g_project->Version,
		       g_project->nDataBCDT.ProjectNo, oplus_prj,
...
static int __init oplus_project_init(void)
{
	...
	p_entry =
		proc_create_data("prjVersion", S_IRUGO, oplus_info,
				 &project_info_fops, UINT2Ptr(PROJECT_VERSION));
	if (!p_entry)
		goto error_init;
