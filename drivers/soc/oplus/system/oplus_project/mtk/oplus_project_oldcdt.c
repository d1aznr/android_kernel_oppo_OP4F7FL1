...
static int init_project_version_oldcdt(void)
{
	...
	ret = of_property_read_u32(np, "nPCBVersion",
				   &(projectInfo.nPCBVersion));
	if (ret) {
		printk("init_project_version error5");
		return -1;
	}

	ret = of_property_read_u32(np, "nENGVersion",
				   &(projectInfo.nENGVersion));
	if (ret) {
		printk("init_project_version error6");
		//return -1;
	}

	ret = of_property_read_u32(np, "isConfidential",
				   &(projectInfo.isConfidential));
	if (ret) {
	...
	printk("KE oldcdt Version Info :Project(%d) Modem(%d) Operator(%d) PCB(%d) ENGVersion(%d) Confidential(%d)\n",
	       format->nProject, format->nModem, format->nOperator,
	       format->nPCBVersion, format->nENGVersion,
	       format->isConfidential);
...
int oplus_project_init_oldcdt(void)
{
	...
	pentry = proc_create("prjVersion", S_IRUGO, oplus_info,
	...
	pentry = proc_create("pcbVersion", S_IRUGO, oplus_info,
			     &pcbVersion_proc_fops);
	if (!pentry) {
		pr_err("create pcbVersion proc failed.\n");
		//goto ERROR_INIT_VERSION;
	}
	pentry = proc_create("engVersion", S_IRUGO, oplus_info,
			     &engVersion_proc_fops);
	if (!pentry) {
	...
	pentry = proc_create("modemType", S_IRUGO, oplus_info,
			     &modemType_proc_fops);
	if (!pentry) {
		pr_err("create modemType proc failed.\n");
		//goto ERROR_INIT_VERSION;
	}
	pentry = proc_create("secureType", S_IRUGO, oplus_info,
	...
	/*Add serialID for fastboot unlock*/
	pentry = proc_create("serialID", S_IRUGO, oplus_info,
			     &serialID_proc_fops);
	if (!pentry) {
