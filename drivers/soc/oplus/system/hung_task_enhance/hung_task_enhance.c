...
static bool is_usersapce_key_process(struct task_struct *t)
{
	...
	if (!strncmp(t->comm, "system_server", TASK_COMM_LEN) ||
		...
		return true;
	if (!strncmp(t->comm, "Binder:", 7) &&
	    (t->group_leader->pid == t->pid) && (tcred->uid.val == 1000) &&
	    (t->parent != 0 && !strcmp(t->parent->comm, "main")))
		return true;
...
static void oplus_check_hung_task(struct task_struct *t, unsigned long timeout,
				  unsigned int *iowait_count, bool *show_lock,
				  bool *call_panic)
{
	...
	defined(CONFIG_OPLUS_SYSTEM_KERNEL_QCOM)
	...
	if (t->in_iowait) {
		...
		if (t->mm != NULL &&
		    t == t->group_leader) // only work on user main thread
		{
	...
	if (is_usersapce_key_process(t)) {
		if (t->state == TASK_UNINTERRUPTIBLE)
			snprintf(sysctl_hung_task_kill, HUNG_TASK_KILL_LEN,
				 "%s,uninterruptible for %lu seconds", t->comm,
				 timeout);
		else if (t->state == TASK_STOPPED)
			snprintf(sysctl_hung_task_kill, HUNG_TASK_KILL_LEN,
				 "%s,stopped for %lu seconds", t->comm,
				 timeout);
		else if (t->state == TASK_TRACED)
			snprintf(sysctl_hung_task_kill, HUNG_TASK_KILL_LEN,
				 "%s,traced for %lu seconds", t->comm, timeout);
		else
			snprintf(sysctl_hung_task_kill, HUNG_TASK_KILL_LEN,
				 "%s,unknown hung for %lu seconds", t->comm,
				 timeout);

		printk(KERN_ERR
		       "DeathHealer: task %s:%d blocked for more than %lu seconds in state 0x%lx. Count:%d\n",
		...
		if (death_count >= MAX_DEATH_COUNT) {
			if (cur_death_time - last_death_time <
			    TWICE_DEATH_PERIOD) {
				printk(KERN_ERR
				       "DeathHealer has been triggered %d times, \
					last time at: %llu\n",
			     ...
			     bool *call_panic)
{
	...
	if (t->state == TASK_UNINTERRUPTIBLE || t->state == TASK_STOPPED ||
		...
		if (!sysctl_hung_task_selective_monitoring ||
			...
			oplus_check_hung_task(t, timeout, iowait_count,
					      show_lock, call_panic);
#endif
	return;
}
