...
static char *const pon_s3_reset_reason[] = {
	[0] = ":N/A 0",	   [1] = ":N/A 1",
	[2] = ":N/A 2",	   [3] = ":N/A 3",
	[4] = ":FAULT_N",  [5] = ":PBS_WATCHDOG_TO",
	[6] = ":PBS_NACK", [7] = ":KPDPWR_ANDOR_RESIN",
};
				...
				struct kobj_attribute *attr, char *buf)
{
	...
	for (pmic_device_index = 0; pmic_device_index < 8;
	     pmic_device_index++) {
		if (DATA_VALID_FLAG !=
		    pmic_first_record.pmic_pon_poff_reason[pmic_device_index]
			    .data_is_valid) {
			continue;
		}

		L1_poff_index =
			ffs(pmic_first_record
				    .pmic_pon_poff_reason[pmic_device_index]
				    .PON_OFF_REASON);
		...
		case INDEX_S3_RESET:
			...
			if (L2_poff_index <= 0 || L2_poff_index > 8) {
				...
				L2_str_ptr =
					pon_s3_reset_reason[L2_poff_index - 1];
			}
			break;
		...
		case INDEX_FAULT_SEQ:
			...
			if (L2_poff_index <= 0 || L2_poff_index > 16) {
				...
				L2_str_ptr =
					pon_fault_reason_str[L2_poff_index - 1];
			}
			break;
		case INDEX_POFF_SEQ:
			L1_str_ptr = pon_poff_reason_str[L1_poff_index - 1];
			...
			if (L2_poff_index <= 0 || L2_poff_index > 8) {
				...
				L2_str_ptr =
					pon_poff_reason1_str[L2_poff_index - 1];
			}
			break;
		default:
			L1_poff_index = 0;
...
static char *const pon_pon_reason1_str[] = {
	[0] = "POWER ON by HARD_RESET", [1] = "POWER ON by SMPL",
	[2] = "POWER ON by RTC",	[3] = "POWER ON by DC_CHG",
	[4] = "POWER ON by USB_CHG",	[5] = "POWER ON by PON1",
	[6] = "POWER ON by CBLPWR_N",	[7] = "POWER ON by KPDPWR_N",
};

static char *const pon_warm_reset_reason1_str[] = {
	[0] = ":SOFT",	  [1] = ":PS_HOLD",  [2] = ":PMIC_WD",
	[3] = ":GP1",	  [4] = ":GP2",	     [5] = ":KPDPWR_AND_RESIN",
	[6] = ":RESIN_N", [7] = ":KPDPWR_N",
};
			       ...
			       struct kobj_attribute *attr, char *buf)
{
	...
	for (pmic_device_index = 0; pmic_device_index < 8;
		...
		if (0 == pon_index) {
			len += snprintf(&page[len], 512 - len,
					"PMIC|%d|0x%02X|NKNOW PON REASON\n",
		...
		if (QPNP_WARM_SEQ &
			...
			len += snprintf(
				...
				pmic_device_index, pon_pon_reason1_reg_vaule,
				pon_pon_reason1_str[pon_index - 1],
				0 == pon_warm_reset_index ?
					"NULL" :
					pon_warm_reset_reason1_str
						[pon_warm_reset_index - 1],
				pmic_history_count);
		} else {
			len += snprintf(&page[len], 512 - len,
					"PMIC|%d|0x%02X|PON_SEQ:%s (1/%ld)\n",
					pmic_device_index,
					pon_pon_reason1_reg_vaule,
					pon_pon_reason1_str[pon_index - 1],
					pmic_history_count);
...
static struct attribute *gen2[] = {
	&pmic_history_magic_attr.attr, &pmic_history_count_attr.attr,
	&poff_reason_attr.attr,	       &pon_reason_attr.attr,
	&ocp_status_attr.attr,	       NULL,
};
...
static const char *const pmic_pon_fault_reason1[8] = {
	[0] = "GP_FAULT0", [1] = "GP_FAULT1", [2] = "GP_FAULT2",
	[3] = "GP_FAULT3", [4] = "MBG_FAULT", [5] = "OVLO",
	[6] = "UVLO",	   [7] = "AVDD_RB",
};

static const char *const pmic_pon_fault_reason2[8] = {
	[0] = "UNKNOWN(0)",  [1] = "UNKNOWN(1)",      [2] = "UNKNOWN(2)",
	[3] = "FAULT_N",     [4] = "FAULT_WATCHDOG",  [5] = "PBS_NACK",
	[6] = "RESTART_PON", [7] = "OVERTEMP_STAGE3",
};

static const char *const pmic_pon_fault_reason3[8] = {
	[0] = "GP_FAULT4",  [1] = "GP_FAULT5",	[2] = "GP_FAULT6",
	[3] = "GP_FAULT7",  [4] = "GP_FAULT8",	[5] = "GP_FAULT9",
	[6] = "GP_FAULT10", [7] = "GP_FAULT11",
};

static const char *const pmic_pon_s3_reset_reason[8] = {
	[0] = "UNKNOWN(0)", [1] = "UNKNOWN(1)",
	[2] = "UNKNOWN(2)", [3] = "UNKNOWN(3)",
	[4] = "FAULT_N",    [5] = "FAULT_WATCHDOG",
	[6] = "PBS_NACK",   [7] = "KPDPWR_AND/OR_RESIN",
};

static const char *const pmic_pon_pon_pbl_status[8] = {
	[0] = "UNKNOWN(0)", [1] = "UNKNOWN(1)", [2] = "UNKNOWN(2)",
	[3] = "UNKNOWN(3)", [4] = "UNKNOWN(4)", [5] = "UNKNOWN(5)",
	[6] = "XVDD",	    [7] = "DVDD",
};
...
static const struct pmic_pon_trigger_mapping pmic_pon_pon_trigger_map[] = {
	{ 0x0084, "PS_HOLD" },	       { 0x0085, "HARD_RESET" },
	{ 0x0086, "RESIN_N" },	       { 0x0087, "KPDPWR_N" },
	{ 0x0621, "RTC_ALARM" },       { 0x0640, "SMPL" },
	{ 0x18C0, "PMIC_SID1_GPIO5" }, { 0x31C2, "USB_CHARGER" },
};
...
static int
get_pon_log_by_state_event(u8 state, u8 event, int skip,
			   struct PMICGen3RecordKernelStruct *pmic_record_ptr,
			   struct PmicGen3PonStateStruct *pon_log)
{
	...
	for (i = 0; i < MAX_STATE_RECORDS; i++) {
		...
		if (((state == tmp_state) &&
		     (event == tmp_event)) || /* match state & event */
		    ((state == PMIC_PON_STATE_MAX) &&
		     (event == tmp_event)) || /* match event */
		    ((state == tmp_state) &&
		     (event == PMIC_PON_EVENT_PMIC_MAX)) /* match state */) {
			if (skip > 0) {
				skip = skip - 1;
			} else {
				*pon_log = pmic_record_ptr
						   ->pmic_state_machine_log[i];
				return i;
			}
		}
	}
			      ...
			      char *parse_log)
{
	...
	case PMIC_PON_EVENT_PON_TRIGGER_RECEIVED:
		...
		pos += scnprintf(buf + pos, BUF_SIZE - pos, "PON Trigger: ");
		if (label) {
	...
	case PMIC_PON_EVENT_OTP_COPY_COMPLETE:
		scnprintf(buf, BUF_SIZE,
			  "OTP Copy Complete: last addr written=0x%04X", data);
		break;
	...
	case PMIC_PON_EVENT_RESET_TRIGGER_RECEIVED:
		...
		pos += scnprintf(buf + pos, BUF_SIZE - pos, "Reset Trigger: ");
		if (label) {
			pos += scnprintf(buf + pos, BUF_SIZE - pos, "%s",
					 label);
		} else {
	...
	case PMIC_PON_EVENT_FAULT_REASON_1_2:
		...
		if (pon_state_machine->data1) {
			pos += scnprintf(
				buf + pos, BUF_SIZE - pos, "%sFAULT_REASON2=",
				(pon_state_machine->data0) ? "; " : "");
			pos += pmic_pon_log_print_reason(
				buf + pos, BUF_SIZE - pos,
	...
	case PMIC_PON_EVENT_FUNDAMENTAL_RESET:
		...
		if (pon_state_machine->data0) {
			pos += scnprintf(
				buf + pos, BUF_SIZE - pos, "%sS3_RESET_REASON=",
				(pon_state_machine->data1) ? "; " : "");
			pos += pmic_pon_log_print_reason(
				buf + pos, BUF_SIZE - pos,
				pon_state_machine->data0,
				     ...
				     struct kobj_attribute *attr, char *buf)
{
	...
	/* Fundamental Reset */
	if (-1 !=
	    get_pon_log_by_state_event(PMIC_PON_STATE_MAX, /* ignore state */
				       PMIC_PON_EVENT_FUNDAMENTAL_RESET, 0,
				       &pmic_first_record, &tmp_pon_log)) {
		/* FAULT */
		L1_poff_code = 0x08;
		...
		len += snprintf(&page[len], 512 - len,
				"PMIC|%d|0x%02X|0x%04X|%s (%ld)\n", 0,
				L1_poff_code, L2_poff_code, parse_log_str1,
				pmic_history_count);
	}
	/* ->fault(state:6) */
	else if (-1 !=
		 get_pon_log_by_state_event(PMIC_PON_STATE_FAULT6,
					    PMIC_PON_EVENT_FAULT_REASON_1_2, 0,
					    &pmic_first_record, &tmp_pon_log)) {
		/* FAULT */
		L1_poff_code = 0x40;
		L2_poff_code = tmp_pon_log.data1 << 8 | tmp_pon_log.data0;
		...
		/* PMIC0 fault log */
		if (-1 != get_pon_log_by_state_event(
				  PMIC_PON_STATE_FAULT6,
				  PMIC_PON_EVENT_FAULT_REASON_3, 0,
				  &pmic_first_record, &tmp_pon_log)) {
			pmic_pon_log_parse(&tmp_pon_log, parse_log_str2);
		} else {
			snprintf(parse_log_str2, 32,
				 "Can't find Fault_REASON3");
		}

		len += snprintf(&page[len], 512 - len,
				"PMIC|%d|0x%02X|0x%04X|%s,%s (%ld)\n", 0,
				L1_poff_code, L2_poff_code, parse_log_str1,
				parse_log_str2, pmic_history_count);
	}
	/* on(state:3)->reset(state:4) */
	else if (-1 != get_pon_log_by_state_event(
			       PMIC_PON_STATE_RESET, PMIC_PON_EVENT_RESET_TYPE,
			       0, &pmic_first_record, &tmp_pon_log)) {
		L1_poff_code = 0x80;
		pmic_pon_log_parse(
			&tmp_pon_log,
		...
		if (PMIC_PON_RESET_TYPE_WARM_RESET == tmp_pon_log.data0) {
			if (-1 != get_pon_log_by_state_event(
					  PMIC_PON_STATE_WARM_RESET,
					  PMIC_PON_EVENT_WARM_RESET_COUNT, 0,
					  &pmic_first_record, &tmp_pon_log)) {
				pmic_pon_log_parse(
					&tmp_pon_log,
					parse_log_str3); /* warm reset count */
				skip = tmp_pon_log.data1 << 8 |
				       tmp_pon_log
					       .data0; /* For warm reset case, we need skip warm reset count-1 */
		...
		if (-1 != get_pon_log_by_state_event(
				  PMIC_PON_STATE_RESET,
				  PMIC_PON_EVENT_RESET_TRIGGER_RECEIVED, skip,
				  &pmic_first_record, &tmp_pon_log)) {
			pmic_pon_log_parse(&tmp_pon_log, parse_log_str2);
			L2_poff_code =
				tmp_pon_log.data1 << 8 | tmp_pon_log.data0;
		} else {
			L2_poff_code = 0;
			snprintf(parse_log_str2, 32,
		...
		len += snprintf(&page[len], 512 - len,
				"PMIC|%d|0x%02X|0x%04X|%s,%s,%s (%ld)\n", 0,
				L1_poff_code, L2_poff_code, parse_log_str1,
				parse_log_str2, parse_log_str3,
				pmic_history_count);
	} else {
		len += snprintf(
			&page[len], 512 - len,
				    ...
				    struct kobj_attribute *attr, char *buf)
{
	...
	if (-1 !=
	    get_pon_log_by_state_event(PMIC_PON_STATE_OFF,
				       PMIC_PON_EVENT_PON_TRIGGER_RECEIVED, 0,
				       &pmic_first_record, &tmp_pon_log)) {
		pon_code = tmp_pon_log.data1 << 8 | tmp_pon_log.data0;
		pmic_pon_log_parse(&tmp_pon_log, parse_log_str1);
		len += snprintf(&page[len], 512 - len,
				"PMIC|%d|0x%02X|%s (%ld)\n", 0, pon_code,
				parse_log_str1, pmic_history_count);
	} else {
		len += snprintf(&page[len], 512 - len,
				    ...
				    struct kobj_attribute *attr, char *buf)
{
	...
	for (pmic_device_index = 0; pmic_device_index < 8;
	     pmic_device_index++) {
		if (DATA_VALID_FLAG !=
		    pmic_first_record.pmic_ocp_record[pmic_device_index]
			    .data_is_valid) {
			continue;
		}
		len += snprintf(
			&page[len], 256 - len, "PMIC|%d|0x%08X%02X%02X\n",
			pmic_device_index,
			pmic_first_record.pmic_ocp_record[pmic_device_index]
				.ldo_ocp_status,
			pmic_first_record.pmic_ocp_record[pmic_device_index]
...
static struct attribute *gen3[] = {
	&pmic_history_magic_gen3_attr.attr, &pmic_history_count_gen3_attr.attr,
	&poff_reason_gen3_attr.attr,	    &pon_reason_gen3_attr.attr,
	&ocp_status_gen3_attr.attr,	    NULL,
};
