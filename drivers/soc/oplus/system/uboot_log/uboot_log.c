...
static int uboot_kboot_buffer_init()
{
	...
	kboot_uboot_logmemory =
		of_get_child_by_name(reserved_memory, "kboot_uboot_logmem");
	...
	pr_debug(
		"xbl_uefi_memory:base:%llx, size:%llx, xbl_uefi_logoffset:%x\n",
...
int ubootback_thread_fn()
{
	...
	while (kboot_log_buf_len > total_size + 2 * LINE_MAX_SIZE) {
		...
		if (back_kmsg_dump_get_buffer(
			    &kboot_dumper, true, kboot_log_buf + total_size,
...
static int __init kernel_uboot_log_init()
{
	...
	ubootback_thread =
		kthread_run(ubootback_thread_fn, NULL, "ubootback_thread");
