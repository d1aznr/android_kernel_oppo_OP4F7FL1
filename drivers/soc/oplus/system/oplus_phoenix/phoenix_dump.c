		     ...
		     struct kmsg_dumper *kmsg_dumper)
{
	...
	file_fd = ksys_open(filepath, O_CREAT | O_WRONLY | O_TRUNC,
			    FILE_MODE_0666);
#else
	file_fd = sys_open(filepath, O_CREAT | O_WRONLY | O_TRUNC,
			   FILE_MODE_0666);
#endif
	if (file_fd < 0) {
		PHX_KLOG_ERROR("sys_open %s failed, error: %d", filepath,
			       file_fd);
		result = -1;
		goto phx_bail;
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	fd = ksys_open(keyinfo_file, O_CREAT | O_WRONLY | O_TRUNC,
		       FILE_MODE_0666);
#else
	fd = sys_open(keyinfo_file, O_CREAT | O_WRONLY | O_TRUNC,
		      FILE_MODE_0666);
#endif
	if (fd < 0) {
