...
#define oprkl_info_print(fmt, arg...) pr_err("[op_kernel_log] " fmt, ##arg)

#define oprkl_err_print(fmt, arg...) pr_err("[op_kernel_log] " fmt, ##arg)
...
#define KERNEL_HEAD_OFFSET                                                     \
	1024 * 1024 * 4 //reserve 0~ 4Mb for uefi log thread use
...
int wb_log(struct block_device *bdev, char *buf, char *line_buf)
{
	...
	while (loop_cnt <
	       STOP_CNT) { //STOP_CNT = 60 sec, so it'll record 60 sec kmsg.
		while (kmsg_dump_get_line(&kmsg_dumper, true, line_buf,
					  BUF_SIZE_OF_LINE, &len)) {
			...
			if (isfull || isforcewb) {
				...
				iov_iter_kvec(&iter, WRITE, &iov, 0,
					      WB_BLOCK_SIZE);
				...
				if (likely(ret > 0)) {
					...
					if (isfull) {
						kiocb.ki_pos +=
							WB_BLOCK_SIZE; //shfit offset of write back size
					}
					...
					oprkl_err_print(
						"generic_perform_write failed\n");
					return 1;
				}
				if (isfull) {
					write_block_cnt++;
					if (write_block_cnt >=
					    PER_LOG_BLOCK) { //it will record 2M
						oprkl_err_print(
							"write_block_cnt >= PER_LOG_BLOCK, This mean log full, stop record\n");
						goto FINISH;
					}
					memset(buf, 0, PER_LOG_BUF_SIZE);
					temp_size = 0;
				...
				if (phx_is_system_server_init_start()) {
					oprkl_info_print(
						"system_server init, stop record\n");
			...
			if (((temp_size + (len - 1)) >= WB_BLOCK_SIZE)) {
				oprkl_info_print(
					"reserve_log failed wb %d line len %d ,buf offset ptr:%x, buf max offset ptr:%x\n",
					temp_size, len, (buf + temp_size),
					(buf + WB_BLOCK_SIZE));
		...
		msleep_interruptible(
			PER_LOOP_MSEC); //check log update once per 0.5 sec
		isforcewb = true;
	}
FINISH:
	oprkl_info_print(
		"Finish! Stop record line_count: %d write_block_cnt: %d, total_write_times: %d\n",
...
int init_header(struct block_device *bdev,
		struct kernel_log_thread_header *head)
{
	int ret = 0;

	memset(head, 0, sizeof(struct kernel_log_thread_header));
	ret = read_header(bdev, head, sizeof(struct kernel_log_thread_header),
			  KERNEL_HEAD_OFFSET);
	if (ret)
		return ret;
	...
	if (memcmp(head->magic, LOG_THREAD_MAGIC, strlen(LOG_THREAD_MAGIC)) ==
	    0) {
		oprkl_info_print("This is initialized header\n");
		if (head->is_monitoring == 1) {
			oprkl_err_print(
				"Last boot, not complete need feedback\n");
		...
		oprkl_info_print(
			"No match ,The boot is first boot, need init\n");
		memset(head, 0, sizeof(struct kernel_log_thread_header));
		memcpy(head->magic, LOG_THREAD_MAGIC, strlen(LOG_THREAD_MAGIC));
	}
	if (get_boot_mode() ==
	    MSM_BOOT_MODE__NORMAL) { //only normal boot need monitor
		oprkl_info_print("The boot is normal boot, need monitor\n");
		head->is_monitoring = 1;
		head->last_bootcount = head->bootcount;
	}
	ret = write_header(bdev, head, sizeof(struct kernel_log_thread_header),
			   KERNEL_HEAD_OFFSET);

	return ret;
}

int clear_monitor_header_flag(struct block_device *bdev,
			      struct kernel_log_thread_header *head)
{
	int ret = 0;
...
static int reserve_log_main(void *arg)
{
	...
	if (read_header(bdev, &uefi_head, sizeof(struct uefi_log_thread_header),
			UEFI_HEAD_OFFSET)) {
		oprkl_err_print("read uefi_log_thread_header failed\n");
		goto EXIT;
	}
	oprkl_info_print(
		"Now kernel boot count %d and uefi boot count %d, is_monitoring %d, log number:[%d / 15]\n",
		kernel_head.bootcount, uefi_head.bootcount,
		kernel_head.is_monitoring, kernel_head.bootcount % MAX_LOG_NUM);
	wb_start_offset =
		(HEADER_BLOCK +
		 (kernel_head.bootcount % MAX_LOG_NUM) * PER_LOG_BLOCK) *
		WB_BLOCK_SIZE;
