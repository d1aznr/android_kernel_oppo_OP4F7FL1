...
static int mpm_parse_dt(void)
{
	...
	np_imem =
		of_find_compatible_node(NULL, NULL, "qcom,msm-imem-boot_stats");
	if (!np_imem) {
	...
	np_mpm2 =
		of_find_compatible_node(NULL, NULL, "qcom,mpm2-sleep-counter");
	if (!np_mpm2) {
...
static int panic_in_reboot_notifier(struct notifier_block *nb,
...
static int bootprof_bootloader(void)
{
	...
	uefi_time = (readl_relaxed(&boot_stats->bootloader_end) * 1000) /
		    mpm_counter_freq;
...
static __init_or_module void tp_initcall_start_cb(void *data, initcall_t fn)
{
	...
	if (!list_empty(&initcall_list)) {
		list_for_each_entry_safe (pos, next, &initcall_list,
					  dev_entry) {
