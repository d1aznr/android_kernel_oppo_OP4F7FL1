...
static int shutdown_timeout_flag_write_now(void *args)
{
	...
	if (IS_ERR(opfile)) {
		...
		opfile = filp_open(OPLUS_PARTITION_OPLUSRESERVE3_LINK, O_RDWR,
				   0600);
		if (IS_ERR(opfile)) {
			...
			/* #ifdef OPLUS_CUSTOM_OP_DEF */
			opfile = filp_open(DERIVED_PARTITION_OPLUSRESERVE3_LINK,
					   O_RDWR, 0600);
			if (IS_ERR(opfile)) {
