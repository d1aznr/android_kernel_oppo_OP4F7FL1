...
bool is_silent_reboot_or_sau(void)
{
	...
	return MSM_BOOT_MODE__SILENCE == boot_mode ||
	       MSM_BOOT_MODE__SAU == boot_mode;
}

inline void oplus_sync_saupwk_event(unsigned int type, unsigned int code,
				    int value)
{
	if (saupwk_en && is_silent_reboot_or_sau() &&
	    (!saupwk_cache_obtained) && type == EV_KEY && code == KEY_POWER &&
	    value == SAUPWK_KEY_DOWN) {
		sau_pwknum += 1;
...
static int __init oppo_saupwk_during_init(void)
{
