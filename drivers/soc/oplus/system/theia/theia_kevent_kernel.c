...
void theia_kevent_receive_from_user(struct sk_buff *__skbbr)
{
	...
	if (skbu && skbu->len >= sizeof(struct nlmsghdr)) {
		...
		if ((nlh->nlmsg_len >= sizeof(struct nlmsghdr)) &&
			...
			if (size > 0) {
				pdata = (char *)kmalloc(size + 0x10,
							GFP_KERNEL);
				if (pdata) {
					memcpy(pdata, NLMSG_DATA(nlh), size);
					pdata[size] = 0x0;

					THEIA_KEVENT_DEBUG_PRINTK(
						"theia_kevent_receive_from_user, type:%d pdata:%s\n",
						nlh->nlmsg_type, pdata);
					if ((nlh->nlmsg_type ==
					     THEIA_KEVENT_CONNECT) &&
					    (!strcmp(pdata,
						     THEIA_KEVENT_MODULE))) {
						theia_kevent_pid =
							nlh->nlmsg_pid;
						THEIA_KEVENT_DEBUG_PRINTK(
							"theia_kevent_receive_from_user, theia_kevent_pid is %u ..\n",
							theia_kevent_pid);
					}
				}
			}
...
int theia_kevent_send_to_user(struct theia_kevent_packet *userinfo)
{
	...
	if (theia_kevent_pid == -1) {
		THEIA_KEVENT_DEBUG_PRINTK(
			"theia_kevent_send_to_user theia_kevent_pid is -1, return\n");
		return -1;
	}
	...
	if (userinfo->len >= MAX_PAYLOAD_DATASIZE) {
		THEIA_KEVENT_DEBUG_PRINTK(
			"theia_kevent_send_to_user payload_length out of range\n");
		return -1;
	}
	...
	if (skbuff == NULL) {
		THEIA_KEVENT_DEBUG_PRINTK(
			"theia_kevent_send_to_user skbuff alloc_skb failed\n");
		return -1;
	}
	...
	if (nlh == NULL) {
		THEIA_KEVENT_DEBUG_PRINTK(
			"theia_kevent_send_to_user nlmsg_put failaure\n");
		nlmsg_free(skbuff);
		return -1;
	}
	...
	ret = netlink_unicast(theia_netlink_fd, skbuff, theia_kevent_pid,
			      MSG_DONTWAIT);
	if (ret < 0) {
		THEIA_KEVENT_DEBUG_PRINTK(
			"theia_kevent_send_to_user send fail=%d, theia_kevent_pid=%d \n",
...
void SendTheiaKevent(int type, char *log_tag, char *event_id, char *payload)
{
	...
	THEIA_KEVENT_DEBUG_PRINTK(
		"SendTheiaKevent, theia_kevent_send_to_user user_msg_info->data:%s\n",
		user_msg_info->data);
...
int theia_kevent_module_init(void)
{
	...
	theia_netlink_fd = netlink_kernel_create(
		&init_net, OPLUS_NETLINK_THEIA_KEVENT, &cfg);
	if (!theia_netlink_fd) {
		THEIA_KEVENT_DEBUG_PRINTK(
			"theia_kevent can not create a socket\n");
		return -1;
	}
