						...
						loff_t *off)
{
	...
	len = sprintf(
		...
		g_black_data.status, g_black_data.timeout_ms,
		g_black_data.is_panic, g_black_data.get_log, systemserver_pid);
...
static bool handle_param_setup(char *key, char *value)
{
	...
	if (!strncmp(key, "state", 5)) {
		...
		if (sscanf(value, "%d", &timeout) == 1) {
			g_black_data.timeout_ms = g_bright_data.timeout_ms =
				timeout;
		}
		...
		if (sscanf(value, "%d", &is_panic) == 1) {
			g_black_data.is_panic = g_bright_data.is_panic =
				is_panic;
		}
						 ...
						 size_t count, loff_t *off)
{
	...
	while ((param = strsep(&pBuffer, ";"))) {
		...
		POWER_MONITOR_DEBUG_PRINTK(
			"%s: param:%s ret:%d key:%s value:%s\n", __func__,
			param, ret, key, value);
		if (ret == 2) {
			if (!handle_param_setup(key, value)) {
				POWER_MONITOR_DEBUG_PRINTK(
					"%s: setup param fail! key:%s, value:%s\n",
					__func__, key, value);
			}
		}
	}
...
ssize_t get_last_pwkey_stage(char *buf)
{
	if (stage_start != flow_index) {
		int last_index =
			(flow_index == 0) ? (flow_size - 1) : (flow_index - 1);
...
ssize_t get_pwkey_stages(char *buf)
{
	...
	POWER_MONITOR_DEBUG_PRINTK(
		"get_pwkey_stages start_index:%d end_index:%d\n", start_index,
		end_index);
					       ...
					       loff_t *off)
{
	...
	POWER_MONITOR_DEBUG_PRINTK(
		"enter theia_powerkey_report_proc_read %d  %d", count, *off);
						   ...
						   size_t count, loff_t *off)
{
	...
	POWER_MONITOR_DEBUG_PRINTK(
		"theia_powerkey_test_node_proc_write buffer:%s\n", buffer);
	if (!strncmp(buffer, "test_d_block\n", 13)) {
		block_thread = get_current();
		POWER_MONITOR_DEBUG_PRINTK(
			"theia_powerkey_test_node_proc_write set TASK_UNINTERRUPTIBLE block_thread pid:%d\n",
			block_thread->pid);
		set_current_state(TASK_UNINTERRUPTIBLE);
		schedule();
		POWER_MONITOR_DEBUG_PRINTK(
			"theia_powerkey_test_node_proc_write set TASK_UNINTERRUPTIBLE, after schedule\n");
		block_thread = NULL;
	} else if (!strncmp(buffer, "test_d_unblock\n", 15)) {
		if (block_thread != NULL) {
			POWER_MONITOR_DEBUG_PRINTK(
				"theia_powerkey_test_node_proc_write call wake_up_process pid:%d\n",
				block_thread->pid);
...
*/
		...
		SendTheiaKevent(THEIA_KEVENT_TYPE_COMMON_STRING, "logTagTest",
				"eventIDTest", "Powerkey test kevent!");
	}
...
{
	POWER_MONITOR_DEBUG_PRINTK(
		"theia_pwk_stage_start start %x:  %x   %x   flow_buf\n",
...
static void show_coretask_state(void)
{
	...
	for_each_process_thread (g, p) {
		if (is_zygote_process(p) ||
		    !strncmp(p->comm, "system_server", TASK_COMM_LEN) ||
...
{
	POWER_MONITOR_DEBUG_PRINTK(
		"powerkey_monitor_init theia_kevent_module_init\n");
	theia_kevent_module_init();
	POWER_MONITOR_DEBUG_PRINTK(
		"powerkey_monitor_init bright_screen_check_init\n");
	bright_screen_check_init();
	POWER_MONITOR_DEBUG_PRINTK(
		"powerkey_monitor_init black_screen_check_init\n");
