...
{
	BLACK_DEBUG_PRINTK(
		"black_screen_timer_restart:blank = %d,status = %d\n",
	...
	if (g_black_data.status != BLACK_STATUS_CHECK_ENABLE &&
		...
		BLACK_DEBUG_PRINTK(
			"black_screen_timer_restart:g_black_data.status = %d return\n",
			g_black_data.status);
...
#endif
	{
		...
		BLACK_DEBUG_PRINTK(
			"black_screen_timer_restart: get_systemserver_pid %d\n",
			bl_start_check_systemid);
		mod_timer(&g_black_data.timer,
			  jiffies + msecs_to_jiffies(g_black_data.timeout_ms));
		BLACK_DEBUG_PRINTK(
			"black_screen_timer_restart: black screen check start %u\n",
...
static bool get_log_swich()
{
	return (BLACK_STATUS_CHECK_ENABLE == get_status() ||
		BLACK_STATUS_CHECK_DEBUG == get_status()) &&
	       g_black_data.get_log;
}
...
static void get_blackscreen_check_dcs_logmap(char *logmap)
{
	...
	snprintf(
		...
		PWKKEY_BLACK_SCREEN_DCS_LOGTYPE, g_black_data.error_id,
...
static void black_error_happen_work(struct work_struct *work)
{
	...
	if (bla_data->error_count == 0) {
		...
		getnstimeofday(&ts);
		sprintf(g_black_data.error_id, "%d.%ld.%ld",
			get_systemserver_pid(), ts.tv_sec, ts.tv_nsec);
	}

	if (bla_data->error_count < BLACK_MAX_WRITE_NUMBER) {
		BLACK_DEBUG_PRINTK(
			"black_error_happen_work error_count< BLACK_MAX_WRITE_NUMBER\n");
		bla_data->error_count++;
		dump_freeze_log();
		black_write_error_count(bla_data);
	}
	BLACK_DEBUG_PRINTK(
		"black_error_happen_work error_id = %s, error_count = %d\n",
...
static void black_timer_func(struct timer_list *t)
{
	...
	if (bl_start_check_systemid == get_systemserver_pid()) {
		...
		BLACK_DEBUG_PRINTK(
			"black_timer_func, not valid for check, skip\n");
	}
}
				      ...
				      unsigned long event, void *data)
{
	...
	if (event == MSM_DRM_EVENT_BLANK && evdata && evdata->data) {
		...
		if (g_black_data.status != BLACK_STATUS_CHECK_DEBUG) {
			...
			BLACK_DEBUG_PRINTK(
				"black_fb_notifier_callback: del timer,event:%lu status:%d blank:%d\n",
				event, g_black_data.status, g_black_data.blank);
		} else {
			BLACK_DEBUG_PRINTK(
				"black_fb_notifier_callback:event = %lu status:%d blank:%d\n",
				event, g_black_data.status, g_black_data.blank);
		}
		...
		BLACK_DEBUG_PRINTK(
			"black_fb_notifier_callback:event = %lu status:%d\n",
...
int black_screen_check_init(void)
{
	...
	proc_create("blackSwitch", S_IRWXUGO, NULL,
		    &black_screen_cancel_proc_fops);
	...
	timer_setup((&g_black_data.timer), (black_timer_func),
		    TIMER_DEFERRABLE);
