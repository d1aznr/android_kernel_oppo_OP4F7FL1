...
{
	BRIGHT_DEBUG_PRINTK(
		"bright_screen_timer_restart:blank = %d,status = %d\n",
	...
	if (g_bright_data.status != BRIGHT_STATUS_CHECK_ENABLE &&
		...
		BRIGHT_DEBUG_PRINTK(
			"bright_screen_timer_restart:g_bright_data.status = %d return\n",
			g_bright_data.status);
		return g_bright_data.status;
	}

#ifdef CONFIG_DRM_MSM
	if (g_bright_data.blank ==
	    MSM_DRM_BLANK_UNBLANK) //MSM_DRM_BLANK_POWERDOWN
#else
...
#endif
	{
		...
		BRIGHT_DEBUG_PRINTK(
			"bright_screen_timer_restart: bright screen check start %u\n",
...
static bool get_log_swich()
{
	return (BRIGHT_STATUS_CHECK_ENABLE == get_status() ||
		BRIGHT_STATUS_CHECK_DEBUG == get_status()) &&
	       g_bright_data.get_log;
}
...
static void get_brightscreen_check_dcs_logmap(char *logmap)
{
	...
	snprintf(
		logmap, 512,
		"logmap{logType:%s;error_id:%s;error_count:%u;systemserver_pid:%d;stages:%s;catchlog:%s}",
		PWKKEY_BRIGHT_SCREEN_DCS_LOGTYPE, g_bright_data.error_id,
...
static bool is_bright_last_stage_skip()
{
	...
	nLen = sizeof(bright_last_skip_block_stages) /
	       sizeof(bright_last_skip_block_stages[0]);
	...
	for (i = 0; i < nLen; i++) {
		...
		if (!strcmp(stage, bright_last_skip_block_stages[i])) {
			BRIGHT_DEBUG_PRINTK(
				"is_bright_last_stage_skip return true, stage:%s",
				stage);
			return true;
		}
	}
...
static bool is_bright_contain_skip_stage()
{
	...
	for (i = 0; i < nArrayLen; i++) {
		if (strstr(stages, bright_skip_stages[i]) != NULL) {
			BRIGHT_DEBUG_PRINTK(
				"is_bright_contain_skip_stage return true, stages:%s",
				stages);
			return true;
		}
	}
...
static void get_bright_resume_dcs_logmap(char *logmap)
{
	snprintf(
		logmap, 512,
		"logmap{logType:%s;error_id:%s;resume_count:%u;normalReborn:%s;catchlog:false}",
		PWKKEY_BRIGHT_SCREEN_DCS_LOGTYPE, g_bright_data.error_id,
		g_bright_data.error_count,
...
static void bright_error_happen_work(struct work_struct *work)
{
	...
	if (bri_data->error_count == 0) {
		struct timespec ts;
		getnstimeofday(&ts);
		sprintf(g_bright_data.error_id, "%d.%ld.%ld",
			get_systemserver_pid(), ts.tv_sec, ts.tv_nsec);
	}
	...
	BRIGHT_DEBUG_PRINTK(
		"bright_error_happen_work error_id = %s, error_count = %d\n",
...
static void bright_timer_func(struct timer_list *t)
{
	...
	if (br_start_check_systemid == get_systemserver_pid()) {
		...
		BRIGHT_DEBUG_PRINTK(
			"bright_timer_func, not valid for check, skip\n");
	}
}
				       ...
				       unsigned long event, void *data)
{
	...
	if (event == MSM_DRM_EVENT_BLANK && evdata && evdata->data) {
		...
		if (g_bright_data.status != BRIGHT_STATUS_CHECK_DEBUG) {
			...
			BRIGHT_DEBUG_PRINTK(
				"bright_fb_notifier_callback: del timer,event:%lu status:%d blank:%d\n",
				event, g_bright_data.status,
				g_bright_data.blank);
		} else {
			BRIGHT_DEBUG_PRINTK(
				"bright_fb_notifier_callback:event = %lu status:%d blank:%d\n",
				event, g_bright_data.status,
				g_bright_data.blank);
		}
	} else if (event == MSM_DRM_EARLY_EVENT_BLANK && evdata &&
		   evdata->data) {
		blank = evdata->data;
		...
		BRIGHT_DEBUG_PRINTK(
			"bright_fb_notifier_callback:event = %lu status:%d\n",
			event, g_bright_data.status);
	}
				       ...
				       unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		...
		if (g_bright_data.status != BRIGHT_STATUS_CHECK_DEBUG) {
			delete_timer("FINISH_FB", true);
			BRIGHT_DEBUG_PRINTK(
				"bright fb_notifier_callback: del timer,event:%d,status:%d,blank:%d\n",
				event, g_bright_data.status,
				g_bright_data.blank);
		} else {
			BRIGHT_DEBUG_PRINTK(
...
int bright_screen_check_init(void)
{
	...
	proc_create("brightSwitch", S_IRWXUGO, NULL,
		    &bright_screen_cancel_proc_fops);

	INIT_WORK(&g_bright_data.error_happen_work, bright_error_happen_work);
	timer_setup((&g_bright_data.timer), (bright_timer_func),
		    TIMER_DEFERRABLE);
