...
static struct wakeup_count_desc_t *const all_modules[] = {
	&wc_powerkey, &wc_rtc_alarm, &wc_modem, &wc_wlan,  &wc_adsp,
	&wc_sensor,   &wc_cdsp,	     &wc_slpi,	&wc_glink, &wc_abort,
	&wc_other,    &wc_sum,	     NULL,
};
...
void alarmtimer_wakeup_count(struct alarm *alarm)
{
	if (alarm->type == ALARM_REALTIME || alarm->type == ALARM_BOOTTIME) {
		...
		if (alarmtimer_suspend_flag_get() ||
			...
			wakeup_reasons_statics(IRQ_NAME_RTCALARM,
					       WS_CNT_RTCALARM);
			...
			if (alarm->function) {
				pr_info("%s: alarm_type=%d, not_netalarm_count=%lld, not_netalarm_wakeup_count=%lld,owner=%s,alarm_func=%pf\n",
					...
					awuc.alarm_wakeup_count, alarm->comm,
					alarm->function);
			}
		}
	}
					      ...
					      char *buf)
{
	...
	for (i = 0; (desc = all_modules[i]) != NULL && buf_offset < PAGE_SIZE;
		...
		if (!strncmp(desc->module_name, "abort", strlen("abort")) ||
		    ...
		    !strncmp(desc->module_name, "wakeup_sum",
			     strlen("wakeup_sum"))) {
...
static struct kobj_attribute wakeup_stastisc_reset =
	__ATTR(wakeup_stastisc_reset, S_IWUSR | S_IRUGO, NULL,
	       wakeup_stastisc_reset_store);
#if 0
