...
static int process_notifier(struct notifier_block *self, unsigned long cmd,
			    void *v)
{
...
static inline struct ns_task_struct *ns_task_insert(pid_t pid, int type)
{
	...
	ntask = kzalloc(sizeof(struct ns_task_struct), GFP_KERNEL);
	if (!ntask)
		goto out;
...
{
	nsi.fn_status =
		(nsi.data_avail < nsi.data_threshold ? NS_CAPACITY :
						       NS_NORMAL) |
		...
		(nsi.swap_out - nsi.swap_total > nsi.quota_today ? NS_QUOTA :
								   NS_NORMAL);
...
static void ns_life_ctrl_update(bool timer)
{
	...
	if (tm.tm_mday != nsi.tm.tm_mday && tm.tm_hour > 2 &&
		...
		nsi.write_lifelong =
			nsi.write_lifelong - write_inc + nsi.write_upper;
...
static int ns_reclaim_pte(pmd_t *pmd, unsigned long addr, unsigned long end,
			  struct mm_walk *walk)
{
...
static ssize_t reclaim_anon(struct task_struct *task)
{
	...
	printk("NandSwap: out %s (pid %d) (size %d-%d to %d-%d) "
	       ...
	       task->comm, task->pid, task_anon, task_swap, a_task_anon,
	       a_task_swap, rp.nr_scanned, rp.nr_reclaimed, rp.nr_act,
	       rp.nr_inact);
#endif
	nsi.swap_out_cnt++;
...
static bool drop_swapcache_task(struct task_struct *task)
{
	...
	printk("NandSwap: cache %s (pid %d) (size %d-%d to %d-%d) scan %d reclaim %d\n",
	       task->comm, task->pid, task_anon, task_swap, a_task_anon,
	       a_task_swap, rp.nr_scanned, rp.nr_reclaimed);
#endif
...
static int swapdropd_fn(void *p)
{
	...
	for (;;) {
		while (!pm_freezing &&
		       dequeue_reclaim_data(&data, &drop_info)) {
			rcu_read_lock();
			task = find_task_by_vpid(data.pid);
...
static int swap_ratio_pte(pmd_t *pmd, unsigned long start, unsigned long end,
			  struct mm_walk *walk)
{
	pte_t *orig_pte;
	unsigned long index;
				     ...
				     unsigned long type)
{
	...
	for (vma = mm->mmap; vma; vma = vma->vm_next) {
		...
		nsr.vma = vma;
		walk_page_range_hook(mm, vma->vm_start, vma->vm_end, &walk,
				     &nsr);
	}
...
static int stat_info_seq_show(struct seq_file *seq, void *offset)
{
	...
	seq_printf(seq,
		   ...
		   "pswpin: %llu\n"
		   "ns_dev_life_end: %u\n",
		   nandswap_enable, nsi.fn_status, nsi.life_protect,
		   nsi.swap_limit, nsi.write_lifelong, nsi.quota_today,
		   nsi.swap_out_cnt, nsi.swap_in_cnt, nsi.swap_out, nsi.swap_in,
		   nsi.swap_out_pg_act, nsi.swap_out_pg_inact, events[PSWPOUT],
		   events[PSWPIN], nsi.dev_life_end);
	return 0;
}
...
static inline void ns_create_proc(struct proc_dir_entry *parent)
{
	if (parent) {
		...
		proc_create("fn_enable", S_IRUGO | S_IWUGO, parent,
			    &fn_enable_fops);
		proc_create("swap_limit", S_IWUGO, parent, &swap_limit_fops);
		proc_create("dev_life", S_IWUGO, parent, &dev_life_fops);
		proc_create("life_protect", S_IRUGO, parent,
			    &life_protect_fops);
		proc_create("stat_info", S_IRUGO, parent, &stat_info_fops);
	}
}
