...
static int get_free_mem(char **data)
{
	...
	len = snprintf(tmp, 64, "%lu %lu %ld %ld\n", freeram, bufferram, cached,
		       availableram);
	*data = tmp;
	return len;
}

struct io_info_entry free_mem_entry = { .name = "free_mem",
					.type = STR_FREE_ENTRY_TYPE,
...
static void __get_disk_info(struct super_block *sb, void *arg)
{
	...
	if (valid_patition) {
		...
		for (i = 0; i < total_segs; i++) {
			total_blocks += of2fs_seg_freefrag(
				sbi, i, disk_status.blocks,
				ARRAY_SIZE(disk_status.blocks));

			cond_resched();
		}
		disk_status.score = total_blocks ?
					    (disk_status.blocks[0] +
					     disk_status.blocks[1]) *
						    100ULL / total_blocks :
					    0;
...
static int get_iowait_info(char **data)
{
	...
	len = snprintf(tmp, 512,
		       ...
		       iowait_status.iowait_total_count,
		       iowait_status.iowait_1ms, iowait_status.iowait_10ms,
		       iowait_status.iowait_20ms, iowait_status.iowait_60ms,
		       iowait_status.iowait_100ms, iowait_status.iowait_200ms,
		       iowait_status.iowait_300ms, iowait_status.iowait_400ms,
		       iowait_status.iowait_500ms, iowait_status.iowait_600ms,
		       iowait_status.iowait_800ms, iowait_status.iowait_1000ms,
		       iowait_status.iowait_1500ms, iowait_status.iowait_2000ms,
		       iowait_status.iowait_above2000ms);
...
void reqstats_record(struct request *req, unsigned int nr_bytes)
{
	...
	if (!sync && isux) {
		...
		req_para.uxreq_block_para.max_delay =
			(req_para.uxreq_block_para.max_delay >= tg2d) ?
				req_para.uxreq_block_para.max_delay :
				tg2d;
	}
#endif
	...
	req_para.req_block_para.max_delay[sync] =
		(req_para.req_block_para.max_delay[sync] >= tg2d) ?
			req_para.req_block_para.max_delay[sync] :
			tg2d;
	...
	req_para.req_device_para[index].max_delay[rw] =
		(req_para.req_device_para[index].max_delay[rw] >= td2c) ?
			req_para.req_device_para[index].max_delay[rw] :
			td2c;
...
struct io_info_entry block_ux_io_entry = { .name = "block_ux_io",
					   .type = STR_FREE_ENTRY_TYPE,
					   .get_value =
						   get_blk_ux_io_distribution };
...
static int get_dev_4k_rw_distribution(char **data)
{
	...
	ret = snprintf(rw4k_buff, 1024,
		       "%lld %lld %lld %lld %lld %lld %lld %lld "
		       "%lld %lld %lld %lld %lld %lld %lld %lld\n",
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .cnt[IO_MONITOR_DELAY_READ],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .total_delay[IO_MONITOR_DELAY_READ],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .max_delay[IO_MONITOR_DELAY_READ],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .stage_one[IO_MONITOR_DELAY_READ],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .stage_two[IO_MONITOR_DELAY_READ],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .stage_thr[IO_MONITOR_DELAY_READ],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .stage_fou[IO_MONITOR_DELAY_READ],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .stage_fiv[IO_MONITOR_DELAY_READ],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .cnt[IO_MONITOR_DELAY_WRITE],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .total_delay[IO_MONITOR_DELAY_WRITE],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .max_delay[IO_MONITOR_DELAY_WRITE],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .stage_one[IO_MONITOR_DELAY_WRITE],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .stage_two[IO_MONITOR_DELAY_WRITE],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .stage_thr[IO_MONITOR_DELAY_WRITE],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .stage_fou[IO_MONITOR_DELAY_WRITE],
		       req_para.req_device_para[IO_MONITOR_DELAY_4K]
			       .stage_fiv[IO_MONITOR_DELAY_WRITE]);
...
struct io_info_entry ligntweight_dmesg_entry = { .name = "dmesg",
						 .type = STR_FREE_ENTRY_TYPE,
						 .get_value = get_dump_log };

static int exit_task_notifier(struct notifier_block *self, unsigned long cmd,
			      void *v)
{
...
void add_pid_to_list(struct task_struct *task, size_t bytes, bool opt)
{
	...
	for (i = 0; i < PID_HASH_LEGNTH; i++) {
		...
		if (ti->used) {
			...
			ti->pid = task->pid;
			ti->tgid = task->tgid;
			ti->uid = from_kuid_munged(current_user_ns(),
						   task_uid(task));
			ti->time = jiffies;
...
int write_abnormal_file(struct file *filp, struct io_info_entry *entry)
{
	...
	case NUM_ENTRY_TYPE:
		...
		if (-1 != ret) {
			...
			kernel_write(filp, name_buf, strlen(name_buf),
				     &file_pos);
		}
		break;
	case STR_ENTRY_TYPE:
	case STR_FREE_ENTRY_TYPE:
		...
		if (-1 != ret) {
			...
			kernel_write(filp, ENTRY_END_TAG, strlen(ENTRY_END_TAG),
				     &file_pos);
...
static void io_abnormal(struct work_struct *work)
{
	...
	snprintf(io_monitor_env[0], IOM_REASON_LENGTH, "REASON=%d",
		 io_abnormal_info.reason);
	snprintf(io_monitor_env[1], IOM_REASON_LENGTH, "PID=%d",
		 io_abnormal_info.pid);
	snprintf(io_monitor_env[2], IOM_REASON_LENGTH, "DELAY=%d",
		 io_abnormal_info.delay);
...
int abnormal_handle(enum iomointor_io_type reason, pid_t pid, u64 delta_ms)
{
	...
	queue_delayed_work(io_abnormal_queue, &io_abnormal_work,
			   msecs_to_jiffies(1));
...
void rt_record_handle(void)
{
	queue_delayed_work(rt_record_queue, &rt_record_work,
			   msecs_to_jiffies(1));
}
...
static int task_status_init(void)
{
	task_buffer = kzalloc(sizeof(struct task_io_info) * PID_HASH_LEGNTH,
			      GFP_KERNEL);
	if (!task_buffer)
		return -1;
...
static int __init io_monitor_init(void)
{
	...
	if (IoMonitor_dir) {
		...
		interval_entry =
			proc_create("interval", S_IRUGO | S_IWUGO,
				    IoMonitor_dir, &seq_abnormal_interval_fops);
		if (!interval_entry) {
		...
		size_entry =
			proc_create("log_size", S_IRUGO | S_IWUGO,
				    IoMonitor_dir, &seq_abnormal_size_fops);
		if (!size_entry) {
