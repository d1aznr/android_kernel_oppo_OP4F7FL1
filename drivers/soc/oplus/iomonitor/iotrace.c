...
#define IO_TRACE_DEVICE_ATTR(_name)                                            \
	DEVICE_ATTR(_name, S_IRUGO | S_IWUSR, sysfs_io_trace_attr_show,        \
	...
	while (pos >= io_trace_this->first && pos < io_trace_this->last) {
		...
		if ((int)entry_head->len <= 0) {
			...
			pr_err("IOMonitor: dead loop will happen, but it is circumvented. "
			       ...
			       io_trace_this->first, io_trace_this->last, pos);
			break;
		}
		...
		case TRACE_SYSCALL:
			syscall_entry = (struct io_trace_syscall_entry *)
						entry_head->val;
			len = snprintf(
				buf, sizeof(buf), "%lu %d %d %s %s %llu\n",
				syscall_entry->time, syscall_entry->action,
				syscall_entry->pid, syscall_entry->comm,
				syscall_entry->f_name, syscall_entry->delay);
...
static int iotrace_debug_seq_show(struct seq_file *seq, void *offset)
{
	seq_printf(seq,
		   ...
		   otrace_dbg_info->before_loop_last,
		   otrace_dbg_info->enter_loop_time,
		   otrace_dbg_info->in_loop_pos, otrace_dbg_info->in_loop_first,
		   otrace_dbg_info->in_loop_last, otrace_dbg_info->err_time);
	return 0;
}
...
static void iotrace_register_tracepoints(void)
{
	...
	/* syscall */
	ret = register_trace_syscall_read_timeout(io_syscall_read_timeout,
						  NULL);
	WARN_ON(ret);
	ret = register_trace_syscall_write_timeout(io_syscall_write_timeout,
						   NULL);
	WARN_ON(ret);
	ret = register_trace_syscall_sync_timeout(io_syscall_sync_timeout,
						  NULL);
	WARN_ON(ret);
	...
	ret = register_trace_scsi_dispatch_cmd_error(scsi_dispatch_cmd_error,
						     NULL);
	WARN_ON(ret);
	ret = register_trace_scsi_dispatch_cmd_timeout(
		scsi_dispatch_cmd_timeout, NULL);
	WARN_ON(ret);
...
static int __init io_trace_init(void)
{
	...
	if (otrace_dbg_info == NULL)
		otrace_dbg_info = (struct iotrace_debug_info *)kzalloc(
			sizeof(struct iotrace_debug_info), GFP_KERNEL);
	if (otrace_dbg_info == NULL)
		return -1;
