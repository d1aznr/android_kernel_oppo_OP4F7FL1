...
#define task_all_bytes(tsk) (tsk->ioac.read_bytes + compute_write_bytes(tsk))
...
static void show_io_uid_tasks(struct seq_file *m, struct uid_entry *uid_entry)
{
	...
	hash_for_each (uid_entry->task_entries, bkt_task, task_entry, hash) {
		...
		task_all_bytes = task_entry->io[UID_STATE_DELTA].read_bytes +
		...
		if (task_all_bytes >= TASK_SHOW_DAILY_LIMIT)
			seq_printf(
				...
				task_entry->is_kthread, task_entry->tgid_comm);
	}
}
...
static void remove_dead_uid_tasks(struct uid_entry *uid_entry)
{
	...
	hash_for_each_safe (uid_entry->task_entries, bkt_task, tmp_task,
			    task_entry, hash) {
...
static int uid_io_show(struct seq_file *m, void *v)
{
	...
	hash_for_each (uid_hash_table, bkt, uid_entry, hash) {
		seq_printf(m, "%d %llu %llu %llu %llu %llu %d\n",
			   uid_entry->uid, uid_entry->io[UID_STATE_DELTA].rchar,
		...
		all_bytes = uid_entry->io[UID_STATE_DELTA].read_bytes +
			    uid_entry->io[UID_STATE_DELTA].write_bytes;
...
struct proc_dir_entry *create_uid_proc(struct proc_dir_entry *parent)
{
	...
	dir_entry = proc_create("uid_io_show", S_IRUGO | S_IWUGO, parent,
