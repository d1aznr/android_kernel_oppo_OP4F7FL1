				    ...
				    unsigned int state)
{
	...
	midas_mmap_buf.cpu_entrys[cpu].time_in_state[state] +=
...
static void update_or_create_entry_locked(uid_t uid, struct task_struct *p,
					  u64 cputime, unsigned int state,
					  unsigned int type)
{
	...
	for (i = 0; i < id_cnt; i++) {
		if (midas_mmap_buf.entrys[i].type == type &&
			...
			midas_mmap_buf.entrys[i].time_in_state[state] +=
				DIV_ROUND_CLOSEST(cputime, NSEC_PER_MSEC);
			return;
		}
	}
	...
	if (type == TYPE_PROCESS) {
		...
		rcu_read_unlock();
		if (task != NULL) {
			strncpy(midas_mmap_buf.entrys[i].tgid_name, task->comm,
				TASK_COMM_LEN);
		}
	}
	/* the unit of time_in_state is ms */
			     ...
			     unsigned int state)
{
	...
	if (!check_track_uid(uid))
		update_or_create_entry_locked(uid, p, cputime, state,
					      TYPE_PROCESS);
...
static int midas_ioctl_get_time_in_state(void *kdata, void *priv_info)
{
	...
	memcpy(info->mmap_addr, &midas_mmap_buf,
	       sizeof(struct midas_mmap_data));
...
static int midas_ioctl_get_em(void *kdata, void *priv_info)
{
	...
	for_each_possible_cpu (cpu) {
		if ((em_pd != NULL) &&
		    cpumask_test_cpu(cpu, to_cpumask((em_pd)->cpus)))
			continue;
...
static const struct midas_ioctl_desc midas_ioctls[] = {
	MIDAS_IOCTL_DEF(MIDAS_IOCTL_GET_TIME_IN_STATE,
			midas_ioctl_get_time_in_state),
	MIDAS_IOCTL_DEF(MIDAS_IOCTL_SET_TRACK_UID, midas_ioctl_set_track_uid),
	MIDAS_IOCTL_DEF(MIDAS_IOCTL_REMOVE_TRACK_UID,
			midas_ioctl_remove_track_uid),
	MIDAS_IOCTL_DEF(MIDAS_IOCTL_CLEAR_TRACK_UID,
			midas_ioctl_clear_track_uid),
	MIDAS_IOCTL_DEF(MIDAS_IOCTL_GET_EM, midas_ioctl_get_em),
};

#define KDATA_SIZE 512
long midas_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
