...
static int binder_notify_fn(struct notifier_block *nb, unsigned long action,
			    void *data);
...
static inline long long hash_key(struct binder_notify *bn,
				 int enable_binder_comm)
{
	long long key = 0;
	int i;
...
static inline bool hash_match(struct binder_notify *bn,
			      struct binder_stats_item *stats_item,
			      int enable_binder_comm)
{
	...
	if (1 == enable_binder_comm)
		return (0 == strncmp(stats_item->caller_comm,
				     bn->caller_task->comm, TASK_COMM_LEN) &&
			0 == strncmp(stats_item->caller_proc_comm,
				     caller_proc_task->comm, TASK_COMM_LEN) &&
			0 == strncmp(stats_item->service_name, bn->service_name,
				     OPLUS_MAX_SERVICE_NAME_LEN) &&
			0 == strncmp(stats_item->binder_comm,
				     bn->binder_task->comm, TASK_COMM_LEN) &&
			0 == strncmp(stats_item->binder_proc_comm,
				     binder_proc_task->comm, TASK_COMM_LEN));
	else
		return (0 == strncmp(stats_item->caller_comm,
				     bn->caller_task->comm, TASK_COMM_LEN) &&
			0 == strncmp(stats_item->caller_proc_comm,
				...
				struct binder_notify *bn)
{
	...
	binder_proc_task = NULL == bn->binder_task->group_leader ?
				   bn->binder_task :
				   bn->binder_task->group_leader;
	binder_uid =
		from_kuid_munged(current_user_ns(), task_uid(bn->binder_task));

	if (!context_ptr->has_intr_srv_name &&
	    !context_ptr->has_intr_proc_comm && !context_ptr->has_intr_uid) {
		intreresting = true;
	} else {
		intreresting = false;
	}
	...
	hash_for_each_possible (
		context_ptr->intre_srv_name_hash, hash_node_srv_name, hentry,
		hash_key_for_str(bn->service_name, strlen(bn->service_name))) {
		if (0 == strcmp(hash_node_srv_name->service_name,
				bn->service_name)) {
	...
	hash_for_each_possible (
		context_ptr->intre_proc_comm_hash, hash_node_proc_comm, hentry,
		hash_key_for_str(binder_proc_task->comm,
				 strlen(binder_proc_task->comm))) {
		if (0 ==
		    strcmp(hash_node_proc_comm->comm, binder_proc_task->comm)) {
	...
	hash_for_each_possible (context_ptr->intre_uid_hash, hash_node_uid,
				hentry, (long long)binder_uid) {
...
static void store_binder_stats_to_kernel(struct binder_notify *data)
{
	...
	if (NULL == data || NULL == data->caller_task ||
	    NULL == data->binder_task)
		return;
	...
	list_for_each_entry (context_ptr, &g_binder_stats_driver.user_list_head,
			     list_node) {
		if (NULL == context_ptr ||
		...
		hash_for_each_possible (
			context_ptr->kernel_binder_stats_hash, ref_hash_node,
			hentry,
			hash_key(data, context_ptr->enable_binder_comm)) {
			if (hash_match(data, ref_hash_node->item,
				       context_ptr->enable_binder_comm)) {
		...
		if (NULL != find_item) {
			...
			if (kernel_binder_stats->valid_item_cnt + 1 <=
			    kernel_binder_stats->max_item_cnt) {
				find_item =
					&(kernel_binder_stats->items
						  [kernel_binder_stats
							   ->valid_item_cnt]);

				/* caller proc comm */
				if (NULL != caller_task->group_leader)
					strncpy(find_item->caller_proc_comm,
						caller_task->group_leader->comm,
						TASK_COMM_LEN);
				else
					strncpy(find_item->caller_proc_comm,
						caller_task->comm,
				...
				/* caller pid tgid uid */
				find_item->caller_pid =
					task_pid_nr(caller_task);
				find_item->caller_tgid =
					task_tgid_nr(caller_task);
				find_item->caller_uid =
					from_kuid_munged(current_user_ns(),
							 task_uid(caller_task));
				/* caller comm */
				strncpy(find_item->caller_comm,
					caller_task->comm, TASK_COMM_LEN);

				/* service name */
				strncpy(find_item->service_name,
					data->service_name,
					OPLUS_MAX_SERVICE_NAME_LEN);

				/* binder proc comm */
				if (NULL != binder_task->group_leader)
					strncpy(find_item->binder_proc_comm,
						binder_task->group_leader->comm,
						TASK_COMM_LEN);
				else
					strncpy(find_item->binder_proc_comm,
						binder_task->comm,
				...
				/* binder tgid uid */
				find_item->binder_tgid =
					task_tgid_nr(binder_task);
				find_item->binder_uid =
					from_kuid_munged(current_user_ns(),
							 task_uid(binder_task));

				/* binder comm pid */
				if (1 == context_ptr->enable_binder_comm) {
					find_item->binder_pid =
						task_pid_nr(binder_task);
					strncpy(find_item->binder_comm,
						binder_task->comm,
						TASK_COMM_LEN);
				} else {
					find_item->binder_pid =
				...
				ref_hash_node =
					&(context_ptr->kernel_binder_stats_refs
						  [kernel_binder_stats
							   ->valid_item_cnt]);
				ref_hash_node->item = find_item;
				hash_add(
					context_ptr->kernel_binder_stats_hash,
					&ref_hash_node->hentry,
					hash_key(data,
						 context_ptr
							 ->enable_binder_comm));
...
static void
binder_stats_clear_user_context(struct binder_stats_user_context *context_ptr)
{
	unsigned long flags;
	struct binder_stats_item_ref *ref_hash_node;
	...
	hash_for_each_safe (context_ptr->kernel_binder_stats_hash, i, tmp,
			    ref_hash_node, hentry) {
...
static int
user_enable_binder_stats(struct binder_stats_user_context *context_ptr,
			 int enable)
{
	unsigned long flags;
	struct binder_stats_item_ref *ref_hash_node;
	...
	if (1 == enable) {
		...
		binder_stats_buffer_size =
			2 * sizeof(unsigned int) +
			max_item_cnt * sizeof(struct binder_stats_item);

		context_ptr->binder_stats_buf_0 =
			vmalloc_user(binder_stats_buffer_size);
		...
		context_ptr->kernel_binder_stats =
			context_ptr->binder_stats_buf_0;

		context_ptr->kernel_binder_stats_refs = vmalloc_user(
			max_item_cnt * sizeof(struct binder_stats_item_ref));
		if (IS_ERR_OR_NULL(context_ptr->kernel_binder_stats_refs)) {
			BINDER_STATS_LOGE("malloc failed!\n");
			goto enable_fail;
		}
		...
		context_ptr->binder_stats_buf_1 =
			vmalloc_user(binder_stats_buffer_size);
		...
		spin_lock_irqsave(&g_binder_stats_driver.user_list_lock, flags);
		list_add(&context_ptr->list_node,
			 &g_binder_stats_driver.user_list_head);
		spin_unlock_irqrestore(&g_binder_stats_driver.user_list_lock,
				       flags);
	...
	enable_fail:
		...
		context_ptr->user_binder_stats = NULL;
		context_ptr->kernel_binder_stats = NULL;
		hash_for_each_safe (context_ptr->kernel_binder_stats_hash, i,
				    tmp, ref_hash_node, hentry) {
			hash_del(&ref_hash_node->hentry);
		}
...
static int
user_update_binder_stats(struct binder_stats_user_context *context_ptr)
{
	int ret = 0;
	unsigned long flags;
	...
	if (NULL != context_ptr->kernel_binder_stats &&
...
static int
user_binder_stats_cfg_clear(struct binder_stats_user_context *context_ptr)
{
	struct binder_stats_filter_srv_name_node *hash_node_srv_name;
	struct binder_stats_filter_proc_comm_node *hash_node_proc_comm;
	struct binder_stats_filter_uid_node *hash_node_uid;
	...
	if (1 == context_ptr->enable) {
		BINDER_STATS_LOGE(
			"can not set config when function enabled already!\n");
		return -1;
	}

	hash_for_each_safe (context_ptr->intre_srv_name_hash, i, tmp,
			    hash_node_srv_name, hentry) {
	...
	hash_for_each_safe (context_ptr->intre_proc_comm_hash, i, tmp,
			    hash_node_proc_comm, hentry) {
	...
	hash_for_each_safe (context_ptr->intre_uid_hash, i, tmp, hash_node_uid,
			    hentry) {
...
static int user_binder_stats_cfg_set_max_item_cnt(
	struct binder_stats_user_context *context_ptr, int max_item_cnt)
{
	if (1 == context_ptr->enable) {
		BINDER_STATS_LOGE(
			"can not set config when function enabled already!\n");
...
static int user_binder_stats_cfg_set_enable_binder_comm(
	struct binder_stats_user_context *context_ptr, int enable_binder_comm)
{
	if (1 == context_ptr->enable) {
		BINDER_STATS_LOGE(
			"can not set config when function enabled already!\n");
...
static int user_binder_stats_add_intreresting_svr_name(
	...
	struct binder_stats_filter_srv_name *filter_srv_name)
{
	...
	hash_node_srv_name =
		vmalloc_user(sizeof(struct binder_stats_filter_srv_name_node));
	if (IS_ERR_OR_NULL(hash_node_srv_name)) {
		BINDER_STATS_LOGE("malloc err\n");
		return -1;
	}
	strncpy(hash_node_srv_name->service_name, filter_srv_name->service_name,
		OPLUS_MAX_SERVICE_NAME_LEN);
	...
	hash_add(context_ptr->intre_srv_name_hash, &hash_node_srv_name->hentry,
		 hash_key_for_str(hash_node_srv_name->service_name,
				  strlen(hash_node_srv_name->service_name)));
...
static int user_binder_stats_add_intreresting_proc_comm(
	struct binder_stats_user_context *context_ptr,
	struct binder_stats_filter_proc_comm *filter_proc_comm)
{
	...
	hash_node_proc_comm =
		vmalloc_user(sizeof(struct binder_stats_filter_proc_comm_node));
	if (IS_ERR_OR_NULL(hash_node_proc_comm)) {
		BINDER_STATS_LOGE("malloc err\n");
		return -1;
	}
	strncpy(hash_node_proc_comm->comm, filter_proc_comm->comm,
		TASK_COMM_LEN);
	hash_node_proc_comm->intreresting = filter_proc_comm->intreresting;
	hash_add(context_ptr->intre_proc_comm_hash,
		 &hash_node_proc_comm->hentry,
		 hash_key_for_str(hash_node_proc_comm->comm,
				  strlen(hash_node_proc_comm->comm)));
...
static int user_binder_stats_add_intreresting_uid(
	struct binder_stats_user_context *context_ptr,
	struct binder_stats_filter_uid *filter_uid)
{
	...
	hash_node_uid =
		vmalloc_user(sizeof(struct binder_stats_filter_uid_node));
	...
	hash_add(context_ptr->intre_uid_hash, &hash_node_uid->hentry,
		 (long long)hash_node_uid->uid);
...
static int binder_stats_driver_release(struct inode *ignored, struct file *filp)
{
	...
	if (g_binder_stats_driver.regist_binder_stats_flag &&
	    ((1 == list_size &&
	      list_is_last(&context_ptr->list_node,
			   &g_binder_stats_driver.user_list_head)) ||
...
{
	struct binder_stats_user_context *context_ptr;
	if (NULL == vma || NULL == vma->vm_file ||
	    NULL == vma->vm_file->private_data) {
...
static int binder_stats_driver_mmap(struct file *filp,
				    struct vm_area_struct *vma)
{
	...
	if (NULL != context_ptr && NULL != context_ptr->user_binder_stats) {
		...
		if (remap_vmalloc_range(vma, context_ptr->user_binder_stats,
					vma->vm_pgoff)) {
			mutex_unlock(&g_binder_stats_driver.lock);
...
static long binder_stats_driver_ioctl(struct file *filp, unsigned int cmd,
				      unsigned long arg)
{
	long ret = -1;
	int enable = 0;
		...
		if (0 != arg) {
			if (0 ==
			    copy_to_user((unsigned int *)arg,
					 ...
					 sizeof(unsigned int))) {
				BINDER_STATS_LOGI(
					"BINDER_STATS_CTL_GET_VERSION %d\n",
					g_binder_stats_driver.version_code);
				ret = BINDER_STATS_CTL_RET_SUCC;
			} else {
				BINDER_STATS_LOGE(
					"BINDER_STATS_CTL_GET_VERSION failed. copy_to_user error!\n");
				ret = BINDER_STATS_CTL_RET_INVALID;
			}
		} else {
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_GET_VERSION failed. arg error!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
		}
	} break;
	case BINDER_STATS_CTL_ENABLE: {
		if (0 != copy_from_user(&enable, (int *)arg, sizeof(int))) {
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_ENABLE failed. copy_to_user error!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
			break;
		}
	...
	case BINDER_STATS_CTL_CFG_CLEAR: {
		if (0 == user_binder_stats_cfg_clear(context_ptr)) {
			...
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_CFG_CLEAR failed!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
		}
	} break;
	case BINDER_STATS_CTL_CFG_MAX_ITEM: {
		if (0 !=
		    copy_from_user(&max_item_cnt, (int *)arg, sizeof(int))) {
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_CFG_MAX_ITEM failed. copy_to_user error!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
			break;
		}
		if (0 == user_binder_stats_cfg_set_max_item_cnt(context_ptr,
								max_item_cnt)) {
			ret = BINDER_STATS_CTL_RET_SUCC;
		} else {
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_CFG_MAX_ITEM failed!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
		}
	} break;
	case BINDER_STATS_CTL_CFG_ENABLE_BINDER_COMM: {
		if (0 != copy_from_user(&enable_binder_comm, (int *)arg,
					sizeof(int))) {
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_CFG_ENABLE_BINDER_COMM failed. copy_to_user error!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
			break;
		}
		if (0 == user_binder_stats_cfg_set_enable_binder_comm(
				 context_ptr, enable_binder_comm)) {
			ret = BINDER_STATS_CTL_RET_SUCC;
		} else {
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_CFG_ENABLE_BINDER_COMM failed!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
		}
	} break;
	case BINDER_STATS_CTL_CFG_SVR_FILTER_NAME: {
		if (0 != copy_from_user(
				 &filter_srv_name, (char *)arg,
				 sizeof(struct binder_stats_filter_srv_name))) {
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_CFG_SVR_FILTER_NAME failed. copy_to_user error!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
			break;
		}
		if (0 == user_binder_stats_add_intreresting_svr_name(
				 context_ptr, &filter_srv_name)) {
			ret = BINDER_STATS_CTL_RET_SUCC;
		} else {
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_CFG_SVR_FILTER_NAME failed!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
		}
	} break;
	case BINDER_STATS_CTL_CFG_SVR_FILTER_PROC_COMM: {
		if (0 !=
		    copy_from_user(
			    &filter_proc_comm, (char *)arg,
			    sizeof(struct binder_stats_filter_proc_comm))) {
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_CFG_SVR_FILTER_PROC_COMM failed. copy_to_user error!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
			break;
		}
		if (0 == user_binder_stats_add_intreresting_proc_comm(
				 context_ptr, &filter_proc_comm)) {
			ret = BINDER_STATS_CTL_RET_SUCC;
		} else {
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_CFG_SVR_FILTER_PROC_COMM failed!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
		}
	} break;
	case BINDER_STATS_CTL_CFG_SVR_FILTER_UID: {
		if (0 !=
		    copy_from_user(&filter_uid, (int *)arg,
				   sizeof(struct binder_stats_filter_uid))) {
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_CFG_SVR_FILTER_UID failed. copy_to_user error!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
			break;
		}
		if (0 == user_binder_stats_add_intreresting_uid(context_ptr,
								&filter_uid)) {
			ret = BINDER_STATS_CTL_RET_SUCC;
		} else {
			BINDER_STATS_LOGE(
				"BINDER_STATS_CTL_CFG_SVR_FILTER_UID failed!\n");
			ret = BINDER_STATS_CTL_RET_INVALID;
		}
	} break;
	default: {
...
int __init binder_stats_dev_init(void)
{
	...
	err = alloc_chrdev_region(&g_binder_stats_driver.dev, 0, 1,
				  "binder_stats");
	if (err < 0) {
	...
	err = cdev_add(&g_binder_stats_driver.cdev, g_binder_stats_driver.dev,
		       1);
	if (err < 0) {
		BINDER_STATS_LOGE(
			"cdev_add g_binder_stats_driver.cdev failed!\n");
		goto unreg_region;
	}

	g_binder_stats_driver.dev_class =
		class_create(THIS_MODULE, "binder_stats_class");
	if (IS_ERR(g_binder_stats_driver.dev_class)) {
		BINDER_STATS_LOGE(
			"create class g_binder_stats_driver.dev_class error\n");
		goto destroy_cdev;
	}

	g_binder_stats_driver.device =
		device_create(g_binder_stats_driver.dev_class, NULL,
			      g_binder_stats_driver.dev, NULL, "binder_stats");
	if (IS_ERR(g_binder_stats_driver.device)) {
		BINDER_STATS_LOGE(
			"device_create g_binder_stats_driver.device error\n");
		goto destroy_class;
	}
...
void __exit binder_stats_dev_exit(void)
{
	if (g_binder_stats_driver.dev_class) {
		device_destroy(g_binder_stats_driver.dev_class,
			       g_binder_stats_driver.dev);
		class_destroy(g_binder_stats_driver.dev_class);
