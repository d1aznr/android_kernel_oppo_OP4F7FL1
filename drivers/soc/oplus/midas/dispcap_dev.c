...
extern bool mtk_drm_cwb_enable(int en, const struct mtk_cwb_funcs *funcs,
			       enum CWB_BUFFER_TYPE type);
extern bool mtk_drm_set_cwb_user_buf(void *user_buffer,
				     enum CWB_BUFFER_TYPE type);
...
static int driver_release(struct inode *ignored, struct file *filp)
{
	...
	if (NULL != context_ptr) {
		...
		if (NULL != context_ptr->dispcap_ion_handle) {
			ion_unmap_kernel(g_disp_driver.ion_client,
...
static long driver_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case DISPCAP_CTL_SET_CAPTURE_RECT: {
		...
		if (0 != copy_from_user(&rect, (void __user *)arg,
			...
			DISPCAP_LOGE(
				"DISPCAP_CTL_SET_CAPTURE_RECT arg error!\n");
	...
	case DISPCAP_CTL_ENABLE_CAPTURE: {
		int capture;
		if (0 !=
		    copy_from_user(&capture, (void __user *)arg, sizeof(int))) {
	...
	case DISPCAP_CTL_SET_BUFFER: {
		...
		if (0 != copy_from_user(&ion_share_fd, (void __user *)arg,
					sizeof(int))) {
	...
	case DISPCAP_CTL_WAIT_BUFFER_COMPLETE: {
		...
		int wait_ret;
		if (0 !=
		    copy_from_user(&wait_ms, (void __user *)arg, sizeof(int))) {
			DISPCAP_LOGE(
				"DISPCAP_CTL_WAIT_BUFFER_COMPLETE arg error!\n");
			ret = DISPCAP_CTL_RET_INVALID;
			break;
		}

		DISPCAP_LOGD(
			"wait_for_completion_interruptible_timeout timeout:%dms begin ...\n",
			wait_ms);
		wait_ret = wait_for_completion_interruptible_timeout(
			&g_disp_driver.dispcap_cmp, msecs_to_jiffies(wait_ms));
		...
		DISPCAP_LOGD(
			"wait_for_completion_interruptible_timeout OK wait_ret:%d\n",
			wait_ret);
		if (0 == wait_ret) {
...
int __init dispcap_dev_init(void)
{
	...
	g_disp_driver.dev_class =
		class_create(THIS_MODULE, "midas_dispcap_class");
	if (IS_ERR(g_disp_driver.dev_class)) {
		DISPCAP_LOGE("create class g_disp_driver.dev_class error\n");
		goto destroy_cdev;
	}

	g_disp_driver.device =
		device_create(g_disp_driver.dev_class, NULL, g_disp_driver.dev,
			      NULL, "midas_dispcap");
	...
	g_disp_driver.ion_client =
		ion_client_create(g_ion_device, "dispcap_ion_client");
	if (NULL == g_disp_driver.ion_client) {
		DISPCAP_LOGE(
			"ion_client_create g_disp_driver.ion_client failed\n");
		goto destroy_device;
	}
