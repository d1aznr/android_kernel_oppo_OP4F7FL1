...
static int rasm_resume(struct device *dev)
{
	...
	data->last_resume_millsec_time =
		resume_time.tv_sec * 1000 + resume_time.tv_usec / 1000;
	do_getboottime(&resume_boot_time);
	data->last_resume_boot_time = resume_boot_time.tv_sec * 1000 +
				      resume_boot_time.tv_usec / 1000;
	dev_dbg(dev, "Now it is %ld, system will resume.",
		data->last_resume_time);

	rasm_env[index++] = kasprintf(GFP_KERNEL, "NAME=rasm");
	rasm_env[index++] =
		kasprintf(GFP_KERNEL, "suspend=%ld", data->last_suspend_time);
	rasm_env[index++] =
		kasprintf(GFP_KERNEL, "resume=%ld", data->last_resume_time);
	rasm_env[index++] = kasprintf(GFP_KERNEL, "resume_boot=%ld",
				      data->last_resume_boot_time);
	rasm_env[index++] = kasprintf(GFP_KERNEL, "resume_millsec=%ld",
				      data->last_resume_millsec_time);
	rasm_env[index++] = kasprintf(GFP_KERNEL, "suspend_millsec=%ld",
				      data->last_suspend_millsec_time);
	rasm_env[index++] = NULL;
...
static int rasm_suspend(struct device *dev)
{
	...
	data->last_suspend_millsec_time =
		suspend_time.tv_sec * 1000 + suspend_time.tv_usec / 1000;
	dev_dbg(dev, "Now it is %ld, system will suspend.",
		data->last_suspend_time);
	return 0;
}
...
static int midas_dev_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (remap_vmalloc_range(vma, info->mmap_addr, vma->vm_pgoff)) {
		pr_err("remap failed\n");
		return -EAGAIN;
	}
...
static int midas_dev_open(struct inode *inode, struct file *filp)
{
	...
	struct midas_priv_info *info =
		kzalloc(sizeof(struct midas_priv_info), GFP_KERNEL);
	if (IS_ERR_OR_NULL(info))
		return -ENOMEM;
...
static int midas_pdev_probe(struct platform_device *pdev)
{
	...
	ret = alloc_chrdev_region(&dev_info->devno, 0, MIDAS_MAX_DEVS,
				  "midas_dev");
	if (ret) {
	...
	dev = device_create(dev_info->class, NULL, dev_info->devno, dev_info,
			    "midas_dev");
