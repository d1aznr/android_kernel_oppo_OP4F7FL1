...
void get_task_time_handler(void *data, struct task_struct *task, struct rq *rq,
			   int tick)
{
	int idx;
	...
	ktask_overtime[cpu].task[idx].uid =
	...
	ktask_overtime[cpu].task[idx].exec_start =
		jiffies_to_nsecs(jiffies - tick);
...
void handle_func(u64 start, u64 end)
{
	...
	while (i < CPU_NUM) {
		for (j = 0; j < MAX_ACCOUNT_NUM; j++) {
			if (ktask_overtime[i].task[j].pid >= MAX_PID ||
			    ktask_overtime[i].task[j].pid <= 0)
				continue;

			if (ktask_overtime[i].task[j].exec_start >= start &&
			    ktask_overtime[i].task[j].end <= end) {
				struct acct_tasktime *act =
					tasktimes +
					ktask_overtime[i].task[j].pid;

				act->sum_exec_runtime +=
					ktask_overtime[i].task[j].delta;
				act->uid = ktask_overtime[i].task[j].uid;
				act->sum_freq += ktask_overtime[i].task[j].freq;
				act->sum_load += ktask_overtime[i].task[j].load;
				act->count++;
				memcpy(act->comm,
				       ktask_overtime[i].task[j].comm,
				       TASK_COMM_LEN);
			}
		}
		i++;
...
static inline ssize_t data_to_user(char __user *buff, size_t count, loff_t *off,
				   char *format_str, int len)
{
	if (len > *off) {
		len -= *off;
	} else {
...
static ssize_t proc_enable_read(struct file *filp, char __user *buff,
...
static ssize_t proc_level_write(struct file *file, const char __user *buf,
				size_t count, loff_t *ppos)
{
	char buffer[PROC_NUMBUF];
	int err;
...
static ssize_t proc_level_read(struct file *flip, char __user *buff,
			       size_t count, loff_t *off)
{
	char page[16] = { 0 };
	int len = 0;
...
static void delayed_work_func(struct work_struct *work)
{
	...
	tasktimes = (struct acct_tasktime *)vzalloc(
		MAX_PID * sizeof(struct acct_tasktime));
	if (!tasktimes) {
	...
	for (idx = 0; idx < MAX_PID; idx++) {
		...
		if (div64_u64(act->sum_exec_runtime, NSEC_PER_MSEC) >=
			    div64_u64(PERIOD * MSEC_PER_SEC * UP_THRESH, 100) &&
			...
			if (sysctl_atd_enable) {
				...
				tsk->pid = idx;
				tsk->uid = act->uid;
				tsk->runtime = div64_u64(act->sum_exec_runtime,
							 NSEC_PER_MSEC);
				tsk->avg_freq =
					div64_u64(act->sum_freq, act->count);
				tsk->avg_load =
					div64_u64(act->sum_load, act->count);

				memcpy(tsk->comm, act->comm, TASK_COMM_LEN);
				tsk->date = boot_time.tv_sec * 1000 +
					    boot_time.tv_usec / 1000;
...
static int register_atd_vendor_hooks(void)
{
	...
	ret = register_trace_android_vh_account_task_time(get_task_time_handler,
							  NULL);
	if (ret != 0) {
...
static int unregister_atd_vendor_hooks(void)
{
	...
	unregister_trace_android_vh_account_task_time(get_task_time_handler,
						      NULL);
	if (ret != 0) {
