...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)
	/* reset qos */
	if (freq_qos_update_request(&cb->qos_req, FREQ_QOS_MAX_DEFAULT_VALUE) <
	    0)
...
static int cb_config_store(const char *buf, const struct kernel_param *kp)
{
	...
	if (sscanf(buf, "%d,%d,%d,%llu,%d,%lld,%d,%lld\n", &v.clus, &v.enable,
		   &v.limit_level, &v.limit_thres_ms, &v.down_speed,
		   &v.down_limit_ms, &v.up_speed, &v.up_limit_ms) != 8)
		goto out;
	...
	if (v.down_limit_ms < 0 || v.up_limit_ms < 0 || v.limit_thres_ms < 0)
		goto out;
...
static int cb_config_show(char *buf, const struct kernel_param *kp)
{
	...
	for (i = 0; i < min(NR_CLUS_MAX, cb_pol_idx); ++i) {
		...
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt,
				...
				cb_switch, cb->first_cpu, cb->ctl_inited);
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "last_ts: %llu\n",
				cb->last_ts);
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt,
				"last_freq_update_ts: %llu\n",
		...
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt,
				"last_core_boost: %d\n", last_core_boost);
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt,
				"last_core_boost_ts: %llu\n",
				last_core_boost_ts);
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "\n");
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "freq resident:");
		for (j = 0; j < cb->freq_levels; ++j)
			cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "\t%lld",
					cb->freqs_resident[j]);
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "\n");
	}
	...
	for (i = 0; i < min(NR_CLUS_MAX, cb_pol_idx); ++i) {
		...
		for (j = cb->limit_level; j < NR_FREQ; ++j) {
			...
			if (cb->freqs_resident[j] > 0) {
				...
				freqs_resident[j] =
					NSEC_TO_MSEC(cb->freqs_resident[j]);
		...
		for (j = cb->limit_level; j < NR_FREQ; ++j) {
			...
			if (freqs_resident[j]) {
				freq_avg += cb->freqs[j] * freqs_resident[j] /
					    total;
				freq_max = max_t(long long, freq_max,
						 cb->freqs[j]);
				freq_min = min_t(long long, freq_min,
						 cb->freqs[j]);
				cb->freqs_resident[j] = 0;
			}
		}
...
void cb_update(struct cpufreq_policy *pol, u64 time)
{
	...
	if (cb->acc >= cb->limit_thres) {
		...
		if (update_delta >= cb->down_limit_ns) {
			cb->cur_level = max(prev_level - cb->down_speed,
					    cb->limit_level);
			cb->last_freq_update_ts = time;
		...
		if (update_delta >= cb->up_limit_ns) {
			cb->cur_level =
				min(prev_level + cb->up_speed, cb->max_level);
			cb->last_freq_update_ts = time;
			cb->freqs_resident[prev_level] += update_delta;
		}
	}
	...
	if (debug)
		pr_info("cpu %d update: ts now %llu last %llu last_update %llu delta %llu update_d %llu cur %u acc %llu, cur_level %d min_over_target_freq %d isolated %d last_core_boost %d\n",
			cpu, NSEC_TO_MSEC(time), NSEC_TO_MSEC(cb->last_ts),
			NSEC_TO_MSEC(cb->last_freq_update_ts),
			NSEC_TO_MSEC(delta), NSEC_TO_MSEC(update_delta),
			pol->cur, NSEC_TO_MSEC(cb->acc), cb->cur_level,
			min_over_target_freq, isolated, last_core_boost);
...
static void cb_do_boundary_change_work(struct work_struct *qos_work)
{
	...
	if (debug)
		pr_info("processing work cpu %d min %u max %u target %u req max %u\n",
			cb->first_cpu, pol->min, pol->max, target,
			cb->qos_req.pnode.prio);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)
			if (!cb_qos_wq) {
				cb_qos_wq = alloc_workqueue("cb_qos_wq",
							    WQ_HIGHPRI, 0);
				if (!cb_qos_wq)
					return -EFAULT;
			}
			INIT_WORK(&cb->qos_work, cb_do_boundary_change_work);
			if (freq_qos_add_request(&pol->constraints,
						 &cb->qos_req, FREQ_QOS_MAX,
...
static int cb_main(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if (self_activate) {
			usleep_range(MSEC_TO_USEC(sleep_range_ms[0]) /* min */,
