...
static void omrg_irq_work(struct irq_work *w)
{
	...
	kthread_queue_work(&g_omrg_worker,
			   ruler->coop_work ?: &ruler->exec_work);
}
...
static int omrg_freq2state(struct omrg_rule *ruler, unsigned int *divider,
			   unsigned long freq)
{
	int i;
	int state = 0;
					      ...
					      unsigned long new_freq)
{
	...
	for (i = 0; i < ruler->slave_num; i++) {
		...
		if (slave->state >= 0 && slave->state < thermal_state) {
...
static int omrg_rule_get_master_state(const struct omrg_rule *ruler)
{
	...
	for (i = 0; i < ruler->master_num; i++) {
		...
		tmp = ruler->master_dev[i].idle ? 0 :
		      ruler->master_dev[i].divider_dispense[stt] ?
						  ruler->divider_num :
						  stt;
		if (tmp < 0)
			continue;

		if (state < 0 || (ruler->master_vote_max && tmp > state) ||
				      ...
				      unsigned long *max_freq)
{
	...
	*max_freq = ULONG_MAX;
	...
	list_for_each_entry (slave, &freq_dev->slave_list, node) {
		...
		if (!ruler->up_limit || state == ruler->divider_num ||
		    state < 0)
			rule_max = ULONG_MAX;
		else
...
static void omrg_cpufreq_apply_limits(unsigned int cpu)
{
	...
	trace_omrg_cpu_policy_adjust(cpu, MAX_OMRG_MARGIN, min_freq, max_freq);
unlock:
	rcu_read_unlock();
}
...
{
	struct omrg_rule *ruler =
		container_of(work, struct omrg_rule, exec_work);
	struct omrg_device *slave = NULL;
	unsigned int cpu;
	int i;
	unsigned long flags;
				   ...
				   unsigned long target_freq)
{
	...
	list_for_each_entry (slave, &freq_dev->slave_list, node)
		slave->state = omrg_freq2state(slave->ruler, slave->divider,
					       target_freq);
}
				...
				struct omrg_rule *ruler)
{
	...
	for (i = 0; i < ruler->master_num; i++) {
		if (ruler->master_dev[i].np == freq_dev->np) {
			...
			tmp_dev->state = omrg_freq2state(
				ruler, tmp_dev->divider, freq_dev->cur_freq);
			pr_debug("%s:match master%d %s\n", ruler->name, i,
				 freq_dev->np->full_name);
			return;
		}
	}
...
void omrg_cpufreq_register(struct cpufreq_policy *policy)
{
	...
	freq_dev = (struct omrg_freq_device *)kzalloc(sizeof(*freq_dev),
						      GFP_ATOMIC);
	if (IS_ERR_OR_NULL(freq_dev)) {
	...
	ret = freq_qos_add_request(&policy->constraints, &freq_dev->min_qos_req,
	...
	ret = freq_qos_add_request(&policy->constraints, &freq_dev->max_qos_req,
...
{
	struct omrg_freq_device *freq_dev =
		container_of(rcu, struct omrg_freq_device, rcu);
...
static bool omrg_disable_ruler(struct omrg_rule *ruler)
{
	...
	for (i = 0; i < ruler->slave_num; i++) {
		...
		if (freq_qos_update_request(&slave->freq_dev->min_qos_req,
					    slave->freq_dev->cpuinfo_min_freq /
						    KHZ) < 0)
			return false;

		if (freq_qos_update_request(&slave->freq_dev->max_qos_req,
					    slave->freq_dev->cpuinfo_max_freq /
						    KHZ) < 0)
			return false;
	}
	put_online_cpus();
...
static ssize_t store_ruler_enable(struct omrg_rule *ruler, const char *buf,
				  size_t count)
{
	int enable;
...
static ssize_t show_ruler_freq_dev_list(const struct omrg_rule *ruler,
					char *buf)
{
	int i, ret = 0;
	...
	for (i = 0; i < ruler->slave_num; i++) {
		...
		ret += scnprintf(buf + ret, PAGE_SIZE - ret, "slave%d:%s ", i,
				 dd->freq_dev->np->full_name);
	}
	...
	for (i = 0; i < ruler->master_num; i++) {
		...
		ret += scnprintf(buf + ret, PAGE_SIZE - ret, "master%d:%s ", i,
				 dd->freq_dev->np->full_name);
	}
...
static struct attribute *default_attrs[] = { &ruler_enable.attr,
					     &ruler_freq_dev_list.attr, NULL };
			    ...
			    struct device_node *np, const char *prop)
{
	...
	*omrg_dev = (struct omrg_device *)devm_kzalloc(
		dev, sizeof(**omrg_dev) * (*dev_num), GFP_KERNEL);
	...
	divider_size =
		sizeof(*(*omrg_dev)->divider) * (unsigned int)divider_num;
	...
	for (i = 0; i < *dev_num; i++) {
		...
		t_omrg_dev->divider = (unsigned int *)devm_kzalloc(
			dev, divider_size, GFP_KERNEL);
		if (IS_ERR_OR_NULL(t_omrg_dev->divider)) {
			dev_err(dev, "alloc %s%d fail\n", prop, i);
			return -ENOMEM;
		}

		if (of_parse_phandle_with_fixed_args(np, prop, divider_num, i,
						     &omrg_dev_spec)) {
			dev_err(dev, "parse %s%d err\n", prop, i);
			return -ENOENT;
		}

		t_omrg_dev->divider_dispense = (bool *)devm_kzalloc(
			dev, sizeof(bool) * divider_num, GFP_KERNEL);
...
static int omrg_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node (np, child) {
		ruler = (struct omrg_rule *)devm_kzalloc(
			&pdev->dev, sizeof(*ruler), GFP_KERNEL);
		...
		ret = omrg_device_init(&pdev->dev, &ruler->master_dev,
				       &ruler->master_num, ruler, child,
				       "oplus,omrg-master");
		if (ret)
			goto err_probe;

		ret = omrg_device_init(&pdev->dev, &ruler->slave_dev,
				       &ruler->slave_num, ruler, child,
				       "oplus,omrg-slave");
		if (ret)
			goto err_probe;
