...
static char *sched_list[OHM_TYPE_TOTAL] = {
	/* SCHED_STATS 0 -11 */
	"iowait", "sched_latency", "fsync", "emmcio", "dstate",
	"sched_default_05", "sched_default_06", "sched_default_07",
	"sched_default_08", "sched_default_09", "sched_default_10",
	"sched_default_11",
	/* OTHER_TYPE 12 - */
	"cur_cpu_load", "memory_monitor", "io_panic", "svm_monitor",
	"rlimit_monitor", "ionwait_monitor", "mem_vma_alloc_err", "blk_monitor"
};
...
/******  Sched record  ******/
static inline void
ohm_sched_stat_record_common(struct sched_stat_para *sched_stat,
...
#define OHM_CTRL_SCHEDTOTAL                                                    \
	OHM_CTRL_EMMCIO | OHM_CTRL_FSYNC | OHM_CTRL_SCHEDLATENCY |             \
		OHM_CTRL_IOWAIT | OHM_CTRL_DSTATE
...
#define LATENCY_STRING_FORMAT(BUF, MODULE, SCHED_STAT)                         \
	sprintf(BUF,                                                           \
		#MODULE                                                        \
		"_ctrl: %s\n" #MODULE "_logon: %s\n" #MODULE                   \
		...
		SCHED_STAT->trig ? "true" : "false", SCHED_STAT->delta_ms,     \
		SCHED_STAT->low_thresh_ms, SCHED_STAT->high_thresh_ms,         \
		SCHED_STAT->all.max_ms, SCHED_STAT->all.high_cnt,              \
		SCHED_STAT->all.low_cnt, SCHED_STAT->all.total_ms,             \
		SCHED_STAT->all.total_cnt, SCHED_STAT->fg.max_ms,              \
		SCHED_STAT->fg.high_cnt, SCHED_STAT->fg.low_cnt,               \
		SCHED_STAT->fg.total_ms, SCHED_STAT->fg.total_cnt,             \
		SCHED_STAT->ux.max_ms, SCHED_STAT->ux.high_cnt,                \
		SCHED_STAT->ux.low_cnt, SCHED_STAT->ux.total_ms,               \
...
#define LWR_STRING_FORMAT(BUF, LENGTH, SCHED_STAT)                                \
	do {                                                                      \
		...
		for (index = 0; index < LWR_SIZE; index++) {                      \
			LENGTH += sprintf(                                        \
				BUF + LENGTH,                                     \
				"long wait record slot %d: %u %u %llu %llu %u\n", \
				index, SCHED_STAT->last_n_lwr[index].pid,         \
...
static ssize_t blk_q2c_wait_read(struct file *filp, char __user *buff,
				 size_t count, loff_t *off)
{
...
static ssize_t blk_q2i_wait_read(struct file *filp, char __user *buff,
				 size_t count, loff_t *off)
{
	char page[BUFFER_SIZE_L];
	int len = 0;
...
static ssize_t blk_i2d_wait_read(struct file *filp, char __user *buff,
				 size_t count, loff_t *off)
{
	char page[BUFFER_SIZE_L];
	int len = 0;
...
static ssize_t blk_d2c_wait_read(struct file *filp, char __user *buff,
				 size_t count, loff_t *off)
{
	char page[BUFFER_SIZE_L];
	int len = 0;
			     ...
			     loff_t *off)
{
	...
	len = sprintf(page,
		      ...
		      "logon: 0x%08x\n"
		      "trig: 0x%08x\n",
		      (ohm_action_ctrl ? "true" : "false"), ohm_ctrl_list,
		      ohm_logon_list, ohm_trig_list);
			      ...
			      size_t len, loff_t *ppos)
{
	...
	ohm_debug(
		...
		write_data, ctrl_list, ohm_ctrl_list, ohm_logon_list,
		ohm_trig_list);
	return len;
}
...
#endif
#if ((LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)) &&                        \
     IS_BUILTIN(CONFIG_OPLUS_FEATURE_ZRAM_OPT)) ||                             \
	((LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0)) &&                     \
	 defined(CONFIG_HYBRIDSWAP_SWAPD))
...
static int __init healthinfo_init(void)
{
	...
	pentry = proc_create("ion_wait", S_IRUGO, healthinfo,
			     &proc_ion_wait_fops);
	if (!pentry) {
...
#ifdef CONFIG_OPLUS_BLK_MONITOR
	pentry = proc_create("blk_q2c_wait", S_IRUGO, healthinfo,
			     &proc_blk_q2c_wait_fops);
	if (!pentry) {
		ohm_err("create blk_q2c_wait proc failed.\n");
		goto ERROR_INIT_VERSION;
	}

	pentry = proc_create("blk_q2i_wait", S_IRUGO, healthinfo,
			     &proc_blk_q2i_wait_fops);
	if (!pentry) {
		ohm_err("create blk_q2i_wait proc failed.\n");
		goto ERROR_INIT_VERSION;
	}

	pentry = proc_create("blk_i2d_wait", S_IRUGO, healthinfo,
			     &proc_blk_i2d_wait_fops);
	if (!pentry) {
		ohm_err("create blk_i2d_wait proc failed.\n");
		goto ERROR_INIT_VERSION;
	}

	pentry = proc_create("blk_d2c_wait", S_IRUGO, healthinfo,
			     &proc_blk_d2c_wait_fops);
	if (!pentry) {
	...
	pentry = proc_create("cpu_info", S_IRUGO, healthinfo,
			     &proc_cpu_info_fops);
	if (!pentry) {
	...
	pentry = proc_create("sched_latency_thresh", S_IRUGO | S_IWUGO,
			     sched_thresh, &proc_sched_latency_thresh_fops);
	if (!pentry) {
