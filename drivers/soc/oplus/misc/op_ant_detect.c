...
int modify_rf_v3_info(int status)
{
	...
	if (IS_ERR_OR_NULL(g_project))
		...
		pr_err("%s: rf_cable: %d\n", __func__,
		       g_project->nDataBCDT.Feature[rf_v3_status_index]);
	}
	return 0;
}
...
int modify_rf_v2_info(int status)
{
	...
	if (IS_ERR_OR_NULL(g_project)) {
		...
		pr_err("%s: rf_cable: %d\n", __func__,
		       g_project->nDataBCDT.Feature[rf_v2_status_index]);
	}
	return 0;
}
...
static void rf_cable_work(struct work_struct *work)
{
	...
	rf_cable_data->rf_state =
		gpio_get_value(rf_cable_data->cable_gpio[0]) ||
		gpio_get_value(rf_cable_data->cable_gpio[1]);
