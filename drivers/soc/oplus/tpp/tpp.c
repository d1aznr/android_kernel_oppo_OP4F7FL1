...
static const char *tpp_strategy_str[] = { "orig", "worker_thread_middle_core" };
...
{
	return tpp_worker_thread(p) ? TPP_UNITY_WORKER_THREAD_ID :
				      TPP_OTHER_THREAD_ID;
}
...
static void __tpp_task_record(int tpp_task_id, int cpu_orig, int cpu)
{
	...
	tpp_task_monitor.buf->data[index][offset + TPP_TASK_TAG_CPU_ORIG] =
		cpu_orig;
...
static int tpp_task_record_on_set(const char *buf,
				  const struct kernel_param *kp)
{
	...
	if (tpp_task_record_on) {
		if (!tpp_task_monitor.buf)
			tpp_task_monitor.buf =
				(struct tpp_task_sample *)vzalloc(
					sizeof(struct tpp_task_sample));
		if (!tpp_cpu_select_monitor.buf)
			tpp_cpu_select_monitor.buf =
				(struct tpp_cpu_select_sample *)vzalloc(
					sizeof(struct tpp_cpu_select_sample));
	} else if (!tpp_task_record_on && tpp_task_monitor.buf &&
		   tpp_cpu_select_monitor.buf) {
...
static inline int tpp_task_record_on_show(char *buf,
					  const struct kernel_param *kp)
{
...
static inline int wk_thrd_ops_show(char *buf, const struct kernel_param *kp)
{
	...
	for (i = 0; i < TPP_NR_CPUS; ++i) {
		l += snprintf(buf + l, PAGE_SIZE - l, "%d ",
			      get_tpp_thread_cnt(TPP_UNITY_WORKER_THREAD_ID,
						 i));
	}
	return l;
}
...
static int tpp_cpu_select_index_set(const char *buf,
				    const struct kernel_param *kp)
{
	int val;
	size_t len = sscanf(buf, "%d\n", &val);

	if (tpp_cpu_select_monitor.buf && len > 0 &&
	    val < TPP_CPU_SELECT_MAX_REPORT_SIZE && val >= -1) {
...
static int tpp_task_index_show(char *buf, const struct kernel_param *kp)
{
	...
	for (i = 0; i < TPP_TASK_MONITOR_SIZE; i++) {
		len += snprintf(buf + len, PAGE_SIZE, "%d ",
				atomic_read(&tpp_task_monitor.index[i]));
	}
...
static inline void print_tpp_task_tag(struct seq_file *m)
{
	...
	for (i = 0; i < TPP_TASK_MONITOR_SIZE; ++i) {
		for (j = 0; j < TPP_TASK_COLUMN; ++j) {
			seq_printf(m, "%s_%s,", tpp_task_monitor_case[i],
				   tpp_task_tags[j]);
		}
	}
	seq_printf(m, "\n");
...
static inline void print_tpp_task_data(struct seq_file *m)
{
	...
	for (i = 0; i < TPP_TASK_MONITOR_SIZE; ++i) {
		...
		report_size[i] =
			min(report_size[i], (unsigned int)TPP_TASK_REPORT_SIZE);
	}

	for (i = 0; i < TPP_TASK_REPORT_SIZE; ++i) {
		for (j = 0; j < TPP_TASK_MONITOR_SIZE; j++) {
			for (k = 0; k < TPP_TASK_COLUMN && i < report_size[j];
			     k++)
				seq_printf(
					m, "%d,",
					tpp_task_monitor.buf
						->data[i][TPP_TASK_COLUMN * j +
							  k]);
			for (k = 0; k < TPP_TASK_COLUMN && i >= report_size[j];
			     k++)
				seq_printf(m, ",");
...
static inline void print_tpp_cpu_select_data(struct seq_file *m)
{
	...
	report_size =
		min(report_size, (unsigned int)TPP_CPU_SELECT_MAX_REPORT_SIZE);

	for (i = 0; i < report_size; ++i) {
		for (j = 0; j < TPP_CPU_SELECT_MONITOR_SIZE; ++j)
			seq_printf(m, "%d,",
				   tpp_cpu_select_monitor.buf->data[i][j]);
		seq_printf(m, "\n");
	}
}
...
static int tpp_tagged_list_show(struct seq_file *m, void *v)
{
	...
	for_each_process_thread (p, t) {
		if (im_tpp(t)) {
			seq_printf(
				m,
				"pid: %u, tgid: %u im_flag: %u tpp_flag: %u\n",
...
static inline void tpp_cpu_select_record(int tpp_task_id, int cpu)
{
	...
	if (index < TPP_CPU_SELECT_MAX_REPORT_SIZE) {
		tpp_cpu_select_monitor.buf
			->data[index][TPP_CPU_SELECT_TPP_TASK_ID] = tpp_task_id;
		tpp_cpu_select_monitor.buf->data[index][TPP_CPU_SELECT_CPU] =
			cpu;
	}
}
...
static inline int worker_thread_middle_core(struct task_struct *p, int cpu_orig)
{
	...
	if (!tpp_worker_thread(p))
		return cpu_orig;
	for (cpu = start_cpu; cpu <= tpp_cpu_clusters[TPP_BIG_CLUSTER].last;
	     cpu++) {
		if (cpu_available(cpu)) {
			int cnt_on_rq = get_tpp_thread_cnt(
				TPP_UNITY_WORKER_THREAD_ID, cpu);
...
module_param_cb(tpp_task_monitor_index, &tpp_task_monitor_index_ops, NULL,
		0664);
...
static unsigned int
save_cpu_cluster_and_ret_last_cpu(struct cpufreq_policy *policy)
{
