...
static struct rt_waker_info_t *waker_mempool_create(void)
{
	...
	waker_mempool = kzalloc(sizeof(struct rt_waker_info_t) * MAX_TID_COUNT,
				GFP_ATOMIC);
...
static void waker_mempool_init(struct rt_waker_info_t *waker_mempool)
{
	...
	if (likely(waker_mempool)) {
		...
		for (i = 0; i < MAX_TID_COUNT; i++)
			list_add_tail(&waker_mempool[i].node,
				      g_mempool_list_ptr);
	}
}
	...
	if (write_trylock_irqsave(&rt_info_rwlock, flags)) {
		for (i = 0; i < rt_num; i++) {
			if (task->tgid == render_thread_info[i].rt_tid) {
				list_for_each_safe (
					pos, n,
					&render_thread_info[i].waker_list) {
					waker = list_entry(
						pos, struct rt_waker_info_t,
						node);
...
static int rt_info_show(struct seq_file *m, void *v)
{
	...
	if (!page)
		return -ENOMEM;
	results1 = kmalloc(sizeof(struct rt_waker_info_t) * MAX_TID_COUNT,
			   GFP_KERNEL);
	if (!results1) {
		kfree(page);
		return -ENOMEM;
	...
	if (num1 > 0) {
		sort(results1, num1, sizeof(struct rt_waker_info_t),
		     &cmp_task_wake_inc, NULL);
		num1 = min(num1, MAX_TASK_NR);
		for (i = 0; i < num1; i++) {
			if (get_task_name(results1[i].waker_tid, waker_name)) {
				len += snprintf(page + len, RT_PAGE_SIZE - len,
						"%d;%s;%d;%d;%d\n",
						results1[i].waker_tid,
						waker_name, wakee1_tid,
						results1[i].total,
						results1[i].increment);
			}
		}
	}
	if (num2 > 0) {
		sort(results2, num2, sizeof(struct rt_waker_info_t),
		     &cmp_task_wake_inc, NULL);
		...
		for (i = 0; i < num2; i++) {
			if (get_task_name(results2[i].waker_tid, waker_name)) {
				len += snprintf(page + len, RT_PAGE_SIZE - len,
						"%d;%s;%d;%d;%d\n",
						results2[i].waker_tid,
						waker_name, wakee2_tid,
						results2[i].total,
						results2[i].increment);
			}
		}
	}
	if (len > 0)
...
int rt_info_init()
{
	...
	proc_create_data("render_thread_info", 0664, game_opt_dir,
			 &rt_info_proc_ops, NULL);
