...
/* called with icpu->lock held */
static void inline reset_cur_state_after_read(struct time_in_state *icpu,
					      u64 now)
{
...
static void get_cpu_load(int cpu, int *util_pct, int *busy_pct)
{
	...
	if (delta_time <= delta_idle)
		...
		*busy_pct =
			div64_u64(100 * (delta_time - delta_idle), delta_time);
...
static int time_in_state_init(void)
{
	...
	for_each_present_cpu (cpu) {
		...
		icpu->time_byte_size =
			icpu->max_idle_state * count * sizeof(u64);
	...
	cpufreq_register_notifier(&cpufreq_transition_notifier,
...
int cpu_load_init()
{
	...
	proc_create_data("cpu_load", 0444, game_opt_dir, &cpu_load_proc_ops,
			 NULL);
