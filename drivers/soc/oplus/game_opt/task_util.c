...
static struct task_runtime_info *rinfo_mempool_create(void)
{
	...
	rinfo_mempool = kzalloc(
		sizeof(struct task_runtime_info) * MAX_TID_COUNT, GFP_ATOMIC);
...
static void rinfo_mempool_init(struct task_runtime_info *rinfo_mempool)
{
	...
	if (likely(rinfo_mempool)) {
		...
		for (i = 0; i < MAX_TID_COUNT; i++)
			list_add_tail(&rinfo_mempool[i].node,
				      g_mempool_list_ptr);
	}
}
...
static int heavy_task_info_show(struct seq_file *m, void *v)
{
	...
	results = kmalloc(sizeof(struct task_util_info) * MAX_TID_COUNT,
			  GFP_KERNEL);
	if (!results) {
		ret = -ENOMEM;
		goto munlock;
	...
	list_for_each_safe (pos, n, &running_task_list) {
		...
		if (now - rinfo->last_update_ts < MAX_TASK_INACTIVE_TIME) {
			...
			results[num].util = cal_util(rinfo->sum_exec_scale,
						     now - window_start);
...
static inline u64 scale_exec_time(u64 delta, struct rq *rq)
{
	...
	if (unlikely(cur_freq <= 0) || unlikely(max_freq <= 0) ||
	    unlikely(cur_freq > max_freq))
		return delta;

	task_exec_scale = DIV64_U64_ROUNDUP(
		cur_freq * arch_scale_cpu_capacity(NULL, cpu), max_freq);
...
int task_util_init(void)
{
	if (unlikely(!game_opt_dir))
		return -ENOTDIR;
	proc_create_data("game_pid", 0664, game_opt_dir, &game_pid_proc_ops,
			 NULL);
	proc_create_data("heavy_task_info", 0444, game_opt_dir,
