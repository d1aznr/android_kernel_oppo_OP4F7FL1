...
static void g_dump_waker_stack(unsigned int waker_nr_entries,
			       unsigned long *waker_entries)
{
	int i;
	char buf[128];
...
static void g_dump_wakee_stack(unsigned int wakee_nr_entries,
			       unsigned long *wakee_entries)
{
	int i;
	char buf[128];
...
static void g_dstate_dump_stack(struct task_struct *task, u64 delay_ms)
{
	...
	snprintf(
		buf, sizeof(buf),
		"delay_ms=%d, waker_nr_entries=%d, wakee_nr_entries=%d,"
		" waker:%s tid:%d pid:%d, wakee:%s tid:%d pid:%d, blocked_func=%pS\n",
		delay_ms, waker_nr_entries, wakee_nr_entries, current->comm,
		current->pid, current->tgid, task->comm, task->pid, task->tgid,
		caller);
	trace_printk(buf);
...
void g_sched_stat_blocked(struct task_struct *task, u64 delay_ns)
{
	...
	if (atomic_read(&dump_enable) && !task->in_iowait &&
...
static ssize_t dump_enable_proc_write(struct file *file, const char __user *buf,
					 ...
					 size_t count, loff_t *ppos)
{
	...
	raw_spin_lock(&lock);
	for (i = 0; i < thread_num; i++)
		len += snprintf(page + len, sizeof(page) - len, "%d ",
				interested_threads[i]);
	if (thread_num > 0)
...
int dstate_dump_init(void)
{
	...
	proc_create_data("dump_enable", 0664, dstate_dir, &dump_enable_proc_ops,
			 NULL);
	proc_create_data("duration", 0664, dstate_dir, &duration_proc_ops,
			 NULL);
	proc_create_data("interested_tids", 0664, dstate_dir,
