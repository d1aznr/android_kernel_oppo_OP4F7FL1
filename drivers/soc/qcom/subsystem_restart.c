...
void __subsystem_send_uevent(struct device *dev, char *reason)
{
	...
	if (reason) {
		snprintf(modem_reason, sizeof(modem_reason), "MODEM_REASON=%s",
			 reason);
	} else {
		snprintf(modem_reason, sizeof(modem_reason),
			 "MODEM_REASON=unkown");
	}
	modem_reason[299] = 0;
				  ...
				  const char *name)
{
	...
	if (reason) {
		snprintf(fw_Reason, sizeof(fw_Reason), "WLAN_MONITER_REASON=%s",
			 reason);
	} else {
		snprintf(fw_Reason, sizeof(fw_Reason),
			 "WLAN_MONITER_REASON=unkown");
	}
