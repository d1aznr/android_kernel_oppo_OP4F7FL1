...
int rpmh_modem_sleepinfo_buffer_clear(void)
{
	...
	snprintf(modeminfo_stats.buf_base, modeminfo_stats.buf_size,
		 "restart state\n");
					  ...
					  size_t count, loff_t *ppos)
{
	...
	if (count == sizeof(restart_string)) {
		...
		cmd_real_len = sizeof(command_string_user) < count ?
				       sizeof(command_string_user) :
				       count;
		if (!copy_from_user(command_string_user, buffer,
				    cmd_real_len)) {
			if (!memcmp(restart_string, command_string_user,
				    cmd_real_len - 1)) {
	...
	cp_count =
		count < modeminfo_stats.buf_size - modeminfo_stats.wr_offset ?
			count :
			modeminfo_stats.buf_size - modeminfo_stats.wr_offset;
	if (cp_count < 0)
		cp_count = 0;

	ret_count = copy_from_user(
		&modeminfo_stats.buf_base[modeminfo_stats.wr_offset], buffer,
		cp_count);
	if (ret_count >= 0) {
...
static int __init rpmh_modem_sleepinfo_init(void)
{
	...
	modeminfo_stats.buf_base =
		(char *)kzalloc(MODEM_SLEEPINFO_BUFFER_SIZE, GFP_KERNEL);
	if (!modeminfo_stats.buf_base) {
		pr_err("%s: malloc buffer failed\n", __func__);
		goto modeminfo_stats_buf_base_error;
	} else {
		modeminfo_stats.buf_size = MODEM_SLEEPINFO_BUFFER_SIZE -
					   MODEM_SLEEPINFO_BUFFER_RESERVED;
		snprintf(modeminfo_stats.buf_base, modeminfo_stats.buf_size,
			 "init_state\n");
		modeminfo_stats.wr_offset = 0;
	...
	modem_sleepinfo_node =
		proc_create_data("sleepinfo", proc_modemsleep_perms,
				 basic_procdir, &modem_sleepinfo_proc_fops,
				 NULL);
