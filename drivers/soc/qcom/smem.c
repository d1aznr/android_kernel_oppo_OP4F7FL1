...
void save_dump_reason_to_smem(char *reason, char *function_name)
{
	...
	if (IS_ERR_OR_NULL(dp_info))
		...
		if (reason != NULL) {
			...
			reason_len = (reason_len < DUMP_REASON_SIZE) ?
					     reason_len :
					     DUMP_REASON_SIZE;
			if (((strlen(dp_info->dump_reason) + reason_len) <
			     DUMP_REASON_SIZE)) {
		...
		if (function_name != NULL) {
			...
			name_len = (name_len < DUMP_REASON_SIZE) ?
					   name_len :
					   DUMP_REASON_SIZE;
			if (((strlen(dp_info->dump_reason) + name_len + 1) <
			     DUMP_REASON_SIZE)) {
	...
	pr_err("\r%s: dump_reason : %s reason_len=%d function caused panic :%s name_len=%d \n",
	       __func__, (reason != NULL) ? reason : "unknown", reason_len,
	       (function_name != NULL) ? function_name : "unknown", name_len);
