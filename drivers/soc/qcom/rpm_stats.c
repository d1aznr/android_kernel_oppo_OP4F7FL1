...
#ifdef OPLUS_FEATURE_POWERINFO_RPMH
static inline int
oplus_rpmstats_append_data_to_buf(char *buf, struct msm_rpm_stats_data *data,
				  int buflength, int i)
{
	u64 actual_last_sleep;
	...
	actual_last_sleep = get_time_in_msec(data->accumulated);
	if (i == 0) {
		return snprintf(buf, buflength, "vlow:%x:%llx\n", data->count,
				actual_last_sleep);
	} else {
		return snprintf(buf, buflength, "vmin:%x:%llx\r\n", data->count,
				actual_last_sleep);
	}
}
#endif /* OPLUS_FEATURE_POWERINFO_RPMH */
...
#ifdef OPLUS_FEATURE_POWERINFO_RPMH
static inline int
oplus_rpmstats_copy_stats(struct msm_rpmstats_private_data *prvdata)
{
	void __iomem *reg;
	struct msm_rpm_stats_data data;
	int i, length;
	...
	for (i = 0, length = 0; i < prvdata->num_records; i++) {
		data.count = msm_rpmstats_read_long_register(
			reg, i, offsetof(struct msm_rpm_stats_data, count));
		data.accumulated = msm_rpmstats_read_quad_register(
			reg, i,
			offsetof(struct msm_rpm_stats_data, accumulated));

		length += oplus_rpmstats_append_data_to_buf(
			prvdata->buf + length, &data,
			sizeof(prvdata->buf) - length, i);
		prvdata->read_idx++;
	}
...
int get_rpmh_deep_sleep_info(u64 *aosd, u64 *cxsd)
{
	...
	accumulated_aosd = msm_rpmstats_read_quad_register(
		rpm_phys_addr, 0,
		offsetof(struct msm_rpm_stats_data, //0:aosd
			 accumulated));

	accumulated_cxsd = msm_rpmstats_read_quad_register(
		rpm_phys_addr, 1,
		offsetof(struct msm_rpm_stats_data, //1:cxsd
			 accumulated));

	*aosd = get_time_in_msec(accumulated_aosd);
	*cxsd = get_time_in_msec(accumulated_cxsd);
	pr_info("%s %d : aosd=%llumS, cxsd=%llumS\n", __func__, __LINE__, *aosd,
		*cxsd);
	return 0;
}
...
#ifdef OPLUS_FEATURE_POWERINFO_RPMH
	rpm_phys_addr =
		ioremap_nocache(pdata->phys_addr_base, pdata->phys_size);
	if (!rpm_phys_addr) {
