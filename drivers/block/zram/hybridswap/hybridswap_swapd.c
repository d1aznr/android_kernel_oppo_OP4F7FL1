...
#define PGDAT_ITEM_DATA(pgdat)                                                 \
	((struct hybridswapd_task *)(pgdat)->android_oem_data1)
...
atomic64_t nothing_ignore_skip_interval =
	ATOMIC_LONG_INIT(NOTHING_IGNORE_VALUE);
atomic64_t max_skip_interval = ATOMIC_LONG_INIT(MAX_SKIP_GAP);
atomic64_t nothing_ignore_check_level =
...
static unsigned long swapd_shrink_limit_per_window =
	SWAPD_SHRINK_SIZE_PER_WINDOW;
...
static ssize_t usable_mem_params_write(struct kernfs_open_file *of, char *buf,
				       size_t nbytes, loff_t off)
{
	...
	if (sscanf(buf, "%u %u %u %llu", &usable_mem_value,
		   &min_mem_watermark_value, &high_mem_watermark_value,
...
static int pagefault_refresh_min_write(struct cgroup_subsys_state *css,
				       struct cftype *cft, s64 val)
{
	if (val < 0)
		return -EINVAL;
...
static s64 pagefault_refresh_min_read(struct cgroup_subsys_state *css,
				      struct cftype *cft)
{
	return atomic64_read(&pagefault_refresh_min);
}
	...
	while ((memcg = fetch_next_memcg(memcg))) {
		...
		for (i = 0; i < level_num; ++i) {
			if (atomic64_read(&hybs->app_grade) >=
				    zswap_param[i].min_grade &&
			    atomic64_read(&hybs->app_grade) <=
				    zswap_param[i].max_grade)
				break;
		}
		atomic_set(&hybs->mem2zram_scale,
			   zswap_param[i].mem2zram_scale);
		atomic_set(&hybs->zram2ufs_scale,
			   zswap_param[i].zram2ufs_scale);
		atomic_set(&hybs->pagefault_level,
			   zswap_param[i].pagefault_level);
	}
}
...
static int memcg_active_app_info_list_show(struct seq_file *m, void *v)
{
	...
	while ((memcg = fetch_next_memcg(memcg))) {
		...
		seq_printf(m, "%s %llu %lu %lu %lu %llu\n",
			   MEMCGRP_ITEM(memcg, name), grade, anon_size,
			   zram_size, eswap_size,
...
unsigned int system_cur_usable_mem(void)
{
	...
	buffers =
		global_zone_page_state(NR_FREE_PAGES) - all_totalreserve_pages;
...
bool fetch_memcg_pagefault_status(struct mem_cgroup *memcg, pg_data_t *pgdat)
{
...
static bool fetch_infos_pagefault_status(void)
{
	...
	scale = (cur_anon_pagefault - infos_last_anon_pagefault) *
		percent_constant /
		(jiffies_to_msecs(cur_time - last_refresh_t) + 1);
	...
	hybp(HYB_INFO,
	     "current %llu t %llu last %llu t %llu scale %llu pagefault_scale %llu\n",
	     cur_anon_pagefault, cur_time, infos_last_anon_pagefault,
	     last_refresh_t, scale, infos_pagefault_level);
false_out:
	return false;
}
...
static u64 max_reclaimin_size_mb_read(struct cgroup_subsys_state *css,
				      struct cftype *cft)
{
				...
				struct pglist_data *pgdat)
{
	...
	if (unlikely(!hyb_task)) {
		hybp(HYB_ERR,
		     "set task %s cpumask %s node %d failed, "
		     "hyb_task is NULL\n",
	...
	if (!cpumask_subset(&temp_mask, cpu_present_mask)) {
		hybp(HYB_ERR,
		     "%s is not subset of cpu_present_mask, use default\n",
		     buf);
		goto use_default;
	}
...
{
	unsigned long curr_refresh = jiffies_to_msecs(jiffies - last_refresh_t);

	if (curr_refresh >= fetch_pagefault_refresh_min_value()) {
...
bool zram_watermark_ok(void)
{
	...
	diff_buffers =
		fetch_high_mem_watermark_value() - system_cur_usable_mem();
	...
	wm = min(fetch_zram_wm_scale_value(),
		 fetch_zram_wm_scale_value() - diff_buffers);
...
static inline u64 calc_shrink_scale(pg_data_t *pgdat)
{
	...
	while ((memcg = fetch_next_memcg(memcg))) {
		...
		if (can_reclaimed <= (nr_zram + nr_eswap))
			...
			hybs->can_reclaimed =
				can_reclaimed - (nr_zram + nr_eswap);
		hybp(HYB_INFO,
		     "memcg %s can_reclaimed %lu nr_anon %lu zram %lu eswap %lu total %lu scale %lu thresh %lu\n",
		     hybs->name, page_to_kb(hybs->can_reclaimed),
		     page_to_kb(nr_anon), page_to_kb(nr_zram),
		     page_to_kb(nr_eswap), page_to_kb(total),
		     (nr_zram + nr_eswap) * 100 / (total + 1), thresh);
				       ...
				       unsigned long nr_to_reclaim)
{
	...
	while (reclaim_cycles) {
		while ((memcg = fetch_next_memcg(memcg))) {
			...
			memcg_to_reclaim = reclaim_size_per_cycle *
					   hybs->can_reclaimed /
					   total_can_reclaimed;
			memcg_nr_reclaimed = try_to_free_mem_cgroup_pages(
				memcg, memcg_to_reclaim, GFP_KERNEL, true);
			reclaim_memcg_cnt++;
			hybs->can_reclaimed -= memcg_nr_reclaimed;
			hybp(HYB_INFO, "memcg %s reclaim %lu want %lu\n",
			     hybs->name, memcg_nr_reclaimed, memcg_to_reclaim);
			...
			if (swapd_nap_jiffies &&
			    time_after_eq(jiffies,
					  start_js + swapd_nap_jiffies)) {
...
out:
	hybp(HYB_INFO,
	     "total_reclaim %lu nr_to_reclaim %lu from memcg %lu total_can_reclaimed %lu\n",
...
static void swapd_shrink_node(pg_data_t *pgdat)
{
	...
	if ((jiffies - swapd_last_window_start) < swapd_shrink_window) {
		if (swapd_last_window_shrink >= swapd_shrink_limit_per_window) {
			...
			hybp(HYB_INFO,
			     "swapd_last_window_shrink %lu, skip shrink\n",
			     swapd_last_window_shrink);
			set_current_state(TASK_INTERRUPTIBLE);
			schedule_timeout(
				msecs_to_jiffies(reclaim_exceed_sleep_ms));
			return;
		}
	} else {
	...
	if (nr_reclaimed < fetch_nothing_ignore_check_level_value()) {
		...
		if (last_round_is_empty)
			swapd_skip_interval =
				min(swapd_skip_interval * increase_rate,
	...
	hybp(HYB_INFO,
	     "total_reclaimed %lu KB, avail buffer %lu %lu MB, swapd_skip_interval %llu\n",
...
#endif
				hybp(HYB_DEBUG,
				     "SWAPD_SWAPOUT curr %u avail %lu size %lu maybe swapout %lu\n",
				     curr_buffers, avail, size / SZ_1M,
				     swapout_size / SZ_1M);
			} else {
				hybp(HYB_INFO,
				     "SWAPD_SKIP_SWAPOUT curr %u avail %lu\n",
				     curr_buffers, avail);
...
void alloc_pages_slowpath_hook(void *data, gfp_t gfp_flags, unsigned int order,
			       unsigned long delta)
{
...
void rmqueue_hook(void *data, struct zone *preferred_zone, struct zone *zone,
...
static int create_swapd_thread(struct zram *zram)
{
	...
	for_each_node (nid) {
		...
		if (!PGDAT_ITEM_DATA(pgdat)) {
			...
			if (!tsk_info) {
				hybp(HYB_ERR,
				     "kmalloc tsk_info failed node %d\n", nid);
				goto error_out;
			}
	...
	ret = cpuhp_setup_state_nocalls(CPUHP_AP_ONLINE_DYN, "mm/swapd:online",
					swapd_cpu_online, NULL);
	if (ret < 0) {
