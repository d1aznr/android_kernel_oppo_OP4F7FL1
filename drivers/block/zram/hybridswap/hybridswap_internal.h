...
enum { HYB_ERR = 0, HYB_WARN, HYB_INFO, HYB_DEBUG, HYB_MAX };
...
#define pt(l, f, ...)                                                          \
	pr_err("[%s]<%d:%s>:" f, #l, __LINE__, __func__, ##__VA_ARGS__)
...
#define hybp(l, f, ...)                                                        \
	do {                                                                   \
		(l <= hybridswap_loglevel()) ? pt(l, f, ##__VA_ARGS__) :       \
					       pr_none();                      \
		if (DUMP_STACK_ON_ERR && l == HYB_ERR)                         \
			dump_stack();                                          \
	} while (0)
...
struct hybridswap_io {
	...
	enum hybridswap_class class;
	void (*done_callback)(struct hybridswap_entry *, int,
			      struct hybridswap_io_req *);
...
static inline void __count_swapd_events(enum hybridswap_event_item item,
					long delta)
{
	raw_cpu_add(swapd_event_states.event[item], delta);
}

static inline void count_swapd_events(enum hybridswap_event_item item,
				      long delta)
{
...
unsigned long hybridswap_eswap_create(struct mem_cgroup *memcg, int *eswapid,
...
extern int hybridswap_batches(struct mem_cgroup *mcg, unsigned long size,
			      bool preload);
...
extern int mem_cgroup_stored_wm_scale_write(struct cgroup_subsys_state *css,
					    struct cftype *cft, s64 val);
extern s64 mem_cgroup_stored_wm_scale_read(struct cgroup_subsys_state *css,
					   struct cftype *cft);
...
static inline bool hybridswap_core_enabled(void)
{
	return false;
}
static inline bool hybridswap_out_to_eswap_enable(void)
{
	return false;
}
#endif
...
#else
static inline bool hybridswap_swapd_enabled(void)
{
	return false;
}
#endif
