			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "%-32s %12lu\n",
			"page_fault_pause",
			atomic_long_read(&page_fault_pause));
	len += snprintf(buf + len, PAGE_SIZE - len, "%-32s %12lu\n",
			"page_fault_pause_cnt",
			atomic_long_read(&page_fault_pause_cnt));
#endif
...
memcg_hybs_t *hybridswap_cache_alloc(struct mem_cgroup *memcg, bool atomic)
{
	...
	spin_lock(&cached_idr_lock);
	hybs->cache.id =
		idr_alloc(&cached_idr, NULL, 1, MEM_CGROUP_ID_MAX, GFP_KERNEL);
	if (hybs->cache.id < 0) {
		spin_unlock(&cached_idr_lock);
		kmem_cache_free(hybridswap_cache, (void *)hybs);
		return NULL;
	...
	ret = atomic64_cmpxchg((atomic64_t *)&memcg->android_oem_data1, 0,
			       (u64)hybs);
	if (ret != 0) {
#ifdef CONFIG_HYBRIDSWAP_ASYNC_COMPRESS
		hybs->cache.dead = 1;
...
#define REGISTER_HOOK(name)                                                    \
	do {                                                                   \
		...
		if (rc) {                                                      \
			hybp(HYB_ERR, "%s:%d register hook %s failed",         \
			     __FILE__, __LINE__, #name);                       \
			goto err_out_##name;                                   \
		}                                                              \
	} while (0)

#define UNREGISTER_HOOK(name)                                                  \
	do {                                                                   \
...
static int register_all_hooks(void)
{
	...
	ERROR_OUT(tune_scan_type) : UNREGISTER_HOOK(rmqueue);
	ERROR_OUT(rmqueue)
		:
#endif
	...
	ERROR_OUT(mem_cgroup_css_online) : UNREGISTER_HOOK(mem_cgroup_free);
	ERROR_OUT(mem_cgroup_free) : UNREGISTER_HOOK(mem_cgroup_alloc);
	ERROR_OUT(mem_cgroup_alloc) : return rc;
}
...
static int memcg_total_info_per_app_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%-8s %-8s %-8s %-8s %-8s %s \n", "anon", "zram_c",
		   "zram_p", "eswap_c", "eswap_p", "memcg_n");
	while ((memcg = fetch_next_memcg(memcg))) {
		...
		zram_compress_size =
			hybridswap_read_mcg_stats(memcg, MCG_ZRAM_STORED_SZ);
		eswap_compress_size =
			hybridswap_read_mcg_stats(memcg, MCG_DISK_STORED_SZ);
		zram_page_size =
		...
		seq_printf(m, "%-8lu %-8lu %-8lu %-8lu %-8lu %s \n", anon_size,
			   zram_compress_size, zram_page_size,
...
static int memcg_swap_stat_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%-32s %12lu KB\n",
		   "zramCompressedSize:", zram_compress_size / SZ_1K);
	seq_printf(m, "%-32s %12lu KB\n",
		   "zramOrignalSize:", zram_page_size << (PAGE_SHIFT - 10));
	seq_printf(m, "%-32s %12lu KB\n",
		   "eswapCompressedSize:", eswap_compress_size / SZ_1K);
	seq_printf(m, "%-32s %12lu KB\n",
		   "eswapOrignalSize:", eswap_page_size << (PAGE_SHIFT - 10));
...
static s64 mem_cgroup_app_uid_read(struct cgroup_subsys_state *css,
				   struct cftype *cft)
{
	struct mem_cgroup *memcg = mem_cgroup_from_css(css);
...
{
	int len = snprintf(
		buf, PAGE_SIZE, "hybridswap %s out_to_eswap %s swapd %s\n",
...
int __init hybridswap_pre_init(void)
{
	...
	hybridswap_cache =
		kmem_cache_create("mem_cgroup_hybridswap",
				  sizeof(struct mem_cgroup_hybridswap), 0,
				  SLAB_PANIC, NULL);
	if (!hybridswap_cache) {
	...
	if (ret) {
		hybp(HYB_INFO,
		     "add mem_cgroup_hybridswap_legacy_files failed\n");
		goto error_out;
	}
