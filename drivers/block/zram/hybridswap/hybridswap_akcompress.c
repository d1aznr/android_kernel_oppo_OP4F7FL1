...
#define WAKEUP_AKCOMPRESSD_WATERMARK                                           \
	((DEFAULT_COMPRESS_BATCH_MB << 20) >> PAGE_SHIFT)
...
static int akcompressd_func(void *data)
{
	...
	finish_last_jobs:
		...
		while (page) {
			...
			put_memcg_cache(
				container_of(cache, memcg_hybs_t, cache));
...
void __exit akcompressd_pre_deinit(void)
{
	...
	if (list_empty(&compress_info.free_page_head))
		goto out;
	list_for_each_entry_safe (page, tmp, &compress_info.free_page_head,
				  lru) {
		list_del(&page->lru);
		free_page(page);
	}
...
int akcompress_cache_page_fault(struct zram *zram, struct page *page, u32 index)
{
	void *src, *dst;

	if (zram_test_flag(zram, index, ZRAM_CACHED)) {
		struct page *src_page =
			(struct page *)zram_fetch_page(zram, index);
		...
		hybp(HYB_DEBUG,
		     "read_anon_page_from_cache index %u page %p passed, ZRAM_CACHED\n",
		     index, src_page);
		return 1;
	}
	...
	if (zram_test_flag(zram, index, ZRAM_CACHED_COMPRESS)) {
		...
		hybp(HYB_DEBUG,
		     "read_anon_page_from_cache index %u page %p passed, ZRAM_CACHED_COMPRESS\n",
		     index, src_page);
		return 1;
	}
