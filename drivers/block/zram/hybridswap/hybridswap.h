...
extern void hybridswap_record(struct zram *zram, u32 index,
...
#define cache_index_val(index)                                                 \
	(((unsigned long)index & CACHE_INDEX_MASK) << ZRAM_INDEX_SHIFT)
#define zram_index_val(id) ((unsigned long)id & ZRAM_INDEX_MASK)
#define mk_page_val(cache_index, index)                                        \
	(cache_index_val(cache_index) | zram_index_val(index))
...
extern int add_anon_page2cache(struct zram *zram, u32 index, struct page *page);
...
extern int akcompress_cache_page_fault(struct zram *zram, struct page *page,
				       u32 index);
