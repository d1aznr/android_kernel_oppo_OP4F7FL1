...
#define ENTRY_DATA_BIT                                                         \
	(ENTRY_PTR_SHIFT + ENTRY_MCG_SHIFT_HALF + ENTRY_MCG_SHIFT_HALF + 1)
...
#define hyb_entries_for_each_entry(index, hindex, tab)                         \
	for ((index) = next_index((hindex), (tab)); (index) != (hindex);       \
	     (index) = next_index((index), (tab)))
#define hyb_entries_for_each_entry_safe(index, tmp, hindex, tab)               \
	for ((index) = next_index((hindex), (tab)),                            \
	    (tmp) = next_index((index), (tab));                                \
	     (index) != (hindex);                                              \
	     (index) = (tmp), (tmp) = next_index((index), (tab)))
#define hyb_entries_for_each_entry_reverse(index, hindex, tab)                 \
	for ((index) = prev_index((hindex), (tab)); (index) != (hindex);       \
	     (index) = prev_index((index), (tab)))
#define hyb_entries_for_each_entry_reverse_safe(index, tmp, hindex, tab)       \
	for ((index) = prev_index((hindex), (tab)),                            \
	    (tmp) = prev_index((index), (tab));                                \
	     (index) != (hindex);                                              \
	     (index) = (tmp), (tmp) = prev_index((index), (tab)))

static unsigned long warn_level[HYB_CLASS_BUTT] = { 0, 200, 500, 0 };

const char *key_point_name[HYB_KYE_POINT_BUTT] = {
	"START",    "INIT",	     "IOENTRY_ALLOC", "FIND_ESWAP",
	"IO_ESWAP", "SEGMENT_ALLOC", "BIO_ALLOC",     "SUBMIT_BIO",
	"END_IO",   "SCHED_WORK",    "END_WORK",      "CALL_BACK",
	"WAKE_UP",  "ZRAM_LOCK",     "DONE"
};

static const char class_name[HYB_CLASS_BUTT][CLASS_NAME_LEN] = {
	"out_to_eswap", "page_fault", "batches", "readahead"
};
...
struct hyb_entries_table *
alloc_table(struct hyb_entries_head *(*fetch_node)(int, void *), void *private,
	    gfp_t gfp);
...
void hyb_entries_add_nolock(int index, int hindex,
			    struct hyb_entries_table *table);
void hyb_entries_add_tail_nolock(int index, int hindex,
				 struct hyb_entries_table *table);
void hyb_entries_del_nolock(int index, int hindex,
			    struct hyb_entries_table *table);
void hyb_entries_add(int index, int hindex, struct hyb_entries_table *table);
void hyb_entries_add_tail(int index, int hindex,
			  struct hyb_entries_table *table);
void hyb_entries_del(int index, int hindex, struct hyb_entries_table *table);
unsigned short hyb_entries_fetch_memcgid(int index,
					 struct hyb_entries_table *table);
void hyb_entries_set_memcgid(int index, struct hyb_entries_table *table,
			     int memcgid);
...
int zram_fetch_mcg_last_index(struct hyb_info *infos, struct mem_cgroup *mcg,
			      int *index, int max_cnt);
int swap_maps_fetch_eswap_index(struct hyb_info *infos, int eswapid,
				int *index);
void swap_sorted_list_add(struct zram *zram, u32 index,
			  struct mem_cgroup *memcg);
void swap_sorted_list_add_tail(struct zram *zram, u32 index,
			       struct mem_cgroup *mcg);
...
static void hybridswapiowrkshow(struct seq_file *m, struct hybstatus *stat)
{
	...
	for (i = 0; i < 2; i++) {
		seq_printf(
			m, "page_fault_timeout_100ms_cnt(%s): %lld\n", fg_bg[i],
			atomic64_read(&stat->fault_stat[i].timeout_100ms_cnt));
		seq_printf(
			m, "page_fault_timeout_500ms_cnt(%s): %lld\n", fg_bg[i],
			atomic64_read(&stat->fault_stat[i].timeout_500ms_cnt));
	}
}

static void hybstatuss_show(struct seq_file *m, struct hybstatus *stat)
{
...
static void hyb_info_info_show(struct seq_file *m, struct hybstatus *stat)
{
...
static void hybridswap_fail_show(struct seq_file *m, struct hybstatus *stat)
{
	int i;

	for (i = 0; i < HYB_CLASS_BUTT; ++i) {
		seq_printf(m, "hybridswap_%s_io_fail_cnt: %lld\n",
			   class_name[i], atomic64_read(&stat->io_fail_cnt[i]));
...
unsigned long long
hybridswap_read_mcg_stats(struct mem_cgroup *mcg,
			  enum hybridswap_mcg_member mcg_member)
{
	...
	case MCG_ESWAPOUT_SZ:
		val = atomic64_read(&mcg_hybs->hybridswap_outextcnt)
		      << ESWAP_SHIFT;
		break;
	case MCG_ESWAPIN_CNT:
		...
		break;
	case MCG_ESWAPIN_SZ:
		val = atomic64_read(&mcg_hybs->hybridswap_inextcnt)
		      << ESWAP_SHIFT;
		break;
	case MCG_DISK_SPACE:
...
void hybridswap_fail_record(enum hybridswap_fail_point point, u32 index,
			    int eswapid, unsigned char *task_comm)
{
	struct hybstatus *stat = NULL;
	unsigned long flags;
...
static void
hybridswap_fail_record_fetch(struct hybridswap_fail_record_info *record_info)
{
	struct hybstatus *stat = NULL;
	unsigned long flags;
...
static ssize_t hybridswap_fail_record_show(char *buf)
{
	...
	for (i = 0; i < record_info.num; ++i)
		size += scnprintf(
			...
			ktime_us_delta(ktime_get(), record_info.record[i].time),
...
static inline meminfo_show(struct hybstatus *stat, char *buf, ssize_t len)
{
	...
	(void)hybridswap_stored_info(&eswap_total_pages,
				     &eswap_compressed_pages);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	size += scnprintf(
		buf + size, PAGE_SIZE - size, "%-32s %12llu\n",
		"reclaimin_cnt:", atomic64_read(&stat->reclaimin_cnt));
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
			  "reclaimin_bytes:",
			  atomic64_read(&stat->reclaimin_bytes) / SZ_1K);
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
			  "reclaimin_real_load:",
			  atomic64_read(&stat->reclaimin_real_load) / SZ_1K);
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
			  "reclaimin_bytes_daily:",
			  atomic64_read(&stat->reclaimin_bytes_daily) / SZ_1K);
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
			  "reclaimin_pages:",
			  atomic64_read(&stat->reclaimin_pages) * PAGE_SIZE /
				  SZ_1K);
	size += scnprintf(
		buf + size, PAGE_SIZE - size, "%-32s %12llu\n",
	...
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
			  "batchout_bytes:",
			  atomic64_read(&stat->batchout_bytes) / SZ_1K);
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
			  "batchout_real_load:",
			  atomic64_read(&stat->batchout_real_load) / SZ_1K);
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
			  "batchout_pages:",
			  atomic64_read(&stat->batchout_pages) * PAGE_SIZE /
				  SZ_1K);
	size += scnprintf(
		buf + size, PAGE_SIZE - size, "%-32s %12llu\n",
	...
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu\n",
			  "hybridswap_fault_cnt:",
			  atomic64_read(&stat->hybridswap_fault_cnt));
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
			  "reout_pages:",
			  atomic64_read(&stat->reout_pages) * PAGE_SIZE /
				  SZ_1K);
	size += scnprintf(
		buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
		"reout_bytes:", atomic64_read(&stat->reout_bytes) / SZ_1K);
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
			  "zram_stored_pages:",
			  atomic64_read(&stat->zram_stored_pages) * PAGE_SIZE /
				  SZ_1K);
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
			  "zram_stored_size:",
			  atomic64_read(&stat->zram_stored_size) / SZ_1K);
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
			  "stored_pages:",
			  atomic64_read(&stat->stored_pages) * PAGE_SIZE /
				  SZ_1K);
	size += scnprintf(
		buf + size, PAGE_SIZE - size, "%-32s %12llu KB\n",
	...
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12lld KB\n",
			  ...
			  (atomic64_read(&stat->reclaimin_real_load) -
			   atomic64_read(&stat->batchout_real_load) -
			   atomic64_read(&stat->stored_size)) /
				  SZ_1K);
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12lld KB\n",
			  "dropped_eswap_size:",
			  atomic64_read(&stat->dropped_eswap_size) / SZ_1K);
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu\n",
	...
	size += scnprintf(buf + size, PAGE_SIZE - size, "%-32s %12llu\n",
			  "used_swap_pages:",
			  atomic64_read(&stat->used_swap_pages) * PAGE_SIZE /
				  SZ_1K);
...
static void hybridswap_iostatus_bytes(struct hybridswap_io_req *req)
{
	...
	if (req->io_para.class == HYB_RECLAIM_IN) {
		...
		atomic64_add(req->page_cnt * PAGE_SIZE,
			     &stat->reclaimin_bytes_daily);
		atomic64_add(atomic64_read(&req->real_load),
			     &stat->reclaimin_real_load);
...
static void hyb_sgm_free(struct hybridswap_io_req *req, struct hyb_sgm *segment)
{
	int i;
...
static void hybridswap_wait_io_finish(struct hybridswap_io_req *req)
{
	...
	wait_time = (req->io_para.class == HYB_RECLAIM_IN) ?
			    HYBRIDSWAP_WRITE_TIME :
			    HYBRIDSWAP_READ_TIME;
...
static void hybridswap_doing_dec(struct hybridswap_io_req *req, int num)
{
	if ((atomic_sub_return(num, &req->eswap_doing) <
	     HYBRIDSWAP_MAX_INFILGHT_NUM) &&
	    req->limit_doing_flag && wq_has_sleeper(&req->io_wait))
		wake_up(&req->io_wait);
}
...
static void hybridswap_ioentry_proc(struct hyb_sgm *segment)
{
	...
	for (i = 0; i < segment->eswap_cnt; ++i) {
		...
		hybp(HYB_DEBUG, "eswap_id[%d] %d page_num %d\n", i,
		     segment->io_entries_fifo[i]->eswapid, page_num);
		...
		if (req->io_para.done_callback)
			req->io_para.done_callback(segment->io_entries_fifo[i],
						   0, req);
		hybperf_async_perf(record, HYB_CALL_BACK, callback_start,
...
static void hybridswap_io_end_work(struct work_struct *work)
{
	...
	hybperf_async_perf(record, HYB_SCHED_WORK, segment->time.end_io, 0);
...
static void hybridswap_end_io(struct bio *bio)
{
	...
	hybperf_async_perf(record, HYB_END_IO, segment->time.submit_bio, 0);

	workqueue = (req->io_para.class == HYB_RECLAIM_IN) ?
			    hybridswap_proc_write_workqueue :
...
static bool hybridswap_eswap_merge_back(struct hyb_sgm *segment,
					struct hybridswap_entry *ioentry)
{
	struct hybridswap_entry *tail_ioentry = list_last_entry(
		&segment->io_entries, struct hybridswap_entry, list);
...
static bool hybridswap_eswap_merge_front(struct hyb_sgm *segment,
					 struct hybridswap_entry *ioentry)
{
	struct hybridswap_entry *head_ioentry = list_first_entry(
		&segment->io_entries, struct hybridswap_entry, list);
...
static int hybridswap_bio_add_page(struct bio *bio, struct hyb_sgm *segment)
{
	int i;
	int k = 0;
	...
	list_for_each_entry_safe (ioentry, tmp, &segment->io_entries, list) {
		for (i = 0; i < ioentry->pages_sz; i++) {
			...
			if (unlikely(!bio_add_page(bio, ioentry->dest_pages[i],
						   PAGE_SIZE, 0))) {
				return -EIO;
			}
...
static void hybridswap_set_bio_opf(struct bio *bio, struct hyb_sgm *segment)
{
...
{
	unsigned int op = (segment->req->io_para.class == HYB_RECLAIM_IN) ?
				  REQ_OP_WRITE :
				  REQ_OP_READ;
	struct hybridswap_entry *head_ioentry = list_first_entry(
		&segment->io_entries, struct hybridswap_entry, list);
...
{
	gfp_t gfp =
		(req->io_para.class != HYB_RECLAIM_IN) ? GFP_ATOMIC : GFP_NOIO;
	...
	if (unlikely(!segment)) {
		hybridswap_errio_record(HYB_FAULT_OUT_SEGMENT_ALLOC_FAIL, req,
					ioentry->eswapid);
...
static int hybridswap_io_submit(struct hybridswap_io_req *req, bool merge_flag)
{
	int ret;
	struct hyb_sgm *segment = req->segment;
...
static bool hybridswap_check_io_para_err(struct hybridswap_io *io_para)
{
	...
	if (unlikely(!io_para->bdev || (io_para->class >= HYB_CLASS_BUTT))) {
...
static bool hybridswap_check_entry_err(struct hybridswap_entry *ioentry)
{
	int i;
	...
	if (unlikely((!ioentry->dest_pages) || (ioentry->eswapid < 0) ||
				  ...
				  struct hybridswap_entry *ioentry)
{
	...
	if (unlikely(hybridswap_check_entry_err(ioentry))) {
		hybridswap_errio_record(HYB_FAULT_OUT_IO_ENTRY_PARA_FAIL, req,
					ioentry ? ioentry->eswapid : -EINVAL);
	...
	hybp(HYB_DEBUG, "eswap id %d, pages_sz %d, addr %llx\n",
	     ioentry->eswapid, ioentry->pages_sz, ioentry->addr);
...
int hyb_io_work_begin(void)
{
	...
	hybridswap_proc_read_workqueue = alloc_workqueue(
		"proc_hybridswap_read", WQ_HIGHPRI | WQ_UNBOUND, 0);
	if (unlikely(!hybridswap_proc_read_workqueue))
		return -EFAULT;

	hybridswap_proc_write_workqueue =
		alloc_workqueue("proc_hybridswap_write", WQ_CPU_INTENSIVE, 0);
...
void *hybridswap_plug_start(struct hybridswap_io *io_para)
{
	...
	gfp = (io_para->class != HYB_RECLAIM_IN) ? GFP_ATOMIC : GFP_NOIO;
	...
	req->limit_doing_flag = (io_para->class == HYB_RECLAIM_IN) ||
				(io_para->class == HYB_PRE_OUT);
	req->wait_io_finish_flag = (io_para->class == HYB_RECLAIM_IN) ||
...
int hybridswap_read_eswap(void *iohandle, struct hybridswap_entry *ioentry)
{
	return hybridswap_io_eswapent(iohandle, ioentry);
}

int hybridswap_write_eswap(void *iohandle, struct hybridswap_entry *ioentry)
{
	return hybridswap_io_eswapent(iohandle, ioentry);
}
...
int hybridswap_plug_finish(void *iohandle)
{
	...
	hybperf_io_stat(req->io_para.record, req->page_cnt, req->segment_cnt);
...
static void
hybridswap_dump_point_lat(struct hybridswap_key_point_record *record,
			  ktime_t start)
{
	int i;
...
static void
hybridswap_dump_no_record_point(struct hybridswap_key_point_record *record,
				char *log, unsigned int *count)
{
	int i;
	...
	if (point < HYB_KYE_POINT_BUTT)
		*count +=
			snprintf(log + *count, (size_t)(DUMP_BUF_LEN - *count),
...
static void hybridswap_dump_lat(struct hybridswap_key_point_record *record,
				ktime_t curr_time, bool perf_end_flag)
{
	...
	count += snprintf(log + count, (size_t)(DUMP_BUF_LEN - count),
			  ...
			  total_time, record->class, record->task_comm,
			  record->nice);

	if (perf_end_flag)
		count += snprintf(
			log + count, (size_t)(DUMP_BUF_LEN - count),
...
static unsigned long hybperf_warn_level(enum hybridswap_class class)
{
...
static void hybperf_init_monitor(struct hybridswap_key_point_record *record,
				 enum hybridswap_class class)
{
...
static void hybperf_stop_monitor(struct hybridswap_key_point_record *record)
{
	if (!record->warn_level)
		return;
...
void hybperf_start_proc(struct hybridswap_key_point_record *record,
			enum hybridswap_key_point type, ktime_t curr_time,
			unsigned long long current_ravg_sum)
{
	struct hybridswap_key_point_info *key_point = &record->key_point[type];
...
void hybperf_end_proc(struct hybridswap_key_point_record *record,
		      enum hybridswap_key_point type, ktime_t curr_time,
		      unsigned long long current_ravg_sum)
{
	struct hybridswap_key_point_info *key_point = &record->key_point[type];
	...
	key_point->proc_ravg_sum += current_ravg_sum - key_point->last_ravg_sum;
	key_point->end_cnt++;
	key_point->last_time = curr_time;
	key_point->last_ravg_sum = current_ravg_sum;
}

void hybperf_async_perf(struct hybridswap_key_point_record *record,
			enum hybridswap_key_point type, ktime_t start,
			unsigned long long start_ravg_sum)
{
	unsigned long long current_ravg_sum =
		((type == HYB_CALL_BACK) || (type == HYB_END_WORK)) ?
	...
	hybperf_end_proc(record, type, ktime_get(), current_ravg_sum);
	spin_unlock_irqrestore(&record->key_point[type].time_lock, flags);
}

void hybperfiowrkpoint(struct hybridswap_key_point_record *record,
...
void hybperf_start(struct hybridswap_key_point_record *record, ktime_t stsrt,
		   unsigned long long start_ravg_sum,
		   enum hybridswap_class class)
{
	hybperf_init(record, class);
	hybperf_start_proc(record, HYB_START, stsrt, start_ravg_sum);
	record->key_point[HYB_START].end_cnt++;
}

void hybperfiowrkstat(struct hybridswap_key_point_record *record)
{
	int task_is_fg = 0;
	struct hybstatus *stat = hybridswap_fetch_stat_obj();
	s64 curr_lat;
	s64 timeout_value[HYB_CLASS_BUTT] = { 2000000, 100000, 500000,
					      2000000 };
	...
	if (record->class == HYB_FAULT_OUT) {
		...
		if (curr_lat > 500000)
			atomic64_inc(
				&stat->fault_stat[task_is_fg].timeout_500ms_cnt);
		else if (curr_lat > 100000)
			atomic64_inc(
				&stat->fault_stat[task_is_fg].timeout_100ms_cnt);
...
void hybperf_end(struct hybridswap_key_point_record *record)
{
	...
	hybridswap_dump_lat(record, record->key_point[HYB_DONE].first_time,
			    true);
}

void hybperfiowrkstart(struct hybridswap_key_point_record *record,
		       enum hybridswap_key_point type)
{
	hybperf_start_proc(record, type, ktime_get(),
			   hybridswap_fetch_ravg_sum());
...
void hybperfiowrkend(struct hybridswap_key_point_record *record,
		     enum hybridswap_key_point type)
{
...
void hybperf_io_stat(struct hybridswap_key_point_record *record, int page_cnt,
		     int segment_cnt)
{
					     ...
					     bool fast, bool nofail)
{
	...
	struct io_eswapent *io_eswap =
		hybridswap_malloc(sizeof(struct io_eswapent), fast, nofail);
	...
	for (i = 0; i < (int)ESWAP_PG_CNT; i++) {
		io_eswap->pages[i] =
			hybridswap_alloc_page(pool, GFP_ATOMIC, fast, nofail);
		if (!io_eswap->pages[i]) {
...
static void discard_io_eswapent(struct io_eswapent *io_eswap, unsigned int op)
{
	...
	for (i = 0; i < (int)ESWAP_PG_CNT; i++)
		if (io_eswap->pages[i])
			hybridswap_page_recycle(io_eswap->pages[i],
						io_eswap->pool);
			...
			struct io_eswapent *io_eswap)
{
	...
	for (i = esentry_pgid(eswpentry) - 1; i >= 0 && io_eswap->pages[i];
	     i--) {
...
static void eswap_add(struct io_eswapent *io_eswap, enum hybridswap_class class)
{
	struct mem_cgroup *mcg = NULL;
	struct zram *zram = NULL;
	int eswapid;
	...
	io_eswap->cnt = swap_maps_fetch_eswap_index(zram->infos, eswapid,
						    io_eswap->index);
...
static int shrink_entry(struct zram *zram, u32 index,
			struct io_eswapent *io_eswap, unsigned long eswap_off)
{
...
static int shrink_entry_list(struct io_eswapent *io_eswap)
{
	...
	for (k = 0; k < swap_cnt && swap_size < (int)ESWAP_SIZE; k++) {
		int size =
			shrink_entry(zram, swap_index[k], io_eswap, swap_size);
...
int hybridswap_manager_init(struct zram *zram)
{
	...
	zram->infos =
		alloc_hyb_info(zram->disksize, zram->nr_pages << PAGE_SHIFT);
	if (!zram->infos) {
		ret = -ENOMEM;
		goto out;
	}
...
void hybridswap_manager_memcg_init(struct zram *zram, struct mem_cgroup *memcg)
{
	memcg_hybs_t *hybs;
	...
	hyb_entries_init(memcgindex(zram->infos, memcg->id.id),
			 zram->infos->objects);
	hyb_entries_init(memcgindex(zram->infos, memcg->id.id),
...
void hybridswap_manager_memcg_deinit(struct mem_cgroup *mcg)
{
	...
	while (1) {
		...
		hyb_entries_set_memcgid(eswap_index(infos, eswapid),
					infos->eswap_table, 0);
...
unsigned long hybridswap_eswap_create(struct mem_cgroup *mcg, int *eswapid,
				   ...
				   void **private)
{
	...
	io_eswap->zram = zram;
	io_eswap->mcg = find_memcg_by_id(hyb_entries_fetch_memcgid(
		eswap_index(zram->infos, eswapid), zram->infos->eswap_table));
	if (io_eswap->mcg)
...
void hybridswap_eswap_exception(enum hybridswap_class class, void *private)
{
	struct io_eswapent *io_eswap = private;
	struct mem_cgroup *mcg = NULL;
	unsigned int op =
		(class == HYB_RECLAIM_IN) ? REQ_OP_WRITE : REQ_OP_READ;
			      ...
			      int *index, int max_cnt)
{
	...
	hyb_entries_for_each_entry_reverse_safe(
		i, tmp, memcgindex(infos, mcg->id.id), infos->objects)
	{
...
int swap_maps_fetch_eswap_index(struct hyb_info *infos, int eswapid, int *index)
{
	int cnt = 0;
	u32 i;
	...
	hyb_entries_for_each_entry(i, eswap_index(infos, eswapid),
				   infos->objects)
	{
			       ...
			       struct mem_cgroup *mcg)
{
	...
	if (!mcg || !MEMCGRP_ITEM(mcg, zram) ||
	    !MEMCGRP_ITEM(mcg, zram)->infos) {
...
bool hyb_entries_empty(int hindex, struct hyb_entries_table *table)
{
	...
	ret = (prev_index(hindex, table) == hindex) &&
	      (next_index(hindex, table) == hindex);
...
static bool pre_is_conted(struct hyb_info *infos, int eswapid, int memcgid)
{
	...
	if (is_first_index(eswap_index(infos, eswapid),
			   memcgindex(infos, memcgid), infos->eswap_table))
		return false;
...
static bool ne_is_conted(struct hyb_info *infos, int eswapid, int memcgid)
{
	...
	if (is_last_index(eswap_index(infos, eswapid),
			  memcgindex(infos, memcgid), infos->eswap_table))
		return false;
...
static void eswap_frag_info_sub(struct hyb_info *infos, int eswapid)
{
	...
	memcgid = hyb_entries_fetch_memcgid(eswap_index(infos, eswapid),
					    infos->eswap_table);
...
static struct hyb_entries_head *fetch_objects_node(int index, void *private)
{
	...
	if (index > 0 && index < infos->memcg_num) {
		...
		if (!mcg)
			goto err_out;
		return (struct hyb_entries_head *)(&MEMCGRP_ITEM(
			mcg, swap_sorted_list));
	}
err_out:
...
static int init_obj_list_table(struct hyb_info *infos)
{
	...
	infos->lru =
		vzalloc(sizeof(struct hyb_entries_head) * infos->total_objects);
	if (!infos->lru) {
...
static struct hyb_entries_head *fetch_eswap_table_node(int index, void *private)
{
	...
	if (index > 0 && index < infos->memcg_num) {
		...
		if (!mcg)
			return NULL;
		return (struct hyb_entries_head *)(&MEMCGRP_ITEM(mcg,
								 eswap_lru));
	}
err_out:
	hybp(HYB_ERR, "index = %d invalid\n", index);
...
static int init_eswap_list_table(struct hyb_info *infos)
{
	...
	if (!infos->eswap)
		goto err_out;
	infos->eswap_table =
		alloc_table(fetch_eswap_table_node, infos, GFP_KERNEL);
	if (!infos->eswap_table)
		goto err_out;
	for (i = 0; i < infos->nr_es; i++)
...
struct hyb_info *alloc_hyb_info(unsigned long ori_size, unsigned long comp_size)
{
	...
	hybp(HYB_INFO,
	     "infos %p size %lu nr_es %lu memcg_num %lu total_objects %lu\n",
...
void hybridswap_check_infos_eswap(struct hyb_info *infos)
{
	...
	for (i = 0; i < infos->nr_es; i++) {
		...
		int memcgid =
			hyb_entries_fetch_memcgid(eswapid, infos->eswap_table);
...
void hybridswap_free_eswap(struct hyb_info *infos, int eswapid)
{
	...
	hyb_entries_set_memcgid(eswap_index(infos, eswapid), infos->eswap_table,
				0);
...
int fetch_eswap(struct hyb_info *infos, int eswapid)
{
	...
	if (!hyb_entries_clear_priv(eswap_index(infos, eswapid),
				    infos->eswap_table))
		return -EBUSY;
	memcgid = hyb_entries_fetch_memcgid(eswap_index(infos, eswapid),
					    infos->eswap_table);
	if (memcgid) {
		...
		hyb_entries_del(eswap_index(infos, eswapid),
				memcgindex(infos, memcgid), infos->eswap_table);
	}
...
void put_eswap(struct hyb_info *infos, int eswapid)
{
	...
	if (memcgid) {
		hyb_lock_with_idx(memcgindex(infos, memcgid),
				  infos->eswap_table);
		hyb_entries_add_nolock(eswap_index(infos, eswapid),
				       memcgindex(infos, memcgid),
				       infos->eswap_table);
		eswap_frag_info_add(infos, eswapid);
		hyb_unlock_with_idx(memcgindex(infos, memcgid),
				    infos->eswap_table);
	}
	if (!hyb_entries_set_priv(eswap_index(infos, eswapid),
				  infos->eswap_table)) {
...
int fetch_memcg_eswap(struct hyb_info *infos, struct mem_cgroup *mcg)
{
	...
	hyb_entries_for_each_entry(
		index, memcgindex(infos, memcgid),
		infos->eswap_table) if (hyb_entries_clear_priv(index,
							       infos->eswap_table))
	{
...
int fetch_memcg_zram_entry(struct hyb_info *infos, struct mem_cgroup *mcg)
{
	...
	hyb_entries_for_each_entry(idx, memcgindex(infos, memcgid),
				   infos->objects)
	{
		index = idx;
		break;
...
int fetch_eswap_zram_entry(struct hyb_info *infos, int eswapid)
{
	...
	hyb_lock_with_idx(eswap_index(infos, eswapid), infos->objects);
	hyb_entries_for_each_entry(idx, eswap_index(infos, eswapid),
				   infos->objects)
	{
		index = idx;
		break;
	...
	if (eswap_para->pool) {
		...
		if (!list_empty(&eswap_para->pool->page_pool_list)) {
			page = list_first_entry(
				&eswap_para->pool->page_pool_list, struct page,
				lru);
			list_del(&page->lru);
		}
...
unsigned long hybridswap_zsmalloc(struct zs_pool *zs_pool, size_t size,
				  struct hybridswap_page_pool *pool)
{
...
struct page *hybridswap_alloc_page(struct hybridswap_page_pool *pool, gfp_t gfp,
				   bool fast, bool nofail)
{
...
int hybridswap_bind(struct zram *zram, const char *file_name)
{
	...
	if (unlikely(err)) {
		hybp(HYB_ERR, "%s set blocksize failed! eno = %d\n", file_name,
		     err);
		goto out;
	}
...
static bool hybridswap_global_setting_init(struct zram *zram)
{
	...
	global_settings.stat =
		hybridswap_malloc(sizeof(struct hybstatus), false, true);
	if (unlikely(!global_settings.stat)) {
	...
	global_settings.reclaim_wq =
		alloc_workqueue("hybridswap_reclaim", WQ_CPU_INTENSIVE, 0);
	...
	timer_setup(&global_settings.lpc_timer,
		    hybridswap_life_protect_ctrl_timer, 0);
				    ...
				    struct device_attribute *attr, char *buf)
{
	int len = snprintf(buf, PAGE_SIZE, "hybridswap %s out_to_eswap %s\n",
			   ...
			   hybridswap_out_to_eswap_enable() ? "enable" :
							      "disable");
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	size += scnprintf(buf + size, PAGE_SIZE - size, "%lu\n",
			  zram->increase_nr_pages >> 8);

	return size;
}

int mem_cgroup_stored_wm_scale_write(struct cgroup_subsys_state *css,
...
s64 mem_cgroup_stored_wm_scale_read(struct cgroup_subsys_state *css,
				    struct cftype *cft)
{
	if (!global_settings.stat)
		return -EINVAL;
...
static void hybridswap_memcg_iter(int (*iter)(struct mem_cgroup *, void *),
				  void *data)
{
	...
	while (mcg) {
		...
		hybp(HYB_DEBUG, "%pS mcg %d %s %s, ret %d\n", iter, mcg->id.id,
		     MEMCGRP_ITEM(mcg, name), ret ? "failed" : "pass", ret);
		if (ret) {
...
void hybridswap_record(struct zram *zram, u32 index, struct mem_cgroup *memcg)
{
	memcg_hybs_t *hybs;
	struct hybstatus *stat;
...
static unsigned long memcg_reclaim_size(struct mem_cgroup *memcg)
{
	...
	new_size = (zram_size + cur_size) * atomic_read(&hybs->zram2ufs_scale) /
		   100;
...
static void hybridswap_flush_cb(enum hybridswap_class class, void *pri,
				struct hybridswap_io_req *req)
{
	switch (class) {
	case HYB_FAULT_OUT:
...
static void hybridswap_flush_done(struct hybridswap_entry *ioentry, int err,
				  struct hybridswap_io_req *req)
{
	struct io_priv *data;
	...
	if (likely(!err)) {
		hybridswap_flush_cb(data->class, ioentry->manager_private, req);
#ifdef CONFIG_HYBRIDSWAP_SWAPD
		if (!zram_watermark_ok())
			wake_all_swapd();
#endif
...
static void hybridswap_free_pagepool(struct io_work_arg *iowork)
{
	...
	while (!list_empty(&iowork->data.page_pool.page_pool_list)) {
		free_page =
			list_first_entry(&iowork->data.page_pool.page_pool_list,
					 struct page, lru);
...
static void hybstatus_alloc_fail(enum hybridswap_class class, int err)
{
	struct hybstatus *stat = hybridswap_fetch_stat_obj();
...
static int hybridswap_reclaim_eswap(struct mem_cgroup *memcg,
				    struct io_work_arg *iowork,
				    unsigned long *require_size,
				    unsigned long *mcg_reclaimed_sz, int *errio)
{
	int ret;
	unsigned long reclaim_size;
	...
	iowork->ioentry =
		hybridswap_malloc(sizeof(struct hybridswap_entry), false, true);
	...
	reclaim_size =
		hybridswap_eswap_create(memcg, &iowork->ioentry->eswapid,
					&iowork->io_buf,
				     ...
				     unsigned long *mcg_reclaimed_sz)
{
	...
	hybperf_start(&iowork->record, start, start_ravg_sum, HYB_RECLAIM_IN);
	hybperfiowrkstart(&iowork->record, HYB_INIT);
	iowork->iohandle =
		hybridswap_init_plug(hybs->zram, HYB_RECLAIM_IN, iowork);
	...
	while (require_size) {
		if (hybridswap_reclaim_eswap(memcg, iowork, &require_size,
					     mcg_reclaimed_sz, &errio))
			break;
...
out:
	hybp(HYB_INFO,
	     "memcg %s %lu %lu out_to_eswap %lu KB eswap %lu zram %lu %d\n",
...
static int hybridswap_permcg_reclaimin(struct mem_cgroup *memcg, void *data)
{
	...
	hybp(HYB_INFO,
	     "memcg %s mcg_reclaimed_size %lu rq->reclaimined_sz %lu rq->size %lu rq->out_size %lu ret %d\n",
	     hybs->name, mcg_reclaimed_size, rq->reclaimined_sz, rq->size,
	     rq->out_size, ret);
				    ...
				    int *errio)
{
	...
	iowork->ioentry->eswapid = hybridswap_find_eswap_by_memcg(
		mcg, &iowork->io_buf, &iowork->ioentry->manager_private);
	hybperfiowrkend(&iowork->record, HYB_FIND_ESWAP);
	if (iowork->ioentry->eswapid < 0) {
		hybstatus_alloc_fail(HYB_BATCH_OUT, iowork->ioentry->eswapid);
		hybridswap_free(iowork->ioentry);
		return -EAGAIN;
	}
				      ...
				      struct mem_cgroup *mcg, bool preload)
{
	...
	iowork = hybridswap_malloc(sizeof(struct io_work_arg), !preload,
				   preload);
	if (unlikely(!iowork)) {
		hybp(HYB_ERR, "alloc iowork failed!\n");
		hybstatus_alloc_fail(HYB_BATCH_OUT, -ENOMEM);
	...
	hybperfiowrkstart(&iowork->record, HYB_INIT);
	iowork->iohandle =
		hybridswap_init_plug(MEMCGRP_ITEM(mcg, zram),
				     preload ? HYB_PRE_OUT : HYB_BATCH_OUT,
				     iowork);
	hybperfiowrkend(&iowork->record, HYB_INIT);
	if (unlikely(!iowork->iohandle)) {
...
static int hybridswap_do_batches(struct mem_cgroup *mcg, unsigned long size,
				 bool preload)
{
	int ret = 0;
	int errio = 0;
...
int hybridswap_batches(struct mem_cgroup *mcg, unsigned long size, bool preload)
{
	int ret;
...
static bool hybridswap_page_fault_check(struct zram *zram, u32 index,
					     ...
					     unsigned long zentry, u32 index)
{
	...
	iowork->io_buf.pool = NULL;
	hybperfiowrkstart(&iowork->record, HYB_FIND_ESWAP);
	iowork->ioentry->eswapid = hybridswap_find_eswap_by_index(
	...
	if (unlikely(iowork->ioentry->eswapid == -EBUSY)) {
		while (1) {
			...
			iowork->ioentry->eswapid =
				hybridswap_find_eswap_by_index(
					zentry, &iowork->io_buf,
					&iowork->ioentry->manager_private);
			hybperfiowrkend(&iowork->record, HYB_FIND_ESWAP);
...
#endif
	if (iowork->ioentry->eswapid < 0) {
		hybstatus_alloc_fail(HYB_FAULT_OUT, iowork->ioentry->eswapid);
...
static int hybridswap_page_fault_exit_check(struct zram *zram, u32 index,
					    int ret)
{
	zram_slot_lock(zram, index);
	if (likely(!ret)) {
				       ...
				       unsigned long zentry)
{
	...
	hybperfiowrkstart(&iowork->record, HYB_IOENTRY_ALLOC);
	iowork->ioentry =
		hybridswap_malloc(sizeof(struct hybridswap_entry), true, true);
	hybperfiowrkend(&iowork->record, HYB_IOENTRY_ALLOC);
	if (unlikely(!iowork->ioentry)) {
		hybp(HYB_ERR, "alloc io entry failed!\n");
		hybstatus_alloc_fail(HYB_FAULT_OUT, -ENOMEM);
		hybridswap_fail_record(HYB_FAULT_OUT_ENTRY_ALLOC_FAIL, index, 0,
				       iowork->record.task_comm);
		return -ENOMEM;
	}
...
int hybridswap_page_fault(struct zram *zram, u32 index)
{
	...
	hybperf_start(&iowork.record, start, start_ravg_sum, HYB_FAULT_OUT);

	hybperfiowrkstart(&iowork.record, HYB_INIT);
	iowork.iohandle = hybridswap_init_plug(zram, HYB_FAULT_OUT, &iowork);
	hybperfiowrkend(&iowork.record, HYB_INIT);
	if (unlikely(!iowork.iohandle)) {
		hybp(HYB_ERR, "plug start failed!\n");
		hybstatus_alloc_fail(HYB_FAULT_OUT, -ENOMEM);
		ret = -EIO;
		hybridswap_fail_record(HYB_FAULT_OUT_INIT_FAIL, index, 0,
				       iowork.record.task_comm);
