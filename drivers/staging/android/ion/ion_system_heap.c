...
#define MIN_ION_POOL_PAGES_BOOTUP                                              \
	102400 /*Allocate 400 MB each ION pool size during bootup */
#endif
				    ...
				    unsigned long flags)
{
	...
	if (boost_pool) {
		...
		while (size_remaining > 0) {
			...
			info = boost_pool_allocate(boost_pool, size_remaining,
						   max_order);
#ifdef OPLUS_FEATURE_UIFIRST
			current->static_ux = 0;
#endif /* OPLUS_FEATURE_UIFIRST */
				 ...
				 int nr_to_scan)
{
	...
	for (i = NUM_ORDERS - 1; i >= 0; i--) {
		 ...
		 */
		if (global_zone_page_state(NR_IONCACHE_PAGES) <
		    MIN_ION_POOL_PAGES)
			break;
#endif
#ifdef CONFIG_OPLUS_ION_BOOSTPOOL
...
int ion_system_heap_create_pools(struct ion_system_heap *sys_heap,
				 struct ion_page_pool **pools, bool cached,
...
static int fill_pool_kworkthread(void *p)
{
	...
	for (i = 0; i < NUM_ORDERS; i++) {
		while (global_zone_page_state(NR_IONCACHE_PAGES) <
			...
			if (fill_page_pool(sh->heap.priv, sh->cached_pools[i]) <
			    0)
				break;
		}
	}
	...
	for (i = 0; i < NUM_ORDERS; i++) {
		while (global_zone_page_state(NR_IONCACHE_PAGES) <
			...
			if (fill_page_pool(sh->heap.priv,
					   sh->uncached_pools[i]) < 0)
				break;
		}
	}
	...
	for (i = 0; i < VMID_LAST; i++)
		if (is_secure_vmid_valid(i))
			if (ion_system_heap_create_pools(
				    heap, heap->secure_pools[i], false, false))
	...
	if (kcrit_scene_init()) {
		...
		if (!IS_ERR_OR_NULL(boost_root_dir)) {
			...
			heap->uncached_boost_pool =
				boost_pool_create(heap, 0, uncached_sz,
						  boost_root_dir,
						  "ion_uncached");
			...
			/* TODO set by total ram pages */
			heap->cam_pool =
				boost_pool_create(heap, ION_FLAG_CACHED, cam_sz,
