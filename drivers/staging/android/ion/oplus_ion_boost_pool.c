...
static int fill_boost_page_pool(struct ion_page_pool *pool)
{
	...
	ion_pages_sync_for_device(dev, page, PAGE_SIZE << pool->order,
				  DMA_BIDIRECTIONAL);
...
void boost_pool_dump(struct ion_boost_pool *pool)
{
	...
	pr_info("Name:%s: %dMib, low: %dMib, high:%dMib\n", pool->name,
		M(boost_pool_nr_pages(pool)), M(pool->low), M(pool->high));
	...
	for (i = 0; i < NUM_ORDERS; i++) {
		...
		pr_info("%d order %u highmem pages in boost pool = %lu total\n",
			...
			(PAGE_SIZE << page_pool->order) *
				page_pool->high_count);
...
int boost_pool_free(struct ion_boost_pool *pool, struct page *page, int order)
{
	if (!boost_pool_enable) {
...
static int boost_pool_proc_show(struct seq_file *s, void *v)
{
	...
	seq_printf(
		s,
		"Name:%s: %dMib, prefill: %d origin: %dMib low: %dMib high: %dMib\n",
		boost_pool->name, M(boost_pool_nr_pages(boost_pool)),
		boost_pool->prefill, M(boost_pool->origin), M(boost_pool->low),
		M(boost_pool->high));
	...
	for (i = 0; i < NUM_ORDERS; i++) {
		...
		seq_printf(
			s,
			"%d order %u highmem pages in boost pool = %lu total\n",
			pool->high_count, pool->order,
			(PAGE_SIZE << pool->order) * pool->high_count);
		seq_printf(
			s,
			"%d order %u lowmem pages in boost pool = %lu total\n",
...
static ssize_t boost_pool_proc_write(struct file *file, const char __user *buf,
					 ...
					 char *name)
{
	...
	boost_pool =
		kzalloc(sizeof(struct ion_boost_pool) +
	...
	if (ion_system_heap_create_pools(heap, boost_pool->pools, false,
					 boost_flag))
		goto free_heap;
	...
	boost_pool->proc_info = proc_create_data(
		name, 0666, root_dir, &boost_pool_proc_ops, boost_pool);
	...
	boost_pool->proc_low_info = proc_create_data(
		buf, 0666, root_dir, &boost_pool_low_proc_ops, boost_pool);
	...
	boost_pool->proc_stat = proc_create_data(
		buf, 0444, root_dir, &boost_pool_stat_proc_ops, boost_pool);
	...
	tsk = kthread_run(boost_pool_kworkthread, boost_pool, "bp_%s", name);
