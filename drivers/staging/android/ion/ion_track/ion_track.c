...
{
	return ((struct ion_buff_info *)lb)->size -
	       ((struct ion_buff_info *)la)->size;
}
...
void dump_ion_info(char *dump_buff, int len)
{
	...
	for (i = 0; i < buff_index; i++) {
		...
		dump_buff_len += scnprintf(
			...
			tsk->pid, tsk->tgid, ion_buff_i->size,
			ion_buff_i->count, ion_buff_i->jfs_min,
			ion_buff_i->jfs_sum / ion_buff_i->count,
			ion_buff_i->jfs_max);
		...
		for (j = 0; j < ion_buff_i->heap_cnt; j++) {
			...
			dump_buff_len += scnprintf(
				...
				"%s %lu %u age %lu/%lu/%lu\n", heap->heap->name,
				heap->size, heap->count, heap->jfs_min,
				heap->jfs_sum / heap->count, heap->jfs_max);
		}
