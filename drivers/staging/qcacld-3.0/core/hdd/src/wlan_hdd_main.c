...
void wlan_driver_send_uevent(char *enable)
{
	...
	snprintf(wifi_switch_event, sizeof(wifi_switch_event),
		 "WLAN_SWITCH_EVENT=driver");

	if (enable) {
		snprintf(wifi_enable, sizeof(wifi_enable), "WIFI_TURNING=%s",
			 enable);
	} else {
		snprintf(wifi_enable, sizeof(wifi_enable),
			 "WIFI_TURNING=unkown");
	}

	if (captue_wlan_hostdriver_loadresult.ini_prase_status ==
	    INI_PRASE_SUCCESS) {
		snprintf(ini_prase, sizeof(ini_prase), "INIPRASE=%s",
			 "success");
	} else if (captue_wlan_hostdriver_loadresult.ini_prase_status ==
		   INI_PRASE_FAIL) {
	...
	if (captue_wlan_hostdriver_loadresult.insmod_status == INSMOD_SUCCESS) {
		snprintf(insmod_stats, sizeof(insmod_stats), "INSMODSTATUS=%s",
			 "success");
	} else if (captue_wlan_hostdriver_loadresult.insmod_status ==
		   INSMOD_FAIL) {
		snprintf(insmod_stats, sizeof(insmod_stats), "INSMODSTATUS=%s",
			 "fail");
	} else {
	...
	if (captue_wlan_hostdriver_loadresult.rmmod_status == RMMOD_SUCCESS) {
		snprintf(rmmod_stats, sizeof(rmmod_stats), "RMMODSTATUS=%s",
			 "success");
	} else if (captue_wlan_hostdriver_loadresult.rmmod_status ==
		   RMMOD_FAIL) {
		snprintf(rmmod_stats, sizeof(rmmod_stats), "RMMODSTATUS=%s",
			 "fail");
	} else {
	...
	if (qdf_dev && qdf_dev->dev) {
		ret_val = kobject_uevent_env(&(qdf_dev->dev->kobj), KOBJ_CHANGE,
					     envp);
		if (!ret_val) {
...
#ifdef OPLUS_FEATURE_WIFI_DCS_SWITCH
		//Add for wifi switch monitor
		captue_wlan_hostdriver_loadresult.ini_prase_status =
			INI_PRASE_FAIL;
#endif /* OPLUS_FEATURE_WIFI_DCS_SWITCH */
		ret = qdf_status_to_os_return(status);
...
void hdd_restart_sap(struct hdd_adapter *ap_adapter)
{
	...
	if (test_bit(SOFTAP_BSS_STARTED, &ap_adapter->event_flags)) {
		...
		qdf_status = qdf_wait_for_event_completion(
			&hostapd_state->qdf_event, SME_CMD_START_BSS_TIMEOUT);
		wlansap_reset_sap_config_add_ie(sap_config,
				eUPDATE_IE_ALL);
		if (!QDF_IS_STATUS_SUCCESS(qdf_status)) {
			hdd_err("SAP Start failed");
