...
static struct wireless_dev *get_wdev_sta()
{
	...
	if (s_hdd_ctx != NULL && s_hdd_ctx->wiphy &&
	    s_hdd_ctx->wiphy->registered) {
...
static int get_phy_capacity(int band)
{
	...
	if (wdev_sta) {
		...
		if (s_hdd_ctx->wiphy == NULL ||
		    s_hdd_ctx->wiphy->bands[NL80211_BAND_2GHZ] == NULL ||
		...
		if (band == BAND_2G) {
			band_supported =
				s_hdd_ctx->wiphy->bands[NL80211_BAND_2GHZ];
		} else if (band == BAND_5G) {
			band_supported =
				s_hdd_ctx->wiphy->bands[NL80211_BAND_5GHZ];
		} else {
			band_supported = NULL;
		}

		if (band_supported != NULL) {
			if (band_supported->iftype_data != NULL &&
			    band_supported->iftype_data->he_cap.has_he) {
...
static void get_supported_channels(int band, int *out_len, int *out_freqs,
				   int max_num)
{
	int errno;
	int index;
	...
	*out_len = 0;
	...
	if (wdev_sta) {
		...
		if (band_supported != NULL) {
			for (index = 0; index < band_supported->n_channels;
			     index++) {
...
static void get_avoid_channels(int *out_len, int *out_freqs, int max_num)
{
	...
	*out_len = 0;
	...
	if (wdev_sta) {
		...
		for (index = 0; index < s_hdd_ctx->unsafe_channel_count &&
				index < NUM_CHANNELS;
		     index++) {
			/*
