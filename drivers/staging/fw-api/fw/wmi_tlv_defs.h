...
#define WMITLV_TABLE_WMI_REQUEST_CTRL_PATH_STATS_CMDID(id, op, buf, len)       \
	WMITLV_ELEM(                                                           \
		id, op, buf, len,                                              \
		WMITLV_TAG_STRUC_wmi_request_ctrl_path_stats_cmd_fixed_param,  \
		wmi_request_ctrl_path_stats_cmd_fixed_param, fixed_param,      \
		WMITLV_SIZE_FIX)                                               \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_UINT32, A_UINT32,       \
		    pdev_ids, WMITLV_SIZE_VAR)                                 \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_UINT32, A_UINT32,       \
		    vdev_ids, WMITLV_SIZE_VAR)                                 \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_FIXED_STRUC,            \
		    wmi_mac_addr, mac_addr_list, WMITLV_SIZE_VAR)              \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_UINT32, A_UINT32,       \
		    twt_dialog_ids, WMITLV_SIZE_VAR)
...
#define WMITLV_TABLE_WMI_SERVICE_READY_EXT2_EVENTID(id, op, buf, len)          \
	WMITLV_ELEM(id, op, buf, len,                                          \
		    WMITLV_TAG_STRUC_wmi_service_ready_ext2_event_fixed_param, \
		    wmi_service_ready_ext2_event_fixed_param, fixed_param,     \
		    WMITLV_SIZE_FIX)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    WMI_DMA_RING_CAPABILITIES, dma_ring_caps, WMITLV_SIZE_VAR) \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
	...
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_STRUC_wmi_nan_capabilities,   \
		    wmi_nan_capabilities, nan_cap, WMITLV_SIZE_FIX)            \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    WMI_SCAN_RADIO_CAPABILITIES_EXT2, wmi_scan_radio_caps,     \
		    WMITLV_SIZE_VAR)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_twt_caps_params, twt_caps, WMITLV_SIZE_VAR)            \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_htt_msdu_idx_to_htt_msdu_qtype,                        \
		    htt_msdu_idx_to_qtype_map, WMITLV_SIZE_VAR)                \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_dbs_or_sbs_cap_ext, dbs_or_sbs_cap_ext,                \
		    WMITLV_SIZE_VAR)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_cust_bdf_version_capabilities,                         \
		    cust_bdf_version_capabilities, WMITLV_SIZE_VAR)
...
#define WMITLV_TABLE_WMI_WOW_WAKEUP_HOST_EVENTID(id, op, buf, len)             \
	WMITLV_ELEM(id, op, buf, len,                                          \
		    WMITLV_TAG_STRUC_WOW_EVENT_INFO_fixed_param,               \
		    WOW_EVENT_INFO_fixed_param, fixed_param, WMITLV_SIZE_FIX)  \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    WOW_EVENT_INFO_SECTION_BITMAP, wow_bitmap_info,            \
		    WMITLV_SIZE_VAR)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_BYTE, A_UINT8,          \
		    wow_packet_buffer, WMITLV_SIZE_VAR)                        \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_hb_ind_event_fixed_param, hb_indevt, WMITLV_SIZE_VAR)  \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    WMI_GTK_OFFLOAD_STATUS_EVENT_fixed_param, wow_gtkigtk,     \
		    WMITLV_SIZE_VAR)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_oic_ping_handoff_event, wow_oic_ping_handoff,          \
		    WMITLV_SIZE_VAR)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_dhcp_lease_renew_event, wow_dhcp_lease_renew,          \
		    WMITLV_SIZE_VAR)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_motion_det_event, md_indevt, WMITLV_SIZE_VAR)          \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_motion_det_base_line_event, bl_indevt,                 \
		    WMITLV_SIZE_VAR)
...
#define WMITLV_TABLE_WMI_DEBUG_MESG_FLUSH_COMPLETE_EVENTID(id, op, buf, len)   \
	WMITLV_ELEM(                                                           \
		id, op, buf, len,                                              \
		WMITLV_TAG_STRUC_wmi_debug_mesg_flush_complete_fixed_param,    \
		wmi_debug_mesg_flush_complete_fixed_param, fixed_param,        \
		WMITLV_SIZE_FIX)                                               \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_debug_mesg_fw_data_stall_param, data_stall,            \
		    WMITLV_SIZE_VAR)
...
#define WMITLV_TABLE_WMI_CTRL_PATH_STATS_EVENTID(id, op, buf, len)             \
	WMITLV_ELEM(id, op, buf, len,                                          \
		    WMITLV_TAG_STRUC_wmi_ctrl_path_stats_event_fixed_param,    \
		    wmi_ctrl_path_stats_event_fixed_param, fixed_param,        \
		    WMITLV_SIZE_FIX)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_ctrl_path_pdev_stats_struct, ctrl_path_pdev_stats,     \
		    WMITLV_SIZE_VAR)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_vdev_extd_stats, vdev_extd_stats, WMITLV_SIZE_VAR)     \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_ctrl_path_mem_stats_struct, ctrl_path_mem_stats,       \
		    WMITLV_SIZE_VAR)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_ctrl_path_twt_stats_struct, ctrl_path_twt_stats,       \
		    WMITLV_SIZE_VAR)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_ctrl_path_calibration_stats_struct,                    \
		    ctrl_path_calibration_stats, WMITLV_SIZE_VAR)              \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_ctrl_path_dfs_channel_stats_struct,                    \
		    ctrl_path_dfs_channel_stats, WMITLV_SIZE_VAR)              \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_ctrl_path_awgn_stats_struct, ctrl_path_awgn_stats,     \
		    WMITLV_SIZE_VAR)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_ctrl_path_btcoex_stats_struct, ctrl_path_btcoex_stats, \
		    WMITLV_SIZE_VAR)
...
#define WMITLV_TABLE_WMI_PDEV_SSCAN_FW_PARAM_EVENTID(id, op, buf, len)         \
	WMITLV_ELEM(id, op, buf, len,                                          \
		    WMITLV_TAG_STRUC_wmi_pdev_sscan_fw_cmd_fixed_param,        \
		    wmi_pdev_sscan_fw_cmd_fixed_param, fixed_param,            \
		    WMITLV_SIZE_FIX)                                           \
	WMITLV_ELEM(id, op, buf, len, WMITLV_TAG_ARRAY_STRUC,                  \
		    wmi_pdev_sscan_fft_bin_index, fft_bin_index,               \
		    WMITLV_SIZE_VAR)
