...
EXPORT_SYMBOL_GPL(dma_buf_get_uuid);
	...
	(defined(OPLUS_FEATURE_PERFORMANCE) && defined(CONFIG_PROC_FS))
...
int dma_buf_init_procfs(void)
{
	...
	p = proc_create_data("bufinfo", S_IFREG | 0664, dma_buf_procfs_root,
			     &dma_buf_debug_fops, NULL);
	if (IS_ERR(p)) {
	...
	p = proc_create_data("dmaprocs", S_IFREG | 0664, dma_buf_procfs_root,
			     &dma_procs_debug_fops, NULL);
	if (IS_ERR(p)) {
