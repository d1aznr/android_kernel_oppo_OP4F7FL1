...
#define MHALL_ERR(fmt, args...)                                                \
	printk(KERN_ERR MHALL_TAG "%s %d : " fmt, __FUNCTION__, __LINE__,      \
	       ##args)
#define MHALL_LOG(fmt, args...)                                                \
	printk(KERN_INFO MHALL_TAG "%s(%d):" fmt, __FUNCTION__, __LINE__,      \
	       ##args)
#else
...
struct blocking_notifier_head hall_notifier =
	BLOCKING_NOTIFIER_INIT(hall_notifier);

static void hall_optional_handle(struct hall_simulated_data *hall_data,
				 hall_status status)
{
	...
	if (last_hall != NULL && last_hall != hall_data) {
		MHALL_ERR("%d--anather hall near now ,return , status = %d",
			  hall_data->id, TYPE_HALL_NEAR == status ? 1 : 0);
		return;
	} else if (last_hall == NULL && status == TYPE_HALL_NEAR) {
		last_hall = hall_data;
		need_report = true;
	} else if (last_hall != NULL && status == TYPE_HALL_FAR &&
		   last_hall == hall_data) {
		last_hall = NULL;
		need_report = true;
	}
	if (need_report) {
		if (TYPE_HANDLE_NOTIFY_WIRELESS == hall_data->handle_option) {
			blocking_notifier_call_chain(
				&hall_notifier,
				TYPE_HALL_NEAR == status ? 1 : 0, NULL);
			MHALL_ERR("%d--TYPE_HANDLE_NOTIFY_WIRELESS , %d",
				  hall_data->id,
				  TYPE_HALL_NEAR == status ? 1 : 0);

		} else if (TYPE_HANDLE_REPORT_KEYS ==
			   hall_data->handle_option) {
			input_report_switch(hall_data->hall_input_dev,
					    SW_PEN_INSERTED,
					    TYPE_HALL_NEAR == status ? 1 : 0);
			input_sync(hall_data->hall_input_dev);
			MHALL_ERR("%d--TYPE_HANDLE_REPORT_KEYS , %d",
				  hall_data->id,
				  TYPE_HALL_NEAR == status ? 1 : 0);

		} else {
			MHALL_ERR(
				"handle_option=0 noritify charge and android");
			// blocking_notifier_call_chain(&hall_notifier, TYPE_HALL_NEAR == status ? 1 : 0, NULL);
			input_report_switch(hall_data->hall_input_dev,
					    SW_PEN_INSERTED,
					    TYPE_HALL_NEAR == status ? 1 : 0);
			input_sync(hall_data->hall_input_dev);
			MHALL_ERR("%d--double report, %d", hall_data->id,
				  TYPE_HALL_NEAR == status ? 1 : 0);
		}
	}
...
static irqreturn_t hall_interrupt_handler_func(int irq, void *dev)
{
	...
	if (hall_data->active_low) {
		if ((TYPE_HALL_NEAR == hall_data->hall_status) && val) {
			...
			hall_data->hall_status =
				(0 == val ? TYPE_HALL_NEAR : TYPE_HALL_FAR);
			MHALL_ERR(
				"[logic wrong]%s hall_status:%d, gpio_val:%d.",
				hall_data->DEV_NAME, hall_data->hall_status,
				val);
			if (hall_data->hall_status == TYPE_HALL_NEAR) {
				hall_optional_handle(hall_data,
						     hall_data->hall_status);
			}
		}
	} else {
		MHALL_ERR("[%s]not support non active low handle yet.",
			  hall_data->DEV_NAME);
	}
...
static int hall_int_gpio_init(struct device *dev,
			      struct hall_simulated_data *hall_data)
{
	...
	pinctrl_select_state(hall_data->hall_pinctrl,
			     hall_data->hall_int_active);
			 ...
			 struct hall_simulated_data *hall_data)
{
	...
	hall_data->irq_gpio = of_get_named_gpio_flags(np, "irq-gpio", 0,
						      &(hall_data->irq_flags));

	hall_data->active_low = of_property_read_bool(np, "irq_active_low");
	rc = of_property_read_u32(np, "optional-handle-type",
				  &hall_data->handle_option);
	if (rc) {
	...
	MHALL_LOG("gpio %d hall_irq %d, flags = %d\n", hall_data->irq_gpio,
		  hall_data->irq_number, hall_data->irq_flags);
	return 0;
}
...
static int hall_simulated_driver_probe(struct platform_device *dev)
{
	...
	err = request_threaded_irq(hall_data->irq_number, NULL,
				   hall_interrupt_handler_func,
				   hall_data->irq_flags, hall_data->DEV_NAME,
				   hall_data);
	if (err < 0) {
