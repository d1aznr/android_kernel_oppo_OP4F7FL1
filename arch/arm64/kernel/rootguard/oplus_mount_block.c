...
int oplus_mount_block(const char __user *dir_name, unsigned long flags)
{
	...
	if (dir_name != NULL && copy_from_user(dname, dir_name, 8) == 0) {
		if ((!strncmp(dname, "/system", 8) ||
		     !strncmp(dname, "/vendor", 8)) &&
		    !(flags & MS_RDONLY) && (is_normal_boot_mode())) {
			printk(KERN_ERR
			...
			dcs_event_payload =
				dcs_stack + sizeof(struct kernel_packet_info);
			...
			dcs_event->payload_length = snprintf(
				dcs_event_payload, 256, "partition@@system");
