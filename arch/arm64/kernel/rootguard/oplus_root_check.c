...
void oplus_root_check_succ(uid_t uid, uid_t euid, uid_t fsuid, uid_t callnum)
{
	...
	dcs_event->payload_length = snprintf(
		dcs_event_payload, 256,
	...
	$$curr_uid@@%u$$curr_euid@@%u$$curr_fsuid@@%u$$curr_name@@%s$$ppid@@%d$$ppidname@@%s$$enforce@@%d",
		uid, euid, fsuid, callnum, get_fs(), current_uid().val,
		current_euid().val, current_fsuid().val,
		get_task_comm(comm, current), ppid, nameofppid,
		selinux_is_enabled());
	printk(KERN_INFO "oplus_root_check_succ,payload:%s\n",
	       dcs_event_payload);
	memcpy(dcs_event->payload, dcs_event_payload,
	       strlen(dcs_event_payload));
...
{
	printk(KERN_INFO
	       "[OPLUS_ROOT_CHECK]:Kill the process of escalation...");
