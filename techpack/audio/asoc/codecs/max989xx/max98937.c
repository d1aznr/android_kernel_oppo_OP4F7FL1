...
static inline bool max98927_can_use_dsm(struct max989xx_priv *max98927)
{
	if (!max98927) {
		...
		return false;
	} else if (max98927->rcv_mix_mode || max98927->factory_test ||
		   !max98927->dsm_enable) {
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	struct max989xx_priv *max98927 = snd_soc_component_get_drvdata(
...
#define TEMPERATURE_VALUE_TO_MILLI_CELSIUS(x)                                  \
	((uint32_t)((((uint64_t)(x)) * DIV_COEF) >> 19))

static inline bool rdc_check_valid(struct max989xx_priv *max98927, uint32_t rdc,
				   int ch)
{
...
#else
			if (!rdc_check_valid(max98927, impedance_r,
					     MAX98927R)) {
#endif /* OPLUS_ARCH_EXTENDS */
				impedance_r =
					SPK_MUTE_VALUE; //calibration failed specail code
...
static ssize_t max989xx_dbg_f0_read(struct file *file, char __user *user_buf,
				    size_t count, loff_t *ppos)
{
#ifdef CONFIG_DEBUG_FS
	struct i2c_client *i2c = file->private_data;
...
#ifdef OPLUS_ARCH_EXTENDS
static ssize_t max989xx_dbg_range_read(struct file *file, char __user *user_buf,
				       size_t count, loff_t *ppos)
{
#ifdef CONFIG_DEBUG_FS
	struct i2c_client *i2c = file->private_data;
				...
				struct i2c_client *i2c)
{
	...
	debugfs_create_file("status", S_IRUGO, max98927->dbg_dir, i2c,
			    &max989xx_dbg_status_fops);
#ifdef OPLUS_ARCH_EXTENDS
	debugfs_create_file("range", S_IRUGO | S_IWUGO, max98927->dbg_dir, i2c,
			    &max989xx_dbg_range_fops);
				...
				int stream)
{
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (mute) {
			...
			max98927_wrapper_read(max98927, 0, amp_enable,
					      &amp_reg);

			pr_info("%s: PA0:0x04=0x%x,0x05=0x%x,0x06=0x%x, 0x%x=0x%x\n",
				__func__, pa0_reg4, pa0_reg5, pa0_reg6,
				amp_enable, amp_reg);
			...
			if (max98927->mono_stereo == 0x0 ||
				...
				if (!rdc_check_valid(
					...
					if (rdc_check_valid(impedance,
					    ...
					    impedance == SPK_MUTE_VALUE) {
#else
					if (rdc_check_valid(max98927, impedance,
							    MAX98927L) ||
					    impedance == SPK_MUTE_VALUE) {
#endif /* OPLUS_ARCH_EXTENDS */
						max98927->ref_RDC[MAX98927L] =
					  ...
					  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!max98927->bIsMax98937[MAX98927L])
		...
		int data;
		max98927_wrapper_read(max98927, 0, MAX98937_Boost_Control_1,
				      &data);
		ucontrol->value.integer.value[0] =
			((data & MAX98937_Boost_Control_1_BST_ILIM_Mask) - 2) >>
			MAX98927_BST_ILIM_SHIFT;
		return 0;
	}
}
	...
	if (i2c_states & MAX98927_CH0) {
		if (!max98927->bIsMax98937[MAX98927L]) {
			regmap_read(max98927->regmap[MAX98927L],
				    MAX98927_Boost_Control_0, &data);
			ucontrol->value.integer.value[0] =
				(data & MAX98927_Boost_Control_0_BST_VOUT_Mask);
		} else {
			regmap_read(max98927->regmap[MAX98927L],
				    MAX98937_Boost_Control_0, &data);
			ucontrol->value.integer.value[0] =
...
static const struct snd_kcontrol_new max98927_snd_controls[] = {
	...
	SOC_SINGLE_EXT("BDE Enable", MAX98937_Brownout_enables, 0, 1, 0,
	...
	SOC_SINGLE_EXT("Amp DSP Enable", MAX98937_Brownout_enables,
		       ...
		       max98927_reg_put_w),
	//0:disable 1:enable
	SOC_SINGLE_EXT("BDE AMP Enable", MAX98937_Brownout_enables, 1, 1, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	//0:disable 1:enable
	...
	SOC_SINGLE_EXT("Level1 Current Limit",
		       MAX98937_Brownout__level_1_current_limit, 0, 63, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 63
	SOC_SINGLE_EXT("Level2 Current Limit",
		       MAX98937_Brownout__level_2_current_limit, 0, 63, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 63
	SOC_SINGLE_EXT("Level3 Current Limit",
		       MAX98937_Brownout__level_3_current_limit, 0, 63, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 63
	SOC_SINGLE_EXT("Level4 Current Limit",
		       MAX98937_Brownout__level_4_current_limit, 0, 63, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 63
	SOC_SINGLE_EXT("Level1 Amp1 Control1",
		       MAX98937_Brownout__level_1_amp_1_control_1, 0, 255, 0,
		       max98927_reg_get_w, max98927_reg_put_w),

	SOC_SINGLE_EXT("Level1 Amp1 Control2",
		       MAX98937_Brownout__level_1_amp_1_control_2, 0, 255, 0,
		       max98927_reg_get_w, max98927_reg_put_w),

	SOC_SINGLE_EXT("Level1 Amp1 Control3",
		       MAX98937_Brownout__level_1_amp_1_control_3, 0, 255, 0,
		       max98927_reg_get_w, max98927_reg_put_w),

	SOC_SINGLE_EXT("Threshold Hysteresis",
		       MAX98937_Brownout_threshold_hysterysis, 0, 255, 0,
		       max98927_reg_get_w, max98927_reg_put_w),

	SOC_SINGLE_EXT("Amp Limiter Attack Release",
		       MAX98937_Brownout_AMP_limiter_attack_release, 0, 255, 0,
		       max98927_reg_get_w, max98927_reg_put_w),
	...
	SOC_ENUM_EXT("Left Boost Output Voltage", max98927_enum[2],
		     max98927_boost_voltage_get_l,
		     max98927_boost_voltage_put_l),

	SOC_ENUM_EXT("Right Boost Output Voltage", max98927_enum[2],
		     max98927_boost_voltage_get_r,
		     max98927_boost_voltage_put_r),

	//boost voltage
	SOC_ENUM_EXT("Left Boost Current Limit", max98927_enum[4],
		     max98927_boost_input_limit_get_l,
		     max98927_boost_input_limit_put_l),

	//boost voltage
	SOC_ENUM_EXT("Right Boost Current Limit", max98927_enum[4],
		     max98927_boost_input_limit_get_r,
		     max98927_boost_input_limit_put_r),
...
#ifdef OPLUS_ARCH_EXTENDS
	snd_soc_add_component_controls(component, ftm_spk_rev_controls,
			     ...
			     int id)
{
	...
	if ((value == RANGE_COUNT) &&
	    !of_property_read_u32_array(dNode, "range_mohms",
					max98927->range_mohms[id], value)) {
	...
	if (presence) {
		...
		if (max98927->dev == NULL) {
			...
			ret = snd_soc_register_component(
				&i2c->dev, &soc_codec_dev_max98927,
