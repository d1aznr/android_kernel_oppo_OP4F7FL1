...
static unsigned int ak4376_reg_read(struct snd_soc_component *component,
				    unsigned int reg);
...
static int ak4376_update_register(struct snd_soc_component *component)
{
	...
	if (ak4376->deviceID == 0x22) {
		for (i = 0; i < ARRAY_SIZE(ak4377a_patch); i++) {
			snd_soc_component_write(component, ak4377a_patch[i].reg,
						ak4377a_patch[i].def);
		}
	} else {
		for (i = 0; i < ARRAY_SIZE(ak4376a_patch); i++) {
			snd_soc_component_write(component, ak4376a_patch[i].reg,
						ak4376a_patch[i].def);
		}
	}
...
#ifdef OPLUS_ARCH_EXTENDS
#ifdef USB_SWITCH_MAX20328
static int ak4376_pdn_control_non_blocked(struct snd_soc_component *component,
					  int pdn)
{
	struct ak4376_priv *ak4376 = snd_soc_component_get_drvdata(component);
	int ret = 0;
...
static ssize_t reg_data_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	int ret, i, fpt;
#ifdef DAC_DEBUG
			      ...
			      const char *buf, size_t count)
{
	...
	for (i = 0; i < pt_count; i += 2) {
		snd_soc_component_write(ak4376_codec, (unsigned int)val[i],
...
static const char *ak4376_ovolcn_select_texts[] = { "Dependent",
						    "Independent" };
...
static const char *ak4376_cpmod_select_texts[] = { "Automatic Switching",
						   "+-VDD Operation",
						   "+-1/2VDD Operation" };
...
static const char *pllmcki_on_select[] = { "9.6MHz", "11.2896MHz", "12.288MHz",
					   "19.2MHz" };
...
static const char *pllmode_on_select[] = { "PLL_OFF", "PLL_BICK_MODE",
					   "PLL_MCKI_MODE", "XTAL_MODE" };
...
static const struct soc_enum ftm_hp_rev_enum =
	SOC_ENUM_SINGLE_EXT(2, ftm_hp_rev_text);
...
static const struct snd_kcontrol_new ak4376_snd_controls[] = {
	...
	SOC_ENUM_EXT("AK4376 PDN Control", ak4376_bitset_enum[4], get_pdn,
		     set_pdn),
	SOC_ENUM_EXT("AK4376 PLL Mode", ak4376_bitset_enum[5], get_pllmode,
		     set_pllmode),
...
static const struct snd_soc_dapm_widget ak4376_dapm_widgets[] = {
...
#ifdef OPLUS_ARCH_EXTENDS
#ifdef USB_SWITCH_MAX20328
	SND_SOC_DAPM_MIXER_E("AK4376 HPR Mixer", AK4376_03_POWER_MANAGEMENT4, 1,
	...
	SND_SOC_DAPM_MIXER_E("AK4376 HPL Mixer", AK4376_03_POWER_MANAGEMENT4, 0,
...
int ak4376_HPLR_HZ(bool HPLR_HZ)
{
	...
	struct ak4376_priv *ak4376 =
		snd_soc_codec_get_drvdata(ak4376_codec_info);
	...
	if (HPLR_HZ) {
		...
		//turn off amp
		ret = ak4376_reg_read(ak4376_codec_info,
				      AK4376_03_POWER_MANAGEMENT4);
		if (ret & 0x3) {
			pr_info("%s: amp still on, turn it off\n", __func__);
			ak4376->amp_pending = true;
			snd_soc_component_update_bits(
				ak4376_codec_info, AK4376_03_POWER_MANAGEMENT4,
				0x03, 0x00);
			msleep(30);
		} else {
		...
		snd_soc_component_update_bits(ak4376_codec_info,
...
unsigned int ak4376_i2c_read(struct snd_soc_component *component,
			     unsigned int reg)
{
	struct ak4376_priv *ak4376 = snd_soc_component_get_drvdata(component);
	int ret = -1;
				    ...
				    unsigned int reg)
{
	...
	if (ak4376->pdn1 == 0) {
		...
		dev_dbg(component->dev,
			"%s Read IC register, reg=0x%x val=0x%x\n",
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_PREPARE:
		if (snd_soc_component_get_bias_level(component) ==
			...
			akdbgprt(
				"\t[AK4376] %s(%d) codec->dapm.bias_level == SND_SOC_BIAS_STANDBY\n",
				__FUNCTION__, __LINE__);

		if (snd_soc_component_get_bias_level(component) ==
		    SND_SOC_BIAS_ON)
			akdbgprt(
				"\t[AK4376] %s(%d) codec->dapm.bias_level >= SND_SOC_BIAS_ON\n",
				__FUNCTION__, __LINE__);
		break;
	case SND_SOC_BIAS_STANDBY:
		if (snd_soc_component_get_bias_level(component) ==
			...
			akdbgprt(
				"\t[AK4376] %s(%d) codec->dapm.bias_level == SND_SOC_BIAS_PREPARE\n",
				__FUNCTION__, __LINE__);
		...
		if (snd_soc_component_get_bias_level(component) ==
		    SND_SOC_BIAS_OFF)
			akdbgprt(
				"\t[AK4376] %s(%d) codec->dapm.bias_level == SND_SOC_BIAS_OFF\n",
				__FUNCTION__, __LINE__);
		break;
...
static int ak4376_set_dai_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (ak4376->nPllMode == 0) {
		if (ak4376->nDACOn == 0) {
			MSmode = snd_soc_component_read32(
				component, AK4376_15_AUDIO_IF_FORMAT);
			if (MSmode & 0x10) { //Master mode
				snd_soc_component_update_bits(
					component, AK4376_15_AUDIO_IF_FORMAT,
...
static struct of_device_id ak4376_i2c_dt_ids[] = { { .compatible =
							     "akm,ak4376" },
						   {} };
			    ...
			    const struct i2c_device_id *id)
{
...
#ifdef OPLUS_ARCH_EXTENDS
	ak4376->priv_pdn_en =
		of_get_named_gpio(i2c->dev.of_node, "ak4376,reset-gpio", 0);
	...
	ret = snd_soc_register_component(&i2c->dev, &soc_codec_dev_ak4376,
					 &ak4376_dai[0],
