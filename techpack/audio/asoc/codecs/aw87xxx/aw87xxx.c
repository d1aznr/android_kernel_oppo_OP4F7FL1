...
static char aw87xxx_cfg_name[AW87XXX_MODE_MAX][AW87XXX_CFG_NAME_MAX] = {
	{ "aw87xxx_pid_xx_off_" },   { "aw87xxx_pid_xx_music_" },
	{ "aw87xxx_pid_xx_voice_" }, { "aw87xxx_pid_xx_fm_" },
	{ "aw87xxx_pid_xx_rcv_" },
};

static char aw87xxx_support_product[][AW87XXX_PRODUCT_NAME_LEN] = {
	{ "aw87339" }, { "aw87359" }, { "aw87369" },
	{ "aw87519" }, { "aw87559" },
};
...
* i2c write and read
**********************************************************/
static int aw87xxx_i2c_write(struct aw87xxx *aw87xxx, unsigned char reg_addr,
			     unsigned char reg_data)
{
	...
	while (cnt < AW_I2C_RETRIES) {
		ret = i2c_smbus_write_byte_data(aw87xxx->i2c_client, reg_addr,
						reg_data);
		if (ret < 0) {
...
static int aw87xxx_i2c_read(struct aw87xxx *aw87xxx, unsigned char reg_addr,
			    unsigned char *reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
...
static void aw87xxx_hw_on(struct aw87xxx *aw87xxx)
{
	...
	if (aw87xxx->product == AW87XXX_339) {
		ret = pinctrl_select_state(aw87xxx->pinctrl,
					   aw87xxx->pins_reset_off);
		if (ret) {
...
static void aw87xxx_hw_reset(struct aw87xxx *aw87xxx)
{
	...
	aw_dev_info(aw87xxx->dev, "%s ,%d, gpio_is_valid(%d) = %d\n", __func__,
		    __LINE__, aw87xxx->reset_gpio,
		    gpio_is_valid(aw87xxx->reset_gpio));
	if (gpio_is_valid(aw87xxx->reset_gpio)) {
		gpio_set_value_cansleep(aw87xxx->reset_gpio, 0);
		mdelay(2);
		aw_dev_info(aw87xxx->dev,
			    "%s ,%d, gpio_get_value_cansleep(%d) = %d\n",
			    __func__, __LINE__, aw87xxx->reset_gpio,
			    gpio_get_value_cansleep(aw87xxx->reset_gpio));
		gpio_set_value_cansleep(aw87xxx->reset_gpio, 1);
		mdelay(2);
		aw_dev_info(aw87xxx->dev,
			    "%s ,%d, gpio_get_value_cansleep(%d) = %d\n",
...
int aw87xxx_set_spk_voltage(int level)
{
	...
	list_for_each (pos, &g_aw87xxx_device_list) {
		...
		if (!aw87xxx->hwen_flag) {
			...
			pr_debug(
				"%s, %d, aw87339_spk is off! curr_spk_voltage = %d\n",
		...
		if (aw87xxx->current_mode != AW87XXX_MUSIC_MODE) {
			pr_debug(
				"%s, %d, aw87xxx->current_mode = %d, no need change!\n",
		...
		// Default
		default:
			pr_warning("%s, %d, aw87xxx_cont->len = %d", __func__,
				   __LINE__, aw87xxx_cont->len);

			switch (aw87xxx->product) {
			case AW87XXX_339:
				if (aw87xxx_cont->len >
				    2 + AW87339_REG_CPOVP * 2 + 1) {
					pr_warning(
						"%s, %d, aw87xxx_cont->data[%d] = %#x",
						__func__, __LINE__,
						2 + AW87339_REG_CPOVP * 2 + 1,
						aw87xxx_cont->data
							[2 +
			...
			for (i = 0; i < aw87xxx_cont->len; i = i + 2) {
				...
				pr_warning(
					"%s, %d, aw87xxx_cont->data[%d] = %#x, aw87xxx_cont->data[%d] = %#x",
					__func__, __LINE__, i,
					aw87xxx_cont->data[i], i + 1,
		...
		pr_debug(
			"%s, %d, level = %d, value = %#x, aw87xxx->hwen_flag = %d",
			__func__, __LINE__, level, value, aw87xxx->hwen_flag);
		...
		pr_warning("%s, %d, curr_spk_voltage = %d, value = %#x",
			   __func__, __LINE__, curr_spk_voltage, value);
	}
...
#ifdef OPLUS_AUDIO_PA_BOOST_VOLTAGE
	if (aw87xxx->current_mode == AW87XXX_MUSIC_MODE) {
		pr_debug("%s, %d, curr_spk_voltage = %d", __func__, __LINE__,
			 curr_spk_voltage);
		...
		if (value != 0) {
			switch (aw87xxx->product) {
			case AW87XXX_339:
				aw87xxx_i2c_write(aw87xxx, AW87339_REG_CPOVP,
						  value);
				break;
			case AW87XXX_359:
				aw87xxx_i2c_write(aw87xxx, AW87359_REG_CPOVP,
						  value);
				break;
			default:
				aw87xxx_i2c_write(aw87xxx, 3, value);
				    ...
				    const struct firmware *cont)
{
	...
	aw_dev_info(aw87xxx->dev, "%s enter, %s_i2c%d@0x%02X\n", __func__,
		    aw87xxx_support_product[aw87xxx->product], aw87xxx->i2c_seq,
		    aw87xxx->i2c_addr);
...
static int
aw87xxx_parse_add_head_firmware(struct aw87xxx *aw87xxx,
				...
				const struct firmware *cont)
{
	...
	current_scene->bin_file = devm_kzalloc(
		aw87xxx->dev, sizeof(struct aw_bin) + cont->size, GFP_KERNEL);
	...
	frame_header = current_scene->frame_header = devm_kzalloc(
		aw87xxx->dev, sizeof(struct ui_frame_header), GFP_KERNEL);
	if (!frame_header) {
		aw_dev_err(aw87xxx->dev, "%s: Error allocating memory\n",
			   __func__);
		return -ENOMEM;
	...
	if (current_scene->bin_file->header_info[0].reg_byte_len !=
		...
		aw_dev_err(
			aw87xxx->dev, "%s: %d bin file type mismatch\n",
			__func__,
			current_scene->bin_file->header_info[0].bin_data_type);
	}
	...
	curr_scene_cnt = current_scene->scene_cont = devm_kzalloc(
		aw87xxx->dev,
	...
	memcpy(curr_scene_cnt->data,
	       cont->data +
		       current_scene->bin_file->header_info[0].valid_data_addr,
	       current_scene->bin_file->header_info[0].valid_data_len);
...
static void
aw87xxx_parse_non_head_firmware(struct aw87xxx *aw87xxx,
				struct aw87xxx_scene_param *current_scene,
				const struct firmware *cont)
{
	int i = 0;
	...
	scene_cont = current_scene->scene_cont = devm_kzalloc(
		aw87xxx->dev, cont->size + sizeof(int), GFP_KERNEL);
	if (!scene_cont) {
		aw_dev_err(aw87xxx->dev, "%s: error! allocating memory\n",
			   __func__);
		return;
...
static int aw87xxx_scene_update(struct aw87xxx *aw87xxx, uint8_t scence_mode)
{
	...
	return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				       ...
				       aw87xxx->dev, GFP_KERNEL, current_scene,
...
static int aw87xxx_vbat_monitor_update(struct aw87xxx *aw87xxx)
{
	aw_dev_info(aw87xxx->dev, "%s enter, dev_i2c%d@0x%02X\n", __func__,
		    aw87xxx->i2c_seq, aw87xxx->i2c_addr);
	return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				       aw87xxx->vmax_cfg_name, aw87xxx->dev,
				       GFP_KERNEL, aw87xxx,
...
{
	struct aw87xxx *aw87xxx =
		container_of(work, struct aw87xxx, ram_work.work);
	...
	if (flag) {
		...
		if (!aw87xxx->actflag_sync_task) {
			aw87xxx->actflag_sync_task =
				kthread_run(aw87xxx_actflag_thread,
					    (void *)aw87xxx, "%s",
					    AW87XXX_I2C_NAME);
	...
	while (!kthread_should_stop()) {
		if (aw87xxx->current_mode == AW87XXX_OFF_MODE ||
			...
			aw_dev_err(
				...
				aw87xxx_support_product[aw87xxx->product],
				aw87xxx->i2c_seq, aw87xxx->i2c_addr,
				aw87xxx->current_mode, aw87xxx->hwen_flag,
				thread_off_cnt);
		} else {
			thread_off_cnt = 0;
			mutex_lock(&aw87xxx->lock);
			...
			if (ret < 0) {
				...
				aw_dev_info(
					aw87xxx->dev,
					"%s, get dsp msg: left_flag: %d, right_flag: %d\n",
					__func__, active_flag[0],
					active_flag[1]);
				err_read_flag = 0;
			}
			ret = aw87xxx_i2c_read(aw87xxx, aw87xxx->gain_reg_addr,
					       &reg_val);
			if (ret < 0) {
				pr_err("%s: failed to read gain register, ret: %d\n",
				       __func__, ret);
				err_read_reg = 1;
			} else {
				aw_dev_info(
					aw87xxx->dev,
					"%s: read gain register true, reg_val:0x%02x\n",
					__func__, reg_val);
				err_read_reg = 0;
			}
			if (!err_read_flag && !err_read_reg) {
				aw_dev_info(
					...
					aw87xxx->i2c_seq, aw87xxx->i2c_addr,
					aw87xxx->pa_channel,
					aw87xxx->current_mode, active_flag[0],
					reg_val,
					aw87xxx->raw_gain_array
						[aw87xxx->current_mode],
					aw87xxx->actflag_gain_array
						[aw87xxx->current_mode]);
				if (active_flag[0]) {
					if (reg_val <
					    aw87xxx->raw_gain_array
...
static struct attribute *aw87xxx_attributes[] = { &dev_attr_reg.attr,
						  &dev_attr_hwen.attr,
				  ...
				  struct device_node *np)
{
	...
	if (aw87xxx->reset_gpio < 0) {
		...
		aw_dev_info(
			aw87xxx->dev,
			"%s: reset gpio aw87xxx->reset_gpio = %d provided ok\n",
				    ...
				    struct device_node *np)
{
	...
	if (ret != 0) {
		...
		for (i = 0; i < 5; i++)
			aw_dev_info(
				...
				__func__, i, aw87xxx->actflag_gain_array[i]);
	}
	aw87xxx->actflag_auto_run =
	...
	ret = of_property_read_u32(np, "gain-reg-addr",
				   &aw87xxx->gain_reg_addr);
	if (ret != 0) {
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (aw87xxx->pa_channel == 0) {
		aw87xxx->pins_reset_on =
			pinctrl_lookup_state(aw87xxx->pinctrl, "spk_reset_on");
		if (IS_ERR_OR_NULL(aw87xxx->pins_reset_on)) {
			dev_err(&client->dev, "%s: loockup failed\n", __func__);
		}

		aw87xxx->pins_reset_off =
			pinctrl_lookup_state(aw87xxx->pinctrl, "spk_reset_off");
		if (IS_ERR_OR_NULL(aw87xxx->pins_reset_off)) {
			dev_err(&client->dev, "%s: loockup failed\n", __func__);
		}
	} else if (aw87xxx->pa_channel == 1) {
		aw87xxx->pins_reset_on =
			pinctrl_lookup_state(aw87xxx->pinctrl, "rcv_reset_on");
		if (IS_ERR_OR_NULL(aw87xxx->pins_reset_on)) {
			dev_err(&client->dev, "%s: loockup failed\n", __func__);
		}

		aw87xxx->pins_reset_off =
			pinctrl_lookup_state(aw87xxx->pinctrl, "rcv_reset_off");
		if (IS_ERR_OR_NULL(aw87xxx->pins_reset_off)) {
			dev_err(&client->dev, "%s: loockup failed\n", __func__);
		}
	}
	...
	if (ret) {
		dev_err(&client->dev, "%s: pinctrl_select_state failed\n",
			__func__);
	}
	mdelay(2);
...
#else
	if (gpio_is_valid(aw87xxx->reset_gpio)) {
		ret = devm_gpio_request_one(&client->dev, aw87xxx->reset_gpio,
					    GPIOF_OUT_INIT_LOW, "aw87xxx_rst");
		if (ret) {
			aw_dev_err(&client->dev, "%s: rst request failed\n",
				   __func__);
    ...
    */
	...
	if (ret < 0) {
		aw_dev_err(&client->dev, "%s: failed to create sysfs nodes\n",
			   __func__);
	}
...
static const struct i2c_device_id aw87xxx_i2c_id[] = { { AW87XXX_I2C_NAME, 0 },
						       {} };
