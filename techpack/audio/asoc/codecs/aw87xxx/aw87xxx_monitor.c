			...
			int channel)
{
	...
	ret = aw_write_data_to_dsp(INDEX_PARAMS_ID_AWDSP_RX_MSG, cmd_msg,
				   sizeof(cmd_msg), channel);
	if (ret < 0) {
	...
	ret = aw_read_data_from_dsp(INDEX_PARAMS_ID_AWDSP_RX_MSG, data_ptr,
				    data_size, channel);
	if (ret < 0) {
	...
	for (i = 0; i < monitor->vmax_cfg->vmax_cfg_num; i++) {
		if (vbat_vol > monitor->vmax_cfg->vmax_cfg_total[i].min_thr) {
			...
			aw_dev_dbg(
				...
				__func__, vmax_set, i,
				monitor->vmax_cfg->vmax_cfg_total[i].min_thr);
			break;
		}
	}
	aw_dev_info(
...
void aw87xxx_monitor_work_func(struct work_struct *work)
{
	...
	struct aw87xxx *aw87xxx =
		container_of(work, struct aw87xxx, monitor.work.work);
	...
	if ((aw87xxx->monitor.timer_cnt == aw87xxx->monitor.timer_cnt_max) ||
		...
		ave_capacity =
			aw87xxx->monitor.vbat_sum / aw87xxx->monitor.timer_cnt;
				...
				struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "vbat capacity=%d\n",
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = aw_read_data_from_dsp(INDEX_PARAMS_ID_RX_VMAX, &vmax_get,
				    sizeof(uint32_t), aw87xxx->pa_channel);
	if (ret)
		aw_dev_err(aw87xxx->dev, "%s: get dsp vmax fail, ret=%d\n",
			   __func__, ret);
	else
		len += snprintf(buf + len, PAGE_SIZE - len, "get_vmax=0x%x\n",
				vmax_get);
				...
				size_t len)
{
	...
	ret = aw_write_data_to_dsp(INDEX_PARAMS_ID_RX_VMAX, &vmax_set,
				   sizeof(uint32_t), aw87xxx->pa_channel);
	if (ret)
				   ...
				   const char *buf, size_t count)
{
	...
	if (aw87xxx->monitor.monitor_flag &&
		...
		schedule_delayed_work(
			&aw87xxx->monitor.work,
			msecs_to_jiffies(aw87xxx->monitor.timer_val));
	return count;
}

static ssize_t aw87xxx_get_rx(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
	ssize_t len = 0;
	...
	ret = aw_read_data_from_dsp(INDEX_PARAMS_ID_RX_ENBALE, &enable,
				    sizeof(uint32_t), aw87xxx->pa_channel);
	if (ret)
		aw_dev_err(aw87xxx->dev, "%s: dsp_msg error, ret=%d\n",
			   __func__, ret);
	else
		len += snprintf(buf + len, PAGE_SIZE - len, "aw87xxx rx: %d\n",
				enable);
	return len;
}

static ssize_t aw87xxx_set_rx(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
	int ret = -1;
...
static DEVICE_ATTR(vmax_time, S_IWUSR | S_IRUGO, aw87xxx_get_vmax_time,
		   aw87xxx_set_vmax_time);

static struct attribute *aw87xxx_monitor_attr[] = {
	&dev_attr_vbat.attr, &dev_attr_vmax.attr,      &dev_attr_monitor.attr,
	&dev_attr_rx.attr,   &dev_attr_vmax_time.attr, NULL
};
