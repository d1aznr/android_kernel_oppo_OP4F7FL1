...
static inline void
sia81xx_process_one_timer_callback(sia81xx_timer_proc_info_t *proc,
				   uint32_t time, int is_first)
{
	int ret = 0;
	...
	if (0 != ret) {
		pr_err("[  err][%s] %s: %s run with ret : %d \r\n", LOG_FLAG,
		       __func__, proc->name, ret);
	}

	proc->run_time_ms = 0;
}

static int sia81xx_timer_task_processor(void *data)
{
	...
	while (SIA81XX_TASK_ENABLE == atomic_read(&timer->task_state)) {
		...
		for (i = 0; i < ARRAY_SIZE(timer->proc); i++) {
			...
			sia81xx_process_one_timer_callback(
				&timer->proc[i], TIMER_TASK_CYCLE_TIME_MS,
				switch_flag);
		}
...
static void sia81xx_timer_task_record_proc_info(
	sia81xx_timer_proc_info_t *proc, const char *name, uint32_t user_id,
	uint32_t wake_up_interval_ms, int (*process)(int is_first, void *data),
	void *data)
{
	...
	if (NULL == name) {
		...
		if ((strlen(name) < sizeof(proc->name)) &&
		    (0 != strlen(name))) {
...
int sia81xx_timer_task_register(uint32_t hdl, const char *name,
				uint32_t user_id, uint32_t wake_up_interval_ms,
				int (*process)(int is_first, void *data),
				void *data)
{
	int i = 0;
	...
	for (i = 0; i < ARRAY_SIZE(timer->proc); i++) {
		if (INVALID_USER_ID == timer->proc[i].user_id) {
			sia81xx_timer_task_record_proc_info(&timer->proc[i],
							    name, user_id,
							    wake_up_interval_ms,
							    process, data);
			...
			pr_debug("[debug][%s] %s: i(%d), hdl(%u), "
				 ...
				 LOG_FLAG, __func__, i, (unsigned int)hdl,
...
int sia81xx_timer_task_unregister(uint32_t hdl, uint32_t user_id)
{
	sia81xx_timer_task_t *timer = NULL;
	int i = 0;
...
int sia81xx_timer_task_start(uint32_t hdl)
{
	...
	timer->task = kthread_create(sia81xx_timer_task_processor, timer,
				     "sia81xx_timer_task_processor");
