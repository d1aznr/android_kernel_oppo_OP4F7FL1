...
static uint32_t sia81xx_read_cur_battery_voltage(void)
{
	...
	if (0 !=
	    psy->desc->get_property(psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &val)) {
		vdd_val = VDD_DEFAULT_VAL;
		goto end;
	}
#else
	...
	pr_debug("[debug][%s] %s: current voltage = %u \r\n", LOG_FLAG,
		 __func__, (unsigned int)vdd_val);

	return vdd_val;
}

static void
sia81xx_record_cur_battery_voltage(struct sia81xx_set_vdd_info *info)
{
...
static uint32_t sia81xx_get_battery_voltage(struct sia81xx_set_vdd_info *info,
					    uint32_t samples)
{
	uint64_t ave_val = 0;
	uint32_t n = 0, pos = 0;
	...
	pr_debug("[debug][%s] %s: average vdd = %llu, n = %u \r\n", LOG_FLAG,
		 __func__, ave_val, (unsigned int)n);

	return (uint32_t)ave_val;
}

static struct sia81xx_set_vdd_info *is_cal_id_exist(uint32_t timer_task_hdl,
						    uint32_t channel_num)
{
	int i = 0;
...
static struct sia81xx_set_vdd_info *
get_one_can_use_info(uint32_t timer_task_hdl, uint32_t channel_num)
{
	...
	for (i = 0; i < MAX_SET_VDD_INFO_NUM; i++) {
		if ((SIA81XX_TIMER_TASK_INVALID_HDL ==
		     info_table[i].timer_task_hdl) &&
...
static struct sia81xx_set_vdd_info *record_info(uint32_t timer_task_hdl,
						uint32_t channel_num,
...
static void delete_all_info(void)
{
	...
	for (i = 0; i < MAX_SET_VDD_INFO_NUM; i++) {
		if (info_table[i].timer_task_hdl <
		    SIA81XX_TIMER_TASK_INVALID_HDL) {
			if (0 != info_table[i].cal_handle) {
				if (0 != tuning_if_opt.close(
						 info_table[i].cal_handle)) {
...
static void send_set_vdd_msg(struct sia81xx_set_vdd_info *info, uint32_t vdd,
			     uint32_t channel_num)
{
	int ret = 0;
	...
	ret = tuning_if_opt.write(info->cal_handle, info->module_id,
				  info->param_id, (uint32_t)sizeof(param),
				  (uint8_t *)&param);
...
static int sia81xx_open_set_vdd_server(uint32_t timer_task_hdl,
				       uint32_t channel_num, uint32_t cal_id)
{
	...
	if (NULL == (info = record_info(timer_task_hdl, channel_num, cal_handle,
					cal_id))) {
...
static int sia81xx_auto_set_timer_task_callback(int is_first, void *data)
{
	struct sia81xx_set_vdd_info *info = NULL;
	if (NULL == data) {
	...
	if (1 == info->is_enable) {
		send_set_vdd_msg(info,
				 sia81xx_get_battery_voltage(
					 info, AUTO_SET_FIRST_SET_SAMPLES),
				 info->channel_num);
	}
...
void sia81xx_set_auto_set_vdd_work_state(uint32_t timer_task_hdl,
					 uint32_t channel_num, uint32_t state)
{
...
int sia81xx_auto_set_vdd_probe(uint32_t timer_task_hdl, uint32_t channel_num,
			       uint32_t cal_id, uint32_t state)
{
	int ret = 0, i = 0;
	...
	struct sia81xx_set_vdd_info *pInfo =
		is_cal_id_exist(timer_task_hdl, channel_num);
	if (NULL == pInfo) {
		ret = sia81xx_open_set_vdd_server(timer_task_hdl, channel_num,
						  cal_id);
		if (0 != ret) {
	...
	ret = sia81xx_timer_task_register(timer_task_hdl, TIMER_TASK_PROC_NAME,
					  channel_num,
...
int sia81xx_auto_set_vdd_remove(uint32_t timer_task_hdl, uint32_t channel_num)
{
...
int sia81xx_set_vdd_init(void)
{
	...
	pr_info("[ info][%s] %s: run !! ", LOG_FLAG, __func__);
