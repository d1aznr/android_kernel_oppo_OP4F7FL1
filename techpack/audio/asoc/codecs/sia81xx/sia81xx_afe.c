...
static int sia81xx_fill_apr_hdr(struct apr_hdr *apr_hdr, uint32_t port,
...
static int32_t sia81xx_afe_callback(struct apr_client_data *data, void *priv)
{
...
static int sia81xx_apr_send(SIA81XX_AFE *afe, void *cmd)
{
	int ret = 0;
...
static int check_apr(SIA81XX_AFE *afe)
{
	int ret = 0;
...
static int check_cal_block(SIA81XX_AFE *afe)
{
	int ret = 0;
...
static int check_afe_port_id(SIA81XX_AFE *afe)
{
	int ret = 0;
...
static inline int chack_afe_validity(SIA81XX_AFE *afe)
{
	if (NULL == afe) {
...
static void
sia81xx_write_param_to_payload(afe_cmd_sia81xx_param_payload *payload,
...
static int
sia81xx_read_param_from_payload(afe_cmd_sia81xx_param_payload *payload,
				uint32_t module_id, uint32_t param_id,
				uint32_t param_size, uint8_t *param)
{
...
static int sia81xx_apr_cmd_memory_unmap(SIA81XX_AFE *afe)
{
	int ret = 0;
	...
	/* fill cmd head */
	sia81xx_fill_apr_hdr(&(apr_cmd.hdr),
			     afe_port_to_apr_port(afe->afe_port_id),
...
static SIA81XX_AFE *find_sia81xx_afe_list(uint16_t afe_port_id)
{
	SIA81XX_AFE *afe = NULL;
...
static void add_sia81xx_afe_list(SIA81XX_AFE *afe)
{
	if (NULL == afe) {
		pr_err("[  err][%s] %s: NULL == afe \r\n", LOG_FLAG, __func__);
		return;
	...
	pr_debug("[debug][%s] %s: add afe port id : %u, count = %u \r\n",
		 LOG_FLAG, __func__, afe->afe_port_id,
		 sia81xx_afe_list_count());
}

static void del_sia81xx_afe_list(SIA81XX_AFE *afe)
{
	if (NULL == afe) {
		pr_err("[  err][%s] %s: NULL == afe \r\n", LOG_FLAG, __func__);
		return;
...
int sia81xx_afe_send(unsigned long afe_handle, uint32_t module_id,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
	/* fill cmd head */
	sia81xx_fill_apr_hdr((struct apr_hdr *)(&apr_cmd.apr_hdr),
			     afe_port_to_apr_port(afe->afe_port_id),
...
#else
	/* fill cmd head */
	sia81xx_fill_apr_hdr((struct apr_hdr *)(&apr_cmd.hdr),
			     afe_port_to_apr_port(afe->afe_port_id),
			     AFE_PORT_CMD_SET_PARAM_V2, sizeof(apr_cmd));
...
int sia81xx_afe_read(unsigned long afe_handle, uint32_t module_id,
		     uint32_t param_id, uint32_t param_size, uint8_t *buf)
{
	int ret = 0;
...
struct sia81xx_cal_opt tuning_if_opt = { .init = sia81xx_afe_init,
