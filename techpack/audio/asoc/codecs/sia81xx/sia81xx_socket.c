...
static struct cal_map *is_cal_id_exist(uint32_t cal_id)
{
	int i = 0;
...
static struct cal_map *get_one_can_use_cal_map(uint32_t cal_id)
{
...
static int record_cal_map(unsigned long cal_handle, uint32_t cal_id)
{
	struct cal_map *map = NULL;
...
static int sia81xx_sock_write(struct socket *sock, char *buf, int len)
{
	struct kvec vec;
	struct msghdr msg;
...
static int sia81xx_sock_read(struct socket *sock, char *buf, int len)
{
	struct kvec vec;
	struct msghdr msg;
...
static int sia81xx_sock_proc(struct socket *sock, char *buf, int len,
			     int max_len)
{
	int ret = 0;
	...
	case OPT_SET_CAL_VAL: {
		...
		if (0 > ret) {
			...
			pr_info("[ info][%s] %s: write module_id = 0x%08x, "
				...
				"payload = %u, ret = %d \r\n",
				LOG_FLAG, __func__, cal->module_id,
				cal->param_id, cal->payload_size,
				cal->payload[0], ret);
	...
	case OPT_GET_CAL_VAL: {
		...
		if (ret > cal->payload_size) {
			...
			cal->rep_code = REP_CODE_OK;
			ret = sia81xx_sock_write(sock, buf,
						 CAL_PACKET_LEN(cal));
		}
		break;
	}
	...
	pr_debug("[debug][%s] %s done with opt = 0x%08x, cal_id = 0x%08x, "
		 ...
		 LOG_FLAG, __func__, cal->opt, cal->cal_id, cal->rep_code,
		 cal->module_id, cal->param_id);

	return ret;
}

static int sia81xx_sock_server_task(void *data)
{
	int ret = 0;
	...
	while (TASK_STATE_RUN == sock_task_state) {
		ret = sia81xx_sock_read(sock_clt, comm_buf, sizeof(comm_buf));
		...
		ret = sia81xx_sock_proc(sock_clt, comm_buf, ret,
					sizeof(comm_buf));
		if (0 > ret) {
...
static int sia81xx_open_sock_task(void)
{
	...
	socket_task = kthread_create(sia81xx_sock_server_task, NULL,
				     "sia81xx_socket");
	if (NULL == socket_task) {
...
static int sia81xx_close_sock_task(void)
{
	...
	pr_debug("[debug][%s] %s: use wait time %d ms \r\n", LOG_FLAG, __func__,
		 (RECV_WATI_TIME_MS * 10) - max_wait_time_ms);
...
int sia81xx_open_sock_server()
{
	...
	kernel_setsockopt(sock_srv, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv,
			  sizeof(tv));
