...
const struct sia81xx_reg_default_val sia8101_reg_default_val = {
	...
	.reg_defaults[AUDIO_SCENE_PLAYBACK] = { .vals = (char *)
							sia8101_palyback_defaults,
						.num = ARRAY_SIZE(
							sia8101_palyback_defaults
								[0]) },
	.reg_defaults[AUDIO_SCENE_VOICE] = { .vals = (char *)
						     sia8101_voice_defaults,
					     .num = ARRAY_SIZE(
						     sia8101_voice_defaults[0]) }
};

static bool sia8101_writeable_register(struct device *dev, unsigned int reg)
{
	switch (reg) {
...
static bool sia8101_readable_register(struct device *dev, unsigned int reg)
{
	switch (reg) {
...
static bool sia8101_volatile_register(struct device *dev, unsigned int reg)
{
	return true;
}
...
static int sia8101_check_chip_id(struct regmap *regmap)
{
	int val = 0;
...
static void sia8101_set_xfilter(struct regmap *regmap, unsigned int val)
{
