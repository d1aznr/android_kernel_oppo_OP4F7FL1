...
const struct sia81xx_reg_default_val sia8108_reg_default_val = {
	...
	.reg_defaults[AUDIO_SCENE_PLAYBACK] = { .vals = (char *)
							sia8108_palyback_defaults,
						.num = ARRAY_SIZE(
							sia8108_palyback_defaults
								[0]) },
	.reg_defaults[AUDIO_SCENE_VOICE] = { .vals = (char *)
						     sia8108_voice_defaults,
					     .num = ARRAY_SIZE(
						     sia8108_voice_defaults[0]) }
};

static bool sia8108_writeable_register(struct device *dev, unsigned int reg)
{
	switch (reg) {
...
static bool sia8108_readable_register(struct device *dev, unsigned int reg)
{
	switch (reg) {
	case SIA8108_REG_SYSCTRL ... SIA8108_REG_Excur_CTRL_4:
...
static bool sia8108_volatile_register(struct device *dev, unsigned int reg)
{
	return true;
}
...
static int sia8108_check_chip_id(struct regmap *regmap)
{
	int val = 0;
...
static void sia8108_set_xfilter(struct regmap *regmap, unsigned int val)
{
	const char buf = 0x10;
