...
static int sia81xx_resume(struct sia81xx_dev_s *sia81xx);
static int sia81xx_suspend(struct sia81xx_dev_s *sia81xx);
static unsigned int get_chip_type(const char *name);
static unsigned int distinguish_chip_type(sia81xx_dev_t *sia81xx)
{
	struct file *fp = NULL;
	char chip_name[32];
	...
	if (!IS_ERR(fp)) {
		...
		if (0 >= kernel_read(fp, 0, chip_name, sizeof(chip_name)))
			...
			pr_debug(
				"[debug][%s] %s: read chip type finished, chip_name = %s\r\n",
...
static int sia81xx_set_mode(struct sia81xx_dev_s *sia81xx, unsigned int mode);
static void set_owi_mod_by_chip_type(sia81xx_dev_t *sia81xx, int chip_type)
{
	int ret = 0;
	...
	pr_info("[ info][%s] %s: set mod : %u, return : %d \r\n", LOG_FLAG,
		__func__, sia81xx->owi_cur_mode, ret);
}
...
static int sia81xx_owi_write_one_bit(struct sia81xx_dev_s *sia81xx,
				     const char bit, s64 *real_duty_ns,
				     s64 *real_idle_ns)
{
	int ret = 0;
	...
	ret = __gpio_produce_one_pulse_cycle(sia81xx->owi_pin,
					     sia81xx->owi_polarity,
					     duty_time_us, idle_time_us,
					     real_duty_ns, real_idle_ns);

	return ret;
}

static int sia81xx_owi_start_write_byte(struct sia81xx_dev_s *sia81xx)
{
...
static void sia81xx_owi_end_write_byte(struct sia81xx_dev_s *sia81xx,
				       s64 *real_idle_ns)
{
	gpio_flipping(sia81xx->owi_pin, real_idle_ns);
}

static s64 sia81xx_owi_calc_deviation_ns(struct sia81xx_dev_s *dev,
					 s64 duty_time_ns, s64 idle_time_ns,
					 const char bit)
{
	s64 deviation = 0;
...
static int sia81xx_owi_write_one_byte(struct sia81xx_dev_s *sia81xx,
				      const char data)
{
	int ret = 0, i = 0;
	...
	for (i = 7; i >= 0; i--) {
		...
		if (7 > i) {
			cur_deviation_ns = sia81xx_owi_calc_deviation_ns(
				sia81xx, last_real_duty_ns, real_idle_ns,
				last_bit);
	...
	if (0 == cur_deviation_ns) {
		...
		cur_deviation_ns = sia81xx_owi_calc_deviation_ns(
			sia81xx, last_real_duty_ns, real_idle_ns, last_bit);
		max_deviation_ns = max_deviation_ns > cur_deviation_ns ?
					   max_deviation_ns :
					   cur_deviation_ns;
	}
...
static int sia81xx_owi_write_data(struct sia81xx_dev_s *sia81xx,
...
static void sia81xx_set_owi_polarity(struct sia81xx_dev_s *sia81xx)
{
	gpio_set_value(sia81xx->owi_pin, sia81xx->owi_polarity);
}

static int __sia81xx_owi_write_mode(struct sia81xx_dev_s *sia81xx,
				    unsigned int cycles)
{
	int ret = 0;
...
static int sia81xx_owi_write_mode(struct sia81xx_dev_s *sia81xx,
				  unsigned int mode)
{
...
static int sia81xx_owi_init(sia81xx_dev_t *sia81xx, unsigned int owi_mode)
{
 ...
 * sia81xx chip option
 ********************************************************************/
static int sia81xx_reg_init(struct sia81xx_dev_s *sia81xx)
{
	if (NULL == sia81xx->client)
		return 0;

	if (0 !=
	    sia81xx_regmap_check_chip_id(sia81xx->regmap, sia81xx->chip_type)) {
		pr_err("[  err][%s] %s: sia81xx_regmap_check_chip_id error !!! \r\n",
		       LOG_FLAG, __func__);
	}

	udelay(10); /* wait for xfilter ready */
	sia81xx_regmap_defaults(sia81xx->regmap, sia81xx->chip_type,
				sia81xx->scene, sia81xx->channel_num);

	udelay(10);
	sia81xx_regmap_set_xfilter(sia81xx->regmap, sia81xx->chip_type,
				   sia81xx->en_xfilter);
...
static int sia81xx_reboot(struct sia81xx_dev_s *sia81xx)
{
	int ret = 0;
...
static int sia81xx_dev_init(sia81xx_dev_t *sia81xx,
			    struct device_node *sia81xx_of_node)
{
	...
	ret = of_property_read_u32(sia81xx_of_node, "channel_num",
				   &channel_num);
	if (0 != ret) {
		channel_num = 0;
	}
	...
	ret = of_property_read_u32(sia81xx_of_node, "timer_task_hdl",
				   &timer_task_hdl);
	if (0 != ret) {
		timer_task_hdl = SIA81XX_TIMER_TASK_INVALID_HDL;
	}

	ret = of_property_read_u32(sia81xx_of_node, "en_dynamic_updata_vdd",
				   &en_dyn_ud_vdd);
	if ((0 != ret) || (1 != en_dyn_ud_vdd)) {
		en_dyn_ud_vdd = 0;
	}

	ret = of_property_read_u32(sia81xx_of_node, "dynamic_updata_vdd_port",
				   &dyn_ud_vdd_port);
	if (0 != ret) {
...
static int sia81xx_dev_remove(sia81xx_dev_t *sia81xx)
{
	pr_debug("[debug][%s] %s: running \r\n", LOG_FLAG, __func__);
...
static int sia81xx_algo_en_write(struct sia81xx_dev_s *sia81xx, int32_t enable)
{
...
static int sia81xx_algo_en_read(struct sia81xx_dev_s *sia81xx, int32_t *enable)
{
	unsigned long cal_handle = 0;
				...
				struct device_attribute *attr, char *buf)
{
	...
	sia81xx_regmap_read(sia81xx->regmap, 0x41, 1, &chip_id);
	sia81xx_regmap_read(sia81xx->regmap, 0x00, 0x0D + 1, vals);
	...
	snprintf(
		tb, 1024,
		"sia81xx_cmd_show : owi pin : %d, chip id = 0x%02x \r\n"
		...
		owi_pin_val, chip_id, vals[0], vals[1], vals[2], vals[3],
		vals[4], vals[5], vals[6], vals[7], vals[8], vals[9], vals[10],
		vals[11], vals[12], vals[13], sia81xx->rst_pin,
		sia81xx->owi_pin, sia81xx->err_info.owi_set_mode_cnt,
		sia81xx->err_info.owi_set_mode_err_cnt,
		sia81xx->err_info.owi_polarity_err_cnt,
		sia81xx->err_info.owi_max_retry_cnt,
		sia81xx->err_info.owi_write_err_cnt, sia81xx->owi_delay_us,
		sia81xx->err_info.owi_max_gap,
		sia81xx->err_info.owi_max_deviation,
		sia81xx->err_info.owi_write_data_err_cnt,
		sia81xx->err_info.owi_write_data_cnt);
				 ...
				 size_t len)
{
	...
	case SIA81XX_CMD_SET_MODE: {
		sia81xx_set_mode(sia81xx,
				 simple_strtoul(strsep(&cur, split_symb),
						&after, 10));
		break;
	}
	case SIA81XX_CMD_GET_REG: {
		...
		if (0 != sia81xx_regmap_read(sia81xx->regmap, addr, 1, &val)) {
			...
			pr_debug(
				"[debug][%s] %s: addr = 0x%02x, val = 0x%02x \r\n",
	...
	case SIA81XX_CMD_SET_REG: {
		unsigned char addr = (unsigned int)simple_strtoul(
			strsep(&cur, split_symb), &after, 16);
		char val = (char)simple_strtoul(strsep(&cur, split_symb),
						&after, 16);
	...
	case SIA81XX_CMD_SET_OWI_DELAY: {
		...
		if (temp_us <
		    1000) /* only for test, pulse width must be < 1ms */
	...
	case SIA81XX_CMD_VDD_SET_OPEN: {
		...
		sia81xx_auto_set_vdd_probe(sia81xx->timer_task_hdl,
					   sia81xx->channel_num,
					   sia81xx->dyn_ud_vdd_port,
					   sia81xx->en_dyn_ud_vdd);
		pr_debug(
			"[debug][%s] %s: set auto vdd state %u, port 0x%04x \r\n",
			LOG_FLAG, __func__, sia81xx->en_dyn_ud_vdd,
			sia81xx->dyn_ud_vdd_port);
		break;
	}
	case SIA81XX_CMD_VDD_SET_CLOSE: {
		...
		sia81xx_set_auto_set_vdd_work_state(sia81xx->timer_task_hdl,
						    sia81xx->channel_num,
						    sia81xx->en_dyn_ud_vdd);

		sia81xx_auto_set_vdd_remove(sia81xx->timer_task_hdl,
					    sia81xx->channel_num);
	...
	case SIA81XX_CMD_ALGORITHM_STATUS: {
		if (0 != sia81xx_algo_en_read(sia81xx, &sia81xx->algo_en)) {
			pr_debug(
				"[debug][%s] %s: err sia81xx_algo_en_read \r\n",
				LOG_FLAG, __func__);
		} else {
			pr_debug("[debug][%s] %s: algo_en = %d \r\n", LOG_FLAG,
				 __func__, sia81xx->algo_en);
		}
		break;
	}
	case SIA81XX_CMD_GET_CHIP_TYPE: {
		...
		for (i = 0; i < CHIP_TYPE_UNKNOWN; i++) {
			...
			if (0 ==
			    sia81xx_regmap_check_chip_id(sia81xx->regmap, i)) {
				sprintf(chip_name, "%s",
	...
	case 901: {
		...
		fp = filp_open(
			"/sys/bus/platform/devices/sia81xx@L/sia81xx_cmd",
			O_RDWR, 0666);
		if (IS_ERR(fp)) {
			pr_err("[  err][%s] %s: create file error : %ld \r\n",
			       LOG_FLAG, __func__, PTR_ERR(fp));
 ...
 ********************************************************************/
static int sia81xx_power_get(struct snd_kcontrol *kcontrol,
...
static int sia81xx_power_set(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	struct snd_soc_component *component =
	...
	pr_debug("[debug][%s] %s: ucontrol = %ld, rst = %d \r\n", LOG_FLAG,
		 __func__, ucontrol->value.integer.value[0], sia81xx->rst_pin);
...
static int sia81xx_audio_scene_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	struct snd_soc_component *component =
...
static int sia81xx_audio_scene_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	struct snd_soc_component *component =
...
static int sia81xx_algo_en_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	struct snd_soc_component *component =
...
static int sia81xx_algo_en_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	struct snd_soc_component *component =
	...
	if (0 >=
	    sia81xx_algo_en_write(sia81xx, ucontrol->value.integer.value[0]))
		return -EINVAL;
...
static int sia81xx_volme_boost_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	struct snd_soc_component *component =
	...
	if (0 != sia81xx_regmap_read(sia81xx->regmap, addr, 1, &val)) {
		...
		for (i = 0; i < vol_length; i++) {
			if (val == voltage[i]) {
				...
				pr_debug(
					"[debug][%s] %s: addr = 0x%02x, val = 0x%x \r\n",
					LOG_FLAG, __func__, addr, val);
			}
		}
...
static int sia81xx_volme_boost_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	struct snd_soc_component *component =
	...
	pr_debug("[debug][%s] %s: ucontrol = %ld", LOG_FLAG, __func__,
		 ucontrol->value.integer.value[0]);
	if (0 != sia81xx_regmap_write(sia81xx->regmap, addr, 1, &val)) {
		pr_err("[err][%s] %s: regmap_write \r\n", LOG_FLAG, __func__);
	}
...
#ifdef OPLUS_ARCH_EXTENDS
#ifdef OPLUS_SIA_BOOST_VOL
static const char *const volume_boost[] = { "Voltage8_5", "Voltage7",
					    "Voltage6_5", "Voltage5" };
#endif /* OPLUS_SIA_BOOST_VOL */
#endif /* OPLUS_ARCH_EXTENDS */
...
static const struct snd_kcontrol_new sia81xx_controls[] = {
	SOC_ENUM_EXT("Sia81xx Power", power_enum, sia81xx_power_get,
		     sia81xx_power_set),
	SOC_ENUM_EXT("Sia81xx Alog", algo_enum, sia81xx_algo_en_get,
		     sia81xx_algo_en_set),
...
static int sia81xx_spkr_pa_event(struct snd_soc_dapm_widget *w,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
static int sia81xx_component_probe(struct snd_soc_component *component)
{
	pr_info("[debug][%s] %s: running \r\n", LOG_FLAG, __func__);
	return 0;
}

static void sia81xx_component_remove(struct snd_soc_component *component)
{
	pr_info("[debug][%s] %s: running \r\n", LOG_FLAG, __func__);
	return;
}
#else
static int sia81xx_codec_probe(struct snd_soc_codec *codec)
{
	pr_debug("[debug][%s] %s: running \r\n", LOG_FLAG, __func__);
	return 0;
}

static int sia81xx_codec_remove(struct snd_soc_codec *codec)
{
	pr_debug("[debug][%s] %s: running \r\n", LOG_FLAG, __func__);
	return 0;
}
...
static sia81xx_dev_t *find_sia81xx_dev(const char *name,
...
static void add_sia81xx_dev(sia81xx_dev_t *sia81xx)
{
...
static void del_sia81xx_dev(sia81xx_dev_t *sia81xx)
{
	if (NULL == sia81xx) {
		pr_err("[  err][%s] %s: NULL == sia81xx \r\n", LOG_FLAG,
		       __func__);
...
static sia81xx_dev_t *get_sia81xx_dev(struct device *dev,
				      struct device_node *sia81xx_of_node)
{
	int ret = 0;
	sia81xx_dev_t *sia81xx = NULL;
	...
	ret = of_property_read_string_index(dev->of_node, "si,sia81xx-dev-name",
					    0, &sia81xx_dev_name);
	if (0 != ret) {
...
static void put_sia81xx_dev(sia81xx_dev_t *sia81xx)
{
	...
	pr_debug(
		"[debug][%s] %s: put dev name : %s, pdev = %p, client = %p \r\n",
		LOG_FLAG, __func__, sia81xx->name, sia81xx->pdev,
		sia81xx->client);
			     ...
			     const struct i2c_device_id *id)
{
	...
	sia81xx_of_node =
		of_parse_phandle(client->dev.of_node, "si,sia81xx-dev", 0);

	/* get chip type name */
	ret = of_property_read_string_index(sia81xx_of_node, "si,sia81xx_type",
					    0, &chip_type_name);
	if (0 != ret) {
...
static int sia81xx_i2c_remove(struct i2c_client *client)
{
	int ret = 0;
	sia81xx_dev_t *sia81xx = NULL;
...
static const struct i2c_device_id si_sia81xx_i2c_id[] = { { SIA81XX_I2C_NAME,
							    0 },
							  {} };
...
static int sia81xx_pm_suspend(struct device *dev)
{
#if 0
	sia81xx_dev_t *sia81xx = NULL;
...
static const struct dev_pm_ops si_sia81xx_pm_ops = { SET_SYSTEM_SLEEP_PM_OPS(
	sia81xx_pm_suspend, sia81xx_pm_resume) };
...
static int sia81xx_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_string_index(
		pdev->dev.of_node, "si,sia81xx_type", 0, &chip_type_name);
	if (0 != ret) {
		pr_err("[  err][%s] %s: get si,sia81xx_type return %d !!! \r\n",
		       LOG_FLAG, __func__, ret);
		return -ENODEV;
	...
	ret = of_property_read_u32(pdev->dev.of_node, "si,sia81xx_disable_pin",
				   &disable_pin);
	if (0 != ret) {
	...
	if (0 == disable_pin) {
		...
		pinctrl_state =
			pinctrl_lookup_state(sia81xx_pinctrl, "sia81xx_gpio");
		...
		if (0 != (ret = pinctrl_select_state(sia81xx_pinctrl,
						     pinctrl_state))) {
		...
		/* get reset pin's sn */
		rst_pin = of_get_named_gpio(pdev->dev.of_node,
					    "si,sia81xx_reset", 0);
		if (rst_pin < 0) {
		...
		/* get owi pin's sn */
		owi_pin = of_get_named_gpio(pdev->dev.of_node, "si,sia81xx_owi",
					    0);
		if (owi_pin < 0) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	ret = snd_soc_register_component(&pdev->dev, &soc_component_dev_sia81xx,
					 NULL, 0);
#else
	ret = snd_soc_register_codec(&pdev->dev, &soc_codec_dev_sia81xx, NULL,
				     0);
#endif
	if (0 != ret) {
...
static int __init sia81xx_pa_init(void)
{
	...
	pr_info("[ info][%s] %s: sia81xx driver version : %s \r\n", LOG_FLAG,
		__func__, SIA81XX_DRIVER_VERSION);
