...
enum { MODULE_STATE_IDLE, MODULE_STATE_WRITEN };
...
static int record_cal_module(struct file *filp, uint32_t cal_id,
			     uint32_t module_id)
{
	struct cal_module_unit *module = NULL;
	...
	if (NULL == module->cmd)
		module->cmd = (struct dev_comm_data *)kmalloc(MAX_CMD_LEN,
							      GFP_KERNEL);
...
static int write_payload_to_cal_module(struct cal_module_unit *module,
				       uint32_t opt, uint32_t param_id,
...
static int read_payload_from_cal_module(struct cal_module_unit *module,
					uint32_t *param_id, uint32_t size,
					uint8_t *payload)
{
	if (NULL == module)
		return -EINVAL;
...
static int sia81xx_tuning_if_dev_type_read(unsigned long handle,
					   uint32_t mode_id, uint32_t param_id,
					   uint32_t size, uint8_t *payload)
{
	...
	if (0 != (ret = write_payload_to_cal_module(module, OPT_GET_CAL_VAL,
						    param_id, size, payload))) {
	...
	if (OPT_REPLY != get_cal_module_param_opt(module)) {
		pr_err("[  err][%s] %s: module opt = %u \r\n", LOG_FLAG,
		       __func__, get_cal_module_param_opt(module));
		return -EINVAL;
	}

	if (0 != (ret = read_payload_from_cal_module(module, &reply_pid, size,
						     payload))) {
		pr_err("[  err][%s] %s: write_payload_to_cal_module ret = %d !! \r\n",
		       LOG_FLAG, __func__, ret);
					    ...
					    uint32_t size, uint8_t *payload)
{
	...
	if (0 != (ret = write_payload_to_cal_module(module, OPT_SET_CAL_VAL,
						    param_id, size, payload))) {
		pr_err("[  err][%s] %s: write_payload_to_cal_module ret = %d !! \r\n",
		       LOG_FLAG, __func__, ret);
...
static ssize_t sia81xx_tuning_if_dev_read(struct file *fp, char __user *buf,
