...
static const struct reg_map_info reg_map_info_table[] = {
	[CHIP_TYPE_SIA8101] = { .config = &sia8101_regmap_config,
				.reg_default = &sia8101_reg_default_val,
				.opt = &sia8101_opt_if },
	[CHIP_TYPE_SIA8108] = { .config = &sia8108_regmap_config,
				.reg_default = &sia8108_reg_default_val,
				.opt = &sia8108_opt_if },
	[CHIP_TYPE_SIA8109] = { .config = &sia8109_regmap_config,
				.reg_default = &sia8109_reg_default_val,
				.opt = &sia8109_opt_if }
};

static int verify_chip_type(unsigned int type)
{
	if (type >= ARRAY_SIZE(reg_map_info_table)) {
		pr_err("[  err][%s] %s: chip_type = %u, "
		       "ARRAY_SIZE(reg_map_info) = %lu \r\n",
		       LOG_FLAG, __func__, type,
		       ARRAY_SIZE(reg_map_info_table));
		return -ENODEV;
	}
...
int sia81xx_regmap_read(struct regmap *regmap, unsigned int start_reg,
			unsigned int reg_num, void *buf)
{
	if (NULL == regmap) {
...
int sia81xx_regmap_write(struct regmap *regmap, unsigned int start_reg,
 ...
 */
void sia81xx_regmap_defaults(struct regmap *regmap, unsigned int chip_type,
			     unsigned int scene, unsigned int channel_num)
{
	...
	pr_debug(
		"[debug][%s] %s: running, chip_type = %u, channel_num = %u \r\n",
	...
	for (i = 0; i < ARRAY_SIZE(reg_map_info_table); i++) {
		if (chip_type == reg_map_info_table[i].reg_default->chip_type) {
			ret = sia81xx_regmap_write(
				...
				reg_map_info_table[i]
					.reg_default->reg_defaults[scene]
					.num,
				reg_map_info_table[i]
						.reg_default
						->reg_defaults[scene]
 ...
 ********************************************************************/
int sia81xx_regmap_check_chip_id(struct regmap *regmap, unsigned int chip_type)
{
	if (NULL == regmap)
		return -EPERM;
...
void sia81xx_regmap_set_xfilter(struct regmap *regmap, unsigned int chip_type,
				unsigned int val)
{
	if (NULL == regmap)
		return;
