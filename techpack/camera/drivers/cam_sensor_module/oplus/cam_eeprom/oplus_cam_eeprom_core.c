...
//********************************************************************************
int EEPROM_RamWrite32A(struct cam_eeprom_ctrl_t *e_ctrl, uint32_t addr,
		       uint32_t data)
{
	int32_t rc = 0;
	int retry = 3;
	...
	for (i = 0; i < retry; i++) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_EEPROM, "write 0x%04x failed, retry:%d",
				addr, i + 1);
		} else {
			return rc;
		}
...
int EEPROM_RamRead32A(struct cam_eeprom_ctrl_t *e_ctrl, uint32_t addr,
		      uint32_t *data)
{
	int32_t rc = 0;
	int retry = 3;
	...
	for (i = 0; i < retry; i++) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_EEPROM, "read 0x%04x failed, retry:%d",
				addr, i + 1);
		} else {
			return rc;
		}
...
void EEPROM_IORead32A(struct cam_eeprom_ctrl_t *e_ctrl, uint32_t IOadrs,
		      uint32_t *IOdata)
{
...
// Function Name 	: IOWrite32A
//********************************************************************************
void EEPROM_IOWrite32A(struct cam_eeprom_ctrl_t *e_ctrl, uint32_t IOadrs,
		       uint32_t IOdata)
{
	EEPROM_RamWrite32A(e_ctrl, CMD_IO_ADR_ACCESS, IOadrs);
...
uint8_t EEPROM_FlashMultiRead(struct cam_eeprom_ctrl_t *e_ctrl, uint8_t SelMat,
			      ...
			      uint8_t UcLength)
{
	...
	EEPROM_IOWrite32A(e_ctrl, 0xE07008,
			  0x00000000 | (uint32_t)(UcLength - 1));
	EEPROM_IOWrite32A(e_ctrl, 0xE0700C,
			  ((uint32_t)SelMat << 16) | (UlAddress & 0x00003FFF));
...
static int RamWriteByte(struct cam_eeprom_ctrl_t *e_ctrl, uint32_t addr,
			uint32_t data, unsigned short mdelay)
{
	...
	for (i = 0; i < retry; i++) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_EEPROM,
				"write 0x%04x=0x%x failed, retry:%d", addr,
				data, i + 1);
		} else {
			return rc;
		}
...
int EEPROM_RamReadByte(struct cam_eeprom_ctrl_t *e_ctrl, uint32_t addr,
		       uint32_t *data)
{
	int32_t rc = 0;
	int retry = 1;
	...
	for (i = 0; i < retry; i++) {
		...
		if (rc < 0) {
			...
			CAM_ERR(CAM_EEPROM, "read 0x%04x = 0x%02x", addr,
				*data);
			return rc;
		}
	}
...
int oplus_cam_eeprom_read_memory(struct cam_eeprom_ctrl_t *e_ctrl,
				 struct cam_eeprom_memory_map_t *emap, int j,
				 uint8_t *memptr)
{
	int rc = 0;
	...
	for (i = 0; i < ((emap[j].mem.valid_size / 4) / 32 + 1); i++) {
		if ((i == (emap[j].mem.valid_size / 4) / 32) &&
		    (((emap[j].mem.valid_size / 4) % 32) != 0))
			read_size = ((emap[j].mem.valid_size / 4) % 32);
		else if ((i == (emap[j].mem.valid_size / 4) / 32) &&
			 (((emap[j].mem.valid_size / 4) % 32) == 0))
			break;
		else
			read_size = 32;
		rc = EEPROM_FlashMultiRead(e_ctrl, USER_MAT,
					   emap[j].mem.addr + i * 32, data,
					   read_size);
		if (rc != 0) {
			...
			for (m = 0; m < read_size; m++) {
				memptr[size + i * 4 * 32 + m * 4] =
					(data[m] & 0xff);
				memptr[size + i * 4 * 32 + m * 4 + 1] =
					((data[m] >> 8) & 0xff);
				memptr[size + i * 4 * 32 + m * 4 + 2] =
					((data[m] >> 16) & 0xff);
				memptr[size + i * 4 * 32 + m * 4 + 3] =
					(data[m] >> 24);
			}
		}
	}
			     ...
			     struct cam_write_eeprom_t *cam_write_eeprom)
{
	...
	EEPROM_RamWrite32A(e_ctrl, CMD_IO_WR_ACCESS,
			   cam_write_eeprom->baseAddr);
	msleep(10);
	for (i = 0; i < cam_write_eeprom->calibDataSize;) {
		i2c_reg_settings.size = 0;
		for (j = 0; (j < WRITE_DATA_MAX_LENGTH &&
			     i < cam_write_eeprom->calibDataSize);
		     j++) {
			i2c_reg_arrays[j].reg_addr = CMD_IO_WRDAT_ACCESS;
			i2c_reg_arrays[j].reg_data =
				(((cam_write_eeprom->calibData[i] << 24) &
				  0xFF000000) |
				 ((cam_write_eeprom->calibData[i + 1] << 16) &
				  0xFF0000) |
				 ((cam_write_eeprom->calibData[i + 2] << 8) &
				  0xFF00) |
				 (cam_write_eeprom->calibData[i + 3] & 0xFF));
			i2c_reg_arrays[j].delay = 0;
			i2c_reg_settings.size++;
			CAM_ERR(CAM_EEPROM, "write regs:0x%0x = 0x%0x",
				i2c_reg_arrays[j].reg_addr,
				i2c_reg_arrays[j].reg_data);
			i += DWORD_DATA_SIZE;
			rc = EEPROM_RamWrite32A(e_ctrl, CMD_IO_WRDAT_ACCESS,
						i2c_reg_arrays[j].reg_data);
		}
			     ...
			     struct cam_write_eeprom_t *cam_write_eeprom)
{
	...
	struct cam_sensor_i2c_reg_array *i2c_reg_arrays =
		(struct cam_sensor_i2c_reg_array *)kmalloc(
	...
	CAM_ERR(CAM_EEPROM, "calibDataSize:%d, m_eeprom_size:%d",
		cam_write_eeprom->calibDataSize, m_eeprom_size);
			   ...
			   struct cam_write_eeprom_t *cam_write_eeprom)
{
	...
	if (cam_write_eeprom->isWRP == 0x01) {
		...
		if (readcalibData != 0x00) {
			...
			if (rc) {
				CAM_ERR(CAM_EEPROM,
					"write WRPaddr failed rc %d", rc);
				return rc;
			}
			CAM_ERR(CAM_EEPROM, "write!cam: WRPaddr: 0x%x",
				readcalibData);
			msleep(30);
		}
	}
	CAM_DBG(CAM_EEPROM,
		"write start, cam: ID: 0x%x, reg_addr: 0x%x, val: %d",
		cam_write_eeprom->cam_id, cam_write_eeprom->baseAddr,
	...
	if (((cam_write_eeprom->cam_id == 0x01) ||
		...
		for (i = 0; i < m_eeprom_size;) {
			...
			star_addr = (cam_write_eeprom->baseAddr + i);
			for (j = 0;
			     j < WRITE_EEPROM_MAX_LENGTH && i < m_eeprom_size;
			     j++) {
				i2c_reg_arrays[j].reg_addr = star_addr;
				i2c_reg_arrays[j].reg_data =
					cam_write_eeprom->calibData[i];
			...
			rc = camera_io_dev_write_continuous(
				&e_ctrl->io_master_info, &i2c_reg_settings, 1);
			if (rc) {
				CAM_ERR(CAM_EEPROM, "eeprom write failed rc %d",
					rc);
				return rc;
			}
		}
				     ...
				     void *arg)
{
	...
	if (copy_from_user(&cam_write_eeprom, (void __user *)cmd->handle,
				     ...
				     void *arg)
{
	...
	if (copy_from_user(&check_eeprom_data, (void __user *)cmd->handle,
	...
	if ((check_eeprom_data.cam_id == 0x01 ||
		...
		for (i = 0; i < check_eeprom_data.checkDataSize; i++) {
			rc = camera_io_dev_read(&e_ctrl->io_master_info,
						(check_eeprom_data.startAddr +
						 i * WRITE_DATA_MAX_LENGTH),
						&readdata,
						CAMERA_SENSOR_I2C_TYPE_WORD,
						CAMERA_SENSOR_I2C_TYPE_BYTE);
			CAM_ERR(CAM_EEPROM, "eeprom read  rc %d", rc);
			if (rc) {
				CAM_ERR(CAM_EEPROM, "eeprom read failed rc %d",
					rc);
				return rc;
			}
		...
		CAM_ERR(CAM_EEPROM, "eepromData_checksum: %d",
			check_eeprom_data.eepromData_checksum);

		if (copy_to_user((void __user *)cmd->handle, &check_eeprom_data,
				 sizeof(struct check_eeprom_data_t))) {
