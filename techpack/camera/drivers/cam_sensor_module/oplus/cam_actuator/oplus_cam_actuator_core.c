...
static uint32_t update_reg_arr[19][4] = {
	{ 0xAE, 0x3B, 0x00, 0x0 }, { 0x10, 0x30, 0x00, 0x0 },
	{ 0x11, 0x3A, 0x00, 0x0 }, { 0x12, 0xF8, 0x00, 0x0 },
	{ 0x13, 0x7C, 0x00, 0x0 }, { 0x14, 0x66, 0x00, 0x0 },
	{ 0x15, 0x22, 0x00, 0x0 }, { 0x16, 0x15, 0x00, 0x0 },
	{ 0x17, 0x62, 0x00, 0x0 }, { 0x18, 0xDB, 0x00, 0x0 },
	{ 0x19, 0x00, 0x00, 0x0 }, { 0x1A, 0x0A, 0x00, 0x0 },
	{ 0x20, 0x36, 0x00, 0x0 }, { 0x21, 0x20, 0x00, 0x0 },
	{ 0x22, 0x00, 0x00, 0x0 }, { 0x23, 0x15, 0x00, 0x0 },
	{ 0x03, 0x01, 0x5A, 0x0 }, { 0x03, 0x02, 0x5A, 0x0 },
	{ 0x03, 0x08, 0x24, 0x0 },
};

static uint32_t pid_r5_reg[29][3] = {
	/*   addr  val   delay */
	{ 0x0A, 0x89, 0x00 }, { 0x0B, 0x88, 0x00 }, { 0x10, 0x24, 0x00 },
	{ 0x11, 0x3C, 0x00 }, { 0x12, 0x54, 0x00 }, { 0x13, 0x56, 0x00 },
	{ 0x14, 0x21, 0x00 }, { 0x15, 0x00, 0x00 }, { 0x16, 0x21, 0x00 },
	{ 0x17, 0x40, 0x00 }, { 0x18, 0xDB, 0x00 }, { 0x1A, 0x00, 0x00 },
	{ 0x1B, 0x5C, 0x00 }, { 0x1C, 0xB0, 0x00 }, { 0x1D, 0x92, 0x00 },
	{ 0x1E, 0x50, 0x00 }, { 0x1F, 0x52, 0x00 }, { 0x20, 0x01, 0x00 },
	{ 0x21, 0x02, 0x00 }, { 0x22, 0x04, 0x00 }, { 0x23, 0x28, 0x00 },
	{ 0x24, 0xFF, 0x00 }, { 0x25, 0x1E, 0x00 }, { 0x26, 0x58, 0x00 },
	{ 0xAE, 0x3B, 0x00 }, { 0x03, 0x02, 0xFF }, { 0x4B, 0x00, 0x00 },
	{ 0xAE, 0x00, 0x00 }, { 0xFF, 0x05, 0x64 }, /* pid version r5 */
};

static int RamWriteByte(struct cam_actuator_ctrl_t *a_ctrl, uint32_t addr,
			uint32_t data, unsigned short mdelay)
{
	...
	for (i = 0; i < retry; i++) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_ACTUATOR, "write 0x%04x failed, retry:%d",
				addr, i + 1);
		} else {
			return rc;
		}
...
static int32_t cam_actuator_update_pid(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (0x18 >> 1 == a_ctrl->io_master_info.cci_client->sid) {
		...
		for (cnt = 0; cnt < UPDATE_REG_SIZE; cnt++) {
			rc = RamWriteByte(a_ctrl, update_reg_arr[cnt][0],
					  update_reg_arr[cnt][1],
					  update_reg_arr[cnt][2]);
			if (rc < 0) {
				CAM_ERR(CAM_ACTUATOR,
					"write imx708 new PID data error in step %d:rc %d",
					cnt, rc);
			}
		}
		for (re = 0; re < retry; re++) {
			rc = camera_io_dev_read(&(a_ctrl->io_master_info), 0x4B,
						&reg_data,
						CAMERA_SENSOR_I2C_TYPE_BYTE,
						CAMERA_SENSOR_I2C_TYPE_BYTE);
			if (rc < 0 || ((reg_data & 0x04) != 0)) {
				CAM_ERR(CAM_ACTUATOR,
					"4Bh's data is 0x%x or the read io is error ,rc :%d",
					reg_data, rc);
				rc = RamWriteByte(a_ctrl, update_reg_arr[16][0],
						  update_reg_arr[16][1],
						  update_reg_arr[16][2]);
				rc = RamWriteByte(a_ctrl, update_reg_arr[17][0],
						  update_reg_arr[17][1],
						  update_reg_arr[17][2]);
			} else {
				break;
...
static int32_t cam_actuator_update_pid_to_r5(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (0xE8 >> 1 == a_ctrl->io_master_info.cci_client->sid) {
		...
		rc = RamWriteByte(a_ctrl, pid_r5_reg[24][0], pid_r5_reg[24][1],
				  pid_r5_reg[24][2]);
		if (rc < 0) {
			CAM_ERR(CAM_ACTUATOR,
				"write change setting mode fail rc %d", rc);
			return rc;
		}

		/* write register data */
		for (cnt = 2; cnt < UPDATE_REG_SIZE; cnt++) {
			rc = RamWriteByte(a_ctrl, pid_r5_reg[cnt][0],
					  pid_r5_reg[cnt][1],
					  pid_r5_reg[cnt][2]);
			if (rc < 0) {
				CAM_ERR(CAM_ACTUATOR,
					"write ov64b new PID data error in step %d:rc %d",
					cnt, rc);
			}
		}

		/* store operation */
		rc = RamWriteByte(a_ctrl, pid_r5_reg[25][0], pid_r5_reg[25][1],
				  pid_r5_reg[25][2]);
		if (rc < 0) {
			CAM_ERR(CAM_ACTUATOR, "write store register fail rc %d",
				rc);
		}
		...
		rc = RamWriteByte(a_ctrl, pid_r5_reg[28][0], pid_r5_reg[28][1],
				  pid_r5_reg[28][2]);
		if (rc < 0) {
			CAM_ERR(CAM_ACTUATOR, "write store register fail rc %d",
				rc);
		...
		rc = RamWriteByte(a_ctrl, pid_r5_reg[27][0], pid_r5_reg[27][1],
				  pid_r5_reg[27][2]);
		if (rc < 0) {
			CAM_ERR(CAM_ACTUATOR, "release setting mode fail rc %d",
				rc);
		}
		...
		for (cnt = 0; cnt < UPDATE_REG_SIZE; cnt++) {
			...
			if (rc < 0) {
				CAM_ERR(CAM_ACTUATOR,
					"read reg 0x%x fail rc %d",
					pid_r5_reg[cnt][0], rc);
				break;
			}
			CAM_DBG(CAM_ACTUATOR, "read reg 0x%x 0x%x",
				pid_r5_reg[cnt][0], reg_data);
			if (reg_data != pid_r5_reg[cnt][1]) {
				CAM_ERR(CAM_ACTUATOR,
					"pid data wrong reg 0x%x val 0x%x acquire 0x%x rc %d",
					pid_r5_reg[cnt][0], reg_data,
					pid_r5_reg[cnt][1], rc);
				rc = -1;
				break;
...
static int32_t cam_actuator_check_firmware(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (0x18 >> 1 == a_ctrl->io_master_info.cci_client->sid) {
		...
		for (cnt = 1; cnt < UPDATE_REG_SIZE; cnt++) {
			...
			if (rc < 0) {
				CAM_ERR(CAM_ACTUATOR,
					"read imx708 new PID data error in step %d:rc %d",
					cnt, rc);
				break;
			}
			if (reg_data != update_reg_arr[cnt][1]) {
				CAM_ERR(CAM_ACTUATOR,
					"imx708 new PID data wrong in step %d:rc %d, reg_data is %x",
					cnt, rc, reg_data);
				rc = -1;
				break;
			}
	...
	if (0xE8 >> 1 == a_ctrl->io_master_info.cci_client->sid ||
		...
		if (reg_data == PID_R2) {
			...
			for (cnt = 0; cnt < 24; cnt++) {
				...
				if (reg_data != pid_r5_reg[cnt][1]) {
					CAM_ERR(CAM_ACTUATOR,
						"reg 0x%2x val 0x%2x different to r5 param val 0x%2x",
						pid_r5_reg[cnt][0], reg_data,
						pid_r5_reg[cnt][1]);
				...
				CAM_INFO(CAM_ACTUATOR, "read reg 0x%x 0x%x",
					 pid_r5_reg[cnt][0], reg_data);
			}
		}
	}
...
int32_t oplus_cam_actuator_power_up(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (a_ctrl->need_check_actuator_data &&
		...
		for (re = 0; re < retry; re++) {
			...
			if (rc < 0) {
				...
				CAM_INFO(
					CAM_ACTUATOR,
					"check the pid data is empty ,will store the pid data!");
				rc = cam_actuator_update_pid(a_ctrl);
			}
			if (rc < 0) {
				CAM_ERR(CAM_ACTUATOR,
					"update the pid data error,check the io ctrl!");
			} else {
				break;
			}
		}
	...
	CAM_INFO(CAM_ACTUATOR,
		 "is_actuator_pid_updated %d,need_check_actuator_data:%d",
	...
	if (a_ctrl->need_check_actuator_data &&
		...
		for (re = 0; re < retry; re++) {
			rc = cam_actuator_check_firmware(a_ctrl);
			CAM_INFO(CAM_ACTUATOR,
				 "cam_actuator_check_firmware rc:%d", rc);

			if (rc < 0) {
				CAM_ERR(CAM_ACTUATOR,
					"update pid version to r5");
				rc = cam_actuator_update_pid_to_r5(a_ctrl);
				if (rc < 0) {
					a_ctrl->is_actuator_pid_updated = false;
					CAM_ERR(CAM_ACTUATOR,
						"actuator updated pid to r5 version failed! check the io ctrl! is_actuator_pid_updated %d",
						a_ctrl->is_actuator_pid_updated);
				} else {
					a_ctrl->is_actuator_pid_updated = true;
					CAM_INFO(
						CAM_ACTUATOR,
						"actuator successfully updated pid to r5 version  %d",
						a_ctrl->is_actuator_pid_updated);
					break;
				}
			} else {
		...
		if (a_ctrl->is_actuator_pid_updated == false) {
			CAM_ERR(CAM_ACTUATOR,
				"actuator pid update fail addr:0x%x",
				a_ctrl->io_master_info.cci_client->sid);
		}
	} else {
...
void oplus_cam_actuator_i2c_modes_util(struct camera_io_master *io_master_info,
				       struct i2c_settings_list *i2c_list)
{
	uint32_t value;
	if (i2c_list->i2c_settings.reg_setting[0].reg_addr == 0x0204) {
		value = (i2c_list->i2c_settings.reg_setting[0].reg_data &
			 0xFF00) >>
			8;
		i2c_list->i2c_settings.reg_setting[0].reg_data =
			((i2c_list->i2c_settings.reg_setting[0].reg_data & 0xFF)
			 << 8) |
			value;
