...
static int32_t cam_actuator_power(struct cam_actuator_ctrl_t *a_ctrl,
				  int enable)
{
	int rc = 0;
	...
	if (enable) {
		...
		if (rc) {
			CAM_ERR(CAM_ACTUATOR,
				"power down the core is failed:%d", rc);
			return rc;
		}
	}
	return rc;
...
static DEVICE_ATTR(fswitch, 0660, cam_actuator_switch_show,
		   cam_actuator_switch_store);
void oplus_cam_actuator_fswitch_dev_file_create(
	struct cam_actuator_ctrl_t *a_ctrl, struct platform_device *pdev)
{
