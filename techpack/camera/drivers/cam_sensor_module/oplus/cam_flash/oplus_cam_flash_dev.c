...
static ssize_t flash_on_off(struct cam_flash_ctrl *flash_ctrl)
{
	...
	pr_info("flash_mode %d,%d-%02d-%02d %02d:%02d:%02d.%09lu UTC\n",
		flash_mode, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
	...
	case 1:
		...
		if (vendor_flash_ctrl->flash_current != 0) {
			flash_data.led_current_ma[0] =
				vendor_flash_ctrl->flash_current;
			flash_data.led_current_ma[1] =
				vendor_flash_ctrl->flash_current;
		}
#endif
...
static ssize_t flash_proc_read(struct file *filp, char __user *buff, size_t len,
			       loff_t *data)
{
	char value[2] = { 0 };
