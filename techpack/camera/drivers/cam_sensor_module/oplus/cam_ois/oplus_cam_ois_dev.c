...
int oplus_cam_ois_deinit(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl->m_ois_fw_mode.ois_fw_prog_ptr != NULL) {
		cma_release(
			dev_get_cma_area((o_ctrl->soc_info.dev)),
			(struct page *)o_ctrl->m_ois_fw_mode.ois_fw_prog_ptr,
			o_ctrl->m_ois_fw_mode.ois_fw_prog_size);
		o_ctrl->m_ois_fw_mode.ois_fw_prog_ptr = NULL;
	}
	if (o_ctrl->m_ois_fw_mode.ois_fw_coeff_ptr != NULL) {
		cma_release(
			dev_get_cma_area((o_ctrl->soc_info.dev)),
			(struct page *)o_ctrl->m_ois_fw_mode.ois_fw_coeff_ptr,
			o_ctrl->m_ois_fw_mode.ois_fw_coeff_size);
		o_ctrl->m_ois_fw_mode.ois_fw_coeff_ptr = NULL;
	}
	return ret;
}
