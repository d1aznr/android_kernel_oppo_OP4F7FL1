...
static int RamWriteWord(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr,
			uint32_t data)
{
	int32_t rc = 0;
	int retry = 1;
	...
	for (i = 0; i < retry; i++) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_OIS,
				"write 0x%x = 0x%0x failed, retry:%d !!!", addr,
				data, i + 1);
		} else {
...
static int oplus_cam_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl->m_ois_fw_mode.ois_fw_prog_ptr == NULL) {
		...
		o_ctrl->m_ois_fw_mode.ois_fw_prog_ptr =
	...
	if (o_ctrl->m_ois_fw_mode.ois_fw_coeff_ptr == NULL) {
		...
		o_ctrl->m_ois_fw_mode.ois_fw_coeff_ptr =
			(struct cam_sensor_i2c_reg_array *)(page_address(page));
...
int cam_ois_bu63169_getmultiHall(struct cam_ois_ctrl_t *o_ctrl,
				 OISHALL2EIS *hall_data)
{
	...
	if ((long)(timeStamp * 1778) / 100 > endTime.tv_usec) {
		...
		endTime.tv_usec = (1000 * 1000 + endTime.tv_usec) -
				  (timeStamp * 1778) / 100;
	} else {
	...
	if (endTime.tv_usec < (long)(4000 * dataNum)) {
		...
		startTime.tv_usec =
			(1000 * 1000 + endTime.tv_usec) - (4000 * dataNum);
	} else {
	...
	for (i = 0; i < dataNum; i++) {
		...
		if ((startTime.tv_usec + 4000) / (1000 * 1000) >= 1) {
			...
			startTime.tv_usec =
				((startTime.tv_usec + 4000) - 1000 * 1000);
		} else {
...
#define OIS_SET_TELE_GET_HALL_DATA_THREAD_MASK 0x02
	...
	if (ioctl_ctrl->reserved & OIS_SET_THREAD_STATUS_MASK) {
		if (ioctl_ctrl->reserved &
		    OIS_SET_MIAN_GET_HALL_DATA_THREAD_MASK) {
			is_enable_main_ois_thread = true;
		}
		if (ioctl_ctrl->reserved &
		    OIS_SET_TELE_GET_HALL_DATA_THREAD_MASK) {
			is_enable_tele_ois_thread = true;
		}
		set_ois_thread_status(is_enable_main_ois_thread,
				      is_enable_tele_ois_thread);
		CAM_INFO(
			CAM_OIS,
			"set ois get hall data thread status : main:[%d] tele: [%d]",
...
int oplus_cam_ois_pkt_parse(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (strstr(o_ctrl->ois_name, "lc898") != NULL) {
		if (o_ctrl->is_ois_calib) {
			rc = oplus_cam_ois_apply_settings(
	...
	if (strstr(o_ctrl->ois_name, "bu63169")) {
		...
		rc = camera_io_dev_read(&(o_ctrl->io_master_info), 0x84F7,
					&sum_check, CAMERA_SENSOR_I2C_TYPE_WORD,
		...
		rc = camera_io_dev_read(&(o_ctrl->io_master_info), 0x84F6,
					&sum_check, CAMERA_SENSOR_I2C_TYPE_WORD,
					CAMERA_SENSOR_I2C_TYPE_WORD);
		if (rc < 0) {
...
int oplus_cam_ois_driver_cmd(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_GET_OIS_EIS_HALL: {
		...
		if (o_ctrl->cam_ois_state == CAM_OIS_START &&
		    (strstr(o_ctrl->ois_name, "lc898") != NULL ||
			...
			if (get_hall_version == GET_HALL_DATA_VERSION_V2) {
				Sem1215sReadOISHALLDataV2(
					o_ctrl, u64_to_user_ptr(cmd->handle));
			} else {
				Sem1215sReadOISHALLData(
					o_ctrl, u64_to_user_ptr(cmd->handle));
			}
		} else {
