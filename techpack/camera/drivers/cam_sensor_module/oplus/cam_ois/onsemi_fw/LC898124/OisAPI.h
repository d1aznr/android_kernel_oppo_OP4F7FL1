...
extern unsigned char SelectDownload(UINT8 GyroSelect, UINT8 ActSelect,
				    UINT8 MasterSlave); // download

/* Code information check such as version */
extern UINT8 GetInfomationAfterDownload(
	DSPVER *Info); // get code information after download.

/* Power off for SPI interface */
extern void
PreparationForPowerOff124(void); // SPI interface stop before power off

/* E2Prom Access */
extern UINT8 E2PromVerification(
	void); // E2Prom verification for user area by using check sum. O:SUCCESS, 1:FAUILRE
extern UINT8 E2PromVerificationONSEMI(
	void); // E2Prom verification for ONSEMI area by using check sum. O:SUCCESS, 1:FAUILRE
extern void
BurstReadE2Prom(UINT8 address, UINT8 *val,
		UINT8 cnt); // E2Prom reading continuously for every area.
extern void ReadE2Prom(unsigned char address,
		       unsigned char *val); // Read data from E2Prom
#if ((SELECT_VENDOR & 0x80) != 0x80)
extern UINT8 WriteE2Prom(UINT8 address,
			 UINT8 data); // E2Prom writing for user area 1byte.
...
extern UINT8 GetGyroOffset124(UINT16 *GyroOffsetX, UINT16 *GyroOffsetY,
			      INT16 GYROF_UPPER,
			      INT16 GYROF_LOWER); // get the gyro offset values.
extern void SetGyroOffset124(
	UINT16 GyroOffsetX, UINT16 GyroOffsetY,
	UINT16 GyroOffsetZ); // set the gyro offset data. before do this before Remapmain.

/* Angle correction for 45deg*/
extern UINT8 SetAngleCorrection124(float DegreeGap, UINT8 SelectAct,
				   unsigned char Arrangement);

extern void SetGyroAccelCoef(UINT8);

/* AF position Control [mandatory] */
extern void
SetTregAf(UINT16 UsTregAf); // Bi-direction :  Min:000h Max:7FFh (11bit)

/* Status Read and OIS enable [mandatory] */
extern UINT8 RdStatus124(
	UINT8 UcStBitChk); // Status Read whether initialization finish or not.
extern void OisEna124(void); // OIS Enable Function

/* Others [option] */
extern UINT8
RtnCen124(UINT8 UcCmdPar); // Return to Center Function. Hall servo on/off
...
extern void
GyroOffsetMeasureStartZ124(void); // start the gyro Z offset adjustment
...
extern void SetAcclOffset124(
	UINT16 AcclOffsetX, UINT16 AcclOffsetY,
	UINT16 AcclOffsetZ); // set the accl offset data. before do this before Remapmain.
extern void
GetAcclOffset124(UINT16 *AcclOffsetX, UINT16 *AcclOffsetY,
		 UINT16 *AcclOffsetZ); // get the accl offset values.
...
/* E2Prom Access */
extern UINT8 WrHallCalData124(
	void); // upload the calibration data except gyro gain to eeprom
...
#if ((SELECT_VENDOR & 0x80) != 0x80)
extern UINT8
WrHallLnData(UINT8 UcMode,
	     mlLinearityValue *linval); // upload linearity position to eeprom
extern UINT8
WrMixCalData124(UINT8 UcMode,
		mlMixingValue *mixval); // upload cross talk to eeprom
