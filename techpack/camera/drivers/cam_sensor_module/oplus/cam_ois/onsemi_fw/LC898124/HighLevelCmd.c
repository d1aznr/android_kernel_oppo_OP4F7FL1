...
void MeasureStart2124(INT32 SlMeasureParameterNum, INT32 SlMeasureParameterA,
		      INT32 SlMeasureParameterB, UINT16 UsTime);
...
void SetTregAf(UINT16 UsTregAf)
{
	...
	RamWrite32A(CMD_AF_POSITION,
		    UsTregAf | 0x00010000); // bit 16 : FST mode
...
void PreparationForPowerOff124(void)
{
	...
	RamWrite32A((GYRO_RAM_GYRO_AF_Switch & 0xFFFC),
		    (UlReadVa | 0x00008000));
...
void SetWaitTime124(UINT16 UsWaitTime)
{
	...
	RamWrite32A(WaitTimerData_UiTargetCount,
		    (UINT32)(ONE_MSEC_COUNT * UsWaitTime));
}
...
void SetTransDataAdr124(UINT16 UsLowAddress, UINT32 UlLowAdrBeforeTrans)
{
	...
	if (UlLowAdrBeforeTrans < 0x00009000) {
		...
		StTrsVal.StDwdVal.UsHigVal =
			(UINT16)((UlLowAdrBeforeTrans & 0x0000F000) >> 8);
		StTrsVal.StDwdVal.UsLowVal =
			(UINT16)(UlLowAdrBeforeTrans & 0x00000FFF);
	}
...
// History			: First edition
//********************************************************************************
void MeasureStart124(INT32 SlMeasureParameterNum, UINT32 SlMeasureParameterA,
		     UINT32 SlMeasureParameterB)
{
	...
	SetTransDataAdr124(
		...
		SlMeasureParameterA); // Set Measure Filter A Ram Address
	SetTransDataAdr124(
		StMeasFunc_MFB_PiMeasureRam2,
		SlMeasureParameterB); // Set Measure Filter B Ram Address
...
// History			: First edition
//********************************************************************************
void SetGyroOffset124(UINT16 GyroOffsetX, UINT16 GyroOffsetY,
		      UINT16 GyroOffsetZ)
{
...
void GyroOffsetMeasureStart124(void)
{
	...
	MeasureStart124(GYROF_NUM, GYRO_RAM_GX_ADIDAT,
			GYRO_RAM_GY_ADIDAT); // Start measure
}
...
{
	MesFil124(THROUGH); // Set Measure Filter
	MeasureStart124(GYROF_NUM, GYRO_RAM_GZ_ADIDAT,
			GYRO_RAM_GZ_ADIDAT); // Start measure
}
...
// History			: First edition
//********************************************************************************
UINT8 GetGyroOffset124(UINT16 *GyroOffsetX, UINT16 *GyroOffsetY,
		       INT16 GYROF_UPPER, INT16 GYROF_LOWER)
{
	...
	*GyroOffsetX = (UINT16)(SlMeasureAveValueA &
				0x0000FFFF); //Measure Result Store
	*GyroOffsetY = (UINT16)(SlMeasureAveValueB &
				0x0000FFFF); //Measure Result Store

	if (((INT16)(*GyroOffsetX) > GYROF_UPPER) ||
	    ((INT16)(*GyroOffsetX) < GYROF_LOWER)) {
		ans |= 1;
	}
	if (((INT16)(*GyroOffsetY) > GYROF_UPPER) ||
	    ((INT16)(*GyroOffsetY) < GYROF_LOWER)) {
		ans |= 2;
	}
	CAM_ERR(CAM_OIS,
...
// History			: First edition
//********************************************************************************
void SetAcclOffset124(UINT16 AcclOffsetX, UINT16 AcclOffsetY,
		      UINT16 AcclOffsetZ)
{
...
// History			: First edition
//********************************************************************************
void GetAcclOffset124(UINT16 *AcclOffsetX, UINT16 *AcclOffsetY,
		      UINT16 *AcclOffsetZ)
{
...
UINT8 TstActMov124(UINT8 UcDirSel)
{
	...
	UlActChkLvl = (UINT32)((float)0x7FFFFFFF * SfLimit * Sfzoom * Sflenz *
			       Sfshift * ACT_MARGIN);
	...
	RamWrite32A(SinWave_Offset,
		    ACT_CHK_FRQ); // Freq Setting = Freq * 80000000h / Fs	: 5Hz
	RamWrite32A(SinWave_Gain, UlActChkLvl); // Set Sine Wave Gain
	RamWrite32A(SinWaveC_Regsiter, 0x00000001); // Sine Wave Start
	if (UcDirSel == X_DIR) {
		SetTransDataAdr124(
			SinWave_OutAddr,
			(UINT32)HALL_RAM_HXOFF1); // Set Sine Wave Input RAM
	} else if (UcDirSel == Y_DIR) {
		SetTransDataAdr124(
			SinWave_OutAddr,
			(UINT32)HALL_RAM_HYOFF1); // Set Sine Wave Input RAM
	}
	...
	UlReturnVal = (INT32)((INT64)StMeasValueA.UllnValue * 100 /
			      (INT64)StMeasValueB.UllnValue);
...
UINT8 RunGea124(void)
{
	...
	for (UcCnt = 0; UcCnt < 10; UcCnt++) {
		...
		// X
		UsGxoVal[UcCnt] =
			(UINT16)(SlMeasureAveValueA >> 16); // 平均値測定

		// Y
		UsGyoVal[UcCnt] =
			(UINT16)(SlMeasureAveValueB >> 16); // 平均値測定
		...
		if (UcCnt > 0) {
			if ((INT16)UsGxoVal[0] > (INT16)UsGxoVal[UcCnt]) {
				UsDif = (UINT16)((INT16)UsGxoVal[0] -
						 (INT16)UsGxoVal[UcCnt]);
			} else {
				UsDif = (UINT16)((INT16)UsGxoVal[UcCnt] -
						 (INT16)UsGxoVal[0]);
			}
			...
			if ((INT16)UsGyoVal[0] > (INT16)UsGyoVal[UcCnt]) {
				UsDif = (UINT16)((INT16)UsGyoVal[0] -
						 (INT16)UsGyoVal[UcCnt]);
			} else {
				UsDif = (UINT16)((INT16)UsGyoVal[UcCnt] -
						 (INT16)UsGyoVal[0]);
			}
...
//********************************************************************************
UINT8 MesRam124(INT32 SlMeasureParameterA, INT32 SlMeasureParameterB,
		INT32 SlMeasureParameterNum, stMesRam124 *pStMesRamA,
		stMesRam124 *pStMesRamB)
{
	UnllnVal StMeasValueA, StMeasValueB;
...
UINT8 RunGea2124(UINT8 UcMode)
{
	...
	RamWrite32A(CMD_GYRO_WR_ACCS,
		    0x1B180000); /* FS_SEL=3固定 & Disable Self Test */
	UcStRd = 1;
	while (UcStRd) {
		UcStRd = RdStatus124(1);
	}
	...
	MesRam124(SlMeasureParameterA, SlMeasureParameterB,
		  SlMeasureParameterNum, &StMesRamA, &StMesRamB);
	...
	if (UcMode == GEA_MINMAX_MODE) { // min, max mode
		...
		// X
		if (abs(StMesRamA.SlMeasureMaxValue) >=
		    abs(StMesRamA.SlMeasureMinValue)) {
			UsGyrXval = (UINT16)(abs(StMesRamA.SlMeasureMaxValue) >>
					     16); // max value
		} else {
			UsGyrXval = (UINT16)(abs(StMesRamA.SlMeasureMinValue) >>
					     16); // max value
		}

		// Y
		if (abs(StMesRamB.SlMeasureMaxValue) >=
		    abs(StMesRamB.SlMeasureMinValue)) {
			UsGyrYval = (UINT16)(abs(StMesRamB.SlMeasureMaxValue) >>
					     16); // max value
		} else {
			UsGyrYval = (UINT16)(abs(StMesRamB.SlMeasureMinValue) >>
					     16); // max value
		}
		...
		// X
		UsGyrXval = (UINT16)(abs(StMesRamA.SlMeasureAveValue) >>
				     16); // ave value

		// Y
		UsGyrYval = (UINT16)(abs(StMesRamB.SlMeasureAveValue) >>
				     16); // ave value
	}
	...
	RamWrite32A(CMD_GYRO_WR_ACCS,
		    0x1BD80000); /* FS_SEL=3固定 & Enable Self Test */
	UcStRd = 1;
	while (UcStRd) {
		UcStRd = RdStatus124(1);
	}
	...
	if (UcMode == GEA_MINMAX_MODE) { // min, max mode
		...
		// X
		UsGyrXval = (UINT16)(StMesRamA.SlMeasureMinValue >>
				     16); // min value

		// Y
		UsGyrYval = (UINT16)(StMesRamB.SlMeasureMinValue >>
				     16); // min value
		...
		// X
		UsGyrXval = (UINT16)(StMesRamA.SlMeasureAveValue >>
				     16); // ave value

		// Y
		UsGyrYval = (UINT16)(StMesRamB.SlMeasureAveValue >>
				     16); // ave value
	}
	...
	RamWrite32A(CMD_GYRO_WR_ACCS,
		    0x1B000000 | (UlGyroConfig >> 8)); /* 元の設定値に戻す */
	UcStRd = 1;
	while (UcStRd) {
		UcStRd = RdStatus124(1);
	}
...
UINT8 SetAngleCorrection124(float DegreeGap, UINT8 SelectAct, UINT8 Arrangement)
{
	...
	switch (SelectAct) {
	case ACT_45DEG:
		OffsetAngle = (double)(45.0f + DegreeGap) *
		      ...
		      INT32 SlMeasureParameterB, UINT16 UsTime)
{
	...
	SetTransDataAdr124(
		StMeasFunc_MFA_PiMeasureRam1,
		(UINT32)SlMeasureParameterA); // Set Measure Filter A Ram Address
	SetTransDataAdr124(
		StMeasFunc_MFB_PiMeasureRam2,
		(UINT32)SlMeasureParameterB); // Set Measure Filter B Ram Address
	RamWrite32A(StMeasFunc_SiSampleNum, 0); // Clear Measure Counter
	ClrMesFil124(); // Clear Delay Ram
	RamWrite32A(StMeasFunc_SiSampleMax,
		    SlMeasureParameterNum); // Set Measure Max Number
...
void LinearityCalculation(void)
{
	...
	TRACE(" %02xh %02xh %02xh %02xh %02xh %02xh %02xh %02xh \n", tempL[0],
	      tempL[1], tempL[2], tempL[3], tempL[4], tempL[5], tempL[6],
	      tempL[7]);
	TRACE(" %02xh %02xh %02xh %02xh %02xh %02xh %02xh %02xh \n", tempL[8],
	      tempL[9], tempL[10], tempL[11], tempL[12], tempL[13], tempL[14],
	      tempL[15]);
	TRACE(" %02xh %02xh %02xh %02xh %02xh %02xh %02xh %02xh \n", tempL[16],
	      tempL[17], tempL[18], tempL[19], tempL[20], tempL[21], tempL[22],
	      tempL[23]);
	TRACE(" %02xh %02xh %02xh %02xh %02xh %02xh %02xh %02xh \n", tempL[24],
	      tempL[25], tempL[26], tempL[27], tempL[28], tempL[29], tempL[30],
	      tempL[31]);
	...
	TRACE("[x] %04xh %04xh %04xh %04xh %04xh %04xh %04xh \n", tblX[0],
	      tblX[1], tblX[2], tblX[3], tblX[4], tblX[5], tblX[6]);
	TRACE("[y] %04xh %04xh %04xh %04xh %04xh %04xh %04xh \n", tblY[0],
	      tblY[1], tblY[2], tblY[3], tblY[4], tblY[5], tblY[6]);
	TRACE("[s] %04xh %04xh \n", stpx, stpy);
	...
	for (i = 0; i < 6; i++) {
		if (i == 3) {
			...
			cffax[i] = ((float)pixx[i + 1] / (float)dacx[i + 1] *
				    524287.0f);
			cffay[i] = ((float)pixy[i + 1] / (float)dacy[i + 1] *
				    524287.0f);
			...
			cffax[i] = (float)(dacx[i] - dacx[i + 1]) /
				   (float)(pixx[i] - pixx[i + 1]);
			cffay[i] = (float)(dacy[i] - dacy[i + 1]) /
				   (float)(pixy[i] - pixy[i + 1]);
			cfax[i] = (INT16)cffax[i];
			cfay[i] = (INT16)cffay[i];
			if (i == 2) {
				cfbx[i] = (INT16)0;
				...
				cfbx[i] = (INT16)((float)dacx[i] -
						  cffax[i] * (float)pixx[i]);
				cfby[i] = (INT16)((float)dacy[i] -
						  cffay[i] * (float)pixy[i]);
			}
		}
		if (i < 5) {
			cfzx[i] = pixx[i + 1];
	...
	TRACE(" [%04xh , %04xh , %04xh] [%04xh , %04xh , %04xh]\n", cfzx[0],
	      cfax[0], cfbx[0], cfzy[0], cfay[0], cfby[0]);
	TRACE(" [%04xh , %04xh , %04xh] [%04xh , %04xh , %04xh]\n", cfzx[1],
	      cfax[1], cfbx[1], cfzy[1], cfay[1], cfby[1]);
	TRACE(" [%04xh , %04xh , %04xh] [%04xh , %04xh , %04xh]\n", cfzx[2],
	      cfax[2], cfbx[2], cfzy[2], cfay[2], cfby[2]);
	TRACE(" [%04xh , %04xh , %04xh] [%04xh , %04xh , %04xh]\n", cfzx[3],
	      cfax[3], cfbx[3], cfzy[3], cfay[3], cfby[3]);
	TRACE(" [%04xh , %04xh , %04xh] [%04xh , %04xh , %04xh]\n", cfzx[4],
	      cfax[4], cfbx[4], cfzy[4], cfay[4], cfby[4]);
	TRACE(" [----- , %04xh , %04xh] [----- , %04xh , %04xh]\n", cfax[5],
	      cfbx[5], cfay[5], cfby[5]);
	adr = HAL_LN_CORRECT;
	RamWrite32A(adr, (UINT32)((UINT16)cfax[0] + ((UINT16)cfax[1] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfax[2] + ((UINT16)cfax[3] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfax[4] + ((UINT16)cfax[5] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfbx[0] + ((UINT16)cfbx[1] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfbx[2] + ((UINT16)cfbx[3] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfbx[4] + ((UINT16)cfbx[5] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfzx[0] + ((UINT16)cfzx[1] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfzx[2] + ((UINT16)cfzx[3] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfzx[4] + ((UINT16)cfay[0] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfay[1] + ((UINT16)cfay[2] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfay[3] + ((UINT16)cfay[4] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfay[5] + ((UINT16)cfby[0] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfby[1] + ((UINT16)cfby[2] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfby[3] + ((UINT16)cfby[4] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfby[5] + ((UINT16)cfzy[0] << 16)));
	adr += 0x0004;
	RamWrite32A(adr, (UINT32)((UINT16)cfzy[1] + ((UINT16)cfzy[2] << 16)));
	adr += 0x0004;
...
void CrosstalkCalculation(void)
{
	...
	TRACE("[XX] %04xh [XY]%04xh [SFT]%02xh\n", tblC[0], tblC[1],
	      (unsigned char)(tblC[4] >> 0));
	TRACE("[YY] %04xh [YX]%04xh [SFT]%02xh\n", tblC[2], tblC[3],
	      (unsigned char)(tblC[4] >> 8));
	RamWrite32A(HF_hx45x, (UINT32)(tblC[0] << 16));
	RamWrite32A(HF_hx45y, (UINT32)(tblC[1] << 16));
	RamWrite32A(HF_hy45y, (UINT32)(tblC[2] << 16));
	RamWrite32A(HF_hy45x, (UINT32)(tblC[3] << 16));
