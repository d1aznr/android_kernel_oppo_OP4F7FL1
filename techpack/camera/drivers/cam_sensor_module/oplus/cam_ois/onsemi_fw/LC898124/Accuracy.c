...
unsigned short Accuracy(float ACCURACY, unsigned short RADIUS,
			unsigned short DEGSTEP, unsigned short WAIT_MSEC1,
			unsigned short WAIT_MSEC2, unsigned short WAIT_MSEC3)
{
	...
	xRadius = 0.10546843F * fabsf(fix2float(xGyrogain)) *
		  fabsf(fix2float(xGLenz)) *
		  (1 << (unsigned char)(xG2x4xb >> 8));
	yRadius = 0.10546843F * fabsf(fix2float(yGyrogain)) *
		  fabsf(fix2float(yGLenz)) *
		  (1 << (unsigned char)(yG2x4xb >> 8));
	TRACE("xRadius = %d, yRadius = %d", (int)(xRadius * 1000),
	      (int)(yRadius * 1000));
...
unsigned short HallCheck(float ACCURACY, unsigned short RADIUS,
			 unsigned short DEGSTEP, unsigned short WAIT_MSEC1,
			 unsigned short WAIT_MSEC2, unsigned short WAIT_MSEC3)
{
	short i;
	//	unsigned short ret = Accuracy();
	unsigned short ret = Accuracy(ACCURACY, RADIUS, DEGSTEP, WAIT_MSEC1,
