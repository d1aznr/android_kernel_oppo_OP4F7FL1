...
//****************************************************
#if (((SELECT_VENDOR & 0x01) == 0x01) ||                                       \
     ((SELECT_VENDOR & 0x80) == 0x80)) // SEMCO or Oplus
#include "LC898124EP3_Calibration_SO2821.h"
#endif
#if (((SELECT_VENDOR & 0x02) == 0x02) ||                                       \
     ((SELECT_VENDOR & 0x80) == 0x80)) // OFILM or Oplus
...
#if USE_BOSCH
extern int8_t bmi2_i2c_read(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data,
			    uint16_t len);
extern int8_t bmi2_i2c_write(uint8_t dev_addr, uint8_t reg_addr,
...
Act_Mov_t StActMov = { 0x19998000, 0xE6668000, 0xFCCD0000 };
...
UINT32 TneGvc(void)
{
	...
	MeasureStart124(SlMeasureParameterNum, SlMeasureParameterA,
			SlMeasureParameterB); // Start measure

	MeasureWait124(); // Wait complete of measurement

	TRACE("Read Adr = %04x, %04xh \n", StMeasFunc_MFA_LLiIntegral1 + 4,
	      StMeasFunc_MFA_LLiIntegral1);
	RamRead32A(StMeasFunc_MFA_LLiIntegral1,
		   &StMeasValueA.StUllnVal.UlLowVal); // X axis
	RamRead32A(StMeasFunc_MFA_LLiIntegral1 + 4,
		   &StMeasValueA.StUllnVal.UlHigVal);
	RamRead32A(StMeasFunc_MFB_LLiIntegral2,
		   &StMeasValueB.StUllnVal.UlLowVal); // Y axis
	RamRead32A(StMeasFunc_MFB_LLiIntegral2 + 4,
	...
	SlMeasureAveValueA =
		(INT32)((INT64)StMeasValueA.UllnValue / SlMeasureParameterNum);
	SlMeasureAveValueB =
		(INT32)((INT64)StMeasValueB.UllnValue / SlMeasureParameterNum);
	...
	UlRsltSts = EXE_END;
	StAdjPar124.StGvcOff.UsGxoVal =
		(UINT16)(SlMeasureAveValueA &
			 0x0000FFFF); //Measure Result Store
	if (((INT16)StAdjPar124.StGvcOff.UsGxoVal > (INT16)GYROF_UPPER) ||
	    ((INT16)StAdjPar124.StGvcOff.UsGxoVal < (INT16)GYROF_LOWER)) {
		UlRsltSts |= EXE_GXADJ;
	}
	RamWrite32A(GYRO_RAM_GXOFFZ, ((SlMeasureAveValueA << 16) &
				      0xFFFF0000)); // X axis Gyro offset

	StAdjPar124.StGvcOff.UsGyoVal =
		(UINT16)(SlMeasureAveValueB &
			 0x0000FFFF); //Measure Result Store
	if (((INT16)StAdjPar124.StGvcOff.UsGyoVal > (INT16)GYROF_UPPER) ||
	    ((INT16)StAdjPar124.StGvcOff.UsGyoVal < (INT16)GYROF_LOWER)) {
		UlRsltSts |= EXE_GYADJ;
	}
	RamWrite32A(GYRO_RAM_GYOFFZ, ((SlMeasureAveValueB << 16) &
				      0xFFFF0000)); // Y axis Gyro offset

	TRACE("GX_AVEOFT_RV = %08xh \n", (unsigned int)SlMeasureAveValueA);
	TRACE("GY_AVEOFT_RV = %08xh \n", (unsigned int)SlMeasureAveValueB);

	RamWrite32A(GYRO_RAM_GYROX_OFFSET,
		    0x00000000); // X axis Drift Gyro offset
	RamWrite32A(GYRO_RAM_GYROY_OFFSET,
		    0x00000000); // Y axis Drift Gyro offset
...
UINT32 HallAdj(ADJ_HALL *Ptr, ADJ_LOPGAN *LopgainPtr)
{
	...
	StAdjPar.StHalAdj.UsHlyCna =
		(UINT16)((INT16)StAdjPar.StHalAdj.UsHlyCna -
			 (INT16)(Ptr->MagneticOffset_Y));
	RamWrite32A(HALL_RAM_HYOFF,
		    (((UINT32)StAdjPar124.StHalAdj.UsHlyCna << 16) &
		     0xFFFF0000));
	StAdjPar.StHalAdj.UsHlxCna =
		(UINT16)((INT16)StAdjPar.StHalAdj.UsHlxCna -
			 (INT16)(Ptr->MagneticOffset_X));
	RamWrite32A(HALL_RAM_HXOFF,
		    (((UINT32)StAdjPar.StHalAdj.UsHlxCna << 16) & 0xFFFF0000));
	...
	StAdjPar.StHalAdj.UsHlyCna =
		(UINT16)((INT16)StAdjPar124.StHalAdj.UsHlyCna -
			 (INT16)Ptr->MagneticOffset_Y);
	RamWrite32A(HALL_RAM_HYOFF,
		    (((UINT32)StAdjPar124.StHalAdj.UsHlyCna << 16) &
		     0xFFFF0000));
	...
	StAdjPar.StHalAdj.UsHlxCna =
		(UINT16)((INT16)StAdjPar124.StHalAdj.UsHlxCna -
			 (INT16)Ptr->MagneticOffset_X);
	RamWrite32A(HALL_RAM_HXOFF,
		    (((UINT32)StAdjPar124.StHalAdj.UsHlxCna << 16) &
		     0xFFFF0000));
#ifdef HALL_ADJ_SERVO_ON
	...
	RamWrite32A(StCaliData_SiHallMax_After_X,
		    (((UINT32)StAdjPar124.StHalAdj.UsHlxMxa << 16) &
		     0xFFFF0000));
	...
	RamWrite32A(StCaliData_SiHallMin_After_X,
		    (((UINT32)StAdjPar124.StHalAdj.UsHlxMna << 16) &
		     0xFFFF0000));
	...
	RamWrite32A(StCaliData_SiHallMax_After_Y,
		    (((UINT32)StAdjPar124.StHalAdj.UsHlyMxa << 16) &
		     0xFFFF0000));
	...
	RamWrite32A(StCaliData_SiHallMin_After_Y,
		    (((UINT32)StAdjPar124.StHalAdj.UsHlyMna << 16) &
		     0xFFFF0000));
...
UINT32 TneRun(void)
{
	...
	//		}
	//	}else{
	if (((UlFinSts & EXE_HXADJ) == EXE_HXADJ) ||
	    ((UlFinSts & EXE_HYADJ) == EXE_HYADJ))
		return (UlFinSts);
		//	}
	...
	RamWrite32A(HALL_RAM_HXOFF,
		    (UINT32)((StAdjPar124.StHalAdj.UsAdxOff << 16) &
			     0xFFFF0000));
	RamWrite32A(HALL_RAM_HYOFF,
		    (UINT32)((StAdjPar124.StHalAdj.UsAdyOff << 16) &
			     0xFFFF0000));
	//20180906 Komori
...
UINT32 TnePtp(UINT8 UcDirSel, UINT8 UcBfrAft, ADJ_HALL *p, UINT8 UcSrvSwitch)
{
	...
	if (UcDirSel == X_DIR) { // X axis
		SlMeasureParameterA =
			HALL_RAM_HXIDAT; // Set Measure RAM Address
		SlMeasureParameterB =
			HALL_RAM_HYIDAT; // Set Measure RAM Address
		if (UcSrvSwitch != ON) {
...
#endif
	MeasureStart2124(SlMeasureParameterNum, SlMeasureParameterA,
			 SlMeasureParameterB, 50); // Start measure
	MeasureWait(); // Wait complete of measurement
	RamRead32A(StMeasFunc_MFA_SiMin1,
		   (UINT32 *)&SlMeasureMinValue); // Min value
	...
	RamRead32A(StMeasFunc_MFA_SiMax1,
		   (UINT32 *)&SlMeasureMaxValue); // Max value

	StTneVal.StDwdVal.UsHigVal =
		(UINT16)((SlMeasureMaxValue >> 16) & 0x0000FFFF);
	StTneVal.StDwdVal.UsLowVal =
		(UINT16)((SlMeasureMinValue >> 16) & 0x0000FFFF);
	...
	if (UcBfrAft == 0) {
		if (UcDirSel == X_DIR) {
			StAdjPar124.StHalAdj.UsHlxCen =
				((INT16)StTneVal.StDwdVal.UsHigVal +
				 (INT16)StTneVal.StDwdVal.UsLowVal) /
				2;
			StAdjPar124.StHalAdj.UsHlxMax =
				StTneVal.StDwdVal.UsHigVal;
			StAdjPar124.StHalAdj.UsHlxMin =
				StTneVal.StDwdVal.UsLowVal;
		} else if (UcDirSel == Y_DIR) {
			StAdjPar124.StHalAdj.UsHlyCen =
				((INT16)StTneVal.StDwdVal.UsHigVal +
				 (INT16)StTneVal.StDwdVal.UsLowVal) /
				2;
			StAdjPar124.StHalAdj.UsHlyMax =
				StTneVal.StDwdVal.UsHigVal;
			StAdjPar124.StHalAdj.UsHlyMin =
				StTneVal.StDwdVal.UsLowVal;
		}
	} else {
		if (UcDirSel == X_DIR) {
			StAdjPar124.StHalAdj.UsHlxCna =
				((INT16)StTneVal.StDwdVal.UsHigVal +
				 (INT16)StTneVal.StDwdVal.UsLowVal) /
				2;
			StAdjPar124.StHalAdj.UsHlxMxa =
				StTneVal.StDwdVal.UsHigVal;
			StAdjPar124.StHalAdj.UsHlxMna =
				StTneVal.StDwdVal.UsLowVal;
		} else if (UcDirSel == Y_DIR) {
			StAdjPar124.StHalAdj.UsHlyCna =
				((INT16)StTneVal.StDwdVal.UsHigVal +
				 (INT16)StTneVal.StDwdVal.UsLowVal) /
				2;
			StAdjPar124.StHalAdj.UsHlyMxa =
				StTneVal.StDwdVal.UsHigVal;
			StAdjPar124.StHalAdj.UsHlyMna =
				StTneVal.StDwdVal.UsLowVal;
		}
	}

	TRACE("		ADJ(%d) MAX = %04x, MIN = %04x, CNT = %04x, ", UcDirSel,
	      StTneVal.StDwdVal.UsHigVal, StTneVal.StDwdVal.UsLowVal,
	      ((signed int)StTneVal.StDwdVal.UsHigVal +
	       (signed int)StTneVal.StDwdVal.UsLowVal) /
		      2);
	StTneVal.StDwdVal.UsHigVal =
		0x7fff -
		StTneVal.StDwdVal
			.UsHigVal; // Maximum Gap = Maximum - Hall Peak Top
	StTneVal.StDwdVal.UsLowVal =
		StTneVal.StDwdVal.UsLowVal -
		0x8000; // Minimum Gap = Hall Peak Bottom - Minimum
...
UINT32 TneCen(UINT8 UcTneAxs, ADJ_HALL *ptr, UINT8 UcSrvSwtich)
{
	...
	if (BeforeControl != 0) {
		StTneVal.UlDwdVal =
			TnePtp(UcTneAxs, PTP_BEFORE, ptr, UcSrvSwtich);
	} else {
		StTneVal.UlDwdVal =
			TnePtp(UcTneAxs, PTP_AFTER, ptr, UcSrvSwtich);
	}
	BeforeControl = 0;
#if ((SELECT_VENDOR & 0x02) == 0x02) //JAHWA
	...
	while (UlTneRst && (UINT32)UcTmeOut) {
		if (UcTmeOut == 1 && (UcSrvSwtich != ON)) {
			TRACE("1st PtoP =%08x \n",
			      ((UINT16)0xFFFF - (StTneVal.StDwdVal.UsHigVal +
						 StTneVal.StDwdVal.UsLowVal)));
			// 初回のPtpで既にTarget Rangeに入っているなら終了。
			if ((((UINT16)0xFFFF - (StTneVal.StDwdVal.UsHigVal +
						StTneVal.StDwdVal.UsLowVal)) <
			     ptr->TargetMax) &&
			    (((UINT16)0xFFFF - (StTneVal.StDwdVal.UsHigVal +
						StTneVal.StDwdVal.UsLowVal)) >
		...
		if (UcTofRst == FAILURE) {
			...
			UlValNow =
				((UINT16)0xFFFF - (StTneVal.StDwdVal.UsHigVal +
						   StTneVal.StDwdVal.UsLowVal));
			if (UcTneAxs == X_DIR)
				UsValNow = StAdjPar.StHalAdj.UsHlxMxa;
			else if (UcTneAxs == Y_DIR)
				UsValNow = StAdjPar.StHalAdj.UsHlyMxa;
		...
		StTneVal.UlDwdVal =
			TnePtp(UcTneAxs, PTP_AFTER, ptr, UcSrvSwtich);
		if (UcTneAxs == X_DIR)
			UsValBef = StAdjPar.StHalAdj.UsHlxMxa;
		else if (UcTneAxs == Y_DIR)
			UsValBef = StAdjPar.StHalAdj.UsHlyMxa;

		if ((((StTneVal.StDwdVal.UsHigVal >
		       (0xFFFF - ptr->OffsetMargin)) &&
		      (StTneVal.StDwdVal.UsLowVal >
		       (0xFFFF - ptr->OffsetMargin))) ||
		     ((StTneVal.StDwdVal.UsHigVal < ptr->OffsetMargin) &&
		      (StTneVal.StDwdVal.UsLowVal <
		       ptr->OffsetMargin)))) { /* position check */
			UcTmeOut += 10;
			TRACE("------OFFSETMARGIN-------\n");
			...
			if ((((UINT16)0xFFFF - (StTneVal.StDwdVal.UsHigVal +
						StTneVal.StDwdVal.UsLowVal)) <
			     (ptr->TargetMax & 0xFFF0)) &&
			    (((UINT16)0xFFFF - (StTneVal.StDwdVal.UsHigVal +
						StTneVal.StDwdVal.UsLowVal)) >
			     (ptr->TargetMin & 0xFFF0))) {
		...
		if ((UcSrvSwtich == ON) &&
		    (UlValNow >
		     ((UINT16)0xFFFF - (StTneVal.StDwdVal.UsHigVal +
					StTneVal.StDwdVal.UsLowVal)))) {
			if ((((UsValNow - UsValBef) < 0x0100) &&
			     ((UsValNow - UsValBef) > 0)) ||
			    (((UsValBef - UsValNow) < 0x0100) &&
			     ((UsValBef - UsValNow) > 0))) {
				TRACE("---------!!Saturation!!---------\n");
				if ((UlBiaBef >
				     (BIAS_LLMT +
				      (UINT32)(ptr->DecrementStep << 16))) &&
				    (UlBiaNow != BIAS_HLMT)) {
	...
	while (UlTneRst && (UINT32)UcTmeOut) {
		...
		if ((StTneVal.StDwdVal.UsHigVal > ptr->OffsetMargin) &&
		    (StTneVal.StDwdVal.UsLowVal >
		     ptr->OffsetMargin)) /* position check */
		{
			UcTofRst = SUCCESS;
		...
		} else if ((StTneVal.StDwdVal.UsHigVal <= ptr->OffsetMargin) &&
			   (StTneVal.StDwdVal.UsLowVal <= ptr->OffsetMargin)) {
			UcTofRst = SUCCESS;
			...
			if (UcTneAxs == X_DIR) {
				RamRead32A(StCaliData_UiHallOffset_X,
					   &UlValNow);
				UsValNow = (UINT16)(UlValNow >> 16);
			} else if (UcTneAxs == Y_DIR) {
				RamRead32A(StCaliData_UiHallOffset_Y,
					   &UlValNow);
				UsValNow = (UINT16)(UlValNow >> 16);
			}
			if ((((UsValBef & 0xFF00) == 0x1000) &&
			     (UsValNow & 0xFF00) == 0x1000) ||
			    (((UsValBef & 0xFF00) == 0xEF00) &&
			     (UsValNow & 0xFF00) == 0xEF00)) {
				UcTmeOut += 10;
				TRACE("	No = %04d (offset count up)\n",
				      UcTmeOut);
				if (UcTneAxs == X_DIR) {
					RamRead32A(StCaliData_UiHallBias_X,
						   &UlBiasVal);
					UsBiasVal = (UINT16)(UlBiasVal >> 16);
				} else if (UcTneAxs == Y_DIR) {
					RamRead32A(StCaliData_UiHallBias_Y,
						   &UlBiasVal);
					UsBiasVal = (UINT16)(UlBiasVal >> 16);
				}
				...
				if (UcTneAxs == X_DIR) {
					...
					RamWrite32A(StCaliData_UiHallBias_X,
						    UlBiasVal);
				} else if (UcTneAxs == Y_DIR) {
					UlBiasVal = (UINT32)(UsBiasVal << 16);
					DacControl(HLYBO, UlBiasVal);
					RamWrite32A(StCaliData_UiHallBias_Y,
						    UlBiasVal);
				}
			}
		}
	    ...
	    UINT8 UcSrvSwitch)
{
	...
	if ((StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal) ==
		...
		UlSetBia /= (UINT32)(0xFFFF - (StTneVal.StDwdVal.UsHigVal +
					       StTneVal.StDwdVal.UsLowVal));
#if ((SELECT_VENDOR & 0x02) == 0x02) //JAHWA
...
void TneOff(UnDwdVal StTneVal, UINT8 UcTneAxs)
{
	...
	if (StTneVal.StDwdVal.UsHigVal > StTneVal.StDwdVal.UsLowVal) {
		UlSetVal =
			(UINT32)((StTneVal.StDwdVal.UsHigVal -
				  StTneVal.StDwdVal.UsLowVal) /
				 OFFSET_DIV); // Calculating Value For Increase Step
		UlSetOff += UlSetVal; // Calculating Value For Increase Step
		if (UlSetOff > 0x0000FFFF)
			UlSetOff = 0x0000FFFF;
	} else {
		UlSetVal =
			(UINT32)((StTneVal.StDwdVal.UsLowVal -
				  StTneVal.StDwdVal.UsHigVal) /
				 OFFSET_DIV); // Calculating Value For Decrease Step
		if (UlSetOff < UlSetVal) {
			UlSetOff = 0x00000000;
		} else {
			UlSetOff -=
				UlSetVal; // Calculating Value For Decrease Step
		}
	}
...
UINT32 LopGan(UINT8 UcDirSel, ADJ_LOPGAN *ptr)
{
	...
	SetTransDataAdr124(SinWave_OutAddr,
			   (UINT32)UsSinAdr); // Set Sine Wave Input RAM

	MesFil124(1 /*LOOPGAIN*/); // Filter setting for measurement
	MeasureStart124(ptr->NoiseNum, SlMeasureParameterA,
			SlMeasureParameterB); // Start measure
	MeasureWait124(); // Wait complete of measurement
	...
	SetTransDataAdr124(SinWave_OutAddr,
			   (UINT32)0x00000000); // Set Sine Wave Input RAM
	RamWrite32A(UsSinAdr, 0x00000000); // DelayRam Clear

	RamRead32A(StMeasFunc_MFA_LLiAbsInteg1,
		   &StMeasValueA.StUllnVal.UlLowVal); // X axis
	RamRead32A(StMeasFunc_MFA_LLiAbsInteg1 + 4,
		   &StMeasValueA.StUllnVal.UlHigVal);
	RamRead32A(StMeasFunc_MFB_LLiAbsInteg2,
		   &StMeasValueB.StUllnVal.UlLowVal); // Y axis
	RamRead32A(StMeasFunc_MFB_LLiAbsInteg2 + 4,
		   &StMeasValueB.StUllnVal.UlHigVal);
	...
	if (UcDirSel == X_DIR) { // X axis
		UllCalculateVal = (StMeasValueB.UllnValue * 1000 /
				   StMeasValueA.UllnValue) *
				  ptr->Hxgain / ptr->Gap;
		if (UllCalculateVal > (UINT64)0x000000007FFFFFFF)
			UllCalculateVal = (UINT64)0x000000007FFFFFFF;
		StAdjPar124.StLopGan.UlLxgVal = (UINT32)UllCalculateVal;
		RamWrite32A(HallFilterCoeffX_hxgain0,
			    StAdjPar124.StLopGan.UlLxgVal);
		TRACE("LopGain UlLxgVal = %08xh \n",
		      (unsigned int)StAdjPar124.StLopGan.UlLxgVal);
		if ((UllCalculateVal > ptr->XJudgeHigh) ||
		    (UllCalculateVal < ptr->XJudgeLow)) {
...
void SetSinWavePara(UINT8 UcTableVal, UINT8 UcMethodVal)
{
	...
	{
		RamWrite32A(SinWave_Offset,
			    0x00000000); // 発生周波数のオフセットを設定
		RamWrite32A(SinWave_Phase, 0x60000000); // 正弦波の位相量

		RamWrite32A(CosWave_Offset,
			    0x00000000); // 発生周波数のオフセットを設定
		RamWrite32A(CosWave_Phase, 0x60000000); // 正弦波の位相量
		...
		SetTransDataAdr124(SinWave_OutAddr,
				   0x00000000); // 出力先アドレス
		SetTransDataAdr124(CosWave_OutAddr,
				   0x00000000); // 出力先アドレス
		RamWrite32A(HALL_RAM_HXOFF1, 0x00000000); // DelayRam Clear
		RamWrite32A(HALL_RAM_HYOFF1, 0x00000000); // DelayRam Clear
	} else {
		RamWrite32A(SinWave_Offset,
			    UlFreqDat); // 発生周波数のオフセットを設定
		RamWrite32A(CosWave_Offset,
			    UlFreqDat); // 発生周波数のオフセットを設定

		RamWrite32A(SinWaveC_Regsiter, 0x00000001); // Sine Wave Start
		SetTransDataAdr124(SinWave_OutAddr,
				   (UINT32)HALL_RAM_HXOFF1); // 出力先アドレス
		SetTransDataAdr124(CosWave_OutAddr,
				   (UINT32)HALL_RAM_HYOFF1); // 出力先アドレス
	}
}
...
void TneHvc(void)
{
	...
	MesFil124(THROUGH); // Set Measure Filter
	MeasureStart124(ADOFF_ROUGH_NUM, HALL_RAM_HXIDAT,
			HALL_RAM_HYIDAT); // Start measure
	MeasureWait124(); // Wait complete of measurement
	...
	SlMeasureAveValueA = (INT32)((((INT64)StMeasValueA.UllnValue * 100) /
				      ADOFF_ROUGH_NUM) /
				     100);
	SlMeasureAveValueB = (INT32)((((INT64)StMeasValueB.UllnValue * 100) /
				      ADOFF_ROUGH_NUM) /
				     100);

	StAdjPar124.StHalAdj.UsHlxCna =
		(UINT16)((SlMeasureAveValueA >> 16) &
			 0x0000FFFF); //Measure Result Store
	StAdjPar124.StHalAdj.UsHlxCen =
		StAdjPar124.StHalAdj.UsHlxCna; //Measure Result Store

	StAdjPar124.StHalAdj.UsHlyCna =
		(UINT16)((SlMeasureAveValueB >> 16) &
			 0x0000FFFF); //Measure Result Store
	StAdjPar124.StHalAdj.UsHlyCen =
		StAdjPar124.StHalAdj.UsHlyCna; //Measure Result Store
...
void TneFin(ADJ_LOPGAN *ptr)
{
	...
	MesFil124(THROUGH); // Filter setting for measurement
	MeasureStart124(ADOFF_FINE_NUM, HALL_RAM_HALL_X_OUT,
			HALL_RAM_HALL_Y_OUT); // Start measure
	MeasureWait124(); // Wait complete of measurement
	...
	while (UcFin) {
		if (UcFin & 0x01) {
			if (UlMinimumValueA >= abs(SlMeasureAveValueA)) {
				...
				if (SlMeasureAveValueA > 0)
					UsAdxOff = (INT16)UsAdxOff +
						   (SlMeasureAveValueA >> 17) +
						   1;
				else
					UsAdxOff = (INT16)UsAdxOff +
						   (SlMeasureAveValueA >> 17) -
		...
		if (UcFin & 0x10) {
			if (UlMinimumValueB >= abs(SlMeasureAveValueB)) {
				...
				if (SlMeasureAveValueB > 0)
					UsAdyOff = (INT16)UsAdyOff +
						   (SlMeasureAveValueB >> 17) +
						   1;
				else
					UsAdyOff = (INT16)UsAdyOff +
						   (SlMeasureAveValueB >> 17) -
		...
		RamRead32A(StMeasFunc_MFA_LLiIntegral1 + 4,
			   &StMeasValueA.StUllnVal.UlHigVal);
		SlMeasureAveValueA =
			(INT32)((((INT64)StMeasValueA.UllnValue * 100) /
				 ADOFF_FINE_NUM) /
				100);
		...
		RamRead32A(StMeasFunc_MFB_LLiIntegral2 + 4,
			   &StMeasValueB.StUllnVal.UlHigVal);
		SlMeasureAveValueB =
			(INT32)((((INT64)StMeasValueB.UllnValue * 100) /
				 ADOFF_FINE_NUM) /
				100);
...
UINT32 TneZeroServo(UINT8 ucposture, float DegreeGap)
{
	...
	if (ucposture < 0x80) {
		...
		for (i = ucposture; i < (ucposture + 2); i++) {
			...
			case 0: /* +Y */
				SlMeasureParameterA =
					ZeroServoRAM_X_IN; // Set Measure RAM Address
				SlMeasureParameterB =
					GYRO_RAM_GZ_ADIDAT; // Set Measure RAM Address
				break;
			case 1:
				SlMeasureParameterA =
					HALL_RAM_HYOUT2; // Set Measure RAM Address
				SlMeasureParameterB =
					ZeroServoRAM_Y_IN; // Set Measure RAM Address
				break;
			case 2: /* +X */
				SlMeasureParameterA =
					ZeroServoRAM_Y_IN; // Set Measure RAM Address
				SlMeasureParameterB =
					ZeroServoRAM_Z_IN; // Set Measure RAM Address
				break;
			case 3:
				SlMeasureParameterA =
					HALL_RAM_HXOUT2; // Set Measure RAM Address
				SlMeasureParameterB =
					ZeroServoRAM_X_IN; // Set Measure RAM Address
				break;
			}

			MeasureStart124(SlMeasureParameterNum,
					SlMeasureParameterA,
					SlMeasureParameterB); // Start measure
			...
			SlMeasureAveValueA =
				(INT32)((INT64)StMeasValueA.UllnValue /
					SlMeasureParameterNum);
			SlMeasureAveValueB =
				(INT32)((INT64)StMeasValueB.UllnValue /
					SlMeasureParameterNum);

			switch (i) {
			case 0:
				StZeroServoX.SlOffset =
					SlMeasureAveValueA; // i case 0 : ZeroServoRAM_X_IN
				GYROZ_OFFSET = SlMeasureAveValueB;
				break;
			case 2:
				StZeroServoY.SlOffset =
					SlMeasureAveValueA; // i case 2 : ZeroServoRAM_Y_IN
		...
		TRACE("VAL(H,A) pos = \t%08xh\t%08xh\t%d \n",
		      (int)SlMeasureAveValueA, (int)SlMeasureAveValueB,
		      ucposture);
		...
		case 0: /* +Y */
			if (SlMeasureAveValueB < (int)(POSTURETH_P << 16)) {
				...
				StZeroServoMesY.SlHallP =
					SlMeasureAveValueA; // i case 1 : HALL_RAM_HYOUT2
				StZeroServoMesY.SlAcclP =
					SlMeasureAveValueB; // i case 1 : ZeroServoRAM_Y_IN
		...
		case 2: /* +X */
			if (SlMeasureAveValueB < (int)(POSTURETH_P << 16)) {
				...
				StZeroServoMesX.SlHallP =
					SlMeasureAveValueA; // i case 3 : HALL_RAM_HXOUT2
				StZeroServoMesX.SlAcclP =
					SlMeasureAveValueB; // i case 3 : ZeroServoRAM_X_IN
		...
		case 0x80: /* 計算 */
			...
			if (UlPostureSt == 0x05L) {
				...
				StZeroServoMesX.SlAcclP -=
					StZeroServoX.SlOffset;
				dTemp = (double)2147483647.0 /
					(double)(StZeroServoMesX.SlAcclP);
				for (StZeroServoX.SlShift = 0;
				     StZeroServoX.SlShift <= 5;
				     StZeroServoX.SlShift++) {
					if (fabs(dTemp) <= 1) {
						break;
					}
					dTemp = dTemp / (double)2;
				}
				StZeroServoX.SlGcora =
					(INT32)(dTemp * (double)2147483647.0);
				StZeroServoX.SlShift = StZeroServoX.SlShift
						       << 8;
				...
				StZeroServoMesY.SlAcclP -=
					StZeroServoY.SlOffset;
				dTemp = (double)2147483647.0 /
					(double)(StZeroServoMesY.SlAcclP);
				for (StZeroServoY.SlShift = 0;
				     StZeroServoY.SlShift <= 5;
				...
				StZeroServoY.SlGcora =
					(INT32)(dTemp * (double)2147483647.0);
				StZeroServoY.SlShift = StZeroServoY.SlShift
						       << 8;

				OffsetAngle = (double)(DegreeGap) *
					      3.141592653589793238 / 180.0f;

				StZeroServoX.SlGaina =
					(INT32)((float)StZeroServoMesX.SlHallP /
						cos(OffsetAngle));
				StZeroServoY.SlGaina =
					(INT32)((float)StZeroServoMesY.SlHallP /
						cos(OffsetAngle));

				TRACE("   X    ,   Y    ,   angle = %f \n",
				      DegreeGap);
				TRACE("%08xh,%08xh(Offset)\n",
				      (int)StZeroServoX.SlOffset,
				      (int)StZeroServoY.SlOffset);
				TRACE("%08xh,%08xh(Gcora)\n",
				      (int)StZeroServoX.SlGcora,
				      (int)StZeroServoY.SlGcora);
				TRACE("%08xh,%08xh(Gaina)\n",
				      (int)StZeroServoX.SlGaina,
				      (int)StZeroServoY.SlGaina);
				TRACE("%08xh,%08xh(Shift)\n",
				      (int)StZeroServoX.SlShift,
				      (int)StZeroServoY.SlShift);
				TRACE("%08xh,%08xh(AZ , GZ)\n",
				      (int)StZeroServoZ.SlOffset,
				      (int)GYROZ_OFFSET);
				RamWrite32A(ZeroServoRAM_X_OFFSET,
					    StZeroServoX.SlOffset);
				RamWrite32A(ZeroServoFilterTableX_gcora,
					    StZeroServoX.SlGcora);
				RamWrite32A(ZeroServoFilterTableX_gaina,
					    StZeroServoX.SlGaina);
				RamWrite32A(ZeroServoFilterTableX_shift,
					    StZeroServoX.SlShift);

				RamWrite32A(ZeroServoRAM_Y_OFFSET,
					    StZeroServoY.SlOffset);
				RamWrite32A(ZeroServoFilterTableY_gcora,
					    StZeroServoY.SlGcora);
				RamWrite32A(ZeroServoFilterTableY_gaina,
					    StZeroServoY.SlGaina);
				RamWrite32A(ZeroServoFilterTableY_shift,
					    StZeroServoY.SlShift);

				RamWrite32A(ZeroServoRAM_Z_OFFSET,
					    StZeroServoZ.SlOffset);
...
UINT8 ZeroServoLmt(UINT8 UCMODE)
{
	...
	TRACE("		before X (gain , shift , coeff) = ( %08x , %08x , %08x )\n",
	      (int)StZeroServoX.SlGcora, (int)StZeroServoX.SlShift,
	      (int)UlFilCoef);
...
UINT32 TneAvc(UINT8 ucposture)
{
	...
	if (ucposture < 0x7f) {
		...
		SlMeasureAveValueA = (INT32)((INT64)StMeasValueA.UllnValue /
					     SlMeasureParameterNum);
		SlMeasureAveValueB = (INT32)((INT64)StMeasValueB.UllnValue /
					     SlMeasureParameterNum);
		...
		TRACE("VAL(X,Y,Z) pos = \t%08xh\t%08xh\t%08xh\t%d \n",
		      ...
		      (unsigned int)SlMeasureRetValueZ, ucposture);
		if ((SlMeasureRetValueZ < (INT32)(POSTURETH_P << 16)) &&
		    (ucposture == 0x10)) {
			UlRsltSts = EXE_ERROR;
			TRACE(" POS14 [ERROR] \t%08xh < %08xh\n",
			      (unsigned int)(SlMeasureRetValueZ),
			      (unsigned int)(POSTURETH_P << 16));
		} else if ((SlMeasureRetValueZ > (INT32)(POSTURETH_M << 16)) &&
			   (ucposture == 0x11)) {
			UlRsltSts = EXE_ERROR;
			TRACE(" POS14 [ERROR] \t%08xh > %08xh\n",
			      (unsigned int)(SlMeasureRetValueZ),
			      (unsigned int)(POSTURETH_M << 16));
		} else {
			TRACE("DEBUG = \t%08xh\t \n",
			      abs((INT32)(ACCL_SENS << 16) -
				  abs(SlMeasureRetValueZ)));
			if (abs(SlMeasureRetValueX) > ZEROG_MRGN_XY)
				UlRsltSts |= EXE_GXADJ;
			if (abs(SlMeasureRetValueY) > ZEROG_MRGN_XY)
				UlRsltSts |= EXE_GYADJ;
			if (abs((INT32)(ACCL_SENS << 16) -
				abs(SlMeasureRetValueZ)) > ZEROG_MRGN_Z)
				UlRsltSts |= EXE_GZADJ;
			if (UlRsltSts == EXE_END) {
				...
				TRACE("POS14(X,Y,Z) st = \t%08xh\t%08xh\t%08xh\t%08xh \n",
				      (unsigned int)StPosOff124.StPos.Pos[4][0],
				      (unsigned int)
					      StPosOff124124124.StPos.Pos[4][1],
				      (unsigned int)
					      StPosOff124124124.StPos.Pos[4][2],
				      (unsigned int)StPosOff124124124.UlAclOfSt);

				SlDiff[0] = SlMeasureRetValueX - (INT32)0;
				SlDiff[1] = SlMeasureRetValueY - (INT32)0;
				if (ucposture == 0x10) {
					SlDiff[2] = SlMeasureRetValueZ -
						    (INT32)(ACCL_SENS << 16);
				} else {
					SlDiff[2] = SlMeasureRetValueZ -
						    (INT32)(ACCL_SENS_M << 16);
				}
		...
		case 0x80: /* 計算 */
			...
			if (StPosOff124.UlAclOfSt == 0x3fL) {
				/*X offset*/
				StAclVal124.StAccel.SlOffsetX =
					StPosOff124.StPos.Pos[4][0];
				/*Y offset*/
				StAclVal124.StAccel.SlOffsetY =
					StPosOff124.StPos.Pos[4][1];
				/*Z offset*/
				StAclVal124.StAccel.SlOffsetZ =
					StPosOff124.StPos.Pos[4][2];
#ifdef DEBUG
				TRACE("ACLOFST(X,Y,Z) = \t%08xh\t%08xh\t%08xh \n",
				      (unsigned int)
					      StAclVal124.StAccel.SlOffsetX,
				      (unsigned int)StAclVal124124124.StAccel
					      .SlOffsetY,
				      (unsigned int)StAclVal124124124.StAccel
					      .SlOffsetZ);
#endif //DEBUG
				...
				RamWrite32A(
					ZeroServoRAM_X_OFFSET,
					StAclVal124.StAccel
						.SlOffsetX); // X axis Accel offset
				RamWrite32A(
					ZeroServoRAM_Y_OFFSET,
					StAclVal124.StAccel
						.SlOffsetY); // Y axis Accel offset
				RamWrite32A(
					ZeroServoRAM_Z_OFFSET,
					StAclVal124.StAccel
						.SlOffsetZ); // Z axis Accel offset

				StAclVal124.StAccel.SlOffsetX =
					(StAclVal124.StAccel.SlOffsetX >> 16) &
					0x0000FFFF;
				StAclVal124.StAccel.SlOffsetY =
					(StAclVal124.StAccel.SlOffsetY >> 16) &
					0x0000FFFF;
				StAclVal124.StAccel.SlOffsetZ =
					(StAclVal124.StAccel.SlOffsetZ >> 16) &
					0x0000FFFF;
				...
				for (j = 0; j < 6; j++) {
					...
					RamWrite32A(
						AcclFilDly_X + k,
						0x00000000); // X axis Accl LPF Clear
					RamWrite32A(
						AcclFilDly_Y + k,
						0x00000000); // Y axis Accl LPF Clear
					RamWrite32A(
						AcclFilDly_Z + k,
						0x00000000); // Z axis Accl LPF Clear
				}
			...
			break;
		case 0xFF: /* RAM clear */
			MemClr((UINT8 *)&StPosOff124,
			       sizeof(stPosOff)); // Adjust Parameter Clear
			MemClr((UINT8 *)&StAclVal124,
			       sizeof(stAclVal)); // Adjust Parameter Clear
...
UINT8 Actuator_Moving(UINT8 uc_axis, Act_Mov_t *pt_parameter, int *ul_readval)
{
	...
	while (targetcode > pt_parameter->endcode) {
		...
		MeasureStart124(1024, HALL_RAM_HXIDAT,
				HALL_RAM_HYIDAT); // Measurement Starting
		...
		if (!uc_axis) { // X Axis
			*ul_readval =
				(INT32)((INT64)StMeasValueA.UllnValue / 1024);
		} else { // Y Axis
			*ul_readval =
				(INT32)((INT64)StMeasValueB.UllnValue / 1024);
		}
...
{
	UINT16 us_peak_to_peak_x, us_rated_peak_to_peak_x, us_peak_to_peak_y,
		us_rated_peak_to_peak_y;
	...
	// Initialize
	if ((short)StAdjPar124.StHalAdj.UsHlxMxa <
	    (short)StAdjPar124.StHalAdj.UsHlxMna) {
		us_status = FAILURE;
		return (us_status);
	}

	if ((short)StAdjPar124.StHalAdj.UsHlyMxa <
	    (short)StAdjPar124.StHalAdj.UsHlyMna) {
		us_status = FAILURE;
		return (us_status);
	}
	...
	us_peak_to_peak_x =
		(unsigned short)((long)StAdjPar124.StHalAdj.UsHlxMxa -
				 (long)StAdjPar124.StHalAdj.UsHlxMna);
	us_rated_peak_to_peak_x = (unsigned short)((long)us_peak_to_peak_x *
						   RATED_STROKE_RANGE / 100L);
	sl_step_x = us_rated_peak_to_peak_x / 6;

	us_peak_to_peak_y =
		(unsigned short)((long)StAdjPar124.StHalAdj.UsHlyMxa -
				 (long)StAdjPar124.StHalAdj.UsHlyMna);
	us_rated_peak_to_peak_y = (unsigned short)((long)us_peak_to_peak_y *
						   RATED_STROKE_RANGE / 100L);
	...
	for (i = 0; i < 7; i++) {
		RamWrite32A(HALL_RAM_GYROX_OUT,
			    (unsigned long)sl_position_x[i]);
		WitTim(100);
	...
	RamWrite32A(StPosition_0,
		    (unsigned long)((long)((ss_laser_y[0] * 10) << 16) |
				    (ss_laser_x[0] * 10)));
	RamWrite32A(StPosition_1,
		    (unsigned long)((long)((ss_laser_y[1] * 10) << 16) |
				    (ss_laser_x[1] * 10)));
	RamWrite32A(StPosition_2,
		    (unsigned long)((long)((ss_laser_y[2] * 10) << 16) |
				    (ss_laser_x[2] * 10)));
	RamWrite32A(StPosition_3,
		    (unsigned long)((long)((ss_laser_y[3] * 10) << 16) |
				    (ss_laser_x[3] * 10)));
	RamWrite32A(StPosition_4,
		    (unsigned long)((long)((ss_laser_y[4] * 10) << 16) |
				    (ss_laser_x[4] * 10)));
	RamWrite32A(StPosition_5,
		    (unsigned long)((long)((ss_laser_y[5] * 10) << 16) |
				    (ss_laser_x[5] * 10)));
	RamWrite32A(StPosition_6,
		    (unsigned long)((long)((ss_laser_y[6] * 10) << 16) |
				    (ss_laser_x[6] * 10)));
...
UINT8 CircleTest(UINT8 uc_freq, UINT8 us_amp)
{
	...
	if (((sl_ach_max - sl_ach_min) > TRACKING_THRESHOLD) ||
	    ((sl_bch_max - sl_bch_min) > TRACKING_THRESHOLD)) {
		uc_status = FAILURE;
	}
...
UINT8 OscillationDetection(INT8 sc_offset_x, INT8 sc_offset_y,
			   UINT16 us_wait_time)
{
	UINT16 us_amplitude_x, us_amplitude_y;
	...
	sl_offset_x = (UINT32)((UINT32)us_amplitude_x * sc_offset_x / 100)
		      << 16;
	sl_offset_y = (UINT32)((UINT32)us_amplitude_y * sc_offset_y / 100)
		      << 16;
	TRACE("Offset X = %x\n", sl_offset_x);
	TRACE("Offset Y = %x\n", sl_offset_y);
	...
	if (((sl_ach_max - sl_ach_min) > OSCILLATION_THRESHOLD) ||
	    ((sl_bch_max - sl_bch_min) > OSCILLATION_THRESHOLD)) {
		uc_status = FAILURE;
	}
...
//For ONSEMI 217
UINT16 Us_Af_Position[9] = { 0x0000, 0x0080, 0x0100, 0x0180, 0x0200,
			     0x0280, 0x0300, 0x0380, 0x03FF };
#elif (SELECT_VENDOR == 0x02)
//For AKM7374
INT16 Us_Af_Position[9] = { 0x0000, 0x2000, 0x4000, 0x6000, 0x8000,
			    0xA000, 0xC000, 0xE000, 0xFFFF };
extern int TWI_WriteMultiple(int SlaveAddr, char *data,
...
UINT8 OIS_PosMes_by_AF(void)
{
	...
	for (i = 0; i < 9; i++) {
		...
		sl_hall_x[i] = (INT32)((INT64)StMeasValueA.UllnValue / 1024) -
			       sl_default_x;
		sl_hall_y[i] = (INT32)((INT64)StMeasValueB.UllnValue / 1024) -
			       sl_default_y;
	...
	for (i = 1; i < 9; i++) {
		if (abs(sl_hall_x[i] - sl_hall_x[i - 1]) >
		    OIS_POSITION_THRESHOLD) {
			return (FAILURE);
		}

		if (abs(sl_hall_y[i] - sl_hall_y[i - 1]) >
		    OIS_POSITION_THRESHOLD) {
			return (FAILURE);
		}
	}
...
void Read_OIS_PosData_by_AF(void)
{
	...
	for (i = 0; i < 8; i++) {
		...
		Sl_Hall_X_Slope_by_AF[i] =
			(Sl_Hall_X_by_AF[i + 1] - Sl_Hall_X_by_AF[i]) /
			(INT32)(Us_Af_Position[i + 1] - Us_Af_Position[i]);
		Sl_Hall_Y_Slope_by_AF[i] =
			(Sl_Hall_Y_by_AF[i + 1] - Sl_Hall_Y_by_AF[i]) /
			(INT32)(Us_Af_Position[i + 1] - Us_Af_Position[i]);
#else
		Sl_Hall_X_Slope_by_AF[i] =
			(INT32)((Sl_Hall_X_by_AF[i + 1] - Sl_Hall_X_by_AF[i]) /
				(Us_Af_Position[i + 1] - Us_Af_Position[i]));
		Sl_Hall_Y_Slope_by_AF[i] =
			(INT32)((Sl_Hall_Y_by_AF[i + 1] - Sl_Hall_Y_by_AF[i]) /
				(Us_Af_Position[i + 1] - Us_Af_Position[i]));
#endif
	}
...
#if (SELECT_VENDOR == 0x02)
			sl_offset_x =
				(INT32)(Sl_Hall_X_Slope_by_AF[i - 1] *
						((INT32)us_af_code -
						 (INT32)Us_Af_Position[i - 1]) +
					Sl_Hall_X_by_AF[i - 1]);
			sl_offset_y =
				(INT32)(Sl_Hall_Y_Slope_by_AF[i - 1] *
						((INT32)us_af_code -
						 (INT32)Us_Af_Position[i - 1]) +
					Sl_Hall_Y_by_AF[i - 1]);
#else
			sl_offset_x = (INT32)(Sl_Hall_X_Slope_by_AF[i - 1] *
						      (us_af_code -
						       Us_Af_Position[i - 1]) +
					      (Sl_Hall_X_by_AF[i - 1] -
					       Sl_Hall_X_by_AF[0]));
			sl_offset_y = (INT32)(Sl_Hall_Y_Slope_by_AF[i - 1] *
						      (us_af_code -
						       Us_Af_Position[i - 1]) +
					      (Sl_Hall_Y_by_AF[i - 1] -
					       Sl_Hall_Y_by_AF[0]));
...
UINT8 CalcSetMizxAndLinearityData(mlLinearityValue *linval,
				  mlMixingValue *mixval)
{
	...
	Xa = ((N * Ysum_xy) - (Xsum_x * Ysum_y)) /
	     ((N * Xsum_xy) - (Xsum_x * Xsum_y));
	...
	Ya = ((N * Xsum_xy) - (Xsum_x * Xsum_y)) /
	     ((N * Ysum_xy) - (Xsum_x * Ysum_y));
#else
	for (i = 0; i < N; i++) {
	...
	Xa = ((N * Xsum_xy) - (Xsum_x * Xsum_y)) /
	     ((N * Xsum_x2) - pow(Xsum_x, 2));
	Ya = ((N * Ysum_xy) - (Ysum_x * Ysum_y)) /
	     ((N * Ysum_x2) - pow(Ysum_x, 2));
#endif
	...
	mixval->hx45x = +(cos(mixval->radianY) /
			  cos(mixval->radianX - mixval->radianY));
	mixval->hx45y = -(sin(mixval->radianY) /
			  cos(mixval->radianX - mixval->radianY));
	...
	mixval->hy45y = +(cos(mixval->radianX) /
			  cos(mixval->radianX - mixval->radianY));
	mixval->hy45x = +(sin(mixval->radianX) /
			  cos(mixval->radianX - mixval->radianY));

	mixval->hxsx =
		(unsigned char)abs(mixval->hx45x); // >1ならばシフト数として設定
	mixval->hysx =
		(unsigned char)abs(mixval->hy45y); // >1ならばシフト数として設定

	mixval->hx45x = mixval->hx45x /
			pow(2, (double)mixval->hxsx); // シフトを加味して再計算
	mixval->hy45y = mixval->hy45y /
			pow(2, (double)mixval->hysx); // シフトを加味して再計算
	...
	RamWrite32A(HF_ShiftX,
		    ((UINT32)mixval->hxsx << 0) | ((UINT32)mixval->hysx << 8));
...
UINT8 Read_BMI260_CHIP_ID(void)
{
	...
	struct bmi2_dev dev = { .dev_id = BMI2_I2C_PRIM_ADDR,
...
UINT8 Configuring_BMI260(void)
{
	...
	do {
		WriteData = 0x0E;
		bmi2_set_regs(PWR_CTRL_260, &WriteData, 1,
			      &dev); // write to BMI260
		bmi2_get_regs(PWR_CTRL_260, &ReadData, 1,
			      &dev); // read from BMI260
