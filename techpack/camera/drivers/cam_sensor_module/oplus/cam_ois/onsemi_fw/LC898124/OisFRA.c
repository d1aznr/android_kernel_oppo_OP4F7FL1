...
UINT32 Freq_Convert(float SfFreq)
{
	...
	UlPhsStep =
		(UINT32)((SfFreq * (float)0x100000000 / FS_FREQ + 0.5F) / 2.0F);
...
void MesStart_FRA_Single(UINT8 UcDirSel)
{
	...
	RamWrite32A(SinWave_Gain,
		    (UINT32)((float)0x7FFFFFFF * SfTmp)); // Set Sine Wave Gain
	...
	if (StFRAParam.StHostCom.UcAvgCycl == 10) { // Actuator Through
		...
		if (UcDirSel == X_DIR) {
			SetTransDataAdr124(
				SinWave_OutAddr,
				(UINT32)HALL_FRA_XSININ); // Set Sine Wave Input RAM
			...
			SetTransDataAdr124(
				SinWave_OutAddr,
				(UINT32)HALL_FRA_YSININ); // Set Sine Wave Input RAM
			// Set parameter and input/output address
		...
		if (UcDirSel == X_DIR) {
			SetTransDataAdr124(
				SinWave_OutAddr,
				(UINT32)HALL_RAM_SINDX1); // Set Sine Wave Input RAM
			// Set parameter and input/output address
			...
			SetTransDataAdr124(
				SinWave_OutAddr,
				(UINT32)HALL_RAM_SINDY1); // Set Sine Wave Input RAM
			// Set parameter and input/output address
			...
			SetTransDataAdr124(
				SinWave_OutAddr,
				(UINT32)CLAF_RAMA_AFOUT); // Set Sine Wave Input RAM
			// Set parameter and input/output address
		...
		if (UcDirSel == X_DIR) {
			...
			SetTransDataAdr124(
				SinWave_OutAddr,
				(UINT32)CLAF_RAMA_AFSINE); // Set Sine Wave Input RAM
			// Set parameter and input/output address
#ifdef CLOSED_RESPONSE
