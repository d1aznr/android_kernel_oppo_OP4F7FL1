...
void MeasureStart2(INT_32, INT_32, INT_32,
		   UINT_16); //!< Measure Start 2 Function
...
void TneBia(UnDwdVal StTneVal, UINT_8 UcTneAxs,
	    UINT_16 UsHalAdjRange); //!< Hall Bias Tuning
...
UINT_32 HallAdj(ADJ_HALL *Ptr, ADJ_LOPGAN *LpPtr)
{
	...
	RamWrite32A(HALL_RAM_HYOFF,
		    (UINT_32)((StAdjPar.StHalAdj.UsAdyOff << 16) & 0xFFFF0000));
	...
	RamWrite32A(HALL_RAM_HXOFF,
		    (UINT_32)((StAdjPar.StHalAdj.UsAdxOff << 16) & 0xFFFF0000));
...
UINT_32 TneRun(void)
{
	...
	// Select parameter
	if (GetInfomationAfterStartUp(&Info) != 0)
		return (EXE_ERR);
...
UINT_32 TnePtp(UINT_8 UcDirSel, UINT_8 UcBfrAft, ADJ_HALL *p)
{
	...
	RamRead32A(StMeasFunc_MFA_SiMin1,
		   (unsigned long *)&SlMeasureMinValue); // Min value
	...
	RamRead32A(StMeasFunc_MFA_SiMax1,
		   (unsigned long *)&SlMeasureMaxValue); // Max value

	StTneVal.StDwdVal.UsHigVal =
		(unsigned short)((SlMeasureMaxValue >> 16) & 0x0000FFFF);
	StTneVal.StDwdVal.UsLowVal =
		(unsigned short)((SlMeasureMinValue >> 16) & 0x0000FFFF);
	...
	WitTim(p->WaitTime);
	MeasureStart(p->SinNum, SlMeasureParameterA,
		     SlMeasureParameterB); // Start measure
	MeasureWait(); // Wait complete of measurement
	RamRead32A(StMeasFunc_MFA_SiMin1,
		   (UINT_32 *)&SlMeasureMinValue); // Min value
	...
	MeasureWait(); // Wait complete of measurement
	RamRead32A(StMeasFunc_MFA_SiMin1,
		   (UINT_32 *)&SlMeasureMaxValue); // Min value

	RamWrite32A(UsSinAdr, 0x00000000); // DelayRam Clear
	TRACE("Max:0x00000000\n");

	StTneVal.StDwdVal.UsHigVal =
		(UINT_16)((SlMeasureMaxValue >> 16) & 0x0000FFFF);
	StTneVal.StDwdVal.UsLowVal =
		(UINT_16)((SlMeasureMinValue >> 16) & 0x0000FFFF);
	...
	if (UcBfrAft == 0) {
		if (UcDirSel == X_DIR) {
			StAdjPar.StHalAdj.UsHlxCen =
				((INT_16)StTneVal.StDwdVal.UsHigVal +
				 (INT_16)StTneVal.StDwdVal.UsLowVal) /
				2;
...
UINT_32 TneCen(UINT_8 UcTneAxs, ADJ_HALL *ptr)
{
	...
	if (BeforeControl != 0)
		StTneVal.UlDwdVal = TnePtp(UcTneAxs, PTP_BEFORE, ptr);
	else
	...
	while (UlTneRst && (UINT_32)UcTmeOut) {
		...
		if ((((UINT_16)0xFFFF - (StTneVal.StDwdVal.UsHigVal +
					 StTneVal.StDwdVal.UsLowVal)) <
		     UsTargetMax) &&
		    (((UINT_16)0xFFFF - (StTneVal.StDwdVal.UsHigVal +
					 StTneVal.StDwdVal.UsLowVal)) >
		     UsTargetMin)) {
			if (UcTofRst == SUCCESS) {
...
void TneBia(UnDwdVal StTneVal, UINT_8 UcTneAxs, UINT_16 UsHalAdjRange)
{
	...
	if ((StTneVal.StDwdVal.UsHigVal + StTneVal.StDwdVal.UsLowVal) ==
		...
		UlSetBia /= (UINT_32)(0xFFFF - (StTneVal.StDwdVal.UsHigVal +
						StTneVal.StDwdVal.UsLowVal));
		if (UlSetBia > (UINT_32)0x0000FFFF)
			UlSetBia = 0x0000FFFF;
...
void TneOff(UnDwdVal StTneVal, UINT_8 UcTneAxs)
{
	...
	if (StTneVal.StDwdVal.UsHigVal > StTneVal.StDwdVal.UsLowVal) {
		UlSetVal =
			(UINT_32)((StTneVal.StDwdVal.UsHigVal -
				   StTneVal.StDwdVal.UsLowVal) /
		...
		UlSetVal =
			(UINT_32)((StTneVal.StDwdVal.UsLowVal -
				   StTneVal.StDwdVal.UsHigVal) /
...
*/
	...
	if (UcDirSel == X_DIR) { // X axis
		...
		SlMeasureParameterA =
			HALL_RAM_HXOUT3; // Set Measure RAM Address
		SlMeasureParameterB =
			HALL_RAM_HALL_X_OUT; // Set Measure RAM Address
		...
		SlMeasureParameterA =
			HALL_RAM_HYOUT3; // Set Measure RAM Address
		SlMeasureParameterB =
			HALL_RAM_HALL_Y_OUT; // Set Measure RAM Address
	...
	SetTransDataAdr(SinWave_OutAddr,
			(UINT_32)UsSinAdr); // Set Sine Wave Input RAM

	MesFil(LOOPGAIN2); // Filter setting for measurement
	//	MesFil( LOOPGAIN ) ;					// Filter setting for measurement

	MeasureStart(SlNum, SlMeasureParameterA,
		     SlMeasureParameterB); // Start measure
	MeasureWait(); // Wait complete of measurement
	...
	SetTransDataAdr(SinWave_OutAddr,
			(UINT_32)0x00000000); // Set Sine Wave Input RAM
	RamWrite32A(UsSinAdr, 0x00000000); // DelayRam Clear
	...
	TRACE("		MFA %08x%08x, MFB %08x%08x\n",
	      StMeasValueA.StUllnVal.UlHigVal, StMeasValueA.StUllnVal.UlLowVal,
	      StMeasValueB.StUllnVal.UlHigVal, StMeasValueB.StUllnVal.UlHigVal);

	if (UcDirSel == X_DIR) { // X axis
		UllCalculateVal = (StMeasValueB.UllnValue * 1000 /
				   StMeasValueA.UllnValue) *
				  ptr->Hxgain / ptr->XGap;
		if (UllCalculateVal > (UINT_64)0x000000007FFFFFFF)
		  ...
		  INT_32 SlMeasureParameterB)
{
	...
	SetTransDataAdr(
		StMeasFunc_MFA_PiMeasureRam1,
		(UINT_32)SlMeasureParameterA); // Set Measure Filter A Ram Address
	SetTransDataAdr(
		StMeasFunc_MFB_PiMeasureRam2,
		(UINT_32)SlMeasureParameterB); // Set Measure Filter B Ram Address
	...
	RamWrite32A(StMeasFunc_SiSampleMax,
		    SlMeasureParameterNum); // Set Measure Max Number
}
...
// History			: First edition
//********************************************************************************
void MeasureStart2(INT_32 SlMeasureParameterNum, INT_32 SlMeasureParameterA,
		   INT_32 SlMeasureParameterB, UINT_16 UsTime)
{
	MemoryClear(StMeasFunc_SiSampleNum, sizeof(MeasureFunction_Type));
...
void SetWaitTime(UINT_16 UsWaitTime)
{
	...
	RamWrite32A(WaitTimerData_UiTargetCount,
		    (UINT_32)(ONE_MSEC_COUNT * UsWaitTime));
}
...
UINT_32 TneGvc(UINT_8 uc_mode)
{
...
#endif //SEL_SHIFT_COR
		SlMeasureParameterA =
			GYRO_RAM_GX_ADIDAT; // Set Measure RAM Address
		SlMeasureParameterB =
			GYRO_RAM_GY_ADIDAT; // Set Measure RAM Address
#ifdef SEL_SHIFT_COR
	} else {
		SlMeasureParameterA =
			GYRO_ZRAM_GZ_ADIDAT; // Set Measure RAM Address
		SlMeasureParameterB =
			GYRO_ZRAM_GZ_ADIDAT; // Set Measure RAM Address
	}
#endif //SEL_SHIFT_COR
	...
	SlMeasureAveValueA = (INT_32)((INT_64)StMeasValueA.UllnValue /
				      SlMeasureParameterNum);
	SlMeasureAveValueB = (INT_32)((INT_64)StMeasValueB.UllnValue /
				      SlMeasureParameterNum);
	...
	if (uc_mode == 0) {
#endif //SEL_SHIFT_COR
		StAdjPar.StGvcOff.UsGxoVal =
			(UINT_16)(SlMeasureAveValueA &
				  0x0000FFFF); //Measure Result Store
		if (((INT_16)StAdjPar.StGvcOff.UsGxoVal >
		     (INT_16)GYROF_UPPER) ||
		    ((INT_16)StAdjPar.StGvcOff.UsGxoVal <
		     (INT_16)GYROF_LOWER)) {
			UlRsltSts |= EXE_GXADJ;
		}
		RamWrite32A(GYRO_RAM_GXOFFZ,
			    ((SlMeasureAveValueA << 16) &
			     0xFFFF0000)); // X axis Gyro offset

		StAdjPar.StGvcOff.UsGyoVal =
			(UINT_16)(SlMeasureAveValueB &
				  0x0000FFFF); //Measure Result Store
		if (((INT_16)StAdjPar.StGvcOff.UsGyoVal >
		     (INT_16)GYROF_UPPER) ||
		    ((INT_16)StAdjPar.StGvcOff.UsGyoVal <
		     (INT_16)GYROF_LOWER)) {
			UlRsltSts |= EXE_GYADJ;
		}
		RamWrite32A(GYRO_RAM_GYOFFZ,
			    ((SlMeasureAveValueB << 16) &
		...
		RamWrite32A(GyroFilterDelayX_GXH1Z2,
			    0x00000000); // X axis H1Z2 Clear
		RamWrite32A(GyroFilterDelayY_GYH1Z2,
			    0x00000000); // Y axis H1Z2 Clear
#ifdef SEL_SHIFT_COR
	} else {
		StAdjPar.StGvcOff.UsGzoVal =
			(UINT_16)(SlMeasureAveValueA &
				  0x0000FFFF); //Measure Result Store
		if (((INT_16)StAdjPar.StGvcOff.UsGzoVal >
		     (INT_16)GYROF_UPPER) ||
		    ((INT_16)StAdjPar.StGvcOff.UsGzoVal <
		     (INT_16)GYROF_LOWER)) {
			UlRsltSts |= EXE_GZADJ;
		}
		RamWrite32A(GYRO_ZRAM_GZOFFZ,
			    ((SlMeasureAveValueA << 16) &
			     0xFFFF0000)); // Z axis Gyro offset

		RamWrite32A(GyroRAM_Z_GYRO_OFFSET,
			    0x00000000); // Z axis Drift Gyro offset
	}
#endif //SEL_SHIFT_COR
	return (UlRsltSts);
}
...
UINT_32 TneAvc(UINT_8 ucposture)
{
	...
	if (ucposture < 0x7f) {
		...
		for (i = 0; i < 2; i++) {
			...
			case 0:
				SlMeasureParameterA =
					ACCLRAM_X_AC_ADIDAT; // Set Measure RAM Address
				SlMeasureParameterB =
					ACCLRAM_Y_AC_ADIDAT; // Set Measure RAM Address
				break;
			case 1:
				SlMeasureParameterA =
					ACCLRAM_Z_AC_ADIDAT; // Set Measure RAM Address
				SlMeasureParameterB =
					ACCLRAM_Z_AC_ADIDAT; // Set Measure RAM Address
				break;
			}
		...
		TRACE("VAL(X,Y,Z) pos = \t%08xh\t%08xh\t%08xh\t%d \n",
		      (INT_32)SlMeasureRetValueX, (INT_32)SlMeasureRetValueY,
		      (INT_32)SlMeasureRetValueZ, ucposture);
		...
		case 0x80: /* 計算 */
			...
			if (StPosOff.UlAclOfSt == 0x3fL) {
				/*X offset*/
				StAclVal.StAccel.SlOffsetX =
					StPosOff.StPos.Pos[4][0];
				/*Y offset*/
				StAclVal.StAccel.SlOffsetY =
					StPosOff.StPos.Pos[4][1];
				/*Z offset*/
				StAclVal.StAccel.SlOffsetZ =
					StPosOff.StPos.Pos[4][2];
#ifdef DEBUG
				TRACE("ACLOFST(X,Y,Z) = \t%08xh\t%08xh\t%08xh \n",
				      (INT_32)StAclVal.StAccel.SlOffsetX,
				      (INT_32)StAclVal.StAccel.SlOffsetY,
				      (INT_32)StAclVal.StAccel.SlOffsetZ);
#endif //DEBUG
				RamWrite32A(
					ACCLRAM_X_AC_OFFSET,
					StAclVal.StAccel
				...
				StAclVal.StAccel.SlOffsetX =
					(StAclVal.StAccel.SlOffsetX >> 16) &
					0x0000FFFF;
				StAclVal.StAccel.SlOffsetY =
					(StAclVal.StAccel.SlOffsetY >> 16) &
					0x0000FFFF;
				StAclVal.StAccel.SlOffsetZ =
					(StAclVal.StAccel.SlOffsetZ >> 16) &
					0x0000FFFF;
		...
		case 0xFF: /* RAM clear */
			MemClr((UINT_8 *)&StPosOff,
			       sizeof(stPosOff)); // Adjust Parameter Clear
			MemClr((UINT_8 *)&StAclVal,
...
UINT_32 MeasGyAcOffset(void)
{
	...
	if (UlRsltSts == EXE_END) {
		...
		RamWrite32A(ACCLRAM_Z_AC_OFFSET,
			    SlMeasureAveValueB[1] -
				    (INT_32)GSENS); // Z axis Accel offset
		...
		RamWrite32A(GyroFilterDelayY_GYH1Z2,
			    0x00000000); // Y axis H1Z2 Clear
		RamWrite32A(AcclFilDly_X + 8,
			    0x00000000); // X axis Accl LPF Clear
		RamWrite32A(AcclFilDly_Y + 8,
			    0x00000000); // Y axis Accl LPF Clear
		RamWrite32A(AcclFilDly_Z + 8,
			    0x00000000); // Z axis Accl LPF Clear
		RamWrite32A(AcclFilDly_X + 12,
			    0x00000000); // X axis Accl LPF Clear
		RamWrite32A(AcclFilDly_Y + 12,
			    0x00000000); // Y axis Accl LPF Clear
		RamWrite32A(AcclFilDly_Z + 12,
			    0x00000000); // Z axis Accl LPF Clear
		RamWrite32A(AcclFilDly_X + 16,
			    0x00000000); // X axis Accl LPF Clear
		RamWrite32A(AcclFilDly_Y + 16,
			    0x00000000); // Y axis Accl LPF Clear
		RamWrite32A(AcclFilDly_Z + 16,
			    0x00000000); // Z axis Accl LPF Clear
		RamWrite32A(AcclFilDly_X + 20,
			    0x00000000); // X axis Accl LPF Clear
		RamWrite32A(AcclFilDly_Y + 20,
			    0x00000000); // Y axis Accl LPF Clear
		RamWrite32A(AcclFilDly_Z + 20,
			    0x00000000); // Z axis Accl LPF Clear
	}
	return (UlRsltSts);
}
...
void SetSinWavePara(UINT_8 UcTableVal, UINT_8 UcMethodVal)
{
	...
	if (UlFreqDat == 0xFFFFFFFF) /* Sine波中止 */
	{
		...
		RamWrite32A(SinWaveC_Regsiter, 0x00000001); // Sine Wave Start
		SetTransDataAdr(SinWave_OutAddr,
				(UINT_32)HALL_RAM_HXOFF1); // 出力先アドレス
		SetTransDataAdr(CosWave_OutAddr,
				(UINT_32)HALL_RAM_HYOFF1); // 出力先アドレス
	}
}
...
UINT_8 TneHvc(void)
{
	...
	SlMeasureParameterA =
		(UINT_32)HALL_RAM_HXIDAT; // Set Measure RAM Address
	SlMeasureParameterB =
		(UINT_32)HALL_RAM_HYIDAT; // Set Measure RAM Address
	...
	SlMeasureAveValueA = (INT_32)((((INT_64)StMeasValueA.UllnValue * 100) /
				       SlMeasureParameterNum) /
				      100);
	SlMeasureAveValueB = (INT_32)((((INT_64)StMeasValueB.UllnValue * 100) /
				       SlMeasureParameterNum) /
				      100);

	StAdjPar.StHalAdj.UsHlxCna =
		(UINT_16)((SlMeasureAveValueA >> 16) &
			  0x0000FFFF); //Measure Result Store
	StAdjPar.StHalAdj.UsHlxCen =
		StAdjPar.StHalAdj.UsHlxCna; //Measure Result Store

	StAdjPar.StHalAdj.UsHlyCna =
		(UINT_16)((SlMeasureAveValueB >> 16) &
			  0x0000FFFF); //Measure Result Store
	StAdjPar.StHalAdj.UsHlyCen =
		StAdjPar.StHalAdj.UsHlyCna; //Measure Result Store

	RamWrite32A(HALL_RAM_HXOFF,
		    (UINT_32)((StAdjPar.StHalAdj.UsHlxCen << 16) & 0xFFFF0000));
	RamWrite32A(HALL_RAM_HYOFF,
		    (UINT_32)((StAdjPar.StHalAdj.UsHlyCen << 16) & 0xFFFF0000));
...
void TneFin(ADJ_LOPGAN *ptr)
{
	...
	MesFil(THROUGH); // Filter setting for measurement
	MeasureStart(ADOFF_FINE_NUM, HALL_RAM_HALL_X_OUT,
	...
	while (UcFin) {
		...
		RamRead32A(StMeasFunc_MFA_LLiIntegral1 + 4,
			   &StMeasValueA.StUllnVal.UlHigVal);
		SlMeasureAveValueA =
			(INT_32)((((INT_64)StMeasValueA.UllnValue * 100) /
				  ADOFF_FINE_NUM) /
				 100);
		...
		RamRead32A(StMeasFunc_MFB_LLiIntegral2 + 4,
			   &StMeasValueB.StUllnVal.UlHigVal);
		SlMeasureAveValueB =
			(INT_32)((((INT_64)StMeasValueB.UllnValue * 100) /
				  ADOFF_FINE_NUM) /
				 100);
...
void SetSinWavGenInt(void)
{
	...
	RamWrite32A(SinWave_Gain,
		    0x00000000); // 発生周波数のアッテネータ(初期値は0[dB])
	...
	RamWrite32A(CosWave_Phase, 0x00000000); // 正弦波の位相量
	RamWrite32A(CosWave_Gain,
		    0x00000000); // 発生周波数のアッテネータ(初期値はCut)
...
void SetTransDataAdr(UINT_16 UsLowAddress, UINT_32 UlLowAdrBeforeTrans)
{
	...
	if (UlLowAdrBeforeTrans < 0x00009000) {
		StTrsVal.StDwdVal.UsHigVal =
			(UINT_16)((UlLowAdrBeforeTrans & 0x0000F000) >> 8);
		StTrsVal.StDwdVal.UsLowVal =
			(UINT_16)(UlLowAdrBeforeTrans & 0x00000FFF);
	} else {
...
UINT_8 GyrIdRead(UINT_8 *UcGyroID)
{
	...
	TRACE("UcGyroID %02X %02X %02X %02X %02X %02X %02X \n", UcGyroID[0],
	      UcGyroID[1], UcGyroID[2], UcGyroID[3], UcGyroID[4], UcGyroID[5],
	      UcGyroID[6]);
...
UINT_8 FrqDet(void)
{
	...
	// Start measure
	MeasureStart2(SlMeasureParameterNum, SlMeasureParameterA,
		      SlMeasureParameterB, 50);
	MeasureWait(); // Wait complete of measurement
...
//********************************************************************************
UINT_8 MesRam(INT_32 SlMeasureParameterA, INT_32 SlMeasureParameterB,
	      INT_32 SlMeasureParameterNum, stMesRam *pStMesRamA,
	      stMesRam *pStMesRamB)
{
	UnllnVal StMeasValueA, StMeasValueB;
	...
	MeasureStart2(SlMeasureParameterNum, SlMeasureParameterA,
		      SlMeasureParameterB, 1); // Start measure

	MeasureWait(); // Wait complete of measurement

	// A : X axis
	RamRead32A(StMeasFunc_MFA_SiMax1,
		   &(pStMesRamA->SlMeasureMaxValue)); // Max value
	RamRead32A(StMeasFunc_MFA_SiMin1,
		   &(pStMesRamA->SlMeasureMinValue)); // Min value
	RamRead32A(StMeasFunc_MFA_UiAmp1,
		   &(pStMesRamA->SlMeasureAmpValue)); // Amp value
	RamRead32A(StMeasFunc_MFA_LLiIntegral1,
		   &(StMeasValueA.StUllnVal.UlLowVal)); // Integration Low
	RamRead32A(StMeasFunc_MFA_LLiIntegral1 + 4,
		   &(StMeasValueA.StUllnVal.UlHigVal)); // Integration Hig
	pStMesRamA->SlMeasureAveValue =
		(INT_32)((INT_64)StMeasValueA.UllnValue /
			 SlMeasureParameterNum); // Ave value

	// B : Y axis
	RamRead32A(StMeasFunc_MFB_SiMax2,
		   &(pStMesRamB->SlMeasureMaxValue)); // Max value
	RamRead32A(StMeasFunc_MFB_SiMin2,
		   &(pStMesRamB->SlMeasureMinValue)); // Min value
	RamRead32A(StMeasFunc_MFB_UiAmp2,
		   &(pStMesRamB->SlMeasureAmpValue)); // Amp value
	RamRead32A(StMeasFunc_MFB_LLiIntegral2,
		   &(StMeasValueB.StUllnVal.UlLowVal)); // Integration Low
	RamRead32A(StMeasFunc_MFB_LLiIntegral2 + 4,
		   &(StMeasValueB.StUllnVal.UlHigVal)); // Integration Hig
...
UINT_32 MeasGain(UINT_16 UcDirSel, UINT_16 UsMeasFreq, UINT_32 UlMesAmp)
{
	...
	if (UcDirSel == X_DIR) { // X axis
		SlMeasureParameterA =
			HALL_RAM_HXOUT0; // Set Measure RAM Address
		SlMeasureParameterB =
			HallFilterD_HXDAZ1; // Set Measure RAM Address
	} else if (UcDirSel == Y_DIR) { // Y axis
		SlMeasureParameterA =
			HALL_RAM_HYOUT0; // Set Measure RAM Address
		SlMeasureParameterB =
			HallFilterD_HYDAZ1; // Set Measure RAM Address
	}
	SetSinWavGenInt();

	SlSineWaveOffset = (INT_32)(FRQOFST * (UINT_32)UsMeasFreq);
	RamWrite32A(SinWave_Offset,
		    SlSineWaveOffset); // Freq Setting = Freq * 80000000h / Fs
	...
	if (UcDirSel == X_DIR) {
		SetTransDataAdr(
			SinWave_OutAddr,
			(UINT_32)HALL_RAM_HXOFF1); // Set Sine Wave Input RAM
	} else if (UcDirSel == Y_DIR) {
		SetTransDataAdr(
			SinWave_OutAddr,
			(UINT_32)HALL_RAM_HYOFF1); // Set Sine Wave Input RAM
	}

	MesFil2(UsMeasFreq); // Filter setting for measurement

	MeasureStart2(SlMeasureParameterNum, SlMeasureParameterA,
		      SlMeasureParameterB, 8000 / UsMeasFreq); // Start measure
	...
	UlReturnVal = (INT_32)((INT_64)StMeasValueA.UllnValue * 100 /
			       (INT_64)StMeasValueB.UllnValue);
...
void MesFil2(UINT_16 UsMesFreq)
{
	...
	UlTempval = (UINT_32)(2147483647 * (float)UsMesFreq /
			      ((float)UsMesFreq + DivOffset));
...
UINT_8 SetAngleCorrection(float DegreeGap, UINT_8 SelectAct, UINT_8 Arrangement)
{
	...
	if ((DegreeGap > 180.0f) || (DegreeGap < -180.0f))
		return (1);
	if (Arrangement >= 2)
		return (1);
	...
	default:
		OffsetAngle =
			(double)(DegreeGap) * 3.141592653589793238 / 180.0f;
		UcCnvF = PACT1Tbl[Arrangement];
		break;
		//		default :
		//			break;
	...
	if (SelectAct == 0x00) {
		OffsetAngleV_slt =
			(double)(45.0f) * 3.141592653589793238 / 180.0f;
	} else {
		OffsetAngleV_slt =
			(double)(0.0f) * 3.141592653589793238 / 180.0f;
	}
...
void SetGyroCoef(UINT_8 UcCnvF)
{
	...
	case 0x00:
		...
		Slgyy = 0x7FFFFFFF;
		Slgyx = 0x00000000;
		break; //HX<==GX(NEG), HY<==GY(NEG)
	case 0x20:
		Slgxx = 0x7FFFFFFF;
		Slgxy = 0x00000000;
		Slgyy = 0x80000001;
		Slgyx = 0x00000000;
		break; //HX<==GX(NEG), HY<==GY(POS)
	case 0x40:
		Slgxx = 0x80000001;
		Slgxy = 0x00000000;
		Slgyy = 0x7FFFFFFF;
		Slgyx = 0x00000000;
		break; //HX<==GX(POS), HY<==GY(NEG)
	case 0x60:
		Slgxx = 0x80000001;
		Slgxy = 0x00000000;
		Slgyy = 0x80000001;
		Slgyx = 0x00000000;
		break; //HX<==GX(POS), HY<==GY(POS)
		/* HX <== GY , HY <== GX */
	case 0x80:
		Slgxx = 0x00000000;
		Slgxy = 0x7FFFFFFF;
		Slgyy = 0x00000000;
		Slgyx = 0x7FFFFFFF;
		break; //HX<==GY(NEG), HY<==GX(NEG)
	case 0xA0:
		Slgxx = 0x00000000;
		Slgxy = 0x7FFFFFFF;
		Slgyy = 0x00000000;
		Slgyx = 0x80000001;
		break; //HX<==GY(NEG), HY<==GX(POS)
	case 0xC0:
		Slgxx = 0x00000000;
		Slgxy = 0x80000001;
		Slgyy = 0x00000000;
		Slgyx = 0x7FFFFFFF;
		break; //HX<==GY(POS), HY<==GX(NEG)
	case 0xE0:
		Slgxx = 0x00000000;
		Slgxy = 0x80000001;
		Slgyy = 0x00000000;
...
void SetAccelCoef(UINT_8 UcCnvF)
{
	...
	case 0x00:
		...
		Slayy = 0x7FFFFFFF;
		Slayx = 0x00000000;
		break; //HX<==AX(NEG), HY<==AY(NEG)
	case 0x02:
		Slaxx = 0x7FFFFFFF;
		Slaxy = 0x00000000;
		Slayy = 0x80000001;
		Slayx = 0x00000000;
		break; //HX<==AX(NEG), HY<==AY(POS)
	case 0x04:
		Slaxx = 0x80000001;
		Slaxy = 0x00000000;
		Slayy = 0x7FFFFFFF;
		Slayx = 0x00000000;
		break; //HX<==AX(POS), HY<==AY(NEG)
	case 0x06:
		Slaxx = 0x80000001;
		Slaxy = 0x00000000;
		Slayy = 0x80000001;
		Slayx = 0x00000000;
		break; //HX<==AX(POS), HY<==AY(POS)
		/* HX <== AY , HY <== AX */
	case 0x08:
		Slaxx = 0x00000000;
		Slaxy = 0x7FFFFFFF;
		Slayy = 0x00000000;
		Slayx = 0x7FFFFFFF;
		break; //HX<==AY(NEG), HY<==AX(NEG)
	case 0x0A:
		Slaxx = 0x00000000;
		Slaxy = 0x7FFFFFFF;
		Slayy = 0x00000000;
		Slayx = 0x80000001;
		break; //HX<==AY(NEG), HY<==AX(POS)
	case 0x0C:
		Slaxx = 0x00000000;
		Slaxy = 0x80000001;
		Slayy = 0x00000000;
		Slayx = 0x7FFFFFFF;
		break; //HX<==AY(POS), HY<==AX(NEG)
	case 0x0E:
		Slaxx = 0x00000000;
		Slaxy = 0x80000001;
		Slayy = 0x00000000;
...
// History			: First edition
//********************************************************************************
void SetGyroOffset(UINT_16 GyroOffsetX, UINT_16 GyroOffsetY,
		   UINT_16 GyroOffsetZ)
{
	RamWrite32A(GYRO_RAM_GXOFFZ,
		    ((GyroOffsetX << 16) & 0xFFFF0000)); // X axis Gyro offset
	RamWrite32A(GYRO_RAM_GYOFFZ,
		    ((GyroOffsetY << 16) & 0xFFFF0000)); // Y axis Gyro offset
	RamWrite32A(GYRO_ZRAM_GZOFFZ,
		    ((GyroOffsetZ << 16) & 0xFFFF0000)); // Z axis Gyro offset
}
...
// History			: First edition
//********************************************************************************
void SetAcclOffset(UINT_16 AcclOffsetX, UINT_16 AcclOffsetY,
		   UINT_16 AcclOffsetZ)
{
	RamWrite32A(ACCLRAM_X_AC_OFFSET,
		    ((AcclOffsetX << 16) & 0xFFFF0000)); // X axis Accl offset
	RamWrite32A(ACCLRAM_Y_AC_OFFSET,
		    ((AcclOffsetY << 16) & 0xFFFF0000)); // Y axis Accl offset
	RamWrite32A(ACCLRAM_Z_AC_OFFSET,
		    ((AcclOffsetZ << 16) & 0xFFFF0000)); // Z axis Accl offset
}
...
UINT_8 TstActMov(UINT_8 UcDirSel)
{
	...
	TRACE(" DIR = %d, lmt = %08x, zom = %08x , lnz = %08x ,sft = %08x \n",
	      UcDirSel, (unsigned int)UlLimit, (unsigned int)Ulzoom,
	      (unsigned int)Ullenz, (unsigned int)Ulshift);
	...
	UlActChkLvl = (UINT_32)((float)0x7FFFFFFF * SfLimit * Sfzoom * Sflenz *
				Sfshift * ACT_MARGIN);
	...
	for (UcCnt = 0; UcCnt < 10; UcCnt++) {
		...
		if (UcCnt > 0) {
			if ((INT_16)UsGxoVal[0] > (INT_16)UsGxoVal[UcCnt]) {
				UsDif = (UINT_16)((INT_16)UsGxoVal[0] -
						  (INT_16)UsGxoVal[UcCnt]);
			} else {
				UsDif = (UINT_16)((INT_16)UsGxoVal[UcCnt] -
						  (INT_16)UsGxoVal[0]);
			}
			...
			if ((INT_16)UsGyoVal[0] > (INT_16)UsGyoVal[UcCnt]) {
				UsDif = (UINT_16)((INT_16)UsGyoVal[0] -
						  (INT_16)UsGyoVal[UcCnt]);
			} else {
				UsDif = (UINT_16)((INT_16)UsGyoVal[UcCnt] -
						  (INT_16)UsGyoVal[0]);
			}
