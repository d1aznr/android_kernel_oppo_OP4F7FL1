...
UINT_8 PmemUpdate128(DOWNLOAD_TBL_EXT *ptr)
{
	...
	data[0] = 0xF0; //CmdID
	data[1] = 0x0E; //CmdID
	data[2] = (unsigned char)((ptr->SizeUpdataCode >> 8) &
				  0x000000FF); //書き込みデータ(MSB)
	data[3] = (unsigned char)(ptr->SizeUpdataCode &
				  0x000000FF); //書き込みデータ
	data[4] = 0x00; //書き込みデータ
	...
	for (i = 0; i < 8; i++) {
		if (ReadData[7 - i] != *p++) { // CheckSum Codeの判定
			TRACE("[2] PMEM verify Error %02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x\n",
			      ...
			      ReadData[3], ReadData[4], ReadData[5],
...
UINT_8 EraseUserMat128(UINT_8 StartBlock, UINT_8 EndBlock)
{
	...
	for (i = StartBlock; i < EndBlock; i++) {
		...
		RamWrite32A(0xF00C,
			    0x00000020); // FromCmd.Controlの設定(ブロック消去)
		...
		do {
			...
			RamRead32A(0xF00C,
				   &UlReadVal); // FromCmd.Controlの読み出し
		} while (UlReadVal != 0);
	}
	IOWrite32A(0xE0701C, 0x00000002);
	return (0);
...
UINT_8 ProgramFlash128_Standard(DOWNLOAD_TBL_EXT *ptr)
{
	...
	for (i = 1; i < (ptr->SizeFromCode / 64); i++) {
		...
		if (UcOddEvn == 0) {
			...
			RamWrite32A(
				0xF00C,
				0x00000004); // FromCmd.Controlの設定(書き込み)
		} else {
			do { // 書き込みの終了判定
				RamRead32A(
					0xF00C,
			...
			RamWrite32A(
				0xF00C,
				0x00000008); // FromCmd.Controlの設定(書き込み)
		}
	}
...
const DOWNLOAD_TBL_EXT Adj_Dtbl[] = {
	{ 0xEEEE, 0, 0, LC898128_PM, LC898128_PmemCodeSize,
	  LC898128_PmemCodeCheckSum, (void *)0, 0, 0, 0 }
};
...
UINT_32 DrvOffAdj(void)
{
	...
	if (ans == 1) {
		...
		ans = PmemUpdate128(
			(DOWNLOAD_TBL_EXT *)
				Adj_Dtbl); // Update the special program for updating the flash memory.
		if (ans != 0)
			return (ans);
		...
		IOWrite32A(0xE07CCC,
			   0x0000ACD5); // UNLK_CODE3(E0_7CCCh) = 0000_ACD5h
		WitTim(10); //neo: need to reduce this time

		IOWrite32A(FLASHROM_FLAMODE, 0x00000002);
		IOWrite32A(
			SYSDSP_REMAP,
			0x00001000); // CORE_RST[12], MC_IGNORE2[10] = 1 PRAMSEL[7:6]=01b
...
UINT_8 FlashUpdate128(DOWNLOAD_TBL_EXT *ptr)
{
	...
	ans = PmemUpdate128(
		ptr); // Update the special program for updating the flash memory.
	if (ans != 0)
		return (ans);
	...
	//--------------------------------------------------------------------------------
	if (UnlockCodeSet() != 0)
		return (0x33); // Unlock Code set ng
	...
	if (ans != 0) {
		if (UnlockCodeClear() != 0)
			return (0x32); // unlock code clear ng
		else
			return (ans);
	}
...
UINT_8 FlashBlockErase(UINT_8 SelMat, UINT_32 SetAddress)
{
	...
	if (SelMat != USER_MAT) {
		if (SelMat == INF_MAT2)
			IOWrite32A(0xE07CCC, 0x00006A4B);
		else
			IOWrite32A(0xE07CCC,
				   0x0000C5AD); // additional unlock for INFO
	}
...
UINT_8 FlashBlockWrite(UINT_8 SelMat, UINT_32 SetAddress, UINT_32 *PulData)
{
	...
	IOWrite32A(FLASHROM_FLA_ADR,
		   ((UINT_32)SelMat << 16) |
			   (SetAddress & 0x000010)); // addressはpageのみ指定
	// page write Start
	IOWrite32A(FLASHROM_FLAMODE, 0x00000000);
...
UINT_8 WrHallCalData(UINT_8 UcMode)
{
	...
	if (UcMode) { // write
		...
		UlMAT0[CALIBRATION_STATUS] |=
			(StAdjPar.StHalAdj.UlAdjPhs |
			 GYRO_GAIN_FLG); // Calibration Status
		UlMAT0[HALL_BIAS_OFFSET] =
			(UINT_32)(((UINT_32)(StAdjPar.StHalAdj.UsHlyOff & 0xFF00)
				   << 16) |
				  ((UINT_32)(StAdjPar.StHalAdj.UsHlyGan & 0xFF00)
				   << 8) |
				  ((UINT_32)(StAdjPar.StHalAdj.UsHlxOff &
					     0xFF00) >>
				   0) |
				  ((UINT_32)(StAdjPar.StHalAdj.UsHlxGan &
					     0xFF00) >>
				   8));
		UlMAT0[LOOP_GAIN_XY] =
			((StAdjPar.StLopGan.UlLygVal & 0xFFFF0000) |
			 (StAdjPar.StLopGan.UlLxgVal >> 16));
		UlMAT0[LENS_OFFSET] =
			(UINT_32)(((UINT_32)(StAdjPar.StHalAdj.UsAdyOff)
				   << 16) |
				  (UINT_32)(StAdjPar.StHalAdj.UsAdxOff));
		UlMAT0[GYRO_GAIN_X] = 0x3FFFFFFF;
		UlMAT0[GYRO_GAIN_Y] = 0x3FFFFFFF;
		UlMAT0[HL_XMAXMIN] =
			(UINT_32)(((UINT_32)StAdjPar.StHalAdj.UsHlxMxa << 16) |
				  (UINT_32)StAdjPar.StHalAdj.UsHlxMna);
		UlMAT0[HL_YMAXMIN] =
			(UINT_32)(((UINT_32)StAdjPar.StHalAdj.UsHlyMxa << 16) |
				  (UINT_32)StAdjPar.StHalAdj.UsHlyMna);
		...
		UlMAT0[CALIBRATION_STATUS] |=
			(HALL_CALB_FLG | 0x000000FF | GYRO_GAIN_FLG);
	...
	UsCkVal += (UINT_8)(UlMAT0[i] >> 8);
	UlMAT0[MAT0_CKSM] =
		((UINT_32)UsCkVal << 16) | (UlMAT0[MAT0_CKSM] & 0x0000FFFF);
...
UINT_8 WrLinCalData(UINT_8 UcMode, mlLinearityValue *linval)
{
	...
	if (UcMode) { // write
		...
		UlMAT0[LN_POS1] = (UINT_32)((UINT_32)(*pPosY++ * 10) << 16) |
				  (UINT_32)(*pPosX++ * 10);
		UlMAT0[LN_POS2] = (UINT_32)((UINT_32)(*pPosY++ * 10) << 16) |
				  (UINT_32)(*pPosX++ * 10);
		UlMAT0[LN_POS3] = (UINT_32)((UINT_32)(*pPosY++ * 10) << 16) |
				  (UINT_32)(*pPosX++ * 10);
		UlMAT0[LN_POS4] = (UINT_32)((UINT_32)(*pPosY++ * 10) << 16) |
				  (UINT_32)(*pPosX++ * 10);
		UlMAT0[LN_POS5] = (UINT_32)((UINT_32)(*pPosY++ * 10) << 16) |
				  (UINT_32)(*pPosX++ * 10);
		UlMAT0[LN_POS6] = (UINT_32)((UINT_32)(*pPosY++ * 10) << 16) |
				  (UINT_32)(*pPosX++ * 10);
		UlMAT0[LN_POS7] = (UINT_32)((UINT_32)(*pPosY++ * 10) << 16) |
				  (UINT_32)(*pPosX++ * 10);
		if (LnCsPtr->XY_SWAP == 1) {
		...
		UlMAT0[LN_STEP] =
			(UINT_32)(((UINT_32)PosDifY << 16) | (UINT_32)PosDifX);
	} else {
		UlMAT0[CALIBRATION_STATUS] |= HLLN_CALB_FLG;
...
UINT_8 WrMixCalData(UINT_8 UcMode, mlMixingValue *mixval)
{
	...
	if (UcMode) { // write
		...
		if ((LnCsPtr->STEPX * LnCsPtr->DRIVEX > 0 &&
		     LnCsPtr->STEPY * LnCsPtr->DRIVEY < 0) ||
		    (LnCsPtr->STEPX * LnCsPtr->DRIVEX < 0 &&
		     LnCsPtr->STEPY * LnCsPtr->DRIVEY > 0)) {
			mixval->hx45yL = (-1) * mixval->hx45yL;
			mixval->hy45xL = (-1) * mixval->hy45xL;
		}
		...
		UlMAT0[MIXING_X] =
			(UINT_32)((mixval->hx45yL & 0xFFFF0000) |
				  ((mixval->hx45xL >> 16) & 0x0000FFFF));
		UlMAT0[MIXING_Y] =
			(UINT_32)((mixval->hy45xL & 0xFFFF0000) |
				  ((mixval->hy45yL >> 16) & 0x0000FFFF));
		UlMAT0[MIXING_SFT] =
			(UINT_32)((((UINT_32)mixval->hysx << 8) & 0x0000FF00) |
				  ((UINT_32)mixval->hxsx & 0x000000FF));
	} else {
		UlMAT0[CALIBRATION_STATUS] |= MIXI_CALB_FLG;
...
// History			: First edition
//********************************************************************************
UINT_8 WrLinMixCalData(UINT_8 UcMode, mlMixingValue *mixval,
		       mlLinearityValue *linval)
{
	...
	if (UcMode) { // write
		...
		//*****************************************************//
		/* X と Y でStep 極性が違う時 */
		if (((INT_32)linval->dacY[4] > 0 &&
		     (INT_32)linval->dacX[4] < 0) ||
		    ((INT_32)linval->dacY[4] < 0 &&
		     (INT_32)linval->dacX[4] > 0)) {
			mixval->hx45yL = (-1) * mixval->hx45yL;
			mixval->hy45xL = (-1) * mixval->hy45xL;
		}
...
UINT_8 WrOptCenerData(UINT_8 UcMode)
{
	...
	if (UcMode) { // write
		...
		UlMAT0[OPTCENTER] =
			(UINT_32)((UlReadLensyoffset & 0xFFFF0000) |
				  ((UlReadLensxoffset >> 16) & 0x0000FFFF));
	} else {
...
UINT_8 LoadUareToPM(DOWNLOAD_TBL_EXT *ptr, UINT_8 mode)
{
	...
	if (!mode) {
		...
		if (UlReadVal != 0x0B) {
			IOWrite32A(
				SYSDSP_REMAP,
				0x00001400); // CORE_RST[12], MC_IGNORE2[10] = 1
			WitTim(15); // Bootプログラムを回すのに15msec必要。
			IORead32A(ROMINFO, (UINT_32 *)&UlReadVal);
			if (UlReadVal != 0x0B) {
				return (0x02);
	...
	RamWrite32A(0x5004,
		    0x00000000); // Trans user data from flash memory to pm
...
// History			: First edition
//********************************************************************************
UINT_8 RdBurstUareaFromPm(UINT_32 UlAddress, UINT_8 *PucData, UINT_8 UcLength,
			  UINT_8 mode)
{
	if (!UcLength)
		return (0xff);
...
UINT_8 RdSingleUareaFromPm(UINT_32 UlAddress, UINT_8 *PucData, UINT_8 UcLength,
			   UINT_8 mode)
{
	UINT_8 i;
	UINT_32 ReadData;
...
// History			: First edition
//********************************************************************************
UINT_8 WrUareaToPm(UINT_32 UlAddress, UINT_8 *PucData, UINT_8 UcLength,
		   UINT_8 mode)
{
	UINT_8 i;
...
UINT_8 WrUareaToFlash(void)
{
	...
	do {
		...
		if (UlCntE++ > 20) {
			IOWrite32A(0xE0701C, 0x00000002);
			if (UnlockCodeClear() != 0)
				return (0x11); // unlock code clear ng
	...
	RamWrite32A(0x5006,
		    0x00000000); // write user area data from PM to flash memory
	...
	do {
		...
		if (UlCntW++ > 300) {
			IOWrite32A(0xE0701C, 0x00000002);
			if (UnlockCodeClear() != 0)
				return (0x21); // unlock code clear ng
	...
	IOWrite32A(0xE0701C, 0x00000002);
	if (UnlockCodeClear() != 0)
		return (0x31); // unlock code clear ng
