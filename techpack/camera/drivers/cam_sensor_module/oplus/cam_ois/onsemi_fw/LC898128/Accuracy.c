...
void LoopGainSet(unsigned char flag, float db)
{
	...
	if (flag) // 1:Gain Up
	{
		...
		RamWrite32A(HallX_hs,
			    (xHs & 0xFF000000) + 0x01000000 +
				    (xHs & 0x00FFFFFF)); // Shift 2 bits
		RamWrite32A(HallY_hs,
			    (yHs & 0x0000FF00) + 0x00000100 +
				    (yHs & 0xFFFF00FF)); // Shift 2 bits

		//Get LoopGain1
		RamRead32A(HallFilterCoeffX_hxgain1, &xLpGan);
		RamRead32A(HallFilterCoeffY_hygain1, &yLpGan);

		RamWrite32A(HallFilterCoeffX_hxgain1,
			    float2fix(fix2float(xLpGan) * db / 2)); // gain1 /2
		RamWrite32A(HallFilterCoeffY_hygain1,
			    float2fix(fix2float(xLpGan) * db / 2)); // gain1 /2
...
unsigned short Accuracy()
{
	...
	xRadius = fabsf(fix2float(xGyroLimit)) * fabsf(fix2float(xGyrogain)) *
		  fabsf(fix2float(xGLenz)) * xShiftRG;
	yRadius = fabsf(fix2float(yGyroLimit)) * fabsf(fix2float(yGyrogain)) *
		  fabsf(fix2float(yGLenz)) * yShiftRG;
	...
	for (deg = 0; deg <= 360; deg += DEGSTEP) // 0-360 degree
	{
		...
		for (short i = 0; i < LOOPTIME; i++) {
			...
			if (fabsf(fix2float(xhall_value) + fix2float(xHav2)) >
			    fabsf(xMaxHall))
				xMaxHall = fix2float(xhall_value) +
					   fix2float(xHav2);
			if (fabsf(fix2float(yhall_value) + fix2float(yHav2)) >
			    fabsf(yMaxHall))
				yMaxHall = fix2float(yhall_value) +
					   fix2float(yHav2);
		}

		if (fabsf(xMaxHall) > xMaxAcc)
			xMaxAcc = fabsf(xMaxHall);
		if (fabsf(yMaxHall) > yMaxAcc)
			yMaxAcc = fabsf(yMaxHall);
		...
		if (fabsf(xMaxHall) > xLimit)
			xng++; // Have NG point;
		if (fabsf(yMaxHall) > yLimit)
			yng++; // Have NG point;
	}
...
UINT_16 HallCheck(void)
{
	...
	if (ret) {
		TRACE("\n VCM has NG points: X = %d, Y = %d", ret >> 8,
		      ret & 0xff);
	} else {
		TRACE("\n VCM is good!");
	}
	...
	for (i = 0; i <= (360 / DEGSTEP); i++) {
		TRACE("\n xPos = %f, xHall = %f, yPos = %f, yHall = %f",
		      xy_raw_data[i].xpos, xy_raw_data[i].xhall,
		      xy_raw_data[i].ypos, xy_raw_data[i].yhall);
	}
	return (ret);
}

unsigned short AccuracyH(float flACCURACY, unsigned short RADIUS,
			 unsigned short usDEGSTEP, unsigned short WAIT_MSEC1,
			 unsigned short WAIT_MSEC2, unsigned short WAIT_MSEC3)
{
	float xpos, ypos;
	unsigned int xhall_value, yhall_value;
	float xMaxHall, yMaxHall;
	...
	//	xRadius = ANGLE_LIMIT * fabsf(fix2float(xGyrogain)) * fabsf(fix2float(xGLenz)) * 8 * fabsf(fix2float(xGoutG));
	//	yRadius = ANGLE_LIMIT * fabsf(fix2float(yGyrogain)) * fabsf(fix2float(yGLenz)) * 8 * fabsf(fix2float(yGoutG));
	xRadius = ANGLE_LIMIT * fabsf(fix2float(xGyrogain)) *
		  fabsf(fix2float(xGLenz)) *
		  (1 << (unsigned char)(xG2x4xb >> 8)) *
		  fabsf(fix2float(xGoutG));
	yRadius = ANGLE_LIMIT * fabsf(fix2float(yGyrogain)) *
		  fabsf(fix2float(yGLenz)) *
		  (1 << (unsigned char)(yG2x4xb >> 8)) *
		  fabsf(fix2float(yGoutG));
	...
	for (deg = 0; deg <= 360; deg += usDEGSTEP) // 0-360 degree
	{
		...
		for (short i = 0; i < LOOPTIME; i++) {
			...
			RamRead32A(HALL_RAM_HXOUT0, &xhall_value);
			RamRead32A(HALL_RAM_HYOUT0, &yhall_value);
			if (fabsf(fix2float(xhall_value) - xpos) >
			    fabsf(xMaxHall))
				xMaxHall = fix2float(xhall_value) - xpos;
			if (fabsf(fix2float(yhall_value) - ypos) >
			    fabsf(yMaxHall))
				yMaxHall = fix2float(yhall_value) - ypos;
		}
...
UINT_16 HallCheckH(float flACCURACY, UINT_16 RADIUS, UINT_16 usDEGSTEP,
		   UINT_16 WAIT_MSEC1, UINT_16 WAIT_MSEC2, UINT_16 WAIT_MSEC3)
{
	INT_16 i;
	//	unsigned short ret = Accuracy();
	UINT_16 ret = AccuracyH(flACCURACY, RADIUS, usDEGSTEP, WAIT_MSEC1,
				WAIT_MSEC2, WAIT_MSEC3);
	...
	// Max Accuracy
	//TRACE("\n X Max Accuracy = %f, Y Max Accuracy = %f", xMaxAcc, yMaxAcc);
	TRACE("\n X Max Accuracy = %d, Y Max Accuracy = %d",
	      (int)(xMaxAcc * 1000), (int)(yMaxAcc * 1000));

	// Limit vale
	//TRACE("\n xLimit = %f, yLimit = %f", xLimit, yLimit);
	TRACE("\n xLimit = %d, yLimit = %d", (int)(xLimit * 1000),
	      (int)(yLimit * 1000));
	...
	for (i = 0; i <= (360 / usDEGSTEP); i++) {
		...
		TRACE("\n xPos = %d, xHall = %d, yPos = %d, yHall = %d",
		      (int)(xy_raw_data[i].xpos * 1000),
		      (int)(xy_raw_data[i].xhall * 1000),
		      (int)(xy_raw_data[i].ypos * 1000),
		      (int)(xy_raw_data[i].yhall * 1000));
	}

	return (ret);
}

unsigned short AccuracyG(float flACCURACY, unsigned short RADIUS,
			 unsigned short usDEGSTEP, unsigned short WAIT_MSEC1,
			 unsigned short WAIT_MSEC2, unsigned short WAIT_MSEC3)
{
	float xpos, ypos;
	...
	for (deg = 0; deg <= 360; deg += usDEGSTEP) // 0-360 degree
	{
		...
		xy_raw_data[deg / usDEGSTEP].xhall =
			xMaxHall + fix2float(xHav2);
		xy_raw_data[deg / usDEGSTEP].ypos = fix2float(yHav2);
		xy_raw_data[deg / usDEGSTEP].yhall =
			yMaxHall + fix2float(yHav2);
...
//unsigned short HallCheck(void)
UINT_16 HallCheckG(float flACCURACY, UINT_16 RADIUS, UINT_16 usDEGSTEP,
		   UINT_16 WAIT_MSEC1, UINT_16 WAIT_MSEC2, UINT_16 WAIT_MSEC3)
{
	INT_16 i;
	UINT_16 ret;
	...
	//	unsigned short ret = Accuracy();
	ret = AccuracyG(flACCURACY, RADIUS, usDEGSTEP, WAIT_MSEC1, WAIT_MSEC2,
			WAIT_MSEC3);
...
UINT_16 AccuracyL(float flACCURACY, UINT_16 RADIUS, UINT_16 usDEGSTEP,
		  UINT_16 WAIT_MSEC1, UINT_16 WAIT_MSEC2, UINT_16 WAIT_MSEC3)
{
	float xpos, ypos;
	UINT_32 xhall_value, yhall_value;
	...
	xRadius = (fabsf(fix2float(sixstp)) * 10.0f) /
		  ((float)uixpxl * pixelsize);
	yRadius = (fabsf(fix2float(siystp)) * 10.0f) /
		  ((float)uiypxl * pixelsize);
...
//unsigned short HallCheck(void)
UINT_16 HallCheckL(float flACCURACY, UINT_16 RADIUS, UINT_16 usDEGSTEP,
		   UINT_16 WAIT_MSEC1, UINT_16 WAIT_MSEC2, UINT_16 WAIT_MSEC3)
{
	INT_16 i;
	UINT_16 ret = AccuracyL(flACCURACY, RADIUS, usDEGSTEP, WAIT_MSEC1,
				WAIT_MSEC2, WAIT_MSEC3);
...
unsigned short AccuracyS(float flACCURACY, unsigned short RADIUS,
			 unsigned short usDEGSTEP, unsigned short WAIT_MSEC1,
			 unsigned short WAIT_MSEC2, unsigned short WAIT_MSEC3,
			 unsigned char ACT_AXIS)
{
	float xpos, ypos;
	unsigned int xhall_value, yhall_value;
	float xMaxHall, yMaxHall;
	...
	for (deg = 0; deg <= 360; deg += usDEGSTEP) // 0-360 degree
	{
		...
		for (short i = 0; i < LOOPTIME; i++) {
			...
			if (ACT_AXIS == X_DIR) {
				RamRead32A(HALL_RAM_HXOUT2, &xhall_value);
				if (fabsf(fix2float(xhall_value) +
					  fix2float(xHav2)) > fabsf(xMaxHall))
					xMaxHall = fix2float(xhall_value) +
						   fix2float(xHav2);
			} else {
				RamRead32A(HALL_RAM_HYOUT2, &yhall_value);
				if (fabsf(fix2float(yhall_value) +
					  fix2float(yHav2)) > fabsf(yMaxHall))
					yMaxHall = fix2float(yhall_value) +
						   fix2float(yHav2);
			}
		}
...
//unsigned short HallCheck(void)
UINT_16 HallCheckS(float flACCURACY, UINT_16 RADIUS, UINT_16 usDEGSTEP,
		   UINT_16 WAIT_MSEC1, UINT_16 WAIT_MSEC2, UINT_16 WAIT_MSEC3,
		   UINT_8 ACT_AXIS)
{
	INT_16 i;
	//	unsigned short ret = Accuracy();
	UINT_16 ret = AccuracyS(flACCURACY, RADIUS, usDEGSTEP, WAIT_MSEC1,
				WAIT_MSEC2, WAIT_MSEC3, ACT_AXIS);
