...
extern unsigned char SelectDownload(uint8_t GyroSelect, uint8_t ActSelect,
				    uint8_t MasterSlave, uint8_t FWType);
extern uint8_t FlashDownload128(uint8_t ModuleVendor, uint8_t ActVer,
				uint8_t MasterSlave, uint8_t FWType);
extern uint8_t LoadUserAreaToPM(void);
extern uint8_t RdBurstUareaFromPm(uint32_t UlAddress, uint8_t *PucData,
				  uint8_t UcLength, uint8_t mode);
extern uint8_t RdSingleUareaFromPm(uint32_t UlAddress, uint8_t *PucData,
				   uint8_t UcLength, uint8_t mode);
extern uint8_t WrUareaToPm(uint32_t UlAddress, uint8_t *PucData,
			   uint8_t UcLength, uint8_t mode);
extern uint8_t WrUareaToPm(uint32_t UlAddress, uint8_t *PucData,
			   uint8_t UcLength, uint8_t mode);
...
{
	int result;
	UINT16 addr_ary[] = { 0x87F0, 0x87F4, 0x01F8, 0x021C, 0x0220, 0x0224,
			      0x0230, 0x234,  0x1F0,  0x214,  0x1E0,  0x204,
			      0x1D8,  0x1FC,  0x1F4,  0x218,  0x82A0, 0x8300,
			      0x8294, 0x82F4, 0x8370, 0x8374, 0x82B4, 0x8314 };
	UINT32 addr, value;
	int i = 0;
	int arry_size = 0;
	...
	while (1) {
		if (ois_ctrls[CAM_OIS_MASTER]->cam_ois_state >=
		    CAM_OIS_CONFIG) {
			for (i = 0; i < arry_size; i++) {
				addr = addr_ary[i];
				result = RamRead16A_oplus(
					ois_ctrls[CAM_OIS_MASTER], addr,
					&value);
				if (result < 0) {
					CAM_ERR(CAM_OIS,
						"read addr = 0x%x, value = 0x%x fail",
						addr, value);
				} else {
					CAM_ERR(CAM_OIS,
						"read addr = 0x%x, value = 0x%x success",
						addr, value);
				}
			}
		}
...
static ssize_t ois_read(struct file *p_file, char __user *puser_buf,
			size_t count, loff_t *p_offset)
{
	return 0;
}

static ssize_t ois_write(struct file *p_file, const char __user *puser_buf,
			 size_t count, loff_t *p_offset)
{
	...
	if (puser_buf) {
		if (copy_from_user(&data, puser_buf, count)) {
			CAM_ERR(CAM_OIS,
				"copy_from_user failed ,please check input parameter ");
		}
	}
	...
	if (ois_ctrls[CAM_OIS_MASTER] && addr != 0) {
		if (value == 0xFFFF) {
			RamRead32A_oplus(ois_ctrls[CAM_OIS_MASTER], addr,
					 &read_data);
			CAM_ERR(CAM_OIS,
				"read ois data addr = 0x%x, value = 0x%x ",
				addr, read_data);
		} else if (addr == 0xFFFF) {
			g_enable_ois_read_gyro_data = value;
			CAM_ERR(CAM_OIS,
				"set thead status  addr = 0x%x, value = 0x%x ",
				addr, value);
		} else {
			result = RamWrite32A_oplus(ois_ctrls[CAM_OIS_MASTER],
						   addr, value);
		}
		if (result < 0) {
			CAM_ERR(CAM_OIS, "write addr = 0x%x, value = 0x%x fail",
				addr, value);
		} else {
			CAM_INFO(CAM_OIS,
				 "write addr = 0x%x, value = 0x%x success",
				 addr, value);
		}
	}
	...
	if (g_ois_read_gyro_data_thread == NULL) {
		g_ois_read_gyro_data_thread =
			kthread_run(ois_read_gyro_data_thread, NULL,
				    "ois_read_gyro_data_thread");
...
int ois_read_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if (o_ctrl->ois_read_thread_start_to_read) {
			...
			CAM_ERR(CAM_OIS, "%s OIS register data: %s",
				o_ctrl->ois_name, buf);
		}
		mutex_unlock(&(o_ctrl->ois_read_mutex));
...
int ois_start_read_thread(void *arg, bool start)
{
	...
	if (start) {
		if (o_ctrl->ois_read_thread) {
			CAM_ERR(CAM_OIS,
				"ois_read_thread is already created, no need to create again.");
		} else {
			o_ctrl->ois_read_thread = kthread_run(
				ois_read_thread, o_ctrl, o_ctrl->ois_name);
			if (!o_ctrl->ois_read_thread) {
				CAM_ERR(CAM_OIS,
					"create ois read thread failed");
				mutex_unlock(&(o_ctrl->ois_read_mutex));
				return -2;
			}
		}
		...
		if (o_ctrl->ois_read_thread) {
			...
			CAM_ERR(CAM_OIS,
				"ois_read_thread is already stopped, no need to stop again.");
		}
	}
	mutex_unlock(&(o_ctrl->ois_read_mutex));
...
void CntRd(uint32_t addr, void *data, uint16_t size)
{
	...
	for (i = 0; i < retry; i++) {
		rc = camera_io_dev_read_seq(&(o_ctrl->io_master_info), addr,
					    (uint8_t *)data,
					    CAMERA_SENSOR_I2C_TYPE_WORD,
					    CAMERA_SENSOR_I2C_TYPE_BYTE, size);
		if (rc < 0) {
			CAM_ERR(CAM_OIS,
				"Continue read failed, rc:%d, retry:%d", rc,
				i + 1);
		} else {
			break;
		}
...
void CntWrt(void *register_data, uint16_t size)
{
	...
	memset(i2c_write_setting_gl, 0,
	       sizeof(struct cam_sensor_i2c_reg_array) * reg_data_cnt);
	...
	for (i = 0; i < reg_data_cnt; i++) {
		if (i == 0) {
			...
			i2c_write_setting_gl[continue_cnt].reg_data =
				data[i + 1];
			i2c_write_setting_gl[continue_cnt].delay = 0x00;
			i2c_write_setting_gl[continue_cnt].data_mask = 0x00;
		}
	...
	for (i = 0; i < retry; i++) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_OIS,
				"Continue write failed, rc:%d, retry:%d", rc,
				i + 1);
		} else {
			break;
		}
...
int RamWrite32A(uint32_t addr, uint32_t data)
{
	...
	for (i = 0; i < retry; i++) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_OIS, "write 0x%04x failed, retry:%d", addr,
				i + 1);
		} else {
			return rc;
		}
...
int RamRead32A(uint32_t addr, uint32_t *data)
{
	...
	for (i = 0; i < retry; i++) {
		rc = camera_io_dev_read(&(o_ctrl->io_master_info),
					...
					CAMERA_SENSOR_I2C_TYPE_DWORD);
		if (rc < 0) {
			CAM_ERR(CAM_OIS, "read 0x%04x failed, retry:%d", addr,
				i + 1);
		} else {
			return rc;
		}
...
int RamWrite32A_oplus(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr,
		      uint32_t data)
{
	int32_t rc = 0;
	int retry = 3;
...
int RamRead32A_oplus(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr,
		     uint32_t *data)
{
	int32_t rc = 0;
	int retry = 3;
...
int RamRead16A_oplus(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr,
		     uint32_t *data)
{
	int32_t rc = 0;
	int retry = 3;
...
void OISCountinueRead(struct cam_ois_ctrl_t *o_ctrl, uint32_t addr, void *data,
		      uint16_t size)
{
	int i = 0;
	int32_t rc = 0;
...
void OISCountinueWrite(struct cam_ois_ctrl_t *o_ctrl, void *register_data,
		       uint16_t size)
{
	...
	memset(i2c_write_setting_gl, 0,
	       sizeof(struct cam_sensor_i2c_reg_array) * MAX_DATA_NUM * 2);
...
static int Download124Or128FW(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_INFO(
		CAM_OIS,
		"MasterSlave 0x%x, GyroVendor 0x%x, GyroPosition 0x%x, ModuleVendor 0x%x, ActVer 0x%x, FWType 0x%x\n",
		o_ctrl->ois_type, o_ctrl->ois_gyro_vendor,
		o_ctrl->ois_gyro_position, o_ctrl->ois_module_vendor,
		o_ctrl->ois_actuator_vendor, o_ctrl->ois_fw_flag);

	if (strstr(o_ctrl->ois_name, "124")) {
		rc = SelectDownload(o_ctrl->ois_gyro_vendor,
				    o_ctrl->ois_actuator_vendor,
				    o_ctrl->ois_type, o_ctrl->ois_fw_flag);
		...
		if (0 == rc) {
			...
			CAM_INFO(CAM_OIS, "Gyro_X:0x%x, Gyro_Y:0x%x",
				 UlReadValX, UlReadValY);
			...
			CAM_INFO(CAM_OIS, "Gyro_gain_X:0x%x, Gyro_gain_Y:0x%x",
				 UlReadValX, UlReadValY);
			...
			case 0x02:
				CAM_ERR(CAM_OIS,
					"Table Data & Program download verify error");
				break;
			case 0xF0:
			...
			case 0xF1:
				CAM_ERR(CAM_OIS,
					"Download code information read error");
				break;
			case 0xF2:
				CAM_ERR(CAM_OIS,
					"Download code information disagreement");
				break;
			case 0xF3:
		...
		rc = FlashDownload128(o_ctrl->ois_module_vendor,
				      o_ctrl->ois_actuator_vendor,
				      o_ctrl->ois_type, o_ctrl->ois_fw_flag);
		...
		if (0 == rc) {
			...
			case 0x00:
				CAM_ERR(CAM_OIS,
					"Error ; during the rom boot changing. Also including 128 power off issue.");
				break;
			case 0x20:
				CAM_ERR(CAM_OIS,
					"Error ; during Initial program for updating to program memory.");
				break;
			case 0x30:
				CAM_ERR(CAM_OIS,
					"Error ; during User Mat area erasing.");
				break;
			case 0x40:
				CAM_ERR(CAM_OIS,
					"Error ; during User Mat area programing.");
				break;
			case 0x50:
				CAM_ERR(CAM_OIS,
			...
			case 0x90:
				CAM_ERR(CAM_OIS,
					"Error ; during the drive offset confirmation.");
				break;
			case 0xA0:
				CAM_ERR(CAM_OIS,
					"Error ; during the MAT2 re-write process.");
				break;
			case 0xF0:
				if (rc == 0xF0)
					CAM_ERR(CAM_OIS,
						"mistake of module vendor designation.");
				else if (rc == 0xF1)
					CAM_ERR(CAM_OIS,
						"mistake size of From Code.");
				break;
			default:
				CAM_ERR(CAM_OIS, "Unkown error code");
				break;
	...
	CAM_INFO(CAM_OIS, "cam_ois_fw_download rc=%d, (Spend: %d ms)", rc,
		 mSpendTime);
...
int Initcheck128(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	RamRead32A(0x82b8, &UlReadValX);
	RamRead32A(0x8318, &UlReadValY);
	CAM_INFO(CAM_OIS, "Gyro_gain_X:0x%x, Gyro_gain_Y:0x%x", UlReadValX,
		 UlReadValY);
	return 0;
}
...
int DownloadFW(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl) {
		...
		if (CAM_OIS_INVALID == ois_state[o_ctrl->ois_type]) {
			if (CAM_OIS_MASTER == o_ctrl->ois_type) {
				rc = Download124Or128FW(
					ois_ctrls[CAM_OIS_MASTER]);
				if (rc) {
					CAM_ERR(CAM_OIS,
						"Download %s FW failed",
						o_ctrl->ois_name);
				} else {
					if (dump_ois_registers &&
					    !ois_start_read_thread(
						    ois_ctrls[CAM_OIS_MASTER],
						    1)) {
						ois_start_read(
							ois_ctrls[CAM_OIS_MASTER],
				...
				if (CAM_OIS_INVALID ==
				    ois_state[CAM_OIS_MASTER]) {
					rc = Download124Or128FW(
						ois_ctrls[CAM_OIS_MASTER]);
					if (!rc && dump_ois_registers &&
					    !ois_start_read_thread(
						    ois_ctrls[CAM_OIS_MASTER],
						    1)) {
						ois_start_read(
			...
			CAM_ERR(CAM_OIS, "OIS state 0x%x is wrong",
...
#define SAMPLE_INTERVAL 4000
	...
	uint32_t fifo_size_in_ois =
		SAMPLE_COUNT_IN_OIS_124 * SAMPLE_SIZE_IN_DRIVER;
	uint32_t fifo_size_in_driver =
		SAMPLE_COUNT_IN_DRIVER * SAMPLE_SIZE_IN_DRIVER;
	unsigned long long timestampQ = 0;

	struct cam_ois_ctrl_t *o_ctrl = (struct cam_ois_ctrl_t *)arg;
	uint32_t ois_hall_registers[SAMPLE_COUNT_IN_OIS_124] = {
		0x89C4, 0x89C0, 0x89BC, 0x89B8, 0x89B4, 0x89B0, 0x89AC
	};
	...
	while (1) {
		...
		for (i = 0; i < SAMPLE_COUNT_IN_OIS_124; i++) {
			...
			OISRead(o_ctrl, ois_hall_registers[i],
				&(data[3 * i + 2]));
			timestampQ -= 2 * CLOCK_TICKCOUNT_MS;
		}

		for (i = SAMPLE_COUNT_IN_OIS_124 - 1; i >= 0; i--) {
			CAM_DBG(CAM_OIS, "OIS HALL data %lld (0x%x 0x%x)",
				((uint64_t)data[3 * i] << 32) +
					(uint64_t)data[3 * i + 1],
				data[3 * i + 2] & 0xFFFF0000 >> 16,
				data[3 * i + 2] & 0xFFFF);
		}

		mutex_lock(&(o_ctrl->ois_hall_data_mutex));
		if ((kfifo_len(&(o_ctrl->ois_hall_data_fifo)) +
		     fifo_size_in_ois) > fifo_size_in_driver) {
			CAM_DBG(CAM_OIS,
				"ois_hall_data_fifo is full, fifo size %d, file len %d, will reset FIFO",
		...
		if ((kfifo_len(&(o_ctrl->ois_hall_data_fifo)) +
			...
			kfifo_in_len = kfifo_in(&(o_ctrl->ois_hall_data_fifo),
						data, fifo_size_in_ois);
			if (kfifo_in_len != fifo_size_in_ois) {
				CAM_DBG(CAM_OIS,
					"kfifo_in %d Bytes, FIFO maybe full, some OIS Hall sample maybe dropped.",
					kfifo_in_len);
			} else {
				CAM_DBG(CAM_OIS, "kfifo_in %d Bytes",
					fifo_size_in_ois);
			}
		}
		mutex_unlock(&(o_ctrl->ois_hall_data_mutex));

		usleep_range(SAMPLE_COUNT_IN_OIS_124 * SAMPLE_INTERVAL - 5,
			     SAMPLE_COUNT_IN_OIS_124 * SAMPLE_INTERVAL);
	}
...
int OISPollThread128(void *arg)
{
	...
	uint32_t fifo_size_in_ois_aligned =
		SAMPLE_COUNT_IN_OIS * SAMPLE_SIZE_IN_OIS_ALIGNED;
	uint32_t fifo_size_in_driver =
		SAMPLE_COUNT_IN_DRIVER * SAMPLE_SIZE_IN_DRIVER;
	uint16_t *p_hall_data_in_ois = NULL;
	struct cam_ois_hall_data_in_ois_aligned *p_hall_data_in_ois_aligned =
		NULL;
	struct cam_ois_hall_data_in_driver *p_hall_data_in_driver = NULL;
	struct cam_ois_ctrl_t *o_ctrl = (struct cam_ois_ctrl_t *)arg;

	uint64_t first_QTimer =
		0; // This will be used for the start QTimer to calculate the QTimer interval
	uint64_t prev_QTimer =
		0; // This is the last QTimer in the last CCI read
	uint64_t current_QTimer =
		0; // This will be used for the end QTimer to calculate the QTimer interval
	uint64_t interval_QTimer =
		0; // This is the QTimer interval between two sample

	uint64_t sample_offset =
		0; // This is timestamp offset between IC and system
	uint64_t readout_time =
		(((1 + 2 + 1 + SAMPLE_SIZE_IN_OIS * SAMPLE_COUNT_IN_OIS) * 8) /
		 1000) *
		CLOCK_TICKCOUNT_MS; // This is the time of CCI read
	uint16_t sample_count = 0; // This is the sample count for one CCI read
	uint16_t sample_num =
		0; // This will be used to detect whehter some HALL data was dropped
	uint32_t total_sample_count =
		0; // This will be used to calculate the QTimer interval
	uint16_t threshold =
		2; // This is the threshold to trigger Timestamp calibration, this means 2ms
	uint16_t tmp = 0;
	...
	p_hall_data_in_ois_aligned =
		kzalloc(fifo_size_in_ois_aligned, GFP_KERNEL);
	if (!p_hall_data_in_ois_aligned) {
		CAM_ERR(CAM_OIS,
			"failed to kzalloc p_hall_data_in_ois_aligned");
		kfree(p_hall_data_in_ois);
		return -1;
	}

	p_hall_data_in_driver = kzalloc(
		SAMPLE_COUNT_IN_OIS * SAMPLE_SIZE_IN_DRIVER, GFP_KERNEL);
	...
	RamWrite32A_oplus(
		o_ctrl, 0xF110,
		0x0); //Clear buffer to all "0" & enable buffer update function.
	...
	while (1) {
		...
		memset(p_hall_data_in_driver, 0,
		       SAMPLE_COUNT_IN_OIS * SAMPLE_SIZE_IN_DRIVER);
		...
		OISCountinueRead(o_ctrl, 0xF111, (void *)p_hall_data_in_ois,
				 fifo_size_in_ois);
		...
		for (i = 0, j = 0; i < SAMPLE_COUNT_IN_OIS; i++) {
			if (((p_hall_data_in_ois[3 * i] == 0) &&
			     (p_hall_data_in_ois[3 * i + 1] == 0) &&
			     (p_hall_data_in_ois[3 * i + 2] == 0)) ||
			    (p_hall_data_in_ois[3 * i] == OIS_MAGIC_NUMBER &&
			     p_hall_data_in_ois[3 * i + 1] ==
				     OIS_MAGIC_NUMBER)) {
				CAM_DBG(CAM_OIS,
					"OIS HALL RAW data %d %d (0x%x 0x%x)",
					i, p_hall_data_in_ois[3 * i],
					p_hall_data_in_ois[3 * i + 1],
					p_hall_data_in_ois[3 * i + 2]);
			} else {
				p_hall_data_in_ois_aligned[j].hall_data_cnt =
					p_hall_data_in_ois[3 * i];
				p_hall_data_in_ois_aligned[j].hall_data =
					((uint32_t)p_hall_data_in_ois[3 * i + 1]
					 << 16) +
					p_hall_data_in_ois[3 * i + 2];
				CAM_DBG(CAM_OIS,
					"OIS HALL RAW data %d %d (0x%x 0x%x)",
					i,
					p_hall_data_in_ois_aligned[j]
						.hall_data_cnt,
					p_hall_data_in_ois_aligned[j].hall_data &
						0xFFFF0000 >> 16,
					p_hall_data_in_ois_aligned[j].hall_data &
		...
		sample_offset =
			(uint64_t)((p_hall_data_in_ois
					    [3 * (SAMPLE_COUNT_IN_OIS - 1) + 2] &
				    0xFF) *
				   CLOCK_TICKCOUNT_MS * 2 / OIS_MAX_COUNTER);

		if (first_QTimer == 0) {
			//Init some parameters
			for (i = 0; i < SAMPLE_COUNT_IN_OIS; i++) {
				if ((p_hall_data_in_ois_aligned[i].hall_data ==
				     0) &&
				    (p_hall_data_in_ois_aligned[i]
					     .hall_data_cnt == 0)) {
					break;
				}
			}
			if ((i >= 1) && (i <= SAMPLE_COUNT_IN_OIS)) {
				first_QTimer = arch_counter_get_cntvct() -
					       readout_time - sample_offset;
				prev_QTimer = first_QTimer;
				sample_num = p_hall_data_in_ois_aligned[i - 1]
						     .hall_data_cnt;
			}
			continue;
		} else {
			vaild_cnt = 0;
			current_QTimer = arch_counter_get_cntvct() -
					 readout_time - sample_offset;
			...
			if (sample_count > 0) {
				...
				for (i = 0; i < SAMPLE_COUNT_IN_OIS; i++) {
					if ((p_hall_data_in_ois_aligned[i]
						     .hall_data != 0) ||
					    (p_hall_data_in_ois_aligned[i]
						     .hall_data_cnt != 0)) {
						if (i == 0) {
							...
							estimate_QTimer =
								((uint64_t)p_hall_data_in_driver
									 [i - 1]
										 .high_dword
								 << 32) +
								(uint64_t)p_hall_data_in_driver
				...
				CAM_DBG(CAM_OIS,
					"OIS HALL data before %lld %lld",
					real_QTimer - pre_real_QTimer -
						(vaild_cnt - 1) *
							interval_QTimer,
					pre_real_QTimer -
						(real_QTimer -
				...
				if ((pre_real_QTimer != 0) &&
				    ((int64_t)(real_QTimer - pre_real_QTimer -
					       (vaild_cnt -
						1) * interval_QTimer) > 42000 ||
				     (int64_t)(real_QTimer - pre_real_QTimer -
					       (vaild_cnt - 1) *
						       interval_QTimer) <
					     34000)) {
				...
				pre_QTimer_offset =
					real_QTimer - pre_real_QTimer -
					(vaild_cnt - 1) * interval_QTimer;

				for (i = 0; i < vaild_cnt; i++) {
					p_hall_data_in_driver[vaild_cnt - i - 1]
						.high_dword = real_QTimer >> 32;
					p_hall_data_in_driver[vaild_cnt - i - 1]
						.low_dword =
						real_QTimer & 0xFFFFFFFF;
					real_QTimer -= interval_QTimer;
				}

				for (i = 0; i < vaild_cnt; i++) {
					CAM_DBG(CAM_OIS,
						"OIS HALL data %lld (0x%x 0x%x) pre :%lld offset reg:%d i2c_read_offset %lld",
						((uint64_t)
							 p_hall_data_in_driver[i]
								 .high_dword
						 << 32) +
				...
				if ((kfifo_len(&(o_ctrl->ois_hall_data_fifoV2)) +
				     vaild_cnt * SAMPLE_SIZE_IN_DRIVER) >
				    fifo_size_in_driver) {
					CAM_DBG(CAM_OIS,
						"ois type=%d,ois_hall_data_fifoV2 is full, fifo size %d, file len %d, will reset FIFO",
						o_ctrl->ois_type,
						kfifo_size(&(
							o_ctrl->ois_hall_data_fifoV2)),
						kfifo_len(&(
							o_ctrl->ois_hall_data_fifoV2)));
					kfifo_reset(&(
						o_ctrl->ois_hall_data_fifoV2));
				...
				if ((kfifo_len(&(o_ctrl->ois_hall_data_fifo)) +
				     vaild_cnt * SAMPLE_SIZE_IN_DRIVER) <=
				    fifo_size_in_driver) {
					kfifo_in_len = kfifo_in(
						&(o_ctrl->ois_hall_data_fifo),
						p_hall_data_in_driver,
						vaild_cnt *
							SAMPLE_SIZE_IN_DRIVER);
					if (kfifo_in_len !=
					    vaild_cnt * SAMPLE_SIZE_IN_DRIVER) {
						CAM_DBG(CAM_OIS,
							"kfifo_in %d Bytes, FIFO maybe full, some OIS Hall sample maybe dropped.",
				...
				if ((kfifo_len(&(o_ctrl->ois_hall_data_fifoV2)) +
					...
					if (kfifo_in_len !=
					    vaild_cnt * SAMPLE_SIZE_IN_DRIVER) {
						CAM_DBG(CAM_OIS,
							"ois type=%d,kfifo_in %d Bytes, FIFOV2 maybe full, some OIS Hall sample maybe dropped.",
							o_ctrl->ois_type,
							kfifo_in_len);
					} else {
						CAM_DBG(CAM_OIS,
							"ois type=%d,kfifo_inV2 %d Bytes",
							o_ctrl->ois_type,
							vaild_cnt *
								SAMPLE_SIZE_IN_DRIVER);
					}
...
#define SEM1215S_SAMPLE_INTERVAL 4000
	...
	uint32_t kfifo_in_len = 0;
	uint32_t fifo_size_in_ois =
		SEM1215S_SAMPLE_COUNT_IN_OIS * OIS_HALL_SAMPLE_BYTE;
	uint32_t fifo_size_in_ois_driver =
		OIS_HALL_SAMPLE_COUNT * OIS_HALL_SAMPLE_BYTE;
	unsigned long long timestampQ = 0;
	...
	while (1) {
		...
		for (i = 0; i < 1; i++) {
			...
			camera_io_dev_read(&(o_ctrl->io_master_info),
					   (uint32_t)ois_hall_registers[i],
					   &data_x, CAMERA_SENSOR_I2C_TYPE_WORD,
					   CAMERA_SENSOR_I2C_TYPE_WORD);
			camera_io_dev_read(&(o_ctrl->io_master_info),
					   (uint32_t)ois_hall_registers[i] + 2,
					   &data_y, CAMERA_SENSOR_I2C_TYPE_WORD,
					   CAMERA_SENSOR_I2C_TYPE_WORD);
			data[3 * i + 2] =
				(data_x & 0xFFFF) | ((data_y & 0xFFFF) << 16);
		...
		mutex_lock(&(o_ctrl->ois_hall_data_mutex));
		if ((kfifo_len(&(o_ctrl->ois_hall_data_fifo)) +
		     fifo_size_in_ois) > fifo_size_in_ois_driver) {
			CAM_DBG(CAM_OIS,
				"ois_hall_data_fifo is full, fifo size %d, file len %d, will reset FIFO",
				kfifo_size(&(o_ctrl->ois_hall_data_fifo)),
				kfifo_len(&(o_ctrl->ois_hall_data_fifo)));
		...
		mutex_unlock(&(o_ctrl->ois_hall_data_mutex));
		usleep_range(SEM1215S_SAMPLE_INTERVAL - 5,
			     SEM1215S_SAMPLE_INTERVAL);
	}
...
int OISPollThread169(void *arg)
{
	...
	pHalldata = kzalloc(OIS_HALL_DATA_SIZE * sizeof(struct hall_info),
			    GFP_KERNEL);
	if (!pHalldata) {
	...
	while (1) {
		...
		memset(pHalldata, 0,
		       OIS_HALL_DATA_SIZE * sizeof(struct hall_info));
		...
		rc = camera_io_dev_read_seq(&(o_ctrl->io_master_info), 0x8A,
		...
		if (dataNum <= 0 || dataNum > HALL_MAX_NUMBER) {
			CAM_ERR(CAM_OIS, "get a wrong number of hall data:%d",
				dataNum);
			continue;
		}
		for (i = 0; i < dataNum; i++) {
			pHalldata[i].mHalldata =
		...
		CAM_DBG(CAM_OIS,
			"end time afters: sec = %d, us = %d,dataNum=%d delayCount=%d,delayTime=%d",
			endTime.tv_sec, endTime.tv_usec, dataNum, delayCount,
			delayTime);
		//1.get gap_usec
		...
		if (dataNum == HALL_MAX_NUMBER || isFirstFrame) {
			...
			isFirstFrame = false;
			CAM_DBG(CAM_OIS,
				"Data drop!!,reset LastPacketEndTime: sec = %d, us = %d",
				LastPacketEndTime.tv_sec,
				LastPacketEndTime.tv_usec);
		}
		gap_usec = 1000 * 1000 *
				   (estimateLastEndTime.tv_sec -
				    LastPacketEndTime.tv_sec) +
			   estimateLastEndTime.tv_usec -
			   LastPacketEndTime.tv_usec;
		CAM_DBG(CAM_OIS,
			"ois packet gap(usec)=%d Calculated last tv_sec=%d tv_usec=%d",
			gap_usec, estimateLastEndTime.tv_sec,
			estimateLastEndTime.tv_usec);
		...
		CAM_DBG(CAM_OIS, "Estimated last tv_sec=%d tv_usec=%d",
			estimateLastEndTime.tv_sec,
			estimateLastEndTime.tv_usec);
		...
		for (i = 0; i < dataNum; i++) {
			...
			pHalldata[i].timeStampUsec =
				estimateLastEndTime.tv_usec;
			CAM_DBG(CAM_OIS,
				"camxhalldata[%d] X:0x%04x  halldataY:0x%04x  sec = %d, us = %d",
				i, mHalldata_X, mHalldata_Y,
		...
		mutex_lock(&(o_ctrl->ois_hall_data_mutex));
		if ((kfifo_len(&(o_ctrl->ois_hall_data_fifo)) + validSize) >
		    fifo_size_in_driver) {
			CAM_DBG(CAM_OIS,
				"ois_hall_data_fifo is full, fifo size %d, file len %d, will reset FIFO",
				kfifo_size(&(o_ctrl->ois_hall_data_fifo)),
		...
		if ((kfifo_len(&(o_ctrl->ois_hall_data_fifoV2)) + validSize) >
		    fifo_size_in_driver) {
			CAM_DBG(CAM_OIS,
				"ois type=%d,ois_hall_data_fifoV2 is full, fifo size %d, file len %d, will reset FIFO",
				o_ctrl->ois_type,
...
void ReadOISHALLData(struct cam_ois_ctrl_t *o_ctrl, void *data)
{
	...
	if (fifo_len_in_ois_driver > 0) {
		if (fifo_len_in_ois_driver >
		    SAMPLE_SIZE_IN_DRIVER * SAMPLE_COUNT_IN_DRIVER) {
			fifo_len_in_ois_driver =
				SAMPLE_SIZE_IN_DRIVER * SAMPLE_COUNT_IN_DRIVER;
		}
		kfifo_to_user(&(o_ctrl->ois_hall_data_fifo), data,
			      fifo_len_in_ois_driver, &data_size);
		CAM_DBG(CAM_OIS, "ReadOISHALLData Copied %d Bytes to UMD",
			data_size);
	} else {
		CAM_DBG(CAM_OIS,
			"ReadOISHALLData fifo_len is %d, no need copy to UMD",
			fifo_len_in_ois_driver);
	}
...
void ReadOISHALLDataV2(struct cam_ois_ctrl_t *o_ctrl, void *data)
{
	...
	if (fifo_len_in_ois_driver > 0) {
		...
		kfifo_to_user(&(o_ctrl->ois_hall_data_fifoV2), data,
			      fifo_len_in_ois_driver, &data_size);
		CAM_DBG(CAM_OIS, "ois type=%d,Copied %d Bytes to UMD EISv2",
			o_ctrl->ois_type, data_size);
	} else {
		CAM_DBG(CAM_OIS,
			"ois type=%d,fifo_len is %d, no need copy to UMD EISv2",
			o_ctrl->ois_type, fifo_len_in_ois_driver);
	}
...
void Sem1215sReadOISHALLData(struct cam_ois_ctrl_t *o_ctrl, void *data)
{
	...
	if (fifo_len_in_ois_driver > 0) {
		...
		kfifo_to_user(&(o_ctrl->ois_hall_data_fifo), data,
			      fifo_len_in_ois_driver, &data_size);
		CAM_INFO(CAM_OIS,
			 "Sem1215sReadOISHALLData Copied %d Bytes to UMD",
			 data_size);
	} else {
		CAM_INFO(
			CAM_OIS,
			"Sem1215sReadOISHALLData fifo_len is %d, no need copy to UMD",
			fifo_len_in_ois_driver);
	}
	mutex_unlock(&(o_ctrl->ois_hall_data_mutex));
}
	...
	if (o_ctrl) {
		if (strstr(o_ctrl->ois_name, "sem1215") != NULL) {
			...
			if (o_ctrl->ois_poll_thread ||
			    false == g_is_enable_tele_ois_thread) {
				CAM_ERR(CAM_OIS,
					"tele camera ois_poll_thread is already created, no need to create again.");
			} else {
				o_ctrl->ois_poll_thread_exit = false;
				o_ctrl->ois_poll_thread =
					kthread_run(Sem1215sOISPollThread,
						    o_ctrl, o_ctrl->ois_name);
			}
			mutex_unlock(&(o_ctrl->ois_poll_thread_mutex));
			if (!o_ctrl->ois_poll_thread) {
				o_ctrl->ois_poll_thread_exit = true;
				CAM_ERR(CAM_OIS,
					"create ois poll thread failed");
				return false;
			}
			return true;
		}
		...
		if (CAM_OIS_READY == ois_state[o_ctrl->ois_type]) {
			CAM_INFO(CAM_OIS, "%s OIS %d is ready ",
				 o_ctrl->ois_name, o_ctrl->ois_type);

			mutex_lock(&(o_ctrl->ois_poll_thread_mutex));
			if (o_ctrl->ois_poll_thread ||
			    false == g_is_enable_main_ois_thread) {
				CAM_ERR(CAM_OIS,
					"main camera ois_poll_thread is already created or don't need to create..");
			} else {
				o_ctrl->ois_poll_thread_exit = false;
				if (strstr(o_ctrl->ois_name, "128")) {
					o_ctrl->ois_poll_thread =
						kthread_run(OISPollThread128,
							    o_ctrl,
							    o_ctrl->ois_name);
				} else if (strstr(o_ctrl->ois_name, "124")) {
					o_ctrl->ois_poll_thread =
						kthread_run(OISPollThread124,
							    o_ctrl,
							    o_ctrl->ois_name);
				} else if (strstr(o_ctrl->ois_name,
						  "bu63169")) {
					o_ctrl->ois_poll_thread =
						kthread_run(OISPollThread169,
							    o_ctrl,
							    o_ctrl->ois_name);
					set_user_nice(o_ctrl->ois_poll_thread,
						      -2);
				}
				if (!o_ctrl->ois_poll_thread) {
					o_ctrl->ois_poll_thread_exit = true;
			...
			do {
				...
				CAM_ERR(CAM_OIS, "OIS %d 0xF100 = 0x%x",
					o_ctrl->ois_type, temp);
				if (temp == 0) {
					ois_state[o_ctrl->ois_type] =
						CAM_OIS_READY;
					return true;
...
void InitOIS(struct cam_ois_ctrl_t *o_ctrl)
{
	if (o_ctrl) {
		...
		if (o_ctrl->ois_type == CAM_OIS_MASTER) {
			...
			CAM_ERR(CAM_OIS, "ois_type 0x%x is wrong",
				o_ctrl->ois_type);
		}
	} else {
		CAM_ERR(CAM_OIS, "o_ctrl is NULL");
	...
	if (o_ctrl) {
		...
		if (o_ctrl->ois_type == CAM_OIS_MASTER) {
			if (dump_ois_registers && ois_ctrls[CAM_OIS_MASTER]) {
				ois_start_read_thread(ois_ctrls[CAM_OIS_MASTER],
						      0);
			}
			ois_state[CAM_OIS_MASTER] = CAM_OIS_INVALID;
		} else if (o_ctrl->ois_type == CAM_OIS_SLAVE) {
			if (ois_ctrls[CAM_OIS_MASTER]) {
				if (dump_ois_registers) {
					ois_start_read_thread(
						ois_ctrls[CAM_OIS_MASTER], 0);
				}
				if (camera_io_release(
					    &(ois_ctrls[CAM_OIS_MASTER]
						      ->io_master_info))) {
...
void InitOISResource(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl) {
		...
		if (o_ctrl->ois_type == CAM_OIS_MASTER) {
			...
			CAM_INFO(CAM_OIS, "ois_ctrls[%d] = %p", CAM_OIS_MASTER,
				 ois_ctrls[CAM_OIS_MASTER]);
		} else if (o_ctrl->ois_type == CAM_OIS_SLAVE) {
			ois_ctrls[CAM_OIS_SLAVE] = o_ctrl;
			CAM_INFO(CAM_OIS, "ois_ctrls[%d] = %p", CAM_OIS_SLAVE,
				 ois_ctrls[CAM_OIS_SLAVE]);
		} else {
			CAM_ERR(CAM_OIS, "ois_type 0x%x is wrong",
				o_ctrl->ois_type);
		...
		if (!ois_dentry) {
			...
			if (ois_dentry) {
				debugfs_create_bool("dump_registers", 0644,
						    ois_dentry,
						    &dump_ois_registers);
			} else {
				CAM_ERR(CAM_OIS,
					"failed to create dump_registers node");
			}
		} else {
	...
	proc_file_entry = proc_create("OISControl", 0777, face_common_dir,
				      &proc_file_fops);
...
void WriteEEpromData(struct cam_write_eeprom_t *cam_write_eeprom)
{
	...
	*/
	...
	for (i = 0;
	     i < (cam_write_eeprom->calibDataSize / MAX_EEPROM_PACK_SIZE);
	     i++) {
		WrUareaToPm((cam_write_eeprom->baseAddr +
			     (i * MAX_EEPROM_PACK_SIZE / EEPROM_UNIT_SIZE)),
			    &cam_write_eeprom->calibData[0],
			    MAX_EEPROM_PACK_SIZE, 0);
	}
	addr_offset = cam_write_eeprom->baseAddr + (i * MAX_EEPROM_PACK_SIZE);
	WrUareaToPm(addr_offset,
		    &cam_write_eeprom->calibData[i * MAX_EEPROM_PACK_SIZE],
		    (cam_write_eeprom->calibDataSize % MAX_EEPROM_PACK_SIZE),
		    0);
