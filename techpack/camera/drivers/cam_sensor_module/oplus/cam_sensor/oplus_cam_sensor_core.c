...
static int RamWriteByte(struct camera_io_master *cci_master_info, uint32_t addr,
			uint32_t data, unsigned short mdelay)
{
	int32_t rc = 0;
	int retry = 1;
	int i = 0;
	...
	for (i = 0; i < retry; i++) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "write 0x%04x failed, retry:%d",
				addr, i + 1);
		} else {
			return rc;
		}
...
static int RamWriteWord(struct camera_io_master *cci_master_info, uint32_t addr,
			uint32_t data)
{
	int32_t rc = 0;
	int retry = 1;
...
static int cam_sem1815s_ois_fw_download(struct cam_sensor_ctrl_t *o_ctrl)
{
	...
	if (data == ois_fw_version || data == SEM1815S_SEC_FW_UPDATE_VERSION ||
	    data == ABNORMAL_FW_UPDATE_VERSION) {
		CAM_INFO(
			CAM_SENSOR,
			"fw version:0x%0x need to update ois_fw_version:0x%0x !!!",
			data, ois_fw_version);
	} else {
		CAM_INFO(
			CAM_SENSOR,
			"fw version:0x%0x ois_fw_version:0x%0x no need to update !!!",
			data, ois_fw_version);
		o_ctrl->io_master_info.cci_client->sid = tmp_slave_addr;
		return 0;
	}
	...
	if (data != 0x01) {
		...
		rc = camera_io_dev_read(&(o_ctrl->io_master_info), 0x0201,
	...
	for (cnt = 0, ptr = (uint8_t *)fw->data; cnt < total_bytes;) {
		...
		for (i = 0; (i < MAX_LENGTH && cnt < total_bytes); i++, ptr++) {
			if (cnt >= SEM1815S_FW_VERSION_OFFSET &&
			    cnt < (SEM1815S_FW_VERSION_OFFSET + 4)) {
				fw_ver[cnt - SEM1815S_FW_VERSION_OFFSET] = *ptr;
				CAM_ERR(CAM_SENSOR, "get fw version:0x%0x",
					fw_ver[cnt -
					       SEM1815S_FW_VERSION_OFFSET]);
			}
	...
	RamWriteWord(&(o_ctrl->io_master_info), 0x1002,
		     ((check_sum & 0x0FF) << 8) | ((check_sum & 0xFF00) >> 8));
	msleep(10);
...
static int writefileData(char *filename, int data)
{
	...
	if (size < 0) {
		CAM_ERR(CAM_SENSOR, "write file:%s data:%d error size:%d",
			filename, data, size);
		set_fs(old_fs);
		filp_close(mfile, NULL);
		return (-1);
	} else {
		CAM_INFO(CAM_SENSOR, "write file:%s data:%d correct size:%d",
			 filename, data, size);
	}
	set_fs(old_fs);
	filp_close(mfile, NULL);
...
uint32_t
oplus_cam_sensor_addr_is_byte_type(struct cam_sensor_ctrl_t *s_ctrl,
				   struct cam_camera_slave_info *slave_info)
{
	int rc = 0;
	uint32_t chipid = 0;
	...
	if (slave_info->sensor_id == 0x02d0 || slave_info->sensor_id == 0x25 ||
	    slave_info->sensor_id == 0x02e0 ||
		...
		rc = camera_io_dev_read(&(s_ctrl->io_master_info), gc02m0_high,
					&chipid_high,
					CAMERA_SENSOR_I2C_TYPE_BYTE,
					CAMERA_SENSOR_I2C_TYPE_BYTE);

		CAM_ERR(CAM_SENSOR,
			"gc02m0_high: 0x%x chipid_high id 0x%x:", gc02m0_high,
			chipid_high);

		rc = camera_io_dev_read(&(s_ctrl->io_master_info), gc02m0_low,
					&chipid_low,
					CAMERA_SENSOR_I2C_TYPE_BYTE,
					CAMERA_SENSOR_I2C_TYPE_BYTE);

		CAM_ERR(CAM_SENSOR,
			"gc02m0_low: 0x%x chipid_low id 0x%x:", gc02m0_low,
			chipid_low);

		chipid = ((chipid_high << 8) & 0xff00) | (chipid_low & 0x00ff);
	}
	CAM_WARN(CAM_SENSOR, "slave_info->sensor_id is %x",
		 slave_info->sensor_id);
	if (slave_info->sensor_id == IMX708_0_9_SOURCE_CHIPID ||
	    slave_info->sensor_id == IMX708_1_0_SOURCE_CHIPID) {
		rc = camera_io_dev_read(&(s_ctrl->io_master_info),
		...
		if ((sensor_version >> 8) >= SONY_SENSOR_MP1) {
			...
			CAM_WARN(CAM_SENSOR,
				 "imx708 slave_info.sensor_version: %d:",
				 s_ctrl->sensordata->slave_info.sensor_version);
		} else {
	...
	if (slave_info->sensor_id == 0x0689) {
		...
		if ((sensor_version >> 8) > SONY_SENSOR_MP1) {
			...
			CAM_WARN(CAM_SENSOR,
				 "imx689 slave_info.sensor_version: %d:",
				 s_ctrl->sensordata->slave_info.sensor_version);
		} else {
	...
	if (slave_info->sensor_id == 0x0766 ||
		...
		if ((sensor_version >> 8) == SONY_SENSOR_CUT0_9) {
			s_ctrl->sensordata->slave_info.sensor_version = 0;
			CAM_WARN(CAM_SENSOR,
				 "imx766 slave_info.sensor_version: %d:",
				 s_ctrl->sensordata->slave_info.sensor_version);
	...
	if (slave_info->sensor_id == IMX766_FIRST_SOURCE_CHIPID ||
		...
		rc = cam_cci_i2c_read(&ee_cci_client, IMX766_EEPROM_VCMID_ADDR,
				      &ee_vcmid, CAMERA_SENSOR_I2C_TYPE_WORD,
				      CAMERA_SENSOR_I2C_TYPE_BYTE);

		CAM_WARN(CAM_SENSOR,
			 "distinguish imx766 camera module, vcm id : 0x%x ",
			 ee_vcmid);
	...
	if (slave_info->sensor_id == 0x2b03) {
		...
		CAM_WARN(CAM_SENSOR, "ov02b chipid: 0x%x", chipid);
	}

	if ((slave_info->sensor_id == OV64B_FIRST_CHIPID) ||
	    (slave_info->sensor_id == OV64B_SECOND_CHIPID)) {
		memset(&ee_cci_client, 0x0,
		       sizeof(struct cam_sensor_cci_client));
		memcpy(&ee_cci_client, s_ctrl->io_master_info.cci_client,
		       sizeof(struct cam_sensor_cci_client));
		ee_cci_client.sid = OV64B_EEPROM_SID;
		rc = cam_cci_i2c_read(&ee_cci_client, OV64B_EEPROM_VCMID_ADDR,
				      &ee_vcmid, CAMERA_SENSOR_I2C_TYPE_WORD,
				      CAMERA_SENSOR_I2C_TYPE_BYTE);
		CAM_WARN(CAM_SENSOR,
			 "OV64B read eeprom vcm id is 0x%x:", ee_vcmid);
...
void oplus_cam_sensor_for_laser_ois(struct cam_sensor_ctrl_t *s_ctrl,
				    struct cam_camera_slave_info *slave_info)
{
...
int32_t oplus_cam_sensor_driver_cmd(struct cam_sensor_ctrl_t *s_ctrl, void *arg)
{
	...
	case CAM_OEM_IO_CMD: {
		...
		if (oem_ctl.num_bytes > 0) {
			cam_regs = (struct cam_oem_i2c_reg_array *)kzalloc(
				sizeof(struct cam_oem_i2c_reg_array) *
					oem_ctl.num_bytes,
				GFP_KERNEL);
			if (!cam_regs) {
				rc = -ENOMEM;
			...
			if (copy_from_user(
				    cam_regs,
				    u64_to_user_ptr(oem_ctl.cam_regs_ptr),
				    sizeof(struct cam_oem_i2c_reg_array) *
					    oem_ctl.num_bytes)) {
		...
		if (oem_ctl.slave_addr != 0) {
			oem_io_master_info.cci_client->sid =
				(oem_ctl.slave_addr >> 1);
		}
		...
		case CAM_OEM_CMD_READ_DEV: {
			...
			if (copy_to_user(u64_to_user_ptr(oem_ctl.cam_regs_ptr),
					 cam_regs,
					 sizeof(struct cam_oem_i2c_reg_array) *
						 oem_ctl.num_bytes)) {
				CAM_ERR(CAM_SENSOR,
		...
		case CAM_OEM_CMD_WRITE_DEV: {
			...
			for (; i < oem_ctl.num_bytes; i++) {
				CAM_DBG(CAM_SENSOR,
					"Get from OEM addr: 0x%x data: 0x%x ",
					cam_regs[i].reg_addr,
					cam_regs[i].reg_data);
			}
			...
			write_setting.reg_setting =
				(struct cam_sensor_i2c_reg_array *)cam_regs;

			rc = cam_cci_i2c_write_table(&oem_io_master_info,
						     &write_setting);
	...
	case CAM_OEM_GET_ID: {
		if (copy_to_user((void __user *)cmd->handle,
				 &s_ctrl->soc_info.index, sizeof(uint32_t))) {
	...
	case CAM_GET_DPC_DATA: {
		if (0x0471 != s_ctrl->sensordata->slave_info.sensor_id) {
			CAM_ERR(CAM_SENSOR,
				"(0x0471 != s_ctrl->sensordata->slave_info.sensor_id");
			rc = -EFAULT;
			return rc;
		}
		CAM_INFO(CAM_SENSOR,
...
void oplus_cam_sensor_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (slave_info->sensor_id == 0x0766 ||
		...
		rc = cam_cci_i2c_read(&ee_cci_client, flag_addr,
				      &sensor_version,
				      CAMERA_SENSOR_I2C_TYPE_WORD,
				      CAMERA_SENSOR_I2C_TYPE_BYTE);
		CAM_WARN(CAM_SENSOR, "QSC tool version is %x", sensor_version);
		if (sensor_version == 0x03) {
			...
			rc = camera_io_dev_write(&(s_ctrl->io_master_info),
						 &qsc_tool_write);
			CAM_WARN(CAM_SENSOR, "update the qsc tool version %d",
				 rc);
		}
	}
}
...
int sensor_gc5035_get_dpc_data(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (gc5035_dpcinfo[0] == 1) {
		...
		CAM_INFO(CAM_SENSOR,
			 ...
			 gc5035_dpcinfo[2]);
		CAM_INFO(CAM_SENSOR, "gc5035SpcWrite_setting totalDpcNum = %d",
			 totalDpcNum);
	}
...
int sensor_gc5035_write_dpc_data(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	for (i = 0; i < totalDpcNum * 4; i++) {
		...
		rc = camera_io_dev_write(&(s_ctrl->io_master_info),
					 &sensor_setting);
		for (j = 0; j < 4; j++) {
			rc = camera_io_dev_read(&(s_ctrl->io_master_info), 0xac,
						&temp_val[j],
						CAMERA_SENSOR_I2C_TYPE_BYTE,
						CAMERA_SENSOR_I2C_TYPE_BYTE);
			if (rc < 0) {
...
int sensor_start_thread(void *arg)
{
	...
	if (s_ctrl->sensor_power_state == CAM_SENSOR_POWER_OFF) {
		...
		if (rc < 0) {
			...
			CAM_INFO(CAM_SENSOR,
				 "sensor power up success sensor id 0x%x",
				 s_ctrl->sensordata->slave_info.sensor_id);
	...
	if (rc == 0) {
		...
		if (s_ctrl->sensor_initsetting_state ==
			...
			CAM_INFO(
				CAM_SENSOR,
				"start write init setting %x , sensorVersion %d ",
				s_ctrl->sensordata->slave_info.sensor_id,
				s_ctrl->sensordata->slave_info.sensor_version);
			if (s_ctrl->sensordata->slave_info.sensor_id == 0x689) {
				sensor_init_setting.reg_setting =
					sensor_init_settings.imx689_setting
						.reg_setting;
				sensor_init_setting.addr_type =
					CAMERA_SENSOR_I2C_TYPE_WORD;
				sensor_init_setting.data_type =
					CAMERA_SENSOR_I2C_TYPE_BYTE;
				sensor_init_setting.size =
					sensor_init_settings.imx689_setting.size;
				sensor_init_setting.delay =
					sensor_init_settings.imx689_setting
						.delay;
				...
				if (rc < 0) {
					CAM_ERR(CAM_SENSOR,
						"write setting failed!");
				} else {
					CAM_INFO(CAM_SENSOR,
						 "write setting1 success!");
					s_ctrl->sensor_initsetting_state =
						CAM_SENSOR_SETTING_WRITE_SUCCESS;
				}
				sensor_init_setting.reg_setting =
					sensor_init_settings.imx689_setting1
						.reg_setting;
				sensor_init_setting.addr_type =
					CAMERA_SENSOR_I2C_TYPE_WORD;
				sensor_init_setting.data_type =
				...
				CAM_ERR(CAM_SENSOR,
					"cant find oem advance init setting !");
			}
		} else {
			CAM_INFO(
				CAM_SENSOR,
				"sensor setting alread wrote or do not need to write it !");
		}
...
int cam_sensor_start(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->sensor_power_state == CAM_SENSOR_POWER_OFF) {
		s_ctrl->sensor_open_thread = kthread_run(
			sensor_start_thread, s_ctrl, s_ctrl->device_name);
		if (!s_ctrl->sensor_open_thread) {
			CAM_ERR(CAM_SENSOR,
				"create sensor start thread failed");
			rc = -1;
		} else {
			CAM_INFO(CAM_SENSOR,
				 "create sensor start thread success");
		}
	} else {
		CAM_INFO(CAM_SENSOR, "sensor have power up");
	}
...
int cam_sensor_stop(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->sensor_power_state == CAM_SENSOR_POWER_ON) {
		...
		if (rc < 0) {
			...
			CAM_INFO(CAM_SENSOR,
				 "sensor power down success sensor id 0x%x",
				 s_ctrl->sensordata->slave_info.sensor_id);
