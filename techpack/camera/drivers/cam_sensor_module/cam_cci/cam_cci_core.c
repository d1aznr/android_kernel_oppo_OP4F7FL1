...
int32_t cam_cci_read_packet(struct cam_cci_ctrl *cci_ctrl, uint32_t addr,
...
int32_t cam_cci_control_interface(void *control)
{
	...
	case CAMERA_CCI_READ:
		...
		//pack read data
		cam_cci_read_packet(&cci_ctrl_interface, pControl->addr,
				    pControl->data, pControl->count);
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		mutex_lock(&cci_dev->init_mutex);
		...
		if (rc < 0) {
			...
			exp_byte = ((cci_ctrl_interface.cfg.cci_i2c_read_cfg
					     .num_byte /
				     4) +
				    1);
			...
			for (i = 0; i < exp_byte; i++) {
				CAM_ERR(CAM_CCI,
					"songyt read byte=0x%x,index=%d",
					cci_ctrl_interface.cfg.cci_i2c_read_cfg
						.data[i],
					i);
			}
		}
	...
	case CAMERA_CCI_WRITE:
		...
		//pack write data
		cam_cci_write_packet(&cci_ctrl_interface, pControl->addr,
				     pControl->data, pControl->count);
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		mutex_lock(&cci_dev->init_mutex);
