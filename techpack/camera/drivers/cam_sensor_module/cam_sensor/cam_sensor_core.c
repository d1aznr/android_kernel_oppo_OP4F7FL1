...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (slave_info->sensor_id == 0x02d0 ||
	    ...
	    slave_info->sensor_id == 0xF708 ||
	    slave_info->sensor_id == 0xE708 ||
	    slave_info->sensor_id == 0x0689 ||
	    slave_info->sensor_id == 0x2b03 || slave_info->sensor_id == 0x25 ||
	    slave_info->sensor_id == 0x5035 ||
	    slave_info->sensor_id == 0x0766 ||
	    slave_info->sensor_id == 0x766F ||
	    slave_info->sensor_id == 0x766E ||
	    slave_info->sensor_id == 0x5a64 ||
	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		for (retry = 0; retry < 3; retry++) {
			...
			mutex_lock(&(s_ctrl->sensor_power_state_mutex));
			if (s_ctrl->sensor_power_state ==
			    CAM_SENSOR_POWER_OFF) {
				tmp = cam_sensor_power_up(s_ctrl);
				if (tmp < 0) {
					CAM_ERR(CAM_SENSOR,
						"sensor power up faild!");
					mutex_unlock(&(
			...
			if (rc < 0) {
				CAM_ERR(CAM_SENSOR,
					"power up failed, need to retry at %d",
					retry + 1);
				if (retry == 2)
					goto free_power_settings;
			} else {
