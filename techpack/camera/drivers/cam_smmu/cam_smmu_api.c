...
//gongqiang.xiao@Camera add for case:04457772
static int
cam_smmu_map_buffer_and_add_to_list(int idx, int ion_fd,
...
int cam_smmu_get_io_region_info(int32_t smmu_hdl, dma_addr_t *iova,
				size_t *len
#ifdef OPLUS_FEATURE_CAMERA_COMMON
				//gongqiang.xiao@Camera add for case:04457772
				,
...
#else
	CAM_DBG(CAM_SMMU,
		"idx=%d, dma_buf=%pK, dev=%pK, paddr=%pK, len=%u,dma_map_attrs=%d",
		idx, buf, (void *)iommu_cb_set.cb_info[idx].dev,
		(void *)*paddr_ptr, (unsigned int)*len_ptr,
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
	//gongqiang.xiao@Camera add for case:04457772
	CAM_DBG(CAM_SMMU, "region_id=%d, paddr=%pK, len=%d, dma_map_attrs=%d",
...
#ifndef OPLUS_FEATURE_CAMERA_COMMON
//gongqiang.xiao@Camera add for case:04457772
int cam_smmu_map_user_iova(int handle, int ion_fd, enum cam_smmu_map_dir dir,
			   dma_addr_t *paddr_ptr, size_t *len_ptr,
			   enum cam_smmu_region_id region_id)
#else
