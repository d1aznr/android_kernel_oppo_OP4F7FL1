...
static void
cam_req_mgr_mark_peer_isp_dev_to_skip(struct cam_req_mgr_core_link *link,
				      int32_t dev_hdl)
{
	int i = 0;
	...
	for (i = 0; i < link->num_devs; i++) {
		...
		if (dev != NULL) {
			if ((dev_hdl != dev->dev_hdl) &&
				...
				CAM_INFO(CAM_CRM,
					 "dev : %s to skip apply dev_hdl 0x%x",
...
static int
__cam_req_mgr_check_for_dual_trigger(struct cam_req_mgr_core_link *link)
{
	int rc = -EAGAIN;
 ...
 */
static void
__cam_req_mgr_reset_slot_sync_mode(struct cam_req_mgr_core_link *link,
				   uint64_t req_id)
{
	struct cam_req_mgr_req_queue *in_q = link->req.in_q;
	int slot_idx = -1;

	slot_idx = __cam_req_mgr_find_slot_for_req(in_q, req_id);

	if (slot_idx != -1) {
		CAM_DBG(CAM_CRM, "link 0x%x req %lld sync mode %d -> %d",
			link->link_hdl, req_id, in_q->slot[slot_idx].sync_mode,
			CAM_REQ_MGR_SYNC_MODE_NO_SYNC);
		in_q->slot[slot_idx].sync_mode = CAM_REQ_MGR_SYNC_MODE_NO_SYNC;
	} else {
		CAM_WARN(CAM_CRM, "Can't get slot on link 0x%x for req %lld",
	...
	struct cam_req_mgr_core_link *link)
{
	...
	if (!in_q) {
		CAM_DBG(CAM_CRM, "link->req.in_q is NULL");
		return;
	}
#endif
	...
	for (idx = 0; idx < in_q->num_slots; idx++) {
		...
		if (sync_link && (slot->req_id != -1) &&
			...
			__cam_req_mgr_reset_slot_sync_mode(sync_link,
							   slot->req_id);
#endif
	...
	for (i = 0; i < link->num_devs; i++) {
		...
		if (dev) {
			...
			CAM_DBG(CAM_REQ,
				...
				link->link_hdl, dev->dev_hdl, pd,
				apply_req.request_id, apply_req.re_apply);
#endif
			if (dev->ops && dev->ops->apply_req) {
				rc = dev->ops->apply_req(&apply_req);
				if (rc < 0) {
	...
	struct cam_req_mgr_slot *slot)
{
	 ...
	 */
	max_idx_diff = 1;
	if (sync_rd_slot->status == CRM_SLOT_STATUS_REQ_APPLIED)
		max_idx_diff += 1;
#endif
	if ((sync_link->req.in_q->slot[sync_slot_idx].status !=
	     CRM_SLOT_STATUS_REQ_APPLIED) &&
	...
	CAM_DBG(CAM_CRM,
		...
		sync_link->sof_timestamp, sof_timestamp_delta,
		master_slave_diff);
	...
	struct cam_req_mgr_error_notify *err_info)
{
	...
	if (link->dual_trigger) {
		...
		cam_req_mgr_mark_peer_isp_dev_to_skip(link, err_info->dev_hdl);
	}
#endif
