...
#define DSI_CTRL_MM_ERR(c, fmt, ...)                                           \
	do {                                                                   \
		DRM_DEV_ERROR(NULL, "[msm-dsi-error]: %s: " fmt,               \
			      c ? c->name : "inv", ##__VA_ARGS__);             \
		mm_fb_display_kevent_named(MM_FB_KEY_RATELIMIT_1H, fmt,        \
					   ##__VA_ARGS__);                     \
	} while (0)
#endif /* OPLUS_BUG_STABILITY */
...
static void dsi_ctrl_dma_cmd_wait_for_done(struct work_struct *work)
{
	...
	if (ret == 0 && !atomic_read(&dsi_ctrl->dma_irq_trig)) {
		...
		if (status & mask) {
			...
			DSI_CTRL_MM_ERR(
				dsi_ctrl,
				"DisplayDriverID@@416$$dma_tx done but irq not triggered\n");
#endif
			...
			if (dsi_ctrl->irq_info.irq_stat_refcount
				...
				mm_fb_display_kevent(
					"DisplayDriverID@@405$$",
					MM_FB_KEY_RATELIMIT_NONE,
					"dma_tx irq trigger fixup irq status=%x",
					status);
			}
			mm_fb_display_kevent(
				"DisplayDriverID@@413$$",
				MM_FB_KEY_RATELIMIT_1H,
				"dma_tx irq trigger err irq status=%x", status);
#endif
		} else {
			...
			DSI_CTRL_MM_ERR(
				dsi_ctrl,
				"DisplayDriverID@@401$$Command transfer failed\n");
#endif
		}
...
static int dsi_ctrl_enable_supplies(struct dsi_ctrl *dsi_ctrl, bool enable)
{
	...
	if (enable) {
		...
		if (rc < 0) {
			...
			DSI_CTRL_MM_ERR(
				dsi_ctrl,
				"DisplayDriverID@@406$$Power resource enable failed, rc=%d\n",
				rc);
#endif
			goto error;
		}
		...
		if (!dsi_ctrl->current_state.host_initialized) {
			...
			if (rc) {
				...
				DSI_CTRL_MM_ERR(
					dsi_ctrl,
					"DisplayDriverID@@406$$failed to enable host power regs\n");
#endif
				goto error_get_sync;
			}
		}
...
static void dsi_kickoff_msg_tx(struct dsi_ctrl *dsi_ctrl,
			       ...
			       struct dsi_ctrl_cmd_dma_info *cmd_mem, u32 flags)
{
	u32 hw_flags = 0;
	u32 line_no = 0x1;
	...
	dsi_ctrl->cmd_mode =
		(dsi_ctrl->host_config.panel_mode == DSI_OP_CMD_MODE);
					...
					u32 *flags)
{
	 ...
	 */
	if (((*flags & DSI_CTRL_CMD_FIFO_STORE) || *flags & DSI_CTRL_CMD_READ ||
	     *flags & DSI_CTRL_CMD_NON_EMBEDDED_MODE || msg->wait_ms ||
			  ...
			  const struct mipi_dsi_msg *msg, u32 *flags)
{
	...
	if (*flags & DSI_CTRL_CMD_NON_EMBEDDED_MODE) {
		...
		cmd_mem.en_broadcast =
			(*flags & DSI_CTRL_CMD_BROADCAST) ? true : false;
		cmd_mem.is_master =
			(*flags & DSI_CTRL_CMD_BROADCAST_MASTER) ? true : false;
			  ...
			  const struct mipi_dsi_msg *msg, u32 *flags)
{
	...
	if (*flags & DSI_CTRL_CMD_READ) {
		...
		if (rc <= 0) {
			...
			DSI_CTRL_MM_ERR(
				dsi_ctrl,
				"read message failed read length, rc=%d\n", rc);
#endif
		}
	} else {
		rc = dsi_message_tx(dsi_ctrl, msg, flags);
		if (rc) {
			...
			DSI_CTRL_MM_ERR(
				dsi_ctrl,
				"command msg transfer failed, rc = %d\n", rc);
#endif
		}
	}
	...
	if (enable) {
		if (dsi_hw_ops.mask_error_intr)
			dsi_hw_ops.mask_error_intr(
				&dsi_ctrl->hw, BIT(DSI_FIFO_OVERFLOW), true);
	} else {
		if (dsi_hw_ops.mask_error_intr && !dsi_ctrl->esd_check_underway)
			dsi_hw_ops.mask_error_intr(
				&dsi_ctrl->hw, BIT(DSI_FIFO_OVERFLOW), false);
	}
}
...
int dsi_ctrl_clear_slave_dma_status(struct dsi_ctrl *dsi_ctrl, u32 flags)
{
	...
	if (wait_for_done == 0)
		DSI_CTRL_ERR(dsi_ctrl, "DSI1 CMD_MODE_DMA_DONE failed\n");
...
int dsi_ctrl_cmd_tx_trigger(struct dsi_ctrl *dsi_ctrl, u32 flags)
{
	...
	if (timing && (dsi_ctrl->host_config.panel_mode == DSI_OP_VIDEO_MODE)) {
		v_total = timing->v_sync_width + timing->v_back_porch +
			  timing->v_front_porch + timing->v_active;
		fps = timing->refresh_rate;
	...
	if ((flags & DSI_CTRL_CMD_BROADCAST) &&
		...
		if ((dsi_ctrl->host_config.panel_mode == DSI_OP_VIDEO_MODE) &&
			...
			while (1) {
				local_irq_save(flag);
				cur_line = dsi_hw_ops.log_line_count(
					&dsi_ctrl->hw, dsi_ctrl->cmd_mode);
				if (cur_line <
