...
static bool _sde_rm_check_dsc(struct sde_rm *rm, struct sde_rm_rsvp *rsvp,
...
static void sde_rm_get_rsvp_nxt_hw_blks(struct sde_rm *rm,
					struct sde_rm_rsvp *rsvp, int type,
					struct sde_rm_hw_blk **blk_arr)
{
	...
	list_for_each_entry (blk, &rm->hw_blks[type], list) {
		if (blk->rsvp_nxt && blk->rsvp_nxt->seq == rsvp->seq)
			blk_arr[i++] = blk;
	}
}
		...
		bool test_only)
{
	...
	if (test_only && rsvp_nxt) {
		...
		if (rsvp_nxt) {
			...
			SDE_MM_ERROR(
				"[sde error] poll timeout cur %d nxt %d enc %d\n",
				rsvp_cur->seq, rsvp_nxt->seq, enc->base.id);
#endif
			SDE_EVT32(rsvp_cur->seq, rsvp_nxt->seq, enc->base.id,
				  SDE_EVTLOG_ERROR);
