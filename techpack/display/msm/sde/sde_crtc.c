...
static struct attribute *sde_crtc_dev_attrs[] = {
	&dev_attr_vsync_event.attr, &dev_attr_measured_fps.attr,
	&dev_attr_fps_periodicity_ms.attr, &dev_attr_retire_frame_event.attr,
	NULL
};
		...
		struct sde_hw_dim_layer *dim_layer)
{
	...
	for (i = 0; i < sde_crtc->num_mixers; i++) {
		...
		SDE_EVT32(DRMID(crtc), dim_layer->stage, cstate->lm_roi[i].x,
			  cstate->lm_roi[i].y, cstate->lm_roi[i].w,
			  cstate->lm_roi[i].h, dim_layer->rect.x,
			  dim_layer->rect.y, dim_layer->rect.w,
		...
		struct drm_crtc_state *state)
{
	...
	for_each_new_connector_in_state(state->state, conn, conn_state, i) {
		...
		SDE_DEBUG("conn_roi x:%u, y:%u, w:%u, h:%u\n", conn_roi.x,
			  conn_roi.y, conn_roi.w, conn_roi.h);
...
static void _sde_crtc_program_lm_output_roi(struct drm_crtc *crtc)
{
	...
	for (lm_idx = 0; lm_idx < sde_crtc->num_mixers; lm_idx++) {
		...
		SDE_EVT32(DRMID(crtc_state->base.crtc), lm_idx, lm_roi->x,
...
static void
_sde_drm_fb_sec_dir_trans(struct sde_kms_smmu_state_data *smmu_state,
			  uint32_t secure_level, struct sde_mdss_cfg *catalog,
			  bool old_valid_fb, int *ops,
			  struct drm_crtc_state *old_crtc_state)
{
	...
	/* secure camera usecase */
		...
		smmu_state->transition_type = PRE_COMMIT;
		*ops |= SDE_KMS_OPS_SECURE_STATE_CHANGE;
		if (old_secure_session == SDE_SECURE_VIDEO_SESSION)
			*ops |= (SDE_KMS_OPS_WAIT_FOR_TX_DONE |
				 SDE_KMS_OPS_CLEANUP_PLANE_FB);
	}
}
		...
		bool old_valid_fb)
{
	...
	case SDE_DRM_FB_SEC_DIR_TRANS:
		_sde_drm_fb_sec_dir_trans(smmu_state, secure_level, catalog,
					  old_valid_fb, &ops, old_crtc_state);
...
static int _sde_validate_hw_resources(struct sde_crtc *sde_crtc)
{
	...
	if (!sde_crtc->num_mixers || sde_crtc->num_mixers > CRTC_DUAL_MIXERS) {
					     ...
					     bool is_video_mode)
{
	...
	if ((old_secure_session == SDE_SECURE_CAMERA_SESSION &&
		...
		SDE_EVT32(DRMID(crtc), old_secure_session, new_secure_session,
			  SDE_EVTLOG_ERROR);
		ret = -EINVAL;
	}
...
static int _sde_crtc_populate_secure_session(struct drm_crtc_state *state,
					     int secure, int fb_ns, int fb_sec,
					     int fb_sec_dir)
{
	struct sde_crtc_state *cstate = to_sde_crtc_state(state);
	...
	if (secure == SDE_DRM_SEC_ONLY && fb_sec_dir && !fb_sec && !fb_ns)
		...
		SDE_ERROR(
			...
			cstate->secure_session, fb_sec_dir, fb_sec, fb_ns);
		return -EINVAL;
	}
		...
		int cnt)
{
	...
	rc = _sde_crtc_populate_secure_session(state, secure, fb_ns, fb_sec,
					       fb_sec_dir);
	if (rc)
		return rc;
