...
#define REQ_SEAMLESS_MODESET_LOCK(adj_mode, is_cmd_mode)                       \
	(msm_is_mode_seamless_dms(adj_mode) ||                                 \
	 (msm_is_mode_seamless_dyn_clk(adj_mode) && is_cmd_mode) ||            \
				      ...
				      struct drm_display_mode *adj_mode)
{
	...
	modeset_lock = REQ_SEAMLESS_MODESET_LOCK(adj_mode, is_cmd_mode);
	if (modeset_lock)
...
static void sde_encoder_get_qsync_fps_callback(struct drm_encoder *drm_enc,
					       u32 *qsync_fps, u32 vrr_fps)
{
	...
	*qsync_fps = disp_info->qsync_min_fps;
	...
	if (disp_info->has_qsync_min_fps_list) {
		if (!sde_enc->cur_master || !(sde_enc->disp_info.capabilities &
	...
	if ((is_support_panel(sde_enc->cur_master->connector) == true)) {
		if (sde_enc->num_phys_encs > 0) {
			if ((get_current_display_framerate(
			    ...
			    is_spread_backlight(g_new_bk_level)) {
				if (g_new_bk_level !=
