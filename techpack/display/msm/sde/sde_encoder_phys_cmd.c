...
ktime_t sde_encoder_get_last_vsync_ts_cmd(struct sde_encoder_phys *phys_enc)
{
	...
	te_timestamp =
		list_last_entry(&cmd_enc->te_timestamp_list,
		...
		bool recovery_events)
{
	...
	if (cmd_enc->pp_timeout_report_cnt == 1) {
		...
		mm_fb_display_kevent(
			"DisplayDriverID@@409$$", MM_FB_KEY_RATELIMIT_NONE,
			"ppdone timeout failed pp:%d kickoff timeout",
			phys_enc->hw_pp->idx - PINGPONG_0);
	...
	*extra_frame_trigger_time = 0;
	...
	if (mode && (qsync_mode == SDE_RM_QSYNC_CONTINUOUS_MODE)) {
		...
		if (phys_enc->parent_ops.get_qsync_fps)
			phys_enc->parent_ops.get_qsync_fps(phys_enc->parent,
							   &qsync_min_fps, 0);
...
#ifdef OPLUS_BUG_STABILITY
		mm_fb_display_kevent("DisplayDriverID@@418$$",
				     MM_FB_KEY_RATELIMIT_30MIN,
				     "wr_ptr_irq timeout failed switch_te:%d",
				     switch_te);
#endif
