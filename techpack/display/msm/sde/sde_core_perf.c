					     ...
					     struct sde_core_perf_params *perf)
{
	...
	if (!perf->bw_ctl[SDE_POWER_HANDLE_DBUS_ID_MNOC] &&
		...
		perf->max_per_pipe_ib[SDE_POWER_HANDLE_DBUS_ID_MNOC] =
			old_perf->max_per_pipe_ib[SDE_POWER_HANDLE_DBUS_ID_MNOC];
		...
		struct drm_crtc *crtc, u32 bus_id)
{
	...
	if (!kms->perf.sde_rsc_available)
		bus_ib_quota =
			max(SDE_POWER_HANDLE_ENABLE_BUS_IB_QUOTA, bus_ib_quota);
					     ...
					     int *update_llcc)
{
	...
	if ((params_changed && new->llcc_active && !old->llcc_active) ||
	    (!params_changed && !new->llcc_active && old->llcc_active)) {
		SDE_DEBUG("crtc=%d p=%d new_llcc=%d, old_llcc=%d\n",
			  crtc->base.id, params_changed, new->llcc_active,
			  old->llcc_active);
	...
	for (i = 0; i < SDE_POWER_HANDLE_DBUS_ID_MAX; i++) {
		...
		if ((params_changed &&
		    ...
		    (!params_changed &&
		     (new->max_per_pipe_ib[i] < old->max_per_pipe_ib[i]))) {
			SDE_DEBUG("crtc=%d p=%d new_ib=%llu,old_ib=%llu\n",
