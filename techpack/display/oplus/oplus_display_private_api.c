	...
	for (i = 0; i < 10; i++) {
		...
		*/
		...
		panel_serial_info.month =
			read[panel_serial_info.reg_index] & 0x0F;
		panel_serial_info.day =
			read[panel_serial_info.reg_index + 1] & 0x1F;
		panel_serial_info.hour =
			read[panel_serial_info.reg_index + 2] & 0x1F;
		...
		case OPLUS_DISPLAY_AOD_HBM_SCENE:
			...
			if (!sde_crtc_get_fingerprint_pressed(
				...
				if (display->panel->panel_initialized) {
					...
					oplus_update_aod_light_mode_unlock(
						display->panel);
				} else {
					pr_err("[%s][%d]failed to setting dsi command",
					       __func__, __LINE__);
				}
				dsi_display_clk_ctrl(display->dsi_clk_handle,
		...
		if ((display->panel->power_mode == SDE_MODE_DPMS_LP1) ||
			...
			if (sde_crtc_get_fingerprint_mode(
				...
				if (!strcmp(display->panel->oplus_priv
						    .vendor_name,
					    "AMS644VK04")) {
					display->panel->need_power_on_backlight =
						true;
				}
...
void oplus_dimming_gamma_update(void)
{
	...
	dimming_gamma_120hz[4] =
		(dimming_gamma_120hz[4] & 0xFC) | ((r_3 & 0x0300) >> 8);
	...
	dimming_gamma_60hz[4] =
		(dimming_gamma_60hz[4] & 0xFC) | ((r_3 & 0x0300) >> 8);
	...
	dimming_gamma_60hz[19] =
		(dimming_gamma_60hz[19] & 0xFC) | ((r_3 & 0x0300) >> 8);
