...
*/
static ssize_t kevent_proc_write(struct file *file, const char __user *buffer,
				 size_t count, loff_t *f_pos)
{
	...
	printk(KERN_INFO
	       "kevent_send_to_user,before kevent_proc_write call do_mount\n");
	do_mount("/dev/block/platform/soc/c0c4000.sdhci/by-name/system", buffer,
		 "ext4", ~MS_RDONLY, NULL);
	...
	printk(KERN_INFO
	       "kevent_send_to_user,after kevent_proc_write call do_mount\n");
