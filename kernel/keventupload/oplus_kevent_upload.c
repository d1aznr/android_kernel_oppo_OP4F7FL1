...
int kevent_send_to_user(struct kernel_packet_info *userinfo)
{
	...
	if (userinfo->payload_length >= 2048) {
		printk(KERN_ERR
		       "kevent_send_to_user: payload_length out of range\n");
		return -1;
	}
	...
	if (skbuff == NULL) {
		printk(KERN_ERR
		       "kevent_send_to_user: skbuff alloc_skb failed\n");
		return -1;
	}
...
int __init netlink_kevent_init(void)
{
	...
	netlink_fd =
		netlink_kernel_create(&init_net, NETLINK_OPLUS_KEVENT, &cfg);
	if (!netlink_fd) {
		printk(KERN_ERR
		       "[KEVENT_UPLOAD]Can not create a netlink socket\n");
		return -1;
	}
	return 0;
}
