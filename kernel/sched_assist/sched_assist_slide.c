...
static u64 calc_freq_ux_load(struct task_struct *p, u64 wallclock)
{
	...
	if (task_is_sf_group(p)) {
		sched_assist_systrace(timeline, "timeline_true_%d",
				      task_cpu(p));
		sched_assist_systrace(freq_exec_load, "freq_exec_true_%d",
				      task_cpu(p));
		sched_assist_systrace(freq_ravg_load, "freq_ravg_true_%d",
				      task_cpu(p));
	}
	return max(freq_exec_load, freq_ravg_load);
}

void _slide_find_start_cpu(struct root_domain *rd, struct task_struct *p,
			   int *start_cpu)
{
...
void slide_calc_boost_load(struct rq *rq, unsigned int *flag, int cpu)
{
	...
	if (slide_scene()) {
		if (rq->curr &&
			...
			ux_task_load[cpu] =
				calc_freq_ux_load(rq->curr, wallclock);
...
int sched_frame_rate_handler(struct ctl_table *table, int write,
...
int sysctl_sched_slide_boost_handler(struct ctl_table *table, int write,
...
int sysctl_sched_assist_input_boost_ctrl_handler(struct ctl_table *table,
