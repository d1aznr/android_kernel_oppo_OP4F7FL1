...
#endif
#if defined(CONFIG_OPLUS_FEATURE_INPUT_BOOST) &&                               \
	defined(CONFIG_OPLUS_FEATURE_IM)
...
void update_ux_sched_cputopo(void)
{
	...
	for_each_possible_cpu (cpu) {
		...
		if (cpu_cap == prev_cap) {
			for (i = 0; i < ux_sched_cputopo.cls_nr; ++i) {
				if (cpu_cap ==
				    ux_sched_cputopo.sched_cls[i].capacity) {
					cpumask_set_cpu(cpu,
							&ux_sched_cputopo
								 .sched_cls[i]
								 .cpus);
		...
		if (!cls_nr) {
			...
			if (insert_idx == ux_sched_cputopo.cls_nr) {
				ux_sched_cputopo.sched_cls[insert_idx] =
					sched_cls;
			} else {
				for (; cls_nr > insert_idx; cls_nr--) {
					ux_sched_cputopo.sched_cls[cls_nr] =
						ux_sched_cputopo
							.sched_cls[cls_nr - 1];
				}
				ux_sched_cputopo.sched_cls[insert_idx] =
	...
	for (i = 0; i < ux_sched_cputopo.cls_nr; i++)
		ux_debug(
			"update ux sched cpu topology [cls_nr:%d cpus:%*pbl cap:%lu]",
			i, cpumask_pr_args(&ux_sched_cputopo.sched_cls[i].cpus),
			ux_sched_cputopo.sched_cls[i].capacity);
}
...
bool test_task_identify_ux(struct task_struct *task, int id_type_ux)
{
	if (id_type_ux == SA_TYPE_ID_CAMERA_PROVIDER) {
		...
		if ((task->ux_state & SA_TYPE_LISTPICK) &&
		    (grp_leader->ux_state & SA_TYPE_ID_CAMERA_PROVIDER))
			return true;
		return (task->ux_state & SA_TYPE_ID_CAMERA_PROVIDER) &&
		       (sysctl_sched_assist_scene & SA_CAMERA);
	} else if (id_type_ux == SA_TYPE_ID_ALLOCATOR_SER) {
		if (task && (task->ux_state & SA_TYPE_ID_ALLOCATOR_SER) &&
		    (sysctl_sched_assist_scene & SA_CAMERA))
			return true;
	}
...
inline bool test_task_ux(struct task_struct *task)
{
	...
	if (task->ux_state & (SA_TYPE_HEAVY | SA_TYPE_LIGHT | SA_TYPE_ANIMATOR |
			      SA_TYPE_LISTPICK))
		return true;
...
int task_lb_sched_type(struct task_struct *tsk)
{
	...
	if (test_task_ux(tsk))
		...
		return SA_TOP;
	else if (cgroup_type == SA_CGROUP_FOREGROUND ||
		 cgroup_type == SA_CGROUP_DEFAULT)
		return SA_FG;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	load = clamp(load, uclamp_eff_value(tsk, UCLAMP_MIN),
...
#endif
void dec_task_lb(struct task_struct *tsk, struct rq *rq, int high_load,
		 int task_type)
{
	int cpu = cpu_of(rq);
...
void inc_task_lb(struct task_struct *tsk, struct rq *rq, int high_load,
		 int task_type)
{
	int cpu = cpu_of(rq);
...
const int sched_prio_to_weight_bg[MAX_ADJ_NICE] = {
	716, 601, 510, 438, 380,
};
const int sched_prio_to_wmult_bg[MAX_ADJ_NICE] = {
...
static struct load_weight sa_new_weight(struct sched_entity *se)
{
	...
	if ((task->static_prio < DEFAULT_PRIO) ||
	    (task->static_prio > MAX_ADJ_PRIO) || (task_util(task) < 51))
		return se->load;
...
u64 sa_calc_delta(struct sched_entity *se, u64 delta_exec, unsigned long weight,
		  struct load_weight *lw, bool calc_fair)
{
	u64 fact = 0;
	u32 inv_weight = 0;
	...
	if (calc_fair && (sa_lw.weight == lw->weight) &&
...
void printf_cpu_spread_nr_info(unsigned int cpu, char *nr_info, int info_size)
{
	...
	if (cpu >= 0 && cpu < NR_CPUS) {
		...
		scnprintf(nr_info, info_size, "(%d:%d:%d:%d:%d)",
			  cpu_rq(cpu)->nr_running, rq_ux_nr, rq_top_nr,
			  rq_fg_nr, rq_bg_nr);
	}
}

static void find_spread_lowest_nr_cpu(struct task_struct *p,
				      cpumask_t *visit_cpus_t, int sched_type,
				      int prev_cpu, int skip_cpu,
				      ...
				      int *lowest_nr_cpu)
{
	...
	for_each_cpu (i, visit_cpus_t) {
		...
		ux_nr = per_cpu(task_lb_count, i).ux_high +
			per_cpu(task_lb_count, i).ux_low;
		top_nr = per_cpu(task_lb_count, i).top_high +
			 per_cpu(task_lb_count, i).top_low;
		fg_nr = per_cpu(task_lb_count, i).foreground_high +
			per_cpu(task_lb_count, i).foreground_low;
		bg_nr = per_cpu(task_lb_count, i).background_high +
			per_cpu(task_lb_count, i).background_low;
...
void sched_assist_spread_tasks(struct task_struct *p,
			       cpumask_t new_allowed_cpus, int start_cpu,
			       int skip_cpu, cpumask_t *cpus, bool strict)
{
	...
	for (cluster = 0; cluster < cputopo.cls_nr; cluster++) {
		if (start_cpu ==
		    cpumask_first(&cputopo.sched_cls[cluster].cpus)) {
			order_index = cluster;
			break;
		}
	}
	...
	for (cluster = 0; cluster < cputopo.cls_nr; cluster++) {
		...
		find_spread_lowest_nr_cpu(p, &visit_cpus, sched_type, prev_cpu,
					  skip_cpu, &lowest_nr, &lowest_nr_load,
					  &lowest_nr_cpu);
...
#else
void sched_assist_spread_tasks(struct task_struct *p,
			       cpumask_t new_allowed_cpus, int order_index,
...
inline bool test_list_pick_ux(struct task_struct *task)
{
	...
	if (is_webview(task))
		return true;
#endif
	return (task->ux_state & SA_TYPE_LISTPICK) ||
...
void pick_ux_thread(struct rq *rq, struct task_struct **p,
	...
	if (!list_empty(&rq->ux_thread_list)) {
		...
		if (key_task) {
			...
			if (key_se && (rq->clock >= key_task->enqueue_time) &&
			    rq->clock - key_task->enqueue_time >=
				    ((u64)ux_min_sched_delay_granularity *
				     S2NS_T)) {
				*p = key_task;
				*se = key_se;
			}
...
#define inherit_ux_mask_of(type)                                               \
	((u64)(INHERIT_UX_MASK_BASE) << (inherit_ux_offset_of(type)))
#define inherit_ux_get_bits(value, type)                                       \
	((value & inherit_ux_mask_of(type)) >> inherit_ux_offset_of(type))
...
static void __inherit_ux_enqueue(struct task_struct *task, int type, int depth)
{
	...
	task->ux_depth =
		task->ux_depth > depth + 1 ? task->ux_depth : depth + 1;
				   ...
				   unsigned int type)
{
	...
	if (test_task_identify_ux(task, SA_TYPE_ID_CAMERA_PROVIDER)) {
		...
		boost = ((scale_demand(task->wts.sum) >= THRESHOLD_BOOST) ||
			 (task_util(task) >= THRESHOLD_BOOST)) ?
				true :
				false;
#else /* KERNEL-5-4 */
	...
	for_each_cpu (i, cpu_active_mask) {
		...
		curr_ux =
			test_task_ux(curr) ||
			test_task_identify_ux(curr, SA_TYPE_ID_CAMERA_PROVIDER);
		if (curr_ux)
			continue;

		capacity_curr = capacity_curr_of(i);
		if ((curr->prio > lowest_prio) ||
		    (boost && (capacity_curr > lowest_prio_max_cap))) {
...
void sched_assist_target_comm(struct task_struct *task)
{
	...
	if (strstr(grp_leader->comm, LAUNCHER_THREAD_NAME) &&
	    strstr(task->comm, MALI_THREAD_NAME)) {
			      ...
			      struct task_struct *curr_task)
{
	...
	if ((wake_task->ux_state & SA_TYPE_ANIMATOR) &&
	    !(curr_task->ux_state & SA_TYPE_ANIMATOR))
		return true;

	/* heavy type can be preemt by other type */
	if (wake_ux && (curr_task->ux_state & SA_TYPE_HEAVY)) {
		bool preempt = !(wake_task->ux_state & SA_TYPE_HEAVY) &&
			       !test_task_identify_ux(
				       wake_task, SA_TYPE_ID_CAMERA_PROVIDER);
		return preempt;
	}
...
static inline bool is_ux_task_prefer_cpu(struct task_struct *task, int cpu)
{
	...
	if (test_sched_assist_ux_type(task, SA_TYPE_HEAVY)) {
		return capacity_orig_of(cpu) >=
		       ux_cputopo.sched_cls[cls_nr].capacity;
	}
...
#define SILVER_NR_6 (6)
	...
	if (!(sysctl_sched_assist_scene & SA_LAUNCH) ||
		...
		if (cpu_rq(cpu)->curr &&
		    (test_sched_assist_ux_type(cpu_rq(cpu)->curr,
					       SA_TYPE_ANIMATOR) ||
...
void set_inherit_ux(struct task_struct *task, int type, int depth,
		    int inherit_val)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
	unsigned long flags;
	...
	sched_assist_systrace_pid(task->tgid, task->ux_state, "ux_state %d",
				  task->pid);
		      ...
		      struct task_struct *ux_task, int reset_type)
{
	...
	inherit_task->ux_state =
		(inherit_task->ux_state & ~SCHED_ASSIST_UX_MASK) |
		reset_inherit;

	sched_assist_systrace_pid(inherit_task->tgid, inherit_task->ux_state,
...
#else
	if ((scale_demand(p->ravg.sum) >= sysctl_boost_task_threshold ||
	     READ_ONCE(p->se.avg.util_avg) > sysctl_boost_task_threshold) &&
	    cpu < num_mincpu)
#endif
		return true;
#endif
				   ...
				   int stats_type)
{
	...
	while (i > 0) {
		tsk->oplus_task_info.sa_info[stats_type][i] =
			tsk->oplus_task_info.sa_info[stats_type][i - 1];
		i--;
	}
...
void try_to_mark_task_type(struct task_struct *tsk, int type)
{
	...
	switch (type) {
	case TASK_SMALL:
		if ((tsk->oplus_task_info
			     .sa_info[TST_EXEC][TASK_INFO_SAMPLE - 1] != 0) &&
		    (tsk->oplus_task_info
			     .sa_info[TST_SLEEP][TASK_INFO_SAMPLE - 1] != 0)) {
			sum_running = task_info_sum(tsk, TST_EXEC);
			while (i < TASK_INFO_SAMPLE) {
				if ((tsk->oplus_task_info.sa_info[TST_EXEC][i] <
				     RUNNING_MIN_THREASH) &&
				    ((tsk->oplus_task_info.sa_info[TST_EXEC][i] *
				      5) < tsk->oplus_task_info
						   .sa_info[TST_SLEEP][i])) {
					fit_small++;
				}
...
void sched_assist_update_record(struct task_struct *p, u64 delta_ns,
				int stats_type)
{
...
void sched_assist_im_systrace_c(struct task_struct *tsk, int tst_type)
{
	...
	preempt_disable();
	if (tst_type != -1) {
		event_trace_printk(mark_addr,
				   "C|10001|short_run_target_%d|%d\n", tsk->pid,
				   tsk->oplus_task_info.im_small ? 1 : 0);
	} else if (tst_type == -1) {
		event_trace_printk(mark_addr,
				   "C|10001|short_run_target_buddy_%d|%d\n",
				   tsk->pid, 1);
		event_trace_printk(mark_addr,
				   "C|10001|short_run_target_buddy_%d|%d\n",
				   tsk->pid, 0);
				   ...
				   size_t count, loff_t *ppos)
{
	...
	if (ux_state == SA_OPT_CLEAR) { /* clear all ux type but animator */
		task->ux_state &=
			~(SA_TYPE_LISTPICK | SA_TYPE_HEAVY | SA_TYPE_LIGHT);
	} else if (ux_state &
		   SA_OPT_SET) { /* set target ux type and clear set opt */
				  ...
				  size_t count, loff_t *ppos)
{
	...
	len = snprintf(
		...
		test_inherit_ux(task, INHERIT_UX_BINDER), grp_id,
		task->ux_im_flag);
#else
	len = snprintf(
		buffer, sizeof(buffer),
				  ...
				  size_t count, loff_t *ppos)
{
	...
	if (cnt != OPT_STR_MAX) {
		if (cnt == (OPT_STR_MAX - 1) &&
			...
			err = kstrtoint(strstrip(opt_str[OPT_STR_PID]), 10,
					&pid);
			if (err)
				return err;
...
static int __init oplus_sched_assist_init(void)
{
	...
	d_sched_assist =
		proc_mkdir(OPLUS_SCHEDASSIST_PROC_DIR, d_oplus_scheduler);
	if (!d_sched_assist) {
