			...
			enum pid_type type, int from_ancestor_ns)
{
	...
	if (1) {
		...
		if ((sig == SIGHUP || sig == 33 || sig == SIGKILL ||
		     sig == SIGSTOP || sig == SIGABRT || sig == SIGTERM ||
		     sig == SIGCONT) &&
		    is_key_process(t)) {
			//#ifdef OPLUS_BUG_STABILITY
			//dump_stack();
			...
			printk("Some other process %d:%s want to send sig:%d to pid:%d tgid:%d comm:%s\n",
			       current->pid, current->comm, sig, t->pid,
			       t->tgid, t->comm);
		}
...
#if defined(VENDOR_EDIT) && defined(CONFIG_ELSA_STUB)
	if (sig == SIGKILL && (freezing(t) || frozen(t)) &&
	    cgroup_freezing(t)) {
		pe_data.pid = task_pid_nr(t);
		pe_data.uid = task_uid(t);
		pe_data.reason = sig;
		process_event_notifier_call_chain_atomic(
			PROCESS_EVENT_SIGNAL_FROZEN, &pe_data);
	}
#endif
			...
			enum pid_type type)
{
	...
	if (is_frozen_tg(p) /*signal receiver thread group is frozen?*/
		...
		sig == SIGQUIT)) {
		if (hans_report(SIGNAL, task_tgid_nr(current),
				task_uid(current).val, task_tgid_nr(p),
				task_uid(p).val, "signal", -1) == HANS_ERROR) {
			printk(KERN_ERR
