...
int get_cpuset_cgrp_idx_by_name(const char *cg_name)
{
	...
	if (dbg_enable)
		pr_err("%s: cgrp_name=%s return idx=%d", __func__, cg_name,
		       idx);
...
static int hash_show(struct seq_file *m, void *v)
{
	...
	hash_for_each (cg_hash_table, bkt, cg, hash) {
		seq_printf(m, "cgid=\"%d\",name=\"%s\",idx=\"%d\"\n", cg->cgid,
			   cg->name, get_cpuset_cgrp_idx(cg->cgid));
	}
	...
	if (cset->mg_src_cgrp && cset->mg_dst_cgrp) {
		...
		if (dbg_enable)
			pr_err("%s: src_cg = %s, id = %d, dst_cg = %s, id = %d, task = %s, pid=%d",
			       __func__, cg_src, cset->mg_src_cgrp->id, cg_dst,
			       cset->mg_dst_cgrp->id, task->comm, task->pid);
		...
		if (cset->mg_src_cgrp == NULL)
			...
			cgroup_name(cset->mg_src_cgrp, cg_src,
		...
		if (cset->mg_dst_cgrp == NULL)
			...
			cgroup_name(cset->mg_dst_cgrp, cg_dst,
				    MAX_CGROUP_TYPE_NAMELEN);
	...
	if (!(task->flags & PF_EXITING)) {
		...
		for (i = 0; i < 1 /*UID_PERF_EVENTS*/; ++i) {
			...
			if (task->uid_pevents[i]) {
				...
				val = perf_event_read_value(
					task->uid_pevents[i], &enabled,
					&running);
				task->uid_group[src_idx] +=
					(val -
					 task->uid_group_prev_counts[src_idx]);
...
int cpuset_get_cgrp_idx(struct task_struct *task)
{
	...
	cset = task_css_set(task);
	if (cset->subsys[cpuset_cgrp_id] &&
	    cset->subsys[cpuset_cgrp_id]->cgroup)
	...
	if (idx < 0)
		if (dbg_enable)
			pr_err("%s: cset->subsys[cpuset_cgrp_id]->cgroup->id = %d, array idx = %d",
			       __func__, cgrp_id, idx);
...
int cpuset_get_cgrp_idx_locked(struct task_struct *task)
{
	...
	if (idx < 0)
		if (dbg_enable)
			pr_err("%s: cset->subsys->cgroup->id = %d, array idx = %d",
			       __func__, cgrp_id, idx);
