...
{
#if defined(OPLUS_FEATURE_POWER_CPUFREQ) &&                                    \
	defined(OPLUS_FEATURE_POWER_EFFICIENCY)
...
void migrate_ed_task(struct task_struct *p, u64 wallclock, struct rq *src_rq,
		     struct rq *dest_rq)
{
...
account_busy_for_task_demand(struct rq *rq, struct task_struct *p, int event)
{
	...
	defined(OPLUS_FEATURE_POWER_EFFICIENCY)
	if (event == TASK_WAKE ||
	    ((!SCHED_ACCOUNT_WAIT_TIME || uclamp_discount_wait_time(p)) &&
			 ...
			 u32 runtime, int samples, int event)
{
	...
	if (p == rq->curr && p == current && event != PUT_PREV_TASK &&
	    p->sched_class == &fair_sched_class && p->ld_flag)
