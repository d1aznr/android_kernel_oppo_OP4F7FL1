...
static u64 window_policy_read(struct cgroup_subsys_state *css,
			      struct cftype *cft)
{
	struct schedtune *st = css_st(css);
...
static int window_policy_write(struct cgroup_subsys_state *css,
			       struct cftype *cft, u64 window_policy)
{
	struct schedtune *st = css_st(css);
...
#define PE_FUNC(NAME)                                                          \
	...
	{                                                                      \
		...
		return st->NAME;                                               \
                                                                               ...
                                                                               \
	static int NAME##_write(struct cgroup_subsys_state *css,               \
				struct cftype *cft, u64 NAME)                  \
	{                                                                      \
		struct schedtune *st = css_st(css);                            \
                                                                               \
		st->NAME = !!NAME;                                             \
