...
struct sugov_policy {
	...
	defined(CONFIG_OPLUS_FEATURE_INPUT_BOOST_V4)
	unsigned int flags;
#endif
};
				       ...
				       unsigned int freq)
{
	...
	for (i = 0; i < tunables->ntarget_loads - 1 &&
		    freq >= tunables->target_loads[i + 1];
	     i += 2)
		;
			     ...
			     unsigned int next_freq, unsigned int flags)
{
	...
	if (!skip_hispeed_delay && next_freq > sg_policy->next_freq &&
		...
		if (delta_ns < NSEC_PER_USEC * delay) {
			trace_sugov_time_limit(
				cpumask_first(sg_policy->policy->cpus),
				  ...
				  unsigned long util, unsigned long max)
{
	...
	trace_sugov_next_freq_tl(policy->cpu, util, max, freq, prev_laf,
				 prev_freq);
#else
...
static ssize_t target_loads_show(struct gov_attr_set *attr_set, char *buf)
{
	...
	for (i = 0; i < tunables->ntarget_loads; i++)
		ret += snprintf(buf + ret, sizeof(buf), "%u%s",
...
static struct attribute *sugov_attributes[] = { &up_rate_limit_us.attr,
