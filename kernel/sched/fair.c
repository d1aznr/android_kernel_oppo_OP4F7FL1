...
extern void update_jank_trace_info(struct task_struct *tsk, int trace_type,
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_CPU_JANKINFO)
		jankinfo_tasktrack_update_time(curtask, TRACE_RUNNING,
					       delta_exec);
#endif
...
#ifdef OPLUS_FEATURE_HEALTHINFO
#ifdef CONFIG_OPLUS_JANK_INFO
		update_jank_trace_info(curtask, JANK_TRACE_RUNNING,
				       cpu_of(rq_of(cfs_rq)), delta_exec);
#endif
#endif /* OPLUS_FEATURE_HEALTHINFO */
	}
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_CPU_JANKINFO)
			jankinfo_tasktrack_update_time(tsk, TRACE_SLEEPING,
						       delta);
#endif
...
#ifdef OPLUS_FEATURE_HEALTHINFO
#ifdef CONFIG_OPLUS_JANK_INFO
			update_jank_trace_info(tsk, JANK_TRACE_SSTATE, 0,
					       delta);
#endif
#endif /* OPLUS_FEATURE_HEALTHINFO */
#ifdef CONFIG_OPLUS_FEATURE_AUDIO_OPT
...
#if defined(OPLUS_FEATURE_TASK_CPUSTATS) && defined(CONFIG_OPLUS_SCHED)
		if (tsk->in_iowait)
			update_task_sched_info(tsk, delta, task_sched_info_IO,
					       task_cpu(tsk));
		else
			update_task_sched_info(tsk, delta, task_sched_info_D,
					       task_cpu(tsk));
#endif /* defined(OPLUS_FEATURE_TASK_CPUSTATS) && defined(CONFIG_OPLUS_SCHED) */
		...
		if (tsk) {
			if (tsk->in_iowait) {
				...
				jankinfo_tasktrack_update_time(
					tsk, TRACE_DISKSLEEP_INIOWAIT, delta);
#endif
#ifdef OPLUS_FEATURE_HEALTHINFO
// Add for get iowait
#ifdef CONFIG_OPLUS_HEALTHINFO
...
#ifdef CONFIG_OPLUS_HEALTHINFO
			if (!tsk->in_iowait) {
				ohm_schedstats_record(OHM_SCHED_DSTATE, tsk,
						      (delta >> 20));
			}
#endif
#endif /* OPLUS_FEATURE_HEALTHINFO */
#ifdef OPLUS_FEATURE_HEALTHINFO
#ifdef CONFIG_OPLuS_JANK_INFO
			update_jank_trace_info(tsk, JANK_TRACE_DSTATE, 0,
					       delta);
#endif
#endif /* OPLUS_FEATURE_HEALTHINFO */
#ifdef CONFIG_OPLUS_FEATURE_AUDIO_OPT
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_CPU_JANKINFO)
			jankinfo_tasktrack_update_time(tsk, TRACE_DISKSLEEP,
						       delta);
#endif
...
static int get_start_cpu(struct task_struct *p)
{
	...
	if (sched_assist_scene(SA_SLIDE) && is_heavy_ux_task(p) &&
		...
		start_cpu = rd->mid_cap_orig_cpu == -1 ? rd->max_cap_orig_cpu :
							 rd->mid_cap_orig_cpu;
	}

	if (sysctl_prefer_silver && sysctl_sched_assist_enabled &&
	    !is_heavy_ux_task(p) && prefer_silver_check_task_util(p)) {
	...
	trace_sched_cpu_sel(p, task_boost, task_skip_min, boosted,
			    task_boost_policy(p), task_util(p),
			    cpu_util(task_cpu(p)), test_task_ux(p),
			    task_demand_fits(p, rd->min_cap_orig_cpu),
			    sysctl_prefer_silver, start_cpu);
					...
					struct find_best_target_env *fbt_env)
{
	...
	if (fbt_env->strict_max || p->in_iowait ||
	    (sysctl_cpu_multi_thread && !is_heavy_load_task(p)))
#else
	if (fbt_env->strict_max || p->in_iowait)
#endif
	...
	sched_assist_spread_tasks(p, p->cpus_allowed, fbt_env->start_cpu,
				  fbt_env->skip_cpu, cpus, strict);
	...
	do {
		for_each_cpu_and (i, &new_allowed_cpus, sched_group_span(sg)) {
			...
			if (sysctl_prefer_silver &&
				...
				if (prefer_silver_check_freq(start_cpu) &&
...
#ifdef OPLUS_FEATURE_SCHED_ASSIST
			else if (spare_wake_cap == most_spare_wake_cap &&
				 sysctl_cpu_multi_thread &&
				 !is_heavy_load_task(p) &&
				 cpu_rq(i)->nr_running <
					 cpu_rq(most_spare_cap_cpu)->nr_running) {
				most_spare_cap_cpu = i;
			}
#endif
...
#ifdef OPLUS_FEATURE_SCHED_ASSIST
		if (sysctl_prefer_silver && sysctl_sched_assist_enabled &&
		    !test_task_ux(p) && is_max_capacity_cpu(env->dst_cpu)) {
			if (prefer_silver_check_freq(env->src_cpu) &&
			    (prefer_silver_check_task_util(p) ||
			     prefer_silver_check_cpu_util(env->src_cpu))) {
				skip_big_cluster = true;

				trace_sched_cpu_skip(
					p, sysctl_prefer_silver,
					test_task_ux(p),
					prefer_silver_check_freq(env->src_cpu),
					prefer_silver_check_task_util(p),
					prefer_silver_check_cpu_util(
						env->src_cpu),
					skip_big_cluster);
				goto next;
			}
...
bool prefer_silver_check_task_util(struct task_struct *p)
{
	...
	if (task_util(p) < reciprocal_divide(thresh_load, spc_rdiv) ||
	    scale_demand(p->ravg.sum) <
		    reciprocal_divide(thresh_load, spc_rdiv))
		return true;
...
void check_for_migration(struct rq *rq, struct task_struct *p)
{
	...
	if (rtg_target && (capacity_orig_of(prev_cpu) <
...
#ifdef OPLUS_FEATURE_SCHED_ASSIST
	if (rq->misfit_task_load ||
	    (sched_assist_scene(SA_SLIDE) && is_heavy_ux_task(p) &&
	     ux_task_misfit(p, prev_cpu))) {
#else
	if (rq->misfit_task_load) {
#endif /* OPLUS_FEATURE_SCHED_ASSIST */
#endif /* CONFIG_OPLUS_FEATURE_INPUT_BOOST_V4 */
