...
static struct worker *create_worker(struct worker_pool *pool)
{
	...
	if (pool->cpu >= 0)
		snprintf(id_buf, sizeof(id_buf), "%d:%d%s", pool->cpu, id,
			 pool->attrs->ux_state > 0 ? "X" :
			 pool->attrs->nice < 0	   ? "H" :
						     "");
	else
		snprintf(id_buf, sizeof(id_buf), "%s%d:%d",
			 pool->attrs->ux_state > 0 ? "X" : "u", pool->id, id);
#else
	if (pool->cpu >= 0)
		snprintf(id_buf, sizeof(id_buf), "%d:%d%s", pool->cpu, id,
			 pool->attrs->nice < 0  ? "H" : "");
...
{
#ifdef OPLUS_FEATURE_SCHED_ASSIST
	int highpri = (wq->flags & WQ_UX)      ? UX_WORKER_POOL_INDEX :
		      (wq->flags & WQ_HIGHPRI) ? 1 :
						 0;
#else
...
#ifdef OPLUS_FEATURE_SCHED_ASSIST
	pr_cont(" flags=0x%x nice=%d ux=%d", pool->flags, pool->attrs->nice,
		pool->attrs->ux_state);
#else
