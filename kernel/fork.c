...
#if defined(OPLUS_FEATURE_MEMLEAK_DETECT) && defined(CONFIG_ION) &&            \
	defined(CONFIG_DUMP_TASKS_MEM)
					...
					int node)
{
	...
	memset(p->uid_pevents, 0,
	       sizeof(struct perf_event *) * UID_PERF_EVENTS);
	...
	memset(p->uid_group_snapshot_prev_counts, 0,
	       sizeof(long long) * UID_GROUP_SIZE);
#endif
