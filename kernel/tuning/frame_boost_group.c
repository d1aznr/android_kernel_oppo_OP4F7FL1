...
static inline u64 scale_exec_time(u64 delta, struct rq *rq)
{
	...
	unsigned int curr_freq =
		(curr_cap * (u64)rq->cluster->max_possible_freq) >>
		SCHED_CAPACITY_SHIFT;

	task_exec_scale = DIV64_U64_ROUNDUP(
		curr_freq * arch_scale_cpu_capacity(NULL, cpu),
		rq->cluster->max_possible_freq);
	trace_task_exec_scale(cpu, task_exec_scale, curr_freq,
...
static void add_to_group_task_time(struct frame_boost_group *grp, struct rq *rq,
...
static void add_to_group_time(struct frame_boost_group *grp, struct rq *rq,
			      u64 wallclock)
{
	u64 delta_exec, delta_scale;
	...
	trace_add_group_delta(wallclock, mark_start, window_start, delta_exec,
			      delta_scale, rq->task_exec_scale);
}

static inline void add_to_group_demand(struct frame_boost_group *grp,
...
void update_group_demand(struct task_struct *p, struct rq *rq, int event,
			 u64 wallclock)
{
	struct frame_boost_group *grp;
	...
	raw_spin_unlock(&grp->lock);
	rcu_read_unlock();
	trace_add_group_demand(p, event, grp->time.curr_window_scale,
			       rq->task_exec_scale, grp->nr_running);
}
...
static inline bool task_demand_fits(struct task_struct *p, int cpu)
{
	...
	return (capacity << SCHED_CAPACITY_SHIFT) >= (boosted_util * margin);
}
...
struct cpumask *find_rtg_target(struct task_struct *p)
{
	...
	struct sched_cluster *prime_cluster =
		sched_cluster[num_sched_clusters - 1];
	int prime_cpu = -1;
...
int find_fbg_cpu(struct task_struct *p)
{
	...
	if (grp) {
		...
		if (is_full_throttle_boost())
			preferred_cluster =
				sched_cluster[num_sched_clusters - 2];
		...
		for_each_cpu_and (i, &search_cpus, preferred_cpus) {
			...
			if (idle_cpu(i) ||
			    (i == task_cpu(p) && p->state == TASK_RUNNING)) {
				rcu_read_unlock();
				return i;
			}
...
int schedtune_grp_margin(unsigned long util)
{
	...
	if ((!isHighFps) &&
	    !(sysctl_slide_boost_enabled || sysctl_input_boost_enabled))
		return 0;
...
static inline bool group_should_invalid_util(struct frame_boost_group *grp,
					     u64 now)
{
...
static bool group_should_update_freq(struct frame_boost_group *grp, int cpu,
				     unsigned int flag, u64 now)
{
...
void sched_set_group_normalized_util(unsigned long util, unsigned int flag)
{
	...
	if (up_migrate &&
	    (now - last_migrate_time >= DEFAULT_UTIL_UPDATE_TIMEOUT))
		up_migrate = false;
	...
	if (need_update_prev_freq) {
		cpufreq_update_util(cpu_rq(prev_cpu),
				    (SCHED_CPUFREQ_WALT | SCHED_INPUT_BOOST));
	}
...
static void add_to_frame_boost_group(struct task_struct *p,
				     struct frame_boost_group *grp)
{
	struct rq *rq;
	struct rq_flags flag;
	...
	if (p->on_cpu) {
		...
		if (grp->nr_running == 1)
			grp->mark_start =
				max(grp->mark_start, sched_ktime_clock());
	}
...
int sched_set_frame_boost_group(struct task_struct *p, bool is_add)
{
	...
	if ((!p->fbg && !is_add) || (p->fbg && is_add) ||
	    (is_add && (p->flags & PF_EXITING)))
		goto done;
...
void binder_thread_set_fbg(struct task_struct *thread, struct task_struct *from,
			   bool oneway)
{
