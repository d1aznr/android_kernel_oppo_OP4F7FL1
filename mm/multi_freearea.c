	...
	for (zone_type = 0; zone_type < MAX_NR_ZONES; zone_type++) {
		...
		for (flc = 0; flc < FREE_AREA_COUNTS; flc++)
			seq_printf(m, "[%d]: label = %lu, segment = %lu\n", flc,
				   zone->zone_label[flc].label,
				   zone->zone_label[flc].segment);
		seq_printf(
			m,
			"\n---------------------------------------------------------------------------------------------------------------\n");
		for (flc = 0; flc < FREE_AREA_COUNTS; flc++) {
		   ...
		   int mt)
{
	...
	if (unlikely(pfn > zone->zone_label[FREE_AREA_COUNTS - 1].label)) {
		list = &(zone->free_area[FREE_AREA_COUNTS - 1][order]
				 .free_list[mt]);
...
void ajust_zone_label(struct zone *zone)
{
	...
	for (i = 0; i < FREE_AREA_COUNTS; i++) {
		zone->zone_label[i].label =
			zone->zone_start_pfn +
			zone->spanned_pages * (i + 1) / FREE_AREA_COUNTS;
	}
