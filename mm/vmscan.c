...
#include <linux/balloon_compaction.h>
	...
	defined(CONFIG_PROCESS_RECLAIM_ENHANCE)
				      ...
				      bool force_reclaim)
{
	...
	defined(CONFIG_PROCESS_RECLAIM_ENHANCE)
		...
		if (lru_gen_enabled() && !force_reclaim && page_mapped(page) &&
		    PageReferenced(page))
			goto keep_locked;
				      ...
				      struct vm_area_struct *vma, bool scan)
{
	...
	nr_reclaimed = shrink_page_list(page_list, NULL, &sc, TTU_IGNORE_ACCESS,
					NULL, true);
...
int isolate_lru_page(struct page *page)
{
	 ...
	 */
	WARN_RATELIMIT((!current_is_reclaimer() && PageTail(page)),
			   ...
			   unsigned long *lru_pages)
{
	...
	if (!current_is_kswapd()) {
		...
		if (strncmp(current->comm,
			    "hybridswapd:", sizeof("hybridswapd:") - 1) == 0) {
		...
		if (swappiness_threshold1_size && vm_swappiness_threshold1 &&
			...
			swappiness = vm_swappiness_threshold1;
		} else if (swappiness_threshold2_size &&
			   vm_swappiness_threshold2 &&
...
#endif
	if (!sc->may_swap ||
	    (mem_cgroup_get_nr_swap_pages(memcg) <= totalswap >> 6)) {
#else
...
static int page_lru_tier(struct page *page)
{
	...
	refs = (flags & LRU_REFS_FLAGS) == LRU_REFS_FLAGS ?
		       ((flags & LRU_REFS_MASK) >> LRU_REFS_PGOFF) + 1 :
		       0;
...
static void reset_ctrl_pos(struct lruvec *lruvec, int type, bool carryover)
{
	...
	unsigned long seq =
		carryover ? lrugen->min_seq[type] : lrugen->max_seq + 1;
			   ...
			   struct mm_walk *walk)
{
	...
	if (!(vma->vm_flags & (VM_READ | VM_EXEC | VM_WRITE)) ||
	    ...
	    (vma->vm_flags &
	     (VM_LOCKED | VM_SPECIAL | VM_SEQ_READ | VM_RAND_READ)) ||
	...
	return shmem_mapping(mapping) ? !priv->can_swap :
 ...
 */
static bool get_next_vma(struct mm_walk *walk, unsigned long mask,
			 ...
			 unsigned long *end)
{
	...
	while (walk->vma) {
		...
		if (should_skip_vma(walk->vma->vm_start, walk->vma->vm_end,
				    walk)) {
			walk->vma = walk->vma->vm_next;
			continue;
		}
		...
		*end = next && next < walk->vma->vm_end ? next :
							  walk->vma->vm_end;
...
restart:
	...
	for (addr = start; addr != end; i++, addr += PAGE_SIZE) {
		...
		VM_BUG_ON(addr < walk->vma->vm_start ||
			  addr >= walk->vma->vm_end);
	...
	if (i < PTRS_PER_PTE &&
	    get_next_vma(walk, PMD_MASK, PAGE_SIZE, &start, &end))
		goto restart;
				  ...
				  struct mm_walk *walk, unsigned long *start)
{
	...
	i = next == -1 ? 0 :
			 ((next - *start) >> PMD_SHIFT) & (PTRS_PER_PMD - 1);
	...
	do {
		...
		unsigned long addr =
			i ? (*start & PMD_MASK) + i * PMD_SIZE : *start;
...
restart:
...
#endif
		if (!priv->full_scan &&
		    !test_bloom_filter(priv->lruvec, priv->max_seq, pmd + i))
			continue;
	...
	if (i < PTRS_PER_PMD &&
	    get_next_vma(walk, PUD_MASK, PMD_SIZE, &start, &end))
		goto restart;
}
...
restart:
	...
	for (addr = start; addr != end; i++, addr = next) {
		...
		if (!pud_present(val) ||
		    WARN_ON_ONCE(pud_trans_huge(val) || pud_devmap(val)))
			continue;
	...
	if (i < PTRS_PER_PUD &&
	    get_next_vma(walk, P4D_MASK, PUD_SIZE, &start, &end))
		goto restart;
		       ...
		       unsigned long min_ttl)
{
	...
	nr_to_scan = get_nr_evictable(lruvec, max_seq, min_seq, swappiness,
				      &need_aging);
	if (!nr_to_scan)
		return false;
...
void lru_gen_look_around(struct page_vma_mapped_walk *pvmw)
{
	...
	for (i = 0, addr = start; addr != end; i++, addr += PAGE_SIZE) {
		...
		VM_BUG_ON(addr < pvmw->vma->vm_start ||
			  addr >= pvmw->vma->vm_end);
...
static bool isolate_page(struct lruvec *lruvec, struct page *page,
			 struct scan_control *sc)
{
	bool success;
...
static int scan_pages(struct lruvec *lruvec, struct scan_control *sc, int type,
			   ...
			   bool *need_aging)
{
	...
	nr_to_scan = get_nr_evictable(lruvec, max_seq, min_seq, can_swap,
				      need_aging);
	if (!nr_to_scan)
		return 0;
...
static void lru_gen_change_state(bool enable)
{
	...
	do {
		...
		for_each_node (nid) {
			...
			while (!(enable ? fill_evictable(lruvec) :
...
static ssize_t show_enable(struct kobject *kobj, struct kobj_attribute *attr,
			   char *buf)
{
	unsigned int caps = 0;
				  ...
				  unsigned long seq)
{
	...
	for (tier = 0; tier < MAX_NR_TIERS; tier++) {
		...
		for (type = 0; type < ANON_AND_FILE; type++) {
			...
			if (seq == max_seq) {
				...
				seq_printf(m, " %10luR %10luT %10lu ", n[0],
					   n[1], n[2]);
				...
				seq_printf(m, " %10lur %10lue %10lup", n[0],
					   n[1], n[2]);
			} else
				seq_puts(
					m,
	...
	for (i = 0; i < NR_MM_STATS; i++) {
		if (seq == max_seq && NR_HIST_GENS == 1)
			seq_printf(m, " %10lu%c",
				   READ_ONCE(lruvec->mm_state.stats[hist][i]),
 ...
 */
static void shrink_node_memcg(struct pglist_data *pgdat,
...
static ssize_t swappiness_para_write(struct file *file, const char __user *buff,
...
static ssize_t swappiness_para_read(struct file *file, char __user *buffer,
