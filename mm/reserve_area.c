...
unsigned long
get_unmmaped_area_from_anti_fragment(struct mm_struct *mm,
...
unsigned long
get_unmmaped_area_from_reserved(struct mm_struct *mm,
				struct vm_unmapped_area_info *info)
{
...
static ssize_t va_feature_write(struct file *file, const char __user *buff,
...
static ssize_t va_feature_read(struct file *file, char __user *buffer,
...
{
	if (parent && proc_create("va_feature", S_IRUSR | S_IWUSR, parent,
				  &proc_va_feature_ops)) {
...
void trigger_svm_oom_event(struct mm_struct *mm, bool brk_risk, bool is_locked)
{
	...
	if (!((va_feature & RESERVE_LOGGING) &&
	      (current->pid == current->tgid) && is_compat_task() &&
	...
	if (change_stack) {
		len = snprintf(
			...
			(long)rlim->rlim_cur, uid, (oom ? "oom" : "no_oom"),
			(res ? "res" : "no_res"),
			(brk_risk ? "brk" : "no_brk"));
		svm_oom_msg[len] = '\0';
...
static void dump_one_vma(struct vm_area_struct *vma)
{
	...
	pr_err("dbgmm %08lx-%08lx %c%c%c%c %08llx %02u:%02u %ld %s", start, end,
...
void dump_mm_info(unsigned long len, unsigned long flags, int dump_vma)
{
	...
	pr_err("dbgmm task %s %d parent %s %d VmSize %luKB VmRSS %luKB "
	       ...
	       current->group_leader->pid, total_vm << (PAGE_SHIFT - 10),
	       total_rss << (PAGE_SHIFT - 10), anon << (PAGE_SHIFT - 10),
	       file << (PAGE_SHIFT - 10), shmem << (PAGE_SHIFT - 10), chunk,
	       chunk_start, used_size >> 10, mm->map_count, len, dump_num,
