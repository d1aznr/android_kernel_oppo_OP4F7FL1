...
static ssize_t vm_fra_op_read(struct file *filp, char __user *buff,
...
static ssize_t vm_fra_op_write(struct file *file, const char __user *buff,
...
void trigger_cpu_oom_event(unsigned long len)
{
	...
	unsigned int tmp = 0, rest = 0, rest_reserve = 0, subtree = 0,
		     subtree_reserve = 0, chunk = 0, chunk_reserve = 0;
	char *svm_oom_msg = NULL;
	struct mm_struct *mm = current->mm;
	struct vm_area_struct *vma = NULL;
	unsigned long gap_start = 0, gap_end = 0, chunk_start = 0,
		      chunk_start_reserve = 0;
	...
	while (vma) {
		...
		tmp = gap_start - gap_end > 0 ? gap_start - gap_end : 0;
		rest += tmp;
...
ignore_reserve_subtree:
	...
	if (pid == prev_pid &&
	    time_before(jiffies, prev_jiffies + CPU_OOM_TRIGGER_GAP) &&
	...
	len = snprintf(svm_oom_msg, 127,
		       ...
		       len, pid, (fra ? "fra" : "no_fra"), rest, subtree,
		       rest_reserve, subtree_reserve, rvma);
	svm_oom_msg[len] = '\0';
...
ssize_t vm_search_two_way_op_read(struct file *filp, char __user *buff,
				  size_t count, loff_t *off)
{
	char page[256] = { 0 };
	int len = 0;
...
static ssize_t vm_search_two_way_op_write(struct file *file,
