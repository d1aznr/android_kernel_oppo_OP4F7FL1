...
#else /* CONFIG_SVELTE */
static inline void logger_kmsg_nwrite(const char *tag, const char *msg,
				      size_t len)
{
	return;
}
...
void dump_meminfo_to_logger(const char *tag, char *msg, size_t total_len)
{
	...
	while (total_len > 0) {
		if ((total_len + tag_len + 3) <= LOGGER_PRELOAD_SIZE) {
			logger_kmsg_nwrite(tag, (const char *)wr_msg,
					   total_len);
			return;
		}
...
static ssize_t vmalloc_debug_enable_read(struct file *file, char __user *buffer,
...
static ssize_t hash_cal_time_read(struct file *file, char __user *buffer,
				  size_t count, loff_t *off)
{
	char kbuf[128] = { '0' };
	int len;
					    ...
					    unsigned int *outlen)
{
	...
	record.data =
		vmalloc((sizeof(vmalloc_debug_list) * vmalloc_entry_count));
	if (!record.data) {
	...
	sort(record.data, record.count, sizeof(vmalloc_debug_list), record_cmp,
	     record_swap);
				      ...
				      int len)
{
	...
	dump_buff_len =
		scnprintf(dump_buff + dump_buff_len, BUFLEN(len, dump_buff_len),
	...
	if (!record.count)
		dump_buff_len +=
			scnprintf(dump_buff + dump_buff_len,
				  BUFLEN_EXT(len, dump_buff_len), "No Data\n");
...
static ssize_t vmalloc_debug_read(struct file *filp, char __user *buff,
				  size_t count, loff_t *off)
{
...
static ssize_t vmalloc_used_read(struct file *filp, char __user *buff,
				 size_t count, loff_t *off)
{
	char kbuf[64];
	unsigned int len;
