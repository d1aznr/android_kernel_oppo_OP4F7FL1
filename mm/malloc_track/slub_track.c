 ...
 */
#define LOCATIONS_TRACK_BUF_SIZE(s)                                            \
	((s->object_size == 128) ? (PAGE_SIZE << 10) : (PAGE_SIZE * 128))
...
const struct kmalloc_info_struct kmalloc_debug_info[] = {
	{ NULL, 0 },
	{ "kmalloc-debug-96", 96 },
	{ "kmalloc-debug-192", 192 },
	{ "kmalloc-debug-8", 8 },
	{ "kmalloc-debug-16", 16 },
	{ "kmalloc-debug-32", 32 },
	{ "kmalloc-debug-64", 64 },
	{ "kmalloc-debug-128", 128 },
	{ "kmalloc-debug-256", 256 },
	{ "kmalloc-debug-512", 512 },
	{ "kmalloc-debug-1k", 1024 },
	{ "kmalloc-debug-2k", 2048 },
	{ "kmalloc-debug-4k", 4096 },
	{ "kmalloc-debug-8k", 8192 },
	{ "kmalloc-debug-16k", 16384 },
	{ "kmalloc-debug-32k", 32768 },
	{ "kmalloc-debug-64k", 65536 },
	{ "kmalloc-debug-128k", 131072 },
	{ "kmalloc-debug-256k", 262144 },
	{ "kmalloc-debug-512k", 524288 },
	{ "kmalloc-debug-1M", 1048576 },
	{ "kmalloc-debug-2M", 2097152 },
	{ "kmalloc-debug-4M", 4194304 },
	{ "kmalloc-debug-8M", 8388608 },
	{ "kmalloc-debug-16M", 16777216 },
	{ "kmalloc-debug-32M", 33554432 },
 ...
 */
atomic64_t kmalloc_debug_caches[NR_KMALLOC_TYPES][KMALLOC_SHIFT_HIGH + 1] = {
	{ ATOMIC64_INIT(0) }
};
...
{
	return ((struct kd_location *)lb)->count -
	       ((struct kd_location *)la)->count;
}
...
static int kd_list_locations(struct kmem_cache *s, char *buf, int buff_len)
{
	...
	int node;
	unsigned long *map = vmalloc(BITS_TO_LONGS(oo_objects(s->max)) *
			   ...
			   char *dump_buff, int len)
{
	...
	map = vmalloc(BITS_TO_LONGS(oo_objects(s->max)) *
		      sizeof(unsigned long));
	if (!map || kd_alloc_loc_track(&t, LOCATIONS_TRACK_BUF_SIZE(s))) {
		vfree(map);
	...
	dump_buff_len =
		scnprintf(dump_buff + dump_buff_len, len - dump_buff_len - 2,
	...
	for (i = 0; i < t.count; i++) {
		...
		dump_buff_len += scnprintf(
			dump_buff + dump_buff_len, BUFLEN(len, dump_buff_len),
			"%ld KB %pS age=%ld/%ld/%ld pid=%ld-%ld\n",
			(l->count * s->object_size) >> 10, (void *)l->addr,
			l->min_time, (long)div_u64(l->sum_time, l->count),
			l->max_time, l->min_pid, l->max_pid);

		for (j = 0; j < l->depth; j++)
			dump_buff_len +=
				scnprintf(dump_buff + dump_buff_len,
					  BUFLEN(len, dump_buff_len), "%pS\n",
					  (void *)l->addrs[j]);
...
static inline const char *kmalloc_debug_cache_name(const char *prefix,
						   unsigned int size)
{
...
create_kmalloc_debug_caches(size_t size, unsigned long flags,
			    enum kmalloc_cache_type kmalloc_type)
{
...
static ssize_t kmalloc_debug_create_read(struct file *file, char __user *buffer,
					  ...
					  loff_t *ppos)
{
	...
	for (type = KMALLOC_NORMAL; type <= KMALLOC_RECLAIM; type++) {
		s = (struct kmem_cache *)atomic64_read(
			&kmalloc_debug_caches[type][index]);
		if (s) {
	...
	for (type = KMALLOC_NORMAL; type <= KMALLOC_RECLAIM; type++) {
		s = create_kmalloc_debug_caches((size_t)size, SLAB_STORE_USER,
						type);
		if (!s) {
			mutex_unlock(&debug_mutex);
			return -ENOMEM;
		}
		atomic64_set(&kmalloc_debug_caches[type][index],
			     (unsigned long)s);
	}
	mutex_unlock(&debug_mutex);
...
static inline void memleak_accumulate_slabinfo(struct kmem_cache *s,
...
static void *kmd_start(struct seq_file *m, loff_t *pos)
{
	...
	while (*pos <= ALL_KMALLOC_HIGH) {
		s = (struct kmem_cache *)atomic64_read(
			&kmalloc_debug_caches[CACHE_TYPE(*pos)]
					     [CACHE_INDEX(*pos)]);
		if (s && (s->flags & SLAB_STORE_USER))
			return (void *)s;
		++*pos;
...
static const struct seq_operations kmalloc_debug_op = { .start = kmd_start,
							.next = kmd_next,
							.show = kmd_show,
							.stop = kmd_stop };

static const struct seq_operations kmalloc_origin_op = { .start = kmo_start,
							 .next = kmo_next,
							 .show = kmd_show,
							 .stop = kmd_stop };
...
static ssize_t kmalloc_debug_enable_read(struct file *file, char __user *buffer,
					 size_t count, loff_t *off)
{
	char kbuf[KD_VALUE_LEN] = { '0' };
	int len;
...
static ssize_t kmalloc_used_read(struct file *file, char __user *buffer,
				 size_t count, loff_t *off)
{
	char kbuf[512];
	int len = 0;
...
int __init create_kmalloc_debug(struct proc_dir_entry *parent)
{
	...
	upentry =
		proc_create("kmalloc_used", S_IRUGO, parent, &kmalloc_used_ops);
	if (!upentry) {
...
#if defined(CONFIG_MEMLEAK_DETECT_THREAD) && defined(CONFIG_SVELTE)
	mpentry = proc_create("memleak_detect_thread", S_IRUGO | S_IWUGO,
			      parent, &memleak_detect_thread_operations);
	if (!cpentry) {
...
static int memleak_detect_thread(void *arg)
{
	...
	do {
		...
		for (type = KMALLOC_NORMAL; type <= KMALLOC_RECLAIM; type++) {
			for (i = 0; i <= KMALLOC_SHIFT_HIGH; i++) {
				struct kmem_cache *s =
					(struct kmem_cache *)atomic64_read(
						&kmalloc_debug_caches[type][i]);
	...
	for (type = KMALLOC_NORMAL; type <= KMALLOC_RECLAIM; type++) {
		for (i = KMALLOC_SHIFT_LOW; i <= KMALLOC_SHIFT_HIGH; i++) {
			...
			s = create_kmalloc_debug_caches(
				kmalloc_debug_info[i].size, flags, type);
			...
			if (KMALLOC_MIN_SIZE <= 32 &&
			    !kmalloc_caches[type][1] && i == 6)
				s = create_kmalloc_debug_caches(
					kmalloc_debug_info[i].size, flags,
					type);
			if (KMALLOC_MIN_SIZE <= 64 &&
			    !kmalloc_caches[type][2] && i == 7)
				s = create_kmalloc_debug_caches(
					kmalloc_debug_info[i].size, flags,
					type);
			...
			atomic64_set(&kmalloc_debug_caches[type][i],
				     (unsigned long)s);
		}
	}
...
static int __init memleak_detect_init(void)
{
	...
	memleak_detect_task =
		kthread_create(memleak_detect_thread, NULL, "memleak_detect");
	if (IS_ERR(memleak_detect_task)) {
		pr_warn("[kmalloc_debug][vmalloc_debug] memleak_detect_init failed.\n");
		memleak_detect_task = NULL;
	} else
