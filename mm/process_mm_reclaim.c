					   ...
					   char *buffer)
{
	 ...
	 */
	if ((type == RECLAIM_INACTIVE) || (type == RECLAIM_INACTIVE_FILE) ||
	    (type == RECLAIM_INACTIVE_ANON))
...
cont:
	...
	if (type == RECLAIM_RANGE) {
		...
		for (vma = mm->mmap; vma; vma = vma->vm_next) {
			 ...
			 */
			err = is_reclaim_addr_over(&reclaim_walk,
						   vma->vm_start);
			if (err) {
				err = -err;
				break;
