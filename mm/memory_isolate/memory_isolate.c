	...
	for (pfn = start_pfn; pfn < end_pfn; pfn += pageblock_nr_pages) {
		...
		if (reserve > 0) {
			...
			if (block_migratetype == MIGRATE_MOVABLE) {
				...
				pages_moved = move_freepages_block(
					zone, page, reserve_migratetype, NULL);
#else
				pages_moved = move_freepages_block(
					zone, page, reserve_migratetype);
#endif
				__mod_zone_page_state(zone, item, pages_moved);
				reserve--;
				continue;
			}
		} else if (!old_reserve) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
			pages_moved = move_freepages_block(
				zone, page, MIGRATE_MOVABLE, NULL);
#else
