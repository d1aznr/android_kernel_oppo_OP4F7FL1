...
struct damos *
damon_new_scheme(unsigned long min_sz_region, unsigned long max_sz_region,
...
void damon_destroy_ctx(struct damon_ctx *ctx)
{
	...
	damon_for_each_scheme_safe(s, next_s, ctx) damon_destroy_scheme(s);
		      ...
		      ssize_t nr_schemes)
{
	...
	damon_for_each_scheme_safe(s, next, ctx) damon_destroy_scheme(s);
...
static unsigned long damos_wmark_wait_us(struct damos *scheme)
{
	...
	case DAMOS_WMARK_FREE_MEM_RATE:
		...
		if (metric > scheme->wmarks.high ||
		    scheme->wmarks.low > metric) {
			if (scheme->wmarks.activated)
				pr_debug(
					"deactivate a scheme (%d) for %s wmark\n",
					scheme->action,
					metric > scheme->wmarks.high ? "high" :
								       "low");
		...
		if ((scheme->wmarks.high >= metric &&
		     metric >= scheme->wmarks.mid) &&
	...
	case DAMOS_WMARK_OPLUS:
		...
		if (scheme->wmarks.activated) {
			...
			nr_reclaim_time = active_interval *
					  active_cnt; // 10ms(0.01s) * cnt;
		}
		...
		last_metric =
			(2 * last_metric + 8 * metric) / 10; // smooth filter
		/*
		...
		*/
		if (diff <= 20 || last_metric < metric) {
			if (scheme->wmarks.activated) {
				if (nr_reclaim_time >=
					    300000 || // time thresold: 5min = 5*60*1000 ms
