...
static int memory_monitor_show(struct seq_file *m, void *p)
{
	...
	unsigned long start = 0, rcu_lock_end = 0, end = 0, task_lock_start = 0,
		      task_lock_end = 0, task_lock_time = 0, gpu_read_time = 0,
		      gpu_read_start = 0;
	...
	seq_printf(
		...
		"task_name", "pid", "uid", "oom_score_adj", "rss",
		"swapents_ori", "ion", "GL+dev", "rssfile", "ppid");
...
static int __init memory_monitor_init(void)
{
	...
	pentry = proc_create("memory_monitor", S_IRWXUGO, NULL,
			     &memory_monitor_fops);
	if (!pentry) {
