...
int __add_to_swap_cache(struct page *page, swp_entry_t entry, void **shadowp)
{
	...
	for (i = 0; i < nr; i++) {
		...
		error = __radix_tree_create(&address_space->i_pages, idx + i, 0,
					    &node, &slot);
		if (unlikely(error))
			break;

		item = radix_tree_deref_slot_protected(
...
void __delete_from_swap_cache(struct page *page, void *shadow)
{
	...
	for (i = 0; i < nr; i++) {
		...
		item = __radix_tree_lookup(&address_space->i_pages, idx + i,
					   &node, &slot);
				  ...
				  unsigned long end)
{
	...
	for (;;) {
		...
		xa_lock_irq(&address_space->i_pages);
		radix_tree_for_each_slot (slot, &address_space->i_pages, &iter,
					  curr) {
			item = radix_tree_deref_slot_protected(
				slot, &address_space->i_pages.xa_lock);
